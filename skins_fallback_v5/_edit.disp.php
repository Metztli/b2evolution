...
if ($Blog->get_setting('in_skin_editing_category')) {	// If categories are allowed to update from front-office:
    $disp_edit_categories = true;
} else {	// Don't allow to update the categories:
    $disp_edit_categories = false;
    if ($edited_Item->ID == 0) {	// Force to store category Id in hidden field only for new creating items:
        $params['disp_edit_categories'] = false;
    }
}

if (! $params['disp_edit_categories']) {	// When categories are hidden, we store a cat_ID in the hidden input:
...
if ($edited_Item->get_type_setting('allow_attachments') && $edited_Item->ID > 0) { // ####################### ATTACHMENTS FIELDSETS #########################
...
if (! $display_item_settings_is_defined) {
    ...
    $exclude_statuses = array_merge(get_restricted_statuses($Blog->ID, 'blog_post!', 'create', $edited_Item->status, '', $edited_Item), ['trash']);
...
// ################### TEXT RENDERERS ###################
if ($Blog->get_setting('in_skin_editing_renderers') && $edited_Item->get_type_setting('use_text') != 'never') {	// If text renderers are allowed to update from front-office and text content is allowed for the item type:
