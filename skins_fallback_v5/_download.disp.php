...
$file_download_link_attrs = 'href="' . $download_Link->get_download_url([
    'type' => 'action',
]) . '"'
    . ' download="' . $download_File->get_name() . '"';
// 1) JavaScript is enabled
printf(
    $params['download_timer_text'],
...
echo '<noscript>';
printf(
    $params['download_nojs_text'],
    $file_download_link_attrs
);
echo '</noscript>';

// Post content
if ($download_Item->has_content_parts($params)) { // Display only text after <more>
    $download_Item->content_extension([
