...
if (! strlen($email_author)) { // Try to get params from $_COOKIE through the param() function
...
if (! empty($recipient_id)) {	// If the email is to a registered user get the email address from the users table
    $UserCache = &get_UserCache();
    $recipient_User = &$UserCache->get_by_ID($recipient_id);

    if ($recipient_User) { // recipient User found
        $recipient_name = $recipient_User->get_username();
        $recipient_address = $recipient_User->get('email');
    }
} elseif (! empty($comment_id)) {	// If the email is to anonymous user of comment
    $CommentCache = &get_CommentCache();
    if ($Comment = &$CommentCache->get_by_ID($comment_id, false)) {
        $recipient_User = &$Comment->get_author_User();
        if (empty($recipient_User) && ($Comment->allow_msgform) && (is_email($Comment->get_author_email()))) {	// Get recipient name and email from comment's author:
...
if (empty($recipient_address)) {	// We should never have called this in the first place!
