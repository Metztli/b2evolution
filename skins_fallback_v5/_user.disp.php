...
if (empty($user_ID)) {	// Grab the current User
...
$avatar_overlay_text = '';
if ($is_logged_in) { // Avatar size for logged in user
    $avatar_image_size = $params['avatar_image_size'];
} else { // Avatar settings for anonymous user
    $avatar_image_size = $params['avatar_image_size_if_anonymous'];
    if ($params['avatar_overlay_text_if_anonymous'] != '#default#') { // Get overlay text from params
        $avatar_overlay_text = $params['avatar_overlay_text_if_anonymous'];
    } else { // Get default overlay text from Back-office settings
...
if ($is_logged_in) {
    if ($User->ID == $current_User->ID && ! $User->has_avatar()) { // If user hasn't an avatar, add a link to go for uploading of avatar
...
if (! empty($first_last_name)) { // Display first last name of it is not empty
...
// Location:
$location = [];
if (! empty($User->city_ID) && user_city_visible()) { // Display city
    load_class('regional/model/_city.class.php', 'City');
    $location[] = $User->get_city_name();
}
if (! empty($User->subrg_ID) && user_subregion_visible()) { // Display sub-region
    load_class('regional/model/_subregion.class.php', 'Subregion');
    $location[] = $User->get_subregion_name();
}
if (! empty($User->rgn_ID) && user_region_visible()) { // Display region
    load_class('regional/model/_region.class.php', 'Region');
    $location[] = $User->get_region_name();
}
if (! empty($User->ctry_ID) && user_country_visible()) { // Display country
    load_class('regional/model/_country.class.php', 'Country');
    $location[] = $User->get_country_name();
}
if (! empty($location)) { // Display location only if at least one selected
...
// - Edit my profile:
if ($is_logged_in && $User->ID == $current_User->ID && ! empty($params['edit_my_profile_link_text'])) { // Display edit link profile for owner:
...
// - Message:
if (! $is_logged_in || $current_User->ID != $User->ID) { // Display a message to send a button only for other users
...
if ($is_logged_in && ($current_User->ID != $User->ID) &&
        ...
        check_user_status('can_edit_contacts')) { // User is logged in, has messaging access permission and is not the same user as displayed user
    $is_contact = check_contact($User->ID);
    if ($is_contact === null) { // User is not in current User contact list, so allow "Add to my contacts" action
        $buttons[] = '<button type="button" class="btn btn-default" onclick="return user_contact_groups( ' . $User->ID . ' )">' . T_('Add to Contacts') . '</button>';
    } elseif ($is_contact === false) { // User is blocked
    ...
    } else { // User is on current User contact list
...
// - Report:
if ($is_logged_in && ($current_User->ID != $User->ID) &&
        check_user_status('can_report_user', $User->ID)) { // Current user must be logged in, cannot report own account, and must has a permission to report
    if (! isset($buttons['group'])) {
        $buttons['group'] = [];
    }
    // get current User report from edited User
    $current_report = get_report_from($User->ID);
    if ($current_report == null) { // current User didn't add any report from this user yet
...
if ($user_perms_backoffice_edit) { // Current user can edit other user's profiles
    global $admin_url;
    ...
    if ($current_User->ID != $User->ID && check_user_perm('users', 'edit')) { // - Delete in back-office:
        $buttons['del'] = [];
...
if (count($buttons)) { // Dispaly all available buttons
    echo '<hr class="profile_separator" />' . "\n";
    echo '<div class="profile_buttons">';
    foreach ($buttons as $button) {
        if (is_array($button)) { // The grouped buttons
...
if ($count_organizations > 0) {
    ...
    foreach ($user_organizations as $org) {
        if (empty($org->url)) { // Display just a text
            $org_names[] = $org->name;
        } else { // Make a link for organization
...
foreach ($User->userfields as $userfield) {
    ...
    if ($group_ID != $userfield->ufgp_ID) { // Start new group
        if ($group_ID > 0) { // End previous group
            $profileForm->end_fieldset();
        }
        $profileForm->begin_fieldset($userfield->ufgp_name, [
            'id' => 'fieldset_user_fields',
        ]);
    }
...
if ($group_ID > 0) {	// End fieldset if userfields are exist
    $profileForm->end_fieldset();
}
...
if ($Blog->get_setting('userdir_lastseen')) {	// Display last visit only if it is enabled by current collection:
...
$profileForm->info(T_('Photos'), '<span class="reputation_message">' . $User->get_reputation_files([
    'file_type' => 'image',
]) . '</span>');

$profileForm->info(T_('Audio'), '<span class="reputation_message">' . $User->get_reputation_files([
    'file_type' => 'audio',
]) . '</span>');

$profileForm->info(T_('Other files'), '<span class="reputation_message">' . $User->get_reputation_files([
    'file_type' => 'other',
]) . '</span>');
