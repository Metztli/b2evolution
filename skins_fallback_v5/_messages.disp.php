...
if (! $error_messages_exist && ! $Messages->has_errors() && (empty($thrd_ID) || empty($edited_Thread))) { // Display this error only when no error above
    $Messages->add(T_('Can\'t show messages without thread!'), 'error');
    $thread_is_missed = true;
} else {	// Preload users to show theirs avatars
    load_messaging_thread_recipients($thrd_ID);
}

$Messages->display();

if ($thread_is_missed) { // If thread is missed by some reeason we should inform user why it happens
