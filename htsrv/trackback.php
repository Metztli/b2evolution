...
$show_statuses = ['published', 'protected', 'private'];
...
if (empty($tb_id)) { // No parameter for ID, get if from URL:
    $path_elements = explode('/', $ReqPath, 30);
    $tb_id = intval($path_elements[count($path_elements) - 1]);
}


if (! empty($_GET['__mode'])) { // some MT extension (AFAIK), that we do not support
    return;
}
...
$Plugins->trigger_event('BeforeTrackbackInsert', [
    'Comment' => &$Comment,
]);
...
$Plugins->trigger_event('AfterTrackbackInsert', [
    'Comment' => &$Comment,
]);
