...
if (init_charsets($current_charset)) { // Reload Blog(s) (for encoding of name, tagline etc):
    $BlogCache->clear();
    $commented_Item->load_Blog();
}
...
if ($Settings->get('system_lock')) { // System is locked for maintenance, users cannot send a comment
...
if ($comment_type == 'meta') { // Internal comment
    if (! $commented_Item->can_meta_comment()) { // Current user has no permission to post an internal comment
        $Messages->add(T_('You cannot leave internal comments on this post!'), 'error');
        header_redirect(); // Will save $Messages into Session
    }
} else { // 'feedback'
    // Normal/Standard comment
    if (! $commented_Item->can_comment(null)) { // Current user has no permission to post a normal comment
        $Messages->add(T_('You cannot leave comments on this post!'), 'error');
        header_redirect(); // Will save $Messages into Session
    }
}

if ($commented_Item->Blog->get_setting('allow_html_comment')) {	// HTML is allowed for this comment
    $text_format = 'html';
} else {	// HTML is disallowed for this comment
...
if (is_logged_in(false)) {
...
} else {	// User is not logged in (registered users), we need some id info from him:
    $User = null;
    // Note: we use funky field names to defeat the most basic guestbook spam bots and/or their most basic authors
    ...
    if ($url != '' && ! $commented_Item->Blog->get_setting('allow_anon_url')) {	// It's an automated/malicious submit and we want to reject it the hard way
        exit(0);
    }
...
$comment_renderers = param('renderers', 'array:string', []);
...
$Plugins->trigger_event('CommentFormSent', [
...
if ($action != 'preview' &&
    ...
    $commented_Item->dbupdate()) {	// Update workflow properties if they are loaded from request without errors and at least one of them has been changed:
...
if (! $User) {	// User is still not logged in
    ...
    if ($commented_Item->Blog->get_setting('comments_detect_email')) {	// Detect email addresses in comments
        if (preg_match('/(([_a-z0-9-]+)(\.[_a-z0-9-]+)*@([a-z0-9-]+)(\.[a-z0-9-]+)*(\.[a-z]{2,}))/i', $comment)) {	// Comment contains an email address
            $action = 'preview';
 ...
 */
$Comment = new Comment();
if ($reply_ID > 0) {	// Set parent ID if this comment is reply to other comment
...
if (! $Comment->is_meta() && $commented_Item->can_rate()) {	// Comment rating:
...
if (param('renderers_displayed', 'integer', 0)) { // use "renderers" value only if it has been displayed (may be empty)
    global $Plugins;
    $renderers = $Plugins->validate_renderer_list($comment_renderers, [
        'Comment' => &$Comment,
    ]);
...
if (! empty($preview_attachments)) { // Get checked attachments. Some attachments was already unchecked, so needs to be separated.
    $attachments = explode(',', $preview_attachments);
    foreach ($attachments as $attachment_ID) { // iterate through all attachments and select checked ones
        if (($commented_Item->get_attachments_limit() === 'unlimit' || $checked_attachments_count < (int) $commented_Item->get_attachments_limit()) &&
            param('preview_attachment' . $attachment_ID, 'integer', 0)) { // this attachment checkbox was checked in, so it needs to be attached
            $checked_attachments = $checked_attachments . $attachment_ID . ',';
            $checked_attachments_count++;
        }
    }
    if (! empty($checked_attachments)) { // cut the last comma
        $checked_attachments = substr($checked_attachments, 0, strlen($checked_attachments) - 1);
    }
}

if ($commented_Item->can_attach($Comment->temp_link_owner_ID, $Comment->type) && ! empty($_FILES['uploadfile']) && ! empty($_FILES['uploadfile']['size']) && ! empty($_FILES['uploadfile']['size'][0])) { // attaching files is permitted
    $FileRootCache = &get_FileRootCache();
    if (is_logged_in()) { // registered user
...
if ($is_empty_comment && ! $workflow_is_updated) {	// Comment text should not be empty!
...
if ($load_workflow_result) {	// Store changed Item workflow properties in session Comment in order to display them after redirect:
    $Comment->item_workflow = [
...
if ($load_custom_fields_result) {	// Store changed Item custom fields in session Comment in order to display them after redirect:
    $custom_fields = $commented_Item->get_custom_fields_defs();
    $Comment->item_custom_fields = [];
...
if (($Messages->has_errors() && $action != 'preview') ||
    ...
    if ($workflow_is_updated &&
        ...
        ! empty($commented_Item->assigned_user_ID)) {	// Send post assignment notification when only workflow assigned User was changed without posting new comment:
...
if ($action == 'preview') { // set the Comment into user's session and redirect.
    ...
    if ($comments_email_is_detected) { // Comment contains an email address, We should show an error about this
        if ($Settings->get('newusers_canregister') == 'yes' && $Settings->get('registration_is_public')) { // Users can register and we give them a links to log in and registration
            if (is_null($commented_Item)) { // Initialize the commented Item object
                $commented_Item = &$ItemCache->get_by_ID($comment_item_ID);
            }
    ...
    header_redirect();
    exit(0);
} else { // delete any preview comment from session data:
...
if ($result && (! empty($preview_attachments))) {
    ...
    foreach ($attachments as $file_ID) { // create links between comment and attached files
        if (in_array($file_ID, $final_attachments)) { // attachment checkbox was checked, create the link
            $LinkOwner->add_link($file_ID, 'aftermore', $order, false);
            $order++;
        } else { // attachment checkbox was not checked, remove unused uploaded file
            $unused_File = $FileCache->get_by_ID($file_ID, false);
            if ($unused_File) {
                if (empty($checked_attachments) && empty($attachment_dir)) { // None of the previously attached file was checked to be attached
    ...
    if (empty($checked_attachments) && (! empty($attachment_dir))) { // None of the previously attached files were finally attached
        $dir_content = get_filenames($attachment_dir, [
            'recurse' => false,
            'inc_hidden' => false,
        ]);
        if (empty($dir_content)) { // The attachment dir is empty, delete the directory
...
if ($Comment->ID) { // comment has not been deleted
    ...
    if (! is_logged_in()) {
        if ($Settings->get('newusers_canregister') == 'yes' && $Settings->get('registration_is_public') && $Comment->Item->Blog->get_setting('comments_register')) { // Redirect to the registration form
