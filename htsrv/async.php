...
if ($action != 'test_api' && ! check_user_perm('admin', 'restricted')) {	// No permission to access admin... (Exclude action of API testing in order to make a quick request without logging in)
...
switch ($action) {
    ...
    case 'add_plugin_sett_set':
        ...
        if (! in_array($set_type, ['Settings', 'UserSettings', 'CollSettings', 'MsgSettings', 'EmailSettings', 'Skin', 'Widget'])) {
        ...
        switch ($set_type) {
            case 'Widget':
                ...
                if (($Plugin = &$Widget->get_Plugin())) {	// Set abstract type for Widget initialized from Plugin:
                    $set_type = 'PluginWidget';
                } else {	// This is a normal Widget:
                    $Plugin = $Widget;
                }
            ...
            default:
                ...
                if ($set_type == 'UserSettings') {	// Initialize User object for this plugin type:
        ...
        $set_node = _set_setting_by_path($Plugin, $set_type, $set_path, []);
    ...
    case 'edit_comment':
        ...
        switch ($comment_action) {
            ...
            case 'update':
                ...
                if ($Blog->get_setting('allow_html_comment')) { // HTML is allowed for this comment
                    $text_format = 'html';
    ...
    case 'delete_comment':
        ...
        if (in_array($request_from, ['items', 'comments'])) {	// AJAX request goes from backoffice and ctrl = items or comments:
            ...
            echo_item_comments($blog, $item_ID, $statuses, $currentpage, $limit, [], $filterset_name, $expiry_status, $comment_type);
        }
        break;
    ...
    case 'delete_comment_url':
        ...
        if ($edited_Comment !== false && $edited_Comment->author_url != null) {	// The comment still exists
            // Check permission:
    ...
    case 'refresh_comments':
        ...
        if (in_array($request_from, ['items', 'comments'])) {	// AJAX request goes from backoffice and ctrl = items or comments
            echo_item_comments($blog, $item_ID, $statuses, $currentpage, null, [], '', $expiry_status, $comment_type);
        } elseif ($request_from == 'dashboard' || $request_from == 'coll_settings') { // AJAX request goes from backoffice dashboard
            load_funcs('dashboard/model/_dashboard.funcs.php');
            show_comments_awaiting_moderation($blog, null, 10, [], false);
        }
        break;
    ...
    case 'item_task_edit':
        ...
        if (! in_array($field, ['priority', 'status', 'assigned'])) { // Invalid field
        ...
        switch ($field) {
            ...
            case 'assigned':
                ...
                if ($Item->assign_to($new_assigned_ID, $new_assigned_login)) { // An assigned user can be changed
                    $Item->dbupdate();
                    $Item->send_assignment_notification();
                } else { // Error on changing of an assigned user
                ...
                if (empty($Item->assigned_user_ID)) {
                    ...
                    $new_title = $User->get_colored_login([
                        'mask' => '$avatar$ $login$',
                    ]);
                }
            ...
            case 'status':
                ...
                if (! empty($new_value)) {	// Add '_' to don't break a sorting by name on jeditable:
    ...
    case 'item_order_edit':
        ...
        if ($item_order === '-' || $item_order === '') {	// Set NULL for these values:
            $item_order = null;
        } else {	// Make an order to double:
    ...
    case 'cat_order_edit':
        ...
        if ($cat_order === '-' || $cat_order === '' || intval($cat_order) == '') { // Set NULL for these values
            $cat_order = null;
        }

        $ChapterCache = &get_ChapterCache();
        if ($Chapter = &$ChapterCache->get_by_ID($cat_ID, false)) { // Update cat order if it exists in DB
    ...
    case 'item_status_order_edit':
        ...
        if ($item_status_order === '-' || $item_status_order === '') {	// Set NULL for these values:
            $item_status_order = null;
        } else {	// Make an order to integer:
            $item_status_order = intval($item_status_order);
        }

        $ItemStatusCache = &get_ItemStatusCache();
        if ($edited_ItemStatus = &$ItemStatusCache->get_by_ID($item_status_ID, false)) { // Update item status order if it exists in DB
    ...
    case 'cat_ityp_ID_edit':
        ...
        if (! empty($cat_ityp_ID)) {	// Remove prefix "_" which is used only for correct order in jeditable selector:
            $cat_ityp_ID = substr($cat_ityp_ID, 1);
        }
        if ($cat_ityp_ID === false || $cat_ityp_ID === '') {	// Convert empty value to NULL to update DB:
            $cat_ityp_ID = null;
        }

        $ChapterCache = &get_ChapterCache();
        if ($Chapter = &$ChapterCache->get_by_ID($cat_ID, false)) {	// Update cat Item Type if it exists in DB:
            $ItemTypeCache = &get_ItemTypeCache();
            if (! empty($cat_ityp_ID) &&
                (! ($ItemType = &$ItemTypeCache->get_by_ID($cat_ityp_ID, false, false)) ||
                  ! $ItemType->is_enabled($blog))) {	// Revert back to use previous Item Type if new is wrong for current category:
    ...
    case 'get_goals':
        ...
        if (empty($cat_ID)) { // Select the goals without category
            $SQL->WHERE_and('goal_gcat_ID IS NULL');
        } else { // Get the goals from a selected category
    ...
    case 'conflict_files':
        ...
        $data = [];
        if ($format == 'full_path_link') { // User link with full path to file
        ...
        if ($result !== true) { // Send an error if it was created during the replacing
            $data['error'] = $result;
        }
    ...
    case 'get_userlist_automation':
        ...
        if ($enlt_ID === null) {	// Get newsletters tied to the automation:
            $NewsletterCache->load_list($Automation->get_newsletter_IDs());
            $autm_data['newsletters'] = [];
        ...
        } else {	// Get automation data for selected newsletter:
    ...
    case 'get_campaign_recipients':
        ...
        if ($edited_Campaign = &$EmailCampaignCache->get_by_ID($ecmp_ID, false, false)) {	// If Email Campaign is found in DB:
        ...
        } else {	// Wrong request, unknown Email Campaign:
            $recipients_data = [
                'status' => 'error',
    ...
    case 'browse_subdirs':
        ...
        if (! preg_match('#^([a-z]+)_(\d+):(.+)$#', $path, $path_data)) {	// Invalid path:
        ...
        if (! ($dir_File = &$FileCache->get_by_root_and_path($path_data[1], $path_data[2], $path_data[3])) ||
            ! $dir_File->is_dir()) {	// Invalid directory:
        ...
        if (! $dir_Filelist->count_dirs()) {	// Wrong requested directory:
    ...
    case 'browse_existing_attachments':
        ...
        switch ($link_owner_class) {
            ...
            case 'Comment':
                ...
                if (! $edited_Item->can_meta_comment()) {	// If current User doesn't have an access to meta comments:
                ...
                if ($ea_Linklist->get_total_rows()) {	// Use first attachment to get the FileRoot:
...
if (! $incorrect_action) {
    if ($current_debug || $current_debug_jslog) {	// debug is ON
        ajax_log_display();
    }

    if ($add_response_end_comment) { // add ajax response end comment
