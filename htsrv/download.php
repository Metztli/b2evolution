...
param('key', 'string', '');
if (! empty($key)) { // Try to record a goal hit
    $GoalCache = &get_GoalCache();
    if ($Goal = &$GoalCache->get_by_name($key, false, false)) { // Record the goal hit
        $Goal->record_hit();
    }
}
...
if ($download_File->get_ext() == 'zip') { // Redirect to direct url for ZIP files case
...
} else { // For other files force the downloading
