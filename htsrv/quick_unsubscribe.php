...
$unsub_Comment = false;
if ($comment_ID !== null) {	// This is a case to unsubscribe anonymous user from comment:
    $CommentCache = &get_CommentCache();
    $unsub_Comment = &$CommentCache->get_by_ID($comment_ID, false, false);
    if ($unsub_Comment->get('author_email') == '' || $unsub_Comment->get('author_user_ID') !== null) {	// Don't allow to do unsubcription action if the requested comment is not from anonymous user:
        $unsub_Comment = false;
    }
}

if (! $unsub_Comment) {	// This is a case to unsubscribe a registered user:
    $UserCache = &get_UserCache();
    $edited_User = $UserCache->get_by_ID($user_ID, false, false);
}

if (empty($edited_User) && empty($unsub_Comment)) {	// Registered or anonymous user is not found:
...
} elseif ($confirmed) { // Unsubscribe is confirmed let's proceed
    switch ($action) {
        case 'unsubscribe':
            ...
            switch ($type) {
                ...
                case 'coll_post':
                    ...
                    if ($coll_ID == 0) {
                        ...
                        if ($Blog->get('advanced_perms')
                                ...
                                && $edited_User->check_perm('blog_ismember', 'view', false, $coll_ID)) { // opt-out collection
                            ...
                            } else { // should be impossible to go here
                                $sub_items_value = 0;
                ...
                case 'post':
                    ...
                    if ($post_ID == 0) {
                        ...
                        if ($unsub_Comment) {	// Anonymous user form Comment:
                ...
                case 'post_moderator_edit':
                    ...
                    if ($notify_edit_pst_moderation) {	// Unsubscribe from ALL collections:
                        ...
                        $hidden_form_params = [
                            'subs_edit_pst_type' => 'all',
                        ];
                    } elseif (! empty($coll_ID)) {	// Unsubscribe ONLY from the selected collection:
                        ...
                        if ($DB->get_var($SQL) === '1') {	// If user is really subscribed on the selected collection:
        ...
        case 'resubscribe':
            ...
            switch ($type) {
                ...
                case 'post_moderator_edit':
                    ...
                    if ($subs_edit_pst_type == 'all') {	// If user was unsubscribed from ALL collections we should subsribe to ALL collections as well:
                        $UserSettings->set('notify_edit_pst_moderation', '1', $edited_User->ID);
                        $UserSettings->dbupdate();
                    } elseif (! empty($coll_ID)) {	// If user was unsubscribed from SINGLE collection we should subsribe ONLY to that collection:
                        $SQL = new SQL('Get user subscription per collection for post modifications');
                        $SQL->SELECT('sub_items_mod');
                        $SQL->FROM('T_subscriptions');
                        $SQL->WHERE('sub_coll_ID = ' . $DB->quote($coll_ID));
                        $SQL->WHERE_and('sub_user_ID = ' . $DB->quote($user_ID));
                        if ($DB->get_var($SQL) === '0') {	// If user is really unsubscribed from the selected collection:
                            $DB->query('UPDATE T_subscriptions
								  SET sub_items_mod = 1
								WHERE sub_user_ID = ' . $DB->quote($user_ID) . '
								  AND sub_coll_ID = ' . $DB->quote($coll_ID));
                ...
                case 'newsletter':
                    ...
                    } else {	// Display a message is the user is not subscribed on the requested newsletter:
...
if (! is_null($params['unsubscribe_form_params'])) { // Use another template param from skin
...
if (isset($error_msg)) {
    ...
    if ($unsub_Comment) {	// Anonymous user from comment:
    ...
    if (! empty($hidden_form_params) && is_array($hidden_form_params)) {	// Set additional hidden params from the array:
    ...
    if ($unsub_Comment) {	// Anonymous user from comment:
        ...
        echo '<h2 class="user_title text-center">' . $avatar_tag . ' ' . $edited_User->get_colored_login([
            'login_text' => 'name',
        ]) . '</h2>';
    }
    ...
    if (isset($unsubscribed)) {
        ...
        $submit_button = [[
            'name' => 're-subscribe',
            'value' => T_('Re-subscribe!'),
            'class' => 'search btn-success btn-lg',
        ]];
    } else {
        ...
        $submit_button = [[
            'name' => 'unsubscribe',
            'value' => T_('Unsubscribe!'),
            'class' => 'search btn-danger btn-lg',
        ]];
    }
