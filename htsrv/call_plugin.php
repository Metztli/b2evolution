...
if ($plugin_ID === -1 & $method == 'test_api') {	// Use this case to test API from ctrl=system:
    echo 'ok';
    exit(0);
}

if (is_null($params)) {	// Use empty array by default if params are not sent by request:
    $params = [];
} else {	// Params given:
    if (param_check_serialized_array('params')) {	// If the params is a serialized array and doesn't contain any object inside:
    ...
    } else {	// Restrict all non array params to empty array:
        bad_request_die('Invalid params! Cannot unserialize.');
    }
}
...
if ($plugin_ID) {
    ...
    if (method_exists($Plugin, 'get_htsrv_methods')) { // TODO: get_htsrv_methods is deprecated, but should stay here for transformation! (blueyed, 2006-04-27)
    ...
    if ($Plugin->code == 'evo_sociallogin' && $method == 'request_login_credentials') {	// Instruct crawlers not to index:
