...
if ($action == 'exit') {	// Display messages and exit
    // Bootstrap
...
if ($comment_Blog->get_setting('comment_quick_moderation') == 'never') {	// comment quick moderation setting was set to 'never' after this comment quick moderation link was created
...
if (($secret != $posted_Comment->get('secret')) || empty($secret)) {	// Invalid secret, no moderation allowed here, go to regular form with regular login requirements:
    $Messages->add(T_('Invalid secret key. Quick moderation not available.'));
}

if ($posted_Comment->status == 'trash') {	// Comment is already in trash
    $Messages->add(T_('The comment was already deleted. Quick moderation not available.'));
}

if ($Messages->has_errors()) {	// quick moderation is not available, redirect to normal edit form
...
switch ($action) {
    case 'publish':
        ...
        if (! is_logged_in()) {	// Don't allow this action for not logged in user:
    ...
    case 'deleteurl':
        ...
        header_redirect(regenerate_url('action', ['cmt_ID=' . $cmt_ID, 'secret=' . $secret], '', '&'));
        break;
...
echo $posted_Comment->get_author([
...
if (! $is_meta && ($posted_Comment->status !== 'draft' || $posted_Comment->author_user_ID == $current_User->ID)) {	// Display Comment permalink icon
    echo $posted_Comment->get_permanent_link('#icon#') . ' ';
}
echo $posted_Comment->get_title([
...
if (($secret == $posted_Comment->get('secret')) && ($secret != null)) {
    if (is_logged_in()) {
        ...
        if (check_user_perm('comment!CURSTATUS', 'edit', false, $posted_Comment)) {
            ...
            if ($posted_Comment->status != 'published') {	// Check what max comment status is allowed for current User:
                $max_allowed_comment_status = $posted_Comment->get_allowed_status('published');

                if ($max_allowed_comment_status != $posted_Comment->get('status')) {	// If current comment status is not max allowed status yet:
                    $status_button_titles = get_visibility_statuses('ordered-array');
                    foreach ($status_button_titles as $status_button_title) {	// Find button status title by status key:
                    ...
                    if (! empty($max_allowed_comment_status_title)) {	// Display the button only if next/higher status is used as moderation status:
        ...
        if ($posted_Comment->author_url != null) {
            if (check_user_perm('comment!CURSTATUS', 'delete', false, $posted_Comment)) {	// delete url button
            ...
            if (check_user_perm('spamblacklist', 'edit')) {	// antispam tool button
        ...
        echo "\n";
    } else {	// Display this message because the button above to open a comment to higher status is available only for logged in users:
