...
switch ($action) {
    ...
    case 'crop':
        ...
        if ($result !== true) { // If error on crop action then redirect to avatar profile page
    ...
    case 'report_user':
        ...
        if (! check_user_status('can_report_user', $user_ID)) { // current User status doesn't allow user reporting
        ...
        if (get_report_status_text($report_status) == '') { // A report status is incorrect
        ...
        if (! param_errors_detected()) {
            ...
            $blocked_message = '';
            if (check_user_perm('perm_messaging', 'reply')) { // user has messaging permission, set/add this user as blocked contact
                $contact_status = check_contact($user_ID);
                if ($contact_status == null) { // contact doesn't exists yet, create as blocked contact
                    create_contacts_user($user_ID, true);
                    $blocked_message = ' ' . T_('You have also blocked this user from contacting you in the future.');
                } elseif ($contact_status) { // contact exists and it's not blocked, set as blocked
    ...
    case 'remove_report':
        ...
        $unblocked_message = '';
        if (set_contact_blocked($user_ID, 0)) { // the user was unblocked
    ...
    case 'contact_group_save':
        ...
        if (! check_user_perm('perm_messaging', 'reply') ||
            ! check_user_status('can_edit_contacts')) { // current User status doesn't allow user reporting
            // Redirect to the account activation page
...
if (empty($Blog)) { // This case should not happen, $blog must be set
    $Messages->add(T_('Unable to find the selected blog'), 'error');
    header_redirect($baseurl);
}

if (param_errors_detected() || $action == 'refresh_regional') { // unable to update, store unsaved user into session
    $Session->set('core.unsaved_User', $current_User);
} elseif (! param_errors_detected()) { // update was successful on user profile
    switch ($action) {
        case 'update':
            if ($user_tab == 'register_finish') {	// After submitting quick data we should redirect user to page like after registration:
            ...
            } elseif (! $current_User->has_avatar()) {	// Redirect to upload avatar if it is not uploaded yet:
                $redirect_to = get_user_avatar_url();
            }

            if (empty($redirect_to)) {	// Redirect to display user page for cases when redirect param cannot be defined above by some reason:
...
if (! param_errors_detected() || ! isset($disp)) {	// User data is updated without errors
    ...
    header_redirect($redirect_to);
    // EXITED
} else {	// Errors exist; Don't redirect; Display a template to save a received data from request
    $SkinCache = &get_SkinCache();
    $Skin = &$SkinCache->get_by_ID($Blog->get_skin_ID());
    $skin = $Skin->folder;
    $ads_current_skin_path = $skins_path . $skin . '/';
    if (! empty($disp) && file_exists($ads_current_skin_path . $disp . '.main.php')) {	// Call custom file for profile disp if it exists:
        require $ads_current_skin_path . $disp . '.main.php';
    } else {	// Call index main skin file to display a profile disp:
