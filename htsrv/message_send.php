...
if ($Settings->get('system_lock')) { // System is locked for maintenance, users cannot send a message
...
$user_fields = param('user_fields', 'array', []);
...
if ($Blog->get_setting('msgform_display_subject') &&
    ...
    empty($subject) &&
    empty($subject_other)) {	// If subject is required:
    $Messages->add_to_group(T_('Please fill in the subject of your message.'), 'error', T_('Validation errors:'));
}

if ($Blog->get_setting('msgform_display_message')) {	// If message field is displayed:
    if ($Blog->get_setting('msgform_require_message') && empty($message)) {	// If message is required:
        $Messages->add_to_group(T_('Please do not send empty messages.'), 'error', T_('Validation errors:'));
    } elseif ($Settings->get('antispam_block_contact_form') && ($block = antispam_check($message))) {	// a blacklisted keyword has been found in the message:
...
if (! empty($recipient_id)) {	// Get the recipient User by requested ID:
    $UserCache = &get_UserCache();
    $recipient_User = &$UserCache->get_by_ID($recipient_id);
} elseif (! empty($comment_id)) {	// Get the recipient data to send a message to comment's author:
    $CommentCache = &get_CommentCache();
    if (! ($Comment = &$CommentCache->get_by_ID($comment_id, false, false))) {	// The requested comment must exists in DB:
    ...
    if (empty($recipient_User)) {	// If comment's author is anonymous user:
        if (empty($Comment->allow_msgform)) {	// If anonymous user didn't allow to send email messages:
...
if (is_logged_in()) {	// Set name and email of the current logged in user:
    $sender_name = '';
    ...
    switch ($Blog->get_setting('msgform_user_name')) {
        case 'fullname':
            ...
            if (in_array($firstname_editing, $edited_user_perms)) {	// Get first name:
                $user_firstname = param('user_firstname', 'string', '');
                if ($firstname_editing == 'edited-user-required' && empty($user_firstname)) {	// First name is required:
                    param_error('user_firstname', T_('Please enter your first name.'), null, T_('Validation errors:'));
                }
                $sender_name .= $user_firstname;
                if (! empty($user_firstname) && $user_firstname != $current_User->get('firstname')) {	// Set new first name if it has been entered as new:
            ...
            if (in_array($lastname_editing, $edited_user_perms)) {	// Get last name:
                $user_lastname = param('user_lastname', 'string', '');
                if ($lastname_editing == 'edited-user-required' && empty($user_lastname)) {	// Lsst name is required:
                    param_error('user_lastname', T_('Please enter your last name.'), null, T_('Validation errors:'));
                }
                $sender_name .= ' ' . $user_lastname;
                if (! empty($user_lastname) && $user_lastname != $current_User->get('lastname')) {	// Set new last name if it has been entered as new:
        ...
        case 'nickname':
            ...
            if (in_array($nickname_editing, $edited_user_perms)) {
                ...
                if ($nickname_editing == 'edited-user-required' && empty($user_nickname)) {	// Nickname is required:
                    param_error('user_nickname', T_('Please enter your nickname.'), null, T_('Validation errors:'));
                }
                $sender_name .= $user_nickname;
                if (! empty($user_nickname) && $user_nickname != $current_User->get('nickname')) {	// Set new nickname if it has been entered as new:
    ...
    if (empty($sender_name)) {	// If sender name has not been detected from first/last/nickname fields then use preferred name of current User:
        $sender_name = $current_User->get_username();
    }
} else {	// Ask name and email only for anonymous users:
    if ($Blog->get_setting('msgform_require_name') && empty($sender_name)) {	// If name is required:
...
if (empty($recipient_User) && empty($recipient_address)) { // should be prevented by UI
    debug_die('No recipient specified!');
}

// Additional fields:
$send_additional_fields = [];
if (is_array($user_fields) && ! empty($user_fields)) {
    ...
    foreach ($user_fields as $user_field_ID => $user_field_value) {
        if (! isset($coll_additional_fields[$user_field_ID])) {	// Skip wrong field which is not selected for current collection as additional field:
            continue;
        }

        if (! ($UserField = &$UserFieldCache->get_by_ID($user_field_ID, false, false))) {	// Skip wrong field:
            continue;
        }
        ...
        if (empty($text_value)) {	// Skip empty values:
            continue;
        }

        if (is_logged_in()) {	// Update user fields of the logged in User:
            $userfields = $current_User->userfields_by_ID($UserField->ID);
            foreach ($user_field_value as $u => $uf_value) {
                if (empty($uf_value)) {	// Skip empty entered values from contact form:
                    continue;
                }
                ...
                if ($field_type == 'url' && ($error_detail = validate_url($uf_value, 'commenting'))) {	// Skip it if the entered URL is wrong:
                ...
                if ($userfields === false) {	// Add new field value:
                    $current_User->userfield_add($UserField->ID, $uf_value);
                    $update_user_fields = true;
                } else {	// Update existing field value:
                    $add_multifield_value = false;
                    foreach ($userfields as $m => $userfield_data) {
                        if (! empty($userfield_data->list)) {	// Update a list field:
                            $s = 0;
                            foreach ($userfield_data->list as $saved_uf_ID => $saved_uf_value) {
                                if ($s == $u && $uf_value != $saved_uf_value) {	// Only if it was changed:
                        ...
                        } elseif ($m == $u && $uf_value != $userfield_data->uf_varchar) {	// Update a single field only if it was changed:
                            $current_User->userfield_update($userfield_data->uf_ID, $uf_value);
                            $update_user_fields = true;
                        }

                        if ($add_multifield_value && $u > $m && $u < 2) {	// It means second value of multiple field should be added:
                            $current_User->userfield_add($UserField->ID, $uf_value);
                            $update_user_fields = true;
                        }
                    }
...
if ($update_user_fields) {	// If at least one field of current user must be updated:
    $current_User->dbupdate();
}

if (intval($contact_method) > 0) {	// If preferred contact method is selected as user field:
    $UserFieldCache = &get_UserFieldCache();
    ...
    if (isset($msgform_additional_fields[$contact_method]) &&
        ...
        if (isset($user_fields[$contact_method])) {	// Check the entered field for preferred contact method:
            ...
            if (! empty($entered_user_field)) {	// Allow to use the selected field for preferred contact method because it has been entered:
                $contact_method_field_is_filled = true;
            }
        }
        if (! $contact_method_field_is_filled) {	// If the selected contact method field is not filled:
    ...
    } else {	// If the selected contact method is not used as additional field or it doesn't exist in DB:
...
if ($success_message) {	// No errors, try to send the message:
    $send_subject = (empty($subject_other) ? $subject : $subject_other);

    $send_contact_method = $contact_method;
    if (! empty($send_contact_method)) {	// If a preferred contact method is selected of the form:
        if ($send_contact_method === 'pm' && is_logged_in() &&
            $current_User->get_msgform_possibility($recipient_User, 'PM')) {	// PM option only if it is allowed between current and recipient users:
            $send_contact_method = T_('Private Message on this Site');
        } elseif ($send_contact_method === 'email') {	// Email option:
            $send_contact_method = T_('Email');
            if (is_logged_in()) {	// Display an email of current user:
                $send_contact_method .= ' (' . $current_User->get('email') . ')';
            }
        } elseif (intval($send_contact_method) > 0) {	// User field option:
            $UserFieldCache = &get_UserFieldCache();
            if ($UserField = &$UserFieldCache->get_by_ID($send_contact_method, false, false)) {	// Get real name of the selected user field:
                $send_contact_method = $UserField->get('name');
                if (is_logged_in()) {	// Display a value of the selected contact method of current user:
                    $user_field_values = $current_User->userfield_values_by_code($UserField->get('code'));
                    if (! empty($user_field_values)) {	// Only if it is entered by user:
                        $send_contact_method .= ' (' . implode(', ', $user_field_values) . ')';
                    }
                }
            } else {	// Wrong user field:
                $send_contact_method = '';
            }
        }
    }
    ...
    if ($send_method_type == 'PM') {	// Send private message:
        load_class('messaging/model/_thread.class.php', 'Thread');
        load_class('messaging/model/_message.class.php', 'Message');

        $send_message = [];

        if (! empty($send_additional_fields)) {	// Append all filled additonal fields:
            $send_message[0] = [];
        ...
        if (! empty($send_contact_method)) {	// Append a preferred contact method to the message text:
            $send_message[1] = T_('Reply method') . ': ' . $send_contact_method;
        }

        if (! empty($message)) {	// Append message text:
        ...
        $edited_Thread->recipients_list = [$recipient_User->ID];
        $edited_Message->Thread = &$edited_Thread;
        $edited_Message->set('text', $send_message);

        if ($edited_Message->dbinsert_discussion()) {	// Successful creating of new thread:
    ...
    if ($send_method_type == 'email' ||
        ($send_method_type == 'PM' && ! $success_message)) {	// Send email message:
        $send_method_type = 'email';
        ...
        $email_headers = [];
        if (! is_logged_in()) {	// Allow to reply only to anonymous user:
            $email_headers['Reply-To'] = $sender_address;
        }

        if (empty($recipient_User)) {	// Send email to visitor/anonymous:
        ...
        } else {	// Send mail to registered user:
    ...
    if ($success_message) { // Email has been sent successfully
        if (! is_logged_in()) { // We should save a counter (only for anonymous users)
...
} else { // Restore the locale to the blog visitor language even in case of errors
    locale_restore_previous();
}
...
if ($success_message) {
    if ($send_method_type == 'PM') {	// If PM has been sent:
        $Messages->add(T_('Your private message has been sent.'), 'success');
    } else {	// If EMAIL has been sent:
        $Messages->add(sprintf(
            T_('You have successfully sent an email to %s.'),
            (empty($recipient_User) ? $recipient_name : $recipient_User->get_username())
        ), 'success');
    }
    if (isset($recipient_User, $Blog) &&
        $Blog->get_owner_User() &&
        $recipient_User->ID == $Blog->get_owner_User()->ID) {	// Message was sent to collection owner:
        $ItemCache = &get_ItemCache();
        if ($redirect_Item = &$ItemCache->get_by_urltitle($Blog->get_setting('msgform_redirect_slug'), false, false)) {	// Use item permanent URL from collection setting:
            $redirect_to = $redirect_Item->get_permanent_url('', '', '&');
        } else {	// Use collection home page URL:
