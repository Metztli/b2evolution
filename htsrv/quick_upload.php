...
{
    $response = [];
    $response['success'] = isset($message['status']) && in_array($message['status'], ['success', 'rename']);

    if (isset($message['error'])) {	// If error message is passed:
        $response['error'] = $message['error'];
        unset($message['error']);
        if (! isset($message['status'])) {	// Set status to error when it is not provided:
    ...
    if (isset($_FILES['qqfile'])) {	// If file is sending to quick uploader:
        $response['specialchars'] = 1; // Used to decode chars on JS side
        if (isset($message['text'])) {	// Convert special chars to HTML entities:
            $message['text'] = htmlspecialchars($message['text']);
        }
    } else {	// Don't try to encode when no file was sent:
...
if (! $Session->assert_received_crumb('file', false)) {	// Send a clear error message:
...
if ($upload_path === false) {	// Stop on wrong upload path:
...
if (! empty($link_owner)) {	// Get link owner data for Item/Comment/EmailCampaign/Message from request param:
...
if ($link_owner_type == 'comment' && $prefix == 'meta_') {	// Set comment type for proper permission check - meta comment attachments are always allowed:
    $LinkOwner->Comment->type = 'meta';
}

if (! check_perm_upload_files($LinkOwner, $fm_FileRoot)) {	// If current User has no permission to upload files into the file root:
...
$size_limits = [return_bytes(ini_get('post_max_size')), return_bytes(ini_get('upload_max_filesize'))];
...
$file->allowedExtensions = []; // all files types allowed by default
...
if (empty($fm_FileRoot)) { // Stop when this object is NULL, it can happens when media path has no rights to write
...
$oldName = $newName;
// validate file name
if ($error_filename = process_filename($newName, true, true, $fm_FileRoot, $path)) { // Not a file name or not an allowed extension
...
if (! isset($result['success']) || ! $result['success']) { // Error on upload
    evo_uploader_error($result['error']);
    // EXIT here.
}
$file_content = $result['contents'];

if (! file_exists($newFile->get_dir())) { // Create a folder for new uploaded file if it doesn't exist yet
...
if (save_to_file($file_content, $newFile->get_full_path(), 'wb')) {
    ...
    if (isset($LinkOwner)) { // Link the uploaded file to the object only if it is found in DB:
    ...
    $warning = '';
    if ($Messages->count > 0) { // Some errors/info messages can be created during prepare_uploaded_files()
        $warning .= $Messages->display(null, null, false);
    }

    if (! empty($message)) { // There is an error message on uploading
    ...
    if ($UserSettings->get('fm_showdate') != 'no') {	// Get last modified datetime:
    ...
    if (! empty($new_Link)) { // Send also the link data if it was created
