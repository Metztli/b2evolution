...
$required_js = param('required_js', 'array:string', [], false, true);
...
switch ($action) {
    case 'get_item_form':
        ...
        if (! empty($blog_skin_ID)) {	// Initialize collection skin folder to check if it has a specific new item form:
    ...
    case 'get_comment_form':
        ...
        $blog_skin_ID = $Blog->get_skin_ID();
        if (! empty($blog_skin_ID)) { // check if Blog skin has specific comment form
            $SkinCache = &get_SkinCache();
            $Skin = &$SkinCache->get_by_ID($blog_skin_ID);
            $ads_current_skin_path = $skins_path . $Skin->folder . '/';
        }
    ...
    case 'get_msg_form':
        ...
        $blog_skin_ID = $Blog->get_skin_ID();
        if (! empty($blog_skin_ID)) { // check if Blog skin has specific concact message form
            $SkinCache = &get_SkinCache();
            $Skin = &$SkinCache->get_by_ID($blog_skin_ID);
            $ads_current_skin_path = $skins_path . $Skin->folder . '/';
        }
    ...
    case 'get_widget_form':
        ...
        if (param('wi_ID', 'integer', 0)) {	// Try to get a Widget by ID if it called from DB:
        ...
        } else {	// Try to get a Widget by code if it called from content with inline short tag like [emailcapture]:
            param('wi_code', 'string', true);
            if (! file_exists($inc_path . 'widgets/widgets/_' . $wi_code . '.widget.php')) {	// For some reason, that widget doesn't seem to exist... (any more?)
                debug_die('Wrong widget request!');
            }
    ...
    case 'get_user_bubbletip':
        ...
        if (strpos($_SERVER["HTTP_REFERER"], $admin_url) !== false) {	// If ajax is requested from admin page we should to set a variable $is_admin_page = true if user has permissions
            // Check global permission:
            if (! check_user_perm('admin', 'restricted')) {	// No permission to access admin...
                require $adminskins_path . '_access_denied.main.php';
            } else {	// Set this page as admin page
                $is_admin_page = true;
            }
        }

        if ($blog_ID > 0) {	// Get Blog if ID is set
            $BlogCache = &get_BlogCache();
            $Collection = $Blog = $BlogCache->get_by_ID($blog_ID);
        }

        if ($user_ID > 0) {	// Print info of the registered users
            ...
            if (check_user_perm('admin', 'restricted') &&
                ...
                check_user_status('can_access_admin')) {	// Display the moderation buttons only if current user has a permission:
                ...
                if ($current_User->ID != $User->ID && check_user_perm('users', 'edit')) {	// Display a button to delete a spammer only for other users and if current user can edit them:
            ...
            } else {	// No permission to moderate users:
                $moderation_buttons = '';
            }

            echo '<div class="bubbletip_user">';

            if ($User->check_status('is_closed')) { // display only info about closed accounts
            ...
            $avatar_overlay_text = '';
            $link_class = '';
            if (is_admin_page()) {	// Set avatar size for Back-office
                $avatar_size = $Settings->get('bubbletip_size_admin');
            } elseif (is_logged_in()) {	// Set avatar size for logged in users in the Front-office
                $avatar_size = $Settings->get('bubbletip_size_front');
            } else {	// Set avatar size for Anonymous users
            ...
            if (! ($Settings->get('allow_anonymous_user_profiles') || (check_user_perm('user', 'view', false, $User)))) { // User is not logged in and anonymous users may NOT view user profiles, or if current User has no permission to view additional information about the User
                echo $moderation_buttons;
                echo '</div>'; /* end of: <div class="bubbletip_user"> */
                break;
            }
            ...
            // Location
            $location = [];
            if (! empty($User->city_ID)) {	// City
                $location[] = $User->get_city_name(false);
            }
            if (! empty($User->subrg_ID)) {	// Subregion
                if (! is_logged_in()) {	// Display subregion for not logged in users
                    $location[] = $User->get_subregion_name();
                } elseif ($current_User->subrg_ID != $User->subrg_ID) {	// If subregions are different
                    $location[] = $User->get_subregion_name();
                }
            }
            if (! empty($User->rgn_ID)) {	// Region
                if (! is_logged_in()) {	// Display region for not logged in users
                    $location[] = $User->get_region_name();
                } elseif ($current_User->rgn_ID != $User->rgn_ID) {	// If regions are different
                    $location[] = $User->get_region_name();
                }
            }
            if (! empty($User->ctry_ID)) {	// Country
                if (! is_logged_in()) {	// Display country for not logged in users
                    $location[] = $User->get_country_name();
                } elseif ($current_User->ctry_ID != $User->ctry_ID) {	// If countries are different
                    $location[] = $User->get_country_name();
                }
            }
            if (! empty($location)) {	// Set location info
            ...
            } elseif (! empty($User->age_min) || ! empty($User->age_max)) {	// Min age equals max age
                $age = ! empty($User->age_min) ? $User->age_min : $User->age_max;
                $user_info[] = sprintf(T_('%d years old '), $age);
            }

            if (! empty($user_info)) {	// Display additional user info
                echo '<ul>';
            ...
            echo $moderation_buttons;
            echo '</div>'; /* end of: <div class="bubbletip_user"> */
        } elseif ($comment_ID > 0) {	// Print info for an anonymous user who posted a comment
    ...
    case 'set_comment_vote':
        ...
        if (! is_logged_in(false)) { // Only active logged in users can vote
            break;
        }
        ...
        if (param('is_backoffice', 'integer', 0)) { // Set admin skin, used for buttons, @see button_class()
        ...
        if ($type != 'spam' || empty($commentid)) {	// Incorrect params
            break;
        }

        $edited_Comment = &Comment_get_by_ID($commentid, false);
        if ($edited_Comment !== false) { // The comment still exists
            if ($current_User->ID == $edited_Comment->author_user_ID) { // Do not allow users to vote on their own comments
                break;
            }
    ...
    case 'voting':
        ...
        if (! empty($vote_action)) { // Use crumb checking only for actions
            // Check that this action request is not a CSRF hacked request:
        ...
        switch ($vote_type) {
            case 'link':
                ...
                if (empty($link_ID) || (! is_decimal($link_ID))) { // There is no correct link ID
                    break 2;
                }

                $LinkCache = &get_LinkCache();
                $Link = &$LinkCache->get_by_ID($link_ID, false);
                if (! $Link) { // Incorrect link ID
                    break 2;
                }

                $File = &$Link->get_File();
                if (! $File) { // The Link File is not available
                    break 2;
                }

                if (empty($File->hash)) { // File hash still is not defined, we should create and save it
                    $File->set_param('hash', 'string', md5_file($File->get_full_path(), true));
                    $File->dbsave();
                }

                if (! empty($vote_action)) { // Vote for this file link
                    link_vote($link_ID, $current_User->ID, $vote_action, $checked);
                }

                $voting_form_params['vote_ID'] = $link_ID;

                if (empty($vote_action) || in_array($vote_action, ['like', 'noopinion', 'dontlike'])) { // Display a voting form if no action
                    // or Refresh a voting form only for these actions (in order to disable icons)
                    if (! empty($blog_ID)) { // If blog is defined we should check if we can display info about number of votes
                        $BlogCache = &get_BlogCache();
                        if (($Collection = $Blog = &$BlogCache->get_by_ID($blog_ID, false, false)) &&
                            ...
                            if ($Skin = &$SkinCache->get_by_ID($blog_skin_ID, false, false) &&
                                $Skin->get_setting('colorbox_vote_' . $LinkOwner->get('name') . '_numbers')) { // Display number of votes for current link type if it is enabled by blog skin
            ...
            case 'comment':
                ...
                if (empty($comment_ID)) { // No comment ID
                    break 2;
                }

                $CommentCache = &get_CommentCache();
                $Comment = $CommentCache->get_by_ID($comment_ID, false);
                if (! $Comment) { // Incorrect comment ID
                    break 2;
                }

                if ($current_User->ID == $Comment->author_user_ID) { // Do not allow users to vote on their own comments
                    break 2;
                }
                ...
                if (! $comment_Item->Blog->get_setting('allow_rating_comment_helpfulness')) { // If Users cannot vote
                    break 2;
                }

                if (! empty($vote_action)) { // Vote for this comment
                    switch ($vote_action) { // Set field value
                        case 'like':
                            $field_value = 'yes';
                            break;
                    ...
                    if (isset($field_value)) { // Update a vote of current user
                        $Comment->set_vote('helpful', $field_value);
                        $Comment->dbupdate();
                    }
                }

                if (! empty($redirect_to)) { // Redirect to back page, It is used by browsers without JavaScript
                    header_redirect($redirect_to, 303); // Will EXIT
                    // We have EXITed already at this point!!
                }

                if (param('skin_ID', 'integer', 0) > 0) {	// If request is from skin:
                    $SkinCache = &get_SkinCache();
                    ...
                    if ($request_Skin && method_exists($request_Skin, 'display_comment_voting_panel')) {	// Request skin to display a voting panel for item:
                        $request_Skin->display_comment_voting_panel($Comment, $request_Skin->get_setting('voting_place'), [
            ...
            case 'item':
                ...
                if (empty($item_ID)) {	// No item ID
                    break 2;
                }

                $ItemCache = &get_ItemCache();
                $Item = $ItemCache->get_by_ID($item_ID, false);
                if (! $Item) {	// Incorrect item ID:
                    break 2;
                }

                if ($current_User->ID == $Item->creator_user_ID) {	// Do not allow users to vote on their own comments:
                    break 2;
                }

                $item_Blog = &$Item->get_Blog();

                if (empty($item_Blog) || ! $item_Blog->get_setting('voting_positive')) {	// If Users cannot vote:
                    break 2;
                }

                if (! empty($vote_action)) {	// Vote for the item:
                    switch ($vote_action) { // Set field value
                        case 'like':
                ...
                if (param('widget_ID', 'integer', 0) > 0) {	// If request is from widget:
                    $WidgetCache = &get_WidgetCache();
                    $item_Widget = &$WidgetCache->get_by_ID(get_param('widget_ID'), false, false);
                    if ($item_Widget && $item_Widget->code == 'item_vote') {	// Request widget to display a voting panel for item:
                        $item_Widget->display_voting_panel($Item, [
                            'display_wrapper' => false,
                        ]);
                        break 2;
                    }
                } elseif (param('skin_ID', 'integer', 0) > 0) {	// If request is from skin:
                    $SkinCache = &get_SkinCache();
                    // Initialize global Collection in order to get Skin settings values from the Collection and not from defaults of the Skin:
                    $Blog = $item_Blog;
                    $request_Skin = &$SkinCache->get_by_ID(get_param('skin_ID'), false, false);
                    if ($request_Skin && method_exists($request_Skin, 'display_item_voting_panel')) {	// Request skin to display a voting panel for item:
                        $request_Skin->display_item_voting_panel($Item, $request_Skin->get_setting('voting_place'), [
                            'display_wrapper' => false,
                        ]);
                        break 2;
                    }
    ...
    case 'get_user_new_field':
        ...
        if ($userfields[0]->ufdf_duplicated == 'forbidden') {	// This field can be only one instance for one user
            ...
            if ($user_field_exist > 0) {	// User already has a current field type
                break;
            }
        } else {	// It Means: this field can be duplicated
            echo '[1]';
        }
    ...
    case 'get_user_field_autocomplete':
        ...
        $field_type_id = 0;
        if ((int) $attr_id > 0) {	// From filter 'Specific criteria'
            $field_type_id = (int) $attr_id;
        } elseif (preg_match('/^uf_(new|add)_(\d+)_/i', $attr_id, $match)) {	// From new fields we can get the value for uf_ufdf_ID
            $field_type_id = (int) $match[2];
        } elseif (preg_match('/^uf_(\d+)$/i', $attr_id, $match)) {	// From fields in DB we can get only uf_ID, then we should get a value uf_ufdf_ID from DB
        ...
        if ($field_type_id == 0) {	// Bad request
            break;
        }
    ...
    case 'get_regions_option_list':
        ...
        if ($mode == 'load_subregions' || $mode == 'load_all') {	// Load also the subregions
            echo '-##-' . get_subregions_option_list($region_ID, 0, $params);
        }
        if ($mode == 'load_all') {	// Load also the cities
    ...
    case 'get_field_bubbletip':
        ...
        if ($field_ID > 0) {	// Get field info from DB
            $field = $DB->get_row('
				SELECT ufdf_bubbletip, ufdf_duplicated
				  FROM T_users__fielddefs
				 WHERE ufdf_ID = ' . $DB->quote($field_ID));

            if (is_null($field)) {	// No field in DB
                break;
            }

            if (! empty($field->ufdf_bubbletip)) {	// Field has a defined bubbletip text
                $field_info = nl2br($field->ufdf_bubbletip);
            } elseif (in_array($field->ufdf_duplicated, ['allowed', 'list'])) {	// Default info for fields with multiple values
                $field_info = T_('To enter multiple values,<br />please click on (+)');
            }
        }

        if (! empty($field_info)) { // Replace mask text (+) with img tag
            // Use the glyph or font-awesome icons if requested by skin
            param('b2evo_icons_type', 'string', '');
    ...
    case 'validate_login':
        ...
        if (param_check_valid_login('login')) {	// Login format is correct
            if (! empty($login)) {
                $SQL = new SQL('Validate if username is available');
                $SQL->SELECT('user_ID');
                $SQL->FROM('T_users');
                $SQL->WHERE('user_login = "' . $DB->escape($login) . '"');
                if ($DB->get_var($SQL)) {	// Login already exists
                    echo 'exists';
                } else {	// Login is available
                    echo 'available';
                }
            }
        } else {	// Incorrect format of login
    ...
    case 'set_comment_status':
        ...
        if (is_logged_in()) { // Only logged in users can moderate comments
        ...
        if ($result_success === false) { // Some errors on deleting of the comment, Exit here
            header_http_response('500 ' . T_('Comment cannot be updated!'), 500);
            exit(0);
        }

        if ($moderation != null && in_array($request_from, ['items', 'comments'])) { // AJAX request goes from backoffice and ctrl = items or comments
            if (param('is_backoffice', 'integer', 0)) { // Set admin skin, used for buttons, @see button_class()
                global $current_User, $UserSettings, $is_admin_page, $adminskins_path;
                $admin_skin = $UserSettings->get('admin_skin', $current_User->ID);
                $is_admin_page = true;
            ...
            echo_item_comments($blog, $item_ID, $statuses, $currentpage, $limit, [], $filterset_name, $expiry_status);
        } elseif ($request_from == 'front') { // AJAX request goes from frontoffice
    ...
    case 'get_user_new_org':
        ...
        $org_suffix = ' &nbsp; <strong>' . T_('Role') . ':</strong> ' . $Form->text_input('org_roles[]', '', 20, '', '', [
            'maxlength' => 255,
        ]) . ' &nbsp; ';
        $org_suffix .= ' &nbsp; <strong>' . T_('Order') . ':</strong> ' . $Form->text_input('org_priorities[]', '', 10, '', '', [
            'type' => 'number',
            'min' => -2147483648,
            'max' => 2147483647,
        ]) . ' &nbsp; ';
        ...
        $org_suffix .= ' ' . get_icon('add', 'imgtag', [
            'class' => 'add_org',
            'style' => 'cursor:pointer',
        ]);
        $org_suffix .= ' ' . get_icon('minus', 'imgtag', [
            'class' => 'remove_org',
            'style' => 'cursor:pointer',
        ]);
        $Form->select_input_object('organizations[]', 0, $OrganizationCache, T_('Organization'), [
            'allow_none' => $first_org ? true : false,
            'field_suffix' => $org_suffix,
        ]);
    ...
    case 'change_user_org_status':
        ...
        if (count($status) != 5 || ($status[2] != 'y' && $status[2] != 'n') || $org_ID == 0 || ! $user_Organization) { // Incorrect format of status param
        ...
        if ($status[2] == 'y') { // Status will be change to "Not accepted"
            $org_is_accepted = false;
        } else { // Status will be change to "Accepted"
        ...
        $accept_icon_params = [
            'style' => 'cursor: pointer;',
            'rel' => 'org_status_' . ($org_is_accepted ? 'y' : 'n') . '_' . $org_ID . '_' . $user_ID,
        ];
        if ($org_is_accepted) { // Organization is accepted by admin
            $accept_icon = get_icon('allowback', 'imgtag', array_merge([
                'title' => T_('Accepted'),
            ], $accept_icon_params));
        } else { // Organization is not accepted by admin yet
            $accept_icon = get_icon('bullet_red', 'imgtag', array_merge([
    ...
    case 'get_user_salt':
        ...
        // Check that this action request is not a CSRF hacked request:
        if (! $get_widget_login_hidden_fields) { // If the request was received from the normal login form check the loginsalt crumb
            $Session->assert_received_crumb('loginsalt');
        }

        $result = [];

        if ($get_widget_login_hidden_fields) {	// Get the loginform crumb, the password encryption salt, and the Session ID for the widget login form:
            $pepper = $Session->get('core.pepper');
            if (empty($pepper)) {	// Session salt is not generated yet, needs to generate:
        ...
        if (empty($salts)) { // User with the given login was not found add one random salt value
            $salts[] = [
        ...
        foreach ($salts as $salt) {
            ...
            if (! $user_PasswordDriver) {	// Skip this user because he has an unknown password driver:
                continue;
            }
    ...
    case 'crop':
        ...
        if (! is_logged_in()) { // Only the logged in user can crop pictures
            break;
        }

        $file_ID = param('file_ID', 'integer');
        $cropped_File = &$current_User->get_File_by_ID($file_ID, $error_code);
        if (! $cropped_File) { // Wrong file for cropping
            break;
        }
    ...
    case 'get_user_report_form':
        ...
        if (! is_logged_in() || (isset($User) && $current_User->ID == $User->ID) || ! check_user_status('can_report_user', $user_ID)) { // Only if current user can reports
            break;
        }
        ...
        if (param('is_backoffice', 'integer', 0)) { // Load the AdminUI class for the skin.
    ...
    case 'get_user_contact_form':
        ...
        if (! is_logged_in() || (isset($User) && $current_User->ID == $User->ID) ||
            ! check_user_perm('perm_messaging', 'reply') ||
                ! check_user_status('can_edit_contacts')) { // Only if current user can reports
            break;
        }
    ...
    case 'get_edit_image_form':
            ...
            switch ($tag_type) {
                ...
                case 'thumbnail':
                    ...
                    if (isset($parts[$opt_index]) &&
                        substr($parts[$opt_index], 0, 1) != '.' &&
                        ! preg_match($href_regexp, $parts[$opt_index]) &&
                        ! in_array($parts[$opt_index], ['small', 'medium', 'large', 'left', 'right'])) {
                        if ($parts[$opt_index] != '-') {
                            $image_alt = $parts[$opt_index];
                        }
                        $opt_index++;
                ...
                case 'inline':
                    ...
                    if (isset($parts[$opt_index]) &&
                        substr($parts[$opt_index], 0, 1) != '.' &&
                        ! in_array($parts[$opt_index], ['small', 'medium', 'large', 'original'])) {
                        if ($parts[$opt_index] != '-') {
                            $image_alt = $parts[$opt_index];
                        }
                        $opt_index++;
                ...
                default:
                    ...
                    $plugin_data = $Plugins->get_trigger_event_first_return('InitImageInlineTagForm', [
                        'source_tag' => $short_tag,
                        'tag_type' => $tag_type,
                        'link_ID' => $link_ID,
                    ]);
                    if (isset($plugin_data['tag_type'])) {	// Override active tag type from plugins:
                        $tag_type = $plugin_data['tag_type'];
                    }
                    if (isset($plugin_data['link_ID'])) {	// Override link ID from plugins:
        ...
        $LinkCache = &get_LinkCache();
        if (! ($Link = &$LinkCache->get_by_ID($link_ID, false, false))) { // Bad request with incorrect link ID
            echo '';
            exit(0);
        }

        if (! ($File = &$Link->get_File())) { // File no longer available
            echo '';
            exit(0);
        }
    ...
    case 'set_object_link_position':
        ...
        $LinkCache = &get_LinkCache();
        if (($Link = &$LinkCache->get_by_ID($link_ID)) === false) {	// Bad request with incorrect link ID
            echo '';
            exit(0);
        }
        $LinkOwner = &$Link->get_LinkOwner();
        ...
        if ($Link->set('position', $link_position) && $Link->dbupdate()) { // update was successful
            echo 'OK';
            ...
            if ($LinkOwner->type == 'item' &&
                ($link_position == 'cover' || $link_position == 'background')) {	// Position "Cover" or "Background" can be used only by one link
    ...
    case 'update_links_order':
        ...
        if (empty($link_IDs)) { // No links to update, wrong request, exit here:
            break;
        }
        ...
        $LinkCache = &get_LinkCache();
        if (($Link = &$LinkCache->get_by_ID($link_IDs[0])) === false) { // Bad request with incorrect link ID
            exit(0);
        }
        $LinkOwner = &$Link->get_LinkOwner();
        // Check permission:
    ...
    case "colorpicker":
        ...
        if (! is_logged_in()) {	// User must be loggedin for this action:
            break;
        }
    ...
    case 'reorder_widgets':
        ...
        if (! is_logged_in()) {	// Only the logged in users can edit widgets:
            echo T_('You must be logged in to edit widgets.');
            break;
        }

        // Check that this action request is not a CSRF hacked request:
        $Session->assert_received_crumb('widget', true, [
            'msg_format' => 'text',
        ]);
        ...
        if (empty($widgets)) {	// No widgets to reorder:
            break;
        }
        ...
        foreach ($all_widgets as $widget) {
            ...
            if ($widget->wi_enabled) {	// Store in this array only enabled widgets:
        ...
        if (! empty($client_widgets) || ! empty($server_widgets)) {	// Display error if at least one widget was added or deleted in the container:
            ...
            if (! empty($client_widgets)) {	// Log what widgets were missed on server side:
                $client_widgets_data = [];
            ...
            if (! empty($server_widgets)) {	// Log what widgets were missed on client side:
                $server_widgets_data = [];
        ...
        $wi_start_orders = [max($container_widgets) + 1, 1];
    ...
    case 'disable_widget':
        ...
        if (! $disabled_Widget || ! $disabled_Widget->get('enabled')) {	// Display error if widget doesn't exist or it is already disabled:
            echo T_('The widgets have been changed since you last loaded this page.') . ' ' . T_('Please reload the page to be in sync with the server.') . ' ' . T_('If the problem persists, check the widgets in the backoffice.');
            break;
        }
    ...
    case 'get_url_alias_new_field':
        ...
        $alias_field_note = get_icon('add', 'imgtag', [
            'class' => 'url_alias_add',
            'style' => 'cursor: pointer; position: relative;',
        ]);
        $alias_field_note .= get_icon('minus', 'imgtag', [
            'class' => 'url_alias_minus',
            'style' => 'margin-left: 2px; cursor: pointer; position: relative;',
        ]);
    ...
    case 'get_item_selector_info':
        ...
        if ($selected_Item = &$ItemCache->get_by_ID($item_ID, false, false)) {
            if (is_logged_in()) {	// Remember what last collection was used for linking in order to display it by default on next linking:
                global $UserSettings;
