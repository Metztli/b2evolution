...
if ($action == 'req_login' && is_logged_in() && $forward_to !== null) {	// Redirect to a requested URL if user is not logged-in:
...
switch ($action) {
    ...
    case 'closeaccount':
        ...
        if (! empty($reasons) && empty($account_close_type)) { // Don't submit a form without a selected reason
        ...
        if (check_user_perm('users', 'edit', false)) { // Admins cannot close own accounts
        ...
        if (is_logged_in() && $current_User->update_status_from_Request(true, 'closed')) { // user account was closed successful
        ...
        } else { // db update was unsuccessful
    ...
    case 'resetpassword':
        ...
        if (empty($login)) { // Don't allow empty request
        ...
        if ($request_ts_login != null) {
            ...
            if (($login == $last_request_login) && (($servertimenow - $pwdchange_request_delay) < $last_request_ts)) { // the same request was sent from the same session in the last $pwdchange_request_delay seconds ( 5 minutes by default )
        ...
        $UserCache->clear();
        if (is_email($login)) { // User gave an email address, get matching User accounts for this email address and also get first activated one as recipient for the reset email:
            ...
            while (($iterator_User = &$UserCache->get_next()) != null) { // Iterate through UserCache
                if ($iterator_User->check_status('is_validated')) {
                    $only_activated = true;
                } else { // strore not activated user Ids for further use
            ...
            $UserCache->rewind();
        } else { // User gave a login, get hat User by login:
            $forgetful_User = &$UserCache->get_by_login($login);
        }

        if (! $forgetful_User) {	// User does not exist
    ...
    case 'changepwd':
        ...
        $changepwd_url = null;
        if (! empty($blog)) { // blog is set, redirect to in-skin change password form
            $BlogCache = &get_BlogCache();
            $Collection = $Blog = $BlogCache->get_by_ID($blog);
            if ($Blog) {
                $changepwd_url = $Blog->get('pwdchangeurl', [
                    'url_suffix' => 'reqID=' . $reqID,
                    'glue' => '&',
                ]);
            }
        }
        ...
        if (empty($changepwd_url)) { // Display standard(non-skin) form to change password
            $action = 'changepwd';
        } else { // redirect will save $Messages into Session:
    ...
    case 'updatepwd':
        ...
        if (! is_logged_in()) { // Don't allow not logged in user here, because it must be logged in on the action 'changepwd' above
        ...
        if ($result !== true) { // Some errors exist on form submit, Display the form again to change them
            $action = 'changepwd';
            break;
        }
    ...
    case 'activateacc_ez':
        ...
        if (! $User->check_status('can_be_validated')) {
            if ($User->check_status('is_validated')) { // Already activated, e.g. clicked on an obsolete email link:
                $Messages->add(T_('Your account has already been activated.'), 'note');
                if (is_logged_in()) {	// Redirect to base url if user is already logged in:
                    header_redirect($baseurl);
                    /* exited */
                } else {	// Display a login form if user is not logged in yet:
                    $action = 'req_login';
                    break;
                }
            } elseif ($User->check_status('is_closed')) { // Account was closed, don't let to activate the account
                $Messages->add(T_('Your account is closed. You cannot activate it.'), 'error');
                // redirect to base url
                header_redirect($baseurl);
                /* exited */
            }
        } elseif (empty($last_reminder_key) || ($last_reminder_key != $reminder_key)) { // the reminder key in db is empty or not equal with the received one
    ...
    case 'activateacc_sec':
        ...
        if (check_user_status('is_validated')) { // Already validated, e.g. clicked on an obsolete email link:
            $Messages->add(T_('Your account has already been activated.'), 'note');
            // no break: cleanup & redirect below
        } else {
            // Check valid format:
            if (empty($reqID)) { // This was not requested
                $Messages->add(T_('Invalid account activation request!'), 'error');
                $action = 'req_activate_email';
                break;
            }
            ...
            if ($sessID != $Session->ID) { // Another session ID than for requesting account validation link used!
            ...
            if (! is_logged_in()) { // this can happen, if a new user registers and clicks on the "validate by email" link, without logging in first
...
switch ($action) {
    case 'req_activate_email':
        ...
        if (check_user_status('is_validated')) { // Activation not required (check this after login, so it does not get "announced")
            $action = '';
            break;
        }
        ...
        if ($req_activate_email_submit) { // Form has been submitted
            ...
            if (! $email_changed) { // the email was not changed
                $last_activation_email_date = $UserSettings->get('last_activation_email', $current_User->ID);
                if (! empty($last_activation_email_date)) { // at least one validation email was sent
                    ...
                    if ($servertimenow - $last_activation_email_ts < $activate_requests_limit) { // a validation email was sent to the same email address less then the x seconds, where x is the "Activation requests limit" value
            ...
            if ($email_changed) { // Update user's email:
                $current_User->set_email($email);
                if (! $current_User->dbupdate()) { // email address couldn't be updated
            ...
            if (empty($current_User->email)) { // add (error) note to be displayed in the form
...
if (strlen($redirect_to)) { // Make it relative to the form's target, in case it has been set absolute (and can be made relative).
    $redirect_to = url_rel_to_same_host($redirect_to, get_htsrv_url('login'));
}
if (preg_match('#/login.php([&?].*)?$#', $redirect_to)) { // avoid "endless loops"
    $redirect_to = $baseurl;
}
...
if (strlen($return_to)) { // Make it relative to the form's target, in case it has been set absolute (and can be made relative).
    $return_to = url_rel_to_same_host($return_to, get_htsrv_url('login'));
}
if (preg_match('#/login.php([&?].*)?$#', $return_to)) { // avoid "endless loops"
    $redirect_to = $baseurl;
}
// Remove login and pwd parameters from URL, so that they do not trigger the login screen again:
 ...
 */
if ($inskin && use_in_skin_login()) { // in-skin display:
    $BlogCache = &get_BlogCache();
    ...
    if (! empty($Blog)) {
        ...
        if (($action == 'req_activate_email') && ! empty($current_User)) { // redirect to inskin activate account page
            $redirect = url_add_param($Blog->gen_blogurl(), 'disp=activateinfo', '&');
            if ($Messages->has_errors()) {	// Redirect to a form for requesting an activation again if some errors exist
                $redirect = url_add_param($redirect, 'force_request=1', '&');
            }
        } elseif ($action == 'lostpassword') { // redirect to inskin lost password page
            $redirect = $Blog->get('lostpasswordurl', [
                'glue' => '&',
            ]);
            if (! empty($lostpassword_error)) { // Set this param to know after redirection if error was here
                $redirect = url_add_param($redirect, 'field_error=1', '&');
            }
        } else { // redirect to inskin login page
