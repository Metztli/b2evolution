...
$mask_params = ['before_title', 'after_title', 'before_title_text', 'after_title_text'];
$mask_values = [];
foreach ($mask_params as $mask_param) {
    if (strpos($params[$mask_param], '$flag_icon$') !== false && ! isset($mask_values['$flag_icon$'])) {	// Flag icon:
        $mask_values['$flag_icon$'] = $Item->get_flag([
            'before' => ' ',
            'only_flagged' => true,
            'allow_toggle' => false,
        ]);
    }
    if (strpos($params[$mask_param], '$item_icon$') !== false && ! isset($mask_values['$item_icon$'])) {	// Item icon:
        $mask_values['$item_icon$'] = get_icon('file_message');
    }
    if (strpos($params[$mask_param], '$item_status$') !== false && ! isset($mask_values['$item_status$'])) {	// Status(only not published):
        $mask_values['$item_status$'] = $Item->status == 'published' ? '' : $Item->get_format_status([
    ...
    if (strpos($params[$mask_param], '$read_status$') !== false && ! isset($mask_values['$read_status$'])) {	// Read status(New/Updated/Read):
        $mask_values['$read_status$'] = $Item->get_unread_status([
            'style' => 'text',
            'before' => '<span class="evo_post_read_status">',
            'after' => '</span>',
        ]);
    }
    if (strpos($params[$mask_param], '$link_view_changes$') !== false && ! isset($mask_values['$link_view_changes$'])) {	// Link to view changes:
        $mask_values['$link_view_changes$'] = $Item->get_changes_link([
