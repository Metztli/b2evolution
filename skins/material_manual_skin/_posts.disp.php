...
if (isset($tag)) { // Display posts list for selected tag
...
} elseif (! empty($cat) && ($cat > 0)) { // Display Category's page
    global $Item;
    ...
    if (empty($intro_Item) || $intro_Item->get('title') == '') { // Display chapter title only if intro post has no title
    ...
    if (! empty($intro_Item)) { // We have a featured/intro post to display:
        $Item = $intro_Item;
    ...
    $ChapterCache->iterate_through_category_children($curr_Chapter, $callbacks, false, [
        'sorted' => true,
    ]);
    ...
    if (! empty($create_new_chapter_url) || ! empty($write_new_post_url)) {
        ...
        if (! empty($create_new_chapter_url)) { // Display button to write a new post
