...
if ($is_sticky_item) { // Special icon for featured & intro posts
...
} elseif ($Item->comment_status == 'closed' || $Item->comment_status == 'disabled' || $Item->is_locked()) { // The post is closed for comments
    $status_icon = 'fa-lock';
    $status_alt = T_('This topic is locked: you cannot edit posts or make replies.');
    $legend_icons['topic_locked'] = 1;
} elseif ($comments_number > 25) { // Popular topic is when coummnets number is more than 25
    $status_icon = 'fa-star';
...
<article class="container group_row posts_panel">
	...
	<div class="ft_status__ft_title col-lg-8 col-md-8 col-sm-6 col-xs-12">
		...
		<?php
        if (! $is_sticky_item && $Skin->get_setting('voting_place') == 'left_score') {	// Display voting panel in score mode:
            $Skin->display_item_voting_panel($Item, 'left_score');
        } else {	// Display read status icon:
            ?>
		<!-- Thread icon -->
...
if ($Item->get_coll_setting('track_unread_content')) { // Update legend array to display the unread status icons in footer legend:
    switch ($Item->get_read_status()) {
        case 'new':
...
if ($disp == 'mustread') {	// Display a link to view changes only for "must read" content:
    $Item->changes_link([
...
?>
				...
				<?php
                if ($Skin->enabled_status_banner($Item->status)) { // Status:
                    $Item->format_status([
...
?>
    ...
    if (! $display_workflow) { // --------------------------------------------------------------------------------------------------------------------------
        ...
        if ($comments_number == 0 && $Item->comment_status == 'disabled') { // The comments are disabled:
            echo /* TRANS: "Not Available" */ T_('N/A');
        } elseif ($latest_Comment = &$Item->get_latest_Comment()) {	// At least one reply exists:
...
if ($latest_Comment = &$Item->get_latest_Comment()) { // Display info about last comment
    $latest_Comment->author2([
        'before' => '',
        'after' => '',
        'before_user' => '',
        'after_user' => '',
    ...
    echo '</div>';
} else { // No comments, Display info of post
    $Item->author([
        'before' => '',
        'after' => '',
        'before_user' => '',
