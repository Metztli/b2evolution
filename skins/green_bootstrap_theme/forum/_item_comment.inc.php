...
echo update_html_tag_attribs($params['comment_start'], [
    'class' =>
...
if ($Skin->get_setting('voting_place') == 'left_score' && ! $Comment->is_meta()) {	// Display voting panel instead of author avatar:
...
<div class="panel-footer small clearfix">
	<a href="<?php
        if ($disp == 'comments') {	// We are displaying a comment in the Latest comments page:
            echo $Blog->get('lastcommentsurl');
        } else {	// We are displaying a comment under a post/topic:
...
if ($bbcode_plugin_is_enabled && $commented_Item && $commented_Item->can_comment(null)) { // Display button to quote this comment
    echo '<a href="' . $commented_Item->get_permanent_url() . '?quote_comment=' . $Comment->ID . '#form_p' . $commented_Item->ID . '" title="' . format_to_output(T_('Reply with quote'), 'htmlattr') . '" class="' . button_class('text') . ' pull-left quote_button">' . get_icon('comments', 'imgtag', [
...
if ($params['display_vote_helpful']) {	// Display a voting panel for comment:
