...
if (! is_array($legend_statuses)) { // Init this array only first time
    $legend_statuses = [];
}
...
if ($current_cat == 0) { // Use main category by default because the category wasn't set
        ...
        */
        ...
        widget_container('item_single_header', [
            ...
            'widget_item_title_params' => [
                'before' => '<div class="evo_post_title">' . (in_array($disp, ['single', 'page']) ? '<h1>' : '<h2>'),
                'after' => (in_array($disp, ['single', 'page']) ? '</h1>' : '</h2>') . '</div>',
                'link_type' => 'permalink',
            ],
...
?>
					...
					<?php
    if ($Skin->enabled_status_banner($Item->status)) { // Status banner
...
?>
		...
		<div class="panel-body">
			<div class="ft_avatar<?php echo $Skin->get_setting('voting_place') == 'under_content' ? ' col-md-1 col-sm-2' : ''; ?>"><?php
                if ($Skin->get_setting('voting_place') == 'left_score') {	// Display voting panel instead of author avatar:
                    $Skin->display_item_voting_panel($Item, 'left_score');
                } else {	// Display author avatar:
                    $Item->author([
...
?></div>
    ...
    if ($disp == 'single') {
        ...
        if (! $Item->is_intro()) { // List all tags attached to this topic:
            $Item->tags([
...
if (class_exists('bbcode_plugin')) { // Plugin exists
    global $Plugins;
    $bbcode_Plugin = &$Plugins->get_by_classname('bbcode_plugin');
    if ($bbcode_Plugin->status == 'enabled' && $bbcode_Plugin->get_coll_setting('coll_apply_comment_rendering', $Blog) != 'never') { // Plugin is enabled and activated for comments
        $bbcode_plugin_is_enabled = true;
    }
}
if ($bbcode_plugin_is_enabled && $Item->can_comment(null)) {	// Display button to quote this post
    echo '<a href="' . $Item->get_permanent_url() . '?quote_post=' . $Item->ID . '#form_p' . $Item->ID . '" title="' . format_to_output(T_('Reply with quote'), 'htmlattr') . '" class="' . button_class('text') . ' pull-left quote_button">' . get_icon('comments', 'imgtag', [
        'title' => T_('Reply with quote'),
    ]) . ' ' . T_('Quote') . '</a>';
}

if ($disp != 'page') {	// Display a panel with voting buttons for item:
...
$Item->next_status_link([
    'before' => ' ',
    'class' => button_class('text'),
    'post_navigation' => 'same_category',
    'nav_target' => $current_cat,
], true);
$Item->next_status_link([
    'class' => button_class('text'),
    'before_text' => '',
...
if (is_single_page()) {	// Display comments only on single Item's page:
    ...
    skin_include('_item_feedback.inc.php', array_merge($params, [
        'disp_comments' => true,
...
?>
		...
		<?php
    if ($Skin->is_visible_sidebar_forums(false, 'single')) {	// Display sidebar:
        ?>
			...
			<?php
                ...
                // Display container contents:
            widget_container('sidebar_single', [
                // The following (optional) params will be used as defaults for widgets included in this container:
                'container_display_if_empty' => false, // If no widget, don't display container at all
