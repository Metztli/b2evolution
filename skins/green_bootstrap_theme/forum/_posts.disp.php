...
if (! is_array($legend_icons)) { // Init this array only first time
    $legend_icons = [];
}
...
if (! in_array($disp, ['single', 'page']) &&
    $Item = &get_featured_Item('posts', null, false, (isset($tag) || $single_cat_ID ? false : null))) {	// We have a intro post to display:
    ...
    skin_include('_item_block_intro.inc.php', [
...
if ($single_cat_ID) {	// Display sub-chapters:
    $ChapterCache = &get_ChapterCache();
...
<?php
        ...
        foreach ($chapters as $Chapter) { // Loop through categories:
            if ($Chapter->meta) { // Meta category
                $chapters_children = $Chapter->get_children(true);
                if ($section_is_started) { // Close previous opened table
                    ?>
<?php
                    $section_is_started = false;
                }
...
<?php
            } else { // Simple category with posts
                $chapters_children = [$Chapter];
            }
...
<?php
            ...
            foreach ($chapters_children as $Chapter) { // Loop through categories:
                if ($Chapter->lock) { // Set icon for locked chapter
                ...
                } else { // Set icon for unlocked chapter
		...
		<article class="container group_row">
			<div class="ft_status__ft_title col-lg-8 col-md-7 col-sm-7 col-xs-6">
                ...
                if (count($sorted_sub_chapters) > 0) { // Subforums exist
                    ...
                    echo T_('Subforums') . ': ';
                    $cc = 0;
                    foreach ($sorted_sub_chapters as $child_Chapter) { // Display subforum
...
<div class="panel panel-default forums_list">
	<?php
        if ($single_cat_ID) {	// Display category title:
            $ChapterCache = &get_ChapterCache();
            if ($category = &$ChapterCache->get_by_ID($single_cat_ID)) {	// Display category title:
                $Skin->display_posts_list_header('<h3 class="panel-title">' . $category->get('name') . '</h3>', [
                    'actions' => $Skin->get_post_button($single_cat_ID, null, [
                        'group_class' => 'pull-right',
                        'button_class' => 'btn-sm',
                    ]),
                ]);
            }
        } else {	// Display header for latest topics:
...
if ($single_cat_ID) {	// Go to grab only featured posts only on pages with defined category:
    while ($Item = &get_featured_Item('posts', null, false, true, false)) {	// We have the featured posts to display:
        if (isset($featured_item_ID) && $featured_item_ID == $Item->ID) {	// Skip featured Item if it is already displayed above in intro style block:
            continue;
        }
...
<?php
    ...
    if (! is_logged_in() && ! $Blog->get_setting('post_anonymous')) {	// Display a warning to log in or register before new post creating:
        $register_link = '';
    ...
    if (check_user_status('can_be_validated')) {	// Display a warning if current user cannot post a topic because he must activate account:
        global $Messages;
        $Messages->clear();
