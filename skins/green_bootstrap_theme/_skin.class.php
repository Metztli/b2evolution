...
class green_bootstrap_theme_Skin extends Skin
{
     ...
     */
    public function get_param_definitions_main($params)
    {
     ...
     * @return array
     */
    public function get_param_definitions_std($params)
    {
        // Load for function get_available_thumb_sizes():
        load_funcs('files/model/_image.funcs.php');
     ...
     * @param local params like 'for_editing' => true
     */
    public function get_param_definitions_photo($params)
    {
     ...
     * @param local params like 'for_editing' => true
     */
    public function get_param_definitions_forum($params)
    {
        // Load for function get_available_thumb_sizes():
        load_funcs('files/model/_image.funcs.php');
...
*/
     ...
     * @param local params like 'for_editing' => true
     */
    public function get_param_definitions_manual($params)
    {
        // Load for function get_available_thumb_sizes():
        load_funcs('files/model/_image.funcs.php');
     ...
     * This may register some CSS or JS...
     */
    public function display_init_main()
    {
        ...
        if (in_array($disp, ['front', 'login', 'register', 'lostpassword', 'activateinfo', 'access_denied', 'access_requires_login', 'content_requires_login'])) {
            // **** Image section / START ****
            // Background image:
            $this->dynamic_style_rule('front_bg_image_file_ID', '.evo_pictured_layout { background-image: $setting_value$ }', [
                'type' => 'image_file',
            ]);
            // Background color:
            ...
            // Position:
            $this->dynamic_style_rule('front_position', 'div.front_main_area { $setting_value$ }', [
                'options' => [
                    'left' => '', // default value
        ...
        if ($Session->get('designer_mode_' . $blog)) {	// On enabled designer mode we should set full window height for pictured layout in pixel instead of 100% percents to avoid issues on scroll page:
     ...
     * This may register some CSS or JS...
     */
    public function display_init_std()
    {
     ...
     * This may register some CSS or JS...
     */
    public function display_init_photo()
    {
...
' . $custom_css . '	-->
     ...
     * This may register some CSS or JS...
     */
    public function display_init_forum()
    {
        ...
        if (in_array($disp, ['single', 'page', 'comments'])) { // Load jquery UI to animate background color on change comment status or on vote
            require_js_defer('#jqueryUI#', 'blog');
        }

        if (in_array($disp, ['single', 'page'])) {	// Init JS to autcomplete the user logins
...
-->
        ...
        if (($this->get_setting('sidebar_general_affix') && $this->is_visible_sidebar_forums(true, 'general')) ||
            ($this->get_setting('sidebar_single_affix') && $this->is_visible_sidebar_forums(true, 'single'))) {	// Init JS to fix sidebars on scroll down:
     ...
     * This may register some CSS or JS...
     */
    public function display_init_manual()
    {
        ...
        if ($this->is_side_navigation_visible()) { // Include JS code for left navigation panel only when it is displayed:
            ...
            case 'photo':
                ...
                switch ($name) {
                    ...
                    case 'Form':
                        ...
                        return array_merge(parent::get_template($name, false), [
    ...
    public function get_column_class_forums($layout = 'general')
    {
    ...
    public function get_column_class_photo($layout = 'general')
    {
        switch ($this->get_setting('layout')) {
            case 'single_column':
                // Single Column Large
     ...
     * @return string
     */
    public function is_visible_sidebar($check_containers = false, $layout = 'general')
    {
        return $this->call_func_by_coll_type(__FUNCTION__, func_get_args());
    }

    // Alias of is_visible_sidebar() for collection kind "Blog":
    public function is_visible_sidebar_std($check_containers = false, $layout = 'general')
    {
    ...
    public function is_visible_sidebar_forums($check_containers = false, $layout = 'general')
    {
     ...
     * @return string
     */
    public function get_setting_layout($layout = 'general')
    {
        global $disp;

        if ($disp == 'single') {	// Single post page has a separate setting for layout:
            if ($layout == 'single') {
                return $this->get_setting('layout_single');
            }
        } elseif ($layout == 'general') {	// Use this settings for all other pages:
     ...
     */
    public function display_button_recent_topics()
    {
        ...
        if ($unread_posts_count > 0) {	// If at least one new unread topic exists
            $btn_class = 'btn-warning';
            $btn_title = T_('New Topics') . ' <span class="badge">' . $unread_posts_count . '</span>';
        } else {	// Current user already have read all topics
            $btn_class = 'btn-info';
     ...
     * @param array Params
     */
    public function display_item_voting_panel($Item, $place, $params = [])
    {
        if ($place != $this->get_setting('voting_place')) {	// Skip because different place for panel is requested:
            return;
        }
        ...
        switch ($place) {
            ...
            case 'left_score':
                ...
                skin_widget(array_merge([
                    ...
                    'score_class' => (in_array($disp, ['posts', 'flagged']) ? 'vote_score__status_' . $Item->get_read_status() : ''),
     ...
     * @param array Params
     */
    public function display_comment_voting_panel($Comment, $place, $params = [])
    {
        if ($place != $this->get_setting('voting_place')) {	// Skip because different place for panel is requested:
            return;
        }
        ...
        switch ($place) {
            case 'under_content':
                // Show under posts/comments:
                $Comment->vote_helpful('', '', '&amp;', true, true, array_merge([
                    'before_title' => '',
     ...
     */
    public function display_post_button($chapter_ID, $Item = null, $params = [])
    {
     ...
     * @return string
     */
    public function get_post_button($chapter_ID, $Item = null, $params = [])
    {
        global $Collection, $Blog;
        ...
        if ($default_new_ItemType === false) { // Do not show button on disabled default item type for new items:
            return '';
        }

        $write_new_post_url = $Blog->get_write_item_url($chapter_ID);
        if ($write_new_post_url != '') { // Display button to write a new post
            if (empty($default_new_ItemType)) {	// Use default button text:
                $button_text = T_('New topic');
            } else {	// Use button text from Item Type:
        ...
        } else { // If a creating of new post is unavailable
            $ChapterCache = &get_ChapterCache();
            $current_Chapter = $ChapterCache->get_by_ID($chapter_ID, false, false);

            if ($current_Chapter && $current_Chapter->lock) { // Display icon to inform that this forum is locked
        ...
        if (! empty($Item)) {
            if ($Item->comment_status == 'closed' || $Item->comment_status == 'disabled' || $Item->is_locked()) { // Display icon to inform that this topic is locked for comments
                if (! $chapter_is_locked) { // Display this button only when chapter is not locked, to avoid a duplicate button
            ...
            } else { // Display button to post a reply
        ...
        if (! empty($post_button)) { // Display button
     ...
     * @param string Title
     */
    public function display_posts_list_header($title, $params = [])
    {
        global $Blog;
        ...
        if ($can_view_workflow) {	// Display status filter only when current User a permission to view workflow properties:
        ...
        if (! empty($params['actions'])) {	// Actions:
     ...
     */
    public function is_side_navigation_visible()
    {
        global $disp;

        if (in_array($disp, ['access_requires_login', 'content_requires_login', 'access_denied'])) { // Display left navigation column on this page when at least one sidebar container is visible:
            return $this->show_container_when_access_denied('sidebar') || $this->show_container_when_access_denied('sidebar2');
        }

        // Display left navigation column only on these pages:
        return in_array($disp, ['front', 'posts', 'comments', 'flagged', 'mustread', 'single', 'search', 'edit', 'edit_comment', 'catdir', '404']);
    }
     ...
     * @return boolean
     */
    public function is_3rd_right_column_layout()
    {
        global $disp, $disp_detail;

        if (! $this->is_side_navigation_visible()) {	// Side navigation is hidden for current page:
            return false;
        }

        // Check when we should use layout with 3 columns:
        if ($disp == 'front') {	// Front page
            return (bool) $this->get_checklist_setting('use_3_cols', 'front');
        }

        if ($disp == 'single') {	// Single post/item page:
            return ($this->get_checklist_setting('use_3_cols', 'single')
                // old setting should be supported:
                || $this->get_setting('single_3_cols'));
        }

        if ($disp_detail == 'posts-topcat-nointro' || $disp_detail == 'posts-topcat-intro') {	// Category page with or without intro:
            return (bool) $this->get_checklist_setting('use_3_cols', 'posts-topcat');
        }

        if ($disp_detail == 'posts-subcat-nointro' || $disp_detail == 'posts-subcat-intro') {	// Sub-category page with or without intro:
            return (bool) $this->get_checklist_setting('use_3_cols', 'posts-subcat');
        }

        // All other disps:
        return (bool) $this->get_checklist_setting('use_3_cols', 'other');
    }
     ...
     */
    public function get_layout_class($place)
    {
        $r = '';
        ...
        switch ($place) {
            case 'container':
                $r .= 'container';
                if ($this->is_3rd_right_column_layout()) {	// Layout with 3 columns on current page:
                    $r .= ' container-xxl';
                }
                break;

            case 'main_column':
                if ($this->is_side_navigation_visible()) {	// Layout with visible left sidebar:
                    if ($this->is_3rd_right_column_layout()) {	// Layout with 3 columns on current page:
