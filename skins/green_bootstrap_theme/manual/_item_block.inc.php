...
$action_links .= $Item->get_copy_link([
    'before' => '',
    'after' => '',
    'text' => '#icon#',
    'class' => button_class(),
]);
if ($Item->is_intro() && ! empty($cat)) { // Link to edit category
    $ItemChapter = &$Item->get_main_Chapter();
    if (! empty($ItemChapter)) {
        $action_links .= $ItemChapter->get_edit_link([
...
if (! empty($action_links)) {	// Group all action icons:
...
if ($disp == 'single') {
    ...
    widget_container('item_single', [
        ...
        'override_params_for_breadcrumb_path' => [
    ...
    if (! $Item->is_intro() && ! $Item->is_featured()) { // Don't display this additional info for intro posts
