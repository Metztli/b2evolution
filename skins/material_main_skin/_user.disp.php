...
if ($is_logged_in && ($current_User->ID != $User->ID) &&
        ...
        $current_User->check_status('can_edit_contacts')) { // User is logged in, has messaging access permission and is not the same user as displayed user
...
// - Report:
if ($is_logged_in && ($current_User->ID != $User->ID) &&
        $current_User->check_status('can_report_user')) { // Current user must be logged in, cannot report own account, and must has a permission to report
...
// - Edit in back-office:
if ($is_logged_in && $current_User->check_perm('users', 'edit') && $current_User->check_status('can_access_admin')) { // Current user can edit other user's profiles
    global $admin_url;
...
if ($is_logged_in && $current_User->check_status('can_view_user', $user_ID)) { // Display other pictures, but only for logged in and activated users:
...
foreach ($User->userfields as $userfield) {
    ...
    $userfield_icon = '';
    if (! empty($userfield->ufdf_icon_name)) { // Icon
...
$profileForm->info(T_('Photos'), $User->get_reputation_files([
    'file_type' => 'image',
]));

$profileForm->info(T_('Audio'), $User->get_reputation_files([
    'file_type' => 'audio',
]));

$profileForm->info(T_('Other files'), $User->get_reputation_files([
    'file_type' => 'other',
]));
