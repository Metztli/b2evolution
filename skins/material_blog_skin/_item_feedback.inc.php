...
if (($params['disp_meta_comments'] && is_logged_in() && $current_User->check_perm('meta_comment', 'view', false, $Blog->ID))
    ...
    if (empty($tb) || ! $Item->can_receive_pings()) {	// Trackback not requested or not allowed
    ...
    if (empty($pb)) {	// Pingback not requested
        $params['disp_pingbacks'] = false;				// DO NOT Display the pingbacks if not requested
    }

    if (! ($params['disp_comments'] || $params['disp_comment_form'] || $params['disp_trackbacks'] || $params['disp_trackback_url'] || $params['disp_pingbacks'] || $params['disp_meta_comments'])) {	// Nothing more to do....
        return false;
    }
    ...
    if ($params['disp_comments']) {	// We requested to display comments
        if ($Item->can_see_comments()) {	// User can see a comments
            ...
            } elseif ($title = $Item->get_feedback_title('comments', '#', '#', '#', $comment_inskin_statuses)) {
                $disp_title[] = $title;
            }
    ...
    if ($params['disp_meta_comments'] && is_logged_in()) {	// We requested to display meta comments
        if ($current_User->check_perm('meta_comment', 'view', false, $Blog->ID)) {	// User can see meta comments
            $type_list[] = 'meta';
            if (! empty($params['comments_title_text'])) {
                $disp_title[] = $params['comments_title_text'];
        ...
        } else {	// User cannot see meta comments
    ...
    if ($params['disp_comments'] || $params['disp_trackbacks'] || $params['disp_pingbacks'] || $params['disp_meta_comments']) {
        if (empty($disp_title)) {	// No title yet
            if ($title = $Item->get_feedback_title('feedbacks', '', T_('Feedback awaiting moderation'), T_('Feedback awaiting moderation'), ['review', 'draft'], false)) { // We have some feedback awaiting moderation: we'll want to show that in the title
                $disp_title[] = $title;
            }
        }
        ...
        if ($params['disp_meta_comment_info'] // If we want it
            ...
            if ($meta_comments_count > 0) {	// Display a badge with meta comments count if at least one exists for this Item:
                echo '<a href="' . $meta_comments_url . '" class="badge badge-meta">' . sprintf(T_('%d meta comments'), $meta_comments_count) . '</a>';
            } elseif ($current_User->check_perm('meta_comment', 'add', false, $Blog->ID)) {	// No meta comments yet, Display a button to add new meta comment:
        ...
        if ($Blog->get_setting('threaded_comments')) {	// Array to store the comment replies
            ...
            if ($Comment = $Session->get('core.preview_Comment')) {	// Init PREVIEW comment
        ...
        if ($params['disp_meta_comments']) {	// Calculate index of first meta comment:
...
if (is_logged_in() && $Item->can_comment(null)) {
    if ($params['disp_notification']) {	// Display notification link:
...
if ($Item->can_see_comments(false) && ($params['disp_comments'] || $params['disp_trackbacks'] || $params['disp_pingbacks'])) {	// user is allowed to see comments
