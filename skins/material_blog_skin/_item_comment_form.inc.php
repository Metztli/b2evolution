 ...
 * Comment form:
 */
if ($params['comment_type'] == 'meta') {	// Use different form anchor for meta comments:
...
if ($params['disp_comment_form'] && ($params['comment_type'] == 'meta' && $Item->can_meta_comment() ||
    ...
    if ($Comment->is_meta() &&
            ...
            $current_User->check_perm('item_post!CURSTATUS', 'edit', false, $Item)) {	// Display workflow properties if current user has a permission:
        $Form->select_input_array('item_priority', $Item->priority, item_priority_titles(), T_('Priority'), '', [
        ...
        if (count($UserCache->cache) > 20) {	// Display a text input field with autocompletion if members more than 20:
        ...
        } else {	// Display a select field if members less than 21:
        ...
        if ($Blog->get_setting('use_deadline')) {	// Display deadline fields only if it is enabled for collection:
    ...
    $Plugins->trigger_event('DisplayCommentFormFieldsetAboveComment', [
        'Form' => &$Form,
        'Item' => &$Item,
    ]);
    ...
    } elseif ($params['comment_type'] != 'meta' && $Blog->get_setting('allow_item_subscriptions')) {	// For registered user and normal(not meta) comment and if item subscriptions are allowed for current collection:
        $comment_options[] = ['comment_user_notify', 1, T_('Notify me of replies'), (isset($comment_user_notify) ? $comment_user_notify : 1)];
    }
    if (count($comment_options) > 0) {	// Display additional options:
