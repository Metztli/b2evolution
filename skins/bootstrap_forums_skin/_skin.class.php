...
class bootstrap_forums_Skin extends Skin
{
    ...
    public function get_declared_containers()
    {
        ...
        return [
            ...
            'forum_front_secondary_area' => [NT_('Forum Front Secondary Area'), 47],
            'item_list' => null,
            'item_in_list' => null,
            'sidebar_single' => [NT_('Sidebar Single'), 95],
        ];
    }
...
*/
    ...
    public function set_setting($parname, $parvalue)
    {
        ...
        if (isset($Blog) &&
            ...
            $parvalue == 'left_score') {	// Turn on positive and negative voting for collection when score voting mode is enabled for this Skin:
    ...
    public function display_init()
    {
        ...
        if ($this->get_setting('voting_place') == 'left_score') {	// Initialize JS for voting for score mode on the left of each post/comment:
            if (in_array($disp, ['posts', 'flagged'])) {	// Used to vote on an item:
                init_voting_item_js('blog');
            }
            if ($disp == 'comments') {	// Used to vote on the comments:
        ...
        if (($this->get_setting('sidebar_general_affix') && $this->is_visible_sidebar(true, 'general')) ||
            ($this->get_setting('sidebar_single_affix') && $this->is_visible_sidebar(true, 'single'))) {	// Init JS to fix sidebars on scroll down:
