...
if ($Blog->get_setting('use_workflow') &&
    check_user_perm('blog_can_be_assignee', 'edit', false, $Blog->ID)) {	// Only if current User can be assigned to tasks of the current Collection:
    $assigned_ItemList = new ItemList2($Blog, null, null, 15, 'ItemCache', 'assigned_');
    $assigned_ItemList->set_filters([
...
<?php
    ...
    if ($assigned_ItemList->result_num_rows > 0) {	// Display panel with assigned posts if at least one is found:
...
if ($Item = &get_featured_Item('front')) { // We have a intro-front post to display:
    ?>
<div id="<?php $Item->anchor_id() ?>" class="<?php $Item->div_classes($params) ?>" lang="<?php $Item->lang() ?>">
    ...
    skin_include('_item_content.inc.php', array_merge($params, [
        'Item' => $Item,
    ]));
...
<?php
        foreach ($chapters as $root_Chapter) { // Loop through categories:
            if ($root_Chapter->meta) { // Meta category
...
<?php
                ...
                $chapters_children = [$root_Chapter];
            }
            ?>
