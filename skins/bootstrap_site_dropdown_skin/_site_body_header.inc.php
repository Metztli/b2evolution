...
} else {	// Display only short site name if the logo file cannot be used by some reason above:
                ...
                if (($header_tabs = $site_Skin->get_header_tabs()) !== false) {	// Display the grouped header tabs:
                    foreach ($header_tabs as $s => $header_tab) {	// Display level 0 tabs:
                        ...
                        if (isset($header_tab['items']) &&
                            count($header_tab['items']) > 0) {	// Create a dropdown list trigger:
                        ...
                        if (isset($header_tab['items']) &&
                            count($header_tab['items']) > 0) {	// Display sub menus of the selected level 0 tab only when at least one exists:
                            echo '<ul class="dropdown-menu">';
                            foreach ($header_tab['items'] as $menu_item) {
                                if (is_array($menu_item)) {	// Display sub menus:
                                    ?>
								...
								<?php
                                } elseif ($menu_item == 'pages') {	// Display menu item for Pages of the info/shared collection:
                                    // --------------------------------- START OF PAGES LIST --------------------------------
...
?>
    ...
    widget_container('right_navigation', [
...
<?php if ($site_Skin->get_setting('back_to_top_button')) { // Check if "Back to Top" button is enabled
    ?>
