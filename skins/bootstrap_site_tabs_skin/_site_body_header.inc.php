...
<?php
if (! $site_has_logo_file) {	// Display site name:
    ?>
...
<?php
if ($site_Skin->has_sub_menus()) {	// Display sub menus of the selected level 0 tab only when at least two exist:
    ?>
...
<?php
        foreach ($header_tabs[$site_Skin->header_tab_active]['items'] as $menu_item) {
            if (is_array($menu_item)) {	// Display menu item for collection:
                ?>
