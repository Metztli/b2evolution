...
if (evo_version_compare($app_version, '3.0') < 0) { // Older skins (versions 2.x and above) should work on newer b2evo versions, but newer skins may not work on older b2evo versions.
...
<?php
if ($disp == 'single') { // ------------------- NAVIGATION BAR FOR ALBUM(POST) -------------------
...
<div class="bPosts<?php echo in_array($disp, ['catdir', 'posts', 'single', 'page', 'mediaidx']) ? ' full_width' : '' ?>">
...
?>
	...
	<?php
if ($disp == 'single' || $disp == 'page') { // ------------------------------------ START OF A POST ----------------------------------------
    // Display message if no post:
    display_if_empty();

    if (isset($single_Item)) { // Use Item that already is defined above
        $Item = &$single_Item;
    } else { // Get next Item object
...
<?php
if ($disp != 'catdir') { // Don't display the pages on disp=catdir because we don't have a limit by page there
