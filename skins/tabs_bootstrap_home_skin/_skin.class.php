...
class tabs_bootstrap_home_Skin extends Skin
{
    ...
    public function get_param_definitions($params)
    {
        ...
        if ($homepage_content_tab_key === false) {	// Homepage Content Tab not found, default to show all posts:
        ...
        $r = array_merge(
            [
                ...
                'tabs_item_type' => [
                    ...
                    'options' => [
                        ['all', T_('All posts')],
                        ['featured', T_('Only featured posts')],
                        ['custom', T_('Only posts of type:')],
                    ],
                ...
                'primary_area' => [
                    ...
                    'field_lines' => true,
                    'options' => [
                        ['below_tab_text', T_('Display at static position below tab text')],
                        ['below_tabs', T_('Display below tabs')],
                    ],
        ...
        foreach ($this->get_screen_sizes() as $screen_key => $screen) {
            $r = array_merge($r, [
                ...
                $screen_key . '_static_text_height' => [
...
<?php
    ...
    public function display_init()
    {
        ...
        foreach ($this->get_screen_sizes() as $screen_key => $screen) {
            ...
            // Tab font size:
            $this->dynamic_style_rule($screen_key . '_tab_font_size', '.nav-tabs>li>a { font-size: $setting_value$; }', [
                'check' => 'not_empty',
            ]);
            // Tab padding:
            $this->dynamic_style_rule($screen_key . '_tab_padding', '.nav-tabs>li>a { padding: $setting_value$; }', [
                'check' => 'not_empty',
            ]);
            // Title font size:
            $this->dynamic_style_rule($screen_key . '_title_font_size', '.tbhs_item_title h1, .tbhs_item_content h1 { font-size: $setting_value$; }', [
                'check' => 'not_empty',
            ]);
            // Text font size:
            $this->dynamic_style_rule($screen_key . '_text_font_size', '.tbhs_item_content, .tbhs_item_content * { font-size: $setting_value$; }', [
                'check' => 'not_empty',
            ]);
            // H2 header tag inside text content:
            $this->dynamic_style_rule($screen_key . '_h2_in_text_font_size', '.tbhs_item_content h2 { font-size: $setting_value$; }', [
                'check' => 'not_empty',
            ...
            if ($this->get_setting('primary_area') == 'below_tab_text') {	// Use the settings only when Primary Area is displayed at static position below tab text:
                // Tab text height:
                $this->dynamic_style_rule($screen_key . '_tab_text_height', '.tbhs_item_content { height: $setting_value$px; }', [
                    'check' => 'not_empty',
                ]);
                // Primary Area height:
                $this->dynamic_style_rule($screen_key . '_static_text_height', '.evo_container__front_page_main_area { height: $setting_value$px; }', [
                    'check' => 'not_empty',
                ]);
            }
     ...
     * @return array
     */
    public function get_front_items()
    {
        if (! isset($this->front_items)) {	// Get Items of the Collection:
            global $Blog;
            ...
            $front_ItemList->set_default_filters([
                'types' => $itemtype_ID,
     ...
     * @return object|false
     */
    public function &get_active_front_Item()
    {
