...
if ($edited_Comment->get_author_User()) {
    ...
    $Form->text_input('newcomment_author_email', $edited_Comment->author_email, 20, T_('Email'), '', [
        'maxlength' => 255,
        'style' => 'width: 99%;',
    ]);
...
$Form->textarea_input('content', $comment_content, $disp_params['textarea_lines'], $disp_params['form_comment_text'], [
    'cols' => 60,
    'style' => 'width:100%',
...
if ($current_User->check_perm('blog_edit_ts', 'edit', false, $Blog->ID)) { // ------------------------------------ TIME STAMP -------------------------------------
    $Form->switch_template_parts([
        'fieldstart' => '',
        'fieldend' => '',
        'labelstart' => '',
...
if ($LinkOwner->count_links()) { // there are attachments to display
    ...
    if ($current_User->check_perm('files', 'view')) { // User has permission to view files
...
$Form->submit(['actionArray[update]', T_('Save Changes!'), 'SaveButton submit']);
echo '</div>';
$Form->end_fieldset();
$Form->end_form();
