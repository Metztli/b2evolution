...
<table class="legend" border="0" cellpadding="0" cellspacing="0">
	<tr>
<?php
            foreach ($legend as $legend_key => $status) {	// Display legend icon with description
                if (isset($legend_icons, $legend_icons[$legend_key])) {	// Display a legend icon only if it realy exists on the current page
                    ?>
		<th>
		<?php
                            if (isset($status['system_icon'])) {	// Use system icon:
                                echo get_icon($status['system_icon'], 'imgtag', [
                                    'title' => $status['title'],
                                ]) . ' ';
                            } elseif (strpos($status['icon'], '.gif') !== false) { // The animated icon
                                ?>
