...
if (! isset($comment_template_counter)) {
    ...
    if ($disp == 'single' || $disp == 'post') { // Increase a number, because Item has 1st number
...
if ($disp == 'comments') { // We are displaying a comment in the Latest comments page, we want to show what post/topic it relates to:
    ?>
...
?>#skin_wrapper" class="postlink"><?php echo T_('Back to top'); ?></a></td>
	<td>
	<?php
if ($Comment->ID > 0) { // Display action buttons only for existing comments(Disable for previewed comment)
    ...
    if ($bbcode_plugin_is_enabled && $commented_Item && $commented_Item->can_comment(null)) { // Display button to quote this comment
        echo '<a href="' . $commented_Item->get_permanent_url() . '?mode=quote&amp;qc=' . $Comment->ID . '#form_p' . $commented_Item->ID . '" title="' . T_('Reply with quote') . '" class="roundbutton_text floatleft quote_button">' . get_icon('comments', 'imgtag', [
            'title' => T_('Reply with quote'),
        ]) . T_('Quote') . '</a>';
    }

    if ($params['display_vote_helpful']) { // Display a voting tool
