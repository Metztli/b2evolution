...
if (! empty($cat) && ($cat > 0)) { // Display sub-chapters
    $ChapterCache = &get_ChapterCache();
...
<?php
        if (! empty($cat)) { // Category title
            ?><tr class="table_title">
		<th colspan="5"><?php
                    $ChapterCache = &get_ChapterCache();
            if ($category = &$ChapterCache->get_by_ID($cat)) { // Display category title
...
if (! empty($cat)) { // Go to grab the featured posts only on pages with defined category:
    while ($Item = &get_featured_Item()) { // We have a intro post to display:
