...
if ($Item->is_featured() || $Item->is_intro()) { // Special icon for featured & intro posts
...
?>
		<tr>
			...
			<?php
                if (strpos($status_icon, '.gif') !== false) { // The animated icon
                    ?>
			...
			<td class="ft_title"><?php
                        echo $status_title;
$Item->load_Blog();
if ($Item->Blog->get_setting('track_unread_content')) { // Display icon about unread status
