...
$chapter_ID = 0;
if (! empty($edited_Item->ID)) {	// Get ID of main chapter
    $main_Chapter = &$edited_Item->get_main_Chapter();
    $chapter_ID = $main_Chapter->ID;
} elseif (param('cat', 'integer', 0) > 0) {	// Get chapter ID from request when we create a new topic
...
if ($action != 'new') {	// DO NOT ADD HIDDEN FIELDS IF THEY ARE NOT SET
    ...
    if ($Blog->get_setting('use_workflow') && $current_User->check_perm('blog_can_be_assignee', 'edit', false, $Blog->ID)) {	// We want to use workflow properties for this blog:
...
} elseif ($edited_Item->ID == 0) {	// If new item - add these hidden fields (on the edit mode the checkbox and radio buttons are used)
...
$disp_edit_categories = true;
if (! $disp_params['disp_edit_categories']) {	// When categories are hidden, we store a cat_ID in the hidden input
    ...
    if ($cat > 0 && $edited_Item->ID == 0) {	// Store a cat_ID
...
if ($use_title != 'none') {
    $Form->text_input('post_title', $item_title, 20, T_('Subject'), '', [
        'maxlength' => 255,
        'style' => 'width:98%;',
...
if ($edited_Item->get_type_setting('use_text') != 'never') { // Display text
    ...
    // ---------------------------- TEXTAREA -------------------------------------
    $Form->textarea_input('content', $item_content, 16, T_('Message body'), [
        'cols' => 60,
...
if ($current_User->check_perm('blog_edit_ts', 'edit', false, $Blog->ID)) {	// ------------------------------------ TIME STAMP -------------------------------------
    ...
    $Form->radio_input('item_dateset', $edited_Item->dateset, $values, T_('Issue date'), [
        'note' => $note,
    ]);
    echo $Form->fieldend;
...
if ($disp_edit_categories) {	// Display categories
    $Form->switch_template_parts([
...
if ($edited_Item->ID > 0) { // If item is editing
    if ($edited_Item->get_type_setting('allow_featured')) { // Display featured
        $Form->checkbox_input('item_featured', $edited_Item->featured, T_('Sticky'), [
            'note' => T_('Make this topic sticky (featured at the top of the list)'),
        ]);
    } else { // Hide featured
        $Form->hidden('item_featured', $edited_Item->featured);
    }

    if ($edited_Item->allow_comment_statuses()) { // Display radio inputs to change the comments status
        $comment_status_options = [[
            'value' => 'open',
            'label' => T_('Open'),
            'note' => T_('Visitors can leave comments on this post.'),
        ]];
        if ($edited_Item->get_type_setting('allow_closing_comments')) { // Allow closing comments
            $comment_status_options[] = [
                'value' => 'closed',
                'label' => T_('Closed'),
                'note' => T_('Visitors can NOT leave comments on this post.'),
            ];
        }
        if ($edited_Item->get_type_setting('allow_disabling_comments')) { // Allow disabling comments
            $comment_status_options[] = [
                'value' => 'disabled',
                'label' => T_('Disabled'),
                'note' => T_('Visitors cannot see nor leave comments on this post.'),
            ];
        }
        $Form->radio_input(
            'post_comment_status',
            $edited_Item->get('comment_status'),
            $comment_status_options,
            T_('Allow replies'),
            [
                'lines' => true,
...
visibility_select($Form, $edited_Item->status, false, [], T_('Visibility / Sharing'));
$Form->switch_template_parts($disp_params['edit_form_params']);
...
if ($edited_Item->get_type_setting('use_tags') != 'never') { // Display tags
    ...
    $suggest_checkbox = ' <span title="' . format_to_output(T_('Check this to let b2evolution auto-suggest tags as you type, based on the tags existing on other posts.'), 'htmlattr') . '">'
        ...
        . '</span>';
    $Form->text_input('item_tags', $item_tags, 40, '<span class="nowrap">' . T_('Tags') . $suggest_checkbox . '</span>', '', [
        'maxlength' => 255,
        'style' => 'width:100%;',
...
if ($edited_Item->get_type_setting('allow_attachments')) { // ####################### ATTACHMENTS FIELDSETS #########################
