...
if ($Settings->get('system_lock')) { // System is locked down for maintenance, Stop cron execution
 ...
 */
$quiet = 0;
if ($is_cli) { // called through Command Line Interface, handle args:
...
} elseif (! is_admin_page()) { // This is a web request: (for testing purposes only. Not designed for production)
...
if (empty($task)) {
    ...
    if ($DB->query($sql, 'Request lock') != 1) { // This has no affected exactly ONE row: error! (probably locked -- duplicate key -- by a concurrent process)
        $DB->show_errors = true;
        $DB->halt_on_error = true;
        cron_log('Could not lock. Task is probably handled by another process.', 2);
    } else {
        if (! empty($task->ctsk_repeat_after)) { // This task wants to be repeated:
            ...
            if ($task->ctsk_repeat_after < 86400) {	// If "Repeat every" < 1 day, then start from current timestamp:
                $new_start_datetime = $localtimenow;
            } else {	// If "Repeat every" >= 1 day, then start from TODAY at time of "Schedule date":
            ...
            if ($task->ctsk_key == 'poll-antispam-blacklist') { // THIS IS A HACK. Guess why we need that!? :P  Please do not override or you'll kill our server :(
                $new_start_datetime += rand(43200, 86400); // 12 to 24 hours
            } else { // Normal
                if (! empty($task->ctsk_repeat_variation)) { // Include variation param as random +/- time value
        ...
        if (! empty($error_message)) {	// Set error task in order to report by email to admin in the function detect_timeout_cron_jobs():
            $error_task = [
                'ID' => $ctsk_ID,
                'name' => $ctsk_name,
            ...
            if ($result_status == 'imap_error' &&
                ($max_consecutive_imap_errors = $Settings->get('cjob_imap_error_' . $task->ctsk_key)) > 1) {	// Check if imap error task can be reported by email to admin:
                ...
                if (count($previous_tasks) < $max_consecutive_imap_errors ||
                    count(array_unique($previous_tasks)) > 1) {	// If X previous consecutive tasks have no same status "IMAP error",
        ...
        if (is_array($result_message)) {	// If result is array we should store it as serialized data
...
if (! $is_cli && ! is_admin_page()) {	// This is a web request:
