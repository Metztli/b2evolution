...
if ($params['notify_full']) {	/* Full notification */
    echo '<p' . emailskin_style('.p') . '>' . sprintf(T_('%s assigned you the following post') . ':', $principal_User->get_colored_login([
        'mask' => '$avatar$ $login$',
        'protocol' => 'http:',
        'login_text' => 'name',
    ])) . "</p>\n";
    ...
    echo '<p' . emailskin_style('.p') . '>' . sprintf(T_('%s assigned you a post on %s with title %s.'), $principal_User->get_colored_login([
        'mask' => '$avatar$ $login$',
        'protocol' => 'http:',
        'login_text' => 'name',
...
if ($assigned_User->check_perm('item_post!CURSTATUS', 'edit', false, $Item)) { // User has permission to edit this post
