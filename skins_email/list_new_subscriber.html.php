...
if (empty($params['subscribed_by_admin'])) {	// Current user unsubscribed:
    echo T_('A user subscribed to one of your list/s') . ':';
} else {	// Admin unsubscribed user:
    printf(T_('A user was subscribed to your list/s by %s') . ':', get_user_colored_login_link($params['subscribed_by_admin'], [
        'use_style' => true,
        'protocol' => 'http:',
        'login_text' => 'name',
    ]));
