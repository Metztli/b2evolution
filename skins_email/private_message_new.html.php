...
if ($params['new_thread']) {
    echo sprintf(
        T_('%s just sent you a private message with the title %s.'),
        $from_User->get_colored_login([
            'mask' => '$avatar$ $login$',
            'protocol' => 'http:',
            'login_text' => 'name',
        ]),
    ...
    echo sprintf(
        T_('%s just replied to your private message in the %s conversation.'),
        $from_User->get_colored_login([
            'mask' => '$avatar$ $login$',
            'protocol' => 'http:',
            'login_text' => 'name',
    ...
    echo sprintf(
        T_('%s just replied to the %s conversation.'),
        $from_User->get_colored_login([
            'mask' => '$avatar$ $login$',
            'protocol' => 'http:',
            'login_text' => 'name',
...
if (count($params['other_unread_threads']) > 0) { // Display other unread threads
