...
$params = array_merge([
    'User' => null,
    'activated_by_admin' => '',
    // Login of admin which activated current user account
], $params);
...
if (empty($params['activated_by_admin'])) { // Current user activated own account
    echo T_('New user account activated') . ':';
} else { // Admin activated current user account
    printf(T_('New user account activated by %s'), $params['activated_by_admin']) . ':';
}
echo '</p>' . "\n";

echo '<table' . emailskin_style('table.email_table') . '>' . "\n";
echo '<tr><th' . emailskin_style('table.email_table th') . '>' . /* TRANS: noun */ T_('Login') . ':</th><td' . emailskin_style('table.email_table td') . '>' . $activated_User->get_colored_login([
    'mask' => '$avatar$ $login$',
    'protocol' => 'http:',
]) . '</td></tr>' . "\n";
...
if ($activated_User->reg_ctry_ID > 0) { // Country field is defined
...
if ($activated_User->ctry_ID > 0) { // Country field is defined
    load_class('regional/model/_country.class.php', 'Country');
...
if (! empty($activated_User->source)) { // Source is defined
...
if (! empty($activated_User->level)) {	// User level:
...
if ($user_Group = &$activated_User->get_Group()) {	// User group:
