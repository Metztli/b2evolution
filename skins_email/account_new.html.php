...
echo '<tr><th' . emailskin_style('table.email_table th') . '>' . /* TRANS: noun */ T_('Login') . ':</th><td' . emailskin_style('table.email_table td') . '>' . get_user_colored_login_link($params['login'], [
    'use_style' => true,
    'protocol' => 'http:',
    'login_text' => 'name',
]) . '</td></tr>' . "\n";
...
if ($params['fullname'] != '') { // Full name is entered
...
if ($params['reg_country'] > 0) { // Country field is entered
...
if (! empty($params['reg_domain'])) {	// Domain field is entered:
...
if ($params['country'] > 0) { // Country field is entered
    load_class('regional/model/_country.class.php', 'Country');
    $CountryCache = &get_CountryCache();
...
if (! empty($params['initial_hit'])) { // Hit info
...
if (! empty($params['trigger_url'])) { // Trigger page
...
if (! empty($params['source'])) { // Source is defined
...
if (! empty($params['level'])) {	// User level:
...
if (! empty($params['group'])) {	// User group:
