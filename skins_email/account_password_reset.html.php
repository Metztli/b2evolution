...
while (($iterator_User = &$UserCache->get_next()) != null) {
    ...
    if ($params['user_count'] > 1) { // Several accounts with the given email address, display last used date for each
        $message_content .= '<div style="margin: 1em 0; border: 1px solid #ccc; border-radius: 4px; padding: 1em 1em 1ex;">';

        $message_content .= '<p' . emailskin_style('.p') . '>' . T_('Login') . ': ' . $iterator_User->get_colored_login([
            'mask' => '$login$',
            'protocol' => 'http:',
        ]) . "</p>\n";
        $user_lastseen_ts = $iterator_User->get('lastseen_ts');
        if (empty($user_lastseen_ts)) { // user has never logged in
...
if ($params['user_count'] > 1) { // exists more account with the given email address
