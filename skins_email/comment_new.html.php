...
$author_name = empty($params['author_ID']) ? $params['author_name'] : get_user_colored_login_link($params['author_name'], [
    'use_style' => true,
    'protocol' => 'http:',
    'login_text' => 'name',
]);
if (is_null($Comment)) {
    ...
    $author_type = $Comment->get_author_label([
...
if ($params['notify_type'] == 'meta_comment' || $params['notify_type'] == 'meta_comment_mentioned') { // Internal comment
...
if ($params['notify_type'] == 'comment_mentioned') {	// Add this info line if user was mentioned in the comment content:
    $notify_message .= '<p' . emailskin_style('.p') . '>' . T_('You were mentioned in this comment.') . "</p>\n";
} elseif ($params['notify_type'] == 'meta_comment_mentioned') {	// Add this info line if user was mentioned in the internal comment content:
...
if ($params['notify_type'] == 'moderator') { // moderation email
    if (($Blog->get_setting('comment_quick_moderation') != 'never') && (! empty($Comment->secret))) { // quick moderation is permitted, and comment secret was set
...
switch ($params['notify_type']) {
    case 'moderator':
        // moderation email
        if ($params['is_new_comment']) {	// about new comment:
