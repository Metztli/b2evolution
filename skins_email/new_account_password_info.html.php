...
echo '<p' . emailskin_style('.p') . '>' . sprintf(
    T_('A new user account has been created for you on %s.'),
    '<a href="' . $baseurl . '"' . emailskin_style('.a') . '>' . $Settings->get('notification_short_name') . '</a>'
) . "</p>\n";

echo '<p' . emailskin_style('.p') . '>' . sprintf(T_('Your username is: %s'), '<b>' . $params['login'] . '</b>') . "</p>\n";

if (empty($params['password'])) {	// No password:
    $lostpassword_url = empty($login_Blog)
        ? get_htsrv_url('login') . 'login.php?action=lostpassword'
        : $login_Blog->get('lostpasswordurl', [
            'glue' => '&',
        ]);
...
} else {	// Password was entered by admin:
    if (empty($login_Blog)) {	// Use standard login forms when collection is not detected for this:
        $login_url = get_htsrv_url('login') . 'login.php';
        $UserCache = &get_UserCache();
        if (($User = &$UserCache->get_by_login($params['login'])) &&
            $User->check_perm('admin', 'restricted')) {	// Allow URL to change password from back-office when the user has an access to back-office:
            $changepwd_url = $admin_url . '?ctrl=user&user_tab=pwdchange';
        }
    } else {	// Use collection for login actions:
        $login_url = $login_Blog->get('loginurl', [
            'glue' => '&',
        ]);
        $changepwd_url = $login_Blog->get('pwdchangeurl', [
            'glue' => '&',
        ]);
    }
    if (! empty($changepwd_url)) {	// Redirect to page to change password after login:
