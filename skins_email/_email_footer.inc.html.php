...
if (! in_array($params['template_name'], ['account_activate', 'account_activated'])) {
    // Add custom messages here
}

if (! empty($params['is_welcome_email'])) {	// If this is a welcome email campaign on newsletter subsribing:
    // Add custom messages here
}

if (isset($params['ecmp_ID']) && $params['ecmp_ID'] == 1) {	// Email Campaign #1 is sending now:
    // Add custom messages here
}

if (isset($params['enlt_ID']) && $params['enlt_ID'] == 1) {	// Newsletter/List #1 is sending now:
    // Add custom messages here
}

if (check_usertags($recipient_user_ID, ['user_tag_1', 'user_tag_2'], 'has_any')) {	// Check user tags of the recipient, 3rd param has 3 values:
...
<?php
if (empty($params['email_headers']['Reply-To'])) {	// Display the message below only when replying is not allowed to current email message,
...
echo "<br />\n";
if (! empty($params['recipient_User'])) {	// Display a login and a link to restore a password only for registered User:
...
if (! empty($params['recipient_User']) || ! empty($params['comment_ID'])) {	// Display info to unsubscribe registered or anonymous user:
    ...
    if (! empty($params['unsubscribe_text'])) { // Display the unsubscribe message with link
        echo "<br />\n";
