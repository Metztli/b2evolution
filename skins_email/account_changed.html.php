...
if ($User = &$UserCache->get_by_ID($params['user_ID'], false, false)) {
    ...
    if ($params['new_avatar_upload']) { // Get warning message about duplicated files when any new profile picture has been uploaded
        $FileCache = &get_FileCache();
        $new_File = &$FileCache->get_by_ID($params['new_avatar_upload']);
        $duplicated_files_message = $new_File->get_duplicated_files_message([
    ...
    if ($params['avatar_changed']) { // If profile pictre has been changed
    ...
    } elseif ($params['new_avatar_upload']) { // Display the newly uploaded file only if it was not set as main profile picture
