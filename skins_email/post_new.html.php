...
if ($params['notify_full']) {	/* Full notification */
    echo $mentioned_user_message;

    echo '<p' . emailskin_style('.p') . '>' . T_('Collection') . ': ' . get_link_tag($Blog->gen_blogurl(), $Blog->get('shortname'), '.a') . "</p>\n";

    echo '<p' . emailskin_style('.p') . '>' . T_('Author') . ': ' . get_user_colored_login_link($Item->creator_User->login, [
        'use_style' => true,
        'protocol' => 'http:',
    ]) . ' (' . $Item->creator_User->get('login') . ")</p>\n";
    ...
    echo '<p' . emailskin_style('.p') . '>' . sprintf(T_('%s created a new post on %s with title %s.'), $Item->creator_User->get_colored_login([
        'mask' => '$avatar$ $login$',
        'protocol' => 'http:',
        'login_text' => 'name',
    ]), '<b>' . $Blog->get('shortname') . '</b>', '<b>' . $Item->get('title') . '</b>') . "</p>\n";
...
if ($recipient_User->check_perm('item_post!CURSTATUS', 'edit', false, $Item)) { // User has permission to edit this post
...
switch ($params['notify_type']) {
    case 'moderator':
        // moderation email
        if ($params['is_new_item']) {	// about new item:
