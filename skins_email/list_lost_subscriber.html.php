...
if (empty($params['unsubscribed_by_admin'])) {	// Current user unsubscribed:
    echo T_('A user unsubscribed from your list/s') . ':';
} else {	// Admin unsubscribed user:
    printf(T_('A user was unsubscribed from your list/s by %s') . ':', get_user_colored_login_link($params['unsubscribed_by_admin'], [
        'use_style' => true,
        'protocol' => 'http:',
        'login_text' => 'name',
    ])) . ':';
...
echo '<tr><th' . emailskin_style('table.email_table th') . '>' . /* TRANS: noun */ T_('Login') . ':</th><td' . emailskin_style('table.email_table td') . '>' . $subscribed_User->get_colored_login([
    'mask' => '$avatar$ $login$',
    'protocol' => 'http:',
]) . '</td></tr>' . "\n";
...
if (! empty($user_domain)) {	// Get user domain status if domain field is defined:
...
if ($subscribed_User->ctry_ID > 0) { // Country field is defined
    load_class('regional/model/_country.class.php', 'Country');

    if ($params['user_account_closed']) {	// Set TRUE to use country settings from back office.
...
if (! empty($initial_sess_ID)) {	// Initial session ID:
...
if (! empty($initial_blog_ID)) { // Hit info
...
if (! empty($registration_trigger_url)) { // Trigger page
...
if (! empty($subscribed_User->source)) { // Source is defined
...
if (! empty($subscribed_User->level)) {	// User level:
...
if ($user_Group = &$subscribed_User->get_Group()) {	// User group:
