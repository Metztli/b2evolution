...
$params = array_merge([
    ...
    'user_ID' => '',
    'closed_by_admin' => '',
    // Login of admin which closed current user account
    'days_count' => 0,
], $params);

echo '<p' . emailskin_style('.p') . '>';
if (empty($params['closed_by_admin'])) { // Current user closed own account
    printf(T_('A user account was closed %s days after creation.'), $params['days_count']);
} else { // Admin closed current user account
    printf(T_('A user account was closed %s days after creation by %s'), $params['days_count'], get_user_colored_login_link($params['closed_by_admin'], [
        'use_style' => true,
        'protocol' => 'http:',
        'login_text' => 'name',
    ]));
...
echo '<p' . emailskin_style('.p') . '>' . /* TRANS: noun */ T_('Login') . ": " . get_user_colored_login_link($params['login'], [
    'use_style' => true,
    'protocol' => 'http:',
]) . "</p>\n";
