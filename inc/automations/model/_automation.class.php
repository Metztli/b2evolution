...
class Automation extends DataObject
{
    ...
    public static function get_delete_cascades()
    {
        return [
            ...
            [
                ...
                'fk' => 'aunl_autm_ID',
                'msg' => T_('%d automation associations with lists'),
            ],
        ];
    }
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                'table' => 'T_automation__step',
                'fk' => 'step_info',
                'and_condition' => 'step_type = "start_automation" AND step_autm_ID != $this_ID$',
                'msg' => T_('Automation is used by %s steps of other automations'),
            ],
        ];
    }
     ...
     */
    public function update_newsletters()
    {
        if (empty($this->update_newsletters)) {	// This action is not requested:
            return false;
        }

        if (empty($this->newsletters)) {	// At least one newsletter must be defined:
            return false;
        }
        ...
        foreach ($this->newsletters as $newsletter) {
            ...
            if (empty($newsletter_ID)) {	// Skip wrong newsletter data:
                continue;
            }
        ...
        if (empty($sql_newsletters_values)) {	// At least one newsletter must be inserted:
            return false;
        }
    ...
    public function load_from_Request()
    {
        ...
        $updated_newsletters = [];
        $aunl_enlt_IDs = param('aunl_enlt_ID', 'array:integer', []);
        foreach ($aunl_enlt_IDs as $n => $aunl_enlt_ID) {
            if ($aunl_enlt_ID > 0) {
                $updated_newsletters[$aunl_enlt_ID] = [
        ...
        if (empty($updated_newsletters)) {	// Display an error and fill first required tied list:
        ...
        } else {	// Update newsletters array with new entered values:
            $updated_newsletters = array_values($updated_newsletters);
            if ($prev_newsletters !== $updated_newsletters) {	// Set flag to update newsletters:
     ...
     */
    public function get_user_states()
    {
        global $DB, $servertimenow;

        if (empty($this->ID)) {	// Automation must be stored in DB:
            return [];
        }
     ...
     */
    public function &get_Newsletter($number = 1)
    {
        $newsletters = $this->get_newsletters();

        if (! isset($newsletters[$number]) || ! isset($newsletters[$number]['ID'])) {	// No detected newsletter by number:
            $r = false;
            return $r;
        }
    ...
    {
        if ($this->newsletters === null) {	// Load newsletters settings once:
            if (empty($this->ID)) {	// Set default first newsletter setting:
                $this->newsletters = [[
                    'ID' => '',
                    'autostart' => 1,
                    'autoexit' => 1,
                ]];
            } else {	// Get newsletters settings from DB:
                global $DB;
     ...
     */
    public function get_newsletter_IDs()
    {
        $newsletters = $this->get_newsletters();
     ...
     */
    public function add_users($user_IDs, $params = [])
    {
        global $DB, $servertimenow;
        ...
        if (empty($user_IDs)) {	// No users to add:
            return $added_users_num;
        }
        ...
        if ($params['users_no_subs'] == 'ignore') {	// Ignore not subscribed users to this Automation:
            // Get newsletter IDs:
            $newsletter_IDs = ($params['newsletter_IDs'] === null ? $this->get_newsletter_IDs() : $params['newsletter_IDs']);
            if (! is_array($newsletter_IDs)) {	// If single newsletter is given:
        ...
        if (empty($user_IDs)) {	// No users to add, Stop here:
            return $added_users_num;
        }
        ...
        if (count($automated_user_IDs) || count($user_IDs)) {	// Get first Step of this Automation:
        ...
        if ($params['users_automated'] == 'requeue' && count($automated_user_IDs)) {	// Requeue already automated users to first Step:
            $added_users_num += $DB->query(
        ...
        if ($params['users_new'] == 'add' && count($user_IDs)) {	// Add new users to this Automation:
            $insert_sql = [];
            foreach ($user_IDs as $user_ID) {
                $insert_sql[] = '( ' . $DB->quote($this->ID) . ', ' . $DB->quote($user_ID) . ', ' . $DB->quote($first_step_ID) . ', ' . $DB->quote(date2mysql($servertimenow)) . ' )';
            }
            $added_users_num += $DB->query(
                'INSERT INTO T_automation__user_state ( aust_autm_ID, aust_user_ID, aust_next_step_ID, aust_next_exec_ts )
    ...
    public function is_user_subscribed($user_ID)
    {
        ...
        if (empty($newsletter_IDs)) {	// No automation newsletters found:
            return false;
        }
        ...
        foreach ($newsletter_IDs as $newsletter_ID) {
            if (($automation_Newsletter = &$NewsletterCache->get_by_ID($newsletter_ID, false, false)) &&
                in_array($user_ID, $automation_Newsletter->get_user_IDs())) {	// If user is subscribed to first tied newsletter, Stop find other subscriptions:
     ...
     * @return array Steps
     */
    public function get_diagram_steps_data()
    {
        $steps = [];
        ...
        if (empty($AutomationStepCache->cache)) {	// Automation has no steps yet:
            return $steps;
        }

        $step_results = ['YES', 'NO', 'ERROR'];
        ...
        foreach ($AutomationStepCache->cache as $AutomationStep) {
            ...
            if ($yes_next_AutomationStep = &$AutomationStep->get_yes_next_AutomationStep()) {	// Next YES step:
                $step['next_steps']['yes'] = $yes_next_AutomationStep->ID;
            }
            if ($no_next_AutomationStep = &$AutomationStep->get_no_next_AutomationStep()) {	// Next NO step:
                $step['next_steps']['no'] = $no_next_AutomationStep->ID;
            }
            if ($error_next_AutomationStep = &$AutomationStep->get_error_next_AutomationStep()) {	// Next ERROR step:
            ...
            foreach ($step_results as $step_result) {	// Initialize step data for each result type(YES|NO|ERROR):
     ...
     */
    public function set_diagram_default_steps_positions(&$steps)
    {
        if (empty($steps)) {	// No steps:
            return $steps;
        }
        ...
        foreach ($this->diagram_steps as $step_ID => $step) {	// Start with first step:
        ...
        foreach ($this->diagram_steps as $step_ID => $step) {
            if (isset($step['xy'])) {	// Step is linked:
                if ($unlinked_row < $step['xy'][1]) {	// Define last row:
                    $unlinked_row = $step['xy'][1];
                }
            } else {	// Step is not linked:
        ...
        foreach ($unlinked_steps as $unlinked_step_ID) {
            if (isset($this->diagram_steps[$unlinked_step_ID]['xy'])) {	// Skip step with already defined position:
                continue;
            }
            ...
            // Set next column:
            $unlinked_col++;
            if ($unlinked_col > 5) {	// Switch to next row:
                $unlinked_col = 1;
                $unlinked_row++;
            }
        }

        foreach ($this->diagram_steps as $step_ID => $step) {	// Convert row and column to CSS coordinates:
     ...
     */
    public function set_diagram_default_step_position($parent_step_ID)
    {
        if (empty($this->diagram_steps[$parent_step_ID]['next_steps'])) {	// No next steps, Finish branch:
            return;
        }
        ...
        foreach ($this->diagram_steps[$parent_step_ID]['next_steps'] as $next_result => $next_step_ID) {
            if (isset($this->diagram_steps[$next_step_ID]) &&
                ...
                ! in_array($next_step_ID, $new_next_step_IDs)) {	// Exclude next steps with already defined position:
        ...
        if ($new_next_steps_count == 0) {	// No next steps without defined position, Finish branch:
            return;
        }
        ...
        if ($new_next_steps_count == 3 && $parent_col == 5) {	// If parent step is located in last column we should start first next step in shifted to 2 columns to the left:
            $current_col = $parent_col - 2;
        } elseif ($new_next_steps_count == 1) {	// Use same column as previous step box for single new next step
            $current_col = $parent_col;
        } else {	// Shift to the left for 2 or 3 new next steps:
            $current_col = $parent_col - 1;
        }

        foreach ($new_next_step_IDs as $next_result => $next_step_ID) {
            if ($current_col < 1) {	// Min column is 1:
                $current_col = 1;
            } elseif ($current_col > 5) {	// Max column is 5:
                $current_col = 5;
            }
            ...
            if ($new_next_steps_count == 1) {	// Don't calculate column for next steps because of single next step:
                break;
            }
        ...
        foreach ($new_next_step_IDs as $next_result => $next_step_ID) {	// Set positions to next step recursively:
     ...
     */
    public function resolve_diagram_step_position_conflicts($step_ID, $col, $row)
    {
        if (! isset($this->diagram_cells)) {	// Initialize array to store the filled cells on diagram:
            $this->diagram_cells = [];
        }

        if (isset($this->diagram_cells[$col . ':' . $row])) {	// If current cell is filled with another step box we should resolve this conflict:
            $conflict_is_resolved = false;

            while (! $conflict_is_resolved) {
                // LEFT shifting:
                if ($col > 1) {	// Find a free cell in the left columns of the same row:
                    for ($c = $col - 1; $c >= 1; $c--) {
                        if (! isset($this->diagram_cells[$c . ':' . $row])) {	// This is a free cell:
                            for ($c2 = $c; $c2 <= $col - 1; $c2++) {	// Move all step boxes before current to the left:
                                $shifted_step_ID = $this->diagram_cells[($c2 + 1) . ':' . $row];
                                $this->diagram_steps[$shifted_step_ID]['xy'] = [$c2, $row];
                ...
                // RIGHT shifting:
                if (! $conflict_is_resolved && $col < 5) {	// Find a free cell in the right columns of the same row:
                    for ($c = $col + 1; $c <= 5; $c++) {
                        if (! isset($this->diagram_cells[$c . ':' . $row])) {	// This is a free cell, Use it for current step:
                            $col = $c;
                            // A free left cell is found, Stop to find others:
                            $conflict_is_resolved = true;
                            break;
                ...
                // DOWN shifting:
                if (! $conflict_is_resolved) {	// Find a free cell in the down rows of the same column:
                    $row++;
                    if (! isset($this->diagram_cells[$col . ':' . $row])) {	// This is a free cell, Use it for current step:
                        // A free left cell is found, Stop to find others:
                        $conflict_is_resolved = true;
                    }
                }
        ...
        $this->diagram_steps[$step_ID]['xy'] = [$col, $row];
     ...
     */
    public function &get_first_AutomationStep()
    {
        if (! isset($this->first_AutomationStep)) {	// Load
            global $DB;
            $first_step_SQL = new SQL('Get first step of automation #' . $this->ID);
            $first_step_SQL->SELECT('step_ID');
            $first_step_SQL->FROM('T_automation__step');
     ...
     */
    public function execute_first_step($user_ID)
    {
        if (! ($first_AutomationStep = &$this->get_first_AutomationStep())) {	// No step:
            return false;
        }

        if ($this->get('status') != 'active') {	// Don't execute a paused automation:
            return false;
        }
