...
function autm_display_breadcrumb()
{
    ...
    echo '<li class="breadcrumb-item' . (isset($edited_Automation) || isset($edited_AutomationStep) ? '' : ' active') . '">'
        ...
        . '</li>';
    if (isset($edited_AutomationStep)) {	// Automation step:
    ...
    } elseif (isset($edited_Automation)) {	// Automation:
...
{
    if (check_user_perm('emails', 'edit')) {	// Make icon to action link if current User has a perm to edit this:
        global $admin_url;
        $r = '';
        ...
        return substr($r, 0, -2);
    } else {	// Newsletter names without link:
...
function autm_td_users_actions($autm_ID, $user_ID, $user_login, $step_ID, $step_order = null)
{
    ...
    if ($step_ID > 0) {	// Only for active step(excluding finished step):
    ...
    $r .= action_icon(TB_('Remove this user from automation'), 'remove', $step_action_url . 'remove_user', '', 0, 0, [
        'onclick' => 'return confirm(\'' . TS_('Are you sure want to remove this user from automation?') . '\');',
    ]);
...
function autm_td_status($autm_ID, $autm_status, $url_params = '')
{
    ...
    if (check_user_perm('options', 'edit')) {	// Display action icon to toggle automation status:
        $r .= ' ' . action_icon(
            '',
            ($autm_status == 'active' ? 'pause' : 'play'),
...
{
    if ($next_step_ID === null) {	// If next step is not used:
        return '';
    }

    if (empty($next_step_ID)) {	// Next ordered step:
        return '<span class="green">' . TB_('Continue') . ' (' . seconds_to_period($next_step_delay) . ')</span>';
    } elseif ($next_step_ID == '-1') {	// Stop workflow:
        return '<span class="red">' . TB_('STOP') . '</span>';
    } elseif ($next_step_ID == $step_ID) {	// Loop:
...
function step_td_actions($step_ID, $is_first_step, $is_last_step)
{
    ...
    if ($is_first_step) {	// First step cannot be moved up, print out blank icon:
        $r .= get_icon('move_up', 'noimg');
    } else {	// Display action icon to move step up:
    ...
    if ($is_last_step) {	// Last step cannot be moved down, print out blank icon:
        $r .= get_icon('move_down', 'noimg');
    } else {	// Display action icon to move step down:
...
{
    if ($step_ID === null) {	// If all steps for automation were completed for user:
        return TB_('Finished');
    }
 ...
 * @param array Params
 */
function automation_results_block($params = [])
{
    global $admin_url, $DB;
    ...
    if ($params['enlt_ID'] > 0) {	// Restrict by newsletter:
    ...
    if ($params['display_create_button'] && check_user_perm('options', 'edit')) {	// User must has a permission to add new automation:
        ...
        $Results->global_icon(TB_('New automation'), 'new', $admin_url . '?ctrl=automations&amp;action=new' . (isset($params['enlt_ID']) ? '&amp;enlt_ID=' . $params['enlt_ID'] : ''), TB_('New automation') . ' &raquo;', 3, 4, [
    ...
    if (check_user_perm('options', 'edit')) {	// Display actions column only if current user has a permission to edit options:
        $Results->cols[] = [
            'th' => TB_('Actions'),
