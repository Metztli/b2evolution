...
if ($edited_Automation->get('status') == 'active') {	// Set status text and button of the Automation for title:
    $automation_status_title = ' <span class="red">(' . T_('RUNNING') . ')</span>';
    $Results->global_icon(T_('Pause'), 'pause', regenerate_url('action', 'action=status_paused&amp;' . url_crumb('automation')), T_('Pause'), 3, 4, [
        'class' => 'action_icon btn-danger',
    ]);
} else {	// Set status text and button of the Automation for title:
    $automation_status_title = ' <span class="orange">(' . T_('PAUSED') . ')</span>';
    $Results->global_icon(T_('Play'), 'play', regenerate_url('action', 'action=status_active&amp;' . url_crumb('automation')), T_('Play'), 3, 4, [
        'class' => 'action_icon btn-success',
    ]);
}

$Results->global_icon(T_('New step'), 'new', regenerate_url('action', 'action=new_step'), T_('New step') . ' &raquo;', 3, 4, [
