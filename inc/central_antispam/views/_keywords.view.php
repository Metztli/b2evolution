...
$Results->global_icon(T_('Add keyword'), 'new', regenerate_url('action', 'action=keyword_new'), T_('Add keyword'), 3, 4, [
    'class' => 'action_icon btn-primary',
]);
$Results->global_icon(T_('Import from local antispam list...'), 'new', regenerate_url('action', 'action=import'), T_('Import from local antispam list...'), 3, 4, [
...
function filter_central_antispam(&$Form)
{
    ...
    $Form->select_input_array('status', get_param('status'), [
        '' => T_('All'),
    ] + ca_get_keyword_statuses(), T_('Status'));
}
$Results->filter_area = [
...
$Results->cols[] = [
    ...
    'extra' => [
        'style' => 'background-color: %ca_get_keyword_status_color( "#cakw_status#" )%;color:#FFF',
...
if (check_user_perm('centralantispam', 'edit')) {	// Check permission to edit central antispam keyword:
