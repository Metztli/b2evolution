...
$ca_keywords_count = $DB->get_var(
    'SELECT cakw_ID FROM T_centralantispam__keyword LIMIT 1',
    0,
    null,
    'Check if at least one keyword exists in Central Antispam'
);
...
if (count($keywords) > 0) {	// Suggest to import keywords only if at least one is found:
    $default_date_start = [
    ...
    foreach ($keywords as $keywords_status => $keywords_count) {
        ...
        $keywords_date_start = $Form->date_input('date_start_' . $keywords_status, $default_date_start[$keywords_status], '')
            . ' ' . $Form->time_input('time_start_' . $keywords_status, $default_date_start[$keywords_status], '', [
                'note' => '',
            ]);
        $Form->output = true;
        ...
        $Form->checkbox_input('import_keywords[]', ($keywords_status != 'local'), '', [
            'value' => $keywords_status,
        ]);
...
if (count($keywords)) {
    $Form->end_form([['submit', 'actionArray[import]', T_('Confirm Import'), 'SaveButton']]);
} else {
    $Form->end_form();
}
