...
function &get_CaKeywordCache($none_name = null)
{
    ...
    if (! isset($CaKeywordCache)) {	// Cache doesn't exist yet:
...
function &get_CaSourceCache($none_name = null)
{
    ...
    if (! isset($CaSourceCache)) {	// Cache doesn't exist yet:
...
class central_antispam_Module extends Module
{
    ...
    public function build_menu_3()
    {
        ...
        if (! check_user_perm('centralantispam', 'view')) {	// Don't display menu if current user has no access to central antispam:
            return;
        }
     ...
     */
    public function check_centralantispam_group_perm($permlevel, $permvalue, $permtarget)
    {
    ...
    public function upgrade_b2evo_tables()
    {
        ...
        if (! in_array($tableprefix . 'centralantispam__keyword', $existing_tables)) {	// Create a table only if it doesn't exist yet:
        ...
        if (! in_array($tableprefix . 'centralantispam__source', $existing_tables)) {	// Create a table only if it doesn't exist yet:
        ...
        if (! in_array($tableprefix . 'centralantispam__report', $existing_tables)) {	// Create a table only if it doesn't exist yet:
        ...
        if ($old_db_version < 12066) {	// part of 6.8.0-alpha
