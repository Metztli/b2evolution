...
switch ($tab) {
    case 'keywords':
        if (param('cakw_ID', 'integer', '', true)) {	// Load keyword from cache:
    ...
    case 'reporters':
        if (param('casrc_ID', 'integer', '', true)) {	// Load source from cache:
...
switch ($action) {
    case 'keyword_new':
        ...
        if (! isset($edited_CaKeyword)) { // We don't have a model to use, start with blank object:
    ...
    case 'keyword_create':
        ...
        if ($edited_CaKeyword->load_from_Request()) {	// We could load data from form without errors:
            $DB->begin();

            $duplicated_keyword_ID = $edited_CaKeyword->dbexists('cakw_keyword', $edited_CaKeyword->keyword);
            if ($duplicated_keyword_ID && $duplicated_keyword_ID != $edited_CaKeyword->ID) { // We have a duplicate entry:
                param_error(
                    'cakw_keyword',
                    sprintf(
                        TB_('This keyword already exists. Do you want to <a %s>edit the existing keyword</a>?'),
                ...
                if (empty($source_ID)) {	// Create new reporter if it doesn't exist in DB yet:
                ...
                } elseif ($source_row['casrc_status'] != 'trusted') {	// Make current baseurl as trusted source:
    ...
    case 'source_save':
        ...
        // Load data from request:
        if ($edited_CaSource->load_from_Request()) {	// We could load data from form without errors:
    ...
    case 'import':
        if (! param('confirm', 'integer', 0)) {	// The import action must be confirmed:
            break;
        }
        ...
        if (empty($import_keywords)) {	// No selected keywords to import:
        ...
        if (param_errors_detected()) {	// Don't allow to import with wrong entered data:
            break;
        }
        ...
        if (count($new_keywords)) {	// If there are new keywords to import:
            // Check if the Reporter/Source already exists in DB
            $source_SQL = new SQL('Get source of current baseurl for central antispam');
            $source_SQL->SELECT('casrc_ID, casrc_status');
            $source_SQL->FROM('T_centralantispam__source');
            ...
            foreach ($new_keywords as $keyword) {
                ...
                if ($keyword['askw_source'] == 'central' && $keyword_timestamp > '2014-02-24 21:10:18') {	// Limit central keywords by this max date:
                ...
                if ($query_result) {	// If new keyword has been inserted:
            ...
            if (count($keywords_imported_count)) {	// Insert reports to know from what host new keyword was added:
...
switch ($tab) {
    case 'keywords':
        ...
        if (empty($action) && check_user_perm('centralantispam', 'edit')) {	// Load JS to edit keyword status from list:
    ...
    case 'reporters':
        ...
        if (empty($action) && check_user_perm('centralantispam', 'edit')) {	// Load JS to edit source status from list:
            require_js_defer('#jquery#', 'rsc_url');
            require_js_defer('customized:jquery/jeditable/jquery.jeditable.js', 'rsc_url');
        }
        break;
