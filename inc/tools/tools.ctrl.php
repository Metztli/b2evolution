...
if (check_user_perm('options', 'edit') &&
    ...
    system_check_charset_update()) { // DB charset is required to update
...
if ($tab3 == 'import') {	// Check permission for import pages:
...
if (! empty($tab)) {	// We have requested a tab which is handled by a plugin:
    if (preg_match('~^plug_ID_(\d+)$~', $tab, $match)) { // Instanciate the invoked plugin:
        $tab_plugin_ID = $match[1];
        $tab_Plugin = &$Plugins->get_by_ID($match[1]);
        if (! $tab_Plugin) { // Plugin does not exist
...
if (empty($tab)) {	// "Main tab" actions:
    ...
    switch ($action) {
        ...
        case 'create_sample_collections':
            ...
            if (empty($perm_management)) {	// At least one option must be selected:
            ...
            if (empty($allow_access)) {	// At least one option must be selected:
                $Messages->add(sprintf(TB_('Please selected at least one option of the setting "%s".'), TB_('Allow access to')), 'error');
            }

            if (param_errors_detected()) {	// If some param errors then stop a creating and display a form to correct:
        ...
        case 'create_sample_comments':
            ...
            if (! (param_check_number('blog_ID', TB_('Blog ID must be a number'), true) &&
                ...
                param_check_number('num_posts', TB_('"How many posts" field must be a number'), true))) { // param errors
        ...
        case 'create_sample_revisions':
            ...
            if (! (param_check_number('blog_ID', TB_('Blog ID must be a number'), true) &&
                    ...
                    param_check_number('max_revisions', TB_('Maximum number of revisions filed must be a number'), true))) { // param errors
        ...
        case 'create_sample_users':
            ...
            if (empty($user_groups)) {	// At least one option must be selected:
            ...
            if (empty($advanced_user_perms)) {	// At least one option must be selected:
        ...
        case 'create_sample_basedomains':
            ...
            if (! param_check_number('num_basedomains', TB_('"How many base domains" field must be a number'), true)) {	// Stop action because of param error:
        ...
        case 'create_sample_messages':
            ...
            if (! (param_check_number('num_loops', TB_('"How many loops" field must be a number'), true) &&
                ...
                param_check_number('max_users', TB_('"Max # of participants in a conversation" field must be a number'), true))) { // param errors
        ...
        case 'create_sample_campaigns':
            ...
            if (empty($campaign_lists)) {	// At least one option must be selected:
