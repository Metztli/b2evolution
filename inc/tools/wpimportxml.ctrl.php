...
if (param('wp_blog_ID', 'integer', 0, true) > 0) {	// Save last import collection in Session:
...
switch ($action) {
    ...
    case 'import':
        ...
        if (! $WordpressImport->load_from_Request()) {	// Don't import if errors have been detected:
            $action = ($action == 'confirm' ? 'file' : 'confirm');
        }

        if (in_array($action, ['confirm', 'delete_extract', 'use_existing_folder'])) {	// Don't log into file for the confirm screen before start importing:
