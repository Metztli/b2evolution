...
if (! empty($template_action)) { // Execute action inside template to display a process in real rime
...
if (check_user_perm('users', 'edit') && empty($action)) { // Setting to lock system
    global $Settings;
    ...
    $Form->checkbox_input('system_lock', $Settings->get('system_lock'), T_('Lock system'), [
    ...
    if (check_user_perm('options', 'edit')) {
        $Form->buttons([['submit', 'submit', T_('Save Changes!'), 'SaveButton']]);
    }
...
if (! empty($action) && in_array($action, ['show_delete_item_versions', 'utf8check'])) { // these actions do not require a back to tools menu button
