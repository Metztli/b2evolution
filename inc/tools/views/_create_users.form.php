...
$Form->text_input('num_users', (is_null($num_users) ? 1000 : $num_users), 11, TB_('How many users'), '', [
    'maxlength' => 10,
    'required' => true,
]);
...
foreach ($GroupCache->cache as $Group) {
    $groups_options[] = ['user_groups[]', $Group->ID, $Group->name, is_null($user_groups) || in_array($Group->ID, $user_groups), 0,
...
$Form->checklist(
    $groups_options,
    'user_groups',
    TB_('Create new users in'),
    'mark_only',
    false,
    [
        'note' => TB_('Note: For each user it creates, the tool will randomly select between the allowed (checked) options above'),
    ]
);

$advanced_user_perms_array = [
...
$advanced_user_perms_options = [];
foreach ($advanced_user_perms_array as $advanced_perm_key => $advanced_perm_title) {
    $advanced_user_perms_options[] = ['advanced_user_perms[]', $advanced_perm_key, $advanced_perm_title, is_null($advanced_user_perms) || in_array($advanced_perm_key, $advanced_user_perms)];
}
$Form->checklist(
    $advanced_user_perms_options,
    'advanced_user_perms',
    TB_('Advanced user perms to grant on existing collections with advanced perms'),
    'mark_only',
    false,
    [
        'note' => TB_('Note: For each new user/existing collection combination, the tool will randomly select between the allowed (checked) options above'),
    ]
);
