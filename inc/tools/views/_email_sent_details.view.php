...
$result = emlog_result_info($edited_EmailLog->result, [], $edited_EmailLog->last_open_ts, $edited_EmailLog->last_click_ts);
$result .= ' <a href="' . url_add_param($admin_url, [
    'ctrl' => 'email',
    'tab' => 'return',
    'email' => $edited_EmailLog->to,
]) . '" class="' . button_class() . ' middle" title="' . format_to_output(T_('Go to return log'), 'htmlattr') . '">'
        . get_icon('magnifier', 'imgtag', [
            'title' => T_('Go to return log'),
        ]) . ' ' . T_('Returns') . '</a>';
...
$to_address .= ' <a href="' . url_add_param($admin_url, [
    'ctrl' => 'email',
    'tab' => 'sent',
    'email' => $edited_EmailLog->to,
]) . '" class="' . button_class() . ' middle" title="' . format_to_output(T_('Go to return log'), 'htmlattr') . '">'
        . get_icon('magnifier', 'imgtag', [
            'title' => T_('Go to send log'),
        ]) . ' ' . T_('Send Log') . '</a>';
...
if (! empty($mail_contents)) {
    if (! empty($mail_contents['text'])) { // Display Plain Text content
    ...
    if (! empty($mail_contents['html'])) { // Display HTML content
        ...
        if (! empty($mail_contents['html']['head_style'])) { // Print out all styles of email message
