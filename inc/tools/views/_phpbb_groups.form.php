...
$path_avatars_note = '';
if (! empty($path_avatars) && ! file_exists($path_avatars)) {	// Path avatars is incorrect
...
if ($phpbb_version == 3) {	// Only for phpBB3:
    $path_attachments = phpbb_get_var('path_attachments');
    $path_attachments_note = '';
    if (! empty($path_attachments) && ! file_exists($path_attachments)) {	// Path attachments is incorrect
...
$Form->select_input_array('phpbb_group_default', phpbb_get_var('group_default'), $b2evo_groups_default, TB_('Default group'), TB_('Use this group as the default for users without a defined rank') . ' (' . phpbb_rank_info('') . ')', [
    'force_keys_as_values' => true,
]);

$Form->select_input_array('phpbb_group_invalid', phpbb_get_var('group_invalid'), $b2evo_groups, '<span class="red">' . TB_('Invalid users') . '</span>', TB_('Use this group as the default for users which were deleted from the DB'), [
    'force_keys_as_values' => true,
]);
...
foreach ($phpbb_ranks as $rank_ID => $rank_name) {
    ...
    if ($rank_users_count == 0) {	// Don't display ranks without users
        continue;
    }
    $rank_value = isset($rank_values[$rank_ID]) ? $rank_values[$rank_ID] : phpbb_get_var('all_group_default');
    $Form->select_input_array('phpbb_ranks[' . $rank_ID . ']', $rank_value, $b2evo_groups, $rank_name, phpbb_rank_info($rank_ID), [
        'force_keys_as_values' => true,
    ]);
}
