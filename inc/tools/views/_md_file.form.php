...
if (! empty($import_files)) {
    ...
    $Form->select_input_object('md_blog_ID', $Session->get('last_import_coll_ID'), $BlogCache, TB_('Destination collection'), [
    ...
    foreach ($MarkdownImport->options_defs['import_type']['options'] as $option_value => $option) {
        $Form->radio_input('import_type', $MarkdownImport->get_option('import_type'), [
            [
                'value' => $option_value,
                'label' => $option['title'],
        ...
        ], ($i == 0 ? $MarkdownImport->options_defs['import_type']['title'] : ''), [
            'lines' => true,
        ]);
        foreach ($MarkdownImport->options_defs as $sub_option_key => $sub_option) {
            if ($sub_option['group'] == 'import_type' && $sub_option['subgroup'] == $option_value) {
                ...
                $Form->checkbox_input($sub_option_key, $MarkdownImport->get_option($sub_option_key), '', [
    ...
    foreach ($MarkdownImport->options_defs as $option_key => $option) {
        ...
        $checklist_options[] = [$option_key, '1', $option['title'], $MarkdownImport->get_option($option_key), (isset($option['disabled']) ? $option['disabled'] : null), (isset($option['note']) ? $option['note'] : null), null, null, $option_attrs];
    }
    ...
    // Radio options:
    foreach ($MarkdownImport->options_defs as $option_key => $option) {
        if ($option['group'] == 'radio') {	// Display here only radio options:
            $radio_options = $option['options'];
            foreach ($radio_options as $radio_option_value => $radio_option_data) {
                $radio_options[$radio_option_value] = [
                    'value' => $radio_option_value,
                ] + $radio_option_data;
            }
            $Form->radio_input($option_key, $MarkdownImport->get_option($option_key), $radio_options, $option['title'], [
                'lines' => true,
            ]);
        }
    }
