...
if ($WordpressImport->info_data['errors'] === false) {	// Display found Item Types as selector:
    wpxml_item_types_selector($WordpressImport->info_data['XML_file_path'], $WordpressImport->info_data['ZIP_folder_path']);
    $form_buttons[] = ['submit', 'actionArray[import]', TB_('Confirm import'), 'SaveButton'];
} elseif ($WordpressImport->info_data['error_type'] == 'folder_exists') {	// Display buttons to delete or use already existing folder:
    $form_buttons[] = ['submit', 'actionArray[delete_extract]', TB_('Delete and extract again'), 'DefaultButton'];
    $form_buttons[] = ['submit', 'actionArray[use_existing_folder]', TB_('Continue with existing folder'), 'SaveButton'];
}
