...
$Form->text_input('num_campaigns', (is_null($num_campaigns) ? 1000 : $num_campaigns), 11, TB_('How many email campaigns'), '', [
    'maxlength' => 10,
    'required' => true,
]);
...
foreach ($NewsletterCache->cache as $Newsletter) {
    $list_options[] = ['campaign_lists[]', $Newsletter->ID, $Newsletter->name, is_null($campaign_lists) || in_array($Newsletter->ID, $campaign_lists), 0];
}
$Form->checklist(
    $list_options,
    'campaign_lists',
    TB_('Create new email campaigns in'),
    'mark_only',
    false,
    [
        'note' => TB_('Note: For each campaign it creates, the tool will randomly select between the allowed (checked) options above'),
    ]
);
