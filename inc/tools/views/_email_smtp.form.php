...
if ($email_send_allow_php_mail) {	// Don't display the settings when only php mail service is disabled by config:
    $Form->begin_fieldset(TB_('Email service settings') . get_manual_link('email-service-settings'));

    $Form->radio('email_service', $Settings->get('email_service'), [
        ['mail', TB_('Regular PHP <code>mail()</code> function'), TB_('Default but not recommended')],
        ['smtp', TB_('External SMTP Server defined below'), TB_('Highly recommended if you send email campaigns')],
...
$Form->text_input('smtp_server_host', $Settings->get('smtp_server_host'), 25, TB_('SMTP Server'), TB_('Hostname or IP address of your SMTP server.'), [
    'maxlength' => 255,
]);

$smtp_server_security = $Settings->get('smtp_server_security');
$Form->radio('smtp_server_security', $smtp_server_security, [
    ['none', TB_('None')],
    ['ssl', TB_('SSL')],
    ['tls', TB_('TLS')],
], TB_('Encryption Method'));

$smtp_server_novalidatecert_params = [
    'lines' => true,
];
if (empty($smtp_server_security) || $smtp_server_security == 'none') {
    $smtp_server_novalidatecert_params['disabled'] = 'disabled';
}
$Form->radio_input('smtp_server_novalidatecert', $Settings->get('smtp_server_novalidatecert'), [
    [
        'value' => 1,
...
$Form->text_input('smtp_server_port', $Settings->get('smtp_server_port'), 5, TB_('Port Number'), TB_('Port number of your SMTP server (Defaults: SSL: 443, TLS: 587).'), [
    'maxlength' => 6,
]);

$Form->text_input(
    'smtp_server_username',
    $Settings->get('smtp_server_username'),
    25,
    TB_('SMTP Username'),
    TB_('User name for authenticating on your SMTP server.'),
    [
        'maxlength' => 255,
        'autocomplete' => 'off',
    ]
...
if (check_user_perm('emails', 'edit')) {
    ...
    $Form->password_input(
        ...
        $Settings->get('smtp_server_password'),
        25,
        TB_('SMTP Password'),
        [
            'maxlength' => 255,
            'note' => TB_('Password for authenticating on your SMTP server.'),
            'autocomplete' => 'off',
        ]
    );
}
...
if ($email_send_allow_php_mail) {	// Don't display the settings when only php mail service is disabled by config:
    ...
    $Form->radio('sendmail_params', $Settings->get('sendmail_params'), [
        ['return', '<code>-r $return-address$</code>'],
        ['from', '<code>-f $return-address$</code>'],
        ['custom', TB_('Custom') . ':', '',
