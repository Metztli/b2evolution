...
if ($statuses === null && param('all_statuses', 'integer', 0, true)) {	// Filter to get email addresses with all statuses:
    $statuses = array_keys(emadr_get_status_titles());
}
if ($statuses === null) {	// Default filter:
    $statuses = ['redemption', 'warning', 'suspicious3'];
}
...
if (! empty($statuses)) {	// Filter by statuses
...
if (check_user_perm('emails', 'edit')) {	// Check permission to edit emails:
    $Results->global_icon(T_('Create a new email address...'), 'new', $admin_url . '?ctrl=email&amp;action=blocked_new', T_('Add an email address') . ' &raquo;', 3, 4, [
...
function filter_email_blocked(&$Form)
{
    ...
    foreach ($statuses as $status_value => $status_title) {	// Display the checkboxes to filter by status
        $Form->checkbox('statuses[]', in_array($status_value, get_param('statuses')), '<span class="label" style="background-color:' . emadr_get_status_color($status_value) . (in_array($status_value, ['redemption', 'warning', 'suspicious1']) ? ';color:#333' : '') . '">' . $status_title . '</span>', '', '', $status_value);
    }
}
$Results->filter_area = [
...
$Results->cols[] = [
    ...
    'extra' => [
        'style' => 'background-color: %emadr_get_status_color( "#emadr_status#" )%;',
