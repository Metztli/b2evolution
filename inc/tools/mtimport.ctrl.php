...
if (param('default_blog', 'integer', 0) > 0) {	// Save last import collection in Session:
...
if (! is_logged_in(false) || $current_User->grp_ID != 1) {	// login failed
...
/*** mode-tabs ***/ ?>
<ul class="tabs"><?php
    foreach (['easy', 'normal', 'expert'] as $tab) {
...
<?php
    ...
    if (empty($exportedfile)) {
        ...
        if (empty($exportedfile)) { // no valid MTEXPORT defined
            chooseexportfile();
...
if ($default_userlevel > 10) {
    $default_userlevel = 10;
}
...
// global arrays
$categories_countprim = [];  // counts posts in primary categories
...
if (empty($action)) {
		        ...
		        ?>
					...
					<div class="input">
		                ...
		                foreach ($evousers as $user) {
		                    ?><option value="<?php echo $user->user_ID ?>"<?php if (utf8_strtolower($author) == utf8_strtolower($user->user_login)) {
			...
			<div class="input"><select name="catmap_select[]">
				<?php
		            if ($import_mode == 'expert') {
		                ...
		                echo '<option value="#DEFAULTBLOG#">Create in default blog:</option>';
		            } ?>
    ...
    if ($import_mode != 'easy') { // we'll use 'default' when importing
...
elseif ($action == 'import') {
    ...
    ?>
		<div class="panelinfo">
		<h4>Importing from [<?php echo $exportedfile ?>]..<?php if ($simulate) {
		    echo ' (simulating)';
		} ?></h4>
    ...
    {{{ // map categories
        ...
        $i_cat = -1;
        // category mapping
        if (! isset($_POST['catmap_select'])) { // no category mapping
            foreach ($categories as $cat) {
                $catsmapped[$cat] = ['blogid', $default_blog, $cat];
            }
        } else {
            foreach ($_POST['catmap_select'] as $cat) {
                $i_cat++;
                if ($cat == '#IGNORE#') {
                    $catsmapped[$categories[$i_cat]] = ['ignore'];
                } elseif ($cat == '#DEFAULTSET#') {
                    if (! isset($default_post_category)) { // get the default category set
                        ...
                        if (isset($_POST['post_extracats'])) { // get extra cats
                            foreach ($_POST['post_extracats'] as $tcat) {
                                $default_post_extracats[] = (int) $tcat;
                            }
                        }
                    }
                    $catsmapped[$categories[$i_cat]] = ['defaultset'];
                } elseif (preg_match('/^\d+$/', $cat, $match)) { // we map to a b2evo cat
                    $catsmapped[$categories[$i_cat]] = ['catid', (int) $cat];
                } elseif ($cat == '#DEFAULTBLOG#'
                                || preg_match('/^#NEW#(\d+)$/', $cat, $match)) { // we want a new category
                    ...
                    $catsmapped[$categories[$i_cat]] = ['blogid', $blog_id, $_POST['catmap_name'][$i_cat]];
                } else {
        ...
        foreach ($catsmapped as $mtcat => $values) {
            if ($values[0] == 'blogid') {
                ...
                if (! $cat_ID) {
                    ...
                    $catsmapped[$mtcat] = ['catid', (int) $cat_ID]; // map to existing category
                }
            }
        }
    ...
    if ($import_mode != 'easy') {
        $default_renderers = [];
        if (! isset($_POST['renderers'])) { // all unchecked
            $default_renderers = [];
        } else {
        ...
        $renderer_params = isset($setting_Blog) ? [
            'Blog' => &$setting_Blog,
            'setting_name' => 'coll_apply_rendering',
        ] : [];
        $default_renderers = $Plugins->validate_renderer_list(['default'], $renderer_params);
        $autop = 1;
    }
    ...
    */
    ...
    if (! isset($_POST['user_select'])) {
        foreach ($authors as $author) {
            $usersmapped[$author] = ['createnew', $author];
        }
    } else {
        foreach ($_POST['user_select'] as $select) {
            ...
            if ($select == '#IGNORE#') {
                $usersmapped[$mtauthor] = ['ignore'];
            } elseif ($select == '#CREATENEW#') {
                $usersmapped[$mtauthor] = ['createnew', $_POST['user_name'][$i_user]];
            } elseif (preg_match('#\d+#', $select, $match)) {
                $usersmapped[$mtauthor] = ['b2evo', $select];
            } else {
    ...
    foreach ($posts as $post) {
        ...
        preg_match("|-----\nBODY:(.*)|s", $post, $body);
        $body = trim($body[1]);
        if (empty($extended)) { // no extended body, so we can use the excerpt
            if (empty($excerpt)) {
                $post_content = $body;
            } else {
                $post_content = $excerpt . "\n[teaserbreak]\n" . $body;
            }
        } else { // we'll use body and extended body
            if (! empty($excerpt)) {
                ...
                switch ($key) {
                    ...
                    case 'ALLOW COMMENTS':
                        ...
                        switch ($post_allow_comments) {
                            case 0: $comment_status = 'disabled';
                                break;
                            case 1: $comment_status = 'open';
                                break;
                            case 2: $comment_status = 'closed';
                                break;
                            default:
                    ...
                    case 'CONVERT BREAKS':
                        ...
                        } elseif ($value == 'textile_2' && array_search('b2DATxtl', $post_renderers) === false) { // add the textile 2 renderer to the post's renderers
                        ...
                        if ($autop == 'depends' && $post_convert_breaks && array_search('b2WPAutP', $post_renderers) === false) { // add the Auto-P renderer
        ...
        if (empty($post_categories)) { // no category metadata found!
            if ($catsmapped['[no category assigned]'][0] == 'defaultset') { // we must convert default set
                ...
                $post_categories['[no category assigned]'] = $catsmapped['[no category assigned]'];
            }
        }
        ...
        if ($post_ID = $DB->get_var("SELECT post_ID
            ...
            foreach ($post_categories as $catname => $checkcat) {
                ...
                switch ($checkcat[0]) {
                    ...
                    case 'ignore': // category is ignored
                        if ($i_cat == 0) { // main category ignored, don't import post
                            $dontimport = 1;
                            $message_ignored .= '<li>Main Category &quot;' . $catname . '&quot; ignored! - no import</li>';
                            break;
                        } else { // ignored category in extracats, remove it there
                    ...
                    case 'blogid': // category has to be created
                        ...
                        $catsmapped[$catname] = ['catid', $cat_id]; // use ID from now on.
                        $post_catids[] = $cat_id;
            ...
            if ($convert_html_tags) {
                ...
                $post_content = preg_replace(['~<(br)>~', '~<(hr\s?.*?)>~', '~<(img\s.*?)>~'], '<\\1 />', $post_content);
    ...
    ?>
		...
		<h4>All done.<?php if ($simulate) {
		    echo ' (simulated - no real import!)';
		} ?></h4>
		<ul>
...
function fieldset_cats()
{
    ...
    {	// callback to display sublist element
        ...
        if (get_allow_cross_posting() >= 1) { // We allow cross posting, display checkbox:
        ...
        if ($current_blog_ID == $blog) {
            if (($default_main_cat == 0) && ($action == 'post')) {	// Assign default cat for new post
    ...
    foreach ($cache_blogs as $i_blog) { // run recursively through the cats
        ...
        echo $ChapterCache->recurse($callbacks, $current_blog_ID, null, 0, 0, [
            'sorted' => true,
        ]);
    }
...
function cats_optionslist($forcat)
{
    ...
    if (! isset($cat_name_id_associations)) { // Create a map fro the category name-id associations to populate during optionlist initialization
        $cat_name_id_associations = [];
    }
    ...
    if (! isset($cache_optionslist)) {
        ...
        foreach ($cache_blogs as $i_blog) {
            ...
            $cache_optionslist .= $ChapterCache->recurse($callbacks, $i_blog->blog_ID, null, 0, 0, [
                'sorted' => true,
            ]);
        }
    }
...
function proces_cat_line($Chapter, $level)
{
    ...
    for ($i = 0; $i <= $level; $i++) { // Add one '-' for each level
        $r .= '-';
    }
...
function renderer_list(&$Blog)
{
    ...
    while ($loop_RendererPlugin = &$admin_Plugins->get_next()) { // Go through whole list of renders
        ...
        if ($apply_rendering == 'stealth'
            || $apply_rendering == 'never') {	// This is not an option.
            continue;
        } elseif ($loop_RendererPlugin->code == 'b2WPAutP') { // special Auto-P plugin
            ?>
			<fieldset>
