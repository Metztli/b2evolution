...
class WordpressImport extends AbstractImport
{
     ...
     */
    public function log($message, $type = null, $type_html_tag = 'p', $display_label = true)
    {
        parent::log($message, $type, $type_html_tag, $display_label);

        if ($this->errors_limit && $this->log_errors_num >= $this->errors_limit) {	// Stop import because of errors limit:
            global $DB;
    ...
    public function load_from_Request()
    {
        ...
        if ($stop_error_enabled) {	// Wrong stop errors number:
            if ($stop_error_num < 1) {
                param_error('stop_error_num', 'Stop errors number must be greater than 0.');
            } else {	// Set limit by errors:
        ...
        } elseif (! preg_match('/\.(xml|txt|zip)$/i', $xml_file)) {	// Extension is incorrect
     ...
     */
    public function display_info($allow_use_extracted_folder = false, $allow_delete_existing_folder = false)
    {
        $this->log('<p style="margin-bottom:0">');

        $wp_file = get_param('import_file');

        if (preg_match('/\.zip$/i', $wp_file)) {	// Inform about unzipping before start this in wpxml_get_import_data():
            $zip_folder_path = substr($wp_file, 0, -4);
            if (! $allow_use_extracted_folder ||
                $allow_delete_existing_folder ||
                ! file_exists($zip_folder_path) ||
                ! is_dir($zip_folder_path)) {	// If folder doesn't exist OR allowed to use existing folder OR allowed to delete existing folder:
                if ($allow_delete_existing_folder &&
                        file_exists($zip_folder_path) &&
                        is_dir($zip_folder_path)) {	// Delete already existing folder:
        ...
        if ($this->info_data['errors'] === false) {
            if (! empty($is_unzipping)) {	// Display end of unzip process:
                $this->log(T_('OK') . '<br />');
            }

            if (preg_match('/\.zip$/i', $wp_file)) {	// ZIP archive:
            ...
            switch (get_param('import_type')) {
                ...
                case 'delete':
                    ...
                    if (get_param('action') == 'confirm') {	// Display an input to confirm replace import mode:
            ...
            if ($this->info_data['error_type'] != 'folder_exists') {	// Don't inform about failed import for this error type because below we suggest to delete or use already existing folder:
    ...
    public function execute()
    {
        ...
        $this->imported_file_names = []; // Key - file name, Value - File ID or FALSE when same file name is used in different folders
        $this->imported_file_paths = []; // Key - relative file path, Value - File ID
        ...
        if ($import_type == 'delete') { // Remove data from selected blog
            ...
            if (! empty($old_categories)) {
                ...
                if (! empty($old_posts)) { // Remove the post's data from related tables
                    if ($delete_files) { // Get the file IDs that should be deleted from hard drive
            ...
            if ($delete_files) { // Delete the files
        ...
        if (isset($xml_data['authors']) && count($xml_data['authors']) > 0) {
            ...
            foreach ($xml_data['authors'] as $author) {
                ...
                if (empty($existing_users[$author_login])) {	// Insert new user into DB if User doesn't exist with current login name
                    $GroupCache = &get_GroupCache();
                    if (! empty($author['author_group'])) {	// Set user group from xml data
                        if (($UserGroup = &$GroupCache->get_by_name($author['author_group'], false)) === false) {	// If user's group doesn't exist yet, we should create new
                            $UserGroup = new Group();
                            $UserGroup->set('name', $author['author_group']);
                            $UserGroup->dbinsert();
                        }
                    } else {	// Set default user group is it is not defined in xml
                        if (($UserGroup = &$GroupCache->get_by_name('Normal Users', false)) === false) {	// Exit from import of users, because we cannot set default user group
                            break;
                        }
                    }

                    unset($author_created_from_country);
                    if (! empty($author['author_created_from_country'])) {	// Get country ID from DB by code
                    ...
                    if (isset($author_created_from_country)) {	// User was created from this country
                        $User->set('reg_ctry_ID', $author_created_from_country);
                    }
                    if (! empty($author_regions['country'])) {	// Country
                        $User->set('ctry_ID', $author_regions['country']);
                        if (! empty($author_regions['region'])) {	// Region
                            $User->set('rgn_ID', $author_regions['region']);
                            if (! empty($author_regions['subregion'])) {	// Subregion
                                $User->set('subrg_ID', $author_regions['subregion']);
                            }
                            if (! empty($author_regions['city'])) {	// City
                    ...
                    if (! $User->dbinsert()) {	// Error on insert new user:
                        $failed_users_num++;
                    ...
                    if (! empty($author['links'])) {	// Store user attachments in array to link them below after importing files:
                        $authors_links[$user_ID] = [];
                        foreach ($author['links'] as $link) {
                            if (isset($author['author_avatar_file_ID']) &&
                                    $link['link_file_ID'] == $author['author_avatar_file_ID']) {	// Mark this link as main avatar in order to update this with new inserted file ID below:
        ...
        if (isset($xml_data['files']) && count($xml_data['files']) > 0) {
            ...
            if (! $attached_files_path || ! file_exists($attached_files_path)) {	// Display an error if files are attached but folder doesn't exist:
            ...
            } else {	// Try to import files from the selected subfolder:
                $files_count = 0;
                ...
                foreach ($xml_data['files'] as $file) {
                    switch ($file['file_root_type']) {
                        ...
                        case 'user':
                            // User's files
                            if (isset($authors_IDs[$file['file_root_ID']])) { // If owner of this file exists in our DB
                    ...
                    if ($File = &$this->create_File($file_source_path, $file)) {	// Store the created File in array because it will be linked to the Items below:
                        $this->imported_files[$file['file_ID']] = $File;

                        if ($import_img) {	// Collect file name in array to link with post below:
                            $file_name = basename($file['file_path']);
                            if (isset($this->imported_file_names[$file_name])) {	// Don't use this file if more than one use same name, e.g. from different folders:
                                $this->imported_file_names[$file_name] = false;
                                $this->log_error(sprintf(
                                    'there are 2+ attachements with conflicting name %s.',
                                    '<code>' . $file_name . '</code>'
                                ));
                            } else {	// This is a first detected file with current name:
                        ...
                        if ($file['file_root_type'] == 'user' &&
                                ...
                                ($User = &$UserCache->get_by_ID($file['file_root_ID'], false, false))) {	// Link file to User:
                            $link = $authors_links[$file['file_root_ID']][$file['file_ID']];
                            $LinkOwner = new LinkUser($User);
                            if (! empty($link['is_main_avatar'])) {	// Update if current file is main avatar for the User:
                                $User->set('avatar_file_ID', $File->ID);
                                $User->dbupdate();
                            }
                            if ($File->link_to_Object($LinkOwner, $link['link_order'], $link['link_position'])) {	// If file has been linked to the post:
        ...
        if (isset($xml_data['categories']) && count($xml_data['categories']) > 0) {
            ...
            foreach ($xml_data['categories'] as $cat) {
                ...
                if (! empty($categories[(string) $cat['category_nicename']])) {
                    ...
                    if (! empty($cat['category_parent']) && isset($categories[(string) $cat['category_parent']])) {	// Set category parent ID
                    ...
                    if (empty($category_default)) {	// Set first category as default
        ...
        if (empty($category_default)) { // No categories in XML file, Try to use first category(from DB) as default
        ...
        if (empty($category_default)) { // If category is still not defined then we should create default, because blog must has at least one category
        ...
        if (isset($xml_data['tags']) && count($xml_data['tags']) > 0) {
            ...
            foreach ($xml_data['tags'] as $tag) {
                ...
                } else {	// Insert new tag into DB if tag doesn't exist with current name
        ...
        if (isset($xml_data['posts']) && count($xml_data['posts']) > 0) {
            ...
            $this->attachment_IDs = [];
            $attachments_count = 0;
            foreach ($xml_data['posts'] as $post) {	// Import ONLY attachment posts here, all other posts are imported below:
                if ($post['post_type'] != 'attachment') {	// Skip not attachment post:
                    continue;
                }

                $this->log('<p>' . sprintf('Importing attachment: %s', '#' . $post['post_id'] . ' - "' . $post['post_title'] . '"'));

                if (! empty($post['post_parent'])) {	// Store what post the File is linked to:
                    if (! isset($attached_post_files[$post['post_parent']])) {
                        $attached_post_files[$post['post_parent']] = [];
                    }
                    $attached_post_files[$post['post_parent']][] = $post['post_id'];
                }

                if (isset($post['postmeta'])) {	// Link the files to the Item from meta data:
                    $attch_imported_files = [];
                    foreach ($post['postmeta'] as $postmeta) {
                        if (! isset($postmeta['key']) || ! isset($postmeta['value'])) {	// Skip wrong meta data:
                            continue;
                        }
                        $attch_file_name = '';
                        $file_params = [
                        ...
                        if ($postmeta['key'] == '_wp_attached_file') {	// Get file name from the string meta data:
                            $attch_file_name = $postmeta['value'];
                        } elseif ($postmeta['key'] == '_wp_attachment_metadata') {	// Try to get file name from the serialized meta data:
                            $postmeta_value = @unserialize($postmeta['value']);
                            if (isset($postmeta_value['file'])) {	// Set file name:
                                $attch_file_name = $postmeta_value['file'];
                            }
                        }
                        if (empty($attch_file_name) || in_array($attch_file_name, $attch_imported_files)) {	// Skip empty file name or if it has been already imported:
                            continue;
                        }
                        ...
                        // Try to import file from source path:
                        if ($File = &$this->create_File($file_source_path, $file_params)) {	// Store the created File in array because it will be linked to the Items below:
            ...
            foreach ($xml_data['posts'] as $post) {
                if ($post['post_type'] == 'revision') {	// Ignore post with type "revision":
                    $this->log_warning(sprintf(
                        'Ignore post "%s" because of post type is %s',
                        '#' . $post['post_id'] . ' - ' . $post['post_title'],
                        '<code>' . $post['post_type'] . '</code>'
                    ));
                    continue;
                } elseif ($post['post_type'] == 'attachment') {	// Skip attachment post because it shoul be imported above:
                    continue;
                } elseif ($post['post_type'] == 'page' && ! isset($categories['standalone-pages'])) {	// Try to create special category "Standalone Pages" for pages only it doesn't exist:
                ...
                $post_extra_cat_IDs = [];
                $post_tags = [];
                if (! empty($post['terms'])) { // Set categories and tags
                    foreach ($post['terms'] as $term) {
                        switch ($term['domain']) {
                            case 'category':
                                if (isset($categories[(string) $term['slug']])) {
                                    if ($post_main_cat_ID == $category_default) { // Set main category
                ...
                if ($post['post_type'] == 'page') {	// Set static category "Standalone Pages" for pages because they have no categories in wordpress DB:
                    $post_main_cat_ID = $categories['standalone-pages'];
                    $post_extra_cat_IDs[] = $categories['standalone-pages'];
                }

                // Set Item Type ID:
                if (! empty($post['itemtype']) && isset($selected_item_type_names[$post['itemtype']])) {	// Try to use Item Type by name:
                    $post_type_ID = $selected_item_type_names[$post['itemtype']];
                    if ($post_type_ID == 0) {	// Skip Item because this was selected on the confirm form:
                        $this->log_warning(sprintf(
                            'Skip Item because Item Type %s is not selected for import.',
                            '<code>&lt;evo:itemtype&gt;</code> = <code>' . $post['itemtype'] . '</code>'
                        ));
                        continue;
                    }
                } elseif (! empty($post['post_type']) && isset($selected_item_type_usages[$post['post_type']])) {	// Try to use Item Type by usage:
                ...
                } else {	// Try to use Item Type without provided value sin XML:
                ...
                if (! ($ItemType = &$ItemTypeCache->get_by_ID($post_type_ID, false, false))) {	// Skip not found Item Type:
                    $this->log_error(sprintf(
                        'Skip Item because Item Type %s is not found.',
                ...
                if (! empty($post['custom_fields'])) {	// Import custom fields:
                    $item_custom_fields = $Item->get_custom_fields_defs();
                    foreach ($post['custom_fields'] as $custom_field_name => $custom_field) {
                        if (! isset($item_custom_fields[$custom_field_name])) {	// Skip unknown custom field:
                            $this->log_error(sprintf(
                                'Skip custom field %s because Item Type %s has no it.',
                                '<code>' . $custom_field_name . '</code>',
                                '#' . $ItemType->ID . ' "' . $ItemType->get('name') . '"'
                            ));
                            continue;
                        }
                        if ($item_custom_fields[$custom_field_name]['type'] != $custom_field['type']) {	// Skip wrong custom field type:
                            $this->log_error(sprintf(
                                'Cannot import custom field %s because it has type %s and we expect type %s',
                                '<code>' . $custom_field_name . '</code>',
                ...
                if (! empty($post['post_urltitle'])) {	// Use top priority urltitle if it is provided:
                    $post_urltitle = $post['post_urltitle'];
                } elseif (! empty($post['post_link']) && preg_match('#/([^/]+)/?$#', $post['post_link'], $post_link_match)) {	// Try to extract canonical slug from post URL:
                    $post_urltitle = $post_link_match[1];
                }

                if (! empty($post['extra_slugs'])) {	// Extra slugs:
                    foreach ($post['extra_slugs'] as $extra_slug) {
                        if (($checke_extra_slug = $this->check_slug($extra_slug)) !== false) {	// Import only not duplicated extra slug:
                            $post_urltitle .= ',' . $checke_extra_slug;
                            // Store new slug in cache:
                            $this->cache_slugs[] = $checke_extra_slug;
                        } else {	// Display warning for duplicated slug:
                ...
                $Item->set('comment_status', (in_array($post['comment_status'], ['open', 'closed', 'disabled']) ? $post['comment_status'] : 'open'));
                if (empty($post['post_excerpt'])) {	// If excerpt is not provided:
                    if (! empty($post_content)) {	// Generate excerpt from content:
                        $Item->set_param('excerpt', 'string', excerpt($post_content), true);
                        $Item->set('excerpt_autogenerated', 1);
                    }
                } else {	// Set excerpt from given value:
                ...
                if (! empty($item_regions['country'])) {	// Country
                    $Item->set('ctry_ID', $item_regions['country']);
                    if (! empty($item_regions['region'])) {	// Region
                        $Item->set('rgn_ID', $item_regions['region']);
                        if (! empty($item_regions['subregion'])) {	// Subregion
                            $Item->set('subrg_ID', $item_regions['subregion']);
                        }
                        if (! empty($item_regions['city'])) {	// City
                ...
                if (! empty($this->imported_files) && ! empty($post['links'])) {	// Link the files to the Item if it has them:
                    foreach ($post['links'] as $link) {
                        if (($File = &$this->get_File_by_ID($link['link_file_ID'])) &&
                            $File->link_to_Object($LinkOwner, $link['link_order'], $link['link_position'])) {	// If imported File has been linked to the post
                            $this->log_success(sprintf(
                                'File %s has been linked to this post as %s from %s.',
                ...
                $this->linked_post_files = [];
                if (isset($post['postmeta'])) {	// Extract additional data:
                    $cover_image_is_imported = false;
                    if ($use_yoast_cover) {	// Use Yoast opengraph or twitter image as Cover image if available:
                        ...
                        if (! $cover_image_is_imported) {	// (2) Twitter is 2nd priority if cover image was not imported from opengraph:
                            $cover_image_is_imported = $this->link_from_meta_data('_yoast_wpseo_twitter-image', 'cover', $link_order, $post['postmeta'], $LinkOwner);
                        }
                    }
                    foreach ($post['postmeta'] as $postmeta) {
                        if (! $cover_image_is_imported && $postmeta['key'] == '_thumbnail_id') {	// (3) Thumbnail ID is 3rd priority after Yoast Opengraph and Twitter:
                            $this->linked_post_files[] = $postmeta['value'];
                            if (($File = &$this->get_File_by_attachment_ID($postmeta['value'])) &&
                                    $File->link_to_Object($LinkOwner, $link_order, 'cover')) {	// If imported File has been linked to the post:
                                $this->log_success(sprintf(
                                    'File %s has been linked to this post as %s from %s.',
                                    '<code>' . $File->_adfp_full_path . '</code>',
                                    '<code>cover</code>',
                                ...
                                $this->log_warning(sprintf(
                                    'Cover file %s could not be attached to this post because it is not found in the source attachments by %s.',
                                    '#' . $postmeta['value'],
                                    '<code>&lt;wp:meta_key&gt;_thumbnail_id&lt;/wp:meta_key&gt;</code> = <code>' . $postmeta['value'] . '</code>'
                                ));
                            }
                        } elseif (strpos($postmeta['key'], 'wpcf-') === 0) {	// Custom field:
                            $custom_field_name = substr($postmeta['key'], 5);

                            if ($custom_field_name === '') {	// Empty custom field name:
                                $this->log_error(sprintf(
                                    'Skip wp custom field without name; value %s',
                                    isset($postmeta['value']) ? '= <code>' . $postmeta['value'] . '</code>' : 'is not defined'
                                ));
                                continue;
                            }

                            if (! isset($postmeta['value']) || $postmeta['value'] === null) {	// No provided value:
                                $this->log_error(sprintf(
                                    'Skip wp custom field %s without value!',
                                    '<code>' . $custom_field_name . '</code>'
                                ));
                                continue;
                            }
                            ...
                            if (! isset($item_custom_fields[$custom_field_name])) {	// Skip unknown custom field:
                                $this->log_warning(sprintf(
                                    'Custom field %s has been ignored because there is no %s custom field in Item Type %s.',
                            ...
                            if ((in_array($item_custom_fields[$custom_field_name]['type'], ['double', 'computed']) && ! empty($custom_field_value) && ! preg_match('/^(\+|-)?[0-9]+(\.[0-9]+)?$/', $custom_field_value)) ||
                                ($item_custom_fields[$custom_field_name]['type'] == 'url' && validate_url($custom_field_value, 'http-https') !== false) ||
                                ($item_custom_fields[$custom_field_name]['type'] == 'image' && ! empty($custom_field_value) && ! is_number($custom_field_value))) {	// Skip wrong custom field type format:
                                $this->log_error(sprintf(
                                    'Custom field %s type mismatch: value %s cannot be imported into %s',
                                    '<code>wpcf-' . $custom_field_name . '</code>',
                            ...
                            $this->log('<p>' . sprintf(
                                'Custom field %s imported into %s with value %s.',
                                '<code>wpcf-' . $custom_field_name . '</code>',
                                '<code>' . $custom_field_name . '</code>',
                                '<code>' . $custom_field_value . '</code>'
                            ) . '</p>');
                ...
                if (preg_match_outcode('#\[caption[^\]]+id="attachment_(\d+)"[^\]]+\].+?\[/caption\]#i', $updated_post_content, $caption_matches)) {	// If [caption ...] tag is detected
                    foreach ($caption_matches[1] as $caption_post_ID) {
                        ...
                        if (($File = &$this->get_File_by_attachment_ID($caption_post_ID)) &&
                            ($link_ID = $File->link_to_Object($LinkOwner, $link_order, 'inline'))) {	// If imported File has been linked to the post
                            $this->log_success(sprintf(
                                'File %s has been linked to this post from %s.',
                                '<code>' . $File->_adfp_full_path . '</code>',
                            ...
                            $this->log_warning(sprintf(
                                'Caption file %s could not be attached to this post because it is not found in the source attachments folder by %s.',
                                '#' . $caption_post_ID,
                                '<code>[caption id="attachment_' . $caption_post_ID . '"]</code>'
                            ));
                        }
                ...
                if ($import_img && count($this->imported_file_names)) {	// Only if it is requested and at least one attachment has been detected above:
                    if (preg_match_outcode('#<img[^>]+src="([^"]+)"[^>]*>#i', $updated_post_content, $img_matches)) {	// If <img /> tag is detected
                        foreach ($img_matches[1] as $i => $img_url) {	// Try to link File:
                            if ($link_ID = $this->link_by_URL($img_url, 'inline', $link_order, $LinkOwner)) {	// Replace this img tag from content with b2evolution format:
                ...
                if (isset($attached_post_files[$post['post_id']])) {	// Link all found attached files for the Item which were not linked yer above as cover or inline image tags:
                    foreach ($attached_post_files[$post['post_id']] as $attachment_post_ID) {
                        if (in_array($attachment_post_ID, $this->linked_post_files)) {	// Skip already linked File:
                            continue;
                        }
                        if (($File = &$this->get_File_by_attachment_ID($attachment_post_ID)) &&
                            $File->link_to_Object($LinkOwner, $link_order, 'aftermore')) {	// If imported File has been linked to the post:
                            $this->log_success(sprintf(
                                'File %s has been linked to this post as %s by %s.',
                                '<code>' . $File->_adfp_full_path . '</code>',
                                '<code>aftermore</code>',
                            ...
                            $this->log_warning(sprintf(
                                'File %s could not be attached to this post because it is not found in the source attachments by %s.',
                                '#' . $attachment_post_ID,
                ...
                if ($updated_post_content != $post_content) {	// Set new content:
                    $Item->set('content', $updated_post_content);
                    $item_content_was_changed = true;
                }

                if ($item_content_was_changed) {	// Update Item:
                    $Item->dbupdate();
                }
            ...
            foreach ($xml_data['posts'] as $post) {	// Set post parents
            ...
            if ($convert_links && ! empty($posts)) {	// Convert wp links like "?p=" or "?page_id=" to b2evo shortlinks:
                ...
                foreach ($posts as $wp_post_ID => $evo_item_ID) {
                    if (! ($Item = &$ItemCache->get_by_ID($evo_item_ID, false, false))) {	// Skip unknown Item:
                    ...
                    if (preg_match_outcode('#<a.+?href="[^"]*\?(page_id|p)=(\d+)".*?>(.+?)</a>#i', $item_content, $link_matches)) {	// If a link like <a href="?page_id=123"> is detected in item content:
                        $converted_links = [];
                        foreach ($link_matches[2] as $l => $link_wp_post_ID) {
                            if (! isset($posts[$link_wp_post_ID])) {	// Skip unknown wordpress post:
                            ...
                            if (! ($link_Item = &$ItemCache->get_by_ID($posts[$link_wp_post_ID], false, false))) {	// Skip not found Item in DB:
                        ...
                        if ($item_converted_links_num > 0) {	// If at least one links is converted:
                            $Item->set('content', $item_content);
                            if ($Item->dbupdate()) {	// Success updating:
        ...
        if (! empty($comments)) {
            ...
            foreach ($comments as $post_ID => $comments) {
                ...
                if (empty($comments)) {	// Skip if no comments
                ...
                foreach ($comments as $comment) {
                    ...
                    if (! empty($comment['comment_user_login']) && isset($existing_users[$comment['comment_user_login']])) {	// Get Author ID by login:
                        $comment_author_user_ID = $existing_users[$comment['comment_user_login']];
                    } elseif (! empty($comment['comment_user_id']) && isset($authors_IDs[(string) $comment['comment_user_id']])) {	// Use Author ID from xml data:
                        $comment_author_user_ID = $authors_IDs[(string) $comment['comment_user_id']];
                    }

                    $comment_parent_ID = 0;
                    if (! empty($comment['comment_parent']) && isset($comments_IDs[(string) $comment['comment_parent']])) {	// Parent comment ID
                        $comment_parent_ID = $comments_IDs[(string) $comment['comment_parent']];
                    }

                    unset($comment_IP_country);
                    if (! empty($comment['comment_IP_country'])) {	// Get country ID by code
                        $CountryCache = &get_CountryCache();
                    ...
                    if (empty($comment['comment_status'])) {	// If comment status is empty (the export of wordpress doesn't provide this field)
                        $Comment->set('status', $comment['comment_approved'] == '1' ? 'published' : 'draft');
                    } else {	// Set status when we have predefined value
                        $Comment->set('status', $comment['comment_status']);
                    }
                    if (! empty($comment_IP_country)) {	// Country
     ...
     */
    public function &create_File($file_source_path, $params)
    {
        global $DB;
        ...
        if (! empty($existing_file_ID) &&
                ...
                $File->exists()) {	// Use already exsiting File:
            $this->log_warning(sprintf(
                'Don\'t copy/import the file %s, use already existing File #%d in %s instead.',
                '<code>' . $file_source_path . '</code>',
                $File->ID,
                '<code>' . $File->get_full_path() . '</code>'
            ));
            return $File;
        }
        ...
        if (! $File || ! copy_r($file_source_path, $File->get_full_path())) {	// No permission to copy to the destination folder
            if (is_dir($file_source_path)) {	// Folder
    ...
    {
        if (isset($this->imported_files[$file_ID])) {	// If File was imported and found by ID:
     ...
     * @return object|false File
     */
    public function &get_File_by_attachment_ID($attachment_post_ID)
    {
        if (isset($this->attachment_IDs[$attachment_post_ID])) {	// If File was imported and found by attachment post ID:
     ...
     */
    public function link_from_meta_data($meta_key, $link_position, &$link_order, $meta_data, &$LinkOwner)
    {
        foreach ($meta_data as $meta) {
            if ($meta['key'] == $meta_key) {
                $meta_key_tag = '<code>&lt;wp:meta_key&gt;' . $meta_key . '&lt;/wp:meta_key&gt;</code>';
                if (empty($meta['value'])) {	// Skip empty value:
                ...
                return $this->link_by_URL($meta['value'], 'cover', $link_order, $LinkOwner, [
     ...
     * @return integer|boolean Link ID on success, FALSE otherwise
     */
    public function link_by_URL($file_url, $link_position, &$link_order, &$LinkOwner, $params = [])
    {
        $params = array_merge([
        ...
        if (! empty($this->info_data['attached_files_folder']) &&
                strpos($file_url, $this->info_data['attached_files_folder']) !== false) {	// Get relative path because image URL contains it:
            $img_file_rel_path = preg_replace('#^.+?' . preg_quote($this->info_data['attached_files_folder']) . '#', '', $file_url);
        }

        if ($img_file_rel_path !== null &&
                ! empty($this->imported_file_paths[$img_file_rel_path])) {	// We find file by relative path:
            $matched_file_ID = $this->imported_file_paths[$img_file_rel_path];
            $matched_file_place = 'path';
        } elseif (isset($this->imported_file_names[$img_file_name])) {	// We find file by name:
            $matched_file_ID = $this->imported_file_names[$img_file_name];
            $matched_file_place = 'file';
        }

        if ($matched_file_ID === false) {	// Skip a duplicated file by name:
            $this->log_error(sprintf(
                $params['msg_no_found'],
                '<code>' . $file_url . '</code>',
        ...
        if (! ($File = &$this->get_File_by_ID($matched_file_ID))) {	// If file could not be linked to the post:
            $this->log_warning(sprintf(
                $params['msg_no_match'],
                '<code>' . $file_url . '</code>',
                '<code>' . $img_file_name . '</code>',
        ...
        if ($linked_post_ID = array_search($File->ID, $this->attachment_IDs)) {	// Store what files were tried to be linked in order to don't link twice:
            $this->linked_post_files[] = $linked_post_ID;
        }

        // Try to link File to the Item/LinkOwner:
        if ($link_ID = $File->link_to_Object($LinkOwner, $link_order, $link_position)) {	// If file has been linked to the Item
            if ($matched_file_place == 'path') {	// Inform the file was matched by path:
                $this->log_success(sprintf(
                    $params['msg_linked_by_path'],
                    '<code>' . $File->_adfp_full_path . '</code>',
                    '<code>' . $link_position . '</code>',
                    '<code>' . $file_url . '</code>',
                    '<code>' . $img_file_rel_path . '</code>'
                ));
            } else {	// Inform the file was matched only by name:
                $this->log_success(sprintf(
                    $params['msg_linked_by_name'],
                    '<code>' . $File->_adfp_full_path . '</code>',
                    '<code>' . $link_position . '</code>',
                    '<code>' . $file_url . '</code>'
                ) . ' '
                    . $this->get_log(sprintf(
                        $params['msg_match_only_by_name'],
     ...
     */
    public function fix_author_login($author_login)
    {
     ...
     */
    public function get_user_ID_by_login($login, $default_user_ID = false)
    {
        if (! isset($this->users)) {	// Load all b2evo users from DB once in cache array:
            global $DB;
    ...
    {
        if (! isset($this->cache_slugs)) {	// Load slugs from DB once:
            global $DB;
        ...
        if (in_array($slug, $this->cache_slugs)) {	// The slug is already used:
            return false;
        }
