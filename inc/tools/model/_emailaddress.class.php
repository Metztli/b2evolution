...
class EmailAddress extends DataObject
{
    ...
    public $sent_last_returnerror = 0;

    public $prmerror_count = 0;

    public $tmperror_count = 0;

    public $spamerror_count = 0;

    public $othererror_count = 0;
    ...
    public function load_from_Request()
    {
        ...
        if ($is_email_correct && $existing_emadr_ID = $this->dbexists('emadr_address', get_param('emadr_address'))) {	// Check if a email address already exists with the same address
            global $admin_url;
            param_error('emadr_address', sprintf(
                T_('This email address already exists. Do you want to <a %s>edit the existing email address</a>?'),
     ...
     */
    public function increase_counter($counter_name)
    {
        $counter_name .= '_count';
        if (isset($this->$counter_name)) {
            $this->set($counter_name, $this->get($counter_name) + 1);
            if ($counter_name != 'sent_count') {	// Update last error date when we increase an error counter
    ...
    {
        if ($this->ID == 0) {	// New record is creating, we can assign any status without level restrictions
            $this->set('status', $new_status);
        } else {	// The record is updating, we should update only status which has hight level then old status
            ...
            if (in_array($this->get('status'), $available_statuses)) {	// Check if we can update this status
                $this->set('status', $new_status);
            }
        }
    }
