...
function tool_create_sample_collections($num_collections, $perm_management, $allow_access)
{
    ...
    for ($i = 0; $i < $num_collections; $i++) {
        ...
        if ($count % 100 == 0) {	// Display a process of creating by one dot for 100 comments:
            echo ' .';
            evo_flush();
        }
...
function tool_create_sample_comments($blog_ID, $num_comments, $num_posts)
{
    ...
    if ($selected_Blog == null) { // Incorrect blog ID, Exit here
        return;
    }
    ...
    foreach ($items_result as $row) {
        ...
        for ($i = 0; $i < $num_comments; $i++) {
            ...
            for ($j = 0; $j < 50; $j++) { // create 50 random word
                $length = rand(1, 15);
...
function tool_create_sample_posts($blog_ID, $num_posts)
{
    ...
    for ($i = 1; $i <= $num_posts; $i++) {
        ...
        $urltitle = strtolower(str_replace(["\n", ' ', '-'], '', substr($word, 50, 20)));
        ...
        if ($count % 100 == 0) { // Display a process of creating by one dot for 100 posts
            echo ' .';
    ...
    if ($num_posts > $num_posts_created) { // Some post creation failed because of concurtent modification error
...
function tool_create_sample_users($user_groups, $num_users, $advanced_user_perms)
{
    ...
    $array_intersect = array_intersect($advanced_user_perms, ['member', 'moderator', 'admin']);
    $assign_adv_user_perms = ! empty($array_intersect);

    if ($assign_adv_user_perms) { // Get all collections with advanced perms:
    ...
    for ($i = 1; $i <= $num_users; $i++) {
        ...
        while (user_exists($login)) { // Generate new unique login
            $login = generate_random_passwd();
        }
        ...
        if ($rand_Group = &$GroupCache->get_by_ID($user_groups[rand(0, $user_groups_max_index)], false, false)) {	// Set a random group from checked options if a group really exists in DB:
            $User->set_Group($rand_Group);
        }

        if ($User->dbinsert()) {	// If user has been created successfully
            $count++;

            if ($assign_adv_user_perms && ! empty($adv_perm_coll_IDs)) {	// Grant advanced user perms on existing collections with advanced perms:
                $adv_perm_coll_insert_values = [];
                foreach ($adv_perm_coll_IDs as $adv_perm_coll_ID) {	// Select random kind of advanced permissions:
                    switch ($advanced_user_perms[rand(0, $advanced_user_perms_max_index)]) {
                        case 'member':
                            // Set member permissions:
                            $adv_perm_coll_insert_values[] = implode(',', [
                ...
                if (! empty($adv_perm_coll_insert_values)) {	// Insert advanced user perms for new created user in single query for all collections with advanced perms:
        ...
        if ($count % 20 == 0) {	// Display a process of creating by one dot for 20 users:
            echo ' .';
            evo_flush();
        }
...
function tool_create_sample_basedomains($num_basedomains)
{
    ...
    for ($i = 0; $i < $num_basedomains; $i++) {
        ...
        if ($i % 100 == 0) {	// Display a process of creating by one dot for 100 base domains:
            echo ' .';
            evo_flush();
        }
    }
...
function tool_create_sample_messages($num_loops, $num_messages, $num_words, $max_users)
{
    ...
    for ($l = 0; $l < $num_loops; $l++) {
        ...
        foreach ($users as $from_user_ID) {
            foreach ($users as $to_user_ID) {
                ...
                for ($m = 0; $m < $num_messages; $m++) {
                    ...
                    if ($count_messages % 100 == 0) { // Display a process of creating by one dot for 100 users
                        echo ' .';
                        evo_flush();
                    }
                }
        ...
        for ($m = 0; $m < $num_messages; $m++) {
            ...
            $count_messages++;
            $user_number++;
            if ($user_number == count($users) || $user_number == $max_users - 1) {	// Reset user number to start of the list
                $user_number = 0;
            }
        }
        ...
        foreach ($users as $u => $user_ID) {
            ...
            if ($u == $max_users - 1) {	// limit by max users in one thread
                break;
            }
        }
        $DB->query('INSERT INTO T_messaging__threadstatus ( tsta_thread_ID , tsta_user_ID, tsta_first_unread_msg_ID )
...
[cta:1:link]Call to action 1 link only[/cta]'));
        ...
        if ($EmailCampaign->dbinsert()) {	// Send email after successfull email campaign creating:
            $count++;
            $loop_user_IDs = array_rand(array_flip($user_IDs), rand(1, count($user_IDs)));
            if (! is_array($loop_user_IDs)) {
                $loop_user_IDs = [$loop_user_IDs];
            }
            if (! empty($loop_user_IDs)) {	// Only if we have found the users in DB
        ...
        if ($count % 20 == 0) {	// Display a process of creating by one dot for 20 campaigns:
            echo ' .';
            evo_flush();
        }
...
function tool_resize_all_images()
{
    ...
    print_log(T_('Resize images...'), 'normal', [
        'text_style' => 'bold',
    ]);
    echo '<br />';
    ...
    print_log(sprintf(T_('Full execution time: %s seconds'), $Timer->get_duration('resize_all_images')), 'normal', [
        'text_style' => 'bold',
    ]);
}
