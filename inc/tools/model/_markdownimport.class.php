...
class MarkdownImport extends AbstractImport
{
    ...
    public function __construct()
    {
        ...
        $this->options_defs = [
            ...
            'import_type' => [
                ...
                'options' => [
                    'update' => [
                        'title' => TB_('Update existing contents'),
                        'note' => TB_('Existing Categories & Posts will be re-used (based on slug).'),
                    ],
                    'append' => [
                        'title' => TB_('Append to existing contents'),
                    ],
                    'delete' => [
            ...
            'slug_diff_coll' => [
                ...
                'options' => [
                    'skip' => [
                        'label' => sprintf(TB_('Display error and skip the %s file'), '<code>.md</code>'),
                    ],
                    'create' => [
                        'label' => TB_('Create a new slug like xyz-1'),
                    ],
                ],
                'type' => 'string',
                'default' => 'skip',
            ],
            'extra_cat_only_in_db' => [
                'group' => 'radio',
                'title' => TB_('If extra-categories are found in DB but not in YAML'),
                'options' => [
                    'keep_db' => [
                        'label' => TB_('Display WARNING and do not resolve'),
                    ],
                    'add_yaml' => [
                        'label' => sprintf(TB_('Resolve by adding extra-categories to YAML in %s file on disk'), '<code>.md</code>'),
                    ],
                    'unassign' => [
                        'label' => TB_('Resolve by deleting from DB'),
                    ],
                ],
                'type' => 'string',
                'default' => 'keep_db',
        ...
        $Plugins->trigger_event('ImporterConstruct', [
    ...
    {
        if (empty($this->source)) {	// File is not selected:
            return TB_('Please select file or folder to import.');
        } elseif (is_dir($this->source)) {
            if (! check_folder_with_extensions($this->source, 'md')) {	// Folder has no markdown files:
                return sprintf(TB_('Folder %s has no markdown files.'), '<code>' . $this->source . '</code>');
            }
        } elseif (! preg_match('/\.zip$/i', $this->source)) {	// Extension is incorrect:
    ...
    {
        if (isset($this->unzip_result)) {	// Don't unzip archive twice:
            return $this->unzip_result;
        }

        if (is_dir($this->source)) {	// Source is a folder:
            $this->unzip_result = false;
            $this->unzip_errors = '';
            return false;
        }

        if (! preg_match('/\.zip$/i', $this->source)) {	// Wrong source:
            $this->unzip_result = false;
            $this->unzip_errors = '';
            return false;
        }
        ...
        if ($this->unzip_result !== true) {	// Store unzip error from unpack_archive():
    ...
    {
        if (! isset($this->data)) {	// Load data in cache:
            $errors = '';

            $folder_path = null;
            if (is_dir($this->source)) {	// Use a folder:
                $folder_path = $this->source;
            } else {	// Try to extract ZIP:
                if ($this->unzip()) {	// Set folder on success unzipping:
            ...
            if (empty($this->unzip_errors) &&
                ($folder_path === null || ! check_folder_with_extensions($folder_path, 'md'))) {	// No markdown is detected in ZIP package:
                $errors .= '<p class="text-danger">' . TB_('No markdown file is detected in the selected source.') . '</p>';
                if (! empty($this->unzip_folder_path) && file_exists($this->unzip_folder_path)) {	// Delete temporary folder that contains the files from extracted ZIP package:
    ...
    {
        if (isset($this->options[$option_name])) {	// Use custom value:
            return $this->options[$option_name];
        }

        if (isset($this->options_defs[$option_name])) {	// Use default value:
    ...
    public function load_from_Request()
    {
        ...
        if ($check_source_result !== true) {	// Don't import if errors have been detected:
        ...
        if ($this->get_option('import_type') == 'delete' &&
            param('import_type_delete_confirm', 'string') !== 'DELETE') {	// If deleting/replacing is not confirmed:
    ...
    public function execute()
    {
        ...
        if (! empty($source_folder_zip_name)) {	// This is an import from ZIP archive
            $zip_file_name = preg_replace('#\.zip$#i', '', $source_folder_zip_name);
            if (file_exists($folder_path . '/' . $zip_file_name)) {	// If folder exists in the root with same name as ZIP file name:
                $skip_single_zip_root_folder = true;
                if ($folder_path_handler = @opendir($folder_path)) {
                    while (($file = readdir($folder_path_handler)) !== false) {
                        if (! preg_match('#^([\.]{1,2}|__MACOSX|' . preg_quote($zip_file_name) . ')$#i', $file)) {	// This is a different file or folder than ZIP file name:
                ...
                if ($skip_single_zip_root_folder) {	// Skip root folder with same name as ZIP file name:
        ...
        if (! $this->check_manifest($folder_path)) {	// Stop import because of restriction from detected file manifest.yaml:
            return;
        }

        $DB->begin();

        if ($this->get_option('import_type') == 'delete') {	// Remove data from selected collection:
            ...
            if (! empty($old_categories)) { // Get existing posts
                $SQL = new SQL();
                $SQL->SELECT('post_ID');
            ...
            if (! empty($old_categories)) {
                ...
                if (! empty($old_posts)) { // Remove the post's data from related tables
                    if ($this->get_option('delete_files')) { // Get the file IDs that should be deleted from hard drive
                        $SQL = new SQL();
                    ...
                    $Plugins->trigger_event('ImporterAfterItemsDelete', [
                        'type' => $this->import_code,
                        'Importer' => $this,
            ...
            if (! empty($old_posts)) { // Remove the tags
            ...
            if ($this->get_option('delete_files')) { // Delete the files
                ...
                if (! empty($deleted_file_IDs)) {
                    ...
                    foreach ($deleted_file_IDs as $deleted_file_ID) {
                        if (! ($deleted_File = &$FileCache->get_by_ID($deleted_file_ID, false, false))) { // Incorrect file ID
                            $this->log('<p class="text-danger">' . sprintf(TB_('No file #%s found in DB. It cannot be deleted.'), $deleted_file_ID) . '</p>');
                        }
                        if (! $deleted_File->unlink()) { // No permission to delete file
        ...
        foreach ($files as $f => $file_path) {
            ...
            if (! is_dir($file_path) ||
                    preg_match('#/((.*\.)?assets|__MACOSX)(/|$)#i', $file_path)) {	// Skip a not folder or reserved folder:
                continue;
            }
            ...
            if ($reuse_cats && $Chapter = &$this->get_Chapter($relative_path)) {	// Use existing category with same full url path:
                $categories[$relative_path] = $Chapter->ID;
                if ($category_name == $Chapter->get('name')) {	// Don't update category with same name:
                    $cat_results_num['no_changed']++;
                    $this->log(TB_('No change'));
                } else {	// Try to update category with different name but same slug:
                    $Chapter->set('name', $category_name);
                    if ($Chapter->dbupdate()) {	// If category is updated successfully:
                        $this->log('<span class="text-warning">' . TB_('Updated') . '</span>');
                        $cat_results_num['updated_success']++;
                    } else {	// Don't translate because it should not happens:
                ...
                if (! empty($parent_path) && isset($categories[$parent_path])) {	// Set category parent ID:
                    $Chapter->set('parent_ID', $categories[$parent_path]);
                }
                if ($Chapter->dbinsert()) {	// If category is inserted successfully:
        ...
        foreach ($files as $file_path) {
            ...
            try {	// Try and catch all SQL errors:
                $file_path = str_replace('\\', '/', $file_path);

                if (! preg_match('#([^/]+)\.md$#i', $file_path, $file_match) ||
                        preg_match('#/(\.[^/]*$|((.*\.)?assets|__MACOSX)/)#i', $file_path)) {	// Skip a not markdown file,
                ...
                if (preg_match($content_regexp, $item_content, $content_match)) {
                    ...
                    if (! empty($this->yaml_fields) && ! empty($item_yaml_data)) {	// Parse YAML data:
                        $item_yaml_data = spyc_load($item_yaml_data);
                    } else {	// Don't parse when no supported YAML fields or no provided YAML data:
                        $item_yaml_data = null;
                    }
                ...
                if ($this->get_data('type') == 'dir') {	// Provide link to md file when import is from forder:
                ...
                if (in_array($item_slug, $imported_slugs)) {	// Skip md file/post with same name from different folder/category:
                ...
                if (isset($categories[$category_path])) {	// Use existing category:
                    $category_ID = $categories[$category_path];
                } else {	// Use default category:
                    if (! isset($default_category_ID)) {	// If category is still not defined then we should create default, because blog must has at least one category
                        $default_category_urlname = $md_Blog->get('urlname') . '-main';
                        if (! ($default_Chapter = &$ChapterCache->get_by_urlname($default_category_urlname, false, false))) {	// Create default category if it doesn't exist yet:
                ...
                if ($this->get_option('import_type') == 'update') {	// For "update" mode try to find existing Item by slug:
                    $Item = &$this->get_Item($item_slug);
                    if ($Item === false) {	// Skip if Item is found by same slug but in another Collection:
                ...
                if ($this->get_option('import_type') != 'update' ||
                    ! $Item) {	// Create new Item for not update mode or if it is not found by slug in the requested Collection:
                    $Item = new Item();
                ...
                if ($this->get_option('convert_md_links')) {	// Convert Markdown links to b2evolution ShortLinks:
                    ...
                    // Do convert:
                    $item_content = preg_replace_callback('#(^|[^\!])\[([^\[\]]*)\]\(((([a-z]*://)?([^\)]+[/\\\\])?([^\)]+?)(\.[a-z]{2,4})?)(\#[^\)]+)?)?\)#i', [$this, 'callback_convert_links'], $item_content);
                    if ($this->item_content_is_updated_by_linked_file) {	// Force to update content when at least one link was replaced with proper link to post with same language as current post:
                ...
                if ($this->get_option('force_item_update') || $item_content_was_changed) {	// Set new fields only when import hash(title + content + YAML data) was really changed:
                    ...
                    if ($Item->get('excerpt_autogenerated') && ! empty($item_content)) {	// Generate excerpt from content:
                        $Item->set('excerpt', excerpt($item_content), true);
                    }
                }

                foreach ($this->yaml_fields as $yaml_field) {	// Set YAML field:
                    if (! isset($item_yaml_data[$yaml_field])) {	// Skip if the Item has no defined value for this YAML field:
                        continue;
                    }
                    $yaml_method = 'set_yaml_' . str_replace('-', '_', $yaml_field);
                    if (method_exists($this, $yaml_method)) {	// Call method to set YAML field:
                        $this->$yaml_method($item_yaml_data[$yaml_field], $Item);
                    }

                    // Call plugin event to set YAML field:
                    $Plugins->trigger_event('ImporterSetItemField', [
                        'type' => $this->import_code,
                        'Importer' => $this,
                        'Item' => $Item,
                        'field_type' => 'yaml',
                ...
                $item_result_messages = [];
                $item_result_class = 'default';
                $item_result_suffix = '';
                if (empty($Item->ID)) {	// Insert new Item:
                    $this->log_list('<span class="label label-info">INSERTING</span> Item in DB...');
                    if ($Item->dbinsert()) {	// If post is inserted successfully:
                    ...
                    if (! $this->get_option('force_item_update') && ! $item_content_was_changed && $prev_category_ID == $category_ID && ! $this->item_yaml_is_updated) {	// Don't try to update item in DB because import hash(title + content) was not changed after last import:
                        ...
                        if (
                            ...
                            if ($this->get_option('force_item_update')) {	// If item update was forced:
                                $item_result_messages[] = /* TRANS: Result of imported Item */ TB_('Forced update');
                            } else {	// Normal update because content or category was changed:
                                $item_result_messages[] = /* TRANS: Result of imported Item */ TB_('Has changed');
                            }
                            if ($prev_category_ID != $category_ID) {	// If moved to different category:
                                $item_result_messages[] = /* TRANS: Result of imported Item */ TB_('Moved to different category');
                            }
                            if ($this->item_yaml_is_updated) {	// If YAML fields were updated:
                                $item_result_messages[] = /* TRANS: Result of imported Item */ TB_('Updated YAML fields');
                            }
                            if ($item_content_was_changed) {	// If content was changed:
                                $item_result_messages[] = /* TRANS: Result of imported Item */ TB_('New revision added to DB');
                                if ($prev_last_import_hash === null) {	// Display additional warning when Item was edited manually:
                                    global $admin_url;
                                    $item_result_suffix = '. <br /><span class="label label-danger">' . TB_('CONFLICT') . '</span> <b>'
                                        . sprintf(
                                            TB_('WARNING: this item has been manually edited. Check <a %s>changes history</a>'),
                                            'href="' . $admin_url . '?ctrl=items&amp;action=history&amp;p=' . $Item->ID . '" target="_blank"'
                                        ) . '</b>';
                                }
                            }
                ...
                if ($Item->ID > 0) {	// Set last message text as link to permanent URL of the inserted/updated Item:
                    $last_msg_i = count($item_result_messages) - 1;
                    $item_result_messages[$last_msg_i] = $Item->get_title([
                ...
                $Plugins->trigger_event('ImporterAfterItemImport', [
                    'type' => $this->import_code,
                    'Importer' => $this,
                    'Item' => $Item,
                    'data' => $item_yaml_data,
                    ...
                    if (preg_match_all('#(\[)?\!\[([^\]]*)\]\(([^\)"]+\.(' . $this->get_image_extensions() . '))\s*("[^"]*")?\)(\{\..+?\})?(\r?\n?\*.*?\*(\r|\n|$))?(.*?\]\((.*?)\))?(\{\..+?\})?#i', $item_content, $image_matches)) {
                        ...
                        foreach ($image_matches[3] as $i => $image_relative_path) {
                            ...
                            if (strtolower($image_alt) == 'img' ||
                                    strtolower($image_alt) == 'image') {	// Don't use this default text for alt image text:
                                $image_alt = '';
                            }
                            ...
                            } else {	// Link image as inline when header is before the image or no header in item's content:
                                $file_params['link_position'] = 'inline';
                            }
                            // Try to find existing and linked image File or create, copy and link image File:
                            if ($link_data = $this->link_file($LinkOwner, $folder_path, $category_path, rtrim($image_relative_path), $file_params)) {	// Replace this img tag from content with b2evolution format:
                                if ($file_params['link_position'] == 'inline') {	// Generate image inline tag:
                                    ...
                                    if ($image_matches[1][$i] == '[' && $image_matches[10][$i] !== '') {	// HRef:
                                        $image_options[] = $image_matches[10][$i];
                                    }
                                    if ($image_inline_class !== '') {	// Class:
                                ...
                                if ($link_data['type'] == 'new') {	// Count new linked files:
                                    $new_links_count++;
                                }
                                $all_links_count++;
                            }
                        ...
                        if ($new_links_count > 0 || ($item_is_updated_step_1 && $all_links_count > 0)) {	// Update content for new markdown image links which were replaced with b2evo inline tags format:
                            if ($new_links_count > 0) {	// Update content with new inline image tags:
                                $this->log_list(
                                    sprintf(TB_('%d new image files were linked to the Item'), $new_links_count)
                                    . ' -> ' . /* TRANS: Result of imported Item */ TB_('Saving to DB') . '.',
                                    'text-warning'
                                );
                            } else {	// Force to update content with inline image tags:
                                $this->log_list(
                                    TB_('No image file changes BUT Item Update is required')
                                    . ' -> ' . /* TRANS: Result of imported Item */ TB_('Saving <code>[image:]</code> tags to DB') . '.',
                                    'text-warning'
                                );
                            }
                            if (! $item_is_filtered_by_plugins) {	// Filter title and content by renderer plugins:
                                $item_Blog = &$Item->get_Blog();
                                $item_plugin_params = [
                                    'object_type' => 'Item',
                                    'object' => &$Item,
                ...
                if (! empty($this->item_file_is_updated)) {	// Update item's file with fixed content:
                    if (($md_file_handle = @fopen($file_path, 'w')) &&
                        fwrite($md_file_handle, $this->item_file_content)) {	// Inform about updated file content:
                        ...
                        $Item->dbupdate(true, true, true, 'no'/* Force to do NOT create new revision because we do this above when store new content */);
                    } else {	// No file rights to write into the file:
            ...
            } catch (Exception $db_Exception) {	// Catch SQL error:
        ...
        if ($this->get_data('type') == 'zip' && file_exists($root_folder_path)) {	// This folder was created only to extract files from ZIP package, Remove it now:
     ...
     */
    public function link_file($LinkOwner, $source_folder_absolute_path, $source_category_folder, $requested_file_relative_path, $params)
    {
        $params = array_merge([
            'file_root_type' => 'collection',
            'file_root_ID' => '',
        ...
        if (strpos(get_canonical_path($file_source_path), $source_folder_absolute_path) !== 0) {	// Don't allow a traversal directory:
            $this->log_list(sprintf('Skipping file %s, because path is invalid.', '<code>' . $requested_file_relative_path . '</code>'), 'error');
            // Skip it:
            return false;
        }

        if (! file_exists($file_source_path)) {	// File doesn't exist
        ...
        if (! empty($file_data) &&
            ...
            if (! empty($file_data['link_ID'])) {	// The found File is already linked to the Item:
                ...
                return [
                    'ID' => $file_data['link_ID'],
                    'type' => 'old',
                ];
            } else {	// Try to link the found File object to the Item:
                if ($link_ID = $File->link_to_Object($LinkOwner, 0, $params['link_position'])) {	// If file has been linked to the post
                ...
                } else {	// If file could not be linked to the post:
        ...
        if ($this->get_option('import_type') == 'update') {	// Try to find existing and linked image File:
            $item_Links = $LinkOwner->get_Links();
            foreach ($item_Links as $item_Link) {
                if (($File = &$item_Link->get_File()) &&
                        $file_source_name == $File->get('name')) {	// We found File with same name:
                    if ($File->get('hash') != $file_source_hash) {	// Update only really changed file:
                        $replaced_File = $File;
                        $replaced_link_ID = $item_Link->ID;
                        $replaced_link_type = 'old';
                        // Don't find next files:
                        break;
                    } else {	// No change for same file:
                        $this->log_list(sprintf(TB_('No file change, because %s is same as %s.'), '<code>' . $source_file_relative_path . '</code>', '<code>' . $File->get_rdfs_rel_path() . '</code>'));
                        return [
                            'ID' => $item_Link->ID,
                            'type' => 'old',
        ...
        if ($this->get_option('import_type') != 'append' &&
                $replaced_File === null) {	// Find an existing File on disk to replace with new:
            ...
            if ($File && $File->exists()) {	// If file already exists:
                $replaced_File = $File;
            }
        }

        if ($replaced_File !== null) {	// The found File must be replaced:
            if (empty($replaced_File->ID)) {	// Create new File in DB with additional params:
                $replaced_File->set('title', $params['file_title']);
                if (! $replaced_File->dbinsert()) {	// Don't translate
            ...
            if (! copy_r($file_source_path, $replaced_File->get_full_path())) {	// No permission to replace file:
            ...
            if ($replaced_link_ID !== null) {	// Inform about replaced file:
            ...
            } elseif ($replaced_link_ID = $replaced_File->link_to_Object($LinkOwner, 0, $params['link_position'])) {	// If file has been linked to the post
        ...
        if (! $File || ! copy_r($file_source_path, $File->get_full_path())) {	// No permission to copy to the destination folder
        ...
        if ($link_ID = $File->link_to_Object($LinkOwner, 0, $params['link_position'])) {	// If file has been linked to the post
            $this->log_list(sprintf(
                TB_('New file %s has been imported to %s as %s successfully.'),
     ...
     */
    public function &get_Chapter($cat_folder_path, $check_full_path = true, $item_Type_ID = null)
    {
        if (isset($this->chapters_by_path[$cat_folder_path])) {	// Get Chapter from cache:
        ...
        foreach ($cat_full_url_path as $c => $cat_slug) {	// Convert title text to slug format:
        ...
        $SQL->SELECT('cat_ID');
        $SQL->FROM('T_categories');
        if ($Settings->get('cross_posting') && $item_Type_ID !== null) {	// Select categories from all possible collections where cross-posting is allowed between the imported collection:
            $SQL->FROM_add('INNER JOIN T_items__type_coll ON itc_coll_ID = cat_blog_ID');
            $SQL->WHERE_and('itc_ityp_ID = ' . $DB->quote($item_Type_ID));
        } else {	// Select categories only from the imported collection because cross-posting is not allowed in system:
            $SQL->WHERE('cat_blog_ID = ' . $DB->quote($this->coll_ID));
        }
        ...
        foreach ($cat_IDs as $cat_ID) {
            if ($Chapter = &$ChapterCache->get_by_ID($cat_ID, false, false)) {
                $full_match = true;
                if ($check_full_path) {	// Check full path:
                ...
                if ($full_match) {	// We found category with same full url path:
                    $r = $Chapter;
                    break;
                }
            }
     ...
     */
    public function &get_Item($item_slug)
    {
        global $DB;

        if ($this->get_option('slug_diff_coll') == 'create') {	// Try to find Item by slug with suffix like "-123" in the requested Collection:
            $item_slug_base = preg_replace('/-\d+$/', '', $item_slug);
            $sql_title = 'Find Item by slug base "' . $item_slug_base . '" in the Collection #' . $this->coll_ID;
        } else {	// Try to find Item by slug in ALL Collections:
        ...
        if ($this->get_option('slug_diff_coll') == 'create') { // Create a new slug like xyz-1 if filename/slug exists in a different collection:
            $SQL->WHERE('cat_blog_ID = ' . $DB->quote($this->coll_ID));
            $SQL->WHERE_and('slug_title REGEXP ' . $DB->quote('^' . $item_slug_base . '(-[0-9]+)?$'));
        } else {	// Display error and skip the .md file
        ...
        if (! empty($post_data)) {	// Item is found by slug
            $post_coll_ID = $post_data->cat_blog_ID;
            // Initalize a found Item by ID:
            $post_ID = $post_data->post_ID;
            $ItemCache = &get_ItemCache();
            if ($Item = &$ItemCache->get_by_ID($post_ID, false, false)) {	// If Item is found by ID
                ...
                if ($this->get_option('slug_diff_coll') == 'skip' &&
                        $post_coll_ID != $this->coll_ID) {	// Skip Item with same slug in another Collection:
                    $BlogCahe = get_BlogCache();
                    $another_Blog = &$BlogCahe->get_by_ID($post_coll_ID, false, false);
                    $this->log_list(sprintf(
                        'Skip this %s file because Item %s already exists with same slug in collection %s!',
                        '<code>.md</code>',
     ...
     */
    public function callback_convert_links($m)
    {
        $link_title = trim($m[2]);
        $link_url = isset($m[3]) ? trim($m[3]) : '';

        if ($link_url === '') {	// URL must be defined:
            $this->log_linked_file('error_link', $m[0]);
            return $m[0];
        }

        if (! empty($m[5])) {	// Use full URL because this is URL with protocol like http://
            $item_url = $m[3];
            // Anchor is already included in the $m[3]:
            $link_anchor = '';
        } elseif (isset($m[8]) && $m[8] === '.md') {	// Extract item slug from relative URL of md file:
            $item_url = get_urltitle($m[7]);
            $item_slug = $item_url;
            $link_anchor = isset($m[9]) ? trim($m[9], '# ') : '';
        } elseif (strpos($m[7], '#') === 0 && strlen($m[7]) > 1) {	// This is anchor URL to current post:
            $item_url = '';
        ...
        if ($this->get_option('check_links') &&
            isset($item_slug) &&
            ($ItemCache = &get_ItemCache()) &&
            ($slug_Item = $ItemCache->get_by_urltitle($item_slug, false, false))) {	// Check internal link (slug) to see if it links to a page of the same language:
            if ($slug_Item->get('locale') != $this->current_item_locale) {	// Different locale:
                $this->log_linked_file('check', $m[0], $slug_Item->get_title([
                    'link_type' => 'admin_view',
                ]), $slug_Item->get('locale'));
                if ($this->get_option('diff_lang_suggest')) {	// Find and suggest equivalent from "linked languages/versions" table:
                    if ($version_Item = &$slug_Item->get_version_Item($this->current_item_locale, false)) {	// We found a version Item with required locale:
                        $version_item_link = $version_Item->get_title([
                            'link_type' => 'admin_view',
                        ]);
                        $this->log_linked_file('recommend', null, $version_item_link);
                        if ($this->get_option('same_lang_replace_link')) {	// Replace the link slug in the post:
                            $item_url = $version_Item->get('urltitle');
                            $this->log_linked_file('content', $m[0], $version_item_link);
                            if ($this->get_option('same_lang_update_file')) {	// Update md file with new replaced links:
    ...
    {
        if (! isset($this->image_extensions)) {	// Load image extensions from DB into cache string:
            global $DB;
     ...
     */
    public function log_linked_file($type, $tag, $info_link = null, $slug_item_locale = null)
    {
        switch ($type) {
            case 'error_link':
            case 'error_image':
                $this->log_list(sprintf('Markdown link %s could not be convered to b2evolution ShortLink.', '<code>' . $tag . '</code>'), 'error');
                if ($type == 'error_image') {	// Special warning when URL to image is used in link markdown tag:
            ...
            case 'check':
                $this->log_list(sprintf(
                    'Link %s points to "%s" which is in %s instead of %s.',
                    '<code>' . $tag . '</code>',
                    $info_link,
            ...
            case 'recommend':
                $this->log_list(sprintf(
                    'We recommend "%s" (%s) as destination.',
                    $info_link,
                    '<code>' . $this->current_item_locale . '</code>'
                ), 'warning');
                break;
     ...
     */
    public function set_yaml_title($value, &$Item)
    {
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_description($value, &$Item)
    {
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_keywords($value, &$Item)
    {
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_excerpt($value, &$Item)
    {
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_short_title($value, &$Item)
    {
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_tags($value, &$Item)
    {
        if (! $this->check_yaml_array('tags', $value, true, true)) {	// Skip wrong data:
            // Don't print error messages here because all messages are initialized inside $this->check_yaml_array().
            return;
        }

        if ($value === '' || $value === []) {	// Clear tags:
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_extra_cats($value, &$Item)
    {
        if (! $this->check_yaml_array('extra-cats', $value)) {	// Skip wrong data:
            // Don't print error messages here because all messages are initialized inside $this->check_yaml_array().
            return;
        }

        // Get current extra-categories:
        if (isset($Item->extra_cat_IDs)) {	// Clear to be sure all etra categories are loaded from DB:
        ...
        foreach ($value as $extra_cat_slug) {
            if ($extra_Chapter = &$this->get_Chapter($extra_cat_slug, (strpos($extra_cat_slug, '/') !== false), $Item->get('ityp_ID'))) {	// Use only existing category:
            ...
            } else {	// Display error on not existing category:
        ...
        $only_db_extra_cat_links = [];
        $only_db_extra_cat_slugs = [];
        foreach ($only_db_extra_cat_IDs as $o => $only_db_extra_cat_ID) {
            if ($only_db_extra_Chapter = &$ChapterCache->get_by_ID($only_db_extra_cat_ID, false, false)) {	// Get link to Category if it is found in DB:
                ...
                if ($this->get_option('extra_cat_only_in_db') == 'unassign' &&
                    $only_db_extra_Chapter->ID == $Item->get('main_cat_ID')) {	// Main category cannot be un-assigned, however it must be stored together with extra-categories in DB table:
                ...
                if ($this->get_option('extra_cat_only_in_db') == 'add_yaml') {	// We need slugs in order to append them in YAML block:
                    $only_db_extra_cat_slugs[] = $only_db_extra_Chapter->get('urlname');
                }
            } else {	// Unset wrong extra-category because it is not found in DB:
                unset($only_db_extra_cat_IDs[$o]);
        ...
        if (! empty($only_db_extra_cat_IDs)) {	// If extra-categories are found in DB but not in YAML:
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_order($value, &$Item)
    {
        if (! preg_match('#^-?[0-9]*(\.[0-9]+)?$#', $value)) {	// Order value must be a decimal number:
     ...
     * @param object Item (by reference)
     */
    public function set_yaml_item_type($value, &$Item)
    {
        if ($value === '') {	// Skip empty Item Type name
            $this->log_list(sprintf('Skip empty yaml field %s.', '<code>item-type</code>'), 'warning');
            return;
        }

        $ItemTypeCache = &get_ItemTypeCache();
        if (! ($ItemType = &$ItemTypeCache->get_by_name($value, false, false))) {	// Skip unknown Item Type:
            $this->log_list(sprintf('Not found Item Type %s for yaml field %s.', '"' . $value . '"', '<code>item-type</code>'), 'error');
            return;
        }

        if (! $ItemType->is_enabled($Item->get_blog_ID())) {	// Skip not enabled Item Type:
     ...
     */
    public function check_yaml_array($field_name, $field_value, $allow_string_format = false, $allow_empty = false)
    {
        if (! $allow_empty) {	// Check for not empty value:
            if (($allow_string_format && $field_value === '') ||
                    ($field_value === [])) {	// Skip empty yaml field:
                $this->log_list(sprintf(TB_('Skipping YAML field %s, because it is empty.'), '<code>' . $field_name . '</code>'), 'warning');
                return false;
            }
        }

        if ($allow_string_format && is_string($field_value)) {	// Don't check array if the YAML field is allowed to be a string:
            return true;
        }

        if (! is_array($field_value)) {	// Wrong not array data:
        ...
        foreach ($field_value as $string) {
            if (is_array($string)) {	// Skip wrong indented data:
     ...
     */
    public function event_after_import()
    {
    }
     ...
     */
    public function set_item_file_content($new_content)
    {
        if ($this->item_file_content != $new_content) {	// Update item content only when content is really changed:
     ...
     */
    public function set_item_file_yaml_field($field_name, $new_field_value)
    {
    ...
    public function display_import()
    {
        ...
        if (preg_match('/\.zip$/i', $this->source)) {	// ZIP archive:
        ...
        if ($this->get_data('errors') !== false) {	// Display errors:
        ...
        $selected_options = [];
        foreach ($this->options_defs as $option_key => $option) {
            if ($option['group'] != 'options') {	// Skip option from different group:
                continue;
            }
        ...
        // Radio options:
        foreach ($this->options_defs as $option_key => $option) {
            if ($option['group'] == 'radio' && $this->get_option($option_key)) {	// Display here only radio options:
                $this->log('<b>' . $option['title'] . ':</b> ' . $option['options'][$this->get_option($option_key)]['label'] . '<br>');
            }
        }
        $this->log('<br>');

        if ($this->get_data('errors') === false) {	// Import the data and display a report on the screen:
            $this->execute();
        } else {	// Display errors if import cannot be done:
