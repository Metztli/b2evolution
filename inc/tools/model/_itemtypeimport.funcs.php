...
function itxml_import($XML_file_path)
{
    ...
    if (is_array($it_blog_IDs) && ! empty($it_blog_IDs)) {
        ...
        foreach ($it_blog_IDs as $it => $it_blog_ID) {
            if (! ($it_Blog = &$BlogCache->get_by_ID($it_blog_ID, false, false))) {	// Exclude wrong collection:
    ...
    if (isset($xml_data['item_types']) && count($xml_data['item_types']) > 0) {
        ...
        foreach ($xml_data['item_types'] as $item_type) {
            ...
            if (empty($existing_item_types[$item_type['name']])) {	// Create new Item Type in order to insert into DB:
                $ItemType = new ItemType();
                $ItemType->set('name', $item_type['name']);
            } elseif ($import_type == 'skip') {	// Skip existing Item Type:
                ...
                evo_flush();
                continue;
            } else {	// Get the existing Item Type in order to update it:
            ...
            foreach ($item_type as $item_type_field_key => $item_type_field_value) {
                if (! in_array($item_type_field_key, ['ID', 'name', 'custom_fields']) &&
                    property_exists('ItemType', $item_type_field_key)) {	// Update only proper itme type field:
            ...
            if (! empty($item_type['custom_fields'])) {	// Import custom fields:
                $ItemType->update_custom_fields = [];
                $ItemType->insert_custom_fields = [];
                foreach ($item_type['custom_fields'] as $custom_field_name => $new_custom_field) {
                    ...
                    foreach ($custom_field_cols as $custom_field_col) {	// Check the imported custom field has all required columns:
                        if (! array_key_exists($custom_field_col, $new_custom_field)) {	// Skip wrong custom field:
                        ...
                        if (in_array($custom_field_col, ['required', 'meta', 'public', 'link_nofollow', 'merge'])) {	// Format number fields in order to avoid error on insert/update data in DB:
                            $new_custom_field[$custom_field_col] = empty($new_custom_field[$custom_field_col]) ? 0 : 1;
                        }
                    }

                    if (isset($old_custom_fields[$custom_field_name])) {	// If Item Type is found:
                        if ($old_custom_fields[$custom_field_name]['type'] == $new_custom_field['type']) {	// Update existing custom field only if type is the same:
                            ...
                            evo_flush();
                        } else {	// Don't allow to update custom field with a different type:
            ...
            if (! empty($old_custom_fields)) {	// Find and delete old custom fields if they are not present in the imported Item Type:
                $deleted_custom_fields = [];
                $ItemType->delete_custom_fields = [];
                foreach ($old_custom_fields as $old_custom_field_key => $old_custom_field) {
                    if (! isset($item_type['custom_fields'][$old_custom_field_key])) {	// Delete custom field from DB:
                        $deleted_custom_fields[] = $old_custom_field_key;
                        $ItemType->delete_custom_fields[] = $old_custom_field['ID'];
                    }
                }
                if (! empty($deleted_custom_fields)) {	// Display what custom fields will be deleted:
            ...
            if ($ItemType && $ItemType->dbsave()) {	// If Item Type is added/updated successfully:
                if (! empty($it_blog_IDs)) {	// Enable the Item Type for the selected Collections:
                    $it_colls_insert_sql_values = [];
...
function itxml_parser($file)
{
    ...
    foreach ($xml->xpath('/rss/channel/evo:itemtype') as $item_type_data) {
        ...
        foreach ($evo as $evo_key => $evo_data) {
            if (is_array($evo_data)) {	// Skip array data like custom field:
                continue;
            }
        ...
        if (isset($evo['customfield'])) {
            ...
            foreach ($evo['customfield'] as $custom_field_data) {	// Get the custom fields:
