...
{
    return [
        'unknown' => get_icon('bullet_white', 'imgtag', [
            'title' => emadr_get_status_title('unknown'),
        ]),
        'working' => get_icon('bullet_green', 'imgtag', [
            'title' => emadr_get_status_title('working'),
        ]),
        'unattended' => get_icon('bullet_magenta', 'imgtag', [
            'title' => emadr_get_status_title('unattended'),
        ]),
        'redemption' => get_icon('bullet_light_blue', 'imgtag', [
            'title' => emadr_get_status_title('redemption'),
        ]),
        'warning' => get_icon('bullet_yellow', 'imgtag', [
            'title' => emadr_get_status_title('warning'),
        ]),
        'suspicious1' => get_icon('bullet_orange', 'imgtag', [
            'title' => emadr_get_status_title('suspicious1'),
        ]),
        'suspicious2' => get_icon('bullet_orange', 'imgtag', [
            'title' => emadr_get_status_title('suspicious2'),
        ]),
        'suspicious3' => get_icon('bullet_orange', 'imgtag', [
            'title' => emadr_get_status_title('suspicious3'),
        ]),
        'prmerror' => get_icon('bullet_red', 'imgtag', [
            'title' => emadr_get_status_title('prmerror'),
        ]),
        'spammer' => get_icon('bullet_brown', 'imgtag', [
            'title' => emadr_get_status_title('spammer'),
        ]),
    ];
}
...
function emadr_get_statuses_less_level($status)
{
    ...
    foreach ($levels as $status => $level) {
        if ($level <= $current_level) {	// Add this status into array if the level is less or equal then current level
            $statuses[] = $status;
        }
    }
 ...
 */
function emlog_result_info($result, $params = [], $last_open = null, $last_click = null)
{
    $params = array_merge([
    ...
    switch ($result) {
        case 'ok':
            if ($params['display_icon']) {
                if (empty($last_open) && empty($last_click)) {
                    $result_info .= get_icon('bullet_black', 'imgtag', [
                        'alt' => T_('Sent'),
                    ]);
                } else {
                    $result_info .= get_icon('bullet_green', 'imgtag', [
                        'alt' => T_('Opened'),
                    ]);
                }
            }
        ...
        case 'error':
            if ($params['display_icon']) {
                $result_info .= get_icon('bullet_orange', 'imgtag', [
                    'alt' => T_('Error'),
                ]);
            }
            if ($params['display_text']) {
        ...
        case 'blocked':
            if ($params['display_icon']) {
                $result_info .= get_icon('bullet_red', 'imgtag', [
                    'alt' => T_('Blocked'),
                ]);
            }
            if ($params['display_text']) {
                if ($params['link_blocked'] && ! empty($params['email'])) {	// Create a link for email address
                    global $admin_url;
        ...
        case 'simulated':
            if ($params['display_icon']) {
                if (empty($last_open) && empty($last_click)) {
                    $result_info .= get_icon('bullet_gray', 'imgtag', [
                        'alt' => T_('Simulated'),
                    ]);
                } else {
                    $result_info .= get_icon('bullet_green', 'imgtag', [
        ...
        case 'ready_to_send':
            if ($params['display_icon']) {
                $result_info .= get_icon('bullet_empty', 'imgtag', [
                    'alt' => T_('Ready to send'),
                ]);
            }
            if ($params['display_text']) {
...
function mail_log($user_ID, $to, $subject, $message, $headers, $result, $email_key = null, $email_campaign_ID = null, $automation_ID = null)
{
    ...
    if ($result == 'ok') { // Save a report about sending of this message in the table T_email__address
...
{
    global $DB, $servertimenow, $localtimenow;
    $valid_results = ['ok', 'error', 'blocked', 'simulated', 'ready_to_send'];
...
function update_mail_log_time($type, $emlog_ID, $emlog_key)
{
    ...
    if ($r) {	// Update last time for email campaign per user:
        // Check if mail is not yet opened
        $unopened_mail = is_unopened_campaign_mail($emlog_ID, $send_data);
        if (empty($send_data[$campaign_time_field]) && ! empty($send_data['csnd_camp_ID'])) { // First image load/click, update appropriate campaign counters
        ...
        if (empty($emlog->emlog_autm_ID)) {	// Update last time for user subscriptions of newsletters:
        ...
        } else {	// Update last time for user subscriptions of all automation newsletters:
            $DB->query('UPDATE T_email__newsletter_subscription
        ...
        if (($EmailAddress = &$EmailAddressCache->get_by_name($emlog->emlog_to, false, false)) &&
            ($emlog->emlog_timestamp > $EmailAddress->get('last_open_ts'))) {	// If email address exists in DB and sent date > previous last opened email date:
 ...
 *     'spammer'     - Spammer
 */
function load_blocked_emails($user_IDs, $blocked_statuses = [])
{
    ...
    if (! isset($cache_mail_is_blocked_status)) { // Init array first time
        $cache_mail_is_blocked_status = [];
    }

    $status_filter_name = implode('_', $blocked_statuses);
    if (! isset($cache_mail_is_blocked_status[$status_filter_name])) { // Init subarray for each filter by statuses
        $cache_mail_is_blocked_status[$status_filter_name] = [];
    }
    ...
    foreach ($blocked_emails as $email => $email_blocked_ID) { // The blocked email has TRUE value; Trust emails - FALSE
        $cache_mail_is_blocked_status[$status_filter_name][$email] = (bool) $email_blocked_ID;
    }
}
 ...
 * @return boolean TRUE
 */
function mail_is_blocked($email, $blocked_statuses = [])
{
    ...
    if (! isset($cache_mail_is_blocked_status[$status_filter_name][$email])) { // If we check status of this email first time - get it from DB and store in cache
        global $DB;
        ...
        $cache_mail_is_blocked_status[$status_filter_name][$email] = (bool) $DB->get_var($SQL);
    }
 ...
 */
function get_mail_blocked_condition($is_blocked = true, $blocked_statuses = [])
{
    global $DB;

    if (empty($blocked_statuses)) {	// Default the blocked statuses
...
function blocked_emails_memorize($email)
{
    ...
    if (empty($email)) { // Empty email, Exit here
        return;
    }

    if (check_user_perm('users', 'view')) { // User has permissions to view other users
        if (mail_is_blocked($email)) { // Check if the email address is blocked
            if (isset($cache_blocked_emails[$email])) { // Icrease a count of blocked email
...
function blocked_emails_display($log_messages = false)
{
    ...
    if (! empty($cache_blocked_emails) && is_array($cache_blocked_emails)) { // Display the messages about the blocked emails (grouped by email)
...
function mail_log_parse_message($headers, $message)
{
    ...
    if (empty($header_matches[1])) { // Incorrect headers, Exit here
        return false;
    }

    $data = [];

    if ($header_matches[1] == 'text/html') { // Message has one content in HTML format
        $data['html'] = [
        ...
        return $data;
    } elseif ($header_matches[1] != 'multipart/mixed') { // Message content is not multipart
        return false;
    }

    preg_match('/Content-Type: multipart\/alternative; boundary="([^;]+)"/i', $message, $boundary_matches);

    if (empty($boundary_matches) || empty($boundary_matches[1])) { // No found boundary delimiter of message contents
        return false;
    }
...
function check_smtp_mailer()
{
    ...
    if (! $Settings->get('smtp_enabled')) { // Swift Mailer is not enabled
    ...
    if (empty($smtp_server_host) || empty($smtp_server_port)) { // These settings must be defined
    ...
    if ($smtp_server_security == 'ssl' || $smtp_server_security == 'tls') { // Check if enabled encryption method is enabled in this system
        $available_transports = stream_get_transports();
        $method_is_available = false;
        foreach ($available_transports as $available_transport) {
            if (preg_match('#^' . $smtp_server_security . '#i', $available_transport)) { // Check if first symbols are match, because transport can be "ssl", "sslv2" or "sslv3"
                $method_is_available = true;
                break;
            }
        }
        if (! $method_is_available) { // Stop the checking here because encryption method is not available
...
{
    try { // Try to intialize a connection by SMTP transport
        $Swift_SmtpTransport->start();
        return true;
    } catch (Swift_TransportException $Swift_TransportException) { // Error connection
...
function &get_Swift_SmtpTransport()
{
    ...
    if (isset($Swift_SmtpTransport)) {	// Get SMTP Swift Transport from cached global variable:
    ...
    if ($smtp_server_security == 'ssl' || $smtp_server_security == 'tls') {
        ...
        if ($Settings->get('smtp_server_novalidatecert')) {	// Do not validate the certificate from the TLS/SSL server:
            $options = [
                'ssl' => [
                    'verify_peer' => false,
    ...
    if (! empty($smtp_server_username)) {	// Set username if it is defined:
        $Swift_SmtpTransport->setUsername($smtp_server_username);
    }
    if (! empty($smtp_server_password)) {	// Set password if it is defined:
...
function &get_Swift_Mailer()
{
    ...
    if (isset($Swift_Mailer)) {	// Get SMTP Swift Mailer from cached global variable:
        return $Swift_Mailer;
    }
    ...
    if ($connection_result === true) { // Create the Mailer using the created Transport
        $Swift_Mailer = new Swift_Mailer($Swift_SmtpTransport);
    } else { // Some errors on SMTP connection
        $Swift_Mailer = null;
    }
 ...
 */
function evo_mail($to, $subject, $message, $headers = [], $additional_parameters = '')
{
    ...
    switch ($Settings->get('email_service')) {
        case 'smtp':
            ...
            if (! $result && $Settings->get('force_email_sending')) {	// SMTP sending failed, Fallback to sending email by php "mail" function:
        ...
        default:
            ...
            $result = evo_mail_php($to, $subject, $message, $headers, $additional_parameters);
            if (! $result && $Settings->get('force_email_sending')) {	// "mail" function failed, Fallback to sending email by SMTP Swift Mailer:
 ...
 */
function evo_mail_php($to, $subject, $message, $headers = [], $additional_parameters = '')
{
    ...
    if (! empty($headers_string)) {	// Write headers to log:
        $php_mail_sending_log .= PHP_EOL . 'Headers:' . PHP_EOL . $headers_string;
    }

    if (! empty($additional_parameters)) {	// Write additional params to log:
    ...
    if (! $result) {	// Store error log in global var:
 ...
 * @return boolean TRUE on success
 */
function evo_mail_smtp($to, $subject, $message, $headers = [])
{
    global $smtp_mail_sending_log;

    // Check if we can use SMTP Swift Mailer
    if (check_smtp_mailer() === true && ($Swift_Mailer = &get_Swift_Mailer())) { // Use Swift Mailer to send emails using SMTP
        ...
        // To:
        if (empty($message['to_email'])) { // Use only email address
            $Swift_Message->setTo($to);
        } else { // Use address with name
        ...
        if (isset($headers['Content-Type']) && preg_match('#^[^;]+#i', $headers['Content-Type'], $content_type)) {
            switch ($content_type[0]) {
                case 'multipart/mixed':
                    // MULTIPLE:
                    if (is_array($message)) { // Body is multiple
                        ...
                        break;
                    } else { // Unknown case, Send email with text/plain format
        ...
        // From:
        if (! empty($message['from_email'])) { // Use address with name
            $Swift_Message->setFrom($message['from_email'], $message['from_name']);
        } elseif (! empty($headers['From'])) { // Use only email address
            $Swift_Message->setFrom($headers['From']);
        }
        if (! empty($headers['Reply-To'])) { // Reply-To:
            $Swift_Message->setReplyTo($headers['Reply-To']);
        }
        if (! empty($headers['Return-Path'])) { // Return-Path:
            $Swift_Message->setReturnPath($headers['Return-Path']);
        }
        if (! empty($headers['Date'])) { // Date:
        ...
        if (! empty($headers)) {	// Write headers to log:
            $headers_string = get_mail_headers($headers);
...
function smtp_connection_test()
{
    ...
    if ($check_smtp_result === true) {	// Success:
    ...
    if ($connection_result === true) { // Success
...
function smtp_email_sending_test()
{
    ...
    if (! $smtp_connection_result) {	// Errors on SMTP connections:
        return $test_mail_messages;
        // Exit here.
    }
    ...
    if ($sending_result === true) {	// Success:
        $test_mail_messages[] = $smtp_mail_sending_log_html . '<b class="green">OK</b>';
 ...
 */
function add_email_tracking($message, $email_ID, $email_key, $params = [])
{
    global $rsc_url;
    ...
    switch ($content_type) {
        case 'text':
            ...
            if ($params['link_click_text']) {
                ...
                $message = preg_replace_callback($re, [$callback, 'callback'], $message);
            }
            break;
        ...
        case 'html':
            if ($params['image_load']) {
                ...
                $message = preg_replace_callback($re, [$callback, 'callback'], $message, 1);
        ...
        switch ($content_type) {
            case 'text':
                ...
                if ($params['link_click_text']) {
                    ...
                    $template_parts[$key]['message'] = preg_replace_callback($re, [$callback, 'callback'], $template_parts[$key]['message']);
                }
                break;
            ...
            case 'html':
                if ($params['image_load']) {
                    ...
                    $re = '/(<img\b.+\bsrc=")([^"]*?)(")/iU';
                    $callback = new EmailTrackingHelper('img', $email_ID, $email_key, 'html');
                    $template_parts[$key]['message'] = preg_replace_callback($re, [$callback, 'callback'], $template_parts[$key]['message'], 1);
                }
...
function emlog_td_receiver($emlog_ID, $emlog_to, $emlog_user_ID)
{
    ...
    if (! empty($emlog_user_ID)) {
        ...
        } else { // could not find user, probably it was deleted
            $deleted_user_note = '( ' . T_('Deleted user') . ' )';
        }
    }

    if (empty($to)) {	// User is not defined
        global $admin_url;
 ...
 * @param array Params
 */
function emails_sent_log_results(&$emails_Results, $params = [])
{
    global $admin_url;
    ...
    if ($params['display_datetime']) {	// Display Date Time column:
        $emails_Results->cols[] = [
            'th' => T_('Date Time'),
    ...
    if ($params['display_receiver']) {	// Display "To"/receiver column:
        $emails_Results->cols[] = [
            'th' => T_('To'),
            'order' => 'emlog_to',
    ...
    if ($params['display_email_campaign']) {	// Display Email campaign column:
        $emails_Results->cols[] = [
    ...
    if ($params['display_last_opened']) {	// Display Last opened column:
        $emails_Results->cols[] = [
    ...
    if ($params['display_last_clicked']) {	// Display Last clicked column:
        $emails_Results->cols[] = [
...
function emret_td_actions($emret_ID, $emret_address)
{
    ...
    if (check_user_perm('emails', 'edit')) {
        $r .= action_icon(T_('Delete this record!'), 'delete', $admin_url . '?ctrl=email&amp;tab=return&amp;action=returned_delete&amp;emret_ID=' . $emret_ID . '&amp;redirect_to=' . rawurlencode(regenerate_url('blog', '', '', '&')) . '&amp;' . url_crumb('email'), '', 1, 0, [
            'onclick' => 'return confirm(\'' . TS_('Are you sure want to delete this record?') . '\');',
        ]);
    }
 ...
 * @param array Params
 */
function email_returns_results(&$email_returns_Results, $params = [])
{
    global $admin_url;
    ...
    if ($params['display_id']) {	// Display Date Time column:
        $email_returns_Results->cols[] = [
            'th' => T_('Date Time'),
    ...
    if ($params['display_address']) {	// Display Address column:
        $email_returns_Results->cols[] = [
            'th' => T_('Address'),
            'order' => 'emret_address',
    ...
    if ($params['display_error_type']) {	// Display Err Type column:
    ...
    if ($params['display_error_msg']) {	// Display Error column:
        $email_returns_Results->cols[] = [
            'th' => T_('Error'),
