...
class EmailTrackingHelper
{
    ...
    public function __construct($url_type, $email_ID, $key, $content_type = 'html', $tag = null)
    {
    ...
    public function get_passthrough_url($params = [])
    {
        $params = array_merge([
    ...
    public function callback($matches)
    {
        ...
        switch ($this->content_type) {
            case 'html':
                ...
                $redirect_url = str_replace(['$mail_log_ID$', '$email_key$'], ['_____mail_log_ID_____', '_____email_key_____'], $redirect_url);
                ...
                } elseif (preg_match($activate_link_re, $redirect_url, $match)) {	// Activate button/link:
                    $passthrough_url = $this->get_passthrough_url([
                        'tag' => 8,
                    ]);
                }

                if (preg_match_all('~(\$secret_content_start\$)(.*?)(\$secret_content_end\$)~', $redirect_url, $secret_contents)) { // Preserve secret content markers
                    ...
                    $redirect_url = str_replace(['_____mail_log_ID_____', '_____email_key_____'], ['$mail_log_ID$', '$email_key$'], $redirect_url);
