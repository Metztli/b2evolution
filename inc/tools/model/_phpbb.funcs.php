...
function phpbb_init_config()
{
    ...
    if (is_array($phpbb_db_config) && count($phpbb_db_config) > 0) {	// Connect to DB when config is already defined
        ...
        if ($allow_install_test_features) {	// Set maximum default values in test mode
...
function phpbb_table_insert_links($table_name, $links)
{
    ...
    foreach ($links as $phpbb_ID => $b2evo_ID) {
        ...
        if ($l == 1000) {	// Insert data by 1000 records
    ...
    if (count($links_data) > 0) {	// Insert the rest records
        $DB->query('INSERT INTO ' . phpbb_table_name($table_name) . ' ( phpbb_ID, b2evo_ID )
					VALUES ' . implode(', ', $links_data));
    }
}
...
function phpbb_table_get_links($table_name)
{
    ...
    if (! is_array($phpbb_cache_links)) {	// Init cache
        $phpbb_cache_links = [];
    }

    if (isset($phpbb_cache_links[$table_name])) {	// The links from this table are already defined
    ...
    if (! empty($table_exists)) {	// Table exists in DB
        $SQL = new SQL();
        $SQL->SELECT('phpbb_ID, b2evo_ID');
        $SQL->FROM(phpbb_table_name($table_name));

        $phpbb_cache_links[$table_name] = $DB->get_assoc($SQL->get());
    } else {	// No table found in DB (it can be happened when we interrupt the processing by refreshing of the page)
        $phpbb_cache_links[$table_name] = [];
    }
...
function phpbb_rank_info($rank_ID, $get_only_count = false)
{
    ...
    if (! isset($rank_users_count)) {	// Init array only first time
        $SQL = new SQL();
    ...
    if ($get_only_count) {	// Return only a count of users of the rank
...
function phpbb_import_users()
{
    ...
    if (! phpbb_check_step('users')) {	// Check current step
    ...
    if (count($phpbb_ranks) > 0) {
        foreach ($phpbb_ranks as $rank_ID => $b2evo_group_ID) {
            if (empty($b2evo_group_ID)) {	// Unset this rank, because it selected as no import
    ...
    if (count($phpbb_ranks) > 0) {	// Limit users by the selected ranks
    ...
    if ($phpbb_version == 3) {	// phpBB v3:
        // Detect what user fields exist in phpBB database:
        $db_user_fields = $phpbb_DB->get_col('SHOW COLUMNS FROM BB_user_fields');
        $sel_fields = ['icq', 'website', 'aim', 'yim', 'msnm', 'interests', 'facebook', 'googleplus', 'location', 'skype', 'twitter', 'yahoo', 'youtube', 'aol'];
        foreach ($sel_fields as $s => $sel_field_key) {
            if (in_array('pf_phpbb_' . $sel_field_key, $db_user_fields)) {	// Try to get a value if user field exists:
                $sel_fields[$sel_field_key] = 'uf.pf_phpbb_' . $sel_field_key;
            } else {	// Select empty value if field isn't detected in DB:
    ...
    do {	// Split by page to optimize process
        ...
        foreach ($phpbb_users as $p => $phpbb_user) {
            if ($p % 100 == 0) {	// Display the processing dots after 100 users
                phpbb_log(' .', 'message', '');
            }

            if ($phpbb_user->user_id < 1) {	// Skip the users with invalid ID
                phpbb_log(sprintf('User "%s" with ID %s ignored', $phpbb_user->username, $phpbb_user->user_id), 'error', ' ', '<br />');
                continue;
            }

            if ($phpbb_user->username == '¥åßßå') {	// Special rule for this username
                $user_login = 'yabba';
            } else {	// Replace unauthorized chars from username
                $user_login = preg_replace('/([^a-z0-9_\-\.]+)/i', '_', $phpbb_user->username);
                $user_login = utf8_substr(utf8_strtolower($user_login), 0, 20);
                if ($user_login == '_') {	// If all username chars are unauthorized
                    $user_login = 'user_1';
                }
            }

            $user_has_duplicated_email = false;
            if (isset($phpbb_emails_duplicated[$phpbb_user->user_email])) {	// The user has the duplicate email
                if (! empty($phpbb_emails_duplicated[$phpbb_user->user_email])) {	// The other user already was imported with such email
                    phpbb_log('<br />' . sprintf(
                        'The phpBB users "%s" and "%s" have the same email address "%s" and will be merged in b2evolution as just "%s"',
                ...
                $phpbb_emails_duplicated[$phpbb_user->user_email] = [
                    'username' => $user_login,
            ...
            if (! empty($b2evo_user)) {	// User already exists in DB of b2evo
            ...
            do {
                ...
                if ($b2evo_user_ID = $DB->get_var($SQL)) {	// Duplicated user login, Change to next login by increasing the number at the end
                    $next_login = $user_login_mask . (++$user_login_number);
                }
            } while ($b2evo_user_ID);
            if ($user_login != $next_login) {	// Duplicated login was changed, Display a message about this event
            ...
            if (! empty($users_ips[$phpbb_user->user_id])) {	// Decode user ip from hex format
            ...
            if ($phpbb_version == 3) {	// phpBB v3:
                if (preg_match('/^(\$[^\$]+)(\$[^\$]+)*\$([^\$]+)$/', $phpbb_user->user_password, $password_data)) {	// Save password value and password driver code in two separate fields:
                ...
                if (! isset($user_data['user_pass'])) {	// Unknown password format:
                    $user_data['user_pass'] = '';
            ...
            if (! empty($phpbb_user->user_rank) && ! empty($phpbb_ranks[$phpbb_user->user_rank])) {	// Define the user's group
                $user_data['user_grp_ID'] = $phpbb_ranks[$phpbb_user->user_rank];
            }
            if (! isset($user_data['user_grp_ID'])) {	// Set default group
            ...
            if (! $DB->get_results()) {	// User was not inserted
...
function phpbb_import_user_fields($phpbb_user, $b2evo_user_ID)
{
    ...
    if (! isset($phpbb_cache_b2evo_users_fields)) {	// Get users fields from b2evo database
        $SQL = new SQL();
    ...
    if (empty($phpbb_cache_b2evo_users_fields)) {	// No users fields
        return;
    }
    ...
    if (empty($fields_types_IDs)) {	// No links between phpBB fields and b2evo fields
        return;
    }

    global $phpbb_cache_phpbb_ranks;

    if (! isset($phpbb_cache_phpbb_ranks)) {	// Get the titles of the user ranks from phpBB
        $SQL = new SQL();
        $SQL->SELECT('rank_id, rank_title');
        $SQL->FROM('BB_ranks');
    ...
    foreach ($fields_types_IDs as $field_type => $field_ID) {
        if (! isset($phpbb_user->{'user_' . $field_type})) {	// Skip this user field because the given phpBB DB doesn't contain it:
            continue;
        }
        $field_value = trim($phpbb_user->{'user_' . $field_type});
        if ($field_type == 'rank') {	// If field is "rank" we should get the value from table "phpbb_ranks" by rank_ID
            $field_value = ! empty($phpbb_cache_phpbb_ranks[$field_value]) ? $phpbb_cache_phpbb_ranks[$field_value] : '';
        }

        if ($field_value != '') {	// field is filled, we can put it into DB
            $import_data[] = '( ' .
    ...
    if (count($import_data) == 0) {	// No data to insert, Exit here
        return;
    }
...
function phpbb_import_user_settings($phpbb_user, $b2evo_user_ID)
{
    ...
    foreach ($settings_links as $phpbb_field => $b2evo_field) {
        if (! isset($phpbb_user->{'user_' . $phpbb_field})) {	// Skip empty value
            continue;
        }
    ...
    if (! empty($import_data)) {	// *** EXECUTE QUERY TO INSERT NEW USERS SETTINGS *** //
...
function phpbb_import_invalid_user($phpbb_user_ID, &$users_IDs, $phpbb_username = '')
{
    ...
    if (empty($group_invalid_ID)) {	// If the invalid group ID is empty it means we shouldn't import the invalid users
        return false;
    }

    if (! isset($users_IDs[(string) $phpbb_user_ID])) {	// If user is not imported yet
        global $DB, $tableprefix;

        $user_email = '';
        if (! empty($phpbb_username)) {	// If username is defined
            $user_login = $phpbb_username;
        } else {
            if ($phpbb_user_ID == '-1') {	// Anonymous user
        ...
        if (empty($b2evo_user_ID)) {	// User doesn't exist in DB of b2evo yet, Insert new user
            $user_data = [
        ...
        phpbb_table_insert_links('users', [
            $phpbb_user_ID => $b2evo_user_ID,
        ]);
...
function phpbb_import_forums()
{
    ...
    if (! phpbb_check_step('forums')) {	// Check current step
        return; // Exit here if we cannot process this step
    }
    ...
    if ($phpbb_version == 3) {	// pnpBB v3:
        $import_forums = array_merge($import_forums, $import_categories);
        if (empty($import_forums)) {	// No selected forums to import
            $phpbb_forums = [];
        } else {	// Get the forums from phpbb database:
            $forum_SQL = new SQL();
        ...
        if (! empty($import_categories)) {	// Select only these categories
            $cat_SQL->WHERE('cat_id IN ( ' . $phpbb_DB->quote($import_categories) . ' )');
        } else {	// If no categories to import
        ...
        if (! empty($import_forums)) {	// Select only these forums
            $forum_SQL->WHERE('forum_id IN ( ' . $phpbb_DB->quote($import_forums) . ' )');
        } else {	// If no forums to import
    ...
    foreach ($phpbb_forums as $p => $phpbb_forum) {
        ...
        if ($phpbb_forum->forum_meta == '1') {	// Category
            $forums_IDs['cat_' . $phpbb_forum->forum_id] = $DB->insert_id;
        } else {	// Forum
            $forums_IDs[$phpbb_forum->forum_id] = $DB->insert_id;
        }

        if (isset($phpbb_forum->cat_id) && $phpbb_forum->cat_id > 0) {	// First level forum has category as parent
            $forums_parents[$phpbb_forum->forum_id] = 'cat_' . $phpbb_forum->cat_id;
        } elseif (isset($phpbb_forum->forum_parent) && $phpbb_forum->forum_parent > 0) {	// Save parent ID to update it in the next step
    ...
    if (count($forums_parents) > 0) {	// Update the parents IDs
...
function phpbb_unique_urlname($source, $table, $field)
{
    ...
    do {	// Check for unique url name in DB
        $SQL = new SQL('phpBB: Get the unique url name');
        $SQL->SELECT($field);
        $SQL->FROM($table);
        $SQL->WHERE($field . ' = ' . $DB->quote($url_name_correct));
        $row = $DB->get_var($SQL);
        if ($row) {	// Row already exists with such field; Make it unique:
...
function phpbb_import_topics()
{
    ...
    if (! phpbb_check_step('topics')) {	// Check current step
        return; // Exit here if we cannot process this step
    }
    ...
    do {	// Split by page to optimize process
        ...
        foreach ($phpbb_topics as $p => $phpbb_topic) {
            if (! isset($forums_IDs[(string) $phpbb_topic->forum_id])) {	// The topic has the incorrect forum's ID by some reason
            ...
            if (! isset($users_IDs[(string) $phpbb_topic->topic_poster])) {	// The topic has the incorrect user's ID by some reason
                if (! phpbb_import_invalid_user($phpbb_topic->topic_poster, $users_IDs, $phpbb_topic->post_username)) {	// We cannot create invalid user
            ...
            if ($phpbb_topic->topic_status == '1') {	// If topic is closed set comment status - closed
                $post_comment_status = 'closed';
            } elseif ($phpbb_topic->topic_type == '2') {	// If topic is "Announcement" set comment status - disabled
                $post_comment_status = 'disabled';
            } else {	// Comment status is open for all other topics
            ...
            if ($path_attachments) {	// Import attachments of this topic:
                phpbb_import_attachments('itm', $path_attachments, $phpbb_topic->post_id, $item_ID);
            }

            $phpbb_topics_count_imported++;

            if ($phpbb_topics_count_imported % 100 == 0) {	// Display the processing dots after 500 topics
                phpbb_log(' .', 'message', '');
            }
        }
    ...
    if (empty($path_attachments)) {	// Count missing attachments of the imported topics if the path is not correct:
...
function phpbb_decode_bbcode($string, $bbcode_uid)
{
    ...
    ****/
    ...
    if (strpos($string, '[') !== false) {	// Replace complex bbcoded tags
        $bbcode_patterns = [
            '/\[url\]([^\1]+?)\[\/url\]/si',
            '/\[quote:([a-z0-9]+)="([^"]+)"\]/si',
...
function phpbb_import_replies()
{
    ...
    if (! phpbb_check_step('replies')) {	// Check current step
        return; // Exit here if we cannot process this step
    }
    ...
    if (! empty($import_forums)) {	// Select the replies only from these forums
    ...
    $comments_import_data = [];
    $comments_slugs_import_data = [];
    $replies_IDs = [];
    do {	// Split by page to optimize process
        ...
        foreach ($phpbb_replies as $p => $phpbb_reply) {
            if (phpbb_post_is_topic($phpbb_reply->post_id)) {	// This post is a content of the topic
            ...
            if (! isset($topics_IDs[(string) $phpbb_reply->topic_id])) {	// The reply has the incorrect topic's ID by some reason
            ...
            if ($phpbb_reply->poster_id == '-1') {	// Comment from anonymous user
                $author_ID = 'NULL';
                $author_name = $DB->quote($phpbb_reply->post_username);
            } elseif (isset($users_IDs[(string) $phpbb_reply->poster_id])) {	// Author is registered user
                $author_ID = $DB->quote($users_IDs[(string) $phpbb_reply->poster_id]);
                $author_name = 'NULL';
            } else {	// Incorrect user ID, Do NOT import this reply
                if (! phpbb_import_invalid_user($phpbb_reply->poster_id, $users_IDs, $phpbb_reply->post_username)) {	// We cannot create invalid user
            ...
            if (count($comments_import_data) == 100) {	// Insert the 100 comments in one query:
            ...
            if ($phpbb_replies_count_imported % 1000 == 0) {	// Display the processing dots after 1000 topics
                phpbb_log(' .', 'message', '');
            }
        }
    ...
    if (count($comments_import_data) > 0) {	// Insert the rest comments:
        phpbb_insert_comments($comments_import_data, $comments_slugs_import_data);
    }
    ...
    if (empty($path_attachments)) {	// Count missing attachments of the imported replies if the path is not correct:
...
function phpbb_insert_comments($comments_import_data, $comments_slugs_import_data)
{
    ...
    if (! $comment_insert_result) {	// Some errors
        phpbb_log('<br />' . sprintf('MySQL error: %s.', $DB->last_error), 'error', ' ');
    } else {
        // Get a path where we should import the reply attachments from:
        $path_attachments = phpbb_get_import_path('path_attachments');

        if ($path_attachments) {	// If attachments should be inserted
            ...
            $attachments_insert_queries = [];
            $c = 0;
            foreach ($comments_import_data as $reply_ID => $insert_data) {	// Import attachments of the last 100 replies:
...
function phpbb_post_is_topic($post_id)
{
    ...
    if (isset($phpbb_cache_topics_posts)) {	// Get result from cache
...
function phpbb_import_messages()
{
    ...
    if (! phpbb_check_step('messages')) {	// Check current step
        return; // Exit here if we cannot process this step
    }
    ...
    do {	// Split by page to optimize process
        ...
        foreach ($phpbb_messages as $message) {
            if (! isset($users_IDs[(string) $message->from_user_id])) {	// The message has the incorrect user's ID by some reason
            ...
            if (! isset($users_IDs[(string) $message->to_user_id])) {	// The message has the incorrect user's ID by some reason
                $phpbb_missing_users++;
            ...
            if ($phpbb_messages_count_imported % 1000 == 0) {	// Display the processing dots after 1000 topics
                phpbb_log(' .', 'message', '');
            }
        }
...
function phpbb_import_messages_texts($thread_ID, $message)
{
    ...
    $phpbb_messages = array_merge($phpbb_messages, [$message]);

    $users_IDs = phpbb_table_get_links('users');

    $message_import_data = [];
    $threadstatus_import_data = [];
    foreach ($phpbb_messages as $message) {
        if (! isset($users_IDs[$message->from_user_id]) || ! isset($users_IDs[$message->to_user_id])) {	// No users
            continue;
        }
    ...
    if ($path_attachments) {	// If attachments should be inserted
        ...
        $m = 0;
        foreach ($message_import_data as $message_id => $insert_data) {	// Import attachments of the inserted private message:
            phpbb_import_attachments('msg', $path_attachments, $message_id, $last_inserted_msg_IDs[$m]);
            $m++;
        }
    } else {	// Count missing attachments of the imported messages if the path is not correct:
...
function phpbb_forums_list(&$Form)
{
    ...
    foreach ($categories as $category) {
        $Form->checkbox_input('phpbb_categories[]', ! is_array($import_categories) || in_array($category->cat_id, $import_categories), '', [
...
function phpbb_subforums_list(&$Form, $cat_id, $forum_parent_id = 0)
{
    ...
    if ($phpbb_version == 3) {	// pnpBB v3:
        ...
        if ($cat_id > 0) {	// Get all top forums of the category:
        ...
        } else {	// Wrong a call of this function
            return;
        }
    ...
    $import_forums = phpbb_get_var('import_forums');
    foreach ($forums as $forum) {	// Display forums
        $Form->checkbox_input('phpbb_forums[]', ! is_array($import_forums) || in_array($forum->forum_id, $import_forums), '', [
...
function phpbb_get_config($name)
{
    ...
    if (! is_array($phpbb_config)) {	// Initialize config array only first time:
        $phpbb_config = [];
    }

    if (! isset($phpbb_config[$name])) {	// Get value only first time and store in cache array:
...
function phpbb_get_import_path($path_var_name)
{
    ...
    if (! empty($path_avatars)) {
        ...
        if (! empty($path_avatars) && file_exists($path_avatars) && is_dir($path_avatars)) {	// Folder with files is correct, we can import them:
            return $path_avatars;
        }
    }
...
function phpbb_import_avatar($phpbb_user, $user_ID, $path_avatars)
{
    ...
    if (empty($user_avatar)) {	// User has no avatar, Don't try to import:
        return true;
    }

    if (empty($path_avatars)) {	// Only count what missing avatars if the path is not correct:
        phpbb_log(sprintf(
            'Impossible to copy avatar file of the user #%s(%s) because the source for avatars %s is wrong.',
            $phpbb_user->user_id,
            $phpbb_user->username . ' / ' . $phpbb_user->user_email,
            '<code>' . phpbb_get_var('path_avatars') . '</code>'
        ), 'error', ' ', '<br />');
    ...
    if ($phpbb_version == 3) {	// phpBB v3:
        if (preg_match('#^https?://#', $phpbb_user->user_avatar)) {	// The avatar file is loaded from remoter url:
            $avatar_file_type = 'url';
            $avatar_file_url = $phpbb_user->user_avatar;
        } else {	// The avatar file is located on the disk:
    ...
    if ($avatar_file_type == 'url') {	// Try to get a file from remote url:
        $avatar_file_content = fetch_remote_page($avatar_file_url, $avatar_file_url_info);
        if ($avatar_file_content === false) {	// Some server restriction to get files from remote server:
            $avatar_file_url_error = ' ' . sprintf('Error: %s', $avatar_file_url_info['message'])
                . '; ' . sprintf('Status code: %s', $avatar_file_url_info['status']);
            phpbb_log(sprintf(
                'Impossible to get avatar file of the user #%s(%s) because your server has a restriction to get avatar file from remote url %s.',
                $phpbb_user->user_id,
                $phpbb_user->username . ' / ' . $phpbb_user->user_email,
                '<code>' . $avatar_file_url . '</code>'
            ) . $avatar_file_url_error, 'error', ' ', '<br />');
        ...
        if (! file_exists($avatar_file_path)) {	// Display an error if avatar file is not found on the disk:
            phpbb_log(sprintf(
                'Avatar file of the user #%s(%s) is not found on %s.',
                $phpbb_user->user_id,
                $phpbb_user->username . ' / ' . $phpbb_user->user_email,
                '<code>' . $avatar_file_path . '</code>'
            ), 'error', ' ', '<br />');
            // Update the count of missing avatars:
            phpbb_set_var('avatars_count_missing', phpbb_get_var('avatars_count_missing') + 1);
            return false;
    ...
    if ($avatar_file_type == 'url') {	// Create avatar file from remote url:
        $avatar_file_name = phpbb_get_config('avatar_salt') . '_' . $phpbb_user->user_id . '.' . $avatar_extension;
        $user_File = new File('user', $user_ID, 'profile_pictures/' . $avatar_file_name);
        if (! $user_File->create() ||
            ! ($avatar_file_handle = fopen($user_File->_adfp_full_path, 'w'))) {	// Impossible to create new file:
            phpbb_log(sprintf(
                'Impossible to create new avatar file of the user #%s(%s) from the remote url %s on %s.',
                $phpbb_user->user_id,
                $phpbb_user->username . ' / ' . $phpbb_user->user_email,
                '<code>' . $avatar_file_url . '</code>',
                '<code>' . $user_File->_adfp_full_path . '</code>'
            ), 'error', ' ', '<br />');
            // Update the count of missing avatars:
            phpbb_set_var('avatars_count_missing', phpbb_get_var('avatars_count_missing') + 1);
            return false;
        ...
        if (empty($imported_file_ID)) {	// Display an error if there is some error on copying the avatar file:
            phpbb_log(sprintf(
                'Impossible to save avatar file of the user #%s(%s) from the remote url %s in DB.',
                $phpbb_user->user_id,
                $phpbb_user->username . ' / ' . $phpbb_user->user_email,
                '<code>' . $avatar_file_url . '</code>'
            ), 'error', ' ', '<br />');
    ...
    } else {	// Copy avatar file to user media folder:
        $imported_file_ID = copy_file($avatar_file_path, $user_FileRoot->ID, 'profile_pictures', false);

        if (empty($imported_file_ID)) {	// Display an error if there is some error on copying the avatar file:
            phpbb_log(sprintf(
                'Impossible to copy avatar file of the user #%s(%s) from %s to %s, please check file rights of the files and folders.',
                $phpbb_user->user_id,
                $phpbb_user->username . ' / ' . $phpbb_user->user_email,
                '<code>' . $avatar_file_path . '</code>',
                '<code>' . $user_FileRoot->ads_path . '</code>'
            ), 'error', ' ', '<br />');
            // Update the count of missing avatars:
            phpbb_set_var('avatars_count_missing', phpbb_get_var('avatars_count_missing') + 1);
            return false;
    ...
    if ($result) {	// Insert a link with new file:
        global $localtimenow;
    ...
    if ($result) {	// Update the count of imported avatars:
        ...
        phpbb_log(sprintf(
            'Error on DB storing record of the importing avatar file of the user #%s(%s) from %s (Error: %s).',
            $phpbb_user->user_id,
            $phpbb_user->username . ' / ' . $phpbb_user->user_email,
            '<code>' . (isset($avatar_file_path) ? $avatar_file_path : $avatar_file_url) . '</code>',
            '<code>' . $DB->last_error . '</code>'
        ), 'error', ' ', '<br />');
        // Update the count of missing avatars:
        phpbb_set_var('avatars_count_missing', phpbb_get_var('avatars_count_missing') + 1);
        return false;
...
{
    if (empty($path_attachments)) {	// Only count what missing attachments if the path is not correct:
        phpbb_count_missing_attachments($target_ID);
        // Display an error to inform user about missing attachments:
        phpbb_log(sprintf(
            'Impossible to copy attachment of #%s because the source for attachments %s is wrong.',
            $target_ID,
            '<code>' . phpbb_get_var('path_attachments') . '</code>'
        ), 'error', ' ', '<br />');
        phpbb_set_var('avatars_count_missing', phpbb_get_var('avatars_count_missing') + 1);
        return false;
    } else {	// Do the import if the path is correct:
...
function phpbb_count_missing_attachments($target_IDs)
{
    ...
    if (empty($target_IDs)) {	// No targets, Don't count:
        return;
    }
...
function phpbb_insert_attachments($target_type, $attachments_insert_data)
{
    ...
    if (empty($attachments_insert_data)) {	// Nothing to insert, Exit here:
        return true;
    }
    ...
    if ($r) {	// Increase a count of the imported attachments:
        $attachments_count_imported += $DB->affected_rows;
    } else {	// Some mysql error:
        $attachments_count_missing += count($attachments_insert_data);
        // Note: do NOT translate the string below, because such error must not occurs on correct DB working, so it is system error and only admin can solve this:
        phpbb_log(sprintf(
            'Error on DB storing record of the importing attachment of "%s" (Error: %s, Data: %s).',
            $target_type,
            '<code>' . $DB->last_error . '</code>',
            '<code>' . implode(', ', $attachments_insert_data) . '</code>'
        ), 'error', ' ', '<br />');
    }
...
function phpbb_get_attachments_insert_data($target_type, $path_attachments, $target_ID, $new_object_ID)
{
    ...
    if ($phpbb_version != 3) {	// This function is only for phpBB3:
        return [];
    }
    ...
    if (empty($attachments)) {	// This target has no attachment:
        return [];
    }
    ...
    $attachments_insert_data = [];

    $link_order = 1;
    foreach ($attachments as $attachment) {
        if (! file_exists($path_attachments . $attachment->physical_filename)) {	// If physical file(like "2_1888733359c9d121321b5cfbe93ba714") doesn't exist then try to use file with real name(like "image.jpg")
            if (! file_exists($path_attachments . $attachment->real_filename)) {	// The file with real name doesn't exist too, Skip this attachment:
                $attachments_count_missing++;
                // Display an error to inform user about the missing attachment:
                phpbb_log(sprintf(
                    'Attachment of the object #%s(%s) is not found on %s or %s.',
                    $target_ID,
                    $target_type,
                    '<code>' . $path_attachments . $attachment->physical_filename . '</code>',
                    '<code>' . $path_attachments . $attachment->real_filename . '</code>'
                ), 'error', ' ', '<br />');
                continue;
            } else {	// Use file with real name instead of physical:
                $attachment->physical_filename = $attachment->real_filename;
            }
        }

        if (! $users_IDs[(string) $attachment->poster_id]) {	// Wrong file author:
            $attachments_count_missing++;
            // Display an error to inform user about the missing attachment:
            phpbb_log(sprintf(
                'Impossible to import attachment of the object #%s(%s) because file author could not be detected by id #%s.',
                $target_ID,
                $target_type,
                $attachment->poster_id
            ), 'error', ' ', '<br />');
            continue;
        }
        ...
        $FileRootCache = &get_FileRootCache();
        if ($target_type == 'msg') {	// Get root for private messages:
        ...
        } else {	// Get root for topics and replies:
        ...
        if ($attachment->physical_filename != $attachment->real_filename &&
            ! @rename($path_attachments . $attachment->physical_filename, $path_attachments . $attachment->real_filename)) {	// Impossible to rename file from phpBB format like "2_1df806e219313f4432b82040685b8ff1" to real file name with extension, Skip it:
            $attachments_count_missing++;
            // Display an error to inform user about the missing attachment:
            phpbb_log(sprintf(
                'Impossible to rename attachment of the object #%s(%s) from physical file name %s to real %s.',
                $target_ID,
                $target_type,
                '<code>' . $path_attachments . $attachment->physical_filename . '</code>',
                '<code>' . $path_attachments . $attachment->real_filename . '</code>'
        ...
        if (empty($imported_file_ID)) {	// Impossible to copy the file:
            $attachments_count_missing++;
            // Display an error to inform user about the missing attachment:
            phpbb_log(sprintf(
                'Impossible to copy attachment of the object #%s(%s) from %s to the folder %s, please check system log and file rights of the files and folders.',
                $target_ID,
                $target_type,
                '<code>' . $path_attachments . $attachment->real_filename . '</code>',
                '<code>' . $object_FileRoot->ads_path . $root_path . '</code>'
            ), 'error', ' ', '<br />');
        }

        if ($attachment->physical_filename != $attachment->real_filename) {	// Rename file back from real name to physical name like 2_1888733359c9d121321b5cfbe93ba714,
        ...
        if (empty($imported_file_ID)) {	// Skip this if file could not be copied,
...
function phpbb_check_step($step_name)
{
    ...
    if (empty($steps_levels[$step_name])) {	// Invalid step name!
        return false;
    }

    $current_step = phpbb_get_var('current_step');

    if (empty($current_step)) {	// It is first step
        phpbb_set_var('current_step', $step_name, true);
        return true;
    } elseif (empty($steps_levels[$current_step])) {	// Invalid current step name!
        return false;
    }/*
