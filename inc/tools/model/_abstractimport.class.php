...
class AbstractImport
{
    ...
    {	// TODO: Factorize with start_install_log():
        ...
        if (! mkdir_r($media_path . 'import/logs/')) {	// Display error if folder cannot be created for log files:
            $this->display_log_file_error('Cannot create the folder <code>' . $media_path . 'import/logs/</code> for log files!');
            return false;
        }

        if (! ($this->log_file_handle = fopen($log_file_path, 'w'))) {	// Display error if the log fiel cannot be created in the log folder:
     ...
     */
    public function display_log_file_error($message)
    {
        if (empty($this->log_file_error_reported)) {	// Report only first detected error to avoid next duplicated errors on screen:
    ...
    {	// TODO: Factorize with end_install_log():
        ...
        if (isset($this->log_file_handle) && $this->log_file_handle) {	// Close the log file:
     ...
     */
    public function get_log($message, $type = null, $type_html_tag = 'p', $display_label = true)
     ...
     * @param string HTML tag for type/styled log: 'p', 'span', 'b', etc.
     */
    public function log_list($message, $type = 'text', $type_html_tag = 'li')
    {
     ...
     */
    public function log_inside($wrapper_type)
    {
        if (in_array($wrapper_type, $this->log_wrappers)) {	// The requested wrapper is already opened,
     ...
     */
    public function close_log_wrapper()
    {
        if (empty($this->log_wrappers)) {	// No opened wrappers
            return false;
        }

        // Get last wrapper:
        if (! ($last_wrapper_type = array_pop($this->log_wrappers))) {	// No found last wrapper
            return false;
        }
     ...
     */
    public function close_log_wrappers()
    {
    ...
    {	// TODO: Factorize with prepare_install_log_message():
        ...
        if ($display_label && $type == 'error') {	// Count a number of errors:
            $this->log_errors_num++;
        }
     ...
     * @param boolean TRUE to display label
     */
    public function log_success($message, $type_html_tag = 'p', $display_label = true)
    {
     ...
     * @param boolean TRUE to display label
     */
    public function log_error($message, $type_html_tag = 'p', $display_label = true)
    {
     ...
     * @param boolean TRUE to display label
     */
    public function log_warning($message, $type_html_tag = 'p', $display_label = true)
    {
     ...
     * @param boolean TRUE to display label
     */
    public function log_info($message, $type_html_tag = 'p', $display_label = true)
    {
    ...
    {	// TODO: Factorize with install_log_to_file():
        if (! $this->log_file) {	// Don't log into file:
            return;
        }

        if (! isset($this->log_file_handle) || ! $this->log_file_handle) {	// Log must be started:
     ...
     * @param string Message
     */
    public function log_to_screen($message)
    {
        global $is_cli;

        if ($is_cli && $this->log_cli_format == 'text') {	// Remove and convert all html tags to text format on CLI mode:
            ...
            $message = preg_replace_callback('#<(/?[a-z\d]+)[^>]*>([\r\n]*)#is', [$this, 'log_to_screen_cli_callback'], $message);
     ...
     * @param array Matches
     */
    public function log_to_screen_cli_callback($m)
    {
        switch ($m[1]) {
            case 'br':
                return '';
     ...
     */
    public function check_manifest($path)
    {
        $manifest_path = rtrim($path, '/') . '/manifest.yaml';

        if (file_exists($manifest_path)) {	// Manifest file is detected:
            ...
            if (empty($manifest) || ! is_array($manifest)) {	// Wrong or empty manifest, Don't restrict import:
                return true;
            }
                ...
                switch ($manifest_rule) {
                    case 'collection-urlname':
                        ...
                        if (($import_Blog = &$this->get_Blog()) &&
                            $import_Blog->get('urlname') != $manifest_value) {	// Stop import:
                    ...
                    case 'collection-locale':
                        ...
                        if (($import_Blog = &$this->get_Blog()) &&
                            ! $import_Blog->has_locale($manifest_value)) {	// Stop import:
                    ...
                    case 'import-mode':
                        if ($this->get_option('import_type') != $manifest_value) {	// Stop import because currently selected import mode is defferent than rule from manifest file:
        ...
        } else {	// Display warning when no manifest bu don't stop import:
