...
function system_check_dir($directory = 'media', $relative_path = null)
{
    ...
    if ($directory == 'cache' && $relative_path != null) { // Create .htaccess file with deny rules
...
function system_get_result($check_dir_code, $before_msg = '')
{
    ...
    return [$status, $before_msg . $system_results[$check_dir_code]];
}
...
function system_create_cache_folder($verbose = false)
{
    ...
    if ($cache_path == $basepath . '_cache/') { // Cache path is the default one
        if (! is_dir($cache_path) && is_dir($basepath . 'cache/')) { // The default cache folder doesn't exists yet, but the old default does exist
            if (@rename($basepath . 'cache/', $cache_path)) { // Successful rename
                if ($verbose) { // Display successefully renamed message
    ...
    if (system_check_dir('cache') > 0) {
        if ($verbose) { // The cache folder is not readable/writable or doesn't exist
...
function system_get_blog_IDs($only_cache_enabled)
{
    ...
    if (! isset($Settings)) {	// Call from install script:
        return [];
    }
...
function system_check_caches($repair = true)
{
    ...
    if ($result > 0) { // error with cache/ folder
        $failed = true;
        if ($repair && ($result == 2)) { // if cache folder not exists, and should repair, then try to create it
            $failed = ($failed && ! system_create_cache_folder());
        }
        if ($failed) { // could/should not repair
    ...
    $error_messages = [];
    if (($result = system_check_blog_cache(null, $repair)) !== false) { // general cache folder should exists
    ...
    $BlogCache = &get_BlogCache();
    foreach ($cache_enabled_blogs as $blog_ID) { // blog's cache folder should exists
...
function system_init_caches($verbose = false, $force_enable = true)
{
    ...
    if (! system_create_cache_folder($verbose)) { // The error message were displayed
        task_end('');
        return false;
    }
    task_end();
    ...
    if ($force_enable) {
        ...
        if (! is_object($Settings)) { // create Settings object
...
function system_check_process_user()
{
    ...
    if (function_exists('posix_geteuid')) {
        ...
        $running_as = sprintf(
            ...
            (! is_null($process_uid) ? $process_uid : '?')
        );
    } else {
        $running_as = '(' . T_('Unknown') . ')';
    }

    return [$process_uid, $process_user, $running_as];
}
...
function system_check_process_group()
{
    ...
    if (function_exists('posix_getegid')) {
        ...
        $running_as = sprintf(
            ...
            (! is_null($process_gid) ? $process_gid : '?')
        );
    } else {
        $running_as = '(' . T_('Unknown') . ')';
    }

    return [$process_gid, $process_group, $running_as];
}
...
{
    if ((string) intval($php_ini_value) === (string) $php_ini_value) { // Bytes
        return $php_ini_value;
    } elseif (strpos($php_ini_value, 'K') !== false) { // Kilobytes
        return intval($php_ini_value) * 1024;
    } elseif (strpos($php_ini_value, 'M') !== false) { // Megabytes
        return intval($php_ini_value) * 1024 * 1024;
    } elseif (strpos($php_ini_value, 'G') !== false) { // Gigabytes
...
function system_check_api_url($title, $url)
{
    ...
    if ($ajax_response !== false) {	// Response is correct data:
        init_system_check($title, 'OK', $url);
        disp_system_check('ok');
    } else {	// Response is not correct data:
...
function display_system_check($params)
{
    ...
    if ($params['check_version'] && b2evonet_get_updates(true) !== null) {	// Updates are allowed, display them:
        // Display info & error messages
        $Messages->display();

        $version_status_msg = $global_Cache->getx('version_status_msg');
        if (! empty($version_status_msg)) {	// We have managed to get updates (right now or in the past):
    ...
    if (! $params['check_version']) {	// Version was not checked above,
    ...
    } else {	// Display error when no access to check version:
        ...
        if ($app_age > 12) {
            $msg .= '<p>' . sprintf(
                T_('Furthermore, this version is old. You should check for newer releases on %s.'),
                '<a href="http://b2evolution.net/downloads/">b2evolution.net</a>'
            ) . '</p>';
        } elseif ($app_age > 6) {
            $msg .= '<p>' . sprintf(
                T_('Furthermore, this version is aging. You may want to check for newer releases on %s.'),
                '<a href="http://b2evolution.net/downloads/">b2evolution.net</a>'
            ) . '</p>';
        }
    ...
    if (! file_exists($htaccess_path)) {	// No .htaccess
        disp_system_check('error', T_('Error') . ': ' . sprintf(T_('%s has not been found.'), '<code>.htaccess</code>'));
    } elseif (! file_exists($sample_htaccess_path)) {	// No sample.htaccess
        disp_system_check('warning', T_('Warning') . ': ' . sprintf(T_('%s has not been found.'), '<code>sample.htaccess</code>'));
    } elseif (trim(file_get_contents($htaccess_path)) != trim(file_get_contents($sample_htaccess_path))) {	// Different .htaccess
    ...
    if ($cachedir_status != 'error') { // 'cache/ directory exists and, it is writable
        ...
        disp_system_check('note');
        if (count($error_messages) > 0) { // show errors
    ...
    $phpinfo_link = action_icon(T_('View PHP info'), 'info', $phpinfo_url, '', 5, '', [
        'target' => '_blank',
        'onclick' => 'return pop_up_window( \'' . $phpinfo_url . '\', \'phpinfo\', 650 )',
    ]);
    ...
    if (empty($max_execution_time)) {
        ...
        disp_system_check('ok');
    } else {	// Time is limited, can we request more?:
    ...
    if ($params['mode'] == 'backoffice') {
        ...
        $api_result = false;
        if ($json_response !== false) {	// Try to decode REST API json data:
            $decoded_response = @json_decode($json_response);
            $api_result = ! empty($decoded_response);
        }
        if ($api_result) {	// Response is correct json data:
            init_system_check($api_title, 'OK', $api_url);
            disp_system_check('ok');
        } else {	// Response is not json data:
            if (! empty($api_info['error'])) {	// Display error message from function fetch_remote_page():
                $api_error = ' <b>' . sprintf(T_('Error: %s'), $api_info['error']) . '; ' . sprintf(T_('Status code: %s'), $api_info['status']) . '</b>';
            } else {	// Display error message from other places:
        ...
        if (defined('CANUSEXMLRPC') && CANUSEXMLRPC === true) {	// Try XML-RPC API only if current server can use it:
        ...
        } else {	// Get an error message if current server cannot use XML-RPC client:
            $result = false;
            $xmlrpc_error_message = CANUSEXMLRPC;
            $api_error_type = T_('This server cannot use XML-RPC client.');
        }
        if ($result && ! $result->faultCode()) {	// XML-RPC request is successful:
            init_system_check($api_title, 'OK', $api_url);
            disp_system_check('ok');
        } else {	// Some error on XML-RPC request:
            if ($result) {	// Get an error message of XML-RPC request:
                $xmlrpc_error_message = $result->faultString();
            }
            if ($xmlrpc_error_message == 'XML-RPC services are disabled on this system.') {	// Exception for this error:
