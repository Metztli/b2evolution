...
function wpxml_get_import_data(&$XML_file_path, $allow_use_extracted_folder = false)
{
    ...
    if (preg_match('/\.(xml|txt)$/i', $XML_file_name)) {	// XML format
        // Check WordPress XML file:
        wpxml_check_xml_file($XML_file_path);
    } elseif (preg_match('/\.zip$/i', $XML_file_name)) {	// ZIP format
        ...
        if (! $allow_use_extracted_folder && $zip_folder_exists) {	// Don't try to extract into already existing folder:
            $error_type = 'folder_exists';
            echo '<p class="text-danger">' . sprintf(
                'The destination folder %s already exists. If you want to unzip %s again, delete %s first.',
                '<code>' . $ZIP_folder_path . '/</code>',
                '<code>' . $XML_file_name . '</code>',
                '<code>' . $ZIP_folder_path . '/</code>'
            ) . '</p>';
        } elseif (($allow_use_extracted_folder && $zip_folder_exists) ||
                unpack_archive($XML_file_path, $ZIP_folder_path, true, $XML_file_name)) {	// If we can use already extracted ZIP archive or it is unpacked successfully now:
            ...
            for ($i = 1; $i <= 2; $i++) {	// Run searcher 1st time to find XML file in a root of ZIP archive
                // and 2nd time to find XML file in 1 level subfolders of the root:
                foreach ($ZIP_files_list as $ZIP_file) {
                    if ($ZIP_file == '.' || $ZIP_file == '..') {	// Skip reserved dir names of the current path:
                        continue;
                    }
                    if ($i == 2) {	// This is 2nd time to find XML file in 1 level subfolders of the root:
                        if (is_dir($ZIP_folder_path . '/' . $ZIP_file)) {	// This is a subfolder, Scan it to find XML files inside:
                            $ZIP_folder_current_path = $ZIP_folder_path . '/' . $ZIP_file;
                            $ZIP_folder_files = scandir($ZIP_folder_current_path);
                        } else {	// Skip files:
                            continue;
                        }
                    } else {	// This is a single file or folder:
                        $ZIP_folder_files = [$ZIP_file];
                        $ZIP_folder_current_path = $ZIP_folder_path;
                    }
                    foreach ($ZIP_folder_files as $ZIP_file) {
                        if (preg_match('/\.(xml|txt)$/i', $ZIP_file)) {	// XML file is found in ZIP package:
                            $XML_file_path = $ZIP_folder_current_path . '/' . $ZIP_file;
                            if (wpxml_check_xml_file($XML_file_path)) {	// XML file is valid:
    ...
    if ($XML_file_path) {	// Get a path with attached files for the XML file:
    ...
    } else {	// Wrong source file:
        $attached_files_path = false;
        $attached_files_folder = false;
    }

    if (isset($xml_exists_in_zip) && $xml_exists_in_zip === false && file_exists($ZIP_folder_path)) {	// No XML is detected in ZIP package:
        $error_type = 'no_xml';
    ...
    if (isset($wpxml_file_path_renamed)) {	// Display these messages only after $errors = ob_get_clean(); because they are not errors of XML parsing:
        if ($wpxml_file_path_renamed === true) {	// Success renaming:
            echo '<p class="text-warning"><span class="label label-warning">WARNING</span> ' . sprintf(
                'We renamed %s to %s. The PHP XML parser doesn\'t support spaces in filenames.',
...
function wpxml_get_xml_from_file(&$file_path)
{
    ...
    if (strpos($file_path, ' ') !== false) {	// Try to rename file:
        $wpxml_file_path_orig = $file_path;
        $new_file_path = str_replace(' ', '-', $file_path);

        if (! file_exists($new_file_path)) {	// If file was not renamed yet:
            if ($wpxml_file_path_renamed = @rename($file_path, $new_file_path)) {
                $file_path = $new_file_path;
            } else {	// Failed renaming:
                echo '<p class="text-warning"><span class="label label-danger">ERROR</span> ' . sprintf(
                    'Cannot rename %s to %s. The PHP XML parser doesn’t support spaces in filenames.',
                    '<code>' . $file_path . '</code>',
                    '<code>' . $new_file_path . '</code>'
                ) . '</p>';
            }
        } else {
            $file_path = $new_file_path;
    ...
    if (! isset($wpxml_file_path_renamed) || $wpxml_file_path_renamed === true) {	// Load XML content from file with xmlutf8 filter:
...
function wpxml_parser($file)
{
    ...
    if ($evo_charset != 'utf-8' && (strpos($language, 'utf8') !== false)) { // We should convert the text values from utf8 to latin1
        $xml_import_convert_to_latin = true;
    } else { // Don't convert, it is correct encoding
    ...
    if (is_array($authors_data)) {
        foreach ($authors_data as $author_arr) {
            ...
            foreach ($ae->link as $link) {	// Get the links:
                $author['links'][] = [
    ...
    foreach ($xml->channel->item as $item) {
        ...
        if (isset($evo->post_extra_urltitle)) {	// Extra slugs:
        ...
        if (isset($evo->custom_field)) {	// Parse values of custom fields of the Item:
            foreach ($evo->custom_field as $custom_field) {
                $custom_field_attrs = $custom_field->attributes();
                $post['custom_fields'][wpxml_convert_value($custom_field_attrs->name)] = [
        ...
        foreach ($evo->link as $link) { // Get the links
            $post['links'][] = [
                'link_ID' => (int) $link->link_ID,
                'link_datecreated' => (string) $link->link_datecreated,
                'link_datemodified' => (string) $link->link_datemodified,
...
function wpxml_check_xml_file(&$file, $halt = false)
{
    ...
    if (isset($wpxml_file_path_renamed) && $wpxml_file_path_renamed === false) {	// Don't display errors messages here, because they arready were displayed inside wpxml_get_xml_from_file():
        return false;
    }

    if (! $xml) {	// Halt/Display if loading produces an error:
        $errors = [];
        if ($halt) {	// Halt on error:
    ...
    $r = false;
    if ($wxr_version = $xml->xpath('/rss/channel/wp:wxr_version')) {	// Check WXR version for correct format:
        $wxr_version = (string) trim($wxr_version[0]);
        $r = preg_match('/^\d+\.\d+$/', $wxr_version);
    } elseif ($app_version = $xml->xpath('/rss/channel/evo:app_version')) {	// Check application version for correct format:
        $app_version = (string) trim($app_version[0]);
        $r = preg_match('/^[\d\.]+(-[a-z]+)?$/i', $app_version);
    }

    if (! $r) {	// If file format is wrong:
        if ($halt) {	// Halt on error:
...
function wp_unique_urlname($source, $table, $field)
{
    ...
    do {	// Check for unique url name in DB
        ...
        if ($category) {	// Category already exists with such url name; Change it
            ...
            if (! empty($region)) {	// Get region ID from DB by name
                ...
                if ($Region = &$RegionCache->get_by_name($region, false)) {
                    if ($Region->ctry_ID == $data['country']) {
                        ...
                        if (! empty($city)) {	// Get city ID from DB by name
...
function wpxml_convert_value($value)
{
    ...
    if ($xml_import_convert_to_latin) { // We should convert a value from utf8 to latin1
        if (function_exists('iconv')) { // Convert by iconv extenssion
            $value = iconv('utf-8', 'iso-8859-1', $value);
        } elseif (function_exists('mb_convert_encoding')) { // Convert by mb extenssion
...
function wpxml_item_types_selector($XML_file_path, $ZIP_folder_path = null)
{
    ...
    $item_type_usages = [];
    $no_item_types = 0;
    if (! empty($xml_data['posts'])) {	// Count items number per item type:
        foreach ($xml_data['posts'] as $post) {
            if ($post['post_type'] == 'attachment' || $post['post_type'] == 'revision') {	// Skip reserved post type:
                continue;
            }

            if (! empty($post['itemtype'])) {	// Use evo field Item Type name as first priority:
                if (! isset($item_type_names[$post['itemtype']])) {
                    $item_type_names[$post['itemtype']] = 1;
                } else {
                    $item_type_names[$post['itemtype']]++;
                }
            } elseif (! empty($post['post_type'])) {	// Use wp field Item Type usage as second priority:
                if (! isset($item_type_usages[$post['post_type']])) {
                    $item_type_usages[$post['post_type']] = 1;
                } else {
                    $item_type_usages[$post['post_type']]++;
                }
            } else {	// If Item Type is not defined at all:
                $no_item_types++;
            }
        }
    }

    if (empty($item_type_names) && empty($item_type_usages) && $no_item_types == 0) {	// No posts:
        echo '<p>No posts found in XML file, you can try to import other data like catefories and etc.</p>';
    } else {	// Display Item Types selectors:
        ...
        if ($no_item_types > 0) {	// Selector for without provided Item Types:
            wpxml_display_item_type_selector([$no_item_types], 'none');
        }
        echo '</ul>';
    }
}
