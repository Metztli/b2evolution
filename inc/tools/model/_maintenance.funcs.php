...
function print_log($message, $type = null, $params = [])
{
    $params = array_merge([
        'message_before' => '',
        'message_after' => '<br />',
...
function dbm_delete_pagecache($display_details = true)
{
    ...
    if ($display_details) {	// Display message only when it is required:
        print_log(T_('Clearing page caches:'), 'normal', [
            'text_style' => 'bold',
        ]);
        evo_flush();
    ...
    if ($blog_array = $DB->get_col($SQL)) {
        foreach ($blog_array as $l_blog) { // Clear blog cache:
...
function dbm_optimize_tables($display_messages = true, $separate_tables = true)
{
    ...
    foreach ($tables as $table) {
        ...
        if (((isset($table->Engine) && $table->Engine == 'MyISAM')
            ...
            && $table->Data_free) {	// Optimization needed for MyISAM table
            $myisam_tables[] = $table->Name;
        } elseif (((isset($table->Engine) && $table->Engine == 'InnoDB')
                || (isset($table->Type) && $table->Type == 'InnoDB'))
            && $table->Data_free) {	// Optimization needed for InnoDB table
 ...
 * @return array Results of the mysql command 'OPTIMIZE'
 */
function dbm_optimize_tables_process($display_messages = true, $separate_tables = true, $tables = [], $table_type = '')
{
    global $DB;
    ...
    if ($display_messages) {	// Display or log messages:
    ...
    if (! empty($tables)) { // Optimize tables
        if ($separate_tables) { // Optimize each table in separate query
            foreach ($tables as $table) {
        ...
        } else { // Optimize all table by one query, Used for cron job
    ...
    if ($display_messages) {	// Display or log messages:
        if (! $separate_tables || empty($tables)) { // Display full report log for case when the tables were optimized by one query
...
function dbm_display_result_messages($results, $command_type, $display_messages = true)
{
    ...
    if (count($results) > 0) {
        foreach ($results as $result) {
            $log_type = null;
            if ($result->Msg_type == 'status' && $result->Msg_text == 'OK') { // OK
                $log_message = sprintf($params['message_ok'], '<code>' . $result->Table . '</code>');
            } elseif ($result->Msg_type == 'note' && $result->Msg_text == 'Table does not support optimize, doing recreate + analyze instead') { // This warning is comming for every innodb table, but that is normal, Display for info
    ...
    } else { // No tables found to optimize, probably all tables already were optimized
        if ($display_messages === 'cron_job') {	// Log a message for cron job:
...
function dbm_delete_broken_posts()
{
    ...
    if (! empty($post_IDs)) {
        ...
        foreach ($broken_items as $r => $row) { // delete broken items
            $broken_Item = new Item($row);
            if ($broken_Item->dbdelete()) { // Post was deleted successfully
                $num_deleted++;
            } else { // Post was NOT deleted
                echo '<p class="red">' . sprintf(T_('Cannot delete post with ID %s'), $broken_Item->ID) . '</p>';
            }
            if ($r % 100 == 0) { // Display a log dot after each 100 processed posts
                echo '. ';
                evo_flush();
            }
        }
...
function dbm_delete_orphan_comments()
{
    ...
    foreach ($comments as $comment_ID) {
        if (($broken_Comment = &$CommentCache->get_by_ID($comment_ID, false, false)) !== false) { // Comment object is created
            if ($broken_Comment->dbdelete(true)) { // Comment is deleted successfully
                $num_deleted++;
            }
        }
...
function dbm_delete_orphan_files($delete_orphan_files, $delete_orphan_linked_files = false)
{
    ...
    if ($delete_orphan_files && $delete_orphan_linked_files) {	// Initalize LinkCache once for code below:
        $LinkCache = &get_LinkCache();
    }

    while ($loaded_files = $DB->get_results($files_SQL, ARRAY_A)) {	// Check the found files:
        ...
        foreach ($loaded_files as $file_data) {
            ...
            if ($File->exists()) {	// File exists on the disk:
                ...
                if (empty($file_data['links_num'])) {	// Count number of missing files without links:
                    $num_missing_no_linked_files++;
                } else {	// Count number of missing files with links:
                    $num_missing_linked_files++;
                    $num_total_links += $file_data['links_num'];
                }
                $file_num_deleted_links = 0;
                if ($delete_orphan_files) {	// Try to delete File only if it is requested:
                    echo ' - ';
                    if ($delete_orphan_linked_files && ! empty($file_data['link_IDs'])) {	// Try to delete ALL Link objects of the orphan File before deleting it:
                    ...
                    if ($File->dbdelete()) {	// Success deleting:
                        if ($file_num_deleted_links) {	// Inform about deleted with links:
                            echo '<span class="text-danger">' . sprintf(T_('Deleted with %d links'), $file_num_deleted_links) . '.</span>';
                        } else {	// Inform about deleted without links:
                            echo '<span class="text-warning">' . T_('Deleted without links') . '.</span>';
                        }
                    } else {	// Some restrictions, see File::get_delete_restrictions():
                        echo '<span class="text-danger">' . T_('Could not be deleted!') . '</span>';
                        // Skip this File on next page searching:
                        $skip_files[] = $File->ID;
                    }
                } else {	// Skip this File on next page searching:
                    $skip_files[] = $File->ID;
                }
            }
            $num_total_files++;
        ...
        if (! empty($skip_files)) {	// Skip files which have been already processed:
...
function dbm_delete_orphan_file_roots()
{
    ...
    if (($media_dir_blogs = @opendir($media_path_blogs)) === false) { // Could not open blogs media dir
        ...
        while (($folder = readdir($media_dir_blogs)) !== false) {
            if ($folder == '.' || $folder == '..' || ! is_dir($media_path_blogs . $folder)) { // Skip files
                continue;
            }
            if (! in_array($media_path_blogs . $folder . '/', $blog_dirs)) { // This dir must be deleted because it is not media dir of the existing blogs
    ...
    if (($media_dir_users = @opendir($media_path_users)) === false) { // Could not open users media dir
        ...
        while (($folder = readdir($media_dir_users)) !== false) {
            if ($folder == '.' || $folder == '..' || ! is_dir($media_path_users . $folder)) { // Skip files
                continue;
            }
            if (! in_array($folder, $user_logins)) { // This dir must be deleted because it is not media dir of the existing users
    ...
    foreach ($delete_dirs as $delete_dir) {
        if (rmdir_r($delete_dir)) { // Success deleting
...
function dbm_recreate_itemslugs()
{
    ...
    foreach ($items as $item_ID) {
        ...
        if ((! empty($item_title)) && (($prev_urltitle == 'title') || (preg_match('#^title-[0-9]+$#', $prev_urltitle)))) {
            ...
            if (($result) && ($prev_urltitle != $Item->get('urltitle'))) { // update was successful, and item urltitle was changed
                $count_slugs++;
            }
        }
    }
...
function dbm_check_tables($display_messages = true, $separate_tables = true)
{
    ...
    } elseif ($display_messages) {	// Print out a message:
        echo '<b>' . T_('Check tables...') . '</b><br />';
        evo_flush();
    }

    $Timer->start('check_tables');
    if ($separate_tables) { // Check each table in separate query
        $check_results = [];
    ...
    } else { // Check all table by one query, Used for cron job
        $check_results = $DB->get_results('CHECK TABLE ' . implode(', ', $tables_names) . ' FAST');
    }
    $Timer->stop('check_tables');

    if ($display_messages) {	// Display or log messages:
        if (! $separate_tables) {	// Display full report log for case when the tables were checked by one query:
...
function dbm_analyze_tables($display_messages = true, $separate_tables = true)
{
    ...
    if ($separate_tables) { // Analyze each table in separate query
        $analyze_results = [];
        foreach ($tables_names as $table) {
            ...
            if ($display_messages) { // Display messages
    ...
    } else { // Analyze all table by one query, Used for cron job
        $analyze_results = $DB->get_results('ANALYZE NO_WRITE_TO_BINLOG TABLE ' . implode(', ', $tables_names));
    }
    $Timer->stop('analyze_tables');

    if ($display_messages) { // Display messages
        if (! $separate_tables) { // Display full report log for case when the tables were analyzed by one query
...
function dbm_convert_item_content_separators()
{
    ...
    */
    ...
    if ($item_updated_count > 0) { // Some separators were updated
