...
if ($tab == 'addresses') {	// Email addresses
    if (param('emadr_ID', 'integer', '', true)) {	// Load Email Address object
        $EmailAddressCache = &get_EmailAddressCache();
        if (($edited_EmailAddress = &$EmailAddressCache->get_by_ID($emadr_ID, false)) === false) {	// We could not find the goal to edit:
...
} elseif ($tab == 'sent' && empty($tab3)) { // Email log
    if (param('emlog_ID', 'integer', 0, true)) { // Load Email Log object
...
switch ($action) {
    case 'settings': // Update the email settings
        ...
        switch ($tab3) {
            ...
            case 'settings':
                ...
                if (strlen($repath_errtype) > 5000) {	// Crop the value by max available size
            ...
            case 'smtp':
                ...
                if ($email_send_allow_php_mail) {	// Update the settings only when php mail service is enabled by config:
                ...
                if ($Settings->get('smtp_enabled') && ($smtp_error = check_smtp_mailer()) !== true) { // No available to use SMTP gateway
                    $Settings->set('smtp_enabled', 0);
                    $Messages->add($smtp_error, 'warning');
                }

                // Save info about SMTP enabling/disabling
                if (! $old_smtp_enabled && $Settings->get('smtp_enabled')) { // Enabled
                    $syslog_message = TB_('SMTP enabled.');
                } elseif ($old_smtp_enabled && ! $Settings->get('smtp_enabled')) { // Disabled
        ...
        if (! $Messages->has_errors()) {
            if ($Settings->dbupdate()) {
                if (! empty($syslog_message)) { // Log system info into DB
                    syslog_insert($syslog_message, 'info', null);
                }
                $Messages->add(TB_('Settings updated.'), 'success');

                if ($tab3 == 'smtp' && $Settings->get('smtp_enabled')) { // Check if connection is available
                    ...
                    if ($smtp_connection_result) { // Success
    ...
    case 'test_3':
        ...
        switch ($action) {
            case 'test_1':
                if ($mbox = dre_connect(false, true)) {	// Close opened connection
                    imap_close($mbox);
                }
                break;
            ...
            case 'test_2':
                if ($mbox = dre_connect()) {
                    ...
                    if ($imap_obj->Nmsgs > 0) {	// We will read only 1 message from server in test mode
            ...
            case 'test_3':
                ...
                if (! empty($test_error_message)) {	// Simulate a message processing
        ...
        if (! empty($dre_messages)) {	// We will display the output in a scrollable fieldset
            $repath_test_output = implode("<br />\n", $dre_messages);
        }
        break;
    ...
    case 'test_email_php':
        ...
        // Test email sending:
        if ($action == 'test_email_smtp') {	// SMTP gateway:
    ...
    case 'blocked_save':
        ...
        $action = 'blocked_edit';
        if (! isset($edited_EmailAddress)) {	// Create a new address
            $edited_EmailAddress = new EmailAddress();
            $action = 'blocked_new';
        }

        // load data from request
        if ($edited_EmailAddress->load_from_Request()) {	// We could load data from form without errors:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) {
            if ($edited_EmailLog === false) { // We could not find the email log to delete:
                unset($edited_EmailLog);
                forget_param('emlog_ID');
                $Messages->add(sprintf(TB_('Requested &laquo;%s&raquo; object does not exist any longer.'), TB_('Email Log')), 'error');
            } else {
...
switch ($tab) {
    case 'sent':
        ...
        switch ($tab3) {
            ...
            case 'smtp':
                ...
                if ($Settings->get('email_service') == 'smtp' && ! $Settings->get('smtp_enabled')) {	// Display this error when primary email service is SMTP but it is not enabled:
            ...
            default:
                ...
                if (empty($emlog_ID)) { // Initialize date picker on list page
                    init_datepicker_js();
                }
    ...
    case 'addresses':
        ...
        if (! isset($edited_EmailAddress)) { // List page with email addresses
    ...
    case 'return':
        ...
        if (empty($emret_ID)) { // Initialize date picker on list page
            init_datepicker_js();
        }

        if (empty($tab3)) {	// Set default tab3 for first tab:
            $tab3 = 'log';
        }
        ...
        switch ($tab3) {
            ...
            case 'settings':
                ...
                if ($Settings->get('repath_enabled')) { // If the decoding the returned emails is enabled
                    ...
                    if (empty($repath_cron)) { // Display a warning if cron job "Process the return path inbox" doesn't exist:
                        $repath_warning = TB_('There is no scheduled job configured to process your Return Path inbox.');
                        if (check_user_perm('options', 'edit')) { // Suggest a link to create a job if current user has a permission:
...
if (! empty($orig_tab)) {	// Restore original tab:
    $tab = $orig_tab;
}
...
switch ($tab) {
    case 'sent':
        switch ($tab3) {
            ...
            default:
                if (! empty($edited_EmailLog) && ($action != 'delete')) {	// Display details of selected email log
    ...
    case 'addresses':
        if (isset($edited_EmailAddress)) {	// Display form to create/edit an email address
    ...
    case 'return':
        ...
        switch ($tab3) {
            ...
            default:
                ...
                if ($emret_ID > 0) {	// Display a details of selected email
                    $MailReturn = $DB->get_row('
						SELECT *
							FROM T_email__returns
						 WHERE emret_ID = ' . $DB->quote($emret_ID));
                    if ($MailReturn) {	// The returned email exists with selected ID
