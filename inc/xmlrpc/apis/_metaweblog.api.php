...
$mwnewpost_sig = [
    ...
    [$xmlrpcStruct, $xmlrpcString, $xmlrpcString, $xmlrpcString, $xmlrpcStruct, $xmlrpcBoolean],
];
/**
 * metaWeblog.newPost
 *
...
function mw_newpost($m, $item_type = 'post')
{
    ...
    $status = 'published';
    if (isset($m->params[4])) {	// getParam(4) is a flag for publish or draft
    ...
    if (! empty($contentstruct['post_type']) && $contentstruct['post_type'] != $item_type) {	// Overwrite from struct
    ...
    if ($item_type == 'page') {	// Force post type 'page'
        global $DB;
    ...
    if (! empty($contentstruct[$item_type . '_status'])) {	// Use WP status
        $status = wp_or_b2evo_item_status($contentstruct[$item_type . '_status'], 'b2evo');
    }

    if (! empty($content_struct['mt_text_more'])) {	// Add content extension
        $content .= '[teaserbreak]' . $content_struct['mt_text_more'];
    }
    //logIO( "Item content:\n".$content );

    if (! empty($content_struct['enclosure']) && is_array($content_struct['enclosure'])) {	// Add content extension
    ...
    if (isset($contentstruct['mt_allow_comments'])) {
        if (! $contentstruct['mt_allow_comments'] || in_array($contentstruct['mt_allow_comments'], [2, 'closed'])) {	// Comments disabled
...
$mweditpost_sig = [
    [$xmlrpcBoolean, $xmlrpcInt, $xmlrpcString, $xmlrpcString, $xmlrpcStruct],
    [$xmlrpcBoolean, $xmlrpcInt, $xmlrpcString, $xmlrpcString, $xmlrpcStruct, $xmlrpcBoolean],
    [$xmlrpcBoolean, $xmlrpcString, $xmlrpcString, $xmlrpcString, $xmlrpcStruct, $xmlrpcBoolean],
];
/**
 * metaWeblog.editPost
 *
...
function mw_editpost($m, $item_type = 'post')
{
     ...
     * @var Item
     */
    if (! $edited_Item = &xmlrpcs_get_Item($m, 0)) {	// Failed, return (last) error:
        return xmlrpcs_resperror();
    }
    ...
    if (isset($content_struct['mt_text_more'])) {	// Add content extension
        $content .= '[teaserbreak]' . $content_struct['mt_text_more'];
    }
    //logIO( "Item content:\n".$content );
...
$mwgetcats_sig = [
    ...
    [$xmlrpcStruct, $xmlrpcString, $xmlrpcString, $xmlrpcString],
];
...
$metawebloggetrecentposts_sig = [
    [$xmlrpcArray, $xmlrpcInt, $xmlrpcString, $xmlrpcString, $xmlrpcInt],
    [$xmlrpcArray, $xmlrpcString, $xmlrpcString, $xmlrpcString, $xmlrpcInt],
];
/**
...
$mwgetusersblogs_sig = [
    [$xmlrpcArray, $xmlrpcInt, $xmlrpcString, $xmlrpcString],
    [$xmlrpcArray, $xmlrpcString, $xmlrpcString, $xmlrpcString],
];
/**
...
function mw_getpost($m)
{
    ...
    // CHECK PERMISSION:
    if (! xmlrpcs_can_view_item($edited_Item, $current_User)) {	// Permission denied
...
$mwdeletepost_sig = [[$xmlrpcBoolean, $xmlrpcString, $xmlrpcString, $xmlrpcString, $xmlrpcString, $xmlrpcBoolean]];
/**
...
function mw_deletepost($m)
{
     ...
     * @var Item
     */
    if (! $edited_Item = &xmlrpcs_get_Item($m, 1)) {	// Failed, return (last) error:
        return xmlrpcs_resperror();
    }
...
$xmlrpc_procs['metaWeblog.newMediaObject'] = [
    // OK
...
$xmlrpc_procs['metaWeblog.newPost'] = [
    // Incomplete (minor): handle 'enclosure', 'custom_fields' in WP
    'function' => 'mw_newpost',
    'signature' => $mwnewpost_sig,
    'docstring' => $mwnewpost_doc,
];

$xmlrpc_procs['metaWeblog.editPost'] = [
    // Incomplete (minor): handle 'enclosure', 'custom_fields' in WP
...
$xmlrpc_procs['metaWeblog.getPost'] = [
    // OK
    'function' => 'mw_getpost',
...
$xmlrpc_procs['metaWeblog.getCategories'] = [
    'function' => 'mw_getcategories',
    'signature' => $mwgetcats_sig,
    'docstring' => $mwgetcats_doc,
];

$xmlrpc_procs['metaWeblog.getRecentPosts'] = [
