...
$mt_supportedMethods_sig = [[$xmlrpcArray]];
...
$mt_setPostCategories_sig = [[$xmlrpcBoolean, $xmlrpcString, $xmlrpcString, $xmlrpcString, $xmlrpcArray]];
...
function mt_setPostCategories($m)
{
    ...
    if (! check_user_perm('item_post!CURSTATUS', 'edit', false, $edited_Item)) {	// Permission denied
    ...
    } elseif (empty($category)) {	// Use first one as default:
        $category = $categories[0];
    }

    // Check if category exists and can be used:
    $Collection = $Blog = &$edited_Item->get_Blog();
    if (! xmlrpcs_check_cats($category, $Blog, $categories)) {	// Error:
        return xmlrpcs_resperror();
    }

    // CHECK PERMISSION: (we need perm on all categories, especially if they are in different blogs)
    if (! check_user_perm('cats_post!' . $edited_Item->status, 'edit', false, $categories)) {	// Permission denied
...
$mt_getPostCategories_sig = [[$xmlrpcArray, $xmlrpcString, $xmlrpcString, $xmlrpcString]];
...
function mt_getPostCategories($m)
{
    ...
    for ($i = 0; $i < $iSize; $i++) {
        ...
        $struct[$i] = new xmlrpcval([
            'categoryId' => new xmlrpcval($categories[$i]),    // Look up name from ID separately
...
$mt_getCategoryList_sig = [[$xmlrpcArray, $xmlrpcString, $xmlrpcString, $xmlrpcString]];
...
$mt_publishPost_sig = [[$xmlrpcBoolean, $xmlrpcString, $xmlrpcString, $xmlrpcString]];
...
function mt_publishPost($m)
{
    ...
    if ($edited_Item->dbupdate() === false) {	// Could not update item...
