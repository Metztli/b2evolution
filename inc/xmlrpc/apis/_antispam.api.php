...
$b2evoreportabuse_sig = [[$xmlrpcString, $xmlrpcInt, $xmlrpcString, $xmlrpcString, $xmlrpcString, $xmlrpcString]];
...
function b2evoreportabuse($m)
{
    ...
    if (empty($enable_blacklist_server_API)) {	// Exit here if the server cannot be used as central antispam:
    ...
    if ($keyword_ID > 0) { // The keyword has been found, Update it
        ...
        if ($query_result === false) { // Query error, Exit here
            $DB->rollback();
    ...
    } else { // Try to insert new keyword
        ...
        if ($query_result === false || empty($keyword_ID)) { // Query error, Exit here
            $DB->rollback();
    ...
    if (empty($source_ID)) { // Create new reporter if it doesn't exist in DB yet
        ...
        if ($query_result === false || empty($source_ID)) { // Query error, Exit here
            $DB->rollback();
    ...
    if (empty($report_ID)) { // No such report, Create it
    ...
    } else { // If report was already done we should update the report date
...
$xmlrpc_procs['b2evo.reportabuse'] = [
...
$b2evopollabuse_sig = [[$xmlrpcStruct, $xmlrpcInt, $xmlrpcString, $xmlrpcString, $xmlrpcDateTime, $xmlrpcInt]];
...
function b2evopollabuse($m)
{
    ...
    if (! empty($keywords)) { // There are some answers
