...
function _wp_mw_newmediaobject($m)
{
    ...
    // CHECK PERMISSION:
    if (! check_user_perm('files', 'add', false, $Blog->ID)) {	// Permission denied
    ...
    if (($maxfilesize = $Settings->get('upload_maxkb') * 1024) && $filesize > $maxfilesize) {
        return xmlrpcs_resperror(4, sprintf(
            T_('The file is too large: %s but the maximum allowed is %s.'),
            bytesreadable($filesize, false),
            bytesreadable($maxfilesize, false)
        ));
    }
    ...
    foreach ($filepath_parts as $filepath_part) {
        if (empty($filepath_part) || $filepath_part == '.') {	// self ref not useful
            continue;
        }

        if ($error = validate_dirname($filepath_part)) {	// invalid relative path:
            logIO($error);
    ...
    if (! empty($image_info)) {	// This is an image file, let's check mimetype and correct extension
        if ($image_info['mime'] != $file_mimetype) {	// Invalid file type
    ...
    if ($newFile->exists()) {
        if ($overwrite && $newFile->unlink()) {	// OK, file deleted
    ...
    if (! mkdir_r($newFile->get_dir())) {	// Dir didn't already exist and could not be created
    ...
    prepare_uploaded_files([$newFile]);
    logIO('Done');
...
function _wp_mw_get_item_struct(&$Item)
{
    ...
    if (is_null($Item->extra_cat_IDs)) {	// Load extra cats
...
function _wp_or_blogger_getusersblogs($type, $m)
{
    ...
    // CHECK LOGIN:
    if (! $current_User = &xmlrpcs_login($m, $username_index, $password_index)) {	// Login failed, return (last) error:
        return xmlrpcs_resperror();
    }
    ...
    $resp_array = [];
    foreach ($blog_array as $l_blog_ID) {	// Loop through all blogs that match the requested permission:
        /**
         * @var Blog
         */
...
function _mw_decode_date($contentstruct)
{
    ...
    if (empty($postdate) && ! empty($contentstruct['dateCreated'])) {
        ...
        if (strpos($postdate, 'T') > 0) {	// Date is in ISO 8601 format
...
function _mw_get_cat_IDs($contentstruct, $Blog, $empty_struct_is_ok = false)
{
    ...
    if (! empty($categories)) {
        ...
        if (! $cat_IDs = $DB->get_col($SQL)) {	// DB error
...
function _b2_or_mt_get_categories($type, $m)
{
    ...
    $callbacks = [
        'line' => 'add_to_categories_data',
    ];

    $ChapterCache = &get_ChapterCache();
    $ChapterCache->recurse($callbacks, $aggregate_coll_IDs, null, 0, 0, [
        'sorted' => true,
    ]);
 ...
 */
function _wp_mw_getcategories($m, $params = [])
{
    global $DB;
     ...
     * @var Blog
     */
    if (! ($Collection = $Blog = &xmlrpcs_get_Blog($m, 0))) {	// Not found the selected blog, return (last) error:
        return xmlrpcs_resperror();
    }
    ...
    if (! empty($params['search'])) {	// Category name starts with 'search'
    ...
    for ($i = 0; $i < $total_rows; $i++) {
        if (! empty($params['limit']) && $i >= $params['limit']) {	// We found enough, exit the loop
            break;
        }
        ...
        if (isset($params['search'])) {	// wp.suggestCategories
            $data[] = new xmlrpcval([
...
function &xmlrpcs_login($m, $login_param, $pass_param)
{
    ...
    if (empty($current_User) || ! $current_User->check_password($password, false)) {	// User not found or password doesn't match
    ...
    if (! $group->check_perm('perm_api', 'always')) {	// Permission denied
        $xmlrpcs_errcode = $xmlrpcerruser + 1;
...
function &xmlrpcs_get_Item($m, $id_param)
{
    ...
    if (empty($edited_Item)) {	// Item not found
...
function &xmlrpcs_get_Chapter($m, $id_param)
{
    ...
    if (empty($edited_Chapter)) {	// Chapter not found
...
function &xmlrpcs_get_Comment($m, $id_param)
{
    ...
    if (empty($edited_Comment)) {	// Comment not found
        $xmlrpcs_errcode = $xmlrpcerruser + 9;
...
function xmlrpcs_resperror($errcode = null, $errmsg = null)
{
    ...
    if (! empty($errcode)) { // Transform into user error code
        $xmlrpcs_errcode = $xmlrpcerruser + $errcode;
    }

    if (! empty($errmsg)) {	// Custom message
        $xmlrpcs_errmsg = $errmsg;
    } elseif (empty($xmlrpcs_errmsg)) {	// Use a standard messsage
        switch ($errcode) {
            case 3:
 ...
 * @return xmlrpcmsg
 */
function xmlrpcs_new_comment($params = [], &$commented_Item)
{
    ...
    if (empty($params['username']) && empty($params['password'])) {	// Anonymous comment
        ...
        if ($commented_Item->Blog->get_setting('require_anon_name') && empty($author)) {	// Author name is required for anonymous users:
            return xmlrpcs_resperror(5, T_('Please fill in your name.'));
        }
        if ($commented_Item->Blog->get_setting('require_anon_email') && empty($email)) {	// Author email address is required for anonymous users:
        ...
        if (! stristr($url, '://') && ! stristr($url, '@')) { // add 'http://' if no protocol defined for URL; but not if the user seems to be entering an email address alone
            $url = 'http://' . $url;
        }

        if (strlen($url) <= 8) {	// ex: https:// is 8 chars
            $url = '';
        }
    ...
    if ($comment === false) {	// ERROR! Restore original comment for further editing:
        $comment = $params['content'];
    }

    if (empty($comment)) { // comment should not be empty!
    ...
    if ($User) { // User is logged in, we'll use his ID
    ...
    if ($perm_comment_edit) {	// User has perm to moderate comments, publish automatically:
        $Comment->set('status', 'published');
    } else { // Assign default status for new comments:
    ...
    $Plugins->trigger_event('BeforeCommentFormInsert', [
        'Comment' => &$Comment,
    ...
    if ($Comment->ID) {	// comment has not been deleted
        // Trigger event: a Plugin should cleanup any temporary data here..
        $Plugins->trigger_event('AfterCommentFormInsert', [
            'Comment' => &$Comment,
            'original_comment' => $params['content'],
        ]);
 ...
 * @return xmlrpcmsg
 */
function xmlrpcs_edit_comment($params = [], &$edited_Comment)
{
    ...
    if (! $edited_Comment->get_author_User()) { // If this is not a member comment
...
function xmlrpcs_new_item($params, &$Blog = null)
{
    ...
    $params = array_merge([
        ...
        'main_cat_ID' => 0,
        'extra_cat_IDs' => [],
        'cat_IDs' => [],	// we may use this to set main and extra cats
        'status' => 'published',
        'tags' => '',
        'excerpt' => '',
        'item_typ_ID' => 1,
    ...
    if (empty($Blog) && ! empty($params['main_cat_ID'])) {	// Get the blog by main category ID
        // Check if category exists and can be used
        $ChapterCache = &get_ChapterCache();
        $main_Chapter = &$ChapterCache->get_by_ID($params['main_cat_ID'], false, false);
        if (empty($main_Chapter)) {	// Cat does not exist:
    ...
    if (empty($Blog)) {	// Blog does not exist:
        return xmlrpcs_resperror();
    }

    if (empty($params['main_cat_ID'])) {
        if (is_array($params['cat_IDs']) && count($params['cat_IDs']) > 0) {	// Let's use first cat for MAIN and others for EXTRA
            $params['main_cat_ID'] = array_shift($params['cat_IDs']);
            $params['extra_cat_IDs'] = $params['cat_IDs'];
        } else {
            if (! $main_cat = $Blog->get_default_cat_ID()) {	// No default category found for requested blog
    ...
    if (empty($params['main_cat_ID'])) {	// Main category does not exist:
        return xmlrpcs_resperror(11);	// User error 11
    }

    // Check if category exists and can be used
    if (! xmlrpcs_check_cats($params['main_cat_ID'], $Blog, $params['extra_cat_IDs'])) {	// Permission denied
        return xmlrpcs_resperror(3);	// User error 3
    }
     ...
     */
    if (! check_user_perm('cats_post!' . $params['status'], 'edit', false, $params['extra_cat_IDs'])) {	// Permission denied
        return xmlrpcs_resperror(3);	// User error 3
    }

    if (! empty($params['item_typ_ID'])) {
        if (! preg_match('~^[0-9]+$~', $params['item_typ_ID'])) { // Only accept numeric values, switch to default value
        ...
        if ($ItemType && ! check_user_perm('cats_item_type_' . $ItemType->perm_level, 'edit', false, $params['extra_cat_IDs'])) { // Permission denied
            return xmlrpcs_resperror(3);	// User error 3
        }
    }
    ...
    if ($edited_Item->allow_comment_statuses()) { // Comment status
    ...
    if (! empty($params['custom_fields']) && is_array($params['custom_fields']) && count($params['custom_fields']) > 0) {	// TODO sam2kb> Add custom fields
        foreach ($params['custom_fields'] as $field) {	// id, key, value
...
function xmlrpcs_edit_item(&$edited_Item, $params)
{
    ...
    if (! is_null($params['main_cat_ID']) && is_array($params['extra_cat_IDs'])) {	// Check new categories
        logIO('Main cat ID: ' . $params['main_cat_ID']);
        logIO('Extra cat IDs: ' . implode(', ', $params['extra_cat_IDs']));
    ...
    if (! is_null($params['order'])) {
        if (! (empty($params['order']) && ! $edited_Item->get_order())) {	// Do not allow 0 order if there was no order set before
            $edited_Item->set('order', $params['order']);
        }
    }
    ...
    if (! is_null($params['author_ID']) && $params['author_ID'] != $this->creator_user_ID) {	// We have already checked perms to edit items created by other users
    ...
    if (! empty($params['comment_status']) && $edited_Item->allow_comment_statuses()) {	// Comment status
        $edited_Item->set('comment_status', $params['comment_status']);
    }
    ...
    if (! is_null($params['custom_fields'])) {	// TODO sam2kb> Add custom fields
...
function xmlrpcs_check_cats(&$maincat, &$Blog, &$extracats)
{
    ...
    // ---- CHECK MAIN CATEGORY ----
    if ($ChapterCache->get_by_ID($maincat, false) === false) {	// Category does not exist!
    ...
    } elseif (get_allow_cross_posting() < 2 && get_catblog($maincat) != $Blog->ID) {	// We cannot use a maincat of another blog than the current one:
    ...
    foreach ($extracats as $ecat) {
        if ($ecat == $maincat) {	// We already checked the maincat above (or reset it):
            continue;
        }

        logIO('Checking extra cat: ' . $ecat);
        if ($ChapterCache->get_by_ID($ecat, false) === false) {	// Extra cat does not exist:
            $xmlrpcs_errcode = $xmlrpcerruser + 11;
...
function xmlrpc_get_comments($params, &$Blog)
{
    ...
    if (empty($params['statuses'])) {	// Return all except 'trash'
        $params['statuses'] = ['published', 'deprecated', 'draft'];
    }
...
function xmlrpcs_delete_item(&$edited_Item)
{
    ...
    // CHECK PERMISSION:
    if (! check_user_perm('item_post!CURSTATUS', 'delete', false, $edited_Item)) {	// Permission denied
        return xmlrpcs_resperror(3);	// User error 3
    }
    logIO('Permission granted.');
...
function wp_or_b2evo_comment_status($raw_status, $convert_to = 'b2evo')
{
    ...
    if ($convert_to == 'b2evo') {
        switch ($raw_status) {	// Map WP statuses to b2evo
        ...
        switch ($raw_status) {	// Map b2evo statuses to WP
            case 'deprecated':
                $status = 'spam';
                break;
