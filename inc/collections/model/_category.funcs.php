...
function cat_create($cat_name, $cat_parent_ID, $cat_blog_ID = null, $cat_description = null, $add_to_cache = false, $cat_order = null, $subcat_ordering = null, $meta = false, $default_item_type_name = null)
{
    ...
    if ($cat_parent_ID === 'NULL') { // fix old use case
        $cat_parent_ID = null;
    }
    ...
    if ($meta) { // Set this category as meta
        $new_Chapter->set('meta', 1);
    }

    if ($default_item_type_name !== null &&
        ($ItemTypeCache = &get_ItemTypeCache()) &&
        ($ItemType = &$ItemTypeCache->get_by_name($default_item_type_name, false, false))) {	// Item type exists in DB by requested name, Use it:
    ...
    if ($add_to_cache) { // add new Category into the Cache
        $ChapterCache = &get_ChapterCache();
...
function postcats_get_by_IDs($item_IDs)
{
    ...
    if (! is_array($cache_postcats)) {	// Initialize cache array first time:
        $cache_postcats = [];
    }

    $not_cached_item_IDs = array_diff($item_IDs, array_keys($cache_postcats));

    if (empty($not_cached_item_IDs)) {	// The category associations are loaded for all requested items:
        return;
    }
...
function postcats_get_byID($post_ID)
{
    ...
    if (! isset($cache_postcats[$post_ID])) {	// Get data from DB if it is not still in cache:
...
function compile_cat_array($cat, $catsel, &$cat_array, &$cat_modifier, $restrict_to_blog = 0)
{
    ...
    if ($cat != 'all' && ! empty($cat)) { // specified a category string:
        ...
        if (strlen($cat)) {	// There are some values to explode...
            ...
            $ChapterCache = &get_ChapterCache();
            if ($restrict_to_blog > 0) { // Load all Chapters from the given blog
                $ChapterCache->reveal_children($restrict_to_blog, true);
            } else { // Load all chapters
                $ChapterCache->reveal_children(null, true);
            }

            foreach ($req_cat_array as $cat_ID) { // run recursively through the cats
                $current_Chapter = $ChapterCache->get_by_ID($cat_ID, false);
                if (empty($current_Chapter)) { // The requested Chapter doesn't exists in the given context
                    continue;
                }
                if (! in_array($cat_ID, $cat_array)) { // Not already in list
                    $cat_array[] = $cat_ID;
                    $ChapterCache->iterate_through_category_children($current_Chapter, [
                        'line' => 'cat_req',
                    ], true, [
                        'sorted' => true,
                    ]);
...
function cat_req($Chapter, $level)
{
    ...
    if (! in_array($Chapter->ID, $cat_array)) { // Not already visited
