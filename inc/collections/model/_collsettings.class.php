...
class CollectionSettings extends AbstractSettings
{
    ...
    {
        parent::__construct('T_coll_settings', ['cset_coll_ID', 'cset_name'], 'cset_value', 1);
    }
     ...
     * @return boolean
     */
    public function _load($coll_ID = null, $arg = null, $arg3 = null)
    {
     ...
     */
    public function get($col_key1, $col_key2)
    {
     ...
     */
    public function set($col_key1, $col_key2, $value)
    {
        if (is_array($value)) {	// Don't crop a serialized value if value is an array,
            // e-g plugin setting with type "checklist":
            if (strlen(serialize($value)) > 10000) {	// Stop here to avoid DB error on inserting of long value:
                debug_die('Impossible to store long data(>10000 chars) of collection setting "' . $col_key2 . '"!');
            }
        } else {	// Limit value with max possible length:
