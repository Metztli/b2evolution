...
class Blog extends DataObject
{
    ...
    public $stub;     // stub file (can be empty/virtual)

    public $urlname;  // used to identify blog in URLs

    public $links_blog_ID = 0;	// DEPRECATED

    public $notes;

    public $keywords;

    public $allowtrackbacks = 0;

    public $allowblogcss = 0;

    public $allowusercss = 0;

    public $in_bloglist = 'public';

    public $media_location = 'default';

    public $media_subdir = '';

    public $media_fullpath = '';

    public $media_url = '';

    public $normal_skin_ID = null;

    public $mobile_skin_ID = null;

    public $tablet_skin_ID = null;

    public $alt_skin_ID = null;

    public $base_collection_ID = 0;
    ...
    public $update_url_aliases = false;
     ...
     */
    public $comment_moderator_user_data;
    ...
    public function __construct($db_row = null)
    {
        ...
        if ($db_row == null) {
            ...
            $this->set_locales([$default_locale]);
            // Use access type(Collection base URL) from general settings:
            $this->set('access_type', $Settings->get('coll_access_type'));
            if (is_logged_in()) {	// Set section what is used as default for group of current user:
                global $current_User;
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_coll_user_favs',
                'fk' => 'cufv_blog_ID',
                'msg' => T_('%d user favorites'),
            ],
            [
                'table' => 'T_coll_settings',
                'fk' => 'cset_coll_ID',
                'msg' => T_('%d blog settings'),
            ],
            [
                'table' => 'T_coll_locales',
                'fk' => 'cl_coll_ID',
                'msg' => T_('%d extra locales associations'),
            ],
            [
                'table' => 'T_coll_url_aliases',
                'fk' => 'cua_coll_ID',
                'msg' => T_('%d URL aliases'),
            ],
            [
                'table' => 'T_coll_user_perms',
                'fk' => 'bloguser_blog_ID',
                'msg' => T_('%d user permission definitions'),
            ],
            [
                'table' => 'T_coll_group_perms',
                'fk' => 'bloggroup_blog_ID',
                'msg' => T_('%d group permission definitions'),
            ],
            [
                'table' => 'T_subscriptions',
                'fk' => 'sub_coll_ID',
                'msg' => T_('%d subscriptions'),
            ],
            [
                'table' => 'T_widget__container',
                'fk' => 'wico_coll_ID',
                'msg' => T_('%d widget container'),
                'class' => 'WidgetContainer',
                'class_path' => 'widgets/model/_widgetcontainer.class.php',
            ],
            [
                'table' => 'T_hitlog',
                'fk' => 'hit_coll_ID',
            ...
            [
                'table' => 'T_hits__aggregate',
                'fk' => 'hagg_coll_ID',
                'msg' => T_('%d hits aggregations'),
            ],
            [
                'table' => 'T_hits__aggregate_sessions',
                'fk' => 'hags_coll_ID',
                'msg' => T_('%d sessions aggregations'),
            ],
            [
                'table' => 'T_items__type_coll',
                'fk' => 'itc_coll_ID',
                'msg' => T_('%d Post type associations with collections'),
            ],
            [
                'table' => 'T_categories',
                'fk' => 'cat_blog_ID',
                'msg' => T_('%d related categories with all of their content recursively'),
                'class' => 'Chapter',
                'class_path' => 'chapters/model/_chapter.class.php',
            ],
            [
                'table' => 'T_files',
                'fk' => 'file_root_ID',
                'and_condition' => 'file_root_type = "collection"',
                'msg' => T_('%d files in this blog file root'),
                'class' => 'File',
                'class_path' => 'files/model/_file.class.php',
            ],
            [
                'table' => 'T_temporary_ID',
                'fk' => 'tmp_coll_ID',
                'msg' => T_('%d temporary uploaded links'),
            ],
        ];
    }
    ...
    public static function get_delete_restrictions()
    {
        ...
        return [
            [
                'table' => 'T_settings',
                'fk' => 'set_value',
                'and_condition' => 'set_name = "default_blog_ID"',
                'msg' => sprintf(T_('This collection cannot be deleted because it is used as <a %s>Default collection to display</a>.'), 'href="' . $admin_url . '?ctrl=collections&amp;tab=site_settings"'),
            ],
            [
                'table' => 'T_settings',
                'fk' => 'set_value',
                'and_condition' => 'set_name = "login_blog_ID"',
                'msg' => sprintf(T_('This collection cannot be deleted because it is used as <a %s>Collection for login/registration</a>.'), 'href="' . $admin_url . '?ctrl=collections&amp;tab=site_settings"'),
            ],
            [
                'table' => 'T_settings',
                'fk' => 'set_value',
                'and_condition' => 'set_name = "msg_blog_ID"',
                'msg' => sprintf(T_('This collection cannot be deleted because it is used as <a %s>Collection for profiles/messaging</a>.'), 'href="' . $admin_url . '?ctrl=collections&amp;tab=site_settings"'),
            ],
            [
                'table' => 'T_settings',
                'fk' => 'set_value',
                'and_condition' => 'set_name = "info_blog_ID"',
                'msg' => sprintf(T_('This collection cannot be deleted because it is used as <a %s>Collection for shared content blocks</a>.'), 'href="' . $admin_url . '?ctrl=collections&amp;tab=site_settings"'),
            ],
        ];
    }
     ...
     */
    public static function compare_blogs($a_Blog, $b_Blog)
    {
        global $Settings;
        ...
        if ($order_by == 'RAND') { // In case of Random order we consider every blog as equal
            return 0;
        }

        $blog_a_value = $a_Blog->get($order_by);
        $blog_b_value = $b_Blog->get($order_by);
        if ($blog_a_value == $blog_b_value) { // The compare fields are equal sort based on the ID
        ...
        if ($order_dir == 'DESC') { // Change the order direction
            $result = $result * (-1);
        }
     ...
     */
    public function init_by_kind($kind, $name = null, $shortname = null, $urlname = null)
    {
        ...
        switch ($kind) {
            ...
            case 'photo':
                ...
                if ($photo_album_type_ID) {	// Set default post type as "Photo Album":
                    $this->set_setting('default_post_type', $photo_album_type_ID);
                }
                if (is_pro()) {	// Enable tracking of unread content only in PRO version:
            ...
            case 'group':
                ...
                if ($forum_topic_type_ID) { // Set default post type as "Forum Topic"
            ...
            case 'manual':
                ...
                if ($manual_page_type_ID) { // Set default post type as "Manual Page"
            ...
            case 'std':
            default:
                if ($kind != 'std') {	// Check if given kind is allowed by system or plugins:
                    $kinds = get_collection_kinds();
                    if (! isset($kinds[$kind])) {	// If unknown kind, restrict this to standard:
                        $kind = 'std';
                    }
                }
                $this->set('type', $kind);
        ...
        if (empty($name) && empty($shortname) && empty($urlname)) {	// Not in installation mode, init custom collection kinds.
            global $Plugins;
     ...
     */
    public function load_from_Request($groups = [])
    {
        ...
        if (param('blog_name', 'string', null) !== null) { // General params:
            ...
            // Language / locale:
            if (param('blog_locale', 'string', null) !== null) { // These settings can be hidden when only one locale is enabled in the system
                ...
                if ($extra_locales !== null || $linked_colls !== null) {	// Set new extra locales and linked collections:
            ...
            if (get_param('action') != 'update_confirm' && $this->ID > 0 &&
                ($prev_allow_access != $new_allow_access || $prev_advanced_perms != $new_advanced_perms)) {	// One of these settings is changing, try to check if max allowed status will be changed too:
                ...
                if ($status_orders[$new_max_allowed_status] < $status_orders[$prev_max_allowed_status]) {	// If max allowed status will be reduced after collection updating:
                    $count_reduced_status_data = $this->get_count_reduced_status_data($new_max_allowed_status);
                    if ($count_reduced_status_data !== false) {	// If some posts or comment will be updated:
                        $this->confirmation = [];
            ...
            if ($prev_allow_access != $this->get_setting('allow_access')) {	// If setting "Allow access to" is changed to:
                switch ($this->get_setting('allow_access')) {	// Automatically enable/disable moderation statuses:
                    case 'public':
                        // Enable "Community" and "Members":
                        $enable_moderation_statuses = ['community', 'protected'];
                        $enable_comment_moderation_statuses = ['community', 'protected', 'review', 'draft'];
                        $disable_comment_moderation_statuses = ['private'];
                        break;
                    case 'users':
                        // Disable "Community" and Enable "Members":
                        $disable_moderation_statuses = ['community'];
                        $enable_moderation_statuses = ['protected'];
                        $enable_comment_moderation_statuses = ['protected', 'review', 'draft'];
                        $disable_comment_moderation_statuses = ['community', 'private'];
                        break;
                    case 'members':
                        // Disable "Community" and "Members":
                        $disable_moderation_statuses = ['community', 'protected'];
                        $enable_comment_moderation_statuses = ['review', 'draft'];
                        $disable_comment_moderation_statuses = ['community', 'protected', 'private'];
                        break;
                }
                $post_moderation_statuses = $this->get_setting('post_moderation_statuses');
                $post_moderation_statuses = empty($post_moderation_statuses) ? [] : explode(',', $post_moderation_statuses);
                $comment_moderation_statuses = $this->get_setting('moderation_statuses');
                $comment_moderation_statuses = empty($comment_moderation_statuses) ? [] : explode(',', $comment_moderation_statuses);
                if (! empty($disable_moderation_statuses)) {	// Disable moderation statuses:
                ...
                if (! empty($enable_moderation_statuses)) {	// Enable moderation statuses:
            ...
            if ($this->get_setting('allow_access') != 'public') {	// Disable site maps, feeds and ping plugins for not public collection:
        ...
        if (param('archive_links', 'string', null) !== null) { // Archive link type:
            $this->set_setting('archive_links', get_param('archive_links'));
        }

        if (param('archive_posts_per_page', 'integer', null) !== null) {	// Archive posts per page:
            $this->set_setting('archive_posts_per_page', get_param('archive_posts_per_page'));
        }

        if (param('chapter_links', 'string', null) !== null) { // Chapter link type:
        ...
        if (in_array('metadata', $groups)) {
            ...
            if (param('blog_shortdesc', 'string', null) !== null) {	// Description:
                $this->set_from_Request('shortdesc');
            }

            if (param('blog_longdesc', 'html', null) !== null) {	// HTML long description:
                param_check_html('blog_longdesc', T_('Invalid long description'));
                $this->set('longdesc', get_param('blog_longdesc'));
            }

            if (param('blog_keywords', 'string', null) !== null) {	// Keywords:
            ...
            if (param('xml_item_footer_text', 'html', null) !== null) { // Blog footer:
        ...
        if (param('tag_links', 'string', null) !== null) { // Tag page link type:
        ...
        if (param('chapter_content', 'string', null) !== null) { // What kind of content on chapter pages?
            $this->set_setting('chapter_content', get_param('chapter_content'));
        }
        if (param('tag_content', 'string', null) !== null) { // What kind of content on tags pages?
            $this->set_setting('tag_content', get_param('tag_content'));
        }
        if (param('archive_content', 'string', null) !== null) { // What kind of content on archive pages?
            $this->set_setting('archive_content', get_param('archive_content'));
        }
        if (param('filtered_content', 'string', null) !== null) { // What kind of content on filtered pages?
            $this->set_setting('filtered_content', get_param('filtered_content'));
        }
        if (param('main_content', 'string', null) !== null) { // What kind of content on main pages?
        ...
        if (param('user_prefix', 'string', null) !== null) {	// User profile page prefix:
        ...
        if (! empty($updated_skin_ID)) {
            ...
            if (! skin_check_compatibility($updated_skin_ID, 'coll')) {	// Redirect to admin skins page selector if the skin cannot be selected:
                $Messages->add(T_('This skin cannot be used as a collection skin.'), 'error');
            }
        }

        if (param('archives_sort_order', 'string', null) !== null) { // Archive sorting
            $this->set_setting('archives_sort_order', param('archives_sort_order', 'string', false));
        }

        if (param('download_delay', 'integer', null) !== null) {	// Enable Download pages:
        ...
        if (param('feed_content', 'string', null) !== null) { // How much content in feeds?
        ...
        if (param('comment_feed_content', 'string', null) !== null) { // How much content in comment feeds?
        ...
        if (param('blog_tagline', 'string', null) !== null) {	// tagline:
            $this->set('tagline', get_param('blog_tagline'));
        }

        if (param('blog_notes', 'html', null) !== null) {	// HTML notes:
        ...
        if (in_array('features', $groups)) { // we want to load the posts related features:
            ...
            if (check_user_perm('blog_admin', 'edit', false, $this->ID)) {	// We have permission to edit advanced admin settings:
                $this->set_setting('in_skin_editing', param('in_skin_editing', 'integer', 0));
                if ($this->get_setting('in_skin_editing')) {	// Only when in-skin editing in enabled
            ...
            if (empty($orderby_1)) { // Delete if first additional field is not defined
            ...
            // Show post types:
            $checked_item_types = param('show_post_types', 'array:integer', []);
            ...
            modules_call_method('update_collection_features', [
                'edited_Blog' => &$this,
            ]);
        ...
        if (in_array('comments', $groups)) { // we want to load the comments settings:
            // load moderation statuses
            $moderation_statuses = get_visibility_statuses('moderation');
            $blog_moderation_statuses = [];
            foreach ($moderation_statuses as $status) {
        ...
        if (in_array('contact', $groups)) {	// we want to load the contact form settings:
            ...
            $msgform_additional_fields = param('msgform_additional_fields', 'array:integer', []);
            ...
            if (get_param('msgform_display_message')) {	// Update these two fields only if message is allowed:
        ...
        if (in_array('userdir', $groups)) { // we want to load the user directory settings:
        ...
        if (in_array('search', $groups)) { // we want to load the search settings:
            // Search results:
        ...
        if (in_array('other', $groups)) { // we want to load the other settings:
            // Latest comments :
        ...
        if (in_array('popup', $groups)) { // we want to load the popups settings:
            // Marketing Popup:
            ...
            $container_disps = ['front', 'posts', 'single', 'page', 'catdir'];
        ...
        if (in_array('more', $groups)) { // we want to load more settings:
            // Tracking:
        ...
        if (param('allow_comments', 'string', null) !== null) { // Feedback options:
            ...
            if ($new_feedback_status != $this->get_setting('new_feedback_status') && ($new_feedback_status != 'published' || check_user_perm('blog_admin', 'edit', false, $this->ID))) { // Only admin can set this setting to 'Public'
            ...
            if ($blog_allowtrackbacks != $this->get('allowtrackbacks') && ($blog_allowtrackbacks == 0 || check_user_perm('blog_admin', 'edit', false, $this->ID))) { // Only admin can turn ON this setting
                $this->set('allowtrackbacks', $blog_allowtrackbacks);
            }
            $blog_webmentions = param('blog_webmentions', 'integer', 0);
            if ($blog_webmentions != $this->get_setting('webmentions') && ($blog_webmentions == 0 || check_user_perm('blog_admin', 'edit', false, $this->ID))) {	// Only admin can turn ON this setting
            ...
            modules_call_method('update_collection_comments', [
                'edited_Blog' => &$this,
            ]);
        ...
        if (in_array('seo', $groups)) { // we want to load the workflow checkboxes:
            ...
            if (! $this->get_setting('canonical_item_urls')) {	// When "301 redirect to canonical URL" is disabled then we always must NOT do 301 redirect cross-posted Items:
        ...
        if (in_array('credits', $groups)) {	// We want to load the software credits settings:
            if (is_pro()) {	// Allow to remove "Powered by b2evolution" logos only for PRO version:
        ...
        if (check_user_perm('blog_admin', 'edit', false, $this->ID) ||
            ($this->ID == 0 && check_user_perm('blogs', 'create', false, $this->sec_ID))) {	// We have permission to edit advanced admin settings,
            // OR user is creating/coping new collection in section where he has an access:
            if (($blog_urlname = param('blog_urlname', 'string', null)) !== null) {	// check urlname
                if (param_check_not_empty('blog_urlname', T_('You must provide an URL collection name!'))) {
                    ...
                    if (isset($blog_urlname) && $DB->get_var(
                        'SELECT COUNT(*)
															FROM T_blogs
                    ...
                    if (isset($blog_urlname)) { // Set new urlname and save old media dir in order to rename folder to new
        ...
        if (check_user_perm('blog_admin', 'edit', false, $this->ID)) {	// We have permission to edit advanced admin settings:
            if (in_array('cache', $groups)) { // we want to load the cache params:
            ...
            if (in_array('styles', $groups)) { // we want to load the styles params:
                ...
                if ($display_alt_skin_referer) {	// Check for not empty value only when setting is enabled:
            ...
            if (in_array('login', $groups)) { // we want to load the login params:
                if (! get_setting_Blog('login_blog_ID', $this)) { // Update this only when no blog is defined for login/registration
            ...
            if (param('blog_head_includes', 'html', null) !== null) {	// HTML header includes:
            ...
            if (param('blog_body_includes', 'html', null) !== null) { // HTML body includes:
            ...
            if (param('blog_footer_includes', 'html', null) !== null) {	// HTML footer includes:
            ...
            if (param('owner_login', 'string', null) !== null) { // Permissions:
            ...
            if (($access_type = param('blog_access_type', 'string', null)) !== null) { // Blog URL parameters:
                ...
                if ($access_type == 'absolute') {
                    ...
                    if (preg_match('#^https?://[^/]+/.*#', $blog_siteurl, $matches)) { // It looks like valid absolute URL, so we may update the blog siteurl
                        $this->set('siteurl', $blog_siteurl);
                    } else { // It is not valid absolute URL, don't update the blog 'siteurl' to avoid errors
                ...
                } elseif ($access_type == 'relative') { // relative siteurl
                ...
                if ($allow_new_access_type) { // The received siteurl value was correct, may update the access_type value
                    $this->set('access_type', $access_type);
                }
            }

            if (($http_protocol = param('http_protocol', 'string', null)) !== null) {	// SSL:
            ...
            if (($url_aliases = param('blog_url_alias', 'array', null)) !== null) {
                ...
                foreach ($url_aliases as $key => $alias) {
                    ...
                    if (! preg_match('#^https?://[^/]+/.*#', $alias, $matches)) { // It is not valid absolute URL
                    ...
                    $SQL->WHERE_and('( cua_url_alias IN (' . implode(',', [$DB->quote('http' . $alias_wo_protocol), $DB->quote('https' . $alias_wo_protocol)]) . ') )'
            ...
            if (param('tinyurl_type', 'string', null) !== null) { // Tiny URL type:
                if (get_param('tinyurl_type') == 'advanced') {
                    $tinyurl_domain = param('tinyurl_domain', 'string', null);
                    if (! preg_match('#^https?://[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]+/$#', $tinyurl_domain, $matches)) { // It is not valid absolute URL
            ...
            if (is_pro() && in_array('urls', $groups)) {	// Only PRO version and on update from tab "URLs":
                // Tag source:
            ...
            if ((param('cookie_domain_type', 'string', null) !== null) && check_user_perm('blog_admin', 'edit', false, $this->ID)) {	// Cookies:
                $this->set_setting('cookie_domain_type', get_param('cookie_domain_type'));
                if (get_param('cookie_domain_type') == 'custom') {	// Update custom cookie domain:
                    $cookie_domain_custom = param('cookie_domain_custom', 'string', null);
                    preg_match('#^https?://(.+?)(:(.+?))?$#', $this->get_baseurl_root(), $coll_host);
                    if (empty($coll_host[1]) || ! preg_match('#(^|\.)' . preg_quote(preg_replace('#^\.#i', '', $cookie_domain_custom), '#') . '$#i', $coll_host[1])) {	// Wrong cookie domain:
                ...
                } else {	// Reset custom cookie domain:
                    $this->set_setting('cookie_domain_custom', '');
                }

                $this->set_setting('cookie_path_type', param('cookie_path_type', 'string', null));
                if (get_param('cookie_path_type') == 'custom') {	// Update custom cookie path:
                    $cookie_path_custom = param('cookie_path_custom', 'string', null);
                    if (! preg_match('#^' . preg_quote(preg_replace('#/$#i', '', $cookie_path_custom), '#') . '(/|$)#i', $this->get_basepath())) {	// Wrong cookie path:
                ...
                } else {	// Reset custom cookie path:
                    $this->set_setting('cookie_path_custom', '');
                }
            }

            if ((param('rsc_assets_url_type', 'string', null) !== null) && check_user_perm('blog_admin', 'edit', false, $this->ID)) { // Assets URLs / CDN:
                // Check all assets types url settings:
                $assets_url_data = [
                    'rsc_assets_url_type' => [
                        'url' => 'rsc_assets_absolute_url',
                        'folder' => '/rsc/',
                    ],
                    'media_assets_url_type' => [
                        'url' => 'media_assets_absolute_url',
                        'folder' => '/media/',
                    ],
                    'skins_assets_url_type' => [
                        'url' => 'skins_assets_absolute_url',
                        'folder' => '/skins/',
                    ],
                    'plugins_assets_url_type' => [
                        'url' => 'plugins_assets_absolute_url',
                        'folder' => '/plugins/',
                    ],
                    'htsrv_assets_url_type' => [
                        'url' => 'htsrv_assets_absolute_url',
                        'folder' => '/htsrv/',
                    ],
                ];
                ...
                foreach ($assets_url_data as $asset_url_type => $asset_url_data) {
                    ...
                    if ($asset_url_type_value === null) { // Skip this setting to don't save when it doesn't exist on form
                        continue;
                    }
                    $this->set_setting($asset_url_type, $asset_url_type_value);

                    $assets_absolute_url_value = param($asset_url_data['url'], 'url', null);
                    if ((get_param($asset_url_type) == 'absolute') && empty($assets_absolute_url_value)) { // Absolute URL cannot be empty
                        $Messages->add(sprintf(T_('Absolute URL for %s cannot be empty!'), $asset_url_data['folder']));
                    } elseif (empty($assets_absolute_url_value) || preg_match('#^(https?:)?//.+/$#', $assets_absolute_url_value)) { // It looks like valid absolute URL, so we may update it
                        $this->set_setting($asset_url_data['url'], $assets_absolute_url_value, true);
                    } else { // It is not valid absolute URL, don't update it to avoid errors
                ...
                if ($this->get('access_type') == 'absolute' &&
                    ($this->get_setting('rsc_assets_url_type') == 'basic' ||
                      ...
                      $this->get_setting('plugins_assets_url_type') == 'basic')) {	// Display warning for such settings combination:
                ...
                if ($this->get_setting('skins_assets_url_type') != 'relative') {	// Display warning if skins path is used as NOT relative url:
                    if ($this->get_setting('rsc_assets_url_type') == 'relative') {	// If rsc path is relative url:
                        $Messages->add(sprintf(
                            T_('WARNING: your %s and %s assets URL seem to be configured in a potentially undesirable way. Please check your Assets URLs below.'),
                            '<code>/rsc/</code>',
                            '<code>/skins/</code>'
                        ), 'warning');
                    }
                    if ($this->get_setting('plugins_assets_url_type') == 'relative') {	// If plugins path is relative url:
                        $Messages->add(sprintf(
                            T_('WARNING: your %s and %s assets URL seem to be configured in a potentially undesirable way. Please check your Assets URLs below.'),
                            '<code>/plugins/</code>',
                            '<code>/skins/</code>'
            ...
            if ($this->type != 'main' || ($this->type == 'main' && $this->ID > 0)) {
                if (param('aggregate_coll_IDs', 'string', null) !== null) { // Aggregate list: (can be '*')
                    $aggregate_coll_IDs = get_param('aggregate_coll_IDs');

                    if ($aggregate_coll_IDs != '*') {	// Sanitize the string
            ...
            } elseif ($this->type == 'main' && $this->ID === 0) { // Only on new Home/Main collection
            ...
            if ($media_location !== null) { // Media files location:
                $old_media_dir = $this->get_media_dir(false);
                ...
                switch ($this->get('media_location')) {
                    ...
                    case 'subdir':
                        ...
                        } else { // Test if it's below $media_path (subdir!)
            ...
            if ($this->ID > 0 && ! param_errors_detected() && ! empty($old_media_dir)) { // No error were detected before and possibly the media directory path was updated, check if it can be managed
     ...
     */
    public function load_inskin_statuses($type)
    {
        if (($type != 'post') && ($type != 'comment')) { // Invalid type
            debug_die('Invalid type to load blog inskin statuses!');
        }

        // Get possible front office statuses
        $inskin_statuses = get_visibility_statuses('keys', ['deprecated', 'trash', 'redirected']);
        $selected_inskin_statuses = [];
        foreach ($inskin_statuses as $status) {
            if (param($type . '_inskin_' . $status, 'integer', 0)) { // This status was selected
     ...
     */
    public function set_media_subdir($path)
    {
     ...
     */
    public function set_media_fullpath($path)
    {
     ...
     */
    public function favorite($user_ID = null, $setting = null)
    {
        ...
        if ($this->ID && $user_ID) {
            if (is_null($setting)) { // just return current favorite status
     ...
     */
    public function gen_blogurl($type = 'default')
    {
        global $baseprotocol, $basehost, $baseport, $baseurl, $Settings;

        if ($type == 'default') {	// Use current access type of this collection:
        ...
        switch ($type) {
            ...
            case 'default':
                ...
                if (($Settings->get('default_blog_ID') == $this->ID)
                    || preg_match('#^https?://#', $this->siteurl)) { // Safety check! We only do that kind of linking if this is really the default blog...
     ...
     */
    public function gen_baseurl($type = 'default')
    {
        if ($type == 'default') {	// Use current access type of this collection:
            $type = $this->get('access_type');
        }
        ...
        if (substr($url, -1) != '/') {	// Crop an url part after the last "/":
     ...
     * @return string
     */
    public function get_baseurl_root()
    {
        if (empty($this->baseurl_root)) {	// Initialize collection domain only first time:
     ...
     * @return string
     */
    public function get_basepath_url()
    {
        if (empty($this->basepath_url)) {
            if ($this->get('access_type') == 'absolute') {	// Use whole url when it is absolute:
                // Remove text like 'index.php' at the end:
                $this->basepath_url = preg_replace('/^(.+\/)([^\/]+\.[^\/]+)?$/', '$1', $this->get('siteurl'));
            } else {	// Build url from base and sub path:
    ...
    {
        if (empty($this->basepath)) {	// Initialize base path only first time:
            if ($this->get('access_type') == 'absolute') {	// If collection URL is absolute we should extract sub path from the URL because it can be different than site url:
                $this->basepath = str_replace($this->get_baseurl_root(), '', $this->get_basepath_url());
            } else {	// Otherwise we can use sub path of site url:
                global $basesubpath;
     ...
     * @return string
     */
    public function get_cookie_domain($force_type = false)
    {
        if (empty($this->cookie_domain)) {	// Initialize only first time:
            $cookie_domain_type = ($force_type === false) ? $this->get_setting('cookie_domain_type') : $force_type;

            if ($cookie_domain_type == 'custom') {	// Custom cookie domain:
                return $this->get_setting('cookie_domain_custom');
            } else {	// Automatic cookie domain:
                if ($this->get('access_type') == 'absolute') {	// If collection URL is absolute we should extract cookie domain from the URL because it can be different than site url:
                    preg_match('#^https?://(.+?)(:(.+?))?$#', $this->get_baseurl_root(), $matches);
                    $coll_host = $matches[1];

                    if (strpos($coll_host, '.') === false) {	// localhost or windows machine name:
                        $this->cookie_domain = $coll_host;
                    } elseif (preg_match('~^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$~i', $coll_host)) {	// The basehost is an IP address, use the basehost as it is:
                        $this->cookie_domain = $coll_host;
                    } else {	// Keep the part of the basehost after the www. :
     ...
     * @return string
     */
    public function get_cookie_path($force_type = false)
    {
        if (empty($this->cookie_path)) {	// Initialize only first time:
            $cookie_domain_type = ($force_type === false) ? $this->get_setting('cookie_path_type') : $force_type;

            if ($cookie_domain_type == 'custom') {	// Custom cookie path:
                return $this->get_setting('cookie_path_custom');
            } else {	// Automatic cookie path:
     ...
     */
    public function get_htsrv_url($force_https = false)
    {
        if (! isset($this->htsrv_urls[$force_https])) {	// Initialize collection htsrv URL only first time and store in cache:
            ...
            if (strpos($this->get_basepath_url(), $required_htsrv_url) !== false) {	// If current request path contains the required htsrv URL:
            ...
            if (isset($coll_url_parts['scheme'], $htsrv_url_parts['scheme']) &&
                ...
                $coll_url_parts['scheme'] != $htsrv_url_parts['scheme']) {	// If this collection uses an url with scheme like "https://" then
     ...
     * @return string URL to /htsrv/ folder
     */
    public function get_local_htsrv_url($url_type = null, $force_https = false)
    {
        $url_type = is_null($url_type) ? $this->get_setting('htsrv_assets_url_type') : $url_type;

        if ($url_type == 'relative') {	// Relative URL:
            return $this->get_htsrv_url($force_https);
        } elseif ($url_type == 'absolute') {	// Absolute URL:
            // Get URL with protocol which is defined by collection setting "SSL":
     ...
     */
    public function get_local_media_url($url_type = null, $force_normal_using = false)
    {
        $url_type = is_null($url_type) ? $this->get_setting('media_assets_url_type') : $url_type;

        if ($url_type == 'relative') {	// Relative URL:
            if (! $force_normal_using && is_admin_page()) {	// Force to absolute base URL on back-office side and email template:
                global $media_url;
                $local_media_url = $media_url;
            } else {	// Use absolute URL relative to collection media folder:
                global $media_subdir;
     ...
     */
    public function get_local_rsc_url($url_type = null)
    {
     ...
     */
    public function get_local_skins_url($url_type = null)
    {
     ...
     */
    public function get_local_plugins_url($url_type = null)
    {
     ...
     */
    public function get_local_xmlsrv_url()
    {
     ...
     * @param string week
     */
    public function gen_archive_url($year, $month = null, $day = null, $week = null, $glue = '&amp;', $paged = 1)
    {
        $blogurl = $this->gen_blogurl();

        $archive_links = $this->get_setting('archive_links');

        if ($archive_links == 'param') {	// We reference by Query
            $separator = '';
        } else {	// We reference by extra path info
            $separator = '/';
        }
        ...
        if ($archive_links == 'param') {	// We reference by Query
            ...
            if (! empty($archive_posts_per_page) && $archive_posts_per_page != $this->get_setting('posts_per_page')) {	// We want a specific post per page count:
        ...
        if ($paged > 1) {	// We want a specific page:
     ...
     */
    public function gen_archive_link($text, $title, $year, $month = null, $day = null, $week = null, $glue = '&amp;', $paged = 1)
    {
        $link = '<a';
     ...
     */
    public function get_archive_url($date, $glue = '&amp;')
    {
     ...
     */
    public function gen_tag_url($tag, $paged = 1, $glue = '&')
    {
        ...
        switch ($link_type) {
            case 'param':
                ...
                if (! empty($tag_posts_per_page) && $tag_posts_per_page != $this->get_setting('posts_per_page')) {	// We want a specific post per page count:
     ...
     */
    public function get_tag_link($tag, $text = null, $attribs = [])
    {
        if ($this->get_setting('tag_rel_attrib') && $this->get_setting('tag_links') == 'prefix-only') {	// add rel=tag attrib -- valid only if the last part of the url is the tag name
     ...
     */
    public function get_allowed_item_status($status = null, $perm_target = null)
    {
        if (! is_logged_in()) {	// User must be logged in:
            return $this->get_max_allowed_status($status);
        }

        if (empty($status)) {	// Get default item status:
        ...
        foreach ($visibility_statuses as $status_key => $status_value) {
            if ($status_key == $status || empty($status)) {	// Start to check statuses only after the requested status:
                $status_is_allowed = true;
            }
            if ($status_key == $max_allowed_status) {	// All next statuses are allowed for this collection:
                $max_status_is_allowed = true;
            }
            if ($status_is_allowed && $max_status_is_allowed && check_user_perm('blog_post!' . $status_key, 'create', false, $this->ID)) {	// This status is allowed for this collection and current has a permission:
                $allowed_status = $status_key;
                break;
            }
        }

        if ($allowed_status === null) {	// Use max allowed status if it could not be found above:
    ...
    public function get_default_cat_ID()
    {
        ...
        if (empty($this->default_cat_ID)) {
            if ($default_cat_ID = $this->get_setting('default_cat_ID')) {	// A specific cat has previosuly been configured as the default:
        ...
        if (empty($this->default_cat_ID)) {	// If the previous query has returned NULL
            ...
            $SQL->WHERE_and('cat_meta = 0');
            $SQL->LIMIT('1');
            if ($this->get_setting('category_ordering') == 'manual') {	// Manual order
    ...
    public function get_media_dir($create = true)
    {
        ...
        if (! $Settings->get('fm_enable_roots_blog')) { // User directories are disabled:
        ...
        if ($create && ! is_dir($mediadir)) {
            ...
            if (! is_writable(dirname($mediadir))) { // add error
                if (is_admin_page()) {
                    $Messages->add_to_group(
                        sprintf(T_("Media directory &laquo;%s&raquo; could not be created, because the parent directory is not writable or does not exist."), $msg_mediadir_path),
                        'error',
                        T_('Media directory file permission error') . get_manual_link('media-file-permission-errors') . ':'
                    );
                }
                return false;
            } elseif (! evo_mkdir($mediadir)) { // add error
                if (is_admin_page()) {
                    $Messages->add_to_group(
                        sprintf(T_("Media directory &laquo;%s&raquo; could not be created."), $msg_mediadir_path),
                        'error',
                        T_('Media directory creation error') . get_manual_link('media-file-permission-errors') . ':'
                    );
                }
                return false;
            } else { // add note:
     ...
     */
    public function get_filemanager_link()
    {
        global $admin_url;
     ...
     */
    public function get_tempskin_url($skin_folder_name, $additional_params = '', $halt_on_error = false)
    {
        $SkinCache = &get_SkinCache();
        if (! $Skin = &$SkinCache->get_by_folder($skin_folder_name, $halt_on_error)) {	// If no requested skin try to fallback to first found feed skin:
            $SkinCache->load_by_type('feed');
            $skin_folder_name = false;
            foreach ($SkinCache->cache as $Skin) {
                if ($Skin->type == 'feed') {	// Use the first found feed skin:
                    $skin_folder_name = $Skin->folder;
                    break;
                }
            }
            if ($skin_folder_name === false) {	// No feed skin found:
                return false;
            }
        }
     ...
     * @param string
     */
    public function get_item_feed_url($skin_folder_name)
    {
     ...
     * @return string|false URL or FALSE if none feed skin is not installed in system
     */
    public function get_comment_feed_url($skin_folder_name)
    {
     ...
     */
    public function get($parname, $params = [])
    {
        ...
        switch ($parname) {
            case 'access_type':
                ...
                if ($access_type_value == 'subdom' && is_valid_ip_format($basehost)) {	// Don't allow subdomain for IP address:
            ...
            case 'customizer_url':
                if (check_user_perm('blog_properties', 'edit', false, $this->ID) ||
                    ($Settings->get('site_skins_enabled') && check_user_perm('options', 'edit'))) {	// Return customizer URL only if current User can edit skin settings of collection or site:
                    if (! empty($params['customizing_url'])) {	// Get customizing URL from passed param:
                        $customizing_url = $params['customizing_url'];
                    } elseif (get_param('customizing_url') != '') {	// Get customizing URL from currently provided _GET param 'customizing_url':
                        $customizing_url = get_param('customizing_url');
                    } else {	// Use current URL for customizing URL:
                        $customizing_url = get_current_url();
                    }
                    if ($customizing_url == '#baseurl#') {	// Use base URL of this collection:
                ...
                } else {	// Return this collection URL instead:
                    return $this->get('url');
                }
            ...
            case 'content_requires_loginurl':
                ...
                if ($login_Blog = &get_setting_Blog('login_blog_ID', $this)) { // Use special blog for login/register actions if it is defined in general settings
                    $url = url_add_param($login_Blog->gen_blogurl(), 'disp=' . $url_disp, $params['glue']);
                } else { // Use login/register urls of this blog
                    $url = url_add_param($this->gen_blogurl(), 'disp=' . $url_disp, $params['glue']);
                }
                if (! empty($params['url_suffix'])) { // Append url suffix
            ...
            case 'alt_skin_ID':
                ...
                if ($parname == 'mobile_skin_ID' || $parname == 'tablet_skin_ID' || $parname == 'alt_skin_ID') {
                    if (empty($result) && ! (isset($params['real_value']) && $params['real_value'])) {	// Empty values(NULL, 0, '0', '') means that use the same as normal case:
        ...
        if (! empty($disp_param)) { // Get url depending on value of param 'disp'
            $this_Blog = &$this;
            if (in_array($disp_param, ['threads', 'messages', 'contacts', 'msgform', 'user', 'profile', 'avatar', 'pwdchange', 'userprefs', 'subs', 'register_finish', 'visits', 'closeaccount'])) { // Check if we can use this blog for messaging actions or we should use spec blog
                if ($msg_Blog = &get_setting_Blog('msg_blog_ID')) { // Use special blog for messaging actions if it is defined in general settings
                    $this_Blog = &$msg_Blog;
                }
            }

            if ($this_Blog->get_setting('front_disp') == $disp_param) {	// Get home page of this blog because front page displays current disp:
                $url = $this_Blog->gen_blogurl('default');
            } elseif ($disp_param == 'user' && (isset($params['user_login']) || isset($params['user_ID']))) {	// Use alias if user login or ID is provided:
                $UserCache = &get_UserCache();
                if (! isset($params['user_ID']) &&
                    ($this_Blog->get_setting('user_links') == 'params' || $this_Blog->get_setting('user_links') == 'prefix_id')) {	// We need get user ID by login:
                    if ($param_User = &$UserCache->get_by_login($params['user_login'])) {	// Set user ID if it is detected by login:
                ...
                if (! isset($params['user_login']) &&
                    $this_Blog->get_setting('user_links') == 'prefix_login') {	// We need get user login by ID:
                    if ($param_User = &$UserCache->get_by_ID($params['user_ID'], false, false)) {	// Use login if user is detected in DB:
                ...
                if ($this_Blog->get_setting('user_links') == 'params' || $this_Blog->get_setting('user_prefix') == '') {	// Use params E-g: ?disp=user&user_ID=4
                    $url = url_add_param($this_Blog->gen_blogurl(), 'disp=user' . $params['glue'] . 'user_ID=' . $params['user_ID'], $params['glue']);
                } elseif ($this_Blog->get_setting('user_links') == 'prefix_id') {	// Use prefix with user ID:
            ...
            } else {	// Add disp param to blog's url when current disp is not a front page:
                $url = url_add_param($this_Blog->gen_blogurl(), 'disp=' . $disp_param, $params['glue']);
            }

            if ($disp_param == 'pwdchange' || $disp_param == 'register_finish') {	// Force these pages to https if it is required by setting "Require SSL":
                $url = force_https_url($url, 'login');
            }
     ...
     */
    public function get_advanced_perms_warning()
    {
        global $admin_url;
     ...
     */
    public function get_setting($parname, $real_value = false)
    {
        global $Settings;
        ...
        switch ($parname) {
            ...
            case 'post_moderation_statuses':
                if ($result === null) { // moderation_statuses was not set yet, set the default value, which depends from the blog type
            ...
            case 'post_inskin_statuses':
                if ($result === null) { // inskin_statuses was not set yet, set the default value, which depends from the blog type
            ...
            case 'new_feedback_status':
                if ($result === null) { // Default post/comment status was not set yet, use a default value corresponding to the blog type
            ...
            case 'msgform_subject_options':
                ...
                if (! empty($subject_list)) {
                    ...
                    foreach ($subject_list as $subject_option) {
                        if (utf8_trim($subject_option) != '') {	// Use only not empty line:
            ...
            case 'webmentions':
                if ($this->get_setting('allow_access') != 'public') {	// Disable receiving of webmentions for not public collections:
                    $result = 0;
                }
                break;

            case 'ping_plugins':
                if ($this->get_setting('allow_access') != 'public') {	// Disable ping plugins for not public collections:
                    $result = '';
                }
                break;
     ...
     */
    public function dget_setting($parname, $format = 'htmlbody')
    {
        $this->load_CollectionSettings();
     ...
     * @param string Output format, see {@link format_to_output()}
     */
    public function disp_setting($parname, $format = 'htmlbody')
    {
        $this->load_CollectionSettings();
     ...
     * Make sure collection settings are loaded.
     */
    public function load_CollectionSettings()
    {
    ...
    public function dbinsert()
    {
        ...
        $skin_types = ['normal', 'mobile', 'tablet', 'alt'];
        foreach ($skin_types as $skin_type) {
            $skin_ID = $this->get($skin_type . '_skin_ID', [
                'real_value' => true,
            ]);
            if (empty($skin_ID)) {	// Only if skin is not selected during creating:
                $default_skin_ID = $Settings->get('def_' . $skin_type . '_skin_ID');
                if (! empty($default_skin_ID)) {	// And if default skin is defined for the skin type:
        ...
        if ($this->get('order') == 0) { // Set an order as max value of previous order + 1 if it is not defined yet
        ...
        if ($Settings->get('default_blog_ID') == -1 || get_setting_Blog('default_blog_ID')) {	// Don't set a default collection if it is already defined and the collection exists in DB OR back-office page is used for this:
        ...
        if (parent::dbinsert()) {
            ...
            if ($set_default_blog_ID) { // Use this blog as default because it is probably first created
            ...
            // Update locales:
            if (empty($this->locales)) {	// Don't forget to store main locale in extra locales table:
                $this->locales = [
                    $this->get('locale') => null,
                ];
            }
            $this->update_locales();
            ...
            if ($this->base_collection_ID == 0) {
                ...
                if (! empty($default_post_type_ID)) { // Enable post type that is used by default for this collection:
                    global $DB;
            ...
            $Plugins->trigger_event('AfterCollectionInsert', $params = [
                'Blog' => &$this,
            ]);
     ...
     * @param array Params
     */
    public function create($kind = '', $params = [])
    {
        ...
        if (is_file($basepath . $stub_filename)) {	// Stub file exists and is waiting ;)
        ...
        } elseif ($this->get('access_type') == 'relative') { // Show error message only if stub file should exists!
        ...
        if (! empty($this->owner_user_ID)) { // Proceed insertion:
        ...
        if ($params['create_demo_contents']) {
            ...
            if ($params['create_demo_org'] && check_user_perm('orgs', 'create', false)) { // Create the demo organization
                if ($new_demo_organization = create_demo_organization($this->get('owner_user_ID'))) {
                    $user_org_IDs = [$new_demo_organization->ID];
                }
            }
        ...
        if ($this->get('advanced_perms') && $default_perms_inserted) {	// Display this warning if coolection has the enabled advanced perms AND default permissions have been inserted for at least one group:
            $Messages->add(
                sprintf(
                    T_('ATTENTION: We have set some advanced permissions for this collection. Please verify the <a %s>User permissions</a> and the <a %s>Group permissions</a> now.'),
    ...
    public function duplicate($params = [])
    {
        ...
        foreach ($source_fields as $source_field_name => $source_field_value) {
            ...
            if ($source_field_name == 'ID') {	// Skip field ID:
                continue;
            }
            if (isset($this->$source_field_name)) {	// Unset current value in order to assing new below, especially to update this in array $this->dbchanges:
        ...
        if (! check_user_perm('blog_admin', 'edit', false, $duplicated_coll_ID)) { // validate the urlname, which was already set by init_by_kind() function
        ...
        $DB->query(
            'INSERT INTO T_coll_group_perms
        ...
        foreach ($widget_containers as $widget_container) {
            ...
            $r = $DB->query(
                'INSERT INTO T_widget__container
            ...
            if ($r && $DB->insert_id > 0) {	// Duplicate all widgets of the inserted container:
                $DB->query(
                    'INSERT INTO T_widget__widget
        ...
        if ($params['duplicate_items']) {	// Duplicate categories and posts/items:
            ...
            $new_cats = []; // Store all new created categories with key as ID of copied category in order to correct assign parent IDs
            $ChapterCache = &get_ChapterCache();
            foreach ($source_cats as $source_cat_fields) {	// Copy each category separately because of unique field "cat_urlname":
                ...
                foreach ($source_cat_fields as $source_cat_field_name => $source_cat_field_value) {
                    ...
                    if ($source_cat_field_name == 'ID' || $source_cat_field_name == 'blog_ID') {	// Skip these fields, they must be new:
                        continue;
                    }
                    $new_Chapter->set($source_cat_field_name, $source_cat_field_value);
                }
                // Insert the duplicated category:
                if ($new_Chapter->dbinsert()) {	// If category has been inserted successfully, then update IDs for correct parent hierarchy:
                    // Key - ID of copied category, Value - new created category:
                    $new_cats[$source_cat_fields['cat_ID']] = $new_Chapter;
                }
            }
            foreach ($new_cats as $duplicated_cat_ID => $new_Chapter) {	// Update wrong parent IDs to IDs of new created categories:
                ...
                if ($this->get_setting('default_cat_ID') == $duplicated_cat_ID) {	// Update wrong default category with correct ID of new duplicated category:
                    $this->set_setting('default_cat_ID', $new_Chapter->ID);
                    $this->dbupdate();
                }
            }

            if (! empty($new_cats)) {	// Duplicate posts if collection has at least one category:
                ...
                foreach ($old_items as $old_item) {
                    ...
                    if (isset($new_cats[$old_item['post_main_cat_ID']])) {	// Use correct category ID:
                    ...
                    // Duplicate a post:
                    $DB->query(
                        'INSERT INTO T_items__item ( ' . implode(', ', array_keys($old_item)) . ' ) VALUES ( ' . $DB->quote($old_item) . ' )',
                ...
                if (! empty($new_items)) {
                    ...
                    if (! empty($new_extra_cats_values)) {
                        $DB->query(
                            'INSERT INTO T_postcats ( ' . $old_extra_cats_SQL->get_select('') . ' ) VALUES ' . implode(', ', $new_extra_cats_values),
                    ...
                    if (! empty($new_tags_values)) {
                        $DB->query(
                            'INSERT INTO T_items__itemtag ( ' . $old_tags_SQL->get_select('') . ' ) VALUES ' . implode(', ', $new_tags_values),
                    ...
                    if (! empty($new_settings_values)) {
                        $DB->query(
                            'INSERT INTO T_items__item_settings ( ' . $old_settings_SQL->get_select('') . ' ) VALUES ' . implode(', ', $new_settings_values),
                    ...
                    if (! empty($new_links_values)) {
                        $DB->query(
                            'INSERT INTO T_links ( ' . $old_links_SQL->get_select('') . ' ) VALUES ' . implode(', ', $new_links_values),
                            'Duplicate links for posts of collection #' . $duplicated_coll_ID
                        );
                    }

                    if ($params['duplicate_comments']) {	// Duplicate comments:
                        ...
                        foreach ($old_comments as $old_comment) {
                            if (isset($new_items[$old_comment['comment_item_ID']])) {	// Insert only comments with correct item ID:
                                ...
                                $DB->query(
                                    'INSERT INTO T_comments ( ' . implode(', ', array_keys($old_comment)) . ' ) VALUES ( ' . $DB->quote($old_comment) . ' )',
                        ...
                        if (! empty($new_comments)) {	// Duplicate links/attachments of the comments:
        ...
        } else {	// Create at least one default category:
            load_class('chapters/model/_chapter.class.php', 'Chapter');
     ...
     */
    public function insert_default_group_permissions()
    {
        ...
        $groups = [
            ...
            'suspect' => [
                'ID' => 5,
                'name' => 'Misbehaving/Suspect Users',
            ],
            'spam' => [
                'ID' => 6,
                'name' => 'Spammers/Restricted Users',
            ],
            'blogb' => [
                'ID' => 7,
                'name' => 'Blog B Members',
        ...
        if ($this->type == 'forum') {	// Set special permissions for collection with type "Forum"
        ...
        if ($this->type == 'std' && $this->get('shortname') == 'Blog B') {	// Set special permission for group "Blog B Members" for collection "Blog B":
            $group_permissions['blogb'] = [
                'ismember' => 1,
                'can_be_assignee' => 0,
                'workflow_status' => 0,
                'workflow_user' => 0,
        ...
        foreach ($groups as $group_key => $group) {
            if (! isset($group_permissions[$group_key])) {	// Skip group without default permissions:
                continue;
            }
            ...
            if (! $DB->get_var($group_SQL)) {	// Skip this group because we cannot find it in DB:
                continue;
            }

            if (! isset($insert_fields)) {	// Initialize DB field names only first time:
        ...
        if (isset($insert_fields)) {	// Insert group permissions into DB:
            $r = $DB->query('INSERT INTO T_coll_group_perms ( bloggroup_blog_ID, bloggroup_group_ID, ' . $insert_fields . ' )
					VALUES ( ' . implode(' ), ( ', $insert_values) . ' )');
            if ($r) {	// The permissions have been inserted successfully
                return true;
            }
        }
    ...
    public function dbupdate()
    {
        ...
        $Plugins->trigger_event('AfterCollectionUpdate', $params = [
            'Blog' => &$this,
        ]);

        if (get_param('action') == 'update_confirm') {	// Reduce statuses of posts and comments by max allowed status of this collection ONLY if this has been confirmed:
     ...
     */
    public function dbdelete($echo = false)
    {
        ...
        if ($root_directory = $FileRootCache->get_root_dir('collection', $old_ID)) { // Delete the folder only when it is detected
        ...
        $this->ID = $old_ID;
        $Plugins->trigger_event('AfterCollectionDelete', $params = [
            'Blog' => &$this,
        ]);
        $this->ID = 0;

        if (isset($Settings)) { // Reset settings related to the deleted blog
            if ($Settings->get('default_blog_ID') == $old_ID) { // Reset default blog ID
                $Settings->set('default_blog_ID', 0);
            }
            if ($Settings->get('info_blog_ID') == $old_ID) { // Reset info blog ID
                $Settings->set('info_blog_ID', 0);
            }
            if ($Settings->get('login_blog_ID') == $old_ID) { // Reset login blog ID
                $Settings->set('login_blog_ID', 0);
            }
            if ($Settings->get('msg_blog_ID') == $old_ID) { // Reset messaging blog ID
     ...
     * Template tag
     */
    public function tagline($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => ' ',
     ...
     * Template tag
     */
    public function longdesc($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => ' ',
     ...
     */
    public function get_maxlen_name($maxlen = 50)
    {
    ...
    public function get_extended_name()
    {
        ...
        if (! empty($this->shortname)) { // Short name
            $names[] = $this->shortname;
        }

        if (! empty($this->name)) { // Title
            $names[] = $this->name;
        }
    ...
    public function get_skin_type()
    {
        ...
        if (! isset($Session)) {	// Session must be defined to autodetect current skin type:
            return 'normal';
        }
        ...
        if ($skin_type == 'mobile' || $skin_type == 'tablet' || $skin_type == 'alt') {	// Check if collection use different mobile/tablet/alt skin or same as normal skin:
            $skin_ID = $this->get($skin_type . '_skin_ID', [
                'real_value' => true,
            ]);
            if (empty($skin_ID)) {	// Force to use widgets for normal skin because collection doesn't use different skin for mobile/tablet session:
                $skin_type = 'normal';
            }
        }
     ...
     */
    public function get_skin_ID($skin_type = 'auto', $real_value = false)
    {
        switch ($skin_type) {
            case 'auto':
    ...
    {
        $skin_ids = [$this->get('normal_skin_ID')];
     ...
     * @return array
     */
    public function get_skin_containers($skin_type = 'auto')
    {
        $SkinCache = &get_SkinCache();
        $coll_Skin = &$SkinCache->get_by_ID($this->get_skin_ID($skin_type, true), false, false);

        return $coll_Skin ? $coll_Skin->get_containers() : [];
    }
     ...
     */
    public function get_main_containers($skin_type = 'normal', $load_container_ids = false)
    {
        if (! isset($this->widget_containers)) {	// Initialize
            $this->widget_containers = [];
        }

        if (! isset($this->widget_containers[$skin_type])) {	// Get widget containers of requested collection skin:
        ...
        if ($load_container_ids &&
            ...
            ! empty($this->widget_containers[$skin_type])) {	// Initialize IDs of containers:
            global $DB;
            ...
            if (! isset($this->widget_containers_ids_loaded)) {
                $this->widget_containers_ids_loaded = [];
            }
     ...
     */
    public function db_save_main_containers($verbose = false, $skin_type = 'all')
    {
        global $DB, $Messages;

        $DB->begin();

        if ($skin_type == 'all') {	// Use all skin types:
            $skin_types = ['normal', 'mobile', 'tablet', 'alt'];
        } elseif (in_array($skin_type, ['normal', 'mobile', 'tablet', 'alt'])) {	// Use single skin type:
            $skin_types = [$skin_type];
        } else {	// Use Normal/Default skin for all other unknown requestes:
            $skin_types = ['normal'];
        }
        ...
        foreach ($skin_types as $skin_type) {
            if (! ($coll_Skin = &$SkinCache->get_by_ID($this->get($skin_type . '_skin_ID'), false, false))) {	// Skip wrong skin to avoid errors:
                continue;
            }
            ...
            $update_widget_containers_sql_rows = [];
            $new_widget_containers = [];
            foreach ($skin_containers as $wico_code => $wico_data) {
                if (! isset($coll_containers_skin_types[$skin_type][$wico_code])) {	// Create only those containers which are not saved yet:
                ...
                } else {	// Check if we should update some container data:
                    $update_widget_container_sql_data = [];
                    if ($coll_containers_skin_types[$skin_type][$wico_code]['name'] != $wico_data[0]) {	// Update if name is different:
                        $update_widget_container_sql_data[] = 'wico_name = ' . $DB->quote($wico_data[0]);
                    }
                    if ($coll_containers_skin_types[$skin_type][$wico_code]['order'] != $wico_data[1]) {	// Update if order is different:
                        $update_widget_container_sql_data[] = 'wico_order = ' . $DB->quote($wico_data[1]);
                    }
                    if (! empty($update_widget_container_sql_data)) {	// Update different data of the container:
                        $updated_containers_num += $DB->query(
                            'UPDATE T_widget__container
            ...
            foreach ($skin_default_containers as $wico_code => $wico_data) {
                if (! isset($skin_containers[$wico_code]) &&
                    ...
                    $coll_containers_skin_types[$skin_type][$wico_code]['count'] == 0) {	// Delete ONLY EMPTY default container if it exists in DB but not declared in the Skin (or cannot be found in fallback files of old skins):
                    $delete_widget_containers[] = $coll_containers_skin_types[$skin_type][$wico_code]['ID'];
                }
            }

            if ($verbose) {	// Display how many containers are declared or scanned by the collection skin:
                $skin_declared_containers = $coll_Skin->get_declared_containers();
                if ($coll_Skin->get_api_version() > 5 || ! empty($skin_declared_containers)) {	// If skin has the declared widget containers:
                    $Messages->add(sprintf(T_('%d containers declared by skin "%s".'), $skin_containers_num, $coll_Skin->get_name()), 'note');
                } else {	// If skin scans widget containers from skin and fallback files:
            ...
            if (! empty($update_widget_containers_sql_rows)) { // Insert all containers defined by the blog skins into the database
                $created_containers_num += $DB->query(
                    'REPLACE INTO T_widget__container( wico_code, wico_skin_type, wico_name, wico_coll_ID, wico_order, wico_main ) VALUES'
                        . implode(', ', $update_widget_containers_sql_rows),
                    'Insert new widget containers for collection #' . $this->ID
                );
            }

            if (! empty($new_widget_containers)) {	// Install default widgets:
                ...
                foreach ($new_widget_containers as $new_widget_container_code) {
                    ...
                    $new_container_messages[] = sprintf(
                        T_('%s has been added (populated with %d widgets)'),
            ...
            if (! empty($delete_widget_containers)) {	// Delete containers which are not used by Skin:
                $deleted_containers_num += $DB->query(
                    'DELETE FROM T_widget__container
     ...
     */
    public function get_skin_folder($skin_type = 'auto')
    {
        $blog_skin_ID = $this->get_skin_ID($skin_type);
        if (empty($blog_skin_ID) && $skin_type != 'auto') { // Get default skin ID if it is not defined for the selected session type yet
     ...
     */
    public function contact_link($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => ' ',
        ...
        if (empty($contact_url)) {	// If contact URL is not available:
            return false;
        }
     ...
     * @param array
     */
    public function help_link($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => ' ',
     ...
     */
    public function footer_text($params)
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => ' ',
        ...
        $text = preg_replace_callback('~\$([a-z]+)\$~', [$this, 'replace_callback'], $text);
     ...
     */
    public function get_contact_url($with_redirect = false)
    {
        $owner_User = &$this->get_owner_User();
        if (! $owner_User->get_msgform_possibility()) { // user does not allow contact form
            return null;
        }

        $blog_contact_url = $this->get('msgformurl', [
        ...
        if ($with_redirect) {
            ...
            } elseif ($owner_User->get_msgform_possibility() != 'login') {	// The URL will be made relative on the next page (this is needed when $htsrv_url is on another domain! -- multiblog situation )
                $redirect_to = regenerate_url('', '', '', '&');
            } else {	// no email option - try to log in and send private message (only registered users can send PM):
     ...
     */
    public function get_aggregate_coll_IDs()
    {
        $aggregate_coll_IDs = $this->get_setting('aggregate_coll_IDs');

        if (empty($aggregate_coll_IDs) || $aggregate_coll_IDs == '-') { // No aggregation, return only the current blog:
            return $this->ID;
        } elseif ($aggregate_coll_IDs == '*') { // Aggregation of all blogs
            return null;
        }
     ...
     */
    public function get_sql_where_aggregate_coll_IDs($field, $force_coll_IDs = null)
    {
        if (is_null($force_coll_IDs)) { // Use collection IDs from blog setting
            $aggregate_coll_IDs = $this->get_setting('aggregate_coll_IDs');
        } else { // Force collections IDs
            $aggregate_coll_IDs = $force_coll_IDs;
        }

        if (empty($aggregate_coll_IDs) || $aggregate_coll_IDs == '-') { // We only want posts from the current blog:
            return ' ' . $field . ' = ' . $this->ID . ' ';
        } elseif ($aggregate_coll_IDs == '*') { // We are aggregating all blogs
            return ' 1 ';
        } else { // We are aggregating posts from several blogs:
            global $DB;
     ...
     */
    public function get_tag_post_count($tag)
    {
        global $DB;
     ...
     */
    public function get_ajax_form_enabled()
    {
        if (is_logged_in()) {
     ...
     * @return string
     */
    public function get_timestamp_min()
    {
     ...
     * @return string
     */
    public function get_timestamp_max()
    {
     ...
     */
    public function get_write_item_url($cat_ID = 0, $post_title = '', $post_urltitle = '', $post_type_usage = '')
    {
        $url = '';

        if (is_logged_in(false) ||
            (! is_logged_in() && $this->get_setting('post_anonymous'))) {	// Only logged in and activated users can write a Post,
            ...
            if ($selected_Chapter &&
                ($selected_Chapter->get('lock') ||
                  ($cat_ItemType = &$selected_Chapter->get_ItemType()) === false)) {	// Don't allow to create new post with this category if it is locked or no default item type for the category:
                return '';
            }

            if (! is_logged_in() || check_user_perm('blog_post_statuses', 'edit', false, $this->ID)) {	// We have permission to add a post with at least one status:
                if ($this->get_setting('in_skin_editing') && ! is_admin_page()) {	// We have a mode 'In-skin editing' for the current Blog
                    // User must have a permission to publish a post in this blog
                    $cat_url_param = '';
                    if ($selected_Chapter) {	// Link to create a Item with predefined category
                        $cat_url_param = '&amp;cat=' . $selected_Chapter->ID;
                        if (empty($post_type_usage) &&
                            ($cat_ItemType = &$selected_Chapter->get_ItemType())) {	// Use predefined Item Type from selected category:
                    ...
                    if (! empty($cat_ID)) {	// Add category param to preselect category on the form
                        $url = url_add_param($url, 'cat=' . $cat_ID);
                    }
                } else { // User does not have any means to edit the post!
                    return '';
                }

                if (! empty($post_title)) { // Append a post title
                    $url = url_add_param($url, 'post_title=' . urlencode($post_title));
                }
                if (! empty($post_urltitle)) { // Append a post urltitle
                    $url = url_add_param($url, 'post_urltitle=' . urlencode($post_urltitle));
                }
                if (! empty($post_type_usage)) { // Append a post type ID
                    global $DB;
     ...
     */
    public function get_create_chapter_url($cat_ID = 0)
    {
        $url = '';

        if (is_logged_in(false)) {	// Only logged in and activated users can write a Post
            if (check_user_perm('admin', 'restricted') &&
                check_user_perm('blog_cats', 'edit', false, $this->ID)) {	// Check permissions to create a new chapter in this blog
                global $admin_url;
                $url = $admin_url . '?ctrl=chapters&amp;action=new&amp;blog=' . $this->ID;
                if (! empty($cat_ID)) {	// Add category param to preselect category on the form:
                    $url = url_add_param($url, 'cat_parent_ID=' . $cat_ID);
                }
                if (! is_admin_page()) {	// Add this param to redirect after saving to parent category permanent url:
    ...
    public function has_access($User = null)
    {
        ...
        if ($allow_access == 'public') {	// Everyone has an access to this collection:
            return true;
        }

        if (! is_logged_in()) {	// Only logged in users have an access to this collection:
            return false;
        } elseif ($allow_access == 'members' && ! $User->check_perm('blog_ismember', 'view', false, $this->ID)) {	// Current user must be a member of this collection:
            return false;
        }
    ...
    public function check_access()
    {
        ...
        if ($allow_access == 'public') { // Everyone has an access to this blog
            return true;
        }

        if (in_array($disp, ['login', 'lostpassword', 'register', 'help', 'msgform', 'access_requires_login', 'content_requires_login'])) { // Don't restrict these pages
            return true;
        }
         ...
         */
        if (! is_logged_in()) { // Only logged in users have an access to this blog
            $Messages->add(T_('You need to log in before you can access this section.'), 'error');

            $login_Blog = &get_setting_Blog('login_blog_ID');
            if ($login_Blog && $login_Blog->ID != $this->ID) {	// If this collection is not used for login actions,
                ...
                // will have exited
            } else {	// This collection is used for login actions
        ...
        } elseif ($allow_access == 'members') { // Check if current user is member of this blog
            if (! check_user_perm('blog_ismember', 'view', false, $this->ID)) { // Force disp to restrict access for current user
                $disp = 'access_denied';
                ...
                if (! empty($blog_skin_ID)) {
                    ...
                    if (file_exists($skin_template_name)) { // Display a special template of this skin
                        require $skin_template_name;
                        exit;
                    } else { // Display a template from site skins
                        siteskin_init();
     ...
     */
    public function check_media_dir_change($old_media_dir, $old_media_location = null)
    {
        global $Messages;

        $new_media_dir = $this->get_media_dir(false);
        if ($new_media_dir == $old_media_dir) { // The media dir was not changed, no need fo further updates
            return true;
        }

        $new_media_location = $this->get('media_location');
        if ($old_media_location == null) { // Media location was not changed
            $old_media_location = $new_media_location;
        }

        switch ($new_media_location) {
            case 'none':
                if (is_empty_directory($old_media_dir)) { // Delete old media dir if it is empty
                ...
                } else { // The old media dir is not empty, but it must be cleared before it can be changed to none
            ...
            case 'custom':
                global $media_path;
                if (file_exists($new_media_dir)) { // Don't use the existing folder twice
                    $Messages->add(sprintf(T_('Folder %s already exists, it cannot be used for several media locations.'), '<b>' . $new_media_dir . '</b>'), 'error');
                    return false;
                }
                if (in_array(trim($new_media_dir, '/\\'), [$media_path . 'blogs', $media_path . 'import', $media_path . 'shared', $media_path . 'users'])) { // Don't use the reserved paths
                    $Messages->add(sprintf(T_('Please use another folder name, because %s is reserved.'), '<b>' . $new_media_dir . '</b>'), 'error');
                    return false;
                }
                if ($new_media_location == 'custom') { // Check for folder is not used by other blog, and it is not a sub-folder of other blog folder
                    $BlogCache = &get_BlogCache();
                ...
                if (($old_media_location == 'none') || (! file_exists($old_media_dir))) { // The media folder was not used before, create the new media folder
                    return $this->get_media_dir(true);
                }
                if (copy_r($old_media_dir, $new_media_dir, '', ['_evocache', '.evocache'])) { // The file have been copied to new folder successfully
                    if (! rmdir_r($old_media_dir)) { // Display a warning if old folder could not be deleted
                ...
                } else { // Display a message if some error on copying
     ...
     * @return array
     */
    public function get_enabled_item_types($usage = null)
    {
        if (empty($this->ID)) {	// This is new blog, it doesn't have the enabled item types:
            return [];
        }

        if (! isset($this->enabled_item_types)) {	// Get all enabled item types by one sql query and only first time to cache result:
            global $DB;
        ...
        if ($usage === null) {	// Get all item types:
            return array_keys($this->enabled_item_types);
        } else {	// Restrict item types by requested usage value:
     ...
     */
    public function is_item_type_enabled($item_type_ID)
    {
        if (empty($this->ID)) { // This is new blog, it doesn't have the enabled item types
            return false;
        }
     ...
     */
    public function can_be_item_type_disabled($item_type_ID, $display_message = false)
    {
        if (empty($this->ID)) { // This is new blog, no restriction to disable any post type
            return true;
        }

        if ($this->get_setting('default_post_type') == $item_type_ID) { // Don't allow to disable an item type which is used as default for this collection:
            if ($display_message) {
                global $Messages;
     ...
     */
    public function has_items_per_item_type($item_type_ID)
    {
        if (! isset($this->used_item_types)) {	// Get all Item Types which are used for Items in this Collection:
            global $DB;
     ...
     */
    public function enable_default_item_types()
    {
        if (empty($this->ID)) {	// Collection doesn't exist in DB yet:
            return;
        }

        global $DB, $cache_all_item_type_data;

        if (! isset($cache_all_item_type_data)) {	// Get all item type data only first time to save execution time:
        ...
        $default_post_types_by_template = ['recipe'];
        $default_post_types = ['Post with Custom Fields', 'Child Post', 'Recipe'];
        switch ($this->type) {
            case 'minisite':
            ...
            case 'forum':
                $default_post_types = []; // Don't install the default Item Types(see above) for forums collection.
            ...
            case 'group':
                $default_post_types = []; // Don't install the default Item Types(see above) for Tracker collection.
                $default_post_types[] = 'Task';
                break;
        ...
        foreach ($cache_all_item_type_data as $item_type) {
            if ($item_type->ityp_usage == 'post' &&
                ...
                ! in_array($item_type->ityp_ID, $enable_post_types)) {	// This "post" item type can be enabled:
                $enable_post_types[] = $item_type->ityp_ID;
            }
        }

        if (empty($enable_post_types)) {	// Display a warning if we cannot find the default post types for current collection kind:
            global $Messages;
        ...
        foreach ($cache_all_item_type_data as $item_type) {
            if ($item_type->ityp_usage == 'post' &&
                ...
                ! in_array($item_type->ityp_ID, $enable_post_types)) {	// Skip this item type for current collection kind:
                continue;
            }
        ...
        if ($i > 0) {	// Insert records to enable the default item types for this collection:
            $DB->query($insert_sql);
        }
    }
     ...
     */
    public function enable_duplicate_item_types()
    {
        if (empty($this->ID)) {	// Collection doesn't exist in DB yet:
            return;
        }
        ...
        $i = 0;
        foreach ($cache_all_item_type_data as $item_type) {
            if (! in_array($item_type->ityp_ID, $enable_post_types)) {	// Skip this item type for current collection kind:
                continue;
            }
     ...
     * @return object Item
     */
    public function &get_first_mainlist_Item()
    {
     ...
     */
    public function get_comment_moderator_user_data()
    {
        if (! isset($this->comment_moderator_user_data)) {	// Get it from DB only first time and then cache in array:
            global $DB;
     ...
     */
    public function get_url_aliases()
    {
     ...
     * @return string
     */
    public function get_access_title()
    {
     ...
     * @return string
     */
    public function get_max_allowed_status($check_status = null)
    {
        switch ($this->get_setting('allow_access')) {
            case 'members':
                if ($this->get('advanced_perms')) {
        ...
        if ($check_status === null) {	// Don't check the requested status:
        ...
        if ($max_allowed_status_order >= $check_status_order) {	// The requested status can be used on this collection:
            return $check_status;
        } else {	// Reduce the requested status by max allowed:
            return $max_allowed_status;
        }
    }
     ...
     */
    public function get_reduced_statuses($max_allowed_status = null)
    {
        if ($max_allowed_status === null) {	// Get current max allowed status for posts and comments o this collection:
        ...
        foreach ($status_orders as $status_key => $status_order) {
            if ($max_allowed_status_order < $status_order) {	// This status must be updated because the level is more than max allowed:
     ...
     */
    public function get_count_reduced_status_data($new_max_allowed_status)
    {
        global $DB;

        if (empty($this->ID)) {	// Collection must be created before:
            return false;
        }
        ...
        if (empty($reduced_statuses)) {	// No status to update:
            return false;
        }
        ...
        if (empty($posts) && empty($comments)) {	// All posts and comments have a correct status:
            return false;
        }
     ...
     */
    public function update_reduced_status_data()
    {
        global $DB;
        ...
        if (empty($reduced_statuses)) {	// Nothing to update:
            return;
        }
    ...
    {
        if ($this->locales === null && ! empty($this->ID)) {	// Load locales from DB once and store in cache variable:
            global $DB;
            ...
            if (! array_key_exists($this->get('locale'), $this->locales)) {	// Add main locale if it is not stored in extra locales by some unknown reason:
                $this->locales[$this->get('locale')] = null;
            }
        }

        if (! is_array($this->locales)) {	// Set default array with main locale:
            $this->locales = [
                $this->get('locale') => null,
            ];
        }
     ...
     */
    public function get_locales($type = 'locale')
    {
        // Load all locales:
        $this->load_locales();

        if ($type == 'locale' || $type == 'coll') {	// Get only locales or linked collections:
            $locales = [];
     ...
     */
    public function set_locales($new_locales = [], $new_linked_colls = [])
    {
        global $locales;
        ...
        foreach ($locales as $locale_key => $locale_data) {
            if ($locale_data['enabled']) {	// If locale is enabled:
        ...
        if (is_array($new_locales)) {
            foreach ($new_locales as $new_locale) {
                if (in_array($new_locale, $enabled_locales)) {	// Set only enabled locale:
                    $this->locales[$new_locale] = null;
                }
            }
        }

        if (! array_key_exists($this->get('locale'), $this->locales)) {	// Make sure main locale will be stored in the table of collection locales:
            $this->locales[$this->get('locale')] = null;
        }

        if (is_array($new_linked_colls)) {	// Set linked collections:
            foreach ($new_linked_colls as $locale_key => $linked_coll_ID) {
                if (! empty($linked_coll_ID) &&
                    in_array($locale_key, $enabled_locales) &&
                    ! array_key_exists($locale_key, $this->locales)) {	// Set collection only with enalbed locale link:
    ...
    public function update_locales()
    {
        ...
        if (empty($this->ID) || ! is_array($this->locales)) {	// Collection must be stored in DB and new locales must be defined:
            return false;
        }
        ...
        if (! empty($this->locales)) {	// If at least one new locale:
        ...
        if (! empty($sql_coll_locales)) {	// Insert new locales:
     ...
     * @return boolean
     */
    public function has_locale($locale_key, $type = 'locale')
    {
     ...
     * @return integer
     */
    public function get_unread_posts_count()
    {
        global $DB, $current_User, $localtimenow;

        if (! is_logged_in() || ! $this->get_setting('track_unread_content')) {	// User must be logged in AND the tracking of unread content is turned ON for the collection:
            return 0;
        }
     ...
     * @return string
     */
    public function get_msgform_recipient_link()
    {
        $recipient_link = '';

        if (get_param('recipient_id') > 0) {	// Get recipient identity link for registered user:
            $UserCache = &get_UserCache();
            $recipient_User = &$UserCache->get_by_ID(get_param('recipient_id'));

            if ($this->get_setting('msgform_display_avatar')) {	// Display recipient name with avatar:
                $recipient_link_params = [
                    'link_text' => 'avatar_name',
                    'thumb_size' => $this->get_setting('msgform_avatar_size'),
                    'thumb_class' => 'avatar_before_login_middle',
                ];
            } else {	// Display only recipient name:
                $recipient_link_params = [
                    'link_text' => 'auto',
                ];
            }
            $recipient_link = $recipient_User->get_identity_link($recipient_link_params);
        } elseif (get_param('comment_id') > 0) {	// Get login name for anonymous user:
     ...
     * @return array
     */
    public function get_msgform_additional_fields()
    {
        $msgform_additional_fields = trim($this->get_setting('msgform_additional_fields'));

        $saved_additional_fields = [];

        if (empty($msgform_additional_fields)) {	// No saved additional fields for this collection:
     ...
     * @return array
     */
    public function display_msgform_additional_fields($Form, $filled_user_fields = [])
    {
        ...
        foreach ($msgform_additional_fields as $UserField) {
            ...
            if (! empty($filled_user_fields[$UserField->ID])) {	// Get values from the submitted form:
                if (is_array($filled_user_fields[$UserField->ID])) {	// Multiple field:
            ...
            if (is_logged_in() && empty($field_values)) {	// Get saved field value from the current logged in User:
                global $current_User;
            ...
            if (empty($field_values)) {	// Display at least one additional field if user is not filled that in profile yet:
                $field_values = [''];
            }

            foreach ($field_values as $f => $field_value) {	// Display additional fields:
     ...
     * @return array
     */
    public function display_msgform_additional_field($Form, $UserField, $field_value = '', $force_not_required = false)
    {
        $field_params = [];

        if (! $force_not_required && $UserField->get('required') == 'require') {	// Field is required and it is not a second copy of the field:
            $field_params['required'] = true;
        }

        if ($UserField->get('suggest') == '1' && $UserField->get('type') == 'word') {	// Mark field with this tag to suggest a values
            $field_params['autocomplete'] = 'on';
        }

        // Field name:
        $field_name = 'user_fields[' . $UserField->ID . ']';
        if (in_array($UserField->get('duplicated'), ['allowed', 'list'])) {	// If field can be duplicated:
            $field_name .= '[]';
        }
        ...
        switch ($UserField->get('type')) {
            ...
            case 'list':
                ...
                if ($UserField->get('required') != 'require') {	// Add an empty value for not required field:
     ...
     */
    public function &get_default_ItemType()
    {
        if (! isset($this->default_ItemType)) {	// Initialize default item type object for this collection:
     ...
     * @param array Context
     */
    public function setup_default_widgets($skin_type = 'all', $context = [])
    {
        global $DB;

        if (empty($this->ID)) {	// This function should be called only for created collection:
            return;
        }

        if ($skin_type == 'all') {	// Setaup default widgets for skins of all types:
        ...
        $SkinCache = &get_SkinCache();
        if (! ($coll_Skin = &$SkinCache->get_by_ID($coll_skin_ID, false, false))) {	// This collection must has a correct skin:
            return;
        }
        ...
        if (isset($skin_widgets['*'])) {	// Depending on how the skin want to use this:
            $use_all_b2evo_default_widgets = $skin_widgets['*'];
            unset($skin_widgets['*']);
        } else {	// Use all default widgets if the skin does NOT say about this:
        ...
        foreach ($b2evo_default_widgets as $container_code => $widgets) {
            if ( // The skin has no widget declarations for this container and it allows to use b2evolution default widgets:
                ...
                (isset($skin_widgets[$container_code]) &&
                  $skin_widgets[$container_code] === true)) {	// Merge widgets from b2evolution default declarations to the skin:
                $skin_widgets[$container_code] = $widgets;
            } elseif (isset($skin_widgets[$container_code]) &&
                    is_array($skin_widgets[$container_code])) {	// Append custom skin widgets to default widgets:
        ...
        foreach ($skin_widgets as $container_code => $container_widgets) {
            if (! empty($container_widgets['type']) &&
                ! in_array($container_widgets['type'], ['main', 'sub', 'page'])) {	// Skip not collection container:
                unset($skin_widgets[$container_code]);
                continue;
            }
            if (isset($container_widgets['coll_type']) &&
                ! is_allowed_option($this->get('type'), $container_widgets['coll_type'])) {	// Skip container because it should not be installed for the given collection kind:
                unset($skin_widgets[$container_code]);
                continue;
            }

            if (isset($container_widgets['skin_type']) &&
                ! is_allowed_option($skin_type, $container_widgets['skin_type'])) {	// Skip container because it should not be installed for the given skin type:
                unset($skin_widgets[$container_code]);
                continue;
            }

            if ($container_widgets === true) {	// Set empty container if it has not been detected in b2evolution default widget declarations above:
                $skin_widgets[$container_code] = [];
            } elseif (! is_array($container_widgets)) {	// Ignore all other not array, probably it is a boolean false or some other string value which should be ignored indeed:
                unset($skin_widgets[$container_code]);
            }
        }

        if (empty($skin_widgets)) {	// No skin default widgets:
            return;
        }
        ...
        foreach ($skin_widgets as $container_code => $container_widgets) {
            if (isset($container_widgets['type']) &&
                ($container_widgets['type'] == 'sub' || $container_widgets['type'] == 'page')) {	// If it is a sub-container or page container:
                $blog_containers[$container_code] = [
        ...
        if (empty($widget_containers_sql_rows)) {	// No collection containers to install:
            return;
        }
        ...
        foreach ($skin_widgets as $container_code => $container_widgets) {
            if (! isset($blog_containers[$container_code])) {	// Skip container which is not supported by current collection's skin:
                continue;
            }
            ...
            foreach ($container_widgets as $key => $widget) {
                if (! is_number($key)) {	// Skip the config data which is used as additional info for container like 'type', 'name', 'order', 'item_ID', 'coll_type':
                    continue;
                }

                if (isset($widget['install']) && ! $widget['install']) {	// Skip widget because it should not be installed by condition from config:
                    continue;
                }

                if (isset($widget['coll_type']) && ! is_allowed_option($this->get('type'), $widget['coll_type'])) {	// Skip widget because it should not be installed for the given collection kind:
                    continue;
                }

                if (isset($widget['is_pro']) && $widget['is_pro'] !== is_pro()) {	// Skip widget because it should not be installed for the current version:
                    continue;
                }

                if (isset($widget['coll_ID']) && ! is_allowed_option($this->ID, $widget['coll_ID'])) {	// Skip widget because it should not be installed for the given collection ID:
                    continue;
                }

                if (isset($widget['skin_type']) && ! is_allowed_option($skin_type, $widget['skin_type'])) {	// Skip widget because it should not be installed for the given skin type:
                    continue;
                }
        ...
        if (! empty($basic_widgets_insert_sql_rows)) {	// Insert the widget records by single SQL query:
     ...
     */
    public function get_default_item_type_name()
    {
     ...
     */
    public function get_protocol_url($url)
    {
        if (empty($this->ID)) {
            return $url;
        }

        switch ($this->get_setting('http_protocol')) {	// Force URL's protocol depending on collection setting "SSL":
            case 'always_http':
     ...
     */
    public function &get_default_new_ItemType()
    {
        global $cat;

        // Get a working category:
        $working_cat = $cat;
        if (empty($cat)) {	// Use default collection category when global category is not defined:
        ...
        if (! $working_Chapter ||
            (($working_cat_ItemType = &$working_Chapter->get_ItemType()) === false)) {	// The working category is not detected in DB or it has no default Item Type:
            $r = false;
            return $r;
        }

        if ($working_cat_ItemType === null) {	// If the working category uses the same as collection default:
     ...
     */
    public function get_item_denomination($position = 'evobar_new')
    {
     ...
     */
    public function get_last_touched_date($format = false, $useGM = false)
    {
        if (empty($this->ID)) {	// Collection must be saved in DB:
            return false;
        }

        if (! isset($this->last_touched_date)) {	// Load last touched date from DB:
            global $DB;
        ...
        if ($format === false) {	// Don't format:
            return $this->last_touched_date;
        }

        if (empty($format)) {	// Use format of current locale:
     ...
     */
    public function get_marketing_popup_container()
    {
        if (is_admin_page()) {	// Don't display on back-office:
            return false;
        }

        if ($this->get_setting('marketing_popup_using') == 'never') {	// Marketing popup is disabled for all users:
            return false;
        }

        if (is_logged_in() && $this->get_setting('marketing_popup_using') == 'anonymous') {	// Marketing popup is enabled only for anonymous users:
            return false;
        }

        // Get widget container code depending on current disp:
        global $disp;
        $container_disp = in_array($disp, ['front', 'posts', 'single', 'page', 'catdir']) ? $disp : 'other_disps';
        $container_code = $this->get_setting('marketing_popup_container_' . $container_disp);

        if (empty($container_code)) {	// Don't try to find a widget container without code:
            return false;
        }
     ...
     */
    public function init_marketing_popup_container()
    {
        $marketing_popup_container_code = $this->get_marketing_popup_container();
        if ($marketing_popup_container_code) {	// If marketing popup is enabled for current page and user:
            ...
            if ($marketing_popup_show_frequency == 'period') {	// Use period frequency like X hours or X days:
            ...
            expose_var_to_js('evo_ddexitpop_config', [
                'container_code' => $marketing_popup_container_code,
                'animation' => $this->get_setting('marketing_popup_animation'),
                'show_repeat' => (bool) $this->get_setting('marketing_popup_show_repeat'),
     ...
     * @return integer
     */
    public function get_mustread_items_count($read_status = 'all')
    {
        if (! $this->get_setting('track_unread_content')) {	// No must read items when tracking of unread content is enabled for this collection:
            return 0;
        }

        if (! in_array($read_status, ['all', 'unread', 'read'])) {	// Ignore wrong param value:
            return 0;
        }

        if (! isset($this->mustread_items_count)) {	// Initialize array for cache:
            $this->mustread_items_count = [];
        }

        if (! isset($this->mustread_items_count[$read_status])) {	// Get it from DB only first time and then cache in var:
            ...
            $mustread_ItemList2->set_default_filters([
     ...
     * @return string
     */
    public function get_link_locale_selector($field_name, $locale, $restrict_used_locales = true)
    {
        global $DB;

        if ($restrict_used_locales &&
            ($this->get('locale') == $locale ||
              in_array($locale, array_keys($this->get_locales())))) {	// Don't allow selector if this collection already uses the requested locale:
            return T_('N/A');
        }

        $linked_colls = $this->get_locales('coll');

        if (! check_user_perm('blogs', 'editall')) {	// Display only the stored value because current User has no permission to edit all collections:
            if (isset($linked_colls[$locale])) {	// Get the linked collection:
                $BlogCache = &get_BlogCache();
        ...
        if (empty($colls)) {	// No collection with requested locale:
            return T_('None');
        }
     ...
     * @param string Skin type: 'all', 'normal', 'mobile', 'tablet'
     */
    public function reset_widgets($skin_type = 'all')
    {
        if (empty($this->ID)) {	// This function should be called only for created collection:
            return;
        }
        ...
        $all_skin_types = ['normal', 'mobile', 'tablet', 'alt'];

        if ($skin_type == 'all') {	// Reset widgets for all skin types:
            $skin_types = $all_skin_types;
        } elseif (in_array($skin_type, $all_skin_types)) {	// Reset widgets only for requested skin type:
            $skin_types = [$skin_type];
        } else {	// No correct skin type is requested:
            return;
        }

        foreach ($skin_types as $skin_type) {	// Remove previous widgets:
