...
class Section extends DataObject
{
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                'table' => 'T_blogs',
                'fk' => 'blog_sec_ID',
                'msg' => T_('%d collections in this group'),
            ],
        ];
    }
    ...
    {
        // Name:
        if ($this->ID != 1) {	// Only if not first default section:
    ...
    {
        if (empty($this->ID)) {	// New section creating has no collections yet, Return an empty array:
            return [];
        }

        if (! isset($this->blogs)) {	// Load collections of this section only on first request and cache the result:
