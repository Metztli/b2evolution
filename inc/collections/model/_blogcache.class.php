...
class BlogCache extends DataObjectCache
{
     ...
     * @var array
     */
    public $cache_siteurl_abs = [];

    /**
     * Cache by urlname
     * @var array
     */
    public $cache_urlname = [];
     ...
     */
    public function __construct($order_by = 'blog_order')
    {
        parent::__construct(
            'Blog',
            false,
    ...
    {
        if (! empty($Blog->siteurl) && preg_match('~^https?://~', $Blog->siteurl)) { // absolute siteurl
     ...
     * @param boolean false if you want to return false on error
     */
    public function &get_by_url_alias($req_url, &$alias, $halt_on_error = true)
    {
        global $DB, $Debuglog;
     ...
     */
    public function load_all($order_by = '', $order_dir = '')
    {
        global $Settings;

        if ($order_by == '') {	// Use default value from settings
            $order_by = $Settings->get('blogs_order_by');
        }

        if ($order_dir == '') {	// Use default value from settings
     ...
     * @return array of IDs
     */
    public function load_public($order_by = '', $order_dir = '')
    {
        global $DB, $Debuglog;
        ...
        foreach ($DB->get_results($SQL->get(), OBJECT, 'Load public blog list') as $row) {	// Instantiate a custom object
     ...
     * @return object SQL
     */
    public function get_public_colls_SQL($order_by = '', $order_dir = '')
    {
        global $Settings;
        ...
        if (is_logged_in()) {	// Allow the collections that available for logged in users:
     ...
     * @return object SQL
     */
    public function get_available_colls_SQL($order_by = '', $order_dir = '')
    {
        global $Settings;
     ...
     * @return array of IDs
     */
    public function load_owner_blogs($owner_ID, $order_by = '', $order_dir = '')
    {
        ...
        foreach ($DB->get_results($SQL->get(), OBJECT, 'Load owner blog list') as $row) {	// Instantiate a custom object
            $this->instantiate($row);
        }
     ...
     */
    public function load_user_blogs($permname = 'blog_ismember', $permlevel = 'view', $user_ID = null, $order_by = '', $order_dir = '', $limit = null, $filter = null)
    {
        global $DB, $Settings, $Debuglog;
        ...
        if ($filter == 'favorite') {	// Get only favorite collections of the user:
        ...
        if ($Group->check_perm('blogs', $permlevel) ||
            ...
            ($permname == 'stats' && $Group->check_perm('stats', 'view'))) { // If group grants a global permission:
            $this->clear();
            if (isset($sql_filter)) {	// Filter collections:
        ...
        if ($permname != 'blog_admin') {	// Only the admin perm is not convered by being the owner of the blog:
     ...
     * @return array of IDs
     */
    public function load_subscription_colls($User, $exclude_coll_IDs = null, $order_by = '', $order_dir = '')
    {
        global $DB, $Settings, $Debuglog;
        ...
        if ($CollectionSettings->get_default('allow_subscriptions') == 0) {	// If default setting disables to subscribe for new ITEMS/POSTS, we should get only the collections which allow the subsriptions:
            $sql_operator = 'IN';
            $sql_value = '1';
        } else {	// If default setting enables to subscribe for new ITEMS/POSTS, we should exclude the collections which don't allow the subsriptions:
            $sql_operator = 'NOT IN';
            $sql_value = '0';
        }
        ...
        if ($CollectionSettings->get_default('allow_comment_subscriptions') == 0) {	// If default setting disables to subscribe for new COMMENTS, we should get only the collections which allow the subsriptions:
            $sql_operator = 'IN';
            $sql_value = '1';
        } else {	// If default setting enables to subscribe for new COMMENTS, we should exclude the collections which don't allow the subsriptions:
            $sql_operator = 'NOT IN';
            $sql_value = '0';
        }
        ...
        if ($Settings->get('subscribe_new_blogs') == 'public') {	// If a subscribing is available only for the public collections:
        ...
        if (! empty($exclude_coll_IDs)) {	// Exclude the collections from the list (for example, if user already is subscribed on them):
     ...
     */
    public function get_option_list($default = 0, $allow_none = false, $method = 'get_name', $ignore_IDs = [], $option_attrs = [])
    {
     ...
     * @return string HTML tags <option>
     */
    public function get_option_list_forums($default = 0, $allow_none = false, $method = 'get_name')
    {
     ...
     * @return string HTML tags <option>
     */
    public function get_option_list_parent($default = 0, $allow_none = false, $method = 'get_name')
    {
        return parent::get_option_list($default, $allow_none, $method);
    }
}
