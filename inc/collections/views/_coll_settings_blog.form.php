...
$Form->select_input_options('blogs_order_dir', array_to_option_list(
    ...
    $Settings->get('blogs_order_dir')
), TB_('Order direction'), TB_('Select default blog list order direction.'));
...
$Form->checkbox_input('general_cache_enabled', $Settings->get('general_cache_enabled'), get_icon('page_cache_on') . ' ' . TB_('Enable general cache'), [
    'note' => TB_('Cache rendered pages that are not controlled by a skin. See Blog Settings for skin output caching.'),
]);

$cache_note = '(' . TB_('See Blog Settings for existing') . ')';
$Form->checklist([
    ['newblog_cache_enabled', 1, TB_('Enable page cache for NEW blogs'), $Settings->get('newblog_cache_enabled'), false, $cache_note],
    ['newblog_cache_enabled_widget', 1, TB_('Enable widget cache for NEW blogs'), $Settings->get('newblog_cache_enabled_widget'), false, $cache_note],
...
$Form->radio_input(
    ...
    [
        [
            'value' => 'off',
            'label' => TB_('Off'),
            'note' => TB_('No notification about your new content will be sent out.'),
        ],
        [
            'value' => 'immediate',
            'label' => TB_('Immediate'),
            'note' => TB_('This is guaranteed to work but may create an annoying delay after each post or comment publication.'),
        ],
        [
            'value' => 'cron',
            'label' => TB_('Asynchronous'),
            'note' => TB_('Recommended if you have your scheduled jobs properly set up.'),
        ],
    ],
...
$Form->begin_fieldset(TB_('Categories') . get_manual_link('categories-global-settings'), [
    'id' => 'categories',
]);
$Form->checkbox_input('allow_moving_chapters', $Settings->get('allow_moving_chapters'), TB_('Allow moving categories'), [
    'note' => TB_('Check to allow moving categories accross blogs. (Caution: can break pre-existing permalinks!)'),
]);
$Form->end_fieldset();
...
$Form->checklist(
    [
        ['cross_posting', 1, TB_('Allow admins to cross-post to several collections'), $Settings->get('cross_posting'), false, TB_('(Extra cats in different blogs)') . get_admin_badge()],
        ['cross_posting_blogs', 1, TB_('Allow admins to move posts between collections'), $Settings->get('cross_posting_blogs'), false, TB_('(Main cat can move to different blog)') . get_admin_badge()]],
    'allow_cross_posting',
    TB_('Cross posting')
);

$redirect_moved_posts_params = [
    'note' => TB_('check to allow redirects to the correct Collection if the requested Item Slug was found in a different collection.'),
];
...
$Form->checkbox_input('always_match_slug', $Settings->get('always_match_slug'), TB_('Always try to match slug'), [
    'note' => TB_('check to redirect to correct Collection if an Item Slug was found in <b>any</b> URL (including invalid URLs).'),
]);

$Form->checklist([
    ['redirect_tinyurl', 1, TB_('301 redirect to canonical URL'), $Settings->get('redirect_tinyurl')],
...
$Form->radio_input(
    'subscribe_new_blogs',
    $Settings->get('subscribe_new_blogs'),
    [
        [
            'value' => 'page',
            'label' => TB_('From blog page only'),
        ],
        [
            'value' => 'public',
            'label' => TB_('Show a list of all <b>Public</b> blogs allowing subscriptions'),
        ],
        [
            'value' => 'all',
            'label' => TB_('Show a list of <b>All</b> blogs allowing subsciptions'),
        ],
    ],
