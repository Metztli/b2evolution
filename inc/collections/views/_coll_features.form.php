...
for ($order_index = 0; $order_index <= 2 /* The number of orderby fields - 1 */; $order_index++) { // The order fields:
    ...
    $field_params = [];
    if (($order_index == 2) && ! $edited_Blog->get_setting('orderdir_1')) { // The third orderby field should be disable if the second was not set yet
        $field_params['disabled'] = 'disabled';
    }
    $orderdir_select = $Form->select_input_array('orderdir' . $field_suffix, $edited_Blog->get_setting('orderdir' . $field_suffix), [
        'ASC' => TB_('Ascending'),
        'DESC' => TB_('Descending'),
    ], '', '', $field_params);
...
$Form->radio(
    'what_to_show',
    $edited_Blog->get_setting('what_to_show'),
    [['days', TB_('days')],
        ['posts', TB_('posts')],
    ],
    ''
...
foreach ($enabled_item_types as $enabled_item_type_ID) {
    if (($enabled_ItemType = &$ItemTypeCache->get_by_ID($enabled_item_type_ID, false, false))) {
        $show_post_types_options[] = ['show_post_types[]', $enabled_item_type_ID, $enabled_ItemType->get_name(), ! in_array($enabled_item_type_ID, $show_post_types_values)];
    }
}
...
$Form->radio(
    'timestamp_min',
    $edited_Blog->get_setting('timestamp_min'),
    [['yes', TB_('yes')],
        ['no', TB_('no')],
        ['duration', TB_('only the last'), '', $timestamp_min_duration_input],
    ],
    TB_('Show past posts'),
    true
);
...
$Form->radio(
    'timestamp_max',
    $edited_Blog->get_setting('timestamp_max'),
    [['yes', TB_('yes')],
        ['no', TB_('no')],
        ['duration', TB_('only the next'), '', $timestamp_max_duration_input],
    ],
    TB_('Show future posts'),
    true
);

$Form->checklist(get_inskin_statuses_options($edited_Blog, 'post'), 'post_inskin_statuses', TB_('Front office statuses'), false, false, [
    'note' => 'Uncheck the statuses that should never appear in the front office.',
]);
...
$Form->begin_fieldset(TB_('Single Item view') . get_manual_link('item-single-item-view-features'), [
    'id' => 'post_options',
]);

$Form->radio(
    'post_navigation',
    $edited_Blog->get_setting('post_navigation'),
    [['same_blog', TB_('same blog')],
        ['same_category', TB_('same category')],
        ['same_author', TB_('same author')],
        ['same_tag', TB_('same tag')]],
    TB_('Default post by post navigation should stay in'),
    true,
    TB_('Skins may override this setting!')
);

$Form->end_fieldset();


$Form->begin_fieldset(TB_('Create/Edit options') . get_manual_link('blog-features-settings'), [
    'id' => 'post_options',
]);
...
$exclude_statuses = array_merge(get_restricted_statuses($edited_Blog->ID, 'blog_post!', 'create'), ['trash']);
$default_post_status_index = array_search($edited_Blog->get_setting('default_post_status'), $exclude_statuses);
if ($default_post_status_index !== false) {	// Allow to select status that is selected currently:
...
if (isset($AdminUI, $AdminUI->skin_name) && $AdminUI->skin_name == 'bootstrap') {	// Use dropdown for bootstrap skin:
    $default_status_field = get_status_dropdown_button([
    ...
    $default_status_field_anon = get_status_dropdown_button([
...
$Form->radio(
    'enable_goto_blog',
    $edited_Blog->get_setting('enable_goto_blog'),
    [['no', TB_('No'), TB_('Check this to view list of the posts.')],
        ['blog', TB_('View home page'), TB_('Check this to automatically view the blog after publishing a post.')],
        ['post', TB_('View new post'), TB_('Check this to automatically view the post page.')], ],
    TB_('View blog after creating'),
    true
);

$Form->radio(
    'editing_goto_blog',
    $edited_Blog->get_setting('editing_goto_blog'),
    [['no', TB_('No'), TB_('Check this to view list of the posts.')],
        ['blog', TB_('View home page'), TB_('Check this to automatically view the blog after editing a post.')],
        ['post', TB_('View edited post'), TB_('Check this to automatically view the post page.')], ],
    TB_('View blog after editing'),
    true
);
...
$Form->radio(
    'post_categories',
    $edited_Blog->get_setting('post_categories'),
    [['one_cat_post', TB_('Allow only one category per post')],
        ['multiple_cat_post', TB_('Allow multiple categories per post')],
        ['main_extra_cat_post', TB_('Allow one main + several extra categories')],
        ['no_cat_post', TB_('Don\'t allow category selections'), TB_('(Main cat will be assigned automatically)')]],
    TB_('Post category options'),
    true
);

$coll_in_skin_editing_options = [];
if (check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID)) {	// Permission to edit advanced admin settings:
    $coll_in_skin_editing_options[] = ['in_skin_editing', 1, TB_('Allow posting/editing from the Front-Office') . get_admin_badge(), $edited_Blog->get_setting('in_skin_editing')];
    $coll_in_skin_editing_options[] = ['in_skin_editing_renderers', 1, TB_('Allow Text Renderers selection in Front-Office edit screen') . get_admin_badge(), $edited_Blog->get_setting('in_skin_editing_renderers'), ! $edited_Blog->get_setting('in_skin_editing')];
}
$coll_in_skin_editing_options[] = ['in_skin_editing_category', 1, TB_('Allow Category selection in Front-Office edit screen'), $edited_Blog->get_setting('in_skin_editing_category'), ! $edited_Blog->get_setting('in_skin_editing')];
$coll_in_skin_editing_options[] = ['in_skin_editing_category_order', 1, TB_('Allow Order field in Category selection in Front-Office edit screen'), $edited_Blog->get_setting('in_skin_editing_category_order'), ! $edited_Blog->get_setting('in_skin_editing') || ! $edited_Blog->get_setting('in_skin_editing_category')];
...
foreach ($all_statuses as $status) {	// Add a checklist option for each possible modeartion status:
    if ($status == $max_allowed_status) {	// This is first allowed status, then all next statuses are also allowed:
        $status_is_hidden = false;
    }
    if (! isset($moderation_statuses[$status])) {	// Don't display a checkbox for non moderation status:
        continue;
    }
    $checklist_options[] = [
...
$Form->checklist($checklist_options, 'post_moderation_statuses', TB_('"Require moderation" statuses'), false, false, [
    'note' => TB_('Posts with the selected statuses will be considered to require moderation. They will trigger "moderation required" notifications and will appear as such on the collection dashboard.'),
]);

$Form->text_input('old_content_alert', $edited_Blog->get_setting('old_content_alert'), 2, TB_('Stale content alert'), TB_('Posts that have not been updated within the set delay will be reported to content moderators. Leave empty if you don\'t want such alerts.'), [
    'input_suffix' => ' ' . TB_('months') . '.',
]);

$Form->end_fieldset();

$Form->begin_fieldset(TB_('Voting options') . get_manual_link('item-voting-options'), [
    'id' => 'voting_options',
]);

$voting_disabled = ! $edited_Blog->get_setting('voting_positive');

$Form->checkbox('voting_positive', $edited_Blog->get_setting('voting_positive'), TB_('Allow Positive vote'), get_icon('thumb_up', 'imgtag', [
    'title' => TB_('Allow Positive vote'),
]));

$Form->checkbox('voting_neutral', $edited_Blog->get_setting('voting_neutral'), TB_('Allow Neutral vote'), get_icon('ban', 'imgtag', [
    'title' => TB_('Allow Neutral vote'),
]), '', 1, $voting_disabled);

$Form->checkbox('voting_negative', $edited_Blog->get_setting('voting_negative'), TB_('Allow Negative vote'), get_icon('thumb_down', 'imgtag', [
    'title' => TB_('Allow Negative vote'),
]), '', 1, $voting_disabled);

$Form->end_fieldset();

// display features settings provided by optional modules:
modules_call_method('display_collection_features', [
    'Form' => &$Form,
...
$Form->radio(
    'feed_content',
    $edited_Blog->get_setting('feed_content'),
    [['none', TB_('No feeds')],
        ['title', TB_('Titles only')],
        ['excerpt', TB_('Post excerpts')],
        ['normal', TB_('Standard post contents (stopping at "[teaserbreak]")')],
        ['full', TB_('Full post contents (including after "[teaserbreak]")')],
    ],
    TB_('Post feed contents'),
    true,
    TB_('How much content do you want to make available in post feeds?')
);
...
if ($notifications_mode != 'off') {
    ...
    $Form->checklist([
        ['allow_subscriptions', 1, TB_('Allow users to subscribe and receive email notifications for each new post.'), $edited_Blog->get_setting('allow_subscriptions')],
        ['allow_item_subscriptions', 1, TB_('Allow users to subscribe and receive email notifications for comments on a specific post.'), $edited_Blog->get_setting('allow_item_subscriptions')],
        ['allow_item_mod_subscriptions', 1, TB_('Allow users to subscribe and receive email notifications when post is modified and user has permission to moderate it.'), $edited_Blog->get_setting('allow_item_mod_subscriptions')],
