...
function coll_perm_checkbox($row, $prefix, $perm, $title, $id = null)
{
    ...
    if (! $row_Blog->get('advanced_perms')) {	// Don't display if advanced permissions are not enabled for the collection:
        return '';
    }
    ...
    if (is_always_coll_perm_enabled($row, $prefix, $perm, $row_Blog->owner_user_ID)) {	// This perm option is almost always enabled:
        if ($perm == 'can_be_assignee') {	// This permission can be edited even for admins:
    ...
    } else {	// Check if perm option is enabled or/and disabled:
        if (! empty($row->{$prefix . $perm})) {
            $r .= ' checked="checked"';
        }
        if (! $permission_to_change_admin
                && ($row->{$prefix . 'perm_admin'} || $perm == 'perm_admin')) { // No permission to touch nOR create admins
            $r .= ' disabled="disabled"';
        }
    }
    $r .= ' class="checkbox" value="1" title="' . $title . '" />';

    if ($perm == 'perm_meta_comment') {	// Add class to easily identify meta status checkbox with matching color:
...
function check_default_create_comment_perm($blog_ID, $perm_statuses)
{
    ...
    if ($row_Blog->get_setting('allow_comments') != 'any') { // Anonymous users are not allowed to post comments
        return true;
    }

    $default_status = $row_Blog->get_setting('new_feedback_status');
    $default_status_perm_value = get_status_permvalue($default_status);
    if ($perm_statuses & $default_status_perm_value) { // Posting comments with default status is allowed
        return true;
    }
...
function coll_perm_status_checkbox($row, $prefix, $perm_status, $title, $type)
{
    ...
    switch ($type) {
        ...
        case 'comment':
            ...
            if (! check_default_create_comment_perm($row_Blog->ID, $row->{$perm_statuses})) { // Doesn't have at least as high comment create permission as anonymous users have
                $default_status = $row_Blog->get_setting('new_feedback_status');
            }
            $type_param = 'cmt_';
            break;
    ...
    if (($perm_status == 'published' || $perm_status == 'community') &&
        $row_Blog->get_setting('allow_access') == 'members') {	// If collection is for members only then Published and Community statuses are not allowed:
        $always_disabled = true;
    } elseif ($perm_status == 'published' &&
            $row_Blog->get_setting('allow_access') == 'users') {	// If collection is for logged-in users only then Published status is not allowed:
        $always_disabled = true;
    }

    if ($always_disabled) {	// This perm option is always disabled:
        $r .= ' disabled="disabled"';
    } elseif ($always_enabled) {	// This perm option is always enabled:
        $r .= ' checked="checked" disabled="disabled"';
    } else {	// Check if perm option is enabled or/and disabled:
    ...
    if ($perm_status == $default_status && ! $always_enabled) {	// This is the default comment status checkbox, and user has no permission to create comment with this status ( like anonymous users ) or a higher status:
...
function coll_perm_edit($row, $prefix)
{
    ...
    if (is_always_coll_perm_enabled($row, $prefix, 'perm_edit', $row_Blog->owner_user_ID)) {	// This perm option is always enabled:
        $r .= ' disabled="disabled"';
        $perm_edit_value = 'all';
    } else {	// Check if perm option is enabled or/and disabled:
        if (! $permission_to_change_admin && $row->{$prefix . 'perm_admin'}) {
...
function coll_perm_edit_cmt($row, $prefix)
{
    ...
    if (is_always_coll_perm_enabled($row, $prefix, 'perm_edit_cmt', $row_Blog->owner_user_ID)) {	// This perm option is always enabled:
        $r .= ' disabled="disabled"';
...
function coll_perm_item_type($row, $prefix)
{
    ...
    if (is_always_coll_perm_enabled($row, $prefix, 'perm_item_type', $row_Blog->owner_user_ID)) {	// This perm option is always enabled:
        $r .= ' disabled="disabled"';
...
function coll_grp_perm_col_name($grp_ID, $grp_name, $grp_usage)
{
    ...
    if ($grp_usage == 'primary') {	// Primary group
...
function coll_grp_perm_col_member($row)
{
    ...
    if ($row_Blog->get_setting('use_workflow')) {	// If the collection uses workflow:
 ...
 */
function colls_groups_perms_results(&$Results, $params = [])
{
    $params = array_merge([
    ...
    if ($params['type'] == 'collection') {	// Collection:
    ...
    if ($params['type'] == 'group') {	// Group columns of collection wihtout enabled advanced permissions:
...
{
    $columns = [$prefix == 'bloguser_' ? 'login' : 'group', 'level', 'member'];
    ...
    $columns[] = 'edit ts';
    $comment_statuses = get_visibility_statuses('keys', ['redirected', 'trash']);
