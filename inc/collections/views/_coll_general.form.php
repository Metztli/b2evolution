...
if ($edited_Blog->ID == 0) { // "New blog" form: Display a form title and icon to close form
    global $kind;
...
} elseif ($action == 'copy') {	// Copy collection form:
...
if (! empty($edited_Blog->confirmation)) {	// Display a confirmation message:
    ...
    $Form->button(['submit', 'actionArray[update_confirm]', TB_('I am sure!'), 'DeleteButton btn-danger']);
    $Form->button(['button', '', TB_('CANCEL'), 'CancelButton', 'location.href="' . $admin_url . '?ctrl=coll_settings&tab=general&blog=' . $edited_Blog->ID . '"']);
...
if ($edited_Blog->get('type') == 'main') { // Only show when collection is of type 'Main'
    ...
    if (! $Settings->get('login_blog_ID')) {
        $set_as_options[] = ['set_as_login_blog', 1, TB_('Collection for login/registration'), param('set_as_login_blog', 'boolean', $set_as_checked)];
    }
    if (! $Settings->get('msg_blog_ID')) {
        $set_as_options[] = ['set_as_msg_blog', 1, TB_('Collection for profiles/messaging'), param('set_as_msg_blog', 'boolean', $set_as_checked)];
    }
    if (! $Settings->get('info_blog_ID')) {
        $set_as_options[] = ['set_as_info_blog', 1, TB_('Collection for shared content blocks'), param('set_as_info_blog', 'boolean', $set_as_checked)];
    }
    ...
    if ($is_creating) {
        ...
        $Form->radio(
            'blog_aggregate',
            empty($aggregate_coll_IDs) ? 0 : 1,
            [
                [1, TB_('Set to aggregate contents of all other collections')],
                [0, TB_('Do not aggregate')],
            ],
            TB_('Aggregate'),
            true,
            ''
...
if (in_array($action, ['create', 'new-name']) && $ctrl = 'collections') {	// Only show demo content option when creating a new collection
    load_funcs('dashboard/model/_dashboard.funcs.php');

    $Form->begin_fieldset(TB_('Demo contents') . get_manual_link('collection-demo-content'));
    $Form->radio(
        'create_demo_contents',
        param('create_demo_contents', 'integer', -1),
        [
            [1, TB_('Initialize this collection with some demo contents')],
            [0, TB_('Create an empty collection')],
        ],
        TB_('New contents'),
        true,
        '',
    ...
    if (get_table_count('T_users__organization') === 0) {
        if (check_user_perm('orgs', 'create', false) && check_user_perm('blog_admin', 'editall', false)) {	// Permission to create organizations
            $Form->checkbox(
                'create_demo_org',
                param('create_demo_org', 'integer', 1),
                TB_('Create demo organization'),
                TB_('Create a demo organization if none exists.')
            );
        }
    }

    if (get_table_count('T_users', 'user_ID != 1') === 0) {
        if (check_user_perm('users', 'edit', false) && check_user_perm('blog_admin', 'editall', false)) {	// Permission to edit users
            $Form->checkbox(
                'create_demo_users',
                param('create_demo_users', 'integer', 1),
                TB_('Create demo users'),
                TB_('Create demo users as comment authors.')
            );
        }
    }
    $Form->end_fieldset();
...
$Form->begin_fieldset(TB_('Branding') . get_manual_link('collection-settings-branding'), [
...
if (check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID) ||
    check_user_perm('blogs', 'create', false, $edited_Blog->sec_ID)) { // Permission to edit advanced admin settings
    ...
    $Form->text(
        ...
        TB_('URL "filename"'),
        sprintf(
            TB_('"slug" used to uniquely identify this blog in URLs. Also used as <a %s>default media folder</a>.'),
            'href="?ctrl=coll_settings&tab=advanced&blog=' . $blog . '"'
        ),
        255
    );
} else {
...
$collection_logo_params = [
    'file_type' => 'image',
    'max_file_num' => 1,
    'window_title' => TB_('Select collection logo/image'),
    'root' => 'shared_0',
    'size_name' => 'fit-320x320',
];
...
$collection_favicon_params = [
    'file_type' => 'image',
    'max_file_num' => 1,
    'window_title' => TB_('Select collection favicon'),
    'root' => 'shared_0',
    'size_name' => 'fit-128x128',
];
...
if (count($SectionCache->cache_available) > 1) { // If we have only one option in the list do not show select input
    $Form->select_input_object('sec_ID', $blog_section_id, $SectionCache, TB_('Section'), [
        'required' => true,
    ]);
}
...
foreach ($locales as $locale_data) {
    ...
    if ($number_enabled_locales > 1) { // We need to know we have more than 1 locale is enabled, Stop here
        break;
    }
}

if (! $is_creating) {
    $Form->begin_fieldset(TB_('Language / locale') . get_manual_link('coll-locale-settings'), [
        'id' => 'language',
    ]);
    if ($number_enabled_locales > 1) { // More than 1 locale
        $blog_locale_note = (check_user_perm('options', 'view')) ?
            '<a href="' . $admin_url . '?ctrl=regional">' . TB_('Regional settings') . ' &raquo;</a>' : '';
        $Form->locale_selector('blog_locale', $edited_Blog->get('locale'), array_keys($edited_Blog->get_locales()), TB_('Collection Locales'), $blog_locale_note, [
            'link_coll_ID' => $edited_Blog->ID,
        ]);

        $Form->radio(
            'blog_locale_source',
            $edited_Blog->get_setting('locale_source'),
            [
                ['blog', TB_('Always force to collection locale')],
                ['user', TB_('Use browser / user locale when possible')],
            ],
            TB_('Navigation/Widget Display'),
            true
        );

        $Form->radio(
            'blog_post_locale_source',
            $edited_Blog->get_setting('post_locale_source'),
            [
                ['post', TB_('Always force to Post locale')],
                ['blog', TB_('Follow navigation locale'), '(' . TB_('Navigation/Widget Display') . ')'],
            ],
            TB_('Post Details Display'),
            true
        );

        $Form->radio(
            'blog_new_item_locale_source',
            $edited_Blog->get_setting('new_item_locale_source'),
            [
                ['select_coll', TB_('Default to collection\'s main locale')],
                ['select_user', TB_('Default to user\'s locale')],
            ],
            TB_('New Posts'),
            true
        );
    } else { // Only one locale
        echo '<p>';
...
if ($action == 'copy') {	// Additional options for collection duplicating:
...
if (! $is_creating) {
    $Form->radio(
        'advanced_perms',
        $edited_Blog->get('advanced_perms'),
        [
            ['0', TB_('Simple permissions'), sprintf(TB_('(the owner above has most permissions on this collection, except %s)'), get_admin_badge())],
            ['1', TB_('Advanced permissions'), sprintf(
                TB_('(you can assign granular <a %s>user</a> and <a %s>group</a> permissions for this collection)'),
...
$Form->radio(
    ...
    $blog_allow_access,
    [
        ['public', TB_('Everyone (Public Blog)')],
        ['users', TB_('Community only (Logged-in users only)')],
        ['members',
            ...
            '<span id="allow_access_members_advanced_note"' . ($edited_Blog->get('advanced_perms') ? '' : ' style="display:none"') . '>' . sprintf(
                TB_('(Assign membership in <a %s>user</a> and <a %s>group</a> permissions for this collection)'),
                'href="' . $admin_url . '?ctrl=coll_settings&amp;tab=perm&amp;blog=' . $edited_Blog->ID . '"',
                'href="' . $admin_url . '?ctrl=coll_settings&amp;tab=permgroup&amp;blog=' . $edited_Blog->ID . '"'
            ) . '</span>' .
...
if (! $is_creating) {
    ...
    $Form->radio(
        'blog_in_bloglist',
        $edited_Blog->get('in_bloglist'),
        [['public', TB_('Always (Public)')],
            ['logged', TB_('For logged-in users only')],
            ['member', TB_('For members only')],
            ['never', TB_('Never')],
        ],
        TB_('Show in front-office / front-end'),
        true,
        TB_('Select when you want this blog to appear in the list of blogs on this system.')
    );
    ...
    $Form->textarea(
        'blog_notes',
        $edited_Blog->get('notes'),
        5,
        TB_('Dashboard notes'),
        TB_('Additional info. Appears in the backoffice.'),
        50
    );
...
$Form->buttons([['submit', 'submit', ($action == 'copy' ? TB_('Duplicate NOW!') : TB_('Save Changes!')), 'SaveButton']]);
