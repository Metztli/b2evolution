...
if (check_user_perm('users', 'edit')) {
    ...
    $Form->checkbox_input('system_lock', $Settings->get('system_lock'), TB_('Lock system'), [
...
$Form->text_input('site_code', $Settings->get('site_code'), 10, TB_('Site code'), '$instance_name = ' . $instance_name, [
    'maxlength' => 20,
]);
$Form->color_input('site_color', $Settings->get('site_color'), TB_('Site color'));
$Form->text_input('notification_short_name', $Settings->get('notification_short_name'), 50, TB_('Short site name'), TB_('Shared with email settings'), [
    'maxlength' => 127,
    'required' => true,
]);
$Form->text_input('notification_long_name', $Settings->get('notification_long_name'), 50, TB_('Long site name'), TB_('Shared with email settings'), [
    'maxlength' => 255,
]);
$site_logo_params = [
    'file_type' => 'image',
    'max_file_num' => 1,
    'window_title' => TB_('Select site logo'),
    'root' => 'shared_0',
...
$Form->checkbox_input('site_skins_enabled', $Settings->get('site_skins_enabled'), TB_('Enable site skins'), [
    'note' => TB_('Enables a sitewide header and footer'),
]);
...
$Form->text_input('site_terms', $Settings->get('site_terms'), 7, TB_('Enable and display Post ID:'), TB_('Enter ID of the page containing the terms & conditions.'), [
    'maxlength' => 11,
]);
$Form->end_line();
...
$Form->select_input_object('default_blog_ID', $Settings->get('default_blog_ID'), $BlogCache, get_icon('coll_default') . ' ' . TB_('Default collection to display'), [
...
$Form->select_input_object('login_blog_ID', $Settings->get('login_blog_ID'), $BlogCache, get_icon('coll_login') . ' ' . TB_('Collection for login/registration'), [
    'note' => TB_('This collection will be used for all login/registration functions.') . $create_new_blog_link,
    'allow_none' => true,
    'loop_object_method' => 'get_maxlen_name',
]);

$Form->select_input_object('msg_blog_ID', $Settings->get('msg_blog_ID'), $BlogCache, get_icon('coll_message') . ' ' . TB_('Collection for profiles/messaging'), [
...
$Form->select_input_object('info_blog_ID', $Settings->get('info_blog_ID'), $BlogCache, get_icon('coll_info') . ' ' . TB_('Collection for shared content blocks'), [
...
$Form->duration_input('reloadpage_timeout', (int) $Settings->get('reloadpage_timeout'), TB_('Reload-page timeout'), 'minutes', 'seconds', [
    'minutes_step' => 1,
    'required' => true,
]);
