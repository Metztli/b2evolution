...
if ($available_ping_plugins) {
    foreach ($available_ping_plugins as $loop_Plugin) {
        if (empty($loop_Plugin->code)) { // Ping plugin needs a code
            continue;
        }
...
$Form->text_input(
    'atom_redirect',
    $edited_Blog->get_setting('atom_redirect'),
    50,
    TB_('Atom Feed URL'),
...
$Form->text_input(
    'rss2_redirect',
    $edited_Blog->get_setting('rss2_redirect'),
    50,
    TB_('RSS2 Feed URL'),
...
$Form->checkbox_input('blog_allow_duplicate', $edited_Blog->get_setting('allow_duplicate'), TB_('Allow duplication'), [
    'note' => TB_('Check to allow anyone to duplicate this collection.'),
]);
$Form->end_fieldset();


if (check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID)) {	// Permission to edit advanced admin settings
    $Form->begin_fieldset(TB_('Caching') . get_admin_badge() . get_manual_link('collection-cache-settings'), [
        'id' => 'caching',
    ]);
    $Form->checklist([
        ['ajax_form_enabled', 1, TB_('Comment, Contact & Quick registration forms will be fetched by javascript'), $edited_Blog->get_setting('ajax_form_enabled')],
        ['ajax_form_loggedin_enabled', 1, TB_('Also use JS forms for logged in users'), $edited_Blog->get_setting('ajax_form_loggedin_enabled'), ! $edited_Blog->get_setting('ajax_form_enabled')],
    ], 'ajax_form', TB_('Enable AJAX forms'));

    $Form->checkbox_input('cache_enabled', $edited_Blog->get_setting('cache_enabled'), get_icon('page_cache_on') . ' ' . TB_('Enable page cache'), [
        'note' => TB_('Cache rendered blog pages'),
    ]);
    $Form->checkbox_input('cache_enabled_widgets', $edited_Blog->get_setting('cache_enabled_widgets'), get_icon('block_cache_on') . ' ' . TB_('Enable widget/block cache'), [
        'note' => TB_('Cache rendered widgets'),
    ]);
    $Form->end_fieldset();

    $Form->begin_fieldset(TB_('In-skin Actions') . get_admin_badge() . get_manual_link('in-skin-action-settings'), [
        'id' => 'inskin_actions',
    ]);
    if ($login_Blog = &get_setting_Blog('login_blog_ID', $edited_Blog)) { // The login blog is defined in general settings
    ...
    } else { // Allow to select in-skin login for this blog
        $Form->checkbox_input('in_skin_login', $edited_Blog->get_setting('in_skin_login'), TB_('In-skin login'), [
            'note' => TB_('Use in-skin login form every time it\'s possible'),
        ]);
    }
    $Form->checkbox_input('in_skin_editing', $edited_Blog->get_setting('in_skin_editing'), TB_('In-skin editing'), [
        'note' => sprintf(TB_('See more options in Features &gt; <a %s>Posts</a>'), 'href="' . $admin_url . '?ctrl=coll_settings&amp;tab=features&amp;blog=' . $edited_Blog->ID . '#post_options"'),
    ]);
    $Form->checkbox_input('in_skin_change_proposal', $edited_Blog->get_setting('in_skin_change_proposal'), TB_('In-skin change proposal'));
    $Form->end_fieldset();

    $Form->begin_fieldset(TB_('Media directory location') . get_admin_badge() . get_manual_link('media-directory-location'), [
        'id' => 'media_dir_location',
    ]);
    global $media_path;
    $Form->radio(
        'blog_media_location',
        $edited_Blog->get('media_location'),
        [
            ['none', TB_('None')],
            ['default', TB_('Default'), $media_path . 'blogs/' . $edited_Blog->urlname . '/'],
            ['subdir', TB_('Subdirectory of media folder') . ':',
                '',
...
if (is_pro()) {	// Allow to remove "Powered by b2evolution" logos only for PRO version:
    ...
    $max_credits_note = '';
} else {	// Inform about free version:
...
if (check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID)) {	// Permission to edit advanced admin settings
    ...
    $Form->output = true;
    $Form->checklist([
        ['display_alt_skin_referer', 1, sprintf(TB_('Referer URL contains %s'), $display_alt_skin_referer_url_input), $edited_Blog->get_setting('display_alt_skin_referer'), false, '', 'checkbox_with_input'],
    ...
    $Form->textarea(
        'blog_head_includes',
        $edited_Blog->get_setting('head_includes'),
        5,
        TB_('Custom meta tag/css section (before &lt;/head&gt;)'),
        TB_('Add custom meta tags and/or css styles to the &lt;head&gt; section. Example use: website verification, Google+, favicon image...'),
        50
    );
    $Form->textarea(
        'blog_body_includes',
        $edited_Blog->get_setting('body_includes'),
        5,
        TB_('Custom javascript section (after &lt;body&gt;)'),
        TB_('Add custom javascript after the opening &lt;body&gt; tag.<br />Example use: tracking scripts, javascript libraries...'),
        50
    );
    $Form->textarea(
        'blog_footer_includes',
        $edited_Blog->get_setting('footer_includes'),
        5,
        TB_('Custom javascript section (before &lt;/body&gt;)'),
        TB_('Add custom javascript before the closing &lt;/body&gt; tag in order to avoid any issues with page loading delays for visitors with slow connection speeds.<br />Example use: tracking scripts, javascript libraries...'),
        50
    );
    $Form->end_fieldset();
}
