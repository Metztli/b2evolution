...
$Form->radio(
    'allow_view_comments',
    $edited_Blog->get_setting('allow_view_comments'),
    [['any', TB_('Any user'), TB_('Including anonymous users')],
        ['registered', TB_('Registered users only')],
        ['member', TB_('Members only'),  TB_('Users have to be members of this blog')],
        ['moderator', TB_('Moderators & Admins only')],
    ],
    TB_('Comment viewing by'),
    true
);
...
$Form->radio(
    'comments_orderdir',
    $edited_Blog->get_setting('comments_orderdir'),
    [['ASC', TB_('Chronologic')],
        ['DESC', TB_('Reverse')],
    ],
    TB_('Display order'),
...
$Form->checklist(get_inskin_statuses_options($edited_Blog, 'comment'), 'comment_inskin_statuses', TB_('Front office statuses'), false, false, [
...
$Form->radio(
    'allow_comments',
    $edited_Blog->get_setting('allow_comments'),
    [['any', TB_('Any user'), TB_('Including anonymous users'),
        ...
        ['member', TB_('Members only'),  TB_('Users have to be members of this blog'),
            '', 'onclick="show_hide_feedback_details(this);"'],
        ['never', TB_('Not allowed'), '',
            '', 'onclick="show_hide_feedback_details(this);"'],
    ],
    TB_('Comment posting by'),
    true,
    $edited_Blog->get_advanced_perms_warning()
);
...
$Form->checklist([
    ['require_anon_name', 1, TB_('Require a name'), $edited_Blog->get_setting('require_anon_name')],
    ['require_anon_email', 1, TB_('Require an email'), $edited_Blog->get_setting('require_anon_email')],
    ['allow_anon_url', 1, TB_('Allow to submit an URL'), $edited_Blog->get_setting('allow_anon_url')],
...
$Form->checkbox(
    'allow_html_comment',
    $edited_Blog->get_setting('allow_html_comment'),
    TB_('Allow HTML'),
    TB_('Check to allow HTML in comments.') . ' (' . TB_('HTML code will pass several sanitization filters.') . ')'
);

$any_option = ['any', TB_('Any user'), TB_('Including anonymous users'), ''];
$registered_option = ['registered', TB_('Registered users only'),  '', ''];
$member_option = ['member', TB_('Members only'), TB_('Users have to be members of this blog'), ''];
$never_option = ['never', TB_('Not allowed'), '', ''];
$Form->radio(
    'allow_attachments',
    $edited_Blog->get_setting('allow_attachments'),
    [$any_option, $registered_option, $member_option, $never_option,
    ],
    TB_('Allow attachments from'),
    true
);

$max_attachments_params = [];
if ($edited_Blog->get_setting('allow_attachments') == 'any') {	// Disable field "Max # of attachments" when Allow attachments from Any user
...
if ($perm_blog_admin || $edited_Blog->get('allowtrackbacks')) { // Only admin can turn ON this setting
...
if ($perm_blog_admin || $edited_Blog->get_setting('webmentions')) {	// Only admin can turn ON this setting
    $Form->checkbox(
        'blog_webmentions',
        $edited_Blog->get_setting('webmentions'),
...
$Form->checkbox(
    ...
    $edited_Blog->get_setting('autocomplete_usernames'),
    TB_('Autocomplete usernames in back-office'),
    TB_('Check to enable auto-completion of usernames entered after a "@" sign in the comment forms')
);
...
$Form->begin_fieldset(TB_('Voting options') . get_manual_link('comment-voting-options'), [
    'class' => 'feedback_details_container',
]);
...
$Form->radio(
    'allow_rating_items',
    $edited_Blog->get_setting('allow_rating_items'),
    [$any_option, $registered_option, $member_option, $never_option,
    ],
    TB_('Allow star ratings from'),
    true
);
...
// echo 'modules';
modules_call_method('display_collection_comments', [
    'Form' => &$Form,
...
$status_options = get_visibility_statuses('', ['redirected', 'trash']);
if ($edited_Blog->get_setting('new_feedback_status') != 'published') {
    if ($perm_blog_admin) { // Only admin can set this setting to 'Public'
        $status_options['published'] .= $is_bootstrap_skin ? get_admin_badge('coll', false) : ' [' . TB_('Admin') . ']';
    } else { // Remove published status for non-admin users
...
foreach ($status_options as $status_key => $status_option) {
    if ($status_key == $max_allowed_status) {	// This is first allowed status, then all next statuses are also allowed:
        $status_is_allowed = true;
    }
    if (! $status_is_allowed && $edited_Blog->get_setting('new_feedback_status') != $status_key) {	// Don't allow to select this status because it is not allowed by collection restriction:
...
if ($is_bootstrap_skin) {	// Use dropdown for bootstrap skin:
    $new_status_field = get_status_dropdown_button([
    ...
    $Form->select_input_array(
        'new_feedback_status',
        $edited_Blog->get_setting('new_feedback_status'),
        $status_options,
        TB_('Status for new Anonymous comments'),
        $newstatus_warning . TB_('Logged in users will get the highest possible status allowed by their permissions. Plugins may also override this default.')
    );
}
echo '</div>';
...
foreach ($all_statuses as $status) {	// Add a checklist option for each possible moderation status:
...
$Form->checklist($checklist_options, 'moderation_statuses', TB_('"Require moderation" statuses'), false, false, [
    'note' => TB_('Comments with the selected statuses will be considered to require moderation. They will trigger "moderation required" notifications and will appear as such on the collection dashboard.'),
]);

$Form->radio(
    'comment_quick_moderation',
    $edited_Blog->get_setting('comment_quick_moderation'),
    [['never', TB_('Never')],
        ['expire', TB_('Links expire on first edit action')],
        ['always', TB_('Always available')],
    ],
    TB_('Comment quick moderation'),
    true
);
$Form->end_fieldset();
...
$Form->radio(
    ...
    $edited_Blog->get_setting('comment_feed_content'),
    [['none', TB_('No feeds')],
        ['excerpt', TB_('Comment excerpts')],
        ['normal', TB_('Standard comment contents')],
    ],
    TB_('Comment feed contents'),
    true,
    TB_('How much content do you want to make available in comment feeds?')
);
...
if ($notifications_mode != 'off') {
    ...
    $Form->checklist([
        ['allow_comment_subscriptions', 1, TB_('Allow users to subscribe and receive email notifications for each new comment.'), $edited_Blog->get_setting('allow_comment_subscriptions')],
    ...
    $Form->checklist([
        ['allow_anon_subscriptions', 1, TB_('Allow users to subscribe and receive email notifications for replies to their comments.'), $edited_Blog->get_setting('allow_anon_subscriptions')],
    ], 'allow_anon_subscriptions', TB_('Anonymous users'));
    $Form->radio('default_anon_comment_notify', $edited_Blog->get_setting('default_anon_comment_notify'), [
        [1, TB_('Checked')],
        [0, TB_('Unchecked')],
    ], TB_('Default option'));
