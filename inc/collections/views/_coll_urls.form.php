...
if (preg_match('#https?://#', $edited_Blog->get('siteurl'))) { // absolute
...
$http_protocol_options = [
    ['always_http', sprintf(TB_('Always use %s'), '<code>http</code>')],
    ['always_https', sprintf(TB_('Always use %s'), '<code>https</code>')],
    ['allow_both', sprintf(TB_('Allow both %s and %s as valid URLs'), '<code>http</code>', '<code>https</code>')],
];
...
if (check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID)) {	// Permission to edit advanced admin settings
    ...
    if ($default_blog_ID = $Settings->get('default_blog_ID')) {
        ...
        if ($default_Blog = &$BlogCache->get_by_ID($default_blog_ID, false)) { // Default blog exists
    ...
    $access_type_options = [
         ...
         */
        ['baseurl', TB_('Default collection on baseurl'),
        ...
        ['default', TB_('Default collection in index.php'),
        ...
        ['extrapath', TB_('Extra path on index.php'),
        ...
        ['relative', TB_('Relative to baseurl') . ': ',
            '',
    ...
    if (! is_valid_ip_format($basehost)) {	// Not an IP address, we can use subdomains:
        $access_type_options[] = ['subdom', TB_('Subdomain of basehost'),
    ...
    } else { // Don't allow subdomain for IP address:
        $access_type_options[] = ['subdom', TB_('Subdomain') . ':',
            sprintf(TB_('(Not possible for %s)'), $basehost),
            '',
            'disabled="disabled"',
        ];
    }
    $access_type_options[] = ['absolute', TB_('Absolute URL') . ':',
        '',
...
} else {	// Display only current values as text if user has no permission to edit:
    $current_http_protocol_option = '';
    foreach ($http_protocol_options as $http_protocol_option) {
        if ($http_protocol_option[0] == $edited_Blog->get_setting('http_protocol')) {	// Get title of current option:
...
$alias_field_note .= get_icon('minus', 'imgtag', [
    ...
    'style' => 'display: none; margin-left: 2px; cursor: pointer; position: relative;',
]);
if (empty($url_aliases)) {
    $Form->text_input('blog_url_alias[]', '', 50, TB_('Alias URL'), $alias_field_note, [
        'class' => 'evo_url_alias',
        'maxlength' => 255,
    ]);
}
...
foreach ($url_aliases as $alias) {
    $Form->text_input('blog_url_alias[]', $alias, 50, TB_('Alias URL'), $alias_field_note, [
        'class' => 'evo_url_alias',
        'maxlength' => 255,
    ]);
}
...
if (check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID)) {	// If current user has a permission to edit collection advanced admin settings:
    ...
    $Form->radio('cookie_domain_type', $edited_Blog->get_setting('cookie_domain_type'), [
        ['auto', TB_('Automatic'), $edited_Blog->get_cookie_domain('auto')],
        ['custom', TB_('Custom') . ':', '', $cookie_domain_custom_field, 'class="radio-input"'],
    ], TB_('Cookie domain'), true);

    $Form->radio('cookie_path_type', $edited_Blog->get_setting('cookie_path_type'), [
        ['auto', TB_('Automatic'), $edited_Blog->get_cookie_path('auto')],
        ['custom', TB_('Custom') . ':', '', $cookie_path_custom_field, 'class="radio-input"'],
    ], TB_('Cookie path'), true);
} else {	// Display only info about collection cookie domain and path if user has no permission to edit:
...
if (check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID)) { // Permission to edit advanced admin settings
    ...
    $assets_url_data = [];
    // media url:
    $assets_url_data['media_assets_url_type'] = [
        'label' => sprintf(TB_('Load %s assets from'), '<code>/media/</code>'),
    ];
    if ($edited_Blog->get('media_location') == 'none') { // if media location is disabled
    ...
    } elseif ($edited_Blog->get('media_location') == 'custom') { // if media location is customized
    ...
    // skins url:
    $assets_url_data['skins_assets_url_type'] = [
    ...
    // rsc url:
    $assets_url_data['rsc_assets_url_type'] = [
    ...
    // plugins url:
    $assets_url_data['plugins_assets_url_type'] = [
    ...
    // htsrv url:
    $assets_url_data['htsrv_assets_url_type'] = [
    ...
    foreach ($assets_url_data as $asset_url_type => $asset_url_data) {
        if (isset($asset_url_data['info'])) { // Display only info for this url type
            $Form->info($asset_url_data['label'], $asset_url_data['info']);
        } else { // Display options full list
            ...
            if (! in_array($asset_url_type, ['media_assets_url_type', 'htsrv_assets_url_type']) &&
            ...
            if (! in_array($asset_url_type, ['skins_assets_url_type', 'media_assets_url_type', 'htsrv_assets_url_type']) &&
            ...
            if (! in_array($asset_url_type, ['plugins_assets_url_type', 'htsrv_assets_url_type'])) {
                $absolute_url_note .= ' -- ' . TB_('This may be located in a CDN zone');
            }
            $Form->radio($asset_url_type, $edited_Blog->get_setting($asset_url_type), [
                ['relative', (
                    in_array($asset_url_type, ['skins_assets_url_type', 'media_assets_url_type', 'htsrv_assets_url_type']) ?
                ...
                ['basic', TB_('URL configured in Basic Config'), $basic_asset_url_note],
                ['absolute', TB_('Absolute URL') . ':', '',
...
$Form->info('Note', sprintf(
    TB_('Login, Registration and Password operations are controlled by the following settings: <a %s>In-skin login</a> and <a %s>Require SSL for login</a>'),
...
$Form->begin_fieldset(TB_('Tag page URLs') . get_manual_link('tag-page-url-settings'), [
...
$Form->text_input(
    'user_prefix',
    $edited_Blog->get_setting('user_prefix'),
    30,
    TB_('Prefix'),
...
$Form->radio(
    'user_links',
    $edited_Blog->get_setting('user_links'),
    [
        ['params', TB_('Use params'), TB_('E-g: ') . '<code>?disp=user&user_ID=4</code>'],
        ['prefix_id', TB_('Use prefix with user ID'), TB_('E-g: ') . '<code>prefix:4</code>'],
        ['prefix_login', TB_('Use prefix with user login'), TB_('E-g: ') . '<code>prefix:login</code>'],
    ],
    TB_('User profile URLs'),
    true
);
...
$tag_setting_params = [];
if (! is_pro()) {	// Disable for not-PRO version:
    $tag_setting_params['disabled'] = 'disabled';
}
$tag_setting_input_params = array_merge($tag_setting_params, [
    'maxlength' => 255,
]);
...
$Form->radio('tinyurl_type', $edited_Blog->get_setting('tinyurl_type'), [
    ['basic', TB_('Basic: Append to collection URL'), TB_('E-g: ')
                 . url_add_tail($blogurl, '/' . $tinyurl_slug)],
    ['advanced', TB_('Advanced: Append to special domain URL') . ':', '', $tinyurl_domain_field . $tinyurl_domain_note, 'class="radio-input"'],
], TB_('Tiny URLs'), true);
