...
$Form->radio('search_sort_by', $edited_Blog->get_setting('search_sort_by'), [
    ['score', TB_('Score')],
    ['date', TB_('Date'), TB_('If sorted by date, everything without a date will be sorted last.')],
], TB_('Sort by'), true);
$Form->checklist([
    ['search_include_posts', 1, TB_('Posts'), $edited_Blog->get_setting('search_include_posts')],
    ['search_include_cmnts', 1, TB_('Comments'), $edited_Blog->get_setting('search_include_cmnts')],
    ['search_include_metas', 1, TB_('Internal comments'), $edited_Blog->get_setting('search_include_metas')],
    ['search_include_files', 1, TB_('Files'), $edited_Blog->get_setting('search_include_files')],
    ['search_include_cats',  1, TB_('Categories'), $edited_Blog->get_setting('search_include_cats')],
    ['search_include_tags',  1, TB_('Tags'), $edited_Blog->get_setting('search_include_tags')],
], 'search_include', TB_('Include'));
// Scoring:
$score_settings = [
    TB_('Scoring for posts') => [
...
$Form->select_input_array('search_result_template_item', $edited_Blog->get_setting('search_result_template_item'), $template_options, sprintf(TB_('Template for %s search result'), TB_('Item')), null, [
    'input_suffix' => $template_input_suffix,
]);
$Form->select_input_array('search_result_template_comment', $edited_Blog->get_setting('search_result_template_comment'), $template_options, sprintf(TB_('Template for %s search result'), TB_('Comment')), null, [
    'input_suffix' => $template_input_suffix,
]);
$Form->select_input_array('search_result_template_meta', $edited_Blog->get_setting('search_result_template_meta'), $template_options, sprintf(TB_('Template for %s search result'), TB_('Internal comment')), null, [
    'input_suffix' => $template_input_suffix,
]);
$Form->select_input_array('search_result_template_file', $edited_Blog->get_setting('search_result_template_file'), $template_options, sprintf(TB_('Template for %s search result'), TB_('File')), null, [
    'input_suffix' => $template_input_suffix,
]);
$Form->select_input_array('search_result_template_category', $edited_Blog->get_setting('search_result_template_category'), $template_options, sprintf(TB_('Template for %s search result'), TB_('Category')), null, [
    'input_suffix' => $template_input_suffix,
]);
$Form->select_input_array('search_result_template_tag', $edited_Blog->get_setting('search_result_template_tag'), $template_options, sprintf(TB_('Template for %s search result'), TB_('Tag')), null, [
    'input_suffix' => $template_input_suffix,
]);
$Form->end_fieldset();
