...
if (! $creating && $perm_section_edit && $edited_Section->ID != 1) {	// Display a link to delete the section only if Current user has no permission to edit it:
...
if ($perm_section_edit && $edited_Section->ID != 1) {
    $Form->text_input('sec_name', $edited_Section->get('name'), 50, TB_('Name'), '', [
        'maxlength' => 255,
        'required' => true,
    ]);
} else {
...
if ($perm_section_edit) {
    $Form->username('sec_owner_login', $owner_User, TB_('Owner'), TB_('Login of this section\'s owner.'), '', [
        'required' => true,
    ]);
} else {
...
// Order:
if ($perm_section_edit) {
    $Form->text_input('sec_order', $edited_Section->get('order'), 5, TB_('Order number'), '', [
        'maxlength' => 11,
        'required' => true,
    ]);
} else {
...
if (! $perm_section_edit) {	// Don't display a submit button if Current user has no permission to edit this section:
    $Form->end_form();
} elseif ($creating) {	// Display a button to create new section:
    $Form->end_form([['submit', 'actionArray[create_section]', TB_('Record'), 'SaveButton']]);
} else {	// Display a button to update the section:
    $Form->end_form([['submit', 'actionArray[update_section]', TB_('Save Changes!'), 'SaveButton']]);
}
