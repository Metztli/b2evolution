 ...
 * Display task errors.
 */
function task_errors($errors = [], $type = 'danger')
{
    if (empty($errors)) {
        return;
    }
...
function get_post_timestamp_data($num_posts = 1, $min = 30, $max = 720, $base_timestamp = null)
{
    if (is_null($base_timestamp)) {
        ...
        if (empty($install_post_random_timestamps)) {	// Start new timestamp from current time:
            $base_timestamp = time();
        } else {	// Continue to use next time after previous calling:
...
function is_available_item_type($blog_ID, $item_type_name_or_template = '#')
{
    ...
    } elseif (preg_match('/^\$(.+)\$$/', $item_type_name_or_template, $ityp_match)) {	// This is a request to check by template name, because param is in format like $template_name$:
        $mode = 'template';
 ...
 */
function echo_installation_options($params = [])
{
    $params = array_merge([
    ...
    foreach ($collections as $coll_index => $coll_title) {	// Display the checkboxes to select what demo collection to install
...
function create_blog(
    ...
    if (! $blog_Skin) {	// Try looking for skin using class name:
        $blog_skin_class = strtolower($blog_skin_name);
        $blog_skin_class = trim(preg_replace(['/\h+/', '/_[s|S]kin$/'], ['_', ''], $blog_skin_class)) . '_Skin';
    ...
    if ($blog_access_type != '#') {	// Force default new collection URL with a given param:
    ...
    if (($kind == 'forum' || $kind == 'manual') && ($Plugin = &$Plugins->get_by_code('b2evMark')) !== false) {	// Initialize special Markdown plugin settings for Forums and Manual blogs
    ...
    if ($kind == 'photo') {	// Display category directory on front page for photo blogs
    ...
    if ($install_test_features) {
        ...
        if ($kind == 'manual') {	// Set a posts ordering by 'postcat_order ASC'
    ...
    if (! empty($blog_allow_access)) {
        ...
        switch ($blog_allow_access) {	// Automatically enable/disable moderation statuses:
            case 'public':
    ...
    if ($local_installation || $Blog->get_setting('allow_access') != 'public') {	// Turn off all ping plugins if the installation is local/test/intranet or this is a not public collection:
...
function create_user($params = [])
{
    ...
    if (! $User->dbinsert(false)) {	// Don't continue if user creating has been failed
        return false;
    }
    ...
    if (! empty($params['org_IDs'])) {	// Add user to organizations:
        $User->update_organizations($params['org_IDs'], $params['org_roles'], $params['org_priorities'], true);
    }

    if (! empty($params['fields'])) {	// Additional user fields
        global $DB;
        $fields_SQL = new SQL();
        ...
        foreach ($fields as $field_ID => $field_code) {
            if (! isset($params['fields'][$field_code])) {	// Skip wrong field:
                continue;
            }

            if (is_string($params['fields'][$field_code])) {
                $params['fields'][$field_code] = [$params['fields'][$field_code]];
            }

            foreach ($params['fields'][$field_code] as $field_value) {	// SQL record for each field value
                $user_field_records[] = '( ' . $User->ID . ', ' . $field_ID . ', ' . $DB->quote($field_value) . ' )';
            }
        }
        if (count($user_field_records)) {	// Insert all user fields by single SQL query
...
function assign_profile_picture(&$User, $login = null)
{
    ...
    if (! $File->exists()) {	// Don't assign if default user avatar doesn't exist on disk:
        return;
    }
...
{
    if (empty($secondary_group_IDs)) {	// Nothing to assign, Exit here:
        return;
    }
    ...
    $DB->query(
        'INSERT INTO T_users__secondary_user_groups ( sug_user_ID, sug_grp_ID )
...
function create_demo_organization($owner_ID, $org_name = 'Company XYZ', $add_current_user = true)
{
    ...
    } else {	// Sample organization does not exist, let's create one
    ...
    if ($add_current_user && $demo_org_ID && isset($current_User)) {
        ...
        if (isset($org_data[$demo_org_ID])) {
            $org_roles = [$org_data[$demo_org_ID]['role']];
            $org_priorities = [$org_data[$demo_org_ID]['priority']];
        }
        $current_User->update_organizations([$demo_org_ID], $org_roles, $org_priorities, true);
    }
...
function get_demo_user($login, $create = false, $output = true, &$error_messages = null)
{
    ...
    if (! isset($demo_users[$login])) {	// Specified login not included in the list of demo users:
        return false;
    }
    ...
    if (isset($demo_users[$login]['group'])
            && $user_default_Group = $GroupCache->get_by_name($demo_users[$login]['group'], false, false)) {	// Get default group ID:
    ...
    if (! $demo_user && $create) {	// Demo user does not exist yet but we can create:
        if ($login == 'admin' && $admin_user = $UserCache->get_by_ID(1, false, false)) {	// Admin user must have been renamed, skip:
            return false;
        }
        ...
        $user_defaults = array_merge($demo_users[$login], [
            'group_ID' => $group_ID,
        ...
        if ($demo_user === false) {	// Cannot create demo user, exiting:
        ...
        if ($demo_user) {	// Insert default user settings:
        ...
        if (! $demo_user->get('avatar_file_ID')) {	// Demo user already exists but has avatar has not been set:
            assign_profile_picture($demo_user);
        }

        if (isset($user_defaults['org_IDs']) && isset($user_defaults['org_roles'])) {
            $org_priorities = isset($user_defaults['org_priorities']) ? $user_defaults['org_priorities'] : [];
...
function create_demo_messages()
{
    ...
    for ($i = 0; $i < count($users); $i++) {
        ...
        $loop_Message->Thread->recipients_list = [$recipient_ID];
        ...
        if ($conversation_saved) {
            ...
            if ($loop_reply_Message->dbinsert()) {
                ...
                $recipient_list = array_diff($recipient_list, [$loop_reply_Message->author_user_ID]);
 ...
 */
function create_demo_contents($demo_users = [], $use_demo_users = true, $initial_install = true)
{
        ...
        default:
            ...
            switch ($demo_content_type) {
                ...
                default: // complex_site
                    $collections = param('collections', 'array:string', []);
    ...
    if (! $install_collection_minisite &&
        ...
        ! $install_collection_tracker) {	// Don't try to install demo content if no collection is selected:
        return 0;
    }
    ...
    if ($install_collection_home) {	// Install Home blog
        $coll_error_messages = [];
        ...
        if ($blog_ID = create_demo_collection('main', $jay_moderator_ID, $use_demo_users, $timeshift, $section_ID, $coll_error_messages)) {
            ...
            } elseif ($Blog = &$BlogCache->get_by_ID($blog_ID, false, false)) {	// Insert basic widgets:
    ...
    if ($install_collection_bloga) {	// Install Blog A
        $coll_error_messages = [];
        $timeshift += 86400;
    ...
    if ($install_collection_blogb) {	// Install Blog B
        $coll_error_messages = [];
        $timeshift += 86400;
    ...
    if ($install_collection_photos) {	// Install Photos blog
        $coll_error_messages = [];
        $timeshift += 86400;
    ...
    if ($install_collection_forums) {	// Install Forums blog
        $coll_error_messages = [];
        $timeshift += 86400;
    ...
    if ($install_collection_manual) {	// Install Manual blog
        $coll_error_messages = [];
        $timeshift += 86400;
    ...
    if ($install_collection_tracker) {	// Install Tracker blog
        $coll_error_messages = [];
        $timeshift += 86400;
    ...
    if ($install_collection_minisite) {	// Install Mini-site collection
        $coll_error_messages = [];
        $timeshift += 86400;
    ...
    if ($demo_content_type == 'minisite') {
        ...
        if ($first_Blog = &$BlogCache->get_first()) {	// Set first blog as default login and default messaging collection
...
[cta:1:link]Call to action 1 link only[/cta]'),
    ...
    foreach ($email_campaigns as $email_campaign) {
        ...
        if ($EmailCampaign->dbinsert() && ! empty($user_IDs)) {	// Add recipients after successfull email campaign creating,
...
function create_default_automations()
{
    ...
    if ($Automation->dbinsert()) {	// Add steps after successfull creating of the automation:
...
function create_demo_emails()
{
    if (param('create_demo_email_lists', 'boolean', false)) {
        ...
        if ($created_lists_num > 0) {	// Install other emails data only if at least one email list has been installed:
            if (param('create_demo_email_campaigns', 'boolean', false)) {	// Install demo email campaigns:
                evo_flush();
                create_default_email_campaigns();
            }

            if (param('create_demo_automations', 'boolean', false)) {	// Install demo automations:
                evo_flush();
...
<strong>' . TD_('The main purpose for this blog is to be included as a side item to other blogs where it will display your favorite/related links.') . '</strong>';
        ...
        case 'main':
            ...
            if ($blog_home_ID) {
                if (! $DB->get_var('SELECT set_value FROM T_settings WHERE set_name = ' . $DB->quote('info_blog_ID')) && ! empty($blog_home_ID)) {	// Save ID of this blog in settings table, It is used on top menu, file "/skins_site/_site_body_header.inc.php"
        ...
        case 'blog_b':
            ...
            assign_secondary_groups($owner_ID, [$blogb_Group->ID]);
...
function create_sample_content($collection_type, $blog_ID, $owner_ID, $use_demo_user = true, $timeshift = 86400, &$error_messages = null)
{
    ...
    if (! isset($installed_collection_info_pages)) {	// Array for item IDs which should be used in default shared widget containers "Main Navigation" and "Navigation Hamburger":
        $installed_collection_info_pages = [];
    }

    $timestamp = time();
    $comment_item_IDs = [];
    $additional_comments_item_IDs = [];
...
<p>' . TD_('This is the extended text. You only see it when you have clicked the "more" link.') . '</p>' . get_filler_text('lorem_2more'),
        'files' => [
            ['monument-valley/john-ford-point.jpg', 'cover'],
            ['monument-valley/monument-valley-road.jpg', 'teaser'],
            ['monument-valley/monuments.jpg', 'aftermore'],
        ],
    ];

    $demo_items['post_with_images'] = [
...
<p>Check out the photoblog (accessible through the links at the top) to see a completely different skin focused more on the photos than on the blog text.</p>'),
        'files' => [
            ['monument-valley/monument-valley.jpg', 'cover'],
            ['monument-valley/monuments.jpg', 'teaser'],
            ['monument-valley', 'aftermore'],
            ['monument-valley/bus-stop-ahead.jpg', 'aftermore'],
        ],
    ];

    $demo_items['mongolian_beef'] = [
...
<ol>
	...
	<li>' . TD_('Serve with rice, top with green onions') . '</li>
        ...
        'custom_fields' => [
            ['course', TD_('Main Course')],
            ['cuisine', TD_('Mongolian')],
            ['servings', '4'],
            ['prep_time', '2'],
            ['cook_time', '35'],
            ['passive_time', '5'],
            ['ingredients', TD_('vegetable oil
1/2 teaspoon ginger
1 tablespoon garlic
1/2 cup soy sauce
1/2 cup water
...
2 large green onions')],
        ...
        'files' => [
            ['recipes/mongolian-beef.jpg', 'teaser'],
        ],
    ];

    $demo_items['stuffed_peppers'] = [
...
<ol>
	...
	<li>' . TD_('fill peppers, place in bowl, top with chives and hot sauce') . '</li>
        ...
        'custom_fields' => [
            ['course', TD_('Main Course')],
            ['cuisine', TD_('South African')],
            ['servings', '2'],
            ['prep_time', '1'],
            ['cook_time', '20'],
            ['passive_time', '3'],
            ['ingredients', TD_('1 jar Peppedew Peppers (or piquante pepper)
...
hearty crack of pepper')],
        ...
        'files' => [
            ['recipes/stuffed-peppers.jpg', 'teaser'],
        ],
    ];

    $demo_items['custom_fields_example'] = [
...
<p>' . sprintf(TD_('It is also possible to create links using a custom field URL: %s'), '[link:url_field:.btn.btn-info]Click me![/link]') . '</p>',
        'custom_fields' => [
            ['first_numeric_field', '123'],
            ['second_numeric_field', '456'],
            ['usd_price', '29.99'],
            ['eur_price', '24.79'],
            ['first_string_field', 'abc'],
            ['multiline_plain_text_field', 'This is a sample text field.
It can have multiple lines.'],
            ['multiline_html_field', 'This is an <b>HTML</b> <i>field</i>.'],
            ['url_field', 'http://b2evolution.net/'],
            ['checkmark_field', '1'],
        ],
        'files' => [
            [
                ...
                'attachment',
                'custom_field' => 'image_1',
                'set_var' => 'custom_item_link_ID',
            ],
        ],
    ];

    $demo_items['another_custom_fields_example'] = [
...
<p>' . sprintf(TD_('It is also possible to create links using a custom field URL: %s'), '[link:url_field:.btn.btn-info]Click me![/link]') . '</p>',
        'custom_fields' => [
            ['first_numeric_field', '123.45'],
            ['second_numeric_field', '456'],
            ['usd_price', '17.50'],
            ['eur_price', '14.95'],
            ['first_string_field', 'abcdef'],
            ['multiline_plain_text_field', 'This is a sample text field.
It can have multiple lines.
...
. '[compare:$this$,$parent$]',
        'custom_fields' => [
            ['first_numeric_field', '123'],
            ['first_string_field', 'abc'],
            ['image_1', '#get_var#custom_item_link_ID#'],
            ['checkmark_field', '1'],
        ],
        'files' => [
            [
    ...
    $demo_items['extended_post_with_no_teaser'] = [
...
<p>And, of course, read the <a href="%s" target="_blank">manual on skins</a>!</p>'), get_manual_url('skin-structure')),
    ...
    $demo_items['apache_optimization'] = [
        'title' => TD_('Apache optimization...'),
        'category' => 'background',
        'mustread' => is_pro(),
        'content' => sprintf(
            TD_('<p>b2evolution comes with an <code>.htaccess</code> file destined to optimize the way b2evolution is handled by your webseerver (if you are using Apache). In some circumstances, that file may not be automatically activated at setup. Please see the man page about <a %s>Tricky Stuff</a> for more information.</p>
...
<p>It appears in multiple categories.</p>'),
    ...
    switch ($collection_type) {
        ...
        case 'minisite':
            ...
            $demo_items['more_info'] = [
                'title' => TD_('More info'),
                'category' => 'b2evolution',
                'type' => 'Widget Page',
                'files' => [
                    ['monument-valley/monuments.jpg', 'cover'],
                ],
...
<p>Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum Lorem ipsum</p>',
            ...
            $demo_items['about_this_site'] = [
                ...
                'content' => TD_('This website is powered by b2evolution.') . "\r\n\r\n"
                    ...
                    . '[/switcher]',
                'files' => [
                    ['logos/b2evolution_1016x208_wbg.png'],
                ],
            ...
            $demo_items['b2evo_the_other_blog_tool'] = [
                ...
                'files' => [
                    ['banners/b2evo-125-other.png'],
                ],
            ];

            $demo_items['b2evo_better_blog_software'] = [
                ...
                'files' => [
                    ['banners/b2evo-125-better.png'],
                ],
            ];

            $demo_items['b2evo_the_software_for_blog_pros'] = [
                ...
                'files' => [
                    ['banners/b2evo-125-pros.png'],
                ],
            ];

            $demo_items['register_content'] = [
            ...
            $demo_items['this_is_a_content_block'] = [
...
<p>A content block can be included in several places.</p>'),
        ...
        case 'blog_b':
            ...
            // Additional sample Items:
            $demo_items['b2evo_skins_repository'] = [
            ...
            $demo_items['widgets_tag_sub_intro_post'] = [
            ...
            $demo_items['b2evolution_tips_category_sub_intro_post'] = [
            ...
            $demo_items['bus_stop_ahead'] = [
                ...
                'files' => [
                    [
                        ...
                        'set_var' => 'photo_link_1_ID',
                    ],
                    [
                        'monument-valley/john-ford-point.jpg',
                        'aftermore',
                        'set_var' => 'photo_link_2_ID',
                    ],
                    ['monument-valley/monuments.jpg', 'aftermore'],
                    [
                        'monument-valley/monument-valley-road.jpg',
                        'aftermore',
                        'set_var' => 'photo_link_4_ID',
                    ],
                    ['monument-valley/monument-valley.jpg', 'aftermore'],
                ],
            ];
            if ($install_test_features) {	// Add examples for infodots plugin
...
[infodot:%s:207:28:15em]Red planet[enddot]', '#get_var#photo_link_1_ID#', '#get_var#photo_link_2_ID#', '#get_var#photo_link_4_ID#');
            ...
            $demo_items['food'] = [
                ...
                'category' => 'landscapes',
                'files' => [
                    ['recipes/mongolian-beef.jpg'],
                    ['recipes/stuffed-peppers.jpg'],
                ],
            ];
            break;
            ...
            $categories = [
                'a_forum_group' => [
                    ...
                    'subs' => [
                        'welcome' => [
                            TD_('Welcome'),
                            'desc' => TD_('Welcome description'),
                            'order' => 1,
                        ],
                        'a_forum' => [
                            TD_('A forum'),
                            'desc' => TD_('Short description of this forum'),
                            'order' => 2,
                        ],
                        'another_forum' => [
                            TD_('Another forum'),
                ...
                'another_group' => [
                    ...
                    'subs' => [
                        'background' => [
                            TD_('Background'),
                            'desc' => TD_('Background description'),
                            'order' => 1,
                        ],
                        'news' => [
                            TD_('News'),
                            'desc' => TD_('News description'),
                            'order' => 2,
                        ],
                        'fun' => [
                            TD_('Fun'),
                            'desc' => TD_('Fun description'),
                            'order' => 3,
                            'subs' => [
                                'in-real-life' => [
                                    TD_('In real life'),
                                    'order' => 4,
                                    'subcat_ordering' => 'alpha',
                                    'subs' => [
                                        'movies' => TD_('Movies'),
                                        'music' => TD_('Music'),
                                    ],
        ...
        case 'manual':
            ...
            $demo_items['wiki_tables']['extra_cats'] = ['user-guide'];
            ...
            $demo_items['post_with_images']['extra_cats'] = ['blogs'];
            $demo_items['post_with_images']['order'] = 10;
            $demo_items['second_post']['order'] = 20;
            $demo_items['second_post']['extra_cats'] = ['getting-started'];
            $demo_items['first_post']['order'] = 10;

            // Additional sample Items:
            $demo_items['chapter_intro'] = [
...
Just to be clear: this is a **demo** of a manual. The user manual for b2evolution is here: http://b2evolution.net/man/.'),
        ...
        case 'group':
            ...
            $task_status = [1, 2]; // New, In Progress
            ...
            for ($i = 0, $j = 0, $k = 0, $m = 0; $i < 20; $i++) {
                ...
                if ($use_demo_user) {	// Assign task to allowed assignee:
    ...
    if (! empty($categories)) {	// If at least one category is defined for the collection:
        // Create sample Categories:
        create_demo_categories($categories, $blog_ID);

        if (($first_category_ID = get_demo_category_ID('#first#', $categories)) &&
            ($edited_Blog = &$BlogCache->get_by_ID($blog_ID, false, false))) {	// Use first category as default:
            $edited_Blog->set_setting('default_cat_ID', $first_category_ID);
            $edited_Blog->dbupdate();
        }

        if (! empty($exclude_demo_items)) {	// Exclude demo items which must not be installed:
        ...
        foreach ($demo_items as $demo_item_key => $demo_item) {
            ...
            if (! is_available_item_type($blog_ID, $item_type)) {	// Skip not supported Item Type:
            ...
            if (! ($category_ID = get_demo_category_ID($demo_item['category'], $categories))) {	// Skip Item without category:
            ...
            if (! empty($demo_item['featured'])) {	// Mark the Item as featured:
                $new_Item->set('featured', 1);
            }

            if (! empty($demo_item['hide_teaser'])) {	// Set setting "Hide Teaser":
            ...
            if (! empty($demo_item['priority'])) {	// Set task priority:
                $new_Item->set('priority', $demo_item['priority']);
            }
            if (! empty($demo_item['pst_ID'])) {	// Set task status:
                $new_Item->set('pst_ID', $demo_item['pst_ID']);
            }
            if (! empty($demo_item['assigned_user_ID'])) {	// Set task assigned user ID:
                $new_Item->set('assigned_user_ID', $demo_item['assigned_user_ID']);
            }
            if (isset($demo_item['mustread']) && $demo_item['mustread'] === true) {	// Set "Must read" flag:
                $new_Item->set_setting('mustread', 1);
            }

            if (isset($demo_item['settings']) && is_array($demo_item['settings'])) {	// Additional settings:
            ...
            if (! $insert_new_item_result) {	// Skip next code if Item could not be inserted successfully:
            ...
            if (! empty($demo_item['files'])) {	// Attach files to the Item:
                ...
                foreach ($demo_item['files'] as $f => $demo_item_file) {
                    ...
                    if ($new_File->exists()) {
                        ...
                        if (isset($demo_item_file['custom_field'])) {	// Update custom field with new linked file:
                            $new_Item->set_custom_field($demo_item_file['custom_field'], $new_file_link_ID);
                            $update_new_item = true;
                        }
                        if (isset($demo_item_file['set_var'])) {	// Set var which may be used for next inserted Items:
            ...
            if (! empty($demo_item['update_content'])) {	// Update content after insert with new var liek Link IDs:
            ...
            if (! empty($demo_item['widget_info_page'])) {	// Update global variable which may be used on install default widgets:
            ...
            switch ($item_type) {
                ...
                case 'Intro-Cat':
                    ...
                    $comment_item_IDs[] = [$new_Item->ID, $item_date];
                    break;
            }
        }
    }
    ...
    if (count($comment_users) === 1) {	// Only 1 demo user, use anonymous users:
        $comment_users = null;
    }
    ...
    if ($install_test_features && count($additional_comments_item_IDs) && $use_demo_user) {	// Create the additional comments when we install all features
        foreach ($additional_comments_item_IDs as $additional_comments_item_ID) {
            ...
            foreach ($demo_users as $demo_user) {	// Insert the comments from each user
...
{
    if (strpos($content, '#get_') === false) {	// No vars in content:
        return $content;
    }

    if (preg_match_all('/#get_item#([a-z0-9_]+)#(([a-z_]+)#)?/i', $content, $matches)) {	// Item field var:
    ...
    if (preg_match_all('/#get_var#([a-z0-9_]+)#/i', $content, $matches)) {	// Demo var:
        foreach ($matches[0] as $m => $match) {
...
{
    if (empty($categories)) {	// No categories:
        return;
    }
    ...
    foreach ($categories as $cat_key => $cat_data) {
        ...
        if ($cat_ID) {	// If category was inserted successfully:
            $categories[$cat_key]['ID'] = $cat_ID;
            if (! empty($cat_data['subs'])) {	// Create sub categories recursively:
...
function get_demo_category_ID($category_key, $categories, $use_first = true)
{
    $first_cat_ID = false;
    foreach ($categories as $cat_key => $cat_data) {
        if ($use_first && $first_cat_ID === false && ! empty($cat_data['ID'])) {	// Set first category ID for case if no requested category will be found below:
            if (empty($cat_data['meta'])) {	// If first category is not meta:
                $first_cat_ID = $cat_data['ID'];
            } elseif (! empty($cat_data['subs']) &&
                ($first_sub_cat_ID = get_demo_category_ID($category_key, $cat_data['subs']))) {	// Get first category from subcategories of the root meta category:
                $first_cat_ID = $first_sub_cat_ID;
            }
            if ($first_cat_ID && $category_key == '#first#') {	// Don't search next when only first category ID is requested:
                return $first_cat_ID;
            }
        }

        if ($category_key == $cat_key && ! empty($cat_data['ID'])) {	// Return ID of the detected category:
            return $cat_data['ID'];
        } elseif (! empty($cat_data['subs']) &&
                ($sub_cat_ID = get_demo_category_ID($category_key, $cat_data['subs'], false))) {	// Return ID of the detected category in sub categories:
            return $sub_cat_ID;
        }
    }
...
function create_demo_poll()
{
    ...
    if (empty($demo_poll_ID)) {
        ...
        if ($result) {
            ...
            // Add poll answers:
            $answer_texts = [
                [TD_('Multiple blogs'), 1],
                [TD_('Photo Galleries'), 2],
                [TD_('Forums'), 3],
                [TD_('Online Manuals'), 4],
                [TD_('Lists / E-mailing'), 5],
                [TD_('Easy Maintenance'), 6],
            ];
...
function install_demo_content()
{
    ...
    if ($create_demo_organization) {
        ...
        if ($create_demo_organization) {
            ...
            if ($user_org_IDs) {
                ...
                $current_User->update_organizations($user_org_IDs, ['King of Spades'], [0], true);
                task_end();
            }
        }
    }
    ...
    if ($create_demo_email_lists) {	// Create demo emails data like lists, campaigns, automations:
    ...
    if ($collections_installed || $emails_data_installed) {
        ...
        if ($collections_installed) {	// Display button to view website if at least one collection was created:
            global $baseurl;
