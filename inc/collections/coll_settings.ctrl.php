...
if ($tab == 'widgets') {	// This is another controller!
    require_once dirname(__FILE__) . '/../widgets/widgets.ctrl.php';
    return;
} elseif ($tab == 'manage_skins') {	// This is another controller!
...
if (empty($tab) || $tab == "dashboard") {
    ...
    if (($tab == 'perm' || $tab == 'permgroup')
        && (empty($blog) || ! $Blog->advanced_perms)) {	// We're trying to access advanced perms but they're disabled!
        $tab = 'general';	// the screen where you can enable advanced perms
        if ($action == 'update') { // make sure we don't update anything here
            $action = 'edit';
        }
    }
}
...
switch ($action) {
    ...
    case 'update_confirm':
        ...
        switch ($tab) {
            ...
            case 'urls':
            case 'comments':
                if ($edited_Blog->load_from_Request([$tab])) { // Commit update to the DB:
                    global $Settings;
            ...
            case 'seo':
                if ($edited_Blog->load_from_Request(['seo'])) { // Commit update to the DB:
                    $edited_Blog->dbupdate();
                    $Messages->add(TB_('The collection settings have been updated.'), 'success');
                    // Redirect so that a reload doesn't write to the DB twice:
                    header_redirect($update_redirect_url, 303); // Will EXIT
            ...
            case 'skin':
                if ($skinpage == 'selection') {	// Set new skin for the collection:
                    if ($edited_Blog->load_from_Request([])) { // Commit update to the DB:
                        $edited_Blog->dbupdate();

                        if (param('reset_widgets', 'integer', 0)) {	// Widget must be reseted:
                            $updated_skin_type = '';
                            if (get_param('normal_skin_ID') !== null) {	// Normal skin has been changed:
                                $updated_skin_type = 'normal';
                            } elseif (get_param('tablet_skin_ID') !== null) {	// Tablet skin has been changed:
                                $updated_skin_type = 'tablet';
                            } elseif (get_param('mobile_skin_ID') !== null) {	// Mobile skin has been changed:
                                $updated_skin_type = 'mobile';
                            }
                            if (! empty($updated_skin_type)) {	// Reset previous widgets with new from skin default widget declarations:
                        ...
                        if ((! $Session->is_mobile_session() && ! $Session->is_tablet_session() && ! $Session->is_alt_session() && param('normal_skin_ID', 'integer', null) !== null) ||
                            ...
                            ($Session->is_alt_session() && param('alt_skin_ID', 'integer', null) !== null)) {	// Redirect to blog home page if we change the skin for current device type
                ...
                } else {	// Update skin params/settings of the collection:
                    ...
                    $skin_ID = $Blog->get($skin_type . '_skin_ID', [
                        'real_value' => ($skin_type != 'normal'),
                    ]);
            ...
            case 'advanced':
                if ($edited_Blog->load_from_Request(['pings', 'cache', 'authors', 'login', 'styles', 'template', 'credits', 'meta'])) { // Commit update to the DB:
    ...
    case 'disable_setting':
        ...
        switch ($setting) {
            ...
            case 'page_cache':
                ...
                if ($setting_value) { // If we are enabling the page caching we should also enable AJAX forms
...
if ($action == 'dashboard') {
    ...
    if (! check_user_perm('blog_ismember', 'view', false, $blog)) { // We don't have permission for the requested blog (may happen if we come to admin from a link on a different blog)
        set_working_blog(0);
    ...
    $AdminUI->breadcrumbpath_init(true, [
        ...
        'url' => $admin_url . '?ctrl=coll_settings&amp;blog=$blog$',
    ]);
    ...
    if ($blog) {
        ...
        if ($user_perm_moderate_cmt) {
            ...
            $CommentList->set_filters([
                ...
                'user_perm' => 'moderate',
                'post_statuses' => ['published', 'community', 'protected'],
                'order' => 'DESC',
                'comments' => 10,
            ]);
        ...
        ob_start();
        foreach ($post_moderation_statuses as $status) { // go through all statuses
            if (display_posts_awaiting_moderation($status, $block_item_Widget)) { // a block was displayed for this status
                $nb_blocks_displayed++;
            }
        }
        ...
        echo action_icon(TS_('View in Front-Office'), '', $Blog->get('url'), TB_('View in Front-Office'), 3, 4, [
            'class' => 'action_icon hoverlink btn btn-info',
        ]);
        echo '</div>';
        echo '<h2 class="col-xs-12 col-sm-9 col-sm-pull-3 col-lg-10 col-lg-pull-2 page-title">'
                . get_coll_fav_icon($Blog->ID, [
                    'class' => 'coll-fav',
                ]) . '&nbsp;'
        ...
        if ($perm_options_edit) {
            ...
            if ($perm_options_edit) { // We have some serious admin privilege:
                // -- Collection stats -- //{
                $chart_data = [];
        ...
        if ($have_comments_to_moderate || $have_posts_to_moderate) {
            ...
            if ($have_comments_to_moderate) {
                ...
                $opentrash_link = get_opentrash_link(true, false, [
                    'class' => 'btn btn-default btn-sm',
                    'before' => '<span id="recycle_bin">',
                    'after' => '</span> ',
                ]);
                $refresh_link = action_icon(TB_('Refresh comment list'), 'refresh', $admin_url . '?blog=' . $blog, ' ' . TB_('Refresh'), 3, 4, [
                    'onclick' => 'startRefreshComments( \'dashboard\' ); return false;',
                    'class' => 'btn btn-default btn-sm',
                ]);
        ...
        if (check_user_perm('meta_comment', 'view', false, $Blog->ID)) {	// If user has a perm to view internal comments of the collection:
            ...
            if ($CommentList->result_num_rows) {	// We have the internal comments
                load_funcs('comments/model/_comment_js.funcs.php');
        ...
        $ItemList->set_filters([
            'visibility_array' => get_visibility_statuses('keys', ['trash']),
        ...
        if ($ItemList->result_num_rows) {	// We have recent edits
            $nb_blocks_displayed++;

            echo '<!-- Start of Recently Edited Post Block-->';
            if (check_user_perm('blog_post_statuses', 'edit', false, $Blog->ID)) {	// We have permission to add a post with at least one status:
                $block_item_Widget->global_icon(TB_('Write a new post...'), 'new', '?ctrl=items&amp;action=new&amp;blog=' . $Blog->ID, TB_('New post') . ' &raquo;', 3, 4, [
                    'class' => 'action_icon btn-primary btn-sm',
                ]);
            }
            echo '<div class="items_container evo_content_block">';
        ...
        if ($nb_blocks_displayed == 0) {	// We haven't displayed anything yet!
            $nb_blocks_displayed++;
    ...
    if (! empty($chart_data)) { // JavaScript to initialize charts
        ?>
	<script>
    ...
    switch ($AdminUI->get_path(1)) {
        ...
        case 'settings':
            switch ($AdminUI->get_path(2)) {
                case 'general':
                    if ($action == 'type') {	// Form to change type
                        $AdminUI->disp_view('collections/views/_coll_type.form.php');
                    } else {	// General settings of blog
                        $next_action = 'update';
