 ...
 */
function score_text($text, $search_term, $words = [], $quoted_terms = [], $score_weight = 1)
{
    global $Debuglog;
    ...
    $all_found = true;
    foreach ($quoted_terms as $quoted_term) {	// find quoted keywords
    ...
    foreach ($words as $word) {
        ...
        if ($word_match_count = preg_match_all('/\b' . preg_quote($word, '/') . '\b/i', $text, $matches)) {	// Every word match gives one more score
        ...
        if ($any_match_count = preg_match_all('/' . preg_quote($word, '/') . '/i', $text, $matches)) {	// Every word match gives one more score
            $scores_map['word_case_insensitive_match'][$word] = $score_weight;
            $score += $scores_map['word_case_insensitive_match'][$word];
            if ($any_match_count > 1) {	// there are multiple occurrences
...
function score_tags($tag_name, $search_term, $score_weight = 4)
{
    ...
    if ($tag_name == utf8_trim($search_term)) {	// We use only EXACT match for post tags:
        $score = $score_weight;
 ...
 * @return integer the result score
 */
function score_date($date, $score_weights = [])
{
    global $localtimenow;

    $score_weights = array_merge([
        'future' => 0,
        'more_month' => 0,
        'last_month' => 1,
        'two_weeks' => 2,
...
function get_percentage_from_result_map($type, $scores_map, $quoted_parts, $keywords)
{
    switch ($type) {
        case 'item':
            $searched_parts = ['title', 'content', 'tags', 'excerpt', 'titletag', 'metakeywords'];
            break;

        case 'comment':
        case 'meta':
            $searched_parts = ['item_title', 'content'];
            break;

        case 'category':
            $searched_parts = ['name', 'description'];
            break;
        ...
        case 'file':
            $searched_parts = ['filename', 'title', 'alt', 'description'];
            break;
    ...
    foreach ($searched_parts as $searched_part) {
        if (isset($scores_map[$searched_part]['map']['whole_term'])) {	// The whole search term was found
            return 100;
        }
    }
...
function search_and_score_items($search_term, $keywords, $quoted_parts, $exclude_posts = '', $authors = '', $content_age = '')
{
    ...
    if (empty($exclude_posts)) {	// No posts to exclude:
        $exclude_posts = '';
    } else {	// Check if values are correct:
    ...
    if (! is_logged_in() && ! empty($authors)) {	// This is necessary because the 'authors_login' filter above will not work for non-existent logins
    ...
    foreach ($query_result as $row) {
        ...
        $scores_map['last_mod_date'] = score_date($row->post_datemodified, /* multipliers: */ [
...
function search_and_score_comments($search_term, $keywords, $quoted_parts, $authors = '', $content_age = '', $type = 'comment')
{
    ...
    if ($type == 'meta') {	// Search only for meta/internal comments:
        $search_filters['types'] = ['meta'];
    }
    ...
    foreach ($query_result as $row) {
        ...
        $scores_map['creation_date'] = score_date($row->comment_date, /* multipliers: */ [
...
function search_and_score_tags($search_term, $keywords, $quoted_parts)
{
    ...
    foreach ($tags as $tag) {
        if ($tag->post_count == 0) {	// Count only those tags which have at least one post linked to it, Skip this:
            continue;
        }
...
function search_and_score_files($search_term, $keywords, $quoted_parts, $authors = '', $content_age = '')
{
    ...
    if (! empty($authors)) {
        if (is_logged_in()) {
            if (preg_match('/^[0-9]+(,[0-9]+)*$/', $authors)) {	// If JS active, we will receive a numeric list:
                $files_SQL->WHERE_and('( comment_author_user_ID IN (' . $authors . ') OR post_creator_user_ID IN (' . $authors . ') )');
            } else {	// If JS not active, we will have more limited results.
    ...
    foreach ($files as $file) {
        if (empty($file->post_IDs) && empty($file->comment_IDs)) {	// Count only those files which have at least one post linked to it, Skip this:
            continue;
        }
...
function perform_scored_search($search_keywords, $searched_content_types = 'all', $exclude_posts = '', $search_authors = '', $content_age = 'anytime')
{
    ...
    $quoted_parts = [];
    if (preg_match_all('/(["\'])(?:(?=(\\\\?))\\2.)*?\\1/', $search_keywords, $matches)) {	// There are quoted search terms
        $quoted_parts = $matches[0];
        for ($index = 0; $index < count($quoted_parts); $index++) {	// Remove douple quotes around the parts
            $quoted_part_length = utf8_strlen($quoted_parts[$index]);
            if ($quoted_part_length > 2) {	// The quoted part is not an empty string
    ...
    if (isset($keywords[0]) && empty($keywords[0])) {	// The first item is an empty string when the search keyword contains only irrelevant characters
        unset($keywords[0]); // remove empty word
    }
    if (empty($keywords) && empty($quoted_parts)) {	// There is nothing to search for
        return [];
        // TODO: return NULL and display a specific error message like "Please enter some keywords to search."
    }

    if ($searched_content_types == 'all' || empty($searched_content_types)) {	// Search all result types:
    ...
    } else {	// Check what types should be searched:
    ...
    if (! empty($search_authors) || (! empty($content_age) && ($content_age != 'anytime'))) {	// If search by author or age is enabled then don't search results in categories and tags because they don't have such data:
    ...
    if ($search_type_item &&
        $Blog->get_setting('search_include_posts')) {	// Perform search on Items:
    ...
    if ($search_type_comment &&
        $Blog->get_setting('search_include_cmnts')) {	// Perform search on Comments:
    ...
    if ($search_type_meta &&
        ...
        check_user_perm('meta_comment', 'view', false, $Blog->ID)) {	// Perform search on Meta/Internal Comments:
    ...
    if ($search_type_category &&
        $Blog->get_setting('search_include_cats')) {	// Perform search on Chapters:
    ...
    if ($search_type_tag &&
        $Blog->get_setting('search_include_tags')) {	// Perform search on Tags:
    ...
    if (count($search_result) > 0) {
        ...
        foreach ($search_result as $r => $result_item) {
            if ($Blog->get_setting('search_sort_by') == 'date') {	// Sort by date:
                if (isset($result_item['date'])) {	// If search item has a date, e-g posts and comments:
                    $score_result[] = preg_replace('#[^\d]+#', '', $result_item['date']) . '-' . $result_item['score'];
                } else {	// If search item has no date, e-g categories and tags:
            ...
            if ($max_score_result['score'] < $result_item['score']) {	// Find item with max score:
                $max_score_result = $result_item;
            }
            if (! $debug) {	// Don't store scores map in Sessions when debug is OFF:
    ...
    modules_call_method('handle_searched_results', [
 ...
 */
function search_result_block($params = [])
{
    ...
    if (! $allow_cache	// Force to load new search results, e-g when form is submitted, but don't force on page switching
        ...
        $searched_content_types = [];
        if ($Blog->get_setting('search_include_posts')) {	// Search items:
            $searched_content_types[] = 'item';
        }
        if ($Blog->get_setting('search_include_cmnts')) {	// Search comments:
            $searched_content_types[] = 'comment';
        }
        if ($Blog->get_setting('search_include_metas') &&
            check_user_perm('meta_comment', 'view', false, $Blog->ID)) {	// Search meta/internal comments:
            $searched_content_types[] = 'meta';
        }
        if ($Blog->get_setting('search_include_cats')) {	// Search categories:
            $searched_content_types[] = 'category';
        }
        if ($Blog->get_setting('search_include_tags')) {	// Search tags:
            $searched_content_types[] = 'tag';
        }
        if ($Blog->get_setting('search_include_files')) {	// Search files:
    ...
    } else {	// We found the desired saved search results in the Session:
        if ($debug) {	// Display counts
    ...
    if ($max_score == 0) {	// This must not happens, however use 1 to avoid error of division by zero:
        $max_score = 1;
    }
    ...
    for ($index = $from; $index < $to; $index++) {
        ...
        if (isset($row['scores_map'])) {	// Scores map is defiend only with enabled debug mode:
 ...
 * @param array params
 */
function search_page_links($params = [])
{
    if (! isset($params['total_pages'])) {	// this param is required
        return;
    }
    ...
    if ($current_page <= intval($page_list_span / 2)) {	// the current page number is small
        $page_list_start = 1;
    } elseif ($current_page > $total_pages - intval($page_list_span / 2)) {	// the current page number is big
    ...
    if ($current_page > $total_pages - intval($page_list_span / 2)) {	//the current page number is big
    ...
    if ($current_page > 1) {	// A link to previous page:
    ...
    if ($page_list_start > 2) {	// Display a link to previous pages range:
    ...
    if (($page_list_end < $total_pages) && ($page_list_end < $total_pages - 1)) {	// Display a link to next pages range:
        $page_no = $page_list_end + floor(($total_pages - $page_list_end) / 2);
    ...
    if ($current_page < $total_pages) {	// A link to next page:
 ...
 */
function display_search_result($params = [])
{
    global $debug, $Blog;
...
function display_score_map($params)
{
    ...
    if (! empty($params['scores_map'])) {	// If score map is provided:
        foreach ($params['scores_map'] as $result_part => $score_map) {
            if (! is_array($score_map)) {
                if ($score_map > 0) {	// Score received for this field
                ...
                continue;
            } elseif (isset($score_map['score_weight']) && $score_map['score_weight'] > 1) {	// add note that the score was multiplied because of the importance of the field where it was found
                $note = sprintf(' - the match scores are multiplied with %d', $score_map['score_weight']);
            } else {	// note is not required
                $note = '';
            }
            ...
            foreach ($score_map['map'] as $match_type => $scores) {
                ...
                if ($keyword_match != $match_type && $keyword_match !== null) {	// close previously started list
                    echo '</ul>';
                }
            ...
            if ($keyword_match != null) {	// display the end of the specific match type
                echo '</ul>';
            }
            echo '</ul>';
        }
