...
if (strpos($action, 'new') !== false || $action == 'copy') { // Simulate tab to value 'new' for actions to create new blog
    $tab = 'new';
}
if (! in_array($action, ['list', 'new', 'new-selskin', 'new-installskin', 'new-name', 'create', 'update_settings_blog', 'update_settings_site', 'new_section', 'edit_section', 'delete_section', 'update_site_skin', 'create_demo_content']) &&
    ! in_array($tab, ['site_settings', 'site_skin'])) {
...
if (strpos($action, 'section') !== false) {	// Initialize Section object:
    ...
    if ($sec_ID > 0) {	// Try to get the existing section by requested ID:
        $SectionCache = &get_SectionCache();
        $edited_Section = &$SectionCache->get_by_ID($sec_ID);
    } else {	// Create new section object:
...
switch ($action) {
    ...
    case 'copy':
        ...
        if ($action == 'copy') {	// Get name of the duplicating collection to display on the form:
            $duplicating_collection_name = $edited_Blog->get('shortname');
        }

        $AdminUI->append_path_level('new', [
            'text' => TB_('New'),
        ]);
        break;
    ...
    case 'new-installskin':
        ...
        $AdminUI->append_path_level('new', [
            'text' => sprintf( /* TRANS: %s can become "Standard blog", "Photoblog", "Group blog" or "Forum" */ TB_('New "%s" collection'), get_collection_kinds($kind)),
        ]);
        break;
    ...
    case 'new-name':
        ...
        $AdminUI->append_path_level('new', [
            'text' => sprintf(TB_('New [%s]'), get_collection_kinds($kind)),
        ]);
        break;
    ...
    case 'create':
        ...
        if ($kind == 'main' && ! check_user_perm('blog_admin', 'editAll', false)) { // Non-collection admins should not be able to create home/main collections
        ...
        $edited_Blog->init_by_kind($kind);
        if (! check_user_perm('blog_admin', 'edit', false, $edited_Blog->ID)) { // validate the urlname, which was already set by init_by_kind() function
        ...
        if ($current_User->check_perm('blog_admin', 'editall', false)) {	// Allow to create demo organization and users only for collection admins:
            param('create_demo_org', 'boolean', false);
            param('create_demo_users', 'boolean', false);
        } else {	// Deny to create demo organization and users for not collection admins:
    ...
    case 'duplicate':
        ...
        if ($edited_Blog->duplicate($duplicate_params)) {	// The collection has been duplicated successfully:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed
            ...
            if ($edited_Blog->dbdelete()) { // Blog was deleted
            ...
            // We have EXITed already at this point!!
        } else { // Check if blog has delete restrictions
            if (! $edited_Blog->check_delete(sprintf(TB_('Cannot delete Blog &laquo;%s&raquo;'), $edited_Blog->get_name()), ['file_root_ID', 'cat_blog_ID'])) { // There are restrictions:
                $action = 'view';
            }
    ...
    case 'update_settings_blog':
        ...
        if (! $Messages->has_errors()) {
            ...
            if ($result != null) { // general cache setting was changed
        ...
        if (param('def_normal_skin_ID', 'integer', null) !== null) { // this can't be NULL
        ...
        if (param('coll_access_type', 'string', null) !== null) {	// Update only if this param has been sent by submitted form:
    ...
    case 'update_settings_site':
        ...
        if (check_user_perm('users', 'edit')) {
            ...
            if ($Settings->get('system_lock') && (! $system_lock) && (! $Messages->has_errors()) && (1 == $Messages->count())) { // System lock was turned off and there was no error, remove the warning about the system lock
                $Messages->clear();
            }
        ...
        if ($old_site_skins_enabled != $Settings->get('site_skins_enabled')) { // If this setting has been changed we should clear all page caches:
    ...
    case 'delete_section':
        ...
        if ($edited_Section->ID == 1) {	// Forbid to delete default section:
    ...
    case 'update_site_skin':
        ...
        if ($skinpage == 'selection') {
            ...
            } elseif (param('mobile_skin_ID', 'integer', null) !== null) {	// Mobile skin ID:
            ...
            } elseif (param('tablet_skin_ID', 'integer', null) !== null) {	// Tablet skin ID:
            ...
            } elseif (param('alt_skin_ID', 'integer', null) !== null) {	// Alt skin ID:
            ...
            if (! empty($updated_skin_ID) && ! skin_check_compatibility($updated_skin_ID, 'site')) {	// Redirect to admin skins page selector if the skin cannot be selected:
            ...
            if ($Settings->dbupdate()) {
                ...
                if ((! $Session->is_mobile_session() && ! $Session->is_tablet_session() && ! $Session->is_alt_session() && param('normal_skin_ID', 'integer', null) !== null) ||
                        ...
                        ($Session->is_alt_session() && param('alt_skin_ID', 'integer', null) !== null)) {	// Redirect to home page if we change the skin for current device type:
                    header_redirect($baseurl);
                } else {	// Redirect to admin skins page if we change the skin for another device type:
                    header_redirect($admin_url . '?ctrl=collections&tab=site_skin&skin_type=' . $updated_skin_type);
                }
            }
        } else {	// Update site skin settings:
            if (! in_array($skin_type, ['normal', 'mobile', 'tablet', 'alt'])) {
            ...
            if (! $edited_Skin) {	// Redirect to don't try update empty skin params:
...
switch ($action) {
    ...
    case 'delete':
        ...
        if ($number_of_items > 0) { // There is at least one item
            $number_of_comments = $edited_Blog->get_number_of_comments();
            if ($number_of_comments > 0) { // There is at least one comment
                $delete_notes[] = [sprintf(TB_('WARNING: This collection contains %d items and %d comments.'), $number_of_items, $number_of_comments), 'warning'];
            } else {
                $delete_notes[] = [sprintf(TB_('WARNING: This collection contains %d items.'), $number_of_items), 'warning'];
            }
        }

        // Check if the deleting blog is used as default blog and Display a warning
        if ($default_Blog = &get_setting_Blog('default_blog_ID') && $default_Blog->ID == $edited_Blog->ID) { // Default blog
            $delete_notes[] = [TB_('WARNING: You are about to delete the default collection.'), 'warning'];
        }
        if ($info_Blog = &get_setting_Blog('info_blog_ID') && $info_Blog->ID == $edited_Blog->ID) { // Info blog
            $delete_notes[] = [TB_('WARNING: You are about to delete the collection used for info pages.'), 'warning'];
        }
        if ($login_Blog = &get_setting_Blog('login_blog_ID') && $login_Blog->ID == $edited_Blog->ID) { // Login blog
            $delete_notes[] = [TB_('WARNING: You are about to delete the collection used for login/registration pages.'), 'warning'];
        }
        if ($msg_Blog = &get_setting_Blog('msg_blog_ID') && $msg_Blog->ID == $edited_Blog->ID) { // Messaging blog
            $delete_notes[] = [TB_('WARNING: You are about to delete the collection used for messaging pages.'), 'warning'];
        }

        $delete_notes[] = [TB_('Note: Some files in this collection\'s fileroot may be linked to users or to other collections posts and comments. Those files will ALSO be deleted, which may be undesirable!'), 'note'];
        $edited_Blog->confirm_delete(
            sprintf(TB_('Delete collection &laquo;%s&raquo;?'), $edited_Blog->get_name()),
            'collection',
            $action,
    ...
    case 'delete_section':
        ...
        if ($action == 'delete_section') {	// We need to ask for confirmation:
    ...
    default:
        ...
        switch ($tab) {
            ...
            default:
                ...
                if ($action == 'create_demo_content' && $collection_count == 0) {	// Create new demo content inside template to display a process in real time:
                ...
                if ($collection_count > 0) {	// Collections list:
