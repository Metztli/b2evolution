...
// INIT params:
if (param('root_and_path', 'filepath', '', false) /* not memorized (default) */ && strpos($root_and_path, '::')) { // root and path together: decode and override (used by "radio-click-dirtree")
    ...
    if (param('new_root', 'string', '')
        && $new_root != $root) { // We have changed root in the select list
        $root = $new_root;
        $path = '';
    }
}

if ($path == './') {	// Fix name of root path to empty string in order to don't store this in DB before each file of root folder:
    $path = '';
}
 ...
 */
if (param('link_type', 'string', null, true, false, false) && param('link_object_ID', 'integer', null, true, false, false)) { // Load Requested LinkOwner object:
    if ($root == 'skins_0') {	// Deny to link skin files to objects:
        debug_die('Skin files are not allowed to link to objects!');
    }
    $LinkOwner = get_LinkOwner($link_type, $link_object_ID);
    if (empty($LinkOwner)) { // We could not find the owner object to link:
...
if (param('user_ID', 'integer', null, true, false, false)) { // Load Requested user:
    if ($root == 'skins_0') {	// Deny to link skin files to objects:
        debug_die('Skin files are not allowed to link to objects!');
    }
    $UserCache = &get_UserCache();
    if (($edited_User = &$UserCache->get_by_ID($user_ID, false)) === false) {	// We could not find the contact to link:
        $Messages->add(sprintf(TB_('Requested &laquo;%s&raquo; object does not exist any longer.'), TB_('User')), 'error');
        unset($edited_User);
        forget_param('user_ID');
        unset($user_ID);
    } else {	// Found User, check perm:
        if ($edited_User->ID != $current_User->ID) {	// if not editing himself, must have user edit permission:
...
if (in_array($action, ['move_copy', 'file_move', 'file_copy'])) {	// Memorize action to select another directory for moving/copying files:
    memorize_param('action', 'string', '', $action);
}

if (! empty($action) && substr($fm_mode, 0, 5) != 'link_' && $fm_mode != 'file_select') {	// The only modes which can tolerate simultaneous actions at this time are link_* modes (item, user...) and file_select
    $fm_mode = '';
}
...
if (! empty($root)) { // We have requested a root folder by string:
    $fm_FileRoot = &$FileRootCache->get_by_ID($root, true);

    if (! $fm_FileRoot || ! isset($available_Roots[$fm_FileRoot->ID]) || ! check_user_perm('files', 'view', false, $fm_FileRoot)) { // Root not found or not in list of available ones
        $Messages->add(TB_('You don\'t have access to the requested root directory.'), 'error');
        $fm_FileRoot = false;
    }
} elseif (! empty($edited_User)) {	// We have a user, check if it already has a linked file in a particular root, in which case we want to use that root!
...
} elseif (! empty($LinkOwner)) {	// We have a post, check if it already has a linked file in a particular root, in which case we want to use that root!
    ...
    // Get list of attached files:
    if ($FileList = $LinkOwner->get_attachment_FileList(1)) {	// Get first file:
        /**
         * @var File
         */
        $File = &$FileList->get_next();
        if (! empty($File) && check_user_perm('files', 'view', false, $File->get_FileRoot())) {	// Obtain and use file root of first file:
...
if (empty($fm_FileRoot) && ! empty($edited_User)) {	// Still not set a root, try to get it for the edited User
    $fm_FileRoot = &$FileRootCache->get_by_type_and_ID('user', $edited_User->ID);
    if (! $fm_FileRoot || ! isset($available_Roots[$fm_FileRoot->ID])) { // Root not found or not in list of available ones
        $fm_FileRoot = false;
    }
}

if (empty($fm_FileRoot) && ! empty($Blog)) {	// Still not set a root, try to get it for the current Blog
...
if (! $fm_FileRoot) { // No root requested (or the requested is invalid),
...
if ($fm_FileRoot) { // We have access to a file root:
    if (empty($fm_FileRoot->ads_path)) {	// Not sure it's possible to get this far, but just in case...
        ...
        if (! preg_match('#^' . preg_quote($fm_FileRoot->ads_path, '#') . '#', $ads_list_path)) { // cwd is OUTSIDE OF root!
        ...
        if ($ajax_request &&
            ...
            check_user_perm('files', 'add', false, $fm_FileRoot)) {	// Try to create the requested directory automatically if current User
            // has a permission and when this is a request e.g. for an import folder:
            mkdir_r($ads_list_path);
        }

        if (! is_dir($ads_list_path)) {	// This may happens when a requested folder e.g. /media/import/users/ doesn't exist:
            $Messages->add(sprintf(TB_('The directory &laquo;%s&raquo; does not exist.'), $path), 'error');
            $path = '';		// fp> added
            $ads_list_path = null;
        } elseif ($ads_list_path != $non_canonical_list_path) {	// We have reduced the absolute path, we should also reduce the relative $path (used in urls params)
            $path = get_canonical_path($path);
        }

        if (($Messages->count() == 0) && (strlen($ads_list_path) > $dirpath_max_length) && check_user_perm('options', 'edit')) { // This folder absolute path exceed the max allowed length, a warning message must be displayed, if there were no other message yet. ( If there are other messages then this one should have been already added )
...
if (! $ajax_request) { // Don't display tabs on AJAX request
    file_controller_build_tabs();
}


if (empty($ads_list_path)) { // We have no Root / list path, there was an error. Unset any action or mode.
    $action = 'nil';
    $fm_mode = null;
 ...
 * @global array
 */
$fm_selected = param('fm_selected', 'array:filepath', [], true);
...
if (param('link_ID', 'integer', null, false, false, false)) {
    ...
    if (($edited_Link = &$LinkCache->get_by_ID($link_ID, false)) === false) {	// We could not find the link to edit:
...
switch ($action) {
    ...
    case 'createnew_dir':
        ...
        if (! $Settings->get('fm_enable_create_dir')) { // Directory creation is gloablly disabled:
            $Messages->add(TB_('Directory creation is disabled.'), 'error');
            break;
        }

        if (! param('create_name', 'string', '')) { // No name was supplied:
            $Messages->add(TB_('Cannot create a directory without name.'), 'error');
            break;
        }
        if ($error_dirname = validate_dirname($create_name)) { // Not valid dirname
    ...
    case 'createnew_file':
        ...
        if (! $Settings->get('fm_enable_create_file')) { // File creation is gloablly disabled:
        ...
        if ($error_filename = validate_filename($create_name, check_user_perm('files', 'all'))) { // Not valid filename or extension
...
if (! in_array($fm_order, ['name', 'path', 'type', 'size', 'lastmod', 'perms', 'fsowner', 'fsgroup'])) {
    $fm_order = null;
}
...
switch ($action) {
    ...
    case 'create_zip':
        ...
        if ($action == 'create_zip') {	// Check permission for action to create new ZIP archive:
            check_user_perm('files', 'edit_allowed', true, $selected_Filelist->get_FileRoot());
        }
        ...
        if ($action_invoked) {	// Action was invoked, check felds:
            // ZIP file name should not be empty:
            param_check_not_empty('zipname', TB_('Please provide the name of the archive.'));
            if (! empty($zipname)) { // Check extenssion of the entered file name:
                param_check_regexp('zipname', '#\.zip$#i', TB_('File name must end with .zip'));
            }
        }

        if (empty($zipname)) {
            if ($selected_Filelist->count() == 1) {	// Auto suggest ZIP file name as name of the single file:
        ...
        if (param_errors_detected()) {	// Stop download if errors detected:
            break;
        }

        if (! pack_archive($fm_Filelist->get_ads_list_path() . $zipname, $fm_Filelist->get_ads_list_path(), $selected_Filelist->get_array('get_name'), '', ($exclude_sd ? 'subdirs' : []), 'msg_error')) {	// Stop on error packing:
            break;
        }

        if ($action == 'download') {	// Download ZIP archive:
            ...
            if ($delete_files) {	// We should delete the files after creating ZIP archive:
                ...
                while ($l_File = &$selected_Filelist->get_next()) {
                    ...
                    $restriction_Messages = $l_File->check_relations('delete_restrictions', [], true);

                    if ($restriction_Messages->count()) { // There are restrictions:
    ...
    case 'copy':
        ...
        if (! check_user_perm('files', 'edit_allowed', false, $selected_Filelist->get_FileRoot())) { // We do not have permission to edit files
        ...
        if ($sources_Root) { // Instantiate the source list for the selected sources
        ...
        if (! $selected_Filelist->count() && (! $source_Filelist || ! $source_Filelist->count())) { // There is nothing to rename
            $Messages->add(TB_('Nothing selected.'), 'error');
            $action = 'list';
            break;
        }
        ...
        param('new_names', 'array:filepath', []);

        // Check params for each file to rename:
        $index = 0;
        while ($loop_src_File = &$source_Filelist->get_next()) {
            if (! $loop_src_File->can_be_manipulated()) {
                param_error('new_names[' . $loop_src_File->get_md5_ID() . ']', $loop_src_File->get_name() . ' - ' . sprintf(
                    TB_('Admins can upload/rename/edit this file type only if %s in the <a %s>configuration files</a>'),
            ...
            if (! isset($new_names[$loop_src_File->get_md5_ID()])) { // We have not yet provided a name to rename to...
                $confirmed = 0;
        ...
        if ($confirmed) { // Rename is confirmed, let's proceed:
            // Check that this action request is not a CSRF hacked request:
            $Session->assert_received_crumb('file');
            ...
            while ($loop_src_File = &$source_Filelist->get_next()) {
                ...
                switch ($action) {
                    case 'rename':
                        ...
                        if (! $loop_src_File->can_be_manipulated()) {
                            $Messages->add_to_group($old_name . ' - ' . sprintf(
                                TB_('Admins can upload/rename/edit this file type only if %s in the <a %s>configuration files</a>'),
                                '<code>$admins_can_manipulate_sensitive_files = true</code>',
                                'href="' . get_manual_url('advanced-php') . '"'
                            ), 'error', TB_('Renaming files:'));
                            continue 2;
                        }

                        if ($new_name == $old_name) { // Name has not changed...
                        ...
                        if (! $loop_src_File->rename_to($new_name)) {
                            $Messages->add_to_group(sprintf(
                                TB_('&laquo;%s&raquo; could not be renamed to &laquo;%s&raquo;'),
                                $old_name,
                                $new_name
                            ), 'error', TB_('Renaming files:'));
                            continue 2;
                    ...
                    case 'copy':
                        ...
                        if (! $loop_src_File->can_be_manipulated()) {
                            $Messages->add_to_group($old_path . ' - ' . sprintf(
                                TB_('Admins can upload/rename/edit this file type only if %s in the <a %s>configuration files</a>'),
                                '<code>$admins_can_manipulate_sensitive_files = true</code>',
                                'href="' . get_manual_url('advanced-php') . '"'
                            ), 'error', TB_('Copying files:'));
                            continue 2;
                        }

                        if ($old_path == $new_path && $loop_src_File->_FileRoot->ID == $selected_Filelist->_FileRoot->ID) { // File path has not changed...
                        ...
                        // Perform copy:
                        if (! $loop_src_File->copy_to($dest_File)) { // failed
                    ...
                    case 'move':
                        ...
                        // Perform move:
                        if (! $loop_src_File->move_to($selected_Filelist->get_root_type(), $selected_Filelist->get_root_ID(), $new_path)) { // failed
                ...
                if ($fm_Filelist->contains($loop_src_File) === false) { // File not in filelist (expected if not same dir)
    ...
    case 'resize':
        ...
        if (! $selected_Filelist->count()) { // There is nothing to resize
            $Messages->add(TB_('Nothing selected.'), 'error');
            $action = 'list';
            break;
        }
        ...
        if ($confirmed) { // Resize is confirmed, let's proceed:
            // Check that this action request is not a CSRF hacked request:
            $Session->assert_received_crumb('file');
            $selected_Filelist->restart();
            $FileCache = &get_FileCache();
    ...
    case 'delete':
        ...
        $selected_Filelist->restart();
        if ($confirmed) { // Delete files, It is possible only file has no links:
            $selected_Filelist->load_meta();
            while ($l_File = &$selected_Filelist->get_next()) {
                // Check if there are delete restrictions on this file:
                $restriction_Messages = $l_File->check_relations('delete_restrictions', [], true);
                ...
                if (! $l_File->can_be_manipulated()) {	// File can not be manipulated:
                    $Messages->add_to_group($l_File->get_prefixed_name() . ': ' . sprintf(
                        TB_('Admins can upload/rename/edit this file type only if %s in the <a %s>configuration files</a>'),
                        '<code>$admins_can_manipulate_sensitive_files = true</code>',
                        'href="' . get_manual_url('advanced-php') . '"'
                    ), 'error', TB_('Deleting files...'));
                    // Skip this file
                    continue;
                }
            ...
            if (! $selected_Filelist->count()) { // no files left in list, cancel action
                $action = 'list';
                // Redirect so that a reload doesn't write to the DB twice:
                header_redirect(regenerate_url('', '', '', '&'), 303); // Will EXIT
                // We have EXITed already at this point!!
    ...
    case 'update_properties':
        ...
        if (! $edited_File->can_be_manipulated()) {
            ...
            $Messages->add(sprintf(
                $edited_File->get_name() . ' - ' . TB_('Admins can upload/rename/edit this file type only if %s in the <a %s>configuration files</a>'),
                '<code>$admins_can_manipulate_sensitive_files = true</code>',
                'href="' . get_manual_url('advanced-php') . '"'
            ), 'error');
            break;
            ...
            if ($resize_image) {
                ...
                $new_dimensions = fit_into_constraint(
                    $current_dimensions['width'],
                    $current_dimensions['height'],
                    $Settings->get('fm_resize_width'),
                    $Settings->get('fm_resize_height')
                );
            ...
            if ($new_name != $old_name) { // Name has changed...
                $allow_locked_filetypes = check_user_perm('files', 'all');
                if ($check_error = check_rename($new_name, $edited_File->is_dir(), $edited_File->get_dir(), $allow_locked_filetypes)) {
                    ...
                    if ($edited_File->rename_to($new_name)) {
                        $Messages->add(sprintf(
                            TB_('&laquo;%s&raquo; has been successfully renamed to &laquo;%s&raquo;'),
                            $old_name,
                            $new_name
                        ), 'success');
        ...
        if ($mode == 'link') {	// Don't redirect when we updated a linked File:
            exit;
        }

        if ($LinkOwner = &get_LinkOwner($link_owner_type, $link_owner_ID)) {	// Redirect back to link owner object edit form where the File was edited:
    ...
    case 'duplicate_user':
        ...
        // Link File to User:
        if (! isset($edited_User)) {	// No User to link to
        ...
        if ($action == 'duplicate_user') {	// Copy file to the folder "profile_pictures" before linking:
            ...
            if ($old_path == $new_path && $edited_File->_FileRoot->ID == $selected_Filelist->_FileRoot->ID) {	// File path has not changed:
                ...
                // Perform copy:
                if (! $edited_File->copy_to($dest_File)) {	// Failed copying:
        ...
        if ($image_sizes[0] < $min_size || $image_sizes[1] < $min_size) {	// Don't use this file as profile picture because it has small sizes
    ...
    case 'link_inpost':	// In the context of a post
        ...
        if (! isset($LinkOwner)) {	// No Owner to link to - end link_object mode.
            $fm_mode = null;
            break;
        }
        ...
        while ($edited_File = &$selected_Filelist->get_next()) {	// Let's make the link!
            $edited_File->link_to_Object($LinkOwner);

            // Reset LinkCache to autoincrement link_order
            if ($files_count > 1) {
                unset($GLOBALS['LinkCache']);
            }
        }
    ...
    case 'edit_perms':
        ...
        param('perms', 'array:integer', []);
        param('edit_perms_default', 'string'); // default value when multiple files are selected
        param('use_default_perms', 'array:string', []); // array of file IDs that should be set to default
        ...
        $field_options_read_readonly = [
            ...
            [
                'value' => 666,
                'label' => TB_('Read and write'),
            ]];
        $more_than_one_selected_file = ($selected_Filelist->count() > 1);

        if (count($perms) || count($use_default_perms)) { // New permissions given, change them
            $selected_Filelist->restart();
            while ($l_File = &$selected_Filelist->get_next()) {
                if (in_array($l_File->get_md5_ID(), $use_default_perms)) { // use default
                    $chmod = $edit_perms_default;
                } elseif (! isset($perms[$l_File->get_md5_ID()])) { // happens for an empty text input or when no radio option is selected
                    ...
                    $Messages->add($l_File->get_name() . ' - ' . sprintf(
                        TB_('Admins can upload/rename/edit this file type only if %s in the <a %s>configuration files</a>'),
                        '<code>$admins_can_manipulate_sensitive_files = true</code>',
                        'href="' . get_manual_url('advanced-php') . '"'
                    ), 'error');
                    continue;
                } else { // provided for this file
        ...
        if (! $selected_Filelist->count()) { // No file left selected... (everything worked fine)
            $action = 'list';
        }
        break;
}
...
switch ($fm_mode) {
    case 'link_object':
        ...
        if (! isset($LinkOwner)) { // No Object to link to...
            $fm_mode = null;
            break;
        }
...
if (! isset($Blog) || $fm_FileRoot->type != 'collection' || $fm_FileRoot->in_type_ID != $Blog->ID) {	// Display only if we're not browsing our home blog
    $AdminUI->breadcrumbpath_add(
        $fm_FileRoot->name,
        '?ctrl=files&amp;blog=$blog$&amp;root=' . $fm_FileRoot->ID,
        (isset($Blog) && $fm_FileRoot->type == 'collection') ? sprintf(
            TB_('You are ready to post files from %s into %s...'),
            $fm_FileRoot->name,
            $Blog->get('shortname')
        ) : ''
    );
...
if ($mode != 'modal') {
    ...
    if ($mode == 'upload' || $mode == 'import') { // Add css to remove spaces around window
        require_css('fileadd.css', 'rsc_url');
    }

    if ($mode == 'popup') { // Don't display navigation on popup mode
...
if (! empty($action) && $action != 'list' && $action != 'nil') {
    ...
    switch ($action) {
        ...
        case 'edit_properties':
            ...
            if ($mode == 'modal') { // Unmemorize the mode param in order to submit form in mormal mode
                forget_param('mode');
            }
...
if ($mode != 'modal') {
    ...
    // Browsing interface:
    // -------------------
    $show_existing_attachments_tab = ($mode == 'upload') && isset($LinkOwner) && in_array(get_class($LinkOwner->link_Object), ['Comment', 'Item', 'EmailCampaign']);
    if ($show_existing_attachments_tab) {	// Display existing attachments tab:
