...
function &get_FileCache()
{
    ...
    if (! isset($FileCache)) {	// Cache doesn't exist yet:
...
function &get_FileRootCache()
{
    ...
    if (! isset($FileRootCache)) {	// Cache doesn't exist yet:
...
function &get_FiletypeCache()
{
    ...
    if (! isset($FiletypeCache)) {	// Cache doesn't exist yet:
...
class files_Module extends Module
{
    ...
    public function get_available_group_permissions()
    {
        ...
        if (check_user_perm('options', 'edit')) { // Set the links to icons if current user has a permission to edit the file types:
        ...
        $permissions = [
            'perm_files' => [
                ...
                'options' => [
                    ...
                    ['none', T_('No Access')],
                    ['view', T_('View files for all allowed roots'), T_('You can allow specific collection roots in each collection\'s advanced user/group permissions.')],
                    ['add', T_('Add/Upload files to allowed roots'), sprintf(T_('Only %sunlocked file types can be uploaded.'), $filetypes_allowed_icon)],
                    ['edit_allowed', T_('Edit files in allowed roots'), sprintf(T_('Only %sunlocked file types can be edited.'), $filetypes_allowed_icon)],
                    ['edit', sprintf(T_('Edit %sunlocked files in all roots'), $filetypes_allowed_icon)],
                    ['all', sprintf(T_('Edit / Unzip all files, including %slocked ones, in all roots'), $filetypes_not_allowed_icon), T_('Needed for editing PHP files in skins.')],
                ],
                'perm_type' => 'radiobox',
                'field_lines' => true,
            ],
     ...
     */
    public function check_files_user_perm($permlevel, $permvalue, $permtarget)
    {
        return true;
    }
     ...
     * @return boolean True on success (permission is granted), false if permission is not granted
     */
    public function check_files_group_perm($permlevel, $permvalue, $permtarget)
    {
        $perm = false;
        switch ($permvalue) {
            case 'all':
                global $demo_mode;
                if ((! $demo_mode) && ($permlevel == 'all')) { // All permissions granted
                    $perm = true;
                    break;
                }
        ...
        if ($perm && isset($permtarget) && ($permtarget instanceof FileRoot)) {
            ...
            switch ($permtarget->type) {
                case 'shared':
                    if ($permlevel == 'edit_allowed') {	// We have no perm level 'edit_allowed' for this root type, Use 'edit' instead:
                        $permlevel = 'edit';
                    }
                ...
                case 'user':
                    if (check_user_perm('users', 'moderate') && check_user_perm('files', 'all')) { // Current user can edits all files of other users
                        return true;
                    } else { // Allow user to see only own file root
                        return $permtarget->in_type_ID == $current_User->ID;
                    }
                    // no break
                case 'collection':
                    if ($permvalue != 'edit' && $permvalue != 'all') {	// Check specific blog perms:
                        if ($current_User->check_perm_blogowner($permtarget->in_type_ID)) {	// Collection owner has all permissions for files root:
                            $perm = true;
                            return $perm;
                        }
                        if (check_user_perm('blogs', $permlevel)) {	// If current user has access to view or edit all collections:
                            $perm = true;
                            return $perm;
                        }
                        $perm = $current_User->check_perm_blogusers('files', $permlevel, $permtarget->in_type_ID);
                        if (! $perm) {	// Check primary and secondary groups for permissions for this specific collection:
     ...
     * @return boolean True on success (permission is granted), false if permission is not granted
     */
    public function check_sharedroot_group_perm($permlevel, $permvalue, $permtarget)
    {
        $perm = false;
        switch ($permvalue) {
            case 'edit':
                if ($permlevel == 'edit') { // User can ask for normal edit perm...
                    $perm = true;
                    break;
                }
     ...
     * @return boolean True on success (permission is granted), false if permission is not granted
     */
    public function check_importroot_group_perm($permlevel, $permvalue, $permtarget)
    {
        $perm = false;
        switch ($permvalue) {
            case 'edit':
     ...
     * @return boolean True on success (permission is granted), false if permission is not granted
     */
    public function check_skinsroot_group_perm($permlevel, $permvalue, $permtarget)
    {
        $perm = false;
        switch ($permvalue) {
            case 'edit':
     ...
     * @return boolean True on success (permission is granted), false if permission is not granted
     */
    public function check_pluginsroot_group_perm($permlevel, $permvalue, $permtarget)
    {
        $perm = false;
        switch ($permvalue) {
            case 'edit':
    ...
    public function build_evobar_menu()
    {
        ...
        if (check_user_perm('admin', 'standard')) {
            if (! empty($Blog) && check_user_perm('files', 'view', false, $Blog->ID)) {	// Manage blog files:
        ...
        if (check_user_perm('admin', 'restricted')) {
            if (check_user_perm('files', 'view', false, null)) {	// Manage files generally:
                // FM enabled and permission to view files:
                $entries['files'] = [
                    'text' => T_('Files') . '&hellip;',
                    'href' => $admin_url . '?ctrl=files',
    ...
    public function build_menu_1()
    {
        ...
        if (check_user_perm('files', 'view', false, $blog ? $blog : null)) {	// FM enabled and permission to view files:
