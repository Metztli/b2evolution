...
class FileRoot
{
     ...
     */
    public function __construct($root_type, $root_in_type_ID, $create = true)
    {
        global $Messages;
        ...
        switch ($root_type) {
            case 'user':
                ...
                if (! $User = &$UserCache->get_by_ID($root_in_type_ID, false, false)) {	// User not found
                    return false;
                }
            ...
            case 'collection':
                ...
                if (! ($fileroot_Blog = &$BlogCache->get_by_ID($root_in_type_ID, false, false))) {	// Collection not found
                    return false;
                }
            ...
            case 'shared':
                ...
                if (! $Settings->get('fm_enable_roots_shared')) { // Shared dir is disabled:
                ...
                elseif (! mkdir_r($ads_shared_dir)) {
                    ...
                    if (isset($Blog) && ! is_admin_page()) {	// (for now) Let's make shared files appear as being part of the currently displayed collection:
                        $this->ads_url = $Blog->get_local_media_url() . $rds_shared_subdir;
                    } else {	// If back-office or current collection is not defined:
            ...
            case 'skins':
                ...
                if (! $Settings->get('fm_enable_roots_skins')) { // Skins root is disabled:
                    ...
                    if (isset($Blog) && ! is_admin_page()) {	// (for now) Let's make skin files appear as being part of the currently displayed collection:
            ...
            case 'import':
                ...
                if (! mkdir_r($ads_import_dir)) {
                    if (is_admin_page()) { // Only display error on an admin page:
            ...
            case 'emailcampaign':
                ...
                if (! mkdir_r($ads_emailcampaign_dir)) {
                    if (is_admin_page()) {	// Only display error on back-office side:
            ...
            case 'plugins':
                // Plugins dir
                if (! $Settings->get('fm_enable_roots_plugins')) {	// Plugins root is disabled:
    ...
    public function get_typegroupname()
    {
        switch ($this->type) {
            case 'user':
     ...
     * @return string
     */
    public static function gen_ID($root_type, $root_in_type_ID)
    {
     ...
     * @return boolean
     */
    public function contains($rel_path)
    {
        ...
        if (! empty($real_abs_path) && strpos($real_abs_path, $this->ads_path) !== 0) {	// Deny access from another file root:
