...
class FileCache extends DataObjectCache
{
    ...
    public function &instantiate(&$db_row)
    {
        ...
        if (! empty($obj_ID)) { // If the object ID is valid:
            if (! isset($this->cache[$obj_ID])) { // If not already cached:
                // Instantiate a File object for this line:
                $current_File = new File($db_row->file_root_type, $db_row->file_root_ID, $db_row->file_path); // COPY!
                if (! $current_File->_FileRoot) { // File root is not initialized for this file, probably it is disabled by settings.
                    // We cannot work with such file object. Use NULL instead.
                    $current_File = null;
                } else { // Flow meta data into File object:
                    $current_File->load_meta(false, $db_row);
                }
                $this->add($current_File);
            } else { // Already cached:
                $current_File = &$this->cache[$obj_ID];
                if ($current_File) { // Flow meta data into File object:
                    $current_File->load_meta(false, $db_row);
                }
            }
        }
     ...
     */
    public function &get_by_root_and_path($root_type, $root_in_type_ID, $rel_path, $load_meta = false)
    {
        global $Debuglog, $cache_File;

        if (is_windows()) {	// We probably don't need the windows backslashes replacing any more but leave it for safety because it doesn't hurt:
