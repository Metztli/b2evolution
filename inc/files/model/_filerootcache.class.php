...
class FileRootCache
{
     ...
     */
    public static function get_available_FileRoots($special_root_ID = null, $coll_filter = null)
    {
        global $current_User;
        ...
        if (! empty($special_root_ID) &&
            ...
            check_user_perm('files', 'view', false, $special_FileRoot)) { // Try to add special file root if current user has an access
        ...
        if ($user_FileRoot) { // We got a user media dir:
            $r[$user_FileRoot->ID] = &$user_FileRoot;
        }

        if (isset($collections_Module)) {	// Blog/collection media dirs:
        ...
        if ($shared_FileRoot) { // We got a shared dir:
            $r[$shared_FileRoot->ID] = &$shared_FileRoot;
        }

        if (isset($collections_Module) && ! get_param('link_type') && ! get_param('link_object_ID') && ! get_param('user_ID')) { // Skins root, Don't allow files of this root to link to objects:
            $skins_FileRoot = &$FileRootCache->get_by_type_and_ID('skins', 0, false);
            if ($skins_FileRoot) { // We got a skins dir:
            ...
            if ($plugins_FileRoot) {	// We got a plugins dir:
        ...
        if ($import_FileRoot) { // We got an import dir:
        ...
        if (($emailcampaign_FileRoot = &$FileRootCache->get_by_type_and_ID('emailcampaign', $emailcampaign_ID, true)) &&
                ...
                check_user_perm('emails', 'view')) { // Try to add Email campaign file root if current user has an access:
     ...
     */
    public function &get_by_ID($id, $create = false)
    {
        $part = explode('_', $id);
        $root_type = $part[0];
     ...
     * @return FileRoot|false FileRoot on success, false on failure (ads_path is false).
     */
    public function &get_by_type_and_ID($root_type, $root_in_type_ID, $create = false)
    {
        $root_ID = FileRoot::gen_ID($root_type, $root_in_type_ID);

        if (! isset($this->cache[$root_ID])) {	// Not in Cache, let's instantiate:
     ...
     */
    public function get_root_dir($root_type, $root_in_type_ID, $create = false)
    {
        if ($tmp_FileRoot = &$this->get_by_type_and_ID($root_type, $root_in_type_ID, $create)) { // Return  path only when file root is detected
            return $tmp_FileRoot->ads_path;
        } else { // Impossible to get path by root type and ID
            return false;
        }
    }
