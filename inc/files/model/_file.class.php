...
class File extends DataObject
{
     ...
     */
    public function __construct($root_type, $root_ID, $rdfp_rel_path, $load_meta = false)
    {
        global $Debuglog;
        ...
        if ($this->_FileRoot) {
            ...
            if ($load_meta) { // Try to load DB meta info:
                $this->load_meta();
            }
        }
    }
    ...
    public static function get_delete_restrictions()
    {
        return [
            ...
            [
                ...
                'fk' => 'link_file_ID',
                'field' => 'link_usr_ID',
                'msg' => T_('%d linked users (profile pictures)'),
                'class' => 'Link',
                'class_path' => 'links/model/_link.class.php',
            ],
        ];
     ...
     */
    public function load_meta($force_creation = false, $row = null)
    {
        global $DB, $Debuglog;

        if ($this->meta == 'unknown') { // We haven't tried loading yet:
            if (is_null($row)) {	// No DB data has been provided:
                $row = $DB->get_row(
                    "
					SELECT * FROM T_files
            ...
            if ($row &&
                ($row->file_path == $this->_rdfp_rel_path ||
                  $row->file_path == '/' . $this->_rdfp_rel_path)) { // We found meta data
                if ($row->file_path == '/' . $this->_rdfp_rel_path) {	// Fix wrong path started with "/":
                ...
                if ($force_creation) {	// No meta data, we have to create it now!
                    $this->dbinsert();
                }
            }
        }
     ...
     */
    public function create($type = 'file', $chmod = null)
    {
        if ($type == 'dir') { // Create an empty directory:
        ...
        if ($success) {	// The file/folder has been successfully created:
            // Initializes file properties (type, size, perms...)
            $this->load_properties();
            ...
            // Let's recycle it! :
            if (! $this->load_meta()) { // No meta data could be loaded, let's make sure localization info gets recorded:
    ...
    public function load_properties()
    {
        ...
        if (is_dir($this->_adfp_full_path)) {	// The File is a directory:
            $this->_is_dir = true;
     ...
     */
    public function dir_or_file($dir_string = null, $file_string = null)
    {
    ...
    {
        if (empty($this->type)) { // Detect and set file type
            $this->set_file_type();
        }
     ...
     */
    public function is_editable($allow_locked = null)
    {
     ...
     * @return bool
     */
    public function can_be_manipulated()
    {
        if ($this->is_dir()) { // directories don't have filetypes to restrict them:
            return true;
        }
    ...
    public function &get_Filetype()
    {
        if (! isset($this->Filetype)) {
            ...
            if ($ext = $this->get_ext()) { // The file has an extension, load filetype object
                $FiletypeCache = &get_FiletypeCache();
                $this->Filetype = &$FiletypeCache->get_by_extension(strtolower($ext), false);
            }

            if (! $this->Filetype) { // remember as being retrieved.
                $this->Filetype = false;
            }
        }
     ...
     * @return string
     */
    public function get_file_link($params = [])
    {
        $params = array_merge([
            'before' => '',
            'after' => '',
     ...
     * @return string
     */
    public function get_description($params = [])
    {
        $params = array_merge([
            'before' => '',
            'after' => '',
     ...
     * @return string
     */
    public function get_prefixed_name()
    {
        if ($this->is_dir()) {
     ...
     */
    public function get_dir_rel_path()
    {
     ...
     */
    public function get_rdfp_rel_path()
    {
     ...
     * @return string full path
     */
    public function get_rdfs_rel_path()
    {
    ...
    public function get_url()
    {
        ...
        if ($this->is_dir()) { // Directory
            if ($public_access_to_media) { // Public access: full path
            ...
            } else { // Private Access: doesn't show the full path
     ...
     */
    public function get_root_and_rel_path()
    {
    ...
    {
        if (isset($this->_type)) { // The type is already cached for this object:
            return $this->_type;
        }
     ...
     */
    public function load_lastmod_ts()
    {
        if (! isset($this->_lastmod_ts)) {	// Get timestamp from disk file:
            $this->_lastmod_ts = @filemtime($this->_adfp_full_path);
            if ($this->_lastmod_ts === false) {	// Log failed result:
        ...
        if ($this->ID && // File is stored in DB before
            ...
            $this->_lastmod_ts != strtotime($this->ts)) {	// We must update timestamp in DB if it is defferent than last modification date of this File on DISK:
            $this->set('ts', date2mysql($this->_lastmod_ts));
            if ($this->is_image() &&
                ($image_dimensions = imgsize($this->_adfp_full_path, 'widthheight'))) {	// Also update dimensions for image file if they can be extracted from disk file successfully:
     ...
     */
    public function get_lastmod_ts()
    {
        // Load timestamp of last modification on DISK:
     ...
     */
    public function get_lastmod_formatted($format = '#')
    {
        global $localtimenow;
        ...
        switch ($format) {
            ...
            case 'compact':
                ...
                if ($age < 3600) {	// Less than 1 hour: return full time
                    return date_i18n('H:i:s', $lastmod_ts);
                }
                if ($age < 86400) {	// Less than 24 hours: return compact time
                    return date_i18n('H:i', $lastmod_ts);
                }
                if ($age < 31536000) {	// Less than 365 days: Month and day
     ...
     */
    public function get_perms($type = null)
    {
     ...
     * @return null|string
     */
    public function get_fsgroup_name()
    {
     ...
     * @return null|string
     */
    public function get_fsowner_name()
    {
    ...
    public function get_icon($params = [])
    {
        ...
        if ($this->is_dir()) { // Directory icon:
            $icon = 'folder';
        } else {
            $Filetype = &$this->get_Filetype();
            if (isset($Filetype->icon) && $Filetype->icon) { // Return icon for known type of the file
                return $Filetype->get_icon($params);
            } else { // Icon for unknown file type:
                $icon = 'file_unknown';
            }
        }
     ...
     */
    public function get_image_size($param = 'widthxheight', $check_ts = false)
    {
        if ($check_ts) {	// If DISK timestamp is defferent than DB timestamp then width and height will be updated to new values:
            $this->load_lastmod_ts();
        }
        ...
        if (($this->get('width') === null || $this->get('height') === null) &&
            ...
            $this->load_meta() && // Meta data was loaded successfully
            ($image_dimensions = imgsize($this->_adfp_full_path))) {	// Set dimensions for image file if it was not stored before:
            $this->set('width', $image_dimensions[0]);
            $this->set('height', $image_dimensions[1]);
            $this->dbupdate(false);
        }

        if ($this->get('width') !== null && $this->get('height') !== null) {	// Get image dimensions from DB(cached):
            switch ($param) {
                case 'width':
    ...
    public function get_tag(
        ...
        if ($this->is_dir()) { // We can't reference a directory
            return '';
        }

        $this->load_meta();

        if ($this->is_image()) { // Make an IMG link:
            $r = $before_image;

            $x_sizes = [1]; // Standard ratio size = 1x
            $image_size_x = intval($image_size_x);
            if ($image_size_x > 1) { // Additional ratio size
            ...
            foreach ($x_sizes as $x_size) {
                ...
                if ($this->check_image_sizes($size_name, 64, $img_attribs) && $add_loadimg) { // If image larger than 64x64 add class to display animated gif during loading
                    $image_class = trim($image_class . ' loadimg');
                }

                $image_class_attr = $image_class;
                if ($x_size > 1) { // Add class to detect what image is resized for speacial ratio
                    $image_class_attr = trim($image_class_attr . ' image-' . $x_size . 'x');
                }

                if ($image_class_attr != '') { // Image class
                    $img_attribs['class'] = $image_class_attr;
                }

                if ($image_align != '') { // Image align
                    $img_attribs['align'] = $image_align;
                }

                if ($image_alt == '-') {	// Don't display any alt text:
                    if (isset($img_attribs['alt'])) {
                        unset($img_attribs['alt']);
                    }
                } elseif ($image_alt != '') {	// Overrride original image alt store in DB per this File:
                    $img_attribs['alt'] = $image_alt;
                }

                if ($image_style != '') { // Image style
            ...
            if ($this->exists()) {	// file exists, we can safely link to this file:
                if ($image_link_to == 'original') { // special case
                ...
                if (! empty($image_link_to)) {
                    ...
                    if (! empty($image_link_id)) { // Set attribute "id" for link
                        $a .= ' id="' . $image_link_id . '"';
                    }
                    $img = $a . '>' . $img . '</a>';
                }
    ...
    public function get_gallery($params)
    {
        ...
        if (! $this->is_dir()) {	// Not a directory
            return '';
        }
        if (! $FileList = $this->get_gallery_images($params['gallery_image_limit'], $params['gallery_order'])) {	// No images in this directory
            return '';
        }
        ...
        foreach ($FileList as $l_File) {
            ...
            if ($params['gallery_link_rel'] == '#') {	// Make one "gallery" per directory:
            ...
            if ($count % $params['gallery_colls'] == 0) {
                $r .= $params['gallery_row_start'];
            }
            $count++;
            ...
            if ($count % $params['gallery_colls'] == 0) {
                $r .= $params['gallery_row_end'];
            }
        }
        if ($count && ($count % $params['gallery_colls'] != 0)) {
            $r .= $params['gallery_row_end'];
        }
     ...
     */
    public function get_gallery_images($limit = 1000, $order = '')
    {
        if ($filenames = $this->get_directory_files('relative')) {
            ...
            foreach ($filenames as $filename) {
                if ($i > $limit) {	// We've got enough images
                    break;
                }
                ...
                */
                ...
                if (! $l_File->is_image()) {	// Not an image
                    continue;
                }
                $Files[] = $l_File;
     ...
     */
    public function get_directory_files($path_type = 'relative')
    {
        global $Settings;

        $path = trailing_slash($this->_adfp_full_path);

        if ($dir = @opendir($path)) {	// Scan directory and list all files
            $filenames = [];
            while (($file = readdir($dir)) !== false) {
                if ($file == '.' || $file == '..' || $file == $Settings->get('evocache_foldername')) {	// Invalid file
                    continue;
                }
     ...
     */
    public function get_recursive_size()
    {
     ...
     */
    public function modify_path($rel_dir, $full_dir)
    {
        if ($this->is_dir()) {
        ...
        if ($this->meta == 'loaded') {	// We have meta data, we need to deal with it:
            // unchanged : $this->set( 'root_type', $this->_FileRoot->type );
            // unchanged : $this->set( 'root_ID', $this->_FileRoot->in_type_ID );
            $this->set('path', $this->_rdfp_rel_path);
            // Record to DB:
            $this->dbupdate();
        } else {	// There might be some old meta data to *recycle* in the DB...
            $this->load_meta();
        }
    }
     ...
     */
    public function rename_to($newname)
    {
        if (! $this->can_be_manipulated()) {	// check if we can manipulate the file first:
            return false;
        }
        ...
        if ($this->is_dir()) { // modify folder content file paths in db
        ...
        if (! @rename($this->_adfp_full_path, $this->_dir . $newname)) { // Rename will fail if $newname already exists (at least on windows)
            syslog_insert(sprintf('File %s could not be renamed to %s', '[[' . $old_file_name . ']]', '[[' . $newname . ']]'), 'info', 'file', $this->ID);
            $DB->rollback();
            return false;
        }
        ...
        if ($this->meta == 'loaded') {	// We have meta data, we need to deal with it:
            ...
            $this->set('path', $this->_rdfp_rel_path);
            // Record to DB:
            if (! $this->dbupdate()) {	// Update failed, try to rollback the rename on disk:
                if (! @rename($this->_adfp_full_path, $this->_dir . $oldname)) { // rename failed
                    $DB->rollback();
                    syslog_insert(sprintf('File %s could not be renamed to %s', '[[' . $old_file_name . ']]', '[[' . $newname . ']]'), 'info', 'file', $this->ID);
                    return false;
                }
     ...
     * @return boolean true on success, false on failure
     */
    public function move_to($root_type, $root_ID, $rdfp_rel_path, $keep_unique = false)
    {
        if (! $this->can_be_manipulated()) {	// check if we can manipulate the file first:
            return false;
        }
        ...
        if ($keep_unique && preg_match('#(.+\/)?(([^.\/]+)(\.[^.]+)?)$#', $rdfp_rel_path, $new_path_match)) {	// Try to find free unique file name if same name is already used in the destination folder:
            $file_unique_name = $new_path_match[2];
            $file_extension = isset($new_path_match[4]) ? $new_path_match[4] : '';
            $file_unique_num = 1;
            while (file_exists($new_FileRoot->ads_path . $new_path_match[1] . $file_unique_name)) {	// Find next free file with unique name in the same folder:
     ...
     * @return boolean true on success, false on failure
     */
    public function copy_to(&$dest_File)
    {
        if (! $this->can_be_manipulated()) {	// check if we can manipulate the file first:
            return false;
        }
        ...
        if (! copy_r($this->get_full_path(), $dest_File->get_full_path(), $new_folder_name)) {	// Note: unlike rename() (at least on Windows), copy() will not fail if destination already exists
        ...
        // Meta data...:
        if ($this->load_meta()) {	// We have source meta data, we need to copy it:
     ...
     * @return boolean true on success, false on failure
     */
    public function unlink($use_transactions = true, $recursively = false)
    {
        if (! $this->can_be_manipulated()) {	// check if we can manipulate the file first:
            return false;
        }
        ...
        if ($this->load_meta()) { // remove meta data from DB:
            $this->dbdelete();
        }
        ...
        if (! $unlinked && ! $file_exists) { // Add additional message which shows that unlink was unsuccesful becuse the file didn't exist
    ...
    public function dbinsert()
    {
        ...
        if (is_logged_in()) {	// Use ID of current logged in user:
            $this->set_param('creator_user_ID', 'integer', $current_User->ID);
        } elseif ($this->_FileRoot->type == 'user') {	// Try to use ID of root user:
        ...
        if (! $this->is_dir()) { // create hash value only for files but not for folders
     ...
     */
    public function dbupdate($update_link_owner_dates = true)
    {
        if ($this->meta == 'unknown') {
        ...
        if ($file_path_hash != $this->path_hash) { // The file path was changed
            $this->set_param('path_hash', 'string', $file_path_hash);
        }
        // Let parent do the update:
        if (($r = parent::dbupdate()) !== false) {
            if ($update_link_owner_dates) {	// Update field 'last_touched_ts'(and 'contents_last_updated_ts' id exists) of each object(Item, Comment, Message and etc.) that has a link with this File:
                $LinkCache = &get_LinkCache();
                $links = $LinkCache->get_by_file_ID($this->ID);
                foreach ($links as $Link) {
                    if ($LinkOwner = &$Link->get_LinkOwner()) {	// Update last touched date and content last updated date of the Owner:
     ...
     */
    public function get_view_url($always_open_dirs_in_fm = true)
    {
        global $public_access_to_media;
        ...
        if ($this->is_dir()) { // Directory
            if ($always_open_dirs_in_fm || ! $public_access_to_media) { // open the dir in the filemanager:
            ...
            } else { // Public access: direct link to folder:
                return $this->get_url();
            }
        } else { // File
            $Filetype = &$this->get_Filetype();
     ...
     */
    public function get_view_link($text = null, $title = null, $no_access_text = null, $format = '$text$', $class = '', $url = null)
    {
        global $Collection, $Blog;

        if (is_null($text)) { // Use file root+relpath+name by default
            $text = ($this->is_dir() ? $this->get_root_and_rel_path() : $this->get_root_and_rel_path());
        }

        if (is_null($title)) { // Default link title
            $this->load_meta();
            $title = $this->title;
        }

        if (is_null($no_access_text)) { // Default text when no access:
            $no_access_text = $text;
        }

        if (is_null($url)) { // Get the URL for viewing the file/dir:
            $url = ($this->is_dir() ? $this->get_linkedit_url() : $this->get_view_url(false));
            $ignore_popup = false;
        } else { // Ignore a popup window when URL is defined from param
            $ignore_popup = true;
        }

        if (empty($url)) { // Display this text when current user has no access
        ...
        $Filetype = &$this->get_Filetype();
        if ($this->is_dir() || $ignore_popup || ($Filetype && in_array($Filetype->viewtype, ['external', 'download']))) { // Link to open in the current window
            return '<a href="' . $url . '" title="' . $title . '"' . $class_attr . $rel_attr . '>' . $text . '</a>';
        } else { // Link to open in a new window
     ...
     */
    public function get_linkedit_link(
        $link_type = null,
        $link_obj_ID = null,
        $text = null,
        $title = null,
     ...
     * @return string
     */
    public function get_linkedit_url($link_type = null, $link_obj_ID = null, $actionurl = '#')
    {
        global $admin_url;
        ...
        if (! is_null($link_obj_ID)) { // We want to open the filemanager in link mode:
     ...
     */
    public function get_thumb_url($size_name = 'fit-80x80', $glue = '&amp;', $size_x = 1)
    {
        global $public_access_to_media;

        if (! $this->is_image()) { // Not an image
            debug_die('Can only thumb images');
        }

        if ($public_access_to_media) {
            $af_thumb_path = $this->get_af_thumb_path($size_name, null, false, $size_x);
            if ($af_thumb_path[0] != '!') { // If the thumbnail was already cached, we could publicly access it:
                if (@is_file($af_thumb_path)) {	// The thumb IS already in cache! :)
     ...
     * @return string
     */
    public function get_getfile_url($glue = '&amp;')
    {
     ...
     * @return string
     */
    public function get_thumb_imgtag(
        $size_name = 'fit-80x80',
        $class = '',
        $align = '',
        $title = '',
        ...
        if ($this->check_image_sizes($size_name, 64, $img_attribs)) { // If image larger than 64x64 add class to display animated gif during loading
     ...
     */
    public function get_thumb_size($size_name = 'fit-80x80')
    {
        global $thumbnail_sizes;

        if (! isset($thumbnail_sizes[$size_name])) { // Wrong thumbnail size name
            return false;
        }
        ...
        if (check_thumbnail_sizes($thumb_type, $thumb_width, $thumb_height, $orig_width, $orig_height)) { // Use the sizes of the original image
            $width = $orig_width;
            $height = $orig_height;
        } else { // Calculate the sizes depending on thumbnail type
            if ($thumb_type == 'fit') {
     ...
     */
    public function get_img_attribs($size_name = 'fit-80x80', $title = null, $alt = null, $size_x = 1, $tag_size = null, $image_sizes = null)
    {
        $img_attribs = [
        ...
        if (! isset($img_attribs['alt'])) { // use title for alt, too
            $img_attribs['alt'] = $img_attribs['title'];
        }
        if (! isset($img_attribs['alt'])) { // always use empty alt
            $img_attribs['alt'] = '';
        }

        if ($size_name == 'original') {	// We want src to link to the original file
            $img_attribs['src'] = $this->get_url();
            if ($tag_size != 'none') {	// Add attributes "width" & "height" only when they are not disabled:
                if ($tag_size !== null) {	// Use size values:
        ...
        } elseif ($size_name == 'fit') { // We want src to link to the original file
            $img_attribs['src'] = $this->get_url();

            if ($tag_size !== null) { // Get target dimension
                $tag_size = explode('x', $tag_size);
                if (empty($tag_size[1])) {
                ...
                if ($size_arr[0] > $tag_size[0] || $size_arr[1] > $tag_size[1]) { // Scale image to fit
        ...
        } elseif (substr($this->_adfp_full_path, -4) == '.svg') {	// Special case for SVG file because we cannot generate thumbnail for this file type:
            $img_attribs['src'] = $this->get_url();
            global $thumbnail_sizes;
            if (isset($thumbnail_sizes[$size_name])) {	// Set attributes for SVG file from config of thumbnail sizes:
                $img_attribs['width'] = $thumbnail_sizes[$size_name][1];
                $img_attribs['height'] = $thumbnail_sizes[$size_name][2];
            }
        } else { // We want src to link to a generated thumbnail:
            $img_attribs['src'] = $this->get_thumb_url($size_name, '&', $size_x);

            global $generate_srcset_sizes;
            if ($generate_srcset_sizes && ! empty($image_sizes)) {	// We want a responsive image with a srcset= and sizes=
                $img_attribs['sizes'] = preg_replace_callback('#(^|[\s,]+)(\d+px|xs|sm|md):\s*#', [$this, 'callback_img_attrib_sizes'], $image_sizes);
            ...
            if ($tag_size != 'none') {	// Add attributes "width" & "height" only when they are not disabled:
                ...
                if ($tag_size !== null) { // Change size values
                    $tag_size = explode('x', $tag_size);
                    $img_attribs['width'] = $tag_size[0];
                    $img_attribs['height'] = empty($tag_size[1]) ? $tag_size[0] : $tag_size[1];
                } elseif (substr($thumb_path, 0, 1) != '!'
                    && ($size_arr = imgsize($thumb_path, 'widthheight_assoc'))) {	// no error, add width and height attribs
                    $img_attribs += $size_arr;
                } elseif ($thumb_sizes = $this->get_thumb_size($size_name)) {	// Get sizes of the generated thumbnail:
        ...
        if (! $this->exists()) {	// We cannot find the file, force use of getfile.php to handle missing display of missing file:
            $img_attribs['src'] = url_add_param($this->get_getfile_url('&'), [
                'size' => $size_name,
            ], '&');
        }
     ...
     * @return string
     */
    public function callback_img_attrib_sizes($m)
    {
        switch ($m[2]) {
            case 'xs':
     ...
     */
    public function get_img_srcset($image_sizes, $size_name = 'fit-80x80', $glue = '&amp;', $size_x = 1)
    {
        global $generate_srcset_sizes, $thumbnail_sizes, $grouped_srcset_thumbnail_sizes;

        if (! $generate_srcset_sizes) {	// Disabled in config
            return '';
        }

        if (! isset($thumbnail_sizes[$size_name])) {	// Wrong thumbnail size:
            return '';
        }
        ...
        if (! preg_match_all('/(\(max-width:\s*\d+px\)\s*)?(\d+)px(,\s*|$)/', $image_sizes, $image_sizes_match) ||
            empty($image_sizes_match[2])) {	// Wrong value for attribute "sizes":
            return '';
        }

        if ($grouped_srcset_thumbnail_sizes === null) {	// Group and sort thumbnail sizes ONCE and put into cache array:
            $grouped_srcset_thumbnail_sizes = [];
            foreach ($thumbnail_sizes as $thumb_size_name => $thumb_size_data) {
                ...
                if (! isset($grouped_srcset_thumbnail_sizes[$thumb_size_type])) {	// Init array to group by type:
                    $grouped_srcset_thumbnail_sizes[$thumb_size_type] = [];
                }
                if (! isset($grouped_srcset_thumbnail_sizes[$thumb_size_type][$thumb_size_blur])) {	// Init array to group by blur effect:
                    $grouped_srcset_thumbnail_sizes[$thumb_size_type][$thumb_size_blur] = [];
                }
                if (! isset($grouped_srcset_thumbnail_sizes[$thumb_size_type][$thumb_size_blur][$thumb_size_aspect_ratio])) {	// Init array to group by aspect ratio:
                    $grouped_srcset_thumbnail_sizes[$thumb_size_type][$thumb_size_blur][$thumb_size_aspect_ratio] = [];
                }
        ...
        if (! isset($grouped_srcset_thumbnail_sizes[$thumb_size_type][$thumb_size_blur][$thumb_size_aspect_ratio][$size_name])) {	// Wrong thumbnail size, cannot be detected in group,
        ...
        $requested_image_sizes = [];
        foreach ($image_sizes_match[2] as $image_sizes_m) {	// Don't use thumbnail size with width more than original:
            if ($image_sizes_m < $original_image_width) {	// 1x size:
                $requested_image_sizes[] = $image_sizes_m;
            }
            if ($image_sizes_m * 2 < $original_image_width) {	// 2x size for retina screen:
        ...
        foreach ($requested_image_sizes as $requested_image_size => $r) {
            ...
            foreach ($grouped_srcset_thumbnail_sizes[$thumb_size_type][$thumb_size_blur][$thumb_size_aspect_ratio] as $grouped_thumb_size_name => $grouped_thumb_size_data) {
                if ($grouped_thumb_size_data[1] >= $requested_image_size) {	// If thumbnail size more than requested size:
                    if ($grouped_thumb_size_data[1] < $original_image_width) {	// Allow thumbnail size only with less width than original image:
            ...
            if (! $is_detected_proper_size) {	// Don't use requested size if it is not proper:
        ...
        if ($thumb_size_type == 'fit' && ! $thumb_size_blur) {	// Add original size as max possible instead of thumbnail:
     ...
     */
    public function get_preview_thumb($format_for_non_images = '', $cbox_params = [])
    {
        if ($this->is_image()) {	// Ok, it's an image:
            $type = $this->get_type();
            $size_name = 'fit-80x80';
            ...
            $cbox_params = array_merge([
                'init' => false,
                'lightbox_rel' => 'lightbox',
                'link_id' => 'f' . $this->ID,
            ], $cbox_params);
            if ($cbox_params['init']) { // Create link to preview image by colorbox plugin
                $link = '<a href="' . $this->get_url() . '" rel="' . $cbox_params['lightbox_rel'] . '" id="' . $cbox_params['link_id'] . '">' . $img . '</a>';
            } else { // Get link to view the file (fallback to no view link - just the img):
                $link = $this->get_view_link($img);
            }

            if (! $link) { // no view link available:
                $link = $img;
            }
     ...
     * @return boolean TRUE
     */
    public function check_image_sizes($thumb_size, $min_size = 64, $img_attribs = [])
    {
        if (! $this->is_image()) { // Only for images
            return false;
        }

        if (isset($img_attribs['width'], $img_attribs['height'])) { // If image larger than 64x64 add class to display animated gif during loading
            return ($img_attribs['width'] > $min_size && $img_attribs['height'] > $min_size);
        }

        global $thumbnail_sizes;

        if (isset($thumbnail_sizes[$thumb_size])) { // If thumb size name is defined we can calculate what sizes will be of the thumbnail image
            $thumb_type = $thumbnail_sizes[$thumb_size][0];
            $thumb_width = $thumbnail_sizes[$thumb_size][1];
            $thumb_height = $thumbnail_sizes[$thumb_size][2];
            if ($thumb_type == 'crop' || $thumb_type == 'crop-top') { // When thumbnail has "crop" format - width and height have the same values
                if ($thumb_width > $min_size && $thumb_height > $min_size) { // Only check if they are more than $min_size
                    return true;
                }
            } elseif ($thumb_type == 'fit') { // Calculate what height will be for the generated thumbnail image
                ...
                if (isset($orig_sizes['width'], $orig_sizes['height'])) {
                    ...
                    if ($thumb_width > $min_size && $result_height > $min_size) { // Width & height of the generated thumbnail image will be more than $min_size
                        return true;
                    }
                }
            }
     ...
     */
    public function get_ads_evocache($create_if_needed = false)
    {
        global $Settings;
        if (strpos($this->_dir, '/' . $Settings->get('evocache_foldername') . '/') !== false) {	// We are already in an evocache folder: refuse to go further!
            return '!Recursive caching not allowed';
        }

        $adp_evocache = $this->_dir . $Settings->get('evocache_foldername') . '/' . $this->_name;

        if ($create_if_needed && ! is_dir($adp_evocache)) {	// Create the directory:
            if (! mkdir_r($adp_evocache)) {	// Could not create
     ...
     */
    public function get_af_thumb_path($size_name, $thumb_mimetype = null, $create_evocache_if_needed = false, $size_x = 1)
    {
        $Filetype = &$this->get_Filetype();
        if (isset($Filetype)) {
        ...
        if ($ads_evocache[0] != '!') {	// Not an error
     ...
     * @param integer Ratio size, can be 1, 2 and etc.
     */
    public function save_thumb_to_cache($thumb_imh, $size_name, $thumb_mimetype, $thumb_quality = 90, $size_x = 1)
    {
        global $Plugins;

        $Plugins->trigger_event('BeforeThumbCreate', [
            'imh' => &$thumb_imh,
            'size' => &$size_name,
            'size_x' => &$size_x,
        ...
        if ($af_thumb_path[0] != '!') {	// We obtained a path for the thumbnail to be saved:
     ...
     */
    public function output_cached_thumb($size_name, $thumb_mimetype, $mtime = null, $size_x = 1)
    {
        ...
        if ($af_thumb_path[0] != '!') {	// We obtained a path for the thumbnail to be saved:
            if (! file_exists($af_thumb_path)) {	// The thumbnail was not found...
                global $Settings;
     ...
     */
    public function link_to_Object(&$LinkOwner, $set_order = 0, $position = null)
    {
        global $DB;
    ...
    public function get_restriction_link($restriction)
    {
        ...
        switch ($restriction['table']) { // can be restricted to different tables
            case 'T_links':
        ...
        foreach ($query_result as $row) { // create links for each related object
            $result_link .= '<br/>' . sprintf($link, $row->$object_ID, $row->$object_name);
        }

        if (($count = count($query_result)) > 0) { // there are restrictions
     ...
     * @return string Link
     */
    public function get_target_icon()
    {
        $r = '';
        if (check_user_perm('files', 'view', false, $this->get_FileRoot())) {	// Check permission
            if ($this->is_dir()) {	// Dir
            ...
            $r .= '<a href="' . $url . '" title="' . $title . '">' . get_icon('locate', 'imgtag', [
                'title' => $title,
            ]) . '</a> ';
        }
    ...
    {
        if (! empty($this->type)) { // Don't detect file type if File type is already defined
            return;
        }

        // AUDIO:
        $file_extension = $this->get_ext();
        if (! empty($file_extension)) { // Set audio file type by extension:
            ...
            if (count($FiletypeCache->cache)) {
                foreach ($FiletypeCache->cache as $Filetype) {
                    if (preg_match('#^audio/#', $Filetype->mimetype) &&
                        in_array($file_extension, $Filetype->get_extensions())) { // This is audio file
            ...
            if (count($FiletypeCache->cache)) {
                foreach ($FiletypeCache->cache as $Filetype) {
                    if (preg_match('#^video/#', $Filetype->mimetype) &&
                            in_array($file_extension, $Filetype->get_extensions())) { // This is a video file
        ...
        if (is_image_file($this->_adfp_full_path) || $this->get_image_size() !== false) { // This is image file
     ...
     */
    public function update_file_type($file_type)
    {
        if ($file_type == $this->type) { // File type is already defined
            return;
        }
        ...
        if (! empty($this->ID) && ! empty($file_type)) { // Update file type in DB
            global $DB;
            $DB->query('UPDATE T_files
     ...
     */
    public function get_thumb_name($size_name, $size_x = 1)
    {
        $size_x = intval($size_x);

        if ($size_x == 1 || $size_x != 2) { // 1x size or wrong $size_x
            return $size_name . '.' . $this->get_ext();
        }

        if (preg_match('#^([^\d]+)(\d+)x(\d+)(.*)$#', $size_name, $size_match)) { // Modify size name by increasing width and height values, E.g. crop-32x32 will be crop-64x64 for $size_x = 2
     ...
     */
    public function get_duplicated_files($params = [])
    {
        $params = array_merge([
            'file_type' => 'image',
        ...
        if (! empty($link_object_ID_field)) { // Check to object ID field is not NULL
            $SQL->WHERE_and($link_object_ID_field . ' IS NOT NULL');
        }
        if ($params['root_ID'] !== null) { // Restrict by root ID
     ...
     */
    public function get_duplicated_files_message($params = [])
    {
        $params = array_merge([
            'message' => '%s',
            'file_type' => 'image',
        ...
        if (empty($duplicated_file_IDs)) { // No duplicates
            return false;
        }
        ...
        foreach ($duplicated_file_IDs as $file_ID => $file_root_ID) {
            if (! ($duplicated_File = &$FileCache->get_by_ID($file_ID, false, false))) { // Broken file object
                continue;
            }

            if ($params['link_to'] == 'user') { // Link to profile picture edit form
                global $admin_url;
                $UserCache = &get_UserCache();
                $User = &$UserCache->get_by_ID($file_root_ID, false, false);

                $link_text = $User ? $User->get_colored_login([
                    'use_style' => $params['use_style'],
                    'login_text' => 'name',
                ]) : T_('Deleted user');
     ...
     * @return string
     */
    public function get_votes_count_info($type, $params = [])
    {
        $params = array_merge([
            'message' => T_('%d times by %s'),
        ], $params);

        if (empty($this->ID)) { // This file is not exists in DB
            return '0';
        }

        if (! isset($this->votes_count)) { // Get the votes count only first time from DB
            global $DB;
            $SQL = new SQL();
        ...
        if (empty($this->votes_count)) { // No votes yet
            return '0';
        }
        ...
        if ($count == 0) { // No votes for the selected type yet
            return '0';
        }
     ...
     */
    public function increment_download_count($count = 1)
    {
    ...
    public function get_background_image_css($params = [])
    {
        ...
        $styles = ['background-image:url( ' . $img_attribs_1x['src'] . ' )'];

        if ($params['size'] != $params['size_2x']) {	// Set image-set backgrounds only when 1x and 2x size are different:
