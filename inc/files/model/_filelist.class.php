...
class Filelist
{
     ...
     * @var boolean
     */
    public $_show_hidden_files = false;
     ...
     * @access protected
     */
    public $_use_recursive_dirsize = false;
     ...
     */
    public function __construct($FileRoot, $path = null)
    {
        global $AdminUI;
    ...
    public function load()
    {
        ...
        if (! $this->_ads_list_path) {	// We have no path to load from: (happens when FM finds no available root OR we have an arbitrary)
        ...
        $this->_entries = [];
        $this->_md5_ID_index = [];
        $this->_full_path_index = [];
        $this->_rdfs_rel_path_index = [];
        $this->_order_index = [];
        ...
        foreach ($filepath_array as $adfp_path) {
            ...
            if ((! $this->_show_hidden_files) && (substr($name, 0, 1) == '.')) { // Do not load & show hidden files (prefixed with .)
                continue;
            }

            // Check for _evocache...
            if ((! $this->_show_evocache) && ($name == '_evocache')) { // Do not load & show _evocache folder
                continue;
            }

            // Check filter...
            if ($this->_filter !== null) { // Filter: must match filename
                if ($this->_filter_is_regexp) { // Filter is a reg exp:
                    if (! preg_match($this->_filter, $name)) { // does not match the regexp filter
                        continue;
                    }
                } else { // Filter is NOT a regexp:
     ...
     */
    public function add($File, $mustExist = false)
    {
        if (! ($File instanceof file)) {	// Passed object is not a File!! :(
            return false;
        }
        ...
        if ($mustExist && ! $File->exists()) {	// File does not exist..
            return false;
        }
        ...
        if ($File->is_dir()) {	// Count 1 more directory
            $this->_total_dirs++;
        } else {	// Count 1 more file
            $this->_total_files++;
        }

        if (! $File->is_dir()) {	// Count total bytes of files in this dir:
     ...
     */
    public function get_File_size_formatted($File)
    {
     ...
     *                 file does not exist)
     */
    public function add_by_subpath($rel_path, $mustExist = false)
    {
        $FileRoot = &$this->get_FileRoot();

        if ($FileRoot->contains($rel_path)) {	// If a file is really contained in the FileRoot of this list:
     ...
     */
    public function sort($order = null, $orderasc = null, $dirsattop = null)
    {
        ...
        if ($order !== null) { // New order
            $this->_order = $order;
        } elseif ($this->_order === null) { // Init
            $this->_order = 'name';
        }

        if ($orderasc !== null) { // New ascending/descending setting
            $this->_order_asc = $orderasc;
        } elseif ($this->_order_asc === null) { // Init: ascending for 'name' and 'path', else descending
        ...
        usort($this->_order_index, [$this, '_sort_callback']);
    ...
    public function _sort_callback($a, $b)
    {
        ...
        switch ($this->_order) {
            ...
            case 'name':
                ...
                if ($r == 0) { // same name: look at path
                    $r = strnatcmp($FileA->get_dir(), $FileB->get_dir());
                }
                break;
        }

        if (! $this->_order_asc) { // We want descending order: switch order
            $r = -$r;
        }

        if (! $this->_dirs_not_at_top) {	// We want dirs to be on top, always:
     ...
     * @return string
     */
    public function get_sort_link($type, $atext)
    {
        global $AdminUI;
        ...
        // Sorting icon:
        if ($this->_order != $type) { // Not sorted on this column:
            $r .= ' class="basic_sort_link">' . $result_params['basic_sort_off'];
        } elseif ($this->is_sorting_asc($type)) { // We are sorting on this column , in ascneding order:
     ...
     */
    public function is_sorting_asc($col = '')
    {
        if ($this->_order_asc === null) { // We have not specified a sort order by now, use default:
            if (empty($col)) {
                $col = $this->_order;
            }
            return ($col == 'name' || $col == 'path') ? 1 : 0;
        } else {	// Use previsously specified sort order:
     ...
     */
    public function set_filter($filter_string, $filter_is_regexp)
    {
        global $Messages;
     ...
     */
    public function get_filter($verbose = true)
    {
        if ($this->_filter === null) {	// Filtering is not active
            return $verbose ? T_('No filter') : '';
        } else {	// Filtering is active
            return $this->_filter
     ...
     */
    public function is_filter_regexp()
    {
     ...
     */
    public function &get_next($type = '')
    {
        for (; ;) {
            if (! isset($this->_order_index[$this->_current_idx + 1])) {	// End of list:
                $r = false;
                return $r;
            }
            $this->_current_idx++;
            ...
            if ($type != '') {
                ...
                if (($type == 'dir' && ! $is_dir)
                    || ($type == 'file' && $is_dir)) { // we want another type
                    continue;
                }
            }
            break;
     ...
     */
    public function &get_by_rdfs_path($rdfs_path)
    {
     ...
     * @return mixed File object (by reference) on success, false on failure.
     */
    public function &get_by_full_path($adfs_path)
    {
        // We probably don't need the windows backslashes replacing any more but leave it for safety because it doesn't hurt:
     ...
     * @return mixed File object (by reference) on success, false on failure.
     */
    public function &get_by_md5_ID($md5id)
    {
     ...
     * @return File
     */
    public function &get_by_idx($index, $halt_on_error = true)
    {
     ...
     */
    public function get_ads_list_path()
    {
     ...
     */
    public function get_rds_list_path()
    {
     ...
     * @return string
     */
    public function rdfs_relto_root_from_adfs($adfs_path)
    {
    ...
    public function remove(&$File)
    {
        if (isset($this->_md5_ID_index[$File->get_md5_ID()])) {
            ...
            if ($this->_current_idx > -1 && $this->_current_idx >= $order_key) { // We have removed a file before or at the $order_key'th position
                $this->_current_idx--;
            }
            return true;
        }
     ...
     */
    public function get_array($method = null)
    {
        $r = [];
    ...
    public function load_meta()
    {
        ...
        for ($i = 0; $i < count($this->_entries); $i++) {	// For each file:
            $loop_File = &$this->_entries[$i];
            // echo '<br>'.$loop_File->get_full_path();

            if ($loop_File->meta != 'unknown') { // We have already loaded meta data:
                continue;
            }
        ...
        if (count($to_load)) {	// We have something to load...
            /**
             * @var FileCache
             */
            $FileCache = &get_FileCache();
            ...
            $rows = $DB->get_results(
                ...
                OBJECT,
                'Load FileList meta data'
            );

            if (count($rows)) { // Go through rows of loaded meta data...
                foreach ($rows as $row) {
     ...
     */
    public function get_cwd_clickable($clickableOnly = true)
    {
        if (empty($this->_ads_list_path)) {
