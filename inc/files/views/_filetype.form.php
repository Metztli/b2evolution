...
$Form->text_input('ftyp_extensions', $edited_Filetype->extensions, 40, TB_('Extensions'), '', [
    'maxlength' => 30,
    'required' => true,
    'note' => sprintf(TB_('E.g. %s'), '<code>html</code>') . ', ' . TB_('separated by whitespace'),
]);

$Form->text_input('ftyp_name', $edited_Filetype->name, 40, TB_('File type name'), sprintf(TB_('E.g. %s'), '<code>' . TB_('HTML file') . '</code>'), [
    'maxlength' => 30,
    'required' => true,
]);

$Form->text_input('ftyp_mimetype', $edited_Filetype->mimetype, 40, TB_('Mime type'), sprintf(TB_('E.g. %s'), '<code>text/html</code>'), [
    'maxlength' => 50,
    'required' => true,
]);
...
if (empty($admins_can_manipulate_sensitive_files)) {
    ...
    $allow_upload_note = sprintf(
        TB_('You can unlock this for admins by setting %s in the <a %s>configuration files</a>'),
        '<code>$admins_can_manipulate_sensitive_files = true</code>',
        'target="_blank" href="' . get_manual_url('advanced-php') . '"'
    );
} else {
...
$Form->radio(
    'ftyp_allowed',
    $edited_Filetype->allowed,
    [
        ['any', TB_('Allow anyone (including anonymous users) to upload/rename/edit files of this type')],
        ['registered', TB_('Allow only registered users to upload/rename/edit files of this type')],
        ['admin', $admin_allow_text],
    ],
    TB_('Allow upload'),
    true,
    $allow_upload_note
...
if ($creating) {
    $Form->end_form([['submit', 'submit', TB_('Record'), 'SaveButton'],
        ['submit', 'submit', TB_('Record, then Create New'), 'SaveButton'],
        ['submit', 'submit', TB_('Record, then Create Similar'), 'SaveButton']]);
} else {
