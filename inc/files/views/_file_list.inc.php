...
if (get_param('fm_sources_root') == '') { // Set the root only when it is not defined, otherwise it is gone from memorized param
...
if (empty($fm_Filelist->_rds_list_path)) { // cannot go higher
    echo '&nbsp;';	// for IE
} else {
...
if ($UserSettings->get('fm_imglistpreview')) { // Image file preview:
...
if ($fm_Filelist->count() > 0) {	// Display table headers only when at least file is found in the selected folder and filter:
    echo $table_headers;
}
?>
...
/***********************************************************/
$countFiles = 0;
while ($lFile = &$fm_Filelist->get_next()) { // Loop through all Files:
    echo '<tr class="' . ($countFiles % 2 ? 'odd' : 'even') . '"';

    if (isset($fm_highlight) && $lFile->get_name() == $fm_highlight) { // We want a specific file to be highlighted (user clicked on "locate"/target icon
    ...
    if ($mode == 'upload') {	// This mode allows to insert img tags into the post...
        // Hidden info used by Javascript:
    ...
    if ($UserSettings->get('fm_imglistpreview')) {	// Image preview OR full type:
        if ($lFile->is_dir()) { // Navigate into Directory
    ...
    } else {	// No image preview, small type:
        if ($lFile->is_dir()) { // Navigate into Directory
    ...
    if (! $fm_flatmode ||
        ...
        ($selected_Filelist->get_rds_list_path() == dirname($lFile->get_rdfs_rel_path()) . '/')) { // Use a hidden field only for current folder and not for subfolders
    ...
    } else { // Don't use the hidden field for this file because it is from another folder
    ...
    if (! $lFile->is_dir()) {
        if ($error_filename = validate_filename($lFile->get_name())) { // TODO: Warning icon with hint
            echo get_icon('warning', 'imgtag', [
                'class' => 'filenameIcon',
                'title' => strip_tags($error_filename),
                'data-toggle' => 'tooltip',
            ]) . '&nbsp;';
            syslog_insert(sprintf('The unrecognized extension is detected for file %s', '[[' . $lFile->get_name() . ']]'), 'warning', 'file', $lFile->ID);
        }
    } elseif ($error_dirname = validate_dirname($lFile->get_name())) { // TODO: Warning icon with hint
        echo get_icon('warning', 'imgtag', [
            'class' => 'filenameIcon',
            'title' => strip_tags($error_dirname),
            'data-toggle' => 'tooltip',
        ]) . '&nbsp;';
    ...
    {	// Only provide link/"chain" icons for files.
        ...
        if (isset($LinkOwner) && $LinkOwner->check_perm('edit')) {	// Offer option to link the file to an Item (or anything else):
            $link_attribs = [
                'class' => 'action_icon link_file btn btn-primary btn-xs',
            ];
            $link_action = 'link';
            if ($mode == 'upload') {	// We want the action to happen in the post attachments iframe:
        ...
        if (isset($edited_User)) { // fp> Perm already checked in controller
            ...
            if ($lFile->is_image()) {
                echo action_icon(
                    T_('Use this as my profile picture!'),
                    'link',
                ...
                echo action_icon(
                    T_('Duplicate and use as profile picture'),
                    'user',
    ...
    if ($UserSettings->get('fm_showtypes')) { // Show file types
    ...
    if ($UserSettings->get('fm_showdownload')) { // Show download count
    ...
    if ($UserSettings->get('fm_showdate') != 'no') { // Show last modified datetime (always full in title attribute)
    ...
    if ($UserSettings->get('fm_showfsperms')) { // Show file perms
        echo '<td class="perms">';
        $fm_permlikelsl = $UserSettings->param_Request('fm_permlikelsl', 'fm_permlikelsl', 'integer', 0);

        if ($edit_allowed_perm) { // User can edit:
    ...
    if ($UserSettings->get('fm_showfsowner')) { // Show file owner
    ...
    if ($UserSettings->get('fm_showfsgroup')) { // Show file owner
...
$noresults = '';
if ($countFiles == 0) {	// Filelist errors or "directory is empty":
...
// --------------
if ($Settings->get('upload_enabled') && check_user_perm('files', 'add', false, $fm_FileRoot)) {	// Upload is enabled and we have permission to use it...
    ?>
		<tr class="evo_fileuploader_form listfooter firstcol lastcol">
            ...
            if (isset($LinkOwner) && $LinkOwner->check_perm('edit')) {	// Offer option to link the file to an Item (or anything else):
                ...
                $icon_to_link_files = action_icon(
                    T_('Link this file!'),
                    'link',
    ...
    if ($UserSettings->get('fm_showdate') == 'no') { // Display status in the last column if column with datetime is hidden
    ...
    display_dragdrop_upload_button([
...
$Form->checkbox_controls('fm_selected', [
 ...
 */
if ($fm_mode == 'link_object' && $mode != 'upload') {	// We are linking to an object...
...
if (($fm_Filelist->get_root_type() == 'collection' || (! empty($Blog)
    ...
    && $mode != 'upload'
    && check_user_perm('admin', 'normal')) { // We are browsing files for a collection:
...
$Form->submit_input([
    'name' => 'actionArray[group_action]',
    'value' => T_('Go!'),
    'onclick' => 'return js_act_on_selected();',
]);
...
if (($Settings->get('fm_enable_create_dir') || $Settings->get('fm_enable_create_file'))
            && check_user_perm('files', 'add', false, $fm_FileRoot)) {	// dir or file creation is enabled and we're allowed to add files:
    global $create_type;

    echo '<div class="evo_file_folder_creator">';
    if (! $Settings->get('fm_enable_create_dir')) {	// We can create files only:
        echo '<label for="fm_createname" class="tooltitle">' . T_('New file:') . '</label>';
        $Form->hidden('create_type', 'file');
    } elseif (! $Settings->get('fm_enable_create_file')) {	// We can create directories only:
        echo '<label for="fm_createname" class="tooltitle">' . T_('New folder:') . '</label>';
        $Form->hidden('create_type', 'dir');
    } else {	// We can create both files and directories:
        echo T_('New') . ': ';
...
if ($countFiles) {
    {{ // include JS
        ...
        if ($fm_highlight) { // we want to highlight a file (e.g. via "Locate this file!"), scroll there and do the success fade
            ?>
