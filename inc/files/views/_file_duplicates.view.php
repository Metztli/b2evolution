...
if (($min_inappropriate_votes > 0) || ($min_spam_votes > 0)) { // we need to join files vote table to be able to filter by votes
    ...
    if ($min_inappropriate_votes > 0) {	// Filter by minimum count of inappropriate votes
        $hash_SQL->HAVING_and('SUM( lvot_inappropriate ) >= ' . $DB->quote($min_inappropriate_votes));
    }
    if ($min_spam_votes > 0) {	// Filter by minimum count of spam votes
        $hash_SQL->HAVING_and('SUM( lvot_spam ) >= ' . $DB->quote($min_spam_votes));
    }
}
if ($file_ID > 0) {	// Filter by hash of File ID
...
if ($num_file_results > 0) { // Create SQL query to build a results table
    $SQL = new SQL();
    $SQL->SELECT('f.*,
...
{
    if (is_object($File)) { // Check if File object is correct
...
function td_file_duplicates_path($File, $file_root_type, $file_root_ID, $file_path)
{
    if (is_object($File)) { // Check if File object is correct
        ...
        if (check_user_perm('files', 'edit_allowed', false, $File->get_FileRoot())) { // Allow to delete a file only if current user has an access
            global $admin_url;
            $r .= action_icon(
                T_('Delete'),
                'file_delete',
                ...
                null,
                [
                    'onclick' => 'return confirm(\'' . TS_('Are you sure want to delete this file?') . '\');',
                ]
            );
        }
        return $r;
    } else { // Broken File object
        if (empty($file_path)) { // No file data exist in DB
            return T_('File no longer exists on disk.');
        } else { // Display file info from DB
...
{
    if (is_object($File) && check_user_perm('files', 'edit_allowed', false, $File->get_FileRoot())) { // Check if File object is correct and current user has an access
