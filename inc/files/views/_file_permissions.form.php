...
if ($more_than_one_selected_file) { // more than one file, provide default
...
if ($more_than_one_selected_file) { // generate checkbox input to use with every file
    $Form->output = false;
    $Form->switch_layout('none');
    $use_default_perms_checkbox = $Form->checkbox_input(
        'use_default_perms[]',
        $checked = 0,
        TB_('Use default value'),
        [
            'value' => '%file_ID%',
            'id' => 'use_default_perms_%file_ID%',
        ]
    );
...
while ($l_File = &$selected_Filelist->get_next()) {
    if ($perms_read_readonly) { // readonly/write only (WINDOWS): display radio inputs to change readonly/write
    ...
    } else { // display a text input with UNIX perms
        $field_params = [];
        if (! empty($use_default_perms_checkbox)) {
            $field_params['field_suffix'] = str_replace('%file_ID%', $l_File->get_md5_ID(), $use_default_perms_checkbox);
        }
...
$Form->end_form([['submit', 'submit', TB_('Set new permissions'), 'ActionButton']]);
