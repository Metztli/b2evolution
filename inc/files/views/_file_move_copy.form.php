...
if (! empty($new_names)) {
    ...
    $buttons = [];
    if ($action == 'move_copy' || $action == 'file_move') {	// Display a button to move files/folders:
        $buttons[] = ['submit', 'actionArray[move]', TB_('Move'), 'SaveButton'];
    }
    if ($action == 'move_copy' || $action == 'file_copy') {	// Display a button to copy files/folders:
        $buttons[] = ['submit', 'actionArray[copy]', TB_('Copy'), 'SaveButton'];
    }
