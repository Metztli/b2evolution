...
if ($min_inappropriate_votes <= 1 && $min_spam_votes <= 1 && (! ($min_inappropriate_votes && $min_spam_votes))) { // We must show all votes or where is at least one spam vote or where is at least one inappropriate ( one filter must be 0 and none of them > 1 )
    if ($min_inappropriate_votes) { // Min inappropriate filter is set to 1 but min spam is 0
        $sql_where = 'lvot_inappropriate = 1';
    } elseif ($min_spam_votes) { // Min spam filter is set to 1 but min inappropriate is 0
        $sql_where = 'lvot_spam = 1';
    } else { // We have to show all files which has any kind of spam vote
...
} else { // check to fit at least one of the minimum requirements
