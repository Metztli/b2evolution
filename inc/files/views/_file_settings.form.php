...
$Form->begin_fieldset(TB_('Accessible file roots') . get_manual_link('accessible-file-roots'), [
    'id' => 'ffset_fileroots',
    'class' => 'additional_file_settings',
]);
...
$Form->begin_fieldset(TB_('File creation options') . get_manual_link('file-creation-options'), [
    'id' => 'ffset_filecreate',
    'class' => 'additional_file_settings',
]);
...
$Form->checkbox_input('upload_enabled', $Settings->get('upload_enabled', true), TB_('Enable upload of files'), [
    'note' => TB_('Check to allow uploading files in general.'),
    'onclick' => JS_showhide_ffield_on_this('upload_maxkb'),
]);
...
$Form->begin_fieldset(TB_('Advanced options') . get_manual_link('advanced-file-options'), [
    'id' => 'ffset_fileadvanced',
    'class' => 'additional_file_settings',
]);
...
if (empty($force_regexp_filename) || empty($force_regexp_dirname)) { // At least one of these strings can be configured in the UI:
...
$Form->radio_input('evocache_foldername', $Settings->get('evocache_foldername'), [
    [
        'value' => '.evocache',
        'label' => TB_('Use .evocache folders (system hidden folders)'),
    ],
    [
        'value' => '_evocache',
        'label' => TB_('Use _evocache folders (compatible with all webservers)'),
    ]], TB_('Cache folder names'), [
        'lines' => 2,
    ]);
...
if (check_user_perm('options', 'edit', false)) { // We have permission to modify:
    $Form->buttons([
        ['submit', 'submit[update]', TB_('Save Changes!'), 'SaveButton'],
        ['submit', 'submit[restore_defaults]', TB_('Restore defaults'), 'ResetButton'],
    ]);
}

$Form->end_form();

if (check_user_perm('options', 'edit', false)) {	// TODO: better perm check
