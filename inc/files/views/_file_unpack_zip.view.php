...
foreach ($selected_Filelist->get_array() as $selected_File) {
    ...
    if (! is_file($selected_File->get_full_path()) ||
        ! preg_match('/\.zip$/i', $selected_File->get_full_path())) {	// Not ZIP archive was selected:
        ...
        $folder_suffix = '';
        while (file_exists($unpack_folder . $folder_suffix)) {	// Find free(not existing) folder to unpack:
            $folder_suffix++;
        }
        ...
        if ($unpack_result === true) {	// Display success result:
...
if ($unpack_is_done && ! empty($fm_Filelist)) {	// Reload files list to display new folders after at least one unpacking:
    global $fm_Filelist;
    $fm_Filelist->load();
}
