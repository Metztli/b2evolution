...
if (param('ftyp_ID', 'integer', '', true)) {// Load file type:
    $FiletypeCache = &get_FiletypeCache();
    if (($edited_Filetype = &$FiletypeCache->get_by_ID($ftyp_ID, false)) === false) {	// We could not find the file type to edit:
...
if (isset($edited_Filetype) && ($edited_Filetype !== false)) {	// We are editing a division:
...
switch ($action) {
    ...
    case 'create':
        ...
        if ($edited_Filetype->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Filetype->check_delete(sprintf(TB_('Cannot delete file type &laquo;%s&raquo;'), $edited_Filetype->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
