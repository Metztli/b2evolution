...
class POFile
{
    ...
    public function __construct($filename = nulll)
    {
     ...
     */
    public function addmsgid($msgid, $sourcefile = '', $trans = '')
    {
        if (in_array($msgid, ['trans_locale', 'trans_charset', 'trans_available'])) { // don't put those into POT file
            return;
        }
    ...
    public function translate($msgid)
    {
        ...
        if (preg_match_all('/<a\s+([^>]*)>/', $msgid, $matches)) { // we have to replace links
     ...
     */
    public function read($log_source_info = true)
    {
        ...
        foreach ($lines as $line) {
            ...
            } elseif (($status == '-') && preg_match('@^#:(.*)@', $line, $matches)) {	// Encountered a source code location comment
                // echo $matches[0],'<br />';
     ...
     */
    public function write_evo_trans($file_path, $locale, $params = [])
    {
...
class POTFile extends POFile
{
    ...
    public function write()
    {
        ...
        foreach ($this->msgids as $msgid => $arr) {
            if (isset($arr['source'])) { // write sources of string
