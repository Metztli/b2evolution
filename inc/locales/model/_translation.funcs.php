...
function translation_update_table_pot()
{
    ...
    foreach ($lines as $line) {
        if (trim($line) == '') {	// Blank line, go back to base status:
            if ($status == 't' && ! empty($msgid)) {	// ** End of an original text ** :
...
function translation_update_table_po($locale)
{
    ...
    if (! file_exists($po_file_name)) {	// No locale file, Exit here
        global $Messages;
    ...
    foreach ($lines as $line) {
        if (trim($line) == '') {	// Blank line, go back to base status:
            if ($status == 't' && ! empty($msgstr)) {	// ** End of an original text ** :
...
function translation_update_table_po_row($locale, $original_string, $translated_string)
{
    ...
    if (! $original_string_ID) {	// No original string, Exit here
        return;
    }
    ...
    if ($translated_string_ID) {	// Update already existing string
...
function translation_generate_po_file($locale)
{
    ...
    if (! $ok) { // Inform user about no permission to write PO file
        global $Messages;
...
function translation_generate_pot_file()
{
    ...
    foreach ($translation_strings as $string => $files) { // Format the translation strings to write in .POT file
        if (isset($files['trans'])) { // Text of TRANS info
            if (is_array($files['trans'])) { // Multiline TRANS info
                foreach ($files['trans'] as $ft => $files_trans) {
                    $pot_content[] = '#. ' . ($ft == 0 ? 'TRANS: ' : '') . $files_trans;
                }
            } else { // Single TRANS info
                $pot_content[] = '#. TRANS: ' . $files['trans'];
            }
            unset($files['trans']);
        }
        foreach ($files as $file) { // File name and line number where string exists
            $pot_content[] = '#: ' . $file[1] . ':' . $file[0];
        }
        if (strpos($string, '%') !== false) { // Char '%' is detected in the string
            if (preg_match('/%(s|\d*d)/', $string)) { // The string contains a mask like %s or %d
                $pot_content[] = '#, php-format';
            } else { // The string contains a simple char '%'
    ...
    if (! $ok) { // Inform user about no permission to write POT file
        global $Messages;
...
function translation_scandir($path, &$translation_strings)
{
    ...
    foreach ($files as $file) {
        if (is_file($path . $file) && preg_match('/\.php$/i', $path . $file)) {	// PHP file; Find all translation strings in current file
            translation_find_T_strings($path . $file, $translation_strings);
        } elseif ($file != '.' && $file != '..' && is_dir($path . $file)) {	// Directory; Scan each directory recursively to find all PHP files
...
function translation_find_T_strings($file, &$translation_strings)
{
    ...
    foreach ($file_lines as $line_number => $line_string) {
        if (! $T_string_is_opened && strpos($line_string, 'T_') === false && strpos($line_string, 'TS_') === false &&
            ! $TRANS_is_opened && strpos($line_string, 'TRANS:') === false) { // This line doesn't contain any T_ string AND TRANS info, Skip it
            continue;
        }
        ...
        for ($l = 0; $l < strlen($line_string); $l++) {
            ...
            if ($T_string_is_opened && $char_prev1 != '\\' && $char == $T_string_quote_sign) { // CLOSE T_ string
                // Save T_ string to array
                $T_string_text = str_replace(['"', "\'", "\r\n", "\n", '\n', "\t"], ['\"', "'", '\n', '\n', '\n"' . "\n" . '"', '\t'], $T_string_text);
                if (strpos($T_string_text, "\n") !== false) { // Add empty new line before multiline string
                    $T_string_text = '"' . "\n" . '"' . $T_string_text;
                }
                if (! isset($translation_strings[$T_string_text])) { // Set array for each string in order to store the file paths where this string is found
                    $translation_strings[$T_string_text] = [];
                }
                if (! empty($TRANS_text) && ! isset($translation_strings[$T_string_text]['trans'])) { // Text of TRANS info
            ...
            if ($T_string_is_opened) { // Store chars of the current opened T_ string
                if ($prev_line_number != $line_number) { // String is multiline
                    if ($T_string_text != '') {
                        $T_string_text .= "\n";
                    }
            ...
            /************ TRANS info ************/
            if ($T_string_is_opened) { // T_ is opened skip this
                continue;
            }

            // Try to find TRANS
            $char_prev5 = translation_get_chars($line_string, $l, -5);
            if ($char == ':' && $char_prev5 == 'TRANS' && (strpos($line_string, '//') !== false || strpos($line_string, '/*') !== false)) { // TRANS info is opened
            ...
            if ($TRANS_is_opened &&
                ($char_next2 == '*/' ||
                  ($prev_line_number != $line_number &&
                    (strpos($line_string, 'T_') !== false || strpos($line_string, 'TS_') !== false)))) { // CLOSE TRANS info
                $TRANS_is_opened = false;
            }

            if ($TRANS_is_opened) { // Store chars of the current opened TRANS info
