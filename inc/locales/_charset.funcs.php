...
function evo_iconv_transliterate($str, $post_locale = null)
{
    ...
    if ($post_locale !== null) {	// Try to switch to the post locale:
        array_unshift($locales_to_try, str_replace('-', '_', $post_locale) . '.' . $lc_evo_charset);
    }

    if (setlocale(LC_CTYPE, $locales_to_try) === false) {	// The last thing we try is to use the system locale with our charset.
        if (($pos = strrpos($orig_lc_ctype, '.')) !== false) {	// Remove existing charset string:
            $syslocale = substr($orig_lc_ctype, 0, $pos);
        } else {
            $syslocale = $orig_lc_ctype;
        }

        if (setlocale(LC_CTYPE, $syslocale . '.' . $lc_evo_charset) === false) {	// We could not set a usable locale, giving up...
            return false;
        }
    }
...
function replace_special_chars($str, $post_locale = null, $accept_period = false, $separator = '-')
{
    ...
    if ($our_locale === null) {	// post locale is not set, try to guess current locale
        if (! empty($default_locale)) {
            $our_locale = $default_locale;
        }
        if (! empty($current_locale)) {	// Override with current locale if available
            $our_locale = $current_locale;
        }
    }
    if ($our_locale !== null && isset($locales[$our_locale]) && ! empty($locales[$our_locale]['transliteration_map'])) {	// Use locale 'transliteration_map' if present
        if (! array_key_exists('', $locales[$our_locale]['transliteration_map'])) {	// Make sure there's no empty string key, otherwise strtr() returns false
            if ($tmp_str = strtr($str, $locales[$our_locale]['transliteration_map'])) {	// Use newly transliterated string
                $str = $tmp_str;
            }
        }
    }

    if (($newstr = evo_iconv_transliterate($str, $post_locale)) !== false) {	// iconv allows us to get nice URL titles by transliterating non-ASCII chars.
    ...
    elseif (can_convert_charsets('UTF-8', $evo_charset) && can_convert_charsets('UTF-8', 'ISO-8859-1') /* source */) {	// Fallback to the limited old method: Transliterate only a few known chars.
        $newstr = convert_charset($str, 'UTF-8', $evo_charset);
        $newstr_charset = 'UTF-8';

        $search = ['Ä', 'ä', 'Ö', 'ö', 'Ü', 'ü', 'ß', 'à', 'ç', 'è', 'é', 'ì', 'ò', 'ô', 'ù']; // iso-8859-1
        $replace = ['Ae', 'ae', 'Oe', 'oe', 'Ue', 'ue', 'ss', 'a', 'c', 'e', 'e', 'i', 'o', 'o', 'u'];

        foreach ($search as $k => $v) { // convert $search to UTF-8
