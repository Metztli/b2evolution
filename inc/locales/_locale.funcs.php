...
// LOCALIZATION:
if (isset($use_l10n) && $use_l10n) { // We are going to use localization:
    /**
     * TRANSLATE!
     *
     ...
     */
    function T_($string, $req_locale = '', $params = [])
    {
        /**
        ...
        if (empty($req_locale)) { // By default we use the current locale
            if (empty($current_locale)) { // don't translate if we have no locale
                return $string;
            }
        ...
        if (! isset($trans[$messages])) { // Translations for current locale have not yet been loaded:
            ...
            if (! isset($trans[$messages])) { // Still not loaded... file doesn't exist, memorize that no translations are available
                // echo 'file not found!';
                $trans[$messages] = [];
            } else {
                if (! isset($trans[$messages]['__meta__'])) { // Unknown/old messages format (< version 1):
                    ...
                    foreach ($trans[$messages] as $k => $v) {
                        if (($pos = strpos($k, '\\')) === false) { // fast-path-skip
                            continue;
                        }
        ...
        if (! empty($params['add_transarray'])) {
            ...
            if (! isset(${$add_transarray_var}[$messages])) { // Additional translations for current locale have not yet been loaded:
                ...
                } else {	// Load our additional translation file.
                ...
                if (! isset(${$add_transarray_var}[$messages])) { // Still not loaded... file doesn't exist, memorize that no translations are available
                    // echo 'file not found!';
                    ${$add_transarray_var}[$messages] = [];
                } else {
                    if (! isset(${$add_transarray_var}[$messages]['__meta__'])) { // Unknown/old messages format (< version 1):
                        $Debuglog->add('Found deprecated messages format (no __meta__ info).', 'locale');
                        // Translate keys (e.g. 'foo\nbar') to real strings ("foo\nbar")
                        // Doing this here for all strings, is actually faster than doing it on key lookup (like it has been done before always)
        ...
        if (! empty($params['add_transarray']) && isset(${$add_transarray_var}[$messages][$search_string])) { // If the string has been translated:
            //$Debuglog->add( 'String ['.$string.'] found', 'locale' );
            $r = ${$add_transarray_var}[$messages][$search_string];
            if (isset(${$add_transarray_var}[$messages]['__meta__']['charset'])) { // new format: charset in meta data:
                $messages_charset = ${$add_transarray_var}[$messages]['__meta__']['charset'];
            } else { // old format.. extract charset from content type or fall back to setting from global locale definition:
        ...
        } elseif (isset($trans[$messages][$search_string])) { // If the string has been translated:
            //$Debuglog->add( 'String ['.$string.'] found', 'locale' );
            $r = $trans[$messages][$search_string];
            if (isset($trans[$messages]['__meta__']['charset'])) { // new format: charset in meta data:
        ...
        if ($params['for_helper']) { // translation is for the b2evoHelper object
     ...
     */
    function TB_($string, $req_locale = '', $params = [])
    {
        $params = array_merge([
     ...
     * @return string The translated string
     */
    function TD_($string, $req_locale = '', $params = [])
    {
...
} else { // We are not localizing at all:
    /**
     * @ignore
     */
    function T_($string, $req_locale = '', $params = [])
 ...
 */
function TS_($string, $req_locale = '', $params = [])
{
...
function locale_by_lang($lang, $fallback_to_default = true)
{
    ...
    foreach ($locales as $locale => $locale_params) {
        if (substr($locale, 0, 2) == $lang) { // found first matching locale
            return $locale;
        }
    }
...
function locale_options($default = '', $disp = true, $array_if_onelocale = false)
{
    ...
    if ($disp) {	// the result must be displayed
        echo $r;
    }

    if ($array_if_onelocale && $enabled_count == 1) {	// We've only one enabled locale:
        return [$enabled_lastkey, $locales[$enabled_lastkey]['name']];
...
function locale_from_httpaccept()
{
    ...
    if (isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
        ...
        foreach ($accept_list as $k => $v) {
            if ($v === '') { // should be kept in order
                $accept_list[$k] = $maxq--;
            } elseif ($v == 0) { // not acceptable (RFC 2616)
                unset($accept_list[$k]);
            }
        }
        arsort($accept_list);
        ...
        for ($i = 0, $n = count($accept_list); $i < $n; $i++) {
            ...
            if (isset($enabled_locales[$test])) { // the accepted locale is enabled and a full match
                return $enabled_locales[$test];
            }
            if (isset($short_locales[$test])) { // this is a short locale: find the best/first match in accepted locales
                $first = null;
                ...
                if (isset($first)) { // found exact match, via short locale.
                    return $enabled_locales[$accept_list[$first]];
                }
                if (isset($enabled_locales[$test . '-' . $test])) { // test for e.g. "de-DE" when only "de" is accepted
...
function locale_overwritefromDB()
{
    ...
    $usedprios = [];  // remember which priorities are used already.
    $priocounter = 0;
    $query = 'SELECT
    ...
    foreach ($DB->get_results($query, ARRAY_A) as $row) { // Loop through loaded locales:
        if ($row['loc_priority'] == $priocounter) { // priority conflict (the same)
            $priocounter++;
        } else {
        ...
        if (isset($locales[$row['loc_locale']], $locales[$row['loc_locale']]['charset'])) { // Use charset from locale file if such file exists on disk
            $loc_charset = $locales[$row['loc_locale']]['charset'];
        } else { // Use charset from DB
            $loc_charset = 'utf-8';
        }

        if (($ctrl == 'locales' || $ctrl == 'regional') && $row['loc_enabled'] && $loc_charset != 'utf-8') { // Check wrong non utf-8 locales in order to deactivate them
            $Messages->add(sprintf(T_('The locale %s has been deactivated because it\'s not UTF-8'), '<b>' . $row['loc_locale'] . '</b>'));
            if ($Settings->get('default_locale') == $row['loc_locale'] && $default_locale != $row['loc_locale']) { // Change main locale to default
    ...
    if (count($wrong_locales)) { // Deactivate locales that have wrong charset
        $DB->query('UPDATE T_locales
			  SET loc_enabled = 0
			WHERE loc_locale IN( ' . $DB->quote($wrong_locales) . ' )');
        if (! empty($wrong_default_locale)) { // If main locale is wring we sgould updated it to default and use this for all users and blogs that used this wrong locale
    ...
    foreach ($locales as $l_ley => $locale) { // Change priority of non-db locales to put them at the end of list
    ...
    if ($DB->num_rows != count($locales)) { // we have locales from conf file that need a priority
        $priocounter = 1;
        foreach ($locales as $lkey => $lval) { // Loop through memory locales:
            if (! isset($lval['priority'])) { // Found one that has no assigned priority
...
function locale_updateDB()
{
    ...
    foreach ($_POST as $pkey => $pval) {
        ...
        if ($matches[1] != $lnr) { // we have a new locale
            $lnr = $matches[1];
            $plocale = $pval;

            if ($templocales[$plocale]['enabled'] || ($plocale == $current_default_locale && ! isset($_POST['loc_' . $lnr . '_enabled']))) {	// Collect previously enabled locales AND if current default locale is not enabled yet:
            ...
            if ($lfield == 'startofweek' && ($pval < 0 || $pval > 6)) { // startofweek must be between 0 and 6
                continue;
            }
            if ($lfield == 'enabled') { // This locale is enabled, remove from the list of the disabled locales
                unset($disabled_locales[$plocale]);
            } elseif ($plocale == $current_default_locale && isset($disabled_locales[$plocale]) && ! isset($_POST['loc_' . $lnr . '_enabled'])) { // The default locale must be always enabled
                unset($disabled_locales[$plocale]);
    ...
    if ($disabled_locales && ($action != 'confirm_update')) { // Some locales were disabled, create warning message if required
        ...
        if (count($locale_users) > 0) {
            ...
            foreach ($locale_users as $user_locale => $locale_user_IDs) {
                ...
                foreach ($locale_user_IDs as $locale_user_ID) {
                    ...
                    $warning_message .= '<li>' . $locale_User->get_identity_link([
                        'user_tab' => 'userprefs',
                    ]) . '</li>';
                }
    ...
    foreach ($locales as $localekey => $lval) {
        if (empty($lval['messages'])) { // if not explicit messages file is given we'll translate the locale
            $lval['messages'] = strtr($localekey, '-', '_');
        }

        $transliteration_map = '';
        if (! empty($lval['transliteration_map'])) {
            if (is_string($lval['transliteration_map'])) {	// The value is already serialized and encoded
                $transliteration_map = $lval['transliteration_map'];
            } else {	// Encode the value
    ...
    $result = true;
    if ($action == 'confirm_update') { // Update users and blogs locale to the default if the prevously used locale was disabled
    ...
    if ($result && ($DB->query($query) !== false)) { // Commit after successful update
        $DB->commit();
        return true;
    }
...
function convert_charset($string, $dest_charset, $src_charset)
{
    ...
    if ($dest_charset == $src_charset || $dest_charset == '' /* may happen if $evo_charset is not defined yet */) {	// no conversation required
        if (isset($GLOBALS['Timer'])) {
    ...
    if (function_exists('mb_convert_variables')) {	// Convert by mbstring extension if it is enabled:
        mb_convert_variables($dest_charset, $src_charset, $string);
    } elseif (function_exists('iconv')) {	// Convert by iconv extension it it is enabled:
...
function can_convert_charsets($dest_charset, $src_charset)
{
    ...
    if (function_exists('mb_internal_encoding')) { // mb_string extension:
        $orig = mb_internal_encoding();

        $r = false;
        if (@mb_internal_encoding($dest_charset) && @mb_internal_encoding($src_charset)) { // we can set both encodings, so we should be able to convert:
            $r = true;
        }
...
function init_charsets($req_io_charset)
{
    ...
    if ($req_io_charset == $io_charset) { // no conversation/init needed
        return false;
    }

    // check, if we want to force a specific charset (e.g. 'utf-8'):
    if (! empty($force_io_charset_if_accepted)) { // we want to force a specific charset:
    ...
    if (empty($evo_charset)) { // empty evo_charset follows I/O charset:
        // TODO: $evo_charset will not follow, if it has followed before.. (because not empty anymore)
        $Debuglog->add('$evo_charset follows $io_charset (' . $io_charset . ').', ['locale']);
        $evo_charset = $io_charset;
    } elseif ($evo_charset != $io_charset) { // we have to convert for I/O
        // TODO: dh> $io_charset has to forcefully follow $evo_charset, if we cannot convert, e.g. utf-8/iso-8859-1
        if (! function_exists('mb_convert_encoding')) {
            $Debuglog->add('$evo_charset differs from $io_charset, but mbstrings is not available - cannot convert I/O to internal charset!', ['errors', 'locale']);
            $evo_charset = $io_charset; // we cannot convert I/O to internal charset
        } else { // check if the encodings are supported:
            ...
            if (! @mb_internal_encoding($io_charset)) {
                $Debuglog->add('Cannot I/O convert because I/O charset [' . $io_charset . '] is not supported by mbstring!', ['error', 'locale']);
                $evo_charset = $io_charset;
                mb_internal_encoding($old_mb_internal_encoding);
            } elseif (! @mb_internal_encoding($evo_charset)) {
                $Debuglog->add('Cannot I/O convert because $evo_charset=' . $evo_charset . ' is not supported by mbstring!', ['error', 'locale']);
                $evo_charset = $io_charset;
                mb_internal_encoding($old_mb_internal_encoding);
            } else { // we can convert between I/O
...
function locales_load_available_defs()
{
    ...
    if (! empty($locales_loaded_form_disk)) { // All locales have been already loaded
        return;
    }
    ...
    foreach ($locale_folders as $locale_folder) {
        ...
        if ($locale_def_files === false) {	// Impossible to read the locale folder:
        ...
        foreach ($locale_def_files as $locale_def_file) {	// Check if it's a definition file:
            // pre_dump( $locale_def_file );
            if (preg_match('~[a-z0-9\-.]\.locale\.php$~i', $locale_def_file)) {	// We found a definition file:
                // pre_dump( $locale_def_file );
    ...
    foreach ($locale_defs as $locale_code => $locale_def) {
        if (! isset($locales[$locale_code])) {	// New locale, add to main array:
...
function locale_file_po_info($po_file_name, $calc_percent_done = false)
{
    ...
    if (file_exists($po_file_name)) {
        ...
        foreach ($lines as $line) {
            if (trim($line) == '') {	// Blank line, go back to base status:
                if ($status == 't') {	// ** End of a translation ** :
                    if ($msgstr == '') {
                        $untranslated++;
                    ...
                    if ($msgid == '' && $this_fuzzy) {	// It's OK if first line is fuzzy
                        $fuzzy--;
                    }
                    $msgid = '';
                    $msgstr = '';
                ...
                $status = '-';
            } elseif (($status == '-') && preg_match('#^msgid "(.*)"#', $line, $matches)) {	// Encountered an original text
                $status = 'o';
                $msgid = $matches[1];
                // echo 'original: "', $msgid, '"<br />';
                $all++;
            } elseif (($status == 'o') && preg_match('#^msgstr "(.*)"#', $line, $matches)) {	// Encountered a translated text
                $status = 't';
                $msgstr = $matches[1];
                // echo 'translated: "', $msgstr, '"<br />';
            } elseif (preg_match('#^"(.*)"#', $line, $matches)) {	// Encountered a followup line
                if ($status == 'o') {
                    $msgid .= $matches[1];
                } elseif ($status == 't') {
                    $msgstr .= $matches[1];
...
function locale_file_po_percent_done($po_file_info, $pot_file)
{
    ...
    if (! isset($messages_pot_file_info[$pot_file])) {	// Initialize a file info for the main language file if it doesn't yet set
        global $locales_path;
...
function locale_insert_default()
{
    ...
    if (! empty($install_test_features)) {	// Activate also additional locales on install:
    ...
    if (! empty($current_locale)) { // Make sure the user sees his new system localized.
        $activate_locales[] = $current_locale;
    }

    $activate_locales = array_unique($activate_locales);

    if (! empty($activate_locales)) { // Insert locales into DB
        ...
        foreach ($activate_locales as $a_locale) {
            if (! isset($locales[$a_locale]) || in_array($a_locale, $existing_locales)) { // Skip an incorrect locale or it already exists in DB
                continue;
            }
            ...
            $insert_data[] = '( ' . $DB->quote($a_locale) . ', '
                ...
                . $DB->quote(empty($locales[$a_locale]['extdatefmt']) ? str_replace(['.', ',', '\\', '/'], ' ', str_replace(['y', 'm'], ['Y', 'M'], $locales[$a_locale]['datefmt'])) : $locales[$a_locale]['extdatefmt']) . ', '
        ...
        if (count($insert_data)) { // Do insert only if at least one locale requires this
...
function locale_restore_defaults($restored_locales)
{
    ...
    if (empty($restored_locales) && ! is_array($restored_locales)) {	// No locales to restore, Exit here:
        return;
    }

    foreach ($restored_locales as $restored_locale_key) {
        if (! isset($locales[$restored_locale_key])) {	// Skip an incorrect locale:
            continue;
        }
        ...
        $DB->query('UPDATE T_locales SET
			...
			loc_extdatefmt          = ' . $DB->quote(empty($restored_locale['extdatefmt']) ? str_replace(['.', '-', '\\', '/'], ' ', str_replace(['y', 'm'], ['Y', 'M'], $restored_locale['datefmt'])) : $restored_locale['extdatefmt']) . ',
...
function locale_check_default()
{
    ...
    if (isset($locales[$current_default_locale]) && $locales[$current_default_locale]['enabled']) {	// Current default locale is correct and enabled
        // Nothing to fix, Exit here:
        return;
    }

    if (! isset($locales[$current_default_locale])) {	// If current default locale is wrong and doesn't exist anymore then use default locale from config:
    ...
    if ($current_default_locale_enabled !== null) {	// Current default locale exists in DB
        if (! $current_default_locale_enabled) {	// Enable current default locale if it is not enabled yet:
            $DB->query('UPDATE T_locales
				  SET loc_enabled = 1
				WHERE loc_locale = ' . $DB->quote($current_default_locale));
        }
    } else {	// No record of default locale
        // Make sure default transliteration_map is set
        $transliteration_map = '';
        ...
        $DB->query('INSERT INTO T_locales '
            ...
            . $DB->quote(empty($locales[$current_default_locale]['extdatefmt']) ? str_replace(['.' . '-', '\\', '/'], ' ', str_replace(['y', 'm'], ['Y', 'Y'], $locales[$current_default_locale]['datefmt'])) : $locales[$current_default_locale]['extdatefmt']) . ', '
...
function locale_get($field, $locale = null, $default = null)
{
    ...
    if ($locale == '#' || empty($locales[$locale][$field])) {
        ...
        switch ($field) {
            ...
            case 'extdatefmt':
                if (isset($locales[$locale]['datefmt'])) {
                    return str_replace(['.', '-', '\\', '/'], ' ', str_replace(['m', 'y'], ['M', 'Y'], $locales[$locale]['datefmt']));
                } else {
...
function locale_currency($locale = '#', $return = 'code')
{
    ...
    if (! ($locale_Country = &$CountryCache->get_by_name($country_code, false, false))) {	// Couldn't detect country by code:
        return false;
    }

    if (! ($country_Currency = &$locale_Country->get_Currency())) {	// Couldn't detect currency for the locale country:
        return false;
    }
