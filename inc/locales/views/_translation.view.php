...
function filter_translation(&$Form)
{
    ...
    if (get_param('untranslated_only')) {
        $Form->text_input('translated', null, 20, T_('Translated string'), '', [
            'disabled' => 'disabled',
        ]);
    } else {
...
if (check_user_perm('options', 'edit')) {
    ...
    if (is_file($locales_path . $locales[$edit_locale]['messages'] . '/LC_MESSAGES/messages.po')) {
        $buttons[] = ['submit', 'actionArray[import_po]', T_('Import .PO file into DB'), 'SaveButton'];
    }
    $buttons[] = ['submit', 'actionArray[generate_po]', T_('Generate .PO file'), 'SaveButton'];
