...
if (param('pqst_ID', 'integer', '', true)) {	// Load poll:
    $PollCache = &get_PollCache();
    if (($edited_Poll = &$PollCache->get_by_ID($pqst_ID, false)) === false) {	// We could not find the poll to edit:
        unset($edited_Poll);
...
if (param('popt_ID', 'integer', '', true)) {	// Load poll option:
    $PollOptionCache = &get_PollOptionCache();
    if (($edited_PollOption = &$PollOptionCache->get_by_ID($popt_ID, false)) === false) {	// We could not find the poll option to edit:
...
switch ($action) {
    ...
    case 'create':
        ...
        if ($edited_Poll->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'create_option':
        ...
        // load data from request
        if ($edited_PollOption->load_from_Request($edited_Poll->ID)) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'update_option':
        ...
        // load data from request:
        if ($edited_PollOption->load_from_Request()) {	// We could load data from form without errors:
            // Update poll in DB:
...
switch ($action) {
    ...
    case 'delete_option':
        ...
        $edited_PollOption->confirm_delete(
            ...
            'poll',
            $action,
            array_merge(get_memorized('action'), [
                'action' => 'edit',
            ])
        );
        // no break
