...
class Poll extends DataObject
{
    ...
    public function __construct($db_row = null)
    {
        ...
        if ($db_row != null) {	// Get poll data from DB:
        ...
        } else {	// Set default poll data for new poll:
            if (is_logged_in()) {
                global $current_User;
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_polls__option',
                'fk' => 'popt_pqst_ID',
                'msg' => T_('%d poll options'),
            ],
            [
    ...
    public function load_from_Request()
    {
        ...
        // Owner:
        if (check_user_perm('polls', 'edit')) {	// Update the owner if current user has a permission to edit all polls:
            ...
            if (! empty($pqst_owner_login)) {	// If the login is entered:
                $UserCache = &get_UserCache();
                $owner_User = &$UserCache->get_by_login($pqst_owner_login);
                if (empty($owner_User)) {	// Wrong entered login:
        ...
        } elseif (empty($this->ID)) {	// Set owner user ID on creating new poll:
            $this->set('owner_user_ID', $current_User->ID);
        ...
        if (empty($this->ID)) {
            ...
            foreach ($answer_options as $answer_option_text) {
                if (empty($answer_option_text)) {	// Skip empty option:
                    continue;
                }
    ...
    public function dbinsert()
    {
        ...
        if (parent::dbinsert()) { // The poll was inserted successful:
            ...
            foreach ($answer_options as $answer_option_text) {
                ...
                if (! $new_PollOption->dbinsert()) {	// Rollback if the poll option could not be inserted:
                    $DB->rollback();
                    return false;
                }
    ...
    {
        if (! isset($this->owner_User)) {	// Get the owner User only first time:
            $UserCache = &get_UserCache();
     ...
     */
    public function get_vote_nums($type = null)
    {
        global $DB;

        if ($this->vote_nums === null) {	// Get from DB once and store in cache array:
        ...
        if ($type === null) {	// Get all numbers:
            return $this->vote_nums;
        } else {	// Get a specific number:
    ...
    {
        if (empty($this->ID)) {	// New creating poll doesn't have the options, Return empty array:
            return [];
        }

        if ($this->poll_options === null) {	// Get options from DB only first time, and store them in cache array:
            global $DB;

            // Get numbers of votes and voters for the edited poll:
            $poll_voters_num = $this->get_vote_nums('voters');
            if ($poll_voters_num == 0) {	// To don't devide by zero
                $poll_voters_num = 1;
            }
     ...
     */
    public function get_max_poll_options_percent()
    {
        ...
        foreach ($poll_options as $poll_option) {	// Find max percent in all poll options:
     ...
     * @param integer User ID of specific user
     */
    public function clear_user_votes($user_ID = null)
    {
        global $current_User, $DB;
