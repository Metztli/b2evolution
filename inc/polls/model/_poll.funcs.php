...
function poll_td_question($Poll)
{
    ...
    if (check_user_perm('polls', 'view', false, $Poll)) {	// Display the question text as link to view the details:
...
function poll_td_actions($Poll)
{
    ...
    if (check_user_perm('polls', 'edit', false, $Poll)) {	// Display the action icons to edit and delete the poll:
    ...
    } elseif (check_user_perm('polls', 'view', false, $Poll)) {	// Display the action icons to view the poll:
...
function poll_filters_callback(&$Form)
{
    ...
    if (! empty($poll_owner_login)) {	// Get user by login:
        $UserCache = &get_UserCache();
        $poll_filter_User = &$UserCache->get_by_login($poll_owner_login);
    } else {	// No filter by owner:
 ...
 * @param array Params
 */
function polls_results_block($params = [])
{
    global $admin_url, $DB;
    ...
    if (! check_user_perm('polls', 'create')) {	// Check minimum permission:
        return;
    }
    ...
    if (! empty($params['edited_User'])) {	// Use a filter user ID from params:
        $edited_User = $params['edited_User'];
        $poll_filter_user_ID = $edited_User->ID;
    } else {	// Get a filter by poll owner from request:
        $poll_filter_user_ID = null;
        $poll_owner_login = param('owner', 'string', '', true);
        if (! empty($poll_owner_login)) {	// Get user by login:
            $UserCache = &get_UserCache();
            if ($poll_filter_User = &$UserCache->get_by_login($poll_owner_login)) {	// Set user ID to filter then below:
                $poll_filter_user_ID = $poll_filter_User->ID;
            } else {	// Set this to display empty list when the netered login is wrong:
    ...
    if (! $perm_poll_view) {	// If current user has no permission to view all polls, Display only the owner's polls:
        $SQL->WHERE('pqst_owner_user_ID = ' . $DB->quote($current_User->ID));
    } elseif ($poll_filter_user_ID !== null) {	// Filter by owner:
    ...
    if ($perm_poll_view && $params['display_filters']) {	// Allow to filter by owner only if current user has a perm to view the polls of all users:
        $polls_Results->filter_area = [
    ...
    if ($params['display_owner']) {	// Display owner:
        $polls_Results->cols[] = [
            'th' => T_('Owner'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'shrinkwrap',
    ...
    if ($params['display_btn_add']) {	// Display button to add new poll:
        $polls_Results->global_icon(T_('New poll'), 'new', $admin_url . '?ctrl=polls&action=new', T_('New poll') . ' &raquo;', 3, 4, [
            'class' => 'action_icon btn-primary',
        ]);
    }

    if ($params['display_btn_user_del'] && isset($edited_User) && $polls_Results->get_total_rows() > 0) {	// Display button to delete user polls:
