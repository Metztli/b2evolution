...
class Timer
{
     ...
     * Start a timer.
     */
    public function start($category, $log = true)
    {
        $this->reset($category);
     ...
     * @return boolean false, if the timer had not been started.
     */
    public function pause($category, $log = true)
    {
        global $Debuglog;
        ...
        if ($this->get_state($category) != 'running') { // Timer is not running!
     ...
     */
    public function resume($category, $log = true)
    {
        //global $Debuglog;
     ...
     * @return string
     */
    public function get_duration($category, $decimals = 3)
    {
     ...
     */
    public function log_duration($category)
    {
     ...
     */
    public function get_microtime($category)
    {
     ...
     */
    public function get_state($category)
    {
        if (! isset($this->_times[$category])) {
            return 'unknown';
        }
