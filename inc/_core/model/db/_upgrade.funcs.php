 ...
 */
function db_delta($queries, $exclude_types = [], $execute = false)
{
    ...
    foreach ($queries as $qry) {
        ...
        if (preg_match('|^(\s*CREATE TABLE\s+)(IF NOT EXISTS\s+)?([^\s(]+)(.*)$|is', $qry, $match)) {
            ...
            $items[strtolower($tablename)][] = [
                'queries' => [$qry],
                'note' => sprintf('Created table &laquo;<strong>%s</strong>&raquo;', $tablename),
                'type' => 'create_table',
            ];
        } elseif (preg_match('|^\s*CREATE DATABASE\s([\S]+)|i', $qry, $match)) { // add to the beginning
            array_unshift($items, [
                'queries' => [$qry],
            ...
            $items[strtolower($tablename)][] = [
                'queries' => [$match[1] . $tablename . $match[3]],
                'note' => '',
                'type' => 'insert',
            ];
    ...
    foreach ($tables as $table) { // For every table in the database
        $table_lowered = strtolower($table);  // table names are treated case insensitive

        if (! isset($items[$table_lowered])) { // This table exists in the database, but not in the creation queries.
            continue;
        }
        ...
        if (($wanted_engine = get_engine_from_query($items[$table_lowered][0]['queries'][0])) !== false) {
            ...
            if (strtolower($current_engine) != strtolower($wanted_engine)) {
                $items[$table_lowered][] = [
                    'queries' => ['ALTER TABLE ' . $table . ' ENGINE=' . $wanted_engine],
        ...
        $prev_fld = '';
        foreach ($flds as $create_definition) { // For every field line specified in the query
            ...
            if (in_array($fieldname_lowered, ['', 'primary', 'foreign', 'index', 'fulltext', 'unique', 'key'])) { // INDEX (but not in column_definition - those get handled later)
                $add_index = [
                ...
                if (! preg_match('~^(PRIMARY(?:\s+KEY)|(?:FULLTEXT|UNIQUE)(?:\s+(?:INDEX|KEY))?|KEY|INDEX) (?:\s+()     (\w+)      )? (\s+USING\s+\w+)? \s* \((.*)\)$~ix', $create_definition, $match)
                    ...
                    && ! preg_match('~^(FOREIGN\s+KEY) \s* \((.*)\) \s* (REFERENCES) \s* ([^( ]*) \s* \((.*)\) \s* (.*)$~ixs', $create_definition, $match)) { // invalid type, should not happen
                    debug_die('Invalid type in $indices: ' . $create_definition);
                    // TODO: add test: Invalid type in $indices: KEY "coord" ("lon","lat")
                }

                if ($fieldname_lowered == 'foreign') { // Remember FOREIGN KEY fields, but they don't have to be indexed
                    ...
                    $foreign_key_fields[] = [
                        ...
                        'reference_columns' => $match[5],
                        'fk_definition' => $match[6],
                        'create' => true,
                    ];
                    continue;
                }
                ...
                if ($fieldname_lowered == 'primary') { // Remember PRIMARY KEY fields to be indexed (used for NULL check)
                ...
                $indices[] = $add_index;
            } else { // "normal" field, add it to the field array
                $wanted_fields[strtolower($fieldname_lowered)] = [
        ...
        if (! empty($tableindices)) {
            ...
            foreach ($tableindices as $tableindex) {
                ...
                $existing_indices[$keyname]['columns'][] = [
        ...
        foreach ($wanted_fields as $fieldname_lowered => $field_info) {
            ...
            if (preg_match('~ \b UNIQUE (?:\s+ KEY)? \b ~ix ', $parse, $match)) { // This has an "inline" UNIQUE index:
                if (! is_in_quote($parse, ' ' . $match[0])) { // isn't between quotation marks, so it must be a primary key.
                    $indices[] = [
                        'name' => $fieldname_lowered,
                        'is_PK' => false,
                        'create_definition' => null, // "inline"
                        'col_names' => [$fieldname_lowered],
                        'keyword' => null,
            ...
            if (preg_match('~ \b (PRIMARY\s+)? KEY \b ~ix', $parse, $match)) { // inline PK:
                // Check if this key is between quotation marks
                if (! is_in_quote($parse, ' ' . $match[0])) { // it isn't between quotation marks, so it must be a primary key.
                    $indices[] = [
                        'name' => 'PRIMARY',
                        'is_PK' => true,
                        'create_definition' => null, // "inline"
                    ...
                    $fields_with_keys[] = $fieldname_lowered;
                    $primary_key_fields = [$fieldname_lowered];
        ...
        foreach ($existing_indices as $index_name => $index_data) {
            ...
            */
            ...
            foreach ($indices as $k => $index) {
                ...
                if (! preg_match('~^\w+\s+[^(]~', $index['create_definition'], $match)) { // no key name given, make the name part optional, if it's the default one:
                    ...
                    if ($auto_key == $index_name) { // the auto-generated keyname is the same as the one we have, so make it optional in the pattern:
                        $pattern .= '?';
                    }
                }
                ...
                if (preg_match('~' . $pattern . '~i', trim($index['create_definition']))) { // This index already exists: remove the index from our indices to create
                    unset($indices[$k]);
        ...
        foreach ($existing_indices as $l_key_name => $l_key_info) {
            ...
            if ($l_key_name == 'PRIMARY') { // Remember _existing_ PRIMARY KEYs
        ...
        if (! in_array('drop_column', $exclude_types)) {
            ...
            if (! $at_least_one_col_stays) { // all columns get dropped: so we need to DROP TABLE and then use the original CREATE TABLE
                array_unshift($items[$table_lowered], [
        ...
        foreach ($tablefields as $tablefield) {
            ...
            if (! isset($wanted_fields[$fieldname_lowered])) { // This field exists in the table, but not in the creation queries
                if (in_array('drop_column', $exclude_types)) {
                    if (preg_match('~\bAUTO_INCREMENT\b~i', $tablefield->Extra)) { // must be modified with a ADD COLUMN which drops a PK
                        $obsolete_autoincrement = $tablefield;
                    }
                } else {
                    $items[$table_lowered][] = [
                        'queries' => ['ALTER TABLE ' . $table . ' DROP COLUMN ' . $tablefield->Field],
            ...
            if (preg_match('~^' . $pattern_field . '\s+ (TINYINT|SMALLINT|MEDIUMINT|INTEGER|INT|BIGINT|REAL|DOUBLE|FLOAT|DECIMAL|DEC|NUMERIC) ( \s* \([\d\s,]+\) )? (\s+ UNSIGNED)? (\s+ ZEROFILL)? (.*)$~ix', $column_definition, $match)) {
                ...
                if ($fieldtype == 'INTEGER') { // synonym
                    $fieldtype = 'INT';
                } elseif ($fieldtype == 'DEC') { // synonym
                    $fieldtype = 'DECIMAL';
                }

                if (isset($match[2])) { // append optional "length" param (trimmed)
                    $fieldtype .= preg_replace('~\s+~', '', $match[2]);
                }
                if (! empty($match[3])) { // "unsigned"
                    $fieldtype .= ' ' . trim($match[3]);
                }
                if (! empty($match[4])) { // "zerofill"
                ...
                if (! empty($match[3])) { // "binary", "ascii", "unicode"
                    $fieldtype .= ' ' . $match[3];
                }
                $field_to_parse = $match[5];

                if (strtoupper($match[1]) == 'VARCHAR') {
                    if ($len < 4) { // VARCHAR shorter than 4 get converted to CHAR (but reported as VARCHAR in MySQL 5.0)
                        $type_matches = preg_match('~^(VAR)?CHAR\(' . $len . '\)' . ($match[3] ? ' ' . $match[3] : '') . '$~i', $tablefield->Type);
                    }
                } elseif ($has_variable_length_field && strtoupper($match[1]) == 'CHAR') { // CHARs in a row with variable length fields get silently converted to VARCHAR (but reported as CHAR in MySQL 5.0)
            ...
            if (preg_match('~^(.*) \b (?: (UNIQUE) (?:\s+ (?:INDEX|KEY))? | (?:PRIMARY \s+)? KEY ) \b (.*)$~ix', $field_to_parse, $match)) { // fields got added to primary_key_fields and fields_with_keys before
                // Check if this key is between quotation marks
                if (! has_open_quote($match[1])) { // is not between quotation marks, so it must be a key.
            ...
            if (preg_match('~(.*?) \b AUTO_INCREMENT \b (.*)$~ix', $field_to_parse, $match)) {
                ...
                if (! preg_match('~\bAUTO_INCREMENT\b~i', $tablefield->Extra)) { // not AUTO_INCREMENT yet
                    $type_matches = false;
                }

                if (! in_array($fieldname_lowered, $fields_with_keys)) { // no KEY defined (but required for AUTO_INCREMENT fields)
                ...
                } else { // a key for this AUTO_INCREMENT field does not exist yet, we search it in $indices
                    foreach ($indices as $k_index => $l_index) { // go through the indexes we want to have
                        if (array_search($fieldname_lowered, $l_index['col_names']) === false) { // this is not an index for our column
                            continue;
                        }
                        ...
                        if ($l_index['is_PK']) { // Part of a PRIMARY key..
                            if (! empty($existing_primary_fields)) { // and a PRIMARY key exists already
                                $column_definition .= ', DROP PRIMARY KEY';
                                unset($obsolete_indices['PRIMARY']);
                            }
                            $existing_primary_fields = []; // we expect no existing primary key anymore
                            $primary_key_fields = $l_index['col_names']; // this becomes our primary key
                        } elseif (isset($existing_indices[$l_index['name']])) { // this index already exists, drop it:
                            ...
                            if (! in_array($fieldname_lowered, $fields_with_keys)) { // add te field to the list of keys we want/expect to have:
                                $fields_with_keys[] = $fieldname_lowered;
                            }
                        }
            ...
            if (preg_match('~(.*?) \b (NOT\s+)? NULL \b (.*)$~ix', $field_to_parse, $match)) { // if "NOT" not matched it's NULL
                $want_null = empty($match[2]);
                $field_to_parse = $match[1] . $match[3];
            } else { // not specified: "NULL" is default
                $want_null = true;
            }

            if (in_array($fieldname_lowered, $primary_key_fields) || $is_auto_increment) { // If part of PRIMARY KEY or AUTO_INCREMENT field "NULL" is implicit
                $change_null = false; // implicit NULL
                $want_null = 'IMPLICIT';
            } elseif (in_array($fieldname_lowered, $existing_primary_fields) && ! in_array($fieldname_lowered, $primary_key_fields)) { // the field was in PRIMARY KEY, but is no longer. It should get altered only if we want "NOT NULL"
            ...
            if (! isset($type_matches)) { // not tried to match before
            ...
            } else { // implicit default, see http://dev.mysql.com/doc/refman/4.1/en/data-type-defaults.html
                if (preg_match('~^(TINYINT|SMALLINT|MEDIUMINT|INTEGER|INT|BIGINT|REAL|DOUBLE|FLOAT|DECIMAL|DEC|NUMERIC)~', $fieldtype)) { // numeric
                    $update_default = '0';
                    $update_default_set = '0';
                } elseif ($fieldtype == 'TIMESTAMP') { // TODO: the default should be current date and time for the first field - but AFAICS we won't have NULL fields anyway
            ...
            if (! $type_matches || $change_null) { // Change the whole column to $column_definition:
                /*
                echo '<h2>No_Match</h2>';
                ...
                */
                ...
                if ($change_null && ! $want_null && isset($update_default_set)) { // Prepend query to update NULL fields to default
                    ...
                    if (substr($tablefield->Type, 0, 5) == 'enum(') {
                        ...
                        if (! in_array($update_default, $existing_enum_field_values)) { // we cannot update straight to the new default, because it does not exist yet!
            ...
            } else { // perhaps alter or drop DEFAULT:
                if ($want_default !== false) { // DEFAULT given
                    ...
                    if ($existing_default != $want_default) { // DEFAULT is case-sensitive
                        ...
                        $item_queries = [];
                        if ($existing_default == 'CURRENT_TIMESTAMP') {	// MySQL command "SET DEFAULT" does NOT upgrade a column with default value CURRENT_TIMESTAMP by some reason,
                            // So we must use the command "MODIFY" to update a column completely:
                            $item_queries[] = 'ALTER TABLE ' . $table . ' MODIFY ' . $column_definition;
                        } else {	// Use "SET DEFAULT" for all other normal column types:
                ...
                } elseif (! empty($tablefield->Default) && $tablefield->Default != $update_default) { // No DEFAULT given, but it exists one, so drop it (IF not a TIMESTAMP or DATETIME field)
                    if ($tablefield->Type != 'timestamp' && $tablefield->Type != 'datetime') {
                        $items[$table_lowered][] = [
                            'queries' => ['ALTER TABLE ' . $table . ' ALTER COLUMN ' . $tablefield->Field . ' DROP DEFAULT'],
        ...
        foreach ($wanted_fields as $fieldname_lowered => $fielddef) { // For every remaining field specified for the table
            ...
            if (preg_match('~^(.*) \b (?: (UNIQUE) (?:\s+ (?:INDEX|KEY))? | (?:PRIMARY \s+)? KEY ) \b (.*)$~ix', $column_definition, $match) // "has_inline_primary_key"
                    ...
                    && ! in_array($fieldname_lowered, $existing_primary_fields)) { // the column is part of the PRIMARY KEY, which needs to get dropped before (we already handle that for AUTO_INCREMENT fields)
        ...
        foreach ($result as $foreign_key_update) { // loop through foreign key differences in this table
            if ($foreign_key_update['type'] == 'alter_engine') { // this is an alter engine command, check if this command was already added during engine difference detection, and skip to the next if it was added
                $skip = false;
                foreach ($items[$table_lowered] as $itemlist) {
                    if (($itemlist['type'] == 'alter_engine') && ($itemlist['queries'][0] == $foreign_key_update['queries'][0])) { // the same command was already added, don't add again
                        $skip = true;
                        break;
                    }
                }
                if ($skip) { // skip is set, don't add the alter engine command again
                    continue;
                }
            }
            // add FK updates
        ...
        $add_index_queries = [];
        foreach ($indices as $k => $index) {
            if (empty($index['create_definition'])) { // skip "inline"
                continue;
            }
            ...
            $query = [
                'queries' => [$query . ' ADD ' . $index['create_definition']],
            ...
            if ($append_after_drops) { // do this after any DROPs (i.e. KEY name changes)
                $add_index_queries[] = $query;
            } else { // this needs to get done before any other DROPs
        ...
        foreach ($obsolete_indices as $index_info) {
            ...
            $items[$table_lowered][] = [
                'queries' => ["ALTER TABLE {$table} DROP " . ($index_info['name'] == 'PRIMARY' ? 'PRIMARY KEY' : 'INDEX ' . $index_info['name'])],
    ...
    if (! empty($exclude_types)) {
        foreach ($items as $table => $itemlist) {
            ...
            foreach ($itemlist as $k => $item) {
                if (in_array($item['type'], $exclude_types)) { // this type of update should be excluded
                    if ($item['type'] == 'drop_index') { // drop index command should not be excluded in case when we would like to update an index!
                        $skip = false;
                        foreach ($itemlist as $other_item) { // check if there are an add_index command for the same table with the same index name
                            if (($other_item['type'] == 'add_index') && (strcasecmp($item['name'], $other_item['name']) === 0)) { // add index with the same index name was found so we need to process this drop_index command to be able to add a new correct index with the same name
                                $skip = true;
                                break;
                            }
                        }
                        if ($skip) { // skip excluding this item
                            continue;
                        }
                    }
                    unset($items[$table][$k]);
    ...
    foreach ($items as $table => $itemlist) {
        ...
        for ($i = 0; $i < count($itemlist); $i++) {
            if (($itemlist[$i]['type'] == 'alter_engine') && ($alter_engine_index == null)) { // save alter engine query index
                $alter_engine_index = $i;
            } elseif (($itemlist[$i]['type'] == 'drop_foreign_key') && ($alter_engine_index !== null) && ($alter_engine_index < $i)) { // switch engine update and drop foreign key queries, because in many cases we must drop the foreign key first to be able to chagne the table engine
...
function db_delta_remove_quotes($fieldname, $quotes = '`"')
{
    ...
    for ($i = 0; $i < $quotes_len; $i++) {
        $char = $quotes[$i];
        if (substr($fieldname, 0, 1) == $char && substr($fieldname, -1) == $char) { // found quotes:
...
function db_delta_foreign_keys($foreign_key_fields, $table, $silent = true, $action = null)
{
    ...
    if (! empty($foreign_key_fields) && ($action != 'drop')) { // we have foreign key contraints, and we don't want to drop that ( In case of drop FK the table engine doesn't matter )
        // which tables engine must be changed
        $modify_table_eninges = [
            $table => 'InnoDB',
        ];
        foreach ($foreign_key_fields as $foreign_key) { // check reference tables engine
    ...
    $existing_foreign_key_fields = [];
    $db_fieldlines = get_fieldlines_from_query($ct_sql);
    foreach ($db_fieldlines as $fieldname => $fieldline) { // loop through all field lines and get those lines where are foreign key definitions
        $fieldline = str_replace(['`', '"'], '', $fieldline);
        if (preg_match('~^(?:(CONSTRAINT)* \s* ([^ ]*)) \s* (FOREIGN\s+KEY) \s* \((.*)\) \s* (REFERENCES) \s* ([^( ]*) \s* \((.*)\) \s* (.*)$~ixs', $fieldline, $match)) { // add existing foreign key fields, but set drop param to true only if we don't want to add/drop a single foreign key!
            $existing_foreign_key_fields[] = [
                'fk_symbol' => $match[2],
                'fk_fields' => $match[4],
                'reference_table' => $match[6],
                'reference_columns' => $match[7],
                'fk_definition' => $match[8],
                'drop' => ($action == null),
            ];
        }
    }

    foreach ($existing_foreign_key_fields as &$existing_foreign_key) { // loop through existing foreign key definitions
        foreach ($foreign_key_fields as &$foreign_key) { // loop through the install script foreign key definitions
            if (($action == null) && (! $existing_foreign_key['drop']) && (! $foreign_key['create'])) { // if we already know that an old key should not be removed, and the recent key already exists skip this check
                continue;
            }
            ...
            if ((! empty($action)) && $match_found) { // action is not empty it means that we would like to add or drop a single foreign key
                if ($action == 'drop') { // set existing foreign key to be droped
                    $existing_foreign_key['drop'] = true;
                    break;
                } elseif ($action = 'add') { // add a new foreign key
                    if ($exact_match_found) { // Exact match found so the FK already exists with the same definition
                        return;
                    }
        ...
        if (($action !== null) && ($existing_foreign_key['drop'])) { // in case of add/drop a single foreign key, if we have found a match, then we don't have to look forward
            break;
        }
    }
    ...
    foreach ($existing_foreign_key_fields as $existing_foreign_key) { // loop through existing foreign keys
        if ($existing_foreign_key['drop']) { // this foreign key constraint should be removed, create the query
            $query = 'ALTER TABLE ' . $table . ' DROP FOREIGN KEY ' . $existing_foreign_key['fk_symbol'];
            if ($silent) { // execute query in silent mode
                $DB->query($query);
            } else { // set query definition
                $result[] = [
                    'queries' => [$query],
    ...
    foreach ($foreign_key_fields as $foreign_key) { // loop through in up to date foreign keys
        if ($foreign_key['create']) { // // this foreign key constraint is new, it must be created
            ...
            if ($silent) { // execute queries in silent mode
                if ($DB->query($delete_query) !== false) { // orphan child entries have been deleted, create foreign key
                    $DB->query($query);
                }
            } else { // set query definition
                $result[] = [
...
function db_delta_table_engines($tables, $silent)
{
    ...
    if (empty($tables)) { // no tables to check
        return null;
    }

    $modify_engine_queries = [];
    foreach ($tables as $table => $engine) {
        // get table engine from db
        $current_engine = $DB->get_row('SHOW TABLE STATUS LIKE ' . $DB->quote($table));
        $current_engine = $current_engine->Engine;
        if (strtolower($current_engine) != strtolower($engine)) { // table engine is not the expected one
            $modify_engine_queries[] = [
                'queries' => ['ALTER TABLE ' . $table . ' ENGINE=' . $engine],
...
function install_make_db_schema_current($display = true)
{
    ...
    foreach ($schema_queries as $table => $query_info) {
        ...
        $items_need_update = db_delta($query_info[1], ['drop_column', 'drop_index'], false);

        if (empty($items_need_update)) {
            continue;
        }

        if (! $display) { // just execute queries
            ...
            foreach ($items_need_update as $table => $itemlist) {
                if (count($itemlist) == 1 && $itemlist[0]['type'] == 'create_table') {
                    ...
                    evo_flush();
                    foreach ($itemlist[0]['queries'] as $query) { // should be just one, but just in case
                        if ($debug >= 2) {
                            pre_dump($query);
                        }
                        $DB->query($query);
...
function is_in_quote($subject, $needle)
{
    ...
    if ($length === false) { // needle is not in the subject
        return false;
    }
...
function remove_comments_from_query($query)
{
    ...
    $in_string = false;
    for ($i = 0; $i < $n; $i++) {
        if ($query[$i] == '\\') { // backslash/escape; skip
            continue;
        }
        if ($query[$i] == '"' || $query[$i] == "'") {
            if (! $in_string) { // string begins:
                $in_string = $query[$i];
            } elseif ($query[$i] === $in_string) {
                $in_string = false;
            }
        } elseif ($in_string === false) { // not in string, check for comment start:
            if ($query[$i] == '#' || substr($query, $i, 3) == '-- ') { // comment start
                // search for newline
                for ($j = $i + 1; $j < $n; $j++) {
                    if ($query[$j] == "\n" || $query[$j] == "\r") {
                        break;
...
function get_fieldlines_from_query($query)
{
    ...
    for ($i = 0; $i < strlen($qrylines); $i++) {
        ...
        if (($c == ',') && (! $in_parens) && (! $in_quote)) { // split here:
            $line = trim($buffer);
        ...
        if ((! $in_parens) && ($c == "'") && ($qrylines[$i - 1] != '\\')) { // Text between quotation marks outside from parentheses, must be in a field COMMENT
...
function get_columns_with_charset_definition($query)
{
    ...
    foreach ($fields as $field_name => $field_definition) {
        ...
        if (strpos($field_definition, 'CHARACTER SET') !== false) { // Column contains 'CHARACTER SET' definition
            $fields_with_charset[$field_name] = $field_definition;
        } elseif (strpos($field_definition, 'COLLATE') !== false) { // Column contains 'COLLATE' definition
            $fields_with_charset[$field_name] = $field_definition;
        }
    }
...
function convert_table_to_utf8_ascii($table)
{
    ...
    if (empty($columns_with_charset)) { // There is no explicit charset or collation definition in this table, we update everything utf8
        $text_columns = $DB->get_results('SHOW FULL COLUMNS FROM `' . $table . '` WHERE type LIKE "%TEXT%"');
    } else { // There are columns with charset or collation definition
        ...
        foreach ($text_columns as $column_key => $text_column) { // Update each columns character set where the charset is not explicitly defined ( but it could be ) to utf8
            if (isset($columns_with_charset[$text_column->Field])) { // Do not update those columns where the charset or the collation was explicitly defined
    ...
    $col_definitions = [];
    foreach ($text_columns as $text_column) { // Update each text column character set explicitly to utf8. Those columns which should not be updated were already removed from this array.
    ...
    if (empty($columns_with_charset)) { // Update the table default character set and convert fields to utf8
        $DB->query('ALTER TABLE `' . $table . '`
			DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci,
			CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci');
    } else { // Update the table default character set, but not convert the fields to utf8, since they were already converted
        $DB->query('ALTER TABLE `' . $table . '`
			DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci');
...
function db_upgrade_to_utf8_ascii($show_title = true)
{
    ...
    foreach ($db_tables as $table) { // Convert all tables charset to utf8
