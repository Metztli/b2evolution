...
class SQL
{
     ...
     */
    public function get_select($prefix = ' SELECT ')
    {
     ...
     */
    public function get_from($prefix = ' FROM ')
    {
        if (! empty($this->from)) {
     ...
     */
    public function get_where($prefix = ' WHERE ')
    {
        if (! empty($this->where)) {
     ...
     */
    public function get_group_by($prefix = ' GROUP BY ')
    {
     ...
     */
    public function get_having($prefix = ' HAVING ')
    {
     ...
     */
    public function get_order_by($prefix = ' ORDER BY ')
    {
     ...
     */
    public function get_limit($prefix = ' LIMIT ')
    {
        if (! empty($this->limit)) {
     ...
     */
    public function get_append($prefix = ' ')
    {
     ...
     */
    public function SELECT_add($select_add)
    {
        if (empty($this->select)) {
            debug_die('Cannot extend empty SELECT clause');
        }
    ...
    {
        if (empty($this->from)) {
            debug_die('Cannot extend empty FROM clause');
        }
     ...
     * @param string
     */
    public function WHERE_and($where_and)
    {
        if (empty($where_and)) {	// Nothing to append:
            return false;
        }

        if (! empty($this->where)) { // We already have something in the WHERE clause:
            $this->where .= ' AND ';
        }
    ...
    {
        if (empty($where_or)) {	// Nothing to append:
            return false;
        }
     ...
     * @param string
     */
    public function HAVING_and($having_and)
    {
        if (empty($having_and)) {	// Nothing to append:
            return false;
        }

        if (! empty($this->having)) { // We already have something in the HAVING clause:
            $this->having .= ' AND ';
        }
    ...
    public function ORDER_BY_prepend($order_by_prepend)
    {
     ...
     */
    public function add_search_field($field, $reg_exp = '')
    {
        $this->search_field[] = $field;

        if (! empty($reg_exp)) {	// We want to use a regular expression on the search for this field, so add to the search field regexp array
     ...
     */
    public function WHERE_kw_search($search, $search_kw_combine = 'AND')
    {
        global $DB;
        ...
        switch ($search_kw_combine) {
            ...
            case 'OR':
                ...
                $keyword_array = array_filter($keyword_array, ['SQL', '_WHERE_kw_search_callback']);
     ...
     * @param string operator( AND , OR , PHRASE ) for the filter
     */
    public function WHERE_regexp($search, $search_kw_combine)
    {
        $where = '';
        ...
        foreach ($this->search_field_regexp as $field => $reg_exp) {
            ...
            if (! empty($search_reg_exp)) {	// The reg exp search is not empty, so we add it to the request with an 'OR' operator:
                switch ($search_kw_combine) {
                    case 'AND':
                    case 'OR':
                    case 'PHRASE':
