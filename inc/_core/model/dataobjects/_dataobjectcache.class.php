...
class DataObjectCache
{
     ...
     * @param object DB row
     */
    public function &new_obj($row = null)
    {
     ...
     */
    public function load_list($req_list, $invert = false)
    {
        global $Debuglog, $DB;
     ...
     */
    public function load_where($sql_where)
    {
        $SQL = $this->get_SQL_object('Get the ' . $this->objtype . ' rows to load the objects into the cache by ' . get_class() . '->' . __FUNCTION__ . '()');
        $SQL->WHERE($sql_where);
        return $this->load_by_sql($SQL);
     ...
     */
    public function load_by_sql($SQL)
    {
        global $DB, $Debuglog;
        ...
        if (empty($SQL->title)) {	// Set SQL title for debug info:
     ...
     * @return SQL
     */
    public function get_SQL_object($title = null)
    {
        $select = '';
        if (! empty($this->select)) {	// Additional select fields
            $select = ', ' . $this->select;
        }
        $SQL = new SQL($title);
        $SQL->SELECT('*' . $select);
    ...
    public function get_ID_array()
    {
        if (! isset($this->ID_array)) {
            ...
            foreach ($this->cache as $obj) {
                if ($obj !== null) { // A cached object can be NULL, e.g. File object when the directory is disabled by settings
     ...
     * @return object
     */
    public function &instantiate(&$db_row)
    {
        if (is_null($db_row)) {	// we can't access NULL as an object
            return $db_row;
        }
        ...
        if (isset($this->cache[$obj_ID])) { // Already in cache, do nothing!
        } elseif (isset($this->shadow_cache[$obj_ID])) {	// Already in shadow, recycle object:
            // echo "adding shadow {$this->objtype} $obj_ID ";
            $this->add($this->shadow_cache[$obj_ID]);
        } else { // Not already cached, add new object:
    ...
    {
        if ($keep_shadow) {	// Keep copy of cache in case we try to re instantiate previous object:
     ...
     */
    public function &get_by_ID($req_ID, $halt_on_error = true, $halt_on_empty = true)
    {
        global $DB, $Debuglog;

        $req_ID = intval($req_ID);

        if (empty($req_ID)) {	// Don't allow request with empty ID:
            if ($halt_on_empty) {
        ...
        if (! empty($this->cache[$req_ID])) { // Already in cache
            ...
            if ($this->load_all) { // It's ok to just load everything:
                $this->load_all();
    ...
    public function &get_by_name($req_name, $halt_on_error = true, $halt_on_empty = true)
    {
        ...
        if (empty($req_name)) {	// Don't allow request with empty name:
            if ($halt_on_empty) {
        ...
        if (isset($this->cache_name[$req_name])) {	// Get object from cache by name:
        ...
        if ($db_row = $DB->get_row($SQL->get(), OBJECT, 0, 'DataObjectCache::get_by_name()')) {
            ...
            if (! isset($this->cache_name[$req_name])) {	// Add object in cache by name:
                $this->cache_name[$req_name] = $this->new_obj($db_row);
            }
        }

        if (empty($this->cache_name[$req_name])) {	// Object does not exist by requested name:
     ...
     */
    public function remove_by_ID($req_ID)
    {
     ...
     * @return boolean
     */
    public function dbdelete_by_ID($req_ID)
    {
    ...
    public function get_option_list($default = 0, $allow_none = false, $method = 'get_name', $ignore_IDs = [], $option_attrs = [])
    {
        ...
        foreach ($this->cache as $loop_Obj) {
            if (in_array($loop_Obj->ID, $ignore_IDs)) {	// Ignore this ID
                continue;
            }
            ...
            foreach ($option_attrs as $option_attr_name => $option_attr_field) {	// Additional attribute:
     ...
     * @return string
     */
    public function get_group_country_option_list($default = 0, $allow_none = false, $method = 'get_name', $ignore_IDs = [])
    {
        if (! is_array($default)) {
            $default = [$default];
        }
        ...
        if ($allow_none) {	// we set current value of a country if it is sent to function
            $r .= '<option value="' . $this->none_option_value . '"';
            if (empty($default)) {
                $r .= ' selected="selected"';
            }
        ...
        $pref_countries = []; //preferred countries.
        $rest_countries = []; // not preffered countries (the rest)

        foreach ($this->cache as $loop_Obj) {
            if (in_array($loop_Obj->ID, $ignore_IDs)) {	// if we have ID of countries that we have to ignore we just do not include them here.
                //Ignore this ID
                continue;
            }

            if ($loop_Obj->preferred == 1) {  // if the country is preferred we add it to selected array.
                $pref_countries[] = $loop_Obj;
            }
            $rest_countries[] = $loop_Obj;
        }

        if (count($pref_countries)) {	// if we don't have preferred countries in this case we don't have to show optgroup
            // in option list
            ...
            if (count($rest_countries)) { // if we don't have rest countries we do not create optgroup for them
     ...
     */
    public function get_checklist_options($field_name, $ignore_IDs = [])
    {
        $options = [];

        $names = $this->get_option_array($ignore_IDs);
        foreach ($names as $ID => $name) {
            $options[] = [$field_name . '[]', $ID, $name, 0];
        }
     ...
     * @return string
     */
    public function get_option_array_worker($method = 'get_name', $ignore_IDs = [])
    {
        if (! $this->all_loaded && $this->load_all) { // We have not loaded all items so far, but we're allowed to.
            if (empty($ignore_IDs)) {	// just load all items
                $this->load_all();
