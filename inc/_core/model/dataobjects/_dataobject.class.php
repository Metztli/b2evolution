...
class DataObject
{
     ...
     * @access protected
     */
    public $ID = 0;  // This will be the ID in the DB

    public $allow_ID_insert = false;
    ...
    public $datecreated_field;

    public $datemodified_field;

    public $creator_field;

    public $lasteditor_field;
     ...
     */
    public $delete_restrictions = null;

    /**
     * Relations that will cascade deletion.
     */
    public $delete_cascades = null;
     ...
     * @param string datetime field name
     */
    public function __construct($tablename, $prefix = '', $dbIDname = 'ID', $datecreated_field = '', $datemodified_field = '', $creator_field = '', $lasteditor_field = '')
    {
    ...
    public static function db_delete_where($sql_where, $object_ids = null, $params = null)
    {
        ...
        if (isset($params['delete_cascades'])) { // Use predefined delete cascades
            $delete_cascades = $params['delete_cascades'];
            unset($params['delete_cascades']);
        } else { // Get delete cascades from the given class
            $delete_cascades = $class_name::get_delete_cascades();
        }

        // Init dbconfig variables
        if (isset($params['class_db_config'])) { // Use predefined class dbconfig
            $self_db_config = $params['class_db_config'];
            unset($params['class_db_config']);
        } else { // Get class dbconfig from the given class
            $self_db_config = $class_name::get_class_db_config();
        }

        if (isset($params['force_delete'])) { // In case of force deletion, also delete restrictions
        ...
        foreach ($delete_cascades as $cascade) {
            if (! isset($db_config['aliases'][$cascade['table']])) {	// We have no declaration for this table, we consider we don't deal with this table in this app:
                continue;
            }
            ...
            if (isset($cascade['class'])) { // The cascade class attributum is set, call the corresponding db_delete_where() function
                if (empty($object_ids)) { // delete by where condition
                    $sub_query = 'SELECT ' . $self_db_config['dbIDname'] . '
						FROM ' . $self_db_config['dbtablename'] . '
						WHERE ' . $sql_where . $more_restriction;
                    if ($self_db_config['dbtablename'] == $cascade['table']) { // We are removing items from the same table, avoid infinite loop
                        $object_ids = $DB->get_col($sub_query);
                        if (empty($object_ids)) { // There are no entries in the foreign key table with the given condition, so nothing to delete
                            continue;
                        }
                    }
                }
                ...
                if ($result === false) { // Delete cascade operation failed in a cascade class
                    if ($use_transaction) { // Rollback transaction
                        $DB->rollback();
                    }
                    return false;
                }
            ...
            if ($result === false) { // There is an open transaction, needs to be closed.
                if ($use_transaction) {
                    $DB->rollback();
                }
                return $result;
        ...
        if ($use_transaction) { // There is an open transaction, needs to be closed.
     ...
     */
    public function add_relations($type, $new_relations)
    {
     ...
     */
    public function dbchange($dbfieldname, $dbfieldtype, $valuepointer) // TODO: dh> value by reference? see above..
    {
    ...
    protected function dbupdate_worker($auto_track_modification = true)
    {
        ...
        if ($auto_track_modification) { // We wnat to track modification date and author automatically:
            if (! empty($this->datemodified_field)) {	// We want to track modification date:
                $this->set_param($this->datemodified_field, 'date', date('Y-m-d H:i:s', $localtimenow));
            }
            if (! empty($this->lasteditor_field) && is_object($current_User)) {	// We want to track last editor:
        ...
        foreach ($this->dbchanges as $loop_dbfieldname => $loop_dbchange) {
            if ($loop_dbchange['type'] == 'dbfield') {	// Set to dbfield only:
        ...
        $Plugins->trigger_event('AfterObjectUpdate', $params = [
            'Object' => &$this,
            'type' => get_class($this),
        ]);
    ...
    public function dbinsert()
    {
        ...
        if (! empty($this->datecreated_field)) { // We want to track creation date:
            $this->set_param($this->datecreated_field, 'date', date('Y-m-d H:i:s', $localtimenow));
        }
        if (! empty($this->datemodified_field)) { // We want to track modification date:
            $this->set_param($this->datemodified_field, 'date', date('Y-m-d H:i:s', $localtimenow));
        }
        if (is_logged_in()) { // Assign user's ID only when user is logged in
            if (! empty($this->creator_field)) { // We want to track creator:
            ...
            if (! empty($this->lasteditor_field)) { // We want to track last editor:
                if (empty($this->lastedit_user_ID)) { // No editor assigned yet, use current user:
                    $this->set_param($this->lasteditor_field, 'number', $current_User->ID);
                }
            }
        }
        ...
        if (! ($this->allow_ID_insert && $this->ID)) {// store ID for newly created db record. Do not if allow_ID_insert is true and $this->ID is not 0
        ...
        if (! empty($Plugins)) {
            $Plugins->trigger_event('AfterObjectInsert', $params = [
                'Object' => &$this,
                'type' => get_class($this),
            ]);
        }
    ...
    {
        if ($this->ID == 0) {	// Object not serialized yet, let's insert!
            // echo 'INSERT';
            return $this->dbinsert();
        } else {	// Object already serialized, let's update!
            // echo 'UPDATE';
     ...
     * @return boolean true on success
     */
    protected function dbdelete_worker($ignore_restrictions = [])
    {
        ...
        if (! $this->check_delete(T_('Some restrictions prevent deletion:'), $ignore_restrictions)) {	// Some restrictions still prevent deletion
        ...
        $result = $this->db_delete_where(null, [$this->ID], $params);

        $Plugins->trigger_event('AfterObjectDelete', $params = [
            'Object' => &$this,
            'type' => get_class($this),
        ]);

        if ($need_transaction) { // Transaction was started, call commit or rollback
     ...
     */
    public function dbexists($unique_fields, $values)
    {
        global $DB;

        if (is_array($unique_fields) && is_array($values)) {	// UNIQUE index consists of MULTIPLE fields
            $sql_where = [];
            foreach ($unique_fields as $i => $unique_field) {
                $sql_where[] = $unique_field . " = " . $DB->quote($values[$i]);
            }
            $sql_where = implode(' AND ', $sql_where);
        } else {	// UNIQUE index consists of ONE field
     ...
     */
    public function check_relations($what, $ignore = [], $addlink = false)
    {
        global $DB;
        ...
        if (is_null($this->$what)) { // The relations are not defined
            return $restriction_Messages;
        }

        foreach ($this->$what as $restriction) {
            if (! in_array($restriction['fk'], $ignore)) {
                if ($addlink) {	// Get a link from derived class method:
                    $link = $this->get_restriction_link($restriction);
                    if (! empty($link)) {	// Add a restriction message with a link for current table:
                        $restriction_Messages->add($link);
                    }
                    if ($link !== false) {	// If this Object has no restriction in current table, go to next table checking:
                        continue;
                    }
                    // ELSE $link === false
     ...
     * @param array Params
     */
    public function confirm_delete($confirm_title, $crumb_name, $delete_action, $hiddens, $additional_messages = [], $params = [])
    {
        global $Messages;
        ...
        if (! empty($additional_messages)) { // Initialize additional messages
        ...
        if ($restriction_Messages->count()) {	// The will be cascading deletes, issue WARNING:
        ...
        $Form->button(['submit', '', T_('I am sure!'), 'DeleteButton btn-danger']);
        $Form->end_form();

        $Form = new Form($redirect_to, 'form_cancel', 'get', '');

        $Form->begin_form('inline');
        if (empty($redirect_to)) { // If redirect url is not defined we should go to current url after cancel action
            $Form->hiddens_by_key($hiddens);
        }
        $Form->button(['submit', '', T_('CANCEL'), 'CancelButton']);
        $Form->end_form();
     ...
     */
    public function dget($parname, $format = 'htmlbody')
    {
     ...
     * @param string Output format, see {@link format_to_output()}
     */
    public function disp($parname, $format = 'htmlbody')
    {
        // Note: we call get again because of derived objects specific handlers !
     ...
     */
    public function set_param($parname, $fieldtype, $parvalue, $make_null = false)
    {
        global $Debuglog;
     ...
     * @return boolean true, if value has been set/changed, false if not.
     */
    public function set_from_Request($parname, $var = null, $make_null = false, $cleanup_function = null)
    {
        if (empty($var)) {
        ...
        if (! empty($cleanup_function)) {	//We want to apply a cleanup function
     ...
     */
    public function set_string_from_param($parname, $required = false, $cleanup_function = null, $validation_function = null, $error_message = null)
    {
        $var = $this->dbprefix . $parname;
     ...
     */
    public function get_action_title($action)
    {
        switch ($action) {
     ...
     */
    public function action_icon($action, $help_texts = [])
    {
        if (! $this->check_perm($action, false)) {	// permission denied:
            return '';
        }

        return action_icon(
            $this->get_action_title($action),
            $action,
     ...
     */
    public function action_link($action, $link_text, $help_texts = [])
    {
        if (! $this->check_perm($action, false)) {	// permission denied:
            return '';
        }
     ...
     */
    public function history_info_icon()
    {
        $history = [];
        ...
        if (! empty($this->creator_field) && ! empty($this->{$this->creator_field})) {	// We have a creator:
            $creator_User = &$UserCache->get_by_ID($this->{$this->creator_field});

            if (! empty($this->datecreated_field) && ! empty($this->{$this->datecreated_field})) {	// We also have a create date:
                $history[0] = sprintf(
                    T_('Created on %s by %s'),
                    mysql2localedate($this->{$this->datecreated_field}),
                    $creator_User->dget('preferredname')
                );
            } else {	// We only have a cretaor:
                $history[0] = sprintf(T_('Created by %s'), $creator_User->dget('preferredname'));
            }
        } elseif (! empty($this->datecreated_field) && ! empty($this->{$this->datecreated_field})) {	// We only have a create date:
            $history[0] = sprintf(T_('Created on %s'), mysql2localedate($this->{$this->datecreated_field}));
        }

        // HANDLE LAST UPDATE STUFF
        if (! empty($this->lasteditor_field) && ! empty($this->{$this->lasteditor_field})) {	// We have a creator:
            $creator_User = &$UserCache->get_by_ID($this->{$this->lasteditor_field});

            if (! empty($this->datemodified_field) && ! empty($this->{$this->datemodified_field})) {	// We also have a create date:
                $history[1] = sprintf(
                    T_('Last mod on %s by %s'),
                    mysql2localedate($this->{$this->datemodified_field}),
                    $creator_User->dget('preferredname')
                );
            } else {	// We only have a cretaor:
                $history[1] = sprintf(T_('Last mod by %s'), $creator_User->dget('preferredname'));
            }
        } elseif (! empty($this->datemodified_field) && ! empty($this->{$this->datemodified_field})) {	// We only have a create date:
        ...
        return get_icon('history', 'imgtag', [
            'title' => implode(' - ', $history),
        ], true);
    }
}
