...
class Log
{
     ...
     */
    public function add($message, $category = null)
    {
        if ($category === null) { // By default, we use the default category:
            $category = $this->defaultcategory;
        }

        if (is_string($category) && isset($GLOBALS['debug_' . $category]) && $GLOBALS['debug_' . $category] == false) {	// We don't want to debug this category
            return;
        }
     ...
     */
    public static function get_head_foot($headfoot, $category, $template = null)
    {
        if (is_string($headfoot) && $category == 'container') { // container head or foot
            $r = $headfoot;
        } elseif (is_array($headfoot)) { // head or foot for categories
            if (isset($headfoot[$category])) {
                $r = $headfoot[$category];
            } elseif (isset($headfoot['all']) && $category != 'container') { // use 'all' info, except if for container
                $r = $headfoot['all'];
            } else {
                return false;
            }
     ...
     */
    public function display_paragraphs($category = 'all', $outerdivclass = 'panelinfo', $cssclass = null)
    {
        if (is_null($cssclass)) {
            $cssclass = [
                'all' => [
     ...
     *                  of useful/required)
     */
    public function disp($before = '<div class="action_messages">', $after = '</div>', $skip_if_preview = true)
    {
     ...
     */
    public function display($head = null, $foot = null, $display = true, $category = 'all', $cssclass = null, $style = null, $outerdivclass = 'log_container')
    {
        if (is_null($head)) { // Use object default:
            $head = isset($this->head) ? $this->head : '';
        }
        if (is_null($foot)) { // Use object default:
        ...
        if (! $this->count($category)) { // no messages
            return false;
        } else {
        ...
        if (! is_array($cssclass)) {
            $cssclass = [
                'all' => [
                    'class' => is_null($cssclass) ? null : $cssclass,
                    'divClass' => true,
                ],
            ];
        ...
        foreach ($messages as $lcategory => $lmessages) {
            ...
            if ($style == null) { // 'br' for a single message, 'ul' for more
     ...
     */
    public function get_string($head = '', $foot = '', $category = null, $implodeBy = ', ', $format = 'striptags')
    {
    ...
    {
        if (is_null($category)) {	// use default category:
            $category = $this->defaultcategory;
        }
     ...
     */
    public function get_messages($category = null, $singleDimension = false)
    {
        $messages = [];
        ...
        while ($lcategory = array_shift($category)) {
            if ($lcategory == 'all') { // Put those categories in queue, which have not been processed already
            ...
            if (! isset($this->messages[$lcategory][0])) { // no messages
                continue;
            }
...
class Log_noop
{
     ...
     * This is a no-operation method.
     */
    public function display_paragraphs()
    {
    }
