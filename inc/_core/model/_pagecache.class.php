...
class PageCache
{
     ...
     */
    public $max_age_seconds = 900;  // 15 minutes for now
    ...
    public function __construct($Blog = null)
    {
        ...
        if (is_null($Blog)) {	// Cache for "other" "genereic" "special" pages:
            $this->ads_collcache_path = $cache_path . 'general/';

            if ((! empty($Settings)) && (! $Settings->get('general_cache_enabled'))) {	// We do NOT want caching for this collection
                $Debuglog->add('General cache not enabled.', 'pagecache');
            } else {
                $this->is_enabled = true;
            }
        } else {	// Cache for a specific Blog/Collection:
            ...
            if (! $Blog->get_setting('cache_enabled')) {	// We do NOT want caching for this collection
     ...
     */
    public function get_af_filecache_path()
    {
        global $Debuglog;
        global $ReqURL;
     ...
     * @return string
     */
    public function gen_filecache_path($url)
    {
        global $skin;

        if (! empty($skin)) { // add skin folder into the end of the url before creating the hash to have different cache files for different skins
            $url .= $skin;
        }
     ...
     */
    public function cache_create($clear = true)
    {
        global $cache_path;
        ...
        if ($clear) { // Clear contents of folder, if any:
    ...
    public function check()
    {
        ...
        if (! $this->is_enabled) {	// We do NOT want caching for this page
            $Debuglog->add('Cache not enabled. No lookup nor caching performed.', 'pagecache');
            return false;
        }

        if ($disp == '404') {	// We do NOT want caching for 404 pages (illimited possibilities!)
            $Debuglog->add('Never cache 404s!', 'pagecache');
            return false;
        }

        if ($disp == '403') {	// We do NOT want caching for 403 pages (illimited possibilities!)
            $Debuglog->add('Never cache 403s!', 'pagecache');
            return false;
        }

        if (in_array($disp, ['login', 'access_requires_login', 'content_requires_login', 'register', 'lostpassword', 'anonpost'])) {	// We do NOT want caching for in-skin login, register, lostpassord pages and new item anonymous form:
        ...
        if (is_logged_in()) {	// We do NOT want caching when a user is logged in (private data)
            $Debuglog->add('Never cache pages for/from logged in members!', 'pagecache');
            return false;
        }

        if ($Messages->count()) {	// There are some messages do be displayed. That means the user has done some action.
        ...
        if ($this->retrieve()) { // We could retrieve:
            return true;
        }
    ...
    public function retrieve()
    {
        ...
        */
        ...
        if (! empty($lines)) {	// We have data in the cache!
            ...
            if (($cache_age > $this->max_age_seconds) || ($last_invalidation_timestamp > $retrieved_ts)) {	// Cache has expired
                return false;
            }
            ...
            if (array_key_exists('HTTP_IF_MODIFIED_SINCE', $_SERVER)) {
                ...
                if ($retrieved_ts <= $if_modified_since) {	// Cached version is equal to (or older than) $if_modified since; contents probably not modified...
                    ...
                    if (array_key_exists('HTTP_IF_NONE_MATCH', $_SERVER)) {
                        ...
                        if ($if_none_match == gen_current_page_etag()) {	// Ok, this seems to be really the same:
            ...
            if (! $this->is_valid_cached_content($body)) {	// The stored content of the cached page is not valid HTML page,
     ...
     */
    public function is_valid_cached_content($content = null)
    {
        if ($content === null) {	// Use current cached page content:
            $content = $this->cached_page_content;
        }

        // Check if the cached page content is valid HTML page:
        if (preg_match('#<html[^>]*>(.+)<\/html>#is', $content)) {	// Valid HTML page
            return true;
        } else {	// Wrong HTML page
            global $Debuglog;
     ...
     */
    public function abort_collect($flush = true)
    {
        global $Debuglog;
        ...
        if ($flush) { // Flush the output buffer and turn off buffering
            ob_end_flush();
        } else { // Erase the output buffer and turn off buffering
            ob_end_clean();
        }
    ...
    public function end_collect()
    {
        ...
        if (! $this->is_valid_cached_content()) {	// Current cached page content is not valid HTML page, We cannot use this for storing in cache file:
            return;
        }
        ...
        if (! $fh = @fopen($af_cache_file . '.tmp', 'x', false)) {
            ...
            global $skin;
            if (! empty($skin)) { // add current skin folder into the cache file header
                $file_head .= 'skin:' . $skin . "\n";
            }
            ...
            if (! @rename($af_cache_file . '.tmp', $af_cache_file)) {	// Rename failed, we are probably on windows PHP <= 5.2.5... http://bugs.php.net/bug.php?id=44805
                ...
                if (! @rename($af_cache_file . '.tmp', $af_cache_file)) { // Hide errors bc another PHP process could have beaten us to writing a new file there
     ...
     */
    public static function checkDelete($file_path, $display_messages = false)
    {
        global $cache_path;

        $folder_file = substr($file_path, strlen($cache_path));

        if (strpos($file_path, 'CVS') !== false) { // skip CVS folders - This could be more specific
        ...
        if (($file_name == 'index.html') || (substr($file_name, 0, 1) == '.') || ($file_name == 'sample.htaccess')) { // this file is index.html or sample.htaccess or it is hidden, should not delete it.
            if ($display_messages === 'cron_job') {	// Log a message for cron job:
                cron_log_action_end('File <code>' . $folder_file . '</code> was not deleted because of reserved name.');
            }
            return false;
        ...
        if ($datediff < 86400 /* 60*60*24 = 24 hour*/) { // the file is not older then 24 hour, should not delete it.
            if ($display_messages === 'cron_job') {	// Log a message for cron job:
     ...
     * @param boolean|string 'cron_job' - to log messages for cron job
     */
    public static function deleteDirContent($path, $display_messages = false)
    {
        global $cache_path;
        ...
        if ($dir = @opendir($path)) {	// If folder is opened successfully:
            while (($file = readdir($dir)) !== false) {
                if ($file == '.' || $file == '..') {	// Skip reserved file names:
                    continue;
                }
                $file_path = $path . $file;
                $folder_file = substr($file_path, strlen($cache_path));
                if (is_dir($file_path)) {	// Folder:
                    ...
                    if (PageCache::checkDelete($file_path, $display_messages)) {	// If file is allowed to be deleted from cache currently:
                        if (@unlink($file_path)) {	// Success deleting:
                            if ($display_messages === 'cron_job') {	// Log a message for cron job:
                                cron_log_action_end('File <code>' . $folder_file . '</code> has been deleted.');
                            }
                        } else {	// If a deleting the file failed: return error
                            if ($display_messages === 'cron_job') {	// Log a message for cron job:
        ...
        } else {	// No permission to open the cache folder:
            if ($display_messages === 'cron_job') {	// Log a message for cron job:
     ...
     */
    public static function prune_page_cache($display_messages = false)
    {
        global $cache_path;
