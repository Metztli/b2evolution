...
class Messages
{
     ...
     * @var boolean
     */
    public $message_group_open = false;
     ...
     * @var string
     */
    public $message_group_header = null;
     ...
     * @var string
     */
    public $message_group_type = 'error';
     ...
     * @var boolean
     */
    public $display_empty_group = false;
     ...
     */
    public function add_messages($p_Messages)
    {
        ...
        if ($this->has_open_group()) {	// Check if this is still the same group by comparing the first text of message group, message group type and display empty group values
            ...
            if (! $same_group) { // No longer the same message group, close the previous group and append to current message queue
                $r = $p_Messages->close_group(true);
                $this->add($r[0], $r[1]);
            } else { // Append group messages
        ...
        } else { // No open group, just copy the other object's group values
     ...
     * @param string the message type, it can have this values: 'success', 'warning', 'error', 'note'
     */
    public function start_group($header = null, $type = 'error')
    {
        $this->close_group();
     ...
     */
    public function add_to_group($text, $type = 'error', $header = null, $force_new_group = false)
    {
     ...
     */
    public function close_group($output = false)
    {
        if ($this->has_open_group()) {
            if ($this->group_count === 1 && empty($this->message_group_header)) { // There is only a single group item and no header text to display, display as single line message
            ...
            return [$message, $this->message_group_type];
        }
     ...
     * @return boolean
     */
    public function has_open_group()
    {
     ...
     */
    public function disp($before = '<div class="action_messages">', $after = '</div>')
    {
        if ($this->has_open_group()) {
            $this->close_group();
        }

        if ($this->affixed) {	// Add "affixed_messages" class to $before param
            $old_before = $before;
            $before = add_tag_class($before, 'affixed_messages');
            if ($old_before == $before) {	// No tag to add class to, wrap in DIV with "affixed_messages" class:
     ...
     */
    public function display($head = null, $foot = null, $display = true, $outerdivclass = 'log_container')
    {
        if ($this->has_open_group()) {
            $this->close_group();
        }
        ...
        if (isset($this->params['class_outerdiv']) && $outerdivclass == 'log_container') { // Use default class from object params instead of default function param
     ...
     */
    public function get_string($head = '', $foot = '', $implodeBy = ', ', $format = 'striptags')
    {
        if ($this->has_open_group()) {
            $this->close_group();
        }
    ...
    {
        if (! empty($params)) { // Change default params
