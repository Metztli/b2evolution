...
class FilteredResults extends Results
{
     ...
     */
    public function __construct($filterset_name)
    {
     ...
     */
    public function get_active_filter($key)
    {
    ...
    public function get_active_filters()
    {
        ...
        foreach ($this->default_filters as $key => $value) {
            if (! isset($this->filters[$key])) {	// Some value has not been copied over from defaults to active or specifically set:
     ...
     */
    public function dump_active_filters()
    {
        foreach ($this->default_filters as $key => $value) {
            if (! isset($this->filters[$key])) {	// Some value has not been copied over from defaults to active or specifically set:
     ...
     */
    public function set_default_filters($default_filters, $preset_filters = null)
    {
        $this->default_filters = array_merge($this->default_filters, $default_filters);
        if ($preset_filters !== null) {	// Overrride preset filters only when this is requested:
     ...
     */
    public function activate_preset_filters()
    {
        if (empty($this->filters['filter_preset'])) { // No filter preset, there are no additional defaults to use:
            return;
        }

        if (isset($this->preset_filters[$this->filters['filter_preset']])) {	// Override general defaults with the specific defaults for the preset:
     ...
     */
    public function save_filterset()
    {
        /**
         * @var Session
         */
     ...
     */
    public function restore_filterset()
    {
        /**
         * @var Session
         */
        ...
        if (empty($filters)) { // set_filters() expects array
            $filters = [];
        }
     ...
     */
    public function check_filter_query($filter, $value, $condition = '=')
    {
        if (empty($this->filters['filter_query'])) {	// Filter query is not defined for this results list:
            return false;
        }

        if (! preg_match_all('/{"id":"' . $filter . '"[^}]+"value":"([^"]*)"[^}]*}/i', $this->filters['filter_query'], $filter_values)) {	// Current filter query has no value of the requested filter:
            return false;
        }
        ...
        foreach ($filter_values[1] as $filter_value) {
            ...
            if ($result) {	// Stop of the first searched value:
                return $result;
            }
        }
