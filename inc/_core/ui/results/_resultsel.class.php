...
class ResultSel extends Results
{
    ...
    public $current_selection_ID;

    public $table_selections;

    public $field_selected;

    public $field_selection;
    ...
    public function __construct(
        ...
        if (! check_user_perm('selections', 'view')) {	// User is NOT allowed to view selections
    ...
    public function display_list_start()
    {
        ...
        if ($this->total_pages > 0) {	// We have rows to display, we want the selection stuff:
    ...
    public function selection_menu()
    {
        ...
        if ($can_edit) { // links to check all and uncheck all
            echo $this->Form->checkbox_controls('$all$', [
        ...
        if ($can_edit) {
            ...
            $this->Form->submit(['actionArray[update_' . $this->param_prefix . 'selection]', T_('Update selection'), 'SaveButton']);
        }
    }
}
...
function selection_checkbox($item_ID, $param_prefix)
{
    ...
    if (in_array($item_ID, $item_ID_array)) {	// We have already displayed a checkbox for this ID
    ...
    if (check_user_perm('selections', 'edit')) {	// User is allowed to edit
        $r .= '<input type="checkbox" class="checkbox" name="' . $param_prefix . 'items[]" value="' . $item_ID . '"';
        if (in_array($item_ID, $cols_check)) {	// already in selection:
            $r .= ' checked="checked" ';
        }
        $r .= ' />';
...
function selection_select_tag(
    ...
    if (! empty($rows)) {
        $selection_name = '';
        foreach ($rows as $row) { // construction of the option tags
            if ($row->$selections_table_ID == $selection_ID) { // option selected by default
...
function handle_selection_actions($selection_ID, $prefix, $prefix_sel)
{
    ...
    if ($selection_ID == $previous_sel_ID) { // the databse has to be updated
        ...
        if ($selection_ID == 0) { // A new selection must be created
            $action = 'create';
        } elseif ($selection_ID > 0) { // An existing selection is being updated
            $action = 'update';
        }
...
{ // the form has been submitted to act on the database and not only to change the display
    ...
    $items = param($prefix . 'items', 'array:string', [], false);	// do NOT memorize // ?????????????
    ...
    switch ($action) {
        ...
        case 'create':
            ...
            if (empty($selection_name)) {	// No name provided:
            ...
            if (! empty($items)) { // nothing must be inserted if no items are selected
                $sql_sel = 'INSERT INTO ' . $sel_table . '( ' . $sel_table_item . ', ' . $sel_table_selection . ')  VALUES ';
                $sel_array = [];

                $i = 0;
                foreach ($items as $item) { // construction of the sql query depending on selected values in the result table
        ...
        case 'update':
            ...
            if (preg_match('#[0-9,]+#', $item_ID_list)) { // check the format of the item list to avoid sql injection
            ...
            if (! empty($items)) { // there have been some items selected in the result table: they must be inserted into the database
                $sql_sel = 'INSERT INTO ' . $sel_table . '( ' . $sel_table_item . ', ' . $sel_table_selection . ')  VALUES ';
                $sel_array = [];
        ...
        case 'delete':
            ...
            if (! $confirm) { // ask for confirmation before deleting
				...
				<?php
            } else { // the deletion has been confirmed
