...
class Results extends Table
{
     ...
     */
    public $functions_area;

    /**
     * Should there be nofollows on page navigation
     */
    public $nofollow_pagenav = false;
     ...
     */
    public $checkbox_toggle_selectors;
     ...
     */
    public function __construct($sql, $param_prefix = '', $default_order = '', $default_limit = null, $count_sql = null, $init_page = true, $force_order_by_count = null)
    {
        ...
        if ($init_page) { // attribution of a page number
     ...
     * @param integer Default number of lines displayed on one page (0 to disable paging; null to use $UserSettings/results_per_page)
     */
    public function init_limit_param($default_limit)
    {
        global $UserSettings;
        ...
        if (! empty($this->param_prefix) &&
            $this->limit > -1 &&
            $this->limit != (int) $UserSettings->get($this->limit_param)) {	// Change a limit number in DB for current user and current list
            if ($this->limit == $UserSettings->get('results_per_page') || $this->limit == 0) {	// Delete a limit number for current list if it equals a default value
                $UserSettings->delete($this->limit_param);
                $this->limit = $UserSettings->get('results_per_page');
            } else {	// Set a new value of limit number current list
                $UserSettings->set($this->limit_param, $this->limit);
            }
            $UserSettings->dbupdate();
        }

        if (! empty($this->param_prefix) && $this->limit == -1) {	// Set a limit number from DB
            if ($UserSettings->get($this->limit_param) > 0) {	// Set a value for current list if it was already defined
                $this->limit = $UserSettings->get($this->limit_param);
            }
        }

        if ($this->limit == -1 || empty($this->limit)) {	// Set a default value
     ...
     */
    public function init_order_param()
    {
        global $UserSettings;
        ...
        if ($this->force_order_by_count !== null && ! empty($order_request)) { // Check if we should force an order filed to default value
            if ($this->get_total_rows() > $this->force_order_by_count) { // This table has very much records we should force an order to default
                ...
                if ($order_request != $this->default_order && $order_request != $reverse_default_order) { // If an order from request is not default then we must change it to default
                    $this->order = $this->default_order;
                    $order_request_title = $order_request;
                    if (isset($this->cols)) { // Try to find a title of the ordered field to display it in warning message
        ...
        if (empty($this->order)) { // Set an order from GET request
            $this->order = $order_request;
        }

        if (! empty($this->param_prefix) &&
            ! empty($this->order) &&
            $this->order != $UserSettings->get($this->order_param)) {	// Change an order param in DB for current user and current list
            if ($this->order == $this->default_order) {	// Delete an order param for current list if it is a default value
                $UserSettings->delete($this->order_param);
            } else {	// Set a new value of an order param for current list
                $UserSettings->set($this->order_param, $this->order);
            }
            $UserSettings->dbupdate();
        }

        if (! empty($this->param_prefix) && empty($this->order)) {	// Set an order param from DB
            if ($UserSettings->get($this->order_param) != '') {	// Set a value for current list if it was already defined
                $this->order = $UserSettings->get($this->order_param);
            }
        }

        if (empty($this->order)) {	// Set a default value
            $this->order = $this->default_order;
        }
    }
    ...
    {
        if (! isset($this->total_rows)) { // Count total rows to be able to display pages correctly
    ...
    public function run_query(
        ...
        if (empty($this->sql)) { // the sql query is empty so we can't process it
            return;
        }

        if (isset($this->total_rows) && intval($this->total_rows) === 0) { // The number of total rows was counted and the sql query has no results
            return;
        }

        // Make sure we have colum definitions:
        if (is_null($this->cols) && $create_default_cols_if_needed) { // Let's create default column definitions:
            $this->cols = [];
            ...
            foreach (preg_split('#\s*,\s*#', $matches[1]) as $l_select) {
                if (is_numeric($l_select)) { // just a single value (would produce parse error as '$x$')
                    $this->cols[] = [
                        'td' => $l_select,
                    ];
                } elseif (preg_match('#^(\w+)$#i', $l_select, $match)) { // regular column
                    $this->cols[] = [
                        'td' => '$' . $match[1] . '$',
                    ];
                } elseif (preg_match('#^(.*?) AS (\w+)#i', $l_select, $match)) { // aliased column
                    $this->cols[] = [
                        'td' => '$' . $match[2] . '$',
                    ];
                }
        ...
        if ($append_order_by && ($orders = $this->get_order_field_list())) {	// We have orders to append
            if (! preg_match('# \s ORDER \s+ BY \s+ [^\)]+$#xi', $sql)) { // there is no ORDER BY clause in the original SQL query at the end(excluding order clause in sub queries)
                $sql .= ' ORDER BY ' . $orders . ' ';
            } else { // try to insert the chosen order at an existing '*' point
                if (preg_match('#[^\s]+\s(DESC|ASC)#i', $orders, $match_orders)) { // Set an order direction for additional order field to the same value as it has first order field
                ...
                if ($inserted_sql != $sql) { // Insertion ok:
                    $sql = $inserted_sql;
                } else { // No insert point found:
                    // the chosen order must be appended to an existing ORDER BY clause
                    $sql .= ', ' . $orders;
                }
            }
        } else {	// Make sure there is no * in order clause:
            $sql = preg_replace('# \s ORDER \s+ BY (.+) \* #xi', ' ORDER BY $1 ', $sql);
        }

        $add_limit = $append_limit && ! empty($this->limit);

        if ($add_limit && ! $this->order_callbacks) {	// No callbacks to be called, so we can limit the line range to the requested page:
        ...
        if ($this->order_callbacks) {
            ...
            foreach ($this->order_callbacks as $order_callback) {
                ...
                if (empty($order_callback['use_rows'])) { // default: instantiate objects for the callback:
                    usort($this->rows, [&$this, 'order_callback_wrapper_objects']);
                } else {
                    usort($this->rows, [&$this, 'order_callback_wrapper_rows']);
                }
            }
        ...
        if (! empty($this->group_by_obj_prop)) {
            if (! is_array($this->group_by_obj_prop)) {
                $this->group_by_obj_prop = [$this->group_by_obj_prop];
            }

            $this->mergesort($this->rows, [&$this, 'callback_group_by_obj_prop']);
        }
     ...
     * @return integer
     */
    public function callback_group_by_obj_prop($row_a, $row_b, $depth = 0)
    {
        ...
        if ($a_value == $b_value) {
            ...
            } else { // on the last level of grouping:
                return 0;
            }
        }
     ...
     * @return integer
     */
    public function order_callback_wrapper_objects($row_a, $row_b)
    {
        $a = $this->Cache->instantiate($row_a);
        $b = $this->Cache->instantiate($row_b);

        return (int) call_user_func(
            $this->order_callback_wrapper_data['callback'],
            $a,
            $b,
            $this->order_callback_wrapper_data['order']
        );
    }
     ...
     * @return integer
     */
    public function order_callback_wrapper_rows($row_a, $row_b)
    {
        return (int) call_user_func(
            $this->order_callback_wrapper_data['callback'],
            $row_a,
     ...
     */
    public function count_total_rows($sql_count = null)
    {
        global $DB;

        if (is_integer($sql_count)) {	// we have a total already
            $this->total_rows = $sql_count;
        } else { // we need to query
            if (is_null($sql_count)) {
                if (is_null($this->sql)) { // We may want to remove this later...
                    $this->total_rows = 0;
                    $this->total_pages = 0;
                    return;
                }
                ...
                } else { // Single table request: we must NOT use field names in the count.
        ...
        if (empty($this->limit)) {	// If no page limiting, Display all results on single page:
            $this->total_pages = $this->total_rows > 0 ? 1 : 0;
        } else {	// If the results should be limited by page size:
     ...
     */
    public function instantiate_page_to_Cache(&$Cache)
    {
        $this->Cache = &$Cache;
     ...
     */
    public function display($display_params = null, $fadeout = null)
    {
        ...
        if (! is_ajax_content()) { // Display TITLE/FILTERS only if NO AJAX request
            // DISPLAY TITLE:
            if (isset($this->title)) { // A title has been defined for this result set:
        ...
        if (! isset($this->params['disable_evo_flush']) || ! $this->params['disable_evo_flush']) {	// Flush in order to show the filters before slow SQL query will be executed below
            evo_flush();
        }
        ...
        if (! is_ajax_content()) { // Display COL SELECTION only if NO AJAX request
        ...
        if ($this->total_pages == 0) { // There are NO RESULTS! Nothing to display!
            // START OF LIST/TABLE:
            $this->display_list_start();

            // END OF LIST/TABLE:
            $this->display_list_end();
        } else { // We have rows to display:
     ...
     * @param array Fadeout settings array( 'key column' => array of values ) or 'session'
     */
    public function display_init($display_params = null, $fadeout = null)
    {
        // Lazy fill $this->params:
        parent::display_init($display_params, $fadeout);

        // Make sure query has executed and we're at the top of the resultset:
        if ((! isset($this->total_rows)) || ((intval($this->total_rows) > 0) && (! isset($this->rows)))) { // The result is not empty but the rows are not set yet, the query needs to be executed
            $this->restart();
        }
    }
    ...
    public function display_body()
    {
        ...
        if (! empty($this->group_by)) {
            $group_by_all['row'] = is_array($this->group_by) ? $this->group_by : [$this->group_by];
        }
        if (! empty($this->group_by_obj_prop)) {
            $group_by_all['obj_prop'] = is_array($this->group_by_obj_prop) ? $this->group_by_obj_prop : [$this->group_by_obj_prop];
        }

        $this->current_group_count = []; // useful in parse_col_content()

        foreach ($this->rows as $row) { // For each row/line:
            /*
             * GROUP ROW stuff:
             */
            if (! empty($group_by_all)) {	// We are grouping (by SQL and/or object property)...
                $group_depth = 0;
                $group_changed = false;
                ...
                if ($group_changed) { // We have just entered a new group!
                    ...
                    $col_count = 0;
                    foreach ($this->grp_cols as $grp_col) { // For each column:
                        if (isset($grp_col['td_class'])) {	// We have a class for the total column
                            $class = $grp_col['td_class'];
                        } else {	// We have no class for the total column
                            $class = '';
                        }

                        if (($col_count == 0) && isset($this->params['grp_col_start_first'])) { // Display first column column start:
                            $output = $this->params['grp_col_start_first'];

                            // Add the total column class in the grp col start first param class:
                            $output = str_replace('$class$', $class, $output);
                        } elseif (($col_count == count($this->grp_cols) - 1) && isset($this->params['grp_col_start_last'])) { // Last column can get special formatting:
                            $output = $this->params['grp_col_start_last'];

                            // Add the total column class in the grp col start end param class:
                            $output = str_replace('$class$', $class, $output);
                        } else { // Display regular column start:
                        ...
                        if (isset($grp_col['td_colspan'])) {
                            ...
                            if ($colspan < 0) { // We want to substract columns from the total count
                                $colspan = $this->nb_cols + $colspan;
                            } elseif ($colspan == 0) { // use $nb_cols
                                $colspan = $this->nb_cols;
                            }
                            $output = str_replace('$colspan_attrib$', 'colspan="' . $colspan . '"', $output);
                        } else { // remove non-HTML attrib:
             ...
             * DATA ROW stuff:
             */
            if (! empty($this->ID_col) && empty($row->{$this->ID_col})) {	// We have detected an empty data row which we want to ignore... (happens with empty groups)
                continue;
            }

            if (! is_null($this->Cache)) { // We want to instantiate an object for the row and cache it:
            ...
            if (! empty($this->fadeout_array)) {
                foreach ($this->fadeout_array as $key => $crit) {
                    ...
                    if (isset($row->$key) && in_array($row->$key, $crit)) { // Col is in the fadeout list
                        // TODO: CLEAN THIS UP!
                        $fadeout_line = true;
                        break;
                    }
            ...
            foreach ($this->cols as $col) { // For each column:
                if (isset($this->current_colspan) && $this->current_colspan > 1) {	// Skip this column because previous column has a colspan:
    ...
    public function display_totals()
    {
        ...
        foreach ($this->cols as $col) {
            if (isset($col['total'])) {	// We have to display a totals line
                $total_enable = true;
                break;
            }
        }

        if ($total_enable) { // We have to dispaly a totals line
            ...
            foreach ($this->cols as $col) {
                if (isset($col['total_class'])) {	// We have a class for the total column
                    $class = $col['total_class'];
                } else {	// We have no class for the total column
                    $class = '';
                }

                if ($loop == 0) {	// The column is the first
                    $output = $this->params['total_col_start_first'];
                    // Add the total column class in the total col start first param class:
                    $output = str_replace('$class$', $class, $output);
                } elseif ($loop == (count($this->cols) - 1)) {	// The column is the last
                ...
                if (isset($col['total'])) {	// The column has a total set, so display it:
                    $output = $col['total'];
                    $output = $this->parse_col_content($output);
                    eval("echo '$output';");
                } else {	// The column has no total
                    echo '&nbsp;';
                }
                // </td>
     ...
     */
    public function display_functions()
    {
        if (empty($this->functions_area)) {	// We don't want to display a functions section:
            return;
        }

        echo $this->replace_vars($this->params['functions_start']);

        if (! empty($this->functions_area['callback'])) {	// We want to display functions:
            if (is_array($this->functions_area['callback'])) {	// The callback is an object function
                $obj_name = $this->functions_area['callback'][0];
                if ($obj_name != 'this') {	// We need the global object
                    global $$obj_name;
                }
                $func = $this->functions_area['callback'][1];

                if (isset($this->Form)) {	// There is a created form
                    $$obj_name->$func($this->Form);
                } else { // There is not a created form
                    $$obj_name->$func();
                }
     ...
     */
    public function display_nav($template)
    {
        if (empty($this->limit) && isset($this->params[$template . '_text_no_limit'])) { // No LIMIT (there's always only one page)
            $navigation = $this->params[$template . '_text_no_limit'];
        } elseif (($this->total_pages <= 1)) { // Single page (we probably don't want to show navigation in this case)
        ...
        if (! empty($navigation_text)) { // Display navigation only when it is really filled with some text
     ...
     */
    public function display_list_end()
    {
        if ($this->total_pages == 0) { // There are no results! Nothing to display!
            ...
            if (! empty($this->checkbox_toggle_selectors)) {
                ...
                if (! empty($this->list_mass_actions)) {	// Additional actions:
            ...
            if (! empty($this->list_mass_actions) && isset($this->Form)) {	// Start form for list with mass actions:
                $this->Form->end_form();
            }
        }
    }
     ...
     * @return array
     */
    public function get_col_sort_values($col_idx)
    {
        // Current order:
        ...
        for ($i = 0; $i < $this->nb_cols; $i++) {
            if ($i == $col_idx) { // Link ordering the current column
                $order_asc .= 'A';
                $order_desc .= 'D';
            } else {
                $order_asc .= '-';
        ...
        if (! $col_sort_values['current_order'] && isset($this->cols[$col_idx]['default_dir'])) {	// There is no current order on this column and a default order direction is set for it
            // So set a default order direction for it

            if ($this->cols[$col_idx]['default_dir'] == 'A') {	// The default order direction is A, so set its toogle  order to the order_asc
                $col_sort_values['order_toggle'] = $col_sort_values['order_asc'];
            } else { // The default order direction is A, so set its toogle order to the order_desc
                $col_sort_values['order_toggle'] = $col_sort_values['order_desc'];
            }
        } elseif ($col_sort_values['current_order'] == 'ASC') {	// There is an ASC current order on this column, so set its toogle order to the order_desc
            $col_sort_values['order_toggle'] = $col_sort_values['order_desc'];
        } else { // There is a DESC or NO current order on this column,  so set its toogle order to the order_asc
            $col_sort_values['order_toggle'] = $col_sort_values['order_asc'];
        }
     ...
     */
    public function get_order_field_list()
    {
        if (is_null($this->order_field_list)) { // Order list is not defined yet
            if ((! empty($this->order)) && (! empty($this->cols)) && (substr($this->order, 0, 1) == '/')) { // order is set in format '/order_field_name/A' or '/order_field_name/D'
                $order_parts = explode('/', $this->order);
                $this->order = '';
                if ((count($order_parts) == 3) && (($order_parts[2] == 'A') || ($order_parts[2] == 'D'))) {
                    foreach ($this->cols as $col) { // iterate thrugh columns and find matching column name
                        if (isset($col['order']) && ($col['order'] == $order_parts[1])) { // we have found the requested orderable column
            ...
            if (empty($this->order)) { // We have no user provided order:
                if (empty($this->cols)) {	// We have no columns to pick an automatic order from:
                ...
                $this->order = '';
                foreach ($this->cols as $col) {
                    if (isset($col['order']) || isset($col['order_objects_callback']) || isset($col['order_rows_callback'])) { // We have found the first orderable column:
                        $this->order .= 'A';
                        break;
                    } else {
                        $this->order .= '-';
            ...
            for ($i = 0; $i <= strlen($this->order); $i++) {	// For each position in order string:
                if (isset($this->cols[$i]['order'])) {	// if column is sortable:
                ...
                if (isset($this->cols[$i]['order_objects_callback'])) {	// if column is sortable by object callback:
                    switch (substr($this->order, $i, 1)) {
                        case 'A':
                            $this->order_callbacks[] = [
                ...
                if (isset($this->cols[$i]['order_rows_callback'])) {	// if column is sortable by callback:
                    switch (substr($this->order, $i, 1)) {
                        case 'A':
                            $this->order_callbacks[] = [
     ...
     */
    public function parse_col_content($content)
    {
        ...
        $content = str_replace(
            ['{CUR_IDX}', '{TOTAL_ROWS}', '{ROW_IDX_TYPE}'],
            [$this->current_idx, $this->total_rows, "'" . $this->get_row_order_type() . "'"],
            $content
        );
     ...
     */
    public function move_icons()
    {
        $r = '';

        $reg = '#^' . $this->param_prefix . 'order (ASC|DESC).*#';

        if (preg_match($reg, $this->order_field_list, $res)) {	// The table is sorted by the order column
            $sort = $res[1];
            ...
            // Move up arrow
            if ($this->global_is_first) {	// The element is the first so it can't move up, display a no move arrow
                $r .= get_icon('nomove') . ' ';
            } else {
                if ($sort == 'ASC') {	// ASC sort, so move_up action for move up arrow
                    $action = 'move_up';
                    $alt = T_('Move up') . '!';
                } else {	// Reverse sort, so action and alt are reverse too
                    $action = 'move_down';
            ...
            // Move down arrow
            if ($this->global_is_last) {	// The element is the last so it can't move up, display a no move arrow
                $r .= get_icon('nomove') . ' ';
            } else {
                if ($sort == 'ASC') {	// ASC sort, so move_down action for move down arrow
                    $action = 'move_down';
                    $alt = T_('Move down') . '!';
                } else { // Reverse sort, so action and alt are reverse too
                    $action = 'move_up';
            ...
            return $r;
        } else {	// The table is not sorted by the order column, so we display no move arrows
            if ($this->global_is_first) {
                // The element is the first so it can't move up, display a no move up arrow
                $r = get_icon('nomove') . ' ';
            } else {	// Display no move up arrow
            ...
            if ($this->global_is_last) {
                ...
                $r .= get_icon('nomove') . ' ';
            } else { // Display no move down arrow
    ...
    public function replace_callback($matches)
    {
        ...
        switch ($matches[1]) {
            ...
            case 'prev':
                ...
                $attr_rel = [];
                if ($this->nofollow_pagenav) { // We want to NOFOLLOW page navigation
                    $attr_rel[] = 'nofollow';
                }
                ...
                if (! empty($this->params['prev_class'])) { // Add "class" for prev link
            ...
            case 'next':
                ...
                if (! empty($this->params['next_class'])) { // Add "class" for prev link
    ...
    {
        if ($this->page <= intval($this->params['list_span'] / 2)) { // the current page number is small
            return 1;
        } elseif ($this->page > $this->total_pages - intval($this->params['list_span'] / 2)) { // the current page number is big
    ...
    {
        if ($this->page > $this->total_pages - intval($this->params['list_span'] / 2)) { //the current page number is big
            return $this->total_pages;
        } else {
     ...
     */
    public function display_first($page_url = '')
    {
        $r = '';
        ...
        if ($this->page == 1) {	// no link for the current page
        ...
        } else {	// a link for non-current pages
     ...
     */
    public function display_last($page_url = '')
    {
        $r = '';
        ...
        if ($this->page == $this->total_pages) {	// no link for the current page
            if (! isset($this->params['page_current_template'])) {
                $this->params['page_current_template'] = '<strong class="current_page">$page_num$</strong>';
            }
     ...
     */
    public function display_prev($page_url = '')
    {
        if ($this->first() > 2) { //the list has to be displayed
     ...
     */
    public function display_next($page_url = '')
    {
        if ($this->last() < $this->total_pages - 1) { //the list has to be displayed
     ...
     */
    public function display_page_size($page_url = '')
    {
        ...
        if (is_logged_in()) {	// Get default page size for current user
            global $UserSettings;

            $default_page_size_value = $UserSettings->get('results_per_page');
            $default_page_size_option = [
                '0' => sprintf(T_('Default (%s lines)'), $default_page_size_value),
            ];
     ...
     */
    public function page_list($min, $max, $page_url = '')
    {
        $hidden_active_distances = [1, 2];
        ...
        for ($i = $min; $i <= $max; $i++) {
            ...
            if (in_array($active_dist, $hidden_active_distances) && ($i < $this->page) && ($i > 2) && ($this->page > 4)) { // show pseudo prev_list
                $page_no = ceil($this->first() / 2);
                if ($page_no == 1) {
                    $page_no++;
                }
            ...
            if ($i == $this->page && isset($this->params['page_item_current_before'])) { // current page
                $list .= $this->params['page_item_current_before'];
            } elseif (isset($this->params['page_item_before'])) { // other page
                if ($active_dist) {
            ...
            if ($i == $this->page) { // no link for the current page
                if (! isset($this->params['page_current_template'])) {
                    $this->params['page_current_template'] = '<strong class="current_page">$page_num$</strong>';
                }
                ...
                if ($page_prev_i == $i) { // Add attribute rel="prev" for previous page
                    $attr_rel[] = 'prev';
                } elseif ($page_next_i == $i) { // Add attribute rel="next" for next page
                    $attr_rel[] = 'next';
                }
                if (! empty($attr_rel)) {
            ...
            if ($i != $max) { // Separator between page numbers
                if (isset($this->params['page_list_separator'])) { // Use the customized separator from skin
                    $list .= $this->params['page_list_separator'];
                } else { // Use a space as default separator
                    $list .= ' ';
                }
            }

            if (in_array($active_dist, $hidden_active_distances) && ($i > $this->page) && ($i < ($this->total_pages - 1))) { // show pseudo next_list
                $page_no = $this->last() + floor(($this->total_pages - $this->last()) / 2);
     ...
     */
    public function page_scroll_list()
    {
        $scroll = '';
        $i = 0;
        ...
        if ($range > $this->total_pages) { //the range is greater than the total number of pages, the list goes up to the number of pages
            $max = $this->total_pages;
        } else { //initialisation of the range
            $max = $range;
        }
        ...
        while ($max <= $this->total_pages) { //construction loop
            if ($this->page <= $max && $this->page >= $min) { //display all the pages belonging to the range where the current page is located
                for ($i = $min; $i <= $max; $i++) { //construction of the <option> tags
            ...
            } else { //inits the ranges inside the list
                $range_display = '<option value="' . $min . '">'
                    . T_('Pages') . ' ' . $min . ' ' . /* TRANS: Pages x _to_ y */ T_('to') . ' ' . $max;
                $scroll = $scroll . $range_display;
            }

            if ($max + $range > $this->total_pages && $max != $this->total_pages) { //$max has to be the total number of pages
                $max = $this->total_pages;
            } else {
     ...
     */
    public function display_if_empty($params = [])
    {
    ...
    {
        if (! is_array($this->messages)) { // Initialize array only first time
            $this->messages = [];
        }
    ...
    {
        if (empty($this->messages)) { // No messages, Don't print
            return;
        }
     ...
     */
    public function get_row_order_type()
    {
        if ($this->total_rows == 1) { // Only one row in list
            return 'single';
        } elseif ($this->page == 1 && $this->current_idx == 0) { // First row
            return 'first';
        } elseif (($this->page == $this->total_pages) && ($this->current_idx == $this->total_rows - (($this->total_pages - 1) * $this->limit) - 1)) { // Last row
            return 'last';
        } else { // Middle row
            return 'middle';
        }
