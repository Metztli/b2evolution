...
class Table extends Widget
{
     ...
     */
    public $displayed_lines_count;

    /**
     * Number of cols already displayed (in current line)
     */
    public $displayed_cols_count;

    /**
     * @var array
     */
    public $fadeout_array;

    public $fadeout_count = 0;

    /**
     * @var boolean
     */
    public $is_fadeout_line;
     ...
     */
    public $current_filter_preset = null;

    /**
     * Parameters for the columns select area:
     */
    public $colselect_area;

    /**
     * Columns preset button that is currently active:
     */
    public $current_colselect_preset = null;
     ...
     */
    public function __construct($ui_template = null, $param_prefix = '')
    {
    ...
    public function display_init($display_params = null, $fadeout = null)
    {
        ...
        if (empty($this->params) && isset($AdminUI)) { // Use default params from Admin Skin:
            $this->params = $AdminUI->get_template('Results');
        }

        // Make sure we have display parameters:
        if (! is_null($display_params)) { // Use passed params:
        ...
        if ($fadeout == 'session') {	// Get fadeout_array from session:
     ...
     */
    public function register_filter_preset($preset_codename, $preset_label, $preset_url)
    {
        // Append a preset
        $this->filter_area['presets'][$preset_codename] = [
            $preset_label,
     ...
     * @param object Form
     */
    public function display_advanced_filter_fields(&$Form)
    {
        if (empty($this->filter_area['callback_advanced'])) {	// No callback function is defined for advanced filters:
            return;
        }
        ...
        if (! is_array($filter_fields)) {	// Don't allow wrong callback for advanced filters:
        ...
        foreach ($filter_fields as $field_ID => $params) {
            if ($field_ID == '#default') {	// Skip a reserved field for default filters:
                continue;
            }

            $js_filter = ['id:\'' . $field_ID . '\''];

            if (isset($params['type'])) {	// Set default params depending on field type:
                switch ($params['type']) {
                    case 'date':
            ...
            if (! isset($params['operators'])) {	// Use default operator if it is not defined:
                $params['operators'] = '=,!=';
            }
                ...
                switch ($param_name) {
                    case 'operators':
                        ...
                        if (! empty($param_value)) {
                            ...
                            foreach ($operators as $o => $operator) {	// Replace aliases with corrent name which is used in jQuery QueryBuilder plugin:
                    ...
                    case 'input':
                        if (strpos($param_value, 'function') === 0) {	// Don't convert this param to string if it is a function:
                            break;
                        }

                        // no break
                    default:
                        if (is_array($param_value)) {	// Array param:
                            $param_values = [];
                            foreach ($param_value as $sub_param_name => $sub_param_value) {
                                if ($sub_param_value == 'true' || $sub_param_value == 'false' ||
                                    strpos($sub_param_value, '[') === 0) {	// This is a not string value:
        ...
        if (empty($filter_query) || $filter_query === 'null') {	// Set filter values if no request yet:
            $filter_query = [
                'rules' => [],
                'valid' => true,
            ];
            if (isset($filter_fields['#default'])) {	// Set filters from default config:
...
<?php
     ...
     */
    public function display_colselect()
    {
        if (empty($this->colselect_area)) {	// We don't want to display a col selection section:
            return;
        }

        if (empty($this->param_prefix)) {	// Deny to use a list without prefix
        ...
        echo '<span onclick="toggle_filter_area(\'' . $option_name . '\')"'
            ...
            . get_icon('filters_show', 'imgtag', [
                'id' => 'clickimg_' . $option_name,
            ])
            . ' ' . T_('Columns')
        . '</span>';
    ...
    public function display_filters()
    {
        ...
        if (empty($this->filter_area)) {	// We don't want to display a filters section:
            return;
        }
        ...
        if ($this->current_filter_preset !== null) {	// Store new preset in Session:
            $Session->set($preset_name, $this->current_filter_preset);
            $Session->dbsave();
        }
        if ($this->current_filter_preset === null) {	// Try to get preset from Session:
            $this->current_filter_preset = $Session->get($preset_name);
        }
        if ($this->current_filter_preset === null) {	// Use 'all' preset filter by default:
        ...
        // Display all presers;
        if (! empty($this->filter_area['presets'])) {	// Display preset filters:
        ...
        if (! empty($this->filter_area['callback'])) {	// "Custom preset" with JS toggle to reveal form:
            echo '<span onclick="toggle_filter_area(\'' . $custom_option_name . '\')"'
                    ...
                    . get_icon(($custom_fold_state == 'collapsed' ? 'filters_show' : 'filters_hide'), 'imgtag', [
                        'id' => 'clickimg_' . $custom_option_name,
                    ])
                    . ' ' . T_('Custom filters')
                . '</span>';
        }

        if (is_admin_page() && ! empty($this->filter_area['callback_advanced'])) {	// "Advanced preset" with JS toggle to reveal form ONLY on back-office:
            echo '<span onclick="toggle_filter_area(\'' . $advanced_option_name . '\')"'
                    ...
                    . get_icon(($advanced_fold_state == 'collapsed' ? 'filters_show' : 'filters_hide'), 'imgtag', [
                        'id' => 'clickimg_' . $advanced_option_name,
                    ])
                    . ' ' . T_('Advanced filters')
                . '</span>';
        }
        ...
        // ---------------- CUSTOM FILTERS ---------------- :
        if (! empty($this->filter_area['callback'])) {	// Display Custom Filters Form fields:
            ...
            if ($create_new_form) {	// We do not already have a form surrounding the whole results list:
            ...
            if (! isset($this->filter_area['apply_filters_button']) || $this->filter_area['apply_filters_button'] != 'none') {	// Display a filter button only when it is not hidden by param:  (Hidden example: BackOffice > Contents > Posts)
                echo $this->params['filter_button_before'];
                $submit_name = empty($this->filter_area['submit']) ? 'colselect_submit' : $this->filter_area['submit'];
                $this->Form->button_input([
                    'tag' => 'button',
                    'name' => $submit_name,
            ...
            if (! empty($this->force_checkboxes_to_inline)) { // Set this to TRUE in order to display all checkboxes before labels
            ...
            if (method_exists($this, 'save_filterset')) {	// For table that can save filterset we should set new filter on each form submit:
                $this->Form->hidden('filter', 'new');
            }

            if ($create_new_form) { // We do not already have a form surrounding the whole result list:
                $this->Form->end_form('');
        ...
        // ---------------- ADVANCED FILTERS ---------------- :
        if (is_admin_page() && ! empty($this->filter_area['callback_advanced'])) {	// Display Advanced Filters Form fields ONLY on back-office:
            ...
            if (isset($this->filter_area['advanced_defaults_jsfunc'])) {	// Display a button to change default advanced filters:
     ...
     */
    public function display_list_start()
    {
        if ($this->total_pages == 0) { // There are no results! Nothing to display!
            if (isset($this->filter_area['is_filtered']) && ! $this->filter_area['is_filtered']) {	// If this table list is filtered and no results then we should collapse the filter area because nothing to filter:
            ...
            echo str_replace([' $list_class$', ' $list_attrib$'], [$list_class, $list_attrs], $this->params['list_start']);
        }
    }
    ...
    {
        if (is_ajax_content()) {	// Don't display this content on AJAX request
            return;
        }
     ...
     */
    public function display_col_headers()
    {
        echo $this->params['head_start'];

        if (isset($this->cols)) {
            if (! isset($this->nb_cols)) {	// Needed for sort strings:
            ...
            foreach ($this->cols as $col) {
                if (isset($col['th_group'])) {	// We have a th_group column, so break:
            ...
            foreach ($this->cols as $key => $col) {
                ...
                if (isset($col['th_group'])) {	// The column has a th_group
                    if (is_null($current_th_group_title) || $col['th_group'] != $current_th_group_title) {	// It's the begining of a th_group colspan (line0):
                    ...
                    } else {	// The column is part of a th group colspan
                ...
                if (is_null($current_th_title) || $col['th'] != $current_th_title) {	// It's the begining of a th colspan (line1)
                    ...
                    if ($th_group_activated && ! isset($col['th_group'])) { // We have to lines and the column has no th_group, so it will be a "rowspan2"
                    ...
                    } else {	// The cell has no rowspan
                        $header_cells[1][$key]['colspan'] = 1;
                        $header_cells[1][$key]['rowspan'] = 1;
                    }
                } else {	// The column is part of a th colspan
                    if ($th_group_activated && ! isset($col['th_group'])) {	// We have to lines and the column has no th_group, the colspan is "a rowspan 2"
                        ...
                        $header_cells[0][$key]['colspan'] = 0;
                        $header_cells[0][$key]['rowspan'] = 0;
                    } else { // Update the first th colspan cell in line1
            ...
            if (! $th_group_activated) {	// We have only the "th" line to display
                $start = 1;
            } else {	// We have the "th_group" and the "th" lines to display
                $start = 0;
            }
            ...
            for ($i = $start; $i < 2; $i++) {
                ...
                foreach ($header_cells[$i] as $key => $cell) {
                    if ($cell['colspan']) {	// We have to dispaly cell:
                        if ($i == 0 && $cell['rowspan'] != 2) {	// The cell is a th_group
                            $th_title = $this->cols[$key]['th_group'];
                            $col_order = isset($this->cols[$key]['order_group']);
                        } else {	// The cell is a th
                        ...
                        if (isset($this->cols[$key]['th_class'])) {	// We have a class for the th column
                            $class = $this->cols[$key]['th_class'];
                        } else {	// We have no class for the th column
                            $class = '';
                        }

                        if ($key == 0 && isset($this->params['colhead_start_first'])) { // Display first column start:
                            $output = $this->params['colhead_start_first'];

                            // Add the total column class in the grp col start first param class:
                            $output = str_replace('$class$', $class, $output);
                        } elseif (($key + $cell['colspan']) == (count($this->cols)) && isset($this->params['colhead_start_last'])) { // Last column can get special formatting:
                            $output = $this->params['colhead_start_last'];

                            // Add the total column class in the grp col start end param class:
                            $output = str_replace('$class$', $class, $output);
                        } else { // Display regular colmun start:
                        ...
                        if (isset($this->cols[$key]['th_title'])) { // Column header title is set
                            $output = str_replace('$title_attrib$', ' title="' . $this->cols[$key]['th_title'] . '"', $output);
                        } else { // Column header title is not set, replace with empty string
                        ...
                        if ($col_order) { // The column can be ordered:
                            ...
                            if (! empty($col_sort_values['current_order'])) { // We are currently sorting on the current column:
                                $class_suffix = '_current';
                            } else {	// We are not sorting on the current column:
                                $class_suffix = '_sort_link';
                            }

                            // Display title depending on sort type/mode:
                            if ($this->params['sort_type'] == 'single') { // single column sort type:
                                // Title with toggle:
                            ...
                            } else { // basic sort type (toggle single column):
                                if ($col_sort_values['current_order'] == 'ASC') { // the sorting is ascending and made on the current column
                                    $sort_icon = $this->params['basic_sort_asc'];
                                } elseif ($col_sort_values['current_order'] == 'DESC') { // the sorting is descending and made on the current column
                                    $sort_icon = $this->params['basic_sort_desc'];
                                } else { // the sorting is not made on the current column
                        ...
                        } elseif ($th_title) { // the column can't be ordered, but we still have a header defined:
    ...
    public function display_line_start($is_last = false, $is_fadeout_line = false)
    {
        if ($this->displayed_lines_count % 2) { // Odd line:
            if ($is_last) {
        ...
        if ($is_fadeout_line) {	// Add css class for this row to highlight:
            $start_tag = update_html_tag_attribs($start_tag, [
                'class' => 'evo_highlight',
            ]);
        }
    ...
    public function display_line_end()
    {
        echo $this->params['line_end'];
     ...
     */
    public function display_col_start($extra_attr = [], $row = null)
    {
        // Get colum definitions for current column:
        $col = $this->cols[$this->displayed_cols_count];

        if (isset($col['td_class'])) {	// We have a class for the total column
            $class = $col['td_class'];
        } else {	// We have no class for the total column
            $class = '';
        }

        if (($this->displayed_cols_count == 0) && isset($this->params['col_start_first'])) { // Display first column column start:
            $output = $this->params['col_start_first'];
            // Add the total column class in the col start first param class:
            $output = str_replace('$class$', $class, $output);
        } elseif (($this->displayed_cols_count == count($this->cols) - 1) && isset($this->params['col_start_last'])) { // Last column can get special formatting:
        ...
        if (isset($col['td_colspan'])) {	// Initialize colspan attribute:
            ...
            } elseif ($colspan == 0) {	// Use a count of columns:
                $colspan = $this->nb_cols;
            }
            $colspan = intval($colspan);
            if ($colspan === 1) {	// Don't create attribute "colspan" with value "1":
    ...
    public function display_col_end()
    {
    ...
    public function replace_callback($matches)
    {
        ...
        switch ($matches[1]) {
            case 'reset_filters_button':
                ...
                if (! isset($this->filter_area['presets']['all'])) {	// Preset "all" not defined, we don't know how to reset.
                    return '';
                }
                if (empty($this->current_filter_preset) || $this->current_filter_preset == 'all') { // No filters applied:
                    return '';
                }
     ...
     * - #var#
     */
    public function parse_class_content($content)
    {
        // Make variable substitution for RAWS:
        while (preg_match('!\# (\w+) \#!ix', $content, $matchesarray)) { // Replace all matches to the content of the current row's cell. That means that several variables can be inserted to the class.
     ...
     */
    public function init_params_by_skin($skin_type, $skin_name)
    {
        switch ($skin_type) {
