...
class Widget
{
     ...
     */
    public function __construct($ui_template = null)
    {
        global $AdminUI;

        if (! empty($ui_template)) { // Get template params from Admin Skin:
     ...
     */
    public function global_icon($title, $icon, $url, $word = '', $icon_weight = 3, $word_weight = 2, $link_attribs = [], $group = null, $group_options = null)
    {
        $link_attribs = array_merge([
            'class' => 'action_icon',
            'before' => '',
            'after' => '',
        ], $link_attribs);
     ...
     */
    public function disp_template_raw($param_name)
    {
     ...
     */
    public function disp_template_replaced($param_name)
    {
     ...
     */
    public function replace_vars($template, $params = null)
    {
        if (! is_null($params)) {
            $this->params = $params;
        }
        ...
        return preg_replace_callback(
            '~\$([a-z_]+)(?:\s+"([^"]*)")?\$~', # pattern
            [$this, 'replace_callback_wrapper'], # callback
            $template
        );
    }
     ...
     * @return string
     */
    public function replace_callback_wrapper($match)
    {
     ...
     */
    public function gen_global_icons()
    {
        global $AdminUI;
        ...
        foreach ($this->global_icons as $icon_params) {
            if (isset($this->params, $this->params['global_icons_class'])) { // Append a link class from global params
            ...
            if ($icon_params['group'] != null) { // It is a groupped button
                if (! isset($icons[$icon_params['group']])) { // Initialize an array for grouped icons:
                    $icons[$icon_params['group']] = [];
                }
                ...
                $icons[] = $before . action_icon(
                    $icon_params['title'],
                    $icon_params['icon'],
                    $icon_params['url'],
                    $icon_params['word'],
                    $icon_params['icon_weight'],
                    $icon_params['word_weight'],
                    $icon_params['link_attribs']
                ) . $after;
            }
        }
        ...
        foreach ($icons as $group => $icon) {
            if (is_array($icon) && count($icon)) { // Grouped icons
                $first_icon = $icon[0];
                ...
                foreach ($this->global_icons as $icon_params) {
                    if (isset($icon_params['group_options']['parent']) && $icon_params['group_options']['parent'] == $group) {	// Append also items from others buttons if they a linked with this button by group:
