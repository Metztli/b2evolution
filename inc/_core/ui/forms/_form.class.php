...
class Form extends Widget
{
     ...
     * @var array
     */
    public $append_javascript = [];

    /**
     * Display param errors with fields, appended to the note?
     * @var boolean
     */
    public $disp_param_err_messages_with_fields = true;
     ...
     * @var array
     */
    public $included_input_field_names = [];

    /**
     * Indicates if the form is ended
     * @var boolean
     */
    public $_form_ended = false;
     ...
     * @var boolean
     */
    public $force_checkboxes_to_inline;
     ...
     */
    public function __construct($form_action = null, $form_name = '', $form_method = 'post', $layout = null, $enctype = '', $form_type = 'form')
    {
        global $pagenow;
        ...
        $this->saved_layouts = [$layout];
        $this->saved_templates = [$template];
        ...
        if (strpos($this->form_action, '?') !== false && $this->form_method == 'get') {
            ...
            foreach ($query_args as $query_arg) {
                ...
                $this->possible_hiddens_from_action[] = [$field_name, $field_value];
            }
            // Remove params.
     ...
     */
    public function get_template(&$layout, $use_default_template = false)
    {
        global $AdminUI, $Skin;
        ...
        if (is_object($AdminUI)) {
            if ($layout == 'fieldset' || ($use_default_template && (empty($layout) || $layout == 'split' || $layout == 'none'))) { // Get default skin setting
                $template = $AdminUI->get_template('Form', 0, $use_default_template);
            } else { // Get special form skin
                $template = $AdminUI->get_template($layout . '_form', 0, $use_default_template);
            }
            if (is_array($template) && isset($template['layout'])) { // Get layout name from template
                $layout = $template['layout'];
            }
        } else { // This happens for comment forms & login screen for example...
            $template_is_empty = true;
            if (is_object($Skin)) { // Get skin setting:
                if ($layout == 'fieldset' || ($use_default_template && (empty($layout) || $layout == 'split' || $layout == 'none'))) { // Get default skin setting:
     ...
     */
    public function switch_layout($layout, $use_default_layout = true)
    {
        if ($layout == null) { // we want to restore previous layout:
            if (count($this->saved_layouts)) {
                ...
                if (! empty($template)) {
                    ...
                    if (isset($template['global_icons_class'])) { // Set class for global icons
        ...
        } else { // We want to switch to a new layout
            // Try to get template of current skin
            $template = $this->get_template($layout, false);

            if (empty($template) && ! $use_default_layout) { // Don't use default layout
                return;
            }
            ...
            if (is_array($template) && ! empty($template)) { // Template is detected on current skin, Use it
                if (! isset($template['fieldset_title'])) {	// Set default fieldset title if old template doesn't define it:
     ...
     *    formend
     */
    public function switch_template_parts($parts)
    {
        if (is_array($parts) && count($parts) > 0) {	// Change a default form params from defined vars on current skin
     ...
     */
    public function begin_field($field_name = null, $field_label = null, $reset_common_params = false, $type = '')
    {
        ...
        $field_classes = [];
        if (! empty($this->_common_params['wide']) && $this->_common_params['wide']) { // Wide field
            $field_classes[] = 'field_wide';
        }
        if (! empty($this->_common_params['inline']) && $this->_common_params['inline']) { // Inline field
        ...
        } else { // No ID in case there's no id/name given for a field.
            $ffield_id = '';
        }
        // quick and dirty "required_field" addition, needs tidying once tested
        if (! empty($this->_common_params['required'])) { // required field
            $field_classes[] = 'field_required';
        }

        if (isset($this->{'fieldstart_' . $type})) { // Use special field start for element with type
            $r = $this->{'fieldstart_' . $type};
        } else { // Use default field start
            $r = $this->fieldstart;
        }
        if (! empty($this->_common_params['hide']) && $this->_common_params['hide']) {	// Hidden field
            $r = preg_replace('/>$/', ' style="display:none">', $r);
        }

        if (count($field_classes) > 0) {
            if (strpos($r, 'class="') === false) { // Add class attribute
                $ffield_id .= ' class="' . implode(' ', $field_classes) . '"';
            } else { // Append classes to attribute
        ...
        if ($type == 'info') { // Use info start:
            $r .= $this->infostart;
        } elseif (isset($this->{'inputstart_' . $type})) { // Use special input start for element with type:
            $r .= $this->{'inputstart_' . $type};
        } else { // Use default input start:
            $r .= $this->inputstart;
        }
     ...
     * @return The generated HTML
     */
    public function end_field($type = '')
    {
        $r = '';

        if (! empty($this->_common_params['note'])) { // We have a note
            if (! empty($this->is_lined_fields)) { // Fix note format for fields that are used in one line
        ...
        if ($type == 'info') { // Use info end:
            $r .= $this->infoend;
        } elseif (isset($this->{'inputend_' . $type})) { // Use special input end for element with type:
            $r .= $this->{'inputend_' . $type};
        } else { // Use default input end:
            $r .= $this->inputend;
        }

        if (isset($this->{'fieldend_' . $type})) { // Use special field end for element with type
            $r .= $this->{'fieldend_' . $type};
        } else { // Use default field end
            $r .= $this->fieldend;
        }
     ...
     */
    public function begin_group($params = [])
    {
        $params = array_merge([
            'id' => 'evo_accordion',
            'class' => '',
        ...
        if (! empty($params['id'])) {	// If group id is defined:
        ...
        $r = str_replace(
            ['$group_class$', '$group_attribs$'],
            [$params['class'], get_field_attribs_as_string($field_params)],
            $this->group_begin
        );
    ...
    public function end_group()
    {
        ...
        if (isset($this->current_group_ID)) {	// Unset current group ID and counter of group items:
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function open_tab_pane($tab_pane_params = [])
    {
        $tab_pane_params = array_merge([
            'class' => 'tab-pane fade',
        ], $tab_pane_params);
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function close_tab_pane()
    {
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function begin_fieldset($title = '', $field_params = [])
    {
        ...
        if ($field_params['fold']) { // Display icon to collapse/expand fildset
            $folding_icon = get_fieldset_folding_icon($field_params['id'], $field_params);
            if (! $field_params['deny_fold'] && is_logged_in()) { // Only loggedin users can fold fieldset
                ...
                if ($value === null && $field_params['default_fold'] !== null) {	// Use custom default value for this fieldset:
        ...
        if (! empty($this->fieldset_title)) {	// Replace text part of fieldset title with provided html code:
            $fieldset_title = str_replace('$fieldset_title$', '$1', $this->fieldset_title);
            $title = preg_replace('/^([^<]+)/', $fieldset_title, $title);
        }

        if (isset($this->current_group_ID)) {	// If fieldset is grouped, Replace the group masks with values:
            $group_item_ID = $this->current_group_ID . '_item_' . $this->current_group_item_num;
            $title = str_replace(['$group_ID$', '$group_item_ID$'], [$this->current_group_ID, $group_item_ID], $title);
        ...
        switch ($this->layout) {
            case 'table':
                ...
                if ($title != '') { // there is a title to display
                    $r .= $title;
                }
            ...
            default:
                if (! empty($field_params['legend_params'])) { // We have params specifically passed for the title
                ...
                if (empty($legend_params)) { // there are no legend_params, remove the placeholder
                    $r = str_replace('$title_attribs$', '', $r);
                } else {
                    $r = str_replace('$title_attribs$', get_field_attribs_as_string($legend_params), $r);
                }

                if (isset($group_item_ID)) {	// Replace a mask of current group item ID with value:
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function text_input($field_name, $field_value, $field_size, $field_label, $field_note = '', $field_params = [])
    {
        $field_params = array_merge([
            'inline' => false,
            'type' => 'text',
        ...
        if (isset($field_params['force_to'])) {
            if ($field_params['force_to'] == 'UpperCase') { // Force input to uppercase (at front of onchange event)
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function color_input($field_name, $field_value, $field_label, $field_note = '', $field_params = [])
    {
        $field_params = array_merge([
            'inline' => false,
            'type' => 'text',
        ...
        if ($field_params['transparency']) {	// Set class to initialize colorpicker with transparency option:
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function password_input($field_name, $field_value, $field_size, $field_label, $field_params = [])
    {
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function email_input($field_name, $field_value, $field_size, $field_label, $field_params = [])
    {
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function file_input($field_name, $field_value, $field_label, $field_note = '', $field_params = [])
    {
        $field_params = array_merge([
            'type' => 'file',
            'value' => $field_value,
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function username($field_name, &$User, $field_label, $field_note = '', $field_class = '', $field_params = [])
    {
        $field_params = array_merge([
            'type' => 'text',
            'note' => $field_note,
        ...
        if (isset($required_param)) { // restore original value of field param 'required'
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function login_input($field_name, $field_value, $field_size, $field_label, $field_note = '', $field_params = [])
    {
        $field_params = array_merge([
            'size' => $field_size,
            'class' => 'input_text',
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function date_input($field_name, $field_value, $field_label, $field_params = [])
    {
        global $month, $weekday_letter;

        if (empty($field_params['date_format'])) {	// Use locale date format:
        ...
        $js_date_format = preg_replace_callback('~(\\\)?(\w)~', ['Form', '_date_input_format_callback'], $date_format);

        // Get max length of each date component
        $js_date_length = preg_replace_callback('~(\\\)?(\w)~', ['Form', '_date_input_length_callback'], $date_format);

        $field_params['type'] = 'text';

        if (param_has_error($field_name)
            && ! preg_match('~^\d\d\d\d-\d\d-\d\d(?: \d\d:\d\d:\d\d)?$~', $field_value)) { // There is an error message for this field:
        ...
        } else { // Make the date value clean for display:
            // The date value may be compact, in this case we have to decompact it
            if (preg_match('/^[0-9]+$/', $field_value)) {	// The date is compact, so we decompact it
            ...
            if (! $field_params['value']) { // Conversion failed (e.g. for dates before ~1902 / PHP < 5.1 (Windows) / 32bit / without DateTime support), use the original value (better than 1970-01-01 anyway!).
                $field_params['value'] = $field_value;
            }
        }

        if (! empty($field_params['add_date_format_note'])) { // Prepend $date_format to note
        ...
        if (! isset($field_params['size'])) { // Get size out of $date_format if not explicitly set
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function date($field_name, $field_value, $field_label, $date_format = null)
    {
        $field_params = [
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function time_input($field_name, $field_value, $field_label, $field_params = [])
    {
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function time($field_name, $field_value, $field_label, $field_format = 'hh:mm:ss', $note = null)
    {
        $field_params = [
    ...
    public function time_select($field_name, $field_label, $field_value = null, $precision = '5mn', $field_note = null, $field_class = null, $field_onchange = null)
    {
        ...
        if (! isset($matches[1]) && ! isset($matches[2])) {	// precison has a bad format
            return;
        }
        ...
        if (strlen($field_value) <= 8) {	// Add date part:
        ...
        if ($modulo < ($precision / 2)) { // The round time is before
            $nbsec -= $modulo;
        } else { // The round time is after
        ...
        if ($precision_s) {/*********  select options for the minutes  ***********/
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function duration_input($field_prefix, $duration, $field_label, $from_subfield = 'days', $to_subfield = 'minutes', $field_params = [])
    {
        $field_params = array_merge([
        ...
        if ($field_params['allow_none_value']) {	// Allow null value:
        ...
        $r .= "\n" . '<select name="' . $field_prefix . '_name" id="' . Form::get_valid_id($field_prefix) . '_name"' . $field_class
                . (isset($field_params['required']) && $field_params['required'] == 'required' ? ' required="required"' : '') . '>';
        if ($field_params['allow_none_title']) {	// Allow none period name:
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function duration($field_prefix, $duration, $field_label)
    {
     ...
     * @return true|string
     */
    public function dayOfWeek_input($field_name, $field_value, $field_label, $field_params = [])
    {
     ...
     * @return true|string
     */
    public function dayOfWeek($field_name, $field_value, $field_label, $field_note = null, $field_class = null)
    {
        $field_params = [];
        if (isset($field_note)) {
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function checkbox_input($field_name, $field_checked, $field_label, $field_params = [])
    {
        $field_params = array_merge([
            'value' => 1,
            'name' => $field_name,
        ...
        if (! empty($this->force_checkboxes_to_inline) && ! isset($field_params['inline'])) { // Force to display checkbox before label
     ...
     */
    public function checkbox_basic_input($field_name, $field_checked, $field_label, $field_params = [])
    {
        $field_params = array_merge([
            'name' => $field_name,
            'type' => 'checkbox',
    ...
    {
        return $this->checkbox_controls('$all$', [
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function checkbox_controls($field_name = '$all$', $params = [])
    {
        $params = array_merge([
        ...
        $r .= $button_tag_start . '="check">' . get_icon('check_all', 'imgtag', [
            'class' => $params['icon_class'],
        ]) . '</button> ';
        $r .= $button_tag_start . '="uncheck">' . get_icon('uncheck_all', 'imgtag', [
            'class' => $params['icon_class'],
        ]) . '</button>';
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function begin_form($form_class = null, $form_title = '', $form_params = [])
    {
        global $use_strict;
        if ($use_strict) {
            unset($form_params['target']);
        }// target isn't valid for XHTML Strict
        ...
        if (isset($form_params['title'])) { // Additional title when $form_title has html tags. It is used for js confirmation message on leave the changed form.
            $clear_title = $form_params['title'];
            unset($form_params['title']);
        }

        if (isset($form_params['formstart_class'])) { // CSS class for formstart tag
            $formstart_class = $form_params['formstart_class'];
            unset($form_params['formstart_class']);
        }

        if ($this->form_type == 'div') { // Use <div> tag instead of <form>
            unset($form_params['action']);
            unset($form_params['method']);
            unset($form_params['enctype']);
            $r = "\n\n<div" . get_field_attribs_as_string($form_params) . ">\n";
        } else { // Standard form
            if ($form_params['action'] == '') { // Remove action attribute instead of an empty one which is not HTML4/5 compliant
                unset($form_params['action']);
            }
        ...
        if (empty($form_title)) {
            if (empty($this->global_icons)) {	// No title, no icons:
                $r .= $this->replace_vars($this->no_title_no_icons_fmt);
            } else { // No title, but there are icons:
                $r .= $this->replace_vars($this->no_title_fmt);
            }
        } else {	// Title and icons:
        ...
        if (isset($UserSettings) && $UserSettings->get('control_form_abortions')
            && preg_match('#^(.*)_checkchanges#', $this->form_name)) { // This form will trigger the bozo validator, preset a localized bozo confirm message:
            $r .= '<script>
     ...
     * @return true|string true (if output) or the generated HTML if not outputting.
     */
    public function end_form($buttons = [])
    {
        if ($this->_form_ended) {	// The form is already ended
            return;
        }
        ...
        if ($this->form_type == 'div') { // Use <div> tag instead of <form>:
            ...
            if ($this->append_javascript) { // Append Javascript that we have added
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function checklist($options, $field_name, $field_label, $required = false, $dummy = null, $field_params = [])
    {
        $field_params = array_merge([
            'wide' => false,
            'type' => 'checkbox',
        ...
        foreach ($options as $option) { //loop to construct the list of 'input' tags
            $loop_field_name = $option[0];

            if (! empty($option[7])) {	// Print out this checkbox as hidden field:
                if ($option[3]) {	// Only if it is checked:
            ...
            $after_field = '';
            if (param_has_error($field_name)) { // There is an error message for this field, we want to mark the checkboxes with a red border:
                $r .= '<span class="checkbox_error">';
                $after_field .= '</span>';
            }
            if ($required) {	//this field is required
                $r .= '<span class="checkbox_required">';
                $after_field .= '</span>';
            }

            $r .= "\t" . '<input type="checkbox" name="' . $loop_field_name . '" value="' . $option[1] . '" ';
            if ($option[3]) { //the checkbox has to be checked by default
                $r .= ' checked="checked" ';
            }
            if (isset($option[4]) && $option[4]) { // the checkbox has to be disabled
                $r .= ' disabled="disabled" ';
            }
            ...
            if (! empty($loop_field_note)) { // We want to display a note:
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function select_input($field_name, $field_value, $field_list_callback, $field_label, $field_params = [])
    {
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function select_input_object($field_name, $field_value, &$field_object, $field_label, $field_params = [])
    {
        ...
        $field_options = '';
        if (isset($field_params['prepend_options'])) {	// Prepend additional options before cached object (Used to use several none options):
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function select_country($field_name, $field_value, &$field_object, $field_label, $field_params = [])
    {
        global $edited_User;

        if (isset($field_params['required'])) {	// 'allow_none' param value should depend on 'required' param value:
        ...
        if (isset($field_params['object_callback'])) {
            ...
            if (empty($edited_User->ctry_ID)) {	// if edited user didn't select a country then we form a dropdown list of countries using 'get_group_country_option_list' function.
                $field_object_callback = 'get_group_country_option_list';
            } else {	// if edited user has a selected country then we display a usual list of countries.
                $field_object_callback = 'get_option_list';
            }
        }
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function select_input_options($field_name, $field_options, $field_label, $field_note = '', $field_params = [])
    {
        $this->handle_common_params($field_params, $field_name, $field_label, $field_note);

        $r = $this->begin_field();
        if (! empty($field_params['parent'])) {	// need to display an arrow to show that this select list options has a preselection from a parent
        ...
        if (! empty($field_params['parent'])) { // Set up the dynamic preselection array from the parent to this select list options
            $r .= "<script>
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function select_input_array($field_name, $field_value, $field_options, $field_label, $field_note = null, $field_params = [])
    {
     ...
     * @return string
     */
    public static function get_select_options_string($field_options, $field_value = null, $force_keys_as_values = false, $color_array = false)
    {
        $r = '';

        foreach ($field_options as $l_key => $l_option) {
            if (is_array($l_option)) {	// If option is array then it is a group of the options:
                $r .= Form::get_select_group_options_string([
                    $l_key => $l_option,
                ], $field_value, $force_keys_as_values, $color_array);
                continue;
            }
     ...
     * @return string
     */
    public static function get_select_group_options_string($group_options, $field_value = null, $force_keys_as_values = false, $color_array = false)
    {
        $r = '';
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function combo_box($field_name, $field_value, $field_options, $field_label, $field_params = [])
    {
        // Default params:
        $field_params = array_merge([
            ...
            'new_field_params' => [], // Additional attributes except of the defined below (Array: key - attr name, value - attr value)
        ...
        $input_params = array_merge($field_params['new_field_params'], [
        ...
        if ($field_params['placeholder'] !== null) {	// Use placeholder only when it is defined:
            $input_params['placeholder'] = $field_params['placeholder'];
        }

        if (param_has_error($field_name)) {	// There is an error on the combo, so we need to add error style class to the combo input:
            $input_params['class'] .= ' ' . $field_params['new_field_class_error'];
        } elseif ($field_params['required']) {	// The field is required, so update its class:
        ...
        if ($is_input_hidden) {	// Add style to hide the input on page loading:
        ...
        if ($is_input_hidden) {	// The input text is hidden, so if no javascript activated, we must always display the input text to enter new option:
     ...
     */
    public function textarea_input($field_name, $field_value, $field_rows, $field_label, $field_params = [])
    {
        global $rsc_url;
        ...
        if (isset($field_params['input_prefix'])) { // Some text should be displayed before textarea element
            $input_prefix = $field_params['input_prefix'];
            unset($field_params['input_prefix']);
        } else { // Nothing before textarea element
            $input_prefix = '';
        }
     ...
     * @param string Placeholder text
     */
    public function textarea($field_name, $field_value, $field_rows, $field_label, $field_note = '', $field_cols = 50, $field_class = '', $required = false, $placeholder = '')
    {
        $field_params = [
            'note' => $field_note,
            'cols' => $field_cols,
        ...
        if ($required) { // Set required only for case TRUE, because in the following code we have a condition "isset($required)" instead of "$required == true"
            $field_params['required'] = $required;
        }
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function info_field($field_label, $field_info, $field_params = [])
    {
        $field_params = array_merge([
            'note_format' => ' <small class="notes">%s</small>',
        ], $field_params);
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function info($field_label, $field_info, $field_note = null, $format = 'htmlbody')
    {
        $field_params = [
            'note' => $field_note,
            'format_info' => $format,
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function buttons_input($buttons = [])
    {
        $r = '';
        ...
        foreach ($buttons as $l_button) {
            if (! isset($l_button['type']) || $l_button['type'] != 'hidden') { // not a hidden button
                $hidden = false;
            }
        ...
        if (! $hidden) { // there are not only hidden buttons : additional tags
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function button_input($field_params = [])
    {
        $field_params = array_merge([
            'tag' => 'input',
            'type' => 'submit',
        ...
        if (empty($field_params['type'])) { // default type
            $field_params['type'] = 'submit';
        }

        // Use bootstrap classes for buttons
        $field_params['class'] .= ' btn';
        if (strpos($field_params['class'], 'btn-') === false) { // Only when it is not defined from skin
            if (empty($this->btn_primary_is_used) &&
                    (strpos($field_params['class'], 'SaveButton') !== false ||
                      strpos($field_params['class'], 'SaveEditButton') !== false)) { // Submit button
            ...
            } elseif (strpos($field_params['class'], 'ResetButton') !== false) { // Reset button
                $field_params['class'] .= ' btn-danger';
            } elseif (strpos($field_params['class'], 'PreviewButton') !== false) { // Preview button
                $field_params['class'] .= ' btn-info';
            } elseif (strpos($field_params['class'], 'SmallButton') !== false) { // Small button
                $field_params['class'] .= ' btn-xs';
            }
            if (! preg_match('/btn\-(primary|success|info|warning|danger)/', $field_params['class'])) { // This button is default
     ...
     */
    public function convert_button_to_field_params($options)
    {
        $field_params = [];
        ...
        foreach (array_keys($options) as $l_key) {
            if (is_int($l_key)) {
                switch ($l_key) {
                    case 0: $field_params['type'] = $options[0];
                        break;
                    case 1: $field_params['name'] = $options[1];
                        break;
                    case 2: $field_params['value'] = $options[2];
                        break;
                    case 3: $field_params['class'] = $options[3];
                        break;
                    case 4: $field_params['onclick'] = $options[4];
                        break;
                    case 5: $field_params['style'] = $options[5];
                        break;
                }
            } else {
     ...
     */
    public function hidden($field_name, $field_value, $field_params = [])
    {
        if (is_array($field_value)) { // this happens for example when we've POSTed an array (for PHP it's an array then)
            ...
            if (strpos($field_name, '[]')) { // array-style name or we don't want to overwrite, just add it:
     ...
     */
    public function add_crumb($crumb_name)
    {
    ...
    public function hiddens($hiddens)
    {
        ...
        foreach ($hiddens as $hidden) {
            $this->hidden($hidden[0], $hidden[1], isset($hidden[2]) ? $hidden[2] : []);
        }
        $this->output = $save_output;
    }
     ...
     */
    public function hiddens_by_key($hiddens, $exclude = null)
    {
        if ($this->output) { // only save output once, if necessary (recursion!)
            $save_output = $this->output;
            $this->output = false;
        }
        ...
        foreach ($hiddens as $l_name => $l_value) {
            ...
            if (is_array($l_value)) { // this happens for example when we've POSTed an array (for PHP it's an array then)
                foreach ($l_value as $ll_key => $ll_value) {
                    // Recursion:
                    $this->hiddens_by_key([
                        $l_name . '[' . $ll_key . ']' => $ll_value,
                    ], $exclude);
                }
            } else {
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function submit_input($field_params = [])
    {
        $field_params['type'] = 'submit';
        return $this->button_input($field_params);
    }
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function radio_input($field_name, $field_value, $field_options, $field_label, $field_params = [])
    {
        $field_params = array_merge([
        ...
        foreach ($field_options as $loop_radio) {
            ...
            // Defaults:
            if (! isset($loop_radio['type'])) {
                $loop_radio['type'] = 'radio';
            }
            if (! isset($loop_radio['class'])) {
                $loop_radio['class'] = $this->inputclass_radio;
            } // 'radio'
            if (! isset($loop_radio['name'])) {
                $loop_radio['name'] = $field_name;
            }
            if (! isset($loop_radio['id'])) { // build unique id:
            ...
            if (isset($loop_radio['checked'])) { // convert boolean:
                if ($loop_radio['checked']) {
                    $loop_radio['checked'] = 'checked';
                }
            } elseif ($field_value == $loop_radio['value']) { // Current selection:
                $loop_radio['checked'] = 'checked';
            }
            ...
            // the label:
            $r .= str_replace(['$radio_option_ID$', '$radio_option_label$'], [$loop_radio['id'], $label], $this->radio_label_format);

            if (! empty($note)) { // Add a note for the current radio option:
            ...
            if ($field_lines) { // End of radio option for multi line format
                $r .= $this->radio_newline_end;
            } else { // End of radio option for single line format
     ...
     * @return mixed true (if output) or the generated HTML if not outputting
     */
    public function radio($field_name, $field_value, $field_options, $field_label = '', $field_lines = false, $field_note = '', $field_required = false)
    {
        $new_field_options = [];

        foreach ($field_options as $l_key => $l_options) {
            $new_field_options[$l_key] = [
                'value' => $l_options[0],
                'label' => $l_options[1],
            ];
            ...
            if (isset($l_options[4])) { // Convert "inline attribs" to "params" array
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function interval($field_name_from, $field_value_from, $field_name_to, $field_value_to, $field_size, $field_label, $field_note = '', $field_params = [])
    {
        $save_output = $this->output;
        $save_params = [
        ...
        if (isset($field_params['input_prefix'])) { // Save an input prefix and append it only to input "From"
            $input_prefix = $field_params['input_prefix'];
            unset($field_params['input_prefix']);
        }
        ...
        if (isset($input_prefix)) { // Display an input prefix before first input "From"
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function input_field($field_params = [])
    {
        $field_params = array_merge([
            'inline' => false,
        ], $field_params);
        ...
        if ($field_params['inline']) { // Display label and input field in one line
        ...
        if (! $field_params['inline']) { // Append input field
            $r .= $element;
        }
        $r .= $this->end_field($input_type);

        if ($field_params['inline']) { // Restore the changed form elements in mode 'inline'
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function fileselect($field_name, $field_value, $field_label, $field_note = '', $field_params = [])
    {
        ...
        if (! isset($thumbnail_sizes[$field_params['size_name']])) { // Wrong thumbnail size name
     ...
     */
    public function usertag_input($field_name, $field_value, $field_size, $field_label, $field_note = '', $field_params = [])
    {
     ...
     */
    public function tag_input($field_name, $field_value, $field_size, $field_label, $field_note = '', $field_params = [])
    {
        ...
        if (isset($field_params['required']) && $field_params['required'] === true) {	// We can only mark this field as required, the actual input is hidden and cannot be focused on when validation fails
     ...
     */
    public function get_input_element($field_params = [], $parse_common = true)
    {
        if ($parse_common) {
        ...
        if (isset($field_params['inline'])) { // Delete 'inline' param from attributes list
            unset($field_params['inline']);
        }

        if (isset($field_params['hide_label'])) { // Delete 'hide_label' param from attributes list
            unset($field_params['hide_label']);
        }

        if (isset($field_params['input_required'])) { // Set html attribute "required" (used to highlight input with red border/shadow by bootstrap)
            $field_params['required'] = $field_params['input_required'];
            unset($field_params['input_required']);
        }

        if (isset($field_params['disabled'])) {	// Set html attribute "disabled":
            if (empty($field_params['disabled'])) {	// Don't set attribute if it must be not disabled:
                unset($field_params['disabled']);
            } elseif ($field_params['disabled'] === true) {	// Use proper string value instead of boolean:
                $field_params['disabled'] = 'disabled';
            }
     ...
     * @return string The <input /> element.
     */
    public function get_button_element($field_params = [], $parse_common = true)
    {
        if ($parse_common) {
            $this->handle_common_params($field_params);
        }
     ...
     * @return string
     */
    public static function get_valid_id($id)
    {
        static $id_count = 0;
        if (substr($id, -2) == '[]') {
    ...
    public function get_label()
    {
        ...
        if (isset($this->_common_params['hide_label']) && $this->_common_params['hide_label']) { // Hide label if it is required by param
            return $r;
        }
        ...
        if (strlen($label)) {
            ...
            if (isset($this->_common_params['clickable_label']) && ! $this->_common_params['clickable_label']) {	// Not set if this method is invoked by ::begin_field()
                ...
                if (empty($this->is_lined_fields)) { // Don't print a label tag for "lined" mode:
                    $r .= '<label'
                ...
                if (empty($this->is_lined_fields)) { // Don't print a label tag and suffix for "lined" mode:
                    $r .= $this->label_suffix;
     ...
     */
    public function handle_common_params(&$field_params, $field_name = null, $field_label = null, $field_note = null)
    {
        #pre_dump( 'handle_common_params (before)', $field_params );

        $this->_common_params = []; // Reset
        ...
        if (! empty($field_params['name'])) {
            if (! isset($field_params['id'])) { // Autogenerate id attrib (not for hidden, radio and submit types)
                if (empty($field_params['type'])
                        || (
                            $field_params['type'] != 'hidden'
        ...
        if (isset($this->_common_params['required']) && $this->_common_params['required']) { // add "field_required" class:
            if (isset($field_params['type']) && $field_params['type'] == 'checkbox') { // checkboxes need a span
        ...
        // Error handling:
        if (isset($field_params['name']) && param_has_error($field_params['name'])) { // There is an error message for this field:
            if (isset($field_params['type']) && $field_params['type'] == 'checkbox') { // checkboxes need a span
     ...
     */
    public function display_or_return($r)
    {
        if ($this->output) {
            echo $r;
            return true;
     ...
     * @param array Params
     */
    public function begin_line($field_label = null, $field_name = null, $field_type = '', $field_params = [])
    {
        $this->handle_common_params($field_params, $field_name, $field_label);
     ...
     * @param array Params
     */
    public function end_line($suffix_text = null, $field_type = '', $field_params = [])
    {
        $this->handle_common_params($field_params);

        $r = '';

        if (! is_null($suffix_text)) { // Display a suffix:
            $r .= $suffix_text;
        }
     ...
     */
    public function attachments_fieldset($object, $fold = false, $fieldset_prefix = '')
    {
        ...
        if ($object_type != 'Comment' && ! is_logged_in()) {	// User must logged in for all other objects like Item, Message and EmailCampaign:
        ...
        switch ($object_type) {
            case 'Comment':
                ...
                if (! $comment_Item->check_blog_settings('allow_attachments', $Comment) || ! $comment_Item->can_attach($Comment->temp_link_owner_ID, $Comment->type)) {	// Comment attachments must be allowed by collection setting depending on user type(anonymous, registered, member and etc.).:
                    return;
                }
            ...
            case 'Item':
                $Item = $object;
                if (! $Item->get_type_setting('allow_attachments')) {	// Item attachments must be allowed for the item type
                    return;
                }
            ...
            case 'Message':
                if (! is_admin_page()) {	// Message attachments are allowed only on back-office
                    return;
                }
                $Message = $object;
     ...
     * @param array Params
     */
    public function locale_selector($field_name, $main_locale, $extra_locales, $field_label, $field_note = '', $field_params = [])
    {
        global $locales;
        ...
        foreach ($locales as $locale_key => $locale_data) {
            if (! $locale_data['enabled']) {	// Skip disabled locale:
                continue;
            }
        ...
        if ($link_Blog) {	// JavaScript to link with collections:
     ...
     * @return true|string true (if output) or the generated HTML if not outputting
     */
    public function item_selector($field_name, $selected_item_ID, $field_label, $field_params = [])
    {
        global $thumbnail_sizes, $file_select_js_initialized;
        ...
        $r .= '<input' . get_field_attribs_as_string([
            'type' => 'hidden',
            'id' => $field_name,
            'name' => $field_name,
        ...
        if (! $selected_Item) {	// Hide the deselector icon if no selected Item yet:
        ...
        if (empty($this->item_selector_js_initialized)) {	// Initialize JS code for Item selector once:
