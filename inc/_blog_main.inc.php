...
if ($show_toolbar !== null && $show_toolbar !== 'hidden') {	// Convert all not string possible values to boolean type:
...
if (isset($ReqURL)) {	// Check protocol of the current URL:
    if ($Blog->get_setting('http_protocol') == 'always_http') {	// Check current URL is really using HTTP protocol as it is specified with collection setting "SSL" = "Always use http":
        if (strpos($ReqURL, 'https://') === 0) {	// Redirect to URL with allowed protocol because current URL has a protocol HTTPS instead of expected HTTP:
    ...
    } elseif ($Blog->get_setting('http_protocol') == 'always_https') {	// Check current URL is really using HTTPS protocol as it is specified with collection setting "SSL" = "Always use https":
        if (strpos($ReqURL, 'http://') === 0) {	// Redirect to URL with allowed protocol because current URL has a protocol HTTP instead of expected HTTPS:
...
if ($debug == 2 || is_logged_in()) {	// Allow debug info only for logged-in users OR when debug == 2:
    // Enable/Disable designer mode:
    $designer_mode = param('designer_mode', 'string');
    if ($designer_mode == 'enable' && $Session->get('customizer_mode_' . $blog)) {	// Allow to enable designer mode only together with enabled customizer mode:
...
if ($Session->get('customizer_mode_' . $Blog->ID) && $redir != 'no') {	// Redirect to customize collection if such mode is enabled:
    header_redirect($Blog->get('customizer_url', [
        'glue' => '&',
    ]));
}
...
if ($Blog->get_setting('locale_source') == 'blog' ||
    ($Blog->get_setting('locale_source') == 'user' && ! $Blog->has_locale($current_locale))) { // Activate main collection locale when this is defined in settings of current collection
...
if ($coll_locale !== null) {	// Overriding locale from REQUEST with extra collection locale:
    $Debuglog->add('Overriding collection locale from REQUEST: ' . $coll_locale, 'locale');
    if ($Blog->has_locale($coll_locale)) {	// If locale is selected for current collection:
        locale_activate($coll_locale);
    } else {	// Wrong collection locale is requested:
...
if (init_charsets($current_charset)) {
    ...
    if (is_logged_in()) { // We also need to reload the current User with the new final charset
...
if ($resolve_extra_path) {
    ...
    if ($coll_baseuri_matched_in_url = preg_match('~(^' . preg_quote($coll_baseuri, '~') . '|\.php[0-9]*/)(.*)$|\.php[0-9]*$~', $sanitized_ReqPath, $matches)) {  // Either the ReqPath starts with collection base URI (always including trailing slash) followed by some extra path info.
        ...
        if (preg_match('~/([a-zA-Z0-9._\-:;]+)/?$~', $sanitized_ReqPath, $matches)) {
            ...
            if ($last_part != $Blog->stub  // Ignore stub file (if it ends with .php it should already have been filtered out above
                 && $last_part != $Blog->urlname) {	// e-g: http://localhost/x/y/image-post instead of http://localhost/x/y/index.php/a/image-post
    ...
    if (count($path_elements)) {
        ...
        if ($Blog->get_setting('tag_links') == 'prefix-only'
            ...
            if (! $posts = $Blog->get_setting('tag_posts_per_page')) { // use blog default
            ...
            if (($last_char == '-' && (! $tags_dash_fix || $last_len != 40))   // In very old b2evo version we had ITEM slugs truncated at 40 and possibly ending with `-`
                || $last_char == ':'
                || $last_char == ';') {	// - : or ; -> We'll consider this to be a tag page
                ...
                $disp = 'posts';
                    ...
                    substr($path_elements[0], 0, $user_page_prefix_length) == $user_page_prefix) {	// Alias for disp=user:
                ...
                if ($User = &$UserCache->get_by_login($user_request) ||
                        (is_number($user_request) && $User = &$UserCache->get_by_ID($user_request, false, false))) {	// If user is detected in DB by login or ID:
                    $user_ID = $User->ID;
                }
            ...
            } elseif (($tags_dash_fix && $last_char == '-' && $last_len == 40) || $last_char != '/') {	// NO ENDING SLASH or ends with a dash, is 40 chars long and $tags_dash_fix is true
                ...
                if (preg_match('#^p([0-9]+)$#', $last_part, $req_post)) { // The last param is of the form p000
                    $p = $req_post[1];		// Post to display
                } else { // Last param is a string, we'll consider this to be a post urltitle
                    $title = $last_part;
                    $Debuglog->add('Post slug to look for: ' . $title, 'url_decode_part_2');
                }
            } else {	// ENDING SLASH -> we are looking for a daterange OR a chapter:
                $Debuglog->add('Last part: ' . $last_part, 'url_decode_part_2');
                // echo $last_part;
                if (preg_match('|^w?[0-9]+$|', $last_part)) { // Last part is a number or a "week" number:
                    $i = 0;
                    ...
                    if (isset($path_elements[$i])) {
                        if (preg_match('#^\d{4}$#', $path_elements[$i])) { // We'll consider this to be the year
                            $m = $path_elements[$i++];
                            ...
                            if (! $posts = $Blog->get_setting('archive_posts_per_page')) { // use blog default
                                $posts = $Blog->get_setting('posts_per_page');
                            }

                            if (isset($path_elements[$i]) && preg_match('#^(0[1-9]|1[0-2])$#', $path_elements[$i])) { // We'll consider this to be the month
                                $m .= $path_elements[$i++];
                                $Debuglog->add('Setting month from extra path info. $m=' . $m, 'url_decode_part_2');

                                if (isset($path_elements[$i]) && preg_match('#^(0[1-9]|[12][0-9]|3[01])$#', $path_elements[$i])) { // We'll consider this to be the day
                                    $m .= $path_elements[$i++];
                                    $Debuglog->add('Setting day from extra path info. $m=' . $m, 'url_decode_part_2');
                                }
                            } elseif (isset($path_elements[$i]) && preg_match('#^w(0?[0-9]|[1-4][0-9]|5[0-3])$#', $path_elements[$i])) { // We consider this a week number
                                $w = substr($path_elements[$i], 1, 2);
                            }
                            $disp = 'posts';
                        } else {	// We did not get a number/year...
                            $disp = '404';
                    ...
                    if (empty($Chapter)) {	// We could not match a chapter...
                    ...
                    } else {	// We could match a chapter from the extra path:
                        $cat = $Chapter->ID;
                        // Also use the prefered posts per page for a cat
                        if (! $posts = $Blog->get_setting('chapter_posts_per_page')) { // use blog default
                            $posts = $Blog->get_setting('posts_per_page');
                        }
                        $disp = 'posts';
                    }
                } else {	// We did not get anything we can decode...
    ...
    } elseif (! $coll_baseuri_matched_in_url && ! empty($path_elements)) { // Case of calling http://baseurl/slug when main coll is set as http://baseurl/index.php and we did not elect to always match the slug.
        $disp = '404';
...
if (! empty($p) || ! empty($title)) { // We are going to display a single post
    ...
    if (! empty($p)) {	// Get from post ID:
        ...
        if (empty($Item) && substr($title, -1) == '-') { // Try lookup by removing last invalid chars, which might have been e.g. > | "> | , | ,. | ">?!
        ...
        if (! empty($Item)) {
            ...
            if (! $Item->is_part_of_blog($blog)) {	// We have found an Item object, but it doesn't belong to the current collection!
                // Check if we want to redirect moved posts:
                if ($Settings->get('redirect_moved_posts')) {	// Set disp to 'redirect' in order to store this value in hitlog table:
                    $disp = 'redirect';
            ...
            if (! empty($Item) &&
                ...
                $canonical_url = $Item->get_permanent_url('', $Blog->get('url'), '&', [], $blog);
    ...
    if (empty($Item)) {	// Post doesn't exist!
        ...
        if (! $tag_fallback && ! empty($title) && empty($already_looked_into_chapters)) {	// Let's try to fall back to a category/chapter...
            ...
            if (! empty($Chapter)) {	// We could match a chapter from the extra path:
                $cat = $Chapter->ID;
                $title_fallback = true;
                $title = null;
                // Also use the prefered posts per page for a cat
        ...
        if (! empty($title)) {	// Let's try to fall back to a tag...
            $Debuglog->add('Trying to identify a Tag...', 'url_decode_part_2');
            if ($tag_fallback) {
                $title = substr($orig_title, 0, -1);
            }
            if ($Blog->get_tag_post_count($title)) { // We could match a tag from the extra path:
                $tag = $title;
                $title_fallback = true;
                $title = null;
            }
        ...
        if (! $title_fallback) {	// Let's try to fall back to a help slug...
            $Debuglog->add('Trying to identify a help slug..', 'url_decode_part_2');
            $SlugCache = &get_SlugCache();
            $Slug = &$SlugCache->get_by_name($title, false, false);
            if (! empty($Slug) && $Slug->get('type') == 'help') { // We could match a help slug from the extra path:
                $disp = 'help';
                $title_fallback = true;
                $title = null;
            }
        ...
        if (! $title_fallback) {	// We were not able to fallback to anything meaningful:
...
if (! empty($force_skin)) { // Set the forced skin from request
    if ($force_skin == 'auto') { // Delete the forced skin from Session to use default skin
        $Session->delete('force_skin');
    } else { // Save the forced skin in Session
        $Session->set('force_skin', $force_skin);
    }
} else { // Try to get a skin from session
    $force_skin = $Session->get('force_skin');
}
if (! empty($force_skin)) { // The forced skin is defined in request or in Session
...
if (! empty($tempskin)) { // This will be handled like any other skin:
    // TODO: maybe restrict that to authorized users
    $skin = $tempskin;
    if (empty($disp) || $disp == '-') { // Set default disp for RSS skins
        $disp = 'posts';
    }
}
 ...
 */
if ($stats || $disp == 'stats') {	// This used to be a spamfest...
    siteskin_init();
    siteskin_include('_410_stats_gone.main.php'); // error
    exit(0);
    // EXIT.
} elseif (! empty($preview)) {	// Preview
    memorize_param('disp', 'string', 'single', 'single');
    // Consider this as an admin hit!
    $Hit->hit_type = 'admin';
} elseif ($disp == '-' && ! empty($Item)) { // We have not requested a specific disp but we have identified a specific post to be displayed
    // We are going to display a single post
    if (in_array($Item->get_type_setting('usage'), ['special', 'content-block'])) {	// Display 404 page for all "Content Blocks" and "Special" items intead of normal single page:
        $disp = '404';
...
} elseif ($disp == '-' || ($disp == 'front' && $disp == $Blog->get_setting('front_disp'))) {	// No specific request of any kind OR
    $requested_disp = $disp;
    ...
    if ($disp == 'single') {	// We must find first item from disp=posts and display it on front page:
        if ($Item = &$Blog->get_first_mainlist_Item()) {	// The item is found, Use it:
            memorize_param('p', 'integer', $Item->ID, $Item->ID);
            $c = 1; // Display comments
        }

        if (empty($Item)) {	// If item is not found, display 404 page with below error message:
            $Messages->add(sprintf(T_('Front page is set to display first post but there is nothing to display.'), $p), 'error');
        }
    } elseif ($disp == 'page') { // Specific page is displayed on front page
        ...
        if (empty($Item) || ! in_array($Item->get_type_setting('usage'), ['page', 'widget-page'])) {	// Display error when page or widget-page Item is not found:
            $Messages->add(sprintf(T_('Front page is set to display page ID=%d but it does not exist.'), $p), 'error');
        } elseif ($Item->get_type_setting('usage') == 'widget-page') {	// Switch to proper disp for Widget-Page Item in order to set correct filters on init $MainList:
            memorize_param('disp', 'string', 'widget_page', 'widget_page');
        }
    }
}

if ($disp == 'terms') {	// Display a page of terms & conditions:
    $terms_item_ID = intval($Settings->get('site_terms'));
    if ($Settings->get('site_terms_enabled') && $terms_item_ID > 0) {	// Only if item ID is defined for terms page:
        ...
        if (is_logged_in() && $UserSettings->get('terms_accepted', $current_User->ID)) {	// Display the message if current user already accepted the terms:
...
if (! empty($is_front)) {	// Do we need to handle the canoncial url for front page?
    if (($Blog->get_setting('canonical_homepage') && $redir == 'yes')
        || $Blog->get_setting('relcanonical_homepage')
        || $Blog->get_setting('self_canonical_homepage')) { // Check if the URL was canonical:
        $canonical_url = $Blog->gen_blogurl();
        if (! empty($Item)) {	// Also keep front Item's switchable params in the collection canonical URL:
            $keep_additional_front_canonicals_params = array_keys($Item->get_switchable_params());
        } else {	// No additional canonicals params for current front page:
            $keep_additional_front_canonicals_params = [];
        }
        ...
        if (! is_same_url($current_url, $canonical_url, $Blog->get_setting('http_protocol') == 'allow_both')) {	// We are not on the canonical blog url:
            ...
            } elseif ($Blog->get_setting('relcanonical_homepage')) {	// Use link rel="canoncial":
                add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
            }
        } elseif ($Blog->get_setting('self_canonical_homepage')) {	// Use self-referencing rel="canonical" tag:
            add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
        }
    }
}

if ($disp == 'single' || $disp == 'page' || $disp == 'widget_page') {	// Check if the requested Item can be correctly displayed for disp 'single', 'page' and 'widget_page':
    if (! $preview && empty($Item)) {	// If Item is not defined/not found in DB
        // Note: The 'preview' action is the only one exception, but that is handled above in this if statement
        $disp = '404';
        $disp_detail = '404-item-not-found';
    } elseif (! $preview && $Item->status == 'deprecated') {	// If the requested Item is deprecated
        $disp = '404';
        $disp_detail = '404-item-deprecated';
    } elseif (! $preview && $Item->status == 'redirected') {	// $redir=no here allows to force a 'single post' URL for commenting
        // Redirect to the URL specified in the post:
        $Debuglog->add('Redirecting to post URL [' . $Item->url . '].');
        header_redirect($Item->url, true, true);
    } elseif (! $preview && ! in_array($Item->status, get_inskin_statuses($Blog->ID, 'post'))) {	// If the requested Item is not allowed to be displayed on front-office
        $disp = '404';
        $disp_detail = '404-item-disallowed-for-frontoffice';
    } elseif (! is_logged_in() && in_array($Item->status, ['community', 'protected'])) {	// If the requested Item is allowed only for community or members:
        $login_Blog = &get_setting_Blog('login_blog_ID');
        if ($login_Blog && $login_Blog->ID != $Blog->ID) {	// If current collection is not used for login actions,
            ...
            // will have exited
        } else {	// Current collection is used for login actions
    ...
    } elseif (! $preview && ! $Item->can_be_displayed()) {	// If current User has no permission to view the requested Item
        $disp = '403';
        $disp_detail = '403-item-disallowed-for-user';
    } elseif (! $preview) {	// Check single/page view:
        switch ($Item->get('single_view')) {
            ...
            case 'redirected':
                ...
                if (empty($Item->url)) {	// Display 404 page if no url is provided to redirect:
                    $disp = '404';
                    $disp_detail = '404-item-missing-redirect-url';
                } else {	// Redirect only with filled URL:
...
if (($disp == 'user') && isset($user_ID) && isset($current_User) && ($user_ID != $current_User->ID) && ($Settings->get('enable_visit_tracking') == 1)) { // add or increment to user profile visit
...
if (is_logged_in() && // Only for logged in users
    ! in_array($disp, ['terms', 'help', 'msgform', 'activateinfo']) && // Allow these pages
    ...
    if ($terms_page_ID &&
        ...
        $terms_item_Blog = &$terms_Item->get_Blog()) {	// Redirect to view page with terms & conditions if it is defined correctly in settings:
        $Messages->add(T_('You need to accept the following before you can enter this site.'), 'note');
        header_redirect($terms_item_Blog->get('termsurl', [
...
if (isset($skin)) {	// A skin has been requested by folder_name (url or stub):
    ...
    if ($Skin->type == 'feed') {	// Check if we actually allow the display of the feed; last chance to revert to 404 displayed in default skin.
        ...
        if (($disp == 'posts' && $Blog->get_setting('feed_content') == 'none') ||
            ($disp == 'comments' && $Blog->get_setting('comment_feed_content') == 'none')) { // We don't want to provide feeds; revert to 404!
            unset($skin);
            unset($Skin);
            $disp = '404';
            $disp_detail = '404-feeds-disabled';
        }
    } elseif ($Skin->type == 'sitemap') {	// Check if we actually allow the display of sitemaps.
        // Note: Skins with the type "sitemap" can always be accessed, even when they're not installed.
        if (! $Blog->get_setting('enable_sitemaps')) { // We don't want to show this sitemap, revert to error 404:
            unset($skin);
            unset($Skin);
            $disp = '404';
            $disp_detail = '404-sitemaps-disabled';
        }
    } elseif (skin_exists($skin) && ! skin_installed($skin)) {	// The requested skin is not a feed skin and exists in the file system, but isn't installed:
        debug_die(sprintf(T_('The skin [%s] is not installed on this system.'), htmlspecialchars($skin)));
    } elseif (! empty($tempskin)) { // By definition, we want to see the temporary skin (if we don't use feedburner... )
        $redir = 'no';
    }
}
...
if (! empty($skin) && ! skin_exists($skin)) { // We want to use a skin, but it doesn't exist!
    $err_msg = sprintf(
        T_('The skin [%s] set for blog [%s] does not exist.'),
...
if (! empty($skin)) { // We want to display with a skin now:
    ...
    if (! $PageCache->check()) {	// Cache miss, we have to generate:
        ...
        if ($skin_provided_by_plugin = skin_provided_by_plugin($skin)) {
            ...
            if (! empty($disp) && ($disp == 'single' || $disp == 'page' || $disp == 'widget_page') &&
                ! empty($Item) && ($ItemType = &$Item->get_ItemType()) && $ItemType->get('template_name') != '') { // Get template name for the current Item if it is defined by its Item Type:
                $disp_handler_custom = $ItemType->get('template_name') . '.main.php';


                if ($Skin->get_api_version() == 7 && file_exists($disp_handler = $ads_current_skin_path . $Blog->get('type') . '/' . $disp_handler_custom)) { // Custom template is found in skin folder for current collection kind:
                ...
                } elseif (file_exists($disp_handler = $ads_current_skin_path . $disp_handler_custom)) { // Custom template is found in skin folder:
                    $disp_handler_custom_found = true;
                    $Debuglog->add('blog_main: include ' . rel_path_to_base($disp_handler) . ' (custom for item type)', 'skins');
                } else { // Custom template not found:
                    $disp_handler = null;
                }
            }

            if (empty($disp_handler)) { // Set $disp_handler only if it is not defined above:
                if (! empty($disp_handlers[$disp])) {
                    if ($Skin->get_api_version() == 7 && file_exists($disp_handler = $ads_current_skin_path . $Blog->get('type') . '/' . $disp_handlers[$disp])) {	// The current skin has a customized page handler for this disp and current collection kind:
                        $Debuglog->add('blog_main: include ' . rel_path_to_base($disp_handler) . ' (custom to this theme and collection kind)', 'skins');
                    } elseif (file_exists($disp_handler = $ads_current_skin_path . $disp_handlers[$disp])) {	// The current skin has a customized page handler for this disp:
                        $Debuglog->add('blog_main: include ' . rel_path_to_base($disp_handler) . ' (custom to this skin)', 'skins');
                    } elseif ($Skin->get_api_version() == 7 && file_exists($disp_handler = $ads_current_skin_path . $Blog->get('type') . '/index.main.php')) {	// Fallback to the default "index" handler from the current skin dir for current collection kind:
                        $Debuglog->add('blog_main: include ' . rel_path_to_base($disp_handler) . ' (default handler for collection kind)', 'skins');
                    } else {	// Fallback to the default "index" handler from the current skin dir:
                ...
                } else {	// Use the default handler from the skins dir:
                    if ($Skin->get_api_version() == 7 && file_exists($disp_handler = $ads_current_skin_path . $Blog->get('type') . '/index.main.php')) {	// For current collection kind:
                        $Debuglog->add('blog_main: include ' . rel_path_to_base($disp_handler) . ' (default handler for collection kind)', 'skins');
                    } else {	// For all other kinds:
                        $disp_handler = $ads_current_skin_path . 'index.main.php';
    ...
    if ($Timer->get_state('PageCache') == 'running') {	// Pause only when the page cache timer was not stoped above:
        $Timer->pause('PageCache');
    }
    ...
    exit(0);
} else {	// We don't use a skin. Hopefully the caller will do some displaying.
