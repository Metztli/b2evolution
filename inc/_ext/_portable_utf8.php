...
function utf8_strrpos($haystack, $needle, $offset = 0)
{
    ...
    if (mbstring_loaded()) { // mb_strrpos returns wrong position if invalid characters are found in $haystack before $needle
...
{
    if (empty($chrs)) { // No chars to trim specified, get array of whitespace chars:
        $chrs = utf8_ws();
    } elseif (is_string($chrs)) { // Split provided chars into array:
...
{
    if ($string === null || $string === '') { // asimo> Don't try to trim a NULL or empty string, otherwise in some cases it will return false instead of ''
        return $string;
    }
...
function mbstring_loaded()
{
    ...
    if ($flag === null) {
        ...
        if ($flag) { // mbstring is loaded, set encoding to "UTF-8"
...
function iconv_loaded()
{
    ...
    if ($flag === null) {
        ...
        if ($flag && version_compare(phpversion(), '5.6', '<')) { // iconv is loaded and version < 5.6 set encodings to "UTF-8"
