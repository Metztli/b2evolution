 ...
 */
function wrap_php_function($funcname, $newfuncname = '', $extra_options = [])
{
 ...
 */
function wrap_php_class($classname, $extra_options = [])
{
        ...
        if ($methodfilter == '' || preg_match($methodfilter, $mname)) {
            ...
            if (! $func->isPrivate() && ! $func->isProtected() && ! $func->isConstructor() && ! $func->isDestructor() && ! $func->isAbstract()) {
                if (($func->isStatic && ($methodtype == 'all' || $methodtype == 'static' || ($methodtype == 'auto' && is_string($classname)))) ||
                    ...
                    $methodwrap = wrap_php_function([$classname, $mname], '', $extra_options);
                    if ($methodwrap) {
    ...
    if ($response->faultCode()) {
        ...
        if (! is_array($msig) || count($msig) <= $signum) {
            ...
            if ($newfuncname != '') {
                ...
                // php function name
                $xmlrpcfuncname = $prefix . '_' . preg_replace(
                    ['/\./', '/[^a-zA-Z0-9_\x7f-\xff]/'],
                    ['_', ''],
                    $methodname
                );
            ...
            $results = build_remote_method_wrapper_code(
                $client,
                $methodname,
                $xmlrpcfuncname,
                $msig,
    ...
    if ($response->faultCode()) {
        ...
        if (! is_array($mlist) || ! count($mlist)) {
            ...
            if ($newclassname != '') {
                ...
                $xmlrpcclassname = $prefix . '_' . preg_replace(
                    ['/\./', '/[^a-zA-Z0-9_\x7f-\xff]/'],
                    ['_', ''],
                    $client->server
                ) . '_client';
            ...
            $opts = [
                ...
                'timeout' => $timeout,
                'protocol' => $protocol,
                'encode_php_objs' => $encode_php_objects,
                'prefix' => $prefix,
            ...
            foreach ($mlist as $mname) {
                if ($methodfilter == '' || preg_match($methodfilter, $mname)) {
                    $opts['new_function_name'] = preg_replace(
                        ['/\./', '/[^a-zA-Z0-9_\x7f-\xff]/'],
                        ['_', ''],
                        $mname
                    );
 ...
 * @access private
 */
function build_remote_method_wrapper_code(
    $client,
    $methodname,
    $xmlrpcfuncname,
    $msig,
