...
switch ($action) {
    case 'delete':
        ...
        if (! file_exists($file_path)) {	// Display error when a requested file/folder doesn't exist:
        ...
        if ($is_dir) {	// Delete a folder:
            $del_result = rmdir_r($file_path);
        } else {	// Delete a file:
            $del_result = @unlink($file_path);
        }

        if ($del_result) {	// Successful deleting:
            $Messages->add(sprintf($is_dir
    ...
    case 'force_download':
        ...
        if (! $auto_upgrade_from_any_url) {	// Don't allow upgrade from any URL:
            global $global_Cache;
            // Extract array of info about available update:
            $updates = $global_Cache->getx('updates');
            if (empty($updates[0]['url'])) {	// No new available version on the upgrade server:
                $Messages->add( /*Don't translate because it must not happens on normal back-office UI*/
                    'Auto upgrade is not allowed from URL!'
...
switch ($action) {
    ...
    case 'export_git':
        ...
        if (param_check_not_empty('git_url', TB_('Please enter the URL of repository')) &&
            ...
            preg_match('#([^/]+)/([^/]+)/?$#', $git_url, $git_data)) {	// Generate an URL to download GIT branch as ZIP archive:
        ...
        } else {	// Display the errors and the config form again to fix data:
            $Messages->display();
    ...
    case 'force_download':
        ...
        } elseif (! empty($updates[0]['url'])) {	// New available version on the upgrade server:
        ...
        } else {	// This must not happens because of redirect on checking above:
            debug_die('Auto upgrade is not allowed from URL!');
        }

        // Check the download url for correct http, https, ftp URI
        $success = param_check_url('upd_url', 'download_src', null);
        if ($success && ! preg_match('#\.zip$#i', $download_url)) { // Check the download url is a .zip or .ZIP
            param_error('upd_url', sprintf(TB_('The URL "%s" must point to a ZIP archive.'), $download_url));
        }

        if ($Messages->count()) { // Display the errors and the download form again to fix url
            $Messages->display();
        ...
        if (file_exists($upgrade_file)) { // The downloading file already exists
            if ($action == 'force_download') { // Try to delete previous package if the downloading is forced
    ...
    case 'force_unzip':
        ...
        if (! preg_match('#\.zip$#i', $upd_file)) {	// Check the provided file is a .zip or .ZIP:
        ...
        if (file_exists($upgrade_dir)) { // The downloading file already exists
            if ($action == 'force_unzip') { // Try to delete previous package if the downloading is forced
    ...
    case 'backup_and_overwrite':
        ...
        if (param_errors_detected()) {	// Display the previous step form in order to fix the detected errors:
            $upgrade_name = param('upd_dir', 'string', '', true);
        ...
        if (! isset($block_item_Widget)) {
            ...
            if ($upgrade_name === null) { // Get an upgrade name from url (Used for auto-upgrade, not git)
        ...
        if ($success) {
            ...
            if ($success && empty($read_only_list)) { // We can backup files and database
                if (! function_exists('gzopen')) {
                    $Backup->pack_backup_files = false;
                }

                // Start backup
                if ($success = $Backup->start_backup()) { // We can upgrade files and database
                    ...
                    if ((! $success) || (! empty($read_only_list))) { // In case if something was changed before the previous verify_overwrite check
                        ...
                        if (empty($read_only_list)) { // There was some error in the verify_overwrite() function, but the corresponding error message was already displayed.
                            echo '.</strong></p>';
                        } else { // Some file/folder could not be overwritten, display it
                            echo ':</strong></p>';
        ...
        if ($success) { // Pause a process before upgrading, and display a link to the normal upgrade action
            $block_item_Widget->disp_template_replaced('block_end');
            $Form = new Form();
            ...
            $Form->end_form([['button', 'continue', TB_('Continue to installer'), 'SaveButton', $continue_onclick]]);
            unset($block_item_Widget);
            ...
            $Form->begin_form('fform');
            $Form->begin_fieldset(TB_('Actions'));
            $Form->end_form([['submit', 'actionArray[' . $action . ']', TB_('Retry'), 'SaveButton']]);
            unset($block_item_Widget);
        }
        break;
}
...
if (! empty($files_upgraded)) {	// We cannot require new footer files because they may contain new functions
