...
function get_upgrade_folder_path($version_folder_name)
{
    ...
    if (empty($version_folder_name) || ! file_exists($upgrade_path . $version_folder_name)) { // Don't allow an invalid upgrade folder
    ...
    if ($dir_handle = @opendir($upgrade_folder_path)) {
        while (($dir_name = readdir($dir_handle)) !== false) {
            ...
            if (is_dir($dir_path) && preg_match('#^b2evolution#i', $dir_name)) {	// Use any folder which name is started with "b2evolution":
                if (file_exists($dir_path . '/blogs')) {	// Use 'b2evolution*/blogs' folder:
                    $upgrade_folder_path = $dir_path . '/blogs';
                    break;
                } elseif (file_exists($dir_path . '/site')) {	// Use 'b2evolution*/site' folder:
                    $upgrade_folder_path = $dir_path . '/site';
                    break;
                } elseif (file_exists($dir_path)) {	// Use 'b2evolution*' folder:
...
function check_version($new_version_dir)
{
    ...
    if (! file_exists($new_version_file)) { // Invalid structure of the downloaded upgrade package
    ...
    if (empty($result)) {	// New version:
        return null;
    } elseif ($result == 'old') {	// Old version:
        return [
            'error' => 'old',
    ...
    } elseif ($result == 'same') {	// Same version:
        return [
            'error' => 'same',
...
function switch_maintenance_mode($enable, $mode = 'all', $msg = '', $silent = false)
{
    ...
    if ($enable) {	// Create maintenance file
...
-->
        ...
        if (save_to_file($content, $conf_path . $maintenance_mode_file, 'w+')) { // Maintenance file has been created
            echo ' OK.</p>';
        } else { // Maintenance file has not been created
        ...
        if ($maintenance_mode == 'unknown') {
            ...
            if (is_writable($conf_path . $maintenance_mode_file) && @unlink($conf_path . $maintenance_mode_file)) {	// Unlink was successful:
                if (! $silent) {	// Dispaly OK message:
                    echo ' OK.</p>';
                    evo_flush();
                }
            } else {	// Unlink failed:
...
function switch_maintenance_lock($enable)
{
    ...
    if ($Settings->get('system_lock') != $enable) { // Enable system lock
...
function prepare_maintenance_dir($dir_name, $deny_access = true)
{
    ...
    if (! file_exists($dir_name)) {	// We can create directory
        if (! mkdir_r($dir_name)) {
    ...
    if ($deny_access) {	// Create .htaccess file
        echo '<p>' . TB_('Checking .htaccess denial for directory: ') . $dir_name;
        evo_flush();

        $htaccess_name = $dir_name . '.htaccess';

        if (! file_exists($htaccess_name)) {	// We can create .htaccess file
            ...
            if (! file_exists($dir_name . 'index.html')) {	// Create index.html to disable directory browsing
...
function unpack_archive($src_file, $dest_dir, $mk_dest_dir = false, $src_file_name = '', $print_error = true)
{
    ...
    if (! check_user_perm('files', 'all')) {	// No permission to unzip files:
        $error = '<span class="text-danger">' . TB_('You don\'t have permission to UNZIP files automatically on the server.') . '</span>';
        if (check_user_perm('users', 'edit')) {	// Link to edit permissions:
            global $admin_url;
    ...
    if (strpos($src_file, '://') !== false) {	// Deny ZIP file from urls:
        $invalid_path_error = sprintf(TB_('Path must not contain %s'), '<code>://</code>');
    } else {	// Check if ZIP path is inside $basepath or $upgrade_path:
        $canonical_path = get_canonical_path($src_file);
        if (strpos($canonical_path, $basepath) !== 0 &&
            strpos($canonical_path, get_canonical_path($upgrade_path)) !== 0) {	// ZIP file path must be started with $basepath or $upgrade_path:
    ...
    if (isset($invalid_path_error)) {	// Don't allow wrong ZIP file path:
    ...
    if (! file_exists($dest_dir) && ! mkdir_r($dest_dir)) {	// Destination directory doesn't exist and it couldn't be created:
    ...
    if (class_exists('ZipArchive')) {	// Unpack using 'ZipArchive' extension:
        $ZipArchive = new ZipArchive();
        if ($ZipArchive->open($src_file) &&
            $ZipArchive->extractTo($dest_dir)) {	// Change rights for unpacked folders and files after successful unpacking:
            chmod_r($dest_dir);
            $ZipArchive->close();
        } else {
 ...
 */
function pack_archive($archive_path, $source_dir_path, $files, $add_in_subdirs = '', $exclude_folder_names = [], $log_type = 'print')
{
    global $Settings, $Messages;

    if (! class_exists('ZipArchive')) {	// Stop when no installed extension:
        debug_die('Unable to compress the files because there is no \'ZipArchive\' extension installed in your PHP!');
    }

    if (file_exists($archive_path)) {	// Don't try to create ZIP if same file already exists:
    ...
    if ($ZipArchive->open($archive_path, ZipArchive::CREATE) !== true) {	// Cannot create new ZIP archive:
    ...
    if (! is_array($files)) {	// Make array from single file:
        $files = [$files];
    }
    ...
    if (! is_array($add_in_subdirs)) {
        $add_in_subdirs = [$add_in_subdirs];
    }
    foreach ($add_in_subdirs as $a => $add_in_subdir) {
        if (! empty($add_in_subdir)) {	// Format sub-directory:
            $add_in_subdirs[$a] = trim($add_in_subdir, '/') . '/';
        }
    }

    if (is_array($exclude_folder_names)) {	// Initialize array to exclude subfolders by name:
    ...
    $zip_result = true;
    foreach ($files as $file) {	// Add files into archive:
        if ($log_type == 'print') {
        ...
        if (is_dir($source_dir_path . $file)) {	// Add directory:
            if ($exclude_folder_names_regexp !== false &&
                preg_match($exclude_folder_names_regexp, $file)) {	// Skip file by excluded folder name:
                continue;
            }
            $file_result = $ZipArchive->addEmptyDir('/' . $add_in_subdir . trim($file, '/'));
            if ($file_result && ($dir_files = get_filenames($source_dir_path . $file, [
                'inc_evocache' => true,
                'recurse' => ($exclude_folder_names !== 'subdirs'),
            ]))) {	// Add files of the directory:
                foreach ($dir_files as $dir_file) {
                    $rel_dir_file_path = '/' . $add_in_subdir . substr($dir_file, $source_dir_path_length);
                    if ($exclude_folder_names_regexp !== false &&
                       preg_match($exclude_folder_names_regexp, $rel_dir_file_path)) {	// Skip file by excluded folder name:
                        continue;
                    }
                    if (is_dir($dir_file)) {	// Add empty sub-directory:
        ...
        if ($file_result) {	// Display success result:
            if ($log_type == 'print') {
                echo ' OK.<br />';
                evo_flush();
            }
...
function download_archive($archive_path)
{
    if (! file_exists($archive_path) ||
        ! preg_match('/\.zip$/', $archive_path)) {	// Don't try to download not existing of not ZIP file:
        return false;
        ;
    }
...
function verify_overwrite($src, $dest, $action = '', $overwrite = true, &$read_only_list)
{
    ...
    if ($dir === false) { // $dir is not a valid directory or it can not be opened due to permission restrictions
    ...
    while (false !== ($file = readdir($dir))) {
        if (($file != '.') && ($file != '..')) {
            ...
            if (isset($read_only_list) && file_exists($destfile) && ! is_writable($destfile)) { // Folder or file is not writable
                $read_only_list[] = $destfile;
            }

            if (is_dir($srcfile)) {
                $dir_list[$srcfile] = $destfile;
            } elseif ($overwrite) { // Add to overwrite
    ...
    if (! empty($action) && $action == 'Copying') { // Display errors about config file or the unknown and incorrect commands from config file
        $config_has_errors = false;
        if (is_string($config_ignore_files)) { // Config file has some errors, but the upgrade should not fail because of that
            ...
            if (! empty($config_unknown_commands) && is_array($config_unknown_commands)) { // Unknown commands
            ...
            if (! empty($config_incorrect_commands) && is_array($config_incorrect_commands)) { // Incorrect commands
        ...
        if ($config_has_errors) { // The upgrade config file contains the errors, Stop the upgrading process
    ...
    foreach ($dir_list as $src_dir => $dest_dir) {
        ...
        if (! empty($action)) {
            ...
            } else { // progressive display of what backup is doing
                echo $action . ' &laquo;<strong>' . $dest_dir . '</strong>&raquo;...';
                $dir_success = true;
            }
            evo_flush();
        } elseif ($ignore_dir) { // This subfolder must be ingored, Display message about this
        ...
        if ($ignore_dir) { // Skip the ignored folder
            continue;
        }

        if ($overwrite && ! file_exists($dest_dir)) {
            // Create destination directory
            if (! evo_mkdir($dest_dir)) { // No permission to create a folder
    ...
    foreach ($file_list as $src_file => $dest_file) { // Overwrite destination file
        $dest_file_name = str_replace($basepath, '', $dest_file);
        if (is_array($config_ignore_files) && in_array($dest_file_name, $config_ignore_files)) { // Ignore this file
        ...
        if (is_array($config_softmove_files) && ! empty($config_softmove_files[$dest_file_name])) { // Action 'softmove': This file should be copied to other location with saving old file
            $copy_file_name = $config_softmove_files[$dest_file_name];
            // Don't rewrite old file
            $rewrite_old_file = false;
        }
        if (is_array($config_forcemove_files) && ! empty($config_forcemove_files[$dest_file_name])) { // Action 'forcemove': This file should be copied to other location with rewriting old file
            $copy_file_name = $config_forcemove_files[$dest_file_name];
            // Rewrite old file
            $rewrite_old_file = true;
        }

        if (! empty($copy_file_name)) { // This file is marked in config to copy to other location
            $copy_file = $basepath . $copy_file_name;
            if (! $rewrite_old_file && file_exists($copy_file)) { // Display warning if we cannot rewrite an existing file
                echo '<div class="orange">' . sprintf(
                    TB_('Ignoring softmove of %s because %s is already in place (see %s)'),
                    '&laquo;<b>' . $dest_file_name . '</b>&raquo;',
                    '&laquo;<b>' . $copy_file_name . '</b>&raquo;',
                    '<code>' . get_upgrade_config_file_name() . '</code>'
                ) . '</div>';
                evo_flush();
                unset($copy_file_name);
                continue; // Skip this file
            } else { // We can copy this file to other location
                echo '<div class="orange">' . sprintf(
                    TB_('Moving %s to %s as stated in %s'),
                    '&laquo;<b>' . $dest_file_name . '</b>&raquo;',
                    '&laquo;<b>' . $copy_file_name . '</b>&raquo;',
                    '<code>' . get_upgrade_config_file_name() . '</code>'
                ) . '</div>';
...
function check_upgrade_config($display_message = false)
{
    ...
    if (! file_exists($conf_path . 'upgrade_policy.conf')) {	// No upgrade config file
        if ($display_message) {	// Display error message:
            global $Messages;
...
function get_upgrade_config_file_name()
{
    ...
    if (file_exists($conf_path . 'upgrade_policy.conf')) {	// Use custom file firstly:
...
function get_upgrade_config($action)
{
    ...
    if (! isset($upgrade_policy_config)) { // Init global array first time
        $upgrade_policy_config = [];
    } elseif (is_string($upgrade_policy_config)) { // Return error about config file
        return $upgrade_policy_config;
    }

    if (isset($upgrade_policy_config[$action])) { // The config files were already initialized before, Don't make it twice
        return $upgrade_policy_config[$action];
    }

    $config_handle = @fopen($conf_path . get_upgrade_config_file_name(), 'r');
    if (! $config_handle) { // No permissions to open file
    ...
    if (empty($config_content)) { // Config file is empty for required action
    ...
    $available_actions = ['ignore', 'remove', 'softmove', 'forcemove'];

    $all_actions = array_merge($available_actions, ['unknown', 'incorrect']);
    foreach ($all_actions as $available_action) { // Init array for all actions only first time
        if (! isset($upgrade_policy_config[$available_action])) {
            $upgrade_policy_config[$available_action] = [];
        }
    }
    ...
    foreach ($config_content as $config_line) {
        if (substr($config_line, 0, 1) == ';') { // This line is comment text, Skip it
            continue;
        }
        ...
        if (in_array($line_action, $available_actions)) { // This line has an available action
            if (empty($config_line_params[1])) { // Incorrect command
                $upgrade_policy_config['incorrect'][] = $config_line;
                continue;
            }
            if ($line_action == 'softmove' || $line_action == 'forcemove') { // These actions have two params
                if (empty($config_line_params[1]) || empty($config_line_params[2])) { // Incorrect command
                    $upgrade_policy_config['incorrect'][] = $config_line;
                    continue;
                }
                $upgrade_policy_config[$line_action][$config_line_params[1]] = $config_line_params[2];
            } else { // Actions 'ignore' & 'remove' have only one param
                $upgrade_policy_config[$line_action][] = $config_line_params[1];
            }
        } elseif (! empty($line_action)) { // Also save all unknown actions to display error
...
function remove_after_upgrade()
{
    ...
    if (is_string($upgrade_removed_files)) { // Errors on opening of upgrade_policy.conf
        $config_error = $upgrade_removed_files;
    } elseif (empty($upgrade_removed_files)) { // No files/folders to remove, Exit here
        $config_error = sprintf(TB_('No "remove" sections have been defined in the file %s.'), '<code>' . get_upgrade_config_file_name() . '</code>');
    }

    if (! empty($config_error)) { // Display config error
        echo '<div class="red">';
        echo $config_error;
    ...
    foreach ($upgrade_removed_files as $file_path) {
        ...
        $success = true;
        if (file_exists($file_path)) { // File exists
            if (is_dir($file_path)) { // Remove folder recursively
                if (rmdir_r($file_path)) { // Success
                    $log_message .= TB_('OK');
                } else { // Failed
                    $log_message .= TB_('Failed') . ': ' . TB_('No permissions to delete the folder');
                    $success = false;
                }
            } elseif (is_writable($file_path)) { // Remove file
...
{
    if (empty($steps) || empty($current_step)) { // Bad input data
        return '';
    }
