...
class Backup
{
     ...
     * @var boolean
     */
    public $backup_db_structure = true;

    /**
     * Add "DROP TABLE IF EXISTS" before every "CREATE TABLE"
     */
    public $drop_table_first = true;

    /**
     * True if pack backup files
     * @var boolean
     */
    public $pack_backup_files;
     ...
     */
    public function load_from_Request($memorize_params = false)
    {
        global $backup_paths, $backup_exclude_folders, $backup_tables, $Messages;

        // Load folders/files settings from request
        foreach ($backup_paths as $name => $settings) {
            if (array_key_exists('label', $settings) && ! is_null($settings['label'])) {	// We can set param
    ...
    public function start_backup()
    {
        ...
        if ($success && $this->has_included($this->backup_paths)) {
            ...
            if ($success = prepare_maintenance_dir($backup_files_path, false)) {	// We can backup files
        ...
        if ($success && ($this->has_included($this->backup_tables) || $this->backup_db_structure)) {
            ...
            if ($success = prepare_maintenance_dir($cbackup_path, false)) {	// We can backup database
     ...
     */
    public function backup_files($backup_dirpath)
    {
        ...
        $backup_current_exclude_folders = [];
        ...
        if ($this->ignore_config) {	// Ignore files and folders listed in "conf/backup_ignore.conf":
            global $conf_path;
        ...
        if ($this->pack_backup_files) {	// Create ZIPped backup:
        ...
        } else {	// Copy directories and files to backup directory
            foreach ($included_files as $included_file) {
                $this->recurse_copy(
                    no_trailing_slash($basepath . $included_file),
     ...
     * @param string backup directory path
     */
    public function backup_database($backup_dirpath)
    {
        ...
        $backup_structure = [];
        $backup_data = [];
        if ($this->backup_db_structure) {	// Backup structure of all tables:
        ...
        if (file_exists($backup_sql_filepath)) {	// Stop tables backup, because backup file exists
        ...
        if ($f == false) {	// Stop backup, because it can't open backup file for writing
        ...
        if ($this->drop_table_first) {	// Disable foreign key check to avoid errors due to referencial constraints:
        ...
        foreach ($backup_structure as $table) {
            ...
            if ($this->drop_table_first) {	// Drop existing table before creating a new one:
                fwrite($f, 'DROP TABLE IF EXISTS `' . $table . "`;\n");
            }
            $row_table_data = $DB->get_row('SHOW CREATE TABLE ' . $table, ARRAY_N);
            fwrite($f, $row_table_data[1] . ";\n\n");

            if (in_array($table, $backup_data)) {	// Dump data of the table:
                $page = 0;
                $page_size = 500;
                $is_insert_sql_started = false;
                $is_first_insert_sql_value = true;
                while (! empty($rows) || $page == 0) { // Get the records by page(500) in order to save memory and avoid fatal error
                    $rows = $DB->get_results('SELECT * FROM ' . $table . ' LIMIT ' . ($page * $page_size) . ', ' . $page_size, ARRAY_N);

                    if ($page == 0 && ! $is_insert_sql_started && ! empty($rows)) { // Start SQL INSERT clause
                    ...
                    foreach ($rows as $row) {
                        ...
                        for ($index = 0; $index < $num_fields; $index++) {
                            if (isset($row[$index])) {
                                ...
                                $values .= '\'' . $row[$index] . '\'';
                            } else { // The $row[$index] value is not set or is NULL
                                $values .= 'NULL';
                            }
                        ...
                        $values .= ')';
                        if ($is_first_insert_sql_value) { // Don't write a comma before first row values
                            $is_first_insert_sql_value = false;
                        } else { // Write a comma between row values
                            $values = ',' . $values;
                        }
                ...
                if ($is_insert_sql_started) { // End SQL INSERT clause
                    fwrite($f, ";\n\n");
                }
            } else {	// Display info to know only structure is backed up of the table:
        ...
        if ($this->drop_table_first) {	// Restore foreign key check:
        ...
        if ($this->pack_backup_files) { // Pack created backup SQL script
     ...
     */
    public function recurse_copy($src, $dest, $root = true, $exclude_folders = [])
    {
        if (is_dir($src)) {
            if (! ($dir = opendir($src))) {
                return false;
            ...
            while (false !== ($file = readdir($dir))) {
                ...
                if ($file == 'upload-tmp') { // Skip temp folder
                    continue;
                }

                if (in_array($file, $exclude_folders)) {	// Skip the excluded files/folders:
                    continue;
                }

                $srcfile = $src . '/' . $file;
                if (is_dir($srcfile)) {
                    if ($root) { // progressive display of what backup is doing
     ...
     * @return boolean
     */
    public function has_included(&$data_list)
    {
     ...
     * @return array
     */
    public function path_to_array($path)
    {
        if (is_array($path)) {
            return $path;
        }
