...
$Form->begin_fieldset(TB_('Advanced backup options') . get_manual_link('upgrade-advanced-backup-options'), [
    'id' => 'upgrade_backup_options',
    'fold' => true,
]);
...
foreach ($backup_exclude_folders as $name => $settings) {
    ...
    $backup_exclude_checkboxes[] = ['exclude_bk_' . $name, $current_Backup->exclude_folders[$name], sprintf(TB_('Exclude all %s folders'), $exclude_folder_names), $settings['excluded']];
}
...
$db_structure_checkboxes = [
    ['db_structure', 1, sprintf(TB_('Add %s statements for ALL tables, in order to allow quick restore.'), '<code>CREATE TABLE</code>'), $current_Backup->backup_db_structure],
    ['drop_table_first', 1, sprintf(TB_('Add %s before every %s.'), '<code>DROP TABLE IF EXISTS</code>', '<code>CREATE TABLE</code>'), $current_Backup->drop_table_first],
];
...
$Form->begin_fieldset(TB_('File options') . get_manual_link('upgrade-file-options'), [
    'id' => 'upgrade_file_options',
    'fold' => true,
]);
...
$Form->end_form([['submit', 'actionArray[backup_and_overwrite]', $action_backup_title, 'SaveButton' . (empty($new_version_status) ? '' : ' btn-warning')]]);
