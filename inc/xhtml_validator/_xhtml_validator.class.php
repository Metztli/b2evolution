...
{
    public $tags;      // Array showing allowed attributes for tags

    public $tagattrs;  // Array showing URI attributes

    public $uri_attrs;

    public $allowed_uri_scheme;
     ...
     */
    public function __construct($context = 'posting', $allow_css_tweaks = false, $allow_iframes = false, $allow_javascript = false, $allow_objects = false, $encoding = null, $msg_type = 'error')
    {
        global $inc_path;
        ...
        if (! in_array($encoding, ['ISO-8859-1', 'UTF-8', 'US-ASCII'])) { // passed encoding not supported by xml_parser_create()
     ...
     * check(-)
     */
    public function check($xhtml)
    {
        // Convert encoding:
        // TODO: use convert_encoding()
        if (empty($this->xml_parser_encoding) || $this->encoding != $this->xml_parser_encoding) { // we need to convert encoding:
            if (function_exists('mb_convert_encoding')) { // we can convert encoding to UTF-8
                $this->encoding = 'UTF-8';
    ...
    public function tag_open($parser, $tag, $attrs)
    {
        ...
        foreach ($attrs as $attr => $value) {
            ...
            if (in_array($attr, $this->uri_attrs)) { // This attribute must be checked for URIs
                $matches = [];
                $value = trim($value);
    ...
    public function html_error($string)
    {
        global $Messages;
        $this->error = true;
