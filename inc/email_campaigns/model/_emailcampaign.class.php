...
class EmailCampaign extends DataObject
{
    ...
    public $email_defaultdest;

    public $email_html;

    public $email_text;

    public $email_plaintext;

    public $sync_plaintext;
    ...
    public $user_tag_sendskip;

    public $user_tag_sendsuccess;

    public $user_tag;

    public $user_tag_cta1;

    public $user_tag_cta2;

    public $user_tag_cta3;

    public $user_tag_like;

    public $user_tag_dislike;

    public $user_tag_activate;

    public $user_tag_unsubscribe;
    ...
    public $cta1_autm_execute = 1;

    public $cta2_autm_ID;

    public $cta2_autm_execute = 1;

    public $cta3_autm_ID;

    public $cta3_autm_execute = 1;

    public $like_autm_ID;

    public $like_autm_execute = 1;

    public $dislike_autm_ID;

    public $dislike_autm_execute = 1;

    public $activate_autm_ID;

    public $activate_autm_execute = 1;
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                ...
                'fk' => 'csnd_camp_ID',
                'msg' => T_('%d links with users'),
            ],
            [
                'table' => 'T_links',
                'fk' => 'link_ecmp_ID',
                'msg' => T_('%d links to destination email campaigns'),
                'class' => 'Link',
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                ...
                'fk' => 'step_info',
                'and_condition' => 'step_type = "send_campaign"',
                'msg' => T_('%d automation steps use this email campaign'),
            ],
        ];
    }
     ...
     */
    public function add_recipients($filtered_users_IDs = null)
    {
        global $DB;

        if ($filtered_users_IDs === null) {	// Get user IDs from current filterset of users list:
            $filtered_users_IDs = get_filterset_user_IDs();
        }

        if (count($filtered_users_IDs)) {	// If users are found in the filterset
            ...
            if (count($new_users)) {	// Insert new users for this campaign:
     ...
     * Remove the filtered recipients which didn't receive email newsletter yet
     */
    public function remove_recipients()
    {
        if (empty($this->ID)) {	// Email campaign must be created in DB:
            return;
        }
    ...
    public function get($parname)
    {
        switch ($parname) {
            ...
            case 'plaintext_template_preview':
                ...
                $mail_template = str_replace(['$email_key$', '$mail_log_ID$', '$email_key_start$', '$email_key_end$'], ['***email-key***', '', '', ''], $mail_template);
                $mail_template = preg_replace('~\$secret_content_start\$.*\$secret_content_end\$~', '***secret-content-removed***', $mail_template);
                if ($parname == 'html_template_preview') {	// Clear all html tags that may break styles of main html page:
                    return preg_replace('#</?(html|head|meta|body)[^>]*>#i', '', $mail_template);
                } else {	// Convert newline to html <br>:
    ...
    {
        if (! isset($this->Newsletter)) {	// Initialize Newsletter:
     ...
     */
    public function get_recipients($type = 'all')
    {
        // Make sure all recipients are loaded into cache:
        $this->load_recipients();

        if (isset($this->users[$type])) {	// Get subscribed OR unsubscribed users:
            return $this->users[$type];
        } elseif (substr($type, 0, 5) === 'full_') {	// Get subscribed AND unsubscribed users:
    ...
    public function load_recipients()
    {
        ...
        if ($this->users !== null) {	// Recipients already were loaded into cache:
            return;
        }
        ...
        foreach ($users as $user_data) {
            if (! $user_data->enls_subscribed) {	// This user is unsubscribed from newsletter of this email campaign:
                $this->users['unsub_all'][] = $user_data->user_ID;
            } else {	// This user is subscribed to newsletter of this email campaign:
                $this->users['all'][] = $user_data->user_ID;
            }

            if ($user_data->csnd_status == 'sent') {	// This user already received newsletter email:
                if (! $user_data->enls_subscribed) {	// This user is unsubscribed from newsletter of this email campaign:
            ...
            } elseif ($user_data->csnd_status == 'skipped') { // This user will be skipped from receiving newsletter email:
                if (! $user_data->enls_subscribed) {	// This user is unsubscribed from newsletter of this email campaign:
            ...
            } elseif (check_usertags($user_data->user_ID, explode(',', $this->get('user_tag_sendskip')), 'has_any')) {	// This user will be skipped from receiving newsletter email because of skip tags:
                if (! $user_data->enls_subscribed) {	// This user is unsubscribed from newsletter of this email campaign:
            ...
            } elseif ($user_data->csnd_status == 'send_error' || is_blocked_email_status($user_data->emadr_status)) {	// We encountered a send error the last time we attempted to send email, or if current status of email address is "Permanent error" or "Spammer":
                if (! $user_data->enls_subscribed) {	// This user is unsubscribed from newsletter of this email campaign:
     ...
     */
    public function get_recipients_count($type = 'all', $link = false)
    {
        $recipients_count = count($this->get_recipients($type));

        if ($link) {	// Initialize URL to page with reciepients of this Email Campaign:
            if ($type == 'all') {	// Get URL to display ALL subscribers:
                global $admin_url;
                $url = $admin_url . '?ctrl=newsletters&amp;action=edit&amp;tab=subscribers&amp;enlt_ID=' . $this->get('enlt_ID');
            } else {	// Get URL to display filtered and other users which are linked to this email campaign:
                switch ($type) {
                    case 'receive':
                        $recipient_type = 'sent';
                        break;
            ...
            if ($link === true && ($unsub_recipients_count = count($this->get_recipients('unsub_' . $type))) > 0) {	// Display a counter of unsubscribed users if they exist and it is requested:
    ...
    public function dbinsert()
    {
        ...
        if (isset($Plugins)) {	// Note: Plugins may not be available during maintenance, install or test cases
            $Plugins->trigger_event('PrependEmailInsertTransact', $params = [
                'EmailCampaign' => &$this,
            ]);
        }
    ...
    public function dbupdate()
    {
        ...
        if (isset($Plugins)) {	// Note: Plugins may not be available during maintenance, install or test cases
            $Plugins->trigger_event('PrependEmailUpdateTransact', $params = [
                'EmailCampaign' => &$this,
            ]);
        }
     ...
     */
    public function update_message_fields()
    {
        global $Plugins;
        ...
        $Plugins->trigger_event('FilterEmailContent', [
            'data' => &$email_text,
            'EmailCampaign' => $this,
        ]);
     ...
     */
    public function update_plaintext($force_update = false)
    {
        if ($force_update || $this->get('sync_plaintext')) {	// Update plain-text message only when it is enabled for this email campaign:
            $email_plaintext = preg_replace_callback('#<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>#i', [$this, 'update_plaintext_callback_a'], $this->get('email_html'));
            ...
            $email_plaintext = preg_replace_callback('#<h([1-4])[^>]*>([^<]*)</h\1>#i', [$this, 'update_plaintext_callback_h'], $email_plaintext);
            ...
            $email_plaintext = preg_replace('#[\n\s]{3,}#i', "\n\n", $email_plaintext);
            $email_plaintext = str_replace(['&ndash;', '&mdash;', '&#8211;', '&#8212;'], '--', $email_plaintext);
     ...
     * @return string
     */
    public function update_plaintext_callback_h($m)
    {
        return "\n\n"
            // Header text:
            . $m[2] . "\n"
     ...
     * @return string
     */
    public function update_plaintext_callback_a($m)
    {
     ...
     */
    public function update_recipients($force_update = false)
    {
        if (empty($this->ID)) {	// Email campaign must be created in DB:
            return;
        }

        if (! $force_update && empty($this->newsletter_is_changed)) {	// Newsletter of this email campaign was not changed, Don't update recipients:
            return;
        }
    ...
    public function load_from_Request()
    {
        ...
        if (param('ecmp_enlt_ID', 'integer', null) !== null) {	// Newsletter ID:
            ...
            if ($this->newsletter_is_changed) {	// If the list/newsletter has been changed, the welcome flag must be automatically be lost:
                $this->set('welcome', 0);
            }
        }

        if (param('ecmp_name', 'string', null) !== null) {	// Campaign name:
            param_string_not_empty('ecmp_name', T_('Please enter a name.'));
            $this->set_from_Request('name');
        }

        if (param('ecmp_email_title', 'string', null) !== null) {	// Email title:
        ...
        if ($email_defaultdest !== null) {	// Email default destination:
        ...
        if (param('ecmp_email_html', 'html', null) !== null) {	// Email HTML message:
        ...
        if (param('ecmp_email_text', 'html', null) !== null) {	// Save original message:
            $this->set_from_Request('email_text');
        }

        if (param('ecmp_sync_plaintext', 'integer', null) !== null) {	// Keep in sync with HTML / Edit separately:
            $this->set_from_Request('sync_plaintext');
        }

        if (! $this->get('sync_plaintext') &&
            param('ecmp_email_plaintext', 'text', null) !== null) {	// Save plain-text message only when it is allowed for this email campaign:
            $this->set_from_Request('email_plaintext');
        }

        if (param('ecmp_user_tag_sendskip', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_sendskip');
        }

        if (param('ecmp_user_tag_sendsuccess', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_sendsuccess');
        }

        if (param('ecmp_user_tag', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag');
        }

        if (param('ecmp_user_tag_cta1', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_cta1');
        }

        if (param('ecmp_user_tag_cta2', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_cta2');
        }

        if (param('ecmp_user_tag_cta3', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_cta3');
        }

        if (param('ecmp_user_tag_like', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_like');
        }

        if (param('ecmp_user_tag_dislike', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_dislike');
        }

        if (param('ecmp_user_tag_activate', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_activate');
        }

        if (param('ecmp_user_tag_unsubscribe', 'string', null) !== null) {	// User tag:
            $this->set_from_Request('user_tag_unsubscribe');
        }

        $cta1_autm_ID = param('ecmp_cta1_autm_ID', 'integer', null);
        if ($cta1_autm_ID !== null) {	// Automation CTA 1:
        ...
        if ($cta2_autm_ID !== null) {	// Automation CTA 2:
        ...
        if ($cta3_autm_ID !== null) {	// Automation CTA 3:
        ...
        if ($like_autm_ID !== null) {	// Automation LIKE:
        ...
        if ($dislike_autm_ID !== null) {	// Automation DISLIKE:
        ...
        if ($activate_autm_ID !== null) {	// Automation ACTIVATE:
     ...
     */
    public function check($display_messages = true, $mode = '')
    {
        ...
        if (empty($this->email_title)) { // Email title is empty
            if ($display_messages) {
        ...
        if (empty($this->email_text)) {	// Email message is empty:
            if ($display_messages) {
        ...
        if ($mode != 'test' && count($this->get_recipients('wait')) == 0) { // No users found which wait this newsletter
            if ($display_messages) {
     ...
     */
    public function send_email($user_ID, $email_address = '', $mode = '', $update_sent_ts = false, $newsletter_ID = null, $automation_ID = null)
    {
        global $localtimenow;
        ...
        if ($mode == 'test') { // Send a test newsletter
            $newsletter_params['boundary'] = 'b2evo-' . md5(rand());
            $headers = [
                'Content-Type' => 'multipart/mixed; boundary="' . $newsletter_params['boundary'] . '"',
            ];

            if ($test_User = &$UserCache->get_by_ID($user_ID, false, false)) {	// Send a test email only when test user exists:
            ...
            } else { // No test user found
                $result = false;
            }
        } else {	// Send a newsletter to real user:
            ...
            if (in_array($user_ID, $this->get_recipients('full_receive')) ||
                ...
                $result = false;
            } else {	// Try to send email campaign to the user only if he is really waiting this:
                ...
                $result = send_mail_to_User($user_ID, $this->get('email_title'), 'newsletter', $newsletter_params, $force_on_non_activated, [], $email_address);
            }

            if ($result) {
                $last_sent_ts_field = ($update_sent_ts == 'auto' ? 'enls_last_sent_auto_ts' : 'enls_last_sent_manual_ts');
                if (empty($automation_ID)) {	// Update last sending data for newsletter per user:
                    $last_sent_ts_sql_join = '';
                    $last_sent_ts_sql_where = ' AND enls_enlt_ID = ' . $DB->quote($this->get('enlt_ID'));
                } else {	// Update last sending data for all newsletters tied to the automation and where the user is subscribed to:
                ...
                if (! empty($user_tag_sendsuccess)) {	// Only if at least one tag is defined:
            ...
            if (empty($mail_log_insert_ID)) {	// ID of last inserted mail log is defined in function mail_log()
                // If it was not inserted we cannot mark this user as received this email campaign:
                $result = false;
            }

            if ($result) {	// Email newsletter was sent for user successfully:
                $this->update_user_send_status($user_ID, 'sent');
            } elseif (($User = &$UserCache->get_by_ID($user_ID, false, false)) &&
                    is_blocked_email_status($User->get_email_status())) {	// Unable to send email due to blocked email addresses:
                $this->update_user_send_status($user_ID, 'send_error');
            }
        }

        if ($update_sent_ts == 'auto') {	// Update auto date of sending:
            $this->set('auto_sent_ts', date('Y-m-d H:i:s', $localtimenow));
            $this->dbupdate();
        } elseif ($update_sent_ts == 'manual') {	// Update manual date of sending:
     ...
     */
    public function send_all_emails($display_messages = true, $user_IDs = null, $update_sent_ts = 'manual')
    {
        global $DB, $localtimenow, $Settings, $Messages, $mail_log_insert_ID;

        if ($user_IDs === null) {	// Send emails only for users which still don't receive emails:
            $user_IDs = $this->get_recipients('wait');
        } else {	// Exclude users(which already received this email campaign or skipped manually or skipped by user tag) to avoid double sending even with forcing user IDs:
        ...
        if (empty($user_IDs)) {	// No users, Exit here:
            return false;
        }
        ...
        foreach ($user_IDs as $user_ID) {
            if ($email_campaign_chunk_size > 0 && $email_success_count >= $email_campaign_chunk_size) {	// Stop the sending because of chunk size:
                break;
            }

            if (! ($User = &$UserCache->get_by_ID($user_ID, false, false))) {	// Skip wrong recipient user:
                continue;
            }

            if ($email_campaign_max_domain > 0) {	// Limit a sending by max emails to same domain:
                ...
                if ($email_domains_number[$email_domain] >= $email_campaign_max_domain) {	// Skip sending because max emails to same domain is reached:
            ...
            if ($result) {	// Email newsletter was sent for user successfully:
                $email_success_count++;
            } else {	// This email sending was skipped:
                $email_skip_count++;
                if (is_blocked_email_status($User->get_email_status())) {	// Unable to send email due to permanent error:
                    $email_error_count++;
                }
            }

            if ($display_messages === true || $display_messages === 'cron_job') {	// Print the messages:
                if ($result === true) { // Success
                ...
                } else { // Failed, Email was NOT sent
                    if (! check_allow_new_email('newsletter_limit', 'last_newsletter', $user_ID)) { // Newsletter email is limited today for this user
                    ...
                    } elseif (is_blocked_email_status($User->get_email_status())) { // Email has permanent error
                    ...
                    } else {	// Another error from function send_mail():
        ...
        if ($update_sent_ts == 'auto' || $update_sent_ts == 'welcome') {	// Update auto date of sending:
            $this->set('auto_sent_ts', date('Y-m-d H:i:s', $localtimenow));
            $this->dbupdate();
        } elseif ($update_sent_ts == 'manual') {	// Update manual date of sending:
            $this->set('sent_ts', date('Y-m-d H:i:s', $localtimenow));
        ...
        if ($display_messages === true || $display_messages === 'cron_job') {	// Print the messages:
            ...
            if ($wait_count > 0) {	// Some recipients still wait this newsletter:
                $warning_msg = sprintf(
                    T_('Emails have been sent to a chunk of %s recipients. %s recipients were skipped. %s recipients have not been sent to yet.'),
                    $email_campaign_chunk_size,
                    $email_skip_count + $skipped_count,
                    $wait_count
                );
                if ($display_messages === 'cron_job') {
                    cron_log_append("\n" . $warning_msg, 'warning');
            ...
            } else {	// All recipients received this bewsletter:
            ...
            if ($display_messages !== 'cron_job') {	// Print out messages right now:
                $Messages->display();
            }
        }
     ...
     */
    public function update_user_send_status($user_ID, $status, $mail_log_ID = null)
    {
        global $DB, $mail_log_insert_ID, $servertimenow;

        if (empty($this->ID)) {	// Email Campaign must be stored in DB:
            return;
        }

        if ($mail_log_ID === null && isset($mail_log_insert_ID)) {	// Use log ID of last sending email:
        ...
        if (in_array($user_ID, $all_user_IDs)) {	// Update user send status for this email campaign:
            if ($status == 'sent') {
                ...
                if (! empty($r)) {
                    ...
                    if (empty($last_sent_ts) && $previous_status != 'sent') { // First time to send the email to this user
        ...
        } else {	// Insert new record for user send status:
        ...
        if ($r) {	// Update the CACHE array where we store email sending status for users:
            ...
            if (isset($statuses_keys[$status]) && ! in_array($user_ID, $this->users['filter'])) {	// Add user ID to filtered array:
                $this->users['filter'][] = $user_ID;
            }

            foreach ($statuses_keys as $email_status => $array_key) {
                if ($email_status == $status) {	// Add user ID to proper cache array:
                    if (! in_array($user_ID, $this->users[$array_key])) {
                        $this->users[$array_key][] = $user_ID;
                    }
                } elseif (($unset_user_ID_key = array_search($user_ID, $this->users[$array_key])) !== false) {	// Remove user ID from previous status cache array:
     ...
     */
    public function &get_Cronjob($exclude_error = true)
    {
        ...
        if ($exclude_error && $Cronjob &&
            ! in_array($Cronjob->get_status(), ['pending', 'started', 'finished'])) {	// Exclude cron job if it was stopped with an error:
            $Cronjob = null;
        }
     ...
     */
    public function create_cron_job($next_chunk = false)
    {
        global $Messages, $servertimenow;

        if (! $next_chunk && ($email_campaign_Cronjob = &$this->get_Cronjob())) {	// If we create first cron job but this email campaign already has one:
            if (check_user_perm('options', 'view')) {	// If user has an access to view cron jobs:
                global $admin_url;
                $Messages->add(sprintf(
                    T_('A scheduled job was already created for this campaign, <a %s>click here</a> to view it.'),
                    'href="' . $admin_url . '?ctrl=crontab&amp;action=view&amp;cjob_ID=' . $email_campaign_Cronjob->ID . '" target="_blank"'
                ), 'error');
            } else {	// If user has no access to view cron jobs:
        ...
        if ($this->get_recipients_count('wait') > 0) {	// Create cron job only when at least one user is waiting a newsletter of this email campaing:
            ...
            if ($next_chunk) {	// Send next chunk only after delay:
                global $Settings;
                ...
                foreach ($user_IDs as $user_ID) {
                    if (check_allow_new_email('newsletter_limit', 'last_newsletter', $user_ID)) {	// Newsletter email is NOT limited today for this user:
                ...
                if ($all_waiting_users_limited) {	// Force a delay between chunks if all waiting users are limited to receive more newsletters for today:
                ...
                } else {	// Use a delay between chunks from general setting:
            ...
            $email_campaign_Cronjob->set('params', [
                'ecmp_ID' => $this->ID,
            ]);
            ...
            if (! $r) {	// Error on cron job inserting:
                return false;
            }
        ...
        } else {	// If no waiting users then don't create a cron job and reset ID of previous cron job:
    ...
    public function duplicate()
    {
        ...
        $skipped_fields = ['ID', 'welcome', 'send_count', 'open_count', 'img_loads', 'link_clicks',
     ...
     */
    public function add_user_to_automation($click_type, $user_ID)
    {
        $autm_ID = $this->get($click_type . '_autm_ID');

        if (empty($autm_ID)) {	// Automation is not defined for this email campaign:
            return false;
        }

        $AutomationCache = &get_AutomationCache();
        if (! $click_Automation = &$AutomationCache->get_by_ID($autm_ID, false, false)) {	// Wrong automation:
            return false;
        }

        // Add user to automation:
        $added_users_num = $click_Automation->add_users($user_ID, [
            'users_no_subs' => 'add', // Add anyway users who are not subscribed to Newsletter of the Automation
        ]);

        if ($added_users_num && $this->get($click_type . '_autm_execute')) {	// Execute first step(s) immediately:
