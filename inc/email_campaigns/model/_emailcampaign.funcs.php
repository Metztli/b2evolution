    ...
    switch ($type) {
        case 'current':
            ...
            if (! empty($modes[$current_tab])) {
                if ($current_tab == 'plaintext' && $EmailCampaign->get('sync_plaintext')) {	// Don't allow tab plaintext when it is not enabled:
        ...
        default:
            ...
            foreach ($modes as $tab_name => $tab_info) {
                if ($this_tab) { // We find URL for next tab
                    return ($tab_name == 'plaintext' && $EmailCampaign->get('sync_plaintext')) ? $modes['send']['href'] : $tab_info['href'];
                }
                if ($tab_name == $current_tab) { // The next tab will be what we find
                    $this_tab = true;
                }
            }
            break;
 ...
 * @param array Params
 */
function campaign_results_block($params = [])
{
    ...
    if (isset($params['enlt_ID'])) {	// Filter by Newsletter:
    ...
    if (! empty($username)) {	// Filter by user login, first name, last name, nickname:
        $sql_where = [];
        $kw_array = explode(' ', $username);
        foreach ($kw_array as $kw) {	// Note: we use CONCAT_WS (Concat With Separator) because CONCAT returns NULL if any arg is NULL
    ...
    if (check_user_perm('emails', 'edit') && $params['display_create_button']) { // User must has a permission to edit emails
        $Results->global_icon(T_('Create new campaign') . '...', 'new', $admin_url . '?ctrl=campaigns&amp;action=new' . (isset($params['enlt_ID']) ? '&amp;enlt_ID=' . $params['enlt_ID'] : ''), T_('Create new campaign') . ' &raquo;', 3, 4, [
    ...
    $Results->register_filter_preset(
        'all',
        T_('All'),
...
{
    if ($ecmp_welcome) {	// If email campaign is used as welcome message:
        $welcome_icon = get_icon('bullet_green', 'imgtag', [
            'title' => T_('The email campaign is used as "Welcome" for its list.'),
        ]);
        if ($ecmp_activate) {	// If email campaign is used as activate message:
            $activate_icon = get_icon('bullet_dark_blue', 'imgtag', [
                'title' => T_('This welcome email doubles as an activation message. No separate activation email will be sent.'),
            ]);
        } else {	// If email campaign is NOT used as activate message:
            $activate_icon = get_icon('bullet_empty_grey', 'imgtag', [
                'title' => T_('This welcome email does not act as an activation message.'),
            ]);
        }
    } else {	// If email campaign is NOT used as welcome message:
        $welcome_icon = get_icon('bullet_empty_grey', 'imgtag', [
            'title' => T_('The email campaign is not used as "Welcome" for its list.'),
        ]);
        // Don't use email campaign as activate message when it is not used as welcome message:
        $activate_icon = '';
    }

    if (check_user_perm('emails', 'edit')) {	// Make icon(s) toggle welcome/activate statuses if current User has a perm to edit this:
        ...
        if (! empty($activate_icon)) {	// Make activate icon clickable:
