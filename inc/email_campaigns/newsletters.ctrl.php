...
if ($tab == 'automations') {	// Check other permission for automations:
    check_user_perm('options', 'view', true);
}

if (param('enlt_ID', 'integer', '', true)) {	// Load Newsletter object:
    $NewsletterCache = &get_NewsletterCache();
    if (($edited_Newsletter = &$NewsletterCache->get_by_ID($enlt_ID, false)) === false) {	// We could not find the newsletter to edit:
...
switch ($action) {
    ...
    case 'create':
        ...
        if ($edited_Newsletter->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Newsletter->check_delete(sprintf(TB_('Cannot delete list "%s"'), $edited_Newsletter->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
    ...
    case 'disable':
        ...
        if ($action == 'enable' && $enlt_index === false) {	// Enable newsletter:
            $def_newsletters[] = $edited_Newsletter->ID;
            $update_def_newsletters = true;
        } elseif ($action == 'disable' && $enlt_index !== false) {	// Disable newsletter:
            unset($def_newsletters[$enlt_index]);
            $update_def_newsletters = true;
        }

        if ($update_def_newsletters) {	// Update default setting for newsletters:
...
if (! empty($edited_Newsletter)) {
    ...
    if ($edited_Newsletter->ID > 0) {	// Edit newsletter
...
switch ($action) {
    case 'new':
    case 'edit':
        if ($edited_Newsletter->ID > 0) { // Add menu level 3 entries:
            $AdminUI->add_menu_entries(['email', 'newsletters'], [
                'general' => [
                    'text' => TB_('General'),
            ...
            if (check_user_perm('options', 'view')) {	// If current user has a permissions to view options:
                $AdminUI->add_menu_entries(['email', 'newsletters'], [
                    'automations' => [
