...
if (check_user_perm('emails', 'edit')) {	// Print out this fake button on top in order to use submit action "save" on press "Enter" key:
...
$Form->text_input('ecmp_name', $edited_EmailCampaign->get('name'), 60, TB_('Campaign name'), TB_('for internal use'), [
    'maxlength' => 255,
    'required' => true,
]);
...
$Form->select_input_object('ecmp_enlt_ID', $edited_EmailCampaign->get('enlt_ID'), $NewsletterCache, TB_('Send to subscribers of'), [
    'required' => true,
...
$Form->info_field(TB_('After additional filter'), $edited_EmailCampaign->get_recipients_count('filter', true), [
...
$Form->usertag_input(
    'ecmp_user_tag_sendskip',
    param('ecmp_user_tag_sendskip', 'string', $edited_EmailCampaign->get('user_tag_sendskip')),
    60,
    TB_('Skip users who have any of these tags'),
    TB_('users will be skipped') . ' ' . action_icon(TB_('Refresh'), 'refresh', '#', null, null, null, [
        'onclick' => 'return update_campaign_recipients_count( ' . $edited_EmailCampaign->ID . ' )',
    ]),
    [
        'maxlength' => 255,
...
$Form->usertag_input('ecmp_user_tag_sendsuccess', param('ecmp_user_tag_sendsuccess', 'string', $edited_EmailCampaign->get('user_tag_sendsuccess')), 60, TB_('On successful send, tag users with'), '', [
    'maxlength' => 255,
]);
$Form->end_fieldset();
...
$tag_options = [
    [TB_('Tag users who click on content links with'), 'user_tag'],
    [/* TRANS: CTA means Call To Action */ TB_('Tag users who click CTA 1 with'), 'user_tag_cta1'],
    [/* TRANS: CTA means Call To Action */ TB_('Tag users who click CTA 2 with'), 'user_tag_cta2'],
    [/* TRANS: CTA means Call To Action */ TB_('Tag users who click CTA 3 with'), 'user_tag_cta3'],
    [TB_('Tag users who liked the email with'), 'user_tag_like'],
    [TB_('Tag users who disliked the email with'), 'user_tag_dislike'],
    [TB_('Tag users who click Activate with'), 'user_tag_activate'],
    [TB_('Tag users who (really) unsubscribe with'), 'user_tag_unsubscribe'],
];
foreach ($tag_options as $tag_option) {
    $Form->usertag_input('ecmp_' . $tag_option[1], $edited_EmailCampaign->get($tag_option[1]), 60, $tag_option[0], '', [
        'maxlength' => 255,
    ]);
}
...
$automation_options = [
    [TB_('Add users who click CTA 1 to'),       'cta1_autm_ID',    'cta1_autm_execute'],
    [TB_('Add users who click CTA 2 to'),       'cta2_autm_ID',    'cta2_autm_execute'],
    [TB_('Add users who click CTA 3 to'),       'cta3_autm_ID',    'cta3_autm_execute'],
    [TB_('Add users who like the email to'),    'like_autm_ID',    'like_autm_execute'],
    [TB_('Add users who dislike the email to'), 'dislike_autm_ID', 'dislike_autm_execute'],
    [TB_('Add users who click Activate to'),    'activate_autm_ID', 'activate_autm_execute'],
];
foreach ($automation_options as $automation_option) {
    ...
    $Form->select_input_object('ecmp_' . $automation_option[1], $action_autm_ID, $AutomationCache, '', [
        'allow_none' => true,
    ]);
    echo action_icon(TB_('Steps'), 'edit', $admin_url . '?ctrl=automations&amp;action=edit&amp;tab=steps&amp;autm_ID=' . $action_autm_ID, null, null, null, empty($action_autm_ID) ? [
        'style' => 'display:none',
    ] : []);
    $Form->checkbox_input('ecmp_' . $automation_option[2], $edited_EmailCampaign->get($automation_option[2]), '', [
        'input_prefix' => '<label>',
        'input_suffix' => ' ' . TB_('Execute first step(s) immediately') . '</label> &nbsp; ',
    ]);
    $Form->end_line();
}
$Form->end_fieldset();
...
if (check_user_perm('emails', 'edit')) { // User must has a permission to edit emails
    $buttons[] = ['submit', 'actionArray[save]', TB_('Save info'), 'SaveButton'];
}
