...
if (! $creating && check_user_perm('emails', 'edit')) {	// Display a button to delete existing newsletter if current User has a perm:
...
$Form->text_input('enlt_name', $edited_Newsletter->get('name'), 30, TB_('Name'), '', [
    'maxlength' => 255,
    'required' => true,
]);

$Form->text_input('enlt_label', $edited_Newsletter->get('label'), 150, TB_('Label'), '', [
    'maxlength' => 255,
]);

$Form->text_input('enlt_order', $edited_Newsletter->get('order'), 10, TB_('Order'), '', [
    'maxlength' => 11,
]);

$Form->username('enlt_owner_login', $edited_Newsletter->get_owner_User(), TB_('Owner'), '', '', [
    'required' => true,
]);
...
$Form->radio_input('enlt_perm_subscribe', $edited_Newsletter->get('perm_subscribe'), [
    [
        'value' => 'admin',
        'label' => TB_('Forbidden (Only admin can subscribe users)'),
    ],
    [
        'value' => 'anyone',
        'label' => TB_('Anyone can subscribe'),
    ],
    [
        'value' => 'group',
        'label' => TB_('Only for members of any of the following groups can subscribe:'),
    ],
], TB_('Self-subscribing'), [
    'lines' => true,
]);

$GroupCache = &get_GroupCache();
$perm_groups = $edited_Newsletter->get('perm_groups') != '' ? explode(',', $edited_Newsletter->get('perm_groups')) : [];
$groups_options = [];
$groups = $GroupCache->get_option_array();
foreach ($groups as $group_ID => $group_name) {
    $groups_options[] = ['enlt_perm_groups[]', $group_ID, $group_name, in_array($group_ID, $perm_groups), $edited_Newsletter->get('perm_subscribe') != 'group'];
}
...
$buttons = [];
if (check_user_perm('emails', 'edit')) {	// Display a button to create/update newsletter if current User has a perm:
    if ($creating) {	// Create:
