...
if (check_user_perm('emails', 'edit')) {	// Print out this fake button on top in order to use submit action "test" on press "Enter" key:
...
if (! empty($template_action) && $template_action == 'send_campaign') { // Execute action to send campaign to all users
...
$Form->info(TB_('Email title'), mail_autoinsert_user_data($edited_EmailCampaign->get('email_title'), $current_User, 'text', null, null, [
    'enlt_ID' => $edited_EmailCampaign->get('enlt_ID'),
]));
...
$Form->radio('ecmp_sync_plaintext', $edited_EmailCampaign->get('sync_plaintext'), [
    [1, TB_('Keep in sync with HTML')],
    [0, TB_('Edit separately')],
], '');
$Form->button([
    'value' => TB_('Edit'),
...
if ($edited_EmailCampaign->get_recipients_count('wait') > 0) {	// Display message to send emails only when users exist for this campaign:
    $Form->checklist([
        ['track_email_image_load', 1, TB_('track image loads in HTML version'), 1],
        ['track_email_click_html', 1, TB_('track clickthroughs in HTML version'), 1],
        ['track_email_click_plain_text', 1, TB_('track clickthroughs in plain text version'), 1],
    ], 'track_email', TB_('Track email opens'));
    if ($Settings->get('email_campaign_send_mode') == 'cron') {	// Asynchronous sending mode:
        if ($edited_EmailCampaign->get_Cronjob()) {	// Cron job was already created:
            $button_title = TB_('See scheduled send jobs for this campaign');
            $button_action = 'view_cron';
        } else {	// Cron job is not created yet:
    ...
    $send_button = [[
        'name' => 'actionArray[' . $button_action . ']',
        'value' => $button_title,
        'class' => 'SaveButton btn btn-default',
    ]];
...
if (check_user_perm('emails', 'edit')) { // User must has a permission to edit emails
    ...
    $Form->checklist([
        ['track_test_email_image_load', 1, TB_('track image loads in HTML version'), 1],
        ['track_test_email_click_html', 1, TB_('track clickthroughs in HTML version'), 1],
        ['track_test_email_click_plain_text', 1, TB_('track clickthroughs in plain text version'), 1],
    ], 'track_test_email', TB_('Track email opens'));
    $Form->text_input('test_email_address', $Session->get('test_campaign_email'), 30, TB_('Email address'), TB_('Fill your email address and press button "Send test email" if you want to test this list'), [
        'maxlength' => 255,
    ]);
    $test_button = [[
        'name' => 'actionArray[test]',
        'value' => TB_('Send test email'),
