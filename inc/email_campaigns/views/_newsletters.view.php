...
if (check_user_perm('emails', 'edit')) {	// Display a button to add newsletter if current User has a perm:
    $Results->global_icon(T_('Create new list') . '...', 'new', $admin_url . '?ctrl=newsletters&amp;action=new', T_('Create new list') . ' &raquo;', 3, 4, [
...
{
    if ($enlt_active) {	// If newsletter is active:
        $active_icon = get_icon('bullet_green', 'imgtag', [
            'title' => T_('The list is active.'),
        ]);
    } else {	// If newsletter is NOT active:
        $active_icon = get_icon('bullet_empty_grey', 'imgtag', [
            'title' => T_('The list is not active.'),
        ]);
    }
