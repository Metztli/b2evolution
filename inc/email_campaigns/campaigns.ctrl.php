...
if (param('ecmp_ID', 'integer', '', true)) {	// Load Email Campaign object:
    $EmailCampaignCache = &get_EmailCampaignCache();
    if (($edited_EmailCampaign = &$EmailCampaignCache->get_by_ID($ecmp_ID, false)) === false) {	// We could not find the goal to edit:
...
if ($tab == 'plaintext' &&
    ...
    $edited_EmailCampaign->get('sync_plaintext')) {	// Don't allow plaintext tab if it is not enabled by email campaign setting:
    $tab = 'compose';
}
...
switch ($action) {
    ...
    case 'add':
        ...
        if (! $new_EmailCampaign->load_from_Request()) { // We could not load data from form with errors:
            $action = 'new';
            break;
        }
    ...
    case 'save_sync_plaintext':
        ...
        if (! $edited_EmailCampaign->load_from_Request()) { // We could not load data from form with errors:
            $action = 'edit';
            $tab = $current_tab;
            break;
        }
        ...
        if ($action == 'save') {	// Save & continue to next step:
            $tab = $current_tab;
            $redirect_tab_type = 'next';
        } elseif ($action == 'save_sync_plaintext') {	// Save & redirect to edit plain-text manually:
            $tab = 'plaintext';
        }

        if ($action == 'save_sync_plaintext' && $display_mode == 'js') {	// Exit here to don'r render a page when it is updated from AJAX request:
            // Return new plain-text to update it on the form:
            echo $edited_EmailCampaign->get('plaintext_template_preview');
            exit;
        } else {	// Redirect after saving:
    ...
    case 'update_users':
        ...
        $NewsletterCache = &get_NewsletterCache();
        if (! ($Newsletter = &$NewsletterCache->get_by_ID($newsletter_ID, false, false)) || ! $Newsletter->get('active')) {	// If the selected newsletter cannot be used for email campaigns (because it doesn't exist or is not active):
        ...
        if ($action == 'create_for_users') {	// Create new email campaign if admin want creates it from free users list:
            ...
            if ($edited_EmailCampaign->get('enlt_ID') != $Newsletter->ID) {	// Update newsletter if it was changed on users list filtering:
                $edited_EmailCampaign->set('enlt_ID', $Newsletter->ID);
                $edited_EmailCampaign->dbupdate();
            }
        }

        if (! $edited_EmailCampaign) {	// If campaign is not defined we cannot assign recipients, Redirect to campaigns list:
        ...
        if ($action == 'update_users') {	// Display a message for updating of users selection:
    ...
    case 'disable_welcome':
        ...
        if ($edited_EmailCampaign->dbupdate()) {
            if ($action == 'enable_welcome') {	// If email campaign has been set as "Welcome" we should other campaign of its list set to not "Welcome":
    ...
    case 'disable_activate':
        ...
        if ($edited_EmailCampaign->dbupdate() && $action == 'enable_activate') {	// If email campaign has been set as "Activate" we should other campaign of its list set to not "Activate":
            $DB->query('UPDATE T_email__campaign
				  SET ecmp_activate = 0
				WHERE ecmp_enlt_ID = ' . $DB->quote($edited_EmailCampaign->get('enlt_ID')) . '
				  AND ecmp_activate = 1
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) {	// Delete from DB if confirmed:
            ...
            if (! $edited_EmailCampaign->check_delete(sprintf(TB_('Cannot delete email campaign "%s"'), $edited_EmailCampaign->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
    ...
    case 'test':
        ...
        if (param_errors_detected()) { // Some errors were detected in the campaign's fields or test email address
            break;
        }

        // Send one test email
        if (! $edited_EmailCampaign->send_email($current_User->ID, $test_email_address, 'test')) { // Sending is failed
    ...
    case 'create_cron':
        ...
        if (param_errors_detected()) { // The campaign has some empty fields
            break;
        }

        if ($Settings->get('email_campaign_send_mode') == 'cron') {	// Asynchronous sending mode:
            // Create a scheduled job to send newsletters of this email campaign:
            $edited_EmailCampaign->create_cron_job();
        } else {	// Immediate sending mode:
    ...
    case 'view_cron':
        ...
        if (! ($email_campaign_Cronjob = &$edited_EmailCampaign->get_Cronjob())) {	// No cron job found:
            $action = 'edit';
            $tab = param('current_tab', 'string');
            break;
        }

        if (! check_user_perm('options', 'view')) {	// No access to view cron jobs:
...
switch ($action) {
    ...
    case 'edit_users':
    case 'duplicate':
        if (empty($edited_EmailCampaign)) {	// If no edited email campaign - clear action and display list of campaigns:
            $action = '';
            break;
        }

        if ($action == 'edit_users') {	// Check a recipients count after redirect from users list:
            if ($edited_EmailCampaign->get_recipients_count('filter') == 0) {	// No users in the filterset:
                $Messages->add(TB_('No account matches the filterset. Please try to change the filters.'), 'error');
            }

            if ($edited_EmailCampaign->get_recipients_count('all') == 0) {	// No users for newsletter:
...
if ($tab == 'recipient') {	// Load jQuery QueryBuilder plugin files for user list filters:
...
switch ($action) {
    ...
    case 'delete':
        ...
        if ($ecmp_ID > 0) {	// Build special tabs in edit mode of the campaign:
            $campaign_edit_modes = get_campaign_edit_modes($ecmp_ID);
            $AdminUI->clear_menu_entries(['email', 'campaigns']);
            $AdminUI->add_menu_entries(['email', 'campaigns'], $campaign_edit_modes);
...
switch ($action) {
    ...
    case 'edit':
        ...
        switch ($tab) {
            ...
            case 'compose':
                if ($edited_EmailCampaign->get('email_text') == '' && ! param_errors_detected()) { // Set default value for HTML message
