...
if (! empty($is_admin_page)) {	// Make sure we are calling the right page (on the right domain) to make sure that session cookie goes through:
    if (! is_same_url($ReqHost . $ReqPath, get_admin_url(), true)) {	// The requested URL does not look like it's under the admin URL...
        if (is_same_url($ReqHost . $ReqPath, $baseurl . 'admin.php', true)) {	// Permanent redirect from obsolete admin.php url:
...
if (is_logged_in() && $current_User->get('locale') != $current_locale && ! $locale_from_get) { // change locale to users preference
    /*
    ...
    if (init_charsets($current_charset)) { // Charset was changed reload current User from db to make sure that all of it's data is in the current charset
