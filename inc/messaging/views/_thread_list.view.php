...
if ($perm_abuse_management) { // In case of abuse management
    $Results->register_filter_preset('all', T_('All'), get_dispctrl_url('abuse'));
} else { // In case of simple thread list view
...
threads_results($Results, array_merge([
    'abuse_management' => (int) $perm_abuse_management,
], $display_params));

if (! $perm_abuse_management) { // Show link to create a new conversation
