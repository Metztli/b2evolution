...
if (is_admin_page()) { // Set an url for manual page:
...
function filter_contacts(&$Form)
{
    ...
    if ($item_ID > 0) {	// Save item ID for the filter request
...
if ($item_ID > 0) {	// Save item ID in the filter url
...
function contact_block($block, $user_ID, $user_status)
{
    ...
    if (! is_admin_page()) { // in front office the action will be processed by messaging module handle_htsrv_action() through action.php
...
{
    if ($user_status == 'closed') { // contact account was closed
...
if ($Settings->get('allow_avatars')) {
    ...
    function user_avatar($user_ID)
    {
        ...
        if ($User) {
            ...
            if (! empty($avatar_tag)) {
                if (empty($identity_url)) { // current_User has no permission to view user settings, and Blog is empty
                    return $avatar_tag;
                }
    ...
    function user_login($user_ID, $link = true)
    {
        ...
        if ($User) {
            if ($link) {
                ...
                if ($User->check_status('is_closed')) { // add (closed account) note to corresponding contacts!
...
function contacts_groups($user_ID)
{
    ...
    if (! is_array($cache_user_contacts_groups)) {	// Execute only first time to init cache
        ...
        foreach ($groups as $group) {	// Init cache for groups for each contact of current user
            ...
            if (isset($cache_user_contacts_groups[$group->user_ID])) {	// nth group of this user
                $cache_user_contacts_groups[$group->user_ID] .= '<br />' . $group_name;
            } else {	// first group of this user
                $cache_user_contacts_groups[$group->user_ID] = $group_name;
            }
        }
    }

    if (isset($cache_user_contacts_groups[$user_ID])) {	// user has groups
...
if ($Results->total_rows > 0) {	// Display actions buttons if current User has at least one contact:
    ...
    $multi_action_icon = get_icon('multi_action', 'imgtag', [
        'style' => 'margin:0 2px 0 14px;position:relative;top:-5px;',
    ]);
    ...
    $Form->hidden('users', '');
    if (isset($module_contacts_list_params['form_hiddens']) && ! empty($module_contacts_list_params['form_hiddens'])) {	// Append the hidden input elements from module
        foreach ($module_contacts_list_params['form_hiddens'] as $hidden_input) {
            $Form->hidden($hidden_input['name'], $hidden_input['value']);
        }
    }

    $Form->combo_box('group', param('group_combo', 'string', ''), get_contacts_groups_options(param('group', 'string', '-1'), false), $multi_action_icon . T_('Add all selected contacts to this group'), [
        'new_field_size' => '8',
    ]);

    $Form->buttons([['submit', 'actionArray[add_group]', T_('Add'), 'SaveButton btn-primary btn-sm']]);

    echo '</span>';

    if (isset($group_filtered)) {	// Contacts list is filtered by group
