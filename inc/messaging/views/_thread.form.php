...
$Form->begin_form($params['form_class_thread'], $params['form_title'], [
    'onsubmit' => 'return check_form_thread()',
]);
...
if (! empty($Blog)) { // Set blog as hidden param, because we may need the blog locale after submit
    // This issues should be solved differently
    $Form->hidden('blog', $Blog->ID);
}

if ($params['allow_select_recipients']) {	// User can select recipients
    $Form->text_input(
        'thrd_recipients',
        $edited_Thread->recipients,
        $params['cols'],
        TB_('Recipients'),
    ...
    $Form->radio('thrdtype', $params['thrdtype'], [
        ['discussion', TB_('Start a group discussion')],
        ['individual', TB_('Send individual messages')],
    ], TB_('Multiple recipients'), true);
    echo '</div>';
} else {	// No available to select recipients, Used in /contact.php
...
$Form->text_input('thrd_title', $edited_Thread->title, $params['cols'], TB_('Subject'), '', [
    'maxlength' => 255,
    'required' => true,
    'class' => 'wide_input large',
]);
...
if (is_admin_page() && check_user_perm('files', 'view')) {	// If current user has a permission to view the files AND it is back-office:
...
$Form->textarea_input('msg_text', $edited_Message->original_text, 10, TB_('Message'), [
    'cols' => $params['cols'],
    'required' => true,
]);
...
if (! empty($thrd_recipients_array)) {	// Initialize the preselected users (from post request or when user send a message to own contacts)
...
$Form->end_form([
    ['submit', 'actionArray[preview]', /* TRANS: Verb */ TB_('Preview'), 'SaveButton btn-info'],
    ['submit', 'actionArray[create]', TB_('Send message'), 'SaveButton'],
]);
...
if ($action == 'preview') {	// ------------------ PREVIEW MESSAGE START ------------------ //
    if (isset($edited_Thread->recipients_list)) {
        $recipients_list = $edited_Thread->recipients_list;
    } else {
        $recipients_list = ! empty($edited_Thread->recipients) ? explode(',', $edited_Thread->recipients) : [];
    }
    ...
    if ($creating_success) {	// Display error messages again before preview of message
        global $Messages;
        $Messages->display();
    }
