...
switch ($action) {
    case 'new':
        if (has_cross_country_restriction('users') && empty($current_User->ctry_ID)) { // Cross country contact is restricted but user country is not set
        ...
        if (check_create_thread_limit(true)) { // user has already reached his limit, don't allow to create new thread
            $action = '';
            break;
        }

        if (! isset($edited_Message)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create': // Record new thread
        ...
        if (create_new_thread()) { // new thread has been created successful
    ...
    case 'delete': // Delete thread:
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Thread->check_delete(sprintf(TB_('Cannot delete thread &laquo;%s&raquo;'), $edited_Thread->dget('title')))) { // There are restrictions:
                $action = 'view';
            }
        }
        break;
    ...
    case 'close_and_block': // Close thread and block contact:
        ...
        if ($action == 'close_and_block') { // also block the given contact
