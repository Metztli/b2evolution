...
class Thread extends DataObject
{
    ...
    public function __construct($db_row = null)
    {
        ...
        if ($db_row != null) {
            ...
            if ($Session) {	// check if there is unsaved Thread object stored in Session
                $unsaved_Thread = $Session->get('core.unsaved_Thread');
                if (! empty($unsaved_Thread)) {	// unsaved thread exists, delete it from Session
            ...
            if (! empty($user_login)) {	// Set recipient list from $user_login
                $logins[] = $user_login;
            } else {	// Set recipients from Contacts form
                global $DB, $current_User;

                $recipients = param('recipients', 'string', '');
                $group_ID = param('group_ID', 'integer', 0);

                if (! empty($recipients)) {	// Selected users
                    $recipients = explode(',', $recipients);
                    foreach ($recipients as $r => $recipient) {
                        if ((int) trim($recipient) == 0) {	// remove bad data
                            unset($recipients[$r]);
                        }
                    }
                ...
                } elseif ($group_ID > 0) {	// All users from one group
                    $SQL = new SQL();
                    $SQL->SELECT('user_ID, user_login');
            ...
            if (! empty($logins)) {	// Set this var to initialize the preselected users for fbautocomplete jQuery plugin
    ...
    public static function get_delete_cascades()
    {
        return [
            ...
            [
                ...
                'fk' => 'tsta_thread_ID',
                'msg' => T_('%d read statuses in thread'),
            ],
        ];
    }
     ...
     */
    public function param_check__recipients($var, $recipients, $recipients_array)
    {
        global $DB, $current_User, $UserSettings, $Messages;

        if (! empty($recipients_array)) {	// These data is created by jQuery plugin fbautocomplete
            $recipients_list = $recipients_array['login'];
        } else {	// For browsers without JavaScript
        ...
        $unavailable_recipients_list = [];
        $closed_recipients_list = [];
        $status_restricted_recipients = [];
        $recipients_without_perm = [];
        $recipients_from_different_country = [];
        $recipients_restricted_pm = [];
        ...
        foreach ($recipients_list as $recipient) {
            ...
            if ($recipient_User === false) { // user doesn't exists
                $unavailable_recipients_list[] = $recipient;
                continue;
            }

            if (! $recipient_User->check_status('can_receive_pm')) { // user status restrict to receive private messages
                if ($recipient_User->check_status('is_closed')) { // user account was closed
            ...
            if (! $recipient_User->check_perm('perm_messaging', 'reply')) { // user doesn't have permission to read private messages
                $recipients_without_perm[] = $recipient;
                continue;
            }

            if ($cross_country_restrict && $current_User->ctry_ID != $recipient_User->ctry_ID) { // user can contact with other users only from the same coutnry, but this recipient country is different
                $recipients_from_different_country[] = $recipient;
                continue;
            }

            if (! $UserSettings->get('enable_PM', $recipient_User->ID)) { // recipient doesn't want to receive private messages
                $recipients_restricted_pm[] = $recipient;
                if ($check_enable_pm) { // sender is not a user with delete ( "admin" ) messaging permission, so this user can't be in the recipients list
                    continue;
                }
            }
        ...
        if ($restricted_pm_count > 0) { // there is at least one recipient who doesn't want to receive private messages
            if ($check_enable_pm) { // sender is not a user with delete ( "admin" ) messaging permission, so this user can't be in the recipients list
                if (! empty($error_msg)) {
                    $error_msg .= '<br />';
                }
                $error_msg .= sprintf('The following users don\'t want to receive private messages: %s', implode(', ', $recipients_restricted_pm));
            } else { // send is an admin
                $manual_link = get_manual_link('messaging', T_('See manual') . '.');
                if ($restricted_pm_count > 1) { // more then one recipient don't want to receive private messages
                ...
                } else { // one recipient doesn't want to receive private messages
        ...
        if (! empty($blocked_contacts)) { // There is at least one blocked recipient
            if (! empty($error_msg)) {
                $error_msg .= '<br />';
            }
        ...
        if (empty($error_msg)) { // no errors yet
            $recipients_count = count($recipients_list);
            if (($recipients_count > 1) && (param('thrdtype', 'string', 'discussion') != 'discussion')) { // user want's to send more then one individual messages, check if is allowed
                list($max_new_threads, $new_threads_count) = get_todays_thread_settings();
                if ((! empty($max_new_threads)) && (($max_new_threads - $new_threads_count) < $recipients_count)) { // user has a create thread limit, and recipients number exceed that limit
                    if (! empty($error_msg)) {
                        $error_msg .= '<br />';
                    }
     ...
     */
    public function check_thread_recipient($user_ID)
    {
     ...
     */
    public function check_allow_reply()
    {
        global $DB, $current_User;
        ...
        if (! $this->check_thread_recipient($current_User->ID)) { // Deny to write a new reply for not involved users
        ...
        foreach ($this->recipients_list as $recipient_ID) {
            if ($recipient_ID == $current_User->ID) { // skip current User
                continue;
            }
            $recipient_User = $UserCache->get_by_ID($recipient_ID, false);
            if ($recipient_User && $recipient_User->check_status('can_receive_pm')) { // this recipient exists and status allows to receive private messages
                $all_closed = false;
                break;
            }
        }
        if ($all_closed) { // all recipients are closed or deleted
            param_error('', T_('You cannot reply because all the other users involved in this conversation have closed their account.'));
            return false;
        }

        if (check_user_perm('perm_messaging', 'delete')) { // users with delete permission are always able to reply to a conversation where they are involved
            return true;
        }
        ...
        if ($DB->get_var($SQL) > 0) { // there is at least one recipient who accept the reply
            return true;
        }
