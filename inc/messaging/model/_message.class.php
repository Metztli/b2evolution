...
class Message extends DataObject
{
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_messaging__prerendering',
                'fk' => 'mspr_msg_ID',
            ...
            [
                'table' => 'T_links',
                'fk' => 'link_msg_ID',
                'msg' => T_('%d links to destination private messages'),
                'class' => 'Link',
    ...
    public function load_from_Request()
    {
        ...
        // Text
        if ($Settings->get('allow_html_message')) { // HTML is allowed for messages
            $text_format = 'html';
        } else { // HTML is disallowed for messages
        ...
        $Plugins->trigger_event('MessageThreadFormSent', [
            'content' => &$msg_text,
        ...
        if ($text_format == 'html') { // message text may contain html, check the html sanity
        ...
        } else { // this is a reply to an existing conversation, check if current User is allowed to reply
            $this->get_Thread();
            if ($this->Thread->check_allow_reply()) { // If reply is allowed we should check if this message is not a duplicate
    ...
    public function link_from_Request()
    {
        ...
        if ($this->ID == 0) {	// The message must be stored in DB:
            return;
        }
        ...
        if ($TemporaryID->type != 'message') {	// Wrong temporary object:
            return;
        }
        ...
        foreach ($LinkOwner->Links as $msg_Link) {
            if ($msg_File = &$msg_Link->get_File() &&
                ...
                if (! file_exists($msg_FileRoot->ads_path . 'private_message/pm' . $this->ID . '/')) {	// Create if folder doesn't exist for files of new created message:
     ...
     */
    public function link_from_Message($source_msg_ID)
    {
        if ($this->ID == 0 || $source_msg_ID == 0) {	// Current and source message must be created in DB:
            return;
        }
        ...
        if (empty($inlines[0])) {	// If content of source message doesn't contain inline tags then we should update a content of current message,
        ...
        } else {	// The source message content contains at least one inline tag,
            ...
            if (empty($source_LinkOwner->Links)) {	// No links:
                return;
            }
            ...
            foreach ($source_LinkOwner->Links as $source_Link) {
                if ($new_link_ID = $this_LinkOwner->add_link($source_Link->file_ID, $source_Link->position, $source_Link->order)) {	// If new link is added then store this in array in order to update the message content for inline tags like [image:123]:
     ...
     */
    public function dbinsert_discussion($from_User = null, $source_msg_ID = null)
    {
        global $DB, $Plugins;
        ...
        if ($this->Thread->dbinsert()) {
            ...
            if (isset($Plugins)) {	// Note: Plugins may not be available during maintenance, install or test cases
                $Plugins->trigger_event('PrependMessageInsertTransact', $params = [
                    'Message' => &$this,
                ]);
            }
                ...
                if ($this->dbinsert_threadstatus($this->Thread->recipients_list)) {
                    if ($this->dbinsert_contacts($this->Thread->recipients_list)) {
                        if ($this->dbupdate_last_contact_datetime()) {
                            ...
                            if ($source_msg_ID === null) {	// Link attachments from temporary object to new created Message:
                                $this->link_from_Request();
                            } else {	// Link attachments from source Message object to this Message:
     ...
     */
    public function dbinsert_individual($from_User = null)
    {
        $source_msg_ID = null;
        ...
        foreach ($this->Thread->recipients_list as $recipient_ID) {
            ...
            $message->Thread->recipients_list = [$recipient_ID];
            ...
            if ($source_msg_ID === null) {	// Use first message as source for all next messages:
     ...
     * @return true if success, instead false
     */
    public function dbinsert_message()
    {
        ...
        if ($this->Thread->dbupdate()) {
            ...
            if (parent::dbinsert()) {
                ...
                $recipient_list = array_diff($recipient_list, [$this->author_user_ID]);
     ...
     * @return true if success, instead false
     */
    public function dbinsert_threadstatus($recipients_list)
    {
        global $DB;
     ...
     * @return true if success, instead false
     */
    public function dbinsert_contacts($recipients)
    {
        ...
        if (! empty($contact_list) || ! empty($reverse_contact_list)) { // insert users/recipients which are not in contact list
            ...
            foreach ($contact_list as $contact_ID) {
                if ($contact_ID != $this->author_user_ID) { // Don't insert a contact from the same user
            ...
            if (! empty($statements)) { // Do insert sql only when data exist
     ...
     */
    public function dbupdate_last_contact_datetime()
    {
        global $DB, $localtimenow;
     ...
     */
    public function clone_message($message)
    {
    ...
    public static function db_delete_where($sql_where, $object_ids = null, $params = null)
    {
        ...
        if (empty($thread_ids_to_delete)) { // Make sure thread ids of the messages are collected
        ...
        if ($result) { // Remove messages with all of its delete cascade relations
            $result = parent::db_delete_where($sql_where, $object_ids);
        }

        if ($result !== false) { // Delete those threads where all of the messages were deleted
            ...
            if ((! empty($orphan_thread_ids)) && (Thread::db_delete_where(null, $orphan_thread_ids) === false)) { // Deleting threads was unsuccessful
                $result = false;
            }
        }
     ...
     */
    public function check_perm($action, $assert = true)
    {
        global $current_User;
     ...
     */
    public function send_email_notifications($new_thread = true, $from_User = null)
    {
        ...
        foreach ($thrd_recipients as $recipient_ID => $notify_messages) { // Send mail to recipients who needs to be notified. recipients are already loaded into the UserCache
            if (! ($notify_messages || (is_null($notify_messages) && $def_notify_messages))) { // User should NOT be notified
                continue;
            }
            ...
            if ($message_link === false) {	// If the recipient has no access to messages page:
            ...
            if (send_mail_to_User($recipient_ID, $localized_subject, 'private_message_new', $email_template_params)) { // email sent successful, update las_unread_message_reminder timestamp, because the notification contains all unread messages
    ...
    public function get_prerendered_content($format = 'htmlbody')
    {
        ...
        if ($use_cache) { // the format/comment can be cached:
            ...
            if (isset($MessagePrerenderingCache[$format][$this->ID][$cache_key])) { // already in PHP cache.
                ...
                if (! isset($MessagePrerenderingCache[$format])) { // only do the prefetch loading once.
                    $MessagePrerenderingCache[$format] = [];
                    ...
                    foreach ($rows as $row) {
                        ...
                        if (! isset($MessagePrerenderingCache[$format][$row->mspr_msg_ID])) { // init list
                            $MessagePrerenderingCache[$format][$row->mspr_msg_ID] = [];
                        }
        ...
        if (! isset($r)) {
            ...
            $data = render_inline_files($data, $this, [
                'check_code_block' => true,
                'image_size' => 'original',
                'image_link_rel' => 'lightbox[m' . $this->ID . ']',
            ]);

            $Plugins->trigger_event('FilterMsgContent', [
                'data' => &$data,
                'Message' => $this,
            ]);
     ...
     * @return string
     */
    public function get_content($format = 'htmlbody')
    {
        ...
        */
        ...
        if (empty($this->ID)) {	// Preview mode for new creating message:
    ...
    public function get_images($params = [], $format = 'htmlbody')
    {
        ...
        } else {	// Normal mode for existing Message in DB:
            $tmp_object_ID = null;
        }
        ...
        $exclude_link_IDs = [];
        if ($params['exclude_inline_tagged']) {	// Find all links which are already rendered in content by inline tags like '[image:123]':
            preg_match_all('/\[(image|file|inline|video|audio|thumbnail):(\d+)(:?)([^\]]*)\]/i', $this->text, $inlines);
        ...
        while ($image_counter < $params['limit'] && $Link = &$LinkList->get_next()) {
            if (in_array($Link->ID, $exclude_link_IDs)) {	// Skip this link because it is already rendered by inline tag like '[image:123]':
                continue;
            }

            if (! ($File = &$Link->get_File())) {	// No File object:
                global $Debuglog;
                $Debuglog->add(sprintf('Link ID#%d of message #%d does not have a file object!', $Link->ID, $this->ID), ['error', 'files']);
                continue;
            }

            if (! $File->exists()) { // File doesn't exist
                global $Debuglog;
                $Debuglog->add(sprintf('File linked to message #%d does not exist (%s)!', $this->ID, $File->get_full_path()), ['error', 'files']);
                continue;
            }
            ...
            if (! $params['get_rendered_attachments']) { // Save $r to temp var in order to don't get the rendered data from plugins
                $temp_r = $r;
            }
            ...
            if (count($Plugins->trigger_event_first_true('RenderMessageAttachment', $params)) != 0) {	// This attachment has been rendered by a plugin (to $params['data']), Skip this from core rendering:
                if (! $params['get_rendered_attachments']) { // Restore $r value and mark this message has the rendered attachments
                    $r = $temp_r;
        ...
        if (empty($r) && $plugin_render_attachments) {	// This message doesn't contain the images but it has the rendered attachments by plugins:
    ...
    public function get_files($params = [], $format = 'htmlbody')
    {
        ...
        while (($Link = &$LinkList->get_next()) && $params['limit_attach'] > $i) {
            ...
            if ($File->is_image() && $Link->get('position') != 'attachment') {	// Skip images (except those in the attachment position) because these are displayed inline already:
