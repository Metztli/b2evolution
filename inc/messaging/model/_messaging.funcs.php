...
function check_blocked_contacts($recipients_list)
{
    ...
    if (empty($recipients_list)) { // there are no recipients to check
        return null;
    }

    if (check_user_perm('perm_messaging', 'delete')) { // user with delete messaging permission are allowed to send private messages to anyone who has access to read them.
        return null;
    }
    ...
    if (check_user_perm('perm_messaging', 'write', false)) { // get blocked contacts for user with write permission
        $sub_SQL = new SQL();
    ...
    } else { // get every user, except non blocked contacts, for users with only reply permission
...
function set_contact_blocked($user_ID, $blocked)
{
    ...
    if ($is_contact === null) { // Add user to the contacts list
        global $localtimenow;
...
function create_new_thread()
{
    ...
    if ($Settings->get('system_lock')) { // System is locked for maintenance, All users cannot send a message
    ...
    $Plugins->trigger_event('MessageFormSent', [
    ...
    if ($edited_Message->load_from_Request()) { // We could load data from form without errors:
        if ($action == 'preview') { // Don't insert message in preview mode
...
function leave_thread($thread_ID, $user_ID, $close_thread = false)
{
    ...
    if (! $edited_Thread->check_thread_recipient($user_ID)) { // user is not between the thread recipients
    ...
    if (! $close_thread) { // don't close the thread for all user only the given user wants to leave
...
function get_messages_link_to($thread_ID = null, $user_ID = null)
{
    ...
    if ($user_ID === null) {	// Get current user:
        global $current_User;
        if (is_logged_in()) {
            $User = $current_User;
        }
    } else {	// Get user by requested ID:
        $UserCache = &get_UserCache();
    ...
    if ($msg_Blog = &get_setting_Blog('msg_blog_ID') && ! empty($User) &&
        ($msg_Blog->get_setting('allow_access') != 'members' || $User->check_perm('blog_ismember', 'view', false, $msg_Blog->ID))) {	// Link to blog that is used for messaging:
        $message_link = url_add_param($msg_Blog->gen_blogurl(), 'disp=' . $link_tail);
        $prefs_link = $msg_Blog->get('userprefsurl');
    } elseif (! empty($User) && $User->check_perm('admin', 'restricted')) {	// Link to admin messaging managment:
    ...
    return [$message_link, $prefs_link];
}
...
function get_messaging_sub_entries($is_admin)
{
    ...
    if ($is_admin && check_user_perm('options', 'edit')) {
        $messaging_sub_entries['msgsettings'] = [
            'text' => T_('Settings'),
...
function get_thread_recipient_status_icons($thread_ID, $status)
{
    ...
    if (! $perm_abuse_management) {	// If non abuse management
    ...
    if (! empty($user_logins)) {	// Display status icons if at least one user is found for the requested status:
...
function get_users_unread_threads($userid_list, $skip_thread_ID = null, $threads_format = 'string', $login_format = 'text', $protocol = '')
{
    ...
    if (empty($userid_list)) { // requested user id list is empty, return empty result
        return $result;
    }
    ...
    if (empty($user_unread_threads)) { // requested users have no unread threads
        return $result;
    }

    $all_involved_threads = [];
    $user_threads = [];
    ...
    foreach ($all_threads_recipients as $row) {
        if (! isset($thread_recipients[$row->thread_ID])) {
            $thread_recipients[$row->thread_ID] = [];
        }
        if (empty($row->user_login)) { // User was deleted
            $thread_recipients[$row->thread_ID][] = 'Deleted user';
        } else { // User exists
            if ($login_format == 'text') { // Use simple login as text
    ...
    foreach ($userid_list as $user_ID) {
        ...
        $unread_threads = $threads_format == 'string' ? '' : [];
        ...
        foreach ($threads as $thread_ID) {
            ...
            if ($threads_format == 'string') {	// Store all threads in one string
                $unread_threads .= "\t - " . sprintf('"%s" ( %s )', $thread_titles[$thread_ID], $recipient_names) . "\n";
            } else {	// Store all threads in array
                $unread_threads[] = sprintf('"%s" ( %s )', $thread_titles[$thread_ID], $recipient_names);
            }
        }
 ...
 */
function get_threads_results($params = [])
{
    ...
    $filter_sql = '';
    if (! empty($params['search_word']) || ! empty($params['search_user']) || ! empty($params['sent_user_ID']) || ! empty($params['received_user_ID'])) {	// We want to filter on search keyword:
        $filter_sql = [];
        if (! empty($params['search_word'])) { // Search by title
            $filter_sql[] = 'thrd_title LIKE "%' . $DB->escape($params['search_word']) . '%"';
        }
        if (! empty($params['search_user'])) {	// Search by user names:
            ...
            if (empty($threads_IDs)) {	// No found related threads
                $threads_IDs[] = '-1';
            }

            $filter_sql[] = 'tsta_thread_ID IN ( ' . implode(',', $threads_IDs) . ' )';
        }
        if (! empty($params['sent_user_ID'])) {	// Limit messages only for sent by user:
        ...
        if (! empty($params['received_user_ID'])) {	// Limit messages only for received by user:
    ...
    if (! $perm_abuse_management) { // Limit threads by current user
        $select_SQL->WHERE('tsta_user_ID = ' . $params['user_ID']);
        if ($params['show_closed_threads'] === null) { // Explicit param value was not set, use the default
        ...
        if (! $params['show_closed_threads']) { // Don't show the closed conversations
            $select_SQL->WHERE_and('( tsta_thread_leave_msg_ID IS NULL )');
        }
    }
    if (! empty($filter_sql)) {	// Filter
    ...
    if ($params['only_sql']) { // Return only SQL object
        return $select_SQL;
    }
...
function create_contacts_user($user_ID, $blocked = false)
{
    ...
    if (! is_null($contact)) { // This user already exists in the contact list for current user
        return true;
    }
...
function create_contacts_group($group_name)
{
    ...
    if ($group_name != '') {	// Check new group name for duplicates
        ...
        if (! is_null($group)) {	// Duplicate group
            $Messages->add(T_('You already have a group with this name.'), 'error');
        }
    }

    if (param_errors_detected()) {	// Errors exist, Exit here
        return false;
    }
    ...
    if ($DB->query($sql, 'Insert contacts group')) {	// Success query, Return ID of new group
        return $DB->insert_id;
    } else {	// Failed query
        return false;
    }
...
function create_contacts_group_users($group, $users, $new_group_field_name = 'group_combo')
{
    ...
    if (count($users_IDs) == 0 || strlen($users) == 0) {	// No selected users
        $Messages->add(T_('Please select at least one user.'), 'error');
        return false;
    }

    if ($group == 'new' || (int) $group < 0) {	// Add new group
        if ((int) $group < 0) {	// Default group
            $default_groups = get_contacts_groups_default();
            if (isset($default_groups[$group])) {	// Get group name
                $group_name = $default_groups[$group];
            } else {	// Error
                $Messages->add('No found this group.', 'error');
                return false;
            }
        } else {	// New entered group
        ...
        if ($group_ID = create_contacts_group($group_name)) {	// Create group
        ...
        if ($group_ID == 0) {	// No defined group ID
            return false;
        }
        ...
        if (is_null($group)) {	// User try use a group of another user
            return false;
        }
    ...
    foreach ($users_IDs as $user_ID) {
        ...
        if ($user_ID == 0) {	// User ID is empty
            continue;
        } elseif (isset($users_already_grouped[$user_ID])) {
            if ($users_already_grouped[$user_ID] == $group_ID) {	// This user already is added in selected group
                continue;
            }
        }
    ...
    if (count($records) == 0) {	// No data to add
        return false;
    }

    if ($DB->query($sql, 'Insert users for contacts group')) {	// Success query
        return [
...
function remove_contacts_group_user($group_ID, $user_ID)
{
    ...
    if ($DB->query($sql, 'Remove user from contacts group')) {	// Success query
        $UserCache = &get_UserCache();
...
function update_contacts_groups_user($user_ID, $groups, $blocked = null)
{
    ...
    if (empty($user_ID)) { // No selected user
        return false;
    }

    if ($blocked !== null) { // Update the contact block status
    ...
    foreach ($groups as $group) {
        if ($group == 'new' || intval($group) < 0) { // Add new group
            if (intval($group) < 0) { // Default group
                $default_groups = get_contacts_groups_default();
                if (isset($default_groups[$group])) { // Get group name
                    $group_name = $default_groups[$group];
                }
            ...
            if ($group_ID = create_contacts_group($group_name)) { // Create new group
                $Messages->add(T_('New contacts group has been created.'), 'success');
            }
        } else { // Existing group
        ...
        if (empty($group_ID)) { // No defined group ID
            continue;
        }
    ...
    if (count($current_user_groups) > 0) { // Clear previous selected groups for the user before new updating, in order to delete the user from the unchecked groups
    ...
    if ($DB->query($insert_sql, 'Insert user for contact groups')) { // Success query
        return true;
    } else { // Failed query
        return false;
    }
...
function get_contacts_groups_array($include_default_groups = true)
{
    ...
    $groups = [];
    if ($include_default_groups) { // Include the default groups
        $default_groups = get_contacts_groups_default();
        foreach ($default_groups as $group_ID => $group_name) {
            if (! in_array($group_name, $user_groups)) { // Set this default group If it doesn't exist in DB
...
function get_contacts_groups_options($selected_group_ID = null, $value_null = true)
{
    ...
    if ($value_null) { // Null option
        $options .= '<option value="0">' . T_('All') . '</option>' . "\n";
    }

    foreach ($groups as $group_ID => $group_name) {
        $selected = '';
        if ($selected_group_ID == $group_ID) { // Group is selected
 ...
 */
function get_contacts_groups_list($user_ID, $params = [])
{
    ...
    $groups = [];
    foreach ($default_groups as $group_ID => $group_name) {
        if (! in_array($group_name, $all_groups)) {	// Set this default group If it doesn't exist in DB
            $groups[$group_ID] = $group_name;
        }
    }
    ...
    if (count($all_groups) == 0) {	// No groups
        return;
    }
    ...
    if (! empty($Blog)) {	// Set url to contacts list
    ...
    $groups_2nd = [];
    foreach ($all_groups as $group_ID => $group_name) {
        if (! empty($contacts_url) && $group_ID > 0) {	// Set link to contacts list with filtered by current group
            $group_link = '<a href="' . url_add_param($contacts_url, 'g=' . $group_ID) . '">' . $group_name . '</a>';
        } else {	// No link, use name
            $group_link = $group_name;
        }
        if (isset($user_groups[$group_ID])) {	// User is in this group
...
function update_todays_thread_settings($new_threads_count)
{
    ...
    if (empty($last_new_thread_ts)) { // this is the current User first new thread
        ...
        if ($last_new_thread_date < $today) { // this is the current User first new thread today
            $UserSettings->set('new_thread_count', $new_threads_count, $current_User->ID);
        } else { // current User has already created other new threads today
...
function get_todays_thread_settings()
{
    ...
    if ($max_new_threads === '0') { // user limit is 0, user must not be able to create new threads
        return ['0', 0];
    }

    $last_new_thread_ts = $UserSettings->get('last_new_thread', $current_User->ID);
    if (empty($last_new_thread_ts)) { // user has not created any new threads yet
        return [$max_new_threads, 0];
    }

    $today = date('Y-m-d', $servertimenow);
    $last_new_thread_date = date('Y-m-d', $last_new_thread_ts);
    if ($last_new_thread_date < $today) { // user's last new thread was not created today, it's older
        return [$max_new_threads, 0];
    }

    // User has already created at least one new thread today
    $new_thread_count = $UserSettings->get('new_thread_count', $current_User->ID);
    return [$max_new_threads, $new_thread_count];
}
...
function check_create_thread_limit($add_message = false)
{
    ...
    if ($max_new_threads === '0') { // user new conversation limit is 0
        if ($add_message) {
    ...
    if (empty($max_new_threads)) { // user has no limit
        return false;
    }

    if ($max_new_threads > $new_threads_count) { // user didn't reach his limit yet
        if ($add_message) {
    ...
    if ($add_message) { // user has already reached his limit
...
function get_unread_messages_count($user_ID = 0)
{
    ...
    if (! is_array($cache_unread_messages_count)) {	// Initialize array first time:
        $cache_unread_messages_count = [];
    }

    if (! isset($cache_unread_messages_count[$user_ID])) {	// Get a result from DB only first time and store it in global cache array:
...
function get_next_reminder_info($user_ID)
{
    ...
    if (! $UserSettings->get('notify_unread_messages', $user_ID)) { // The user doesn't want to recive unread messages reminders
    ...
    if (empty($first_unread_message_date)) { // The user doesn't have unread messages
    ...
    if (empty($last_unread_messages_reminder)) { // User didn't get new message notification or unread message reminder yet
    ...
    } else { // Count next unread message reminder date, this can be delayed if the edited User didn't logged in since many days
        $UserCache = &get_UserCache();
        ...
        foreach ($unread_message_reminder_delay as $lastseen => $delay) { // Get the corresponding number of delay for the edited User
            if ($days_since_lastseen < $lastseen) { // We have found the correct delay value, reminders should be sent
        ...
        if ($dont_send_reminder) { // User was not logged in since too long
    ...
    if ($reminder_issue_ts > $servertimenow) { // The next reminder issue date is in the future
    ...
    if ($reminder_is_delayed) { // Reminder is delayed, add a note about this
    ...
    } elseif (empty($last_unread_messages_reminder)) { // The user didn't get unread messages reminder emails before
...
function mark_as_read_by_user($thrd_ID, $user_ID)
{
    ...
    $first_unread_msg_ID_subquery = '(SELECT msg_ID
		...
		LIMIT 1)';
    $DB->query(
        'UPDATE T_messaging__threadstatus
...
function delete_orphan_threads($user_ids = null)
{
    ...
    if (empty($affected_threads_ids)) { // There are no affected thread ids, nothing to delete
        $DB->commit();
        return true;
    }
    ...
    if (! empty($orphan_thread_ids)) { // There are orphan threads ( or orphan thread targets )
        load_class('messaging/model/_thread.class.php', 'Thread');
        // Delete all orphan threads with all cascade relations
        if (Thread::db_delete_where(null, $orphan_thread_ids, [
            'use_transaction' => false,
        ]) === false) { // Deleting orphan threads failed
            $DB->rollback();
            return false;
        }
    }
...
function get_thread_prevnext_ID($current_thread_ID, $type = 'prev')
{
    ...
    if (! isset($thread_prevnext_ids_cache)) {	// Initialize list with threads IDs
        global $DB;

        $threads_SQL = get_threads_results([
            'only_sql' => true,
        ]);
    ...
    foreach ($thread_prevnext_ids_cache as $t => $thread_ID) {
        if (isset($thread_prevnext_ids_cache[$t + $side_thread_i]) && $thread_prevnext_ids_cache[$t + $side_thread_i] == $current_thread_ID) {	// This thread is previous/next for current thread
            return $thread_ID;
        }
    }
}
 ...
 * @return integer Thread ID
 */
function get_thread_prevnext_links($current_thread_ID, $params = [])
{
    $params = array_merge([
    ...
    if (! empty($prev_thread_ID)) {	// Link to previous thread
    ...
    if (! empty($next_thread_ID)) {	// Link to previous thread
        $next_link = '<a href="' . get_dispctrl_url('messages', 'thrd_ID=' . $next_thread_ID) . '">' . $params['next_text'] . '</a>';
    }

    if (empty($prev_link) && empty($next_link)) {	// No found previous and next threads
        return;
    }
...
function get_msgform_contact_methods($recipient_User = null)
{
    ...
    if ($recipient_User !== null) {	// Check what contact methods are allowed between recipient and current users:
        if ($recipient_User->get_msgform_possibility(null, 'PM') == 'PM') {	// PM method is allowed:
            $contact_methods['pm'] = T_('Private Message on this Site');
        }
        if ($recipient_User->get_msgform_possibility(null, 'email') == 'email') {	// Email method is allowed:
 ...
 * @param array Params
 */
function threads_results_block($params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
        'edited_User' => null,
    ...
    if (! check_user_perm('users', 'moderate') || ! check_user_perm('perm_messaging', 'reply')) {	// Check minimum permission:
        return;
    }
    ...
    if (check_user_perm('perm_messaging', 'abuse')) {
        ...
        if ($params['action'] != 'view' && $threads_Results->get_total_rows() > 0) {	// Display action icon to delete all records if at least one record exists
    ...
    } else { // No permission for abuse management
 ...
 * @param array Params
 */
function received_threads_results_block($params = [])
{
    threads_results_block(array_merge($params, [
 ...
 * @param array Params
 */
function threads_results(&$threads_Results, $params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
    ...
    if ($params['display_recipients']) {	// Display Recipients column
        $threads_Results->cols[] = [
    ...
    if ($params['display_subject']) {	// Display Subject column
        $threads_Results->cols[] = [
            'th' => T_('Subject'),
    ...
    if ($params['display_read']) {	// Display Read column
        $threads_Results->cols[] = [
            'th' => T_('Read?'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'center',
    ...
    if ($params['display_actions']) {	// Display Actions column
        if (check_user_perm('perm_messaging', 'delete')) {	// We have permission to modify:
            $threads_Results->cols[] = [
                'th' => T_('Del'),
                'th_class' => 'shrinkwrap',
                'td_class' => 'shrinkwrap',
...
function col_thread_recipients($thread_ID, $abuse_management)
{
    ...
    if (! $abuse_management) {	// Show current user only in abuse management
        global $current_User;
    ...
    if (empty($recipients)) { // There are no recipients in this list. This is only possible if the recipients were deleted as spammer
...
function col_thread_subject_link($thrd_ID, $thrd_title, $thrd_msg_ID, $thrd_leave_msg_ID, $abuse_management)
{
    ...
    $read_icon = '';
    if ($thrd_msg_ID != 'abuse') {	// Don't show the read status for abuse management
        if (! empty($thrd_leave_msg_ID)) { // Conversation is left
            $read_icon = get_icon('bullet_black', 'imgtag', [
                'style' => 'margin:0 2px',
                'alt' => T_("Conversation left"),
            ]);
            if ($thrd_msg_ID > 0 && $thrd_msg_ID <= $thrd_leave_msg_ID) { // also show unread messages icon, because user has not seen all messages yet
                $read_icon .= get_icon('bullet_red', 'imgtag', [
                    'style' => 'margin:0 2px',
                    'alt' => T_("You have unread messages"),
                ]);
            }
        } elseif ($thrd_msg_ID > 0) { // Message is unread OR Show all messages in Abuse Management as unread
            $read_icon = get_icon('bullet_red', 'imgtag', [
                'style' => 'margin:0 2px',
                'alt' => T_("You have unread messages"),
            ]);
            ...
            $read_icon = get_icon('allowback', 'imgtag', [
                'alt' => "You have read all messages",
            ]);
        }
    }

    $tab = '';
    if ($abuse_management) { // We are in Abuse Management
        $tab = '&amp;tab=abuse';
    }
...
{
    $author = get_user_avatar_styled($user_ID, [
        'size' => 'crop-top-80x80',
    ]);
...
function col_msg_read_by($message_ID)
{
    ...
    if (empty($Blog)) {	// Set avatar size for a case when blog is not defined
        $avatar_size = 'crop-top-32x32';
    } else {	// Get avatar size from blog settings
    ...
    foreach ($read_status_list as $user_ID => $first_unread_msg_ID) {
        if ($user_ID == $current_User->ID) { // Current user status: current user should not be displayed except in case of abuse management
            if ($perm_abuse_management) { // current user has seen all received messages for sure, set first unread msg to NULL
                $first_unread_msg_ID = null;
            } else { // not abuse management
                continue;
            }
        }

        $recipient_User = $UserCache->get_by_ID($user_ID, false);
        if (! $recipient_User) { // user not exists
            continue;
        }

        $leave_msg_ID = $leave_status_list[$user_ID];
        if ($message_ID > 0 && ! empty($leave_msg_ID) && ($leave_msg_ID < $message_ID)) { // user has left the conversation and didn't receive this message
            $left_recipients[] = $recipient_User->login;
        } elseif ($message_ID > 0 && (empty($first_unread_msg_ID) || ($first_unread_msg_ID > $message_ID))) { // user has read all message from this thread or at least this message
            // user didn't leave the conversation before this message
            $read_recipients[] = $recipient_User->login;
        } else { // User didn't read this message, but didn't leave the conversation either
            $unread_recipients[] = $recipient_User->login;
        }
    }

    $read_by = '';
    if (! empty($read_recipients)) { // There are users who have read this message
    ...
    if (! empty($unread_recipients)) { // There are users who didn't read this message
        asort($unread_recipients);
        $read_by .= '<div>' . get_avatar_imgtags($unread_recipients, true, false, $avatar_size, '', '', false, false) . '</div>';
    }

    if (! empty($left_recipients)) { // There are users who left the conversation before this message
...
function col_msg_actions($thrd_ID, $msg_ID)
{
    ...
    if ($msg_ID < 1) { // Don't display actions in preview mode
        return '&nbsp;';
    } elseif (is_admin_page()) {
        $tab = '';
        if ($perm_abuse_management) {	// We are in Abuse Management
            $tab = '&tab=abuse';
        }
