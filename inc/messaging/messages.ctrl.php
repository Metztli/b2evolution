...
if ($tab == 'abuse' && check_user_perm('perm_messaging', 'abuse')) {	// We go from abuse management and have a permissions
    $perm_abuse_management = true;
} else {
    $perm_abuse_management = false;
}

if (param('thrd_ID', 'integer', '', true)) { // Load thread from cache:
    ...
    if (($edited_Thread = &$ThreadCache->get_by_ID($thrd_ID, false)) === false) { // Thread doesn't exists with this ID
        ...
        $action = 'nil';
    } elseif (! $edited_Thread->check_thread_recipient($current_User->ID) && ! $perm_abuse_management) { // Current user is not recipient of this thread and he is not abuse manager
        unset($edited_Thread);
        forget_param('thrd_ID');
...
if (param('msg_ID', 'integer', '', true)) { // Load message from cache:
...
if (! $Messages->has_errors() && (empty($thrd_ID) || empty($edited_Thread))) { // Display this error only when no error above
...
$param_tab = '';
if ($perm_abuse_management) {	// After completing of the action ( create | delete ) we want back to the abuse managment
    $param_tab = '&tab=abuse';
}
...
switch ($action) {
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Message->check_delete(TB_('Cannot delete message.'))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
    ...
    default:
        ...
        if (empty($edited_Thread)) { // there are no thread what to show
            break;
        }
...
switch ($action) {
    ...
    case 'delete':
        if ($perm_abuse_management) {	// Save a tab param for hidden fields of the form
            memorize_param('tab', 'string', 'abuse');
        }
        // We need to ask for confirmation:
        $edited_Message->confirm_delete(
            TB_('Delete message?'),
            'messaging_messages',
            $action,
