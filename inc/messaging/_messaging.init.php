...
function &get_MessageCache()
{
    ...
    if (! isset($MessageCache)) {	// Cache doesn't exist yet:
...
function &get_MessagePrerenderingCache()
{
    ...
    if (! isset($MessagePrerenderingCache)) { // Cache doesn't exist yet:
        $MessagePrerenderingCache = [];
    }
...
function &get_ThreadCache()
{
    ...
    if (! isset($ThreadCache)) {	// Cache doesn't exist yet:
...
class messaging_Module extends Module
{
        ...
        $permissions = [
            'perm_messaging' => [
                ...
                'options' => [
                    ...
                    ['none', T_('No Access'), ''],
                    ['reply', T_('Read & Send messages to people in contacts list only (except for blocked contacts)'), ''],
                    ['write', T_('Read & Send messages to anyone (except for blocked contacts)'), ''],
                    ['delete', T_('Read, Send & Delete any messages (including for blocked contacts)'), ''],
                    ['abuse', T_('Abuse Management'), '']],
            ],
            'max_new_threads' => [
     ...
     */
    public function check_messaging_user_perm($permlevel, $permvalue, $permtarget)
    {
        global $current_User;

        if ($permtarget > 0) {   // Check user permission for current thread
     ...
     * @return boolean True on success (permission is granted), false if permission is not granted
     */
    public function check_messaging_group_perm($permlevel, $permvalue, $permtarget)
    {
        $perm = false;
        switch ($permvalue) {
            case 'abuse':
            ...
            case 'delete':
                ...
                if ($permlevel == 'delete') { // User can ask for delete perm...
                    $perm = true;
                    break;
                }
                // no break
     ...
     */
    public function get_group_settings($permname, $permvalue, $permtarget)
    {
        return $permvalue;
    }
    ...
    public function build_evobar_menu()
    {
        ...
        if (check_user_perm('perm_messaging', 'reply')) {
            ...
            if (! empty($messages_url)) { // Display this menu item only when url is available to current user
                $right_entries['messages'] = [
                    'text' => T_('Messages'),
                    'href' => $messages_url,
                    'class' => 'evo_messages_link',
                ];
            }
            if (! empty($contacts_url)) { // Display this menu item only when url is available to current user
                $right_entries['contacts'] = [
                    'text' => T_('Contacts'),
                    'href' => $contacts_url,
                    'class' => 'evo_messages_link',
                ];
        ...
        $topright_Menu->insert_menu_entries_after(['userprefs', 'name'], $right_entries);
    }
    ...
    public function build_menu_3()
    {
        ...
        if (check_user_perm('perm_messaging', 'reply')) { // Permission to view messaging:
            // Count unread messages for current user
            $unread_messages_count = get_unread_messages_count();
            if ($unread_messages_count > 0) {
    ...
    public function handle_htsrv_action()
    {
        ...
        if (($disp != 'contacts') && ($thrd_ID = param('thrd_ID', 'integer', '', true))) { // Load thread from cache:
            $ThreadCache = &get_ThreadCache();
            if (($edited_Thread = &$ThreadCache->get_by_ID($thrd_ID, false)) === false) { // Thread doesn't exists with this ID
                unset($edited_Thread);
        ...
        switch ($disp) {
            // threads action
            case 'threads':
                if ($action != 'create' && $action != 'preview') { // Make sure we got a thrd_ID:
                ...
                switch ($action) {
                    ...
                    case 'preview': // preview message
                        ...
                        if (check_create_thread_limit()) { // max new threads limit reached, don't allow to create new thread
                            debug_die('Invalid request, new conversation limit already reached!');
                        }

                        $creating_success = create_new_thread();
                        if (! $creating_success || $action == 'preview') { // unsuccessful new thread creation OR preview mode
                    ...
                    case 'close_and_block': // close the thread and block contact
                        ...
                        if ($action == 'close_and_block') { // user also wants to block contact with the other user involved in this thread
            ...
            case 'contacts':
                ...
                if (($action != 'block') && ($action != 'unblock')) { // only block or unblock is valid
                ...
                switch ($action) {
                    ...
                    case 'delete': // delete message
                        ...
                        if ($confirmed) { // delete message
                            ...
                            if (! empty($Blog)) { // Add blog ID to correctly redirect after deleting:
    ...
    public function get_contacts_list_params()
    {
        ...
        if (! isset($module_contacts_list_params)) {	// Initialize this array first time
            $module_contacts_list_params = [];
        }
