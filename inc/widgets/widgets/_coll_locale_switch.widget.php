...
class coll_locale_switch_Widget extends ComponentWidget
{
    ...
    public function display($params)
    {
        ...
        if (count($coll_locales) < 2) {	// Don't display this widget when less 2 locales:
        ...
        foreach ($coll_locales as $coll_locale => $linked_coll_ID) {
            if (! isset($locales[$coll_locale]) || ! $locales[$coll_locale]['enabled']) {	// Skip wrong or disabled locale:
                continue;
            }

            if (! $this->disp_params['show_current'] && $view_locale == $coll_locale) {	// Don't show current locale:
                continue;
            }

            if ($current_Item &&
                ($version_Item = &$current_Item->get_version_Item($coll_locale))) {	// Use permanent URL of the version Item with requested locale:
                $locale_switch_url = $version_Item->get_permanent_url();
            } else {	// Use URL to front page of the current collection:
                if (! empty($linked_coll_ID)) {	// Use linked collection:
                ...
                if ($locale_Blog) {
                    ...
                    if ($coll_locale != $locale_Blog->get('locale')) {	// Append locale param only when main locale of the Collection is different than current needed:
                        $locale_switch_url = url_add_param($locale_switch_url, 'coll_locale=' . urlencode($coll_locale));
                    }
                } else {	// Skip wrong Collection:
                    continue;
                }
            }
