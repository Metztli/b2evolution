...
class org_members_Widget extends ComponentWidget
{
    ...
    public function display($params)
    {
        ...
        if (empty($Organization)) { // No organization found
            global $Messages;
		...
		if (! empty($this->disp_params['icon_colors'])) {
            ...
            if (count($users)) {
                ...
                foreach ($users as $org_User) {
                    ...
                    if (! empty($user_url)) { // Display url to user page only when it is allowed by widget setting
                    ...
                    if (! empty($user_url)) { // End of user link, see above
                        echo '</a>';
                    ...
                    if ($this->disp_params['display_icons']) { // Display user links as icons
                    ...
                    if (! empty($this->disp_params['field_code']) && ($field_values = $org_User->userfield_values_by_code($this->disp_params['field_code']))) {
                        ...
                        foreach ($field_values as $f => $field_value) {
                            if ($f > 0) { // Space between each field value
                                echo ' ';
                            }
                            echo preg_replace("/[\r\n]+/", ' ', $field_value);
                        }
