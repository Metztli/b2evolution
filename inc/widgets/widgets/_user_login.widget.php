...
class user_login_Widget extends ComponentWidget
{
    ...
    {
        if (is_logged_in()) {	// Don't display because user is already logged in:
            $this->display_debug_message('Widget "' . $this->get_name() . '" is hidden because user is already logged in.');
            return false;
        }

        if (get_param('disp') == 'login') {	// Don't display a duplicate form for inskin login mode:
        ...
        if (empty($redirect_to)) {	// Set redirect URL if it is not set in general settings:
            global $disp;
            if ($disp == 'access_requires_login' || $disp == 'content_requires_login') {	// Use a collection main page for disp "access_requires_login" and "content_requires_login":
                $redirect_to = $Blog->get('url');
            } else {	// Use a current page URL after log in action:
