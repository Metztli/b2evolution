...
class coll_current_filters_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        $r = array_merge([
            ...
            'show_filters' => [
                ...
                'options' => [
                    ['category', T_('Category'), 1],
                    ['archive', T_('Archive'), 1],
                    ['keyword', T_('Keyword'), 1],
                    ['tag', /* TRANS: noun */ T_('Tag'), 1],
                    ['author', T_('Author'), 1],
                    ['assignee', T_('Assignee'), 1],
                    ['locale', T_('Locale'), 1],
                    ['status', T_('Status'), 1],
                    ['itemtype', T_('Item Type'), 0],
                    ['visibility', T_('Visibility'), 0],
                    ['time', T_('Past/Future'), 0],
                    ['limit', T_('Limit by days'), 1],
                    ['flagged', T_('Flagged'), 1],
                    ['mustread', T_('Must read'), 1],
                ],
            ],
    ...
    public function display($params)
    {
        ...
        if (empty($params['ItemList'])) { // Empty ItemList object
        ...
        if (empty($filters)) { // No filters
            if ($params['display_empty_filter']) {
                if (is_admin_page() && get_param('tab') == 'type') { // Try to get a title for current selected post type on back-office pages:
                ...
                if (empty($current_post_type_title)) { // Use this title by default for unknown selected post type:
