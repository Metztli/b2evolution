...
class coll_media_index_Widget extends ComponentWidget
{
    ...
    public function init_display($params)
    {
        ...
        if ($this->disp_params['item_visibility'] == 'all') { // Don't cache the widget when we display the images of posts with ALL statuses
    ...
    public function display($params)
    {
        ...
        if (empty($blog_ID)) { // Use current blog by default
            $blog_ID = $Blog->ID;
        }

        $BlogCache = &get_BlogCache();
        if (! $BlogCache->get_by_ID($blog_ID, false, false)) {	// No collection exists
        ...
        if ($this->disp_params['item_visibility'] == 'public') { // Get images only of the public items
            $ItemQuery->where_visibility(['published']);
        } else { // Get image of all available posts for current user
        ...
        if (! empty($this->disp_params['item_type'])) { // Get items only with specified type
        ...
        $ItemQuery->ORDER_BY($ItemQuery->gen_order_clause(
            $this->disp_params['order_by'],
        ...
        while ($File = &$FileList->get_next()) {
            if ($count >= $this->disp_params['limit']) {	// We have enough images already!
                break;
            }
            ...
            if ($item_permanent_url !== false) {	// Some items may have no permanent url:
            ...
            if ($this->disp_params['disp_image_title']) { // Dislay title of image or item
        ...
        if (empty($r)) {	// Exit if no files found:
