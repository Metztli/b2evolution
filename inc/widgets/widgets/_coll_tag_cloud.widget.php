...
class coll_tag_cloud_Widget extends ComponentWidget
{
    ...
    public function load_from_Request()
    {
        ...
        // SPECIAL treatments:
        if (empty($this->param_array['tag_separator'])) {	// Default name, don't store:
    ...
    public function display($params)
    {
        ...
        if ($this->disp_params['blog_ids'] == '*') {
            ...
            if (empty($blog) && empty($blog_ids)) {	// Nothing to display
                $this->display_debug_message('Widget "' . $this->get_name() . '" is hidden because there are no selected collections.');
                return false;
            } elseif (empty($blog_ids)) {	// Use current Blog
                $blog_ids = $blog;
            }
        }

        // Destination:
        if ($destination_coll_ID = $this->disp_params['destination_coll_ID']) {	// Get destination Collection, but allow error, in that case we'll get NULL
        ...
        if ($this->disp_params['tag_ordering'] == 'ASC') {
            usort($results, [$this, 'tag_cloud_cmp']);
        ...
        foreach ($results as $row) {
            ...
            if ($count_span === 0) { // edge case where there is only a single tag or all tags have the same count
            ...
            } else {	// Automatic destination decision. Note: this may not be be best decision.
                $l_Blog = $BlogCache->get_by_ID($row->cat_blog_ID);
            }

            echo $l_Blog->get_tag_link($row->tag_name, $tag_name_disp, [
    ...
    public function tag_cloud_cmp($a, $b)
    {
