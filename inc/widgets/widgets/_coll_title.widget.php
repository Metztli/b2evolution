...
class coll_title_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        $r = array_merge([
            'add_title_link' => [
                ...
                'defaultvalue' => 'auto',
                'options' => [
                    ['auto', T_('Automatically (only when not already on the collection frontpage)')],
                    ['always', T_('Always')],
                    ['never', T_('Never')]],
                'defaultvalue' => 'auto',
                'field_lines' => true,
    ...
    public function display($params)
    {
        ...
        if ($linked_title == 'always' || ($linked_title == 'auto' && ! $is_front)) { // Add a link to the collection in the title
            $title = '<a href="' . $Blog->get('url') . '">' . $title . '</a>';
        }
        if ($this->disp_params['add_tagline']) { // Add a tagline after blog title
