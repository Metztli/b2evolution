...
class basic_menu_link_Widget extends generic_menu_link_Widget
{
    ...
    public function get_short_desc()
    {
        ...
        if (! empty($this->param_array['link_text'])) {	// We have a custom link text:
            return $this->param_array['link_text'];
        }

        if (! empty($this->param_array['link_type'])) {	// TRANS: %s is the link type, e. g. "Blog home" or "Log in form"
    ...
    public function get_param_definitions($params)
    {
        ...
        $coll_id_is_disabled = in_array($current_link_type, ['ownercontact', 'owneruserinfo', 'myprofile', 'profile', 'avatar', 'messages', 'contacts']);
        ...
        $r = array_merge([
            ...
            'coll_logo_size' => [
                ...
                'defaultvalue' => '',
                'hide' => in_array($current_link_type, ['item', 'admin', 'url']),
            ],
            'profile_picture_size' => [
                ...
                'defaultvalue' => '',
                'hide' => ! in_array($current_link_type, ['logout', 'myprofile', 'visits', 'profile', 'avatar', 'useritems', 'usercomments']),
            ],
            'link_text' => [
                'label' => T_('Link text'),
            ...
            'show_badge' => [
                ...
                'type' => 'checkbox',
                'defaultvalue' => true,
                'hide' => ! in_array($current_link_type, ['messages', 'flagged']),
            ],
            'blog_ID' => [
            ...
            'cat_ID' => [
                ...
                'size' => 5,
                'defaultvalue' => '',
                'hide' => ! in_array($current_link_type, ['recentposts', 'postnew']),
            ],
            ...
            'show_to' => [
                ...
                'note' => '',
                'type' => 'radio',
                'options' => [['any', T_('All users')],
                    ['loggedin', T_('Logged in users')],
                    ['perms', T_('Users with permissions only')]],
                'defaultvalue' => 'perms',
                'field_lines' => true,
                'hide' => ! in_array($current_link_type, ['messages', 'contacts']),
            ],
            'highlight_current' => [
                ...
                'type' => 'radio',
                'options' => [
                    ['yes', T_('Highlight the current item (not cacheable)')],
                    ['no', T_('Do not try to highlight (cacheable)')],
                ],
                'defaultvalue' => 'yes',
                'field_lines' => true,
            ],
            ...
            'visibility' => [
                ...
                'type' => 'radio',
                'options' => [
                    ['always', T_('Always show (cacheable)')],
                    ['access', T_('Only show if access is allowed (not cacheable)')]],
                'defaultvalue' => 'always',
                'field_lines' => true,
            ],
    ...
    public function init_display($params)
    {
        ...
        if ($this->disp_params['highlight_current'] == 'yes' ||
            $this->disp_params['visibility'] == 'access') {	// Disable block caching for this widget when it highlights the selected items or show only for users with access to collection:
    ...
    public function display($params)
    {
        ...
        if (isset($this->disp_params['link_text_' . $this->disp_params['link_type']])) {	// Use custom link text per type from skin side:
        ...
        if (! ($entry_Blog = &$SiteMenuEntry->get_Blog())) {	// We cannot use this widget without a current collection:
            $this->display_debug_message();
            return false;
        }

        if (! ($url = $SiteMenuEntry->get_url())) {	// Don't display this menu entry because of some restriction for current User or by general settings:
        ...
        switch ($this->disp_params['link_type']) {
            case 'recentposts':
                if (is_same_url($url, $Blog->get('url'))) { // This menu item has the same url as front page of blog
                    ...
                    if (! empty($Widget_array)) {
                        foreach ($Widget_array as $Widget) {
                            ...
                            if (isset($Widget->param_array, $Widget->param_array['link_type']) && $Widget->param_array['link_type'] == 'home') {	// Don't display this menu if 'Blog home' menu item exists with the same url:
                                $this->display_debug_message();
                                return false;
                            }
                        }
            ...
            case 'ownercontact':
                if ($entry_Blog->get_setting('msgform_nofollowto')) {	// Use nofollow attribute:
            ...
            case 'login':
            case 'register':
                if (isset($this->BlockCache)) { // Do NOT cache because some of these links are using a redirect_to param, which makes it page dependent.
            ...
            case 'messages':
            case 'flagged':
                if ($SiteMenuEntry->get('show_badge') && isset($this->BlockCache)) {	// Do not cache if bage is displayed because the number of unread messages are always changing:
                    $this->BlockCache->abort_collect();
                }
                break;
        }
