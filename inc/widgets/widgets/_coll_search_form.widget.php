...
class coll_search_form_Widget extends ComponentWidget
{
    ...
    {
        if (is_logged_in()) {	// Load JS to edit tags if it is enabled by widget setting and current User has a permission to edit them:
    ...
    public function display($params)
    {
        ...
        if ($blog_ID > 0) {	// Get Blog for widget setting
            $BlogCache = &get_BlogCache();
            $widget_Blog = &$BlogCache->get_by_ID($blog_ID, false, false);
        }
        if (empty($widget_Blog)) {	// Use current blog
            global $Collection, $Blog;
            $widget_Blog = &$Blog;
        }

        if (! $widget_Blog->get_setting('search_enable')) {	// A search page for widget's collection is disabled:
            $coll_name_link = '<a href="' . $widget_Blog->get('url') . '">' . $widget_Blog->get('name') . '</a>';
            $coll_setting_links = '';
            if (check_user_perm('blog_properties', 'edit', false, $widget_Blog->ID)) {	// Display a link to edit collection search setting:
        ...
        if (! empty($search_form)) {
            ...
            if (empty($this->disp_params['search_class'])) {	// Class name is not defined, Use class depend on serach options
                $search_form_class = 'search_form';
            } else {	// Use class from params
