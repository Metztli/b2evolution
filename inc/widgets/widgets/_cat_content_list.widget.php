...
class cat_content_list_Widget extends ComponentWidget
{
    ...
    public function display($params)
    {
        ...
        // Get current Chapter:
        $ChapterCache = &get_ChapterCache();
        if (! empty($cat) && ! ($curr_Chapter = &$ChapterCache->get_by_ID($cat, false, false))) {	// Display error if no cat is found by current cat ID:
        ...
        if (! empty($this->disp_params['subcat_template']) &&
            ! ($cat_Template = &$TemplateCache->get_by_code($this->disp_params['subcat_template'], false, false))) {	// Display error when no or wrong template for listing a category:
        ...
        if (! empty($this->disp_params['item_template']) &&
            ! ($item_Template = &$TemplateCache->get_by_code($this->disp_params['item_template'], false, false))) {	// Display error when no or wrong template for listing a category:
        ...
        $exclude_cats = (empty($this->disp_params['exclude_cats']) ? [] : $this->disp_params['exclude_cats']);
        //pre_dump( $exclude_cats );

        if (empty($curr_Chapter)) {	// Display all ROOT categories of the current Collection:
            global $Blog;
            $ChapterCache->clear();
            $SQL = $ChapterCache->get_SQL_object();
            $SQL->WHERE('cat_blog_ID = ' . $Blog->ID);
            $SQL->WHERE_and('cat_parent_ID IS NULL');
            if (! empty($exclude_cats)) {	// Exclude categories:
                $SQL->WHERE_and('cat_ID NOT IN ( ' . $DB->quote($exclude_cats) . ' )');
            }
            if ($Blog->get_setting('category_ordering') == 'manual') {	// Manual order
            ...
            foreach ($ChapterCache->cache as $Chapter) {
                if (in_array($Chapter->ID, $exclude_cats)) {	// Skip excluded category:
                    continue;
                }
                $this->display_subcat_template($Chapter, 0, $this->disp_params);
            }
        } else {	// Display CHILD categories and posts of the current Category:
            $callbacks = [
                'line' => [$this, 'display_subcat_template'],
                'posts' => [$this, 'display_item_template'],
            ];
            $ChapterCache->iterate_through_category_children($curr_Chapter, $callbacks, false, array_merge($this->disp_params, [
                'sorted' => true,
     ...
     * @param array Params
     */
    public function display_subcat_template($param_Chapter, $level, $params = [])
    {
        if (empty($params['subcat_template'])) {	// No template is provided for listing a category:
            return;
        }

        // Render Chapter by quick template:
        echo render_template_code($params['subcat_template'], $params, [
     ...
     * @param object Item
     */
    public function display_item_template($param_Item, $level, $params = [])
    {
        global $Item;

        if (empty($params['item_template'])) {	// No template is provided for listing an item:
            return;
        }
        ...
        if ($highlight_current_item) {	// Get template for currently active Item:
            ...
            if ($active_item_template != $item_template &&
                ($active_item_Template = &$TemplateCache->get_by_code($active_item_template, false, false))) {	// Use active item template when it is different and found in DB:
                $item_template = $active_item_template;
            }
        } elseif ($param_Item->main_cat_ID != $params['chapter_ID']) {	// Get template for crossposted Item:
            ...
            if ($crossposted_item_template != $item_template &&
                ($crossposted_item_Template = &$TemplateCache->get_by_code($crossposted_item_template, false, false))) {	// Use crossposted item template when it is different and found in DB:
                $item_template = $crossposted_item_template;
            }
        }

        $item_template_params = array_merge($params, [
        ...
        echo render_template_code($item_template, $item_template_params, [
            'Item' => $param_Item,
        ]);
    }
}
