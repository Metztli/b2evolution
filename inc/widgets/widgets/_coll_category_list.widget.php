...
class coll_category_list_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        if (isset($r['allow_blockcache'])) { // Disable "allow blockcache" because this widget uses the selected items
    ...
    public function display($params)
    {
        ...
        if ($this->disp_params['use_form']) {	// We want a complete form:
        ...
        if ($depth_level > 0) { // Don't limit if depth is null
            $depth_level += $start_level > 1 ? $start_level - 1 : 0;
        }

        $aggregate_coll_IDs = $Blog->get_setting('aggregate_coll_IDs');
        if (empty($aggregate_coll_IDs)) { // ____________________ We want to display cats for ONE blog ____________________
            $tmp_disp = '';

            if ($this->disp_params['option_all'] && intval($this->disp_params['start_level']) < 2) { // We want to display a link to all cats:
                $tmp_disp .= $this->add_cat_class_attr($this->disp_params['item_start'], 'evo_cat_all');
                $tmp_disp .= '<a href="';
                if ($this->disp_params['link_type'] == 'context') {	// We want to preserve current browsing context:
            ...
            foreach ($coll_ID_array as $c => $curr_blog_ID) {
                if ($max_colls > 0 && $max_colls <= $c) {	// Limit by max collections number:
                    break;
                }

                $loop_Blog = &$BlogCache->get_by_ID($curr_blog_ID, false);
                if (empty($loop_Blog)) {	// That one doesn't exist (any more?)
                    continue;
                }
        ...
        if ($this->disp_params['use_form'] || $this->disp_params['display_checkboxes']) { // We want to add form fields:
            if ($cat_modifier == '-' || (empty($cat_modifier) && $this->disp_params['default_match'] == 'nor')) { // Select NOR
                $cat_modifier_selected = 'nor';
            } elseif ($cat_modifier == '*' || (empty($cat_modifier) && $this->disp_params['default_match'] == 'and')) { // Select AND
		...
		<div class="multiple_cat_match_options">
			...
			<div class="tile">
				<input type="radio" name="cat" value="|" id="cat_or" class="radio"<?php if ($cat_modifier_selected == 'or') {
				    echo ' checked="checked"';
				} ?> />
				<label for="cat_or"><?php echo T_('Any selected category (OR)'); ?></label>
			</div>
			<div class="tile">
				<input type="radio" name="cat" value="-" id="cat_nor" class="radio"<?php if ($cat_modifier_selected == 'nor') {
				    echo ' checked="checked"';
				} ?> />
				<label for="cat_nor"><?php echo T_('None of the selected categories (NOR)'); ?></label>
			</div>
			<div class="tile">
				<input type="radio" name="cat" value="*" id="cat_and" class="radio"<?php if ($cat_modifier_selected == 'and') {
				    echo ' checked="checked"';
				} ?> />
     ...
     * @return string HTML
     */
    public function cat_line($Chapter, $level)
    {
        global $cat_array;
        ...
        if (in_array($Chapter->get('parent_ID'), $this->excluded_cat_IDs)) {	// Exclude also all child categories if parent category is excluded:
            $this->excluded_cat_IDs[] = $Chapter->ID;
        }

        if (in_array($Chapter->ID, $this->excluded_cat_IDs)) {	// Category is excluded, Skip it:
            return;
        }
        ...
        if (! isset($this->disp_params['current_parents'])) { // Try to find the parent categories in order to select it because of widget setting is enabled
            $this->disp_params['current_all_cats'] = []; // All children of the root parent of the selcted category
            $this->disp_params['current_parents'] = []; // All parents of the selected category
            ...
            if ($first_selected_cat_ID > 0) {
                ...
                while ($parent_Chapter !== null) { // Go up to the first/root category
        ...
        if ($start_level > 1 &&
            ...
            return '';
        } elseif (! isset($this->disp_params['current_cat_level'])) { // Save level of the current selected category
        ...
        if ( // First category that should be selected:
            ...
            ($this->disp_params['mark_parents'] && $Chapter->ID != $first_selected_cat_ID && in_array($Chapter->ID, $this->disp_params['current_parents']))) { // This category should be selected
        ...
        if (empty($Chapter->children)) {	// Add class name "evo_cat_leaf" for categories without children:
            $start_tag = $this->add_cat_class_attr($start_tag, 'evo_cat_leaf');
        } else {	// Add class name "evo_cat_node" for categories with children:
            $start_tag = $this->add_cat_class_attr($start_tag, 'evo_cat_node');
        }

        if ($Chapter->meta) {	// Add class name "evo_cat_meta" for meta categories:
        ...
        if ($this->disp_params['use_form'] || $this->disp_params['display_checkboxes']) { // We want to add form fields:
            $cat_checkbox_params = '';
            if ($Chapter->meta) { // Disable the checkbox of meta category ( and hide it by css )
                $cat_checkbox_params = ' disabled="disabled"';
            }

            $r .= '<label><input type="checkbox" name="catsel[]" value="' . $Chapter->ID . '" class="checkbox middle"';
            if (in_array($Chapter->ID, $cat_array)) { // This category is in the current selection
                $r .= ' checked="checked"';
            }
        ...
        if ($Chapter->lock && isset($this->disp_params['show_locked']) && $this->disp_params['show_locked']) {
            $cat_name .= '<span style="padding:0 5px;" >' . get_icon('file_not_allowed', 'imgtag', [
                'title' => T_('Locked'),
            ]) . '</span>';
        }
    ...
    {
        $start_level = intval($this->disp_params['start_level']);
        if ($start_level > 1 && $this->disp_params['current_cat_level'] >= $level) { // Don't show a start of group because of level restriction
            return;
        }

        if ($level > 0) { // If this is not the root:
     ...
     */
    public function add_cat_class_attr($start_tag, $class_name)
    {
        if (preg_match('/ class="[^"]*"/i', $start_tag)) { // Append to already existing attribute
            return preg_replace('/ class="([^"]*)"/i', ' class="$1 ' . $class_name . '"', $start_tag);
        } else { // Add new attribute for meta class
     ...
     */
    public function load_category_children($cat_ID, &$cats, $allowed_parents = [])
    {
        global $DB;
        ...
        foreach ($category_children as $category_child_ID) {
            ...
            if (in_array($category_child_ID, $allowed_parents)) { // Load children if it is not restricted by level depth
