...
class display_item_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        $r = array_merge([
            ...
            'title_display' => [
                ...
                'type' => 'radio',
                'options' => [
                    ['item_title', T_('Item Title')],
                    ['custom_title', T_('Custom Title')],
                ],
    ...
    public function init_display($params)
    {
        ...
        if (! $widget_Item) {	// No correct Item:
            return;
        }

        if (! in_array($widget_Item->get('status'), get_inskin_statuses($Blog->ID, 'post'))) {	// Disable block caching for this widget because target Item is not public for current collection:
    ...
    public function display($params)
    {
        ...
        if (! $widget_Item || ! in_array($widget_Item->get_type_setting('usage'), ['post', 'page'])) {	// Item is not found by ID and slug or it is not a post or page:
            if ($widget_Item) {	// It is not a post or page:
    ...
    public function &get_widget_Item()
    {
        ...
        if (! ($widget_Item = &$ItemCache->get_by_ID($this->disp_params['item_ID'], false, false))) {	// Try to get item by slug if it is not found by ID:
