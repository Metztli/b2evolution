...
class item_workflow_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        $r = array_merge([
            ...
            'show_properties' => [
                ...
                'options' => [
                    ['status', T_('Task status'), 1],
                    ['user', T_('Assigned user'), 1],
                    ['priority', T_('Priority'), 1],
                    ['deadline', T_('Deadline'), 1],
                ],
            ],
            'allow_edit' => [
    ...
    public function display($params)
    {
        ...
        if (! $Item->get_coll_setting('use_workflow')) {	// Workflow is disabled for current Collection:
            $this->display_debug_message('Widget "' . $this->get_name() . '" is hidden because workflow is disabled for Item\'s Collection.');
            return false;
        }

        if (! check_user_perm('blog_can_be_assignee', 'edit', false, $Item->get_blog_ID())) {	// Current User has no permission to be assigned for tasks of the Item's Collection:
        ...
        if (! empty($this->disp_params['show_properties']['status'])) {	// Display task status:
            if ($allow_edit) {
        ...
        if (! empty($this->disp_params['show_properties']['user'])) {	// Display assigned user:
            $UserCache = &get_UserCache();
            $assigned_User = &$UserCache->get_by_ID($Item->get('assigned_user_ID'), false, false);
            $this->display_workflow_property(T_('Assigned to'), ($assigned_User ? $assigned_User->get_identity_link([
                'display_bubbletip' => false,
            ]) : T_('No user')));
        }

        if (! empty($this->disp_params['show_properties']['priority'])) {	// Display priority:
            if ($allow_edit) {
        ...
        if (! empty($this->disp_params['show_properties']['deadline'])) {	// Display deadline:
            if ($allow_edit) {
     ...
     */
    public function display_workflow_property($title, $value)
    {
        echo str_replace(
            ['$title$', '$workflow_property_value$'],
            [$title, $value],
