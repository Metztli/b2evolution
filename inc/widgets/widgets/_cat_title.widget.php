...
class cat_title_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        $r = array_merge([
            ...
            'display_when' => [
                ...
                'field_lines' => true,
                'options' => [
                    ['browse_cat', T_('Any time we are browsing a category')],
                    ['no_intro', T_('Only if the category has no intro post')],
                ],
            ...
            'display_buttons' => [
                ...
                'type' => 'checklist',
                'options' => [
                    ['edit_cat', T_('Button to edit category'), 1],
                    ['add_intro', T_('Button to add an intro post'), 1],
                ],
            ],
    ...
    public function display($params)
    {
        ...
        if (empty($cat)) {	// Display error when no cat is found:
            ...
            if ($this->disp_params['display_when'] == 'browse_cat' || empty($intro_Item) || $intro_Item->get('title') == '') {	// Display chapter title only if intro post has no title
                $ChapterCache = &get_ChapterCache();
                // Load blog's categories
                $ChapterCache->reveal_children(empty($Blog) ? null : $Blog->ID);
                if (! ($curr_Chapter = &$ChapterCache->get_by_ID($cat, false))) {	// Display error when no cat is found by requested ID:
                ...
                if (! empty($this->disp_params['display_buttons']['edit_cat']) || ! empty($this->disp_params['display_buttons']['add_intro'])) {
                    ...
                    if (! empty($this->disp_params['display_buttons']['edit_cat'])) {	// Display button to edit category:
                        echo $curr_Chapter->get_edit_link([
                    ...
                    if (! empty($this->disp_params['display_buttons']['add_intro'])) {	// Display button to create a new page:
                        $write_new_intro_url = $Blog->get_write_item_url($cat, '', '', 'intro-cat');
                        if (! empty($write_new_intro_url)) {	// Display button to write a new intro:
