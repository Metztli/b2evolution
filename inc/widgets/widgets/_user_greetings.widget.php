...
class user_greetings_Widget extends ComponentWidget
{
    ...
    {
        if (! is_logged_in()) {	// Don't display because user is not logged in yet:
        ...
        if ($this->get_param('profile_picture_size') != '') {	// Display profile picture
            echo $current_User->get_avatar_imgtag($this->disp_params['profile_picture_size'], 'avatar', 'middle');
        }

        if ($this->get_param('greeting_show')) {	// Display greeting text
            $user_login = $current_User->get_identity_link([
                'link_text' => 'name',
        ...
        if ($this->get_param('group_show')) {	// Display user group
        ...
        if ($this->get_param('level_show')) {	// Display user group
