...
class coll_item_list_sort_order_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        foreach ($sort_options as $sort_key => $sort_title) {
            $widget_sort_options[] = [$sort_key, $sort_title, 1];
        }
    ...
    public function request_required_files()
    {
        ...
        if (! empty($MainList) &&
            ...
            $this->get_param('display_mode') == 'dropdown') {	// Load JS to sort items list:
    ...
    public function display($params)
    {
        ...
        foreach ($allowed_orders as $order_key => $order_is_allowed) {
            if (! isset($sort_options[$order_key]) || ! $order_is_allowed) {	// Exclude disallowed order:
                unset($allowed_orders[$order_key]);
            }
        }

        if (empty($allowed_orders)) {	// No allowed sort ooptions
