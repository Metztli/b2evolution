...
class item_comment_form_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        if (isset($r['allow_blockcache'])) {	// Disable "allow blockcache" because item content may includes other items by inline tags like [inline:item-slug]:
    ...
    public function display($params)
    {
        ...
        $params = array_merge([
            'widget_item_comment_form_params' => [],
        ], $params);
        ...
        $widget_params = array_merge([
            ...
            'form_params' => [
                // Use to change structure of form, i.e. fieldstart, fieldend and etc.
        ...
        if ($widget_params['disp_comment_form'] && ($widget_params['comment_type'] == 'meta' && $Item->can_meta_comment() ||
                $Item->can_comment($widget_params['before_comment_error'], $widget_params['after_comment_error'], '#', $widget_params['comment_closed_text'], $section_title, $widget_params))) {	// We want to display the comments form and the item can be commented on:
            echo '<a name="' . ($widget_params['comment_type'] == 'meta' ? 'meta-comment-form' : 'comment-form') . '"></a>';

            echo $widget_params['before_comment_form'];

            // INIT/PREVIEW:
            if ($Comment = get_comment_from_session('preview', $widget_params['comment_type'])) {	// We have a comment to preview
                if ($Comment->item_ID == $Item->ID) {	// display PREVIEW:
                    ...
                    if (empty($Comment->in_reply_to_cmt_ID)) {	// Display the comment preview here only if this comment is not a reply, otherwise it was already displayed
                        ...
                        skin_include($widget_params['comment_template'], [
                            'Comment' => &$Comment,
                ...
                if (($Comment = get_comment_from_session('unsaved', $widget_params['comment_type'])) == null) {	// there is no saved Comment in Session
                    $Comment = new Comment();
                ...
                if ($quoted_comment_ID || $quoted_post_ID) {	// Quote for comment/post
                    ...
                    if (! empty($quoted_comment_ID) &&
                            ...
                            $widget_params['comment_type'] == $quoted_Comment->get('type')) {	// Allow comment quoting only for the same comment type form:
                    ...
                    } elseif (! empty($quoted_post_ID) && $widget_params['comment_type'] != 'meta') {	// Allow item quoting only for normal(not meta) comment type form:
            ...
            if ($widget_params['comment_type'] != 'meta') {	// Display a message before comment form:
            ...
            $Form->begin_form('evo_form evo_form__comment', '', [
            ...
            if (! is_logged_in(false)) {	// User is not logged in or not activated:
                ...
                $Form->email_input($dummy_fields['email'], $comment_author_email, 40, T_('Email'), [
            ...
            if (! $Comment->is_meta() && $Item->can_rate()) {	// Comment rating:
                ob_start();
            ...
            if (! empty($widget_params['policy_text'])) {	// We have a policy text to display
                $Form->info_field('', $widget_params['policy_text']);
            }

            // Workflow properties:
            if ($Comment->is_meta()) {	// Display workflow properties if current user can edit at least one workflow property:
                skin_include('_item_comment_workflow.inc.php', array_merge($params, [
                    'Form' => &$Form,
                    'Comment' => &$Comment,
                ]));
            }
            ...
            $Form->textarea_input($dummy_fields['content'], $comment_content, $widget_params['textarea_lines'], $widget_params['form_comment_text'], [
                'note' => $note,
                'cols' => 38,
            ...
            if ($Item->can_attach(false, $Comment->type)) {	// If current user has permission to attach files for the item:
                ...
                if (empty($Comment->temp_link_owner_ID)) {	// Set Comment temp_link_owner_ID:
            ...
            // Additional options:
            $comment_options = [];
            if (! is_logged_in(false)) {	// For anonymous or not activated user:
                // TODO: If we got info from cookies, Add a link called "Forget me now!" (without posting a comment).
                $comment_options[] = ['comment_cookies', 1, T_('Remember me'), $comment_cookies, false, '(' . T_('Set cookies so I don\'t need to fill out my details next time') . ')'];
                ...
                $comment_options[] = ['comment_allow_msgform', 1, T_('Allow message form'), $comment_allow_msgform, false, '(' . T_('Allow users to contact me through a message form -- Your email will <strong>not</strong> be revealed!') . ')', ($email_is_detected ? 'comment_recommended_option' : '')];
                if ($Blog->get_setting('allow_anon_subscriptions')) {	// If item anonymous subscriptions are allowed for current collection:
                    $comment_options[] = ['comment_anon_notify', 1, T_('Notify me of replies'), isset($comment_anon_notify) ? $comment_anon_notify : $Blog->get_setting('default_anon_comment_notify')];
                }
            } elseif ($widget_params['comment_type'] != 'meta' && $Blog->get_setting('allow_item_subscriptions')) {	// For registered user and normal(not meta) comment and if item subscriptions are allowed for current collection:
                $comment_options[] = ['comment_user_notify', 1, T_('Notify me of replies'), (isset($comment_user_notify) ? $comment_user_notify : 1)];
            }
            ...
            if ($Item->can_attach()) {	// Display attach file input field when JavaScript is disabled:
                echo '<noscript>';
                $Form->input_field([
                    'label' => T_('Attach files'),
                    'note' => $widget_params['comment_attach_info'],
                    'name' => 'uploadfile[]',
                    'type' => 'file',
                ]);
            ...
            if (! $Comment->is_meta()) {	// Display workflow properties for normal comment form here:
                skin_include('_item_comment_workflow.inc.php', array_merge($params, [
                    'Form' => &$Form,
                    'Comment' => &$Comment,
                ]));
            ...
            if ($Item->can_edit_workflow()) {	// Prepend info for the form submit button title to inform user about additional action when workflow properties are on the form:
            ...
            $Form->button_input([
                ...
                'value' => $widget_params['form_submit_text'],
                'tabindex' => 10,
            ]);

            if ($Item->can_attach()) {	// Don't display "/Add file" on the preview button if JS is enabled:
                expose_var_to_js('evo_comment_form_preview_button_config', evo_json_encode([
                    'button_value' => T_('Preview'),
                ]));
            }
