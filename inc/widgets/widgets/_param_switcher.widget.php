...
class param_switcher_Widget extends generic_menu_link_Widget
{
     ...
     * Constructor
     */
    public function __construct($db_row = null, $type = 'core', $code = 'param_switcher')
    {
    ...
    public function display($params)
    {
        ...
        if ($this->get_param('param_code') == '') {	// Param code must be defined:
        ...
        if (empty($buttons)) {	// No buttons to display:
     ...
     */
    public function display_switchable_tabs($buttons, $defaults = [])
    {
	...
	// Metztli IT 01-21-2024 for some reason $buttons becomes string(5) "Array" only PHP 8 when it should be iterable;
        ...
        foreach ($buttons as $button) {	// Display button:
            if ($param_value === $button['value']) {	// Active button by current param value:
                $button_is_active = true;
                    ...
                    $defaults[$this->get_param('param_code')] == $button['value']) {	// Active button by default with empty param:
                $button_is_active = true;
            ...
            if ($button_is_active) {	// Set active button value:
        ...
        if ($this->get_param('allow_switch_js')) {	// Initialize JS to allow switching by JavaScript:
