...
class breadcrumb_path_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        $r = array_merge([
            ...
            'start_with' => [
                ...
                'type' => 'select',
                'options' => [
                    'blog' => T_('Collection name'),
                    'cat' => T_('Root parent category'),
                ],
                'defaultvalue' => 'blog',
            ],
            'coll_logo_size' => [
    ...
    public function display($params)
    {
        ...
        if (! empty($this->disp_params['suffix_text'])) {	// Append custom breadcrumb item at the end:
            $breadcrumbs[] = [
        ...
        if (! empty($disp) && $disp == 'single') { // Include current post
            global $Item;
            if (! empty($Item)) {
                $breadcrumbs[] = [
                ...
                if (empty($chapter_ID)) {	// Use main category of the current Item:
                    $chapter_ID = $Item->get('main_cat_ID');
                }
            }
        }

        if (! empty($chapter_ID)) { // Include full path of the selected chapter
            ...
            do {	// Get all parent chapters:
                ...
                } else { // No parent chapter else, Stop here
                    break;
                }
            } while (! empty($chapter_ID));
        }

        if ($this->disp_params['start_with'] == 'blog') { // Include Blog name
            $breadcrumbs[] = [
                'title' => $Blog->get('name'),
        ...
        if (empty($breadcrumbs)) {	// Nothing to display
        ...
        if (! empty($this->disp_params['coll_logo_size']) &&
            ...
            $coll_logo_File = $Blog->get('collection_image')) {	// Display collection logo in the breadcrumb path:
        ...
        foreach ($breadcrumbs as $b => $breadcrumb) {
            if ($b == count($breadcrumbs) - 1) { // Last crumb is active
                if ($b === 0 && isset($breadcrumb_logo)) {	// Display logo:
            ...
            } else { // All other crumbs are not active
                if ($b === 0 && isset($breadcrumb_logo)) {	// Display logo:
