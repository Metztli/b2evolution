...
class subcontainer_row_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        for ($i = 1; $i <= 6; $i++) {	// 6 columns for widget containers:
        ...
        if (isset($r['allow_blockcache'])) {	// Disable "allow blockcache":
    ...
    public function dbupdate()
    {
        ...
        for ($i = 1; $i <= 6; $i++) {
            if ($this->get_param('column' . $i . '_container') == '!create_new') {	// This is a request to create new sub-container:
                $new_container_code = $this->create_auto_subcontainer(' Column ' . $i);
                if ($new_container_code === false) {	// Stop updating if some new container cannot be created:
                    $result = false;
                    break;
                }
    ...
    public function display($params)
    {
        ...
        for ($i = 1; $i <= 6; $i++) {
            if (empty($this->disp_params['column' . $i . '_container'])) {	// Skip column without selected container:
                continue;
            }
     ...
     * @param array Params
     */
    public function display_column_container($subcontainer_code, $params)
    {
        ...
        if (count($code_data) != 2 || ! in_array($code_data[0], ['coll', 'shared'])) {	// Invalid container code:
        ...
        if (! isset($displayed_subcontainers)) {	// Initialize the dispalyed subcontainers array at first usage:
            // Use this array to avoid embedded containers display in infinite loop
            $displayed_subcontainers = [];
        } elseif (in_array($subcontainer_code, $displayed_subcontainers)) {	// Do not try do display the same subcontainer which were already displayed to avoid infinite display:
        ...
        if (! empty($container_widgets)) {
            foreach ($container_widgets as $ComponentWidget) {	// Let the Widget display itself (with contextual params):
        ...
        } elseif (is_logged_in() && $Session->get('designer_mode_' . $Blog->ID)) {	// Display text for empty container on designer mode:
