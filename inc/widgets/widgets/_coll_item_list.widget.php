...
class coll_item_list_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        $r = [
            ...
            'featured' => [
                ...
                'options' => [
                    ['all', T_('All posts')],
                    ['featured', T_('Only featured')],
                    ['other', T_('Only NOT featured')],
                ],
                'defaultvalue' => 'all',
            ],
            'flagged' => [
            ...
            'follow_mainlist' => [
                ...
                'options' => [
                    ['no', T_('No')],
                    ['tags', T_('By any tag included in Main List (OR match)')],
                    ['tags_and', T_('By all tags included in Main List (AND match)')],
                    ['tags_order', T_('By priority to best match (OR match + ORDER BY highest number of matches)')],
                ],
                'defaultvalue' => 'no',
                'field_lines' => true,
            ],
            ...
            'item_group_by' => [
                ...
                'type' => 'radio',
                'options' => [['none', T_('None')],
                    ['chapter', T_('By category/chapter')]],
                'defaultvalue' => 'none',
            ],
        ];
        ...
        for ($order_index = 0; $order_index <= 2 /* The number of orderby fields - 1 */; $order_index++) {
            ...
            $r = array_merge($r, [
                ...
                'orderby' . $field_suffix . '_end_line' => [
                    'type' => 'end_line',
                ],
            ]);
        }
        ...
        $r = array_merge($r, [
            ...
            'disp_cat' => [
                ...
                'options' => [
                    ['no',   T_('No display')],
                    ['main', T_('Display main category')],
                    ['all',  T_('Display all categories')]],
                'defaultvalue' => 'no',
            ],
            'disp_title' => [
                'label' => T_('Titles'),
            ...
            'attached_pics' => [
                ...
                'options' => [
                    ['none', T_('None')],
                    ['category', T_('Display main category picture')],
                    ['first', T_('Display first post picture')],
                    ['all', T_('Display all post pictures')]],
                'defaultvalue' => 'none',
            ],
            'disp_first_image' => [
                ...
                'type' => 'radio',
                'options' => [
                    ['special', T_('Special placement before title')],
                    ['normal', T_('No special treatment (same as other pictures)')]],
                'defaultvalue' => 'normal',
            ],
            'max_pics' => [
        ...
        if (isset($r['allow_blockcache'])) {
            if ($this->get_param('highlight_current', 1)) {	// Disable "allow blockcache" because this widget uses the selected items:
     ...
     * @return string
     */
    public function get_order($return = 'field')
    {
        $result = '';
        ...
        switch ($return) {
            case 'field':
                ...
                if ($this->get_param('order_by_1') != '') {	// Append second order field:
                    $result .= ',' . $this->get_param('order_by_1');
                    if ($this->get_param('order_by_2') != '') {	// Append third order field:
            ...
            case 'dir':
                ...
                if ($this->get_param('order_by_1') != '' && $this->get_param('order_dir_1') != '') {	// Append second order direction
                    $result .= ',' . $this->get_param('order_dir_1');
                    if ($this->get_param('order_by_2') != '' && $this->get_param('order_dir_2') != '') {	// Append third order direction:
    ...
    public function init_display($params)
    {
        ...
        if ($this->disp_params['highlight_current']) {	// Disable block caching for this widget when it highlights the selected items:
    ...
    public function display($params)
    {
        ...
        if (empty($listBlog)) {	// Display error when wrong collection is requested by this widget:
        ...
        if ($this->disp_params['attached_pics'] != 'none' && // If "Display first image"
            ...
            if (isset($thumbnail_sizes[$this->disp_params['thumb_size']])) {	// Get thumbnail width & height from config:
        ...
        } else {	// no excerpts, use ItemListLight
        ...
        if ($this->disp_params['item_visibility'] == 'public') {	// Get only the public items
            $filters['visibility_array'] = ['published'];
        }
        ...
        if ($this->disp_params['item_type'] != '' &&
            $this->disp_params['item_type'] != '#' /* deprecated value, it was used as default value of ItemList filter */) {	// Not "default", restrict to a specific type (or '' for all)
            $filters['types'] = $this->disp_params['item_type'];
        }

        if (isset($this->disp_params['item_type_usage'])) {	// Not "default", restrict to a specific type usage (or '' for all):
            $filters['itemtype_usage'] = $this->disp_params['item_type_usage'];
        }

        if ($this->disp_params['featured'] == 'featured') {	// Restrict to featured Items:
            $filters['featured'] = true;
        } elseif ($this->disp_params['featured'] == 'other') {	// Restrict to NOT featured Items:
            $filters['featured'] = false;
        }

        if ($this->disp_params['flagged'] == 1) {	// Restrict to flagged Items:
            $filters['flagged'] = true;
        }

        if (strpos($this->disp_params['follow_mainlist'], 'tags') === 0) {	// Restrict to Item tagged with some or all tags used in the Mainlist:
            if (! isset($MainList)) {	// Nothing to follow, don't display anything
            ...
            if (empty($all_tags)) {	// Nothing to follow, don't display anything
            ...
            if ($this->disp_params['follow_mainlist'] == 'tags_and') {	// Filter posts which have all tags:
                $filters['tags_operator'] = 'AND';
            }
            // else 'OR' operator by default

            if ($this->disp_params['follow_mainlist'] == 'tags_order') {	// Order by highest number of matched tags:
            ...
            if (! empty($Item)) {	// Exclude current Item
        ...
        if ($this->disp_params['item_group_by'] == 'chapter') {	// Group by chapter:
        ...
        if ($items_list_result !== true) {	// Display error message:
            $this->display_error_message($items_list_result);
            return false;
        }

        if (empty($items_list_content)) {	// Nothing to display:
            return true;
        }

        if (! empty($this->disp_params['template'])) {	// For template mode we cannot know what will be displayed so no extra classes:
            echo $this->disp_params['block_start'];
        } else {	// Get extra classes depending on widget settings:
            $block_css_class = $this->get_widget_extra_class();
            if (empty($block_css_class)) {	// No extra class, Display default wrapper:
                echo $this->disp_params['block_start'];
            } else {	// Append extra classes for widget block:
                echo update_html_tag_attribs($this->disp_params['block_start'], [
                    'class' => $block_css_class,
                ]);
            }
        }
     ...
     * @return string
     */
    public function get_widget_extra_class()
    {
        ...
        if (! $this->disp_params['disp_title'] && in_array($this->disp_params['attached_pics'], ['first', 'all'])) {	// Don't display bullets when we show only the pictures:
            $block_css_class .= ' nobullets';
        }

        if ($this->disp_params['item_group_by'] == 'chapter') {	// If items are grouped by category/chapter:
            $block_css_class .= ' evo_withgroup';
        } else {	// No grouping:
            $block_css_class .= ' evo_nogroup';
        }

        if ($this->disp_params['disp_title']) {	// If item title is displayed:
            $block_css_class .= ' evo_withtitle';
        } else {	// Item title is hidden:
            $block_css_class .= ' evo_notitle';
        }

        if ($this->disp_params['attached_pics'] == 'first') {	// If only first picture is displayed:
            $block_css_class .= ' evo_1pic';
        } elseif ($this->disp_params['attached_pics'] == 'all') {	// If all item pictures are displayed:
            $block_css_class .= ' evo_pics';
        } else {	// Item pictures are hidden:
            $block_css_class .= ' evo_nopic';
        }

        if ($this->disp_params['attached_pics'] != 'none') {	// If at least one picture should be displayed:
            if ($this->disp_params['disp_first_image'] == 'special') {	// Special placement for first image:
                $block_css_class .= ' evo_1pic__special';
            } else {	// Normal placement for first image:
        ...
        if ($this->disp_params['disp_excerpt']) {	// If item excerpt is displayed:
            $block_css_class .= ' evo_withexcerpt';
        } else {	// Item excerpt is hidden:
            $block_css_class .= ' evo_noexcerpt';
        }

        if ($this->disp_params['disp_teaser']) {	// If item teaser is displayed:
            $block_css_class .= ' evo_withteaser';
        } else {	// Item teaser is hidden:
