...
class coll_subscription_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        $r = array_merge([
            ...
            'display_mode' => [
                ...
                'field_lines' => true,
                'options' => [
                    ['checkboxes', T_('show options using checkboxes')],
                    ['text', T_('show options using text')],
                ],
                'defaultvalue' => 'text',
            ],
    ...
    public function display($params)
    {
        ...
        if ($current_User && ($allow_subscriptions || $allow_comment_subscriptions)) {	// If subscriptions are allowed on current collection:
            ...
            switch ($this->disp_params['display_mode']) {
                case 'checkboxes':
                    ...
                    if ($allow_subscriptions) {
                        $options[] = ['cb_sub_items', 1, T_('All posts'), $items_subscribed, false];
                    }
                    if ($allow_comment_subscriptions) {
                        $options[] = ['cb_sub_comments', 1, T_('All comments'), $comments_subscribed, false];
                    }

                    $Form->checklist($options, 'subscriptions', null);
                    $Form->end_form([['submit', 'submit', $button_label, 'btn btn-primary']]);
                    ?>
					<script>
            ...
            return true;
        } else {	// If subscriptions are not allowed on current collection:
