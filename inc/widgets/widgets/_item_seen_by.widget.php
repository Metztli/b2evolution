...
class item_seen_by_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        $r = array_merge([
            ...
            'include' => [
                ...
                'options' => [
                    ['author', T_('Author'), 1],
                    ['commenters', T_('Commenters'), 0],
                    ['assignees', T_('Potential assignees'), 1],
                    ['members', T_('Collection members'), 0],
                ],
            ],
            'limit' => [
                'label' => T_('Limit to'),
    ...
    public function display($params)
    {
        ...
        if (empty($Blog) || ! $Blog->get_setting('track_unread_content')) {	// Don't display this widget if current collection doesn't track the unread content:
        ...
        if (! check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) {	// Don't display this widget if user is NOT logged in OR user has no permission to edit this Item:
        ...
        if (! empty($this->disp_params['include']['author'])) {	// Include author:
            $user_IDs[] = $Item->get('creator_user_ID');
        }

        if (! empty($this->disp_params['include']['commenters'])) {	// Include commenters:
        ...
        if (! empty($this->disp_params['include']['assignees'])) {	// Include potential assignees:
            $UserCache->clear();
            $UserCache->load_blogmembers($Blog->ID, $users_limit, false/*assignees*/);
            $user_IDs = array_merge($user_IDs, array_keys($UserCache->cache));
        }

        if (! empty($this->disp_params['include']['members'])) {	// Include collection members:
            $UserCache->clear();
            $UserCache->load_blogmembers($Blog->ID, $users_limit, true/*members*/);
            $user_IDs = array_merge($user_IDs, array_keys($UserCache->cache));
        }

        if (empty($user_IDs)) {	// Don't display this widget if no users:
        ...
        foreach ($user_IDs as $user_ID) {
            if (! isset($read_statuses[$user_ID])) {	// Append users that don't see the item at the end of list:
                if (count($read_statuses) == $users_limit) {	// Limit user statuses by max value for this widget:
                    break;
                }
        ...
        foreach ($read_statuses as $read_user_ID => $read_status) {
            if (! ($seen_post_User = &$UserCache->get_by_ID($read_user_ID, false, false))) {	// Skip unexisting user:
                continue;
            }

            if ($read_status == 'read') {	// The item was read by user completely:
                $status_icon = get_icon('bullet_green', 'imgtag', [
                    'title' => '',
                ]);
            } elseif ($read_status == 'updated') {	// The item was read by user but it has new modifications for reading again:
                $status_icon = get_icon('bullet_orange', 'imgtag', [
                    'title' => '',
                ]);
            } else {	// The item is not read by user:
                $status_icon = get_icon('bullet_brown', 'imgtag', [
                    'title' => '',
                ]);
            }

            // Display each user as login with colored status icon:
            $login_users[] = '<span class="nowrap">' . $status_icon . ' ' . $seen_post_User->get_identity_link([
                'link_text' => 'auto',
            ]) . '</span>';
        }
