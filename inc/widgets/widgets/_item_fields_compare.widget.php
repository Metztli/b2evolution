...
class item_fields_compare_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        for ($order_index = 0; $order_index <= 2; $order_index++) {	// Default order settings:
            ...
            $r = array_merge($r, [
                'order_begin_line' . $field_suffix => [
                    'type' => 'begin_line',
                    'label' => ($order_index == 0 ? T_('Default order') : ''),
                ],
                'orderby' . $field_suffix => [
                    'type' => 'select',
                    'options' => array_merge(
                        [
     ...
     */
    public function get_advanced_param_definitions()
    {
        return [
            'add_css' => [
                'type' => 'checklist',
    ...
    public function display($params)
    {
        ...
        if (empty($items)) {	// No items to compare:
        ...
        if (empty($custom_fields)) {	// No fields to compare:
        ...
        if ($this->disp_params['show_headers'] || $show_status !== false) {	// Display item column headers row if it is enabled by widget settings:
            ...
            foreach ($items as $i => $item_ID) {
                ...
                if ($this->disp_params['show_headers']) {	// Get permanent item link:
                    $item_title_link = $widget_Item->get_title([
                        'title_field' => 'short_title,title',
                    ]);
                    if ($this->disp_params['merge_headers']) {	// Get title text in order to compare on merging:
                        $item_title_text = $widget_Item->get_title([
                            'title_field' => 'short_title,title',
                            'link_type' => 'none',
                        ]);
                    }
                } else {	// Item title should not be displayed:
                    $item_title_link = '';
                    $item_title_text = '';
                }
                ...
                if ($show_status == 'always' ||
                    ...
                    ($show_status == 'not_public' && $widget_Item->get('status') != 'published')) {	// Get item status:
                    $item_status_badge = $widget_Item->get_format_status([
                        'template' => $this->disp_params['custom_fields_item_status_template'],
                    ]);
                    if ($this->disp_params['merge_headers']) {	// Get status text in order to compare on merging:
                        $item_status_text = $widget_Item->get('status');
                    }
                } else {	// Don't display item status:
                    $item_status_badge = '';
                    $item_status_text = '';
                }

                if ($this->disp_params['merge_headers']) {	// Check if previous header same as currect:
                    if (isset($prev_item_title_text, $prev_item_status_text) &&
                        $prev_item_title_text == $item_title_text &&
                        $prev_item_status_text == $item_status_text) {	// This is a duplicated header cell as before:
                    ...
                    } else {	// Don't skip different column header cell:
                ...
                if (empty($skip_duplicate_header_cell)) {	// Display header cell only if it not hidden on merging same headers:
                    $col_class = '';
                    if (! empty($this->disp_params['add_css']['col'])) {	// Add CSS class to identify columns with "comp_col_" + Item slug:
                        $col_class .= ' comp_col_' . $widget_Item->get('urltitle');
                    }
                    if (! empty($this->disp_params['add_css']['cat']) &&
                            ($widget_main_Chapter = &$widget_Item->get_main_Chapter())) {	// Add CSS class to identify column category with "comp_cat_" + main Cat slug:
                    ...
                    if (! empty($this->disp_params['display_condition']) &&
                        ($display_condition = $widget_Item->get_custom_field_value($this->disp_params['display_condition'])) != '') {	// Use a display condition for column of the Item:
            ...
            foreach ($table_header_cells as $table_header_cell) {	// Print out table header cells:
                $cell_params = [
        ...
        foreach ($custom_fields as $custom_field) {
            if ($custom_field['display_mode'] == 'normal') {	// Display a row of one compared field between all selected items:
            ...
            if (! empty($custom_field['repeat_fields'])) {	// Display the repeated fields after separator if it has them:
                foreach ($custom_field['repeat_fields'] as $repeat_field_name) {	// Display a row of the repeated custom field between all selected items:
                    $this->display_field_row_template($custom_fields[$repeat_field_name], $items, $this->disp_params);
                }
            }
        }

        if ($this->disp_params['edit_links'] && is_logged_in()) {	// Display buttons to edit the compared items if user has a permission:
            global $Item;
            $items_edit_links = [];
            $items_can_be_edited = false;
            foreach ($items as $item_ID) {
                if (! ($widget_Item = &$ItemCache->get_by_ID($item_ID, false, false))) {	// Skip wrong Item:
                    continue;
                }

                $item_edit_link_params = [];
                if (isset($Item) && ($Item instanceof Item) && $Item->ID == $item_ID && count($items) == 1) {	// Don't display an edit link when this is a page of currently displayed Item:
                    $item_edit_link_params['link'] = '';
                } else {	// Try to display an edit link depending on permissions of current User:
                    $item_edit_link = $widget_Item->get_edit_link([
                        'class' => $this->disp_params['custom_fields_edit_link_class'],
                    ]);
                    if ($item_edit_link === false) {	// The edit link is not available for current User:
                        $item_edit_link_params['link'] = '';
                    } else {	// The Item can be edited by current User:
            ...
            if ($items_can_be_edited) {	// Display the footer row with edit links if at least one compared item can be edited by current User:
                echo str_replace('$row_attrs$', '', $this->get_field_template('row_start'));

                echo $this->get_field_template('topleft_cell');

                foreach ($items_edit_links as $items_edit_link) {
                    $item_edit_cell_params = [
     ...
     * @return array
     */
    public function get_custom_fields(&$items)
    {
        global $Item;
        ...
        if ($fields_source == 'exclude' || $fields_source == 'include') {	// Check what fields should be excluded/included for this widget:
            $widget_fields = trim($this->disp_params['fields']);
            $widget_fields = empty($widget_fields) ? [] : preg_split('#[\s\n\r]+#', $widget_fields);
        }

        $all_custom_fields = [];
        foreach ($items as $i => $item_ID) {
            if (! $widget_Item = &$ItemCache->get_by_ID($item_ID, false, false)) {	// Skin wrong post:
                unset($items[$i]);
                continue;
            }

            if (! empty($Item) && $Item->ID == $widget_Item->ID && $Item->is_revision()) {	// Display data from requested revision of the Item:
            ...
            foreach ($search_custom_fields as $search_custom_field_key) {
                ...
                $field_options = '';
                if ($fields_source == 'include' && strpos($search_custom_field_key, '+') !== false) {	// Parse additional field options, e.g. separators may have names as 'separator+repeat', 'separator+fields', 'separator+repeat+fields':
                    $search_custom_field_options = explode('+', $search_custom_field_key, 2);
                    if (isset($search_custom_field_options[1])) {	// The field has additional options:
                ...
                if (! isset($all_custom_fields[$search_custom_field_name])) {	// Initialize array to keep fields in the requested order:
                    $all_custom_fields[$search_custom_field_name] = [];
                }
                if (! isset($item_custom_fields[$search_custom_field_name])) {	// Skip because the post has no this custom field:
                    continue;
                }
                $item_custom_field = $item_custom_fields[$search_custom_field_name];
                if (! $item_custom_field['public']) {	// Skip not public custom field:
                    continue;
                }
                if (isset($all_custom_fields[$search_custom_field_key]['display_mode']) &&
                    $all_custom_fields[$search_custom_field_key]['display_mode'] == 'repeat') {	// Reinitialize custom field with correct order if it was initialized before as repeat field of some separator above this custom field:
                    unset($all_custom_fields[$search_custom_field_key]);
                }
                if (empty($all_custom_fields[$search_custom_field_key])) {	// Initialize array to store items which really have this custom field:
                    $all_custom_fields[$search_custom_field_key] = $item_custom_field;
                    $all_custom_fields[$search_custom_field_key]['display_mode'] = 'normal';
                    $all_custom_fields[$search_custom_field_key]['items'] = [];
                }
                if (! in_array($item_ID, $all_custom_fields[$search_custom_field_key]['items'])) {	// Store ID of the post which has this custom field:
                    $all_custom_fields[$search_custom_field_key]['items'][] = $item_ID;
                }

                if ($item_custom_field['type'] == 'separator') {	// Initialize the repeat fields and fields under separator field until next separtor:
                    if (! empty($item_custom_field['format']) &&
                        ...
                        if ($separator_format[0] != 'repeat' || empty($separator_format[1])) {	// Skip wrong separator format:
                            continue;
                        }
                        $repeat_fields = explode(',', $separator_format[1]);
                    } else {	// The separator has no repeat fields:
                        $repeat_fields = [];
                    }
                    ...
                    if ($fields_source == 'include' &&
                        ...
                        foreach ($item_custom_fields as $ic_field_name => $ic_field) {
                            if ($ic_field_name == $search_custom_field_name) {	// We found the current separtor, set flag to use next fields:
                                $is_under_separator_field = true;
                                continue;
                            }
                            if ($is_under_separator_field) {	// This is a field under current separator:
                                if ($ic_field['type'] == 'separator') {	// Stop here because it is another separator:
                                    break;
                                }
                    ...
                    foreach ($repeat_fields as $r => $repeat_field_name) {
                        ...
                        if (! isset($item_custom_fields[$repeat_field_name]) ||
                            ! $item_custom_fields[$repeat_field_name]['public']) {	// Skip unknown or not public field:
                            unset($repeat_fields[$r]);
                            continue;
                        }
                        $repeat_fields[$r] = $repeat_field_name;
                        $item_custom_field = $item_custom_fields[$repeat_field_name];
                        if (empty($all_custom_fields[$repeat_field_name])) {	// Initialize array to store items which really have this custom field:
                            ...
                            $all_custom_fields[$repeat_field_name]['items'] = [];
                        }
                        if (! in_array($item_ID, $all_custom_fields[$repeat_field_name]['items'])) {	// Store ID of the post which has this custom field:
                            $all_custom_fields[$repeat_field_name]['items'][] = $item_ID;
                        }
                    }
                    if (! isset($all_custom_fields[$search_custom_field_key]['repeat_fields']) &&
                        ! empty($repeat_fields)) {	// Initialize array to store the repeat fields of the separator:
        ...
        if ($fields_source == 'all' || $fields_source == 'exclude') {	// Sort custom fields from all requested posts by custom field order:
            uasort($all_custom_fields, [$this, 'sort_custom_fields']);
        }

        if (empty($all_custom_fields)) {	// Don't display widget if all selected items have no custom fields:
            return false;
        }
        ...
        foreach ($all_custom_fields as $c => $custom_field) {
            if ($custom_field['type'] == 'separator') {	// Separator fields have no values:
                continue;
            }

            $all_custom_fields[$c]['is_different'] = false;
            $is_numeric_type = in_array($custom_field['type'], ['double', 'computed']);
            if ($is_numeric_type) {	// Compare only numeric fields:
                $all_custom_fields[$c]['highest_value'] = null;
                $all_custom_fields[$c]['lowest_value'] = null;
            }
            if (isset($this->disp_params['cell_colors']['diff']) &&
                $items_count != count($custom_field['items'])) {	// If some post has no field then it is a different:
            ...
            foreach ($custom_field['items'] as $item_ID) {
                ...
                if ($this_line_values_are_empty &&
                    ($custom_field_value !== null || $custom_field['type'] == 'separator')) {	// At least one field is not empty:
                ...
                if (isset($this->disp_params['cell_colors']['diff']) &&
                    ...
                    $i > 0) {	// Don't search differences in all fields if at least two fields are different:
                    switch ($custom_field['type']) {
                        case 'image':
                            ...
                            if ($prev_Link = &$LinkCache->get_by_ID($prev_custom_field_value, false, false) &&
                                    ...
                                    $cur_File = &$cur_Link->get_File()) {	// Compare hashes of the two files:
                                $is_different = $prev_File->get('hash') != $cur_File->get('hash');
                            } else {	// If at least one field has a wrong link ID then compare IDs:
                    ...
                    if ($is_different) {	// Mark this field is different:
                        $all_custom_fields[$c]['is_different'] = true;
                    }
                }
                $prev_custom_field_value = $custom_field_value;

                if ($is_numeric_type && is_numeric($custom_field_value)) {	// Compare only numeric values:
            ...
            if ($this_line_values_are_empty) {	// Don't display row/line of custom field if values from all compared items are empty and if it is required by widget setting to hide empty lines:
     ...
     */
    public function display_field_row_template($custom_field, $items, $params = [])
    {
        ...
        if (empty($custom_field['description'])) {	// The custom field has no description:
            $field_description_icon = '';
        } else {	// Display a description in tooltip of the help icon:
            $field_description_icon = ' ' . get_icon('help', 'imgtag', [
                'data-toggle' => 'tooltip',
        ...
        if (! empty($this->disp_params['add_css']['row'])) {	// Add CSS class to identify row with "comp_row_" + field code:
        ...
        // Custom field title:
        echo str_replace(
            ['$field_title$', '$cols_count$', '$field_description_icon$', '$header_cell_class$'],
        ...
        if ($custom_field['type'] != 'separator') {	// Separator fields have no values:
            ...
            foreach ($items as $item_ID) {
                ...
                if (in_array($item_ID, $custom_field['items'])) {	// Get a formatted value if post has this custom field:
                ...
                } else {	// This post has no this custom field:
                    $custom_field_value = '';
                ...
                if (isset($this->disp_params['cell_colors']['diff']) &&
                    (($custom_field['is_different'] && $custom_field['line_highlight'] == 'differences') ||
                      ($custom_field['line_highlight'] == 'always' && count($items) > 1))) {	// Mark the field value as different only when it is defined in the settings of the custom field:
                    $field_value_template = $this->get_field_template('value_difference_highlight', $custom_field['type']);
                }

                if (in_array($custom_field['type'], ['double', 'computed']) &&
                    is_numeric($custom_field_orig_value)) {	// Compare only numeric values:
                    if ($custom_field_orig_value === $custom_field['highest_value'] &&
                        $custom_field_orig_value !== $custom_field['lowest_value']) {	// Check if we should mark the highest field:
                        if (isset($this->disp_params['cell_colors']['green']) &&
                            $custom_field['green_highlight'] == 'highest') {	// The highest value must be marked as green:
                            $field_value_template = $this->get_field_template('value_green', $custom_field['type']);
                        } elseif (isset($this->disp_params['cell_colors']['red']) &&
                                $custom_field['red_highlight'] == 'highest') {	// The highest value must be marked as red:
                    ...
                    if ($custom_field_orig_value === $custom_field['lowest_value'] &&
                        $custom_field_orig_value !== $custom_field['highest_value']) {	// Check if we should mark the lowest field:
                        if (isset($this->disp_params['cell_colors']['green']) &&
                            $custom_field['green_highlight'] == 'lowest') {	// The lowest value must be marked as green:
                            $field_value_template = $this->get_field_template('value_green', $custom_field['type']);
                        } elseif (isset($this->disp_params['cell_colors']['red']) &&
                                $custom_field['red_highlight'] == 'lowest') {	// The lowest value must be marked as red:
                            $field_value_template = $this->get_field_template('value_red', $custom_field['type']);
                        }
                    }
                }

                if ($custom_field['merge']) {	// Check if previous field value same as currect:
                    if (isset($prev_field_value) && $prev_field_value == $custom_field_value) {	// This is a duplicated field value cell as before:
                    ...
                    } else {	// Don't skip different field value cell:
                ...
                if (empty($skip_duplicate_field_value)) {	// Display field value cell only if it not hidden on merging same values:
                    $table_row_cell = [
                        'template' => str_replace(['$data_cell_class$', '$field_value$'], [$cell_class, $custom_field_value], $field_value_template),
                        'cols' => 1,
                    ];
                    if (! empty($this->disp_params['display_condition']) &&
                        ($display_condition = $widget_Item->get_custom_field_value($this->disp_params['display_condition'])) != '') {	// Use a display condition for column of the Item:
            ...
            foreach ($table_row_cells as $table_row_cell) {	// Print out table field value cells:
                echo str_replace(
                    '$data_cell_attrs$',
    ...
    public function get_items_IDs()
    {
        ...
        if (empty($items) && $items != 'all') {	// No items to compare:
            return [];
        }
        ...
        if (is_array($items)) {	// Check item IDs which are loaded from URL param 'items=' or from specific widget settings list:
            foreach ($items as $i => $item_ID) {
                $item_ID = trim($item_ID);
                if (empty($item_ID)) {	// Remove wrong item ID:
                    unset($items[$i]);
                }
                if ($item_ID == '$this$') {	// Try to get a current post ID:
                    if (isset($Item) && $Item instanceof Item) {	// Use ID of current post:
                        $items[$i] = $Item->ID;
                    } else {	// Remove it because no current post:
                        unset($items[$i]);
                    }
                } elseif ($item_ID == '$parent$') {	// Try to get a parent post ID:
                    if (isset($Item) && $Item instanceof Item && $Item->get('parent_ID') > 0) {	// Use ID of parent post:
                        $items[$i] = $Item->get('parent_ID');
                    } else {	// Remove it because no parent post:
                        unset($items[$i]);
                    }
                }
            }
            ...
            foreach ($items as $i => $item_ID) {
                if (! is_number($item_ID)) {	// Try to get a post ID by slug:
                    if ($widget_Item = &$ItemCache->get_by_urltitle($item_ID, false, false)) {	// Use ID of post detected by slug:
                        $items[$i] = $widget_Item->ID;
                    } else {	// Remove it because cannot find post by slug:
                        unset($items[$i]);
                    }
                }
            }
        ...
        if (empty($Blog)) {	// Cannot use filter by ItemList below because current collection is not defined:
            return $items;
        }
        ...
        if ($this->disp_params['items_type'] == 'default') {	// Exclude items with types which are hidden by collection setting "Show post types":
            $filter_item_type = $Blog->get_setting('show_post_types') != '' ? '-' . $Blog->get_setting('show_post_types') : null;
        } else {	// Filter by selected Item Type:
        ...
        for ($order_index = 0; $order_index <= 2; $order_index++) {
            ...
            if ($widget_orderby == 'coll_default') {	// Use order from collection:
            ...
            } elseif (! empty($widget_orderby)) {	// Use order from widget settings:
        ...
        if (! empty($this->disp_params['restrict_cats'])) {	// Restrict by categories:
            $default_filters['cat_array'] = explode(',', $this->disp_params['restrict_cats']);
        }
        if (! empty($this->disp_params['restrict_tags'])) {	// Restrict by tags:
        ...
        if ($this->disp_params['allow_filter']) {	// Filter items from request:
        ...
        if (isset($orig_ordered_items) && count($items)) {	// Revert original orders of items:
            $fix_ordered_items = [];
     ...
     */
    public function get_field_template($template_name, $field_type = '')
    {
        // Convert field types to non-devs names:
        $field_type = ($field_type == 'double' ? 'numeric' : ($field_type == 'varchar' ? 'string' : $field_type));

        if (isset($this->disp_params['custom_fields_' . $field_type . '_' . $template_name])) {	// Use special template for current type if it is defined:
            return $this->disp_params['custom_fields_' . $field_type . '_' . $template_name];
        } elseif (isset($this->disp_params['custom_fields_' . $template_name])) {	// Use generic template for all types:
     ...
     */
    public function check_show_status($items)
    {
        switch ($this->disp_params['show_status']) {
            ...
            case 'differences':
                ...
                foreach ($items as $item_ID) {
                    ...
                    if (count($diff_item_statuses) > 1) {	// When we find second different status we can stop here:
                        return $this->disp_params['show_status'];
                    }
                }
                break;
            ...
            case 'not_public':
                ...
                foreach ($items as $item_ID) {
                    $widget_Item = &$ItemCache->get_by_ID($item_ID, false, false);
                    if ($widget_Item->get('status') != 'published') {	// We found first status which should be displayed, stop search others:
                        return $this->disp_params['show_status'];
                    }
                }
                break;
    ...
    public function get_cache_keys()
    {
        ...
        if ($this->disp_params['edit_links']) {	// When the edit links setting is enabled we should invalidate keys per user,
        ...
        foreach ($items as $item_ID) {
            ...
            if ($Item = &$ItemCache->get_by_ID($item_ID, false, false)) {	// Add cache key for item type of the compared item:
     ...
     */
    public function get_display_condition_attr($condition, $items = [])
    {
        if ($condition == '') {	// No display condition:
            return '';
        }
        ...
        $ItemCache = &get_ItemCache();
        foreach ($items as $item_ID) {
            if ($widget_Item = &$ItemCache->get_by_ID($item_ID, false, false)) {	// If Item is detected:
        ...
        foreach ($disp_conditions as $disp_condition) {
            ...
            $disp_condition_values = isset($disp_condition[1]) ? explode('|', $disp_condition[1]) : [''];
