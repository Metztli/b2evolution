...
class coll_featured_intro_Widget extends ComponentWidget
{
    ...
    public function display($params)
    {
        ...
        if ($Item = &get_featured_Item($disp, $this->disp_params['blog_ID'], true)) {	// We have a featured/intro post to display:
            $item_style = '';
            ...
            if (empty($item_style)) {	// No item style:
                echo $this->disp_params['featured_intro_before'];
            } else {	// Append item style to use cover as background:
                echo update_html_tag_attribs($this->disp_params['featured_intro_before'], [
                    'style' => $item_style,
                    'class' => 'evo_hasbgimg',
                ]);
            }
