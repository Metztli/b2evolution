...
class user_register_quick_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        $r = array_merge([
            ...
            'subscribe' => [
                ...
                'type' => 'checklist',
                'options' => [
                    ['post', T_('check to auto subscribe new user to current collection posts'), 1],
                    ['comment', T_('check to auto subscribe new user to current collection comments'), 1],
                ],
            ],
            'button' => [
    ...
    public function display($params)
    {
        ...
        if (is_logged_in()) {	// No display when user is already registered
        ...
        if ($Blog->get_ajax_form_enabled()) {	// Load widget form through AJAX:
            $widget_params = [
                'action' => 'get_widget_form',
                'blog' => $Blog->ID,
                'params' => $params,
            ];
            if (empty($this->ID)) {	// Use code for calling widget from content with inline short tag like [emailcapture]:
                $widget_params['wi_code'] = $this->get('code');
            } else {	// Use ID for calling widget from DB:
                $widget_params['wi_ID'] = $this->ID;
            }
            display_ajax_form($widget_params);
        } else {	// Display widget form:
     ...
     * @param array Params
     */
    public function display_form($params)
    {
        ...
        if (! empty($widget_param_input_err_messages)) { // Convert param errors to global $param_input_err_messages that is used to display an error text under input field
        ...
        if (trim($this->disp_params['intro']) !== '') { // Intro text
            echo '<p>' . $this->disp_params['intro'] . '</p>';
        }

        // Get current form display mode:
        $form_display_mode = $this->get_form_display('compact');
        $is_nolabels_mode = in_array($form_display_mode, ['nolabels', 'inline', 'grouped']);

        $Form = new Form(get_htsrv_url('login') . 'register.php', 'register_form', 'post');

        $Form->begin_form(null, '', [
            'class' => 'widget_register_form evo_widget_form__' . $form_display_mode,
        ]);

        $Plugins->trigger_event('DisplayRegisterFormBefore', [
            'Form' => &$Form,
            'inskin' => true,
            'Widget' => &$this,
        ]);
        ...
        if ($form_display_mode == 'grouped') {	// Start a group of all inputs and submit button in single line:
            $Form->begin_line();
            $Form->inputend = '';
            $Form->fieldend = '';
        }

        if ($this->disp_params['ask_firstname'] != 'no') { // First name
            ...
            if ($this->disp_params['ask_firstname'] == 'required') {	// Params if first name is required:
            ...
            if ($is_nolabels_mode) {	// Display placeholder only in mode when labels are hidden:
        ...
        if ($this->disp_params['ask_lastname'] != 'no') { // Last name
            ...
            if ($this->disp_params['ask_lastname'] == 'required') {	// Params if first name is required:
                // Set css class "field_required":
        ...
        if ($this->disp_params['ask_country'] != 'no' && empty($this->disp_params['hide_country_by_plugin'])) {	// Country
            ...
            $Form->select_country('country', $country_value, $CountryCache, ($is_nolabels_mode ? '' : T_('Country')), [
                'allow_none' => true,
        ...
        if ($form_display_mode == 'grouped') {	// End of the group of all inputs and submit button in single line:
            $Form->end_line();
        }
        ...
        if (! is_logged_in()) {	// JS code to get crumb from AJAX request when page caching is enabled:
            echo '<script>
...
jQuery( ".widget_register_form" ).submit( function()
{
	...
	return false;
        ...
        if (! empty($widget_param_input_err_messages)) { // Clear param errors here because we already display them above
