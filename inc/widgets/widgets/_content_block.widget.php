...
class content_block_Widget extends ComponentWidget
{
    ...
    public function get_desc_for_list()
    {
        ...
        if ($widget_Item = &$this->get_widget_Item()) {	// Display a title of widget Item if it is defined:
            $r .= ' (' . $widget_Item->dget('title') . ')';
        } elseif (! empty($short_desc)) {	// Display a widget name:
            $r .= ' (' . $this->get_name() . ')';
        }
    ...
    public function load_from_Request()
    {
        ...
        if (get_param($this->get_param_prefix() . 'item_ID') != '' && get_param($this->get_param_prefix() . 'item_slug') != '') {	// Don't allow both entered fields:
    ...
    public function init_display($params)
    {
        ...
        if ($this->get_param('select_type') == 'random') {	// Disable block caching for this widget when items are displayed randomly:
            $this->disp_params['allow_blockcache'] = 0;
        }

        $widget_Item = &$this->get_widget_Item();

        if ($widget_Item && ! in_array($widget_Item->get('status'), get_inskin_statuses($widget_Item->get_blog_ID(), 'post'))) {	// Disable block caching for this widget because target Item is not public for its collection:
            $this->disp_params['allow_blockcache'] = 0;
        }
    }
    ...
    public function display($params)
    {
        ...
        if (! empty($this->disp_params['template']) &&
            ! ($cat_Template = &$TemplateCache->get_by_code($this->disp_params['template'], false, false))) {	// Display error when no or wrong template:
        ...
        if (! $widget_Item && $this->get_param('select_type') == 'random') {	// If no item found ramdomly:
            $this->display_error_message(T_('No Item is found randomly.'));
            return false;
        }

        if (! $widget_Item || $widget_Item->get_type_setting('usage') != 'content-block') {	// Item is not found by ID and slug or it is not a content block:
            if ($widget_Item) {	// It is not a content block:
        ...
        if (! $widget_Item->can_be_displayed()) {	// Current user has no permission to view item with such status:
        ...
        if (isset($Item) && ($Item instanceof Item)) {	// 1. Content block Item has same owner as owner of the current Item:
        ...
        if (! $content_block_is_allowed) {	// Display error if the requested content block item cannot be used in this place:
            if (isset($Item) && ($Item instanceof Item)) {	// For page with current Item:
                $this->display_error_message(sprintf(
                    ...
                    $widget_Item->ID,
                    '<code>' . $widget_Item->get('urltitle') . '</code>', // Content block #%d %s
                    ...
                    $Item->ID,
                    get_user_identity_link(null, $Item->get('creator_user_ID')), // the current Item (Item #%d) (Owner: %s)
            ...
            } else {	// For page without current Item:
                $this->display_error_message(sprintf(
                    T_('Content block #%d %s (Coll #%d) (Owner: %s) cannot be included here. It must be in the same collection as this Widget (Coll #%d) or the info pages collection (Coll #%d)') . '. ' .
        ...
        echo $widget_Item->get_content_block(array_merge($params, [
     ...
     * @return object Item
     */
    public function &get_widget_Item()
    {
        if ($this->widget_Item === null) {	// Get widget Item once:
            ...
            switch ($this->get_param('select_type')) {
                case 'random':
                    ...
                    if (! ($widget_Blog = &$BlogCache->get_by_ID($this->get_param('coll_ID'), false, false))) {	// Use current Collection if not defined in widget settings:
                        global $Collection, $Blog;
                        $widget_Blog = $Blog;
                    }
                    ...
                    if (! empty($item_type_ID)) {	// Filter by Exact Item Type:
                ...
                default:
                    ...
                    if (! ($this->widget_Item = &$ItemCache->get_by_ID($this->disp_params['item_ID'], false, false))) {	// Try to get item by slug if it is not found by ID:
                        $this->widget_Item = &$ItemCache->get_by_urltitle(trim($this->disp_params['item_slug']), false, false);
                    }
            }

            if ($this->widget_Item === null) {	// Set false to don't call this twice:
