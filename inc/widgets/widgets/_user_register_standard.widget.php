...
class user_register_standard_Widget extends ComponentWidget
{
    ...
    public function display($params)
    {
        ...
        if (! is_null($params['register_form_params'])) {	// Use another template param from skin:
        ...
        if ($register_user = $Session->get('core.register_user')) {	// Get an user data from predefined session (after adding of a comment):
            ...
            $render_template_objects['register_user_data'] = [
                'login' => $login,
                'email' => $email,
            ];
        ...
        if (empty($register_form_Template)) {	// Display error when no or wrong template for the registration form:
        ...
        $required_fields = isset($this->disp_params['reg1_required']) ? array_map('trim', explode(',', $this->disp_params['reg1_required'])) : [];
        ...
        if (! empty($register_form)) {
            ...
            if (is_logged_in()) {	// Don't allow to register if a user is already logged in:
                echo '<p>' . T_('You are already logged in') . '</p>';
            } elseif ($invitation_code_status == 'deny') {	// If registration is disabled
                echo '<p class="error red">';
                if ($Settings->get('newusers_canregister') == 'no' || ! $Settings->get('system_lock')) {	// If registration is disabled:
                    echo T_('User registration is currently not allowed.');
                } else {	// If system is locked:
                    echo T_('The site is currently locked for maintenance.');
                }
                echo '</p>';

                if ($params['register_disp_home_button']) {	// Display button to go home:
                    global $baseurl;
                    echo '<p class="center"><a href="' . $baseurl . '" class="btn btn-default">' . T_('Home') . '</a></p>';
                }
            } else {	// Display a registration form:
                if ($params['register_disp_messages']) {	// Display the form messages before form inside wrapper:
                    messages([
                ...
                if (isset($Blog)) {	// For in-skin registration form:
                ...
                if (! empty($widget)) {	// Widget ID may be used on this form when user cannot be quickly registered because of suspected data:
                    $Form->hidden('widget', $widget);
                }

                if ($invitation_code_status == 'input') { // Display an input field to enter invitation code manually or to change incorrect code
                    $invitation_field_params = [
                        'maxlength' => 32,
                        'class' => 'input_text',
                        'style' => 'width:138px',
                    ];
                    if ($Settings->get('newusers_canregister') == 'invite') { // Invitation code must be required when users can register ONLY with this code
                ...
                } elseif ($invitation_code_status == 'info') { // Display info field (when invitation code is correct)
                ...
                // Plugin fields:
                $Plugins->trigger_event('DisplayRegisterFormFieldset', [
                    'Form' => &$Form,
                    'inskin' => true,
