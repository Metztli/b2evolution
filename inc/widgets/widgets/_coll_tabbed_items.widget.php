...
class coll_tabbed_items_Widget extends param_switcher_Widget
{
    ...
    public function display($params)
    {
        ...
        if (! isset($Item) ||
            ! $Item instanceof Item) {	// No current Item:
        ...
        if (! $Item->get_type_setting('allow_switchable') ||
            ! $Item->get_setting('switchable')) {	// Item doesn't use switcher params:
            $this->display_debug_message('Widget "' . $this->get_name() . '" is hidden because current Item does not use switcher params.');
            return false;
        }

        if ($this->get_param('param_code') == '') {	// Display error when param code is not defined:
        ...
        if ($Item->get_switchable_param($this->get_param('param_code')) === null) {	// No default value:
        ...
        $items_tabs = [];
        while ($row_Item = &$ItemList->get_item()) {	// Initialize tabs from items list:
            $items_tabs[] = [
                'value' => $row_Item->get('urltitle'),
                'text' => $row_Item->get_title([
        ...
        if (! in_array($disp, ['single', 'page', 'widget_page']) && isset($items_tabs[0])) {	// Use first item tab by default for not single Item pages:
            $default_tabs = [
                $this->get_param('param_code') => $items_tabs[0]['value'],
            ];
        } elseif (isset($Item) && $Item instanceof Item) {	// Use default params from current Item:
        ...
        // Get items list:
        ob_start();
        $items_list_result = $ItemList->display_list(array_merge([
