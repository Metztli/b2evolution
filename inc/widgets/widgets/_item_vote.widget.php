...
class item_vote_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        $r = array_merge([
            ...
            'note' => [
                ...
                'type' => 'info',
                'info' => sprintf(
                    T_('To configure what buttons to show (Positive/Neutral/Negative), go to Post Features &gt; <a %s>Voting Options</a>'),
            ...
            'display_summary' => [
                ...
                'options' => [
                    ['no', T_('No')],
                    ['replace', T_('Replace label after vote')],
                    ['always', T_('Always display after icons')]],
                'defaultvalue' => 'always',
            ],
            'display_summary_author' => [
    ...
    public function display($params)
    {
        ...
        if (empty($this->disp_params['Item'])) {	// Use current global Item:
            global $Item;
            $widget_Item = &$Item;
        } else {	// Use a specific Item:
     ...
     * @param array Params
     */
    public function display_voting_panel($Item, $params = [])
    {
        $this->init_display($params);

        // Display buttons to vote on item:
        $Item->display_voting_panel(array_merge([
