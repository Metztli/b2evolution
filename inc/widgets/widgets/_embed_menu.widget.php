...
class embed_menu_Widget extends generic_menu_link_Widget
{
    ...
    public function get_short_desc()
    {
        ...
        if (! empty($this->param_array['menu_ID'])) {	// TRANS: %s is the link type, e. g. "Blog home" or "Log in form"
            return T_('Menu') . ': ' . (
    ...
    public function display($params)
    {
        ...
        if (empty($menu_ID)) {	// We cannot use this widget because Menu is not defined:
            $this->display_error_message('Menu not defined');
            return false;
        }

        $SiteMenuCache = &get_SiteMenuCache();
        if (! ($SiteMenu = &$SiteMenuCache->get_by_ID($menu_ID, false, false))) {	// We cannot use this widget without Menu:
        ...
        if (empty($menu_entries)) {	// Don't display if menu has no entries:
        ...
        foreach ($menu_entries as $SiteMenuEntry) {
            if ($url = $SiteMenuEntry->get_url()) {	// Display a layout with menu link only if it is not restricted by some permission for current User:
