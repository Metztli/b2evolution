    ...
    public function get_param_definitions($params)
    {
        $r = array_merge([
            ...
            'image_source' => [
                ...
                'type' => 'radio',
                'options' => [
                    ['site_logo', T_('Site logo')],
                    ['skin', T_('Site skin folder')],
            ...
            'check_file' => [
                ...
                'field_lines' => true,
                'options' => [
                    ['none', T_('Don\'t check. Assume image file exists.')],
                    ['check', T_('Check -> if image doesn\'t exist, display nothing.')],
                    ['title', T_('Check -> if image doesn\'t exist, display the site title instead.')]],
                'defaultvalue' => 'title',
            ],
    ...
    public function display($params)
    {
        ...
        switch ($this->disp_params['image_source']) {
            case 'site_logo':
                ...
                if (! $File || ! $File->is_image()) {	// Get site logo image if the file exists in DB and it is an image:
                    $File = false;
                }
                break;

            case 'skin':
                if ($site_Skin = &get_site_Skin()) {	// If site skin is enabled and it is set:
        ...
        if ($check_file != 'title' && empty($image_url)) {	// If no image file:
        ...
        $image_attrs = '';
        if (! empty($this->disp_params['width'])) {	// Image width
            $image_attrs .= ' width="' . intval($this->disp_params['width']) . '"';
        }
        if (! empty($this->disp_params['height'])) {	// Image height
            $image_attrs .= ' height="' . intval($this->disp_params['height']) . '"';
        }

        if ($check_file == 'title' && empty($image_url)) {	// Logo file doesn't exist, Display a site title because widget setting requires this:
