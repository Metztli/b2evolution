...
class user_action_Widget extends ComponentWidget
{
    ...
    {
        $button_options = $this->get_param_definitions([]);
    ...
    public function display($params)
    {
        ...
        switch ($this->get_param('button')) {
            case 'edit_profile':
                // Edit my profile:
                if (is_logged_in() && $current_User->ID == $target_User->ID) {	// Only if the logged in user is viewing own profile:
            ...
            case 'send_message':
                // Send Message:
                if (! is_logged_in() || $current_User->ID != $target_User->ID) {	// Only if user is viewing other profile:
                    $msgform_url = $target_User->get_msgform_url($Blog->get('msgformurl'));
                    if (! empty($msgform_url)) {	// And user can send a message to the viewing user:
            ...
            case 'block_report':
                ...
                // Report User:
                if (is_logged_in() && ($current_User->ID != $target_User->ID) &&
                        check_user_status('can_report_user', $target_User->ID)) {	// Current user must be logged in, cannot report own account, and must has a permission to report:
        ...
        if (empty($r)) {	// The requested user action button is not allowed, Nothing to display:
            return true;
        }
