...
class user_profile_pics_Widget extends ComponentWidget
{
    ...
    public function get_param_definitions($params)
    {
        ...
        $r = array_merge([
            ...
            'link_to' => [
                ...
                'field_lines' => true,
                'options' => [
                    ['userpage', T_('User profile')],
                    ['fullsize', T_('Full-size profile picture (using colorbox)')]],
    ...
    public function display($params)
    {
        ...
        if ($this->get_param('display_main')) {	// Display main profile pictures:
            // Set overlay text:
            $thumb_overlay_text = '';
            if (! is_logged_in() && $this->get_param('anon_overlay_show')) {	// If it is enabled by widget setting:
                if (trim($this->get_param('anon_overlay_text')) != '') {	// Get overlay text from params:
                    $thumb_overlay_text = nl2br($this->get_param('anon_overlay_text'));
                } else {	// Get default overlay text from general settings:
                    global $Settings;
                    $thumb_overlay_text = $Settings->get('bubbletip_overlay');
                }
            }

            $profile_image_tag = $target_User->get_link([
            ...
            if (is_logged_in() && $target_User->ID == $current_User->ID && ! $target_User->has_avatar()) {	// If user hasn't an avatar, add a link to go for uploading of avatar:
        ...
        if ($this->get_param('display_other')) {	// Display additional pictures:
            if (check_user_status('can_view_user', $target_User->ID)) {	// Only for logged in and activated users
