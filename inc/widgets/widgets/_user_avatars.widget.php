...
class user_avatars_Widget extends ComponentWidget
{
    ...
    public function display($params)
    {
        ...
        $SQL->SELECT('*');
        $SQL->FROM('T_users');
        if ($this->disp_params['order_by'] == 'numposts') { // Highest number of posts
        ...
        if ($this->disp_params['style'] == 'simple') { //Display users with pictures
            $SQL->WHERE('user_avatar_file_ID IS NOT NULL');
        }
        $SQL->WHERE_and('user_status <> "closed"');
        if (is_logged_in()) { // Add filters
            global $current_User;
            switch ($this->disp_params['gender']) { // Filter by gender
                case 'same':
            ...
            switch ($this->disp_params['location']) { // Filter by location
                case 'city':
                ...
                case 'country':
                    ...
                    break;
                case 'closest':
                    if (! empty($current_User->city_ID)) { // Check if users exist with same city
                    ...
                    if (! empty($current_User->subrg_ID) && empty($user_exists)) { // Check if users exist with same sub-region
                        $user_exists = $DB->get_var('SELECT user_ID
							 FROM T_users
                    ...
                    if (! empty($current_User->rgn_ID) && empty($user_exists)) { // Check if users exist with same region
                        $user_exists = $DB->get_var('SELECT user_ID
							 FROM T_users
                    ...
                    if (! empty($current_User->ctry_ID) && empty($user_exists)) { // Check if users exist with same country
                        $user_exists = $DB->get_var('SELECT user_ID
							 FROM T_users
        ...
        switch ($this->disp_params['order_by']) {
            case 'opt_random':
                ...
                if ($user_nums->cnt == 0) {	// If no users for current filter:
                    $search_users_by_sql = false;
                } elseif ($user_nums->cnt <= $users_limit) {	// If filtered users number is less or equal than the requested limit:
                ...
                } else {	// If filtered users number is more than the requested limit:
                    $user_IDs = [];
                    while (count($user_IDs) < $users_limit) {
                        $random_user_ID = rand($user_nums->min_user_ID, $user_nums->max_user_ID);
                        if (! in_array($random_user_ID, $user_IDs)) {	// Use only new random user ID is not array yet:
                            ...
                            if ($DB->get_var($check_user_SQL)) {	// Add to array only when user is realted to current filter:
            ...
            case 'php_random':
                ...
                if ($filtered_user_IDs_num == 0) {	// If no users for current filter:
                    $search_users_by_sql = false;
                } else {	// If at least one user is found by widget filter:
                    if ($users_limit > $filtered_user_IDs_num) {	// If filtered users are less than max limit is required by widget setting:
                        $users_limit = $filtered_user_IDs_num;
                    }
                    $user_IDs = [];
                    while (count($user_IDs) < $users_limit) {
                        $random_user_ID = $filtered_user_IDs[rand(0, $filtered_user_IDs_num - 1)];
                        if (! in_array($random_user_ID, $user_IDs)) {	// Add only new random user ID is not array yet:
                            $user_IDs[] = $random_user_ID;
                        }
                    }
                    $SQL->WHERE('user_ID IN ( ' . implode(', ', $user_IDs) . ' )');
        ...
        if ($search_users_by_sql) {	// Search users by SQL query:
            $UserList->sql = $SQL->get();
            $UserList->run_query(false, false, false, 'Get users by filter of widget #' . $this->ID . ' "' . $this->get_name() . '"');

            $avatar_link_attrs = '';
            if ($this->disp_params['style'] == 'badges') {	// Remove borders of <td> elements:
            ...
            while ($User = &$UserList->get_next()) {
                ...
                if ($this->disp_params['bubbletip'] == '1') {	// Bubbletip is enabled
                ...
                if (! empty($identity_url)) {
                    ...
                    if ($this->disp_params['style'] != 'username') {	// Display only username:
                        $r .= $avatar_tag;
                    }

                    if ($this->disp_params['style'] == 'badges') {	// Add user login after picture:
                        $r .= '<br >' . $User->get_colored_login([
                            'login_text' => 'name',
                        ]);
                    } elseif ($this->disp_params['style'] == 'username') {	// Display username without <br>:
        ...
        if ($count == 0) {	// Exit if no users found:
