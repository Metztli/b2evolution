...
class item_checklist_lines_Widget extends ComponentWidget
{
    ...
    public function request_required_files()
    {
        ...
        if (! empty($Item) && $this->get_param('allow_edit') && $Item->can_meta_comment()) {	// Load JS to edit checklist lines if it is enabled by widget setting and current User has a permission to edit them:
    ...
    public function display($params)
    {
        ...
        if (! empty($params['Item'])) {	// Used Item provided by params:
            $Item = $params['Item'];
        } else {	// use global Item:
            global $Item;
        }
        ...
        if ($can_update) {
            ...
            $js_config = [
                ...
                'checklist_line_template' => '<div class="checkbox checklist_line">
                                                ...
                                                action_icon(T_('Delete'), 'delete', '#', null, null, null, [
                                                    'class' => 'checklist_line_delete',
                                                    'style' => 'visibility:hidden;',
                                                ]) .
                                            '</div>',
                'checklist_line_input_template' => $Form->textarea_input('$checklist_line_ID$', '$checklist_line_label$', 1, '', [
        ...
        foreach ($checklist_lines as $line) {
            ...
            echo '</label>';
            if ($can_update) {
                echo action_icon(T_('Delete'), 'delete', '#', null, null, null, [
                    'class' => 'checklist_line_delete',
                    'style' => 'visibility:hidden;',
                ]);
            }
        ...
        if ($can_update) {
            ...
            $Form->textarea_input('checklist_input_' . $this->ID, null, 1, '', [
