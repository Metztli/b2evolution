...
class item_attachments_Widget extends ComponentWidget
{
    ...
    public function display($params)
    {
        ...
        $TemplateCache = &get_TemplateCache();
        if (! $TemplateCache->get_by_code($this->disp_params['template'], false, false)) {	// No template:
        ...
        if (empty($item_files)) {	// Don't display this widget when Item has no attachments:
            $this->disp_params = array_merge([
