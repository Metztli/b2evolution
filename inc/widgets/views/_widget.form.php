...
if ($mode == 'customizer') {	// Display customizer tabs to switch between skin and widgets in special div on customizer mode:
...
if (($display_mode == 'normal' && empty($mode)) || ! isset($AdminUI) || ! isset($AdminUI->skin_name) || $AdminUI->skin_name != 'bootstrap') {	// Display a link to close form (Don't display this link on bootstrap skin, because it already has an icon to close a modal window)
    $Form->global_icon(TB_('Cancel editing') . '!', 'close', regenerate_url('action'), '', 3, 2, [
        'class' => 'action_icon close_link',
    ]);
}

if ($mode == 'customizer') {	// Don't display this title because it is displayed on customizer mode:
    $form_title = '';
} else {	// Set form title for all other display modes:
    $form_title = sprintf($creating ? TB_('New widget "%s" in container "%s"') : TB_('Edit widget "%s" in container "%s"'), $edited_ComponentWidget->get_name(), $edited_ComponentWidget->get_container_param('name'))
        . ' ' . action_icon(TB_('Open relevant page in online manual'), 'manual', $edited_ComponentWidget->get_help_url(), null, 5, null, [
            'target' => '_blank',
        ]);
}
$Form->begin_form('fform', $form_title, [
    'data-widget-code' => $edited_ComponentWidget->get('code'),
]);

// Plugin widget form event:
$Plugins->trigger_event('WidgetBeginSettingsForm', [
    'Form' => &$Form,
...
// Display properties:
$Form->begin_fieldset(TB_('Widget info'), [
    'id' => 'widget_info',
]);
if ($mode == 'customizer') {
...
foreach ($edited_ComponentWidget->get_param_definitions($tmp_params) as $l_name => $l_meta) {
    if (isset($l_meta['layout']) && $l_meta['layout'] == 'begin_fieldset') {	// Flag to know fieldset is started by widget params config:
        $opened_fieldsets++;
        if ($default_fieldset_is_opened) {	// End default fieldset before new start of config fieldset:
    ...
    } elseif (isset($l_meta['layout']) && $l_meta['layout'] == 'end_fieldset') {	// Flag to know fieldset is ended by widget params config:
        $opened_fieldsets--;
    } elseif ($opened_fieldsets == 0 && (! isset($l_meta['layout']) || $l_meta['layout'] != 'html')) {	// Start default fieldset if it is not defined in widget params config,
    ...
    if ($l_name == 'allow_blockcache') {
        if (isset($l_meta['disabled'])
            && ($l_meta['disabled'] == 'disabled')) { // Force checkbox "Allow caching" to unchecked when it is disallowed from widget config
            $l_value = 0;
        }

        if (! $Blog->get_setting('cache_enabled_widgets')) { // Widget/block cache is disabled by blog setting
            $l_meta['allow_blockcache']['note'] = sprintf(
                TB_('This widget could be cached but the block cache is OFF. Click <a %s>here</a> to enable.'),
...
for ($o = 0; $o < $opened_fieldsets; $o++) {	// End all not closed fieldsets:
    $fieldset_name = 'settings_layout_start';
    $fieldset_meta = [
        'layout' => 'end_fieldset',
    ];
...
// Plugin widget form event:
$Plugins->trigger_event('WidgetEndSettingsForm', [
    'Form' => &$Form,
    'ComponentWidget' => &$edited_ComponentWidget,
]);
...
$buttons[] = [
    'submit',
    'submit',
    ($mode == 'customizer' ? TB_('Apply Changes!') : TB_('Save Changes!')),
    'SaveButton',
    'data-shortcut' => 'ctrl+enter,command+enter',
];
if ($mode == 'customizer') {	// Display buttons in special div on customizer mode:
    echo '<div class="evo_customizer__buttons">';
    if ($mode == 'customizer') {	// Display a button-link to go back (only in customizer mode):
        $buttons[] = [
            'button',
            'button',
            TB_('Cancel'),
    ...
    $buttons = [];
} else {	// Additional button for normal mode in back-office:
    $buttons[] = [
        'submit',
        'actionArray[update_edit]',
        TB_('Save and continue editing...'),
        'SaveButton',
...
if ($mode == 'customizer') {	// End of customizer content:
    echo '</div>';
}
...
if ($display_mode == 'js') {	// Reset previous and Initialize new bozo validator for each new opened widget edit form in popup window,
