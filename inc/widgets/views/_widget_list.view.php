...
$skin_ID = $Blog->get(get_param('skin_type') . '_skin_ID', [
    'real_value' => true,
]);

if (empty($skin_ID) && get_param('skin_type') != 'normal') {	// Don't allow to control widgets if same skin is used for mobile/tablet/alt:
...
} else {	// Allow to control widgets if different skin is used for mobile/tablet/alt:
    ...
    echo action_icon(
        T_('Reload container definitions'),
        'reload',
        get_admin_url('ctrl=widgets&amp;blog=' . $Blog->ID . '&amp;action=reload&amp;skin_type=' . get_param('skin_type') . '&amp;' . url_crumb('widget')),
        T_('Reload container definitions'),
        3,
        4,
        [
            'class' => 'action_icon hoverlink btn btn-info pull-right',
        ]
    );
    ...
    echo action_icon(
        T_('Add Sub-Container'),
        'add',
        get_admin_url('ctrl=widgets&amp;blog=' . $Blog->ID . '&amp;action=new_container&amp;container_type=sub&amp;skin_type=' . get_param('skin_type')),
        T_('Add Sub-Container') . ' &raquo;',
        3,
        4,
        [
            'class' => 'action_icon hoverlink btn btn-default pull-right',
        ]
    );
    echo '<div class="clearfix"></div>';
    ...
    echo action_icon(
        T_('Add Page container'),
        'add',
        get_admin_url('ctrl=widgets&amp;blog=' . $Blog->ID . '&amp;action=new_container&amp;container_type=page&amp;skin_type=' . get_param('skin_type')),
        T_('Add Page Container') . ' &raquo;',
        3,
        4,
        [
            'class' => 'action_icon hoverlink btn btn-default pull-right',
    ...
    echo action_icon(
        T_('Add Shared Container'),
        'add',
        get_admin_url('ctrl=widgets&amp;blog=' . $Blog->ID . '&amp;action=new_container&amp;container_type=shared&amp;skin_type=' . get_param('skin_type')),
        T_('Add Shared Container') . ' &raquo;',
        3,
        4,
        [
            'class' => 'action_icon hoverlink btn btn-default pull-right',
    ...
    // Display a button to add new shared container:
    echo action_icon(
        T_('Add Shared Sub-Container'),
        'add',
        get_admin_url('ctrl=widgets&amp;blog=' . $Blog->ID . '&amp;action=new_container&amp;container_type=shared-sub&amp;skin_type=' . get_param('skin_type')),
        T_('Add Shared Sub-Container') . ' &raquo;',
        3,
        4,
        [
            'class' => 'action_icon hoverlink btn btn-default pull-right',
