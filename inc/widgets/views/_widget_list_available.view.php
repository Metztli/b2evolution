...
} else {	// Display this title for normal view from back-office:
...
foreach ($Plugin_array as $k => $Plugin) {
    if (empty($Plugin->code)) {
        ...
        if (! isset($Plugin_array_grouped[$plugin_group])) {
            $Plugin_array_grouped[$plugin_group] = [];
        }
...
if (is_pro()) {	// Additional widget for pro version:
...
foreach ($widget_groups as $widget_group_code => $widget_group_title) {
    ...
    if (! isset($core_componentwidget_defs[$widget_group_code])) { // No widgets for this group
        $Form->end_fieldset();
        continue;
    }
    ...
    if (isset($core_componentwidget_defs[$widget_group_code])) {
        foreach ($core_componentwidget_defs[$widget_group_code] as $widget_code) {
            if (! file_exists($inc_path . 'widgets/widgets/_' . $widget_code . '.widget.php')) {	// Skip not found widget to avoid die error:
