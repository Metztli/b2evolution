...
if ($selected = autoselect_blog('blog_properties', 'edit')) { // Includes perm check
     ...
     */
    $edited_Blog = &$Blog;
} else {	// We could not find a blog we have edit perms on...
...
$display_mode = (in_array($display_mode, ['js', 'normal']) ? $display_mode : 'normal');
if ($display_mode == 'js') {	// JavaScript mode:
...
switch ($action) {
    ...
    case 're-order' : // js request
        ...
        $containers = [];
        $passed_widget_IDs = []; // Store here all passed IDs in order to compare them with current DB
        foreach ($containers_list as $a_container) {	// Add each container and grab its widgets:
            $a_container_widgets = explode(',', param(trim($a_container, ','), 'string', true));
            foreach ($a_container_widgets as $a => $a_container_widget) {
                if ($a_container_widget_ID = preg_replace('~[^0-9]~', '', $a_container_widget)) {	// Use only correct widget ID for reordering:
                    $a_container_widgets[$a] = $a_container_widget_ID;
                    $passed_widget_IDs[] = $a_container_widget_ID;
                } else {	// Skip wrong passed widget ID:
...
switch ($display_mode) {
    case 'js': // js response needed
        ...
        if (! check_user_perm('blog_properties', 'edit', false, $blog)) {	// user doesn't have permissions
...
switch ($action) {
    ...
    case 'new_container':
        ...
        if ($container_type == 'shared') {	// Default settings for new shared container:
            $edited_WidgetContainer->set('main', 1);
        } elseif ($container_type != 'shared-sub') {	// Default settings for new container:
    ...
    case 'create':
        ...
        if (! in_array($WidgetContainer->get('code'), array_keys($Blog->get_main_containers($skin_type)))) { // The container is not part of the current skin
        ...
        if ($WidgetContainer->ID == 0) { // New widget container needs to be saved
            $WidgetContainer->dbinsert();
        }
        $edited_ComponentWidget->set('wico_ID', $WidgetContainer->ID);
        $edited_ComponentWidget->set('enabled', 1);

        if ($action == 'duplicate') {	// Copy all params from the original widget:
            $duplicated_Widget->load_param_array();
            ...
            if (! empty($duplicated_Widget->get_param('title'))) {	// Append "(copy)" to title:
        ...
        $Messages->add(sprintf(
            TB_('Widget &laquo;%s&raquo; has been added to container &laquo;%s&raquo;.'),
            $edited_ComponentWidget->get_name(),
            $edited_ComponentWidget->get_container_param('name')
        ), 'success');
        ...
        switch ($display_mode) {
            case 'js':	// this is a js call, lets return the settings page -- fp> what do you mean "settings page" ?
                ...
                if ($mode != 'customizer') {	// Open widget settings, except when in customizer mode:
                    $methods['editWidget'] = ['wi_ID_' . $edited_ComponentWidget->ID];
                }
    ...
    case 'update_edit':
        ...
        if (! param_errors_detected()) { // Update settings:
            $edited_ComponentWidget->dbupdate();
            $Messages->add(TB_('Widget settings have been updated'), 'success');
            switch ($display_mode) {
                case 'js': // js reply
                    $edited_ComponentWidget->init_display([]);
                    $methods = [];
                    $methods['widgetSettingsCallback'] = [
                        $edited_ComponentWidget->ID,
                        $edited_ComponentWidget->get_desc_for_list(),
                        $edited_ComponentWidget->get_cache_status(true),
                    ];
                    if ($action == 'update') {	// Close window after update, and don't close it when user wants continue editing after updating:
                        $methods['closeWidgetSettings'] = [$action];
                    } else {	// Scroll to messages after update:
                        $methods['showMessagesWidgetSettings'] = ['success'];
                        if (! empty($edited_ComponentWidget->reload_page_after_update)) {	// Reload page because it is required to update widget form with some new content which was created during updating:
            ...
            if ($action == 'update_edit') {	// Stay on edit widget form:
                ...
                if ($mode == 'customizer' && ! empty($edited_ComponentWidget->reload_page_after_update)) {	// Set flag to refresh customizer page because it is required to update widget form with some new content which was created during updating:
        ...
        } elseif ($display_mode == 'js') {	// Send errors back as js:
            send_javascript_message([
                'showMessagesWidgetSettings' => ['failed'],
            ], true);
        }
        break;
    ...
    case 'toggle':
        ...
        if ($display_mode == 'js') {
            // EXITS:
            send_javascript_message([
                'doToggle' => [$edited_ComponentWidget->ID, (int) ! $enabled, $plugin_disabled],
            ]);
        }
        header_redirect(get_admin_url('ctrl=widgets&blog=' . $Blog->ID, '&'), 303);
        break;
    ...
    case 'cache_disable':
        ...
        if ($edited_ComponentWidget->get_cache_status() == 'disallowed') { // Don't allow to change cache status because it is not allowed by widget config
            $Messages->add(TB_('This widget cannot be cached in the block cache.'), 'error');
        } else { // Update widget cache status
    ...
    case 'deactivate':
        ...
        if (count($widgets)) { // Enable/Disable the selected widgets
        ...
        if (! empty($updated_widgets)) { // Display a result message only when at least one widget has been updated
            if ($action == 'activate') {
        ...
        if ($mode == 'customizer') {	// Set an URL to redirect back to customizer mode:
        ...
        } else {	// Set an URL to redirect to normal mode:
    ...
    case 're-order' : // js request
        ...
        if ($wico_ID > 0) {	// Re-order widgets of ONE given container:
            $blog_container_IDs = [$wico_ID];
        } else {	// Re-order widgets of ALL containers:
        ...
        if ($result && $blog_container_IDs) {
            ...
            foreach ($containers as $container_fieldset_id => $widgets) { // loop through each container and set new order
                $WidgetContainer = &get_WidgetContainer_by_coll_skintype_fieldset($Blog->ID, $skin_type, $container_fieldset_id);
                if (($WidgetContainer->ID == 0) && (count($widgets) > 0)) { // Widget was moved to an empty main widget container, it needs to be created
                    $WidgetContainer->dbinsert();
                }
                $order = 0; // reset counter for this container
                foreach ($widgets as $widget) { // loop through each widget
                    $order++;
                    $DB->query('UPDATE T_widget__widget
						SET wi_order = ' . $order . ',
        ...
        if ($result) {	// Send success message:
            $DB->commit();
            $Messages->add(TB_('Widgets updated'), 'success');
        } else {	// Send error message if widgets cannot be reordered:
            $DB->rollback();
        ...
        send_javascript_message([
            'sendWidgetOrderCallback' => ['blog=' . $Blog->ID],
        ]); // exits() automatically
        break;
    ...
    case 'update_container':
        ...
        if ($wico_ID > 0) {	// Get the existing widget container:
            $WidgetContainerCache = &get_WidgetContainerCache();
            $edited_WidgetContainer = &$WidgetContainerCache->get_by_ID($wico_ID);
        } else {	// Get new widget container:
            $edited_WidgetContainer = new WidgetContainer();
        }
        if ($edited_WidgetContainer->load_from_Request()) {	// If widget container has been saved successfully:
            ...
            if ($mode == 'customizer') {	// Redirect back to customizer mode:
                $redirect_to = get_admin_url('ctrl=widgets&blog=' . $Blog->ID . '&skin_type=' . $skin_type . '&action=customize&mode=customizer', '&');
            } else {	// Redirect back to back-office widgets list:
    ...
    case 'reload_container':
        ...
        if (get_default_widgets_by_container($edited_WidgetContainer->get('code')) == false) {	// Display error when container is not found
...
if ($display_mode == 'normal') {	// this is a normal (not a JS) request
	 ...
	 */
	var edit_icon_tag = \'' . get_icon('edit', 'imgtag', [
        'title' => TB_('Edit widget settings!'),
    ]) . '\';
	var duplicate_icon_tag = \'' . get_icon('duplicate', 'imgtag', [
        'title' => TB_('Duplicate'),
    ]) . '\';
	var delete_icon_tag = \'' . get_icon('delete', 'imgtag', [
        'title' => TB_('Remove this widget!'),
    ]) . '\';
	var enabled_icon_tag = \'' . get_icon('bullet_green', 'imgtag', [
        'title' => TB_('The widget is enabled.'),
    ]) . '\';
	var disabled_icon_tag = \'' . get_icon('bullet_empty_grey', 'imgtag', [
        'title' => TB_('The widget is disabled.'),
    ]) . '\';
	var disabled_plugin_tag = \'' . get_icon('warning', 'imgtag', [
        'title' => TB_('Inactive / Uninstalled plugin'),
    ]) . '\';
	var activate_icon_tag = \'' . get_icon('activate', 'imgtag', [
        'title' => TB_('Enable this widget!'),
    ]) . '\';
	var deactivate_icon_tag = \'' . get_icon('deactivate', 'imgtag', [
        'title' => TB_('Disable this widget!'),
    ]) . '\';
	var cache_enabled_icon_tag = \'' . get_icon('block_cache_on', 'imgtag', [
        'title' => TB_('Caching is enabled. Click to disable.'),
    ]) . '\';
	var cache_disabled_icon_tag = \'' . get_icon('block_cache_off', 'imgtag', [
        'title' => TB_('Caching is disabled. Click to enable.'),
    ]) . '\';
	var cache_disallowed_icon_tag = \'' . get_icon('block_cache_disabled', 'imgtag', [
        'title' => TB_('This widget cannot be cached.'),
    ]) . '\';
	var cache_denied_icon_tag = \'' . get_icon('block_cache_denied', 'imgtag', [
        'title' => TB_('This widget could be cached but the block cache is OFF. Click to enable.'),
    ]) . '\';
    ...
    init_tokeninput_js();
    init_hotkeys_js('blog', ['ctrl+s', 'command+s', 'ctrl+enter', 'command+enter']);

    if ($action == 'list' && $Session->get('refresh_customizer_window')) {	// This is a request to refresh customizer back-office window:
        // (e.g. used to display new auto created sub-container by widget "Columns(Sub-Containers)" - subcontainer_row_Widget)
    ...
    $AdminUI->breadcrumbpath_init(true, [
        'text' => T_('Collections'),
        'url' => get_admin_url('ctrl=collections'),
    ]);
...
switch ($action) {
    ...
    case 'update_edit':
        switch ($display_mode) {
            case 'js': // js request
                ...
                $output = ob_get_clean();
                send_javascript_message([
                    'widgetSettings' => [$output, $edited_ComponentWidget->get('type'), $edited_ComponentWidget->get('code')],
    ...
    case 'add_list':
        ...
        switch ($mode) {
            case 'customizer':
                if (preg_match('#^(coll|shared):(.+)$#', $container_code, $container_match)) {	// Sub-container may be from Collection and may be Shared:
    ...
    case 'customize':
        ...
        if (! empty($container_code)) {	// Try to get widget container by collection ID, container code and requested skin type:
            $WidgetContainerCache = &get_WidgetContainerCache();
...
if ($display_mode == 'normal') {	// Normal mode:
