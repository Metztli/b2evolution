...
class WidgetContainer extends DataObject
{
    ...
    public static function get_class_db_config()
    {
        ...
        if (! isset($widget_container_db_config)) {
            $widget_container_db_config = array_merge(
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_widget__widget',
                'fk' => 'wi_wico_ID',
                'msg' => T_('%d widget in this container'),
            ],
        ];
    }
    ...
    public function dbinsert()
    {
        ...
        if ((! isset($this->order)) || ($this->order <= 0)) {	// Set the order of the container only if it was not defined explicitly
    ...
    public function dbsave()
    {
        ...
        // Page Container:
        if (isset($this->container_ityp_ID)) {	// We should create new Item automatically for selected Item Type on the form of Page Container:
            ...
            if (($widget_page_ItemType = &$ItemTypeCache->get_by_ID($this->container_ityp_ID, false, false)) &&
                    ...
                    ($widget_container_Blog = &$this->get_Blog())) {	// Allow to create new Item only with usage "Widget Page" of Item Type:
        ...
        if ($result && ($result = parent::dbsave())) {	// If container has been saved successfully:
            $DB->commit();
            if (isset($result_message)) {	// Display message only after success widget container saving:
                global $Messages;
                $Messages->add($result_message, 'success');
            }
        } else {	// Rollback changes because of some error above:
            $DB->rollback();
        }
    ...
    {
        if (empty($this->ID)) {	// For new creating container we shoud get a type from hidden field:
            $container_type = param('container_type', 'string');
        } else {	// For existing container we should get current type and don't change it:
                    ...
                    case 'item':
                        ...
                        if (param_check_not_empty('wico_item_ID', sprintf(T_('The field &laquo;%s&raquo; cannot be empty.'), T_('For an existing page')))) {
                            ...
                            if (! ($widget_page_Item = &$ItemCache->get_by_ID($wico_item_ID, false, false)) ||
                                    $widget_page_Item->get_type_setting('usage') != 'widget-page') {	// Display error for unavailable item:
                                param_error('wico_item_ID', T_('Item can be used for page container only with Item Type usage "Widget Page"!'));
                            }
                            if ($widget_page_Item->get_blog_ID() != $wico_coll_ID) {	// Display error for Item from another Collection:
        ...
        if ($this->ID == 0) {	// Allow to set skin type only on creating new widget container:
        ...
        if ($this->ID > 0) {	// Field "Order" is required for existing container:
            param_string_not_empty('wico_order', sprintf(T_('The field &laquo;%s&raquo; cannot be empty.'), T_('Order')));
        } else {	// Order is set automatically only if it was not defined explicitly:
            param('wico_order', 'integer', 0);
        }
        $this->set_from_Request('order');

        if (! param_errors_detected()) {	// Widget container code must be unique for collection, Check it only when no errors on the form:
            if ($wico_ID = $this->dbexists(['wico_code', 'wico_coll_ID', 'wico_skin_type'], [$this->get('code'), $this->get('coll_ID'), $this->get('skin_type')])) {	// We have a duplicate entry:
                param_error(
                    'ufdf_code',
                    sprintf(
                        T_('Another widget container already uses this code for this skin type. Do you want to <a %s>edit the existing widget container</a>?'),
    ...
    {
        if ($this->get('coll_ID')) {	// Collection/skin container:
            if ($this->get('item_ID') !== null) {	// Page container:
                return 'page';
            } elseif ($this->get('main')) {	// Main container:
                return 'main';
            } else {	// Sub container:
                return 'sub';
            }
        } else {	// Shared container for ALL collections:
            if ($this->get('main')) {	// Shared main container:
                return 'shared';
     ...
     * @return string
     */
    public function get_type_title($type = null)
    {
        if ($type === null) {	// Use current type of this container:
            $type = $this->get_type();
        }
