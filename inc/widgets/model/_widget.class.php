...
class ComponentWidget extends DataObject
{
     ...
     */
    public function __construct($db_row = null, $type = 'core', $code = null)
    {
    ...
    public function &get_WidgetContainer()
    {
     ...
     */
    public function get_container_param($param)
    {
    ...
    public function load_from_Request()
    {
        ...
        foreach ($this->get_param_definitions([
            ...
            if ($parname == 'allow_blockcache'
                    ...
                    && ($parmeta['disabled'] == 'disabled')) { // Force checkbox "Allow caching" to unchecked when it is disallowed from widget config
                $parvalue = 0;
            }
    ...
    {
        if ($this->type == 'plugin') {	// Use widget icon from plugin:
            if ($this->get_Plugin()) {	// Get widget icon from plugin:
                return $this->Plugin->get_widget_icon();
            } else {	// Set icon for inactive / uninstalled plugin:
                $this->icon = 'warning';
            }
        }
     ...
     * @return string
     */
    public function get_desc_for_list()
    {
        $name = $this->get_name();

        if ($this->type == 'plugin') {	// Plugin widget:
    ...
    {
        if ($widget_Plugin = &$this->get_Plugin()) { // Get url of the plugin widget
            $help_url = $widget_Plugin->get_help_url('$widget_url');
        } else { // Core widget must defines this URL
            $help_url = null;
        }
     ...
     * @return string icon
     */
    public function get_help_link($icon = 'help', $use_tooltip = true)
    {
        $widget_url = $this->get_help_url();

        if (empty($widget_url)) { // Return empty string when widget URL is not defined
            return '';
        }
    ...
    public function get_param_definitions($params)
    {
        ...
        if (! isset($r['widget_css_class']) ||
            ! isset($r['widget_ID']) ||
            ! isset($r['allow_blockcache'])) {	// Start fieldset of advanced settings:
            $r['advanced_layout_start'] = [
        ...
        if (! empty($this->allow_link_css_params)) {	// Enable link/button CSS classes only for specific widgets like menu widgets:
            $r['widget_link_class'] = [
        ...
        if (! isset($r['widget_css_class'])) {	// Widget CSS class:
            $r['widget_css_class'] = [
        ...
        if (! isset($r['widget_ID'])) {	// Widget ID:
            $r['widget_ID'] = [
        ...
        if (! isset($r['allow_blockcache'])) {	// Allow widget/block caching:
        ...
        if (! empty($advanced_layout_is_started)) {	// End fieldset of advanced settings:
            $r['advanced_layout_end'] = [
                'layout' => 'end_fieldset',
            ];
     ...
     * @return mixed
     */
    public function get_param($parname, $default_value = null, $group = null)
    {
        $this->load_param_array();

        if (strpos($parname, '[') !== false) {	// Get value for array setting like "sample_sets[0][group_name_param_name]":
            ...
            if (isset($this->disp_params[$parname])) {	// Get an overridden value from skin:
                return $this->disp_params[$parname];
            } elseif (isset($this->param_array[$parname])) {	// Get value from DB:
     ...
     */
    public function set($parname, $parvalue, $make_null = false, $group = null)
    {
        $params = $this->get_param_definitions(null);

        if (isset($params[$parname]) ||
            ($group !== null && isset($params[$group]['inputs'][substr($parname, strlen($group))]))) { // This is a widget specific param:
     ...
     */
    public function request_required_files()
    {
    }
    ...
    public function init_display($params)
    {
        ...
        if (! is_null($this->disp_params)) { // Params have been initialized before...
            return;
        }

        // Generate widget defaults (editable params) array:
        $widget_defaults = [];
        $defs = $this->get_param_definitions([]);
        ...
        if (isset($params['override_params_for_' . $this->code])) {	// Use specific widget params if they are defined for this widget by code:
        ...
        // Replace the values:
        $this->disp_params = str_replace(['$wi_ID$', '$wi_class$'], [$widget_ID, $widget_css_class], $params);
    }
     ...
     */
    public function convert_legacy_param($old_name, $new_name)
    {
        //pre_dump( $this->disp_params );
        if (isset($this->disp_params[$old_name]) && ! isset($this->disp_params[$new_name])) {	// We have old param but NOT new param, duplicate old to new:
        ...
        switch ($this->type) {
            case 'plugin':
                ...
                if ($Plugins->call_by_code($this->code, $this->disp_params)) {
                    return true;
                } else {	// Plugin failed (happens when a plugin has been disabled for example):
                    if ($this->mode == 'designer') {	// Display red text in customizer widget designer mode in order to make this plugin visible for editing:
     ...
     */
    public function display_with_cache($params, $keys = [])
    {
        ...
        if (is_logged_in() && $Session->get('designer_mode_' . $Blog->ID)) {	// Initialize data which is used by JavaScript to build overlay designer mode html elements:
            $designer_mode_data = [
                'data-id' => $this->ID,
            ...
            if ($this->get('code') == 'subcontainer' &&
                ($sub_WidgetContainer = &$this->get_sub_WidgetContainer())) {	// For Sub-Container widget we should know what sub-container is used in order to list and add widgets on customizer mode:
        ...
        if ($force_nocaching
            ...
            || $this->get_cache_status() == 'disallowed') { // NO CACHING - We do NOT want caching for this collection or for this specific widget:
            if ($display_containers) { // DEBUG:
            ...
            if (! empty($designer_mode_data)) {	// Append designer mode html tag attributes to first not empty widget wrapper/container:
                $widget_wrappers = [
                    'block_start',
                    'block_body_start',
                    'list_start',
                ...
                foreach ($widget_wrappers as $widget_wrapper_items) {
                    if (! is_array($widget_wrapper_items)) {
                        $widget_wrapper_items = [$widget_wrapper_items];
                    }
                    $wrapper_is_found = false;
                    foreach ($widget_wrapper_items as $widget_wrapper) {
                        if (! empty($params[$widget_wrapper]) || ! empty($params['override_params_for_' . $this->code][$widget_wrapper])) {	// Append new data for widget wrapper:
                            $attrib_actions = [
                                'data-id' => 'replace',
                                'data-type' => 'replace',
                            ...
                            if (! empty($params[$widget_wrapper])) {	// If this wrapper is filled and used with current widget,
                                $params[$widget_wrapper] = update_html_tag_attribs($params[$widget_wrapper], $designer_mode_data, $attrib_actions);
                            }

                            if (! empty($params['override_params_for_' . $this->code][$widget_wrapper])) {	// Also update override params:
                            ...
                            if (isset($this->disp_params[$widget_wrapper])) {	// Also update params if they already have been initialized before:
                    ...
                    if ($wrapper_is_found) {	// Stop search other wrapper in order to use only first filled wrapper:
                        break;
                    }
                }
                if (! $wrapper_is_found) {	// Display error if widget has no wrappers to enable designer mode:
                    echo ' ' . get_rendering_error('Widget <code>' . $this->code . '</code> cannot be manipulated because it lacks a wrapper tag.', 'span') . ' ';
                }
            }

            if (! isset($params['widget_' . $this->code . '_display']) || ! empty($params['widget_' . $this->code . '_display'])) {	// Display widget content:
                $this->display($params);
            } else {	// Hide the widget by code if it is requsted from skin:
                $this->display_debug_message('Widget "' . $this->get_name() . '" is hidden by code <code>' . $this->code . '</code> from skin template.');
            }

            $widget_content = ob_get_clean();

            if (! check_user_perm('blog_admin', 'edit', false, $Blog->ID)) {	// Clean up rendering errors from content if current User is not collection admin:
            ...
            if ($content !== false) { // cache hit, let's display:
                if ($display_containers) { // DEBUG:
            ...
            } else { // Cache miss, we have to generate:
                if ($display_containers) { // DEBUG:
    ...
    public function get_cache_keys()
    {
        ...
        if ($this->type == 'plugin' && $this->get_Plugin()) {	// Get widget cache keys from plugin:
     ...
     */
    public function get_cache_status($check_blog_restriction = false)
    {
        $default_widget_params = $this->get_param_definitions([]);
        if (! empty($default_widget_params)
            ...
            && ($default_widget_params['allow_blockcache']['disabled'] == 'disabled')) { // Widget cache is NOT allowed by widget config
            return 'disallowed';
        } else { // Check current cache status if it is allowed
            if ($check_blog_restriction) { // Check blog restriction for widget caching
                $widget_Blog = &$this->get_Blog();
                if ($widget_Blog && ! $widget_Blog->get_setting('cache_enabled_widgets')) {	// Widget/block cache is not allowed by collection setting:
                    return 'denied';
                }
            }

            if ($this->get_param('allow_blockcache')) { // Enabled
                return 'enabled';
            } else { // Disabled
                return 'disabled';
            }
     ...
     * @access protected
     */
    public function disp_title($title = null, $display = true)
    {
        if (is_null($title)) {
     ...
     * @param array MUST contain at least the basic display params
     */
    public function disp_coll_list($filter = 'public', $order_by = 'ID', $order_dir = 'ASC')
    {
        /**
         * @var Blog
         */
        ...
        // fp> TODO: init default value for $this->disp_params['list_type'] to avoid error
        {
            ...
            foreach ($blog_array as $l_blog_ID) {	// Loop through all public blogs:
                $l_Blog = &$BlogCache->get_by_ID($l_blog_ID);

                if ($Blog && $l_blog_ID == $Blog->ID) { // This is the blog being displayed on this page:
    ...
    public function dbupdate()
    {
        ...
        if ($result) {	// If widget has been really updated
    ...
    {
        if ($this->Blog === null) { // Get blog only first time
            $BlogCache = &get_BlogCache();
     ...
     * @return string
     */
    public function get_layout_start()
    {
     ...
     * @return string
     */
    public function get_layout_end($cell_index = 0)
    {
     ...
     * @return string
     */
    public function get_layout_item_start($cell_index = 0, $is_selected = false, $disp_param_prefix = '')
    {
     ...
     * @return string
     */
    public function get_layout_item_end($cell_index = 0, $is_selected = false, $disp_param_prefix = '')
    {
    ...
    public function &get_target_User()
    {
        if ($this->target_User === null) {	// Initialize target User only first time:
            ...
            if ($this->disp_params['widget_context'] == 'user') {	// Use an user of current page disp=user (Only if we are in the context of displaying an User, not if $User is set from before):
                $user_ID = get_param('user_ID');
                if (empty($user_ID) && is_logged_in()) {	// Use current logged in User:
                    global $current_User;
                    $user_ID = $current_User->ID;
                }
                if (! empty($user_ID)) {	// Try to get User by ID:
                    $UserCache = &get_UserCache();
                    $this->target_User = &$UserCache->get_by_ID($user_ID, false, false);
                }
            }

            if (empty($this->target_User) && $this->disp_params['widget_context'] == 'item' && ! empty($Item)) {	// Use an author of the current $Item (Only if we are in the context of displaying an Item, not if $Item is set from before):
                $this->target_User = &$Item->get_creator_User();
            }

            if (empty($this->target_User) && ! empty($Blog)) {	// Use an owner of the current $Blog:
     ...
     */
    public function get_rendered_content($content)
    {
        if (! isset($this->disp_params['renderers'])) {	// This widget has no render settings, Return original content:
            return $content;
        }
        ...
        $widget_Blog = &$this->get_Blog();
        if (empty($widget_Blog)) {	// Use current collection if it is not defined, e.g. for shared widget containers:
            global $Blog;
            $widget_Blog = $Blog;
        }
        ...
        $Plugins->render($content, $widget_renderers, 'htmlbody', [
            'Blog' => &$widget_Blog,
            'Widget' => $this,
        ], 'Render');
        $Plugins->render($content, $widget_renderers, 'htmlbody', [
     ...
     */
    public function get_form_display($default_form_display = 'standard')
    {
        $form_display = isset($this->disp_params['form_display']) ? $this->disp_params['form_display'] : $default_form_display;
        return in_array($form_display, ['standard', 'compact', 'nolabels', 'inline', 'grouped']) ? $form_display : $default_form_display;
    }
     ...
     * @return string
     */
    public function get_param_item_info($param_name)
    {
        $param_value = $this->get_param($param_name, '');
        if (empty($param_value)) {	// Param is not defined:
            return '';
        }
        ...
        if (! ($param_value_is_ID && $param_Item = &$ItemCache->get_by_ID($param_value, false, false)) &&
            ! (! $param_value_is_ID && $param_Item = &$ItemCache->get_by_urltitle($param_value, false, false))) {	// Item is not detected:
            return get_rendering_error(T_('Item is not found.'), 'span');
        }

        $item_info = '';
        $status_icons = get_visibility_statuses('icons');
        if (isset($status_icons[$param_Item->get('status')])) {	// Status colored icon:
            $item_info .= $status_icons[$param_Item->get('status')];
        }
        // Title with link to permament url:
        $item_info .= ' ' . $param_Item->get_title([
            'link_type' => 'admin_view',
        ]);
        // Icon to edit:
        $item_info .= ' ' . $param_Item->get_edit_link([
            'text' => '#icon#',
        ]);
    ...
    public function display_debug_message($message = null)
    {
        if ($this->mode == 'designer') {	// Display message on designer mode:
            ...
            if (preg_match('#class="[^"]*evo_widget[^"]*"#i', $this->disp_params['block_start'] . $this->disp_params['block_body_start'])) {	// If standard widget wrappers have special style class "evo_widget" we can use it:
                echo $this->disp_params['block_start'];
                $this->disp_title();
                ...
                echo $this->disp_params['block_end'];
            } else {	// Otherwise we should use more wrappers to design widgets correctly, e-g for Menu container:
                echo $this->disp_params['block_start'];
                $this->disp_title();
                echo $this->disp_params['block_body_start'];
    ...
    public function display_error_message($message = null)
    {
        ...
        $this->disp_title();
        echo $this->disp_params['block_body_start'];
        if (check_user_perm('blog_admin', 'edit', false, $Blog->ID)) {	// Display error only for collection admin:
     ...
     */
    public function create_auto_subcontainer($name_suffix = '')
    {
        if (! isset($this->cached_existing_containers)) {	// Get existing containers to avoid duplicate error on inserting:
            global $DB;
        ...
        if (strlen($auto_container_code) > 125) {	// Limit widget code to avoid mysql error of long data:
            $auto_container_code = substr($auto_container_code, strlen($auto_container_code) - 123);
        }
        $new_container_code = $auto_container_code;
        $c = 1;
        while (in_array($new_container_code, $this->cached_existing_containers)) {	// Find unique container code per collection and skin type:
        ...
        if (! $new_WidgetContainer->dbinsert()) {	// Stop updating if some new container cannot be created:
            return false;
        }
