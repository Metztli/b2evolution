...
class WidgetCache extends DataObjectCache
{
     ...
     */
    public function __construct($enabled_only = false)
    {
     ...
     */
    public function &get_by_coll_ID($coll_ID, $by_code = false, $skin_type = 'all')
    {
        global $DB, $Session;

        if (! isset($this->cache_container_Widget_array[$coll_ID])) {	// Not in Cache yet:
            ...
            if ($this->load_enabled_only) {	// We want to load enabled widgets only:
            ...
            $this->cache_container_Widget_array[$coll_ID] = [];
            $this->cache_container_Widget_array_by_code[$coll_ID] = [];
        ...
        if ($skin_type == 'auto') {	// Auto detect skin type:
            global $Blog;
            if (empty($coll_ID)) {	// Widgets of shared container:
                $widget_Blog = $Blog;
            } else {	// Widgets of collection/skin container:
        ...
        if ($by_code) {	// Get widgets grouped by container code:
            $widgets = $this->cache_container_Widget_array_by_code[$coll_ID];
        } else {	// Get widgets grouped by container name:
    ...
    public function &new_obj($row = null)
    {
        ...
        if ($row->wi_type == 'core') {
            ...
            */
            ...
            if (! file_exists($inc_path . 'widgets/widgets/_' . $row->wi_code . '.widget.php')) {	// For some reason, that widget doesn't seem to exist... (any more?)
    ...
    public function &get_by_container_ID($wico_ID)
    {
        global $DB;

        if (! isset($this->cache_container[$wico_ID])) { // Not in Cache yet:
     ...
     */
    public function &get_by_coll_container($coll_ID, $container, $by_code = false, $skin_type = 'auto')
    {
        $widgets = $this->get_by_coll_ID($coll_ID, $by_code, $skin_type);
        $widgets = isset($widgets[$container]) ? $widgets[$container] : [];
