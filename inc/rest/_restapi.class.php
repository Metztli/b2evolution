...
class RestApi
{
    ...
    public function __construct($request)
    {
        ...
        if (isset($this->args[0])) {	// Initialize module name:
    ...
    private function user_log_in($entered_login, $entered_password)
    {
        ...
        if (is_email($entered_login)) {	// We have an email address instead of login name
            // Get user by email and password:
            list($User, $exists_more) = $UserCache->get_by_emailAndPwd($entered_login, $entered_password);
        } elseif (is_valid_login($entered_login) === true) {	// Make sure that we can load the user:
            $User = &$UserCache->get_by_login($entered_login);
        }

        if (empty($User)) {	// Wrong authentication because user is not found by requested login/email and password in DB:
            if (is_logged_in()) {	// Logout current user:
                logout();
            }
        ...
        if (is_logged_in()) {	// If current user is logged in
            if ($User->ID == $current_User->ID) {	// The requested user is already logged in, Don't relog in it on each request:
                return true;
                // Exit here.
            } else {	// Log out current user because we should log in new user:
                logout();
            }
        }
        ...
        $login_attempts = empty($login_attempts) ? [] : explode(';', $login_attempts);
        if ($failed_logins_lockout > 0 && count($login_attempts) >= $failed_logins_before_lockout - 1) {	// User already has a maximum value of the attempts:
            $first_attempt = explode('|', $login_attempts[0]);
            if ($localtimenow - $first_attempt[0] < $failed_logins_lockout) {	// User has used N attempts during X minutes, Display error and Refuse login
        ...
        if (! $User->check_password($entered_password)) {	// The entered password is not right for requested user
            // Save new login attempt into DB:
            if (count($login_attempts) >= $failed_logins_before_lockout - 1) { // Unset first attempt to clear a space for new attempt
        ...
        if ($User->check_status('is_closed')) {	// User account was closed, don't log in it:
            $this->halt(T_('This account is closed. You cannot log in.'), 'closed_account', '403');
            // Exit here.
        } elseif ($Settings->get('system_lock') && ! $User->check_perm('users', 'edit')) { // System is locked for maintenance and current user has no permission to log in this mode
        ...
        if (! empty($login_attempts)) {	// Clear the attempts list:
    ...
    private function user_authentication()
    {
        ...
        if (isset($_SERVER, $_SERVER['PHP_AUTH_USER']) &&
            (! is_logged_in() || $current_User->get('login') != $_SERVER['PHP_AUTH_USER'])) {	// Do basic HTTP authentication when user login is provided AND
    ...
    {
        if (empty($this->module)) {	// Wrong request because module is not defined:
            $this->halt('Module is not defined');
            // Exit here.
        }

        if (! method_exists($this, 'module_' . $this->module)) {	// Unknown module:
    ...
    private function halt_with_Messages($status_code = 'no_access', $status_number = 403, $message_types = 'error,warning')
    {
        ...
        foreach ($Messages->messages_text as $m => $message_text) {
            if (in_array($Messages->messages_type[$m], $message_types)) {	// Get this message for halt because of message type:
    ...
    {
        if ($value === null) {	// Use auto key:
            $this->response[] = $key;
        } elseif ($key === null && is_array($value)) {	// Merge new data array to response array:
            $this->response = array_merge($this->response, $value);
        } else {	// Use defined key:
            switch ($type) {
                case 'array':
                    if (! isset($this->response[$key]) || ! is_array($this->response[$key])) {	// Initialize array:
                        $this->response[$key] = [];
                    }
    ...
    private function module_collections()
    {
        ...
        switch ($_SERVER['REQUEST_METHOD']) {
            case 'GET':
                ...
                $valid_resources = ['', 'view', 'items', 'posts', 'search', 'assignees', 'linked'];
                break;

            case 'PUT':
                // List of valid resources
                $valid_resources = ['favorite', 'items_flag'];
                break;
        ...
        if (! empty($coll_controller)) {	// Initialize data for request of the selected collection:
            // Collection urlname:
            $coll_urlname = $this->args[1];

            $BlogCache = &get_BlogCache();
            if (($Collection = $Blog = $BlogCache->get_by_urlname($coll_urlname, false)) === false) {	// Collection is not detected in DB by requested url name:
            ...
            if ($allow_access != 'public') {	// If the collection is not public:
                if (! is_logged_in()) {	// Only logged in users have an access to the collection:
                    $this->halt(T_('You need to log in before you can access this section.'), 'access_requires_login', 403);
                    // Exit here.
                } elseif ($allow_access == 'members') {	// Check if current user is member of the collection:
                    if (! check_user_perm('blog_ismember', 'view', false, $Blog->ID)) {	// Current user cannot access to the collection:
        ...
        $coll_controller_params = [];

        if ($object_ID && $object_action) {	// Set controller name for single object and action:
            $coll_controller = $coll_controller . '_' . $object_action;
            $coll_controller_params[] = $object_ID;
        }

        if (! method_exists($this, 'controller_coll_' . $coll_controller)) {	// Unknown controller:
            $this->halt('Unknown collection controller "' . $coll_controller . '"', 'unknown_controller');
            // Exit here.
        }

        if (! in_array($coll_controller, $valid_resources, true)) { // Invalid request method:
            $this->halt('Request method not supported for the requested resource', 'wrong_request', 405);
            // Exit here.
        }

        // Call collection controller to prepare current request:
        call_user_func_array([$this, 'controller_coll_' . $coll_controller], $coll_controller_params);
    }
    ...
    private function controller_coll_()
    {
        ...
        if ($api_list_in_frontoffice == 'public') {	// SQL to get ONLY public collections:
            $SQL = $BlogCache->get_public_colls_SQL();
            $count_SQL = $BlogCache->get_public_colls_SQL();
            $count_SQL->SELECT('COUNT( blog_ID )');
        } else {	// SQL to get ALL collections that can be seen by currently logged in User:
            $SQL = $BlogCache->get_available_colls_SQL();
            $count_SQL = $BlogCache->get_available_colls_SQL();
            $count_SQL->SELECT('COUNT( blog_ID )');
        }

        if (! empty($api_q)) {	// Search collections by keyword:
            $search_sql_where = [];
            $search_fields = empty($api_fields) ? ['shortname'] : explode(',', $api_fields);
        ...
        if ($api_restrict_to_available_fileroots &&
            ...
            if ($api_restrict_to_available_fileroots) {	// Restrict collections by available file roots for current user:
        ...
        // Prepare pagination:
        if ($api_per_page > 0 && $result_count > $api_per_page) {	// We will have multiple search result pages:
            if ($api_page < 1) {	// Limit by min page:
                $api_page = 1;
            }
            $total_pages = ceil($result_count / $api_per_page);
            if ($api_page > $total_pages) {	// Limit by max page:
        ...
        if ($result_count > 0) {	// Select collections only from current page:
            if ($api_per_page > 0) {	// Limit results by page size only when this is no unlimitted request:
        ...
        foreach ($BlogCache->cache as $Blog) {	// Add each collection row in the response array:
            $this->add_response('colls', [
                'id' => intval($Blog->ID),
    ...
    private function controller_coll_linked()
    {
        ...
        foreach ($linked_colls as $linked_locale => $linked_coll_ID) {	// Add each collection row in the response array:
            if (! ($linked_Blog = &$BlogCache->get_by_ID($linked_coll_ID, false, false))) {	// Skip wrong linked collection:
                continue;
            }
            $this->add_response('colls', [
     ...
     */
    private function controller_coll_items($force_filters = [])
    {
        global $Collection, $Blog;
        ...
        $content_params = param('content_params', 'array', []);
        ...
        if ($post_ID) {	// Get only one requested post:
            if (ctype_digit((string) $post_ID)) {	// Get by post ID:
                $ItemList2->set_filters([
                    'post_ID' => $post_ID,
                ], true, true);
            } else {	// Get by url slug:
                $ItemList2->set_filters([
                    'post_title' => $post_ID,
                ], true, true);
            }
        } else {	// Load all available params from request to filter the posts list:
            $ItemList2->load_from_Request(false);
        }

        if ($ItemList2->filters['types'] == $ItemList2->default_filters['types']) {	// Allow all post types by default for this request:
            $ItemList2->set_filters([
                'itemtype_usage' => null,
            ], true, true);
        }

        if ($page !== null) {	// Set page from request:
            $ItemList2->set_filters([
                'page' => $page,
            ], true, true);
        }

        if (! empty($force_filters)) {	// Force filters:
        ...
        if (! $post_ID) {	// Add data of posts list to response:
        ...
        if ($api_details == '*' || ($post_ID && empty($api_details))) {	// Use all possible fields for single post request or if it is defined to current request:
            $api_details = [
                'id',
                'datestart',
                'urltitle',
        ...
        } elseif (empty($api_details)) {	// For posts list get only ID and title by default:
            $api_details = ['id', 'title'];
        } else {	// Use custom fields from request:
        ...
        while ($Item = &$ItemList2->get_next()) {
            ...
            foreach ($api_details as $api_details_field) {
                switch ($api_details_field) {
                    ...
                    case 'content':
                        $item_data['content'] =
                            $Item->get_images(array_merge($content_params, [
                            ...
                            $Item->get_content_teaser('#', '#', 'htmlbody', array_merge($content_params, [
                            ...
                            $Item->get_content_extension('#', true, 'htmlbody', array_merge($content_params, [
            ...
            if ($post_ID) {	// If only one post is requested then response should as one level array with post fields:
                $this->add_response(null, $item_data);
            } else {	// Add data of each post in separate array of response:
                $this->add_response('items', $item_data, 'array');
            }
        }

        if (empty($item_data)) {	// No posts detected:
            if ($post_ID) {	// Wrong post request:
    ...
    private function controller_coll_search()
    {
        ...
        if (empty($search_params)
            ...
            || ($search_result === null)) {	// We need to perform a new search:
            $search_params = [
        ...
        $search_result = $Session->get('search_result');
        if (empty($search_result)) {	// Nothing found:
            $this->add_response('found', 0, 'integer');
            $this->add_response('page', $api_page, 'integer');
            $this->add_response('page_size', $api_per_page, 'integer');
            $this->add_response('pages_total', 0, 'integer');
            $this->add_response('results', []);
        } else {
            // Prepare pagination:
            $result_count = count($search_result);
            $result_per_page = $api_per_page;
            if ($result_count > $result_per_page) {	// We will have multiple search result pages:
            ...
            if (! $search_result_loaded) {	// Search result objects are not loaded into memory yet, load them:
            ...
            for ($index = $from; $index < $to; $index++) {
                ...
                switch ($row['type']) {
                    case 'item':
                        ...
                        if (empty($Item)) { // This Item was deleted, since the search process was executed
                            continue 2; // skip from switch and skip to the next item in loop
                        }

                        $result_data['title'] = $Item->get_title([
                            'link_type' => 'none',
                        ]);
                    ...
                    case 'comment':
                        ...
                        if (empty($Comment) || ($Comment->status == 'trash')) { // This Comment was deleted, since the search process was executed
                            continue 2; // skip from switch and skip to the next item in loop
                        }

                        $comment_Item = &$Comment->get_Item();
                        $result_data['title'] = $comment_Item->get_title([
                            'link_type' => 'none',
                        ]);
                    ...
                    case 'category':
                        ...
                        if (empty($Chapter)) { // This Chapter was deleted, since the search process was executed
                            continue 2; // skip from switch and skip to the next item in loop
                        }
    ...
    private function controller_coll_assignees()
    {
        ...
        if (! check_user_perm('blog_can_be_assignee', 'edit', false, $Blog->ID)) {	// Check permission: Current user must has a permission to be assignee of the collection:
            $this->halt('You are not allowed to view assigness of the collection "' . $Blog->get('name') . '".', 'no_access', 403);
            // Exit here.
        }

        if (! $Blog->get_setting('use_workflow')) {	// If workflow is not enabled for the collection:
         ...
         */
        if (preg_match('~%u[0-9a-f]{3,4}~i', $api_q)) {	// Decode UTF-8 string:
            $api_q = preg_replace('~%u([0-9a-f]{3,4})~i', '&#x\\1;', $api_q);
            $api_q = html_entity_decode($api_q, ENT_COMPAT, 'UTF-8');
        }

        if (empty($api_q)) {	// Don't allow empty request:
            $this->halt('Please enter at least one char to find assignees', 'no_access', 403);
            // Exit here.
        }

        if ($Blog->get('advanced_perms')) {	// Load group and user permissions ONLY if collection advanced permissions are enabled:
    ...
    private function controller_coll_favorite()
    {
        ...
        if (! is_logged_in()) {	// Check permission: Current user must be logged in
    ...
    private function controller_coll_items_flag($item_ID)
    {
        ...
        $ItemCache = &get_ItemCache();
        if (($Item = &$ItemCache->get_by_ID($item_ID, false, false)) === false) {	// Item is not detected in DB by requested ID:
            $this->halt('No item found in DB by requested ID #' . $item_ID, 'unknown_item', 404);
            // Exit here.
        }

        if ($Item->get_blog_ID() != $Blog->ID) {	// Item should be called for current collection:
            $this->halt('You request item #' . $Item->ID . ' from another collection "' . $Blog->get('urlname') . '"', 'wrong_item_coll', 403);
            // Exit here.
        }

        if (! $Item->can_flag()) {	// Don't display the flag button if it is not allowed by some reason:
    ...
    private function module_users()
    {
            ...
            case 'GET':
                ...
                if (! empty($this->args[1])) {
                    if (! empty($this->args[2])) {
                        ...
                        if (preg_match('/^\d+$/', $this->args[1])) { // args[1] should be a positive integer and not just any number:
                ...
                $valid_resources = ['', 'view', 'recipients', 'authors', 'autocomplete', 'logins', 'search'];
                if (isset($user_ID)) { // Set controller to view the requested user profile:
        ...
        if (! method_exists($this, 'controller_user_' . $user_controller)) {	// Unknown controller:
            $this->halt('Unknown user controller "' . $user_controller . '"', 'unknown_controller');
            // Exit here.
        }

        if (! in_array($user_controller, $valid_resources, true)) { // Invalid request method:
            $this->halt('Request method not supported for the requested resource', 'wrong_request', 405);
            // Exit here.
        }
    ...
    private function controller_user_()
    {
        ...
        if ($api_restrict_to_available_fileroots) {	// Check if current user has an access to file roots of other users:
            global $current_User;
            if (is_logged_in()) {	// Check perms for logged in user:
                if (! (check_user_perm('users', 'moderate') && check_user_perm('files', 'all'))) {	// Current user has an access only to file root of own account:
                    $user_filters = [
                        'userids' => [$current_User->ID],
                    ];
                }
                // otherwise current user has an access to file roots of all users
            } else {	// Anonymous user has no access to file roots:
                $user_filters = [
                    'userids' => [-1],
                ];
            }
        } else {	// Default restriction:
            if (($access_error_message = check_access_users_list('api')) !== true) {	// Current user has no access to public list of the users,
        ...
        $api_list_params = param('list_params', 'array:string', []);
        ...
        if (! empty($user_filters)) {	// Filter list:
        ...
        while ($User = &$UserList->get_next()) {
            ...
            // Picture:
            if ($Settings->get('allow_avatars')) {	// Only if it is allowed by general settings:
        ...
        if (empty($user_data)) {	// No users found:
    ...
    private function controller_user_view()
    {
        ...
        if (($access_error_message = check_access_user_profile($user_ID, 'api')) !== true) {	// Current user has no access to public list of the users,
            // Display error message:
            $this->halt($access_error_message, 'no_access', 403);
            // Exit here.
        }
        ...
        // Other pictures:
        $user_data['pictures'] = [];
        if (check_user_status('can_view_user', $user_ID)) { // Display other pictures, but only for logged in and activated users:
            $user_pic_links = $User->get_avatar_Links();
            foreach ($user_pic_links as $user_pic_Link) {
                $user_pic_url = $user_pic_Link->get_tag([
                    'before_image' => '',
                ...
                if ((preg_match('#src="([^"]+)"#', $user_pic_url, $user_pic_url))) {	// Extract image url:
        ...
        // Location:
        $user_data['location'] = [
            'country' => empty($User->ctry_ID) ? null : [
                'id' => intval($User->ctry_ID),
                'name' => $User->get_country_name(),
            ],
            'region' => empty($User->rgn_ID) ? null : [
                'id' => intval($User->rgn_ID),
                'name' => $User->get_region_name(),
            ],
            'subregion' => empty($User->subrg_ID) ? null : [
                'id' => intval($User->subrg_ID),
                'name' => $User->get_subregion_name(),
            ],
            'city' => empty($User->city_ID) ? null : [
                'id' => intval($User->city_ID),
                'name' => $User->get_city_name(),
            ],
        ];
        ...
        foreach ($User->userfields as $userfield) {
            if (! isset($user_data['userfields'][$userfield->ufgp_name])) {	// Initalize an array for each group of the user fields:
                $user_data['userfields'][$userfield->ufgp_name] = [];
            }
    ...
    private function controller_user_save()
    {
        ...
        if ($user_ID > 0) {	// Initialize User object to update it:
            $UserCache = &get_UserCache();
            $edited_User = &$UserCache->get_by_ID($user_ID, false, false);

            if (! $edited_User) {	// Wrong user request:
                $this->halt('Invalid user ID', 'user_invalid_id');
                // Exit here.
            }

            if (! $current_User->can_moderate_user($edited_User->ID)
                && $edited_User->ID != $current_User->ID) {	// Current user has no permission to delate the requested user:
                $this->halt(T_('You have no permission to edit other users!'), 'no_access', 403);
                // Exit here.
            }
        } else { // Initialize User object to create new one:
        ...
        if ($result !== true) {	// There are errors on update the requested user:
            $Messages->close_group(); // Make sure any open message group are closed
            $this->halt($Messages->messages_text[0], 'update_failed', 403);
            // Exit here.
        } else {	// The requested user has been updated successfully
            $Messages->close_group(); // Make sure any open message group are closed
    ...
    private function controller_user_delete()
    {
        ...
        if (! check_user_perm('users', 'edit')) {	// Current user has no permission to delete the requested user:
            $this->halt(T_('You have no permission to edit other users!'), 'no_access', 403);
            // Exit here.
        }
        ...
        if (! $User->check_delete(sprintf(T_('Cannot delete User &laquo;%s&raquo;'), $User->get('login')))) {	// There are restrictions on delete the requested user:
            $Messages->close_group(); // Make sure any open message group are closed
            $this->halt(strip_tags($Messages->messages_text[0]), 'delete_restriction', 403);
            // Exit here.
        }

        if ($User->dbdelete($Messages) !== false) {	// The requested user has been deleted successfully
            $this->halt(sprintf(T_('User &laquo;%s&raquo; deleted.'), $User->get('login')), 'delete_success', 200);
            // Exit here.
        } else {	// Cannot delete the requested user
    ...
    private function controller_user_recipients()
    {
        ...
        if (! check_user_perm('perm_messaging', 'reply')) {	// Check permission: User is not allowed to view threads
            $this->halt('You are not allowed to view recipients.', 'no_access', 403);
            // Exit here.
        }

        if (check_create_thread_limit(true)) {	// User has already reached his limit, don't allow to get a users list:
        ...
        foreach ($users as $User) {
            if (! $User->check_status('can_receive_pm')) {	// This user is probably closed so don't show it:
                continue;
            }
    ...
    private function controller_user_autocomplete()
    {
        ...
        if (is_valid_login($api_q) !== true) {	// Restrict a wrong request:
        ...
        if (! empty($api_mentioned)) {	// Mentioned logins must be ordered on the top:
            $order_priorities[] = 'WHEN user_login IN ( ' . $DB->quote($api_mentioned) . ' ) THEN 1';
        }

        if (! empty($api_blog)) {	// Collection assignees and members must be ordered with priorities 2 and 3:
        ...
        if (! empty($order_priorities)) {	// Order users by custom priority:
    ...
    private function controller_user_logins()
    {
        ...
        $func_user_search_params = [
            'sql_limit' => 10,
        ];
        if ($api_status == 'all') {	// Get users with all statuses:
            $func_user_search_params['sql_where'] = '';
        } elseif (! empty($api_status)) {	// Restrict users with requested statuses:
            global $DB;
        ...
        $user_logins = [];
        foreach ($users as $User) {
            if ($can_view_all_users || in_array($User->get('status'), ['activated', 'autoactivated', 'manualactivated'])) {	// Allow to see this user only if current User has a permission to see users with current status:
     ...
     * @return array Users
     */
    private function func_user_search($search_string, $params = [])
    {
        global $DB;
        ...
        if (! empty($params['sql_from_add'])) {	// Additional tables:
            $users_SQL->FROM_add($params['sql_from_add']);
        }
        if (! empty($search_string)) {	// Filter by login:
            $users_SQL->WHERE('user_login LIKE ' . $DB->quote(str_replace('$login$', $search_string, $params['sql_mask'])));
        }
        if (! empty($params['sql_where'])) {	// Additional restrict:
        ...
        if (empty($api_page)) {	// Force wrong page number to first:
            $api_page = 1;
        }
        if ($api_page > $count_pages) {	// Don't allow page number more than total pages:
            $api_page = $count_pages;
        }
        if ($api_per_page > 0) {	// Limit users by current page:
        ...
        if (empty($UserCache->cache)) {	// No users found:
    ...
    private function module_tags()
    {
        ...
        if (substr($term, 0, 1) == '-') {	// Prevent chars '-' in first position:
        ...
        foreach ($tags as $tag) {
             ...
             */
            if (utf8_strtolower($tag['name']) == utf8_strtolower($term)) { // Current term is an existing tag
                $term_is_new_tag = false;
            }
        }

        if ($term_is_new_tag && ! empty($term)) {	// Add current term in the beginning of the tags list:
            array_unshift($tags, [
                'id' => $term,
                'name' => $term,
            ]);
    ...
    {
        if (! empty($this->args[2])) {	// Get action from request string:
            $link_action = $this->args[2];
        } else {	// Get action from param:
        ...
        if (! method_exists($this, 'controller_link_' . $link_controller)) {	// Unknown controller:
    ...
    {
        if (! ($Link = &$this->get_Link())) {	// Wrong link request:
        ...
        if (! $LinkOwner->check_perm('edit', false)) {	// Current user has no permission to unlink the requested link:
    ...
    private function controller_link_delete()
    {
        ...
        if ($action == 'delete' && $deleted_Link->can_be_file_deleted()) {	// Get a linked file to delete it after unlinking if it is allowed for current user:
            $linked_File = &$deleted_Link->get_File();
        }

        // Unlink File from Item/Comment:
        $deleted_link_ID = $deleted_Link->ID;
        if ($LinkOwner->remove_link($deleted_Link)) {	// If Link has been removed successfully:
            $LinkOwner->after_unlink_action($deleted_link_ID);

            if ($action == 'delete' && ! empty($linked_File)) {	// Delete a linked file from disk and DB completely:
                $linked_File->unlink();
            }
            ...
            // Exit here.
        } else {	// The requested link cannot be deleted:
    ...
    private function controller_link_change_position()
    {
        ...
        if ($edited_Link->dbupdate() !== false) { // This returns NULL if no change was made
            ...
            if ($LinkOwner->type == 'item' &&
                ($link_position == 'cover' || $link_position == 'background')) { // Position "Cover" can be used only by one link
            ...
            // Exit here
        } else { // return the current value on failure
    ...
    private function controller_link_change_order()
    {
        ...
        foreach ($ownerLinks as $loop_Link) {	// Find nearest order:
        ...
        if ($i > -1 && $i < PHP_INT_MAX) {	// Switch the links:
            ...
            // Exit here.
        } else {	// The requested link order has been changed successfully:
    ...
    private function controller_link_attach()
    {
        ...
        if (! ($current_File = $FileCache->get_by_root_and_path($root_type, $root_in_type_ID, $file_path))) {	// No file:
        ...
        $LinkCache = &get_LinkCache();
        if ($Link = &$LinkCache->get_by_ID($link_ID, false, false)) {	// Add data of new link to response:
    ...
    private function controller_link_refresh()
    {
        ...
        if (! $LinkOwner->check_perm('view', false)) {	// Current user has no permission to unlink the requested link:
            $this->halt('You have no permission to list of the links!', 'no_access', 403);
            // Exit here.
        }

        if (get_param('action') == 'sort') {	// Sort the links:
            $ownerLinks = $LinkOwner->get_Links();
        ...
        if (get_param('action') == 'sort') {	// The sort has been done successfully:
            $this->halt(T_('The attachments have been sorted by file name.'), 'sort_success', 200);
            // Exit here.
        } else {	// The refresh has been done successfully:
    ...
    private function controller_link_copy()
    {
        ...
        if (! $dest_LinkOwner->check_perm('edit', false)) {	// Current user has no permission to copy the requested link:
            $this->halt('You have no permission to list of the links!', 'no_access', 403);
            // Exit here.
        }
        ...
        if (! $source_LinkOwner || ! ($source_LinkList = $source_LinkOwner->get_attachment_LinkList(1000, $source_position, $source_file_type, $link_list_params))) {	// No requested links, Exit here:
        ...
        while ($source_Link = &$source_LinkList->get_next()) {	// Copy a Link to new object:
            if ($limit_position) {
                if ($source_position == '' && $all_position_counts >= $limit_position) {	// Stop copy other positions because we are finding any position:
                    break;
                }
                if (! isset($position_counts[$source_Link->position])) {
                    $position_counts[$source_Link->position] = 0;
                }
                if ($position_counts[$source_Link->position] >= $limit_position) {	// Skip this because of limit per position:
                    if ($source_position == $source_Link->position) {	// Stop copy other positions because we are finding only current:
                        break;
                    } else {	// Continue to find other positions:
                        continue;
                    }
                }
            ...
            if ($source_File = &$source_Link->get_File()) {
                if (isset($dest_files_links[$source_File->ID])) {	// Don't attach the File twice if it is already linked to the destination LinkOwner:
                    $new_link_ID = $dest_files_links[$source_File->ID];
                } else {	// Attach new file because it is not linked to the destination LinkOwner yet:
    ...
    private function module_tools()
    {
        if (empty($this->args[1])) {
            ...
            if (! method_exists($this, 'controller_tool_' . $tool_controller)) {	// Unknown controller:
