...
function trackbacks($post_trackbacks, $Item)
{
    ...
    foreach ($trackback_urls as $tb_url) { // trackback each url:
        $tb_url = trim($tb_url);
        if (empty($tb_url)) {
            continue;
        }
...
function trackback(
    ...
    $result = '';
    if (strstr($trackback_url, '?')) {	// use a HTTP GET request
        ...
        if (! empty($trackback_url['host']) && ! empty($trackback_url['path'])) { // Only try trackback if we have host and path:
