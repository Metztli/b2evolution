...
if (check_comment_mass_delete($CommentList)) {	// A form for mass deleting is available, Display link
...
if ($tab3 != 'meta' && check_user_perm('blogs', 'editall')) {
    if ($CommentList->is_trashfilter()) {
        ...
        $block_item_Widget->global_icon(
            T_('Open recycle bin'),
            'recycle_full',
            $admin_url . '?ctrl=comments&amp;blog=' . $blog . '&amp;' . $CommentList->param_prefix . 'show_statuses[]=trash',
            T_('Open recycle bin'),
            5,
            3,
            [
