...
$Form->submit(['submit', T_('Apply filters'), 'btn-info']);
echo '</div>';

// COMMENTS TO SHOW:
if ($tab3 != 'meta' && ! $CommentList->is_trashfilter()) { // These filters only for normal comments:
    ...
    $Form->begin_fieldset(T_('Comments to show'), [
        'id' => 'comment_filter_comment_to_show',
        'fold' => true,
        'default_fold' => $fold_comments_to_show,
    ]);

    $exclude_statuses = array_merge(get_restricted_statuses($Blog->ID, 'blog_comment!'), ['redirected']);
    $statuses = get_visibility_statuses('notes-array', $exclude_statuses);
    foreach ($statuses as $status_key => $status_name) { // show statuses
        ?>
		<input type="checkbox" name="<?php echo $pp ?>show_statuses[]" value="<?php echo $status_key; ?>" id="sh_<?php echo $status_key; ?>" class="checkbox" <?php if (in_array($status_key, $show_statuses)) {
	...
	<input type="checkbox" name="<?php echo $pp ?>expiry_statuses[]" value="active" id="show_active" class="checkbox" <?php if (in_array("active", $expiry_statuses)) {
	    echo 'checked="checked" ';
	}?> />
	<label for="show_active"><?php echo T_('Show active') ?> </label><br />
	<input type="checkbox" name="<?php echo $pp ?>expiry_statuses[]" value="expired" id="show_expired" class="checkbox" <?php if (in_array("expired", $expiry_statuses)) {
	    echo 'checked="checked" ';
	}?> />
...
$Form->begin_fieldset(T_('Keywords'), [
    'id' => 'comment_filter_keywords',
    'fold' => true,
    'default_fold' => $fold_keywords,
]);
?>
...
<div class="tile">
	<input type="radio" name="<?php echo $pp ?>sentence" value="AND" id="sentAND" class="radio" <?php if ($sentence == 'AND') {
	    echo 'checked="checked" ';
	}?> />
	<label for="sentAND"><?php echo T_('AND') ?></label>
</div>
<div class="tile">
	<input type="radio" name="<?php echo $pp ?>sentence" value="OR" id="sentOR" class="radio" <?php if ($sentence == 'OR') {
	    echo 'checked="checked" ';
	}?> />
	<label for="sentOR"><?php echo T_('OR') ?></label>
</div>
<div class="tile">
	<input type="radio" name="<?php echo $pp ?>sentence" value="sentence" id="sentence" class="radio" <?php if ($sentence == 'sentence') {
	    echo 'checked="checked" ';
	}?> />
	<label for="sentence"><?php echo T_('Entire phrase') ?></label>
</div>
<div class="tile">
	<input type="checkbox" name="<?php echo $pp ?>exact" value="1" id="exact" class="checkbox" <?php if ($exact) {
	    echo 'checked="checked" ';
	}?> />
...
// RATING:
if ($tab3 != 'meta') { // These filters only for normal comments:
    ...
    $Form->begin_fieldset(T_('Rating'), [
        'id' => 'comment_filter_rating',
        'fold' => true,
        'default_fold' => $fold_rating,
    ]);
    ?>
	<div class="rating">
		<input type="checkbox" name="<?php echo $pp ?>rating_toshow[]" value="norating" id="rating_ts_norating" class="checkbox" <?php if (isset($rating_toshow) && in_array("norating", $rating_toshow)) {
		    echo 'checked="checked" ';
		}?> />
		<label for="rating_ts_norating"><?php echo T_('No rating') ?> </label><br />

		<input type="checkbox" name="<?php echo $pp ?>rating_toshow[]" value="haverating" id="rating_ts_haverating" class="checkbox" <?php if (isset($rating_toshow) && in_array("haverating", $rating_toshow)) {
		    echo 'checked="checked" ';
		}?> />
		<label for="rating_ts_haverating"><?php echo T_('Have rating') ?> </label><br />
	</div>
	<div class="rating">
		<input type="radio" name="<?php echo $pp ?>rating_turn" value="above" id="rating_above" class="radio" <?php if ($rating_turn == 'above') {
		    echo 'checked="checked" ';
		}?> />
		<label for="rating_above"><?php echo T_('Above') ?></label>

		<input type="radio" name="<?php echo $pp ?>rating_turn" value="below" id="rating_below" class="radio" <?php if ($rating_turn == 'below') {
		    echo 'checked="checked" ';
		}?> />
		<label for="rating_below"><?php echo T_('Below') ?></label><br />

		<input type="radio" name="<?php echo $pp ?>rating_turn" value="exact" id="rating_exact" class="radio" <?php if ($rating_turn == 'exact') {
		    echo 'checked="checked" ';
		}?> />
    ...
    $Form->begin_fieldset(T_('Author'), [
        'id' => 'comment_filter_author',
        'fold' => true,
        'default_fold' => $fold_authors,
    ]);
    if ($user_count) {
        if ($user_count > 20) {	// Display an input box to enter user login:
    ...
    $Form->begin_fieldset(T_('Author URL'), [
        'id' => 'comment_filter_author_url',
        'fold' => true,
        'default_fold' => $fold_author_url,
    ]);
    ?>
	...
	<div class="tile">
		<input type="radio" name="<?php echo $pp ?>url_match" value="=" id="with_url" class="radio" <?php if ($url_match == '=') {
		    echo 'checked="checked" ';
		}?> />
		<label for="with_url"><?php echo T_('With this') ?></label>

		<input type="radio" name="<?php echo $pp ?>url_match" value="!=" id="without_url" class="radio" <?php if ($url_match == '!=') {
		    echo 'checked="checked" ';
		}?> />
		<label for="without_url"><?php echo T_('Without this') ?></label>
	</div>
	<div class="tile">
		<input type="checkbox" name="<?php echo $pp ?>include_emptyurl" value="true" id="without_any_url" class="checkbox" <?php if ($include_emptyurl) {
		    echo 'checked="checked" ';
		}?> />
    ...
    $Form->begin_fieldset(T_('IP'), [
        'id' => 'comment_filter_ip_address',
        'fold' => true,
        'default_fold' => $fold_ip_address,
    ]);
    ?>
