...
if (empty($item_id)) { // Try to get an item ID from param "p" that is used on edit post page
...
if (! $is_meta_comments_list && $CommentList->total_rows > 0) {	// Allow to select ONLY normal comments(EXCLUDE internal comments) for action on item view page:
...
if (($item_id != 0) && ($comments_number > 0)) { // Display a pagination:
    $comment_params = array_merge($AdminUI->get_template('pagination'), [
        'page_size' => $CommentList->limit,
    ]);
    echo_comment_pages($item_id, $currentpage, $comments_number, $comment_params);
}

if ($item_id > 0) {	// Don't display additional info when we are already viewing a selected post page:
    $display_meta_title = false;
} else {	// Display additional info of internal comment when no post page, e.g. on "Internal comments" tab:
...
if ($threaded_comments_mode) {	// This is "Threaded comments" mode, Initialize global array to store replies:
    ...
    if ((get_param('reply_ID') > 0) &&
        ...
        ($Comment->item_ID == $item_ID)) {	// Put a preview comment in array to display it in proper place:
...
while ($Comment = &$CommentList->get_next()) { // Loop through comments:
    if (($show_comments == 'draft') && ($Comment->get('status') != 'draft')) { // if show only draft comments, and current comment status isn't draft, then continue with the next comment
        continue;
    }

    if ($threaded_comments_mode && $Comment->in_reply_to_cmt_ID > 0) {	// Store the comment replies in a special array:
    ...
    if ($threaded_comments_mode) {	// Display the comment replies:
        echo_comment_replies($Comment->ID, [
    ...
    if (! $comments_can_be_recycled && $Comment->get('status') != 'trash' && check_user_perm('comment!CURSTATUS', 'delete', false, $Comment)) {	// Set flag to know at least one comment from the current list can be recycled:
        $comments_can_be_recycled = true;
    }
    if (! $comments_can_be_deleted && check_user_perm('comment!CURSTATUS', 'delete', false, $Comment)) {	// Set flag to know at least one comment from the current list can be deleted:
...
if (! $is_meta_comments_list && $CommentList->total_rows > 0) {	// Allow to select ONLY normal comments(EXCLUDE internal comments) for action on item view page:
    ...
    if ($item_id > 0 && check_user_perm('blog_post_statuses', 'edit', false, $blog)) {	// Display a button to create a post from selected comments:
        echo ' ' . T_('or') . ' ';
        $Form->button(['submit', 'actionArray[create_comments_post]', T_('Create new Post'), 'btn-warning']);
    }

    if ($comments_can_be_recycled || $comments_can_be_deleted) {	// Display buttons to recycle or delete the selected comments:
        echo ' ' . T_('or') . ' ';
        if ($comments_can_be_recycled && $comments_can_be_deleted) {	// Group recycle and delete buttons:
            echo '<div class="btn-group">';
        }
        if ($comments_can_be_recycled) {	// Button to recycle the comments:
            $Form->button_input([
                'tag' => 'button',
        ...
        if ($comments_can_be_deleted) {	// Button to delete the comments:
            $Form->button_input([
                'tag' => 'button',
        ...
        if ($comments_can_be_recycled && $comments_can_be_deleted) {	// End of group recycle and delete buttons:
            echo '</div>';
        }
    }
