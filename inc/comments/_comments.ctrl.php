...
switch ($action) {
    ...
    case 'update_publish':
    case 'delete':
        if ($action != 'edit' && $action != 'switch_view') { // Stop a request from the blocked IP addresses or Domains
        ...
        if ($edited_Comment->is_meta()) { // Use special permissions for internal comment
            $check_permname = 'meta_comment';
            $check_permlevel = $action == 'delete' ? 'delete' : 'edit';
        } elseif ($action == 'publish' || $action == 'update_publish') { // Load the new comment status from publish request and set perm check values
        ...
        } elseif ($action == 'deprecate') { // set perm check values
            $check_permname = 'comment!deprecated';
            $check_permlevel = 'moderate';
        } else { // set default perm check values
        ...
        if ($action == 'edit' || $action == 'switch_view') {	// Restrict comment status by parent item:
    ...
    case 'list':
    case 'mass_delete':
        if ($action == 'mass_delete') { // Check permission:
            check_user_perm('blogs', 'all', true);
        }

        // Check permission:
        if ($tab3 == 'meta') {	// For internal comments:
...
switch ($action) {
    ...
    case 'edit':
        ...
        $AdminUI->set_coll_list_params('blog_comments', 'view', [
            'ctrl' => 'comments',
            'filter' => 'restore',
        ]);
    ...
    case 'switch_view':
        ...
        if ($edited_Comment->get_author_User()) {	// This comment has been created by member
            if (check_user_perm('users', 'edit') && param('comment_author_login', 'string', null) !== null) {	// Only admins can change the author
                if (param_check_not_empty('comment_author_login', TB_('Please enter valid author login.')) && param_check_login('comment_author_login', true)) {
                    if (($author_User = &$UserCache->get_by_login($comment_author_login)) !== false) {	// Update author user:
                        $edited_Comment->set_author_User($author_User);
                    }
                }
            }
        } else {	// If this is not a member comment
            ...
            if ($can_edit_backoffice_settings) {	// It can be updated only from back-office:
        ...
        if (param('moveto_post', 'string', false)) { // Move to post is set
            $comment_Item = &$edited_Comment->get_Item();
            if ($comment_Item->ID != $moveto_post) { // Move to post was changed
                // Check destination post
                $ItemCache = &get_ItemCache();
                if (($dest_Item = $ItemCache->get_by_ID($moveto_post, false, false)) !== false) { // the item exists
                    ...
                    if (($current_User->ID == $dest_Item_Blog_User->ID &&
                        ...
                        check_user_perm('blog_admin', 'edit', false, $comment_Item_Blog->ID))) { // current user is the owner of both the source and the destination blogs or current user is admin for both blogs
                        $edited_Comment->set_Item($dest_Item);
                    } else {
                        $Messages->add(TB_('Destination post blog owner is different!'), 'error');
                    }
                } else { // the item doesn't exists
        ...
        if ($edited_Comment_Item->Blog->get_setting('allow_html_comment')) {	// HTML is allowed for this comment
            $text_format = 'html';
        ...
        if ($edited_Comment_Item->Blog->get_setting('threaded_comments') &&
            (param('in_reply_to_cmt_ID', 'integer', null, false, false, false) !== false)) { // Change a field "In reply to comment ID" if threaded comments are enabled for the blog
            $in_reply_to_cmt_ID = get_param('in_reply_to_cmt_ID');
            $CommentCache = &get_CommentCache();
            if ($in_reply_to_cmt_ID > 0) { // Check new entered comment ID
                if (! empty($edited_Comment->ID) && $in_reply_to_cmt_ID == $edited_Comment->ID) { // Restrict such brake case
                    $Messages->add(TB_('This comment cannot be a reply to itself.'), 'error');
                } elseif (! ($Comment = &$CommentCache->get_by_ID($in_reply_to_cmt_ID, false, false))) { // No comment exists
                    $Messages->add(TB_('The ID of the parent comment you entered does not exist.'), 'error');
                } elseif ($Comment->item_ID != $edited_Comment_Item->ID) { // Item of new reply comment is not same
                    $Messages->add(TB_('The ID of the parent comment must belong to the same post.'), 'error');
                }
            } else { // Deny wrong comment ID
        ...
        if (check_user_perm('admin', 'restricted') &&
            check_user_perm('blog_edit_ts', 'edit', false, $Blog->ID)) { // We use user date
            param_date('comment_issue_date', TB_('Please enter a valid comment date.'), true);
            if (strlen(get_param('comment_issue_date'))) { // only set it, if a date was given:
        ...
        if (! empty($comment_status)) { // Update status only if it was defined on the submitted form
        ...
        if (isset($old_comment_status) && $old_comment_status != $comment_status) { // Comment moderation is done, handle moderation "secret"
            $edited_Comment->handle_qm_secret();
        }

        // If action is switch_view then don't save the edited Comment yet, only change the edit view
        if ($action != 'switch_view') { // UPDATE DB:
            ...
            if ($action == 'update_edit') {	// Redirect back to the edit comment form in order to see the updated content correctly:
                ...
                /* exited */
            } else {	// Redirect to previous page(e.g. comments list) after updating:
    ...
    case 'trash_delete':
        ...
        if (isset($blog_ID) && ($blog_ID != 0)) { // delete by comment ids
    ...
    case 'elevate':
        ...
        if ($type == 'quote') { // Set a post data for a quote mode:
            $item_content = $edited_Comment->get_author_name() . ' ' . TB_('wrote') . ':';
            if ($new_Item->get_type_setting('allow_html')) { // Use html quote format if HTML is allowed for new creating post:
                $item_content .= ' <blockquote>' . $edited_Comment->get('content') . '</blockquote>';
            } else { // Use markdown quote format if HTML is NOT allowed for new creating post:
            ...
            if (empty($author_User)) { // Use current user:
                $new_Item->set_creator_by_login($current_User->login);
            } else { // Use a comment author:
        ...
        if (! $comment_has_links) {	// If comment has at least one attached file then set content only after
        ...
        if ($comment_has_links) {	// Link all attached files from the elevated Comment to new create Item:
            $item_LinkOwner = new LinkItem($new_Item);
            $inline_link_IDs = [];
            foreach ($comment_Links as $comment_Link) {
                if ($item_link_ID = $item_LinkOwner->add_link($comment_Link->get('file_ID'), $comment_Link->get('position'), $comment_Link->get('order'), false)) {	// If file was linked successfully:
                    if ($comment_Link->get('position') == 'inline') {	// Store what inline link IDs should updated in Item's content:
                        $inline_link_IDs[$comment_Link->ID] = $item_link_ID;
                    }
                }
            }

            if (! empty($inline_link_IDs)) {	// Replace comment inline link IDs with new item link IDs:
    ...
    case 'mass_delete':
        ...
        // Filter list:
        if ($tab3 == 'meta') {	// Internal comments:
...
if ($tab3 == 'fullview' || $tab3 == 'meta') { // Load jquery UI to animate background color on change comment status and to transfer a comment to recycle bin
    require_js_defer('#jqueryUI#');
}

if ($tab3 == 'fullview' || $tab3 == 'listview') {	// Init JS to autcomplete the user logins:
    init_autocomplete_login_js('rsc_url', $AdminUI->get_template('autocomplete_plugin'));
}

if (in_array($action, ['edit', 'update_publish', 'update', 'update_edit', 'elevate', 'switch_view'])) { // Initialize date picker for _comment.form.php
    init_datepicker_js();
...
if (in_array($action, ['edit', 'elevate', 'update_publish', 'update', 'update_edit', 'switch_view']) || $tab3 == 'meta') { // Page with comment edit form
...
switch ($action) {
    ...
    default:
        ...
        if (! empty($process_action) && $process_action == 'mass_delete' && ! empty($mass_type)) { // Mass deleting of the comments
