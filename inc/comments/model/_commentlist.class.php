...
class CommentList2 extends DataObjectList2
{
    ...
    public function __construct(
        ...
        $this->set_default_filters([
            ...
            'exact' => 0,
            'statuses' => null,
            'expiry_statuses' => ['active'], // Show active/expired comments
     ...
     * @param boolean
     */
    public function query_init($force_init = false)
    {
        global $DB;

        if (! $force_init && ! empty($this->query_is_initialized)) { // Don't initialize query because it was already done
            return;
        }
        ...
        if (! is_null($this->Blog) &&
            ...
            ($this->filters['comment_ID_list'] == null || (! empty($this->filters['comment_ID_list']) && substr($this->filters['comment_ID_list'], 0, 1) == '-'))) { // restriction for blog
        ...
        if (! is_null($this->Blog) && isset($this->filters['user_perm'])) { // If Blog and required user permission is set, add the corresponding restriction
         ...
         * ORDER BY stuff:
         */
        $available_sort_options = ['date', 'type', 'author', 'author_url', 'author_email', 'author_IP', 'spam_karma', 'status', 'item_ID'];
        ...
        if ($this->filters['threaded_comments']) {	// In mode "Threaded comments" we should get all replies in the begining of the list
        ...
        if (isset($this->filters['post_statuses'])) { // Set post statuses by filters
            $post_show_statuses = $this->filters['post_statuses'];
        } elseif (is_admin_page()) { // Allow all kind of post status ( This statuses will be filtered later by user perms )
            $post_show_statuses = get_visibility_statuses('keys');
        } else { // Allow only inskin statuses for posts
        ...
        if (strpos($this->ItemQuery->get_from(), 'T_categories') === false &&
            strpos($this->ItemQuery->get_where(), 'cat_blog_ID') !== false) { // Join categories table because it is required here for the field "cat_blog_ID"
    ...
    public function get_filter_titles($ignore = [], $params = [])
    {
        ...
        if ($params['display_visibility'] && isset($this->filters['statuses']) && ! $this->is_trashfilter()) {
            $exclude_statuses = array_merge(get_restricted_statuses($this->Blog->ID, 'blog_comment!'), ['redirected']);
            ...
            if (count($this->filters['statuses']) != count($comment_statuses)) {
                ...
                if ($this->filters['statuses'] == '-') {
                    ...
                    foreach ($status_IDs as $status_ID) {
                        if (! isset($comment_statuses[$status_ID])) {	// User has no permission:
                            continue;
                        }
                        ...
                        $statuses[] = str_replace(
                            ...
                            [$params['visibility_text'], $comment_statuses[$status_ID], $status_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask']
                        );
                    }
                    $title_array[] = str_replace(
                        ...
                        ((count($statuses) > 1) ?
                            [$params['visibility_text'], $params['before_items'] . implode($params['separator_comma'], $statuses) . $params['after_items']] :
                            [$params['visibility_text'], implode($params['separator_comma'], $statuses)]),
                        $params['group_mask']
                    );
                }
                $filter_class_i++;
        ...
        if ($params['display_active']) {
            if (! in_array('active', $this->filters['expiry_statuses']) && ! empty($this->filters['expiry_statuses'])) {	// Display when only "Show expired" comments:
                $filter_class_i = ($filter_class_i > count($filter_classes) - 1) ? 0 : $filter_class_i;
                ...
                $title_array['active'] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$'],
                    [$params['active_hide_text'], $unit_clear_icon, $filter_classes[$filter_class_i]],
                    $params['filter_mask_nogroup']
                );
                $filter_class_i++;
            }
        ...
        if ($params['display_expired']) {
            if (in_array('expired', $this->filters['expiry_statuses'])) {	// Display when only "Show expired" comments:
                $filter_class_i = ($filter_class_i > count($filter_classes) - 1) ? 0 : $filter_class_i;
                ...
                $title_array['expired'] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$'],
                    [$params['expired_show_text'], $unit_clear_icon, $filter_classes[$filter_class_i]],
                    $params['filter_mask_nogroup']
                );
                $filter_class_i++;
            }
        ...
        if ($params['display_rating'] && isset($this->filters['rating_toshow'])) {
            ...
            if (in_array('norating', $this->filters['rating_toshow'])) {
                ...
                $ratings[] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$', '$group_title$'],
                    [$params['rating_none_text'], $unit_clear_icon, $filter_classes[$filter_class_i], $params['rating_text']],
                    $params['filter_mask']
                );
            }
            ...
            if (in_array('haverating', $this->filters['rating_toshow'])) {
                ...
                $ratings[] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$', '$group_title$'],
                    [$rating_operator . ' ' . $this->filters['rating_limit'], $unit_clear_icon, $filter_classes[$filter_class_i], $params['rating_text']],
                    $params['filter_mask']
                );
            }
            ...
            $title_array[] = str_replace(
                ['$group_title$', '$filter_items$'],
                [$params['rating_text'], $params['before_items'] . $ratings . $params['after_items']],
                $params['group_mask']
            );
            $filter_class_i++;
        }
        ...
        if ($params['display_author_url']) {
            ...
            if (! empty($this->filters['author_url'])) {
                ...
                $author_url[] = ($this->filters['url_match'] == '!=' ? $params['author_url_without_text'] : '')
                    . str_replace(
                        ['$filter_name$', '$clear_icon$', '$filter_class$', '$group_title$'],
                        [$this->filters['author_url'], $unit_clear_icon, $filter_classes[$filter_class_i], $params['author_url_text']],
                        $params['filter_mask']
                    );
            }

            if (! empty($this->filters['include_emptyurl'])) {	// Display when only "include empty url" is checked:
                $unit_clear_icon = $clear_icon ? action_icon(T_('Remove this filter'), 'remove', regenerate_url($this->param_prefix . 'include_emptyurl')) : '';
                $author_url[] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$', '$group_title$'],
                    [T_('Include comments with no url'), $unit_clear_icon, $filter_classes[$filter_class_i], $params['author_url_text']],
                    $params['filter_mask_nogroup']
                );
            }
            ...
            if (! empty($author_url)) {
                ...
                $title_array[] = str_replace(
                    ['$group_title$', '$filter_items$'],
                    [$params['author_url_text'], $params['before_items'] . $author_url . $params['after_items']],
                    $params['group_mask']
                );
                $filter_class_i++;
            }
        ...
        if ($params['display_author_IP']) {
            ...
            if (! empty($this->filters['author_IP'])) {
                ...
                $title_array['author_IP'] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$', '$group_title$'],
                    [$this->filters['author_IP'], $unit_clear_icon, $filter_classes[$filter_class_i], $params['author_IP_text']],
                    $params['filter_mask']
                );
            }
        }
    ...
    public function page_links($params = [])
    {
        ...
        if (! is_null($this->Blog) && $this->Blog->get_setting('paged_nofollowto')) {	// We prefer robots not to follow to pages:
    ...
    public function get_col_sort_values($col_idx)
    {
        ...
        $col_sort_values['order_asc'] = regenerate_url(
            [$this->param_prefix . 'order', $this->param_prefix . 'orderby'],
            $this->param_prefix . 'order=ASC&amp;' . $this->param_prefix . 'orderby=' . $col_order_fields
        );
        $col_sort_values['order_desc'] = regenerate_url(
            [$this->param_prefix . 'order', $this->param_prefix . 'orderby'],
     ...
     * @return boolean
     */
    public function is_trashfilter($only_trash = true)
    {
        if (! $only_trash) { // Check if statuses filter contains the 'trash' value
            return is_array($this->filters['statuses']) && in_array('trash', $this->filters['statuses']);
        }
        if (! empty($this->filters['statuses']) && count($this->filters['statuses']) == 1) { // Check if statuses filter contains only the 'trash' value
     ...
     */
    public static function dbprune()
    {
        /**
         * @var DB
         */
        ...
        if ($last_prune >= date('Y-m-d', $localtimenow) && $last_prune <= date('Y-m-d', $localtimenow + 86400)) { // Already pruned today (and not more than one day in the future -- which typically never happens)
     ...
     */
    public function load_list_data($params = [])
    {
        $params = array_merge([
            'load_votes' => true,
        ...
        if (empty($page_comment_ids)) {	// There are no items on this list:
            return;
        }
        ...
        if ($params['load_votes']) {	// Load the vote statuses:
            $this->load_vote_statuses();
        }

        if ($params['load_links']) {	// Load the links:
            $LinkCache = &get_LinkCache();
            $LinkCache->load_by_comment_list($page_comment_ids);
        }

        if ($params['load_items_data']) {	// Load items data:
            $comment_items_IDs = [];
            ...
            if (count($comment_items_IDs)) {	// Load all items of the current page in single query:
     ...
     */
    public function load_vote_statuses()
    {
        ...
        if (empty($not_cached_comment_ids)) {	// The vote statuses are loaded for all comments:
            return;
        }
    ...
    public function display_init($display_params = null, $fadeout = null)
    {
        ...
        if (! isset($display_params['init_order_numbers_mode'])) {	// Set default mode to initialize order numbers:
     ...
     */
    public function init_order_numbers($mode = 'list')
    {
        $this->inlist_orders = [];
