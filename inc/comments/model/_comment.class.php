...
class Comment extends DataObject
{
     ...
     * @var string
     */
    public $anon_notify_last;

    /**
     * Active values for attribute "rel" of author URL link
     */
    public $author_url_nofollow;

    public $author_url_ugc;

    public $author_url_sponsored;
     ...
     * @var integer
     */
    public $temp_link_owner_ID = null;
    ...
    public function __construct($db_row = null)
    {
        ...
        if ($db_row == null) {
            ...
            if (! empty($url) && ! preg_match('~^\w+://~', $url)) { // URL given and does not start with a protocol:
                $url = 'http://' . $url;
            }
            $this->author_url = $url;
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_links',
                'fk' => 'link_cmt_ID',
                'msg' => T_('%d links to destination comments'),
                'class' => 'Link',
            ...
            [
                ...
                'fk' => 'cmvt_cmt_ID',
                'msg' => T_('%d votes on comment'),
            ],
            [
                'table' => 'T_comments__prerendering',
                'fk' => 'cmpr_cmt_ID',
    ...
    public static function db_delete_where($sql_where, $object_ids = null, $params = null)
    {
        ...
        if (! $object_ids) { // There is no comment to delete
            if ($use_transaction) { // Commit transaction if it was started
                $DB->commit();
            }
            return;
        }
        ...
        if (($result !== false) && (! empty($attached_file_ids))) { // Delete orphan attachments and empty comment attachment folders
            load_funcs('files/model/_file.funcs.php');
            remove_orphan_files($attached_file_ids, null, true);
        }

        if ($use_transaction) { // Commit or rollback the transaction
     ...
     */
    public function set_correct_parent_comment()
    {
        if (empty($this->in_reply_to_cmt_ID)) {	// Nothing to fix because this comment has no parent Comment:
            return;
        }
        ...
        while ($parent_Comment) {
            if ($parent_Comment->get('item_ID') == $this->get('item_ID')) {	// This comment is located in same new created Item then we should use this as parent:
    ...
    public function get($parname)
    {
        switch ($parname) {
            case 't_status':
                // Text status:
                $visibility_statuses = get_visibility_statuses('', ['redirected']);
                return $visibility_statuses[$this->status];

            case 'notif_flags':
                return empty($this->notif_flags) ? [] : explode(',', $this->notif_flags);
        }
    ...
    public function set($parname, $parvalue, $make_null = false)
    {
        switch ($parname) {
            case 'rating':
                ...
                if ($parvalue < 1) { // cannot be less than 0
                    $parvalue = null;
                } elseif ($parvalue > 5) { // cannot be more than 5
                    $parvalue = 5;
                }
            ...
            case 'status':
                ...
                if (! isset($this->previous_status)) {	// Set once previous status to know what status was original on several rewriting per same page request:
     ...
     */
    public function set_author_User(&$author_User)
    {
     ...
     * @access protected
     */
    public function set_spam_karma($spam_karma)
    {
     ...
     * @access protected
     */
    public function set_vote($vote_type, $vote_value)
    {
        global $DB, $current_User;

        if (! in_array($vote_type, ['spam', 'helpful'])) { // Restrict access for bad requests
            return;
        }

        switch ($vote_value) { // Set a value for spam vote
            case 'spam':
            case 'yes':
                $vote = '1';
                break;
        ...
        if (empty($this->ID)) { // If comment doesn't exist
            return;
        }
        ...
        if ($existing_vote === null) {	// Add a new vote for first time:
            ...
            if ($existing_vote->value == $vote) {	// Undo previous vote:
                $vote = null;
            }
            $DB->query(
                'UPDATE T_comments__votes
        ...
        if ($vote_type == 'spam' && $vote_value == 'spam') {	// This is a voting about spam comment we should inform moderators:
     ...
     * @return boolean
     */
    public function get_vote_status($type = 'spam')
    {
        global $current_User, $DB, $cache_comments_vote_statuses;

        if (! is_logged_in()) {	// Current user must be logged in:
            return false;
        }

        if (! is_array($cache_comments_vote_statuses)) {	// Initialize array first time:
            $cache_comments_vote_statuses = [];
        }

        if (! isset($cache_comments_vote_statuses[$this->ID])) {	// Get a vote status from DB and cache in global variable:
        ...
        if (isset($cache_comments_vote_statuses[$this->ID][$type])) {	// Return a vote status:
            return $cache_comments_vote_statuses[$this->ID][$type];
        } else {	// Current user didn't vote on this comment yet:
            return false;
        }
    }
     ...
     */
    public function get_vote_spam_disabled()
    {
        global $DB;
        ...
        if ($vote !== false) {	// Get a spam vote for current comment and user:
     ...
     */
    public function get_vote_helpful_disabled()
    {
        global $DB;
        ...
        if ($vote !== false) {	// Get a helpful vote for current comment and user:
            $result['is_voted'] = true;
            $class_disabled = 'disabled';
            $class_voted = 'voted';
            switch ($vote) {
     ...
     * @return string
     */
    public function get_vote_summary($type, $params = [])
    {
        $params = array_merge([
            'result_title' => '',
        ...
        if (! in_array($type, ['spam', 'helpful'])) {	// Bad request
            return '';
        }

        if ($this->{$type . '_countvotes'} == 0) {	// No votes for current comment
            return '';
        }
        ...
        if ($summary < -20) {	// Comment is OK
            $summary = abs($summary) . '% ' . ($type == 'spam' ? T_('OK') : T_('Negative'));
        } elseif ($summary >= -20 && $summary <= 20) {	// Comment is UNDECIDED
        ...
        } elseif ($summary > 20) {	// Comment is SPAM
     ...
     * @return string
     */
    public function get_author_gender()
    {
        if ($this->get_author_User()) {
     ...
     * @return string
     */
    public function get_author_name_anonymous($format = 'htmlbody', $params = [])
    {
        global $Plugins;
        ...
        $gender_class = '';
        if (check_setting('gender_colored')) { // Set a gender class if the setting is ON
        ...
        $Plugins->trigger_event('FilterCommentAuthor', [
            'data' => &$author_name,
            'Comment' => &$this,
        ]);

        if (is_null($params['rel'])) { // Set default rel:
            $params['rel'] = 'bubbletip_comment_' . $this->ID;
        }

        if (! empty($params['rel'])) { // Initialize attribure "rel"
    ...
    {
        if ($this->get_author_User()) { // Author is a user
     ...
     * @param array
     */
    public function avatar($size = 'crop-top-64x64', $class = 'bCommentAvatar', $params = [])
    {
     ...
     * @return string
     */
    public function get_avatar($size = 'crop-top-64x64', $class = 'bCommentAvatar', $params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
        ...
        if (! $Settings->get('allow_avatars')) { // Avatars are not allowed generally, Exit here
            return;
        }

        $comment_Item = $this->get_Item();
        $comment_Item->load_Blog();
        if (! $this->Item->Blog->get_setting('comments_avatars')) { // Avatars are not allowe for this Blog, Exit here
            return;
        }

        $author_link = get_user_identity_url($this->author_user_ID);

        if ($comment_author_User = &$this->get_author_User()) { // Author is a user
            if ($comment_author_User->has_avatar()) { // Get an image
                $r = $comment_author_User->get_avatar_imgtag($size, $class, '', $params['thumb_zoomable']);
                if ($author_link != '' && ! $params['thumb_zoomable']) { // Add author link
        ...
        return get_avatar_imgtag_default($size, $class, '', [
     ...
     * @param array
     */
    public function author2($params = [])
    {
    ...
    public function get_author($params = [])
    {
        ...
        if (empty($Blog)) { // Set Blog if it is still not defined
            $comment_Item = $this->get_Item();
            $Collection = $Blog = $comment_Item->get_Blog();
        }

        if ($Blog->get_setting('allow_comments') != 'any' && $params['after_user'] == '#' && $params['after'] == '#') { // The blog does not allow anonymous comments, Don't display a type of comment author
            $params['after_user'] = '';
            $params['after'] = '';
        }

        if ($params['after_user'] == '#' && $this->is_meta()) {	// Don't display a commenter type for internal comment, because only memebers can create them:
            $params['after_user'] = '';
        }

        if (! $Blog->get_setting('comments_avatars') && $params['link_text'] == 'avatar') { // If avatars are not allowed for this Blog
            $params['link_text'] = 'name';
        }

        if ($this->get_author_User()) { // Author is a registered user:
            if ($params['after_user'] == '#') {
                $params['after_user'] = ' ' . $this->get_author_label($params);
            }

            $r = $this->author_User->get_identity_link($params);

            $r = $params['before_user'] . $r . $params['after_user'];
        } else { // Not a registered user, display info recorded at edit time:
            if ($params['after'] == '#') {
                $params['after'] = ' ' . $this->get_author_label($params);
            }

            if (utf8_strlen($this->author_url) <= 10) { // URL is too short anyways...
                $params['link_to'] = '';
            }

            $author_name_params = [];
            if (strpos($params['link_text'], 'avatar') !== false) { // Get avatar for anonymous user
     ...
     * @param array Params
     */
    public function get_author_label($params = [])
    {
        global $Skin;

        // Default params:
        if (is_admin_page() || (isset($Skin) && $Skin->get_css_framework() == 'bootstrap')) {	// for v6 bootstrap skins:
            $default_params = [
        ...
        // Type of author:
        if ($this->get_author_User()) {	// If author is a registered user:
            $r .= $params['member_before'] . T_('Member') . $params['member_after'];
        } else {	// If author is not a registered user:
     ...
     */
    public function author_ip($before = '', $after = '', $IP_link_to = false, $display_antispam_link = false)
    {
        if (! empty($this->author_IP)) {
            global $Plugins, $CommentList;

            $author_IP = $this->author_IP;
            if ($IP_link_to === 'antispam') { // Add link to antispam page
                $author_IP = implode(', ', get_linked_ip_list([$author_IP]));
            } elseif ($IP_link_to === 'filter' || $IP_link_to) { // Add link to filter by IP
            ...
            echo $Plugins->get_trigger_event(
                'FilterIpAddress',
                [
                    'format' => 'htmlbody',
                    'data' => $author_IP,
            ...
            if ($display_antispam_link) { // Display a link to antispam ip page
                $antispam_icon = get_icon('lightning', 'imgtag', [
                    'title' => T_('Go to edit this IP address in antispam control panel'),
                ]);
                echo ' ' . implode(', ', get_linked_ip_list([$this->author_IP], null, $antispam_icon));
            }
     ...
     */
    public function author_email($linktext = '', $before = '', $after = '', $makelink = true)
    {
        $email = $this->get_author_email();

        if (strlen($email) > 5) { // If email exists:
            echo $before;
            if ($makelink) {
     ...
     */
    public function ip_country($before = '', $after = '')
    {
     ...
     */
    public function get_ip_country($before = '', $after = '')
    {
        $country = '';

        if (! empty($this->IP_ctry_ID)) {	// Country ID is defined
     ...
     */
    public function get_author_url_link($linktext = '', $before = '', $after = '', $makelink = true, $link_class = '')
    {
        global $Plugins;
        ...
        $Plugins->trigger_event('FilterCommentAuthorUrl', [
            'data' => &$r,
            'makelink' => $makelink,
            'Comment' => $this,
        ]);
     ...
     * @return boolean true if URL has been displayed
     */
    public function author_url($linktext = '', $before = '', $after = '', $makelink = true)
    {
     ...
     */
    public function author_url_with_actions($redirect_to = null, $ajax_button = false, $check_perms = true, $save_context = true)
    {
        if ($this->author_url('', ' &bull; Url: <span id="commenturl_' . $this->ID . '" class="bUrl">', '')) { // There is an URL
            if (! $this->get_author_User() && check_user_perm('comment!CURSTATUS', 'edit', false, $this)) { // Author is anonymous user and we have permission to edit this comment...
                if ($redirect_to == null) {
     ...
     */
    public function spam_karma($template = '%s%', $template_unknown = null)
    {
     ...
     * @return boolean
     */
    public function edit_link($before = ' ', $after = ' ', $text = '#', $title = '#', $class = '', $glue = '&amp;', $save_context = true, $redirect_to = null)
    {
        global $admin_url;

        if (! $this->can_be_edited()) {	// Don't allow to edit this Comment if it cannot be edited by curren User:
            return false;
        }
        ...
        if ($save_context) {	// Use a param to redirect after action:
            if ($redirect_to === null) {	// Get current url for redirect:
                $redirect_to = regenerate_url('', 'filter=restore', '', '&');
            }
        ...
        if ($this->is_meta()) { // Edit internal comment by ajax
     ...
     */
    public function deleteurl_link($redirect_to, $ajax_button = false, $check_perm = true, $glue = '&amp;', $save_context = true)
    {
        global $admin_url;
        ...
        if ($check_perm && ! check_user_perm('comment!CURSTATUS', 'delete', false, $this)) { // If current user has no permission to edit this comment
            return false;
        }
        ...
        } else {	// Don't allow a redirect after action:
            $redirect_to = '';
        }
     ...
     * @return link on success, false otherwise
     */
    public function banurl_link($redirect_to, $ajax_button = false, $check_perm = true, $glue = '&amp;', $save_context = true)
    {
        global $admin_url;
        ...
        if ($check_perm && ! check_user_perm('spamblacklist', 'edit')) { // if current user has no permission to edit spams
            return false;
        }
     ...
     * @param string Redirect url. NOTE: This param MUST NOT be encoded before sending to this func, because it is executed by this func inside.
     */
    public function delete_link($before = ' ', $after = ' ', $text = '#', $title = '#', $class = '', $button = false, $glue = '&amp;', $save_context = true, $ajax_button = false, $confirm_text = '#', $redirect_to = null)
    {
        global $admin_url;
        ...
        if (empty($this->ID)) {	// Happens in Preview
            return false;
        }

        $this->get_Item();

        if (! check_user_perm('comment!CURSTATUS', 'delete', false, $this)) { // If User has no permission to delete a comments:
            return false;
        }

        if ($text == '#') { // Use icon+text as default, if not displayed as button (otherwise just the text)
            $text = ($this->status == 'trash' || $this->is_meta()) ? T_('Delete') . '!' : T_('Recycle') . '!';
            if (! $button) { // Append icon before text
                $text = ($this->status == 'trash' || $this->is_meta() ? get_icon('delete') : get_icon('recycle')) . ' ' . $text;
            }
        }
        if ($title == '#') { // Set default title
        ...
        if ($ajax_button && ($this->status != 'trash')) {
            ...
            if ($display_js_confirm && ($confirm_text == '#')) { // Set js confirm text on comment delete action
                if ($this->is_meta()) {
     ...
     */
    public function get_deprecate_link($before = ' ', $after = ' ', $text = '#', $title = '#', $class = '', $glue = '&amp;', $save_context = true, $ajax_button = false, $redirect_to = null)
    {
        global $admin_url;
        ...
        if (($this->status == 'deprecated') // Already deprecated!
            || ! check_user_perm('comment!deprecated', 'moderate', false, $this)) { // User has no right to deprecated this comment:
            return false;
        }

        $status = 'deprecated';
        $status_order = get_visibility_statuses('ordered-array');
        $status_index = get_visibility_statuses('ordered-index', ['redirected']);
        if (isset($status_index[$status]) &&
            isset($status_order[$status_index[$status]]) &&
            ! empty($status_order[$status_index[$status]][3])) { // Get color of button icon
            $status_icon_color = $status_order[$status_index[$status]][3];
        } else { // Use grey arrow as default
     ...
     * @param array Params
     */
    public function get_vote_link($vote_type, $vote_value, $class = '', $glue = '&amp;', $save_context = true, $ajax_button = false, $params = [])
    {
        $params = array_merge([
        ...
        $is_voted = false;
        $icon_params = [];
        if ($class == 'voted') { // Current user already voted for this
            $class = '';
            $is_voted = true;
            $icon_params = [
                'class' => 'voted',
        ...
        if (strpos($class, 'disabled') !== false) { // add rollover action for disabled buttons
     ...
     * @param string Redirect url. NOTE: This param MUST NOT be encoded before sending to this func, because it is executed by this func inside.
     */
    public function deprecate_link($before = ' ', $after = ' ', $text = '#', $title = '#', $class = '', $glue = '&amp;', $save_context = true, $ajax_button = false, $redirect_to = null)
    {
        $deprecate_link = $this->get_deprecate_link($before, $after, $text, $title, $class, $glue, $save_context, $ajax_button, $redirect_to);
        if ($deprecate_link === false) { // The deprecate link is unavailable for current user and for this comment
            return false;
        }
     ...
     * @param array Params
     */
    public function vote_spam($before = '', $after = '', $glue = '&amp;', $save_context = true, $ajax_button = false, $params = [])
    {
        $params = array_merge([
        ...
        if ($this->is_meta()) {	// Don't allow voting on internal comments:
            return;
        }
        ...
        if (! check_user_perm('blog_vote_spam_comments', 'edit', false, $this->Item->get_blog_ID(), false)) { // If User has no permission to vote spam
            return false;
        }
        ...
        if ($current_User->ID == $this->author_user_ID) { // Display only vote summary for users on their own comments
            $result_summary = $this->get_vote_summary('spam', [
            ...
            foreach ($vote_result['icons_statuses'] as $vote_type => $vote_class) { // Print out 3 buttons for spam voting
                echo $this->get_vote_link('spam', $vote_type, $vote_class, $glue, $save_context, $ajax_button, $params);
            }
            echo '</span>';

            if ($vote_result['is_voted']) { // Display vote summary if user already voted on this comment
                echo ' ' . $this->get_vote_summary('spam', [
     ...
     * @param array Params
     */
    public function vote_helpful($before = '', $after = '', $glue = '&amp;', $save_context = true, $ajax_button = false, $params = [])
    {
        $params = array_merge([
            'before_title' => ' &nbsp; ',
            'skin_ID' => 0,
        ...
        if (! is_logged_in(false) || ! $comment_Item->Blog->get_setting('allow_rating_comment_helpfulness')) { // If User is not logged OR Users cannot vote
            return false;
        }
        ...
        if ($current_User->ID == $this->author_user_ID) {	// Display only vote summary/score for users on their own comments:
            ...
            if ($params['title_text'] === null) {	// Use default title text:
                if (! $vote_result['is_voted']) { // Current user didn't vote on this comment
                    $title_text = $params['helpful_text'];
                } else { // Display vote summary if user already voted on this comment
                    $title_text = $this->get_vote_summary('helpful', $params);
                }
                $title_text .= ' ';
            } else {	// Use custom title text:
        ...
        if ($params['display_wrapper']) {	// Display end of wrapper:
            echo '</span>';
        }
     ...
     */
    public function get_next_status($publish, $current_status = null)
    {
        if (! is_logged_in()) {
            return false;
        }
        ...
        if (is_null($current_status)) { // Use status of comment if param is NULL
        ...
        if ((! $publish) && ($curr_index == 0) && ($current_status != 'trash') && ($current_status != 'deprecated')) { // Increase curr_index value to allow deprecated status for the other statuses from the same public level
            $curr_index = $curr_index + 1;
        }
        $has_perm = false;
        while (! $has_perm && ($publish ? ($curr_index < 4) : ($curr_index > 0))) { // Check until the user has permission or there is no more status to check
            $curr_index = $publish ? ($curr_index + 1) : ($curr_index - 1);
            if (in_array($status_order[$curr_index][0], $restricted_statuses)) {	// The status is restricted for this comment by its item or collection settings:
                $has_perm = false;
            } else {	// Check if current user can moderate this comment to the next/prev status:
                $has_perm = check_user_perm('comment!' . $status_order[$curr_index][0], 'moderate', false, $this);
            }
        }
        if ($has_perm) { // An available status has been found
     ...
     * @param string Redirect url. NOTE: This param MUST NOT be encoded before sending to this func, because it is executed by this func inside.
     */
    public function get_publish_link($before = ' ', $after = ' ', $text = '#', $title = '#', $class = '', $glue = '&amp;', $save_context = true, $ajax_button = false, $redirect_to = null)
    {
        global $admin_url;
        ...
        if ($text == '#') {
            $text = get_icon('publish', 'imgtag') . ' ' . $publish_text;
        }
        if ($title == '#') {
        ...
        if ($ajax_button) {	// This is AJAX button:
     ...
     */
    public function get_moderation_link($params)
    {
        global $admin_url;
        ...
        if ($params['save_context']) {	// Allow to redirect after action:
            if ($redirect_to === null) {	// Use current url to redirect ater action:
                $redirect_to = regenerate_url('', 'filter=restore', '', '&');
            }
            $r .= $glue . 'redirect_to=' . rawurlencode($redirect_to);
        }
        ...
        if ($params['ajax_button']) {	// This is AJAX button:
            $comment_type = $this->is_meta() ? 'meta' : 'feedback';
     ...
     */
    public function publish_link($before = ' ', $after = ' ', $text = '#', $title = '#', $class = '', $glue = '&amp;', $save_context = true, $ajax_button = false, $redirect_to = null)
    {
        if (! check_user_perm('comment!CURSTATUS', 'edit', false, $this, false)) { // User has no permission to edit this comment
            return false;
        }
        ...
        if (compare_visibility_status($highest_status, $this->status) <= 0) { // Current User has no permission to change this comment status to a more public status
            return false;
        }
        ...
        if (isset($status_index[$highest_status]) &&
            ...
            ! empty($status_order[$status_index[$highest_status]][3])) { // Get color of button icon
            $status_icon_color = $status_order[$status_index[$highest_status]][3];
        } else { // Use green arrow as default
     ...
     */
    public function next_status_link($params, $raise, $current_status = null)
    {
        if (! is_logged_in(false)) {
            return false;
        }
        ...
        if (! $next_status_in_row) { // Next status is not allowed for current user
            return false;
        }
        ...
        if ($raise) {
            $action = 'publish';
            $action_icon = get_icon('move_up_' . $next_status_in_row[2], 'imgtag', [
                'title' => '',
            ]);
            $class .= 'btn_raise_status_' . $next_status;
        } else {
            $action = 'restrict';
            $action_icon = get_icon('move_down_' . $next_status_in_row[2], 'imgtag', [
                'title' => '',
            ]);
     ...
     */
    public function raise_link($params)
    {
     ...
     * @return boolean true if link was displayed, false otherwise
     */
    public function lower_link($params)
    {
     ...
     * @return boolean true if link was displayed, false otherwise
     */
    public function moderation_links($params)
    {
        if (! is_logged_in(false)) {
            return false;
        }
        ...
        $last_status = '';
        if ($params['detect_last']) { // We should detect what button is last
            $last_status = $last_status_in_row ? $last_status_in_row[0] : '';
        }

        $r = '';
        $prev_status = '';
        foreach ($statuses as $status) { // Print the buttons to increase status
            $next_status_in_row = $this->get_next_status(true, $status[0]);
            if ($next_status_in_row && $prev_status != $next_status_in_row[0]) {
                $tmp_params = $params;
                if ($first_status == $next_status_in_row[0]) { // Mark this button as first visible
                    $tmp_params['class'] = (isset($tmp_params['class']) ? $tmp_params['class'] : '') . ' first-child';
                    if ($params['detect_last'] && empty($last_status)) { // This first button is also last button
                ...
                if (! in_array($next_status_in_row[0], $frontoffice_statuses)) { // Don't make ajax button for those statuses which are not allowed in the front office
                    $tmp_params = array_merge($tmp_params, [
                        'ajax_button' => false,
                    ]);
                }
        ...
        $prev_status = '';
        foreach ($statuses as $status) { // Print the buttons to decrease status
            $next_status_in_row = $this->get_next_status(false, $status[0]);

            if ($next_status_in_row && $prev_status != $next_status_in_row[0]) {
                $tmp_params = $params;
                $tmp_params['class'] = (isset($tmp_params['class']) ? $tmp_params['class'] : '');
                if ($params['detect_last'] && $last_status == $next_status_in_row[0]) { // Mark this button as last visible
                    $tmp_params['class'] .= ' last-child';
                }
                if (empty($first_status)) { // This last button also is first button
     ...
     * @param string class name
     */
    public function msgform_link($form_url, $before = ' ', $after = ' ', $text = '#', $title = '#', $class = '')
    {
        if ($this->get_author_User()) { // This comment is from a registered user:
            $msg_type = $this->author_User->get_msgform_possibility();
            if (empty($msg_type)) { // message form is not allowed
                return false;
            }
            $form_url = url_add_param($form_url, 'recipient_id=' . $this->author_User->ID);
        } else { // This comment is from a visitor:
            if (empty($this->author_email)) { // We have no email for this comment :(
                return false;
            } elseif (empty($this->allow_msgform)) { // Anonymous commentator does not allow message form (for this comment)
                return false;
            }
            $msg_type = 'email';
     ...
     */
    public function get_permanent_url($glue = '&amp;', $meta_anchor = '#')
    {
        $this->get_Item();
     ...
     * @return string Link
     */
    public function get_permanent_link($text = '#', $title = '#', $class = '', $nofollow = false, $restrict_status = true)
    {
    ...
    public function get_prerendered_content($format = 'htmlbody')
    {
        global $CommentList, $Plugins, $DB;

        $use_cache = $this->ID && in_array($format, ['htmlbody', 'entityencoded', 'xml', 'text']);
        if ($use_cache) {	// the format/comment can be cached:
            if (empty($CommentList)) {	// set comments Blog from comment Item
                $this->get_Item();
                $comments_Blog = &$this->Item->get_Blog();
            } else {	// set comments Blog from CommentList
            ...
            if (isset($CommentPrerenderingCache[$format][$this->ID][$cache_key])) {	// already in PHP cache.
            ...
            } else {	// try loading into Cache
                if (! isset($CommentPrerenderingCache[$format])) {	// only do the prefetch loading once.
                    $CommentPrerenderingCache[$format] = [];
                    ...
                    if (empty($CommentList)) {	// Load prerendered cache for each comment which belongs to this comments Item:
                        $SQL->FROM_add('INNER JOIN T_comments ON cmpr_cmt_ID = comment_ID');
                        $SQL->WHERE('comment_item_ID = ' . $this->Item->ID);
                    } else {	// Load prerendered cache for each comment from the CommentList:
                        $comments_page_ID_array = $CommentList->get_page_ID_array();
                        if (! empty($comments_page_ID_array)) {	// If at least one comment is loaded in current comments list:
                    ...
                    foreach ($rows as $row) {
                        ...
                        if (! isset($CommentPrerenderingCache[$format][$row->cmpr_cmt_ID])) {	// init list
                            $CommentPrerenderingCache[$format][$row->cmpr_cmt_ID] = [];
                        }
        ...
        if (! isset($r)) {
            $data = $this->content;
            $Plugins->trigger_event('FilterCommentContent', [
                'data' => &$data,
                'Comment' => $this,
            ]);
        ...
        $r = $Plugins->render($r, $this->get_renderers_validated(), $format, [
            'Comment' => $this,
            'Item' => $this->get_Item(),
        ], 'Display');
     ...
     * @return string
     */
    public function get_content($format = 'htmlbody', $params = [])
    {
        $params = array_merge([
     ...
     * @return string
     */
    public function get_excerpt($params = [])
    {
        $params = array_merge([
            'format' => 'htmlbody',
            'maxlen' => 254,
     ...
     */
    public function content($format = 'htmlbody', $ban_urls = false, $show_attachments = true, $params = [])
    {
        global $Plugins;
        ...
        if ($show_attachments) {
            if (empty($this->ID)) {	// PREVIEW mode, this comment is not stored in DB yet:
                if (! empty($this->temp_link_owner_ID)) {	// This method is used to attach several files by quick uploader JS button:
                    ...
                    if (($TemporaryID = &$TemporaryIDCache->get_by_ID($this->temp_link_owner_ID, false, false)) &&
                        (($this->get('type') != 'meta' && $TemporaryID->type == 'comment') ||
                          ($this->get('type') == 'meta' && $TemporaryID->type == 'metacomment'))) {	// Get all links of the temporary object which is used for new creating comment:
                        $LinkOwner = new LinkComment(new Comment(), $TemporaryID->ID);
                        $attachments = &$LinkOwner->get_Links();
                    }
                } elseif (isset($this->checked_attachments)) {	// This method is used to attach file per one from submit with standard file input element:
            ...
            } else {	// Get all Links of this comment from DB:
        ...
        foreach ($attachments as $index => $attachment) {
            if (! empty($this->ID) || ! empty($this->temp_link_owner_ID)) {	// Normal mode when comment exists in DB (NOT PREVIEW mode)
                ...
                if ($link_position == 'inline') {	// Skip inline attachments because they are rendered in content by spec function:
                    unset($attachments[$index]);
                    continue;
                }
            } else {	// Set default position for preview files:
                $link_position = 'aftermore';
            }

            $File = $attachment;

            if (empty($File)) { // File object doesn't exist in DB
                global $Debuglog;
                $Debuglog->add(sprintf('File object linked to comment #%d does not exist in DB!', $this->ID), ['error', 'files']);
                continue;
            }

            if (! $File->exists()) { // File doesn't exist on the disk
                global $Debuglog;
                $Debuglog->add(sprintf('File linked to comment #%d does not exist (%s)!', $this->ID, $File->get_full_path()), ['error', 'files']);
                continue;
            }
            ...
            if (count($Plugins->trigger_event_first_true('RenderCommentAttachment', $params)) != 0) {	// This attachment has been rendered by a plugin (to $params['data']), Skip this from core rendering:
                if ($link_position == 'teaser') { // Image should be displayed above content
                    $images_above_content .= $r;
                } elseif ($link_position == 'aftermore') { // Image should be displayed below content
            ...
            if ($File->is_image()) { // File is image
                if ($params['attachments_mode'] == 'view') { // Only preview attachments
                    $image_link_rel = '';
                    $image_link_to = '';
                ...
                if ($link_position == 'teaser') { // Image should be displayed above content
                    $images_above_content .= $r;
                } elseif ($link_position != 'inline') { // Image should be displayed below content
                    $images_below_content .= $r;
                }
        ...
        if (! empty($images_above_content)) { // Display images above content
            echo $images_above_content;
            if ($params['image_text'] != '') { // Display info text below pictures
                echo $params['image_text'];
            }
        }

        if ($ban_urls) { // add ban icons if user has edit permission for this comment
            $ban_urls = check_user_perm('comment!CURSTATUS', 'edit', false, $this);
        }

        $output = $this->get_content($format, $params);

        if ($ban_urls) { // ban urls and user has permission
            echo add_ban_icons($output);
        } else { // don't ban urls
            echo $output;
        }

        if (! empty($images_below_content)) { // Display images below content
        ...
        if (isset($attachments)) { // show not image attachments
            $after_docs = '';
            ...
            foreach ($attachments as $attachment) {
                ...
                echo '<li>';
                if (empty($doc_File)) { // Broken File object
                ...
                } elseif ($params['attachments_mode'] == 'view') { // Only preview attachments
                    $attachment_download_link = '';
                ...
                if (! empty($doc_File) && $doc_File->exists()) { // Display file size if it exists
                    echo ' (' . bytesreadable($doc_File->get_size()) . ')';
                } else { // Display warning if File is broken
     ...
     */
    public function renderer_checkboxes($comment_renderers = null, $display = true)
    {
        global $Plugins;
        ...
        $r = $Plugins->get_renderer_checkboxes($comment_renderers, [
            'Comment' => &$this,
        ]);
    ...
    public function get_title($params = [])
    {
        ...
        switch ($this->get('type')) {
            ...
            case 'meta': // Display an internal comment:
                $href = '';
                if ($params['linked_type']) {	// Make a comment type as link to permanent url:
     ...
     */
    public function date($format = '', $useGM = false)
    {
        if (empty($format)) {	// Get the current locale's default date format
     ...
     * @param boolean true if you want GMT
     */
    public function time($format = '', $useGM = false)
    {
        if (empty($format)) {	// Get the current locale's default time format
            $format = locale_timefmt();
        }

        if ($format == '#short_time') {	// Use short time format of current locale
     ...
     * Rating input
     */
    public function rating_input($params = [])
    {
        global $rsc_uri;
        ...
        if (empty($this->item_ID) && ! empty($params['item_ID'])) {	// Set item ID for form with new comment
            $this->item_ID = $params['item_ID'];
        }

        if ($comment_Item = &$this->get_Item()) {
            if ($item_Blog = &$comment_Item->get_Blog()) {
                if ($item_Blog->get_setting('rating_question') != '') {	// Display star rating question
        ...
        if ($params['reset']) {	// Init "reset" button
            $raty_params['cancel'] = true;
            $this->rating_none_input([
                'before' => '<p>',
                'after' => '</p>',
            ]);
        }
     ...
     * Rating reset input
     */
    public function rating_none_input($params = [])
    {
        $params = array_merge([
            'before' => '',
            'after' => '',
     ...
     */
    public function get_status($format = 'htmlbody', $params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => '',
     ...
     * @param array Params
     */
    public function status($format = 'htmlbody', $params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => '',
    ...
    public function format_status($params = [])
    {
        ...
        if ($this->is_meta()) {	// Don't display a status banner of internal comments:
            return;
        }

        if (is_null($params['status'])) { // Use current status of this comment
            $params['status'] = $this->status;
        }

        if (is_null($params['status_title'])) { // Use current status title of this comment
            $params['status_title'] = $this->get('t_status');
        }

        $r = str_replace(
            ['$status$', '$status_title$', '$tooltip_title$'],
            [$params['status'], $params['status_title'], get_status_tooltip_title($params['status'])],
            $params['template']
        );
     ...
     * - draft
     */
    public function format_statuses($params = [])
    {
     ...
     */
    public function handle_notifications($executed_by_userid = null, $is_new_comment = false, $force_members = false, $force_community = false)
    {
        ...
        if ($notifications_mode == 'off') {	// Don't send notifications:
            return false;
        }
        ...
        if (($force_members != 'force' && $force_community != 'force') &&
            $this->check_notifications_flags(['members_notified', 'community_notified'])) {	// All possible notifications have already been sent:
        ...
        if ($notifications_mode == 'immediate') {	// Send email notifications now!:
        ...
        } elseif ($this->get('notif_status') != 'todo' && $this->get('notif_status') != 'started') {	// Create scheduled job to send notifications:
     ...
     */
    public function send_moderation_emails($executed_by_userid = null, $is_new_comment = false, $already_notified_user_IDs = [])
    {
        ...
        if (! $this->is_meta()) {	// Get the moderators which can be notified about this NORMAL comment:
            ...
            foreach ($moderators_to_notify as $moderator) {
                ...
                if ($notify_moderator) {	// add user to notify:
                    $moderators[] = $moderator->user_ID;
                }
            }
            if ($UserSettings->get($notify_moderation_setting_name, $owner_User->ID) && is_email($owner_User->get('email'))) {	// add blog owner:
            ...
            foreach ($moderators as $index => $moderator_ID) {
                if (in_array($moderator_ID, $already_notified_user_IDs) ||
                    ...
                    ! $moderator_User->check_perm('comment!CURSTATUS', 'edit', false, $this)) {	// User was already notified or it doesn't exist any more, or has no permission to edit this comment!
        ...
        if ($executed_by_userid !== null && isset($notify_users[$executed_by_userid])) {	// Don't notify the user who just created/updated this comment:
     ...
     */
    public function send_email_notifications($executed_by_userid = null, $is_new_comment = false, $already_notified_user_IDs = [], $force_members = false, $force_community = false, $log_messages = false)
    {
        ...
        if (! $comment_item_Blog->get_setting('allow_item_subscriptions')) {	// Subscriptions not enabled!
        ...
        if (! in_array($this->get('status'), ['protected', 'community', 'published'])) {	// Don't send notifications about comments with not allowed status:
        ...
        if ($this->get('status') == 'protected') {	// If the comment is visible for members only...
            if ($force_members == 'force' || ! $this->check_notifications_flags('members_notified')) {	// Members have not been notified yet, do so:
                $notify_members = true;
            }
        } elseif ($this->get('status') == 'community' || $this->get('status') == 'published') {	// If the comment is visible to the community or is public...
            if ($force_members == 'force' || ! $this->check_notifications_flags('members_notified')) {	// Members have not been notified yet (which means the community has not been notified either), notify them all:
                $notify_members = true;
            }
            if ($force_community == 'force' || ! $this->check_notifications_flags('community_notified')) {	// Community have not been notified yet, do so:
        ...
        if (! $this->is_meta()) {	// Get the notify users for NORMAL comments:
            // Send only for active users:
            $active_users_condition = 'AND user_status IN ( "activated", "autoactivated", "manualactivated" )';

            $except_condition = '';
            if (! empty($already_notified_user_IDs)) {	// Set except moderators condition. Exclude moderators who already got a notification email:
            ...
            if ($UserSettings->get('notify_published_comments', $creator_User->ID) && (! empty($creator_User->email))
                && (! (in_array($creator_User->ID, $already_notified_user_IDs)))) {	// Comment creator wants to be notified, and comment author is not a moderator:
            ...
            if ($comment_item_Blog->get_setting('allow_item_subscriptions')) {	// If item subscriptions is allowed:
                $sql = 'SELECT user_ID
						FROM (
                ...
                foreach ($notify_list as $notification) {
                    if (! isset($notify_users[$notification->user_ID])) {	// Don't rewrite a notify type if user already is notified by other type before:
                ...
                foreach ($notify_list as $anon_email => $anon_comment_ID) {
                    if (! isset($notify_anon_users['cmnt_' . $anon_comment_ID])) {	// Don't rewrite a notify type if user already is notified by other type before:
            ...
            if ($comment_item_Blog->get_setting('allow_comment_subscriptions')) {	// If blog subscription is allowed:
                $sql = 'SELECT user_ID
								FROM (
        ...
        } else {	// Get the notify users for INTERNAL comments:
            ...
            foreach ($notify_list as $notification_user_ID) {
                if (! isset($notify_users[$notification_user_ID])) {	// Don't rewrite a notify type if user already is notified by other type before:
        ...
        if (empty($notify_users) && empty($notify_anon_users)) {	// No-one to notify:
            return $notified_flags;
        }
     ...
     */
    public function send_email_messages($notify_users, $is_new_comment = false, $log_messages = false)
    {
        ...
        if (! count($notify_users)) {	// No-one to notify:
            return 0;
        }
        ...
        if ($this->get_author_User()) { // Comment from a registered user:
        ...
        } elseif (! empty($this->author_email)) { // non-member, but with email address:
            $reply_to = $this->author_email;
            $author_name = $this->dget('author');
            $author_user_ID = null;
        } else { // Fallback (we have no email address):  fp>TODO: or the subscriber is not allowed to view it.
            $reply_to = null;
            $author_name = $this->dget('author');
            $author_user_ID = null;
        }
        ...
        foreach ($notify_users as $notify_user_ID => $notify_type) {
            if (is_integer($notify_user_ID)) {	// Get only ID of the registered Users and skip emails of anonymous users:
        ...
        foreach ($notify_users as $notify_user_ID => $notify_type) {
            if ($notify_type == 'anon_subscription') {	// Get data of anonymous user:
                ...
                $notify_full = false;
            } else {	// Get data of registered user:
            ...
            switch ($this->type) {
                ...
                default:
                    if ($notify_type == 'moderator') {	// Subject for moderators:
                    ...
                    } else {	// Subject for subscribed users:
                        switch ($this->get_author_type($notify_User, $notify_email)) {	// Set email title depending on author type:
                            case 'parent':
            ...
            if ($debug) {
                ...
                if ($debug >= 2) { // output mail content - NOTE: this will kill sending of headers.
                    echo "<p>$mail_dump</p>";
                }
            ...
            $email_headers = [];
            if (! empty($user_reply_to)) {	// Allow to reply only to anonymous user and when it is allowed depending on notify user type:
                $email_headers['Reply-To'] = $user_reply_to;
            }

            if ($notify_User) {	// Send the email to registered User:
            ...
            } else {	// Send the email to anonymous user:
            ...
            if ($send_mail_result) {	// Count of successful notified users:
                $notified_users_num++;
            }

            if ($log_messages == 'cron_job') {	// Log mail sending for cron job:
                $user_name = ($notify_User ? $notify_User->get_identity_link() : $notify_user_name . ' <' . $notify_email . '>');
                if ($send_mail_result) {	// Log success mail sending:
                    cron_log_action_end('User ' . $user_name . ' has been notified');
                } else {	// Log failed mail sending:
    ...
    public function send_assignment_notification($executed_by_userid = null)
    {
        ...
        if (! ($commented_Item = &$this->get_Item())) {	// Wrong Comment without Item:
            return $notified_user_IDs;
        }

        if ($commented_Item->assigned_to_new_user &&
            $commented_Item->assigned_user_ID &&
            $executed_by_userid != $commented_Item->assigned_user_ID) {	// Item has assigned user and the assigned user is not the one who posted this Comment:
            $UserCache = &get_UserCache();
            ...
            if (($assigned_User = $commented_Item->get_assigned_User()) &&
                    ...
                    $assigned_User->check_perm('blog_can_be_assignee', 'view', false, $commented_Item->get_blog_ID())) {	// Assigned user wants to receive post assignment notifications and can be assigned to items of this Item's collection:
                ...
                // Send the email:
                if (send_mail_to_User($assigned_User->ID, $subject, 'comment_assignment', $email_template_params)) {	// A send notification email request to the assigned user was processed:
     ...
     */
    public function get_author_type($author_User, $author_email = null)
    {
        if (! isset($this->item_authors)) {	// Load it once and store into a cache array to don't load twice:
            global $DB;
            ...
            if ($comment_item_Blog->get_setting('threaded_comments')) {	// If the threaded comments are enabled for the collection:
                $parent_comment_ID = $this->get('in_reply_to_cmt_ID');
                while ($parent_comment_ID !== null) {	// Find all parents from this comment to root:
                    ...
                    if ($parent_comment = $DB->get_row($SQL)) {	// If parent comment is detected:
            ...
            foreach ($other_authors as $other_author_ID_or_email) {
                if (! isset($this->item_authors[$other_author_ID_or_email])) {	// Mark such author as normal commenter:
                    $this->item_authors[$other_author_ID_or_email] = 'normal';
                }
            }
        }

        if ($author_User && isset($this->item_authors[$author_User->ID])) {	// If a registered user posted at least one comment in the same item where this comment is posted:
            return $this->item_authors[$author_User->ID];
        } elseif ($author_email && isset($this->item_authors[$author_email])) {	// If anonymous user posted at least one comment in the same item where this comment is posted:
     ...
     */
    public function send_vote_spam_emails()
    {
        global $current_User, $Settings, $UserSettings;

        if (! is_logged_in()) {	// Only loggen in users can vote on comments
            return;
        }

        if ($this->is_meta()) {	// Internal comments have no spam voting
            return;
        }
        ...
        foreach ($moderators_to_notify as $moderator) {
            ...
            if ($notify_moderator) {	// Include user to notify because of enabled setting:
                $moderators[] = $moderator->user_ID;
            }
        }
        if ($UserSettings->get('notify_spam_cmt_moderation', $coll_owner_User->ID) && is_email($coll_owner_User->get('email'))) {	// Include collection owner:
        ...
        foreach ($moderators as $moderator_ID) {
            if ($moderator_ID == $current_User->ID) {	// Don't send email to the voter:
                continue;
            }
            $moderator_User = $UserCache->get_by_ID($moderator_ID, false);
            if ($moderator_User && $moderator_User->check_perm('comment!CURSTATUS', 'edit', false, $this)) {	// If moderator has a permission to edit this comment:
     ...
     */
    public function handle_qm_secret($save_comment = false)
    {
        $comment_Item = &$this->get_Item();
        $comment_Item->load_Blog();
        if ($comment_Item->Blog->get_setting('comment_quick_moderation') == 'expire') { // comment secret expires after first comment moderation
     ...
     */
    public function get_frontoffice_statuses()
    {
        if (! ($comment_Item = &$this->get_Item()) ||
            ! ($comment_blog_ID = $comment_Item->get_blog_ID())) {	// Comment's collection ID must be detected to get front-office comment statuses:
            return [];
        }
     ...
     */
    public function may_be_seen_in_frontoffice()
    {
    ...
    public function dbupdate()
    {
        ...
        if (isset($this->previous_status)) {	// Restrict comment status by parent item:
        ...
        if (isset($Plugins)) {	// Note: Plugins may not be available during maintenance, install or test cases
            $Plugins->trigger_event('PrependCommentUpdateTransact', $params = [
                'Comment' => &$this,
            ]);
        }

        if (($r = parent::dbupdate()) !== false) {
            if (isset($dbchanges['comment_content']) || isset($dbchanges['comment_renderers'])) {	// Delete a prerendered content if content or text renderers have been updated:
                $this->delete_prerendered_content();
            }

            $update_item_contents_last_updated_date = false;
            if ($this->may_be_seen_in_frontoffice()) {	// Update contents last update date of the comment's post ONLY when the updated comment may be seen in frontoffice:
                if (isset($dbchanges['comment_content']) ||
                    isset($dbchanges['comment_rating']) ||
                    isset($dbchanges['comment_item_ID']) ||
                    (isset($dbchanges['comment_status']) && isset($this->previous_status) && ! $this->can_be_displayed($this->previous_status))) {	// AND if content, rating or parent Item have been updated
            ...
            if (! empty($this->previous_item_ID)) {	// If comment has been moved from another post:
                if ($previous_Item = &$ItemCache->get_by_ID($this->previous_item_ID, false, false)) {	// Update ONLY last touched date of previous item:
                    $previous_Item->update_last_touched_date(false, true);
                    if ($refresh_previous_item_contents_last_updated_date) {	// Refresh contents last updated ts of the previous parent Item if this Comment was the latest FRONT-OFFICE VISIBLE Comment of the previous parent Item:
            ...
            if ($refresh_parent_item_contents_last_updated_date) {	// Refresh contents last updated ts of the parent Item:
    ...
    public function dbinsert()
    {
        ...
        $spam_karma = $Plugins->trigger_karma_collect('GetSpamKarmaForComment', [
            'Comment' => &$this,
        ]);
        ...
        if (! is_null($spam_karma)) {
            if ($spam_karma < $Settings->get('antispam_threshold_publish')) { // Publish:
                $this->set('status', 'published');
            } elseif ($spam_karma > $Settings->get('antispam_threshold_delete')) { // Delete/No insert:
                return false;
            }
        }
        ...
        if ($comment_Blog->get_setting('comment_quick_moderation') != 'never') { // quick moderation is permitted, set comment secret
        ...
        if (isset($Plugins)) {	// Note: Plugins may not be available during maintenance, install or test cases
            $Plugins->trigger_event('PrependCommentInsertTransact', $params = [
                'Comment' => &$this,
            ]);
        }
        ...
        if ($r = parent::dbinsert()) {	// The comment object could be inserted
            // Link attachments from temporary object to new created Comment:
            $this->link_from_Request();

            if (isset($this->user_notify) && $this->user_notify && $this->author_user_ID > 0) {	// Subscribe user to the comment's post replies if such option has been selected on the comment submitted form:
                global $DB;
     ...
     * @return boolean true on success
     */
    public function dbdelete($force_permanent_delete = false, $use_transaction = true)
    {
        global $Plugins, $DB;
        ...
        if ($this->status != 'trash') { // The comment was not recycled yet
            if ($this->has_replies()) { // Move the replies to the one level up
        ...
        if ($force_permanent_delete || ($this->status == 'trash') || $this->is_meta()) {	// Permamently delete comment from DB:
        ...
        } else { // don't delete, just move to the trash:
            $this->set('status', 'trash');
            $r = $this->dbupdate();
        }

        if ($r) {
            if ($this->ID == 0) {	// Update only last touched date of item if comment was deleted from DB,
            ...
            if ($refresh_parent_item_contents_last_updated_date) {	// Refresh contents last updated ts of parent Item if this Comment was the latest Comment of parent Item:
                $comment_Item->refresh_contents_last_updated_ts();
            }
        } else {
            if ($use_transaction) {
     ...
     * @param string class name
     */
    public function reply_link($before = ' ', $after = ' ', $text = '#', $title = '#', $class = '')
    {
        if (empty($this->ID)) {	// Happens in Preview
            return false;
        }
        ...
        if (! $this->Item->Blog->get_setting('threaded_comments')) {	// A blog's setting is OFF for replying to the comment
            return false;
        }

        if (! $this->Item->can_comment(null)) {	// If current User cannot create a comment for the Item:
            return false;
        }
        ...
        $class .= ' comment_reply';
        if ($this->ID == $comment_reply_ID) {	// This comment is using for replying now
            $class .= ' active';
        }
        $class = ' class="' . trim($class) . '"';

        // Initialize an url to reply on comment:
        if (is_admin_page()) {	// for back-office:
            global $admin_url;
    ...
    public function has_replies()
    {
        ...
        if (! isset($cache_comments_has_replies)) { // Init an array to cache
            $cache_comments_has_replies = [];
        }

        if (! isset($cache_comments_has_replies[$this->item_ID])) { // Get all comments that have the replies from DB (first time)
            global $DB;
     ...
     */
    public function set_last_touched_date()
    {
        global $localtimenow;
     ...
     */
    public function update_last_touched_date($update_item_last_touched_ts = true, $update_item_contents_last_updated_ts = false)
    {
        global $localtimenow;

        if ($this->is_meta()) { // Don't touch Item when this Comment is meta
            return;
        }

        $comment_Item = &$this->get_Item();

        if (empty($comment_Item)) { // Don't execute the following code because this comment is broken
            return;
        }

        $timestamp = date2mysql($localtimenow);

        if ($this->ID && ($this->last_touched_ts !== $timestamp)) { // If the comment was not deleted then update last touched date
            $this->set_param('last_touched_ts', 'date', $timestamp);
            $this->dbupdate();
        }

        if ($update_item_last_touched_ts || $update_item_contents_last_updated_ts) {	// Update last touched timestamp or content last update timestamp of the Item:
     ...
     */
    public function get_permanent_item_link($params = [])
    {
        $params = array_merge([
            'text' => '#item#',
            'title' => '#',
     ...
     */
    public function get_child_comment_IDs($parent_comment_ID = null)
    {
        global $DB;

        if ($parent_comment_ID === null) { // Use current comment ID as main parent ID
        ...
        $comment_IDs = [];
        foreach ($parent_comment_IDs as $comment_ID) { // Get all children recursively
     ...
     */
    public function get_allowed_status($current_status = null)
    {
        $comment_Item = &$this->get_Item();
        $item_Blog = &$comment_Item->get_Blog();

        if ($current_status === null) {	// Use current comment status:
        ...
        if (empty($item_restricted_status)) {	// If max allowed status is not detected because for example current User has no perm to item status,
        ...
        foreach ($visibility_statuses as $visibility_status => $visibility_status_title) {
            if ($status_order == 0) {	// Set max allowed comment status:
                $max_allowed_comment_status = $visibility_status;
            }
            if ($visibility_status == $current_status) {	// Set an order for current status of this comment:
                $comment_status_order = $status_order;
            }
            if ($visibility_status == $item_restricted_status) {	// Set an order for max allowed status of the comment's item:
                $item_status_order = $status_order;
            }
            $status_order++;
        }

        if ($comment_status_order === null) {	// Current comment status is higher than max allowed by parent item,
            // So restrict it by max allowed for comments:
            $comment_restricted_status = $max_allowed_comment_status;
        } elseif ($comment_status_order < $item_restricted_status) {	// Restrict comment status to max allowed by parent item:
            $comment_restricted_status = $item_restricted_status;
        } else {	// Don't restrict because current comment status is allowed:
     ...
     */
    public function restrict_status($update_status = false)
    {
        ...
        if ($this->is_meta()) {	// Internal comment:
            if (! is_logged_in() || ($commented_Item && ! check_user_perm('meta_comment', 'view', false, $commented_Item->get_blog_ID()))) {	// Change internal comment status to 'protected' if user has no perm to view them:
                $comment_allowed_status = 'protected';
            } else {	// Do not restrict if internal comment and user has the proper permission:
                $comment_allowed_status = $current_status;
            }
        } else {	// Restrict status of normal comment to max allowed by parent item:
            $comment_allowed_status = $this->get_allowed_status();
            if (empty($comment_allowed_status) && $commented_Item && ($item_Blog = &$commented_Item->get_Blog())) {	// If min allowed status is not found then use what default status is allowed:
        ...
        } else {	// Only change status to update it on the edit forms and Display a warning:
            $this->status = $comment_allowed_status;

            if ($current_status != $this->get('status') && ! $this->is_meta()) {	// If current comment status cannot be used because it is restricted by parent item:
                global $Messages;
                ...
                $Messages->add(sprintf(
                    T_('Since the parent post of this comment have its visibility set to "%s", the visibility of this comment will be restricted to "%s".'),
                    $visibility_statuses[$item_restricted_status],
                    $visibility_statuses[$this->status]
                ), 'warning');
            }
        }
    }
     ...
     * @return boolean
     */
    public function can_be_displayed($status = null)
    {
        if (empty($this->ID)) {	// Comment is not created yet, so it cannot be displayed:
            return false;
        }
        ...
        if ($status === null) {	// Use current status of this comment:
     ...
     * @return integer|null
     */
    public function get_inlist_order()
    {
        if (empty($this->ID)) {	// This comment must exist in DB
            return null;
        }

        global $CommentList;

        if (empty($CommentList)) {	// Comment list must be initialized globally
            return null;
        }

        if (! isset($CommentList->inlist_orders[$this->ID])) {	// Order number is not found in list for this comment:
            return null;
        }
    ...
    public function link_from_Request()
    {
        ...
        if ($this->ID == 0) {	// The comment must be stored in DB:
            return;
        }
        ...
        if (($this->get('type') != 'meta' && $TemporaryID->type != 'comment') ||
            ($this->get('type') == 'meta' && $TemporaryID->type != 'metacomment')) {	// Wrong temporary object:
            return;
        }
        ...
        foreach ($LinkOwner->Links as $comment_Link) {
            if ($comment_File = &$comment_Link->get_File() &&
                ...
                if (! file_exists($comment_FileRoot->ads_path . 'quick-uploads/c' . $this->ID . '/')) {	// Create if folder doesn't exist for files of new created comment:
        ...
        if (isset($tmp_folder_path) && file_exists($tmp_folder_path)) {	// Remove temp folder from disk completely:
    ...
    {
        if ($commnet_Item = &$this->get_Item()) {	// Call same function of the comment's Item for proper permission checking:
