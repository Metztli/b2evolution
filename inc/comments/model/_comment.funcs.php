...
function generic_ctp_number($post_id, $mode = 'comments', $status = 'published', $count_expired = false, $filter_by_perm = true)
{
    ...
    if ($preview) { // we are in preview mode, no comments yet!
        return 0;
    }

    $show_statuses = (is_admin_page() || (! $filter_by_perm)) ? get_visibility_statuses('keys', ['trash', 'redirected']) : get_inskin_statuses($blog, 'comment');
    $filter_index = $filter_by_perm ? 0 : 1;
    if (! isset($cache_ctp_number) || ! isset($cache_ctp_number[$filter_index][$post_id])) { // we need a query to count comments
        $count_SQL = new SQL();
    ...
    if (! isset($cache_ctp_number) || ! isset($cache_ctp_number[$filter_index])) {
        ...
        if (! empty($postIDlist)) {	// This can happen when displaying a featured post of something that's not in the MainList
            foreach ($postIDarray as $tmp_post_id) {	// Initializes each post to nocount!
                $cache_ctp_number[$filter_index][$tmp_post_id] = [
            ...
            foreach ($DB->get_results($countall_SQL->get()) as $row) {
                ...
                if ($row->comment_type != 'meta') { // Exclude internal comments from feedbacks
    ...
    if (! isset($cache_ctp_number[$filter_index][$post_id])) { // this should be extremely rare...
        // echo "CACHE not set for $post_id";

        // Initializes post to nocount!
        $cache_ctp_number[$filter_index][intval($post_id)] = [
            'comments' => $statuses_array,
            'trackbacks' => $statuses_array,
            'pingbacks' => $statuses_array,
            'feedbacks' => $statuses_array,
    ...
    if (! in_array($mode, ['comments', 'trackbacks', 'pingbacks', 'metas', 'webmentions'])) {
        $mode = 'feedbacks';
    }

    if (is_array($status)) { // $status is an array and probably contains more then one visibility status
        $result = 0;
    ...
    } elseif (isset($cache_ctp_number[$filter_index][$post_id][$mode][$status])) { // $status is a string with one visibility status
        $result = $cache_ctp_number[$filter_index][$post_id][$mode][$status];
    } else { // $status is not recognized return total feedback number
...
function echo_comment_buttons($Form, $edited_Comment)
{
    ...
    if ($edited_Comment->is_meta()) { // Internal comments don't have a status, Display only one button to update
        $Form->submit(['actionArray[update]', T_('Save Changes!'), 'SaveButton']);
    } else { // Normal comments have a status, Display the buttons to change it and update
        if ($edited_Comment->status != 'trash') {
            ...
            if ($edited_Comment->is_meta()) {	// Don't restrict statuses for internal comments:
                $restricted_statuses = [];
            } else {	// Restrict statuses for normal comments:
            ...
            $exclude_statuses = array_merge($restricted_statuses, ['redirected', 'trash']);
        ...
        // ---------- SAVE BUTTONS ----------
        $Form->submit(['actionArray[update_edit]', /* TRANS: This is the value of an input submit button */ T_('Save & edit'), 'SaveEditButton btn-status-' . $edited_Comment->status]);
        $Form->submit(['actionArray[update]', T_('Save'), 'SaveButton btn-status-' . $edited_Comment->status]);
...
function echo_comment_status_buttons($Form, $edited_Comment = null, $max_allowed_status = '', $action = 'update')
{
    ...
    if ($edited_Comment !== null) {	// If the edited comment is defined, e-g on edit form:
        if ($edited_Comment->is_meta()) {	// Don't suggest to change a status of internal comment:
            $Form->submit(['actionArray[' . $action . ']', T_('Save Changes!'), 'SaveButton', '']);
            return;
        }

        $comment_Item = &$edited_Comment->get_Item();
        $comment_status = $edited_Comment->status;
    } else {	// If comment is not defined, e-g on action for several comments from list:
        $comment_Item = null;
    ...
    $exclude_statuses = array_merge(get_restricted_statuses($Blog->ID, 'blog_comment!', 'edit', $comment_status, $restrict_max_allowed_status), ['redirected', 'trash']);
    // Get allowed visibility statuses:
    ...
    if (! isset($status_options[$comment_status])) {	// Check if the status is allowed for current User:
        foreach ($status_options as $status_key => $status_title) {
...
{
    if (! check_user_perm('spamblacklist', 'edit')) { // Current user has no permission to edit the spam contents
        return $content;
    }

    if (stristr($content, '<code') !== false || stristr($content, '<pre') !== false) { // Add icons only outside <code> and <pre>
        return callback_on_non_matching_blocks(
            $content,
...
function add_ban_icons_callback($content)
{
    ...
    $used_urls = [];
    while (isset($urls[$i])) { // there is unprocessed url
        $url = $urls[$i];
        if (validate_url($url, 'posting', false)) { // skip not valid urls
            $i++;
            continue;
        }
        if (in_array($url, $used_urls)) { // skip already passed url
            $i++;
            continue;
        }
        $used_urls[] = $url;
        while (isset($imgtags[$k]) && (strpos($content, $imgtags[$k]) < $from)) { // skip already passed img tags
            $k++;
        }
        ...
        if (isset($imgtags[$k]) && (strpos($imgtags[$k], $url) !== false)
            && ($pos > strpos($content, $imgtags[$k], $from))) { // current url is inside the img tag, we need to skip this url.
        ...
        // check a tags
        if (isset($atags[$j])) { // there is unprocessed "a" tag
            $tag = $atags[$j];
            if ((($urlpos = strpos($tag, $url)) !== false) && ($pos > strpos($content, $tag, $from))) { // the url is inside the current tag, we have to add ban icon after the tag
                $pos = strpos($content, $tag, $from);
                $length = strlen($tag);
                while (isset($urls[$i]) && (($urlpos = strpos($tag, $urls[$i], $urlpos + 1)) !== false)) { // skip all other urls from this tag
                    $i++;
                }
                $j++;
            }
 ...
 */
function get_opentrash_link($check_perm = true, $force_show = false, $params = [])
{
    ...
    if ($show_recycle_bin && (! $force_show)) { // get number of trash comments:
    ...
    if ($show_recycle_bin) { // show "Open recycle bin"
        global $CommentList;
        ...
        $result .= action_icon(
            T_('Open recycle bin'),
            'recycle_full',
            $admin_url . '?ctrl=comments&amp;blog=' . $blog . '&amp;' . $comment_list_param_prefix . 'show_statuses[]=trash',
            ' ' . T_('Open recycle bin'),
            5,
            3,
            [
 ...
 */
function echo_disabled_comments($allow_comments_value, $item_url, $params = [])
{
    global $Settings;
    ...
    $params['form_params'] = array_merge([
    ...
    } elseif (check_user_status('can_be_validated')) { // logged in but the account is not activated
    ...
    } else { // not logged in, add login and register links
...
function save_comment_to_session($Comment, $kind = 'unsaved', $type = '')
{
    ...
    if ($type != 'meta') {	// Use default type if it is not allowed:
        $type = '';
    }
...
function get_comment_from_session($kind = 'unsaved', $type = '')
{
    ...
    if (! isset($b2evo_session_comments[$session_var_name])) {	// Set Comment to cache array:
        if (! is_array($b2evo_session_comments)) {	// Initialize array for caching:
            $b2evo_session_comments = [];
        }

        if (($Comment = $Session->get($session_var_name)) && $Comment instanceof Comment) {	// If Comment is detected for current Session:
            // Delete Comment to clear Session data:
            $Session->delete($session_var_name);
        } else {	// Comment is not detected, Return NULL:
            $Comment = null;
        }
 ...
 * @param integer Level
 */
function display_comment_replies($comment_ID, $params = [], $level = 1)
{
    ...
    if (isset($CommentReplies[$comment_ID])) { // This comment has the replies
        foreach ($CommentReplies[$comment_ID] as $Comment) { // Loop through the replies:
            if (empty($Comment->ID)) { // Get html tag of the comment block of preview
                $comment_start = $Comment->email_is_detected ? $params['comment_error_start'] : $params['preview_start'];
            } else { // Get html tag of the comment block of existing comment
                $comment_start = $params['comment_start'];
            }

            // Set margin left for each sub level comment
            $attrs = ' style="margin-left:' . (20 * $level) . 'px"';
            if (strpos($comment_start, 'class="') === false) { // Add a class attribute for the replied comment
                $attrs .= ' class="replied_comment"';
            } else { // Add a class name for the replied comment
                $comment_start = str_replace('class="', 'class="replied_comment ', $comment_start);
            }
            $comment_start = str_replace('>', $attrs . '>', $comment_start);

            if (! empty($Comment->ID)) { // Comment from DB
                skin_include($params['comment_template'], array_merge($params, [
                    'Comment' => &$Comment,
                    'comment_start' => $comment_start,
                ]));
            } else { // PREVIEW comment
                skin_include($params['comment_template'], array_merge($params, [
                    'Comment' => &$Comment,
...
function echo_comment_replies($comment_ID, $params, $level = 1)
{
    ...
    if (! isset($CommentReplies[$comment_ID])) {	// This comment has no replies, Exit here:
        return false;
    }
...
function echo_comment_reply_js($Item)
{
    ...
    if (! isset($Blog) || ! $Blog->get_setting('threaded_comments')) {	// If threaded comments are not enabled for current Collection:
        return false;
    }
...
{
    if (! check_user_perm('blogs', 'all')) {	// Check permission
        return false;
    }

    if ($CommentList->get_total_rows() == 0) {	// No comments for current list
        return false;
    }
...
function check_autocomplete_usernames($Comment)
{
    ...
    if ($Comment->is_meta()) {	// Always enable autocomplete username for internal comments
        return true;
    } elseif (is_admin_page() && ! empty($Blog)) {	// Check setting in the Back office or when Blog is not defined
...
function display_comment_mass_delete($CommentList)
{
    ...
    if ($action != 'mass_delete') {	// Display this form only for action "mass_delete"
        return;
    }

    if (! check_comment_mass_delete($CommentList)) {	// Form is not available
        return;
    }
...
{
    if ($mass_type != 'recycle' && $mass_type != 'delete') { // Incorrect action
        return;
    }
    ...
    while (! empty($deletable_comment_ids)) {
        ...
        while (($iterator_Comment = &$CommentCache->get_next()) != null) { // Delete all comments from CommentCache
...
function move_child_comments_to_item($comment_IDs, $item_ID, $convert_to_root = true)
{
    ...
    if (empty($child_comments_IDs)) { // No child comments, Exit here
        return;
    }
    ...
    if ($convert_to_root) { // Make these comments as root comments (remove their parent depending)
 ...
 * @param array Params
 */
function comments_results_block($params = [])
{
    ...
    if ($params['action'] != 'view' && $comments_Results->get_total_rows() > 0 && $edited_User->has_comment_to_delete()) {	// Display action icon to delete all records if at least one record exists & current user can delete at least one comment posted by user
 ...
 * @param array Params
 */
function comments_results(&$comments_Results, $params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
        'field_prefix' => '',
    ...
    if ($params['display_item']) {	// Display Date column
        $td = '%{Obj}->get_permanent_item_link()%';
        if ($params['display_status']) { // Dislpay status banner
    ...
    if ($params['display_kind']) {	// Display Kind column
        $comments_Results->cols[] = [
            'th' => T_('Kind'),
    ...
    if ($params['display_url']) {	// Display Url column
        $comments_Results->cols[] = [
            'th' => T_('URL'),
    ...
    if ($params['display_email']) {	// Display Email column
        $comments_Results->cols[] = [
            'th' => T_('Email'),
    ...
    if ($params['display_ip']) {	// Display IP column
        $comments_Results->cols[] = [
            'th' => T_('IP'),
    ...
    if ($params['display_additional_columns']) {	// Display additional columns from the Plugins
        global $Plugins;
    ...
    if ($params['display_spam']) {	// Display Spam karma column
        $comments_Results->cols[] = [
            'th' => T_('Spam karma'),
    ...
    if ($params['display_visibility']) {	// Display Visibility column
        $comments_Results->cols[] = [
            'th' => T_('Visibility'),
...
function echo_comment_change_item_js()
{
    ...
    if (! empty($evo_comment_change_item_js_initialized)) {	// Don't initialize this JS code twice on same page:
        return;
    }
...
function get_author($Comment)
{
    if ($Comment->can_be_displayed() || check_user_perm('comment!CURSTATUS', 'moderate', false, $Comment)) {
        ...
        if ($author_User != null) {	// author is a registered user
...
function get_author_ip($Comment, $param_prefix = '')
{
    if (check_user_perm('comment!CURSTATUS', 'moderate', false, $Comment)) {
        if (empty($Comment->author_IP)) {
            ...
            $antispam_link = ' ' . implode(', ', get_linked_ip_list([$Comment->author_IP], null, $antispam_icon));
...
function comment_edit_actions($Comment)
{
    ...
    if ($user_has_edit_perm || $user_has_delete_perm) { // Display edit and delete button if current user has the rights:
        $redirect_to = rawurlencode(regenerate_url('comment_ID,action', 'filter=restore', '', '&'));

        if ($user_has_edit_perm) { // Display edit button only if current user can edit comment with current status
            $Comment->get_Item();
        ...
        if ($user_has_delete_perm) { // Display delete/recycle button because current user has permission to delete/recycle this comment
            $params = [];
            if ($Comment->status == 'trash') { // Comment is already in the recycle bin, display delete action and add js confirm
                $title = T_('Delete this comment') . '!';
                $params['onclick'] = "return confirm('" . TS_('You are about to delete this comment!\\nThis cannot be undone!') . "')";
            } else { // Comment will be moved into the recycle bin
