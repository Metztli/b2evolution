...
class CommentQuery extends SQL
{
    ...
    public $include_emptyurl;

    public $author_IP;

    public $rating_toshow;
     ...
     */
    public function __construct($dbtablename = 'T_comments', $dbprefix = 'comment_', $dbIDname = 'comment_ID')
    {
     ...
     */
    public function where_ID($c = '', $author = '')
    {
        $r = false;

        $this->c = $c;
        if (empty($this->author)) { // Change $this->author only if it is empty, because possible that it was set previously
            $this->author = $author;
        }
     ...
     */
    public function where_ID_list($cl = '')
    {
        $this->cl = clear_ids_list($cl);

        if (empty($this->cl)) {	// Nothing to filter:
            return;
        }

        if (substr($cl, 0, 1) == '-') {	// List starts with MINUS sign:
            $eq = 'NOT IN';
        } else {
            $eq = 'IN';
        }
     ...
     */
    public function where_post_ID($post)
    {
        $this->post = $post;
        ...
        if (substr($post, 0, 1) == '-') {	// List starts with MINUS sign:
            $eq = 'NOT IN';
     ...
     */
    public function where_comment_date($timestamp_min, $timestamp_max)
    {
        global $time_difference, $DB;

        if (empty($timestamp_min) && empty($timestamp_max)) {	// Don't restrict
            return;
        }
     ...
     */
    public function where_post_datestart($timestamp_min, $timestamp_max)
    {
        if (empty($timestamp_min) && empty($timestamp_max)) {	// Don't restrict
            return;
        }
     ...
     */
    public function where_author($author)
    {
        ...
        if (substr($author, 0, 1) == '-') {	// List starts with MINUS sign:
            $eq = 'NOT IN';
        } else {
            $eq = 'IN';
        }
    ...
    public function where_author_logins($author_logins)
    {
        ...
        if (substr($this->author_login, 0, 1) == '-') {	// Exclude the users IF a list starts with MINUS sign:
            ...
            if (empty($users_IDs)) {	// If users are not found with login, we should not allow selection with all users:
                $users_IDs = '-1';
            }
        }
        if (! empty($users_IDs)) {
     ...
     */
    public function where_author_email($author_email)
    {
        global $DB;
        ...
        if (substr($author_email, 0, 1) == '-') {	// List starts with MINUS sign:
            $eq = 'NOT IN';
     ...
     */
    public function where_author_url($author_url, $url_match, $include_emptyurl)
    {
        global $DB;
        ...
        $include_empty = '';
        if ($include_emptyurl) {	// include comments with no url
     ...
     */
    public function where_author_IP($author_IP)
    {
     ...
     */
    public function where_rating($rating_toshow, $rating_turn, $rating_limit)
    {
        $this->rating_toshow = $rating_toshow;
        $this->rating_turn = $rating_turn;
        $this->rating_limit = $rating_limit;

        if (empty($rating_toshow)) {	//	no nead where condition for ratings
            return;
        }

        $search = '';
        if (in_array('norating', $rating_toshow)) {	//	include comments with no rating
            $search = $this->dbprefix . 'rating IS NULL';
        }

        if (in_array('haverating', $rating_toshow)) {	//	specify search direction
            switch ($rating_turn) {
                case 'exact':
                    $comp = '=';
                    break;
     ...
     */
    public function where_statuses($show_statuses, $filter_by_perm = true)
    {
        global $DB;

        if (empty($show_statuses)) { // initialize if emty
            $show_statuses = get_visibility_statuses('keys', ['trash', 'redirected']);
        }
        $this->show_statuses = $show_statuses;

        if ($filter_by_perm) { // show not published comments corresponding to the given blog perms
     ...
     */
    public function where_keywords($keywords, $phrase, $exact)
    {
        global $DB;
        ...
        if ($exact) {	// We want exact match of author or contents
            $n = '';
        } else { // The words/sentence are/is to be included in in the author or the contents
            $n = '%';
        }

        if (($phrase == '1') or ($phrase == 'sentence')) { // Sentence search
     ...
     */
    public function blog_restrict($Blog)
    {
        $this->Blog = $Blog;
     ...
     */
    public function expiry_restrict($expiry_statuses)
    {
     ...
     */
    public function user_perm_restrict($user_perm, $blog_ID)
    {
        global $current_User, $DB;

        if (! is_logged_in(false)) { // Anonymous users can see only published comments, no need further restriction
            return;
        }

        if (($user_perm !== 'moderate') && ($user_perm !== 'edit')) { // No need furhter restriciton because the user doesn't want to edit/moderate these comments
            return;
        }

        if (check_user_perm('blogs', 'editall')) { // User has global permission one ach blog
            return;
        }

        $BlogCache = &get_BlogCache();
        $Collection = $Blog = $BlogCache->get_by_ID($blog_ID);
        if ($current_User->ID == $Blog->get('owner_user_ID')) { // User is the blog owner, so has permission on edit/moderate each comment
            return;
        }

        if (! $Blog->get('advanced_perms')) { // Blog advanced perm settings is not enabled, user has no permission to edit/moderate comments
     ...
     */
    public function where_dates($ymdhms_min, $ymdhms_max)
    {
        global $DB;
        $this->ymdhms_min = $ymdhms_min;
        $this->ymdhms_max = $ymdhms_max;
        if (! empty($this->ymdhms_min)) {	// Restrict by min date:
        ...
        if (! empty($this->ymdhms_max)) {	// Restrict by max date:
