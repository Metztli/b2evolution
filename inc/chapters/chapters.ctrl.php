...
if (param($ChapterCache->dbIDname, 'integer', null, true, false, false)) {
    if (($edited_Chapter = &$ChapterCache->get_by_ID(${$ChapterCache->dbIDname}, false, true, $subset_ID)) === false) {	// We could not find the element to edit:
...
if (! is_null(param($ChapterCache->dbprefix . 'parent_ID', 'integer', null))) {
    ...
    if ($edited_parent_Chapter === false) { // Parent chapter doesn't exist any longer.
...
function get_chapter_redirect_url($redirect_page, $parent_ID, $chapter_ID = 0)
{
    ...
    if ($redirect_page == 'front' || $redirect_page == 'parent') { // Get Chapter for front page redirect
        if (empty($chapter_ID)) { // Chapter ID is invalid, redirect to chapters list
            $redirect_page = 'list';
        } else {
            $ChapterCache = &get_ChapterCache();
            $Chapter = &$ChapterCache->get_by_ID($chapter_ID, false, false);
            if ($Chapter === false) { // Chapter doesn't exist anymore, redirect to chapters list
                $redirect_page = 'list';
            }
        }
    }
    ...
    switch ($redirect_page) {
        case 'parent':
            ...
            if ($parent_Chapter = &$Chapter->get_parent_Chapter()) {	// If
        ...
        case 'manual':
            ...
            if (! empty($parent_ID)) { // Open parent category to display new created category
...
switch ($action) {
    ...
    case 'create':
        ...
        if ($edited_Chapter->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
            if ($edited_Chapter->dbinsert() !== false) {
                ...
                $Session->set('fadeout_array', [$edited_Chapter->ID]);
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Chapter->check_delete(sprintf(TB_('Cannot delete element &laquo;%s&raquo;'), $edited_Chapter->dget('name')))) {	// There are restrictions:
                $action = 'edit';
            }
        }
        break;
    ...
    case 'make_default':
        ...
        if ($edited_Chapter->get('meta')) {	// If category is meta:
            $Messages->add(TB_('Meta category cannot be used as default!'), 'error');
            break;
        }

        if ($edited_Chapter->get('ityp_ID') === '0') {	// Force "No default type" of default category to "Same as collection default":
            $edited_Chapter->set('ityp_ID', null, true);
            if ($edited_Chapter->dbupdate()) {	// Inform user about this modification:
        ...
        if ($edited_Blog->get_setting('default_cat_ID') != $edited_Chapter->ID) {	// Move all "content-block" items from previous main Category to new:
        ...
        if (! empty($moved_items_num)) {	// Inform about moved items:
    ...
    case 'set_meta':
        ...
        if ($edited_Blog->get_default_cat_ID() == $edited_Chapter->ID) {	// If category is default:
            $Messages->add(TB_('Meta category cannot be used as default!'), 'error');
        ...
        // Save category
        if ($result && $edited_Chapter->dbsave()) { // Category has no posts and it was saved successful
...
if ($action == 'list') { // Load JS to edit chapter order inline
    require_js_defer('customized:jquery/jeditable/jquery.jeditable.js', 'rsc_url');
}

if (in_array($action, ['list', 'edit', 'move'])) {	// Initialize Hotkeys:
    init_hotkeys_js();
}
...
$AdminUI->breadcrumbpath_init(true, [
    ...
    'url' => $admin_url . '?ctrl=colls_settings&amp;tab=dashboard&amp;blog=$blog$',
]);
$AdminUI->breadcrumbpath_add(TB_('Categories'), $admin_url . '?ctrl=chapters&amp;blog=$blog$');

if (in_array($action, ['new', 'edit', 'copy', 'create', 'update'])) {
...
switch ($action) {
    ...
    case 'delete':
        ...
        if ($action == 'delete') {	// We need to ask for confirmation:
