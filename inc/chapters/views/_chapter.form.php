...
$move = '';
if ($Settings->get('allow_moving_chapters') && (! $creating)) { // If moving cats between blogs is allowed:
...
$Form->select_input_options(
    'cat_parent_ID',
    $ChapterCache->recurse_select($edited_Chapter->parent_ID, $subset_ID, true, null, 0, [$edited_Chapter->ID]),
    TB_('Parent category')
);

$Form->text_input('cat_name', $edited_Chapter->name, 40, TB_('Name'), '', [
    'required' => true,
    'maxlength' => 255,
]);

$Form->text_input('cat_urlname', $edited_Chapter->urlname, 40, TB_('URL "slug"'), TB_('Used for clean URLs. Must be unique.'), [
    'maxlength' => 255,
]);
...
$cat_image_params = [
    'file_type' => 'image',
    'max_file_num' => 1,
    'window_title' => TB_('Select category image'),
...
$social_media_image_params = [
    'file_type' => 'image',
    'max_file_num' => 1,
    'window_title' => TB_('Select social media boilerplate image'),
    'size_name' => 'fit-320x320',
];
...
$Form->text_input('cat_description', $edited_Chapter->description, 40, TB_('Description'), TB_('May be used as a title tag and/or meta description.'), [
    'maxlength' => 255,
]);

$parent_cat_order = $edited_Chapter->get_parent_subcat_ordering();
if ($parent_cat_order == 'manual') {
    $Form->text_input('cat_order', $edited_Chapter->order, 5, TB_('Order'), TB_('For manual ordering of the categories.'), [
        'maxlength' => 11,
    ]);
}

$Form->radio_input('cat_subcat_ordering', $edited_Chapter->get('subcat_ordering'), [
    [
        'value' => 'parent',
        'label' => TB_('Same as parent'),
    ],
    [
        'value' => 'alpha',
        'label' => TB_('Alphabetically'),
    ],
    [
        'value' => 'manual',
        'label' => TB_('Manually'),
...
$Form->checkbox_input('cat_meta', $edited_Chapter->meta, TB_('Meta category'), [
    'note' => TB_('If you check this box you will not be able to put any posts into this category.'),
]);
...
$Form->select_input_array('cat_ityp_ID', $edited_Chapter->ityp_ID, $item_type_options, TB_('Default Item Type'), null, [
    'force_keys_as_values' => true,
]);
echo '</div>';

$Form->checkbox_input('cat_lock', $edited_Chapter->lock, TB_('Locked category'), [
    'note' => TB_('Check this to lock all posts under this category. (Note: for posts with multiple categories, the post is only locked if *all* its categories are locked.)'),
]);
