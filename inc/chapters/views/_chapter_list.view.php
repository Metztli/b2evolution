...
function cat_line($Chapter, $level)
{
    ...
    // Default
    if ($Chapter->get('meta')) {	// Deny to use meta chapter as default:
        $makedef_icon = '';
    } elseif ($current_default_cat_ID == $Chapter->ID) {	// This chapter is default currently:
        $makedef_icon = get_icon('enabled', 'imgtag', [
            'title' => format_to_output(T_('This is the default category'), 'htmlattr'),
        ]);
    } else {	// Display action icon to make this chapter default:
        ...
        $makedef_icon = '<a href="' . $makedef_url . '" title="' . $makedef_title . '">' . get_icon('disabled', 'imgtag', [
            'title' => $makedef_title,
        ]) . '</a>';
    }
    ...
    // Name
    if ($permission_to_edit) {	// We have permission permission to edit:
    ...
    // Order
    if ($Chapter->get_parent_subcat_ordering() == 'manual') { // Manual ordering
    ...
    if ($permission_to_edit) {	// We have permission permission to edit, so display these columns:
        // Meta
        if ($current_default_cat_ID == $Chapter->ID) {	// Deny to use default chapter as meta:
            $makemeta_icon = false;
        } elseif ($Chapter->meta) {	// This chapter is meta:
            $makemeta_icon = 'enabled';
            $makemeta_title = format_to_output(T_('Click to revert this from meta category'), 'htmlattr');
            $action = 'unset_meta';
        } else {	// This chapter is NOT meta:
            $makemeta_icon = 'disabled';
            $makemeta_title = format_to_output(T_('Click to make this as meta category'), 'htmlattr');
            $action = 'set_meta';
        }
        if ($makemeta_icon) {	// Display action icon to change meta property of this chapter:
            $makemeta_url = regenerate_url('action,cat_ID', 'cat_ID=' . $Chapter->ID . '&amp;action=' . $action . '&amp;' . url_crumb('element'));
            $r .= '<td class="center"><a href="' . $makemeta_url . '" title="' . $makemeta_title . '">' . get_icon($makemeta_icon, 'imgtag', [
                'title' => $makemeta_title,
            ]) . '</a></td>';
        } else {
            $r .= '<td></td>';
        }
        ...
        // Default Item Type:
        if ($Chapter->get('meta')) {	// Don't allow default Item Type for meta category because it cannot has items:
            $r .= '<td>&nbsp;</td>';
        } else {	// Normal category can has items, Allow to change its default Item Type:
            if ($Chapter->get('ityp_ID') === null) {	// Same as collection default:
                $cat_item_type_name = T_('Same as collection default');
            } elseif ($Chapter->get('ityp_ID') == '0') {	// No default type:
                $cat_item_type_name = '<b>' . T_('No default type') . '</b>';
            } elseif (($ItemTypeCache = &get_ItemTypeCache()) &&
                            ($cat_ItemType = &$ItemTypeCache->get_by_ID($Chapter->get('ityp_ID'), false, false))) {	// Custom Item Type:
                $cat_item_type_name = $cat_ItemType->get('name');
                if (! $cat_ItemType->is_enabled($Chapter->get('blog_ID'))) {	// Mark not enabled Item Type with red color:
                    $cat_item_type_name = '<span class="red">' . $cat_item_type_name . '</span>';
                }
            } else {	// Not found Item Type in DB:
                $cat_item_type_name = '<span class="red">' . T_('Not Found') . ' #' . $Chapter->get('ityp_ID') . '</span>';
            }
            if (($cat_Blog = &$Chapter->get_Blog()) &&
                $cat_Blog->get_default_cat_ID() == $Chapter->ID) {	// For default category use a separate options without "No default type":
                $cat_cell_edit_class = 'default_cat_ityp_ID_edit';
            } else {	// For not default categories use full options list:
        ...
        $r .= '<td class="center"><a href="' . $makelock_url . '" title="' . $makelock_title . '">' . get_icon($makelock_icon, 'imgtag', [
            'title' => $makelock_title,
        ]) . '</a></td>';
    }
    ...
    } else {	// no posts in this category
        $r .= '<td class="center"> - </td>';
    }

    // Actions
    $r .= '<td class="lastcol shrinkwrap">';
    if ($permission_to_edit) {	// We have permission permission to edit, so display action column:
...
$Table->global_icon(T_('Refresh'), 'refresh', regenerate_url('action,' . $ChapterCache->dbIDname), T_('Refresh'), 3, 4, [
    'class' => 'action_icon btn-warning',
]);
$Table->global_icon(T_('Create a new category...'), 'new', regenerate_url('action,' . $ChapterCache->dbIDname, 'action=new'), T_('New category') . ' &raquo;', 3, 4, [
...
echo $ChapterCache->recurse($callbacks, $subset_ID, null, 0, 0, [
    'sorted' => true,
]);
...
if ($permission_to_edit) {	// Print JS to edit default Item Type of category:
