...
class Chapter extends DataObject
{
     ...
     * Social media image
     */
    public $social_media_image_file_ID;
     ...
     */
    public function __construct($db_row = null, $subset_ID = null)
    {
    ...
    public static function get_delete_restrictions()
    {
        return [
            ...
            [
                'table' => 'T_items__item',
                'fk' => 'post_main_cat_ID',
                'msg' => T_('%d posts within category through main cat'),
                'class' => 'Item',
            ...
            [
                'table' => 'T_postcats',
                'fk' => 'postcat_cat_ID',
                'msg' => T_('%d posts within category through extra cat'),
            ],
        ];
    }
     ...
     */
    public static function compare_chapters($a_Chapter, $b_Chapter)
    {
        if ($a_Chapter == null || $b_Chapter == null) {
            debug_die('Invalid category objects received to compare.');
        }

        if ($a_Chapter->ID == $b_Chapter->ID) { // The two chapters are the same
            return 0;
        }

        if ($a_Chapter->blog_ID != $b_Chapter->blog_ID) { // Sort based on the ordering between different blogs
            $a_Chapter->load_Blog();
            $b_Chapter->load_Blog();
        ...
        if ($a_Chapter->parent_ID != $b_Chapter->parent_ID) { // Two chapters from the same blog, but different parrents
            ...
            while (isset($path_to_root_a[$index]) && isset($path_to_root_b[$index])) {
                if ($path_to_root_a[$index] != $path_to_root_b[$index]) { // The first different parent on the same level was found, compare parent objects
        ...
        switch ($cat_order) {
            ...
            case 'manual':
                if ($a_Chapter->order === null) { // NULL values are greater than any number
                    $result = ($b_Chapter->order !== null) ? 1 : 0;
                    break;
                }

                if ($b_Chapter->order === null) { // NULL values are greater than any number, so a is lower than b
                    $result = -1;
                    break;
                }
        ...
        if ($result == 0) { // In case if the order fields are equal order by ID
    ...
    {
        if ($this->children_sorted) { // Category children list is already sorted
            return;
        }

        // Sort children list
        uasort($this->children, ['Chapter', 'compare_chapters']);
    }
     ...
     */
    public function get_children($sorted = false)
    {
        if ($sorted) { // sort childrens if required
            $this->sort_children();
        }
    ...
    public function load_from_request()
    {
        ...
        if (param('cat_parent_ID', 'integer', -1) !== -1) {	// Set parent ID:
        ...
        if ($cat_Blog && $cat_Blog->get_setting('category_ordering') == 'manual') { // Manual ordering
        ...
        if ($this->has_posts() && $cat_meta) {	// Display error message if we want make the meta category from category with posts
            global $Messages;
        ...
        } else {	// Save the category as 'Meta' only if it has no posts
    ...
    public function &get_parent_Chapter()
    {
        if (! isset($this->parent_Chapter)) {	// Not resoleved yet!
    ...
    public function get_url_path()
    {
        ...
        if (! is_null($parent_Chapter)) {	// Recurse:
     ...
     */
    public function get_permanent_url($link_type = null, $blogurl = null, $paged = 1, $chapter_posts_per_page = null, $glue = '&amp;')
    {
        global $DB, $cacheweekly, $Settings;

        if (empty($link_type)) {	// Use default from settings:
            $this->get_Blog();
        ...
        switch ($link_type) {
            case 'param_num':
                ...
                if (empty($chapter_posts_per_page)) {	// Use default from Blog
                    $this->get_Blog();
                    $chapter_posts_per_page = $this->Blog->get_setting('chapter_posts_per_page');
                }
                if (! empty($chapter_posts_per_page) && $chapter_posts_per_page != $this->Blog->get_setting('posts_per_page')) {	// We want a specific post per page count:
     ...
     */
    public function get_subcat_ordering($actual_ordering = true)
    {
     ...
     */
    public function get_parent_subcat_ordering()
    {
        if (empty($this->parent_ID)) { // Return the default blog setting
            $this->load_Blog();
    ...
    public function dbinsert()
    {
        ...
        if (parent::dbinsert()) { // The chapter was inserted successful
            $DB->commit();
            return true;
        }
    ...
    public function dbupdate()
    {
        ...
        // validate url title / slug
        if (empty($this->urlname) || isset($this->dbchanges['cat_urlname'])) { // Url title has changed or is empty
            $this->set('urlname', urltitle_validate($this->urlname, $this->name, $this->ID, false, $this->dbprefix . 'urlname', $this->dbIDname, $this->dbtablename));
        }

        if (count($this->dbchanges) > 0 && ! isset($this->dbchanges['last_touched_ts'])) { // Update last_touched_ts field only if it wasn't updated yet and the datemodified will be updated for sure.
            global $localtimenow;
            $this->set_param('last_touched_ts', 'date', date('Y-m-d H:i:s', $localtimenow));
        }

        if (parent::dbupdate() === false) { // The update was unsuccessful
            $DB->rollback();
            return false;
        }
     ...
     * @return string
     */
    public function get_name($format = 'htmlbody')
    {
    ...
    public function has_posts()
    {
        ...
        if ($this->ID == 0) {	// New category has no posts
            return false;
        }
     ...
     */
    public function update_last_touched_date()
    {
        global $localtimenow;
     ...
     * @param array
     */
    public function get_permanent_link($params = [])
    {
     ...
     */
    public function get_last_touched_date($format = '', $useGM = false)
    {
        if (empty($format)) {
    ...
    public function get_edit_url($params = [])
    {
        ...
        if (! $this->ID) { // New chapter
            return false;
        }

        if (! check_user_perm('admin', 'restricted') ||
            ! check_user_perm('blog_cats', '', false, $this->blog_ID)) { // User has no right to edit this chapter
            return false;
        }
    ...
    public function get_edit_link($params = [])
    {
        ...
        if ($params['title'] == '#') {
            $params['title'] = T_('Edit this chapter...');
        }
     ...
     * @return string
     */
    public function get_path_name($params = [])
    {
        $params = array_merge([
            'separator' => ' / ',
        ]);
        ...
        while ($parent_Chapter) {	// Append all parent chapters name:
    ...
    public function set($parname, $parvalue, $make_null = false)
    {
        switch ($parname) {
            ...
            case 'ityp_ID':
                if ($this->get('meta')) {	// Don't allow default Item Type for meta category because it cannot has items:
                    $parvalue = null;
                } elseif (($parvalue === 0 || $parvalue === '0') &&
                        $this->ID > 0 &&
                        ($cat_Blog = &$this->get_Blog()) &&
                        $cat_Blog->get_default_cat_ID() == $this->ID) {	// Force "No default type" of default category to "Same as collection default":
                    $parvalue = null;
                }
                return $this->set_param($parname, 'integer', $parvalue, true);
     ...
     */
    public function add_child_category(&$Chapter)
    {
        if (! isset($this->children[$Chapter->ID])) {	// Add only if it was not added yet:
    ...
    public function &get_image_File()
    {
        ...
        if (! ($cat_image_File = &$FileCache->get_by_ID($this->get('image_file_ID'), false, false))) {	// This chapter has no image file or it is broken:
            $r = null;
            return $r;
        }

        if (! $cat_image_File->is_image()) {	// The file must be an image:
            $r = null;
            return $r;
        }
    ...
    public function get_image_url($params = [])
    {
        ...
        if (! ($image_File = &$this->get_image_File())) {	// Wrong image file:
            return null;
        }
     ...
     */
    public function get_image_tag($params = [])
    {
        $params = array_merge([
        ...
        if (! empty($params['before_classes'])) {	// Inject additional classes into 'before':
            $params['before'] = update_html_tag_attribs($params['before'], [
                'class' => $params['before_classes'],
            ]);
        }
        ...
        if ($params['link_to'] == '#category_url') {	// We want to link to the category URL:
            $params['link_to'] = $this->get_permanent_url();
        }

        return $cat_image_File->get_tag(
            $params['before'],
     ...
     */
    public function &get_ItemType($load_coll_default_item_type = false)
    {
        if ($this->get('ityp_ID') === null && ! $load_coll_default_item_type) {	// Item Type is same as collection default,
            // Return NULL because no request to get real Item Type object:
            $r = null;
            return $r;
        }

        if ($this->ItemType === null) {	// Load Item Type into cache:
            if ($this->get('ityp_ID') === null && $load_coll_default_item_type) {	// Try to get real Item Type object of this category's collection:
                if (($cat_Blog = &$this->get_Blog()) &&
                    ($coll_ItemType = &$cat_Blog->get_default_ItemType())) {	// Use real Item Type object:
                    $this->ItemType = $coll_ItemType;
                } else {	// Impossible to get default Item Type by some unknown reason:
                    $this->ItemType = false;
                }
            } elseif (($this->get('ityp_ID') > 0) &&
                    ($ItemTypeCache = &get_ItemTypeCache()) &&
                    ($cat_ItemType = &$ItemTypeCache->get_by_ID($this->get('ityp_ID'), false, false)) &&
                    ($cat_ItemType->is_enabled($this->get('blog_ID')))) {	// Default Item Type is found in DB and it is enabled for chapter's collection:
                $this->ItemType = $cat_ItemType;
            } else {	// No default type or Item Type is not found in DB or Item Type is not enabled for chapter's collection:
                $this->ItemType = false;
            }
        }
