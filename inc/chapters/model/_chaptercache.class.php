...
class ChapterCache extends DataObjectCache
{
     ...
     */
    public $subset_property;

    /**
     * Which subsets have been loaded
     */
    public $loaded_subsets = [];
     ...
     */
    public $subset_root_cats = [];

    /**
     * Have the children been revealed for all subsets yet?
     */
    public $revealed_all_children = false;

    /**
     * Have the children been revealed for all subsets yet?
     */
    public $revealed_subsets = [];
     ...
     */
    public function clear($keep_shadow = false)
    {
        $this->subset_cache = [];
        $this->loaded_subsets = [];
        $this->root_cats = [];
        $this->subset_root_cats = [];
        $this->revealed_all_children = false;
        $this->revealed_subsets = [];
        $this->sorted_flags = [];
        $this->urlname_index = [];
    ...
    {
        if (parent::add($Chapter)) {	// Successfully added
            if (! empty($this->subset_property)) {	// Also add to subset cache:
     ...
     */
    public function &get_by_ID($req_ID, $halt_on_error = true, $halt_on_empty = true, $subset_ID = null)
    {
        global $DB, $Debuglog;
        ...
        if (! empty($this->cache[$req_ID])) { // Already in cache
            $Debuglog->add("Accessing Chapter($req_ID) from cache", 'dataobjects');
            return $this->cache[$req_ID];
        } elseif (! $this->all_loaded) { // Not in cache, but not everything is loaded yet
            if ($this->load_all || is_null($subset_ID)) { // It's ok to just load everything:
                $this->load_all();
            } else { // Load just the requested object:
        ...
        if (empty($this->cache[$req_ID])) { // Requested object does not exist
     ...
     */
    public function get_chapter_path($blog_ID, $chapter_ID)
    {
     ...
     */
    public function &get_by_urlname($req_urlname, $halt_on_error = true)
    {
        global $DB, $Debuglog;

        if (! isset($this->urlname_index[$req_urlname])) { // not yet in cache:
            ...
            $row = $DB->get_row($sql);
            if (empty($row)) {	// Requested object does not exist
                if ($halt_on_error) {
                    debug_die("Requested $this->objtype does not exist!");
                }
                // put into index:
     ...
     */
    public function load_urlname_array($req_array)
    {
        global $DB, $Debuglog;
     ...
     */
    public function load_subset($subset_ID, $force_order_by = '')
    {
        global $DB, $Debuglog, $Settings;

        if ($this->all_loaded || isset($this->loaded_subsets[$subset_ID])) { // Already loaded
            return false;
        }
     ...
     */
    public function get_chapters_by_subset($subset_ID)
    {
        if (! isset($this->loaded_subsets[$subset_ID])) { // Load subset
     ...
     */
    public function get_chapters($coll_ID, $parent_ID = 0, $sorted = false)
    {
     ...
     */
    public function has_chapters_in_subset($subset_ID)
    {
        $this->load_subset($subset_ID);
     ...
     * @param integer
     */
    public function move_Chapter_subtree($chapter_ID, $src_collection_ID, $dest_collection_ID)
    {
        /**
         * @var DB
         */
        ...
        // Move to root:
        if ($parent_Chapter = $Chapter->get_parent_Chapter()) {	// Was not already at root, cut it and move it:
            // echo 'Move to root';
     ...
     * @param array
     */
    public function recurse_move_subtree(&$Chapter, &$list_array)
    {
     ...
     */
    public function &new_obj($row = null, $subset_ID = null)
    {
        // Instantiate a custom object
     ...
     */
    public function reveal_children($subset_ID = null, $sort_root_cats = false)
    {
        global $Debuglog;
        global $Timer;

        if ($this->revealed_all_children) {	// All children have already been revealed: (can happen even if we require a subset *now*)
            if ($sort_root_cats) {
        ...
        if (empty($this->subset_property)) {	// We are not handling a subset but everything instead:
            ...
            // Reveal children:
            if (! empty($this->cache)) {	// There are loaded categories, so loop on all loaded categories to set their children list if it has:
                foreach ($this->cache as $cat_ID => $Chapter) {
                    // echo $Chapter->name;
                    if (! is_null($Chapter->parent_ID)) {	// This category has a parent, so add it to its parent children list:
                        $this->cache[$Chapter->parent_ID]->add_child_category($this->cache[$cat_ID]);
                    } else {	// This category has no parent, so add it to the parent categories list
                        $this->root_cats[] = &$this->cache[$cat_ID];
                    }
                }
            }

            if ($sort_root_cats) { // Sort root categories as it was requested
                $this->sort_root_cats(null);
            }

            $this->revealed_all_children = true;
        } else {	// We are handling a subset (for example Chapers of a specific blog_ID only):
            if (is_null($subset_ID)) {	// No specific subset requested, we are going to reveal all subsets in a row:
                // Make sure everything has been loaded:
                $this->load_all();
                ...
                $this->revealed_all_children = true;
            } else {	// We're interested in a specific subset:
                // *** THIS IS THE CASE USED IN B2EVOLUTION ***

                if (! empty($this->revealed_subsets[$subset_ID])) {	// Children have already been revealed:
                    if ($sort_root_cats) {
                        $this->sort_root_cats($subset_ID);
                    }
                    return;
                ...
                // Reveal children:
                if (! empty($this->subset_cache[$subset_ID])) {	// There are loaded categories
                    ...
                    if ($sort_root_cats) { // Sort subset root categories as it was requested
                        $this->sort_root_cats($subset_ID);
                    }
     ...
     */
    public function check_path_to_root($subset_ID, $cat_ID, $path_array = [])
    {
        global $Debuglog;
        ...
        $Chapter = &$this->subset_cache[$subset_ID][$cat_ID];
        if (is_null($Chapter->parent_ID)) {	// Found root parent
        ...
        if (in_array($Chapter->parent_ID, $path_array)) {	// We are about to enter an infinite loop
     ...
     */
    public function compare_item_with_chapter($Item, $Chapter, $order_type)
    {
        if ($order_type == 'alpha') {
     ...
     */
    public function sort_root_cats($subset_ID = null)
    {
        if ($subset_ID == null && empty($this->subset_property)) {
            if (! $this->sorted_flags['root_cats']) { // Root cats were not sorted yet
                usort($this->root_cats, ['Chapter', 'compare_chapters']);
        ...
        if ($subset_ID == null) {
            foreach (array_keys($this->subset_root_cats) as $key) {
                if ($this->sorted_flags[$key]) { // This subset was already sorted
                    continue;
                }

                usort($this->subset_root_cats[$key], ['Chapter', 'compare_chapters']);
                $this->sorted_flags[$key] = true;
            }
            return;
        }

        if (empty($this->sorted_flags[$subset_ID])) { // This subset was not sorted yet
            usort($this->subset_root_cats[$subset_ID], ['Chapter', 'compare_chapters']);
     ...
     */
    public function recurse($callbacks, $subset_ID = null, $cat_array = null, $level = 0, $max_level = 0, $params = [])
    {
        $params = array_merge([
        ...
        if ($display_posts && ! $params['posts_are_loaded'] && ($params['expand_all'] || ! empty($params['chapter_path']))) { // Posts needs to be loaded
            $ItemCache = &get_ItemCache();
            $ItemCache->load_by_categories($params['chapter_path'], $subset_ID);
            $params['posts_are_loaded'] = true;
        }

        if (is_null($cat_array)) {	// Get all parent categories:
            if (is_null($subset_ID)) {
                $cat_array = $this->root_cats;
            } elseif (isset($this->subset_root_cats[$subset_ID])) { // We have root cats for the requested subset:
        ...
        foreach ($cat_array as $cat) {
            if (isset($params['excluded_cat_IDs']) && in_array($cat->ID, $params['excluded_cat_IDs'])) {	// Category is excluded, Skip it:
                continue;
            }
            ...
            // Display a category:
            if (is_array($callbacks['line'])) { // object callback:
            ...
            if ($params['is_opened'] || ($max_level > $level + 1)) {	// Iterate through sub categories recursively:
                $params['level'] = $level;
                $r .= $this->iterate_through_category_children($cat, $callbacks, true, $params);
            }
        }

        if (! empty($r)) { // We have something to display at this level, wrap in in before/after:
            $r_before = '';
            if (is_array($callbacks['before_level'])) { // object callback:
            ...
            if (is_array($callbacks['after_level'])) { // object callback:
     ...
     */
    public function iterate_through_category_children($Chapter, $callbacks, $recurse = false, $params = [])
    {
        $r = "";
        $cat_items = [];
        ...
        if ($has_sub_cats || ! empty($cat_items)) { // Display category or posts
            $cat_index = 0;
            $item_index = 0;
            $subcats_to_display = [];
            ...
            while ($has_more_children || $has_more_items) {
                ...
                if ($current_Item != null
                    && ($current_sub_Chapter == null
                        || ($this->compare_item_with_chapter($current_Item, $current_sub_Chapter, $cat_order) <= 0))) {	// Next Item should display before Next Chapter/subcat
                    if (! empty($subcats_to_display)) {
                        if ($recurse) {	// Display sub-contents:
                            $r .= $this->recurse($callbacks, $params['subset_ID'], $subcats_to_display, $params['level'] + 1, $params['max_level'], $params);
                        } else {	// Display each category without recursion:
                            foreach ($subcats_to_display as $sub_Chapter) { // Display each category:
                                if (in_array($sub_Chapter->ID, $params['exclude_cats'])) {	// Exclude category:
                                    continue;
                                }
                                if (is_array($callbacks['line'])) { // object callback:
                    ...
                    // Display an Item:
                    if (is_array($callbacks['posts'])) { // object callback:
                    ...
                    $has_more_items = isset($cat_items[++$item_index]);
                } elseif ($current_sub_Chapter != null) { 	// Next Chapter should display before next Item:
                    $subcats_to_display[] = $current_sub_Chapter;
                    $has_more_children = isset($chapter_children_ids[++$cat_index]);
                }
            }

            if (! empty($subcats_to_display)) { // Display all subcats which were not displayed yet:
                if ($recurse) {
                    $r .= $this->recurse($callbacks, $params['subset_ID'], $subcats_to_display, $params['level'] + 1, $params['max_level'], $params);
                } else { // Display each category without recursion:
                    foreach ($subcats_to_display as $sub_Chapter) { // Display each category:
        ...
        } elseif (isset($callbacks['no_children'])) { // Display message when no children:
            if (is_array($callbacks['no_children'])) { // object callback:
     ...
     */
    public function recurse_select_options(
        $subset_ID = null,
        $include_root = false,
        $Cat_array = null,
        $level = 0,
        ...
        if (is_null($Cat_array)) {	// Get all parent categorie:
            $Cat_array = $this->root_cats;
        }

        // Sort categories alphabetically or manually depending on settings:
        usort($Cat_array, ['Chapter', 'compare_chapters']);
        ...
        foreach ($Cat_array as $Chapter) {
            if (in_array($Chapter->ID, $exclude_array)) {	// We want to exclude that cat.
                continue;
            }
            ...
            if (! empty($Chapter->children)) {	// Add children categories:
     ...
     */
    public function recurse_select(
        $selected = null,
        $subset_ID = null,
        $include_root = false,
        $Cat_array = null,
