...
$Form->checkbox_input('use_gravatar', $Settings->get('use_gravatar'), TB_('Use gravatar'), [
    'note' => TB_('Fall back to Gravatar if a user has not uploaded a profile picture.'),
]);
...
$Form->radio(
    'default_gravatar',
    $Settings->get('default_gravatar'),
    [['b2evo', TB_('Default image'), $default_images_info],
        ['', 'Gravatar'],
        ['identicon', 'Identicon'],
        ['monsterid', 'Monsterid'],
        ['wavatar', 'Wavatar'],
    ...
    TB_('Default gravatars'),
    true,
    TB_('Gravatar users can choose to set up a unique icon for themselves, and if they don\'t, they will be assigned a default image.')
);
...
$Form->radio(
    'username_display',
    $Settings->get('username_display'),
    [['login', TB_('Usernames/Logins'), TB_('Most secure option')],
        ['name', TB_('Friendly names (Nickname or Firstname if available)'), TB_('WARNING: this may allow users to fake their identity')],
    ],
    TB_('What to display'),
    true
);

$Form->checkbox_input('gender_colored', $Settings->get('gender_colored'), TB_('Display gender in back-office'), [
    'note' => TB_('Use colored usernames to differentiate men & women.'),
]);
...
$Form->checkbox_input('bubbletip', $Settings->get('bubbletip'), TB_('Username bubble tips'), [
    'note' => TB_('Check to enable bubble tips on usernames'),
]);
...
$Form->radio(
    'user_url_loggedin',
    $Settings->get('user_url_loggedin'),
    [['page', TB_('Always user page'), ''],
        ['url', TB_('User website if available (fallback to user page)'), ''],
    ],
    TB_('Link to'),
    true
);
...
$Form->radio(
    'user_url_anonymous',
    $Settings->get('user_url_anonymous'),
    [['page', TB_('Always user page (if allowed)'), ''],
        ['url', TB_('User website if available (fallback to user page)'), ''],
    ],
    TB_('Link to'),
    true
...
$Form->checkbox_input('bubbletip_anonymous', $Settings->get('bubbletip_anonymous'), TB_('Allow to see bubbletips'), [
    'note' => TB_('Check to enable bubble tips on usernames'),
]);
...
$user_level_params = [];
if (! $Settings->get('allow_anonymous_user_list') && ! $Settings->get('allow_anonymous_user_profiles')) { // Disable the user groups levels interval because the users pages are not available for anonymous users
