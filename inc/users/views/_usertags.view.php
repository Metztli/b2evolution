...
if (get_param('utag_filter')) { // add tag_name filter
...
if ($filter_user_ID = get_param('tag_user_ID')) { // add filter for user ID
...
{
    $Form->text_input('utag_filter', get_param('utag_filter'), 24, /* TRANS: noun */ T_('Tag'), '', [
        'maxlength' => 50,
    ]);

    $user_ID_filter_note = '';
    if ($filter_user_ID = get_param('tag_user_ID')) { // check user_Id filter. It must be a number
        if (! is_number($filter_user_ID)) { // It is not a number
            $user_ID_filter_note = T_('Must be a number');
        }
    }
    $Form->text_input('tag_user_ID', $filter_user_ID, 9, T_('User ID'), $user_ID_filter_note, [
        'maxlength' => 9,
    ]);
}
$Results->filter_area = [
...
function tag_td_name($utag_ID, $utag_name)
{
    ...
    if (check_user_perm('options', 'edit')) {	// Display tag name as link to edit form only if current user has a perm:
...
if (check_user_perm('options', 'edit')) {
    ...
    if (check_user_perm('options', 'edit')) {	// Allow to clean up tags only if current user has a permission to edit tags:
    ...
    $Results->global_icon(T_('Add a new tag...'), 'new', regenerate_url('action', 'action=new&amp;return_to=' . urlencode(regenerate_url('action', '', '', '&'))), T_('New tag') . ' &raquo;', 3, 4, [
