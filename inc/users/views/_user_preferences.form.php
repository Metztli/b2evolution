...
$Form->begin_fieldset($is_admin ? TB_('Other preferences') . get_manual_link('user-preferences') : '', [
...
if ($timeout_sessions > $Settings->get('auto_prune_stats') * 86400 &&
        $timeout_sessions > $def_timeout_session) {	// Display a warning if the user session can be deleted earlier:
...
if ($action != 'view') { // We can edit the values:
    ...
    if ($multiple_sessions_field_hidden) {
        ...
        $Form->checkbox(
            ...
            $multiple_sessions_value,
            TB_('Multiple sessions'),
            ...
            '',
            1,
            $multiple_sessions_field_disabled
        );
    }

    if (($current_User->ID == $edited_User->ID) || (check_user_perm('users', 'edit'))) {
        $Form->radio_input('edited_user_timeout_sessions', $timeout_sessions_selected, [
            [
                'value' => 'default',
        ...
        if ($timeout_sessions_selected == 'default') { // Hide the field to customize a session duration when default duration is selected
            $Form->fieldstart = str_replace('>', ' style="display:none">', $Form->fieldstart);
        }
        $Form->duration_input('timeout_sessions', $timeout_sessions, TB_('Custom duration'), 'months', 'seconds', [
            'minutes_step' => 1,
            'note' => $timeout_sessions_note,
        ]);
...
if ($action != 'view') { // Edit buttons
    $action_buttons = [['', 'actionArray[update]', TB_('Save Changes!'), 'SaveButton']];
    if ($is_admin) {
...
if ($Settings->get('account_close_enabled') && isset($Blog) &&
    ($current_User->ID == $edited_User->ID) && ! check_user_perm('users', 'edit', false)) { // Display a linkt to close account
