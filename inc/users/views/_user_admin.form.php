...
if (! $current_User->can_moderate_user($edited_User->ID)) { // Check permission:
...
$Form->begin_fieldset(TB_('User permissions') . get_manual_link('user-admin-permissions'), [
    'class' => 'fieldset clear',
]);

// Status:
$status_icon = '<div id="user_status_icon" class="status_icon">' . $user_status_icons[$edited_User->get('status')] . '</div>';
if ($edited_User->ID == 1) {	// This is Admin user, Don't allow to change status:
    $Form->info(TB_('Account status'), $status_icon . ' ' . TB_('Autoactivated'));
} else {	// Allow to change status for non-admin users:
...
if ($edited_User->ID == 1) {	// This is Admin user, Don't allow to change level:
    $Form->info_field(TB_('User level'), $edited_User->get('level'), [
        'note' => $level_fieldnote,
    ]);
} else {	// Allow to change level for non-admin users:
    $Form->text_input('edited_user_level', $edited_User->get('level'), 2, TB_('User level'), $level_fieldnote, [
        'required' => true,
    ]);
}
...
$Form->email_input('edited_user_email', $edited_User->get('email'), 30, '', [
    'maxlength' => 255,
    'required' => true,
...
if (check_user_perm('users', 'edit')) {
    $Form->select_input_array('edited_email_status', $email_status, emadr_get_status_titles(), '<b class="evo_label_inline">' . TB_('Status') . ': </b>' . $email_status_icon, '', [
        'force_keys_as_values' => true,
        'background_color' => emadr_get_status_colors(),
    ]);
} else { // Moderators can only view the email status
...
$Form->email_input('notification_sender_email', $notifcation_sender_email, 50, TB_('Sender email address'), [
    'note' => $notifcation_sender_email_note,
]);
...
foreach ($account_activation_info as $field_options) { // Display each info field
    $field_note = isset($field_options[2]) ? $field_options[2] : '';
    $Form->info_field($field_options[0], $field_options[1], [
        'note' => $field_note,
    ]);
}
...
$Form->info_field(TB_('Latest unread messages reminder'), empty($last_unread_messages_reminder) ? TB_('None yet') : format_to_output($last_unread_messages_reminder), [
    'note' => TB_(' Scheduled job responsible for reminders is "Send reminders about unread messages".'),
]);
// Display next unread message reminder info
$reminder_info = get_next_reminder_info($edited_User->ID);
if (is_array($reminder_info)) { // We have field note to display
    $Form->info_field(TB_('Next unread messages reminder'), $reminder_info[0], [
        'note' => $reminder_info[1],
    ]);
} else { // Display next reminder info, without note
...
if (empty($last_notification_email)) { // Notification email to the edited User was not sent yet
    $Form->info_field(TB_('Latest notification email'), TB_('None yet'), [
        'note' => TB_('The latest between all kind of notification emails.'),
    ]);
} else { // At least one notification email was sent
    ...
    $Form->info_field(TB_('Latest notification email'), $last_notificaiton_date, [
        'note' => TB_('The latest between all kind of notification emails.'),
    ]);
...
if (empty($last_newsletter)) { // Newsletter to the edited User was not sent yet
    $Form->info_field(TB_('Latest list'), TB_('None yet'));
} else { // At least one newsletter was sent
...
if ($blogs_owned > 0) {
    $blogs_owned .= ' - <a href="' . $activity_tab_url . '#owned_blogs_result" class="' . button_class() . ' middle" title="' . format_to_output(TB_('Go to user activity'), 'htmlattr') . '">' . get_icon('magnifier', 'imgtag', [
        'title' => TB_('Go to user activity'),
    ]) . '</a>';
}
$Form->info_field(TB_('Blogs owned'), $blogs_owned, [
    'class' => $blogs_owned_num > 0 ? 'info_full_height' : '',
]);
...
if ($posts_created > 0) {
    $posts_created .= ' - <a href="' . $activity_tab_url . '#created_posts_result" class="' . button_class() . ' middle" title="' . format_to_output(TB_('Go to user activity'), 'htmlattr') . '">' . get_icon('magnifier', 'imgtag', [
        'title' => TB_('Go to user activity'),
    ]) . '</a>';
    if ($action != 'view') {	// If current user can edit this user:
        $posts_created .= ' - ' . action_icon(TB_('Delete All') . '...', 'delete', $admin_url . '?ctrl=user&amp;user_tab=deldata&amp;user_ID=' . $edited_User->ID, ' ' . TB_('Delete All') . '...', 3, 4, [
            'onclick' => 'return user_deldata( ' . $edited_User->ID . ', \'' . get_param('user_tab') . '\')',
        ]);
...
if ($posts_edited > 0) {
    $posts_edited .= ' - <a href="' . $activity_tab_url . '#edited_posts_result" class="' . button_class() . ' middle" title="' . format_to_output(TB_('Go to user activity'), 'htmlattr') . '">' . get_icon('magnifier', 'imgtag', [
        'title' => TB_('Go to user activity'),
    ]) . '</a>';
}
...
if ($comments_created > 0) {
    $comments_created .= ' - <a href="' . $activity_tab_url . '#comments_result" class="' . button_class() . ' middle" title="' . format_to_output(TB_('Go to user activity'), 'htmlattr') . '">' . get_icon('magnifier', 'imgtag', [
        'title' => TB_('Go to user activity'),
    ]) . '</a>';
    if ($action != 'view') {	// If current user can edit this user:
        $comments_created .= ' - ' . action_icon(TB_('Delete All') . '...', 'delete', $admin_url . '?ctrl=user&amp;user_tab=deldata&amp;user_ID=' . $edited_User->ID, ' ' . TB_('Delete All') . '...', 3, 4, [
            'onclick' => 'return user_deldata( ' . $edited_User->ID . ', \'' . get_param('user_tab') . '\')',
        ]);
        $comments_created .= get_manual_link('delete-user-data');
    }
}
$Form->info_field(TB_('Comments'), $comments_created, [
    'class' => $comments_created_num > 0 ? 'info_full_height' : '',
]);

// Number of edited User's sessions:
$num_sessions = $edited_User->get_num_sessions(true);
$Form->info_field(TB_('# of sessions'), $num_sessions, [
    'class' => $num_sessions > 0 ? 'info_full_height' : '',
]);
...
if ($messages_sent > 0) {
    $messages_sent .= ' - <a href="' . $activity_tab_url . '#threads_result" class="' . button_class() . ' middle" title="' . format_to_output(TB_('Go to user activity'), 'htmlattr') . '">' . get_icon('magnifier', 'imgtag', [
        'title' => TB_('Go to user activity'),
    ]) . '</a>';
    ...
    if ($action != 'view') {	// If current user can edit this user:
        $messages_sent .= ' - ' . action_icon(TB_('Delete All') . '...', 'delete', $admin_url . '?ctrl=user&amp;user_tab=deldata&amp;user_ID=' . $edited_User->ID, ' ' . TB_('Delete All') . '...', 3, 4, [
            'onclick' => 'return user_deldata( ' . $edited_User->ID . ', \'' . get_param('user_tab') . '\')',
        ]);
...
if ($messages_received > 0 && check_user_perm('perm_messaging', 'abuse')) {
    $messages_received .= ' - <a href="' . $admin_url . '?ctrl=abuse&amp;colselect_submit=Filter+list&amp;u=' . $edited_User->login . '" class="' . button_class() . ' middle" title="' . format_to_output(TB_('Go to abuse management'), 'htmlattr') . '">' . get_icon('magnifier', 'imgtag', [
        'title' => TB_('Go to abuse management'),
    ]) . '</a>';
}
...
$Form->info(TB_('Comments'), '<span class="reputation_message">' . $edited_User->get_reputation_comments([
    'view_type' => 'extended',
]) . '</span>');

$Form->info(TB_('Photos'), '<span class="reputation_message">' . $edited_User->get_reputation_files([
    'file_type' => 'image',
    'view_type' => 'extended',
]) . '</span>');

$Form->info(TB_('Audio'), '<span class="reputation_message">' . $edited_User->get_reputation_files([
    'file_type' => 'audio',
]) . '</span>');

$Form->info(TB_('Video'), '<span class="reputation_message">' . $edited_User->get_reputation_files([
    'file_type' => 'video',
]) . '</span>');

$Form->info(TB_('Other files'), '<span class="reputation_message">' . $edited_User->get_reputation_files([
    'file_type' => 'other',
]) . '</span>');
...
$from_country = '';
if (! empty($edited_User->reg_ctry_ID)) {	// Get country that was defined by GeoIP Plugin on registration
...
$Form->info_field('', mysql2localedatetime($edited_User->dget('datecreated')), [
    'note' => '(' . date_ago(strtotime($edited_User->get('datecreated'))) . ')',
]);
$Form->info_field(
    '<b class="evo_label_inline">' . TB_('From IP') . ': </b>',
...
if (check_user_perm('spamblacklist', 'view')) { // User can view IP ranges
    // Get status and name of IP range
    $IPRangeCache = &get_IPRangeCache();
    if ($IPRange = &$IPRangeCache->get_by_ip($user_ip_address)) { // IP range exists in DB
        $iprange_status = $IPRange->get('status');
        $iprange_name = $IPRange->get_name();
        if (check_user_perm('spamblacklist', 'view')) {	// Display IP range as link to edit form if current user has the permissions:
            $iprange_name = '<a href="' . $admin_url . '?ctrl=antispam&amp;tab3=ipranges&amp;action=iprange_edit&amp;iprange_ID=' . $IPRange->ID . '">' . $iprange_name . '</a>';
        }
    } else { // There is no IP range in DB
        $iprange_status = '';
        $iprange_name = '';
    }
    ...
    if ($perm_spamblacklist) { // User can edit IP ranges
        $Form->select_input_array('edited_iprange_status', $iprange_status, aipr_status_titles(true), '<b class="evo_label_inline">' . TB_('Status') . ': </b>' . $email_status_icon, '', [
            'force_keys_as_values' => true,
            'background_color' => aipr_status_colors(),
        ]);
    } else { // Only view status of IP range
...
$Form->info_field(TB_('From Country'), $from_country, [
    'field_suffix' => $user_from_country_suffix,
]);
...
$Form->text_input('edited_user_source', $edited_User->source, 30, TB_('Source link/code'), '', [
    'maxlength' => 30,
]);
...
if ($Domain && $perm_stat_edit) {
    $initial_referer_formatted = preg_replace(
...
if ($display_initial_referer) { // User can view Domains
    ...
    if ($perm_stat_edit) { // User can edit Domain
        ...
        if (! $Domain || $initial_referer_domain != $Domain->get('name')) { // Link to create a new domain
        ...
        if ($Domain) { // Link to edit existing domain
        ...
        $Form->select_input_array('edited_initial_referer_status', $domain_status, stats_dom_status_titles(), '<b class="evo_label_inline">' . TB_('Status') . ': </b>' . $domain_status_icon, '', [
            'force_keys_as_values' => true,
...
<?php
if (check_user_perm('users', 'edit')) { // START OF email status change script
    ?>
...
<?php
    ...
    foreach ($iprange_status_icons as $status => $icon) { // Init js array with IP range status icons
        ?>
...
<?php
if (check_user_perm('stats', 'edit')) { // User can edit Domain
    ?>
var domain_status_icons = new Array;
<?php
    $domain_status_icons = stats_dom_status_icons();
    foreach ($domain_status_icons as $status => $icon) { // Init js array with Domain status icons
        ?>
