            ...
            if (array_key_exists($perm_name, $permissions)) {
                ...
                if ($perm['perm_block'] == $perm_block) {
                    ...
                    switch ($perm['perm_type']) {
                        case 'checkbox':
                            $Form->checkbox_input('edited_grp_' . $perm_name, $GroupSettings->permission_values[$perm_name] == 'allowed', $perm['label'], [
                                'input_suffix' => ' ' . $perm['note'],
                                'value' => 'allowed',
                            ]);
                            break;
                        ...
                        case 'checklist':
                            ...
                            if (! empty($checklist_values)) {	// If at least one option is selected:
                                foreach ($perm['options'] as $o => $checklist_option) {
                                    if (in_array($checklist_option[1], $checklist_values)) {	// This option is selected for the group:
                                        $perm['options'][$o][3] = 1;
                                    }
                                }
                            }
                            $Form->checklist($perm['options'], 'edited_grp_' . $perm_name, $perm['label'], false, false, [
                                'note' => $perm['note'],
                            ]);
                            break;
                        ...
                        case 'text_input':
                            $Form->text_input('edited_grp_' . $perm_name, $GroupSettings->permission_values[$perm_name], 5, $perm['label'], $perm['note'], [
...
$perm_none_option = ['none', TB_('No Access')];
$perm_view_option = ['view', TB_('View details')];
$perm_edit_option = ['edit', TB_('Edit/delete all')];
...
$Form->radio('edited_grp_usage', $edited_Group->get('usage'), [
    [
        'primary',
...
$Form->text_input('edited_grp_level', $edited_Group->get('level'), 2, TB_('Group level'), '[0 - 10]', [
    'required' => true,
]);
...
$Form->begin_fieldset(TB_('Evobar & Back-office') . get_manual_link('group-properties-evobar'), [
    'id' => 'evobar',
    'style' => $primary_panels_style,
]);
...
$Form->begin_fieldset(TB_('Blogging permissions') . get_manual_link('group-properties-blogging'), [
    'id' => 'blogging',
    'style' => $primary_panels_style,
]);

$Form->radio(
    'edited_grp_perm_blogs',
    $edited_Group->get('perm_blogs'),
    [['user', TB_('Users can only see Collections they have access to and the Sections these collections belong to + all Collections in the Sections they own.')],
        ['viewall', TB_('View all: User can see all Sections and call Collections (with no additional edit permissions as above)')],
        ['editall', sprintf(TB_('Full Access %s: Users can edit all Sections and Collections, create and delete Collections in any Section, create new Sections and delete empty Sections.'), get_admin_badge('coll', '#', TB_('Site Admin'), TB_('Select to give Collection Admin permission')))],
    ],
    TB_('Collections & Site Sections'),
    true
);

$Form->radio(
    'perm_xhtmlvalidation',
    $edited_Group->get('perm_xhtmlvalidation'),
    [['always', TB_('Force valid XHTML + strong security'),
        TB_('The security filters below will be strongly enforced.')],
        ['never', TB_('Basic security checking'),
            TB_('Security filters below will still be enforced but with potential lesser accuracy.')],
    ],
    TB_('XHTML validation'),
    true
);

$Form->radio(
    'perm_xhtmlvalidation_xmlrpc',
    $edited_Group->get('perm_xhtmlvalidation_xmlrpc'),
    [['always', TB_('Force valid XHTML + strong security'),
        TB_('The security filters below will be strongly enforced.')],
        ['never', TB_('Basic security checking'),
            TB_('Security filters below will still be enforced but with potential lesser accuracy.')],
    ],
    TB_('XHTML validation on XML-RPC calls'),
    true
);

$Form->checklist([
    ['prevent_css_tweaks', 1, TB_('Prevent CSS tweaks'), ! $edited_Group->get('perm_xhtml_css_tweaks'), false,
        TB_('WARNING: if allowed, users may deface the site, add hidden text, etc.')],
    ['prevent_iframes', 1, TB_('Prevent iframes'), ! $edited_Group->get('perm_xhtml_iframes'), false,
        TB_('WARNING: if allowed, users may do XSS hacks, steal passwords from other users, etc.')],
    ['prevent_javascript', 1, TB_('Prevent javascript'), ! $edited_Group->get('perm_xhtml_javascript'), false,
        TB_('WARNING: if allowed, users can easily do XSS hacks, steal passwords from other users, etc.')],
    ['prevent_objects', 1, TB_('Prevent objects'), ! $edited_Group->get('perm_xhtml_objects'), false,
        TB_('WARNING: if allowed, users can spread viruses and malware through this blog.')],
], 'xhtml_security', TB_('Security filters'));

$Form->checkbox(
    'apply_antispam',
    ! $edited_Group->get('perm_bypass_antispam'),
    TB_('Antispam filtering'),
...
$Form->begin_fieldset(TB_('Additional permissions') . get_manual_link('group-properties-additional-permissions'), [
    'id' => 'additional',
    'style' => $primary_panels_style,
]);

$Form->radio(
    'edited_grp_perm_stats',
    $edited_Group->get('perm_stats'),
    [$perm_none_option,
        ['user', TB_('View stats for specific blogs'), TB_('Based on each blog\'s edit permissions')], // fp> dirty hack, I'll tie this to blog edit perm for now
        ['view', TB_('View stats for all blogs')],
        ['edit', TB_('Full Access'), TB_('Includes deleting/reassigning of stats')],
    ],
    TB_('Analytics'),
    true
);
...
$Form->begin_fieldset(TB_('File permissions') . get_manual_link('group-properties-file-permissions'), [
    'id' => 'file',
    'style' => $primary_panels_style,
]);
...
$Form->begin_fieldset(TB_('System admin permissions') . get_manual_link('group-properties-system-permissions'), [
    'id' => 'system',
    'style' => $primary_panels_style,
]);
...
$Form->begin_fieldset(TB_('Notification options') . get_manual_link('notification-options'), [
    'id' => 'notification',
    'style' => $primary_panels_style,
]);
...
if ($action != 'view') {	// If current User can edit this group:
    ...
    while ($loop_Plugin = &$Plugins->get_next()) {
        if (! $loop_Plugin->GroupSettings) {	// Skip plugin without group settings:
            continue;
        }
        ...
        if (is_array($plugin_group_settings)) {
            foreach ($plugin_group_settings as $l_name => $l_meta) {	// Display form field for this setting:
        ...
        if ($has_contents) {	// Print out plugin group settings:
            ob_end_flush();
            ob_end_flush();
    ...
    $Form->buttons([['', '', TB_('Save Changes!'), 'SaveButton']]);
}
...
<?php
global $Settings;
if ($Settings->get('fm_enable_roots_shared')) {	// asimo> this may belong to the pluggable permissions display
