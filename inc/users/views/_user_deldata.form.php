...
$Form->begin_fieldset(TB_('Delete user data') . get_manual_link('delete-user-data') . $close_icon, [
...
$delete_options = [];
if ($posts_created) {
    $delete_options[] = ['delete_posts', 1, sprintf(TB_('%s posts'), $posts_created), 1];
}
if ($comments_created) {
    $delete_options[] = ['delete_comments', 1, sprintf(TB_('%s comments'), $comments_created), 1];
}
if ($messages_sent) {
    $delete_options[] = ['delete_messages', 1, sprintf(TB_('%s private messages'), $messages_sent), 1];
}
$Form->checklist($delete_options, 'default_user_notification', sprintf(TB_('Please confirm deletion of the contents contributed by %s'), $edited_User->get_colored_login()), false, false, [
    'wide' => true,
]);

echo '<p class="center">';
$Form->button(['', 'actionArray[delete_data]', TB_('Delete selected data'), 'SaveButton']);
echo '</p>';
