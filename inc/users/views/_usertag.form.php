...
if (! empty($edited_UserTag->merge_tag_ID)) { // Display a for to confirm merge the tag to other one
...
$Form->text_input('utag_name', $edited_UserTag->get('name'), 50, /* TRANS: noun */ TB_('Tag'), '', [
    'maxlength' => 255,
    'required' => true,
]);
...
if ($edited_UserTag->ID > 0) {
    ...
    function taguser_edit_actions($User)
    {
        ...
        if ($current_User->can_moderate_user($User->ID)) {	// Display the action icons if current User has the rights to moderate the User:
            $r = action_icon(TB_('Edit this user...'), 'edit', regenerate_url('ctrl,action', 'ctrl=user&amp;user_ID=' . $User->ID . '&amp;user_tab=marketing'));
            $r .= action_icon(
                TB_('Unlink this tag from user!'),
                'unlink',
