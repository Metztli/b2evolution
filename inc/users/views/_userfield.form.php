...
$Form->select_input_array(
    'ufdf_ufgp_ID',
    $edited_Userfield->ufgp_ID,
    $edited_Userfield->get_groups(),
    TB_('Group'),
    '',
    [
        'required' => true,
...
$Form->text_input('ufdf_code', $edited_Userfield->code, 20, TB_('Field code'), '', [
    'maxlength' => 20,
    'required' => true,
]);

$Form->text_input('ufdf_name', $edited_Userfield->name, 50, TB_('Field name'), '', [
    'maxlength' => 255,
    'required' => true,
]);

$Form->text_input('ufdf_icon_name', $edited_Userfield->icon_name, 20, TB_('Icon name'), '', [
    'maxlength' => 100,
]);

if (is_pro()) {	// Allow to select user group for PRO version:
    load_funcs('_core/_pro_features.funcs.php');
    $user_group_selector = pro_user_field_group_selector($edited_Userfield->grp_ID, $Form);
} else {	// Display info about unavailable feature:
    $user_group_selector = sprintf(TB_('This is a %s feature'), get_pro_label());
}
$Form->select_input_array(
    'ufdf_type',
    $edited_Userfield->type,
    Userfield::get_types(),
    TB_('Field type'),
    '',
    [
...
$Form->textarea_input('ufdf_options', $edited_Userfield->options, 10, TB_('Options'), [
    'required' => ($edited_Userfield->type == 'list' ? true : 'mark_only'),
    'maxlength' => 255,
    'note' => TB_('Enter one option per line. Max length 255 symbols.'),
]);
echo '</div>';
...
$Form->radio_input('ufdf_duplicated', $edited_Userfield->duplicated, Userfield::get_duplicateds('radio'), TB_('Multiple values'), [
    'required' => true,
    'lines' => true,
]);

$Form->radio_input('ufdf_required', $edited_Userfield->required, Userfield::get_requireds('radio'), TB_('Required?'), [
    'required' => true,
]);

$Form->radio_input('ufdf_visibility', $edited_Userfield->visibility, Userfield::get_visibilities('radio'), TB_('Field visibility'), [
    'required' => true,
]);
