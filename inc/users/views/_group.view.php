...
{
    if ($primary_users_count == 0 && $secondary_users_count == 0) {	// This group is not used at all
        return '<span class="label label-default">0</span>';
    } else {
        global $admin_url;
        $users_url = $admin_url . '?ctrl=users&amp;filter=new&amp;';
        $r = '';
        if ($primary_users_count > 0) {	// This group is used as primary for several users
            $r .= ' <a href="' . $users_url . 'group=' . $group_ID . '" class="label label-primary">' . $primary_users_count . '</a> ';
        }
        if ($secondary_users_count > 0) {	// This group is used as secondary for several users
...
if (check_user_perm('users', 'edit', false)) { // If user can edit the users - Init js to edit group level by AJAX
    $group_levels = [];
    for ($l = 0; $l <= 10; $l++) {
        $group_levels[$l] = $l;
    }
