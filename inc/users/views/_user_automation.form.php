...
$Form->begin_fieldset(TB_('Add user to an automation...') . get_manual_link('add-user-to-automation') . $close_icon, [
    'style' => 'width:420px',
]);
...
if (count($AutomationCache->cache) > 0) {	// Allow to select automation if at least one is avaialble:
    $Form->select_input_object('autm_ID', '', $AutomationCache, TB_('Automation'), [
        'required' => true,
    ]);

    echo '<p class="center">';
    $Form->button(['', 'actionArray[add_automation]', TB_('Add'), 'SaveButton']);
    echo '</p>';
