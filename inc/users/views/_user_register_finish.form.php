...
if ((empty($reqID) || $reqID != $Session->get('core.changepwd.request_id')) &&
    ...
    if (! check_user_perm('users', 'edit') || $edited_User->ID == $current_User->ID) {	// Current user has no full access or editing his own password
...
$Form->email_input(
    ...
    TB_('Email'),
    [
        'note' => '<br />' . TB_('We respect your privacy. Your email will remain strictly confidential.'),
    ],
    [
...
if (in_array('country', $required_fields)) {	// Country is required:
    $CountryCache = &get_CountryCache();
    $Form->select_country('country', $edited_User->ctry_ID, $CountryCache, TB_('Country'), [
        'allow_none' => true,
        'required' => true,
    ]);
}

if (in_array('firstname', $required_fields)) {	// Firstname is visible:
    $Form->text_input('firstname', $edited_User->firstname, 18, TB_('First name'), TB_('Your real first name.'), [
        'maxlength' => 50,
        'class' => 'input_text',
        'required' => true,
    ]);
...
if (in_array('lastname', $required_fields)) {	// Lastname is visible:
    $Form->text_input('lastname', $edited_User->lastname, 18, TB_('Last name'), TB_('Your real last name.'), [
        'maxlength' => 50,
        'class' => 'input_text',
        'required' => true,
    ]);
...
if (in_array('gender', $required_fields)) {	// Display a gender field if it is not hidden:
    $Form->radio_input('gender', $edited_User->gender, [
        [
            'value' => 'M',
            'label' => TB_('A man'),
        ],
...
if (in_array('locale', $required_fields)) {	// Ask user language:
    $Form->select('locale', $edited_User->get('locale'), 'locale_options_return', TB_('Locale'), TB_('Preferred language'));
}

$Form->end_form([
    ['', 'actionArray[update]', TB_('Finish Registration') . '!', 'SaveButton'],
]);
