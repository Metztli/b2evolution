...
if ($current_User->ID != $user_ID && ! $current_User->can_moderate_user($edited_User->ID)) { // Check permission:
...
user_sent_emails_results_block([
...
user_email_returns_results_block([
    'edited_User' => $edited_User,
    'action' => $action,
]);
evo_flush();
...
user_reports_results_block([
    'edited_User' => $edited_User,
]);
evo_flush();

/**** Collections owned by the User ****/
blogs_user_results_block([
    'edited_User' => $edited_User,
    'action' => $action,
]);
evo_flush();
...
received_threads_results_block([
    'edited_User' => $edited_User,
    'action' => $action,
]);
evo_flush();
...
if ($action != 'view' && $current_User->ID != $edited_User->ID && $edited_User->ID != 1 && check_user_perm('users', 'edit')) { // User can NOT delete admin and own account
    echo '<div style="margin:25px 0">';
    echo action_icon(T_('Delete User and All his contributions'), 'delete', '?ctrl=user&amp;user_tab=activity&amp;action=delete_all_userdata&amp;user_ID=' . $edited_User->ID . '&amp;' . url_crumb('user'), ' ' . T_('Delete User and All his contributions'), 3, 4, [
        'class' => 'btn btn-danger',
    ]);
    echo action_icon(T_('Delete User and All his contributions') . ' + ' . T_('Mark as email Spammer'), 'delete', '?ctrl=users&amp;action=delete&amp;deltype=spammer&amp;user_ID=' . $edited_User->ID, ' ' . T_('Delete User and All his contributions') . ' + ' . T_('Mark as email Spammer'), 3, 4, [
        'class' => 'btn btn-danger',
