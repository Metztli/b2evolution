...
// Colors:
$donut_chart['series_color'] = [];
$donut_chart['series_color'][0] = []; // Outer donut
$donut_chart['series_color'][1] = $user_gender_color; // Middle donut
$donut_chart['series_color'][2] = []; // Inner donut
$c = 0;
foreach ($user_gender_color as $color) {
    ...
    if ($c % 3 == 0) { // The inner donut uses only 3 main colors
...
$scheme_active = ['a', 'i', 'c'];
$scheme_photos = ['p', 'n'];
foreach ($scheme_gender as $gender) { // Genders
    $donut_chart['data'][2][$gender] = 0;
    foreach ($scheme_active as $active) { // Actives
        $donut_chart['data'][1][$gender . $active] = 0;
        foreach ($scheme_photos as $photo) { // Photos
...
if (count($res_users)) {
    ...
    */
    ...
    foreach ($res_users as $row_stats) {
        ...
        if (! empty($col_mapping[$row_stats['user_gender_status']])) {	// those users are calculated here
