...
if ($action != 'view') {	// If current user can edit this user:
    $Form->usertag_input('edited_user_tags', param('edited_user_tags', 'string', $user_tags), 40, TB_('Tags'), '', [
        'maxlength' => 255,
        'style' => 'width: 100%;',
    ]);
} else {	// If current user cannot edit this user:
...
if ($action != 'view') {	// If current user can edit this user:
    $Results->global_icon(TB_('Add user to an automation...'), 'new', regenerate_url('action,user_tab', 'action=new_automation&amp;user_tab=automation'), TB_('Add user to an automation...'), 3, 4, [
