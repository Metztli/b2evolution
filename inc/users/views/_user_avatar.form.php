...
$Form->begin_fieldset($is_admin ? TB_('Profile picture') . get_manual_link('user-profile-picture-tab') : '', [
...
if ($edited_User->has_avatar() && ($avatar_Link = &$edited_User->get_avatar_Link())) {
    ...
    if (($current_User->ID == $edited_User->ID) || $can_moderate_user) { // Display actions only if current user can edit this user
        if (is_admin_page()) {
        ...
        $rotate_icons = $edited_User->get_rotate_avatar_icons($edited_User->avatar_file_ID, [
            'before' => '',
            'after' => '<br />',
            'text' => ' ' . TB_('Rotate'),
        ]);

        $crop_icon = $edited_User->get_crop_avatar_icon($edited_User->avatar_file_ID, [
            'before' => '',
            'after' => '<br />',
            'text' => ' ' . TB_('Crop'),
        ...
        if (is_admin_page() && $can_moderate_user) { // Only if current user can edit this user
            ...
            $duplicated_files_message = $avatar_File->get_duplicated_files_message([
                'message' => '<p class="duplicated_avatars">'
                    . get_icon('warning_yellow', 'imgtag', [
        ...
        if ($action != 'view') {	// If current user can edit this User:
            $action_picture_links = '<div class="avatar_actions">' .
                    ...
                    $forbid_link .
                    action_icon($delete_picture_text, 'delete', $delete_picture_url, ' ' . $delete_picture_text, 3, 4, [
...
if (($current_User->ID == $edited_User->ID) || $can_moderate_user) {
    // Upload or select:
    global $Settings;
    if ($Settings->get('upload_enabled') && ($Settings->get('fm_enable_roots_user'))) { // Upload is enabled and we have permission to use it...
        ...
        if (count($user_avatars) > 0) {
            ...
            foreach ($user_avatars as $user_Link) {
                ...
                $info_content .= $user_Link->get_tag([
                    'before_image' => '',
                    'before_image_legend' => '',
                    'after_image_legend' => '',
                    'after_image' => '',
                ...
                if ($action != 'view') {	// If current user can edit this User:
                    if ($user_Link->File->get('can_be_main_profile')) { // Link to set picture as Main
                        ...
                        $info_content .= '<br />' . action_icon(TB_('Use as main picture'), 'move_up', $url_update, TB_('Main'), 3, 4, [], [
                            'style' => 'margin-right:4px',
                        ]);
                    } elseif (is_admin_page() && $can_moderate_user) { // Link to Restore picture if it was forbidden (only for admins)
                        ...
                        $info_content .= '<br />' . action_icon(TB_('Restore to use as main picture'), 'move_up', $url_restore, TB_('Restore'), 3, 4, [], [
                            'style' => 'margin-right:4px',
                        ]);
                    } else { // Display empty line
                        $info_content .= '<br />';
                    }
                    ...
                    $info_content .= '<br />' . action_icon(TB_('Delete this picture'), 'delete', $url_delete, TB_('Delete'), 3, 4, [
                        'onclick' => 'return confirm(\'' . TS_('Are you sure want to delete this picture?') . '\');',
                    ], [
                        'style' => 'margin-right:4px',
                    ]);
                    ...
                    $info_content .= $edited_User->get_crop_avatar_icon($user_Link->File->ID, [
                        'onclick' => 'return user_crop_avatar( ' . $edited_User->ID . ', ' . $user_Link->File->ID . ' )',
                    ]);
                    if (is_admin_page() && $can_moderate_user) { // Only if current user can edit this user
                        // Display a message about the duplicated profile picture
                        $info_content .= $user_Link->File->get_duplicated_files_message([
        ...
        if ($action != 'view') {	// If current user can edit this User:
            ...
            $Form->file_input('uploadfile[]', null, TB_('Upload a new picture'), '', [
                'size' => 10,
            ]);
            // Display a button to upload new picture:
            $Form->buttons([['submit', null, '> ' . /* TRANS: verb */ TB_('Upload!'), 'btn btn-primary ActionButton']]);
        }
    }

    $more_content = '';

    if ($action != 'view' && check_user_perm('files', 'view')) {	// If current user can edit this User:
