...
if ($new_user_creating) {
    $Form->begin_fieldset(TB_('New user') . get_manual_link('user-edit'), [
    ...
    $Form->text_input('edited_user_level', $edited_User->get('level'), 2, TB_('User level'), '[0 - 10]', [
        'required' => true,
    ]);

    $email_fieldnote = '<a href="mailto:' . $edited_User->get('email') . '">' . get_icon('email', 'imgtag', [
        'title' => TB_('Send an email'),
    ]) . '</a>';
    $Form->email_input('edited_user_email', $edited_User->email, 30, TB_('Email'), [
        'maxlength' => 255,
        'required' => true,
...
if (($url = $edited_User->get('url')) != '') {
    ...
    $url_fieldnote = '<a href="' . $url . '" target="_blank">' . get_icon('play', 'imgtag', [
        'title' => TB_('Visit the site'),
    ]) . '</a>';
} else {
    $url_fieldnote = '';
}
...
if ($action != 'view') {	// We can edit the values:
    if ($action != 'new') {
        ...
        if ($edited_User->has_avatar() || count($user_avatars)) { // If user uploaded at least one profile picture
            $change_picture_text = T_('Change') . ' &raquo;';
            $change_picture_title = T_('Change profile picture') . '...';
            $change_picture_icon = 'edit';
        } else { // If user has no profile picture yet
        ...
        if ($edited_User->has_avatar() && ($avatar_Link = &$edited_User->get_avatar_Link())) { // Display these actions only for existing avatar file
            $user_pictures .= '<br />'
                    ...
                    . $forbid_link
                    . action_icon(T_('Delete this profile picture'), 'delete', $delete_picture_url, ' ' . T_('Delete this profile picture'), 3, 4, [
                        'onclick' => 'return confirm(\'' . TS_('Are you sure want to delete this picture?') . '\');',
                    ]) . '<br />'
                    . $edited_User->get_rotate_avatar_icons($edited_User->avatar_file_ID, [
                        'before' => '',
                        'after' => '<br />',
                        'text' => ' ' . T_('Rotate'),
                        'user_tab' => 'avatar',
                    ])
                    . $edited_User->get_crop_avatar_icon($edited_User->avatar_file_ID, [
                        'before' => '',
                        'after' => '',
                        'text' => ' ' . T_('Crop'),
                        'user_tab' => 'avatar',
        ...
        foreach ($user_avatars as $user_Link) {
            $user_pictures .= $user_Link->get_tag([
    ...
    $Form->text_input('edited_user_login', $edited_User->login, 20, /* TRANS: noun */ TB_('Login'), '', [
        'maxlength' => 20,
        'required' => true,
    ]);
    ...
    if ((in_array($firstname_editing, $edited_user_perms) && $edited_User->ID == $current_User->ID) || ($firstname_editing != 'hidden' && $has_moderate_access)) {
        $Form->text_input('edited_user_firstname', $edited_User->firstname, 20, TB_('First name'), '', [
            'maxlength' => 50,
    ...
    if ((in_array($lastname_editing, $edited_user_perms) && $edited_User->ID == $current_User->ID) || ($lastname_editing != 'hidden' && $has_moderate_access)) {
        $Form->text_input('edited_user_lastname', $edited_User->lastname, 20, TB_('Last name'), '', [
            'maxlength' => 50,
    ...
    if ((in_array($nickname_editing, $edited_user_perms) && $edited_User->ID == $current_User->ID) || ($nickname_editing != 'hidden' && $has_moderate_access)) {
        $Form->text_input('edited_user_nickname', $edited_User->nickname, 20, TB_('Nickname'), '', [
            'maxlength' => 50,
    ...
    if ($edited_User->ID == $current_User->ID || $has_moderate_access) {
        $Form->radio('edited_user_gender', $edited_User->get('gender'), [
            ['M', TB_('A man')],
            ['F', TB_('A woman')],
            ['O', TB_('Other')],
        ], TB_('I am'), false, '');
    ...
    if (user_country_visible()) {	// Display a select input for Country
    ...
    if (user_region_visible()) {	// Display a select input for Region
        $regions_option_list = get_regions_option_list($edited_User->ctry_ID, $edited_User->rgn_ID, [
    ...
    if (user_subregion_visible()) {	// Display a select input for Subregion
        $subregions_option_list = get_subregions_option_list($edited_User->rgn_ID, $edited_User->subrg_ID, [
            'none_option_text' => TB_('Unknown'),
        ]);
        $Form->select_input_options(
    ...
    if (user_city_visible()) {	// Display a select input for City
        $cities_option_list = get_cities_option_list($edited_User->ctry_ID, $edited_User->rgn_ID, $edited_User->subrg_ID, $edited_User->city_ID, [
            'none_option_text' => TB_('Unknown'),
        ]);
        $Form->select_input_options(
            'edited_user_city_ID',
    ...
    if ($Settings->get('self_selected_age_group') != 'hidden') {
        $Form->begin_line(TB_('My age group'), 'edited_user_age_min', '', [
            'required' => $Settings->get('self_selected_age_group') == 'required',
        ]);
        $Form->text_input('edited_user_age_min', $edited_User->age_min, 3, '', '', [
            'required' => $Settings->get('self_selected_age_group') == 'required',
            'input_suffix' => ' ' . TB_('to') . ' ',
        ]);
        $Form->text_input('edited_user_age_max', $edited_User->age_max, 3, '', '', [
            'required' => $Settings->get('self_selected_age_group') == 'required',
        ]);
        $Form->end_line();
    }
    ...
    if ($Settings->get('birthday_year') != 'hidden' || $Settings->get('birthday_month') != 'hidden' || $Settings->get('birthday_day') != 'hidden') {
        $Form->begin_line(TB_('Birthday'), 'edited_user_birthday_month', '', [
            'required' => ($Settings->get('birthday_year') == 'required' || $Settings->get('birthday_month') == 'required' || $Settings->get('birthday_day') == 'required'),
        ]);
        if ($Settings->get('birthday_month') != 'hidden') {
            ...
            $Form->select_input_array('edited_user_birthday_month', $edited_User->birthday_month, $birthday_months, '', '', [
    ...
    $count_user_orgs = 0;
    if ($count_all_orgs > 0) { // Display an organization select box if at least one is defined
        $user_orgs = $edited_User->get_organizations_data();
        $org_allow_none = false;
        if (empty($user_orgs)) { // Set it for first(empty) organization select box
            $user_orgs[0] = 0;
        ...
        $org_add_icon = ' ' . get_icon('add', 'imgtag', [
            'class' => 'add_org',
            'style' => 'cursor:pointer' . $add_org_icon_style,
        ]);

        foreach ($user_orgs as $org_ID => $org_data) {
            $perm_edit_orgs = false;
            if (! empty($org_ID)) {	// $org_ID can be 0 for case when user didn't select an organization yet
            ...
            $org_remove_icon = ' ' . get_icon('minus', 'imgtag', [
                'class' => 'remove_org',
                'style' => 'cursor:pointer' . $remove_org_icon_style,
            ]);

            $form_infostart = $Form->infostart;
            $form_inputstart = $Form->inputstart;
            $inputstart_icon = '';
            if ($org_ID > 0) { // User is assigned to this organization, Display the accepted status icon
                if ($perm_edit_orgs) { // Set the spec params for icon if user is admin
                    $accept_icon_params = [
                        'style' => 'cursor:pointer',
                        'rel' => 'org_status_' . ($org_data['accepted'] ? 'y' : 'n') . '_' . $org_ID . '_' . $edited_User->ID,
                    ];
                } else {
                    $accept_icon_params = [];
                }
                if ($org_data['accepted']) { // Organization is accepted by admin
                    $accept_icon = get_icon('allowback', 'imgtag', array_merge([
                        'title' => TB_('Membership has been accepted.'),
                    ], $accept_icon_params));
                } else { // Organization is not accepted by admin yet
                    $accept_icon = get_icon('bullet_red', 'imgtag', array_merge([
                        'title' => TB_('Membership pending acceptance.'),
                    ], $accept_icon_params));
                }
                $inputstart_icon = $accept_icon . ' ';
            }

            if ($org_ID > 0 && ! $perm_edit_orgs && $org_data['accepted']) { // Display only info of the assigned organization
                ...
                if ($perm_edit_org_role) {
                    $org_role_input = ' &nbsp; <strong>' . TB_('Role') . ':</strong> ' .
                            $Form->text_input('org_roles[]', $org_data['role'], 20, '', '', [
                                'maxlength' => 255,
                            ]) . ' &nbsp; ';
                } else {
                    $org_role_input = (empty($org_data['role']) ? '' : ' &nbsp; <strong>' . TB_('Role') . ':</strong> ' . $org_data['role']) . ' &nbsp; '
                ...
                if ($perm_edit_org_priority) {
                    $org_priority_input = ' &nbsp; <strong>' . TB_('Order') . ':</strong> ' .
                            $Form->text_input('org_priorities[]', $org_data['priority'], 10, '', '', [
                                'type' => 'number',
                                'min' => -2147483648,
                                'max' => 2147483647,
                            ]) . ' &nbsp; ';
                ...
                $Form->select_input_object('organizations[]', $org_ID, $OrganizationCache, TB_('Organization'), [
    ...
    if ($new_user_creating) {
        $Form->text_input('edited_user_source', $edited_User->source, 30, TB_('Source'), '', [
            'maxlength' => 30,
        ]);
    }

    if ($edited_User->get_field_url() != '') {	// At least one url field is existing for current user
...
if (empty($edited_User->ID) && $action != 'view') {	// Display password fields for new creating user:
    $Form->begin_fieldset(TB_('Password'));
    $Form->radio('init_pass', param('init_pass', 'string', 'user'), [
        ['user', TB_('User must initialize')],
        ['admin', TB_('Initialize as below:')],
...
if (empty($edited_User->ID) && $action != 'view') {	// New user will be created with default multiple_session setting
...
if ($action != 'view') {	// Edit mode
    ...
    if ($Messages->has_errors()) {	// Display new added fields(from submitted form) only if errors are exist
        if (is_array($add_field_types) && count($add_field_types) > 0) {	// This case happens when user add a new required field and he doesn't fill it, then we should show all fields again
            foreach ($add_field_types as $add_field_type) {	// We use "foreach" because sometimes the user adds several fields with the same type
                $userfields = $DB->get_results('
    ...
    $Form->select_input('new_field_type', param('new_field_type', 'integer', 0), 'callback_options_user_new_fields', TB_('Add a field of type'), [
        'field_suffix' => $button_add_field,
    ]);
...
$Plugins->trigger_event('DisplayProfileFormFieldset', [
    'Form' => &$Form,
    'User' => &$edited_User,
    'edit_layout' => 'private',
...
if ($action != 'view') { // Edit buttons
    $action_buttons = [['', 'actionArray[update]', $new_user_creating ? TB_('Create User!') : TB_('Save Changes!'), 'SaveButton']];
    if ($is_admin) {
        // dh> TODO: Non-Javascript-confirm before trashing all settings with a misplaced click.
        $action_buttons[] = [
            'type' => 'submit',
            'name' => 'actionArray[default_settings]',
            'value' => TB_('Restore defaults'),
            'class' => 'ResetButton',
