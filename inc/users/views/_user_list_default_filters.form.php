...
$userlist_default_filters = empty($userlist_default_filters) ? [] : explode(',', $userlist_default_filters);
for ($i = 1; $i <= 10; $i++) {
    $Form->select_input_array(
        'filter_' . $i,
        (isset($userlist_default_filters[$i - 1]) ? $userlist_default_filters[$i - 1] : null),
        $filters_options,
        sprintf(TB_('Default filter criteria %d'), $i)
    );
}

$Form->end_fieldset();

$Form->button(['', 'actionArray[save_default_filters]', TB_('Save defaults'), 'SaveButton']);
