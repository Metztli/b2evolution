...
if (! empty($s_type)) {	// We want to filter on user field type:
...
if (check_user_perm('users', 'edit', false)) {	// We have permission to modify:
    $Results->grp_cols[] = [
        'td' => '$ufgp_order$',
        'td_class' => 'center',
    ];
    ...
    function grp_actions(&$row)
    {
        ...
        if (check_user_perm('users', 'edit', false)) {
            ...
            if (! in_array($row->ufgp_ID, $usedgroups)) { // delete
...
if (check_user_perm('users', 'edit')) {	// We have permission to modify:
    function order_actions(&$row)
    {
        ...
        if (in_array($row->ufdf_ID, $userfields_group_sides['first'])) {	// First record, no change ordering, print blank icon
        ...
        if (in_array($row->ufdf_ID, $userfields_group_sides['last'])) {	// Last record, no change ordering, print blank icon
            $r .= get_icon('move_down', 'noimg');
        } else {
            $r .= action_icon(
                T_('Move down'),
    ...
    $Results->global_icon(
        T_('Create a new user field...'),
        'new',
        '?ctrl=userfields&action=new',
        T_('New user field') . ' &raquo;',
        3,
        4,
        [
    ...
    $Results->global_icon(
        T_('Create a new user field group...'),
        'new',
        '?ctrl=userfieldsgroups&action=new',
        T_('New user field group') . ' &raquo;',
        3,
        4
    );
}
