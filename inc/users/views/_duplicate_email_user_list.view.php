...
if (get_param('action') == 'merge' && get_param('merging_user_ID') > 0) {	// Merging mode(1st or 2nd step):
    if (get_param('selected_user_ID') > 0) {	// Run and display a merging process if a remaining user is selected too:
        merge_users(get_param('merging_user_ID'), get_param('selected_user_ID'));
    } else {	// Enable a merging mode when only a merging user is selected without a remaining user:
        $users_results_params = array_merge([
