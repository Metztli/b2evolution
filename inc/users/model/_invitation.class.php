...
class Invitation extends DataObject
{
    ...
    public function load_from_Request()
    {
        ...
        // Expire date
        if (param_date('ivc_expire_date', sprintf(T_('Please enter a valid date using the following format: %s'), '<code>' . locale_input_datefmt() . '</code>'), true) && (param_time('ivc_expire_time'))) { // If date and time were both correct we may set the 'expire_ts' value
        ...
        if (mysql2timestamp($this->get('expire_ts')) < $localtimenow) { // Display a warning if date is expired
