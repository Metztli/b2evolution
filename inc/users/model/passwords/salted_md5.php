...
class saltedMd5PasswordDriver extends PasswordDriver
{
    ...
    {
        if (empty($salt)) {	// Generate salt for new hashing:
...
';
    ...
    {
        if (empty($salt) || strlen($salt) != $this->salt_length) {	// Wrong salt format:
            return false;
        }

        $count_log2 = strpos($this->itoa64, $salt[0]);

        if ($count_log2 < 7 || $count_log2 > 30) {	// Not allowed value:
            return false;
        }
