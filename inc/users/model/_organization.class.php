...
class Organization extends DataObject
{
     ...
     */
    public $perm_role = 'owner and member';
    ...
    public function __construct($db_row = null)
    {
        ...
        } else {	// Set default organization data for new object:
            if (is_logged_in()) {
                global $current_User;
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                ...
                'fk' => 'uorg_org_ID',
                'msg' => T_('%d users in this organization'),
            ],
        ];
    }
    ...
    public function load_from_Request()
    {
        ...
        // Owner:
        if (check_user_perm('orgs', 'edit')) {	// Update the owner if current user has a permission to edit all organizations:
        ...
        } elseif (empty($this->ID)) {	// Set onwer user ID on creating new poll:
            $this->set('owner_user_ID', $current_User->ID);
     ...
     */
    public function get_users($order_by = 'user_id', $accepted_only = false)
    {
        global $DB;

        $users = [];

        if (empty($this->ID)) { // Return empty array for new creating organization
            return $users;
        }
     ...
     * @return boolean
     */
    public function can_be_autoaccepted()
    {
        if ($this->get('accept') == 'yes') {	// This organization should be accepted immediately:
            return true;
        } else {	// Check permission:
            if (check_user_perm('orgs', 'edit', false, $this)) {	// If current user has a perm to edit this organization then also allow to auto accept it:
                return true;
            }
        }
