...
class UserCache extends DataObjectCache
{
    ...
    */
     ...
     */
    public function &get_by_login($login, $force_db_check = true)
    {
        // Make sure we have a lowercase login:
        // We want all logins to be lowercase to guarantee uniqueness regardless of the database case handling for UNIQUE indexes.
        $login = utf8_strtolower($login);

        if (! ($force_db_check || isset($this->cache_login[$login]))) { // force db check is false and this login is not set in the cache it means that user with the given login doesn't exists
     ...
     * @return false|User
     */
    public function &get_by_loginAndPwd($login, $pass, $pass_is_hashed = true)
    {
        if (! ($User = &$this->get_by_login($login))) {
            return false;
        }

        if (! $User->check_password($pass, $pass_is_hashed)) {	// The password doesn't match
            return false;
        }
     ...
     */
    public function get_by_emailAndPwd($email, $pass, $pwd_hashed = null, $pepper = null)
    {
        global $DB;
        ...
        if (empty($result)) { // user was not found with the given email address
            return false;
        }
        ...
        foreach ($result as $row) {
            ...
            if (empty($pwd_hashed)) {
                ...
                if (! $user_PasswordDriver || ! $user_PasswordDriver->check($pass, $row->user_salt, $row->user_pass)) { // password doesn't match
                    continue;
                }
            } else {
                $pwd_matched = false;
                foreach ($pwd_hashed as $encrypted_password) {
                    ...
                    if ($pwd_matched) { // The corresponding user was found
                        break;
                    }
                }
                if (! $pwd_matched) { // If the user with matched login credentials was not fount continue with the next row
                    continue;
                }
            }
            // a user with matched password was found
            $first_matched_index = $index;
            if (($row->user_status == 'activated') || ($row->user_status == 'autoactivated') || ($row->user_status == 'manualactivated')) { // an activated user was found, break from the iteration
                $User = new User($row);
                break;
            }
        ...
        if (! isset($User)) { // There is no activated user with the given email and password
            if (isset($first_notclosed_User)) { // Get first not closed user with the given email and password
                $User = $first_notclosed_User;
            } elseif ($first_matched_index !== false) { // There is only closed user with the given email and password
                $User = new User($result[$first_matched_index]);
            } else { // No matched user was found
                return false;
            }
        }
     ...
     */
    public function load_blogmembers($blog_ID, $limit = 0, $load_members = true)
    {
        global $DB, $Debuglog;

        $BlogCache = &get_BlogCache();
        if (! ($Collection = $Blog = &$BlogCache->get_by_ID($blog_ID, false, false))) {	// Wrong request:
        ...
        if (! $load_members && ! empty($blog_ID)) {	// Load assignees only when advanced perms are available and workflow is used:
            $load_assignees = $Blog->get('advanced_perms') && $Blog->get_setting('use_workflow');
        }
        if ($load_assignees) { // Load users which can be assigneed to items of the blog
            $cache_name = 'blogassignees';
            $db_field = 'can_be_assignee';
        } else { // Load members of the blog
        ...
        if ($owner_User = &$Blog->get_owner_User()) {	// Add collection owner to the cache:
            $this->add($owner_User);
        }
        ...
        if ($load_members) {	// Get members which primary groups have a setting to VIEW/EDIT ALL collections:
            $group_setting_SQL->WHERE('grp_perm_blogs = "viewall" OR grp_perm_blogs = "editall"');
        } else {	// Get assignees which primary groups have a setting to EDIT ALL collections:
        ...
        foreach ($users as $row) {
            if (! isset($this->cache[$row->user_ID])) { // Save reinstatiating User if it's already been added
     ...
     */
    public function get_blog_member_option_list($blog_ID, $default = 0, $allow_none = false, $always_load_default = false)
    {
        if ($blog_ID) { // Load requested blog members:
        ...
        } else { // No blog specified: load ALL members:
            $this->load_all();
        }
