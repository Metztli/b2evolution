...
class Userfield extends DataObject
{
    ...
    public $required = 'recommended';

    public $visibility = 'unrestricted';

    public $duplicated = 'allowed';
     ...
     */
    public static function format_options($options, $format = null)
    {
        if ($format == 'radio') {	// Format for radio form element:
            $formatted_options = [];
     ...
     * @return array
     */
    public static function get_requireds($format = null)
    {
        return Userfield::format_options([
            'require' => T_('Required'),
     ...
     * @return array
     */
    public static function get_visibilities($format = null)
    {
        return Userfield::format_options([
     ...
     * @return array
     */
    public static function get_duplicateds($format = null)
    {
        return Userfield::format_options([
     ...
     * @return integer
     */
    public function get_last_order($group_ID)
    {
        global $DB;
    ...
    public function load_from_Request()
    {
        ...
        // User group
        if ($this->get('type') == 'user') {	// Group is required for type "User select":
        ...
        } else {	// Reset group for not user select type:
        ...
        // Options
        if (param('ufdf_type', 'string') == 'list') { // Save 'Options' only for Field type == 'Option list'
            $ufdf_options = param('ufdf_options', 'text');
            if (count(explode("\n", $ufdf_options)) < 2) { // We don't want save an option list with one item
                param_error('ufdf_options', T_('Please enter at least 2 options on 2 different lines.'));
            } elseif (utf8_strlen($ufdf_options) > 255) { // This may not happen in normal circumstances because the textarea max length is set to 255 chars
        ...
        // Order
        if ($old_group_ID != $this->ufgp_ID) { // Group is changing, set order as last
            $this->set('order', $this->get_last_order($this->ufgp_ID));
        }

        // Suggest
        if (param('ufdf_type', 'string') == 'word') { // Save 'Suggest values' only for Field type == 'Single word'
        ...
        if (! param_errors_detected()) { // Field code must be unique, Check it only when no errors on the form
            if ($field_ID = $this->dbexists('ufdf_code', $this->get('code'))) { // We have a duplicate entry:
                param_error(
                    'ufdf_code',
                    sprintf(
                        T_('Another user field already uses this code. Do you want to <a %s>edit the existing user field</a>?'),
