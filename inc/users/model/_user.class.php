...
class User extends DataObject
{
     ...
     * User fields
     */
    public $userfields_loaded = false;

    public $userfields = [];

    public $userfields_by_code = [];

    public $userfields_by_type = [];
     ...
     * @var boolean
     */
    public $newsletter_subscriptions_updated = false;
     ...
     * @var array
     */
    public $newsletter_subscription_changed_values = [];
    ...
    public function __construct($db_row = null)
    {
        ...
        if ($db_row == null) { // Setting those object properties, which are not "NULL" in DB (MySQL strict mode):
            ...
            } else { // We don't know local time here!
                $this->set_datecreated(time());
                $this->set('profileupdate_date', date('Y-m-d H:i:s'));
            }

            if (isset($Settings)) { // Set Group for this user:
    ...
    public static function get_class_db_config()
    {
        ...
        if (! isset($user_db_config)) {
            $user_db_config = array_merge(
                parent::get_class_db_config(),
                [
                    'dbtablename' => 'T_users',
                    'dbprefix' => 'user_',
                    'dbIDname' => 'user_ID',
    ...
    public static function get_delete_restrictions()
    {
        ...
        return [
            [
                'table' => 'T_blogs',
                'fk' => 'blog_owner_user_ID',
                'msg' => T_('%d blogs owned by this user'),
            ],
            //array( 'table'=>'T_items__item', 'fk'=>'post_lastedit_user_ID', 'msg'=>T_('%d posts last edited by this user') ),
            [
                'table' => 'T_items__item',
                'fk' => 'post_assigned_user_ID',
                'msg' => T_('%d posts assigned to this user'),
            ],
            [
                'table' => 'T_users__organization',
                'fk' => 'org_owner_user_ID',
                'msg' => T_('%d organizations'),
            ],
            [
                'table' => 'T_polls__question',
                'fk' => 'pqst_owner_user_ID',
                'msg' => T_('%d polls owned by this user'),
            ],
            [
                'table' => 'T_automation__automation',
                'fk' => 'autm_owner_user_ID',
                'msg' => T_('%d automations'),
            ],
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_users__usersettings',
                'fk' => 'uset_user_ID',
                'msg' => T_('%d user settings on collections'),
            ],
            [
                'table' => 'T_sessions',
                'fk' => 'sess_user_ID',
                'msg' => T_('%d sessions opened by this user'),
                'class' => 'Session',
                'class_path' => 'sessions/model/_session.class.php',
            ],
            [
                'table' => 'T_coll_user_perms',
                'fk' => 'bloguser_user_ID',
                'msg' => T_('%d user permissions on blogs'),
            ],
            [
                'table' => 'T_comments__votes',
                'fk' => 'cmvt_user_ID',
                'msg' => T_('%d user votes on comments'),
            ],
            [
                'table' => 'T_subscriptions',
                'fk' => 'sub_user_ID',
                'msg' => T_('%d blog subscriptions'),
            ],
            [
                'table' => 'T_items__item',
                'fk' => 'post_creator_user_ID',
                'msg' => T_('%d posts created by this user'),
                'class' => 'Item',
            ...
            [
                ...
                'fk' => 'isub_user_ID',
                'msg' => T_('%d post subscriptions'),
            ],
            [
                'table' => 'T_messaging__contact',
                'fk' => 'mct_to_user_ID',
                'msg' => T_('%d contacts from other users contact list'),
            ],
            [
                'table' => 'T_messaging__contact',
                'fk' => 'mct_from_user_ID',
                'msg' => T_('%d contacts from this user contact list'),
            ],
            [
                'table' => 'T_messaging__contact_groups',
                'fk' => 'cgr_user_ID',
                'msg' => T_('%d contact groups'),
            ],
            [
                'table' => 'T_messaging__contact_groupusers',
                'fk' => 'cgu_user_ID',
                'msg' => T_('%d contacts from contact groups'),
            ],
            [
                'table' => 'T_pluginusersettings',
                'fk' => 'puset_user_ID',
                'msg' => T_('%d user settings on plugins'),
            ],
            [
                'table' => 'T_users__fields',
                'fk' => 'uf_user_ID',
                'msg' => T_('%d user fields'),
            ],
            [
            ...
            [
                'table' => 'T_files',
                'fk' => 'file_root_ID',
                'and_condition' => 'file_root_type = "user"',
                'msg' => T_('%d files from this user file root'),
                'style' => 'bold',
            ],
            [
                'table' => 'T_email__campaign_send',
                'fk' => 'csnd_user_ID',
                'msg' => T_('%d list emails for this user'),
            ],
            [
                'table' => 'T_users__reports',
                'fk' => 'urep_target_user_ID',
                'msg' => T_('%d reports about this user'),
            ],
            [
                'table' => 'T_users__reports',
                'fk' => 'urep_reporter_ID',
                'msg' => T_('%d reports created by this user'),
            ],
            [
                'table' => 'T_users__user_org',
                'fk' => 'uorg_user_ID',
                'msg' => T_('%d organization membership'),
            ],
            [
                'table' => 'T_users__usertag',
                'fk' => 'uutg_user_ID',
            ...
            [
                'table' => 'T_users__secondary_user_groups',
                'fk' => 'sug_user_ID',
                'msg' => T_('%d secondary groups'),
            ],
            [
                'table' => 'T_users__profile_visits',
                'fk' => 'upv_visited_user_ID',
                'msg' => T_('%d profile visits'),
            ],
            [
                'table' => 'T_users__profile_visit_counters',
                'fk' => 'upvc_user_ID',
                'msg' => T_('%d profile visit counter'),
            ],
            [
                'table' => 'T_users__social_network',
                'fk' => 'usn_user_ID',
                'msg' => T_('%d connected social networks'),
            ],
            [
                'table' => 'T_email__newsletter_subscription',
                'fk' => 'enls_user_ID',
                'msg' => T_('%d list subscriptions'),
            ],
            [
                'table' => 'T_automation__user_state',
                'fk' => 'aust_user_ID',
                'msg' => T_('%d states of User in Automation'),
            ],
        ];
    }
     ...
     */
    public function init_relations($params = [])
    {
        if (! is_null($this->delete_cascades) || ! is_null($this->delete_restrictions)) { // Initialize the relations only once
            return;
        }
        ...
        if ($params['delete_messages']) {	// Delete messages:
            $this->delete_cascades[] = [
                'table' => 'T_messaging__message',
                'fk' => 'msg_author_user_ID',
                'msg' => T_('%d messages from this user'),
                'class' => 'Message',
                'class_path' => 'messaging/model/_message.class.php',
                'style' => 'bold',
            ];
            $this->delete_cascades[] = [
                'table' => 'T_messaging__threadstatus',
                'fk' => 'tsta_user_ID',
                'msg' => T_('%d message read statuses from this user'),
            ];
        }
        if ($params['delete_comments']) {	// Delete comments:
            $this->delete_cascades[] = [
                'table' => 'T_comments',
                'fk' => 'comment_author_user_ID',
                'msg' => T_('%d comments by this user'),
                'class' => 'Comment',
                'class_path' => 'comments/model/_comment.class.php',
                'style' => 'bold',
            ];
        }
        if ($params['delete_files']) {	// Delete files and links:
            $this->delete_cascades[] = [
                'table' => 'T_links',
                'fk' => 'link_creator_user_ID',
                'msg' => T_('%d links created by this user'),
                'class' => 'Link',
                'class_path' => 'links/model/_link.class.php',
            ];
            $this->delete_cascades[] = [
                'table' => 'T_files',
                'fk' => 'file_creator_user_ID',
                'and_condition' => 'file_root_type != "user"',
                'msg' => T_('%d files will lose their creator ID.'),
            ];
        }
    }
    ...
    {
        if ($restriction['table'] == 'T_polls__question') {	// Check restriction for poll questions:
            global $DB, $admin_url;
            ...
            if ($polls_count > 0) {	// Display this restriction as link to polls list with filter by this user login:
                $msg = sprintf($restriction['msg'], $polls_count);
                if (check_user_perm('polls', 'view')) {	// Set a link to poll questions list with filter by this user if current user has a perm to view all polls:
                    $msg = '<a href="' . $admin_url . '?ctrl=polls&amp;owner=' . $this->login . '">' . $msg . '</a>';
                }
                return $msg;
            } else {	// No restriction for current table:
                return '';
            }
        }
     ...
     */
    public function check_delete($restrict_title, $ignore = [], $addlink = false)
    {
        // Prepend user login before restriction messages to know what user is deleting:
        $user_login_link = $this->get_identity_link([
            'link_text' => 'login',
        ]);
     ...
     */
    public function load_from_register_form()
    {
        ...
        if ($request_password_confirmation) {	// Request a password confirmation:
        ...
        if ($is_new_user ||
                ...
                ($this->get('pass_driver') == 'nopass')) {	// Current password is not required:
            ...
            if ($request_password_confirmation) {	// Request a password confirmation:
                if (param_check_passwords('edited_user_pass1', 'edited_user_pass2', true, $Settings->get('user_minpwdlen'))) { // We can set password
                    $this->set_password($edited_user_pass2);
                }
            } else {	// Don't request a password confirmation and use only first entered password (Used on REST API):
            ...
            if ($this->ID != $current_User->ID) { // Set the messages when admin changes a password of other user
                $checkpwd_params = [
            ...
            if (! strlen($current_user_pass)) {
                ...
                if ($has_full_access && $this->ID != $current_User->ID) {	// Admin is changing a password of other user, Check a password of current admin
                    $pass_User = $current_User;
                } else {	// User is changing own password
                    $pass_User = $this;
                }
                if ($pass_User->check_password($current_user_pass)) {
                    if (param_check_passwords('edited_user_pass1', 'edited_user_pass2', true, $Settings->get('user_minpwdlen'), $checkpwd_params)) { // We can set password
                        $this->set_password($edited_user_pass2);
                    }
                } else {
        ...
        if ($_POST['edited_user_pass1'] != trim($_POST['edited_user_pass1']) ||
                $_POST['edited_user_pass2'] != trim($_POST['edited_user_pass2'])) { // If new password was entered with spaces then inform user about this
        ...
        $paramsList = [];
        if (in_array('country', $required_fields)) {	// Set and check country:
            $this->set('ctry_ID', $country);
            $paramsList['country'] = $country;
        }
        if (in_array('firstname', $required_fields)) {	// Set and check first name:
            $this->set('firstname', $firstname);
            $paramsList['firstname'] = $firstname;
        }
        if (in_array('lastname', $required_fields)) {	// Set last name:
            $this->set('lastname', $lastname);
            $paramsList['lastname'] = $lastname;
        }
        if (in_array('gender', $required_fields)) {	// Set or check gender:
            $this->set('gender', $gender);
            $paramsList['gender'] = $gender;
        }
        if (in_array('country', $required_fields)) {	// Only update locale without checking:
    ...
    public function load_from_Request()
    {
        ...
        if (($is_admin_form || ($is_identity_form && $is_new_user) || ($is_api_request && $is_new_user)) && $has_moderate_access) { // level/group and email options are displayed on identity form only when creating a new user.
            if ($this->ID != 1) { // the admin user group can't be changed
                ...
                if (isset($edited_user_Group)) {
                    ...
                    if ($edited_user_Group->can_be_assigned()) {	// Update group only if current user has a permission for this:
            ...
            if ($edited_user_secondary_grp_IDs !== null) {	// Save secondary groups for this user:
            ...
            if ($is_admin_form) {	// Admin form
                ...
                if (isset($notification_sender_email)) {
                    ...
                    if (! empty($notification_sender_email)) { // Change a value of setting
                        $UserSettings->set('notification_sender_email', $notification_sender_email, $this->ID);
                    } elseif ($UserSettings->get('notification_sender_email', $this->ID) != '') { // Delete a setting record from DB
                ...
                if (isset($notification_sender_name)) {
                    if (! empty($notification_sender_name)) { // Change a value of setting
                        $UserSettings->set('notification_sender_name', $notification_sender_name, $this->ID);
                    } elseif ($UserSettings->get('notification_sender_name', $this->ID) != '') { // Delete a setting record from DB
                        $UserSettings->delete('notification_sender_name', $this->ID);
                    }
                }

                if ($has_full_access && ! isset($this->dbchanges['user_email'])) {	// If email address is not changed
                    ...
                    if (! $EmailAddress && $edited_email_status != 'unknown') {	// Create new record in the T_email_address table
                        $EmailAddress = new EmailAddress();
                        $EmailAddress->set('address', $this->get('email'));
                    }
                    if (! empty($EmailAddress)) {	// Save status of an email address
                        $EmailAddress->set('status', $edited_email_status);
                        $EmailAddress->dbsave();
                    }
                }

                if (check_user_perm('spamblacklist', 'edit')) { // User can edit IP ranges
                    ...
                    if (! $IPRange && ! empty($edited_iprange_status)) { // IP range doesn't exist in DB, Create new record
                    ...
                    if ($IPRange) { // Save status of IP range
                        if ($IPRange->get('status') != 'blocked' && $edited_iprange_status == 'blocked') {	// Status was changed to blocked, we should increase counter
                            $IPRange->set('block_count', $IPRange->block_count + 1);
                        } elseif ($IPRange->get('status') == 'blocked' && $edited_iprange_status != 'blocked') { // Status was changed from blocked to another, we should decrease counter
                            $IPRange->set('block_count', $IPRange->block_count - 1);
                        }
                ...
                if (check_user_perm('stats', 'edit')) { // User can edit Domains
                    ...
                    if (! $Domain && $edited_email_domain_status != 'unknown' && ! empty($email_domain)) {	// Domain doesn't exist in DB, Create new record:
                        $Domain = new Domain();
                        $Domain->set('name', $email_domain);
                    }
                    if ($Domain) {	// Save status of Domain:
                    ...
                    if (! $Domain && $edited_domain_status != 'unknown' && ! empty($user_domain)) { // Domain doesn't exist in DB, Create new record
                        $Domain = new Domain();
                    ...
                    if (! $Domain && $edited_initial_referer_status != 'unknown' && ! empty($initial_referer_domain)) { // Domain doesn't exist in DB, Create new record
                        $Domain = new Domain();
        ...
        if ($has_moderate_access) {	// If current user can moderate other users:
            if (param('edited_user_tags', 'string', null) !== null) {	// Update user tags if they has been submitted:
        ...
        if ($is_identity_form || ($is_api_request && $is_new_user)) {
            ...
            if ($Settings->get('self_selected_age_group') != 'hidden') {
                ...
                if (isset($age_min) || isset($age_max) || $is_identity_form) {
                    ...
                    if (! param_has_error('edited_user_age_min') && $Settings->get('minimum_age') > 0 &&
                            ! empty($edited_user_age_min) && $edited_user_age_min < $Settings->get('minimum_age')) {	// Limit user by minimum age
            ...
            $birthdate = [];
            if ($Settings->get('birthday_month') != 'hidden') {	// Save Birthday Month
            ...
            if ($Settings->get('birthday_day') != 'hidden') {	// Save Birthday Day
            ...
            if ($Settings->get('birthday_year') != 'hidden') {	// Save Birthday Year
                ...
                if (isset($edited_user_birthday_year) || $is_identity_form) {
                    ...
                    if (isset($edited_user_birthday_year)) {
                        ...
                        if ($Settings->get('minimum_age') > 0 && ! empty($edited_user_birthday_year)) { 	// We can only assume age if year is provided:
                            if (empty($birthdate['month'])) {	// Assume latest month:
                                $birthdate['month'] = 12;
                            }
                            if (empty($birthdate['day'])) {	// Assume latest day of the month-year:
            ...
            if ((in_array($firstname_editing, $edited_user_perms) && $this->ID == $current_User->ID) || ($firstname_editing != 'hidden' && $has_moderate_access)) {	// User has a permissions to save Firstname
                ...
                if (isset($edited_user_firstname) || $is_identity_form) {
                    if ($firstname_editing == 'edited-user-required') {	// First name is required
            ...
            if ((in_array($lastname_editing, $edited_user_perms) && $this->ID == $current_User->ID) || ($lastname_editing != 'hidden' && $has_moderate_access)) {	// User has a permissions to save Lastname
                ...
                if (isset($edited_user_lastname) || $is_identity_form) {
                    if ($lastname_editing == 'edited-user-required') {	// Last name is required
            ...
            if ((in_array($nickname_editing, $edited_user_perms) && $this->ID == $current_User->ID) || ($nickname_editing != 'hidden' && $has_moderate_access)) {	// User has a permissions to save Nickname
                ...
                if (isset($edited_user_nickname) || $is_identity_form) {
                    if ($nickname_editing == 'edited-user-required') {	// Nickname is required
            ...
            if ($this->ID == $current_User->ID || $has_moderate_access) {	// No user latitude setting:
            ...
            if (user_country_visible()) { // Save country
                ...
                if (isset($edited_user_ctry_ID) || $is_identity_form) {
                    if ($country_is_required && $has_moderate_access && $edited_user_ctry_ID == 0) { // Display a note message if user can edit all users
            ...
            if (user_region_visible()) { // Save region
                ...
                if (isset($edited_user_rgn_ID) || $is_identity_form) {
                    if ($region_is_required && $has_moderate_access && $edited_user_rgn_ID == 0) { // Display a note message if user can edit all users
            ...
            if (user_subregion_visible()) { // Save subregion
                ...
                if (isset($edited_user_subrg_ID) || $is_identity_form) {
                    if ($subregion_is_required && $has_moderate_access && $edited_user_subrg_ID == 0) { // Display a note message if user can edit all users
            ...
            if (user_city_visible()) { // Save city
                ...
                if (isset($edited_user_city_ID) || $is_identity_form) {
                    if ($city_is_required && $has_moderate_access && $edited_user_city_ID == 0) { // Display a note message if user can edit all users
            ...
            foreach ($userfield_IDs as $userfield) {
                if (! isset($this->userfield_defs[$userfield->uf_ufdf_ID])) {	// If user field definition doesn't exist in DB then delete field value of this user:
                    $this->userfield_update($userfield->uf_ID, null);
                    continue;
                }

                if (! userfield_is_viewable($this->ID, $this->userfield_defs[$userfield->uf_ufdf_ID][5], $this->userfield_defs[$userfield->uf_ufdf_ID][0])) {	// Current user cannot update the user field:
                    continue;
                }

                $uf_val = param('uf_' . $userfield->uf_ID, 'raw');
                $field_type = $this->userfield_defs[$userfield->uf_ufdf_ID][0];
                if ($field_type == 'number') {	// Change number type of integer because we have this type name preparing in function param_format():
                    $field_type = 'integer';
                } elseif ($field_type != 'text' && $field_type != 'url') {	// Use all other params as string, Only text and url have a preparing in function param_format():
                    $field_type = 'string';
                }
                $uf_val = param_format($uf_val, $field_type);

                if ($this->userfield_defs[$userfield->uf_ufdf_ID][0] == 'list' && $uf_val == '---') {	// Option list has a value '---' for empty value
                    $uf_val = '';
                }

                if (empty($uf_val) && $this->userfield_defs[$userfield->uf_ufdf_ID][2] == 'require') {	// Display error for empty required field
                    if ($has_full_access) {	// Display a note message if user can edit all users
                    ...
                    if ($this->userfield_defs[$userfield->uf_ufdf_ID][0] == 'url') {	// Check url fields
                        param_check_url('uf_' . $userfield->uf_ID, 'commenting');
                    }
                    if ($this->userfield_defs[$userfield->uf_ufdf_ID][4] == 'list') {	// Option "Multiple values" == "List style"
                        // Split by comma and save each phrase as separate field
                        $uf_val = explode(',', $uf_val);
                        foreach ($uf_val as $v => $val) {
                            $val = trim($val);
                            if ($v == 0) {	// Update field with first value
                                $this->userfield_update($userfield->uf_ID, $val);
                            } elseif (! empty($val)) {	// Add a new field for new values
                                $this->userfield_add($userfield->uf_ufdf_ID, $val);
                            }
                        }
                    } else {	// Forbidden & Allowed fields
            ...
            if ($is_new_user) {
                ...
                if ($user_id !== 0) {
                    ...
                    foreach ($copied_userfields as $copied_userfield) {
                        ...
                        $uf_val = param_format($uf_val, $field_type);
                        if (! empty($uf_val)) {	// Copy user field only if it is not empty:
            ...
            if ($action == 'add_field') {	// Button 'Add' new field is pressed
                $new_field_type = param('new_field_type', 'integer', 0);
                if (empty($new_field_type)) {	// We cannot add a new field without type
                    param_error('new_field_type', T_('Please select a field type.'));
                } else {	// Save an adding field(in the array) to display it again if errors will be exist
                    $new_field_type_exists = false;

                    if ($this->userfield_defs[$new_field_type][4] == 'allowed' || $this->userfield_defs[$new_field_type][4] == 'list') {	// This field can be duplicated
                        global $add_field_types;
                        $add_field_types = [$new_field_type];
                    } else {	// We should to solve we can add this field or don't
                        if (! isset($uf_new_fields[$new_field_type]) && ! isset($uf_add_fields[$new_field_type])) {	// User is adding this field first time
                            if (is_array($userfield_IDs) && count($userfield_IDs) > 0) {	// User has fields that saved in DB
                                foreach ($userfield_IDs as $userfield) {
                                    if ($userfield->uf_ufdf_ID == $new_field_type) {	// New adding field already exists for current user in DB
                                        $new_field_type_exists = true;
                                        break;
                                    }
                                }
                            }
                            if (! $new_field_type_exists) {	// Field doesn't still exist for current user
                                global $add_field_types;
                                $add_field_types = [$new_field_type];
                            }
                        } else {	// Field exists, no duplicates available
                            $new_field_type_exists = true;
                        }

                        if ($new_field_type_exists) {	// Field already is added for current user, we should display error about this
                            param_error('new_field_type', T_('You already added this field, please select another.'));
                        }
                    }

                    if (! $new_field_type_exists) {	// Mark a new field to enter a value
            ...
            if (count($uf_new_fields) > 0 || count($uf_add_fields) > 0) {
                ...
                foreach ($uf_fields as $uf_type => $uf_new_fields) {
                    if ($uf_type == 'add') {	// Save an adding fields to display it again if errors will be exist
                        global $add_field_types;
                        if (! isset($add_field_types)) {	// Don't rewrite already existing array
                            $add_field_types = [];
                        }
                    }
                    foreach ($uf_new_fields as $uf_new_id => $uf_new_vals) {
                        if (! userfield_is_viewable($this->ID, $this->userfield_defs[$uf_new_id][5], $this->userfield_defs[$uf_new_id][0])) {	// Current user cannot add the user field:
                            continue;
                        }

                        foreach ($uf_new_vals as $uf_new_val) {
                            if ($this->userfield_defs[$uf_new_id][0] == 'list' && $uf_new_val == '---') {	// Option list has a value '---' for empty value
                                $uf_new_val = '';
                            }

                            if ($uf_new_val != '') {	// Insert a new field in DB if it is filled:
                                ...
                                if ($this->userfield_defs[$uf_new_id][4] == 'list') {	// Option "Multiple values" == "List style"
                                    // Split by comma and save each phrase as separate field
                                    $uf_new_val = explode(',', $uf_new_val);
                                    foreach ($uf_new_val as $val) {
                                        $val = param_format($val, 'string');
                                        if (! empty($val)) {	// Exclude empty values(spaces)
                            ...
                            } elseif (empty($uf_new_val) && $this->userfield_defs[$uf_new_id][2] == 'require') {	// Display error for empty required field & new adding field
                                if ($has_full_access) {	// Display a note message if user can edit all users
                            ...
                            if ($uf_type == 'add') {	// Save new added field, it used on the _user_identity.form
        ...
        if ($is_password_form || $is_new_user) {
            ...
            if ($is_new_user ||
                ...
                if ($is_new_user && param('init_pass', 'string') == 'user') {	// Allow to create new user without password from back-office by admin:
                    $this->set('pass_driver', 'nopass');
                    $this->set('pass', '');
                    $this->set('salt', '');
                } elseif ($request_password_confirmation) {	// Request a password confirmation:
                    if (param_check_passwords('edited_user_pass1', 'edited_user_pass2', true, $Settings->get('user_minpwdlen'))) { // We can set password
                        $this->set_password($edited_user_pass2);
                    }
                } else {	// Don't request a password confirmation and use only first entered password (Used on REST API):
        ...
        if ($is_subscriptions_form) {
            if ($action == 'subscribe') { // Do only subscribe to new blog (Don't update the user's settings from the same form)
                ...
                if ($notifications_mode != 'off') {
                    ...
                    if ($subscribe_blog_ID) { // We need to record values:
                        $BlogCache = &get_BlogCache();
                        $subscribe_Blog = &$BlogCache->get_by_ID($subscribe_blog_ID, false, false);

                        if ($subscribe_Blog->has_access($this)) { // user has access to the collection
                            // erhsatingin > it seems we do need to check if subscription is allowed, at least for new posts
                            if (! $subscribe_Blog->get_setting('allow_subscriptions')) { // Subscription to new posts is not allowed so we subscribe to new comments instead
                                $sub_comments = 1;
                            }
                    ...
                    } else { // Display an error message to inform user about incorrect actions
                        $Messages->add(T_('Please select at least one setting to subscribe on the selected blog.'), 'error');
                    }
                }
            } else { // Update user's settings
                // Email communication
                ...
                if (($emails_msgform == 'userset') || (($emails_msgform == 'adminset') && $has_full_access)) { // enable email option is displayed only if user can set or if admin can set and current User is an administrator
                ...
                if ($has_messaging_perm) { // update 'notify messages' only if user has messaging rights and this option was displayed
                ...
                if ($this->check_role('post_owner')) { // update 'notify_published_comments' only if user has at least one post or user has right to create new post
                ...
                if ($is_comment_moderator || $this->check_role('comment_editor')) {	// update 'notify_comment_moderation', 'notify_edit_cmt_moderation' and 'notify_spam_cmt_moderation' only if user is comment moderator/editor at least in one collection:
                ...
                if ($this->check_perm('admin', 'restricted', false)) {	// update 'notify_meta_comment_mentioned' ans 'notify_meta_comments' only if edited user has a permission to back-office:
                ...
                if ($is_comment_moderator) { // update 'send_cmt_moderation_reminder' only if user is comment moderator at least in one blog
                ...
                if ($this->check_role('post_moderator')) {	// update 'notify_post_moderation', 'notify_edit_pst_moderation' and 'send_cmt_moderation_reminder' only if user is post moderator at least in one collection:
                ...
                if ($Settings->get('inactive_account_reminder_threshold') > 0) {	// If setting "Trigger after" of cron job "Send reminders about inactive accounts" is selected at least to 1 second:
                    $UserSettings->set('send_inactive_reminder', param('edited_user_send_inactive_reminder', 'integer', 0), $this->ID);
                }

                if ($this->check_perm('users', 'edit')) { // edited user has permission to edit all users, save notification preferences
                ...
                if ($this->check_perm('options', 'edit')) { // edited user has permission to edit options, save notification preferences
                    $UserSettings->set('notify_cronjob_error', param('edited_user_notify_cronjob_error', 'integer', 0), $this->ID);
                }

                if ($has_full_access && $this->check_perm('options', 'view')) {	// current User is an administrator and the edited user has a permission to automations:
                ...
                if ($notifications_mode != 'off') {
                    ...
                    if ($subs_blog_IDs) {
                        ...
                        foreach ($subs_blog_IDs as $loop_blog_ID) {
                            ...
                            if ($sub_items || $sub_comments || $sub_items_mod) {	// We have a subscription for this blog
                                $subscription_values[] = "( $loop_blog_ID, $this->ID, $sub_items, $sub_items_mod, $sub_comments )";
                            } else {	// No subscription here:
                    ...
                    // Note: we do not check if subscriptions are allowed here, but we check at the time we're about to send something
                    if (count($subscription_values)) {	// We need to record values:
                        $DB->query('REPLACE INTO T_subscriptions( sub_coll_ID, sub_user_ID, sub_items, sub_items_mod, sub_comments )
                    ...
                    if (count($unsubscribed)) {	// We need to make sure some values are cleared:
                    ...
                    if (! empty($subs_item_IDs)) { // user was subscribed to at least one post update notification
                        ...
                        foreach ($subs_item_IDs as $loop_item_ID) {
                            ...
                            if (! param('item_sub_' . $loop_item_ID, 'integer', 0) && ! in_array($loop_item_ID, $sub_group_IDs)) { // user wants to unsubscribe from this post notifications
                        ...
                        if (! empty($unsubscribed)) { // unsubscribe list is not empty, delete not wanted subscriptions
        ...
        if ($is_preferences_form || ($is_identity_form && $is_new_user)) {	// Multiple session
    ...
    public function check_login()
    {
        ...
        if (! $is_api_request || ($is_api_request && (isset($edited_user_login) || $is_new_user))) { // login specifically included in API request
            if (empty($edited_user_login)) {	// Empty login
                param_error('edited_user_login', T_('Please enter your login.'));
            }
            if (param_check_valid_login('edited_user_login')) {	// If login is valid
                global $reserved_logins;
                if ($edited_user_login != $this->get('login') &&
                    ! empty($reserved_logins) &&
                    in_array($edited_user_login, $reserved_logins) &&
                    (! is_logged_in() || ! $has_full_access)) {	// If login has been changed and new entered login is reserved and current User cannot use this:
        ...
        if ($UserLogin && $UserLogin->ID != $this->ID) {	// The login is already registered
            $login_error_message = T_('This login already exists.');
            if ($has_full_access) {
                $login_error_message = sprintf(
                    T_('This login &laquo;%s&raquo; already exists. Do you want to <a %s>edit the existing user</a>?'),
                    $edited_user_login,
        ...
        if (! param_has_error('edited_user_login')) {	// We want all logins to be lowercase to guarantee uniqueness regardless of the database case handling for UNIQUE indexes:
    ...
    {
        if ($this->check_status('is_closed') && (! is_admin_page()) && ($parname != 'login') && ($parname != 'status')) { // if this account is closed and we are not in backoffice, don't return other information then login or status
            return null;
        }
    ...
    {
        if ($this->check_status('is_closed') && (! is_admin_page())) { // don't return closed accounts information except login name
            return $this->login;
        }
    ...
    public function get_preferred_name()
    {
        ...
        if (! empty($this->nickname)) {	// Nickname
            return $this->get('nickname');
        } elseif (! empty($this->firstname) || ! empty($this->lastname)) {	// First name
     ...
     */
    public function get_colored_login($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
        ...
        if (strpos($params['mask'], '$login$') !== false) { // Display login or preferred name
            if ($params['login_text'] == 'name') { // Use nickname or fullname
        ...
        if (strpos($params['mask'], '$avatar$') !== false) { // Display avatar:
        ...
        if (! empty($params['extra_class'])) {
            if (! is_array($params['extra_class'])) {
                $params['extra_class'] = [$params['extra_class']];
            }
            $extra_class = ' ' . implode(' ', $params['extra_class']);
        }
        $attr_style = '';
        if ($params['use_style']) { // Use "style"
     ...
     */
    public function get_identity_link($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
        ...
        $attr_bubbletip = '';
        if ($params['display_bubbletip']) {	// Set attribute to initialize a bubbletip
            $attr_bubbletip = ' rel="bubbletip_user_' . $this->ID . '"';
        }

        $avatar_tag = '';
        if (strpos($params['link_text'], 'avatar') !== false) { // Avatar must be displayed in this link
            $avatar_tag = $this->get_avatar_imgtag($params['thumb_size'], $params['thumb_class'], '', $params['thumb_zoomable'], '', '', null, $params['protocol']);
            if ($params['thumb_zoomable']) { // User avatar is zoomable
        ...
        if ($params['link_text'] != 'only_avatar') { // Display a login, nickname, firstname, lastname, fullname or preferredname
            ...
            if (empty($linktext)) { // Use a login or preferred name by default
            ...
            if (! empty($params['login_mask'])) { // Apply login mask
        ...
        $gender_class = $this->get_gender_class();
        $attr_style = '';
        if ($params['use_style']) { // Use "style" (e-g email template)
     ...
     * @return object
     */
    public function &get_Regional($Object, $ID)
    {
        if ($this->check_status('is_closed') && (! is_admin_page())) { // don't return closed accounts regional information to front office
            $r = false;
            return $r;
        }
     ...
     */
    public function get_regional_name($Object, $ID)
    {
        if ($this->get_Regional($Object, $ID)) {	// We have an regional object:
     ...
     * Get subregion name
     */
    public function get_subregion_name()
    {
     ...
     */
    public function get_city_name($show_postcode = true)
    {
     ...
     * @return integer
     */
    public function get_num_posts($status = '')
    {
        global $DB;
     ...
     * @return integer
     */
    public function get_num_comments($status = '', $count_recycled = false)
    {
        global $DB;
        global $collections_Module;
     ...
     * @return integer
     */
    public function get_num_files($type = null)
    {
        global $DB;
     ...
     */
    public function get_total_upload($type = null)
    {
        if (empty($this->ID)) {	// User must be saved in DB:
            return 0;
        }
        ...
        if ($type !== null) {	// Restrict files by type:
            global $DB;
     ...
     */
    public function get_num_sessions($link_sessions = false)
    {
        global $DB;
        ...
        if ($link_sessions && ($num_sessions > 0)) {
            return $num_sessions . ' - <a href="?ctrl=user&amp;user_ID=' . $this->ID . '&amp;user_tab=sessions" class="' . button_class() . ' middle" title="' . format_to_output(T_('Go to user activity'), 'htmlattr') . '">' . get_icon('magnifier', 'imgtag', [
                'title' => T_('Go to user activity'),
            ]) . '</a>';
        }
     ...
     */
    public function get_num_messages($type = 'sent')
    {
        global $DB;

        if ($type == 'received') {	// Get a count of messages received
        ...
        } else {	// Get a count of messages sent
     ...
     */
    public function get_num_edited_posts()
    {
        global $DB;
     ...
     * @return integer
     */
    public function get_num_sent_emails()
    {
        global $DB;
     ...
     */
    public function delete_sent_emails()
    {
        global $DB;
     ...
     * @return integer
     */
    public function get_num_email_returns()
    {
        global $DB;
     ...
     * @return boolean True on success
     */
    public function delete_email_returns()
    {
        global $DB;
     ...
     */
    public function get_media_dir($create = true)
    {
        global $media_path, $Messages, $Settings, $Debuglog;

        if (! $Settings->get('fm_enable_roots_user')) {	// User directories are disabled:
        ...
        if ($create && ! is_dir($userdir)) {
            if (! is_writable(dirname($userdir))) { // add error
                if (is_admin_page()) {
                ...
                return false;
            } elseif (! evo_mkdir($userdir)) { // add error
                if (is_admin_page()) {
    ...
    public function get_media_url()
    {
        ...
        if (! empty($Blog) && ! is_admin_page()) {	// We are currently looking at a collection on front-office.
     ...
     */
    public function get_media_subpath()
    {
        if (is_valid_login($this->login, true) === true) {	// Valid ASCII login, use it as is
            return 'users/' . $this->login . '/';
        } else {	// Non-ASCII login
     ...
     */
    public function get_msgform_url($formurl, $redirect_to = null)
    {
        global $ReqURI;
     ...
     * @return string
     */
    public function get_userpage_url($blog_ID = null, $force_url = false)
    {
        global $Settings;

        if (! empty($blog_ID)) { // Use blog from param by ID
            $BlogCache = &get_BlogCache();
            $current_Blog = &$BlogCache->get_by_ID($blog_ID, false, false);
        }

        if (empty($current_Blog)) { // Use current blog
            global $Collection, $Blog;
            $current_Blog = &$Blog;
        }

        if (empty($current_Blog) || empty($Settings)) { // Wrong request
            return null;
        }

        if ($force_url || is_logged_in()) { // If current User is logged in
            if ($Settings->get('user_url_loggedin') == 'url' && $this->get_field_url(true) != '') { // Use website url if it is defined and setting enables this
                return $this->get_field_url(true);
            } else { // Use an user page if url is not defined
                return $current_Blog->get('userurl', [
                    'user_ID' => $this->ID,
            ...
            if ($this->Group->level >= $Settings->get('allow_anonymous_user_level_min') &&
                $this->Group->level <= $Settings->get('allow_anonymous_user_level_max')) { // Check if anonymous users have an access to view this user
                if ($Settings->get('user_url_anonymous') == 'url' && $this->get_field_url(true) != '') { // Use website url if it is defined and setting enables this
                    return $this->get_field_url(true);
                } elseif ($Settings->get('allow_anonymous_user_profiles')) { // Use an user page if url is not defined and this is enabled by setting for anonymous users
                    return $current_Blog->get('userurl', [
                        'user_ID' => $this->ID,
                        'user_login' => $this->login,
                    ]);
     ...
     * @return string
     */
    public function get_visits_url($blog_ID = null)
    {
        if (! empty($blog_ID)) {	// Use collection from param by ID:
            $BlogCache = &get_BlogCache();
            $url_Blog = &$BlogCache->get_by_ID($blog_ID, false, false);
        }

        if (empty($url_Blog)) {	// Use current collection:
            global $Collection, $Blog;
            $url_Blog = &$Blog;
        }

        if (empty($url_Blog)) {	// Wrong request:
            return null;
        }
     ...
     * @return string Url
     */
    public function get_field_url($from_extra_fields = false)
    {
        global $DB;

        if ($from_extra_fields) {	// Get value from DB or from cache
            if (isset($this->field_url)) {	// Get url from variable already defined in first calling of this method
        ...
        } else {	// Get value from $this->url (T_users)
            return $this->url;
        }
    }
     ...
     * @return string Link
     */
    public function get_field_link($params = [])
    {
        $params = array_merge([
            'target' => '_blank',
            'rel' => 'nofollow',
    ...
    public function set($parname, $parvalue, $make_null = false)
    {
        if ((! isset($this->significant_changed_values[$parname])) && (($old_value = $this->get($parname)) != $parvalue)
            && in_array($parname, ['login', 'grp_ID', 'nickname', 'firstname', 'lastname', 'gender', 'ctry_ID', 'rgn_ID', 'subrg_ID', 'city_ID'])) { // Save previous value of significant changes for later use in send_account_changed_notifications()
     ...
     */
    public function set_datecreated($datecreated, $isYMDhour = false)
    {
        if (! $isYMDhour) {
     ...
     */
    public function set_email($email, $change_status = true)
    {
        global $Settings;

        $r = parent::set_param('email', 'string', utf8_strtolower($email));

        if ($change_status) { // Change user status to 'emailchanged' (if email has changed and Settings are available, which they are not during install):
            if ($r && ($this->ID != 0) && isset($Settings) && $Settings->get('newusers_revalidate_emailchg') && ($this->check_status('is_validated'))) { // Deactivate the account, because the changed email has not been verified yet, but the user account is active:
                $this->set('status', 'emailchanged');
            }
        }

        if (preg_match('#@(.+)#i', $email, $ematch)) {	// Set email domain ID
            global $DB;
            ...
            if (! $dom_ID) {	// The email domains doesn't exist yet, Insert new record
     ...
     * @return boolean
     */
    public function check_password($pass, $pass_is_hashed = false)
    {
        // Get password driver for this user:
        $user_PasswordDriver = $this->get_PasswordDriver();

        if (! $user_PasswordDriver) {	// Password driver cannot be detected for this user, so it cannot be checked:
            return false;
        }
     ...
     */
    public function check_perm($permname, $permlevel = 'any', $assert = false, $perm_target = null)
    {
        ...
        if (isset($this->cache_perms[$cache_permname][$permlevel][$cache_target_ID])) {	// Permission is available in Cache:
            ...
            if (! $perm && $assert) {	// We can't let this go on!
                global $app_name;
        ...
        $pluggable_perms = ['admin', 'spamblacklist', 'slugs', 'templates', 'options', 'files', 'users', 'orgs', 'polls'];
        ...
        switch ($permname) { // What permission do we want to check?
            ...
            case 'cats_item_type_admin':
                ...
                if (! is_array($perm_target)) {	// We need an array here:
                ...
                foreach ($perm_target as $loop_cat_ID) {
                    ...
                    if (! in_array($loop_cat_blog_ID, $perm_target_blogs)) { // not already in list: add it:
                ...
                foreach ($perm_target_blogs as $loop_blog_ID) {
                    if (! $this->check_perm($blogperm, $permlevel, false, $loop_blog_ID)) { // If at least one blog denies the permission:
                        $perm = false;
                        break;
                    }
                }
            ...
            case 'recycle_owncmts':
                ...
                $blog_ID = $Item->get_blog_ID();
                if ($Item->creator_user_ID == $this->ID) { // Current user is owner of this item
                    if ($Item->is_locked() && ! $this->check_perm('blog_cats', 'edit', false, $blog_ID)) { // Comment item is locked and current user is not allowed to edit locked items comment
                        break;
                    }

                    $comment_author_User = &$Comment->get_author_User();
                    if ((empty($comment_author_User) || ($comment_author_User->level <= $this->level))
                        && in_array($Comment->status, ['published', 'community', 'protected'])) { // Comment author is anonymous or his level is lower than current User level, and the Comment was published with some of the above statuses
                        ...
                        if (! $perm) {	// Check primary and secondary groups for permissions to this specific collection:
            ...
            case 'blog_analytics':
                ...
                if ($this->check_perm_blogowner($perm_target_ID)) {	// Owner can do *almost* anything:
                    $perm = true;
                    break;
                }
                /* continue */
            ...
            case 'blog_admin': // This is what the collection owner does not have automatic access to!
                ...
                if ($primary_Group->check_perm('blogs', $group_permlevel)) {	// Primary usergroup grants a global permission:
                    $perm = true;
            ...
            case 'blog_can_be_assignee':
                ...
                if ($perm_target_ID > 0) {	// Check the permissions below only for requested target collection:
                    if ($this->check_perm_bloggroups($permname, $permlevel, $perm_target_ID)) {	// Primary or Secondary advanced usergroups permissions on the target collection grant the requested permission:
                        $perm = true;
                        // Stop checking other perms:
                        break;
                    }
                    ...
                    if ($this->check_perm_blogusers($permname, $permlevel, $perm_target_ID)) {	// Advanced user permissions on the target collection grant the requested permission:
                        $perm = true;
                        // Stop checking other perms:
                        break;
                    }

                    if ($permname == 'blog_properties' && $permlevel == 'copy') {	// Check if user can copy the selected collection:
                        $BlogCache = &get_BlogCache();
                        if ($perm_Blog = &$BlogCache->get_by_ID($perm_target_ID, false, false)) {
                            if ($this->check_perm('blogs', 'create', $assert, $perm_Blog->get('sec_ID'))) {	// If user can create new collection in section of the duplicating collection:
                                if ($perm_Blog->get_setting('allow_duplicate') ||
                                    $this->check_perm('blog_properties', 'edit', false, $perm_Blog->ID)) {	// If the collection is allowed to duplicate by anyone OR user can edit the duplicating collection:
                                    $perm = true;
                                }
                            }
                        }
            ...
            case 'comment!trash':
                ...
                if ($Comment->is_meta() && in_array($permlevel, ['edit', 'moderate', 'delete'])) { // Check the permissions for internal comment with special function
                ...
                if ((($permlevel != 'view') && $Item->is_locked() && ! $this->check_perm('blog_cats', 'edit', false, $blog_ID))
                        && ! ($permlevel == 'delete' && $this->check_perm('users', 'edit'))) { // Comment item is locked, i.e., all of its categories are locked, and current user is not allowed to edit/moderate locked items comment or is not a user admin
                    break;
                }

                if ($this->check_perm_blog_global($blog_ID, $permlevel)) { // User has global permission on this blog:
                    $perm = true;
                    break;
                }

                if ($Comment->status == 'trash') { // only global group 'editall' perm can give rights to 'trash' status, but this is not the case
                    break;
                }

                if ($permlevel == 'delete') { // permlevel is delete so we have to check the 'blog_del_cmts' permission
                ...
                if ($perm && ($Comment->status != $check_status)) { // also check the requested status permissions at the blog level
            ...
            case 'meta_comment':
                ...
                if ($permlevel == 'edit' || $permlevel == 'delete') {	// Set Comment from target object:
                    $Comment = &$perm_target;
                    if (empty($Comment) || ! $Comment->is_meta()) {	// Comment must be defined and meta to check these permissions:
                        $perm = false;
                        break;
                    }
                    $Item = &$Comment->get_Item();
                    $blog_ID = $Item->get_blog_ID();
                } elseif ($permlevel == 'view' || $permlevel == 'add' || $permlevel == 'blog') {	// Set blog ID from target value:
                    $blog_ID = $perm_target_ID;
                } else {	// Permission level is not allowed for all internal comments (For example 'moderate')
                    $perm = false;
                    break;
                }

                if (empty($blog_ID)) {	// Item or Blog must be defined to check these permissions:
                    $perm = false;
                    break;
                }

                if ($this->check_perm_blog_global($blog_ID, $permlevel)) {	// User has global permission on this collection:
                    $perm = true;
                    break;
                }
            ...
            case 'item_post!redirected':
                ...
                if ((($permlevel != 'view') && $Item->is_locked() && ! $this->check_perm('blog_cats', 'edit', false, $blog_ID))
                        && ! ($permlevel == 'delete' && $this->check_perm('users', 'edit', false))) { // Item is locked, i.e., it has all of its categories locked, and current user is not allowed to edit locked items
                ...
                if ($permlevel == 'delete') { // permlevel is delete so we have to check the 'blog_del_post' permission
                ...
                if ($perm && ($Item->status != $check_status)) { // also check the requested status permissions at the blog level
            ...
            case 'stats':
                ...
                // Group may grant VIEW access, FULL access:
                if ($this->Group->check_perm($permname, $permlevel, $perm_target)) { // If group grants a global permission:
                    $perm = true;
                    break;
                }

                if ($perm_target > 0) { // Check user perm for this blog:
                ...
                } elseif ($permlevel == 'list' && $perm_target == null) {	// Check if at least one collection has a perm to view analytics for this user:
            ...
            case 'user':
                ...
                $User = &$perm_target;
                if (! $this->check_status('can_view_user', $User->ID)) { // User can't even view the edited User
                    break;
                }

                // If user has global permission on all users then everything is allowed
                if ($this->check_perm('users', 'edit')) { // Check if user can edit/delete other users
                    $perm = true;
                    break;
                }

                if ($permlevel == 'delete' || $permlevel == 'edit') { // These permission levels are not allowed for this user
                    break;
                }

                if ((! $Settings->get('allow_anonymous_user_profiles')) && (! $this->check_perm('cross_country_allow_profiles')) && (empty($this->ctry_ID) || ($this->ctry_ID !== $User->ctry_ID))) { // Users can view/browse other users only from the same country, but this user country is empty or not the same as the target user country
                    // this User has no permission to even view the target user
                    $User->get_Group();
                    if ($User->Group->level < $Settings->get('allow_anonymous_user_level_min') ||
                        $User->Group->level > $Settings->get('allow_anonymous_user_level_max')) { // The anonymous users have no access to view this user
                        break;
                    }
                }
                ...
                if (($permlevel == 'moderate') && $this->check_perm('users', 'moderate')) { // this user has moderator permission, check if the group level is higher then the target user group level
                    $this->get_Group();
                    $User->get_Group();
...
* fp> yes.
            ...
            default:
                ...
                if ($permname == 'perm_users' && ! $this->check_perm('admin', 'normal')) {	// User must has Normal Back-office Access to view/moderate/edit other users:
                    $perm = false;
                    break;
                }
                $this->get_Group();
                $perm = Module::check_perm($permname, $permlevel, $perm_target, 'user_func', $this->Group);
                if ($perm === true || $perm === null) { // If user_func or user perm not exists in the corresponding module then $perm value will be NULL and we have to check the group permission.
        ...
        if (is_object($perm_target)) {	// Prevent catchable E_FATAL with PHP 5.2 (because there's no __tostring for e.g. Item)
            $taget_name = get_class($perm_target) . '(' . $perm_target_ID . ')';
        } elseif (is_array($perm_target)) {	// Convert to ID list
     ...
     * @return boolean
     */
    public function check_perm_blogowner($blog_ID)
    {
        if (empty($blog_ID)) {
            return false;
        }
     ...
     * @return boolean
     */
    public function check_perm_itemowner($item_ID)
    {
        if (empty($item_ID)) {
            return false;
        }
     ...
     */
    public function check_perm_blog_global($blog_ID, $permlevel = 'edit')
    {
        if ($this->check_perm_blogowner($blog_ID)) { // user is the blog owner
            return true;
        }
        ...
        if ($this->Group->check_perm('blogs', $group_permlevel)) { // If group grants a global permission:
            return true;
        }
     ...
     * @return boolean 0 if permission denied
     */
    public function check_perm_blogusers($permname, $permlevel, $perm_target_blog, $perm_target = null)
    {
        // Check if user blog advanced perms are loaded
        if (! isset($this->blog_post_statuses[$perm_target_blog])) { // Blog post statuses have not been loaded yet:
            $this->blog_post_statuses[$perm_target_blog] = [];
            if (! load_blog_advanced_perms($this->blog_post_statuses[$perm_target_blog], $perm_target_blog, $this->ID, 'bloguser')) { // Could not load blog advanced user perms
                return false;
            }
        }
     ...
     */
    public function check_perm_bloggroups($permname, $permlevel, $perm_target_blog, $perm_target = null, $User = null)
    {
        // Get primary group of this User:
        $primary_Group = &$this->get_Group();

        if ($primary_Group->check_perm_bloggroups($permname, $permlevel, $perm_target_blog, $perm_target, $User)) {	// Primary advanced usergroup permissions on the target collection grant the requested permission:
            return true;
            // Stop checking other perms.
        }
        ...
        if (empty($secondary_groups)) {	// Stop checking other perms if this User has no secondary groups:
            return false;
        }

        // Check which secondary usergroup's advanced permissions must still be loaded: (we may have some in cache already)
        $notloaded_secondary_group_IDs = [];
        foreach ($secondary_groups as $secondary_Group) {
            if (! isset($secondary_Group->blog_post_statuses[$perm_target_blog])) {	// We must still load advanced permissions for this secondary usergroup:
                $notloaded_secondary_group_IDs[] = $secondary_Group->ID;
            }
        }

        if (count($notloaded_secondary_group_IDs)) {	// Load advanced permissions of secondary usergroups in a single query:
        ...
        foreach ($secondary_groups as $secondary_Group) {
            if (isset($coll_advanced_perms, $coll_advanced_perms[$secondary_Group->ID])) {	// Set advanced usergroup permissions from array loaded above:
                $secondary_Group->blog_post_statuses[$perm_target_blog] = $coll_advanced_perms[$secondary_Group->ID];
            }
            if ($secondary_Group->check_perm_bloggroups($permname, $permlevel, $perm_target_blog, $perm_target, $User)) {	// Secondary usergroup grants requested permissions on the target collection:
                return true;
     ...
     */
    public function check_status($action, $target = null)
    {
        global $Settings, $Collection, $Blog;

        switch ($action) {
            case 'can_view_user':
                if ($Settings->get('allow_anonymous_user_profiles') || (! empty($target) && ($target == $this->ID))) { // even anonymous users can see users profile, or user wants to check his own profile
                    return true;
                }
                return (($this->status == 'activated') || ($this->status == 'autoactivated') || ($this->status == 'manualactivated'));
            case 'can_view_users':
                if ($Settings->get('allow_anonymous_user_list')) { // even anonymous users can see users list
                    return true;
                }
                return (($this->status == 'activated') || ($this->status == 'autoactivated') || ($this->status == 'manualactivated'));
            ...
            case 'can_be_validated':
                return in_array($this->status, ['new', 'emailchanged', 'deactivated', 'failedactivation', 'pendingdelete']);
     ...
     */
    public function check_role($rolename)
    {
        global $DB;

        if ($this->check_perm('blogs', 'editall')) { // if user has global editall blogs permission then it has any kind of role in all blogs
            return true;
        }
        ...
        switch ($rolename) {
            case 'post_owner':
                ...
                if ($this->get_num_posts() > 0) { // User already has at least one post
                    return true;
                }
                $role_conditions = [
                    'perm_poststatuses' => ['IS NOT NULL', '<> ""'],
                ];
                break;
            ...
            case 'comment_editor':
                ...
                $role_conditions = [
                    'perm_edit_cmt' => ['IS NOT NULL', '<> "no"', '<> "own"'],
                    'perm_cmtstatuses' => ['IS NOT NULL', '<> 0'],
                ];
                break;
            ...
            case 'comment_moderator':
                ...
                // no break
            case 'post_moderator':
                if ($rolename == 'post_moderator') { // set post moderator perm names
                ...
                $role_conditions = [
                    $edit_perm_name => ['IS NOT NULL', '<> "no"', '<> "own"'],
                    $statuses_perm_name => ['IS NOT NULL', 'BIT_COUNT( $perm_field$ & ' . $perms_value . ' ) > 1'],
                ];
                break;
        ...
        $where_clause = '';
        $perm_prefixes = ['bloguser_', 'bloggroup_'];
        foreach ($perm_prefixes as $prefix) { // Check requred perms on blogusers and bloggroups as well
            $where_part = '';
            foreach ($role_conditions as $perm_name => $conditions) { // Go through each required permission
                $perm_field = $prefix . $perm_name;
                foreach ($conditions as $condition) { // Check all defined conditions and join with 'AND' operator
                    if (strpos($condition, '$perm_field$') !== false) { // The $perm_filed must be replaced in the middle of the condition
                        $where_part .= '( ' . str_replace('$perm_field$', $perm_field, $condition) . ' ) AND ';
                    } else { // The $perm_filed must be added into the beginning of the condition
            ...
            if ($prefix != 'bloggroup_') { // 'bloggroup_' perm check is the last, but everywhere else we need an 'OR' operator
                $where_clause .= ' OR ';
            }
        }
     ...
     * @return boolean
     */
    public function is_collection_owner()
    {
        if (is_null($this->is_collection_owner)) {	// Get a result from DB first time and put in cache var:
            global $DB;
     ...
     */
    public function get_msgform_possibility($current_User = null, $check_level = 'PM')
    {
        if (! $this->is_collection_owner() && ! $this->check_status('can_receive_any_message')) { // In case of a closed account:
            return null;
        }

        if (is_logged_in()) { // current User is a registered user/logged in:
            if ($current_User == null) {
                global $current_User;
            }

            // Check cross-country restrictions:
            if (! $this->is_collection_owner() && has_cross_country_restriction('contact') && (empty($current_User->ctry_ID) || ($current_User->ctry_ID !== $this->ctry_ID))) { // Contact to this user is not enabled
                return null;
            }

            if ($check_level == 'PM' && $this->accepts_pm() && $current_User->accepts_pm() && ($this->ID != $current_User->ID)) {	// Both users have permission to send or receive Private Messages and not the same user...
                // Check if current_User is allowed to create a new conversation with this user:
                $blocked_contact = check_blocked_contacts([$this->ID]);
                if (empty($blocked_contact) && ! $current_User->must_accept_terms()) { // User is allowed to send PM to this user, and he didn't reach his new thread limit yet:
                    return 'PM';
                }
            }

            if ($this->is_collection_owner() || $this->accepts_email()) { // This User allows email => send email OR Force to allow to contact with this user because he is owner of the selected collection:
                return 'email';
            }
        } else { // current User is anonymous/not logged in:
            if ($this->is_collection_owner() || $this->accepts_email()) { // This User allows email => send email OR Force to allow to contact with this user because he is owner of the selected collection:
                return 'email';
            }

            if ($this->accepts_pm()) { // no email option - try to log in and send private message (only registered users can send PM)
                return 'login';
            }
        }
     ...
     * @return string
     */
    public function get_no_msgform_reason()
    {
        global $current_User;

        if ((is_logged_in()) && $this->accepts_pm()) {
            if (has_cross_country_restriction('contact') && (empty($current_User->ctry_ID) || ($current_User->ctry_ID !== $this->ctry_ID))) { // Contat to this user is not enabled
                return T_('You are not allowed to contact with this user.');
            }
            if ($current_User->accepts_pm()) { // current User accepts private messages
                if ($current_User->ID == $this->ID) { // current User and recipient user are the same
     ...
     * @return boolean true on success
     */
    public function dbinsert($create_auto_blog = true)
    {
        global $Plugins, $DB, $Settings;

        $DB->begin();

        if ($result = parent::dbinsert()) { // We could insert the user object..
            ...
            // NEWSLETTERS:
            $insert_newsletters = [];
            if (isset($Settings) && (! isset($this->insert_default_newsletters) || $this->insert_default_newsletters)) {	// Insert default newsletter subscriptions for this user,
                ...
                $insert_newsletters = ($Settings->get('def_newsletters') == '' ? [] : explode(',', trim($Settings->get('def_newsletters'), ',')));
            }
            if (! empty($this->new_newsletter_subscriptions)) {	// Also insert additional newsletters(e-g from widget "Email capture / Quick registration"):
                $insert_newsletters = array_merge($insert_newsletters, $this->new_newsletter_subscriptions);
            }
            if (! empty($insert_newsletters)) {	// Do subscribing if at least one newsletter is selected by default:
            ...
            $Plugins->trigger_event('AfterUserInsert', $params = [
                'User' => &$this,
            ]);

            $Group = &$this->get_Group();
            if ($create_auto_blog && $Group->check_perm('perm_getblog', 'allowed')) { // automatically create new blog for this user
                ...
                $Plugins->trigger_event('InitCollectionKinds', [
                    'Blog' => &$new_Blog,
                    'kind' => 'std',
                ]);
    ...
    public function dbupdate()
    {
        ...
        if ($this->newsletter_subscriptions_updated) {	// Only if they have been changed:
        ...
        // Update user tags:
        if (isset($this->dbchanges_flags['user_tags'])) { // Let's handle the tags:
            $this->insert_update_user_tags('update');
        }

        // Notify plugins:
        // Example: An authentication plugin could synchronize/update the password of the user.
        $Plugins->trigger_event('AfterUserUpdate', $params = [
            'User' => &$this,
        ]);

        if (isset($this->previous_status) &&
            ! in_array($this->previous_status, ['activated', 'manualactivated', 'autoactivated']) &&
            in_array($this->get('status'), ['activated', 'manualactivated', 'autoactivated'])) {	// Complete activation if user has been activated:
     ...
     */
    public static function db_delete_where($sql_where, $object_ids = null, $params = null)
    {
        global $DB;
        ...
        if (! $object_ids) { // There is no user to delete
            $DB->commit();
            return;
        }
        ...
        if ($result) { // Delete the user(s) with all of the cascade objects
            $params['use_transaction'] = false; // no need to start a new transaction
            $result = parent::db_delete_where($sql_where, $object_ids, $params);
        }

        if ($result !== false) { // delete the users' media folders recursively, for all deleted users
            $FileRootCache = &get_FileRootCache();
            foreach ($object_ids as $user_ID) {
                if ($root_directory = $FileRootCache->get_root_dir('user', $user_ID)) { // Delete the folder only when it is detected:
    ...
    public function dbdelete(&$Log = [])
    {
        ...
        if ($this->ID == 1 ||
            (is_logged_in() && $this->ID == $current_User->ID)) {	// Don't allow to delete first admin user and current logged in user:
            return false;
        }
        ...
        if (! $delete_comments) {	// Keep user's comments as from anonymous user,
        ...
        if (! $delete_files) {	// Keep user's files as from anonymous user:
        ...
        if ($deltype == 'spammer') { // User was deleted as spammer, we should mark email of this user as 'Spammer'
            $EmailAddressCache = &get_EmailAddressCache();
            $EmailAddress = &$EmailAddressCache->get_by_name($old_email, false, false);
            if (! $EmailAddress) {	// Create new record in the T_email_address table
                $EmailAddress = new EmailAddress();
        ...
        // Notify plugins:
        $this->ID = $old_ID;
        $Plugins->trigger_event('AfterUserDelete', $params = [
            'User' => &$this,
        ]);
        $this->ID = 0;
     ...
     */
    public function send_validate_email($redirect_to_after, $blog = null, $email_changed = false)
    {
        global $app_name, $Session, $baseurl, $servertimenow;
        global $Settings, $UserSettings;

        if (! empty($this->welcome_activate_email_campaign_sent)) {	// This flag means we don't need to send a separate activation email
        ...
        if (empty($redirect_to_after)) {	// If redirect to was not set:
        ...
        if ($Settings->get('validation_process') == 'easy') {	// Validation process is set to easy, send and easy activation email:
            return send_easy_validate_emails([$this->ID], false, $email_changed, $redirect_to_after);
        }

        // Secure validation process:

        if (mail_is_blocked($this->email)) { // prevent trying to send an email to a blocked email address ( Note this is checked in the send_easy_validate_emails too )
            return false;
        }
        ...
        if ($r) { // save request_id into Session
            $request_ids = $Session->get('core.activateacc.request_ids');
            if ((! is_array($request_ids)) || $email_changed) { // create new request ids array if it doesn't exist yet, or if user email changed ( this way the old request into the old email address won't be valid )
                $request_ids = [];
            }
     ...
     */
    public function activate_from_Request()
    {
    ...
    public function complete_activation()
    {
        ...
        if (empty($UserSettings)) {	// initialize UserSettings:
        ...
        if ($Settings->get('newusers_findcomments')) {	// We have to assign the all old comments from current user by email:
            $DB->query(
                'UPDATE T_comments
            ...
            $DB->query(
                'REPLACE INTO T_items__subscriptions ( isub_item_ID, isub_user_ID, isub_comments ) ' . $SQL->get(),
                'Subscribe user to posts where he selected "Notify me of replies" on creating those anonymous comments'
            );
        }
        ...
        send_admin_notification(NT_('New user account activated'), 'account_activated', [
            'User' => $this,
     ...
     */
    public function login($format = 'htmlbody')
    {
     ...
     */
    public function get_msgform_link($form_url = null, $before = ' ', $after = ' ', $text = '#', $title = '#', $class = '')
    {
        if (empty($this->email)) { // We have no email for this User :(
            return false;
        }

        $available_msgform = $this->get_msgform_possibility();
        if (! $available_msgform) {	// There is no way this user accepts receiving messages.
            return false;
        }
        ...
        if (param('redirect_to', 'url', null) !== null) {	// Use current redirect URL:
            $redirect_to = get_param('redirect_to');
        } else {	// Generate new redirect URL:
     ...
     * @param string class name
     */
    public function msgform_link($form_url = null, $before = ' ', $after = ' ', $text = '#', $title = '#', $class = '')
    {
     ...
     * @param string Output format, see {@link format_to_output()}
     */
    public function preferred_name($format = 'htmlbody')
    {
     ...
     * @param string Output format, see {@link format_to_output()}
     */
    public function url($before = '', $after = '', $format = 'htmlbody')
    {
        if (! empty($this->url)) {
            echo $before;
     ...
     */
    public function num_posts($format = 'htmlbody')
    {
     ...
     */
    public function first_name($format = 'htmlbody')
    {
     ...
     */
    public function last_name($format = 'htmlbody')
    {
     ...
     */
    public function nick_name($format = 'htmlbody')
    {
     ...
     */
    public function get_gender_class()
    {
        $gender_class = 'user';

        if ($this->check_status('is_closed')) { // Set different gender color if user is closed
            return 'user closed';
        }

        if (! check_setting('gender_colored')) { // Don't set gender color if setting is OFF
            return $gender_class;
        }

        switch ($this->gender) { // Set a class name for each gender type
            case 'M':
                $gender_class .= ' man';
                break;
            case 'F':
     ...
     */
    public function email($format = 'htmlbody')
    {
     ...
     * @deprecated
     */
    public function icq($format = 'htmlbody')
    {
    }
     ...
     * @deprecated
     */
    public function aim($format = 'htmlbody')
    {
    }
     ...
     * @deprecated
     */
    public function yim($format = 'htmlbody')
    {
    }
     ...
     * @deprecated
     */
    public function msn($format = 'htmlbody')
    {
    }
     ...
     */
    public function &get_avatar_Link()
    {
        $Link = null;
     ...
     */
    public function get_avatar_Links($exclude_main_picture = true, $ignore_settings = false)
    {
        global $Settings;

        $avatar_Links = [];

        if (! $ignore_settings && ! ($Settings->get('upload_enabled') && $Settings->get('fm_enable_roots_user'))) { // Upload is not enabled and we have no permission to use it...
            return $avatar_Links;
        }
        ...
        foreach ($LinkOwner->get_Links() as $user_Link) {
            ...
            if (! empty($l_File) && $l_File->is_image()) {
                if ($exclude_main_picture && $l_File->ID == $this->avatar_file_ID) { // Exclude the main picture from list of other pictures
                    continue;
                }
     ...
     * @return string
     */
    public function get_avatar_imgtag($size = 'crop-top-64x64', $class = 'avatar', $align = '', $zoomable = false, $avatar_overlay_text = '', $lightbox_group = '', $tag_size = null, $protocol = '')
    {
        /**
         * @var Link
         */
         ...
         * @var File
         */
        if (! $Link || ! $File = &$Link->get_File()) { // User doesn't have an avatar
        ...
        if (! $this->check_status('can_display_avatar') && ! (is_admin_page() && check_user_perm('users', 'edit', false, null, false))) { // if the user status doesn't allow to display avatar and current User is not an admin in admin interface, then show default avatar
            return get_avatar_imgtag_default($size, $class, $align, [
                'email' => $this->get('email'),
                'gender' => $this->get('gender'),
                'tag_size' => $tag_size,
        ...
        if ($zoomable) { // return clickable avatar tag, zoom on click
            if (is_logged_in()) { // Only logged in users can see a big picture of the avatar
            ...
            } else { // Anonymous user get an avatar picture with link to login page
                global $Collection, $Blog;
                $redirect_to = '';
                if (isset($Blog)) { // Redirect user after login
                    $redirect_to = $Blog->get('userurl', [
                        'user_ID' => $this->ID,
                        'user_login' => $this->login,
                    ]);
                }
        ...
        if ($r != '' && $avatar_overlay_text != '') { // Add overlay text if it is enabled
     ...
     */
    public function get_avatar_overlay_text($img_tag, $thumb_size, $avatar_overlay_text, $class = '', $File = null)
    {
        preg_match('/ width="(\d+)" height="(\d+)" /i', $img_tag, $img_sizes);
        if (count($img_sizes) == 3) { // img tag has a defined width & height
            $width = $img_sizes[1];
            $height = $img_sizes[2];
        } else { // We try to get a sizes from config
            global $thumbnail_sizes;
            if (isset($thumbnail_sizes[$thumb_size])) { // Set a sizes
                if (! is_null($File) && $thumb_sizes = $File->get_thumb_size($thumb_size)) { // Get sizes that are used for thumbnail really
                    list($width, $height) = $thumb_sizes;
                } else { // Use the size of thumbnail config if File is not defined
        ...
        if (empty($width) || empty($height)) { // If sizes is not defined we cannot calculate a font-size for an overlay text
            return $img_tag;
        }

        $overlay_lines = explode("\n", str_replace("\r", '', $avatar_overlay_text));
        $max_line_length = 0;
        foreach ($overlay_lines as $line) { // Find the most long line of the overlay text
            if ($max_line_length < strlen($line)) {	// Get max long line
                $max_line_length = strlen($line);
            }
        }
        if ($max_line_length > 0) { // Display an overlay text if max length is defined
            ...
            $tag_is_linked = false;
            if (strpos($img_tag, '</a>') !== false) { // img_tag is located inside tag <a>, we should to remove a end of the tag and then add
            ...
            $img_tag = '<div class="bubletip_overlay_text ' . $class . '">' .
                    ...
                    '</div>';
            if ($tag_is_linked) { // Add end of the tag which is removed above
                $img_tag .= '</a>';
            }
            $img_tag .= '</div>';
        }
     ...
     * @return string
     */
    public function get_avatar_styled($params = [])
    {
        global $thumbnail_sizes;
        ...
        $bubbletip_param = '';
        if ($params['bubbletip']) {	// Init bubbletip param
        ...
        if ($params['show_login']) {	// Display user name
     ...
     */
    public function userfield_add($uf_ufdf_ID, $val)
    {
        global $DB;
     ...
     * @param string Field value
     */
    public function userfield_update($uf_ID, $val)
    {
     ...
     * @return array Fields
     */
    public function userfields_by_type($field_type, $only_with_icon = true)
    {
        global $DB;
     ...
     */
    public function userfields_by_ID($ufdf_ID)
    {
        // Load all user fields once:
        $this->userfields_load();

        if (! empty($this->userfields_by_type[$ufdf_ID])) {	// Get user fields from cache:
            $userfields = [];
     ...
     */
    public function userfield_value_by_ID($field_ID, $prepare_userfield = true)
    {
        // Load all user fields once:
        $this->userfields_load();

        if (isset($this->userfields_by_type[$field_ID], $this->userfields[$this->userfields_by_type[$field_ID][0]])) {	// Get value from cache:
            $userfield = $this->userfields[$this->userfields_by_type[$field_ID][0]];

            if ($prepare_userfield) {	// Prepare user field for correct html displaying:
     ...
     */
    public function userfield_values_by_code($field_code, $prepare_userfield = true)
    {
        global $DB;
        ...
        $field_values = [];
        if (! empty($this->userfields_by_code[$field_code])) { // Get value from cache
     ...
     * Load userfields
     */
    public function userfields_load()
    {
        if ($this->userfields_loaded) { // The user fields already were loaded, Don't call DB query twice
            return;
        }
        ...
        foreach ($userfields as $u => $userfield) {
            if (! userfield_is_viewable($this->ID, $userfield->ufdf_visibility, $userfield->ufdf_type)) {	// Current user cannot view this user field:
                unset($userfields[$u]);
                continue;
            }

            if ($userfield->ufdf_duplicated == 'list') { // Prepare a values of list into one array
                if (! isset($userfield_lists[$userfield->ufdf_ID])) {	// Init array
                    $userfield_lists[$userfield->ufdf_ID] = [];
                }
        ...
        foreach ($userfields as $userfield) {
            if ($userfield->ufdf_duplicated == 'list') { // List style
                if (isset($userfield_lists[$userfield->ufdf_ID])) { // Save all data for this field:
            ...
            } else { // Save all data for this field:
                $this->userfields[$userfield->uf_ID] = $userfield;
                $this->userfields_by_code[$userfield->ufdf_code][] = $userfield->uf_ID;
            }
            // Save index
     ...
     */
    public function userfield_defs_load()
    {
        global $DB;
        ...
        if (! isset($this->userfield_defs)) {
            ...
            foreach ($userfield_defs as $userfield_def) {
                $this->userfield_defs[$userfield_def->ufdf_ID] = [
     ...
     */
    public function userfieldget_first_for_type($type_ID, $prepare_userfield = true)
    {
        $this->userfields_load();
     ...
     */
    public function update_from_request($is_new_user = false)
    {
        global $current_User, $DB, $Messages, $UserSettings, $Settings, $blog, $admin_url;
        global $is_api_request;

        if (! $current_User->can_moderate_user($this->ID) && $this->ID != $current_User->ID) { // user is only allowed to update him/herself
        ...
        if ($is_register_finish_form) {	// load data from register form:
            $load_result = $this->load_from_register_form();
        } else {	// load data from request:
            $load_result = $this->load_from_Request();
        }

        if (! $load_result) {	// We have found validation errors:
            if ($is_new_user || ($is_admin_form && ($this->ID != 1))) { // update user status settings but not save
        ...
        if ($this->dbsave()) {
            ...
            if ($is_new_user) {
                ...
                if (! empty($same_email_logins)) {	// If at least one user exists with same email address:
                    foreach ($same_email_logins as $l => $same_email_login) {	// Display a login as link to account profile page:
                ...
                if ($user_old_login != $this->login && $user_root_path != null) { // user login changed and user has a root directory (another way $user_root_path value would be NULL)
                    $FileRootCache->clear();
                    $user_FileRoot = &$FileRootCache->get_by_type_and_ID('user', $this->ID);
                    if ($user_FileRoot) { // user FilerRooot exists, rename user root folder
                        if (! @rename($user_root_path, $user_FileRoot->ads_path)) { // unsuccessful folder rename
            ...
            if ($update_success) {
                ...
                if ($user_field_url != '') { // Update url from extra user fields
            ...
            if (count($this->significant_changed_values) > 0) { // Send notification email about the changes of user account
        ...
        } elseif ($is_new_user) {	// Some error on inserting new user in DB:
            $DB->rollback();
            $update_success = false;
        ...
        if ($update_success && $try_to_send_validate_email && $this->check_status('can_be_validated')) { // user was deactivated somehow ( or it was just created right now ), check if we need to send validation email
            if (($Settings->get('validation_process') != 'easy') && ($current_User->ID != $this->ID)) { // validation process is set to secure, we may send activation email only if this user is the current_User
            ...
            } else { // validation process is easy, send email with permanent activation link
                if ($this->send_validate_email(null, $blog, ! $is_new_user && ($user_old_email != $this->email))) { // $is_new_user check added to prevent regeneration of $reminder_key when creating new user accounts with welcome campaign email
                    if ($current_User->ID == $this->ID) {
                        $redirect_to = null;
                        if ((! is_admin_page()) && (! empty($blog))) { // set where to redirect in front office, another way it would go to profile_update.php
                            $BlogCache = &get_BlogCache();
        ...
        $is_preferences_form = param('preferences_form', 'boolean', false);
        $is_subscriptions_form = param('subscriptions_form', 'boolean', false);
        if ($is_preferences_form || $is_subscriptions_form || $is_admin_form) { // Update UserSettings
            if ($UserSettings->dbupdate() && ($is_preferences_form || $is_subscriptions_form)) { // Show feature settings update successful message on user preferences form
     ...
     */
    public function update_status_from_Request($dbsave, $new_status = null)
    {
        global $DB, $Settings, $UserSettings, $Messages, $current_User, $servertimenow;
        global $is_api_request;

        if (! is_logged_in() || ! $current_User->can_moderate_user($this->ID) && ! ($Settings->get('account_close_enabled') && $current_User->ID == $this->ID)) {	// Only moderators can update user status:
            $Messages->add_to_group(T_('You do not have permission to perform this action'), 'error', T_('Close account') . ':');
            return false;
        }

        if ($dbsave) { // save required
            $DB->begin();
        }
        ...
        if (! $is_api_request || ($is_api_request && isset($edited_user_status))) {
            ...
            if ((! $this->check_status('is_closed')) && ($new_status == 'closed')) { // account was not closed yet
                if (empty($account_close_reason) && empty($account_close_type)) { // Default value of reason
                ...
                $this->unsubscribe($subscribed_newletter_IDs, [
                    'user_account_closed' => true,
                ]);

                if ($dbsave && $this->dbupdate() && $UserSettings->dbupdate() && ($DB->query($clear_sessions_query) !== false)) { // all db modification was successful
                    $DB->commit();
                    if ($current_User->ID != $this->ID) {	// If admin closed some user account
                ...
                if ($dbsave && $this->dbupdate()) { // db update
                    $UserSettings->dbupdate();
                    $DB->commit();
                    return true;
                }
            ...
            if ($dbsave) { // save was required, but wasn't successful
                $DB->rollback();
                return false;
            }
        }
     ...
     */
    public function set_profileupdate_date()
    {
     ...
     * @return mixed true on success, allowed action otherwise
     */
    public function update_avatar($file_ID, $restore = false)
    {
        global $current_User, $Messages;

        $can_moderate_user = $current_User->can_moderate_user($this->ID);
        if (! $can_moderate_user && ($restore || $this->ID != $current_User->ID)) { // user is only allowed to update him/herself
        ...
        if (! $File->get('can_be_main_profile') && ! $can_moderate_user) { // Deny to restore picture if current user has no perm
            $Messages->add(T_('Profile picture could not be changed!'), 'error');
            return 'edit';
        }

        if ($File->_FileRoot->type == 'user' && $File->_FileRoot->in_type_ID != $this->ID) { // don't allow to use pictures from other users
            $Messages->add(T_('Profile picture could not be changed!'), 'error');
            return 'edit';
        }

        $restored_success = false;
        if ($restore && $can_moderate_user) { // Restore profile picture
     ...
     */
    public function get_rotate_avatar_icons($file_ID, $params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => '<br />',
        ...
        $html .= action_icon(T_('Rotate this picture 90&deg; to the left'), 'rotate_left', $url_rotate_90_left, '', 0, 0, [
            'style' => 'margin-right:4px',
        ]);
        $html .= action_icon(T_('Rotate this picture 180&deg;'), 'rotate_180', $url_rotate_180, '', 0, 0, [
            'style' => 'margin-right:4px',
        ]);
     ...
     */
    public function &get_File_by_ID($file_ID, &$error_code)
    {
        global $current_User, $Messages;
        ...
        $can_moderate_user = $current_User->can_moderate_user($this->ID);
        if (! $can_moderate_user && $this->ID != $current_User->ID) { // user is only allowed to update him/herself
        ...
        if (empty($file_ID)) { // File ID is empty
        ...
        if (empty($File)) { // File does't exist
        ...
        if ($File->_FileRoot->type != 'user' || ($File->_FileRoot->in_type_ID != $this->ID && ! $can_moderate_user)) { // don't allow use the pictures from other users
     ...
     */
    public function rotate_avatar($file_ID, $degrees)
    {
        $File = &$this->get_File_by_ID($file_ID, $error_code);
        if (! $File) { // The file cannot be used for this user, return error code
            return $error_code;
        }
        ...
        if (! rotate_image($File, $degrees)) { // Some errors were during rotate the avatar
     ...
     */
    public function crop_avatar($file_ID, $x, $y, $width, $height)
    {
        $File = &$this->get_File_by_ID($file_ID, $error_code);
        if (! $File) { // The file cannot be used for this user, return error code
            return $error_code;
        ...
        if (! crop_image($File, $x, $y, $width, $height, intval($Settings->get('min_picture_size')), 1024)) { // Some errors were during rotate the avatar
     ...
     */
    public function get_crop_avatar_icon($file_ID, $params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'before' => '',
     ...
     */
    public function remove_avatar($forbid = false)
    {
        global $current_User, $Messages;

        $can_moderate_user = $current_User->can_moderate_user($this->ID);
        if (! $can_moderate_user && ($forbid || $this->ID != $current_User->ID)) { // user is only allowed to update him/herself
            $Messages->add(T_('You are not allowed to update this profile!'), 'error');
            return false;
        }

        $forbidden_success = false;
        if ($forbid && $can_moderate_user) { // Forbid profile picture
            $forbidden_file_ID = $this->get('avatar_file_ID');
            $FileCache = &get_FileCache();
            if (! ($forbidden_File = &$FileCache->get_by_ID($forbidden_file_ID, false, false))) { // Broken avatar file
     ...
     * @return mixed true on success, allowed action otherwise
     */
    public function delete_avatar($file_ID)
    {
        global $current_User, $Messages;
        ...
        if ($file_ID == $this->avatar_file_ID) {	// Unset this picture from user avatar if it is used as main picture
            $this->set('avatar_file_ID', null, true);
            $this->dbupdate();
        }

        if (($File->_FileRoot->type == 'user') && ($File->_FileRoot->in_type_ID == $this->ID)
            && (strpos($File->get_rdfp_rel_path(), 'profile_pictures') === 0)) { // Delete the file if it is in the current User profile_pictures folder
            $File->unlink();
        }
     ...
     * @return mixed true on success, allowed action otherwise.
     */
    public function update_avatar_from_upload()
    {
        ...
        if (! empty($uploadedFiles)) { // upload was successful
            $File = $uploadedFiles[0];
            $duplicated_files = $File->get_duplicated_files([
                'root_ID' => $this->ID,
            ]);
            if (! empty($duplicated_files)) { // The file is the duplicate of other profile picture, we should delete it
                $File->dbdelete();
                $Messages->add(T_('It seems you are trying to upload the same profile picture twice.'), 'error');
                return 'edit';
            } elseif ($File->is_image()) { // uploaded file is an image
                $LinkOwner = new LinkUser($this);
                $File->link_to_Object($LinkOwner);
                $avatar_changed = false;
                if (empty($this->avatar_file_ID)) { // set uploaded image as avatar
                    ...
                    $avatar_changed = true;
                    $Messages->add(T_('Profile picture has been changed.'), 'success');
                } else { // User already has the avatar
                ...
                return true;
            } else { // uploaded file is not an image, delete the file
     ...
     */
    public function get_last_session_param($parname)
    {
        global $DB;
     ...
     */
    public function send_welcome_message()
    {
        global $Settings, $UserSettings;

        if (! $Settings->get('welcomepm_enabled')) {	// Sending of welcome PM is disabled
            return false;
        }

        if ($Settings->get('welcomepm_notag')) {	// Don't send welcome PM if user account already has an user tag:
        ...
        if ($UserSettings->get('welcome_message_sent', $this->ID)) {	// User already received the welcome message
            return false;
        }

        if (! $this->accepts_pm()) {	// user can't read private messages, or doesn't want to receive private messages
            return false;
        }
        ...
        if (! $User) {	// Don't send an welcome email if sender login is incorrect
            return false;
        }
        ...
        $edited_Message->Thread->recipients_list = [$this->ID];
        ...
        if ($edited_Message->dbinsert_individual($User)) {	// The welcome message was sent/created successfully
     ...
     */
    public function update_sender($force_update = false)
    {
        global $Settings, $UserSettings;

        if ($force_update || $UserSettings->get('notification_sender_email', $this->ID) == '') { // Update sender's email
            $UserSettings->set('notification_sender_email', $Settings->get('notification_sender_email'), $this->ID);
        }
        if ($force_update || (($UserSettings->get('notification_sender_name', $this->ID) == '') && ($Settings->get('notification_sender_name') != 'b2evo mailer'))) { // Update sender's name if it was not set and the general settings has different value then the default
    ...
    public function get_deleted_blogs()
    {
        ...
        foreach ($user_Blogs as $user_Blog) {
            if (check_user_perm('blog_properties', 'edit', false, $user_Blog->ID)) {	// Current user has a permission to delete this blog
     ...
     * @return array Items
     */
    public function get_deleted_posts($type, $check_perm = true)
    {
        global $DB;
        ...
        foreach ($user_Items as $user_Item) {
            if ((! $check_perm) || check_user_perm('item_post!CURSTATUS', 'delete', false, $user_Item)) {	// Current user has a permission to delete this item
     ...
     */
    public function get_deleted_posts2($type, $count_only = false)
    {
     ...
     */
    public function get_own_comments_IDs()
    {
        global $DB;
     ...
     */
    public function has_comment_to_delete()
    {
        global $current_User;

        $comments_IDs = $this->get_own_comments_IDs();

        if (! count($comments_IDs)) { // User has no comments
            return false;
        }

        if ($current_User->can_moderate_user($this->ID)) { // If current user can moderate this user then it is allowed to delete all user data even if it wouldn't be allowed otherwise.
            return true;
        }
        ...
        foreach ($comments_IDs as $comment_ID) {
            if (($user_Comment = &$CommentCache->get_by_ID($comment_ID, false, false)) &&
                  check_user_perm('comment!CURSTATUS', 'delete', false, $user_Comment)) { // Current user has a permission to delete this comment
                return true;
            }
        ...
        foreach ($comments_IDs as $comment_ID) {
            ...
            if ($deleted_Comment &&
                ($current_user_can_moderate ||
                  check_user_perm('comment!CURSTATUS', 'delete', false, $deleted_Comment))) { // Current user has a permission to delete this comment
                // Delete from DB
     ...
     * @return boolean True on success
     */
    public function delete_messages($type = 'sent')
    {
        global $DB, $Plugins, $current_User;

        // Check permissions
        // Note: If current user can moderate this user then it is allowed to delete all user data even if it wouldn't be allowed otherwise
        if (! $current_User->can_moderate_user($this->ID)) { // Note: if users have delete messaging perms then they can delete any user messages ( Of course only if the delete action is available/displayed for them )
        ...
        $MessageCache->clear();
        if ($type == 'received') {	// Received messages:
        ...
        $thread_IDs = [];
        while (($iterator_Message = &$MessageCache->get_next()) != null) { // Iterate through MessageCache
        ...
        if (! empty($thread_IDs)) {	// Delete read statuses of the threads where message was deleted:
    ...
    public function delete_polls()
    {
        ...
        $result = false;
        while (($iterator_Poll = &$PollCache->get_next()) != null) {	// Iterate through PollCache:
            if ($current_user_can_moderate ||
                check_user_perm('polls', 'edit', false, $iterator_Poll)) { // Current user has a permission to delete this poll
     ...
     */
    public function get_reputation_posts($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
        ...
        if ($total_num_posts > 0) { // Calc percent of published posts
            $public_percent = floor($public_num_posts / $total_num_posts * 100);
        } else { // To avoid devision by zero
            $public_percent = 0;
        }

        if ($total_num_posts > 0) {	// Make a link to page with user's posts:
            global $current_User;
            if (is_admin_page() && is_logged_in() &&
                ($this->ID == $current_User->ID || check_user_perm('users', 'view'))) {	// For back-office
                global $admin_url;
                $total_num_posts_url = $admin_url . '?ctrl=user&amp;user_tab=activity&amp;user_ID=' . $this->ID;
            } else {	// For front-office
                global $Collection, $Blog;
                if (! empty($Blog)) {	// Only if blog is defined
                    $total_num_posts_url = url_add_param($Blog->gen_blogurl(), 'disp=useritems&amp;user_ID=' . $this->ID);
                }
            }
        }

        if (empty($total_num_posts_url)) {	// No link to view posts
            $total_num_posts = '<b>' . $total_num_posts . '</b>';
        } else {	// Set a posts number as link if it is allowed:
     ...
     * @return string Result
     */
    public function get_reputation_comments($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
        ...
        if ($total_num_comments > 0) { // Calc percent of published comments
            $public_percent = floor($public_num_comments / $total_num_comments * 100);
        } else { // To avoid devision by zero
            $public_percent = 0;
        }

        if ($total_num_comments > 0) { // Make a link to page with user's comments
        ...
        if ($params['view_type'] == 'simple') { // Simple view
        ...
        if ($params['view_type'] == 'simple') { // Simple view
            ...
            return sprintf(
                ...
                $total_num_comments,
                $public_percent,
                '<b class="green">' . $votes_count_useful . '</b>',
                '<b>' . $users_count_useful . '</b>'
            );
        } else { // Extended view
            ...
            foreach ($votes as $vote) {
                if ($vote->cmvt_helpful === '1') { // Useful votes
                    $votes_count_useful++;
                    $users_count_useful[$vote->user_ID] = null;
                } elseif ($vote->cmvt_helpful === '-1') { // Not useful votes
                    $votes_count_not++;
                    $users_count_not[$vote->user_ID] = null;
                }
                if ($vote->cmvt_spam === '-1') { // Ok votes
                    $votes_count_ok++;
                    $users_count_ok[$vote->user_ID] = null;
                } elseif ($vote->cmvt_spam === '1') { // Spam votes
                    $votes_count_spam++;
            ...
            return sprintf(
                $params['text_extended'],
                $this->get_username(),
                $total_num_comments,
                $public_percent,
                '<b class="green">' . $votes_count_useful . '</b>',
                '<b>' . count($users_count_useful) . '</b>',
                '<b class="red">' . $votes_count_not . '</b>',
                '<b>' . count($users_count_not) . '</b>',
                '<b class="green">' . $votes_count_ok . '</b>',
                '<b>' . count($users_count_ok) . '</b>',
                '<b class="red">' . $votes_count_spam . '</b>',
                '<b>' . count($users_count_spam) . '</b>'
            );
        }
    }
     ...
     * @return string Result
     */
    public function get_reputation_files($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
            'view_type' => 'simple', // 'simple', 'extended'
        ...
        switch ($params['file_type']) {
            case 'image':
                ...
                if ($params['view_type'] == 'simple') { // Simple view
                    ...
                    return sprintf(
                        $params['text_image_simple'],
                        $this->get_username(),
                        '<b>' . $this->get_num_files('image') . '</b>',
                        '<b class="green">' . $votes_count_up . '</b>',
                        '<b>' . $users_count_up . '</b>'
                    );
                } else { // Extended view
                    ...
                    $users_count_inappropriate = [];
                    $votes_count_spam = 0;
                    $users_count_spam = [];
                    foreach ($votes as $vote) {
                        if ($vote->lvot_like === '1') { // Up votes
                            $votes_count_up++;
                            $users_count_up[$vote->user_ID] = null;
                        } elseif ($vote->lvot_like === '-1') { // Down votes
                            $votes_count_down++;
                            $users_count_down[$vote->user_ID] = null;
                        }
                        if ($vote->lvot_inappropriate === '1') { // Inappropriate votes
                            $votes_count_inappropriate++;
                            $users_count_inappropriate[$vote->user_ID] = null;
                        }
                        if ($vote->lvot_spam === '1') { // Spam votes
                            $votes_count_spam++;
                            $users_count_spam[$vote->user_ID] = null;
                        }
                    }
                    ...
                    return sprintf(
                        $params['text_image_extended'],
                        $this->get_username(),
                        '<b>' . $this->get_num_files('image') . '</b>',
                        '<b class="green">' . $votes_count_up . '</b>',
                        '<b>' . count($users_count_up) . '</b>',
                        '<b class="red">' . $votes_count_down . '</b>',
                        '<b>' . count($users_count_down) . '</b>',
                        '<b class="red">' . $votes_count_inappropriate . '</b>',
                        '<b>' . count($users_count_inappropriate) . '</b>',
                        '<b class="red">' . $votes_count_spam . '</b>',
                        '<b>' . count($users_count_spam) . '</b>'
                    );
                }
     ...
     * @return string Result
     */
    public function get_reputation_total_upload($params = [])
    {
        $params = array_merge([
     ...
     * @return string Result
     */
    public function get_reputation_spam($params = [])
    {
        // Make sure we are not missing any param:
        $params = array_merge([
    ...
    public function get_email_status()
    {
        ...
        $EmailAddressCache = &get_EmailAddressCache();
        if (! empty($user_email) && $EmailAddress = &$EmailAddressCache->get_by_name($user_email, false, false)) { // The email of this user is located in the DB table
            return $EmailAddress->get('status');
        } else { // There is no email address in the DB table
            return 'unknown';
        }
    }
     ...
     * @return boolean TRUE on success
     */
    public function can_moderate_user($user_ID, $assert = false)
    {
        if ($this->check_perm('users', 'edit')) { // User can edit all users
            return true;
        }

        if ($this->check_perm('users', 'moderate', $assert)) { // User can moderate other user but we should to compare levels of users groups
            $UserCache = &get_UserCache();
            if ($target_User = $UserCache->get_by_ID($user_ID, false, false)) {
                if ($target_User->get_Group()->get('level') < $this->get_Group()->get('level')) { // User can moderate only users with level lower than own level
                    return true;
                }
            }
        }
     ...
     */
    public function send_account_changed_notification($avatar_changed = false, $new_avatar_upload = false)
    {
        if (! is_logged_in()) { // User must be logged in for this action
            return;
        }

        if (empty($this->significant_changed_values) && (! $avatar_changed) && (! $new_avatar_upload)) { // Nothing important was changed, so no need to send changed notification
            return;
        }
        ...
        foreach ($email_template_params['fields'] as $user_field_name => $user_field_data) { // Set new and previous values for email template
            ...
            if (isset($user_field_data['className'])) { // The field value is an object ID, get the object name
                ...
                if ($field_was_changed) { // Get the old display value of the field
                    $Object = &$Cache->get_by_ID($this->significant_changed_values[$user_field_name]);
                    $user_field_data['old'] = $Object->get_name();
                }
            } elseif ($user_field_name == 'gender') { // This is the gender field, get the display name of the gender
            ...
            } else { // Get the field value
                $user_field_data['new'] = $this->get($user_field_name);
                if ($field_was_changed) { // The field was changed
                    $user_field_data['old'] = $this->significant_changed_values[$user_field_name];
                }
            }

            if (! isset($user_field_data['old'])) { // The field was not changed or the old value was empty
        ...
        $this->significant_changed_values = [];
    }
    ...
    {
        if (! isset($this->organizations)) {	// Get the organizations from DB
            global $DB;
            $SQL = new SQL();
            ...
            foreach ($organizations as $organization) {
                $this->organizations[$organization->org_ID] = [
     ...
     */
    public function update_organizations($organization_IDs, $organization_roles = [], $organization_priorities = [], $force_accept = false)
    {
        ...
        foreach ($organization_IDs as $o => $organization_ID) {
            if (! ($user_Organization = &$OrganizationCache->get_by_ID($organization_ID, false, false))) { // Organization is not selected or doesn't exist in DB, Skip it
                continue;
            }
            ...
            if (! $perm_edit_orgs && $user_Organization->get('accept') == 'no') {	// Skip this if current user cannot edit the organization and it has a setting to deny a member joining:
                continue;
            }

            if (isset($insert_orgs[$organization_ID])) {	// Don't join this user to same organization twice:
                $Messages->add(sprintf(T_('You are already a member of "%s".'), $user_Organization->get_name()), 'error');
            } elseif (in_array($organization_ID, $curr_org_IDs)) { // User is already in this organization
                if ($user_Organization->perm_role == 'owner and member' ||
                    (is_logged_in() && $user_Organization->owner_user_ID == $current_User->ID) ||
                    ! $curr_orgs[$organization_ID]['accepted']) {	// Update role if current user has permission or it is not accepted yet by admin
                    $insert_orgs[$organization_ID]['role'] = (empty($organization_roles[$o]) ? null : $organization_roles[$o]);
                } else {	// Don't updated role by current user:
                    $insert_orgs[$organization_ID]['role'] = $curr_orgs[$organization_ID]['role'];
                }

                if ($perm_edit_orgs ||
                    (is_logged_in() && $user_Organization->owner_user_ID == $current_User->ID) ||
                    ! $curr_orgs[$organization_ID]['accepted']) {	// Update priority if current user has permission or it is not accepted yet by admin
                    $insert_orgs[$organization_ID]['priority'] = (empty($organization_priorities[$o]) ? null : $organization_priorities[$o]);
                } else {	// Don't updated priority by current user
                    $insert_orgs[$organization_ID]['priority'] = $curr_orgs[$organization_ID]['priority'];
                }
            } else { // Insert user in new organization
                $insert_orgs[$organization_ID]['role'] = (empty($organization_roles[$o]) ? null : $organization_roles[$o]);
                $insert_orgs[$organization_ID]['priority'] = (empty($organization_priorities[$o]) ? null : $organization_priorities[$o]);
            }
        }

        if (count($insert_orgs) > 0) { // Insert new records with user-org relations
            ...
            foreach ($insert_orgs as $insert_org_ID => $insert_org_data) {
                if (isset($curr_orgs[$insert_org_ID])) { // If we are updating - Don't change the accept status
                    $insert_orgs_accepted = $curr_orgs[$insert_org_ID]['accepted'];
                } else { // If we are inserting - Set the accept status depends on user perms or func params
                    $insert_orgs_accepted = '0';
                    if ($force_accept) {	// Force the accept status for this request:
                        $insert_orgs_accepted = '1';
                    } else {	// Check if it can be autoaccepted:
                        $user_Organization = &$OrganizationCache->get_by_ID($insert_org_ID);
                        if ($user_Organization->can_be_autoaccepted()) {	// This organization can be autoaccepted:
                            $insert_orgs_accepted = '1';
                        }
                    }
                }
        ...
        if (count($delete_org_IDs) > 0) { // Delete old records that were replaced with new
     ...
     */
    public function update_secondary_groups()
    {
        global $DB, $current_User;
        ...
        if (! $current_User->can_moderate_user($this->ID)) {	// Current User has no permission to moderate this User:
            return;
        }

        if (! isset($this->old_secondary_groups) || $this->old_secondary_groups === false) {	// Skip a request without updating secondary groups:
            return;
        }

        // Check each old secondary group if it can be deleted by current user:
        $delete_old_secondary_groups = [];
        foreach ($this->old_secondary_groups as $old_secondary_Group) {
            if ($old_secondary_Group->can_be_assigned()) {	// Current user can delete only this group:
                $delete_old_secondary_group_IDs[] = $old_secondary_Group->ID;
            }
        }
        if (! empty($delete_old_secondary_group_IDs)) {	// Clear secondary groups only which can be touched by currrent user:
        ...
        if (! empty($this->secondary_groups)) {	// Update new secondary groups:
            $new_secondary_grp_IDs = [];
            foreach ($this->secondary_groups as $secondary_Group) {
                if ($secondary_Group->can_be_assigned()) {	// We can add this secondary group because current user has a permission:
                    if ($secondary_Group->ID != $this->get('grp_ID') &&
                            ! in_array($secondary_Group->ID, $new_secondary_grp_IDs)) {	// Add except of primary user group and the duplicates:
                        $new_secondary_grp_IDs[] = $secondary_Group->ID;
                    }
                }
            }

            if (! empty($new_secondary_grp_IDs)) {	// Insert new secondary groups:
     ...
     */
    public function get_secondary_groups()
    {
        if (! is_array($this->secondary_groups)) {	// Initialize the secondary groups:
            if ($this->ID > 0) {	// For existing user:
                global $DB;
            ...
            } else {	// For new creating user:
                $this->secondary_groups = [];
            }
        }
     ...
     */
    public function get_username($format = null)
    {
        global $Settings;

        if ($Settings->get('username_display') == 'name') { // Get nickname or fullname
            $username = $this->get_preferred_name();
        } else {
            $username = $this->get('login');
        }

        if (! is_null($format)) { // Format output string
     ...
     * @return float
     */
    public function get_days_count_close()
    {
     ...
     * @return integer
     */
    public function get_own_blogs_count()
    {
        global $DB;
     ...
     * @return boolean
     */
    public function must_accept_terms()
    {
        global $UserSettings, $Settings;

        if (! $Settings->get('site_terms_enabled')) {	// The terms are not enabled:
            return false;
        }

        if ($UserSettings->get('terms_accepted', $this->ID)) {	// This user already accepted the terms:
            return false;
        }
        ...
        $ItemCache = &get_ItemCache();
        if ($terms_page_ID && $terms_Item = &$ItemCache->get_by_ID($terms_page_ID, false, false)) {	// The post for terms is defined and user still must accept it:
            return true;
        } else {	// No terms for this site:
            return false;
        }
    }
     ...
     * @return integer
     */
    public function get_flagged_items_count($coll_ID = null)
    {
        if (! is_logged_in()) {	// Only logged in users can have the flagged items:
            return 0;
        }

        if ($coll_ID === null) {	// Use current Collection:
            global $Collection, $Blog;
            $flagged_Blog = $Blog;
        } else {	// Use requested Collection:
            $BlogCache = &get_BlogCache();
            $flagged_Blog = &$BlogCache->get_by_ID($coll_ID, false, false);
        }

        if (empty($flagged_Blog)) {	// Collection must be defined:
            return 0;
        }

        if (! isset($this->flagged_items_count)) {	// Initialize array once:
            $this->flagged_items_count = [];
        }

        if (! isset($this->flagged_items_count[$coll_ID])) {	// Get it from DB only first time and then cache in var:
            ...
            $flagged_ItemList2->set_default_filters([
                'flagged' => 1,
            ]);
     ...
     */
    public function get_PasswordDriver()
    {
        if ($this->PasswordDriver === null) {	// Initialize password driver:
     ...
     */
    public function get_allowed_newsletter_IDs()
    {
        if (! isset($this->allowed_newsletter_IDs)) {	// Load allowed newsletters from cache:
            global $DB, $current_User;
            $SQL = new SQL('Get allowed newsletters for User #' . $this->ID);
            $SQL->SELECT('enlt_ID');
            $SQL->FROM('T_email__newsletter');
            $SQL->WHERE('enlt_active = 1');
            $perm_conditions = ['enlt_perm_subscribe = "anyone"'];
            $check_groups = [];
            if (check_user_perm('users', 'edit')) {	// Allow to subscribe to forbidden newsletters by user admins:
     ...
     */
    public function get_allowed_newsletters()
    {
        if (! isset($this->allowed_newsletters)) {	// Load allowed newsletters from cache:
            $newsletter_IDs = $this->get_allowed_newsletter_IDs();
            $this->allowed_newsletters = [];
     ...
     * @return boolean
     */
    public function is_allowed_newsletter($newsletter_ID)
    {
     ...
     * @return array
     */
    public function get_newsletter_subscriptions($type = 'subscribed')
    {
        if (empty($this->ID)) {	// Only created user can has the newsletter subscriptions:
            return [];
        }

        if (! isset($this->newsletter_subscriptions)) {	// Initialize newsletter subscriptions array only on first request:
            global $DB;

            $this->newsletter_subscriptions = [
                'subscribed' => [],
                'unsubscribed' => [],
            ];
     ...
     * @return boolean
     */
    public function is_subscribed($newsletter_ID)
    {
     ...
     */
    public function set_newsletter_subscriptions($new_subscriptions, $params = [])
    {
        $prev_subscriptions = $this->get_newsletter_subscriptions();

        if (! is_array($new_subscriptions)) {	// This must be an array:
            $new_subscriptions = [];
        }
            ...
            foreach ($new_subscriptions as $n => $new_subscription_ID) {
                ...
                if (empty($new_subscription_ID) ||
                    (! $this->is_allowed_newsletter($new_subscription_ID) &&
                      ! in_array($new_subscription_ID, $prev_subscriptions))) {	// Unset wrong value or
        ...
        if (count($prev_subscriptions) != count($new_subscriptions)) {	// If a count of subscriptions are not equals then mark they are updated:
            $this->newsletter_subscriptions_updated = true;
        } else {
            sort($new_subscriptions);
            sort($prev_subscriptions);
            if ($prev_subscriptions != $new_subscriptions) {	// If subscriptions were really updated:
                $this->newsletter_subscriptions_updated = true;
            }
        }
     ...
     */
    public function subscribe($newsletter_IDs, $params = [])
    {
        ...
        if (empty($newsletter_IDs)) {	// No subscriptions:
            return false;
        }

        $DB->begin();

        if (! is_array($newsletter_IDs)) {	// Convert single newsletter ID to array:
        ...
        if (count($insert_newsletter_IDs)) {	// If new records should be inserted:
            $insert_newsletter_sql_values = [];
            ...
            $r += $DB->query(
                'INSERT INTO T_email__newsletter_subscription ( enls_user_ID, enls_enlt_ID, enls_subscribed, enls_subscribed_ts )
        ...
        if (count($update_newsletter_IDs)) {	// If previous unsubscriptions should be subscribed again:
            $r += $DB->query(
                'UPDATE T_email__newsletter_subscription
        ...
        if (count($insert_newsletter_IDs) || count($resubscribe_newsletter_IDs)) {	// Notify list owner of new subscriber:
            $email_template_params = array_merge([
                'subscribed_User' => $this,
                'subscribed_by_admin' => (is_logged_in() && $current_User->login != $this->login ? $current_User->login : ''),
            ], empty($this->newsletter_subscription_params) ? [] : $this->newsletter_subscription_params, $params);

            // Add to changes in subscription values:
            $this->newsletter_subscription_changed_values[] = ['subscribe', array_unique(array_merge($insert_newsletter_IDs, $resubscribe_newsletter_IDs)), $email_template_params];
        }
        ...
        $DB->query(
            'INSERT INTO T_automation__user_state ( aust_autm_ID, aust_user_ID, aust_next_step_ID, aust_next_exec_ts ) ' . $automations_SQL->get(),
     ...
     * @return integer|boolean # of rows affected or false if error
     */
    public function unsubscribe($newsletter_IDs, $params = [])
    {
        ...
        if ($r) {	// If user has been unsubscribed from at least one newsletter,
            // Then this user must automatically exit all automations tied to those newsletters:
            $DB->query(
                'DELETE T_automation__user_state FROM T_automation__user_state
        ...
        if (count($update_newsletter_IDs)) {	// Notify list owner of lost subscriber:
            $email_template_params = array_merge([
                'subscribed_User' => $this,
                'unsubscribed_by_admin' => (is_logged_in() && $current_User->login != $this->login ? $current_User->login : ''),
            ], empty($this->newsletter_unsubscription_params) ? [] : $this->newsletter_unsubscription_params, $params);

            // Add to changes in subscription values:
            $this->newsletter_subscription_changed_values[] = ['unsubscribe', $update_newsletter_IDs, $email_template_params];
        }
     ...
     */
    public function send_auto_subscriptions($newsletter_IDs)
    {
        global $DB;
        ...
        if (count($new_subscriptions)) {	// User is really subscribing to new newsletters:
            ...
            foreach ($EmailCampaignCache->cache as $EmailCampaign) {	// Send a "Welcome" email of the campaign at subscription:
                $r = $EmailCampaign->send_all_emails(false, [$this->ID], 'welcome');
                if ($r && $EmailCampaign->get('activate')) {	// Set a flag to know email campaign which is used as "Activate" message has been sent now for this User:
     ...
     */
    public function send_list_owner_notifications($filter = 'all')
    {
     ...
     */
    public function get_profile_visitors_count($new_only = true)
    {
        global $DB;
     ...
     */
    public function insert_update_user_tags($mode)
    {
        global $DB;

        if (isset($this->user_tags)) { // Okay the tags are defined:
            $DB->begin();
     ...
     */
    public function set_usertags_from_string($user_tags)
    {
        ...
        foreach ($this->user_tags as $t => $user_tag) {
            if (substr($user_tag, 0, 1) == '-') { // Prevent chars '-' in first position
                $user_tag = preg_replace('/^-+/', '', $user_tag);
            }
            if (empty($user_tag)) { // Don't save empty tag
     ...
     */
    public function add_usertags($user_tags)
    {
        if (empty($user_tags)) {
            return;
        }
        ...
        foreach ($user_tags as $t => $user_tag) {
            ...
            if (empty($user_tag)) { // Don't save empty tag
                unset($user_tags[$t]);
            } else { // Append the tag and mark that tags has been updated
                $this->dbchanges_flags['user_tags'] = true;
     ...
     * @param string/array comma separated tags or array of tags
     */
    public function remove_usertags($user_tags)
    {
        if (empty($user_tags)) {
            return;
        }
        ...
        foreach ($user_tags as $t => $user_tag) {
            if (empty($user_tag)) {	// Skip empty tag:
                continue;
            } elseif (($user_tag_index = array_search($user_tag, $this->user_tags)) !== false) {	// Remove user tag from array:
                $this->dbchanges_flags['user_tags'] = true;
