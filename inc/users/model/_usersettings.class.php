...
class UserSettings extends AbstractSettings
{
    ...
    { // constructor
        parent::__construct('T_users__usersettings', ['uset_user_ID', 'uset_name'], 'uset_value', 1);
    }
     ...
     */
    public function get($setting, $user_ID = null)
    {
        global $Settings;
        ...
        if (! isset($user_ID)) {
            ...
            if (! isset($current_User)) { // no current/logged in user:
     ...
     * @param integer User ID (by default $current_User->ID will be used)
     */
    public function set($setting, $value, $user_ID = null)
    {
        if (! isset($user_ID)) {
            global $current_User;
     ...
     * @param integer User ID (by default $current_User->ID will be used)
     */
    public function delete($setting, $user_ID = null)
    {
        if (! isset($user_ID)) {
            global $current_User;
     ...
     */
    public function param_Request($param_name, $uset_name, $type = '', $default = '', $memorize = false, $override = false) // we do not force setting it..
    {
     ...
     */
    public function reset_to_defaults($user_ID, $db_save = true)
    {
     ...
     * @param integer User ID (by default $current_User->ID will be used)
     */
    public function get_collection_setting($setting, $coll_ID = null, $user_ID = null)
    {
        if ($coll_ID === null) { // Use current blog ID by default
        ...
        if ($coll_ID === null) { // Collection is not detected
            return null;
        }
        ...
        if ($value === null) { // The user-collection setting is not defined in DB
            // Try to get a default value for this setting:
            if (isset($this->_defaults[$setting])) { // Default value is defined
