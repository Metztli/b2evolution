...
class UserQuery extends FilterSQL
{
     ...
     */
    public $keywords_fields = 'user_login, user_firstname, user_lastname, user_nickname';
     ...
     */
    public function __construct($dbtablename = 'T_users', $dbprefix = 'user_', $dbIDname = 'user_ID', $params = [])
    {
        ...
        if ($params['join_group']) { // Join Group
        ...
        if ($params['join_sec_groups']) {	// Join Secondary groups:
        ...
        if ($params['join_session']) { // Join Session
        ...
        if ($params['join_country']) { // Join Country
        ...
        if ($params['join_lists']) { // subscribed_list contains comma-separated list of newsletter IDs, "negative" IDs are unsubscribed to newsletter lists
        ...
        if ($params['grouped']) { // Group by user group
     ...
     */
    public function where_user_IDs($user_IDs)
    {
        global $DB;

        if (empty($user_IDs)) {	// Don't restrict:
            return;
        }
     ...
     */
    public function where_members($members)
    {
        global $DB, $Collection, $Blog;

        if (empty($members) || is_admin_page() || empty($Blog) || $Blog->get_setting('allow_access') != 'members') { // Don't restrict
            return;
        }
     ...
     */
    public function where_keywords($keywords, $search_kw_combine = 'AND')
    {
        global $DB;
    ...
    public function where_email($email)
    {
        ...
        if ($email !== '') {	// Filter only by not empty email address:
     ...
     */
    public function where_gender($gender)
    {
        if (empty($gender)) {
            return;
        }
     ...
     */
    public function where_status($status, $include = true, $exactly = false)
    {
        if (empty($status)) {
            return;
        }

        if (($status == 'activated' || $status === 1) && ! $exactly) {	// Activated, Manually activated, Autoactivated users:
            $this->add_filter_rule('status', ['activated', 'autoactivated', 'manualactivated'], '=', 'OR');
        } else { // Other status check
     ...
     */
    public function where_registered_date($min_date = null, $max_date = null)
    {
        if (! empty($min_date) && ! empty($max_date)) {
            $this->add_filter_rule('regdate', [$min_date, $max_date], 'between', null, 'date');
     ...
     */
    public function where_reported($reported)
    {
        if (! empty($reported)) {
     ...
     */
    public function where_custom_sender($custom_sender_email, $custom_sender_name)
    {
        global $DB, $Settings;

        if ($custom_sender_email) {	// Restrict to users with custom notification sender email address:
            $this->add_filter_rule('custom_sender_email', 'yes');
        }

        if ($custom_sender_name) {	// Restrict to users with custom notification sender name:
     ...
     */
    public function where_tag($user_tag = null, $not_user_tag = null)
    {
        if (trim($user_tag) !== '') {
     ...
     */
    public function where_group($group_ID)
    {
        $group_ID = (int) $group_ID;

        if ($group_ID < 1) { // Group Id may be '0' - to show all groups, '-1' - to show all groups as ungrouped list
            return;
        }
     ...
     */
    public function where_secondary_group($secondary_group_ID)
    {
        $secondary_group_ID = intval($secondary_group_ID);

        if ($secondary_group_ID < 1) {	// Group ID may be '0' - to show all groups
            return;
        }
     ...
     */
    public function where_location($location, $ID)
    {
        global $DB;
     ...
     */
    public function where_age_group($age_min, $age_max)
    {
        global $DB;

        $sql_age = [];

        if ($age_min > 0) {	// search_min_value BETWEEN user_age_min AND user_age_max
            $sql_age[] = '( ' . $DB->quote($age_min) . ' >= user_age_min AND ' . $DB->quote($age_min) . ' <= user_age_max )';
        }

        if ($age_max > 0) {	// search_max_value BETWEEN user_age_min AND user_age_max
     ...
     */
    public function where_group_level($group_level_min, $group_level_max)
    {
        global $DB;

        if ($group_level_min < 0) { // Min group level is 0
            $group_level_min = 0;
        }

        if ($group_level_max > 10) { // Max group level is 10
            $group_level_max = 10;
        }
     ...
     */
    public function where_organization($org_ID)
    {
        global $DB;
     ...
     */
    public function where_newsletter($newsletter_ID, $is_subscribed = true)
    {
        ...
        $this->add_filter_rule('newsletter', [$newsletter_ID, $is_subscribed]);
    }
     ...
     * @param integer Newsletter ID
     */
    public function where_not_newsletter($not_newsletter_ID)
    {
     ...
     */
    public function where_email_campaign($ecmp_ID, $recipient_type = '', $recipient_action = '')
    {
        global $DB;
     ...
     */
    public function where_viewed_user($user_ID)
    {
        global $DB;
     ...
     */
    public function where_reg_ip($reg_ip_min, $reg_ip_max)
    {
        global $DB;
        ...
        if (empty($reg_ip_min) && empty($reg_ip_max)) {	// No IP filters:
            return;
        }
        ...
        if (! empty($reg_ip_min)) {	// Restrict with MIN registration IP address:
            $this->WHERE_and('uset_value >= ' . $DB->quote($reg_ip_min));
        }

        if (! empty($reg_ip_max)) {	// Restrict with MAX registration IP address:
     ...
     */
    public function where_level($user_level_min, $user_level_max)
    {
        if ($user_level_min < 0 || is_null($user_level_min)) { // Min group level is 0
            $user_level_min = 0;
        }

        if ($user_level_max > 10 || is_null($user_level_max)) { // Max group level is 10
            $user_level_max = 10;
        }

        if ($user_level_min > 0 || $user_level_max < 10) {	// Filter only with actual values:
            $this->add_filter_rule('level', [$user_level_min, $user_level_max], 'between');
        }
    }
     ...
     */
    public function where_duplicate_email()
    {
     ...
     */
    public function filter_field_name_email($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_firstname($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_lastname($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_nickname($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_email($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_gender($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_country($value, $operator)
    {
        if (! empty($value)) {
     ...
     * @param string Operator
     */
    public function filter_field_level($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_org($value, $operator)
    {
        if ($operator == 'equal' || $operator == 'not_equal') {	// If operator is allowed for this filter:
            global $DB;
     ...
     * @param string Operator
     */
    public function filter_field_custom_sender_email($value, $operator)
    {
        if ($value == 'yes' || $value == 'no') {	// If value is allowed for this filter:
            global $Settings, $DB;
     ...
     * @param string Operator
     */
    public function filter_field_custom_sender_name($value, $operator)
    {
        if ($value == 'yes' || $value == 'no') {	// If value is allowed for this filter:
            global $Settings, $DB;
     ...
     * @param string Operator
     */
    public function filter_field_criteria($value, $operator)
    {
        if (! preg_match('#^(\d+):(contains|not_contains):(.+)$#', $value, $m)) {	// Skip wrong value:
            return;
        }
        ...
        if ($user_field_def_ID <= 0 || $user_field_value == '' || $user_field_operator == '') {	// Skip wrong value:
            return;
        }
        ...
        foreach ($words as $word) {	// Find each word separately:
     ...
     * @param string Operator
     */
    public function filter_field_lastseen($value, $operator)
    {
        if (! empty($value)) {
     ...
     * @param string Operator
     */
    public function filter_field_source($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_report_count($value, $operator)
    {
        $value = intval($value);
     ...
     * @param string Operator
     */
    public function filter_field_group($value, $operator)
    {
        $value = intval($value);
     ...
     * @param string Operator
     */
    public function filter_field_group2($value, $operator)
    {
        $value = intval($value);

        if ($value > 0 && ($operator == 'equal' || $operator == 'not_equal')) {	// If value and operator are allowed for this filter:
            global $DB;
     ...
     * @param string Operator
     */
    public function filter_field_status($value, $operator)
    {
     ...
     * @param string Operator
     */
    public function filter_field_regdate($value, $operator)
    {
        if (! empty($value)) {
     ...
     * @param string Operator
     */
    public function filter_field_newsletter($value, $operator)
    {
        if (is_array($value) && count($value) == 2) {	// Special case for additional param to also get user which will be unsubscribed:
            $is_subscribed = $value[1];
            $value = intval($value[0]);
        } else {	// Get only subscribed users:
            $is_subscribed = 1;
            $value = intval($value);
        }
        ...
        if ($value > 0 && ($operator == 'equal' || $operator == 'not_equal')) {	// If value and operator are allowed for this filter:
            ...
            if ($is_subscribed !== null) {	// Get only subscribed or unsubscribed users:
     ...
     * @param string Operator
     */
    public function filter_field_tags($value, $operator)
    {
        $tags = array_unique(array_map('trim', explode(',', $value)));

        foreach ($tags as $t => $tag) {
            if ($tag === '') {	// Remove empty tags:
                unset($tags[$t]);
            }
        }

        if (count($tags) > 0 && ($operator == 'user_tagged' || $operator == 'user_not_tagged')) {	// If value and operator are allowed for this filter:
            global $DB;
