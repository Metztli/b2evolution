...
class Group extends DataObject
{
    ...
    public $perm_security;

    public $perm_bypass_antispam = false;

    public $perm_xhtmlvalidation = 'always';

    public $perm_xhtmlvalidation_xmlrpc = 'always';

    public $perm_xhtml_css_tweaks = false;

    public $perm_xhtml_iframes = false;

    public $perm_xhtml_javascript = false;

    public $perm_xhtml_objects = false;
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                'table' => 'T_users',
                'fk' => 'user_grp_ID',
                'msg' => T_('%d users in this group'),
            ],
            [
                'table' => 'T_users__invitation_code',
                'fk' => 'ivc_grp_ID',
                'msg' => T_('%d user invitation codes in this group'),
            ],
            [
                'table' => 'T_users__fielddefs',
                'fk' => 'ufdf_grp_ID',
                'msg' => T_('%d user fields use this group'),
            ],
        ];
    }
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_plugingroupsettings',
                'fk' => 'pgset_grp_ID',
                'msg' => T_('%d group settings on plugins'),
            ],
        ];
    }
    ...
    public function load_from_Request()
    {
        ...
        if ($this->ID > 0 && $usage != $this->get('usage')) {	// Display a warning if group usage has been changed:
            global $DB;
            if ($usage == 'primary') {
        ...
        if ($demo_mode && ($apply_antispam || ($perm_xhtmlvalidation != 'always') && ($perm_xhtmlvalidation_xmlrpc != 'always')
             || $prevent_css_tweaks || $prevent_iframes || $prevent_javascript || $prevent_objects)) { // Demo mode restriction: Do not allow to change these settings in demo mode, because it may lead to security problem!
        ...
        foreach ($GroupSettings->permission_values as $name => $value) {
            ...
            if ($name == 'perm_createblog' || $name == 'perm_getblog' || $name == 'perm_centralantispam'
                || $name == 'cross_country_allow_profiles' || $name == 'cross_country_allow_contact') { // These permissions are represented by checkboxes, all other pluggable group permissions are represented by radiobox.
                $value = param('edited_grp_' . $name, 'string', 'denied');
            } elseif (($name == 'perm_admin' || $name == 'perm_users') && ($this->ID == 1)) { // Admin group has always admin perm, it can not be set or changed.
                continue;
            } elseif ($name == 'perm_allowed_sections') {	// This permission is represented by checklist:
                $value = param('edited_grp_' . $name, 'array:integer', '');
                $value = implode(',', $value);
            } else {
                $value = param('edited_grp_' . $name, 'string', '');
            }
            if (($value != '') || ($name == 'max_new_threads'/*allow empty*/) || ($name == 'perm_max_createblog_num')) { // if radio is not set, then doesn't change the settings
        ...
        if ($GroupSettings->get('perm_admin', $this->ID) != 'normal' &&
                $GroupSettings->get('perm_users', $this->ID) != 'none') {	// Display warning when users permissions are not allowed because of not full access to back-office:
     ...
     */
    public function &get_GroupSettings()
    {
     ...
     */
    public function check_perm($permname, $permlevel = 'any', $perm_target = null, $User = null)
    {
        global $Debuglog;
        ...
        } else { // Object's perm-property not set!
        ...
        $pluggable_perms = ['admin', 'shared_root', 'import_root', 'skins_root', 'plugins_root', 'spamblacklist', 'slugs', 'templates', 'options', 'emails', 'files', 'users', 'orgs', 'centralantispam', 'maintenance'];
        ...
        switch ($permname) {
            case 'section':
                ...
                if (! $perm && ! empty($perm_target)) {	// User has an access to any Section he owns, or at a minimum, in the Default Section of this group:
                    ...
                    if ($Section = &$SectionCache->get_by_ID($perm_target, false, false)) {
                        ...
                        if ($permlevel != 'edit' && in_array($Section->ID, $allowed_section_IDs)) {	// Allow to view or create a collection if the requested section is default of this group:
                            $perm = true;
                        }
                        if (! $perm && $User && $Section->owner_user_ID == $User->ID) {	// If user is owner of the requested section:
                            $perm = true;
                        }
                    }
                }
            ...
            case 'blogs':
                switch ($permvalue) { // Depending on current group permission:
                    case 'editall':
                    ...
                    case 'viewall':
                        ...
                        if (($permlevel == 'view') || ($permlevel == 'any')) { // Permission granted
                            $perm = true;
                            break;
                        }
                }

                if (! $perm && ($permlevel == 'create') && $this->check_perm('perm_createblog', 'allowed')) {	// User can create a collection for himself (in any Section he owns, or at a minimum, in the Default Section of this group):
                    $perm = $this->check_perm('section', 'view', $perm_target, $User);
                    if (! $perm && empty($perm_target)) {	// If request to create a collection without specified Section, then try to create it in default Section of this group:
                        $SectionCache = &get_SectionCache();
                        if ($Section = &$SectionCache->get_by_ID($this->get_setting('perm_default_sec_ID'), false, false)) {	// If default section really exists in DB:
                            $perm = true;
                        }
                    }
                }
            ...
            default:
                ...
                if ($perm === null) {	// Even if group permisson check function doesn't exist we should return false value
                    $perm = false;
                }

                if ($perm && // Permission is allowed
                        in_array($permname, ['perm_spamblacklist', 'perm_slugs', 'perm_emails', 'perm_maintenance']) && // These permissions depend on permission "Settings"
    ...
    public function check_perm_bloggroups($permname, $permlevel, $perm_target_blog, $perm_target = null, $User = null)
    {
        if (! isset($this->blog_post_statuses[$perm_target_blog])) {
            ...
            if (! load_blog_advanced_perms($this->blog_post_statuses[$perm_target_blog], $perm_target_blog, $this->ID, 'bloggroup')) { // Could not load blog advanced user perms
                return false;
            }
        }

        $blog_perms = $this->blog_post_statuses[$perm_target_blog];
        if (empty($User)) { // User is not set
            $user_ID = null;
        } else { // User is set, advanced user perms must be loaded
            $user_ID = $User->ID;
            if (isset($User->blog_post_statuses[$perm_target_blog])) { // Merge user advanced perms with group advanced perms
                $edit_perms = [
                    'no' => 0,
                    'own' => 1,
                    'anon' => 2,
                ...
                foreach ($User->blog_post_statuses[$perm_target_blog] as $key => $value) { // For each collection advanced permission use the higher perm value between user and group perms
                    if (($key == 'blog_edit') || ($key == 'blog_edit_cmt')) {
                        if ($edit_perms[$value] > $edit_perms[$blog_perms[$key]]) { // Use collection user edit permission because it is greater than the collection group perm
                            $blog_perms[$key] = $value;
                        }
                    } elseif (isset($blog_perms[$key])) { // Check user and group perm as well
     ...
     * @return string
     */
    public function get_name_without_level()
    {
        return $this->name;
    }
    ...
    public function can_be_assigned()
    {
        ...
        if (! is_logged_in()) {	// User must be assigned:
            return false;
        }

        if (check_user_perm('users', 'edit')) {	// Allow to assing any group if current user has full access to edit users:
            return true;
        }

        if (! check_user_perm('users', 'moderate')) {	// User must has a permission at least to modearate the users:
            return false;
        }

        $user_Group = &$current_User->get_Group();

        if (! $user_Group) {	// User's group must be defined:
            return false;
        }
