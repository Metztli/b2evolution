...
{
    if (! is_logged_in($accept_not_active)) {	// Current User must be logged in:
        return false;
    }
...
function get_user_login_link($before = '', $after = '', $link_text = '', $link_title = '#', $source = 'user login link', $redirect_to = null)
{
    ...
    if ($link_title == '#') {
        $link_title = T_('Log in if you have an account...');
    }
 ...
 */
function get_user_colored_login($login, $params = [])
{
    $params = array_merge([
        'mask' => '$avatar$ $login$',
    ...
    $UserCache = &get_UserCache();
    $User = &$UserCache->get_by_login($login);
    if (! $User) { // User doesn't exist by some reason, maybe it was deleted right now
        // Return only login
        return $login;
    }
 ...
 */
function get_user_colored_login_link($login, $params = [])
{
    $UserCache = &get_UserCache();
    if ($User = &$UserCache->get_by_login($login)) { // Get a link to user page in front office
        return $User->get_identity_link([
            'user_tab' => 'userpage',
            'use_style' => ! empty($params['use_style']),
            'protocol' => empty($params['protocol']) ? '' : $params['protocol'],
        ]);
    } else { // Get login as text
...
function get_login_url($source, $redirect_to = null, $force_normal_login = false, $blog_ID = null, $blog_page = 'loginurl')
{
    ...
    if (! $force_normal_login && use_in_skin_login()) { // Use in-skin login form of the current blog or of the special blog for login/register actions
        if (empty($blog_ID)) { // Use current blog if it is not defined
            global $blog;
            $blog_ID = $blog;
        }
        $BlogCache = &get_BlogCache();
        $Collection = $Blog = $BlogCache->get_by_ID($blog_ID);
        $url = $Blog->get($blog_page, [
            'glue' => '&',
        ]);
    } else { // Use normal/basic login form (without blog skin)
...
function get_lostpassword_url($redirect_to = null, $glue = '&amp;', $return_to = null)
{
    ...
    if (empty($redirect_to) && $redirect_to !== false) { // Redirect back to current URL
    ...
    if (use_in_skin_login()) { // Use in-skin lostpassword form of the current blog or of the special blog for login/register actions
        $lostpassword_url = $Blog->get('lostpasswordurl', [
            'glue' => $glue,
        ]);
    } else { // Use normal/standard lostpassword form (without blog skin)
        $lostpassword_url = get_htsrv_url('login') . 'login.php?action=lostpassword';
    }

    if ($redirect_to !== false) { // Append redirect URL only when it is not restricted:
    ...
    if ($return_to !== false) { // Append return URL only when it is not restricted:
...
function get_activate_info_url($redirect_to = null, $glue = '&')
{
    ...
    if (empty($redirect_to)) { // Redirect back to current URL
        $redirect_to = regenerate_url('', '', '', $glue);
    }

    if (use_in_skin_login()) { // Use in-skin activate info page of the current blog or of the special blog for login/register actions
        $activateinfo_url = $Blog->get('activateinfourl', [
            'glue' => $glue,
        ]);
    } else { // Use normal/standard lostpassword form (without blog skin)
...
function get_notifications_url($glue = '&amp;', $user_ID = null, $comment_ID = null)
{
    ...
    if ($comment_ID !== null) {	// Get notifications url for anonymous user:
        ...
        return $comment_item_Blog->get('registerurl', [
            'url_suffix' => 'comment_ID=' . $Comment->ID,
            'glue' => $glue,
        ]);
    }

    if (! empty($blog) && empty($Blog)) { // Try to initialize global $Collection, $Blog object
        $BlogCache = &get_BlogCache();
        $Collection = $Blog = $BlogCache->get_by_ID($blog, false, false);
    }

    $use_admin_page_url = false;
    if (is_admin_page() || empty($Blog)) { // Check if the user has an access to backoffice
        if (is_null($user_ID)) { // Use current user
            global $current_User;
            $User = &$current_User;
        } else { // Get user by ID
            $UserCache = &get_UserCache();
        ...
        if (! empty($User) && $User->check_perm('admin', 'restricted')) { // The user has an access to backoffice
            $use_admin_page_url = true;
        }
    }

    if ($use_admin_page_url) { // Use backoffice form of notifications form
        return $admin_url . '?ctrl=user' . $glue . 'user_tab=subs';
    } else { // Use in-skin form of notifications form:
        if (empty($Blog)) { // If no current blog, try to use a default blog:
            $url_Blog = &get_setting_Blog('default_blog_ID');
            if (empty($url_Blog)) { // No default blog, Use base url:
                global $baseurl;
                return url_add_param($baseurl, 'disp=subs');
            }
        } else { // Use current blog:
            $url_Blog = &$Blog;
        }
        return $url_Blog->get('subsurl', [
            'glue' => $glue,
        ]);
    }
}
...
function get_redirect_after_registration($inskin = true)
{
    ...
    if ($after_registration == 'return_to_original') {	// Return to original page ( where user was before the registration process ):
        if (empty($redirect_to)) { // redirect_to param was not set
    ...
    } elseif ($after_registration == 'specific_slug') {	// Return to the specific slug which is set in the registration settings form:
        $SlugCache = get_SlugCache();
        if (($Slug = &$SlugCache->get_by_name($Settings->get('after_registration_slug'), false, false)) &&
                ($slug_Item = &$Slug->get_object()) &&
                ($slug_Item instanceof Item)) {	// Use permanent URL of the slug Item:
            $redirect_to = $slug_Item->get_permanent_url('', '', '&');
        }
    } else {	// Return to the specific URL which is set in the registration settings form:
...
function redirect_after_account_activation()
{
    ...
    if ($Settings->get('pass_after_quick_reg')) {	// Check if we should redirect to password setting page if no password has been set yet (email capture/quick registration):
        global $current_User;
        if (! is_logged_in()) {	// Try to log in if activation URL is opened for not logged in user yet (e-g for easy activation process):
            $current_User = &$Session->get_User();
        }
        if (is_logged_in() && $current_User->get('pass_driver') == 'nopass') {	// If user was registered without password:
    ...
    if ($redirect_to == 'return_to_original') { // we want to return to original page after account activation
        ...
        if (empty($redirect_to)) { // session redirect_to was not set, initialize $redirect_to to the home page
            $redirect_to = $baseurl;
        }
...
function send_admin_notification($subject, $template_name, $template_params)
{
    ...
    if (empty($current_User) && ! empty($Session) && $Session->has_User()) { // current_User is not set at the time of registration
        $current_User = &$Session->get_User();
    }
    ...
    $UserCache->rewind();
    while ($User = &$UserCache->get_next()) { // Loop through Users
        if (is_logged_in() && $current_User->ID == $User->ID) { // Don't send a notification to current user, because he already knows about this event
            continue;
        }
        if ($UserSettings->get($check_setting, $User->ID) && $User->check_perm($perm_name, 'edit')) { // this user must be notifed
            locale_temp_switch($User->get('locale'));
            // send mail to user (using his local)
            if (is_array($subject)) {	// If subject string has at least one replaceable var:
                $user_subject = call_user_func_array('sprintf', array_map('T_', $subject));
            } else {	// Subject string has no replaceable vars:
 ...
 * @param array notification email template params
 */
function send_list_owner_notification($newsletter_IDs, $template_name, $template_params = [])
{
    ...
    foreach ($owner_newsletters as $owner_ID => $newsletters) {
        ...
        if (is_logged_in() && $current_User->ID == $owner_ID) {	// Don't send a notification to current user, because user already knows about this event
            return;
        }
        if ($UserSettings->get($check_setting, $owner_ID)) {	// this owner must be notified
...
function use_in_skin_login()
{
    ...
    if (is_admin_page()) { // Back-office page
        return false;
    }

    if (empty($blog)) { // No current blog selected
        return false;
    }

    if (empty($Blog)) { // Try to initialize global $Collection, $Blog object
        $BlogCache = &get_BlogCache();
        $Collection = $Blog = $BlogCache->get_by_ID($blog, false, false);
    }

    if (get_setting_Blog('login_blog_ID', $Blog)) { // If special blog is defined for all login actions
        return true;
    }

    if (empty($Blog)) { // No current blog found in DB
        return false;
    }
...
function check_setting($setting_name)
{
    ...
    if (is_admin_page() || empty($Blog)) { // Check setting in the Back office or when Blog is not defined
        if ($Settings->get($setting_name)) { // Set TRUE if the setting is ON
            return true;
        }
    } else { // Check setting in the Front office for current blog & skin
        $SkinCache = &get_SkinCache();
        $skin = &$SkinCache->get_by_ID($Blog->get('skin_ID'));
        if ($skin->get_setting($setting_name)) { // If setting is ON for current Blog & Skin
            if ($setting_name == 'bubbletip') { // Check separate case for setting 'bubbletip'
                if (is_logged_in() || $Settings->get($setting_name . '_anonymous')) { // If user is logged in OR Anonymous user can see bubbletips
                    return true;
                }
            } else { // Setting 'gender_colored' doesn't depend on user's logged status
                return true;
            }
        }
    }
 ...
 */
function get_user_register_link(
    $before = '',
    $after = '',
    $link_text = '',
    $link_title = '#',
    $disp_when_logged_in = false,
    $redirect = null,
    ...
    if ($link_class != '') { // Use a link class if it is defined
...
function get_user_register_url($redirect_to = null, $default_source_string = '', $disp_when_logged_in = false, $glue = '&amp;', $blog_ID = null)
{
    ...
    if (is_logged_in() && ! $disp_when_logged_in) { // Do not display, when already logged in:
        return false;
    }

    if ($Settings->get('newusers_canregister') == 'no') { // We won't let him register
        return false;
    }

    if ((! is_logged_in()) && (! $Settings->get('registration_is_public'))) { // Don't show registration link if it is not forced to display when a user is already logged in
        return false;
    }

    if (use_in_skin_login()) { // Use in-skin register form of the current blog or of the special blog for login/register actions
        if (empty($blog_ID)) { // Use current blog if it is not defined
            global $blog;
            $blog_ID = $blog;
        }
        ...
        $register_url = $Blog->get('registerurl', [
            'glue' => $glue,
        ]);
    } else { // Use normal/standard register form (without blog skin)
    ...
    if (! isset($redirect_to)) { // Set where to redirect
        $redirect_to = regenerate_url('', '', '', $glue);
    }
 ...
 */
function user_logout_link($before = '', $after = '', $link_text = '', $link_title = '#', $params = [])
{
 ...
 * @return string
 */
function get_user_logout_link($before = '', $after = '', $link_text = '', $link_title = '#', $params = [])
{
    global $current_User;
    ...
    if ($link_text == '') {
        $link_text = T_('Log out');
    }
    if ($link_title == '#') {
        $link_title = T_('Log out from your account');
    }
...
function get_user_logout_url($blog_ID = null)
{
    ...
    if (require_login($redirect_to, true)) { // if redirect_to page is a login page, or also require login ( e.g. evoadm.php )
        if (! empty($blog_ID)) { // Try to use blog by defined ID
            $BlogCache = &get_BlogCache();
        ...
        if (! empty($current_Blog)) { // Blog is set
            // set redirect_to to Blog url
            $redirect_to = $current_Blog->gen_blogurl();
        } else { // Blog is empty, set abort url to baseurl
...
function get_user_admin_link($before = '', $after = '', $link_text = '', $link_title = '#', $not_visible = '')
{
    ...
    if (! check_user_perm('admin', 'normal')) { // If user should NOT see admin link:
        return $not_visible;
    }
    ...
    if ($link_title == '#') {
        $link_title = T_('Go to the back-office...');
    }
...
function get_user_profile_link($before = '', $after = '', $link_text = '', $link_title = '#')
{
    ...
    if ($link_title == '#') {
        $link_title = T_('Edit your profile');
    }
...
function get_user_identity_link($user_login, $user_ID = null, $user_tab = 'profile', $link_text = 'avatar')
{
    ...
    if (empty($user_login)) {
        $User = &$UserCache->get_by_ID($user_ID, false, false);
        if (! $User) { // user with given user_ID doesn't exist
            return null;
        }
    } else {
...
function get_user_identity_url($user_ID, $user_tab = 'profile', $blog_ID = null)
{
    ...
    if (! $User->check_status('can_display_link') && ! (is_admin_page() && check_user_perm('users', 'edit', false, null, false))) { // if the account status restrict to display user profile link and current User is not an admin in admin interface, then do not return identity url!
        return null;
    }

    if (! is_logged_in() || $user_tab == 'userpage') { // user is not logged in
        return $User->get_userpage_url($blog_ID);
    }

    if (has_cross_country_restriction('users', 'profile') && ($current_User->ctry_ID !== $User->ctry_ID)) { // if the current user cannot view the profiles from other countries
        return null;
    }

    if (! check_user_perm('user', 'view', false, $User)) { // if the current user status restrict to view other user profile
        return null;
    }

    if (! is_admin_page()) { // can't display the profile form, display the front office User form
        return $User->get_userpage_url($blog_ID);
    }

    if (check_user_status('can_access_admin') && (($current_User->ID == $user_ID) || check_user_perm('users', 'view'))) {	// Go to backoffice profile:
...
function get_user_settings_url($user_tab, $user_ID = null, $blog_ID = null, $glue = '&amp;')
{
    ...
    $backoffice_user_tabs = ['marketing', 'advanced', 'admin', 'sessions', 'activity'];
    if (is_pro()) {
        $backoffice_user_tabs[] = 'social';
    }
    $frontoffice_user_tabs = ['profile', 'user', 'avatar', 'pwdchange', 'userprefs', 'subs', 'register_finish', 'visits', 'report'];
    ...
    if (is_array($modules_allowed_user_tabs)) {
        foreach ($modules_allowed_user_tabs as $module_allowed_user_tabs) {
            if (isset($module_allowed_user_tabs[$user_tab])) {	// We found the user tab is allowed:
                if ($module_allowed_user_tabs[$user_tab] == 'backoffice') {	// for back-office:
                    $backoffice_user_tabs[] = $user_tab;
                } elseif ($module_allowed_user_tabs[$user_tab] == 'frontoffice') {	// for front-office:
    ...
    if (empty($current_Blog)) { // Use base url when current blog is not defined yet
        global $baseurl;
        $blog_url = $baseurl;
    } else { // Use home page of the current blog
        $blog_url = $current_Blog->gen_blogurl();
    }
    if ($user_tab == 'pwdchange') {	// Force a change password page to https if it is required by setting "Require SSL":
    ...
    if ($is_admin_page || $is_admin_tab || empty($current_Blog) || $current_User->ID != $user_ID) {
        if (($current_User->ID != $user_ID && ! check_user_perm('users', 'view')) ||
                (! check_user_perm('admin', 'restricted') || ! check_user_status('can_access_admin'))) { // Use blog url when user has no access to backoffice
            if (empty($current_Blog)) { // Check if system has at least one blog
                $BlogCache = &get_BlogCache();
                $BlogCache->clear();
            ...
            if (! empty($current_Blog)) { // We should use blog url when at least one blog exist
                if ($is_admin_tab) { // Deny all admin tabs for such users
                    $user_tab = 'user';
                }
                if (in_array($user_tab, ['profile', 'avatar', 'pwdchange', 'userprefs', 'subs', 'register_finish'])) {
            ...
            } else { // No blogs exist in system
                return null;
            }
        }
        if (($user_tab == 'admin') && ($current_User->grp_ID != 1)) { // Only users of the first group can use the admin tab
            $user_tab = 'profile';
        }
    ...
    if (! empty($current_Blog) && in_array($user_tab, ['profile', 'avatar', 'pwdchange', 'userprefs', 'subs', 'register_finish'])) {
        return $current_Blog->get($user_tab . 'url', [
            'glue' => $glue,
        ]);
    } else {
...
function get_user_messaging_link($before = '', $after = '', $link_text = '#', $link_title = '#', $show_badge = false)
{
    ...
    if (! $user_messaging_url) {	// Messages link is not available
        return false;
    }
    ...
    $badge = '';
    if ($show_badge) {	// Show badge with count of uread messages:
        $unread_messages_count = get_unread_messages_count();
        if ($unread_messages_count > 0) {	// If at least one unread message:
...
function get_user_messaging_url()
{
    ...
    if (! check_user_perm('perm_messaging', 'reply')) {	// No minimum permissions for messaging module
        return false;
    }
...
function get_user_contacts_link($before = '', $after = '', $link_text = '#', $link_title = '#')
{
    ...
    if (! $user_contacts_url) {	// Messages link is not available
        return false;
    }
...
function userfield_td_name($ufdf_ID, $ufdf_name, $ufdf_icon_name, $ufdf_code)
{
    ...
    if (check_user_perm('users', 'edit')) { // We have permission to modify:
...
function profile_check_params($params, $User = null)
{
    ...
    if (isset($params['invitation'][0])) {
        if (empty($params['invitation'][0])) { // invitation code can't be empty
    ...
    if (isset($params['login'][0])) {
        if (empty($params['login'][0])) { // login can't be empty
            param_error($dummy_fields[$params['login'][1]], T_('Please enter your login.'));
        } else {
            param_check_valid_login($dummy_fields[$params['login'][1]]);

            if (param_check_valid_login($dummy_fields[$params['login'][1]])) {	// If login is valid
                global $reserved_logins;
                if (! empty($reserved_logins) &&
                    in_array($params['login'][0], $reserved_logins) &&
                    ! check_user_perm('users', 'edit')) {	// If new entered login is reserved and current logged in User cannot use this:
    ...
    if (isset($params['pass1'][0]) && isset($params['pass2'][0])) {
        if ($pass_required || ! empty($params['pass1'][0]) || ! empty($params['pass2'][0])) { // Password is required or was given
            ...
            } elseif (preg_match('/[<>&]/', $_POST[$dummy_fields[$params['pass1'][1]]])) { // Checking the not allowed chars
                param_error_multiple([$dummy_fields[$params['pass1'][1]], $dummy_fields[$params['pass2'][1]]], T_('Passwords cannot contain the characters &lt;, &gt; and &amp;.'));
            }
        }
    }
}
...
function social_profile_check_params($params, $provider)
{
    ...
    $error_messages_header = sprintf(
        T_('We cannot create an account based on your %s profile because your %s profile is missing the following information') . ':',
        '<b>' . $provider . '</b>',
        '<b>' . $provider . '</b>'
    );
...
function get_registration_template_required_fields($template_code = null)
{
    ...
    if (! $TemplateCache->get_by_code($template_code, false, false)) {	// Template not found:
...
function get_avatar_imgtag($user_login, $show_login = true, $link = true, $size = 'crop-top-15x15', $img_class = 'avatar_before_login', $align = '', $avatar_overlay_text = '', $link_class = '', $show_avatar = true, $rel = null)
{
    ...
    $img_tag = '';
    if ($show_avatar) { // Get user avatar
        $img_tag = $User->get_avatar_imgtag($size, $img_class, $align, false, $avatar_overlay_text);
    }

    $login_class = '';
    if (! empty($show_login) &&
        ($show_login !== 'login' || $Settings->get('username_display') != 'name')) { // Dsiplay user login or preffered name
    ...
    if (is_null($rel)) { // Set default rel:
        $rel = 'bubbletip_user_' . $User->ID;
    }

    if (! empty($rel)) { // Initialize attribure "rel"
        $rel = ' rel="' . $rel . '"';
    }

    $identity_url = get_user_identity_url($User->ID);
    if (empty($identity_url)) { // Current user has not permissions to view other user profile
        $img_tag = '<span class="' . $login_class . $User->get_gender_class() . '"' . $rel . '>' . $img_tag . '</span>';
    } elseif (! empty($img_tag)) { // Show avatar & user login as link to the profile page
...
function get_avatar_imgtags($user_logins_list, $show_login = true, $link = true, $size = 'crop-top-15x15', $class = 'avatar_before_login', $align = '', $read_status = null, $show_avatar = true, $separator = '<br />', $show_deleted_users = false)
{
    ...
    foreach ($user_logins_list as $user_login) {
        $icon = '';
        if (! is_null($read_status)) { // Add icon behind user login (read status)
            if ($read_status === 'left') { // user has left the conversation
                $icon = get_icon('bullet_black', 'imgtag', [
                    'alt' => sprintf(T_('%s has left this conversation.'), $user_login),
                    'style' => 'margin:1px 4px',
                ]);
            } elseif ($read_status === 'left_message') { // user has left the conversation before this message
                $icon = get_icon('bullet_black', 'imgtag', [
                    'alt' => sprintf(T_('%s has left the conversation and has not received this message.'), $user_login),
                    'style' => 'margin:1px 4px',
                ]);
            } elseif ($read_status) { // User has seen a message
                $icon = get_icon('allowback', 'imgtag', [
                    'alt' => sprintf(T_('%s has seen this message.'), $user_login),
                    'style' => 'margin:0 2px',
                ]);
            } else { // User has not seen a message
                $icon = get_icon('bullet_red', 'imgtag', [
                    'alt' => sprintf(T_('%s has NOT seen this message yet.'), $user_login),
                    'style' => 'margin:1px 4px',
                ]);
            }
        }
        if (empty($user_login)) { // user login is empty, we can't show avatar
            if ($show_deleted_users) { // show this users as deleted user
 ...
 */
function get_avatar_imgtag_default($size = 'crop-top-15x15', $class = '', $align = '', $params = [])
{
    global $Settings, $thumbnail_sizes;

    if (! $Settings->get('allow_avatars')) { // Avatars are not allowed, Exit here
        return '';
    }
    ...
    if (! $Settings->get('use_gravatar')) { // Gravatars are not allowed, Use default avatars instead
    ...
    } else { // Gravatars are enabled
        $default_gravatar = $Settings->get('default_gravatar');

        if (empty($params['default'])) { // Set default gravatar
            if ($default_gravatar == 'b2evo') { // Use gravatar from b2evo default avatar image
                $params['default'] = get_default_avatar_url($params['gender']);
            } else { // Use a selected gravatar type
        ...
        if (empty($img_url)) {
            ...
            $img_url_params = [];
            if (! empty($params['rating'])) { // Rating
                $img_url_params[] = 'rating=' . $params['rating'];
            }

            if (! empty($gravatar_width)) { // Size
                $img_url_params[] = 'size=' . $gravatar_width;
            }

            if (! empty($params['default'])) { // Type
                $img_url_params[] = 'default=' . urlencode($params['default']);
            }

            if (count($img_url_params) > 0) { // Append url params to request gravatar
                $img_url .= '?' . implode('&', $img_url_params);
            }
        }
    }

    if ($params['tag_size'] !== null) { // Change tag size
    ...
    if (! empty($params['username'])) { // Add alt & title
        $img_params['alt'] = $params['username'];
        $img_params['title'] = $params['username'];
    }
    if (! empty($class)) { // Add class
        $img_params['class'] = $class;
    }
    if (! empty($align)) { // Add align
...
function get_default_avatar_url($gender = '', $size = null)
{
    ...
    if (! ($File = &$FileCache->get_by_root_and_path('shared', 0, $avatar_url))) {	// Return the full size image URL without further ado if the requested file doesn't exist:
        global $media_url;
        return $media_url . 'shared/global' . $avatar_url;
    }

    if ($size !== null && $File->is_image()) {	// Get a thumbnail url only if the default avatar file is a real image:
...
function get_status_permvalue($statuses)
{
    ...
    if (! is_array($statuses)) {	// Convert to array if single string status:
        $statuses = [$statuses];
    }

    $status_permvalue = 0;
    foreach ($statuses as $status) {
        if (! isset($status_permission_map[$status])) {	// Stop on unknown status request:
...
function load_blog_advanced_perms(&$blog_perms, $perm_target_blog, $perm_target_IDs, $prefix)
{
    ...
    if (! $Blog->advanced_perms) { // We do not abide to advanced perms
        return false;
    }

    if (empty($perm_target_IDs)) { // Target object is not in DB, nothing to load!:
        return false;
    }

    if (! empty($blog_perms)) { // perms are already loaded, don't load again
        return false;
    }
    ...
    if (is_array($perm_target_IDs)) {	// This is a request to load perms to multiple targets:
        $single_target = false;
    } else {	// This is a request to load perms to single target:
        $single_target = true;
        $perm_target_IDs = [$perm_target_IDs];
    }
    ...
    $rows = $DB->get_results(
        ...
        ARRAY_A,
        sprintf(
            'Load advanced permissions for collection #%s and %s with IDs = (%s)',
            $perm_target_blog,
    ...
    $blog_perms = [];
    foreach ($rows as $row) {	// Store all found rights in DB to the array:
        $blog_perms[$row[$perm_target_key]] = [
    ...
    if (count($blog_perms) != count($perm_target_IDs)) {	// The rights are not found for some targets Blog - User/Group
        foreach ($perm_target_IDs as $perm_target_ID) {
            if (! isset($blog_perms[$perm_target_ID])) {	// No rights set for this Blog - User/Group: remember this (in order not to have the same query next time)
    ...
    if ($single_target) {	// This was a single request:
...
function check_blog_advanced_perm(&$blog_perms, $user_ID, $permname, $permlevel, $perm_target = null)
{
    ...
    switch ($permname) {
        case 'stats':
            if ($permlevel == 'view' || $permlevel == 'list') {	// If current user has a permission to view the collection:
        ...
        case 'blog_edit':
        case 'blog_edit_cmt':
            if ($permlevel == 'no') { // Doesn't make sensce to check that the user has at least 'no' permission
    ...
    if ($perm && ($permlevel == 'edit' || $permlevel == 'moderate')
        ...
        switch ($edit_permvalue) {
            ...
            case 'le': // Own + Lower or equal level posts only:
                if (empty($creator_user_ID) || ($creator_user_ID == $user_ID)) { // allow if the comment creator is not registered or it is the current User
                    return true;
                }
                $UserCache = &get_UserCache();
                // Get creator User
    ...
    if ($perm && $permlevel == 'moderate' && empty($creator_user_ID)) { // check moderator rights
        return in_array($blog_perms[$edit_permname], ['anon', 'lt', 'le', 'all']);
    }
...
function check_coll_first_perm($perm_name, $target_type, $target_ID)
{
    ...
    if (empty($target_ID)) {	// Target ID must be defined:
        return false;
    }
...
function echo_user_actions($Widget, $edited_User, $action)
{
    ...
    $link_attribs = [
        'style' => 'margin-left:1ex; margin-bottom:1ex;',
        'class' => 'btn btn-sm btn-default action_icon',
    ];

    if ($edited_User->ID != 0) { // show these actions only if user already exists
        if ($action != 'view' && $current_User->ID != $edited_User->ID && check_user_status('can_report_user', $edited_User->ID)) {
            ...
            if ($current_report == null) { // Current user has no report for this user yet
                $report_text_title = $report_text = T_('Report User');
            } else { // Current user already reported about this user
                $report_text_title = $report_text = T_('You have reported this user');
                $report_text = '<span class="red">' . $report_text . '</span>';
            }
            $report_user_link_attribs = array_merge($link_attribs, [
                'onclick' => 'return user_report( ' . $edited_User->ID . ', \'' . (empty($user_tab) ? 'profile' : $user_tab) . '\')',
            ]);
...
function get_user_sub_entries($is_admin, $user_ID)
{
    ...
    if ($user_ID == $current_User->ID || check_user_perm('users', 'view')) {	// If user is viewing own profile or has a permission to view all other users:
        $users_sub_entries['profile'] = [
            'text' => T_('Profile'),
            'href' => url_add_param($base_url, $ctrl_param . 'profile' . $user_param),
        ];

        if ($Settings->get('allow_avatars')) {
            $users_sub_entries['avatar'] = [
                'text' => T_('Profile picture'),
                'href' => url_add_param($base_url, $ctrl_param . 'avatar' . $user_param),
            ];
        }

        if (is_pro()) {	// Social Accounts tab available to PRO version only:
            $users_sub_entries['social'] = [
                'text' => T_('Social Accounts'),
                'href' => url_add_param($base_url, $ctrl_param . 'social' . $user_param),
            ];
        }

        if ($user_ID == $current_User->ID || $current_User->can_moderate_user($user_ID)) {	// If user is editing own profile or has a permission to moderate the user:
            $users_sub_entries['pwdchange'] = [
                'text' => T_('Password'),
                'href' => force_https_url(url_add_param($base_url, $ctrl_param . 'pwdchange' . $user_param), 'login'),
            ];
        }

        $users_sub_entries['userprefs'] = [
            'text' => T_('Preferences'),
            'href' => url_add_param($base_url, $ctrl_param . 'userprefs' . $user_param),
        ];

        $users_sub_entries['subs'] = [
            'text' => T_('Emails'),
            'href' => url_add_param($base_url, $ctrl_param . 'subs' . $user_param),
        ];

        if ($is_admin) {	// Show this only in backoffice:
            if ($current_User->can_moderate_user($user_ID)) {	// For moderators:
                $users_sub_entries['marketing'] = [
                    'text' => T_('Marketing'),
                    'href' => url_add_param($base_url, $ctrl_param . 'marketing' . $user_param),
                ];
            }

            if ($Settings->get('enable_visit_tracking') == 1) {	// If visit tracking is enabled:
                $users_sub_entries['visits'] = [
                    'text' => T_('Visits'),
                    'href' => url_add_param($base_url, $ctrl_param . 'visits' . $user_param),
                ];
            }

            $users_sub_entries['advanced'] = [
                'text' => T_('Advanced'),
                'href' => url_add_param($base_url, 'ctrl=user&amp;user_tab=advanced' . $user_param),
            ];

            if ($current_User->can_moderate_user($user_ID)) { // User have edit/delete all users permission, so this user is an administrator
                $users_sub_entries['admin'] = [
                    'text' => T_('Admin'),
                    'href' => url_add_param($base_url, 'ctrl=user&amp;user_tab=admin' . $user_param),
                ];
            }

            if ($current_User->ID == $user_ID || $current_User->can_moderate_user($user_ID)) {	// Only users with view/edit all users permission can see the 'Sessions' & 'User Activity' tabs
                $users_sub_entries['sessions'] = [
                    'text' => T_('Sessions'),
                    'href' => url_add_param($base_url, 'ctrl=user&amp;user_tab=sessions' . $user_param),
                ];

                $users_sub_entries['activity'] = [
        ...
        $modules_user_menus = modules_call_method('build_user_menu', [
            'user_ID' => $user_ID,
            'page_url' => $base_url,
...
{
    global $DB;
    if (($value < 0) || ($value > 1)) { // Invalid value. It should be 0 for unsubscribe and 1 for subscribe.
        return false;
    }
...
function set_user_subscription($user_ID, $blog, $items = null, $comments = null, $items_mod = null)
{
    ...
    if (($items < 0) || ($items > 1) ||
        ...
        ($comments < 0) || ($comments > 1)) {	// Skip wrong values:
        return false;
    }
...
function get_usertab_header($edited_User, $user_tab, $user_tab_title)
{
    ...
    $form_title = '<h2 class="user_title">' . $edited_User->get_colored_login([
        'login_text' => 'name',
    ]) . '<span class="nowrap">' . $user_status . ' &ndash;</span> ' . $user_tab_title . '</h2>';

    // set avatar tag
    $avatar_tag = $edited_User->get_avatar_imgtag('crop-top-48x48', 'floatleft', '', true);

    // build menu3
    $AdminUI->add_menu_entries(['users', 'users'], get_user_sub_entries(true, $edited_User->ID));
    $AdminUI->set_path('users', 'users', $user_tab);
    $user_menu3 = $AdminUI->get_html_menu(['users', 'users'], 'menu3');
...
function add_user_profile_visit($user_ID, $visitor_user_ID)
{
    ...
    if ($counter_row) {
        if (empty($counter_row['upv_last_visit_ts'])) { // Increment total new unique visitor
        ...
        } elseif ($counter_row['upv_last_visit_ts'] < $counter_row['upvc_last_view_ts']) { // Increment new unique visitors only
            $DB->query('UPDATE T_users__profile_visit_counters
					SET upvc_new_unique_visitors = upvc_new_unique_visitors + 1
					WHERE upvc_user_ID = ' . $DB->quote($user_ID));
        }
    } else { // First profile visit
...
function check_allow_new_email($limit_setting, $last_email_setting, $user_ID)
{
    ...
    if (empty($UserSettings)) {	// Initialize $UserSettings object (for example, it must be done when cron_exec.php is called in CLI mode):
        load_class('users/model/_usersettings.class.php', 'UserSettings');
        $UserSettings = new UserSettings();
    }

    $limit = $UserSettings->get($limit_setting, $user_ID);
    if ($limit == 0) { // user doesn't allow this kind of emails at all
        return false;
    }

    $email_count = 0;
    $last_email = $UserSettings->get($last_email_setting, $user_ID);
    if (! empty($last_email)) { // at least one email was sent
        ...
        if ($last_date == $current_date) { // last email was sent today
            if ($last_email_count >= $limit) { // the limit was already reached
                return false;
            }
...
function check_allow_new_anon_email($comment_ID)
{
    ...
    $CommentCache = &get_CommentCache();
    if (($Comment = &$CommentCache->get_by_ID($comment_ID, false, false)) === false) {	// Wrong request:
        return false;
    }
    ...
    if ($limit == 0) {	// Email notifications are not allowed for anonymous users at all:
        return false;
    }
...
function send_easy_validate_emails($user_ids, $is_reminder = true, $email_changed = false, $redirect_to_after = null, $log_messages = false, $email_template_name = 'account_activate')
{
    ...
    if (empty($user_ids)) {	// No users to send:
        return 0;
    }

    $UserCache = &get_UserCache();

    if (isset($GLOBALS['messaging_Module']) && $email_template_name == 'account_activate') { // Get already received messages for each recepient user:
        $already_received_messages = get_users_unread_threads($user_ids, null, 'string', 'text', 'http:');
    }

    $cache_by_locale = [];
    $email_sent = 0;
    foreach ($user_ids as $user_ID) { // Iterate through user ids and send account activation reminder to all user
        if ($log_messages == 'cron_job' && ! check_cron_job_emails_limit()) {	// Stop execution for cron job because max number of emails has been already sent:
            break;
        }
        ...
        if (! $User->check_status('can_be_validated')) { // User is validated or it is not allowed to be validated
            continue;
        }

        if ($is_reminder && (! $UserSettings->get('send_activation_reminder'))) { // This is an activation reminder, but user wouldn't like to receive this kind of emails
            continue;
        }

        if (mail_is_blocked($User->get('email'))) { // prevent trying to send an email to a blocked email address
            continue;
        }

        $notify_locale = $User->get('locale');
        $reminder_key = $UserSettings->get('last_activation_reminder_key', $User->ID);
        if (empty($reminder_key) || $email_changed) { // reminder key was not generated yet, or the user email address was changed and we need a new one, to invalidate old requests
            $reminder_key = generate_random_key(32);
            $UserSettings->set('last_activation_reminder_key', $reminder_key, $User->ID);
        }
        if (! isset($cache_by_locale[$notify_locale])) { // No subject for this locale generated yet:
        ...
        if (! empty($already_received_messages[$User->ID])) { // add already received message list to email body
        ...
        if (send_mail_to_User($User->ID, $cache_by_locale[$notify_locale]['subject'], $email_template_name, $email_template_params, true)) { // save corresponding user settings right after the email was sent, to prevent not saving if an eroor occurs
            $email_sent++;
    ...
    if ($email_sent && isset($Session) && $redirect_to_after !== null) {	// Set a redirect_to session variable because this way after the account will be activated we will know where to redirect
...
function send_inactive_user_emails($user_ids, $redirect_to_after = null, $log_messages = false)
{
    ...
    foreach ($user_ids as $user_ID) { // Iterate through user ids and send account activation reminder to all user
        ...
        if (! $UserSettings->get('send_inactive_reminder')) { // This is an activation reminder, but user wouldn't like to receive this kind of emails
            continue;
        }
        ...
        if (send_mail_to_User($User->ID, $cache_by_locale[$notify_locale]['subject'], 'account_inactive', $email_template_params, true)) { // save corresponding user settings right after the email was sent, to prevent not saving if an eroor occurs
            $email_sent++;
            // Set last remind activation email date and increase sent reminder emails number in UserSettings
 ...
 */
function check_usertags($user_ID, $test_tags = [], $type = 'has_any')
{
    $UserCache = &get_UserCache();
    if (! ($tag_User = &$UserCache->get_by_ID($user_ID, false, false))) {	// Don't halt on wrong user, and consider this like user has no tags:
...
function get_account_activation_info($edited_User)
{
    ...
    if (! $can_be_validated) {
        if ($edited_User->check_status('is_validated')) { // The user account is already activated
            return [[$field_label, T_('Account is already activated')]];
        }

        if ($edited_User->check_status('is_closed')) {
            return [[$field_label, T_('The account is closed, it cannot be activated')]];
        }

        debug_die('Unhandled user account status!');
    }

    if (! $UserSettings->get('send_activation_reminder', $edited_User->ID)) { // The user doesn't want to receive account activation reminders
        return [[$field_label, T_('This user doesn\'t want to receive account activation reminders')]];
    }

    $field_note = '';
    $is_secure_validation = ($Settings->get('validation_process') != 'easy');
    if ($is_secure_validation) { // The easy validation process is not allowed, so account activation emails are sent only for request
    ...
    if (empty($last_activation_email)) { // latest activation email date is not set because email was not sent yet ( it is possuble that there is some problem with the user email address )
        $result[] = [$field_label, T_('None yet'), $field_note];
    } else { // format last activation email date
        $last_activation_email_info = format_to_output($last_activation_email);
        $result[] = [$field_label, $last_activation_email_info, $field_note];
    }

    if ($is_secure_validation) { // When validation process is secure, then account activation email is not known, and this was already added as a note into the 'Last account activation email' field
        return $result;
    }
    ...
    if ($edited_User->status == 'failedactivation') { // The user account status was changed to failed activation, this user won't be reminded again to activate the account
        $result[] = [$field_label, T_('Account activation has failed'), $field_note . ' ' . $responsible_job_note];
    } elseif ($activation_reminder_count >= $number_of_max_reminders) { // This is the case when the account status was not changed to failed activation yet, but the last reminder was sent
        $result[] = [$field_label, sprintf(T_('We already sent %d account activation reminders of the maximum allowed of %d, no more reminders will be sent'), $activation_reminder_count, $number_of_max_reminders)];
    } elseif (empty($last_activation_email)) { // Account activation email was not sent at all. This can happen when some problem is with the user email
        $result[] = [$field_label, T_('At least one activation email should have been already sent. Check if the user email address is correct, and PHP is sending emails correctly'), $responsible_job_note];
    } else { // Activate account reminder email should be send to the user, set information when it should be done
        ...
        if ($next_activation_email_ts > $servertimenow) { // The next activation email issue date is in the future
            ...
            $result[] = [$field_label, $info, $responsible_job_note];
        } else { // The next reminder issue date was in the past
            ...
            $result[] = [$field_label, $info, $field_note];
        }
    }
    return $result;
}
...
function callback_options_user_new_fields($value = 0)
{
    ...
    $exclude_fields_sql = '';
    if (! empty($edited_User)) {	// Exclude not multiple fields for edited user
    ...
    if (count($userfielddefs) > 0) {	// Field types exist in DB
        ...
        foreach ($userfielddefs as $f => $fielddef) {
            if (! userfield_is_viewable(empty($edited_User) ? null : $edited_User->ID, $fielddef->ufdf_visibility, $fielddef->ufdf_type)) {	// Current user cannot add the user field:
                continue;
            }

            if ($fielddef->ufgp_ID != $current_group_ID) {	// New group
                if ($f != 0) {	// Close tag of previous group
                    $field_options .= "\n" . '</optgroup>';
                }
                $field_options .= "\n" . '<optgroup label="' . $fielddef->ufgp_name . '">';
            }
            $field_options .= "\n" . '<option value="' . $fielddef->ufdf_ID . '"';
            if ($value == $fielddef->ufdf_ID) {	// We had selected this type before getting an error:
                $field_options .= ' selected="selected"';
            }
            if ($fielddef->ufdf_suggest) {	// We can suggest a values for this field type
...
function userfield_is_viewable($user_ID, $user_field_visibility, $user_field_type)
{
    ...
    if (! is_pro() && $user_field_type == 'user') {	// Type "User select" is allowed only on PRO version:
        return false;
    }
...
{
    if (empty($icon_class)) {	// No icon class is provided:
        return '';
    }
...
function get_userfield_visibility_icon($user_field_visibility)
{
    switch ($user_field_visibility) {
        ...
        case 'private':
            return get_icon('file_not_allowed', 'imgtag', [
                'title' => T_('This field cannot be seen by other users.'),
                'color' => '#5bc0de',
            ]);

        case 'admin':
            return get_icon('file_not_allowed', 'imgtag', [
                'title' => T_('This field can only be seen by admins.'),
                'color' => '#F00',
            ]);
    }
...
{
    if ($userfield_row instanceof Userfield) {	// The passed row is already Userfield object:
        $Userfield = $userfield_row;
    } else {	// Initialize Userfield object from row data:
...
function userfields_display($userfields, $Form, $new_field_name = 'new', $add_group_fieldset = true, $user_ID = null)
{
    ...
    foreach ($userfields as $userfield) {
        if (empty($userfield->ufgp_ID)) {	// Don't display user field without group, because the user field definition was deleted from DB incorrectly:
            continue;
        }

        if (! userfield_is_viewable($user_ID, $userfield->ufdf_visibility, $userfield->ufdf_type)) {	// Current user cannot view this user field:
            continue;
        }

        if ($group_ID !== $userfield->ufgp_ID && $add_group_fieldset) {	// Start new group
            if ($group_ID !== 0) {	// End previous group
                $Form->end_fieldset();
            }
            $Form->begin_fieldset($userfield->ufgp_name . (is_admin_page() ? get_manual_link('user-profile-tab-userfields') : ''), [
                'id' => $userfield->ufgp_ID,
            ]);
        }
        ...
        if ($userfield->uf_ID == '0') {	// Try to get a value of new added field (not saved in DB) (recommended & require types):
            ...
            if (isset($uf_new_fields[$userfield->ufdf_ID][$$value_num])) {	// Get a value from submitted form:
                $uf_val = $uf_new_fields[$userfield->ufdf_ID][$$value_num];
                $$value_num++;
            }
        }

        if (is_null($uf_val)) {	// No value submitted yet, get DB val:
            $uf_val = $userfield->uf_varchar;
        } else {	// Format a submitted value for valid format:
            $userfield_type = $userfield->ufdf_type;
            if ($userfield_type == 'number') {	// Change number type of integer because we have this type name preparing in function param_format():
                $userfield_type = 'integer';
            } elseif ($userfield_type != 'text' && $userfield_type != 'url') {	// Use all other params as string, Only text and url have a preparing in function param_format():
        ...
        if ($action != 'view') {
            if (in_array($userfield->ufdf_duplicated, ['allowed', 'list'])) {	// Icon to add a new field for multiple field
                $field_note .= get_icon('add', 'imgtag', [
                    'rel' => 'add_ufdf_' . $userfield->ufdf_ID,
                    'style' => 'display:none !important; cursor: pointer; position: relative;',
                ]);
            }
        }

        if ($userfield->ufdf_type == 'url') { // URL field
            ...
            if (! empty($uf_val)) {
                ...
                $field_note .= '<a href="' . $url . '" target="_blank" class="action_icon" style="vertical-align: 0;">' . get_icon('permalink', 'imgtag', [
                    'title' => T_('Visit the site'),
                ]) . '</a>';
            }
        }
        ...
        if ($userfield->ufdf_required == 'require') {	// Field is required
            $field_params['required'] = true;
        }
        if ($userfield->ufdf_suggest == '1' && $userfield->ufdf_type == 'word') {	// Mark field with this tag to suggest a values
            $field_params['autocomplete'] = 'on';
        }

        if ($action == 'view') {	// Only view
            $Form->info(get_userfield_input_label($userfield), $uf_val . ' ' . $field_note);
        } else {	// Edit mode
            switch ($userfield->ufdf_type) {	// Display existing field:
                case 'text':
                ...
                case 'list':
                    ...
                    if ($userfield->ufdf_required != 'require' || // Not required field
                        ...
                        // Add empty value
                        $uf_options = array_merge(['---'], $uf_options);
                    }
                ...
                case 'user':
                    if (is_pro()) {	// Display user selector by PRO function:
    ...
    if ($group_ID > 0 && $add_group_fieldset) {	// End gruop fieldset if userfields are exist
        $Form->end_fieldset();
    }
}
...
{
    if (isset($userfield->_prepared) && $userfield->_prepared) {	// Userfield is already prepared, skip:
        return;
    }
...
function get_userlist_filters_config($Form = null)
{
    ...
    // Country:
    if (user_country_visible()) {	// Show country filter only on back-office or if it is allowed by collection setting on front-office:
        $CountryCache = &get_CountryCache();
        if (has_cross_country_restriction('users', 'list')) {	// User cannot browse other users from other country:
            ...
            $country_options = [];
        } else {	// User can browse other users from other country:
            $country_options = [
                '0' => T_('All'),
            ];
        }
    ...
    // Organization:
    if (empty($edited_Organization)) {	// Show organization filter only when organization form is not selected on back-office or if it is allowed by collection setting on front-office:
    ...
    $criteria_input = $Form->select_input_array('criteria_operator[]', '', [
        'contains' => T_('contains'),
        'not_contains' => T_('doesn\'t contain'),
    ], '');
    $criteria_input .= $Form->text('criteria_value[]', '', 17, '', '', 50);
    $criteria_input = $Form->select_input('criteria_type[]', '', 'callback_options_user_new_fields', '', [
        'field_suffix' => $criteria_input,
    ]);
    ...
    if (check_user_perm('users', 'moderate')) {	// If current user can moderate other users:
        // User last seen:
        $filters['lastseen'] = [
    ...
    if (check_user_perm('users', 'edit')) {	// Allow "Report count" filter only for users with edit user permission:
        $filters['report_count'] = [
    ...
    // Subscribed to:
    if (empty($edited_Newsletter) && empty($edited_EmailCampaign)) {	// Filter by newsletter(except of newsletter and email campaign edit forms):
        $NewsletterCache = &get_NewsletterCache();
    ...
    if (check_user_perm('users', 'moderate')) {	// Filter by user tags if current user can moderate other users:
        $filters['tags'] = [
            'label' => T_('User tags'),
...
function callback_filter_userlist(&$Form)
{
    ...
    if (! is_admin_page() && ! empty($Blog) && ($Blog->get_setting('allow_access') == 'members') && ($Blog->get_setting('userdir_filter_restrict_to_members'))) { // Restrict by members only when it is frontoffice and Blog allow access only for members
    ...
    if (user_country_visible() && (is_admin_page() || (isset($Blog) && $Blog->get_setting('userdir_filter_country')))) {
        { // Filter by country
            ...
            if (has_cross_country_restriction('users', 'list')) { // User cannot browse other users from other country
                ...
                $Form->hidden('country', $current_User->ctry_ID, [
                    'id' => 'country',
                ]);
            } else { // User can browse other users from other country
                $CountryCache = &get_CountryCache(NT_('All'));
                $Form->select_country('country', get_param('country'), $CountryCache, T_('Country'), [
                    'allow_none' => true,
                ]);
    ...
    if (user_region_visible() && (is_admin_page() || (isset($Blog) && $Blog->get_setting('userdir_filter_region')))) {	// Filter by region
    ...
    if (user_subregion_visible() && (is_admin_page() || (isset($Blog) && $Blog->get_setting('userdir_filter_subregion')))) {	// Filter by subregion
    ...
    if (user_city_visible() && (is_admin_page() || (isset($Blog) && $Blog->get_setting('userdir_filter_city')))) {	// Filter by city
    ...
    if (is_admin_page() && $edited_EmailCampaign) {
        ...
        $Form->select_input_array('recipient_type', get_param('recipient_type'), $campaign_send_status, '<span class="text-info">' . T_('Campaign Status') . '</span>', '', [
            'allow_none' => true,
        ]);
    }
    echo '<br />';
    ...
    if (user_region_visible()) {	// JS functions for AJAX loading of regions, subregions & cities
...
function callback_advanced_filter_userlist(&$Form)
{
    ...
    if (! empty($userlist_default_filters)) {
        ...
        foreach ($userlist_default_filters as $userlist_default_filter) {
            if ($userlist_default_filter == 'country' && has_cross_country_restriction('users', 'list')) {	// Skip default filter "Country" when current User can view other users only from own country:
                continue;
            }
            ...
            $filters['#default'][$userlist_default_filter] = [$default_operator, $default_value];
        }
    }

    if (! empty($filters['tags'])) {	// Filter by user tags if current user can moderate other users:
        ?>
<script>
...
function has_cross_country_restriction($type = 'users', $subtype = '')
{
    ...
    if (! is_logged_in()) { // In case of anonymous users we can't check the country, so anonymous users can't have restriction because of this
        return false;
    }

    if (check_user_perm('users', 'edit')) { // current user has global 'edit users' permission, these users have no restriction
        return false;
    }
    ...
    switch ($type) {
        case 'users': // Check retsriction on users
            switch ($subtype) {
                case 'list':
                    if ($Settings->get('allow_anonymous_user_list')) { // If anonymous users can browse users list from different countries, then logged in users must be always allowed to browse
                        return false;
                    }
                    break;

                case 'profile':
                    if ($Settings->get('allow_anonymous_user_profiles')) { // If anonymous users can browse user profiles from different countries, then logged in users must be always allowed to browse
                        return false;
                    }
                    break;

                default:
                    if ($Settings->get('allow_anonymous_user_list') && $Settings->get('allow_anonymous_user_profiles')) { // If anonymous users can browse users list and user profiles from different countries, then logged in users must be always allowed to browse
                        return false;
                    }
                    break;
            }
...
function get_user_statuses($null_option_name = '')
{
    ...
    if (! empty($null_option_name)) {	// Set null option
        $user_statuses = array_merge(
            [
                '' => $null_option_name,
            ],
            $user_statuses
...
{
    $user_status_icons = [
        'activated' => get_icon('bullet_green', 'imgtag', [
            'title' => T_('Account has been activated by email'),
        ]),
        'manualactivated' => get_icon('bullet_green', 'imgtag', [
            'title' => T_('Account has been manually activated'),
        ]),
        'autoactivated' => get_icon('bullet_green', 'imgtag', [
            'title' => T_('Account has been automatically activated'),
        ]),
        'new' => get_icon('bullet_blue', 'imgtag', [
            'title' => T_('New account'),
        ]),
        'deactivated' => get_icon('bullet_blue', 'imgtag', [
            'title' => T_('Deactivated account'),
        ]),
        'emailchanged' => get_icon('bullet_yellow', 'imgtag', [
            'title' => T_('Email address was changed'),
        ]),
        'closed' => get_icon('bullet_black', 'imgtag', [
            'title' => T_('Closed account'),
        ]),
        'failedactivation' => get_icon('bullet_red', 'imgtag', [
            'title' => T_('Account was not activated or the activation failed'),
        ]),
        'pendingdelete' => get_icon('bullet_red', 'imgtag', [
            'title' => T_('Account is pending delete'),
        ]),
    ];
...
function get_report_from($user_ID)
{
    ...
    return $DB->get_row(
        'SELECT urep_status as status, urep_info as info, urep_datetime as date
...
function count_reports_from($user_ID, $display_format = true)
{
    ...
    if (! $display_format) { // don't display return result
        return $reports;
    }

    if (empty($reports)) { // there are no reports yet from the given user
...
function add_report_from($user_ID, $status, $info)
{
    ...
    if (! $reported_User) { // if user doesn't exists return false
        return false;
    }
    ...
    if ($result) { // if report was successful send user reported notificaitons to admin users
        $email_template_params = [
...
function user_send_report_message($report_user_IDs, $reported_user_login)
{
    ...
    if (empty($report_user_IDs)) { // No users to recieve PM
        return false;
    }

    if (! $Settings->get('reportpm_enabled')) { // Sending of report PM is disabled
        return false;
    }
    ...
    if (! $from_User) { // Don't send a report email if sender login is incorrect
        return false;
    }

    foreach ($report_user_IDs as $u => $user_ID) {
        $User = $UserCache->get_by_ID($user_ID, false, false);
        if (! $User || ! $User->accepts_pm() || $user_ID == $from_User->ID) { // User can't read private messages, or doesn't want to receive private messages, or the same user is receiver
...
function user_increase_spam_score($report_user_IDs)
{
    ...
    if (empty($report_user_IDs)) { // No users to update spam score
        return false;
    }
 ...
 * @return string Form
 */
function get_user_quick_search_form($params = [])
{
    $params = array_merge([
    ...
    $r .= $Form->text_input('user_search', '', 15, $params['title'], '', [
        'maxlength' => 100,
    ]);
 ...
 * @param array Params
 */
function display_voting_form($params = [])
{
    $params = array_merge([
        'vote_type' => 'link',
        'vote_ID' => 0,
    ...
    switch ($params['vote_type']) { // Get a voting results for current user
        case 'link':
            // Picture
            ...
            // Number of votes
            if ($params['display_numbers']) { // Calculate the numbers of votes for current link
                ...
                if ($num_likes == 0 && $num_dislikes == 0) { // No votes
                    $vote_numbers = $params['msg_no_votes'];
                } elseif ($num_likes == 1 && $num_dislikes == 0) { // Only 1 like
                    $vote_numbers = $params['msg_1_like'];
                } elseif ($num_likes > 1 && $num_dislikes == 0) { // Only X likes
                    $vote_numbers = sprintf($params['msg_x_likes'], $num_likes);
                } elseif ($num_likes == 0 && $num_dislikes == 1) { // Only 1 dislike
                    $vote_numbers = $params['msg_1_dislike'];
                } elseif ($num_likes == 0 && $num_dislikes > 1) { // Only X dislikes
                    $vote_numbers = sprintf($params['msg_x_dislikes'], $num_dislikes);
                } elseif ($num_likes == 1 && $num_dislikes == 1) { // 1 like and 1 dislike
                    $vote_numbers = $params['msg_1_like_1_dislike'];
                } elseif ($num_likes > 1 && $num_dislikes == 1) { // X likes and 1 dislike
                    $vote_numbers = sprintf($params['msg_x_likes_1_dislike'], $num_likes);
                } elseif ($num_likes == 1 && $num_dislikes > 1) { // 1 like and X dislikes
                    $vote_numbers = sprintf($params['msg_1_like_x_dislikes'], $num_dislikes);
                } elseif ($num_likes > 1 && $num_dislikes > 1) { // X likes and X dislike
        ...
        case 'comment':
            ...
            if ($params['display_score']) {	// Calculate score:
    ...
    if (! empty($vote) && ! is_null($vote->result)) {	// Current user already voted for this file, We should set a disabled icons correctly
        switch ($vote->result) {
            case '-1':
                // Don't like
                $type_voted = 'dontlike';
    ...
    $checked_spam = '';
    if (! empty($vote)) {	// Current user already marked this file
        if (! empty($vote->inappropriate)) {	// File is marked as 'Inappropriate'
            $checked_inappropriate = ' checked="checked"';
        }
        if (! empty($vote->spam)) {	// File is marked as 'Spam'
    ...
    if (strpos($redirect_to, 'async.php') === false) {	// Append a redirect param:
        $url .= '&redirect_to=' . $redirect_to;
    }
    echo '</span>'; // END OF <span class="vote_title_panel">

    echo '<span class="btn-group">';
    if (! empty($params['widget_ID'])) {	// Append widget ID for action URL:
        $url .= '&widget_ID=' . intval($params['widget_ID']);
    }

    if (! empty($params['skin_ID'])) {	// Append skin ID for action URL:
        $url .= '&skin_ID=' . intval($params['skin_ID']);
    }

    if ($params['display_like']) {	// Display 'Like' icon:
        echo action_icon('', $icon_like, $url . '&vote_action=like', '', 0, 0, [], $params_like);
    }

    if ($params['display_noopinion']) {	// Display 'No opinion' icon:
        echo action_icon('', $icon_noopinion, $url . '&vote_action=noopinion', '', 0, 0, [], $params_noopinion);
    }

    if ($params['display_score'] && isset($score)) {	// Display score:
        echo '<span class="vote_score' . (empty($params['score_class']) ? '' : ' ' . $params['score_class']) . '">' . $score . '</span>';
    }

    if ($params['display_dontlike']) {	// Display 'Dont like' icon:
        echo action_icon('', $icon_dontlike, $url . '&vote_action=dontlike', '', 0, 0, [], $params_dontlike);
    }
    echo '</span>'; // END OF <span class="btn-group">

    echo '<span class="vote_others">';
    if ($params['display_inappropriate'] || $params['display_spam']) {	// Display separator between icons and checkboxes:
        echo '<span class="separator">&nbsp;</span>';
    }

    if ($params['display_inappropriate']) {	// Display 'Inappropriate' checkbox:
    ...
    if ($params['display_spam']) {	// Display 'Spam' checkbox:
    ...
    if (! empty($params['widget_ID'])) {	// Create a hidden input with widget ID:
        echo '<input type="hidden" id="widgetID" value="' . $params['widget_ID'] . '" />';
    }

    if (! empty($params['skin_ID'])) {	// Create a hidden input with skin ID:
...
function find_users_with_same_email($user_ID, $user_email, $message)
{
    ...
    if (empty($email_users)) { // No users found with email
        return false;
    }
...
function display_user_email_status_message($user_ID = 0)
{
    ...
    if (! is_logged_in() || ($user_ID != 0 && $user_ID != $current_User->ID)) { // User must be logged in AND only for current User
        return;
    }

    $email_status = $current_User->get_email_status();

    if (empty($email_status) || ! in_array($email_status, ['redemption', 'warning', 'suspicious1', 'suspicious2', 'suspicious3', 'prmerror'])) { // No message for current email status
        return;
    }
    ...
    if (check_user_status('is_validated')) { // Url to user profile page
        $url_change_email = get_user_settings_url('subs');
    } else { // Url to activate email address
    ...
    switch ($email_status) {
        ...
        case 'prmerror':
            $Messages->add(sprintf(
                T_('Your email address: %s does not seem to work or is refusing our emails%s. Please check your email address carefully. If it\'s incorrect, <a %s>change your email address</a>. If it\'s correct, please add our email: %s to your address book, then <a %s>click here to try again</a>!'),
                '<b>' . $current_User->get('email') . '</b>',
                $last_error_info,
                'href="' . $url_change_email . '"',
                '<b>' . user_get_notification_sender($current_User->ID, 'email') . '</b>',
        ...
        case 'redemption':
            $Messages->add(sprintf(
                T_('We are currently trying to send email to your address: %s again.'),
                '<b>' . $current_User->get('email') . '</b>'
            ), 'note');
            break;
    }
 ...
 * @param array Params
 */
function echo_user_deldata_js($params = [])
{
    global $admin_url;
 ...
 * @param array Params
 */
function user_report_form($params = [])
{
    ...
    if (! is_logged_in() || $current_User->ID == $params['user_ID'] || ! check_user_status('can_report_user', $params['user_ID'])) { // Current user must be logged in, cannot report own account, and must has a permission to report
        return;
    }
    ...
    if ($current_report == null) { // currentUser didn't add any report from this user yet
        ...
        foreach ($report_options as $option => $option_label) { // add select option, none must be selected
        ...
        printf(
            T_('You have reported this user on %s<br />as "%s"<br />with the additional info "%s"'),
...
{
    if (! check_user_perm('orgs', 'create')) {	// Check this min permission, because even owner of one organization can accept it:
        return;
    }

    global $b2evo_icons_type;
    $user_organization_config = [
...
function echo_user_add_organization_js($edited_Organization)
{
    ...
    if (! check_user_perm('orgs', 'edit', false, $edited_Organization)) {	// User must has an edit perm to add user to organization:
        return;
    }
...
function echo_user_edit_membership_js($edited_Organization)
{
    ...
    if (! check_user_perm('orgs', 'edit', false, $edited_Organization)) {	// User must has an edit perm to edit user in organization:
        return;
    }
...
function echo_user_remove_membership_js($edited_Organization)
{
    ...
    if (! check_user_perm('orgs', 'edit', false, $edited_Organization)) {	// User must has an edit perm to remove user in organization:
        return;
    }
...
function check_invitation_code($invitation_name = 'invitation')
{
    ...
    if ($Settings->get('system_lock') || $Settings->get('newusers_canregister') == 'no') {	// Don't display a registration form when it is disabled for all or sysytem is locked:
        return 'deny';
    }

    $invitation = param($invitation_name, 'string');

    if ($invitation == 'y') { // It is used to enter an invatation code manually
        set_param($invitation_name, '');
        return 'input';
    } elseif (! empty($invitation)) { // Check invitation code if it is entered
        if (! preg_match('#^[A-Za-z0-9\-_]{3,32}$#', $invitation)) { // Invitation code has incorrect format
            param_error($invitation_name, T_('Invitation code is incorrect!'));
            return 'input';
        } else { // Check invitation code in DB
            ...
            if (is_null($invitation_ID)) { // Invitation code doesn't exist in DB or it is expired
                param_error($invitation_name, T_('Invitation code is incorrect!'));
                return 'input';
            } else { // Invitation code is correct, Display it as info field on the registration form
                return 'info';
            }
        }
    }

    if (empty($invitation) && $Settings->get('newusers_canregister') == 'invite') { // Users can register only with invitation code
        if ($Settings->get('registration_is_public')) { // Display registration form even when invitation code is empty but setting "Registration links" is enabled
            // The invitation field should be visible for this case
            return 'input';
        } else { // Don't display a registration form
            return 'deny';
        }
    }

    if ($Settings->get('newusers_canregister') == 'invite') { // Always display the invitation field when users can register ONLY with invitation code
        return 'input';
    } else { // Don't display a field for invitation code, when this setting == 'yes'(Users can register themselves freely)
        return '';
    }
}
...
function get_users_IDs_by_logins($logins)
{
    ...
    foreach ($logins as $login) {
        if ($User = $UserCache->get_by_login($login, true)) { // User exists with this login
            $ids[] = $User->ID;
        }
    }
...
function check_access_users_list($mode = 'normal')
{
    ...
    if (! is_logged_in() && ! $Settings->get('allow_anonymous_user_list')) {	// Redirect to the login page if not logged in and allow anonymous user setting is OFF:
        $error_message = T_('You must log in to view the user directory.');
        if ($mode == 'api') {	// It is a request from REST API
    ...
    if (is_logged_in() && (! check_user_status('can_view_users'))) {	// Current user status doesn't permit to view users list
        if (check_user_status('can_be_validated')) {	// Current user is logged in but his/her account is not active yet
    ...
    if (has_cross_country_restriction('users', 'list') && empty($current_User->ctry_ID)) {	// User may browse other users only from the same country
...
function check_access_user_profile($user_ID, $mode = 'normal')
{
    ...
    if (! is_logged_in()) {	// Redirect to the login page if not logged in and allow anonymous user setting is OFF:
        $user_available_by_group_level = true;
        if (! empty($user_ID)) {
            $UserCache = &get_UserCache();
            if ($User = &$UserCache->get_by_ID($user_ID, false)) {	// If user exists we can check if the anonymous users have an access to view the user by group level limitation
                $User->get_Group();
        ...
        if (! $Settings->get('allow_anonymous_user_profiles') || ! $user_available_by_group_level || empty($user_ID)) {	// If this user is not available for anonymous users
    ...
    if (is_logged_in() && ! check_user_status('can_view_user', $user_ID)) {	// Current user is logged in, but his/her status doesn't permit to view user profile
        if (check_user_status('can_be_validated')) { // Current user is logged in but his/her account is not active yet
    ...
    if (! empty($user_ID)) {
        ...
        if (empty($User)) {	// Wrong user request
        ...
        if ($User->check_status('is_closed')) {	// The requested user is closed
        ...
        if (has_cross_country_restriction('any')) {
            if (empty($current_User->ctry_ID)) {	// Current User country is not set
                $error_message = T_('Please specify your country before attempting to contact other users.');
                if ($mode == 'api') {	// It is a request from REST API
                    return $error_message;
                } else {	// Normal request
            ...
            if (has_cross_country_restriction('users', 'profile') && ($current_User->ctry_ID !== $User->ctry_ID)) {	// Current user country is different then edited user country and cross country user browsing is not enabled.
 ...
 * @param array Params
 */
function user_sent_emails_results_block($params = [])
{
    ...
    if (! is_logged_in()) {	// Only logged in users can access to this function
        return;
    }

    if (! check_user_perm('users', 'moderate') || ! check_user_perm('emails', 'view')) {	// Check minimum permission:
        return;
    }

    $edited_User = $params['edited_User'];
    if (! $edited_User) {	// No defined User, probably the function is calling from AJAX request
        $user_ID = param('user_ID', 'integer', 0);
        if (empty($user_ID)) {	// Bad request, Exit here
            return;
        }
        $UserCache = &get_UserCache();
        if (($edited_User = &$UserCache->get_by_ID($user_ID, false)) === false) {	// Bad request, Exit here
            return;
        }
    }
    ...
    if ($params['action'] != 'view' && $emails_Results->get_total_rows() > 0 && check_user_perm('emails', 'edit')) {	// Display action icon to delete all records if at least one record exists & user has a permission:
    ...
    emails_sent_log_results($emails_Results, [
        'display_receiver' => false,
    ]);

    if (is_ajax_content()) {	// init results param by template name
    ...
    if (! is_ajax_content()) {	// Create this hidden div to get a function name for AJAX request:
 ...
 * @param array Params
 */
function user_email_returns_results_block($params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
        'edited_User' => null,
    ...
    if ($params['action'] != 'view' && $email_returns_Results->get_total_rows() > 0 && check_user_perm('emails', 'edit')) {	// Display action icon to delete all records if at least one record exists & user has a permission:
    ...
    // Initialize Results object:
    email_returns_results($email_returns_Results, [
 ...
 * @param array Params
 */
function user_reports_results_block($params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
        'edited_User' => null,
    ...
    if (! check_user_perm('users', 'moderate')) {	// Check minimum permission:
        return;
    }
...
function merge_users($merging_user_ID, $remaining_user_ID)
{
    ...
    $merging_user_login = $merging_User->get_identity_link([
        'thumb_class' => 'avatar_before_login_middle',
    ]);
    $remaining_user_login = $remaining_User->get_identity_link([
        'thumb_class' => 'avatar_before_login_middle',
    ]);
    ...
    $merge_config = [
        ...
        [T_('User settings'),              'T_users__usersettings', 'uset_user_ID'],
        [T_('User fields'),                'T_users__fields', 'uf_user_ID'],
        [T_('User own organizations'),     'T_users__organization', 'org_owner_user_ID'],
        [T_('Membership in organization'), 'T_users__user_org', 'uorg_user_ID'],
        [T_('User reports'),               'T_users__reports', 'urep_reporter_ID'],
        [T_('User secondary groups'),      'T_users__secondary_user_groups', 'sug_user_ID'],
        [T_('User tags'),                  'T_users__usertag', 'uutg_user_ID'],
        [T_('User visits'),                'T_users__profile_visits', 'upv_visitor_user_ID'],
        [T_('Plugin settings'),            'T_pluginusersettings', 'puset_user_ID'],
        [T_('Collections'),                'T_blogs', 'blog_owner_user_ID'],
        [T_('Collection Permissions'),     'T_coll_user_perms', 'bloguser_user_ID'],
        [T_('Collection subscriptions'),   'T_subscriptions', 'sub_user_ID'],
        [T_('Favorite collections'),       'T_coll_user_favs', 'cufv_user_ID'],
        [T_('Own Posts'),                  'T_items__item', 'post_creator_user_ID'],
        [T_('Edited Posts'),               'T_items__item', 'post_lastedit_user_ID'],
        [T_('Assigned Posts'),             'T_items__item', 'post_assigned_user_ID'],
        [T_('Post subscriptions'),         'T_items__subscriptions', 'isub_user_ID'],
        [T_('Post read data'),             'T_items__user_data', 'itud_user_ID'],
        [T_('Post edit histories'),        'T_items__version', 'iver_edit_user_ID'],
        [T_('Post votes'),                 'T_items__votes', 'itvt_user_ID'],
        [T_('Comments'),                   'T_comments', 'comment_author_user_ID'],
        [T_('Comment votes'),              'T_comments__votes', 'cmvt_user_ID'],
        [T_('Private Message'),            'T_messaging__message', 'msg_author_user_ID'],
        [T_('Thread read status'),         'T_messaging__threadstatus', 'tsta_user_ID'],
        [T_('Contacts'),                   'T_messaging__contact', 'mct_from_user_ID'],
        [T_('Contact lists'),              'T_messaging__contact', 'mct_to_user_ID'],
        [T_('Own contact groups'),         'T_messaging__contact_groups', 'cgr_user_ID'],
        [T_('Contact Groups'),             'T_messaging__contact_groupusers', 'cgu_user_ID'],
        [T_('Own Automations'),            'T_automation__automation', 'autm_owner_user_ID'],
        [T_('Automation step states'),     'T_automation__user_state', 'aust_user_ID'],
        [T_('Campaign send statuses'),     'T_email__campaign_send', 'csnd_user_ID'],
        [T_('Email Log'),                  'T_email__log', 'emlog_user_ID'],
        [T_('List subscriptions'),         'T_email__newsletter_subscription', 'enls_user_ID'],
        [T_('Own files'),                  'T_files', 'file_creator_user_ID'],
        [T_('User root files'),            'T_files', 'file_root_ID', 'file_root_type = "user"'],
        [T_('Own file Links'),             'T_links', 'link_creator_user_ID'],
        [T_('Edited file Links'),          'T_links', 'link_lastedit_user_ID'],
        [T_('User file Links'),            'T_links', 'link_usr_ID'],
        [T_('File link votes'),            'T_links__vote', 'lvot_user_ID'],
        [T_('Own Polls'),                  'T_polls__question', 'pqst_owner_user_ID'],
        [T_('Poll answers'),               'T_polls__answer', 'pans_user_ID'],
        [T_('Sessions'),                   'T_sessions', 'sess_user_ID'],
        [T_('System log'),                 'T_syslog', 'slg_user_ID'],
    ];

    // Display the merging actions:
    echo '<ul>';
    foreach ($merge_config as $mc) {	// Print out a merging log of each db column separately:
        ...
        if ($mc[1] == 'T_links' && $mc[2] == 'link_usr_ID') {	// Also move the files from merging user folder to remaining user folder:
...
function user_reports_results(&$reports_Results, $params = [])
{
    ...
    if (check_user_perm('users', 'edit', false)) { // Allow actions if current user has a permission to edit the users
        $reports_Results->cols[] = [
            'th' => T_('Actions'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'shrinkwrap',
 ...
 * @param array Params
 */
function users_results_block($params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
        'org_ID' => null,
    ...
    if (! $params['force_check_user'] &&
        ! check_user_perm('users', 'view')) {	// Check minimum permission to view users:
        return;
    }
    ...
    $UserList = new UserList($params['filterset_name'], $UserSettings->get('results_per_page'), $params['results_param_prefix'], [
    ...
    if ($action == 'show_recent') {	// Reset filters to default in order to view all recent registered users:
    ...
    if ($action == 'show_recent') {	// Sort an users list by "Registered" field:
    ...
    if ($params['display_filters']) {	// Display the filters:
        $UserList->filter_area = [
        ...
        if (is_admin_page()) {	// show "activated users" filter only on admin interface:
            $UserList->register_filter_preset('activated', T_('Activated users'), url_add_param($params['page_url'], $new_filter_baseurl));

            if (check_user_perm('users', 'edit')) {	// Show "Reported Users" filter only to users with edit user permission:
                $UserList->register_filter_preset('reported', T_('Reported users'), url_add_param($params['page_url'], $new_filter_baseurl));
            }
        }

        if (is_admin_page() && check_user_perm('users', 'edit')) {	// Settings for default user list filters:
     ...
     * Table icons:
     */
    if ($params['display_btn_refresh']) { // Display a button to refresh the users list
        $UserList->global_icon(T_('Refresh the list in case some Users have been added or deleted since your last search...'), 'refresh', url_add_param($params['page_url'], 'filter=refresh'), T_('Refresh'), 3, 4, [
            'class' => 'action_icon btn-warning',
        ]);
    }
    ...
    if (check_user_perm('users', 'edit', false)) {
        if ($params['display_btn_adduser']) { // Display a button to add user
            $UserList->global_icon(T_('Create a new user...'), 'new', $admin_url . '?ctrl=user&amp;action=new&amp;user_tab=profile', T_('Add user') . ' &raquo;', 3, 4, [
                'class' => 'action_icon ' . ($action == 'newsletter' ? 'btn-default' : 'btn-primary'),
            ]);
        }
        if ($params['display_btn_addgroup']) { // Display a button to add group
            $UserList->global_icon(T_('Create a new group...'), 'new', $admin_url . '?ctrl=groups&amp;action=new', T_('Add group') . ' &raquo;', 3, 4);
        }
    }

    if ($params['display_btn_adduserorg'] && ! empty($params['org_ID']) && is_logged_in()) {	// Display a button to add user to the organization:
        $OrganizationCache = &get_OrganizationCache();
        if ($Organization = &$OrganizationCache->get_by_ID($params['org_ID'], false, false) &&
            check_user_perm('orgs', 'edit', false, $Organization)) {	// If current user has a perm to edit the organization:
            $UserList->global_icon(T_('Add user'), 'new', '#', T_('Add user'), 3, 4, [
    ...
    if (count($UserList->cols)) {	// Display result only if at least one columns is defined:
        $UserList->display($params['display_params']);
    }

    if ($params['display_btn_merge']) {	// Display a button to merge users:
        echo '<p>';
        echo get_icon('multi_action', 'imgtag', [
            'style' => 'margin:0 5px 0 14px',
        ]);
    ...
    if ($params['display_btn_tags'] && check_user_perm('users', 'edit') && $UserList->result_num_rows > 0) {	// Button to add/remove tags from/to users:
    ...
    if ($params['display_btn_account_status'] && check_user_perm('users', 'edit') && $UserList->result_num_rows > 0) {	// Button to set user account status:
    ...
    if ($params['display_btn_change_groups'] && check_user_perm('users', 'edit') && $UserList->result_num_rows > 0) {	// Button to change user groups:
    ...
    if ($params['display_automation'] && check_user_perm('options', 'edit') && $UserList->result_num_rows > 0) {	// Button to add users to an automation:
    ...
    if ($params['display_newsletter'] && check_user_perm('emails', 'edit') && $UserList->result_num_rows > 0) {	// Button to change users of email campaign OR Create new email campaign for current selection:
    ...
    if ($params['display_btn_export'] && is_logged_in() && $UserList->result_num_rows > 0) {	// Button to export user group data as CSV file:
    ...
    if (check_user_perm('users', 'edit') && $UserList->result_num_rows > 0) {	// Buttons and info to delete spammers:
        if ($params['display_btn_delspam']) {	// Button to go to list with confirmation before spammers deleting:
        ...
        if ($params['display_delspam_info']) {	// Info and button to confirm to delete spammers:
    ...
    if (count($user_list_buttons)) {	// Display action buttons for users list:
 ...
 * @param array Params
 */
function users_results(&$UserList, $params = [])
{
    ...
    if ($UserList->filters['group'] > -1 || $UserList->check_filter_query('group', -1, '>')) { // List is grouped
        /*
         * Grouping params:
         */
    ...
    if ($params['display_orgstatus']) { // Display organization status
        $UserList->cols[] = [
            'th' => T_('Accepted'),
    ...
    if ($params['display_selector']) { // Display ID
        $UserList->cols[] = [
            'th' => '',
            'th_class' => 'shrinkwrap',
            'td_class' => 'shrinkwrap',
    ...
    if ($params['display_ID']) { // Display ID
        $UserList->cols[] = [
            'th' => T_('ID'),
    ...
    if ($params['display_avatar'] && $Settings->get('allow_avatars')) { // Display avatar
        $UserList->cols[] = [
            'th' => T_('Picture'),
    ...
    if ($params['display_login']) { // Display login
        $UserList->cols[] = [
    ...
    if ($params['display_firstname']) {	// Display first name:
        $UserList->cols[] = [
            'th' => T_('First name'),
    ...
    if ($params['display_lastname']) {	// Display last name:
        $UserList->cols[] = [
            'th' => T_('Last name'),
            'order' => 'user_lastname',
            'td' => '$user_lastname$',
    ...
    if ($params['display_nickname']) { // Display nickname
    ...
    if ($params['display_name']) {	// Display full name:
        $col = [
            'th' => T_('Full name'),
        ...
        if (! empty($params['order_name'])) {	// Set an order param for full name field:
    ...
    if ($params['display_role']) { // Display organizational role
        $UserList->cols[] = [
            'th' => T_('Role'),
            'th_class' => 'small',
            'td_class' => 'small',
    ...
    if ($params['display_priority']) {	// Display organizational order:
        $UserList->cols[] = [
            'th' => T_('Order'),
            'th_class' => 'small',
            'td_class' => 'small',
    ...
    if ($params['display_gender']) { // Display gender
        $UserList->cols[] = [
            'th' => T_('Gender'),
            'th_class' => 'shrinkwrap small',
            'td_class' => 'shrinkwrap small',
    ...
    if ($params['display_country']) { // Display country
        load_class('regional/model/_country.class.php', 'Country');
        load_funcs('regional/model/_regional.funcs.php');
        $UserList->cols[] = [
            'th' => T_('Country'),
            ...
            'order' => 'c.ctry_name',
            'td' =>
             (in_array($params['display_country_type'], ['both', 'flag']) ? '%country_flag( #ctry_code#, #ctry_name#, "w16px", "flag", "", false, true, "", false )%' : '')
            . ($params['display_country_type'] == 'both' ? ' ' : '')
            . (in_array($params['display_country_type'], ['both', 'name']) ? '$ctry_name$' : ''),
        ];
    }

    if ($params['display_region']) {	// Display region:
        $UserList->cols[] = [
            'th' => T_('Region'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'nowrap',
    ...
    if ($params['display_subregion']) {	// Display sub-region:
        $UserList->cols[] = [
            'th' => T_('Sub-region'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'nowrap',
    ...
    if ($params['display_city']) { // Display city
        $UserList->cols[] = [
            'th' => T_('City'),
    ...
    if ($params['display_phone']) {	// Display phone:
        $UserList->cols[] = [
            'th' => T_('Phone'),
            'th_class' => 'shrinkwrap',
    ...
    if ($params['display_soclinks']) {	// Display social links:
        $UserList->cols[] = [
            'th' => T_('Social links'),
            'th_class' => 'shrinkwrap',
    ...
    if ($params['display_blogs'] && isset($collections_Module)) { // We are handling blogs:
        $UserList->cols[] = [
            'th' => T_('Collections'),
            'order' => 'nb_blogs',
            'default_dir' => 'D',
    ...
    if ($params['display_source'] && check_user_perm('users', 'edit', false)) { // Display source
        $UserList->cols[] = [
            'th' => T_('Source'),
            'th_class' => 'shrinkwrap small',
            'td_class' => 'center small',
    ...
    if ($params['display_subscribed_list']) {	// Display subscribed lists:
        $UserList->cols[] = [
    ...
    if ($params['display_user_tags']) {	// Display user tags:
        $UserList->cols[] = [
            'th' => T_('User tags'),
    ...
    if ($params['display_pass_status']) {	// Display password status:
        $UserList->cols[] = [
            'th' => T_('Password set?'),
    ...
    if ($params['display_regdate']) { // Display registration date
        $UserList->cols[] = [
            'th' => T_('Registered'),
            'th_class' => 'shrinkwrap small',
            'td_class' => 'center small',
    ...
    if ($params['display_regcountry']) { // Display registration country
        $UserList->cols[] = [
            'th' => T_('RC'),
    ...
    if ($params['display_update']) { // Display date of the last user update
        $UserList->cols[] = [
    ...
    if ($params['display_lastvisit']) { // Display date of the last visit
        $UserList->cols[] = [
            'th' => T_('Last Visit'),
    ...
    if ($params['display_contact']) { // Display contact
        $UserList->cols[] = [
            'th' => T_('Contact'),
            'th_class' => 'shrinkwrap small',
            'td_class' => 'shrinkwrap small',
            ...
            ('~conditional( (#user_url# != \'http://\') && (#user_url# != \'\'), \' <a href="$user_url$" target="_blank" title="' . format_to_output(T_('Website'), 'htmlattr') . ': $user_url$">'
                    . get_icon('www', 'imgtag', [
                        'class' => 'middle',
                        'title' => format_to_output(T_('Website'), 'htmlattr') . ': $user_url$',
                    ]) . '</a>\', \'&nbsp;\' )~'),
        ];
    }
    ...
    if ($params['display_group'] && $UserList->filters['group'] == -1) { // List is ungrouped, Display column with group name
        $UserList->cols[] = [
    ...
    if ($params['display_sec_groups']) {	// Display column with count of secondary groups:
        $UserList->cols[] = [
            'th' => T_('Sec.<br />Groups'),
            'th_class' => 'shrinkwrap small',
            'td_class' => 'shrinkwrap small',
    ...
    if ($params['display_enlt_status']) { // Display list status:
        $UserList->cols[] = [
            'th' => T_('List Status'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'nowrap',
    ...
    if ($params['display_camp_user_status']) {	// Display account status before campaign status:
        $UserList->cols[] = [
    ...
    if ($params['display_email_status']) {	// Display account status before campaign status:
        $UserList->cols[] = [
            'th' => T_('Email status'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'shrinkwrap',
    ...
    if ($params['display_camp_status']) { // Display campaign status
        $UserList->cols[] = [
    ...
    if ($params['display_emlog_date']) { // Display email campaign send date:
        $UserList->cols[] = [
            'th' => T_('Send date'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'timestamp',
    ...
    if ($params['display_enls_subscribed']) { // Display status of user subscription:
        $UserList->cols[] = [
            'th' => T_('Subscribed'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'center',
    ...
    if ($params['display_enls_subscribed_ts']) { // Display subscription date:
        $UserList->cols[] = [
            'th' => T_('Subscribed'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'timestamp',
    ...
    if ($params['display_enls_unsubscribed_ts']) { // Display unsubscription date:
        $UserList->cols[] = [
            'th' => T_('Unsubscribed'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'timestamp',
    ...
    if ($params['display_enls_sent_manual']) {	// Display newsletter last sent date manually:
        $UserList->cols[] = [
            'th' => T_('Sent manually'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'timestamp',
    ...
    if ($params['display_enls_sent_auto']) {	// Display newsletter last sent date automatically:
        $UserList->cols[] = [
    ...
    if ($params['display_enls_last_open']) {	// Display newsletter last opened time:
        $UserList->cols[] = [
            'th' => T_('Last opened'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'timestamp',
    ...
    if ($params['display_enls_last_click']) {	// Display newsletter last clicked time:
        $UserList->cols[] = [
            'th' => T_('Last clicked'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'timestamp',
    ...
    if ($params['display_enls_send_count']) {	// Display email campaign send count:
        global $admin_url;
        $UserList->cols[] = [
    ...
    if (is_logged_in() && ! check_user_perm('users', 'moderate')) { // Current user has no permissions to moderate the users
        if (isset($userlist_col_reputaion)) { // Display the reported users
            $UserList->cols[] = $userlist_col_reputaion;
        }

        if ($params['display_level']) { // Display level
            $UserList->cols[] = [
                'th' => T_('Level'),
                'th_class' => 'shrinkwrap small',
                'td_class' => 'shrinkwrap small',
    ...
    } else { // Current user can moderate the users
        if ($params['display_status']) { // Display status
            $UserList->cols[] = [
        ...
        if ($params['display_actions']) { // Display actions
            $UserList->cols[] = [
                'th' => T_('Actions'),
                'th_class' => 'small',
                'td_class' => 'shrinkwrap small',
        ...
        if ($params['display_org_actions']) {	// Display actions for organization's users:
            $UserList->cols[] = [
                'th' => T_('Actions'),
                'th_class' => 'small',
                'td_class' => 'shrinkwrap small',
        ...
        if ($params['display_campaign_actions']) {	// Display actions for email campaign's users:
            $UserList->cols[] = [
                'th' => T_('Actions'),
                'th_class' => '',
                'td_class' => 'shrinkwrap',
        ...
        if ($params['display_dupl_actions']) {	// Display action for duplicated users:
            $UserList->cols[] = [
                'th' => T_('Actions'),
                'th_class' => 'small',
                'td_class' => 'shrinkwrap small',
...
function user_td_selector($user_ID)
{
    ...
    $r = '';
    if ($current_User->can_moderate_user($user_ID)) {	// Allow to select users for actions only if current user can moderate this user:
...
function user_td_grp_actions(&$row)
{
    ...
    if (check_user_perm('users', 'edit', false)) {
        ...
        if (($row->grp_ID != 1) && ($row->grp_ID != $Settings->get('newusers_grp_ID')) && ! in_array($row->grp_ID, $usedgroups)) { // delete
...
function user_td_grp_name($user_ID, $group_name, $group_level)
{
    ...
    if (is_logged_in() && $current_User->can_moderate_user($user_ID)) {	// Make a link to update the groups if current user can moderate this user:
        global $admin_url;
...
function user_td_sec_groups($user_ID, $secondary_groups_count)
{
    ...
    if (empty($secondary_groups_count)) {	// No secondary groups:
        return '';
    }
...
function user_td_pm($user_ID, $user_login)
{
    ...
    $UserCache = &get_UserCache();
    $User = &$UserCache->get_by_ID($user_ID);
    if ($User && ($User->get_msgform_possibility() == 'PM')) { // return new pm link only, if current User may send private message to User
...
function user_td_status($user_status, $user_ID)
{
    ...
    if (is_admin_page() && (check_user_perm('users', 'edit'))) { // current User is an administrator and view is displayed on admin interface, return link to user admin tab
...
function user_td_subscribed_list($lists, $user_email = '')
{
    ...
    foreach ($lists as $list_ID) {
        ...
        if ($loop_List = $NewsletterCache->get_by_ID($list_ID, false)) {
            if (check_user_perm('emails', 'view')) {	// Display a newsletter as link if current use has a permission to view newsletters list:
            ...
            } else {	// Display a newsletter as text if user has no permission:
...
{
    if ($pass_driver == 'nopass') {	// Use didn't set a password yet, e-g in case of email capture widget:
        return get_icon('close', 'imgtag', [
            'title' => T_('No'),
            'style' => 'color:#F00',
        ]);
    } else {	// Use already set a password:
        return get_icon('allowback', 'imgtag', [
            'title' => T_('Yes'),
        ]);
    }
}
...
function user_td_reg_country($user_ID, $country_code, $country_name)
{
    ...
    if (empty($flag)) { // No flag or registration country
        $flag = '?';
    }

    if (check_user_perm('users', 'edit')) { // Only users with edit all users permission can see the 'Sessions' tab
...
function user_td_level($user_ID, $user_level)
{
    ...
    if ($current_User->can_moderate_user($user_ID)) { // Current user can moderate this user
        return '<a href="#" rel="' . $user_level . '">' . $user_level . '</a>';
    } else { // No perms to edit user
        return $user_level;
    }
}
...
function user_td_actions($user_ID)
{
    ...
    if (check_user_perm('users', 'edit', false)) { // Current user can edit all users
...
function user_td_org_actions($org_ID, $user_ID)
{
    ...
    if (($user_Organization = &$OrganizationCache->get_by_ID($org_ID, false, false)) &&
        check_user_perm('orgs', 'edit', false, $user_Organization)) { // Current user can edit membership information:
        $link_params = [
...
function user_td_dupl_actions($user_ID)
{
    ...
    $r = '';
    if ($user_ID != 1 && $current_User->can_moderate_user($user_ID)) {	// Allow to merge users if current user can moderate this user:
...
function user_td_campaign_actions($campaign_ID, $user_ID, $csnd_status)
{
    ...
    if ($current_User->can_moderate_user($user_ID)) { // Current user can moderate this user
        ...
        $r .= action_icon(
            T_('Queue again'),
            'rewind',
            $admin_url . '?ctrl=campaigns&amp;action=queue&amp;ecmp_ID=' . $campaign_ID . '&amp;user_ID=' . $user_ID . '&amp;' . url_crumb('campaign') . '&amp;redirect_to=' . $redirect_to,
            null,
            null,
            null,
            [
                'class' => 'action_icon' . (in_array($csnd_status, ['ready_to_send', 'ready_to_resend']) ? ' invisible' : ''),
            ]
        );
        $r .= action_icon(
            T_('Skip'),
            'forward',
            $admin_url . '?ctrl=campaigns&amp;action=skip&amp;ecmp_ID=' . $campaign_ID . '&amp;user_ID=' . $user_ID . '&amp;' . url_crumb('campaign') . '&amp;redirect_to=' . $redirect_to,
            null,
            null,
            null,
            [
                'class' => 'action_icon' . (in_array($csnd_status, ['sent', 'send_error', 'skipped']) ? ' invisible' : ''),
            ]
        );
    } else {
        $r .= get_icon('edit', 'noimg');
...
function user_td_orgstatus($user_ID, $org_ID, $is_accepted)
{
    ...
    if (check_user_perm('orgs', 'edit', false, $Organization)) {	// Set the spec params for icon if user can edit the organization:
        $accept_icon_params = [
            'style' => 'cursor: pointer;',
            'rel' => 'org_status_' . ($is_accepted ? 'y' : 'n') . '_' . $org_ID . '_' . $user_ID,
        ];
    } else {
        $accept_icon_params = [];
    }

    if ($is_accepted) { // Organization is accepted by admin
        return get_icon('allowback', 'imgtag', array_merge([
            'title' => T_('Accepted'),
        ], $accept_icon_params));
    } else { // Organization is not accepted by admin yet
        return get_icon('bullet_red', 'imgtag', array_merge([
            'title' => T_('Not accepted'),
        ], $accept_icon_params));
    }
}
...
function user_td_email_status($emadr_status, $emadr_ID)
{
    ...
    if (empty($emadr_status)) {	// If email address does not exist in DB:
        $emadr_status = 'unknown';
    }

    $status_content = emadr_get_status_icon($emadr_status) . ' ' . emadr_get_status_title($emadr_status);

    if (is_admin_page() && $emadr_ID > 0 && check_user_perm('emails', 'view')) {	// Return a link to view email address details if current User has a permission:
...
{
    if ($csnd_status != 'send_error' && is_blocked_email_status($email_status)) {	// Force users with blocked email status("Permanent error" or "Spammer") to fake status "Cannot send":
    ...
    switch ($csnd_status) {
        ...
        case 'cannot_send': // This status doesn't exist in DB!
            ...
            if (! empty($csnd_emlog_ID) && check_user_perm('emails', 'view')) {	// Make a link to view details of error sending:
...
function validate_pwd_reset_session($reqID, $forgetful_User)
{
    ...
    if (! $forgetful_User || empty($reqID)) { // This was not requested
        return false;
    }
...
function user_domain_info_display($field_title, $field_key, $domain_name, &$Form)
{
    ...
    if ($display_user_domain) {
        ...
        if ($Domain = &get_Domain_by_subdomain($domain_name) && $perm_stat_edit) {	// Set a link to edit a top existing domain:
            $domain_name_formatted = preg_replace(
                '#(' . preg_quote(trim($Domain->get('name'), '.'), '#') . ')$#i',
    ...
    if ($display_user_domain) {	// Display status of Domain if current user has a permission:
        ...
        if ($perm_stat_edit) {	// User can edit Domain
            ...
            $Form->select_input_array('edited_' . $field_key, $domain_status, stats_dom_status_titles(), '<b class="evo_label_inline">' . T_('Domain status') . ': </b>' . $domain_name_formatted . ' ' . $domain_status_icon, '', [
                'force_keys_as_values' => true,
                'background_color' => stats_dom_status_colors(),
                'field_suffix' => $domain_status_action,
            ]);
        } else { // Only view status of Domain
...
function get_PasswordDriver($driver_code = '')
{
    ...
    if (isset($cached_password_drivers[$driver_code])) {	// Get password driver from cache if it already exists there:
        return $cached_password_drivers[$driver_code];
    }
    // Else get password driver by code and store in cache:

    if (! is_array($cached_password_drivers)) {	// Initialize the cache array:
        $cached_password_drivers = [];
    }
    ...
    if (! empty($driver_code)
        ...
        && ! in_array($drv_key, $enabled_password_drivers)) {	// If the requested password driver is not enabled currently,
    ...
    foreach ($allowed_password_drivers as $drv_key) {
        if (! isset($config_password_drivers[$drv_key])) {	// Driver is not detected by key, Skip this wrong:
            continue;
        }
        ...
        if ($drv_code == $driver_code || empty($driver_code)) {	// The requested password driver is detected in config
            // Check if password driver class exists in the system:
            $driver_file_path = 'users/model/passwords/' . $drv_key . '.php';
            if (file_exists($inc_path . $driver_file_path)) {	// Class file exists on the disk
                ...
                if (class_exists($driver_class_name)) {	// Class exists in the system
                    $PasswordDriver = new $driver_class_name();
                    if ($PasswordDriver->is_supported()) {	// If the password driver is supported then stop search others:
                        break;
                    } else {	// The password driver is NOT supported by server (probably it has an old PHP verstion), go to search next supported driver:
                        $PasswordDriver = null;
                    }
                }
            }
...
function get_mentioned_user_IDs($type, $content, $exclude_user_IDs = null)
{
    ...
    if (preg_match_all('/(^|[\s\t\n\r>,])@([A-Za-z0-9_\.]+)/', $content, $mentioned_logins)) {	// At least one mentioned user login is found in the given content:
        global $Settings, $DB;
        ...
        if (! empty($exclude_user_IDs)) {	// Set except moderators condition. Exclude moderators who already got a notification email:
...
function display_user_groups_selectors(&$User, &$Form)
{
    ...
    $group_where_sql = '';
    if (! check_user_perm('users', 'edit')) {	// Show the limited list for moderators:
        $group_where_sql = 'grp_level < ' . $current_User->get_Group()->get('level');
    }

    if ($User->ID == 1) {	// This is Admin user, Don't allow to change status, primary group:
    ...
    } else {	// Allow to change status and primary group for non-admin users:
        $GroupCache->clear();
    ...
    if (empty($user_secondary_groups)) {	// If user has no secondary groups yet, Add one empty element to display a select box to select first secondary group:
    ...
    foreach ($user_secondary_groups as $s => $user_secondary_Group) {
        ...
        $field_add_icon = get_icon('add', 'imgtag', [
            'class' => 'add_secondary_group',
            'style' => 'cursor:pointer',
        ]);

        if (empty($user_secondary_Group) || $user_secondary_Group->can_be_assigned()) {	// Current user has a permission to assign this group:
            $user_secondary_group_ID = empty($user_secondary_Group) ? 0 : $user_secondary_Group->ID;
            $Form->select_input_object('edited_user_secondary_grp_ID[]', $user_secondary_group_ID, $GroupCache, $field_title, [
                'allow_none' => true,
                'field_suffix' => $field_add_icon,
            ]);
        } else {	// Current user has no permission to assign this group:
            $Form->info_field($field_title, $user_secondary_Group->get_name() . ' ' . $field_add_icon, [
    ...
    $Form->select_input_object('template_secondary_group_select', 0, $GroupCache, '', [
        'allow_none' => true,
        'field_suffix' => $field_add_icon,
    ]);
    echo '</div>';
