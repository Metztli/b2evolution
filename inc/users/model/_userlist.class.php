...
class UserList extends DataObjectList2
{
    ...
    public function __construct(
        ...
        $this->set_default_filters(
            [
                ...
                'users' => [], // User IDs - used to cache results
                'userids' => [], // User IDs - used to filter by user IDs
    ...
    public function set_filters($filters)
    {
        ...
        // Country
        if (has_cross_country_restriction('users', 'list')) { // In case of cross country restrionction we always have to set the ctry filter
            // In this case we always have a logged in user
            global $current_User;
            if ((! empty($current_User->ctry_ID)) && ($current_User->ctry_ID != $this->filters['country'])) { // current country filter is not the same
        ...
        if ($this->memorize) {	// set back the GLOBALS !!! needed for regenerate_url() :
            /*
            ...
            if ($this->order != $this->filters['order']) {	// Save order from request
    ...
    public function load_from_Request($use_filters = true)
    {
         ...
         */
        if (in_array($this->filters['filter_preset'], ['men', 'women', 'other'])) {
        ...
        if (! empty($prev_filters['order'])) {	// Restore an order from saved session
    ...
    public function query_init()
    {
        ...
        if (isset($this->query_params['keywords_fields'])) { // Change keywords_fields from query params
            $this->UserQuery->keywords_fields = $this->query_params['keywords_fields'];
        }

        if (isset($this->query_params['where_status_closed'])) { // Limit by closed users
        ...
        if (has_cross_country_restriction('users', 'list')) { // Browse users from different countries is restricted, filter to current user country
        ...
        } else { // Browse users from different countries is allowed
        ...
        if (isset($this->filters['org']) || isset($this->query_params['where_org_ID'])) {	// Filter by organization ID:
        ...
        if (isset($this->query_params['where_viewed_user'])) {	// Filter by user profile viewed:
            $this->UserQuery->where_viewed_user($this->query_params['where_viewed_user']);
        }
        if (isset($this->filters['reg_ip_min']) && isset($this->filters['reg_ip_max'])) { // Filter by IP address:
            $this->UserQuery->where_reg_ip($this->filters['reg_ip_min'], $this->filters['reg_ip_max']);
        }
        if (! is_logged_in()) { // Restrict users by group level for anonymous users
            global $Settings;
        ...
        if (isset($this->query_params['order_by_login_length']) && in_array($this->query_params['order_by_login_length'], ['A', 'D'])) {
    ...
    public function run_query(
        ...
        // *** STEP 1 ***
        $user_IDs = isset($this->filters['users']) ? $this->filters['users'] : [];
        if ($this->refresh_query || // Some filters are changed
            ...
            if (! empty($this->query_params['join_colls'])) {	// Initialize count of collections (used on order by this field):
                $step1_SQL->SELECT_add(', COUNT( DISTINCT blog_ID ) AS nb_blogs');
            }
            if (! empty($this->filters['reported']) && (empty($this->filters['filter_query']) || $this->check_filter_query('report_count', 0, '>'))) {	// Filter is selected to 'Report count'
                $step1_SQL->SELECT_add(', user_rep');
            }
            if (! empty($this->query_params['join_sec_groups'])) {	// Initialize count of secondary groups (used on order by this field):
        ...
        if (! empty($this->exclude_users)) {	// Exclude users from filtered list:
            $exclude_users = explode(',', $this->exclude_users);
            foreach ($exclude_users as $exclude_user_ID) {
                if (($exclude_user_ID_index = array_search($exclude_user_ID, $user_IDs)) !== false) {	// This user should be excluded from filtered list:
        ...
        if (! empty($user_IDs_paged)) {	// Init sql query to get users by IDs
     ...
     */
    public function prevnext_user_links($params)
    {
        $params = array_merge([
        ...
        if (array_search($user_ID, $users_list) === false) {	// Selected user is NOT located in this list
            return;
        }
     ...
     */
    public function prevnext_user_link($direction, $before = '', $after = '', $text = '&laquo; $login$', $no_user = '', $user_tab = 'profile', $display = true)
    {
        /**
         * @var User
         */
        $prev_User = &$this->get_prevnext_User($direction);
        if (has_cross_country_restriction('users', 'list')) { // If current user has cross country restriction, make sure we display only users from the same country
        ...
        if (! empty($prev_User)) {	// User exists in DB
            $output = $before;
            $identity_url = get_user_identity_url($prev_User->ID, $user_tab);
            $login = str_replace('$login$', $prev_User->get_colored_login([
                'login_text' => 'name',
            ]), $text);
            if (! empty($identity_url)) {	// User link is available
                // Note: we don't want a bubble tip on navigation links
                $output .= '<a href="' . $identity_url . '">' . $login . '</a>';
            } else {	// No identity link
                $output .= $login;
            }
            $output .= $after;
        } else {	// No user
     ...
     */
    public function back_user_link($before = '', $after = '', $display = true)
    {
        // ID of selected user
        $user_ID = get_param('user_ID');
        ...
        if (is_int($user_key)) {	// Selected user is located in this list
     ...
     */
    public function &get_prevnext_User($direction = 'next', $selected_user_ID = null)
    {
        $users_list = $this->filters['users'];

        if (count($users_list) < 2) {	// Short users list
            $r = null;
            return $r;
        }

        // ID of selected user
        if ($selected_user_ID === null) { // get currently selected user ID from param
            $selected_user_ID = get_param('user_ID');
        }

        $user_key = array_search($selected_user_ID, $users_list);
        if (is_int($user_key)) {	// Selected user is located in the list
            $prevnext_key = $direction == 'next' ? $user_key + 1 : $user_key - 1;
            if (isset($users_list[$prevnext_key])) {	// Prev/next user is located in the list
                $prevnext_ID = $users_list[$prevnext_key];
            }
        }

        if (empty($prevnext_ID)) {	// No prev/next user
            $r = null;
            return $r;
        }
     ...
     */
    public function set_order($order_field, $direction = 'D', $save_filters = false)
    {
        global $Session;

        if (empty($this->cols)) { // The columns are not defined yet, Exit here
            return;
        }
