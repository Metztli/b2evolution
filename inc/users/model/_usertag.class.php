...
class UserTag extends DataObject
{
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                ...
                'fk' => 'uutg_emtag_ID',
                'msg' => /* TRANS: cascade delete */ T_('%d tags from users'),
            ],
        ];
    }
    ...
    public function load_from_Request()
    {
        ...
        if ($existing_tag_ID = $this->dbexists('utag_name', $utag_name)) { // Other tag already exists with the same name:
            if (empty($this->ID)) { // Suggest to edit existing tag for new creating tag
                param_error('utag_name', sprintf(
                    T_('This tag already exists. Do you want to <a %s>edit the existing tag</a>?'),
                    'href="?ctrl=usertags&amp;action=edit&amp;utag_ID=' . $existing_tag_ID . '"'
                ));
            } else { // Suggest to merge for existing tag
                ...
                $this->merge_message = sprintf(
                    T_('The previously named "%s" tag (applied to %d users) will be merged with the existing "%s" tag (already applied to %d users). Are you sure?'),
                    $this->dget('name'),
                    $old_tag_posts,
                    $utag_name,
                    $new_tag_posts,
