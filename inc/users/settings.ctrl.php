...
switch ($action) {
    case 'update':
        ...
        if ($timeout_online < 300) { // lower than 5 minutes: not allowed
            param_error('timeout_online', sprintf(TB_('You cannot set an online/offline timeout below %d minutes.'), 5));
        } elseif ($timeout_online > 21600) { // hihger than 6 hours: notice/warning
        ...
        if ($uset_location_city == 'required') {	// If city is required - all location fields also are required
            $uset_location_country = $uset_location_region = $uset_location_subregion = 'required';
        } elseif ($uset_location_subregion == 'required') {	// If subregion is required - country & region fields also are required
            $uset_location_country = $uset_location_region = 'required';
        } elseif ($uset_location_region == 'required') {	// If region is required - country field also is required
        ...
        if (! $Messages->has_errors()) {
            if ($Settings->dbupdate()) {
                if ($old_allow_avatars != $allow_avatars) { // invalidate all PageCaches
