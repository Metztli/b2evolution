...
if (! check_user_perm('users', 'view')) { // User has no permissions to view: he can only edit his profile
    ...
    if (! in_array($action, ['update', 'edit', 'default_settings', 'change_admin_skin'])) {
...
if (! is_null($user_ID)) {   // User selected
    if (($edited_User = &$UserCache->get_by_ID($user_ID, false)) === false) {	// We could not find the User to edit:
        unset($edited_User);
        forget_param('user_ID');
        $Messages->add(sprintf(TB_('Requested &laquo;%s&raquo; object does not exist any longer.'), TB_('User')), 'error');
        $action = 'list';
    } elseif ($action == 'list') { // 'list' is default, $user_ID given
    ...
    if ($action != 'list') { // check edit permissions
        ...
        */
        ...
        case 'delete':
            ...
            if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
                ...
                $edited_User->init_relations([
                    'delete_messages' => $is_spammer,
                    'delete_comments' => $is_spammer,
                    'delete_files' => $is_spammer,
                ]);

                if (! $edited_User->check_delete($msg, [], true)) { // There are restrictions:
                    $action = 'view';
                }
            }
            break;
        ...
        case 'add_settings_set': // delegates to edit_settings
            ...
            load_funcs('plugins/_plugin.funcs.php');
            _set_setting_by_path($edit_Plugin, 'UserSettings', $set_path, []);
        ...
        case 'search':
            ...
            $UserList = new UserList('admin', $UserSettings->get('results_per_page'), 'users_', [
                'join_city' => false,
            ]);
            ...
            if ($UserList->get_total_rows() == 1) {	// If we find only one user by quick search we do a redirect to user's edit page
        ...
        case 'campaign':
            ...
            if (! ($edited_EmailCampaign = &get_session_EmailCampaign())) {	// Initialize Email Campaign once and store in Session:
        ...
        case 'add_automation':
            ...
            $added_users_num = $Automation->add_users(get_filterset_user_IDs(), [
        ...
        case 'update_tags':
            ...
            $updated_users_num = 0;
            foreach ($UserCache->cache as $filtered_User) {	// Update tags of each filtered User:
        ...
        case 'update_status':
            ...
            $updated_users_num = 0;
            foreach ($UserCache->cache as $filtered_User) {	// Update account status of each filtered User:
                if ($filtered_User->ID == 1) {	// This is Admin user, Don't allow to change status:
                    continue;
                }
        ...
        case 'update_groups':
            ...
            foreach ($UserCache->cache as $filtered_User) {	// Update group membership of each filtered User:
                $updated_group = false;

                if (($filtered_User->ID != 1) && isset($primary_group) && $primary_group->can_be_assigned()) {	// This is Admin user, Don't allow to change status:
                ...
                if (isset($add_secondary_grp_ID) || isset($remove_secondary_grp_ID)) {
                    ...
                    if (isset($add_secondary_grp_ID) && ! in_array($add_secondary_grp_ID, $secondary_grp_IDs)) {	// User not yet a member of new secondary group:
                    ...
                    if (isset($remove_secondary_grp_ID) && (($key = array_search($remove_secondary_grp_ID, $secondary_grp_IDs)) !== false)) {	// User is a member of secondary group to be removed:
        ...
        case 'merge':
            ...
            if ($merging_user_ID == 1) {	// User #1 cannot be deleted so we should not allow to merge it as well:
            ...
            if (empty($selected_user_ID)) {	// Inform to select a remaining account if it is not selected yet:
            ...
            } else {	// Check edit permissions for remaining user as well:
        ...
        case 'delete_spammers':
            ...
            $deleted_spam_logins = [];
            $not_deleted_spam_logins = [];
            $UserCache = &get_UserCache();
            $delspam_Messages = new Messages();
            foreach ($users as $u => $user_ID) {
                if (! ($deleted_spam_User = &$UserCache->get_by_ID($user_ID, false, false))) {	// Skip if user is not found in DB by requested ID:
                    continue;
                }
                $deleted_spam_login = $deleted_spam_User->get('login');
                // Delete user as spammer:
                if ($deleted_spam_User->dbdelete($delspam_Messages)) {	// If user has been deleted:
                    $deleted_spam_logins[] = $deleted_spam_login;
                } else {	// If user cannot be deleted by some reason:
                    $not_deleted_spam_logins[] = $deleted_spam_User->get_identity_link();
                }
            }

            if (count($deleted_spam_logins) > 0) {	// Display a message if at least one spammer have been deleted:
                $Messages->add(sprintf(TB_('Spammers %s have been deleted.'), implode(', ', $deleted_spam_logins)), 'success');
            }
            if (count($not_deleted_spam_logins) > 0) {	// Display a message if at least one spammer have NOT been deleted:
        ...
        case 'export':
            ...
            $UserList->load_from_Request();
            if (empty($UserList->filters['users'])) {	// No users to export:
            ...
            echo get_csv_line(['username', 'groupname', 'type']);
        ...
        case 'import': // PRO-ONLY
            ...
            if ($import_operation === false) {	// Some errors are exist, Stop the importing:
                $action = 'csv';
                break;
            } else {
...
if ($tab == 'stats') {	// Users stats
    ...
    $AdminUI->add_menu_entries(['users', 'users'], $entries);
    ...
    switch ($tab3) {
        ...
        default:
            ...
            if (check_user_perm('users', 'moderate')) {	// Include to edit user level
...
switch ($action) {
    ...
    case 'delete':
        ...
        $confirm_messages = [];
        if ($deltype == 'spammer') {	// Display the notes for spammer deleting:
            $confirm_messages[] = [TB_('Note: this will also delete private messages sent/received by this user.'), 'note'];
            $confirm_messages[] = [TB_('Note: this will also delete comments made by this user.'), 'note'];
            $confirm_messages[] = [TB_('Note: this will also delete files uploaded by this user.'), 'note'];
            $confirm_messages[] = ['<strong>' . sprintf(TB_('Note: the email address %s will be banned from registering again.'), '<code>' . $edited_User->get('email') . '</code>') . '</strong>', 'note'];
        } else {	// Display the notes for standard deleting:
            $confirm_messages[] = [TB_('Note: this will <b>not</b> automatically delete private messages sent/received by this user. However, this will delete any new orphan private messages (which no longer have any existing sender or recipient).')
            ...
            $confirm_messages[] = [TB_('Note: this will <b>not</b> delete comments made by this user. Instead it will transform them from member to visitor comments.')
            ...
            $confirm_messages[] = [TB_('Note: this will <b>not</b> delete files uploaded by this user outside of the user root. Instead the creator ID of these files will be set to NULL.')
        ...
        $delete_form_params = [];
        if ($Settings->get('reportpm_enabled')) { // If this feature is enabled
            $user_count_reports = count(get_user_reported_user_IDs($edited_User->ID));
            if ($user_count_reports > 0) { // If the user has been reported at least one time
        ...
        if ($deltype == 'spammer') {	// Display user activity lists:
            $user_tab = 'activity';
            $AdminUI->disp_view('users/views/_user_activity.view.php');
        } else {	// Display user identity form:
