...
if (param('ufdf_ID', 'integer', '', true)) {// Load userfield from cache:
    $UserfieldCache = &get_UserFieldCache();
    if (($edited_Userfield = &$UserfieldCache->get_by_ID($ufdf_ID, false)) === false) {	// We could not find the user field to edit:
...
switch ($action) {
    case 'new':
        ...
        if (! isset($edited_Userfield)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record Userfield and create similar
        ...
        if ($edited_Userfield->load_from_Request()) { // We could load data from form without errors:
            // Insert in DB:
            $DB->begin();
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Userfield->check_delete(sprintf(TB_('Cannot delete user field &laquo;%s&raquo;'), $edited_Userfield->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
    ...
    case 'move_down':
        ...
        if ($action == 'move_up') {	// Set variables for "move up" action
            $order_condition = '<';
        ...
        if (is_null($switched_Userfield)) {	// Current field is first or last in group, no change ordering
...
if (in_array($action, ['delete', 'new', 'create', 'create_new', 'create_copy', 'edit', 'update'])) { // Init JS for icon color field on the edit form
    init_colorpicker_js();
}
