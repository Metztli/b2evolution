...
if (param('ivc_ID', 'integer', '', true)) { // Load invitation from cache:
    $InvitationCache = &get_InvitationCache();
    if (($edited_Invitation = &$InvitationCache->get_by_ID($ivc_ID, false)) === false) { // We could not find the invitation code to edit:
...
switch ($action) {
    case 'new':
        ...
        if (! isset($edited_Invitation)) { // We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record Invitation and create similar
        ...
        if ($edited_Invitation->load_from_Request()) {	// We could load data from form without errors:
            ...
            if ($duplicated_invitation_ID) {	// Display error if we have a duplicate entry:
                param_error(
                    'ivc_ID',
                    sprintf(
                        TB_('This invitation code already exists. Do you want to <a %s>edit the existing invitation code</a>?'),
            ...
            if (empty($duplicated_invitation_ID)) { // What next?
                switch ($action) {
                    case 'create_copy':
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Invitation->check_delete(sprintf(TB_('Cannot delete invitation code &laquo;%s&raquo;'), $edited_Invitation->dget('code')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
...
if (in_array($action, ['delete', 'new', 'create', 'create_new', 'create_copy', 'edit', 'update'])) { // Initialize date picker for invitation form
    init_datepicker_js();
}
