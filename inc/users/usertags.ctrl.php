...
if (param('utag_ID', 'integer', '', true)) { // Load user tag:
    $UserTagCache = &get_UserTagCache();
    if (($edited_UserTag = &$UserTagCache->get_by_ID($utag_ID, false)) === false) { // We could not find the goal to edit:
...
switch ($action) {
    ...
    case 'create':
        ...
        if ($edited_UserTag->load_from_Request()) { // We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_UserTag->check_delete(sprintf(TB_('Cannot delete tag "%s"'), '<b>' . $edited_UserTag->dget('name') . '</b>'), [], true)) { // There are restrictions:
                $action = 'list';
            }
        }
        break;
    ...
    case 'unlink':
        ...
        if ($result) {
            $Messages->add(sprintf(
                TB_('Tag "%s" has been unlinked from user "%s".'),
                '<b>' . $edited_UserTag->dget('name') . '</b>',
                '<b>' . $edited_User->dget('login') . '</b>'
            ), 'success');
        }
    ...
    case 'merge_cancel':
        ...
        if (empty($edited_UserTag) || empty($old_UserTag)) { // Wrong request, Redirect to user tags
            header_redirect($return_to ? $return_to : $admin_url . '?ctrl=usertags', 303); // Will EXIT
            // We have EXITed already at this point!!
        }
