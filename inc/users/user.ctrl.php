...
if (empty($user_tab)) {
    $user_tab = 'profile';
} elseif ($user_tab == 'social' && ! is_pro()) {	// Don't allow social accounts tabs for non-PRO:
    $user_tab = 'profile';
}
...
if ($user_profile_only) { // User has no permissions to view: he can only edit his profile
    if (isset($user_ID) && $user_ID != $current_User->ID) { // User is trying to edit something he should not: add error message (Should be prevented by UI)
        $Messages->add(TB_('You have no permission to view other users!'), 'error');
    }

    // Make sure the user only edits himself:
    $user_ID = $current_User->ID;
    if (! in_array($action, ['update', 'update_avatar', 'upload_avatar', 'remove_avatar', 'delete_avatar', 'rotate_avatar_90_left', 'rotate_avatar_180', 'rotate_avatar_90_right', 'crop', 'edit', 'default_settings', 'redemption'])) {
        $action = 'edit';
    }
}

if ($action == 'new') {	// Check permission, only admins can create new user:
...
if (! is_null($user_ID)) { // User selected
    if ($action == 'update' && $user_ID == 0) { // we create a new user
        $edited_User = new User();
        $edited_User->set_datecreated($localtimenow);
    } elseif (($edited_User = &$UserCache->get_by_ID($user_ID, false)) === false) {	// We could not find the User to edit:
        unset($edited_User);
    ...
    if ($action != 'view') { // check edit permissions
        if (! $current_User->can_moderate_user($edited_User->ID)
            && $edited_User->ID != $current_User->ID) { // user is only allowed to _view_ other user's profiles
            $Messages->add(TB_('You have no permission to edit other users!'), 'error');
            if (in_array($user_tab, ['pwdchange', 'marketing', 'admin', 'sessions', 'activity', 'social'])) {	// Don't allow the restricted pages for view:
                $user_tab = 'profile';
            }
            $action = 'view';
        } elseif ($demo_mode && ($edited_User->ID <= 7)) { // Demo mode restrictions: users created by install process cannot be edited
            $Messages->add(TB_('You cannot edit the admin and demo users profile in demo mode!'), 'error');

            if (strpos($action, 'delete_') === 0 || $action == 'promote') {   // Fallback to list/view action
...
} elseif ($action != 'new') { // user ID is not set, edit the current user
    $user_ID = $current_User->ID;
 ...
 */
if (! $Messages->has_errors()) { // no errors
    switch ($action) {
        case 'new':
            // We want to create a new user:
            if (isset($edited_User)) { // We want to use a template
                $new_User = $edited_User; // Copy !
                $new_User->set('ID', 0);
                $edited_User = &$new_User;
            } else { // We use an empty user:
                $edited_User = new User();
        ...
        case 'forbid_avatar':
            ...
            if (! $edited_User->remove_avatar(($action == 'forbid_avatar'))) { // could not remove/forbid the avatar
                $action = 'view';
                break;
            }
        ...
        case 'crop':
            ...
            $image_crop_data = empty($image_crop_data) ? [] : explode(':', $image_crop_data);
            foreach ($image_crop_data as $image_crop_value) {
                $image_crop_value = (float) $image_crop_value;
                if ($image_crop_value < 0 || $image_crop_value > 100) { // Wrong data to crop, This value is percent of real size, so restrict it from 0 and to 100
                    $action = 'view';
                    break 2;
                }
            }
            if (count($image_crop_data) < 4) { // Wrong data to crop
                $action = 'view';
                break;
            }
        ...
        case 'subscribe':
            ...
            if ($is_new_user) { // New user is created
                ...
                if (param('send_pass_email', 'integer', 0)) {	// Inform new created user by email:
                    locale_temp_switch($edited_User->get('locale'));
                    send_mail_to_User($edited_User->ID, sprintf(TB_('Your new account on %s'), $Settings->get('notification_short_name')), 'new_account_password_info', [
            ...
            } else { // The user is updated
                if (($user_tab == 'admin') && ($edited_User->ID == $current_User->ID)) { // an admin user has edited his own admin preferences
                    if (check_user_status('is_closed')) { // an admin user has changed his own status to closed, logout the user
                        logout();
                        header_redirect($baseurl, 303);
                        // will have exited
                    }
                    if ($current_User->grp_ID != 1) { // admin user has changed his own group, change user_tab for redirect
                        $user_tab = 'profile';
                    }
                }
                ...
                if (isset($current_User->previous_pass_driver) &&
                        ...
                        $current_User->previous_pass_driver != $current_User->get('pass_driver')) {	// Redirect to page as we use after email validation if current user set password first time, e-g after email capture/quick registration:
        ...
        case 'default_settings':
            ...
            if ($cur_admin_skin
                    ...
                    && ($edited_User->ID == $current_User->ID)) { // admin_skin has changed:
                $reload_page = true;
            }
            ...
            while ($loop_Plugin = &$Plugins->get_next()) {
                ...
                foreach ($pluginusersettings as $k => $l_meta) {
                    if (isset($l_meta['layout']) || ! empty($l_meta['no_edit'])) { // a layout "setting" or not for editing
                        continue;
                    }
            ...
            if ($reload_page) { // reload the current page through header redirection:
        ...
        case 'delete_all_sent_emails':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_sent_emails()) {	// The blogs were deleted successfully
        ...
        case 'delete_all_email_returns':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_email_returns()) {	// The blogs were deleted successfully
        ...
        case 'delete_all_blogs':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_blogs()) {	// The blogs were deleted successfully
        ...
        case 'delete_all_posts_created':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_posts('created')) {	// The posts were deleted successfully
        ...
        case 'delete_all_posts_edited':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_posts('edited')) {	// The posts were deleted successfully
        ...
        case 'delete_all_comments':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_comments()) {	// The posts were deleted successfully
        ...
        case 'delete_all_messages':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_messages('sent')) {	// The messages were deleted successfully
        ...
        case 'delete_all_received_messages':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_messages('received')) {	// The messages were deleted successfully
        ...
        case 'delete_all_polls':
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                if ($edited_User->delete_polls()) {	// The polls were deleted successfully
        ...
        case 'delete_all_userdata':
            ...
            if ($edited_User->ID == $current_User->ID || $edited_User->ID == 1) {	// Don't delete a logged in user
                break;
            }
            ...
            if (param('confirm', 'integer', 0)) {	// confirmed
                ...
                if ($edited_User->dbdelete($Messages)) {	// User and all his contributions were deleted successfully
        ...
        case 'delete_data':
            ...
            if (param('delete_comments', 'integer', 0)) { // Delete the comments
                ...
                if ($comments_created > 0 && $edited_User->delete_comments()) { // The comments were deleted successfully
            ...
            if (param('delete_posts', 'integer', 0)) { // Delete the posts
                $posts_created = $edited_User->get_num_posts();
                if ($posts_created > 0 && $edited_User->delete_posts('created')) { // The posts were deleted successfully
                    $result_message = ($posts_created == 1) ? TB_('1 post was deleted.') : sprintf(TB_('%s posts were deleted.'), $posts_created);
                    $Messages->add($result_message, 'success');
                }
            }

            if (param('delete_messages', 'integer', 0)) { // Delete the messages
                $messages_sent = $edited_User->get_num_messages('sent');
                if ($messages_sent > 0 && $edited_User->delete_messages()) { // The messages were deleted successfully
        ...
        case 'redemption':
            ...
            if ($EmailAddress = &$EmailAddressCache->get_by_name($edited_User->get('email'), false, false) &&
                in_array($EmailAddress->get('status'), ['warning', 'suspicious1', 'suspicious2', 'suspicious3', 'prmerror'])) { // Change to 'redemption' status only if status is 'warning', 'suspicious1', 'suspicious2', 'suspicious3' or 'prmerror'
        ...
        case 'add_automation':
            ...
            if ($user_Automation && $user_Automation->add_users($edited_User->ID, [
                'users_no_subs' => 'add',
        ...
        case 'remove_automation':
            ...
            if ($r) {	// Display message if user has been removed from selected automation really:
...
if ($display_mode != 'js') {
    ...
    if ($action == 'new') {
        ...
        $AdminUI->breadcrumbpath_add($edited_User->get_colored_login([
            'login_text' => 'name',
        ]), '?ctrl=user&amp;user_ID=' . $edited_User->ID);
    }
    ...
    switch ($user_tab) {
        ...
        default:
            ...
            modules_call_method('init_backoffice_UI', [
                'ctrl' => 'user',
                'action' => $action,
                'tab' => $user_tab,
    ...
    default:
        ...
        switch ($user_tab) {
            case 'profile':
                ...
                if ($edited_User->has_avatar()) { // Init JS for form to crop pictures of user
            ...
            case 'marketing':
                ...
                if ($display_mode != 'js') { // Init JS for form to add user to automation:
            ...
            case 'admin':
                ...
                if ($display_mode != 'js') { // Init JS for form to delete the posts, the comments and the messages of user
            ...
            case 'activity':
                ...
                if (in_array($action, ['delete_all_sent_emails', 'delete_all_email_returns', 'delete_all_blogs', 'delete_all_posts_created', 'delete_all_posts_edited', 'delete_all_comments', 'delete_all_messages', 'delete_all_received_messages', 'delete_all_polls', 'delete_all_userdata'])) {	// We need to ask for confirmation before delete:
                    ...
                    switch ($action) {
                        case 'delete_all_sent_emails':
                            ...
                            if ($sent_emails_count > 0) {	// Display a confirm message if current user can delete at least one email log of the edited user:
                        ...
                        case 'delete_all_email_returns':
                            ...
                            if ($email_returns_count > 0) {	// Display a confirm message if current user can delete at least one email returns of the edited user:
                        ...
                        case 'delete_all_blogs':
                            ...
                            if ($deleted_blogs_count > 0) {	// Display a confirm message if current user can delete at least one blog of the edited user
                        ...
                        case 'delete_all_posts_created':
                            ...
                            if ($deleted_posts_created_count > 0) {	// Display a confirm message if current user can delete at least one post created by the edited user
                        ...
                        case 'delete_all_posts_edited':
                            ...
                            if ($deleted_posts_edited_count > 0) {	// Display a confirm message if current user can delete at least one post created by the edited user
                                $confirm_message = sprintf(TB_('Delete %d posts edited by the user?'), $deleted_posts_edited_count);
                            }
                            break;

                        case 'delete_all_comments':
                            if ($edited_User->has_comment_to_delete()) { // Display a confirm message if current user can delete at least one comment posted by the edited user
                        ...
                        case 'delete_all_messages':
                            ...
                            if ($messages_count > 0 && check_user_perm('perm_messaging', 'abuse')) {	// Display a confirm message if current user can delete the messages sent by the edited user
                        ...
                        case 'delete_all_received_messages':
                            ...
                            if ($messages_count > 0 && check_user_perm('perm_messaging', 'abuse')) {	// Display a confirm message if curent user can delete the messages sent by the edited user
                        ...
                        case 'delete_all_polls':
                            ...
                            if ($polls_count > 0) {	// Display a confirm message if current user can delete the polls owned by the edited user
                                $confirm_message = sprintf(TB_('Delete %d polls owned by the user?'), $polls_count);
                            }
                            break;

                        case 'delete_all_userdata':
                            if ($current_User->ID != $edited_User->ID && $edited_User->ID != 1) {	// User can NOT delete admin and own account:
                                $confirm_messages = [];
                                $sent_emails_count = $edited_User->get_num_sent_emails();
                                if ($sent_emails_count > 0 && check_user_perm('emails', 'edit')) {	// Display a confirm message if current user can delete at least one email sent log of the edited user:
                                    $confirm_messages[] = [sprintf(TB_('%d emails sent to the user'), $sent_emails_count), 'warning'];
                                }
                                $email_returns_count = $edited_User->get_num_email_returns();
                                if ($email_returns_count > 0 && check_user_perm('emails', 'edit')) {	// Display a confirm message if current user can delete at least one email return of the edited user:
                                    $confirm_messages[] = [sprintf(TB_('%d email returns from the user\'s email address'), $email_returns_count), 'warning'];
                                }
                                $deleted_blogs_count = count($edited_User->get_deleted_blogs());
                                if ($deleted_blogs_count > 0) {	// Display a confirm message if current user can delete at least one blog of the edited user:
                                    $confirm_messages[] = [sprintf(TB_('%d collections of the user'), $deleted_blogs_count), 'warning'];
                                }
                                $deleted_posts_created_count = count($edited_User->get_deleted_posts('created'));
                                if ($deleted_posts_created_count > 0) {	// Display a confirm message if current user can delete at least one post created by the edited user:
                                    $confirm_messages[] = [sprintf(TB_('%d posts created by the user'), $deleted_posts_created_count), 'warning'];
                                }
                                $deleted_posts_edited_count = count($edited_User->get_deleted_posts('edited'));
                                if ($deleted_posts_edited_count > 0) {	// Display a confirm message if current user can delete at least one post created by the edited user:
                                    $confirm_messages[] = [sprintf(TB_('%d posts edited by the user'), $deleted_posts_edited_count), 'warning'];
                                }
                                if ($edited_User->has_comment_to_delete()) {	// Display a confirm message if current user can delete at least one comment posted by the edited user:
                                    $confirm_messages[] = [sprintf(TB_('%s comments posted by the user'), $edited_User->get_num_comments('', true)), 'warning'];
                                }
                                $messages_count = $edited_User->get_num_messages();
                                if ($messages_count > 0 && check_user_perm('perm_messaging', 'abuse')) {	// Display a confirm message if current user can delete the messages sent by the edited user
                                    $confirm_messages[] = [sprintf(TB_('%d private messages sent by the user'), $messages_count), 'warning'];
                                }
                                ...
                                if ($message_same_email_users !== false) {
                                    $confirm_messages[] = [$message_same_email_users, 'note'];
                                }
                    ...
                    if (! empty($confirm_message)) {	// Displays form to confirm deletion
            ...
            default:
                // Display back-office UI for modules:
                modules_call_method('display_backoffice_UI', [
                    'ctrl' => 'user',
                    'action' => $action,
                    'tab' => $user_tab,
                ]);
