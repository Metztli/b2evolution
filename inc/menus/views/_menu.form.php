...
$Form->text_input('menu_name', $edited_SiteMenu->get('name'), 50, TB_('Name'), '', [
    'maxlength' => 128,
    'required' => true,
]);
...
$Form->select_input_array('menu_translates_menu_ID', $edited_SiteMenu->get('translates_menu_ID'), $parent_menu_options, TB_('Translation of'), null, [
...
if ($edited_SiteMenu->ID == 0) {	// Suggest menu entries based on existing collections:
    $SectionCache = &get_SectionCache();
    $SectionCache->load_all();
    $BlogCache = &get_BlogCache();
    $suggested_menu_entries = [];
    foreach ($SectionCache->cache as $Section) {
        if ($Section->ID == 1) {	// Skip "No Section" in order to add it at the end:
            continue;
        }
        $suggested_menu_entries[] = ['menu_entries[sec_' . $Section->ID . ']', $Section->get('name'), $Section->get('name'), 1];
        $BlogCache->clear();
        $BlogCache->load_where('blog_sec_ID = ' . $Section->ID);
        foreach ($BlogCache->cache as $section_Blog) {
            $suggested_menu_entries[] = ['menu_entries[coll_' . $section_Blog->ID . '_' . $Section->ID . ']', $section_Blog->get('shortname'), $section_Blog->get('shortname'), 1, null, null, null, null, [
    ...
    foreach ($BlogCache->cache as $section_Blog) {
        $suggested_menu_entries[] = ['menu_entries[coll_' . $section_Blog->ID . ']', $section_Blog->get('shortname'), $section_Blog->get('shortname'), 1];
    }
    // Contact menu entry:
    $suggested_menu_entries[] = ['menu_entries[#contact#]', '#contact#', TB_('Contact'), 1];
...
if (check_user_perm('options', 'edit')) {	// Allow to save menu if current User has a permission:
    if ($action == 'copy') {
        $buttons[] = ['submit', 'submit', sprintf(TB_('Save and duplicate all settings from %s'), $edited_SiteMenu->get('name')), 'SaveButton'];
    } else {
...
if ($edited_SiteMenu->ID > 0 && $action != 'copy') {	// Display menu entries:
    ...
    function site_menu_entry_line($SiteMenuEntry, $level)
    {
        ...
        // Name:
        if (check_user_perm('options', 'edit')) {	// We have permission permission to edit:
        ...
        // Destination:
        $destination = '';
        if ($SiteMenuEntry->get('type') == 'url') {	// Destination to any URL:
            $destination = get_link_tag($SiteMenuEntry->get_url());
        } elseif ($SiteMenuEntry->get_url()) {	// Destination to other pages:
        ...
        if (check_user_perm('options', 'edit')) {	// We have permission permission to edit, so display action column:
    ...
    $Table->global_icon(TB_('New menu entry'), 'new', regenerate_url('action,blog', 'action=new_entry'), TB_('New menu entry') . ' &raquo;', 3, 4, [
    ...
    if (check_user_perm('options', 'edit')) {	// We have permission to edit, so display action column:
        $Table->cols[] = [
            'th' => TB_('Actions'),
            'th_class' => 'shrinkwrap',
        ];
    ...
    echo $SiteMenuEntryCache->recurse($callbacks, $edited_SiteMenu->ID, null, 0, 0, [
        'sorted' => true,
    ]);
