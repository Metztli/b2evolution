...
if (param('menu_ID', 'integer', '', true)) {	// Load menu:
    $SiteMenuCache = &get_SiteMenuCache();
    if (($edited_SiteMenu = &$SiteMenuCache->get_by_ID($menu_ID, false)) === false) {	// We could not find the goal to edit:
...
if (param('ment_ID', 'integer', '', true)) {	// Load menu:
    $SiteMenuEntryCache = &get_SiteMenuEntryCache();
    if (($edited_SiteMenuEntry = &$SiteMenuEntryCache->get_by_ID($ment_ID, false)) === false) {	// We could not find the goal to edit:
...
switch ($action) {
    ...
    case 'create':
        ...
        if ($edited_SiteMenu->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_SiteMenu->check_delete(sprintf(TB_('Cannot delete menu &laquo;%s&raquo;'), $edited_SiteMenu->dget('name')), [], true)) {	// There are restrictions:
                $action = 'list';
            }
        }
        break;
    ...
    case 'create_entry':
        ...
        // Load data from request:
        if ($edited_SiteMenuEntry->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete_entry':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_SiteMenuEntry->check_delete(sprintf(TB_('Cannot delete menu &laquo;%s&raquo;'), $edited_SiteMenuEntry->dget('text')), [], true)) {	// There are restrictions:
                $action = 'list';
            }
        }
        break;
...
switch ($action) {
    ...
    case 'delete_entry':
        ...
        $edited_SiteMenuEntry->confirm_delete(
            ...
            'menuentry',
            $action,
            array_merge(get_memorized('action,locale,blog,mode'), [
                'action' => 'edit',
            ])
        );
        // no break
