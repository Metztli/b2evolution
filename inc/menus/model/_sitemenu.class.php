...
class SiteMenu extends DataObject
{
     ...
     */
    public $count_translation_menus = null;

    /**
     * @var array Localized menus
     */
    public $localized_menus = null;
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_menus__menu',
                'fk' => 'menu_translates_menu_ID',
                'msg' => T_('%d child menus'),
            ],
            [
    ...
    public function load_from_Request()
    {
        ...
        if (isset($menu_translates_menu_ID) && $this->has_translations()) {	// Display error message if we want make the meta category from category with posts
            global $Messages;
    ...
    public function dbinsert()
    {
        ...
        if ($r = parent::dbinsert()) {	// If Menu has been inserted successfully:
            if (! empty($this->insert_menu_entries)) {
                ...
                foreach ($this->insert_menu_entries as $menu_entry_key => $menu_entry_text) {
                    ...
                    if ($menu_entry_key == '#contact#') {	// Special "Contact" entry:
                    ...
                    } elseif (preg_match('/^([a-z]+)_(\d+)(_(\d+))?$/', $menu_entry_key, $m)) {	// Section or Collection entry:
                        switch ($m[1]) {
                            case 'sec':
                                // Section entry:
    ...
    public function duplicate()
    {
        ...
        $menu_entry_fields = ['ment_ID', 'ment_menu_ID', 'ment_parent_ID', 'ment_order', 'ment_user_pic_size', 'ment_text', 'ment_type',
    ...
    {
        if ($this->entries !== null) {	// Use already loaded menu entries:
            return $this->entries;
        }
     ...
     * @return integer
     */
    public function get_max_order($parent_ID = null)
    {
        if (empty($this->ID)) {
            return 0;
        }
     ...
     */
    public function get_localized_menus($locale)
    {
        global $DB;
    ...
    public function has_translations()
    {
        ...
        if ($this->ID == 0) {	// New menu has no translation menus:
            return false;
        }
