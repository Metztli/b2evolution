...
class SiteMenuEntryCache extends DataObjectCache
{
     ...
     */
    public function add($SiteMenuEntry)
    {
        if (parent::add($SiteMenuEntry)) {	// Successfully added
     ...
     */
    public function get_menu_entry_path($menu_ID, $menu_entry_ID)
    {
        $this->load_subset($menu_ID);

        $menu_entry_path = [$menu_entry_ID];
     ...
     * @param array
     */
    public function recurse_move_subtree(&$SiteMenuEntry, &$list_array)
    {
     ...
     */
    public function reveal_children($subset_ID = null, $sort_root_entries = false)
    {
        global $Debuglog;
        global $Timer;
        ...
        if (empty($this->subset_property)) {	// We are not handling a subset but everything instead:
            ...
            // Reveal children:
            if (! empty($this->cache)) {	// There are loaded entries, so loop on all loaded entries to set their children list if it has:
                foreach ($this->cache as $ment_ID => $SiteMenuEntry) {
                    if (! is_null($SiteMenuEntry->parent_ID)) {	// This Site Menu Entry has a parent, so add it to its parent children list:
                        $this->cache[$SiteMenuEntry->parent_ID]->add_child_entry($this->cache[$ment_ID]);
                    } else {	// This Site Menu Entry has no parent, so add it to the parent entries list
                        $this->root_entries[] = &$this->cache[$ment_ID];
                    }
                }
            }

            if ($sort_root_entries) { // Sort root entries as it was requested
                $this->sort_root_entries(null);
            }

            $this->revealed_all_children = true;
        } else {	// We are handling a subset (for example Chapers of a specific menu_ID only):
            if (is_null($subset_ID)) {	// No specific subset requested, we are going to reveal all subsets in a row:
                ...
                // Reveal children:
                if (! empty($this->subset_cache[$subset_ID])) {	// There are loaded entries
                    ...
                    if ($sort_root_entries) { // Sort subset root entries as it was requested
                        $this->sort_root_entries($subset_ID);
                    }
     ...
     */
    public function check_path_to_root($subset_ID, $ment_ID, $path_array = [])
    {
        global $Debuglog;
        ...
        $SiteMenuEntry = &$this->subset_cache[$subset_ID][$ment_ID];
        if (is_null($SiteMenuEntry->parent_ID)) {	// Found root parent
        ...
        if (in_array($SiteMenuEntry->parent_ID, $path_array)) {	// We are about to enter an infinite loop
     ...
     */
    public function sort_root_entries($subset_ID = null)
    {
        if ($subset_ID == null && empty($this->subset_property)) {
            if (! $this->sorted_flags['root_entries']) { // Root entries were not sorted yet
                usort($this->root_entries, [$this, 'compare_site_menu_entries']);
        ...
        if ($subset_ID == null) {
            foreach (array_keys($this->subset_root_entries) as $key) {
                ...
                usort($this->subset_root_entries[$key], [$this, 'compare_site_menu_entries']);
        ...
        if (empty($this->sorted_flags[$subset_ID])) { // This subset was not sorted yet
            usort($this->subset_root_entries[$subset_ID], [$this, 'compare_site_menu_entries']);
     ...
     */
    public function recurse($callbacks, $subset_ID = null, $entries_array = null, $level = 0, $max_level = 0, $params = [])
    {
        $params = array_merge([
        ...
        if (is_null($entries_array)) {	// Get all parent entries:
            if (is_null($subset_ID)) {
                $entries_array = $this->root_entries;
            } elseif (isset($this->subset_root_entries[$subset_ID])) { // We have root entries for the requested subset:
        ...
        foreach ($entries_array as $SiteMenuEntry) {
            ...
            if ($params['is_opened'] || ($max_level > $level + 1)) {	// Iterate through sub entries recursively:
                $params['level'] = $level;
     ...
     */
    public function iterate_through_site_menu_entry_children($SiteMenuEntry, $callbacks, $recurse = false, $params = [])
    {
        $r = "";
        ...
        if ($has_sub_entries) {	// Display Site Menu Entry:
            $entry_index = 0;
            $subentries_to_display = [];
            ...
            if (! empty($subentries_to_display)) { // Display all subentries which were not displayed yet
                if ($recurse) {
                    $r .= $this->recurse($callbacks, $params['subset_ID'], $subentries_to_display, $params['level'] + 1, $params['max_level'], $params);
                } else { // Display each Site Menu Entry without recursion
                    foreach ($subentries_to_display as $sub_SiteMenuEntry) { // Display each Site Menu Entry:
                        if (is_array($callbacks['line'])) { // object callback:
    ...
    public function recurse_select(
        ...
        if (is_null($entries_array)) {	// Get all parent menu entries:
            $entries_array = $this->root_entries;
        }

        // Sort entries alphabetically or manually depending on settings:
        usort($entries_array, [$this, 'compare_site_menu_entries']);
        ...
        foreach ($entries_array as $SiteMenuEntry) {
            if (in_array($SiteMenuEntry->ID, $exclude_array)) {	// We want to exclude that Site Menu Entry:
                continue;
            }
            ...
            if ($SiteMenuEntry->ID == $selected) {
                $r .= ' selected="selected"';
            }
            $r .= ' >' . $indent . $SiteMenuEntry->text . '</option>';

            if (! empty($SiteMenuEntry->children)) {	// Add children entries:
     ...
     */
    public static function compare_site_menu_entries($a_SiteMenuEntry, $b_SiteMenuEntry)
    {
        if ($a_SiteMenuEntry == null || $b_SiteMenuEntry == null) {
            debug_die('Invalid Site Menu Entry objects received to compare.');
        }

        if ($a_SiteMenuEntry->ID == $b_SiteMenuEntry->ID) { // The two Site Menu Entries are the same
            return 0;
        }

        if ($a_SiteMenuEntry->parent_ID != $b_SiteMenuEntry->parent_ID) { // Two Site Menu Entries from the same Site Menu, but different parrents
        ...
        if ($a_SiteMenuEntry->order === null) { // NULL values are greater than any number
            $result = ($b_SiteMenuEntry->order !== null) ? 1 : 0;
        } elseif ($b_SiteMenuEntry->order === null) { // NULL values are greater than any number, so a is lower than b
            $result = -1;
        } else {
     ...
     */
    public function get_entries($menu_ID, $parent_ID = 0, $sorted = false)
    {
        ...
        if ($sorted) {	// Sort Site Menu Entry:
