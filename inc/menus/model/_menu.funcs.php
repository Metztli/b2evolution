 ...
 * @return array
 */
function get_site_menu_types($additional_types = [])
{
...
function get_default_site_menu_ID($menu_name)
{
    ...
    if (! $DB->get_var('SHOW TABLES LIKE "T_menus__menu"')) {	// The Menus tables still doesn't exist, probably this is a call from old upgrade version:
        return false;
    }

    $SiteMenuCache = &get_SiteMenuCache();
    if ($SiteMenu = &$SiteMenuCache->get_by_name($menu_name, false, false)) {	// Use existing Menu:
        return $SiteMenu->ID;
    }
    ...
    if (isset($menu_entries)) {	// Create default/demo menu entries:
        $menu_entry_order = 10;
