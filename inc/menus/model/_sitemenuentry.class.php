...
class SiteMenuEntry extends DataObject
{
    ...
    public $user_pic_size;

    public $text;

    public $type;

    public $coll_logo_size;
    ...
    public function __construct($db_row = null)
    {
        ...
        if ($db_row != null) {	// Get menu entry data from DB:
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_menus__entry',
                'fk' => 'ment_menu_ID',
                'msg' => T_('%d menu entries'),
            ],
        ];
    }
    ...
    public function load_from_Request()
    {
        ...
        if ($item_ID) {
            ...
            } elseif (in_array($menu_Item_from_ID->get_type_setting('usage'), ['special', 'content-block'])) {
        ...
        if ($item_slug) {
            ...
            } elseif (in_array($menu_Item_from_slug->get_type_setting('usage'), ['special', 'content-block'])) {
     ...
     */
    public function add_child_entry(&$SiteMenuEntry)
    {
        if (! isset($this->children[$SiteMenuEntry->ID])) {	// Add only if it was not added yet:
    ...
    {
        if ($this->children_sorted) { // Site Menu Entry children list is already sorted
            return;
        }

        // Sort children list
        uasort($this->children, ['SiteMenuEntryCache', 'compare_site_menu_entries']);
    }
    ...
    {
        if ($this->Blog === null) {	// Load collection once:
            $BlogCache = &get_BlogCache();
            $this->Blog = &$BlogCache->get_by_ID($this->get('coll_ID'), false, false);

            if (empty($this->Blog)) {	// Use current Collection if it is not defined or it doesn't exist in DB:
                global $Blog;
                if (isset($Blog)) {
                    $this->Blog = &$Blog;
                }
    ...
    {
        if ($this->Chapter === null) {	// Load collection once:
    ...
    {
        if ($this->Item === null) {	// Load collection once:
            $ItemCache = &get_ItemCache();
            if ($this->get('item_ID')) {	// Item ID has priority because it is faster to resolve:
                $this->Item = &$ItemCache->get_by_ID($this->get('item_ID'), false, false);
            } elseif ($this->get('item_slug')) {	// Use item slug only if there is no Item ID specified:
     ...
     * @return string
     */
    public function get_text($force_default = false)
    {
        global $thumbnail_sizes, $current_User;

        $entry_Blog = &$this->get_Blog();

        if (! $force_default && $this->get('text') != '') {	// Use custom text:
            $text = $this->get('text');
            ...
            switch ($this->get('type')) {
                ...
                case 'recentposts':
                    if ($entry_Chapter = &$this->get_Chapter()) {	// Use category name instead of default if the defined category is found in DB:
                ...
                case 'item':
                    ...
                    if ($entry_Item && $entry_Item->can_be_displayed()) {	// Item is not found or it cannot be displayed for current user on front-office:
                ...
                case 'postnew':
                    if (($entry_Chapter = &$this->get_Chapter()) &&
                        ($cat_ItemType = &$entry_Chapter->get_ItemType(true))) {	// Use text depending on default category's Item Type:
        ...
        // Replace masks:
        $text = preg_replace_callback('#\$([a-z]+)\$#', [$this, 'callback_text_mask'], $text);

        // Profile picture before text:
        if (in_array($this->get('type'), ['logout', 'myprofile', 'visits', 'profile', 'avatar', 'useritems', 'usercomments']) &&
            is_logged_in() &&
        ...
        if (! in_array($this->get('type'), ['item', 'admin', 'url', 'text']) &&
        ...
        if ($this->get('show_badge')) {
            switch ($this->get('type')) {
                ...
                case 'flagged':
                    ...
                    if ($flagged_items_count > 0) {	// If at least one flagged item:
     ...
     */
    public function callback_text_mask($m)
    {
        global $current_User;
        ...
        switch ($m[0]) {
            case '$username$':
                return is_logged_in()
                    ? $current_User->get_colored_login([
                        'login_text' => 'name',
                    ])
                    : '(' . T_('anonymous') . ')';
    ...
    public function get_url()
    {
        ...
        if (empty($entry_Blog)) {	// We cannot use this menu entry without current collection:
            $this->url_error = 'No Collection';
            return false;
        }

        if ($this->get('visibility') == 'access' && ! $entry_Blog->has_access()) {	// Don't use this menu entry because current user has no access to the collection:
        ...
        switch ($this->get('type')) {
            ...
            case 'recentposts':
                if (! $entry_Blog->get_setting('postlist_enable')) {	// This page is disabled:
                    $this->url_error = 'Disabled';
                    return false;
                }
                if ($entry_Chapter = &$this->get_Chapter()) {	// Use category url instead of default if the defined category is found in DB:
                    return $entry_Chapter->get_permanent_url();
                }
                return $entry_Blog->get('recentpostsurl');

            case 'search':
                if (! $entry_Blog->get_setting('search_enable')) {	// This page is disabled:
                    $this->url_error = 'Disabled';
                    return false;
                }
            ...
            case 'latestcomments':
                if (! $entry_Blog->get_setting('comments_latest')) {	// This page is disabled:
                    $this->url_error = 'Disabled';
                    return false;
                }
            ...
            case 'owneruserinfo':
                return $entry_Blog->get('userurl', [
                    'user_ID' => $entry_Blog->owner_user_ID,
                ]);
            ...
            case 'login':
                if (is_logged_in()) {	// Don't display this link for already logged in users:
            ...
            case 'visits':
                ...
                if (! is_logged_in() || ! $Settings->get('enable_visit_tracking')) {	// Current user must be logged in and visit tracking must be enabled:
                    return false;
                }
            ...
            case 'useritems':
                if (! is_logged_in()) {	// Don't allow anonymous users to see items list:
                    return false;
                }
                return url_add_param($entry_Blog->gen_blogurl(), 'disp=useritems');

            case 'usercomments':
                if (! is_logged_in()) {	// Don't allow anonymous users to see comments list:
                    return false;
                }
            ...
            case 'users':
                ...
                if (! is_logged_in() && ! $Settings->get('allow_anonymous_user_list')) {	// Don't allow anonymous users to see users list:
                    return false;
                }
            ...
            case 'item':
                ...
                if (! empty($this->item_ID)) {	// Item ID is specified and has priority:
                    return $entry_Item->get_permanent_url();
                } else {	// No Item ID specified, use item slug:
            ...
            case 'postnew':
                if (! check_item_perm_create($entry_Blog)) {	// Don't allow users to create a new post:
                    return false;
                }
                $url = url_add_param($entry_Blog->get('url'), 'disp=edit');
                if ($entry_Chapter = &$this->get_Chapter()) {	// Append category ID to the URL:
                    $url = url_add_param($url, 'cat=' . $entry_Chapter->ID);
                    $cat_ItemType = &$entry_Chapter->get_ItemType(true);
                    if ($cat_ItemType === false) {	// Don't allow to create a post in this category because this category has no default Item Type:
                        return false;
                    }
                    if ($cat_ItemType) {	// Append item type ID to the URL:
            ...
            case 'myprofile':
                if (! is_logged_in()) {	// Don't show this link for not logged in users:
                    return false;
                }
                global $current_User;
                return $entry_Blog->get('userurl', [
            ...
            case 'admin':
                if (! check_user_perm('admin', 'restricted') && check_user_status('can_access_admin')) {	// Don't allow admin url for users who have no access to backoffice:
                    return false;
                }
                global $admin_url;
                return $admin_url;
            ...
            case 'contacts':
                switch ($this->get('access')) {
                    case 'loggedin':
                        ...
                        break;
                    case 'perms':
                        if (! check_user_perm('perm_messaging', 'reply', false)) {	// User has no access for messaging:
                            $this->url_error = 'No access';
                            return false;
                        }
                        break;
            ...
            case 'flagged':
                if (! is_logged_in()) {	// Only logged in user can flag items:
                    $this->url_error = 'Not logged in';
                    return false;
                }
                global $current_User;
                if ($this->get('hide_empty') && $current_User->get_flagged_items_count() == 0) {	// Hide this menu if current user has no flagged posts yet:
    ...
    public function is_active()
    {
        ...
        if (! $this->get('highlight')) {	// Don't highlight this menu entry:
            return false;
        }
        ...
        if ($current_blog_ID != $entry_Blog->ID) {	// This is a different collection than defined in this Menu Entry:
            return false;
        }
        ...
        switch ($this->get('type')) {
            ...
            case 'profile':
                return in_array($disp, ['profile', 'avatar', 'pwdchange', 'userprefs', 'subs']);
