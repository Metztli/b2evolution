...
function b2evonet_get_updates($force_short_delay = false)
{
    ...
    if (! isset($allow_evo_stats)) {	// Set default value:
        $allow_evo_stats = true; // allow (non-anonymous) stats
    }
    if ($allow_evo_stats === false) { // Get outta here:
        return null;
    }

    if ($debug < 2 && $Settings->get('evonet_last_error') > $servertimenow - $evonetsrv_retry_delay * 60) {	// The previous error was less than 90 minutes ago, skip this:
    ...
    if ($last_version_checked == $version_id) {	// Current version has already been checked, don't check too often:
        if ($servertime_last_update > $servertimenow - $update_every) {	// The previous update was less than 12 hours ago, skip this
            // echo 'recent update';
            return false;
        }

        if ($servertime_last_attempt > $servertimenow - $attempt_every) {	// The previous update attempt was less than 4 hours ago, skip this
 ...
 */
function show_comments_awaiting_moderation($blog_ID, $CommentList = null, $limit = 5, $comment_IDs = [], $script = true)
{
    global $current_User;

    if (is_null($CommentList)) { // Inititalize CommentList
        $BlogCache = &get_BlogCache();
        $Collection = $Blog = &$BlogCache->get_by_ID($blog_ID, false, false);
    ...
    while ($Comment = &$CommentList->get_next()) { // Loop through comments:
        ...
        if (! $is_meta) {	// Display status banner only for normal comments:
        ...
        if (! $is_meta && ($Comment->status !== 'draft' || $Comment->author_user_ID == $current_User->ID)) { // Display Comment permalink icon
        ...
        if ($user_permission) { // Put the internal comment content into this container to edit by ajax:
            echo '<div id="editable_comment_' . $Comment->ID . '" class="editable_comment_content">';
        }
        $Comment->content('htmlbody', true);
        if ($user_permission) { // End of the container that is used to edit internal comment by ajax:
            echo '</div>';
        }
        ...
        $redirect_to = null;
        if (! $script) { // Set page, where to redirect, because the function is called from async.php (regenerate_url gives => async.php)
            global $admin_url;
        ...
        if (! $is_meta) {	// Display Spam Voting system only for normal comment:
            $Comment->vote_spam('', '', '&amp;', true, true, [
                'button_group_class' => button_class('group') . ' btn-group-sm',
            ]);
        }
...
function get_table_count($table_name, $sql_where = '', $sql_from = '', $sql_title = '')
{
    ...
    if (empty($sql_title)) {	// Set default SQL title:
    ...
    $SQL->SELECT('COUNT( * )');
    $SQL->FROM($table_name);
    if (! empty($sql_from)) { // Additional sql for FROM clause
        $SQL->FROM_add($sql_from);
    }
    if (! empty($sql_where)) { // Additional sql for WHERE clause
        $SQL->WHERE($sql_where);
    }
...
function display_posts_awaiting_moderation($status, &$block_item_Widget)
{
    ...
    if (! $ItemList->result_num_rows) { // We don't have posts awaiting moderation with the given status
        return false;
    }
    ...
    while ($Item = &$ItemList->get_item()) {
        ...
        if (($Item->status !== 'draft') || ($Item->creator_user_ID == $current_User->ID)) { // Display Item permalink icon
...
{
    if ($max === null) {	// Use log10 by default where max value is 100000:
        $percent = log10($value) * 2 * 10;
    } else {	// Use log with custom max value:
...
function display_charts($chart_data)
{
    ...
    foreach ($chart_data as $chart_item) {
        if ($chart_item['type'] == 'number') { // Calculate a percent with logarithm functions:
        ...
        } else { // Calculate a real percent
            $chart_percent = empty($chart_item['100%']) ? 0 : floor(intval($chart_item['value']) / $chart_item['100%']) * 100;
            $chart_item['value'] = $chart_percent . '%';
            $chart_color = '#00F';
        }
        if ($chart_item['value'] > 0 && $chart_percent == 0) { // Display a little chart for not null values
            $chart_percent = 0.01;
        }

        // Display chart
        if ($ctrl == 'coll_settings') { // in collection dashboard
...
{
    if ($percent < 50) { // First 50 percents
        $color_to = $color_middle;
        $percent *= 2;
    } else { // Last 50 percents
