...
if (! empty($ip_address)) { // Filter by IP address
    if (is_valid_ip_format($ip_address)) {	// If full IP address is entered:
        $start_ip_address = ip2int($ip_address);
        $end_ip_address = $start_ip_address;
    } elseif (preg_match('/^\d{1,3}(\.\d{1,3}){0,2}\.?$/', $ip_address, $part_ip_address)) {	// Part of IP address is entered like "127.0", we should find "127.0.*.*":
    ...
    if (isset($start_ip_address, $end_ip_address)) {	// Filter only with correct entered IP address:
...
$Results->cols[] = [
    ...
    'order' => 'aipr_status',
    'extra' => [
        'style' => 'background-color: %aipr_status_color( "#aipr_status#" )%;',
...
if (check_user_perm('spamblacklist', 'edit')) { // Check permission to edit IP ranges:
    /**
