...
$Form->checkbox(
    'antispam_block_contact_form',
    $Settings->get('antispam_block_contact_form'),
    TB_('Block spam on contact form'),
    TB_('Check to use the antispam blacklist on contact form submissions')
);
$Form->checkbox(
    'antispam_block_spam_referers',
    $Settings->get('antispam_block_spam_referers'),
    TB_('Block spam referers'),
    TB_('If a referrer has been detected as spam, should we block the request with a "403 Forbidden" page?')
);
$Form->checkbox(
    'antispam_report_to_central',
    $Settings->get('antispam_report_to_central'),
    TB_('Report to central blacklist'),
    TB_('When banning a keyword, offer an option to report to the central blacklist.')
...
$karma_plugins = $Plugins->get_list_by_events(['GetSpamKarmaForComment']);
...
$trust_groups = $Settings->get('antispam_trust_groups') != '' ? explode(',', $Settings->get('antispam_trust_groups')) : [];
$groups_options = [];
$groups = $GroupCache->get_option_array();
foreach ($groups as $group_ID => $group_name) {
    $groups_options[] = ['antispam_trust_groups[]', $group_ID, $group_name, in_array($group_ID, $trust_groups)];
}
