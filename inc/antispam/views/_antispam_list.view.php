...
if (get_param('display_mode') == 'js') {	// This is an Ajax response
...
if (check_user_perm('spamblacklist', 'edit')) { // TODO: check for 'add' here once it's mature.
     ...
     */
    $Form->end_form([['submit', 'submit', T_('Check & ban...'), 'SaveButton']]);
}
...
if (! empty($keywords)) {	// Filter by keywords:
    $SQL->add_search_field('askw_string');
    $SQL->WHERE_kw_search($keywords, 'AND');
}

if (! empty($source)) {	// Filter by source:
...
if (check_user_perm('spamblacklist', 'edit')) {	// Allow to request keywords from Central Antispam if current user has a permission:
    global $admin_url;
    $Results->global_icon(T_('Request update from Central Antispam Blacklist'), '', $admin_url . '?ctrl=antispam&amp;action=poll&amp;' . url_crumb('antispam'), T_('Request update from Central Antispam Blacklist'), 0, 0, [
...
function filter_antispam(&$Form)
{
    ...
    $Form->select_input_array('source', get_param('source'), [
        '' => T_('All'),
        'local' => T_('Local'),
...
if (check_user_perm('spamblacklist', 'edit')) { // User can edit, spamlist: add controls to output columns:
