...
if ($action == 'iprange_edit') {	// Memorize action for users and sessions list below edit form:
    memorize_param('action', 'string', '', $action);
} else {	// Don't initialize this param as array because it must be string for UserList:
...
if (param('iprange_ID', 'integer', '', true)) {	// Load IP Range object
    load_class('antispam/model/_iprange.class.php', 'IPRange');
    $IPRangeCache = &get_IPRangeCache();
    if (($edited_IPRange = &$IPRangeCache->get_by_ID($iprange_ID, false)) === false) {	// We could not find the goal to edit:
...
switch ($action) {
    case 'ban': // only an action if further "actions" given
        ...
        $all_statuses = get_visibility_statuses('keys', ['trash', 'redirected']);
        $delstatuses = [];
        foreach ($all_statuses as $status) { // collect which comments should be delteded
            if (param('del' . $status, 'integer', 0)) { // matching comments with this status should be deleted
                $delstatuses[] = $status;
            }
        }
        ...
        if (empty($confirm)) { // No confirmed action, Execute the ban actions only after confirmation
            break;
        }

        if ($delhits) { // Delete all banned hit-log entries
            $r = $DB->query(
        ...
        if ($delcomments) { // select banned comments
        ...
        if ($blacklist_locally) { // Local blacklist:
            if (antispam_create($keyword)) { // Success
        ...
        if ($report) { // Report this keyword as abuse remotely:
            antispam_report_abuse($keyword);
        }

        if (! $blacklist_locally && ! $report && ! $delhits && ! $delcomments) { // If no action has been selected
        ...
        if ($display_mode == 'js' && ! $Messages->has_errors()) {	// Initialize JS functions to execute after action from modal window:
            $javascript_messages = [];
            if ($delcomments && $r) { // $r not null => means the commentlist was deleted successfully
                $javascript_messages['refreshAfterBan'] = [$deleted_ids];
            }
            $javascript_messages['closeModalAfterBan'] = [];
        }
    ...
    case 'update':
        ...
        param('antispam_plugin_spam_weight', 'array:integer', []);
        ...
        if ($changed_weight) { // Reload plugins table (for display):
        ...
        // Trust groups
        $trust_groups = param('antispam_trust_groups', 'array:integer', []);
    ...
    case 'iprange_create':
        ...
        if ($edited_IPRange->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'bankruptcy_delete':
        ...
        $bankruptcy_blogs_IDs = param('bankruptcy_blogs', 'array:integer', []);

        if (empty($bankruptcy_blogs)) {
            $Messages->add(TB_('Please select at least one blog.'), 'error');
        }

        if (! param_errors_detected()) { // Try to obtain some serious time to do some serious processing (15 minutes)
            set_max_execution_time(900);
...
if ($display_mode != 'js') {
    ...
    switch ($tab3) {
        ...
        case 'ipranges':
            if (empty($action)) {	// View a list of IP ranges
                require_js_defer('customized:jquery/jeditable/jquery.jeditable.js', 'rsc_url');
            } elseif (! check_user_perm('spamblacklist', 'edit')) {	// Check permission to create/edit IP range
...
if (in_array($action, ['iprange_edit'])) { // Initialize date picker
    init_datepicker_js();
...
switch ($tab3) {
    ...
    case 'ipranges':
        switch ($action) {
            case 'iprange_new':
                if (! isset($edited_IPRange)) { // Define new IPRange object only when it was not defined before, e.g. in action 'iprange_create'
                    $edited_IPRange = new IPRange();
                }
                ...
                if (! empty($ip) && is_valid_ip_format($ip) &&
                    ($ip = explode('.', $ip)) && count($ip) == 4) {	// Prefill from single IP address:
                    $edited_IPRange->set('IPv4start', ip2int(implode('.', [$ip[0], $ip[1], $ip[2], 0])));
                    $edited_IPRange->set('IPv4end', ip2int(implode('.', [$ip[0], $ip[1], $ip[2], 255])));
                } else {	// Prefill from start and end IP addresses:
    ...
    case 'blacklist':
    default:
        if (! empty($javascript_messages)) {	// Don't display form and list when we should update a content from modal window by JS:
            break;
        }

        if ($action == 'ban' && (! $Messages->has_errors() || ! empty($confirm)) && ! ($delhits || $delcomments)) {	// Nothing to do, ask user:
            $AdminUI->disp_view('antispam/views/_antispam_ban.form.php');
        } else {	// Display blacklist:
...
if (! empty($javascript_messages)) {	// Execute JS functions after action from modal window:
