...
class IPRangeCache extends DataObjectCache
{
     ...
     */
    public function __construct($objType = 'IPRange', $dbtablename = 'T_antispam__iprange', $dbprefix = 'aipr_', $dbIDname = 'aipr_ID')
    {
     ...
     */
    public function &get_by_ip($req_ip, $halt_on_error = false, $halt_on_empty = false)
    {
        global $DB, $Debuglog;

        if (! isset($this->ip_index[$req_ip])) {	// not yet in cache:
            $IP = ip2int($req_ip);
            ...
            if ($IPRange) {	// It is in IPRangeCache
                $this->ip_index[$req_ip] = $IPRange;
            } else {	// not in the IPRangeCache
                if ($halt_on_error) {
