...
class IPRange extends DataObject
{
    ...
    public function load_from_Request()
    {
        ...
        if (! param_errors_detected()) {	// Check IPs for inside in other ranges
            if ($ip_ranges = get_ip_ranges($aipr_IPv4start, $aipr_IPv4end, $this->ID)) {	// If at least one IP range is found:
                if (param('delete_conflicts', 'integer', 0)) {	// Delete all conflicts:
                    global $DB;
                    $delete_ip_ranges_IDs = [];
                    $ip_ranges_html = '<ul>';
                    ...
                    if ($delete_result) {	// If IP ranges have been deleted successfully:
                        $Messages->add(sprintf(T_('Conflicting IP Ranges have been deleted: %s'), $ip_ranges_html), 'success');
                    }
                } else {	// Inform user about conflicts:
                    global $admin_url;
                    $ip_ranges_html = '<ul>';
                    foreach ($ip_ranges as $ip_range) {
