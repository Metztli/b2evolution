...
if (in_array($tab, ['settings', 'goals'])) { // Change tab to default and blog to 'all' from other controllers
    $tab_real = $tab;
    $tab = 'summary';
}
...
if (($blog == 0 && empty($sec_ID)) || ! check_user_perm('stats', 'list', false, $blog)) {
    if (! $perm_view_all && isset($collections_Module)) { // Find a blog we can view stats for:
        if (! $selected = autoselect_blog('stats', 'list')) { // No blog could be selected
...
switch ($action) {
    ...
    case 'domain_edit':
        ...
        if ($action == 'domain_new') { // New Domain
            ...
            if (($edited_Domain = &$DomainCache->get_by_ID($dom_ID, false)) === false) { // We could not find the goal to edit:
                unset($edited_Domain);
                forget_param('dom_ID');
    ...
    case 'domain_update':
        ...
        $DB->begin();
        if ($edited_Domain->load_from_Request()) { // We could load data from form without errors:
            ...
            // Redirect so that a reload doesn't write to the DB twice:
            if ($tab_from == 'antispam') { // Updating from antispam controller
                $redirect_to = $admin_url . '?ctrl=antispam&tab3=domains';
            } else { // Updating from analitics collection page
    ...
    case 'domain_delete':
        ...
        if (param('confirm', 'integer', 0)) {
            if ($edited_Domain === false) { // We could not find the goal to edit:
                unset($edited_Domain);
                forget_param('dom_ID');
                $Messages->add(sprintf(TB_('Requested &laquo;%s&raquo; object does not exist any longer.'), TB_('Domain')), 'error');
            }
    ...
    case 'filter_hits_diagram':
        ...
        if (param('agg_period', 'string', null) !== null) {	// Filter the aggregated data by date:
    ...
    case 'compare_hits_diagram':
        ...
        if (param('aggcmp_period', 'string', null) !== null) {	// Filter the compared data by date:
...
if (isset($collections_Module) && $tab_from != 'antispam') { // Display list of blogs:
    if ($perm_view_all) {
...
if (isset($tab_real)) { // Restore real tab value
    $tab = $tab_real;
}
...
if ($tab_from == 'antispam') { // User goes from antispam tab, Set the correct paths
    $AdminUI->set_path('options', 'antispam', 'domains');
    $AdminUI->breadcrumbpath = [];
    $AdminUI->breadcrumb_titles = [];
...
if ($tab == 'domains') { // Load jquery UI to highlight cell on change domain type
    require_js_defer('#jqueryUI#');
}

if (in_array($tab, ['hits', 'other', 'referers']) ||
    ($tab == 'refsearches' && in_array($tab3, ['hits', 'keywords'])) ||
    ($tab == 'goals' && $tab3 == 'hits')) { // Initialize date picker for _stats_search_keywords.view.php and _stats_goalhits.view.php
    init_datepicker_js();
}
