...
class Hitlist
{
     ...
     */
    public static function prune($date)
    {
        global $DB;
     ...
     * @return mixed Return value of {@link DB::query()}
     */
    public static function prune_goal_hits($date)
    {
        global $DB;

        return $DB->query(
            'DELETE T_track__goalhit FROM T_track__goalhit
     ...
     * @return mixed Return value of {@link DB::query()}
     */
    public static function change_type($hit_ID, $type)
    {
        global $DB;
     ...
     * @return string
     */
    public static function log_pruning($message, $output_message = false, $is_end_action = false)
    {
        if ($output_message === 'cron_job') {	// Log a message for cron job:
            if ($is_end_action) {
     ...
     * )
     */
    public static function dbprune($output_message = true, $day_limit = true)
    {
        /**
         * @var DB
         */
        ...
        if ($last_prune >= date('Y-m-d', $localtimenow) && $last_prune <= date('Y-m-d', $localtimenow + 86400)) { // Already pruned today (and not more than one day in the future -- which typically never happens)
            ...
            if ($day_limit) {	// Limit pruning to one execution per day:
                return [
                    'result' => 'error',
                    'message' => $error_message,
                ];
            } else {	// Don't limit by day but display a warning:
        ...
        // Get tables info:
        $tables_info = $DB->get_results('SHOW TABLE STATUS WHERE Name IN ( ' . $DB->quote(['T_hitlog', 'T_track__goalhit', 'T_sessions', 'T_basedomains']) . ' )');
        ...
        $Plugins->trigger_event('BeforeSessionsDelete', $temp_array = [
            'cutoff_timestamp' => $oldest_date,
        ]);
        ...
        while ($sessions_rows_affected_i) {
            ...
            $sessions_rows_affected_i = $DB->query(
            ...
            $sessions_i++;
            if ($sessions_rows_affected_i < 1000) {	// Don't try next query if current already is less 1000 records:
                break;
            }
        }
        $hitlist_Timer->stop('sessions_total');
        if ($sessions_i > 1) {	// Display total pruned sessions only if it was executed more 1 time per 1000 limited records:
     ...
     * Aggregate the hits
     */
    public static function aggregate_hits()
    {
        global $DB;
        ...
        $DB->query(
            'REPLACE INTO T_hits__aggregate ( hagg_date, hagg_coll_ID, hagg_type, hagg_referer_type, hagg_agent_type, hagg_count )
     ...
     *               - all_api: ALL API sessions
     */
    public static function aggregate_sessions($type = 'all')
    {
        global $DB;
        ...
        if ($type == 'all' || $type == 'coll_browser') {	// ONLY collection browser sessions:
            $DB->query(
                'REPLACE INTO T_hits__aggregate_sessions ( hags_date, hags_coll_ID, hags_count_browser )
        ...
        if ($type == 'all' || $type == 'coll_api') {	// ONLY collection API sessions:
            $DB->query(
                'INSERT INTO T_hits__aggregate_sessions ( hags_date, hags_coll_ID, hags_count_api )
				SELECT DATE( hit_datetime ) AS hit_date, hit_coll_ID, COUNT( DISTINCT hit_sess_ID )
				  FROM T_hitlog
				 WHERE hit_datetime < ' . $DB->quote($max_aggregate_date) . '
				   AND hit_type = "api"
        ...
        if ($type == 'all' || $type == 'all_browser') {	// ALL browser sessions:
            $DB->query(
                'REPLACE INTO T_hits__aggregate_sessions ( hags_date, hags_coll_ID, hags_count_browser )
        ...
        if ($type == 'all' || $type == 'all_api') {	// ALL API sessions:
            $DB->query(
                'INSERT INTO T_hits__aggregate_sessions ( hags_date, hags_coll_ID, hags_count_api )
				SELECT DATE( hit_datetime ) AS hit_date, 0, COUNT( DISTINCT hit_sess_ID )
				  FROM T_hitlog
     ...
     */
    public static function aggregate_goal_hits()
    {
        global $DB;
        ...
        $DB->query(
            'REPLACE INTO T_track__goalhit_aggregate ( ghag_date, ghag_goal_ID, ghag_count )
