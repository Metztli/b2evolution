...
class Goal extends DataObject
{
    ...
    public $temp_redir_url = '';

    public $temp_start_ts = null;

    public $temp_end_ts = null;
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                'table' => 'T_track__goalhit',
                'fk' => 'ghit_goal_ID',
                'msg' => T_('%d related goal hits'),
            ],
            [
                'table' => 'T_track__goalhit_aggregate',
                'fk' => 'ghag_goal_ID',
                'msg' => T_('%d related goal hits aggregations'),
            ],
        ];
    }
     ...
     */
    public function check_perm($action = 'view', $assert = true)
    {
    ...
    public function load_from_Request()
    {
        ...
        if ($this->get('temp_redir_url') != '') { // Normal Redirection URL is required when Temporary Redirection URL is not empty
            param_check_not_empty('goal_redir_url', T_('Please enter Normal Redirection URL.'));
        }
        $this->set_from_Request('redir_url');

        if ($this->get('temp_redir_url') != '' && $this->get('temp_redir_url') == $this->get('redir_url')) { // Compare normal and temp urls
        ...
        if ($this->get('temp_start_ts') !== null && $this->get('temp_end_ts') !== null &&
            strtotime($this->get('temp_start_ts')) >= strtotime($this->get('temp_end_ts'))) { // Compare Start and End dates
        ...
        if (! param_errors_detected()) {	// Check goal key for duplicating:
            $existing_goal_ID = $this->dbexists('goal_key', $this->get('key'));
            if ($existing_goal_ID) {	// We have a duplicate goal:
                global $Collection, $Blog;
                param_error(
                    'goal_key',
                    sprintf(
                        T_('This goal already exists. Do you want to <a %s>edit the existing goal</a>?'),
     ...
     */
    public function get_active_url($params = [])
    {
        $params = array_merge([
            'before_temp' => '',
            'after_temp' => '',
        ...
        if (empty($this->temp_redir_url)) { // Use normal redirection URL when temporary redirection URL is empty
            return $this->redir_url;
        } else { // Check if we can use temporary redirection URL
            global $localtimenow;

            if ((empty($this->temp_start_ts) || $this->temp_start_ts <= $localtimenow) &&
                (empty($this->temp_end_ts) || $this->temp_end_ts >= $localtimenow)) { // Use temporary redirection URL now
                return $params['before_temp'] . $this->temp_redir_url . $params['after_temp'];
            } else { // Temporary redirection URL is out date, Use normal URL now
                return $this->redir_url;
            }
        }
    }
     ...
     */
    public function record_hit($extra_params = '#')
    {
        global $DB, $Hit;

        if ($extra_params == '#') { // Use standard extra params from query string
            if (isset($_SERVER['QUERY_STRING'])) { // Set additional params in goal hit
                $extra_params = '&' . $_SERVER['QUERY_STRING'] . '&';
                $extra_params = preg_replace('/&key=[^&]+(&)?/i', '$1', $extra_params);
                $extra_params = trim($extra_params, '&');
            }
            if (empty($extra_params)) { // No extra params
                $extra_params = null;
            }
        }
        ...
        // Insert a goal hit:
        $DB->query(
            'INSERT INTO T_track__goalhit ( ghit_goal_ID, ghit_hit_ID, ghit_params )
