...
class Hit
{
     ...
     * @var array 0 - value from['REMOTE_HOST'], 1 - value from gethostbyaddr( $this->IP )
     * @access protected
     */
    public $_remoteHost;
     ...
     * @access protected
     */
    public $search_engine_names;

    /**
     * Extracted from search referers:
     */
    public $_search_params_tried = false;

    public $_keyphrase = null;

    public $_serprank = null;

    public $_search_engine = null;
     ...
     */
    public function __construct($referer = null, $IP = null, $session_id = null, $hit_time = null, $test_mode = null, $test_uri = null, $user_agent = null, $test_admin = null, $test_rss = null)
    {
        global $debug;

        if ($IP !== null) {	// Use the provided IP address value (for test mode):
            $this->IP = $IP;
        } else {	// Get the first IP in the list of REMOTE_ADDR and HTTP_X_FORWARDED_FOR:
            $this->IP = get_ip_list(true);
        }

        if ($session_id !== null) {	// Use the provided value (for test mode):
            $this->session_id = $session_id;
        }

        if ($hit_time !== null) {	// Use the provided value (for test mode):
            $this->hit_time = $hit_time;
        }

        if ($test_mode !== null) {	// Use the provided value (for test mode):
            $this->test_mode = $test_mode;
        }

        if ($test_uri !== null) {	// Use the provided value (for test mode):
            $this->test_uri = $test_uri;
        }

        if ($user_agent !== null) {	// Use the provided value (for test mode):
            $this->user_agent = $user_agent;
        }

        if ($test_admin !== null) {	// Use the provided value (for test mode):
            $this->test_admin = $test_admin;
        }

        if ($test_rss !== null) {	// Use the provided value (for test mode):
        ...
        if ($debug) {	// Initialize data for debug log:
            global $Debuglog;
     ...
     * Detect admin page
     */
    public function detect_admin_page()
    {
        global $Debuglog;

        if (empty($this->test_mode) || ! empty($this->test_admin)) {
            if (is_admin_page()) {	// We are inside of admin, this supersedes 'direct' access
    ...
    public function get_referer()
    {
        ...
        if (isset($HTTP_REFERER)) {	// Use referer which is provided by PHP global var:
            $referer = $HTTP_REFERER;
        } elseif (isset($_SERVER['HTTP_REFERER'])) {	// Try to get from global array $_SERVER:
            $referer = $_SERVER['HTTP_REFERER'];
        } else {	// Fallback method (not thread safe :[[ ) - this function does not work in ISAPI mode.
     ...
     */
    public function detect_referer($referer = null)
    {
        ...
        if ($referer !== null) {	// Use the provided value:
            $this->referer = $referer;
        } else {	// Get referer from HTTP request:
            $this->referer = $this->get_referer();
        }

        if (empty($this->referer)) {	// If NO referer defined yet
            // This type may be superseeded and set to 'admin'
            if (! $this->detect_admin_page()) {	// Not an admin page:
        ...
        if ($error = validate_url($this->referer, 'commenting')) { // This is most probably referer spam!!
            $Debuglog->add('Hit: detect_referer(): ' . $error . ' (SPAM)', 'hit');
            $this->referer_type = 'spam';

            if ($Settings->get('antispam_block_spam_referers')) { // In order to preserve server resources, we're going to stop processing immediatly (no logging)!!
    ...
    public function get_referer_domain_ID()
    {
        if (! isset($this->referer_domain_ID)) {
            ...
            if ($referer_basedomain) {	// This referer has a base domain
                ...
                if (! empty($hit_basedomain_ID)) {	// This basedomain has visited before:
                    $this->referer_domain_ID = $hit_basedomain_ID;
                    // fp> The blacklist handling that was here made no sense.
                } else {	// This is the first time this base domain visits:
                    ...
                    } else {	// INSERT failed: see, try to select again (may become/stay NULL)
     ...
     */
    public function detect_useragent()
    {
        if (isset($this->agent_type)) { // already detected.
            return;
        }
        ...
        if (! empty($user_agent)) { // detect browser
            if (strpos($user_agent, 'Win') !== false) {
                ...
                if ($platform == 'linux' || in_array(substr($platform, 0, 3), ['win', 'mac'])) {
            ...
            if (strpos($user_agent, 'Lynx') !== false) {
                ...
                if (preg_match('/MSIE (\d+)\./', $user_agent, $browser_version)) { // Detect version of IE browser
                    $this->browser_version = (int) $browser_version[1];
                }
                $this->is_IE = true;
                if ($this->agent_platform == 'win') { // Win IE
                    $this->is_winIE = 1;
                }
                if ($this->agent_platform == 'mac') { // Mac IE
                    $this->is_macIE = 1;
                }
     ...
     */
    public function get_browser_caps()
    {
        static $caps = null;

        if ($caps === null) {	// Get only first time:
     ...
     */
    public function log($delayed = false)
    {
        global $Plugins;

        if ($this->logged) {	// Already logged, don't log twice:
            return false;
        }
        ...
        if ($Plugins->trigger_event_first_true('AppendHitLog', [
            'Hit' => &$this,
     ...
     * @return boolean
     */
    public function should_be_logged()
    {
        global $Settings, $Debuglog, $is_admin_page;

        if ($is_admin_page && ! $Settings->get('log_admin_hits') && empty($this->test_mode)) {	// We don't want to log admin hits:
            $Debuglog->add('Hit: Hit NOT logged, (Admin page logging is disabled)', 'request');
            return false;
        }

        if (! $is_admin_page && ! $Settings->get('log_public_hits')) {	// We don't want to log public hits:
            $Debuglog->add('Hit: Hit NOT logged, (Public page logging is disabled)', 'request');
            return false;
        }

        if (($this->referer_type == 'spam' && ! $Settings->get('log_spam_hits')) && empty($this->test_mode)) {	// We don't want to log referer spam hits:
     ...
     * @param boolean Hit saving in the database can be delayed or not because of the hit_ID is required
     */
    public function record_the_hit($delayed = false)
    {
        ...
        if (empty($this->test_uri)) {
            if (! empty($Blog)) {
                ...
                if (! is_numeric($blog)) { // this can be anything given by URL param "blog"! (because it's called on shutdown)
                    // see todo in param().
                    $blog = null;
                }
                $blog_ID = $blog;
        ...
        if ($keyphrase === null || $keyphrase === '') {	// No search hit
            if (! empty($this->test_mode) && ! empty($this->test_uri['s'])) {
                $s = $this->test_uri['s'];
            } elseif (isset($_GET['s']) || isset($_POST['s'])) {	// Consider a request as search ONLY if a keyword is passed through GET or POST,
                // and not in automatic searches like 404 page with search widget:
                $s = get_param('s');
            }
            if (isset($s) && ! empty($blog_ID)) {	// Record Internal Search:
                $keyphrase = $s;
            }
        }

        if ($keyphrase === '') {	// Consider empty string as no search request:
            $keyphrase = null;
        }

        if ($keyphrase !== null) {	// Limit keyphrase but do NOT convert NULL to empty string:
        ...
        if (! empty($http_response_code)) { //  in some cases $http_response_code not set and we can use value by default
        ...
        if (empty($this->hit_type)) {
            ...
            if (! empty($is_api_request)) {	// This is an API request:
                $this->hit_type = 'api';
        ...
        if (empty($this->method)) {	// Initialize a request method:
            if (isset($_SERVER['REQUEST_METHOD']) && in_array($_SERVER['REQUEST_METHOD'], ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'COPY', 'HEAD', 'OPTIONS', 'LINK', 'UNLINK', 'PURGE', 'LOCK', 'UNLOCK', 'PROPFIND', 'VIEW'])) {	// Current request method is allowed
        ...
        if (empty($this->test_mode)) {	// Normal mode
     ...
     */
    public function get_hit_type()
    {
        global $ReqURI;

        if ($this->hit_type) {	// Use the hit type from previous detecting time:
            return $this->hit_type;
        }

        if (strstr($ReqURI, 'htsrv/async.php') || strstr($ReqURI, 'htsrv/anon_async.php')) {	// Mark request from such URLs as AJAX request:
            return 'ajax';
        }

        if (strstr($ReqURI, 'htsrv/')) {	// Mark request from such URLs as service request:
            return 'service';
        }
    ...
    public function get_action()
    {
        ...
        if ($this->action) {	// Use the hit action from previous detecting time:
            return $this->action;
        }

        if (strstr($ReqURI, 'htsrv/getfile.php')) {	// Special case, this script is used to generate thumbnail images:
            return 'getfile';
        }

        if (! empty($action)) {	// Get action value from global variable which is usually defined from $_GET/$_POST:
            return $action;
        }
     ...
     * @return string
     */
    public function get_keyphrase()
    {
        if ($this->_search_params_tried) {	// Use the keyphrase from previous detecting time:
    ...
    public function get_user_agent()
    {
        if (! isset($this->user_agent)) {
            ...
            if ($this->user_agent != strip_tags($this->user_agent)) {	// then they have tried something funky, putting HTML or PHP into the user agent
                global $Debuglog;
    ...
    {
        if (! isset($this->agent_ID)) {	// Detect only first time:
    ...
    {
        if (! isset($this->agent_name)) {	// Detect only first time:
            $this->detect_useragent();
        }
     ...
     * @return string
     */
    public function get_agent_platform()
    {
        if (! isset($this->agent_platform)) {	// Detect only first time:
            $this->detect_useragent();
        }
    ...
    {
        if (! isset($this->agent_type)) {	// Detect only first time:
            $this->detect_useragent();
        }
     ...
     * @return string
     */
    public function get_remote_host($allow_nslookup_by_IP = false)
    {
        global $Timer;

        $Timer->resume('Hit::get_remote_host');

        if (! isset($this->_remoteHost)) {	// Initialize array:
            $this->_remoteHost = [];
        }
        ...
        if (! isset($this->_remoteHost[$host_type])) {	// Initialize remote host only first time:
            if (! empty($_SERVER['REMOTE_HOST'])) {	// Use this(The reverse dns lookup is based off the REMOTE_ADDR of the user) ONLY if it is NOT empty:
                $this->_remoteHost[$host_type] = $_SERVER['REMOTE_HOST'];
            } elseif ($allow_nslookup_by_IP) {	// We allowed reverse DNS lookup:
     ...
     */
    public function get_param_from_string($string, $param)
    {
     ...
     */
    public function get_search_engine_names()
    {
        ...
        if (is_null($this->search_engine_names)) {
            ...
            foreach ($search_engine_params as $url => $info) {
                if (! isset($this->search_engine_names[$info['name']])) {	// Do not overwrite existing keys
     ...
     * @return string
     */
    public function get_lossy_url($url)
    {
        static $countries;

        if (! isset($countries)) {	// Load 2 letter ISO country codes
     ...
     * @return boolean
     */
    public function is_new_view()
    {
        if ($this->get_agent_type() == 'robot') {	// Robot requests are not considered as (new) views:
            return false;
        }
        ...
        if (! isset($this->_is_new_view)) {
            ...
            if (is_logged_in()) {	// select by user ID: one user counts really just once. May be even faster than the anonymous query below..!?
                $SQL->WHERE_and('hit_sess_ID = ' . $Session->ID);
            } else {	// select by remote_addr/hit_agent_type:
     ...
     */
    public function is_browser_reload()
    {
        if ((isset($_SERVER['HTTP_CACHE_CONTROL']) && strpos($_SERVER['HTTP_CACHE_CONTROL'], 'max-age=0') !== false)
            || (isset($_SERVER['HTTP_PRAGMA']) && $_SERVER['HTTP_PRAGMA'] == 'no-cache')) { // Reload
            return true;
        }
     ...
     */
    public function is_search_referer($referer, $return_params = false)
    {
        global $search_engine_params;
        ...
        if (! isset($pu['host'])) {	// Wrong referer without host name:
            return false;
        }
        ...
        if (array_key_exists($ref_host . $ref_path, $search_engine_params)) {
            ...
            if (! strncmp($ref_query, 'cx=partner-pub-', 15)) {	// Google custom search engine
                $ref_host = 'www.google.com/cse';
            } elseif (! strncmp($ref_path, '/pemonitorhosted/ws/results/', 28)) {	// Private-label search powered by InfoSpace Metasearch
                $ref_host = 'infospace.com';
            } else {	// Not a search referer
                return false;
            }
        }

        if ($return_params) {
            return [$ref_host, $ref_path, $ref_query, $ref_fragment];
        }
        return true;
    }
     ...
     *							'serprank' => 4)
     */
    public function extract_params_from_referer($ref)
    {
        ...
        if (empty($ref_host)) {	// Not a search referer
            return false;
        }
        ...
        if (is_null($keyword_param)) {	// Get settings from first item in group
        ...
        if ($search_engine_name == 'Google Images' || ($search_engine_name == 'Google' && strpos($ref, '/imgres') !== false)) {	// Google image search
            $search_engine_name = 'Google Images';

            $query = urldecode(trim($this->get_param_from_string($query, 'prev')));
            $query = str_replace('&', '&amp;', strstr($query, '?'));
        } elseif ($search_engine_name == 'Google' && (strpos($query, '&as_') !== false || strpos($query, 'as_') === 0)) {	// Google with "as_" param
            $keys = [];
        ...
        if (empty($key) && ! empty($keyword_param)) {	// we haven't extracted a search key with the special cases above...
            foreach ($keyword_param as $param) {
                if ($param[0] == '/') {	// regular expression match
                    if (@preg_match($param, $ref, $matches)) {
                        $key = trim(urldecode($matches[1]));
                        break;
                    }
                } else {	// search for keywords now &vname=keyword
        ...
        if (empty($key) && ! empty($keyword_param)) {	// Check if empty key param exists in query, e.g. "/search?q=&other_param=text"
            // OR search engine supports urls without query param like Google:
            foreach ($keyword_param as $k_param) {
                if ($k_param === null || strpos($query, '&' . $k_param . '=') !== false || strpos($query, $k_param . '=') === 0) {	// Search engine supports urls without param OR Key param with empty value exists in query, We can decide this referer url as from search engine:
        ...
        if (empty($key) && ! $key_param_in_query) {	// Not a search referer
            if ($this->referer_type == 'search') {	// If the referer was detected as 'search' we need to change it to 'special'
        ...
        */
        // Fallback to default params
        $serp_param = ['offset', 'page', 'start'];

        if (! is_array($serp_param)) {
            $serp_param = [$serp_param];
        }

        if (strpos($search_engine_name, 'Google') !== false) {	// Append fragment which Google uses in instant search
            $query .= '&' . $fragment;
        }
     ...
     * @return boolean
     */
    public function is_IE($version = null, $operator = '=')
    {
        if (! isset($this->is_IE)) {
            $this->detect_useragent();
        }
        ...
        if (! is_null($version)) { // Check version of IE
            switch ($operator) {
                case '=':
    ...
    {
        if (is_null($this->browser_version)) {	// Initialize browser version on first calling:
            $this->detect_useragent();
        }
