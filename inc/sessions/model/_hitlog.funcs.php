 ...
 */
function hits_results_block($params = [])
{
    ...
    if ($blog == 0) {
        if (! check_user_perm('stats', 'view')) { // Current user has no permission to view all stats (aggregated stats)
            return;
        }
    } else {
        if (! check_user_perm('stats', 'list', false, $blog)) { // Current user has no permission to view the stats of the selected blog
            return;
        }
    }
    ...
    if (! empty($device)) {
        if ($device == 'other') { // Unknown device
            $device = '';
        }
    ...
    if (! empty($sec_ID)) {	// Filter by selected section:
    ...
    if (! empty($blog)) {	// Filter by selected collection:
...
function generate_hit_stat($days, $min_interval, $max_interval, $display_process = false)
{
    ...
    foreach ($blogs_id as $blog_id) {	// Handle all public blogs:
    ...
    $referes = ['http://www.fake-referer1.com',
    ...
    $request_methods = ['GET', 'POST', 'PUT', 'DELETE', 'HEAD', 'unknown'];
    ...
    for ($time_shift = $past_time; $cur_time > $time_shift; $time_shift += mt_rand($min_interval, $max_interval)) {
        ...
        if (strstr($links[$rand_link]['link'], '$keywords$')) { // check if the current search link is selected randomly.
        ...
        if ($cur_session['sess_ID'] == -1) { // This session needs initialization:
            ...
            if (($time_shift - $cur_session['sess_lastseen_ts']) > 3000 || ! empty($cur_session['robot'])) { // This session last updated more than 3000 sec ago. Instead of this session create a new session.
                $cur_session = [
                    'sess_ID' => -1,
                    'sess_key' => generate_random_key(32),
                    'sess_start_ts' => 0,
                ...
                $cur_session['sess_lastseen_ts'] = $time_shift;
                $r_num = mt_rand(0, 100);
                if ($r_num > 40) { // Create anonymous user and make double insert into hits.
                ...
                if (mt_rand(0, 100) > 20) {
                    ...
                    if (strstr($ref_link, '$keywords$')) { // check if the current search link is selected randomly.
                        $keywords = 'fake search ' . mt_rand(0, 9);
                ...
                if ($cur_session['sess_user_ID'] == -1) {
                    ...
                    if (mt_rand(0, 100) < 10) {	// Test hit to admin page:
                        // This is NOT api request:
                        $is_api_request = false;
        ...
        if (isset($Test_hit) && ! empty($Test_hit->ID) && rand(0, 2) == 2) {	// Insert sample goal hits per random new Hit:
        ...
        if ($display_process) {
            if ($insert_data_count % 100 == 0) { // Display a process of creating by one dot for 100 hits
                echo ' .';
                evo_flush();
            }
        }
...
function stats_dom_status_icons()
{
    return [
        ...
        'unknown' => get_icon('bullet_white', 'imgtag', [
            'title' => aipr_status_title('unknown'),
        ]),
...
function &get_Domain_by_subdomain($subdomain_name)
{
    ...
    for ($i = 0; $i < count($subdomain_name); $i++) {
        ...
        foreach ($domain_names as $domain_name) {
            if ($Domain = &$DomainCache->get_by_name($domain_name, false, false)) {	// If domain exists in DB:
                if ($first_existing_Domain === null) {	// Store first top existing domain with any status:
                    $first_existing_Domain = $Domain;
                }
                if ($Domain->get('status') != 'unknown') {	// Use this domain because it exists with not unknown status:
                    return $Domain;
                }
            }
        }
...
function get_hit_agent_name_by_ID($agent_ID)
{
    ...
    if (isset($user_agents[$agent_ID]) && ! empty($user_agents[$agent_ID][2])) { // Agent is found with given ID
        return $user_agents[$agent_ID][2];
    } else { // No agent, Return ID
        return $agent_ID;
    }
}
...
function extract_keyphrase_from_hitlogs($display_messages = true)
{
    ...
    if ($DB->get_var('SELECT IS_FREE_LOCK( ' . $DB->quote($lock_name) . ' )') === '0') { // The "exctract_keyphrase" process is already running on a different request, do not start it again
        // Do not translate.
    ...
    if ($display_messages) {	// Display info:
        ...
        if ($display_messages === 'cron_job') {	// Log a message for cron job:
    ...
    if (! empty($ids['min']) && ! empty($ids['max'])) {	// Extract keyphrases if needed:
...
{
    if (preg_match('/^item_ID=([0-9]+)$/i', $ghit_params, $matches)) { // Parse item ID
        $ItemCache = &get_ItemCache();
        if ($Item = &$ItemCache->get_by_ID(intval($matches[1]), false, false)) { // Display a link to view with current item title
            if (check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) { // Link to admin view
                return $Item->get_title([
                    'link_type' => 'admin_view',
                ]);
            } else { // Link to permament url (it is allowed for current post type)
                return $Item->get_title();
            }
        }
    }
 ...
 */
function display_hits_summary_panel($diagram_columns = [])
{
    global $ReqURL;
    ...
    if (check_user_perm('stats', 'edit')) {	// Display button to aggregate hits right now only if current user has a permission to edit hits:
...
function display_hits_filter_form($mode, $diagram_columns, $display_filter_period = true)
{
    ...
    if ($display_filter_period) {	// Filter the aggregated data by date period:
    ...
    if ($display_filter_diagram_cols) {
        ...
        foreach ($diagram_columns as $diagram_column_key => $diagram_column_data) {	// Filter hits by type:
            $Form->checkbox_basic_input(
                'filter_types[]',
                ...
                [
                    'value' => $diagram_column_key,
                ]
            ); // Value
        }
    }

    $Form->end_form([['submit', 'submit', $submit_button_text, 'btn-info']]);
...
function get_filter_aggregated_hits_dates($mode = 'aggregate')
{
    ...
    if ($mode == 'compare') {	// Use a filter for comparing data:
        $period = $UserSettings->get('aggcmp_period');
    } else {	// Use a filter for aggregate data:
    ...
    switch ($period) {
        ...
        case 'specific_month':
            ...
            if (! empty($agg_month)) {	// Use month from setting:
                $start_date['m'] = $agg_month;
                $end_date['m'] = $agg_month + 1;
            }
            if (! empty($agg_year)) {	// Use year from setting:
...
function get_filtered_hits_diagram_columns($diagram_type, $diagram_columns)
{
    ...
    if (empty($diagram_type)) {	// Unknown diagram type:
        return $diagram_columns;
    }

    $filter_hits_diagram_cols = $UserSettings->get('filter_hits_diagram_cols');

    if (! isset($filter_hits_diagram_cols[$diagram_type])) {	// No filter is defiend yet:
        return $diagram_columns;
    }

    if (empty($filter_hits_diagram_cols[$diagram_type])) {	// If all options are unchecked then check them all:
        return $diagram_columns;
    }
...
function get_hits_summary_mode()
{
    ...
    if (empty($hits_summary_mode)) {	// Set mode to display the aggregated data by default:
...
function fill_empty_hit_days($hits_data, $start_date, $end_date)
{
    ...
    $additional_fields = array_diff_key($hits_data[0], [
        'hits' => 0,
        'year' => 0,
        'month' => 0,
        'day' => 0,
    ...
    foreach ($hits_data as $hit) {
        ...
        if (isset($prev_date) && $prev_date != $this_date) {	// If hits are from another day:
            $prev_time = strtotime($prev_date) - 86400;
            $this_time = strtotime($this_date);

            if ($prev_time != $this_time) {	// If previous date is not previous day(it means some day has no hits):
                $empty_days = ($prev_time - $this_time) / 86400;
                for ($d = 0; $d <= $empty_days; $d++) {	// Add each empty day to array with 0 hits count:
...
function get_hit_full_url($hit_uri, $hit_coll_ID)
{
    ...
    if (! empty($hit_coll_ID)) {	// Try to get a collection if it was a hit from collection page:
        $BlogCache = &get_BlogCache();
        if ($Blog = &$BlogCache->get_by_ID($hit_coll_ID, false, false)) {	// Get collection host:
