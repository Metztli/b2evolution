...
class Session
{
    ...
    public $_session_needs_save = false;
    ...
    public function __construct($session_ID = null)
    {
        ...
        if ($this->user_ID != null) {	// User is not anonymous, get custom session timeout (may return NULL):
            $timeout_sessions = $UserSettings->get('timeout_sessions', $this->user_ID);
        }
        if (empty($timeout_sessions)) {	// User is anonymous or has no custom session timeout. So, we use default session timeout:
            $timeout_sessions = $Settings->get('timeout_sessions');
        }

        if ($session_ID !== null) {	// Get Session by requested ID:
        ...
        } else {	// Get Session from cookie data ID + key:
            ...
            } else {	// We have a valid session cookie:
        ...
        if (! empty($row)) {	// Load session data if row is found in DB:
            ...
            if (empty($row->sess_data)) {
                ...
                $this->_data = [];
            } else { // Some session data has been previsouly stored:
                ...
                if ($old_callback === false || is_null($old_callback) /* disabled, reported with PHP 5.2.5 */) {	// NULL if ini_set has been disabled for security reasons
                ...
                if ($old_callback !== false) {	// Restore the old callback if we changed it:
                ...
                if (! is_array($this->_data)) {
                    $Debuglog->add('Session: Session data corrupted!<br />
								...
								Serialized data was: --[' . var_export($row->sess_data, true) . ']--', ['session', 'error']);
                    $this->_data = [];
                } else {
        ...
        if ($this->ID) { // there was a valid session before
            if ($this->lastseen_ts < $localtimenow - 60) { // lastseen timestamp is older then a minute, it needs to be updated at page exit
                $this->session_needs_save(true);
            }
        } elseif ($session_ID === null) {	// Create a new session! Only for session from cookie:
    ...
    public static function db_delete_where($sql_where, $object_ids = null, $params = null)
    {
        ...
        if (! $object_ids) { // There is no session to delete
            $DB->commit();
            return;
        }
    ...
    public function session_needs_save($session_needs_save)
    {
    ...
    public function set_user_ID($user_ID)
    {
        if ($user_ID != $this->user_ID) {
            ...
            if ($multiple_sessions != 'always' && ($multiple_sessions == 'never' || ! $UserSettings->get('login_multiple_sessions', $user_ID))) { // The user does not want/is not allowed to have multiple sessions open at the same time:
    ...
    public function logout()
    {
        ...
        if ($Settings->get('http_auth_accept') && isset($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'])) {	// Don't log out when user is logged in by HTTP basic authentication because it is possible only after browser closing:
        ...
        $this->key = null;
        $this->_data = []; // We don't need to keep old data
        $this->session_needs_save(true);
        $this->dbsave();
    ...
    public function get($param, $default = null)
    {
        ...
        if (isset($this->_data[$param])) {
            ...
            } else { // expired or old format (without 'value' key)
     ...
     */
    public function set($param, $value, $expire = 0)
    {
        global $Debuglog, $localtimenow;
        ...
        if (! isset($this->_data[$param])
         ...
         || $expire != 0) {	// There is something to update:
            if (isset($this->_data[$param])) {	// Log updated session var:
                if (isset($this->_data[$param][1])) {
                    if (is_scalar($this->_data[$param][1])) {	// Display value for scalar var:
            ...
            } else {	// Log created session var:
                $action_type = 'created';
            }
            $this->_data[$param] = [($expire ? ($localtimenow + $expire) : null), $value];

            if ($param == 'Messages') { // also set boolean to not call CachePageContent plugin event on next request:
    ...
    public function dbsave()
    {
        ...
        if (! $this->_session_needs_save) {	// There have been no changes since the last save.
        ...
        if (! is_null($this->user_ID)) {	// We do NOT erase existing IDs at logout. We only want to set IDs at login:
     ...
     */
    public function create_crumb($crumb_name)
    {
        ...
        if (empty($crumb_value)) {	// We need to generate a new crumb:
     ...
     */
    public function assert_received_crumb($crumb_name, $die = true, $params = [])
    {
        ...
        if (! $crumb_received = param('crumb_' . $crumb_name, 'string', null)) { // We did not receive a crumb!
            if ($die) {
        ...
        if ($crumb_latest_is_valid || $crumb_previous_is_valid) {	// At least one crumb is valid, check for expired:
            if ($crumb_latest_is_valid && $servertimenow - $latest_crumb_time <= $crumb_expires) {	// The latest crumb is valid and not expired:
                return true;
            }
            if ($crumb_previous_is_valid && $servertimenow - $prev_crumb_time <= $crumb_expires) {	// The previous crumb is valid and not expired:
                return true;
            }
        ...
        } else {	// The latest and previous crumbs are invalid:
            $crumb_info_message = $params['msg_incorrect_crumb'];
            $crumb_latest_status = 'Invalid';
            $crumb_previous_status = 'Invalid';
        }

        if (! $die) {	// Return false result when we don't need die action on error:
            return false;
        }
        ...
        if ($params['msg_format'] == 'text') {	// Display error message in TEXT format:
        ...
        if ($debug > 0) {	// Display additional info when debug is enabled:
            echo '<div>';
        ...
        $Form->button(['submit', '', T_('Resubmit now!'), 'ActionButton']);
        $Form->end_form();
        echo '</div>';
     ...
     */
    public function is_mobile_session()
    {
     ...
     * Was this session created from a mobile device
     */
    public function is_tablet_session()
    {
     ...
     */
    public function is_alt_session()
    {
        global $Blog, $Hit;
     ...
     */
    public function is_desktop_session()
    {
     ...
     */
    public function get_first_hit_params()
    {
        if (! isset($this->first_hit_params)) {	// Get the params from DB only first time and then cache them:
            global $DB;
