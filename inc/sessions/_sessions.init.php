...
function &get_GoalCache()
{
    ...
    if (! isset($GoalCache)) { // Cache doesn't exist yet:
...
function &get_GoalCategoryCache($allow_none_text = null)
{
    ...
    if (! isset($GoalCategoryCache)) { // Cache doesn't exist yet:
...
class sessions_Module extends Module
{
    ...
    public function build_evobar_menu()
    {
        ...
        if ((! is_admin_page() || ! empty($activate_collection_toolbar)) && ! empty($Blog) && check_user_perm('stats', 'list', false, $Blog->ID)) { // Permission to view stats for user's blogs:
            $entries = [
                'stats_separator' => [
                    'separator' => true,
                ],
        ...
        if (check_user_perm('stats', 'view')) {	// We have permission to view all stats
            $entries = [
                'stats_separator' => [
                    'separator' => true,
                ],
            ...
            if (! is_admin_page()) {	// Only for front-office:
                $page_menus = [];
                if ($topleft_Menu->get_node_by_path(['page', 'edit_front']) ||
                    $topleft_Menu->get_node_by_path(['page', 'edit_back']) ||
                    $topleft_Menu->get_node_by_path(['page', 'view_back'])) {
                    $page_menus['stats_sep'] = [
                        'separator' => true,
                    ];
                }
                $page_menus['stats_page'] = [
    ...
    public function build_menu_1()
    {
        ...
        if (check_user_perm('stats', 'list')) {	// Permission to view stats for user's blogs:
            ...
            if (check_user_perm('spamblacklist', 'view')) { // Display IP ranges only if current user has access to view Antispam tools
                $ips_entries['ranges'] = [
                    'text' => T_('IP Ranges'),
            ...
            if (check_user_perm('stats', 'view') ||
                autoselect_blog('stats', 'view')) { // Viewing aggregate + Permission to view stats for ALL blogs:
                $AdminUI->add_menu_entries(
                    'stats',
                    [
                        'goals' => [
