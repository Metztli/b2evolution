...
if (! $dtyp_normal && ! $dtyp_searcheng && ! $dtyp_aggregator && ! $dtyp_email && ! $dtyp_unknown) {	// Set default status filters:
    $dtyp_normal = 1;
    $dtyp_searcheng = 1;
    $dtyp_aggregator = 1;
    $dtyp_email = 1;
...
if (empty($blog)) { // Page title when we show domains for all blogs
    $page_title = T_('All referring domains');
} else { // Page title for selected blog domains
...
$selected_agnt_types = [];
if ($dtyp_normal) {
    $selected_agnt_types[] = "'normal'";
}
if ($dtyp_searcheng) {
    $selected_agnt_types[] = "'searcheng'";
}
if ($dtyp_aggregator) {
    $selected_agnt_types[] = "'aggregator'";
}
if ($dtyp_email) {
    $selected_agnt_types[] = "'email'";
}
if ($dtyp_unknown) {
    $selected_agnt_types[] = "'unknown'";
}
$SQL->WHERE('dom_type IN ( ' . implode(', ', $selected_agnt_types) . ' )');
if (count($selected_agnt_types) != 5) {
...
if (check_user_perm('stats', 'edit')) { // Current user has a permission to create new domain
    global $tab_from;
...
if (get_param('ctrl') == 'antispam') { // Set url when we view this page from antispam controller
    $current_url = $admin_url . '?ctrl=antispam&amp;tab3=domains';
} else { // Default url for stats controller
...
if ($tab3 == 'top') {	// Display the hit counts only for "top" tab:
...
$Results->cols[] = [
    ...
    'extra' => [
        'style' => 'background-color: %stats_dom_status_color( "#dom_status#" )%;',
        'format_to_output' => false,
    ],
];

if ($tab3 == 'top') { // Display the hit counts
    $Results->cols[] = [
        'th' => T_('Hit count'),
        'order' => 'hit_count',
        'td_class' => 'right',
...
if (check_user_perm('stats', 'edit')) { // Check permission to edit Domains:
