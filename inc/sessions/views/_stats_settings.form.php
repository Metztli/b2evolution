...
$Form->checklist(
    [
        ['log_public_hits', 1, TB_('on every public page'), $Settings->get('log_public_hits')],
        ['log_admin_hits', 1, TB_('on every admin page'), $Settings->get('log_admin_hits')]],
    'log_hits',
    TB_('Log hits')
);
...
$Form->radio_input(
    ...
    $Settings->get('auto_prune_stats_mode'),
    [
        [
            'value' => 'off',
            'label' => TB_('Never'),
    ...
    TB_('Aggregate & Prune'),
    [
        'note' => TB_('Note: Even if you don\'t log hits, you still need to prune sessions!'),
        'lines' => true,
    ]
);
...
if ($Settings->get('auto_prune_stats_mode') == 'off') { // hide the "days" input field, if mode set to off:
