...
foreach ($goal_rows as $goal_row) {
    $diagram_columns[$goal_row->goal_ID] = [
        'title' => $goal_row->goal_name,
        'color' => ltrim($goal_row->gcat_color, '#'),
        'link_data' => [$goal_row->goal_name],
    ];
}
...
foreach ($goal_rows as $goal_row) { // For each named goal, display name:
    $Table->cols[] = [
...
if (empty($hitgroup_array)) { // No records
    $Table->total_pages = 0;
}
...
if ($Table->total_pages > 0) { // Display table
    ...
    foreach ($hitgroup_array as $day => $hitday_array) {
        ...
        echo $day;
        if ($is_live_mode && check_user_perm('stats', 'edit')) {	// Display a link to prune goal hits only for live data and if current user has a permission:
        ...
        $line_total = 0;
        foreach ($goal_rows as $goal_row) { // For each named goal, display count:
    ...
    $all_total = 0;
    foreach ($goal_rows as $goal_row) { // For each named goal, display total of count:
