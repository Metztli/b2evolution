...
$diagram_columns = [
    ...
    'direct' => [
        'title' => T_('Direct accesses'),
        'link_data' => ['direct',  ''],
    ],
    'self' => [
        'title' => T_('Self referred'),
        'link_data' => ['self',    ''],
    ],
    'ajax' => [
        'title' => T_('Ajax'),
    ...
    'special' => [
        'title' => T_('Special referrers'),
        'link_data' => ['special', ''],
    ],
    'spam' => [
        'title' => T_('Referer spam'),
        'link_data' => ['spam',    ''],
    ],
    'admin' => [
        'title' => T_('Admin'),
...
[$res_hits, $sessions] = get_hits_results_browser($hits_summary_mode);
...
if (count($res_hits)) {
    ...
    display_hits_diagram('browser', $diagram_columns, [$res_hits, $sessions]);

    if (! $is_live_mode) {	// Display diagram to compare hits:
        display_hits_filter_form('compare', $diagram_columns);
        [$prev_res_hits, $prev_sessions] = get_hits_results_browser('compare');
        display_hits_diagram('browser', $diagram_columns, [$prev_res_hits, $prev_sessions], 'cmpcanvasbarschart');
    }
    ...
    foreach ($res_hits as $row_stats) {
        ...
        if ($last_date != $this_date) {	// We just hit a new day, let's display the previous one:
            ...
            $is_live_data = true;
            if (! $is_live_mode) {	// Check only for "Aggregate data":
            ...
            if ($is_live_mode && check_user_perm('stats', 'edit')) {	// Display a link to prune hits only for live data and if current user has a permission:
    ...
    if ($last_date != 0) { // We had a day pending:
        $this_date = mktime(0, 0, 0, $row_stats['month'], $row_stats['day'], $row_stats['year']);
