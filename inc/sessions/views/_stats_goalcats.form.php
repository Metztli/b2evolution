...
global $action;
$creating = in_array($action, ['cat_new', 'cat_copy', 'cat_create', 'cat_create_new', 'cat_create_copy']);
...
$Form->text_input('gcat_name', $edited_GoalCategory->name, 40, TB_('Name'), '', [
    'maxlength' => 50,
    'required' => true,
]);

$Form->color_input('gcat_color', $edited_GoalCategory->color, TB_('Color'), '', [
    'required' => true,
]);

if ($creating) {
    $Form->end_form([['submit', 'actionArray[cat_create]', TB_('Record'), 'SaveButton'],
        ['submit', 'actionArray[cat_create_new]', TB_('Record, then Create New'), 'SaveButton'],
        ['submit', 'actionArray[cat_create_copy]', TB_('Record, then Create Similar'), 'SaveButton']]);
} else {
    $Form->end_form([['submit', 'actionArray[cat_update]', TB_('Save Changes!'), 'SaveButton']]);
}
