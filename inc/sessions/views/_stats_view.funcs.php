 ...
 * @param array Params
 */
function hits_results(&$Results, $params = [])
{
    $params = array_merge([
        'default_order' => '--D',
    ], $params);
    ...
    $Results->cols[] = [
        ...
        'td' => '$hit_agent_type$',
        'extra' => [
            'style' => 'background-color: %hit_agent_type_color( "#hit_agent_type#" )%;',
    ...
    $Results->cols[] = [
        ...
        'td' => '$sess_device$',
        'extra' => [
            'style' => 'background-color: %hit_device_color( "#sess_device#" )%;',
            'format_to_output' => false,
        ],
    ];

    if (! in_array('referer', $hide_columns)) { // Referer Type & Domain
        $Results->cols[] = [
            'th_group' => T_('Referer'),
            'th' => T_('Type'),
            ...
            'td' => '$hit_referer_type$',
            'extra' => [
                'style' => 'background-color: %hit_referer_type_color( "#hit_referer_type#" )%;',
                'format_to_output' => false,
            ],
        ];
    ...
    $Results->cols[] = [
        ...
        'td' => '$hit_type$',
        'extra' => [
            'style' => 'background-color: %hit_type_color( "#hit_type#" )%',
            'format_to_output' => false,
        ],
    ];
...
function filter_hits(&$Form)
{
    ...
    $Form->text_input('remote_IP', get_param('remote_IP'), 15, T_('Remote IP'), '', [
        'maxlength' => 23,
    ]);
    ...
    if (! isset($preset_agent_type)) {
        $Form->select_input_array('agent_type', get_param('agent_type'), $agent_type_array, T_('Agent type'), '', [
            'force_keys_as_values' => true,
            'background_color' => $agent_type_color,
        ]);
    }
    ...
    $Form->select_input_array('device', get_param('device'), $devices_array, T_('Device'), '', [
        'force_keys_as_values' => true,
        'background_color' => $user_devices_color,
    ]);

    if (! isset($preset_referer_type)) {
        $Form->select_input_array('referer_type', get_param('referer_type'), $referer_type_array, T_('Referer type'), '', [
            'force_keys_as_values' => true,
            'background_color' => $referer_type_color,
        ]);
    }
    $Form->select_input_array('hit_type', get_param('hit_type'), $hit_type_array, T_('Hit type'), '', [
        'force_keys_as_values' => true,
        'background_color' => $hit_type_color,
    ]);

    $Form->text_input('reqURI', get_param('reqURI'), 15, T_('Requested URI'), '', [
        'maxlength' => 250,
    ]);
    ...
    $Form->select_input_array('resp_code', get_param('resp_code'), $resp_codes, T_('HTTP resp'), '', [
        'force_keys_as_values' => true,
    ]);
}
...
function stats_format_req_URI($hit_coll_ID, $hit_uri, $max_len = 40, $hit_disp = null, $hit_ctrl = null, $hit_action = null)
{
    ...
    if ($tmp_Blog = &$BlogCache->get_by_ID($hit_coll_ID, false, false)) {	// Use root url of the requested collection if it still exists in DB:
        $full_url = $tmp_Blog->get_baseurl_root() . $hit_uri;
    } else {	// Don't use root url if a request was without collection or it doesn't exist in DB anymore:
        $full_url = $hit_uri;
    }

    $int_search_uri = urldecode($hit_uri);
    if ((utf8_strpos($int_search_uri, '?s=') !== false)
     || (utf8_strpos($int_search_uri, '&s=') !== false)) { // This is an internal search:
    ...
    if ($hit_disp != null || $hit_ctrl != null || $hit_action != null) {
        ...
        if ($hit_disp != null) {
            ...
            if ($hit_disp == 'single' || $hit_disp == 'page') {	// Display item slug:
...
function disp_clickable_log_IP($hit_remote_addr)
{
    ...
    if ($perm == true) {
        return '<a href="?ctrl=stats&tab=' . get_param('tab') . '&colselect_submit=Filter+list&sess_ID=&remote_IP=' . $hit_remote_addr . '&blog=0">' . $hit_remote_addr . '</a>'
                ...
                . get_icon('magnifier', 'imgtag', [
                    'title' => T_('Check domain registration (WHOIS)...'),
                ]) . '</a>';
    } else {
...
{
    if ($hit_response_code >= 500) {	// Server errors:
        return 'text-danger';
    } elseif ($hit_response_code >= 400) {	// Code errors:
        return 'text-warning';
    } elseif ($hit_response_code == 304) {	// 304 means "Not Modified"; Display this as success 2xx codes:
        return 'text-success';
    } elseif ($hit_response_code >= 300) {	// Redirects:
        return 'text-info';
    } elseif ($hit_response_code >= 200) {	// Success pages:
        return 'text-success';
    }
...
function hit_iprange_status($IP_address)
{
    ...
    if (! is_array($hit_iprange_statuses_cache)) { // Initialize it only first time
        $SQL = new SQL();
    ...
    foreach ($hit_iprange_statuses_cache as $hit_iprange_status) {
        if ($IP_address >= $hit_iprange_status->start &&
            $IP_address <= $hit_iprange_status->end) { // IP is detected in this range
...
function hit_iprange_status_title($IP_address)
{
    ...
    if ($ip_range_status === '') { // No IP range for this IP address
        if (check_user_perm('spamblacklist', 'edit')) { // Display a link to create new one if user has an access
            return '<a href="' . $admin_url . '?ctrl=antispam&amp;tab3=ipranges&amp;action=iprange_new&amp;ip=' . $IP_address . '">' . T_('Create') . '</a>';
        } else { // No access to create new IP range
            return '';
        }
    }

    if (check_user_perm('spamblacklist', 'view')) { // Current user has access to view IP ranges
        global $blog;
    ...
    } else { // No view access, Display only the status
...
function hit_method_style($hit_method)
{
    ...
    if (isset($hit_method_color[$hit_method])) {	// Get background color from config array:
...
function get_hits_results_global($mode = 'live')
{
    ...
    if ($mode == 'live') {	// Get the live data:
        ...
        $SQL->FROM('T_hitlog');
        if ($blog > 0) {	// Filter by collection:
    ...
    } else {	// Get the aggregated/compared data:
...
function get_hits_results_browser($mode = 'live')
{
    ...
    if ($mode == 'live') {	// Get the live data:
        ...
        } else {	// Get ALL aggregated sessions:
...
function display_hits_diagram($type, $diagram_columns, $res_hits, $canvas_id = 'canvasbarschart')
{
    ...
    if (isset($diagram_columns['session'])) {	// Draw last data as line only for Sessions:
    ...
    foreach ($res_hits as $row_stats) {
        ...
        if ($last_date != $this_date) {	// We just hit a new day, let's display the previous one:
        ...
        switch ($type) {
            case 'global':
                if ($row_stats['hit_agent_type'] == 'unknown') {	// only those hits are calculated which hit_agent_type = unknown
                    $hit_key = $row_stats['hit_agent_type'];
                } elseif (! empty($col_mapping[$row_stats['hit_type'] . '_' . $row_stats['hit_agent_type']])) {	// those hits are calculated here if hit_type = standard and hit_agent_type = browser, robot
                    $hit_key = $row_stats['hit_type'] . '_' . $row_stats['hit_agent_type'];
                } elseif (! empty($col_mapping[$row_stats['hit_type']])) {	// those hits are calculated here which did not match either of the above rules
            ...
            case 'browser':
                $hit_key = in_array($row_stats['hit_type'], ['ajax', 'admin']) ? $row_stats['hit_type'] : $row_stats['referer_type'];
                break;
