...
if ($c > 0) {	// Filter by country:
    $sql_where[] = 'ctry_ID = "' . $DB->escape($c) . '"';
}
if ($r > 0) {	// Filter by region:
    $sql_where[] = 'subrg_rgn_ID = "' . $DB->escape($r) . '"';
}

if (count($sql_where) > 0) {	// Some filters are applied
...
function filter_subregions(&$Form)
{
    ...
    $Form->select_country('c', get_param('c'), $CountryCache, T_('Country'), [
        'allow_none' => true,
    ]);
...
if (check_user_perm('options', 'edit', false)) {
    ...
    $Results->global_icon(
        T_('Create a new sub-region...'),
        'new',
        regenerate_url('action', 'action=new'),
        T_('New sub-region') . ' &raquo;',
        3,
        4,
        [
    ...
    $Results->global_icon(
        T_('Import sub-regions from CSV file ...'),
        'new',
        regenerate_url('action', 'action=csv'),
        T_('Import CSV') . ' &raquo;',
        3,
        4
    );
