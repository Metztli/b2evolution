...
if (param('rgn_ID', 'integer', '', true)) {// Load region from cache:
...
switch ($action) {
    ...
    case 'enable_region':
        ...
        if ($action == 'disable_region') {	// Disable this region by setting flag to false.
            $edited_Region->set('enabled', 0);
            $Messages->add(sprintf(TB_('Disabled region (%s, #%d).'), $edited_Region->name, $edited_Region->ID), 'success');
        } elseif ($action == 'enable_region') {	// Enable region by setting flag to true.
    ...
    case 'disable_region_pref':
        ...
        if ($action == 'disable_region_pref') {	// Disable this region by setting flag to false.
        ...
        } elseif ($action == 'enable_region_pref') {	// Enable region by setting flag to true.
    ...
    case 'new':
        ...
        if (! isset($edited_Region)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record region and create similar
        ...
        if ($edited_Region->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Region->check_delete(sprintf(TB_('Cannot delete region &laquo;%s&raquo;'), $edited_Region->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
    ...
    case 'import':
        ...
        $Messages->add(sprintf(
            TB_('%s regions have been added and %s regions have been updated for country %s.'),
            $count_regions['inserted'],
            $count_regions['updated'],
            $Country->get_name()
        ), 'success');
        // Redirect so that a reload doesn't write to the DB twice:
