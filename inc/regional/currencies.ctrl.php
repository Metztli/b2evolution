...
if (param('curr_ID', 'integer', '', true)) {// Load currency from cache:
...
switch ($action) {
    ...
    case 'enable_currency':
        ...
        if ($action == 'disable_currency') {	// Disable this currency by setting flag to false.
        ...
        } elseif ($action == 'enable_currency') {	// Enable currency by setting flag to true.
    ...
    case 'new':
        ...
        if (! isset($edited_Currency)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record currency and create similar
        ...
        if ($edited_Currency->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Currency->check_delete(sprintf(TB_('Cannot delete currency &laquo;%s&raquo;'), $edited_Currency->dget('name')), [], true)) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
