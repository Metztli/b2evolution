...
if (param('city_ID', 'integer', '', true)) {// Load city from cache:
...
switch ($action) {
    ...
    case 'enable_city':
        ...
        if ($action == 'disable_city') {	// Disable this city by setting flag to false.
            $edited_City->set('enabled', 0);
            $Messages->add(sprintf(TB_('Disabled city (%s, #%d).'), $edited_City->name, $edited_City->ID), 'success');
        } elseif ($action == 'enable_city') {	// Enable city by setting flag to true.
    ...
    case 'disable_city_pref':
        ...
        if ($action == 'disable_city_pref') {	// Disable this city by setting flag to false.
        ...
        } elseif ($action == 'enable_city_pref') {	// Enable city by setting flag to true.
    ...
    case 'new':
        ...
        if (! isset($edited_City)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record city and create similar
        ...
        if ($edited_City->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
            $DB->begin();
    ...
    case 'update':
        ...
        if ($edited_City->load_from_Request()) {	// We could load data from form without errors:
            ...
            if (empty($q)) {	// If no error, Redirect so that a reload doesn't write to the DB twice:
                header_redirect('?ctrl=cities', 303); // Will EXIT
                // We have EXITed already at this point!!
            }
        }
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_City->check_delete(sprintf(TB_('Cannot delete city &laquo;%s&raquo;'), $edited_City->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
