...
if (in_array($action, ['enable_country', 'disable_country', 'enable_country_pref', 'disable_country_pref'])) {
...
if (param('ctry_ID', 'integer', '', true)) {// Load country from cache:
...
switch ($action) {
    ...
    case 'enable_country':
        ...
        if ($action == 'disable_country') {	// Disable this country by setting flag to false.
            $edited_Country->set('enabled', 0);
            $Messages->add(sprintf(TB_('Disabled country (%s, #%d).'), $edited_Country->name, $edited_Country->ID), 'success');
        } elseif ($action == 'enable_country') {	// Enable country by setting flag to true.
    ...
    case 'disable_country_pref':
        ...
        if ($action == 'disable_country_pref') {	// Disable this country by setting flag to false.
        ...
        } elseif ($action == 'enable_country_pref') {	// Enable country by setting flag to true.
    ...
    case 'new':
        ...
        if (! isset($edited_Country)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record country and create similar
        ...
        if ($edited_Country->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Country->check_delete(sprintf(TB_('Cannot delete country &laquo;%s&raquo;'), $edited_Country->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
...
if (empty($action)) { // JS to edit status of countries from list view
