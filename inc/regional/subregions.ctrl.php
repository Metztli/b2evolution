...
if (param('subrg_ID', 'integer', '', true)) {	// Load subregion from cache:
...
switch ($action) {
    ...
    case 'enable_subregion':
        ...
        if ($action == 'disable_subregion') {	// Disable this subregion by setting flag to false.
        ...
        } elseif ($action == 'enable_subregion') {	// Enable subregion by setting flag to true.
    ...
    case 'disable_subregion_pref':
        ...
        if ($action == 'disable_subregion_pref') {	// Disable this subregion by setting flag to false.
        ...
        } elseif ($action == 'enable_subregion_pref') {	// Enable subregion by setting flag to true.
    ...
    case 'new':
        ...
        if (! isset($edited_Subregion)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record subregion and create similar
        ...
        if ($edited_Subregion->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Subregion->check_delete(sprintf(TB_('Cannot delete sub-region &laquo;%s&raquo;'), $edited_Subregion->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
    ...
    case 'import':
        ...
        if (empty($import_file)) {	// File is not selected:
            $Messages->add(TB_('Please select a CSV file to import.'), 'error');
        } elseif (! preg_match('/\.csv$/i', $import_file)) {	// Extension is incorrect
            $Messages->add(sprintf(TB_('&laquo;%s&raquo; has an unrecognized extension.'), basename($import_file)), 'error');
        }

        if (param_errors_detected()) {	// Some errors are exist, Stop the importing:
            $action = 'csv';
            break;
        }
        ...
        $Messages->add(sprintf(
            TB_('%s sub-regions have been added and %s sub-regions have been updated for country %s.'),
            $count_subregions['inserted'],
            $count_subregions['updated'],
            $Country->get_name()
        ), 'success');

        if ($count_subregions['regions'] > 0) {	// Inform when at least one region has been created automatically:
            $Messages->add(sprintf(
                TB_('%s regions have been automatically created for country %s.'),
                $count_subregions['regions'],
                $Country->get_name()
            ), 'success');
        }
