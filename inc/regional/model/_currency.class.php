...
class Currency extends DataObject
{
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                'table' => 'T_regional__country',
                'fk' => 'ctry_curr_ID',
                'msg' => T_('%d related countries'),
            ],
        ];
    }
    ...
    public function load_from_Request()
    {
        ...
        if (! param_errors_detected()) {	// Check currency code for duplicating:
            $existing_curr_ID = $this->dbexists('curr_code', $this->get('code'));
            if ($existing_curr_ID) {	// We have a duplicate currency:
                param_error(
                    'curr_code',
                    sprintf(
                        T_('This currency already exists. Do you want to <a %s>edit the existing currency</a>?'),
    ...
    public function get_restriction_link($restriction)
    {
        ...
        if ($restriction['fk'] != 'ctry_curr_ID') { // currency restriction exists only for countries
