...
class Region extends DataObject
{
    ...
    public static function get_delete_restrictions()
    {
        return [
            ...
            [
                'table' => 'T_regional__subregion',
                'fk' => 'subrg_rgn_ID',
                'msg' => T_('%d related sub-regions'),
            ],
            [
    ...
    public function load_from_Request()
    {
        ...
        if (! param_errors_detected()) {	// Check region code for duplicating:
            $existing_rgn_ID = $this->dbexists(['rgn_ctry_ID', 'rgn_code'], [$this->get('ctry_ID'), $this->get('code')]);
            if ($existing_rgn_ID) {	// We have a duplicate region:
                param_error(
                    'rgn_code',
                    sprintf(
                        T_('This region already exists. Do you want to <a %s>edit the existing region</a>?'),
