...
class Subregion extends DataObject
{
    ...
    public function __construct($db_row = null)
    {
        ...
        if ($db_row) {
            ...
            if (! empty($this->rgn_ID)) {
                if (($Region = &$RegionCache->get_by_ID($db_row->subrg_rgn_ID, false)) !== false) {	// Get country ID
    ...
    public function load_from_Request()
    {
        ...
        if (! param_errors_detected()) {	// Check sub-region code for duplicating:
            $existing_subrg_ID = $this->dbexists(['subrg_rgn_ID', 'subrg_code'], [$this->get('rgn_ID'), $this->get('code')]);
            if ($existing_subrg_ID) {	// We have a duplicate sub-region:
                param_error(
                    'subrg_code',
                    sprintf(
                        T_('This sub-region already exists. Do you want to <a %s>edit the existing sub-region</a>?'),
