...
{
    // Name / Code:
    $Form->text_input('q', get_param('q'), 20, T_('Name / Code'), '', [
        'maxlength' => 50,
    ]);

    // Context:
    $field_options = [
        null => T_('All contexts'),
    ] + get_template_contexts('raw');
    $Form->select_input_array('context', get_param('context'), $field_options, T_('Context'), '', [
    ...
    $Form->select_input_array('owner', get_param('owner'), $field_options, T_('Owner'), '', [
...
$contexts = get_template_contexts('raw', ['custom1', 'custom2', 'custom3']);
...
if (check_user_perm('options', 'edit')) {
    ...
    $Results->global_icon(T_('New template'), 'new', regenerate_url('action', 'action=new'), T_('New template') . ' &raquo;', 3, 4, [
        'class' => 'action_icon btn-primary',
    ]);
}

// Highlight rows:
$highlight_fadeout = empty($highlight) ? [] : [
    'tpl_code' => [$highlight],
];
