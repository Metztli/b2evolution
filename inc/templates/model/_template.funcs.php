 ...
 */
function render_template_code($code, &$params, $objects = [], &$used_template_tags = null)
{
    $TemplateCache = &get_TemplateCache();
    if ($Template = &$TemplateCache->get_localized_by_code($code, false, false)) {	// Render variables in available Template:
 ...
 */
function render_template($template, &$params, $objects = [], &$used_template_tags = null)
{
    $current_pos = 0;
    $r = '';
    ...
    foreach ($matches[0] as $i => $match) {
        ...
        if (substr($tag, 0, 4) == 'set:' || substr($tag, 0, 6) == 'param:') {	// Set new or default param value in the $params[] array used for the whole template (will affect all future template tags)
            ...
            foreach ($param_strings as $param_string) {
                if (empty($param_string) || ctype_space($param_string)) {	// Nothing here, ignore:
                    continue;
                }

                $param_name = substr($param_string, 0, strpos($param_string, '='));

                if (empty($param_name)) {	// Does not contain a param name, ignore:
                    continue;
                }

                if (strpos($param_name, '//') !== false) {	// We found a comment that we should remove:
                ...
                if ($override_param || ! isset($params[$param_name])) {	// Set new param or default param:
 ...
 */
function get_template_tag_params_from_string($tag_param_strings, $default_params = [])
{
    ...
    foreach ($tag_param_strings as $tag_param_string) {
        ...
        if (strpos($tag_param_name, '//') !== false) {	// We found a comment that we should remove:
        ...
        if ($tag_param_name === '') {	// Skip empty param:
            continue;
        }

        $tag_param_val = substr($tag_param_string, strpos($tag_param_string, '=') + 1);

        if (preg_match('/\$([a-z_]+)\$/i', $tag_param_val, $tag_param_val_matches)) {	// We have a variable to replace: // TODO: allow multiple variable replace
            $found_param_name = $tag_param_val_matches[1];
            if (isset($default_params[$found_param_name])) {	// We have an original param of that name:
 ...
 */
function render_template_callback($var, $params, $objects = [])
{
    ...
    switch ($scope) {
        ...
        case 'echo':
            ...
            if (! isset($params[$param_name])) {	// Param is not found:
                return get_rendering_error('Param <code>' . $param_name . '</code> is not passed.', 'span');
            } elseif (! is_scalar($params[$param_name])) {	// Param is not scalar and cannot be printed on screen:
    ...
    switch ($var) {
        ...
        case 'Cat:image':
            echo $rendered_Chapter->get_image_tag(array_merge([
        ...
        case 'Cat:permalink':
            echo $rendered_Chapter->get_permanent_link(array_merge([
                'text' => '#name',
                'title' => '',
            ], $params));
            break;
        ...
        case 'Comment:permalink':
            $rendered_Comment->permanent_link(array_merge([
                'text' => '#item#',
        ...
        case 'Form:gender':
            ...
            $rendered_Form->radio_input($temp_params['name'], $temp_params['value'], [
                [
                    'value' => 'M',
                    'label' => T_('A man'),
                ],
        ...
        case 'Form:login':
            ...
            $temp_params = [
                // Here, we make sure not to modify $params
        ...
        case 'Form:submit':
            ...
            // Item:
        case 'Item:author':
            $rendered_Item->author(array_merge([
                'link_text' => 'auto',		// select login or nice name automatically
            ], $params));
            break;
        ...
        case 'Item:categories':
            $rendered_Item->categories(array_merge([
        ...
        case 'Item:excerpt':
            $rendered_Item->excerpt(array_merge([
                'before' => '',
                'after' => '',
        ...
        case 'Item:feedback_link':
            echo $rendered_Item->get_feedback_link(array_merge([
        ...
        case 'Item:footer':
            echo $rendered_Item->footer(array_merge([
                // Here, we make sure not to modify $params
        ...
        case 'Item:history_link':
            echo $rendered_Item->get_history_link(array_merge([
        ...
        case 'Item:images':
            echo $rendered_Item->get_images(array_merge([
        ...
        case 'Item:lastedit_user':
            $rendered_Item->lastedit_user(array_merge([
                'link_text' => 'auto',		// select login or nice name automatically
            ], $params));
            break;
        ...
        case 'Item:more_link':
            ...
            echo $rendered_Item->get_more_link(array_merge([
        ...
        case 'Item:page_links':
            echo $rendered_Item->get_page_links(array_merge([
                'separator' => '&middot; ',
            ], $params));
            break;
        ...
        case 'Item:permanent_link':
            $rendered_Item->permanent_link(array_merge([
                'text' => '#title',
                'title' => '',  // No tooltip by default
                // Note: Widget MAY have set the following for same CAT navigation:
                //	'post_navigation' => 'same_category',			// Stay in the same category if Item is cross-posted
        ...
        case 'Item:propose_change_link':
            $rendered_Item->propose_change_link(array_merge([
                'text' => T_('Propose a change'),
            ], $params));
            break;

        case 'Item:read_status':
            $rendered_Item->display_unread_status(array_merge([
                'style' => 'text',
        ...
        case 'Item:tags':
            $rendered_Item->tags(array_merge([
        ...
        case 'Link:disp':
            ...
            switch ($temp_params['disp']) {
                case 'login':
                    ...
                    $disp_url = get_login_url(
                        $temp_params['source'],
                        $temp_params['redirect_to'],
                        $temp_params['force_normal_login'],
                        $temp_params['blog_ID'],
                        $temp_params['blog_page']
                    );
                    break;
        ...
        case 'User:custom':
            ...
            if (is_array($userfield_values)) {	// We expect $userfield_values to be an Array:
        ...
        case 'User:picture':
            ...
            echo $rendered_User->get_avatar_imgtag(
                $temp_params['size'],
                $temp_params['class'],
                $temp_params['align'],
                $temp_params['zoomable'],
                $temp_params['avatar_overlay_text'],
                $temp_params['lightbox_group'],
                $temp_params['tag_size'],
                $temp_params['protocol']
            );
            break;
    ...
    if ($match_found) {
        return $r;
    } else {	// Display error for not recognized variable:
...
function unique_template_code($code, $ID = 0, $db_code_fieldname = 'tpl_code', $db_ID_fieldname = 'tpl_ID', $db_table = 'T_templates')
{
    ...
    foreach ($DB->get_results($SQL->get(), ARRAY_A) as $row) {
        ...
        if (($existing_code == $code) && ($row[$db_ID_fieldname] != $ID)) {	// Specified code already in use by another template, we'll have to change the number.
            $exact_match = true;
        }
        if (preg_match('/_([0-9]+)$/', $existing_code, $matches)) {	// This template code already has a number, we extract it:
            $existing_number = (int) $matches[1];

            if (! isset($use_existing_number) && $row[$db_ID_fieldname] == $ID) {	// if there is a numbered entry for the current ID, use this:
    ...
    if ($exact_match) {	// We got an exact (existing) match, we need to change the number:
 ...
 */
function get_template_contexts($format = 'keys', $exclude = [])
{
    $template_contexts = [
