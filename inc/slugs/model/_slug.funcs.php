...
function generate_tinyurl($last_url = null)
{
    ...
    */
    ...
    if (! $last_url && $Settings != null) { // last_url is not set
        $last_url = $Settings->get('tinyurl');
    }

    if (! $last_url) { // no tinyurl is set, used the default value for the first one
        $last_url = 'aA0';
        return $last_url;
    }
    ...
    while ($index) {
        ...
        $char_value = ord($char);
        if ($index <= 3) { // we have to change one of the first three character
            if ($char_value >= 65) { // the character is a letter
                if ($char_value >= 97) { // the character is a lowercase letter
                    if ($char_value < 122) { // there is still unused lowercase letter
                        $char_value++;
        ...
        } else { // we change a character after the third position
            if ($char_value >= 48 && $char_value <= 57) {
                if ($char_value != 57) {
                    $char_value++;
                } else {
                    if (($index - 3) % 4 == 0) { // every fourth character must be a number
                        $newurl_ending = chr(48) . $newurl_ending;
                        $index--;
                        continue;
                    } else {
            ...
            if ($char_value == 122) { // no more variation we need to shift to another position
                if (($index - 3) % 4 == 3) { // every fourth character must be a letter
                    $newurl_ending = chr(65) . $newurl_ending;
                } else {
                    $newurl_ending = chr(48) . $newurl_ending;
                }
    ...
    if ($index) { // index is not null we change the last_url ending part
...
function getnext_tinyurl()
{
    ...
    $url = null;
    do { // create a new tinyurl, until it is unique
