...
if (! isset($fieldset_prefix)) {	// Define default fieldset prefix:
...
if ($Results->total_pages == 0) { // If no results we should get a template of headers in order to add it on first quick upload
    ob_start();
    $Results->display_col_headers();
    $table_headers = ob_get_clean();
} else { // Headers are already on the page
    $table_headers = '';
}
...
if (! isset($attachment_tab)) {
    ...
    if (is_ajax_request()) {
        ...
        expose_var_to_js('link_initialize_fieldset_' . $fieldset_prefix, [
            'fieldset_prefix' => $fieldset_prefix,
        ], 'evo_link_initialize_fieldset_config');
    }
}
?>
