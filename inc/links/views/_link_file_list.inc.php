...
if ($ea_Linklist->get_total_rows() > 0) {	// Display table headers only when at least file is found in the selected folder and filter:
    echo $table_headers;
}
?>
...
/***********************************************************/
$countFiles = 0;
while ($lLink = &$ea_Linklist->get_next()) {	// Loop through all Files:
    $lFile = &$lLink->get_File();

    if ($lFile->is_dir()) {	// Skip directories:
        continue;
    }

    $lLinkOwner = &$lLink->get_LinkOwner();
    $row_class = [];
    if ((get_class($lLinkOwner->link_Object) == 'Comment') && $lLinkOwner->link_Object->is_meta()) {	// Show different background color for internal comments:
        $row_class[] = 'bg-info';
    }
    ...
    echo file_td_actions($lFile, ['move', 'copy', 'delete']);
    echo '</td>';
    evo_flush();
