...
function &get_LinkOwner($link_type, $object_ID)
{
    ...
    if (empty($object_ID)) {	// ID must be defined to get Link Owner:
        return $LinkOwner;
    }
    ...
    switch ($link_type) {
        case 'item':
            ...
            if ($Item = &$ItemCache->get_by_ID($object_ID, false)) {	// If Item is found in DB by ID:
        ...
        case 'comment':
            ...
            if ($Comment = &$CommentCache->get_by_ID($object_ID, false)) {	// If Comment is found in DB by ID:
        ...
        case 'user':
            ...
            if ($User = &$UserCache->get_by_ID($object_ID, false)) {	// If User is found in DB by ID:
        ...
        case 'message':
            ...
            if ($Message = &$MessageCache->get_by_ID($object_ID, false)) {	// If Message is found in DB by ID:
        ...
        case 'temporary':
            ...
            if ($TemporaryID = &$TemporaryIDCache->get_by_ID($object_ID, false)) {	// If TemporaryID is found in DB by ID:
                switch ($TemporaryID->get('type')) {
                    ...
                    case 'metacomment':
                        ...
                        $Comment = new Comment();
                        if ($TemporaryID->get('type') == 'metacomment') {	// Set comment type to meta to ensure correct permissions:
                            $Comment->type = 'meta';
                        }
...
function display_attachments_fieldset(&$Form, &$LinkOwner, $fold = false, $fieldset_prefix = '')
{
    ...
    if (! isset($GLOBALS['files_Module'])) {	// Files module is not enabled:
        return;
    }

    if (! $LinkOwner->check_perm('edit', false)) {	// Current user has no perm to edit the link owner:
        return;
    }
    ...
    switch ($LinkOwner->type) {
        case 'item':
            if ($LinkOwner->is_temp()) {
                ...
                if (! $LinkOwner->Item->check_proposed_change_restriction()) {	// Display overlay if the Item has a restriction by existing proposed change:
    ...
    if (is_admin_page()) {	// Display a link to manual page only on back-office:
        $fieldset_title .= ' ' . get_manual_link('images-attachments-panel');
    }

    if (check_user_perm('admin', 'restricted') && check_user_perm('files', 'view')) {	// Check if current user has a permission to back-office files manager:
        ...
        if ($linkowner_FileList = $LinkOwner->get_attachment_FileList(1)) {	// Get first file of the Link Owner:
            $linkowner_File = &$linkowner_FileList->get_next();
            if (! empty($linkowner_File) && check_user_perm('files', 'view', false, $linkowner_File->get_FileRoot())) {	// Obtain and use file root of first file:
        ...
        $fieldset_title .= ' - '
            . action_icon(
                ...
                4,
                [
                    'onclick' => 'return link_attachment_window( \'' . ($LinkOwner->is_temp() ? 'temporary' : $LinkOwner->type) . '\', \'' . $LinkOwner->get_ID() . '\', \'\', \'\', \'\', \'' . $fieldset_prefix . '\' )',
                ]
            );
        if (! $LinkOwner->is_temp()) {	// Don't allow this option for new creating objects:
    ...
    $fieldset_title .= '<span class="floatright panel_heading_action_icons">&nbsp;'
            ...
            . action_icon(
                ...
                [
                    ...
                    'onclick' => 'return evo_link_refresh_list( \'' . ($LinkOwner->is_temp() ? 'temporary' : $LinkOwner->type) . '\', \'' . $LinkOwner->get_ID() . '\', \'refresh\', \'' . $fieldset_prefix . '\' )',
                ]
            )
            ...
            . action_icon(
                T_('Sort'),
                'ascending',
                (is_admin_page() || check_user_perm('admin', 'restricted'))
                ...
                [
                    'class' => 'action_icon btn btn-default btn-sm',
                    'onclick' => 'return evo_link_refresh_list( \'' . ($LinkOwner->is_temp() ? 'temporary' : $LinkOwner->type) . '\', \'' . $LinkOwner->get_ID() . '\', \'sort\', \'' . $fieldset_prefix . '\' )',
                ]
            )
    ...
    if (! empty($restriction_overlay)) {	// Restrict attachments with overlay:
    ...
    if (is_ajax_request()) {
        ...
        expose_var_to_js('fieldset_' . $fieldset_prefix . $form_id, [
            'fieldset_prefix' => $fieldset_prefix,
            'form_id' => $form_id,
        ], 'evo_display_attachments_fieldset_config');
    }

    if (check_user_perm('admin', 'restricted') && check_user_perm('files', 'view') && empty($restriction_overlay)) {	// Check if current user has a permission to back-office files manager:
        // Initialize JavaScript to build and open window:
        echo_modalwindow_js();

        $link_attachment_window_config = [
 ...
 */
function display_attachments(&$LinkOwner, $params = [])
{
    global $redirect_to;
    ...
    if (count($links) < 1) { // there are no attachments
        return;
    }
    ...
    echo '</thead><tbody>';
    $row_style = '';
    foreach ($links as $Link) { // display each link attachment in a row
        if (! ($link_File = &$Link->get_File())) { // No File object
            global $Debuglog;
            $Debuglog->add(sprintf('Link ID#%d does not have a file object!', $Link->ID), ['error', 'files']);
            continue;
        }
        ...
        if (check_user_perm('files', 'edit')) { // display delete link action
...
function link_destination()
{
    ...
    if ($current_File->is_dir()) { // Directory
...
function link_actions($link_ID, $row_idx_type = '', $link_type = 'item')
{
    ...
    // Change order.
    if ($LinkOwner->check_perm('edit')) { // Check that we have permission to edit LinkOwner object:
        // Allow to move up all rows except of first, This action icon is hidden by CSS for first row
        $r .= action_icon(
            T_('Move upwards'),
            'move_up',
            $admin_url . '?ctrl=links&amp;link_ID=' . $link_ID . '&amp;action=link_move_up' . $blog_param . '&amp;' . url_crumb('link'),
            null,
            null,
            null,
            [
                'class' => 'action_icon_link_move_up',
        ...
        $r .= ' ' . action_icon(
            T_('Move down'),
            'move_down',
            $admin_url . '?ctrl=links&amp;link_ID=' . $link_ID . '&amp;action=link_move_down' . $blog_param . '&amp;' . url_crumb('link'),
            null,
            null,
            null,
            [
                'class' => 'action_icon_link_move_down',
    ...
    if ($current_File && check_user_perm('files', 'view', false, $current_File->get_FileRoot())) { // Locate file
    ...
    if ($current_File &&
        ...
        check_user_perm('files', 'edit_allowed', false, $current_File->get_FileRoot())) {	// Edit file:
        ...
        $r .= ' <a href="' . $admin_url . '?ctrl=files&amp;root=' . $current_File->get_FileRoot()->ID
                ...
                . ' target="_parent" title="' . format_to_output($title, 'htmlattr') . '">'
            . get_icon('edit', 'imgtag', [
                'title' => $title,
            ]) . '</a> ';
    }

    // Unlink/Delete icons:
    if ($LinkOwner->check_perm('edit')) {	// If current user has a permission to edit LinkOwner object
        // Unlink icon:
        $r .= action_icon(
            T_('Delete this link!'),
            'unlink',
            $admin_url . '?ctrl=links&amp;link_ID=' . $link_ID . '&amp;action=unlink' . $blog_param . '&amp;' . url_crumb('link'),
            null,
            null,
            null,
            [
                'onclick' => 'return evo_link_delete( this, \'' . $LinkOwner->type . '\', ' . $link_ID . ', \'unlink\' )',
            ]
        );
        // Delete icon:
        $LinkCache = &get_LinkCache();
        $Link = &$LinkCache->get_by_ID($link_ID, false, false);
        if ($current_File && ! $current_File->is_dir() && $Link && $Link->can_be_file_deleted()) {	// If current user has a permission to delete a file(not folder) completely
            $File = &$Link->get_File();
            $r .= action_icon(
                T_('Delete this file!'),
                'delete',
                $admin_url . '?ctrl=links&amp;link_ID=' . $link_ID . '&amp;action=delete' . $blog_param . '&amp;' . url_crumb('link'),
                null,
                null,
                null,
                [
        ...
        } else {	// If current user can only unlink the attachment (probably it is linked to several objects)
            $r .= get_icon('delete', 'imgtag', [
                'class' => 'action_icon empty_placeholder',
            ]);
        }
    }
...
function display_link_position(&$row, $show_actions = true, $fieldset_prefix = '')
{
    ...
    if (count($available_positions) > 1) {	// Display a selector for link positions only if owner can has several positions:
    ...
    if ($show_actions && $current_File) {
        if (isset($available_positions['inline'])) {	// If link owner support inline position,
 ...
 */
function get_file_links($file_ID, $params = [])
{
    ...
    if (! empty($links)) { // File is linked with some posts or comments
        $ItemCache = &get_ItemCache();
        $CommentCache = &get_CommentCache();
        $UserCache = &get_UserCache();
        $EmailCampaignCache = &get_EmailCampaignCache();
        ...
        foreach ($links as $link) {
            ...
            if (! empty($link->link_itm_ID)) { // File is linked to a post
                if ($Item = &$ItemCache->get_by_ID($link->link_itm_ID, false)) {
                    $Collection = $Blog = $Item->get_Blog();
                    if (check_user_perm('item_post!CURSTATUS', 'view', false, $Item)) { // Current user can edit the linked post
                        $r .= $params['post_prefix'] . '<a href="' . url_add_param($admin_url, 'ctrl=items&amp;blog=' . $Blog->ID . '&amp;p=' . $link->link_itm_ID) . '">' . $Item->get('title') . '</a>';
                    } else { // No access to edit the linked post
                        $r .= $params['post_prefix'] . $Item->get('title');
                    }
                    $link_object_ID = $link->link_itm_ID;
                }
            } elseif (! empty($link->link_cmt_ID)) { // File is linked to a comment
                if ($Comment = &$CommentCache->get_by_ID($link->link_cmt_ID, false)) {
                    $Item = $Comment->get_Item();
                    if (check_user_perm('comment!CURSTATUS', 'moderate', false, $Comment)) { // Current user can edit the linked Comment
                    ...
                    } else { // No access to edit the linked Comment
                        $r .= $params['comment_prefix'] . $Item->get('title');
                    }
                    $link_object_ID = $link->link_cmt_ID;
                }
            } elseif (! empty($link->link_usr_ID)) { // File is linked to user
                if ($User = &$UserCache->get_by_ID($link->link_usr_ID, false)) {
                    if ($current_User->ID != $User->ID && ! check_user_perm('users', 'view')) { // No permission to view other users in admin form
                        $BlogCache = &get_BlogCache();
                        $BlogCache->load_user_blogs();
                        $user_url = '';
                        if (! empty($BlogCache->cache)) {	// Try to use alias user url:
                            foreach ($BlogCache->cache as $user_Blog) {	// Use first found collection:
                                $user_url = $user_Blog->get('userurl', [
                                    'user_ID' => $User->ID,
            ...
            } elseif (! empty($link->link_ecmp_ID)) {	// File is linked to email campaign:
                if ($EmailCampaign = &$EmailCampaignCache->get_by_ID($link->link_ecmp_ID, false)) {
                    if (! check_user_perm('emails', 'view')) {	// Build a link to display an email campaign in edit back-office form:
                    ...
                    } else {	// No permission to view email campaign in edit back-office form:
                        $r .= $params['emailcampaign_prefix'] . $EmailCampaign->get('name');
                    }
                    $link_object_ID = $link->link_ecmp_ID;
                }
            } elseif (! empty($link->link_msg_ID)) {	// File is linked to message:
                if ($Message = &$MessageCache->get_by_ID($link->link_msg_ID, false)) {
                    $Thread = &$Message->get_Thread();
                    if (! check_user_perm('perm_messaging', 'reply')) {	// Build a link to display a message in edit back-office form:
                        $r .= $params['message_prefix'] . '<a href="?ctrl=messages&thrd_ID=' . $Thread->ID . '">' . $Thread->get('title') . ' #' . $Message->ID . '</a>';
                    } else {	// No permission to view message in edit back-office form:
            ...
            if (! empty($link_object_ID)) { // Action icon to unlink file from object
                if (($edited_Link = &$LinkCache->get_by_ID($link->link_ID, false, false)) !== false &&
                    ($LinkOwner = &$edited_Link->get_LinkOwner()) !== false && $LinkOwner->check_perm('edit', false)) { // Allow to unlink only if current user has an permission
                    $r .= ' ' . action_icon(
                        T_('Delete this link!'),
                        'unlink',
                        ...
                        null,
                        [
                            'onclick' => 'return confirm(\'' . TS_('Are you sure want to unlink this file?') . '\');',
                        ]
                    );
                }
            }
...
function link_vote($link_ID, $user_ID, $vote_action, $checked = 1)
{
    ...
    } else { // Update existing record, because user already has a vote for this file:
        if ($existing_vote->value == $field_value) {	// Undo previous vote:
            $field_value = null;
        }
        $result = $DB->query(
            'UPDATE T_links__vote
...
function sort_links_by_filename($a_Link, $b_Link)
{
    ...
    if ($a_type === $b_type) {	// Compare only two equal types:
        $r = strnatcmp($a_File->_name, $b_File->_name);
    } elseif ($a_type == 'directory') {	// Directories must be before(on the top) files:
        $r = -1;
    } else {	// Files must be after(at the bottom) directories:
        $r = 1;
    }
...
function display_attachments_tab_pane(&$Form, &$LinkOwner, $fold = false, $tab_pane_prefix = '')
{
    ...
    if (is_logged_in() && $current_User->check_perm('admin', 'restricted') && $current_User->check_perm('files', 'view')) {	// Check if current user has a permission to back-office files manager:
        $attach_files_url = $admin_url . '?ctrl=files&amp;fm_mode=link_object&amp;link_type=' . ($LinkOwner->is_temp() ? 'temporary' : $LinkOwner->type) . ($LinkOwner->type != 'message' ? '&amp;link_object_ID=' . $LinkOwner->get_ID() : '');
        if ($linkowner_FileList = $LinkOwner->get_attachment_FileList(1)) {	// Get first file of the Link Owner:
            $linkowner_File = &$linkowner_FileList->get_next();
            if (! empty($linkowner_File) && $current_User->check_perm('files', 'view', false, $linkowner_File->get_FileRoot())) {	// Obtain and use file root of first file:
                $linkowner_FileRoot = &$linkowner_File->get_FileRoot();
                $attach_files_url .= '&amp;root=' . $linkowner_FileRoot->ID;
                $attach_files_url .= '&amp;path=' . dirname($linkowner_File->get_rdfs_rel_path()) . '/';
            }
        }
        $items_left .= action_icon(
            T_('Attach existing files'),
            'folder',
            $attach_files_url,
            T_('Attach existing files'),
            ...
            4,
            [
                'onclick' => 'return link_attachment_window( \'' . ($LinkOwner->is_temp() ? 'temporary' : $LinkOwner->type) . '\', \'' . $LinkOwner->get_ID() . '\', \'\', \'\', \'\', \'' . $tab_pane_prefix . '\' )',
            ]
        );
        if (! $LinkOwner->is_temp()) {	// Don't allow this option for new creating objects:
            $items_left .= action_icon(
    ...
    $items_right .= action_icon(
        ...
        [
            'class' => 'action_icon btn btn-default btn-sm',
            'onclick' => 'return evo_link_refresh_list( \'' . ($LinkOwner->is_temp() ? 'temporary' : $LinkOwner->type) . '\', \'' . $LinkOwner->get_ID() . '\' )',
        ]
    )
        ...
        . action_icon(
            T_('Sort'),
            'ascending',
            (is_admin_page() || (is_logged_in() && $current_User->check_perm('admin', 'restricted')))
        ? $admin_url . '?ctrl=links&amp;action=sort_links&amp;link_type=' . $LinkOwner->type . '&amp;link_object_ID=' . $LinkOwner->get_ID() . '&amp;' . url_crumb('link')
        : $LinkOwner->get_edit_url() . '#',
            T_('Sort'),
            3,
            ...
            [
                'class' => 'action_icon btn btn-default btn-sm',
                'onclick' => 'return evo_link_refresh_list( \'' . ($LinkOwner->is_temp() ? 'temporary' : $LinkOwner->type) . '\', \'' . $LinkOwner->get_ID() . '\', \'sort\' )',
            ]
        );
    ...
    if (is_logged_in() && $current_User->check_perm('admin', 'restricted') && $current_User->check_perm('files', 'view') && empty($restriction_overlay)) {	// Check if current user has a permission to back-office files manager:
        // Initialize JavaScript to build and open window:
        echo_modalwindow_js();
        ?>
<script>
