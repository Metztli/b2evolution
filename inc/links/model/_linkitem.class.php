...
class LinkItem extends LinkOwner
{
     ...
     */
    public function __construct($Item, $tmp_ID = null)
    {
    ...
    public function check_perm($permlevel, $assert = false, $FileRoot = null)
    {
        ...
        if ($this->is_temp()) {	// Check permission for new creating item:
            return check_user_perm('blog_post_statuses', $permlevel, $assert, $this->get_blog_ID());
        } else {	// Check permission for existing item in DB:
    ...
    public function get_positions($file_ID = null)
    {
        ...
        if ($File && $File->is_image()) {	// Only images can have these positions:
        ...
        if ($File && $File->is_image()) { // Only images can have this position
    ...
    public function get_default_position($file_ID)
    {
        $FileCache = &get_FileCache();
        $File = &$FileCache->get_by_ID($file_ID, false, false);
        if (empty($File)) { // If file is broken then get simple default position as "aftermore"
            return 'aftermore';
        }

        if ($File->is_image()) { // If file is image then get position depending on order
            $this->load_Links();

            if ($this->Links) { // There's only one file attached yet, the second becomes "aftermore"
                return 'aftermore';
            } else { // No attachment yet
                return 'teaser';
            }
        } elseif ($File->is_video() || $File->is_audio()) {	// If file is video or audio then always use "aftermore":
            return 'aftermore';
        } else {	// All other file types must use "attachment" position by default:
            return 'attachment';
        }
    }
    ...
    public function add_link($file_ID, $position = null, $order = 1, $update_owner = true)
    {
        ...
        if (! $this->Item->check_proposed_change_restriction('error')) {	// If the Link's Item cannot be updated because of proposed change:
            return false;
        }
        ...
        if ($order > 0 && $order <= $this->get_last_order()) {	// Don't allow order which may be already used:
        ...
        if ($edited_Link->dbinsert()) {
            if (! $this->is_temp()) {	// New link was added to the item, invalidate blog's media BlockCache:
            ...
            if (! $this->is_temp() && $update_owner) {	// Update last touched date and content last updated date of the Item:
     ...
     */
    public function remove_link(&$Link, $force = false)
    {
        global $DB, $localtimenow;

        if (! $force && ! $this->Item->check_proposed_change_restriction('error')) {	// If the Link's Item cannot be updated because of proposed change:
            return false;
        }
        ...
        if (! empty($previous_Revision) && ($localtimenow - strtotime($this->Item->last_touched_ts)) < 90) { // Check if we can remove the link from the previous revision
            $last_revision_ID = (int) $previous_Revision->iver_ID;
            if ($last_revision_ID > 1) { // Check if the file attachment exists in the previous revision
                $sql = new SQL();
                $sql->SELECT('*');
                ...
                if (empty($revision_links)) { // Link is not in previous history, we need to create a new revision
                    $this->Item->create_revision();
                }
            } else { // Link has no attachment history so we'll have to create a new one
                $this->Item->create_revision();
            }
        } else {
            $this->Item->create_revision();
        ...
        if ($Link->dbdelete()) {
            if (! $this->is_temp()) {	// Update last touched date and content last updated date of the Item:
                $this->update_last_touched_date();
                $this->update_contents_last_updated_ts();
            }
    ...
    {
        if ($this->Blog === null) {	// Try to get Item from DB and store in cache to next requests:
            if ($this->is_temp()) {	// If new Comment is creating
                $BlogCache = &get_BlogCache();
                $this->Blog = &$BlogCache->get_by_ID($this->link_Object->tmp_coll_ID, false, false);
            } else {	// If existing Item is editing
    ...
    {
        if ($url_type == 'backoffice' || ($url_type === null && is_admin_page())) {	// Back-office:
            global $admin_url;
            if ($this->is_temp()) {	// New creating Item:
                return $admin_url . '?ctrl=items' . $glue . 'blog=' . $this->get_blog_ID() . $glue . 'action=new';
            } else {	// The edited Item:
                return $admin_url . '?ctrl=items' . $glue . 'blog=' . $this->get_blog_ID() . $glue . 'action=edit' . $glue . 'p=' . $this->get_ID();
            }
        } else {	// Front-office:
            $item_Blog = &$this->get_Blog();
            if ($this->is_temp()) {	// New creating Item:
                return url_add_param($item_Blog->get('url', [
                    'glue' => $glue,
                ]), 'disp=edit', $glue);
            } else {	// The editing Item:
                return url_add_param($item_Blog->get('url', [
                    'glue' => $glue,
                ]), 'disp=edit' . $glue . 'p=' . $this->get_ID(), $glue);
            }
        }
    }
    ...
    {
        if (! empty($this->Item) && ! $this->is_temp()) {	// Update Item if it exists
     ...
     */
    public function update_contents_last_updated_ts()
    {
        if (! empty($this->Item) && ! $this->is_temp()) {	// Update Item if it exists:
     ...
     */
    public function after_unlink_action($link_ID = 0)
    {
        if (empty($this->Item)) { // No existing Item, Exit here
            return;
        }

        if (! empty($link_ID)) { // Find inline image placeholders if link ID is defined
            preg_match_all('/\[(image|file|inline|video|audio|thumbnail):' . $link_ID . ':?[^\]]*\]/i', $this->Item->content, $inline_images);
            if (! empty($inline_images[0])) { // There are inline image placeholders in the post content
     ...
     */
    public function get_attachment_LinkList($limit = 1000, $position = null, $file_type = null, $params = [])
    {
        if ($this->Item->is_revision()) {	// Get Links of current active revision:
            ...
            foreach ($params as $param_key => $param_value) {
                if (strpos($param_key, 'sql_') !== false) {	// Replace column names to revision table name in external SQL:
                    $params[$param_key] = str_replace(
                        ['link_ID', 'link_file_ID', 'link_position', 'link_prder', 'link_itm_ID'],
                        ['ivl_link_ID', 'ivl_file_ID', 'ivl_position', 'ivl_order', 'ivl_iver_itm_ID'],
                        $param_value
                    );
                }
            }
            ...
            if (! is_null($file_type)) {	// Restrict the Links by File type:
            ...
            if ($LinkList->result_num_rows == 0) {	// Nothing found
                $LinkList = null;
            }

            return $LinkList;
        } else {	// Get Links of current Item:
