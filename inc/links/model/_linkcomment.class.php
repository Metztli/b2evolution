...
class LinkComment extends LinkOwner
{
     ...
     */
    public function __construct($Comment, $tmp_ID = null)
    {
    ...
    public function check_perm($permlevel, $assert = false, $FileRoot = null)
    {
        ...
        } elseif ($this->is_temp()) {	// Check permission for new creating comment:
            $comment_Item = &$this->get_Item();
            $r = $comment_Item->can_comment(null, null, null, null, null, [], $this->Comment->type);
        } else {	// Check permission for existing comment in DB:
        ...
        if (! $r && $assert) {	// Halt the denied access:
    ...
    public function add_link($file_ID, $position = null, $order = 1, $update_owner = true)
    {
        ...
        if ($edited_Link->dbinsert()) {
            ...
            if (! $this->is_temp() && $update_owner) {	// Update last touched date of the Comment & Item:
    ...
    {
        if ($this->Blog === null) {	// Load collection of the comment's Item:
            $comment_Item = &$this->get_Item();
    ...
    public function get_edit_url($glue = '&amp;', $url_type = null)
    {
        if ($url_type == 'backoffice' || ($url_type === null && is_admin_page())) {	// Back-office:
            ...
            if ($this->is_temp()) {	// New creating Comment:
                ...
                return url_add_param($comment_Blog->get('url', [
                    'glue' => $glue,
                ]), 'disp=edit_comment' . $glue . 'c=' . $this->get_ID(), $glue);
            }
        }
    }
    ...
    {
        if (! empty($this->Comment) && ! $this->is_temp()) {	// Update Item & Comment if it exist:
    ...
    {
        if (empty($this->Comment) || $this->is_temp()) {	// Comment must be defined:
            return;
        }

        if (! $this->Comment->may_be_seen_in_frontoffice()) {	// Don't change item contents updated date if comment cannot be displayed on front-office:
            return;
        }

        if ($comment_Item = &$this->Comment->get_Item()) {	// Update item field contents_last_updated_ts:
