...
class LinkCache extends DataObjectCache
{
     ...
     * @var array
     */
    public $cache_comment = [];

    /**
     * @access private
     * @var array Remember full loads
     */
    public $loaded_cache_comment = [];
     ...
     * @var array
     */
    public $cache_emailcampaign = [];

    /**
     * @access private
     * @var array Remember full loads
     */
    public $loaded_cache_emailcampaign = [];
     ...
     * @var array
     */
    public $cache_message = [];

    /**
     * @access private
     * @var array Remember full loads
     */
    public $loaded_cache_message = [];
     ...
     * @var array
     */
    public $cache_temporary = [];

    /**
     * @access private
     * @var array Remember full loads
     */
    public $loaded_cache_temporary = [];
     ...
     */
    public function add($Obj, $cache_type = '')
    {
        if ((! isset($Obj->ID)) || ($Obj->ID == 0)) { // The object is not cachable, becuase it has no valid ID
            return false;
        }

        // If the object wasn't already cached and is valid:
        $this->cache[$Obj->ID] = $Obj;
        if ($cache_type == 'file') { // Cache by File ID
        ...
        if ($LinkOwner == null) { // LinkOwner is not valid
            return false;
        }
     ...
     * @return boolean TRUE on success
     */
    public function remove(&$Obj, $cache_type = '')
    {
        if ((! isset($Obj->ID)) || ($Obj->ID == 0)) { // The object is not cachable, so it can't be in the cache
            return false;
        }

        $this->remove_by_ID($Obj->ID);

        if (($cache_type == 'file') && (isset($this->cache_file[$Obj->file_ID][$Obj->ID]))) { // Cache by File ID
     ...
     * @return array of refs to Link objects
     */
    public function &get_by_emailcampaign_ID($emailcampaign_ID)
    {
        // Make sure links are loaded:
     ...
     * @return array of refs to Link objects
     */
    public function &get_by_message_ID($message_ID)
    {
        // Make sure links are loaded:
     ...
     * @return array of refs to Link objects
     */
    public function &get_by_temporary_ID($temporary_ID)
    {
        // Make sure links are loaded:
     ...
     */
    public function load_type_by_sql($SQL, $cache_type)
    {
        global $DB;
        ...
        foreach ($links as $row) { // Cache each matching object:
     ...
     * @param integer item ID to load links for
     */
    public function load_by_item_ID($item_ID)
    {
        ...
        } else {	// NO, load Links for this single Item:
     ...
     * @param integer comment ID to load links for
     */
    public function load_by_comment_ID($comment_ID)
    {
        global $DB, $Debuglog;
     ...
     * @param integer Email campaign ID to load links for
     */
    public function load_by_emailcampaign_ID($emailcampaign_ID)
    {
        global $DB, $Debuglog;
     ...
     * @param integer Message ID to load links for
     */
    public function load_by_message_ID($message_ID)
    {
        global $DB, $Debuglog;
     ...
     * @param integer Temporary ID to load links for
     */
    public function load_by_temporary_ID($temporary_ID)
    {
        global $DB, $Debuglog;
     ...
     */
    public function load_by_item_list($itemIDarray)
    {
        global $DB, $Debuglog;

        if (empty($itemIDarray)) {	// Nothing to load:
            return false;
        }
        ...
        foreach ($itemIDarray as $item_ID) { // Remember this special load:
            $this->cache_item[$item_ID] = [];
            $this->loaded_cache_item[$item_ID] = true;
        }
     ...
     */
    public function load_by_comment_list($comment_IDs)
    {
        global $DB, $Debuglog;

        if (empty($comment_IDs)) {	// Nothing to load:
            return;
        }
        ...
        foreach ($comment_IDs as $comment_ID) { // Remember this special load for each comment in list:
            $this->cache_comment[$comment_ID] = [];
            $this->loaded_cache_comment[$comment_ID] = true;
        }
     ...
     * @param integer file ID to load links for
     */
    public function load_by_file_ID($file_ID)
    {
        global $DB, $Debuglog;
     ...
     */
    public function load_linked_files(&$link_rows)
    {
        if (empty($link_rows)) { // There are nothing to load
            return;
        }
        ...
        foreach ($link_rows as $row) {
            if (! empty($row->link_file_ID)) {	// Skip a link with worn file ID:
                $link_file_ids[] = $row->link_file_ID;
            }
        }

        if (! empty($link_file_ids)) {	// Load required Files into FileCache:
     ...
     */
    public function clear($keep_shadow = false, $object = 'all', $object_ID = 0)
    {
        ...
        switch ($object) {
            case 'all':
                ...
                $this->cache_item = [];
                $this->loaded_cache_item = [];
                $this->cache_comment = [];
                $this->loaded_cache_comment = [];
                $this->cache_user = [];
                $this->loaded_cache_user = [];
                $this->loaded_cache_emailcampaign = [];
                $this->loaded_cache_message = [];
                $this->cache_file = [];
                $this->loaded_cache_file = [];
                break;
            ...
            case 'file':
                ...
                if (empty($object_ID)) { // Clear all cached objects of this type
                    $this->{'cache_' . $object} = [];
                    $this->{'loaded_cache_' . $object} = [];
                } else { // Clear a cache only one object
