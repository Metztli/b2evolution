...
class Link extends DataObject
{
     ...
     */
    public function get_preview_thumb()
    {
        $File = &$this->get_File();
        if (! $File) { // No file
            return '';
     ...
     */
    public function get_download_url($params = [])
    {
        $params = array_merge([
        ...
        if (! ($File = &$this->get_File()) ||
            ! ($LinkOwner = &$this->get_LinkOwner())) { // Broken Link
            return false;
        }

        if ($LinkOwner->type == 'item' && $LinkOwner->Item) { // Use specific url for Item to download
            if (! $LinkOwner->Item->get_coll_setting('download_enable')) {	// If ?disp=download is disabled then use simple original URL to the File:
                return $File->get_url();
            }
            ...
            switch ($params['type']) {
                case 'action':
                    ...
                    if ($File->get_ext() == 'zip') { // Provide direct url to ZIP files
                        // NOTE: The same hardcoded place is in the file "htsrv/download.php", lines 56-60
                        return $File->get_url();
                    } else { // For other files use url through special file that forces a download action
        ...
        } else { // Use standard url for all other types
     ...
     * @return boolean
     */
    public function can_be_file_deleted()
    {
        if (! is_logged_in()) {	// Not logged in user
            return false;
        }

        $LinkOwner = &$this->get_LinkOwner();
        if (empty($LinkOwner) || ! $LinkOwner->check_perm('edit')) {	// User has no permission to edit current link owner
            return false;
        }

        if (! ($File = &$this->get_File()) ||
            ! ($FileRoot = &$File->get_FileRoot()) ||
            ! check_user_perm('files', 'edit_allowed', false, $FileRoot)) {	// Current user has no permission to edit this file
            return false;
        }

        if (! $this->is_single_linked_file()) {	// We cannot delete the file of this link because it is also linked to another object
            return false;
        }
     ...
     * @return boolean
     */
    public function is_single_linked_file()
    {
        if (! isset($this->is_single_linked_file)) {	// Try to find at least one another link by same file ID:
            global $DB;
    ...
    public function dbinsert()
    {
        ...
        $link_owner_fields = ['link_itm_ID', 'link_cmt_ID', 'link_usr_ID', 'link_ecmp_ID', 'link_msg_ID', 'link_tmp_ID'];
        ...
        if ($auto_order && ! empty($link_owner_ID_field) && ! empty($link_owner_ID)) { // Auto generate link_order in DB
    ...
    public function dbupdate()
    {
        ...
        if ($LinkOwner && ($LinkOwner->type == 'item') && (isset($this->dbchanges['link_position']) || isset($this->dbchanges['link_order']))) {
            if (! $LinkOwner->Item->check_proposed_change_restriction('error')) {	// If the Link's Item cannot be updated because of proposed change:
                $DB->rollback();
                return false;
            }
            ...
            if (! empty($update_values)) {
                if (($localtimenow - strtotime($LinkOwner->Item->last_touched_ts)) > 90) { // Create a new revision...
                    $revision_ID = $LinkOwner->Item->create_revision();
                    if (is_int($revision_ID)) {
                        $new_Revision = $LinkOwner->Item->get_revision($revision_ID);
                    }

                    if (! empty($new_Revision)) { // ...but newly created link history has current position and order values, restore it to previous values
        ...
        if (parent::dbupdate()) {	// Update last touched date and content last updated date of the Item:
