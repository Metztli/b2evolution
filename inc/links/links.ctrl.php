...
switch ($action) {
    ...
    case 'link_move_down':
        ...
        if (($edited_Link = &$LinkCache->get_by_ID($link_ID, false)) !== false) {	// We have a link, get the LinkOwner it is attached to:
            $LinkOwner = &$edited_Link->get_LinkOwner();

            // Load the blog we're in:
            $Collection = $Blog = &$LinkOwner->get_Blog();
            set_working_blog($Blog->ID);
        } else {	// We could not find the link to edit:
...
if ($action == 'edit_links' || $action == 'sort_links') { // set LinkOwner from params
    ...
    if (empty($Blog)) { // Load the blog we're in:
        $Collection = $Blog = &$LinkOwner->get_Blog();
        set_working_blog($Blog->ID);
    }
}

if (empty($LinkOwner)) { // If LinkOwner object is not set, we can't process any action
...
switch ($action) {
    ...
    case 'delete': // Unlink and Delete a file from disk and DB completely:
        ...
        if ($action == 'delete' && $edited_Link->can_be_file_deleted()) {	// Get a linked file to delete it after unlinking if it is allowed for current user:
