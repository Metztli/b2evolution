...
class PluginUserSettings extends AbstractSettings
{
    ...
    { // constructor
        parent::__construct('T_pluginusersettings', ['puset_plug_ID', 'puset_user_ID', 'puset_name'], 'puset_value', 2);
    ...
    public function get($setting, $user_ID = null)
    {
        if (! isset($user_ID)) {
            ...
            if (! isset($current_User)) {
                global $Debuglog;
                $Debuglog->add('No $current_User available in PluginUserSettings::get()/[ID' . $this->plugin_ID . ']!', ['errors', 'plugins']);
                return false;
            }
            $user_ID = $current_User->ID;
        }

        if (strpos($setting, '[') !== false) {	// Get value for array setting like "sample_sets[0][group_name_param_name]":
    ...
    public function set($setting, $value, $user_ID = null)
    {
        if (! isset($user_ID)) {
            ...
            if (! isset($current_User)) {
                global $Debuglog;
                $Debuglog->add('No $current_User available in PluginUserSettings::set()/[ID' . $this->plugin_ID . ']!', ['errors', 'plugins']);
                return false;
            }
            $user_ID = $current_User->ID;
        }
    ...
    public function delete($setting, $user_ID = null)
    {
        if (! isset($user_ID)) {
            ...
            if (! isset($current_User)) {
                global $Debuglog;
                $Debuglog->add('No $current_User available in PluginUserSettings::delete()/[ID' . $this->plugin_ID . ']!', ['errors', 'plugins']);
                return false;
            }
            $user_ID = $current_User->ID;
        }
