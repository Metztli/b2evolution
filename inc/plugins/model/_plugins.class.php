...
class Plugins
{
     ...
     * @var boolean
     */
    public $loaded_plugins_table = false;

    /**
     * Current object index in {@link $sorted_IDs} array.
     * @var integer
     */
    public $current_idx = -1;
     ...
     * @return array
     */
    public function get_Plugins_in_group($group)
    {
        $result = [];
     ...
     * @return array
     */
    public function get_Plugins_in_sub_group($group, $sub_group = '')
    {
        $result = [];
     ...
     */
    public function set_Plugin_status(&$Plugin, $status)
    {
        global $DB, $Debuglog;
        ...
        if ($status == 'enabled') { // Reload plugins tables, which includes the plugin in further requests
     ...
     * @return string Path
     */
    public function get_classfile_path($classname)
    {
        ...
        if (! is_readable($classfile_path)) { // Look directly in $plugins_path
     ...
     */
    public function &register($classname, $ID = 0, $priority = -1, $classfile_path = null, $must_exists = true)
    {
        ...
        if (! is_readable($classfile_path)) { // Plugin file not found!
            if ($must_exists) {
                ...
                if ($this->unregister($Plugin)) {
                    $Debuglog->add('Unregistered plugin [' . $classname . ']!', ['plugins', 'error']);
                } else {
        ...
        if (! class_exists($classname)) { // the given class does not exist
            if ($must_exists) {
        ...
        if ($ID == 0) {
            ...
            if ($ID > 0) { // Properties from T_plugins
                // Code
        ...
        if (! empty($this->index_code_ID[$Plugin->code]) && $this->index_code_ID[$Plugin->code] != $Plugin->ID) { // The plugin's default code is already in use!
            $Plugin->code = null;
        } else {
        ...
        if ($Plugin->ID > 0) {
            $tmp_params = [
                'db_row' => $this->index_ID_rows[$Plugin->ID],
                'is_installed' => true,
            ];
            ...
            // Version check:
            elseif ($Plugin->version != $this->index_ID_rows[$Plugin->ID]['plug_version'] && $must_exists) { // Version has changed since installation or last update
                $db_deltas = [];

                // Tell the Plugin that we've detected a version change:
                $tmp_params = [
                    'old_version' => $this->index_ID_rows[$Plugin->ID]['plug_version'],
                    'db_row' => $this->index_ID_rows[$Plugin->ID],
                ];
                ...
                if ($this->call_method($Plugin->ID, 'PluginVersionChanged', $tmp_params) === false) {
                    ...
                    if ($this->unregister($Plugin)) { // only unregister the Plugin, if it's not the admin list's class:
                        $Plugin = '';
                    }
                } else {
                    ...
                    if (empty($db_deltas)) { // No DB changes needed, update (bump or decrease) the version
                        global $DB;
                        ...
                        if ($Plugins_admin->save_events($Plugin, [])) {
                    ...
                    } else { // If there are DB schema changes needed, set the Plugin status to "needs_config"
        ...
        } else { // This gets called for non-installed Plugins:
     ...
     */
    public function unregister(&$Plugin, $force = false)
    {
        global $Debuglog;
        ...
        if (isset($this->index_ID_rows[$Plugin->ID])) { // It has an associated DB row (load_plugins_table() was called)
            unset($this->index_ID_rows[$Plugin->ID]);
        }

        $sort_key = array_search($Plugin->ID, $this->sorted_IDs);
        if ($sort_key === false) { // this may happen if a Plugin has unregistered itself
        ...
        if ($this->current_idx >= $sort_key) { // We have removed a file before or at the $sort_key'th position
            $this->current_idx--;
        }
     ...
     */
    public function forget_events($plugin_ID)
    {
        // Forget events:
     ...
     */
    public function instantiate_Settings(&$Plugin, $set_type)
    {
        ...
        if ($set_type == 'Settings') {	// If general settings are requested we should also append custom, collection, widgets, messages, emails and shared settings:
            if (! is_array($defaults)) {	// Initialize array for default settings:
                $defaults = [];
            }
            ...
            if (isset($Blog)) {	// Only when current collection is defined in order to avoid errors because global $Blog may be used inside:
        ...
        if (empty($defaults) && empty($other_defaults)) {	// No settings, no need to instantiate:
            $Timer->pause('plugins_inst_' . $set_type);
            return null;
        }

        if (! is_array($defaults)) {	// Invalid format of default settings:
            $Debuglog->add($Plugin->classname . '::GetDefault' . $set_type . '() did not return array!', ['plugins', 'error']);
            return null; // fp> correct me if I'm wrong.
        }

        if ($set_type == 'UserSettings') { // User specific settings:
        ...
        } elseif ($set_type == 'GroupSettings') {	// Group specific settings:
        ...
        foreach ($defaults as $l_name => $l_meta) {
            if (isset($l_meta['layout'])) { // Skip non-value entries
                continue;
            }
            ...
            } elseif (isset($l_meta['type']) && $l_meta['type'] == 'input_group' && is_array($l_meta['inputs'])) {	// Get default values from input group fields:
    ...
    public function &get_next()
    {
        ...
        if (isset($this->sorted_IDs[$this->current_idx])) {
            ...
            if (! $Plugin) { // recurse until we've been through whole $sorted_IDs!
     ...
     */
    public function trigger_event($event, $params = [])
    {
        global $Debuglog;

        $Debuglog->add('Trigger event ' . $event, 'plugins');

        if (empty($this->index_event_IDs[$event])) { // No events registered
        ...
        foreach ($this->index_event_IDs[$event] as $l_plugin_ID) {
            ...
            if (! empty($this->_stop_propagation)) {	// A plugin has requested to stop propagation.
     ...
     */
    public function trigger_event_first_true($event, $params = null)
    {
        global $Debuglog;
        ...
        } else {	// We have some events registered, loop through them:
            $Debuglog->add('Registered plugin IDs: ' . implode(', ', $this->index_event_IDs[$event]), 'plugins');
            foreach ($this->index_event_IDs[$event] as $l_plugin_ID) {
     ...
     *               Empty array if no Plugin returned true or no Plugin has this event registered.
     */
    public function trigger_event_first_true_with_params($event, &$params)
    {
        global $Debuglog;
     ...
     *               Empty array if no Plugin returned true or no Plugin has this event registered.
     */
    public function trigger_event_first_false($event, $params = null)
    {
        global $Debuglog;
     ...
     *               Empty array if no Plugin returned true or no Plugin has this event registered.
     */
    public function trigger_event_first_return($event, $params = null, $search = null)
    {
        global $Debuglog;
        ...
        foreach ($this->index_event_IDs[$event] as $l_plugin_ID) {
            $r = $this->call_method($l_plugin_ID, $event, $params);
            if (isset($r)) {
                if (isset($search)) { // Apply $search:
                    foreach ($search as $k => $v) { // Check search criterias and continue if it does not match:
                        switch ($k) {
                            case 'in_array':
                                if (! in_array($v, $r)) {
     ...
     */
    public function get_trigger_event($event, $params = null, $get = 'data')
    {
     ...
     * @return mixed The requested index of $params
     */
    public function get_trigger_event_first_true($event, $params = null, $get = 'data')
    {
        $params[$get] = &$params[$get]; // make it a reference, so it can get changed
     ...
     */
    public function get_trigger_event_first_return($event, $params = null)
    {
     ...
     */
    public function trigger_collect($event, $params = null, $ignore_empty = true)
    {
     ...
     */
    public function trigger_karma_collect($event, $params)
    {
        global $Debuglog;
     ...
     */
    public function call_method($plugin_ID, $method, &$params)
    {
        ...
        if ($method == 'CacheObjects') {	// Deny plugins with deprecated event:
     ...
     */
    public function call_method_if_active($plugin_ID, $method, &$params)
    {
     ...
     * @return boolean
     */
    public function call_by_code($code, $params = [])
    {
        $Plugin = &$this->get_by_code($code);

        if (! $Plugin) {
            global $Debuglog;
            $Debuglog->add('No plugin available for code [' . $code . ']!', ['plugins', 'error']);
            return false;
        }
     ...
     */
    public function render(&$content, $renderers, $format, $params, $event_prefix = 'Render')
    {
        $params['data'] = &$content;
        $params['format'] = $format;

        if (in_array($format, ['htmlbody', 'htmlfeed', 'entityencoded'])) {
        ...
        if (isset($params['Item']) && ! empty($params['Item'])) { // the Item is set, get Item Blog
            $Item = &$params['Item'];
            $setting_Blog = &$Item->get_Blog();
        } else { // use global Blog if it is set
            global $Collection, $Blog;
            if (! empty($Blog)) {
                $setting_Blog = $Blog;
            } else {	// Try to get Blog by global blog ID
                global $blog;
                if (! empty($blog)) {
        ...
        if (empty($setting_Blog)) { // This should be impossible, but make sure $setting_Blog is set
        ...
        if (! is_array($evo_renderers_used_in_current_page)) {	// Initialize array for curently used renderer plugins once:
            $evo_renderers_used_in_current_page = [];
        }
        $evo_renderers_used_in_current_page += $renderers;

        foreach ($renderer_Plugins as $loop_RendererPlugin) { // Go through whole list of renders
            $apply_rendering_value = $loop_RendererPlugin->get_coll_setting('coll_apply_rendering', $setting_Blog);
            if ($loop_RendererPlugin->is_renderer_enabled($apply_rendering_value, $renderers)) { // Plugin is enabled to call method
     ...
     */
    public function quick($plugin_code, $params)
    {
        global $Debuglog;
        ...
        if ($Plugin) {
            ...
            if ($event) {
                ...
                $Debuglog->add($Plugin->classname . '(ID ' . $Plugin->ID . '): failed to quick-render (tried method ' . $event . ')!', ['plugins', 'error']);
            }
            return format_to_output($params['data'], $params['format']);
        } else {
            $Debuglog->add('Plugins::quick() - failed to instantiate Plugin by code [' . $plugin_code . ']!', ['plugins', 'error']);
            return format_to_output($params['data'], $params['format']);
        }
    }
    ...
    public function load_plugins_table()
    {
        ...
        foreach ($DB->get_results($this->sql_load_plugins_table, ARRAY_A, 'Loading plugins table data') as $row) { // Loop through installed plugins:
     ...
     */
    public function load_plugin_by_classname($classname)
    {
        global $Debuglog, $DB;
        ...
        if ($plugin = $DB->get_row($SQL, ARRAY_A)) {
            if (isset($this->index_ID_rows[$plugin['plug_ID']])) { // Plugin already was loaded before
                return;
            }
     ...
     */
    public function load_index_apply_rendering($setting_name, &$Blog, $type = 'coll')
    {
        if (is_null($Blog)) { // Use general settings (e.g. for Messages)
            $blog_ID = 0;
        } else { // Use collection settings (for Items and Comments)
            $blog_ID = $Blog->ID;
        }

        if (isset($this->index_apply_rendering_codes[$blog_ID][$setting_name])) { // This data is already loaded
            return;
        }
        ...
        foreach ($index as $plugin_ID) { // iterate through each plugin
            $Plugin = $this->get_by_ID($plugin_ID);
            if ($Plugin->group != 'rendering') { // This plugin doesn't belong to the rendering plugins group
                continue;
            }
            switch ($type) {
                case 'coll':
     ...
     */
    public function &get_by_ID($plugin_ID)
    {
        global $Debuglog;

        if (! isset($this->index_ID_Plugins[$plugin_ID])) { // Plugin is not instantiated yet
            ...
            if (! isset($this->index_ID_rows[$plugin_ID]) || ! $this->index_ID_rows[$plugin_ID]) { // no plugin rows cached
                #debug_die( 'Cannot instantiate Plugin (ID '.$plugin_ID.') without DB information.' );
                $Debuglog->add('get_by_ID(): Plugin (ID ' . $plugin_ID . ') not registered/enabled in DB!', ['plugins', 'error']);
                $r = false;
                return $r;
            }
     ...
     * @return Plugin (false in case of error)
     */
    public function &get_by_classname($classname)
    {
     ...
     * @return Plugin (false in case of error)
     */
    public function &get_by_code($plugin_code)
    {
        global $Debuglog;

        $r = false;

        if (! isset($this->index_code_Plugins[$plugin_code])) { // Plugin is not registered yet
            $this->load_plugins_table();
     ...
     */
    public function get_list_by_event($event)
    {
        $r = [];
     ...
     * @return array plugin_ID => & Plugin
     */
    public function get_list_by_events($events)
    {
        $r = [];
     ...
     * @return array plugin_ID => & Plugin
     */
    public function get_list_by_all_events($events)
    {
        $candidates = [];
     ...
     * @return array
     */
    public function get_enabled_events($plugin_ID)
    {
        $r = [];
     ...
     * @return boolean
     */
    public function has_event($plugin_ID, $event)
    {
     ...
     * @return boolean
     */
    public function are_events_available($events, $require_all_in_same_plugin = false)
    {
        if (! is_array($events)) {
            $events = [$events];
        }
     ...
     * @param string Class name
     */
    public function load_events_by_classname($classname)
    {
        global $Debuglog, $DB;
     ...
     */
    public function get_object_from_cacheplugin_or_create($objectName, $eval_create_object = null)
    {
        $get_return = $this->trigger_event_first_true(
            'CacheObjects',
            [
                'action' => 'get',
                'key' => 'object_' . $objectName,
            ]
        );
        ...
        // Try to set in cache:
        $set_return = $this->trigger_event_first_true(
            'CacheObjects',
            [
                'action' => 'set',
                'key' => 'object_' . $objectName,
        ...
        if (isset($set_return['plugin_ID'])) { // success, register a shutdown function to save this data on shutdown
    ...
    public function validate_renderer_list($renderers = ['default'], $params)
    {
        ...
        if (isset($params['Item'])) { // Validate post renderers
            ...
            $setting_type = 'coll';
        } elseif (isset($params['Comment'])) { // Validate comment renderers
            $Comment = &$params['Comment'];
            $Item = &$Comment->get_Item();
            $Collection = $Blog = &$Item->get_Blog();
            $setting_name = 'coll_apply_comment_rendering';
            $setting_type = 'coll';
        } elseif (isset($params['Message'])) { // Validate message renderers
            $Collection = $Blog = null;
            $setting_name = 'msg_apply_rendering';
            $setting_type = 'msg';
        } elseif (isset($params['EmailCampaign'])) {	// Validate message renderers:
            $Collection = $Blog = null;
            $setting_name = 'email_apply_rendering';
            $setting_type = 'email';
        } elseif (isset($params['setting_name'])) {	// Validate the given rendering option:
            $setting_name = $params['setting_name'];
            if (isset($params['Blog'])) {	// If Collection is given:
                $Collection = $Blog = &$params['Blog'];
                $setting_type = 'coll';
                if ($setting_name == 'shared_apply_rendering') {	// Force to posts/items rendering settings:
                    $setting_name = 'coll_apply_rendering';
                }
                if (! in_array($setting_name, ['coll_apply_rendering', 'coll_apply_comment_rendering', 'shared_apply_rendering'])) {
                    debug_die('Invalid apply rendering param name received!');
                }
            }
            if ($setting_name == 'shared_apply_rendering') {	// Set shared type instead of collection for this spec setting name:
                $Collection = $Blog = null;
                $setting_type = 'shared';
            }
        } else { // Invalid params to validate renderers!
            return [];
        }
    ...
    public function get_renderer_options($current_renderers = null, $params)
    {
        ...
        $setting_Blog = null;
        if (isset($params['setting_name']) && $params['setting_name'] == 'msg_apply_rendering') { // get Message apply_rendering setting
            $setting_name = $params['setting_name'];
        } elseif (isset($params['setting_name']) && $params['setting_name'] == 'email_apply_rendering') {	// get EmailCampaign apply_rendering setting:
            $setting_name = $params['setting_name'];
        } elseif (isset($params['setting_name']) && $params['setting_name'] == 'shared_apply_rendering') {	// get apply_rendering setting for widget from sharaed container:
            $setting_name = $params['setting_name'];
        } elseif (isset($params['Comment']) && ! empty($params['Comment'])) { // get Comment apply_rendering setting
            $Comment = &$params['Comment'];
            $comment_Item = &$Comment->get_Item();
            $setting_Blog = &$comment_Item->get_Blog();
            $setting_name = 'coll_apply_comment_rendering';
        } elseif (isset($params['Item'])) { // get Post apply_rendering setting
            $setting_name = 'coll_apply_rendering';
            $Item = &$params['Item'];
            $setting_Blog = &$Item->get_Blog();
        } elseif (isset($params['setting_name'])) {	// Get given setting:
            $setting_name = $params['setting_name'];
            if (! empty($params['Blog'])) {	// If Collection is given::
        ...
        switch ($setting_name) {
            case 'msg_apply_rendering':
                ...
                $RendererPlugins = $this->get_list_by_events(['FilterMsgContent']);
                break;

            case 'email_apply_rendering':
                // Get Email Campaign renderer plugins
                $RendererPlugins = $this->get_list_by_events(['FilterEmailContent']);
                break;

            case 'coll_apply_comment_rendering':
                // Get Comment renderer plugins
                $RendererPlugins = $this->get_list_by_events(['FilterCommentContent']);
                break;
            ...
            default:
                ...
                $RendererPlugins = $this->get_list_by_events(['RenderItemAsHtml', 'RenderItemAsXml', 'RenderItemAsText', 'DisplayItemAsHtml', 'DisplayItemAsXml', 'DisplayItemAsText']);
                break;
        }

        foreach ($RendererPlugins as $loop_RendererPlugin) { // Go through whole list of renders
            // echo ' ',$loop_RendererPlugin->code;
            if (empty($loop_RendererPlugin->code)) { // No unique code!
                continue;
            }
            if (empty($setting_Blog) && $setting_name != 'msg_apply_rendering' && $setting_name != 'email_apply_rendering' && $setting_name != 'shared_apply_rendering') { // If $setting_Blog is not set we can't get collection apply_rendering options
                continue;
            }

            if ($setting_name == 'msg_apply_rendering') { // get rendering setting from plugin message settings
                $apply_rendering = $loop_RendererPlugin->get_msg_setting($setting_name);
            } elseif ($setting_name == 'email_apply_rendering') {	// get rendering setting from plugin email settings:
                $apply_rendering = $loop_RendererPlugin->get_email_setting($setting_name);
            } elseif ($setting_name == 'shared_apply_rendering') {	// get rendering setting from plugin shared settings:
                $apply_rendering = $loop_RendererPlugin->get_shared_setting($setting_name);
            } else { // get rendering setting from plugin coll settings
                $apply_rendering = $loop_RendererPlugin->get_coll_setting($setting_name, $setting_Blog);
            }

            $ignored_apply_rendering = ['stealth', 'never'];
            ...
            if (in_array($apply_rendering, $ignored_apply_rendering)
                || empty($apply_rendering)) { // This is not an option.
                continue;
            }
    ...
    public function get_renderer_checkboxes($current_renderers = null, $params)
    {
        if (isset($params['setting_name'])) {	// Use the defined setting name from params:
            $setting_name = $params['setting_name'];
        } elseif (! empty($params['Comment'])) {	// Use setting name for Comment:
            $setting_name = 'coll_apply_comment_rendering';
        } elseif (! empty($params['Item'])) {	// Use setting name for Item:
            $setting_name = 'coll_apply_rendering';
        } else {	// Invalid params, Exit here:
            return '';
        }
        ...
        foreach ($renderer_checkbox_options as $option) {
            ...
            if ($option['checked']) {	// Is checked:
                $r .= ' checked="checked"';
            }
            if ($option['disabled']) {	// Is disabled:
                $r .= ' disabled="disabled"';
            }
            $r .= ' title="' . format_to_output($option['short_desc'], 'formvalue') . '"';
            if (! empty($js_prefix)) {	// Set prefix, Used in JS code to disable/enable plugin toolbar:
        ...
        if (empty($renderer_checkbox_options)) {
            if (is_admin_page()) { // Display info about no renderer plugins only in backoffice
                if (check_user_perm('admin', 'normal')) {
                    global $admin_url;
                    switch ($setting_name) {
                        case 'msg_apply_rendering':
                            if (check_user_perm('perm_messaging', 'reply') && check_user_perm('options', 'edit')) { // Check if current user can edit the messaging settings
                                $settings_url = $admin_url . '?ctrl=msgsettings&amp;tab=renderers';
                            }
                            break;

                        case 'email_apply_rendering':
                            if (check_user_perm('perm_messaging', 'reply') && check_user_perm('options', 'edit')) { // Check if current user can edit the email settings
                                $settings_url = $admin_url . '?ctrl=email&amp;tab=settings&amp;tab3=renderers';
                            }
                            break;

                        case 'shared_apply_rendering':
                            if (check_user_perm('options', 'edit')) {	// Check if current user can edit the plugin settings for shared container:
                        ...
                        case 'coll_apply_rendering':
                        default:
                            if (! empty($setting_Blog) && check_user_perm('blog_properties', 'edit', false, $setting_Blog->ID)) { // Check if current user can edit the blog plugin settings
                ...
                if (! empty($settings_url)) { // Display a link to plugin settings only when current user has a permission to edit them
     ...
     */
    public function display_captcha($params = [])
    {
        if (! isset($params['Form']) ||
            ! isset($params['form_type']) ||
            ! isset($params['form_position'])) {	// Exit here if the mandatory params are not defined:
            return;
        }
        ...
        if (! isset($this->captcha_data)) {	// Initialize array to cache captcha data per current page request:
            $this->captcha_data = [];
        }

        if (! isset($this->captcha_data[$form_type])) {	// Load once captcha data per form type and use this for all next calls of this function:
        ...
        if (isset($captcha_data['captcha_position'], $captcha_data['captcha_html']) &&
            $captcha_data['captcha_position'] == $params['form_position']) {	// Display captcha html code only for requested form type and position:
            $Form = &$params['Form'];
            if (! isset($params['form_use_fieldset']) || $params['form_use_fieldset']) {	// Begin fieldset if it is required from skin file:
                $Form->begin_fieldset();
            }

            $Form->info_field(T_('Antispam'), $captcha_data['captcha_html'], [
                'note' => (isset($captcha_data['captcha_info']) ? $captcha_data['captcha_info'] : $params['captcha_info']),
                'required' => true,
            ]);

            if (! isset($params['form_use_fieldset']) || $params['form_use_fieldset']) {	// End fieldset if it is required from skin file:
                $Form->end_fieldset();
            }
        }
    }
     ...
     */
    public function count_regs($classname)
    {
        global $Debuglog;
     ...
     * @return array validated array of renderer codes
     */
    public function validate_list($renderers = ['default'])
    {
