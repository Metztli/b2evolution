...
class Plugins_admin extends Plugins
{
    ...
    public function get_supported_events()
    {
        ...
        if (empty($supported_events)) {
            ...
            if (! defined('EVO_IS_INSTALLING') || ! EVO_IS_INSTALLING) { // only call this, if we're not in the process of installation, to avoid errors from Plugins in this case!
        ...
        foreach ($subdirs as $subdir) {
            ...
            foreach ($files as $filename) {
                ...
                if ($this->get_by_classname($classname)) {
                    $Debuglog->add('Skipping duplicate plugin (classname ' . $classname . ')!', ['error', 'plugins']);
                    continue;
                }
     ...
     * @return array
     */
    public function get_apply_rendering_values($with_desc = false)
    {
     ...
     * @return array
     */
    public function get_registered_events($Plugin)
    {
        ...
        if ($Plugin->group == 'rendering') { // All Plugin from 'rendering' groups handle the FilterCommentContent
        ...
        if (! function_exists('token_get_all')) {
            $Debuglog->add('get_registered_events(): PHP function token_get_all() is not available', ['plugins', 'error']);
            return [];
        }

        if (! is_readable($Plugin->classfile_path)) {
            $Debuglog->add('get_registered_events(): "' . $Plugin->classfile_path . '" is not readable.', ['plugins', 'error']);
            return [];
        }

        if (($classfile_contents = @file_get_contents($Plugin->classfile_path)) === false) {
            $Debuglog->add('get_registered_events(): "' . $Plugin->classfile_path . '" could not get read.', ['plugins', 'error']);
            return [];
        }
        ...
        foreach ($token_all as $token) {
            if (! is_array($token)) {	// Single characters does not interest us:
                continue;
            }

            if ($had_func_token && $token[0] == T_STRING) {	// We got a function name...
                if (! in_array($token[1], $plugin_class_methods)
                    && in_array($token[1], $supported_events)) {	// ...and it is unique and matches one of our supported events:
                    $plugin_class_methods[] = $token[1];
                }

                // Search for the next "function" token:
                $had_func_token = false;
            } elseif (! $had_func_token && $token[0] == T_FUNCTION) {	// Begin searching for the function name which must be a T_STRING:
                $had_func_token = true;
            }
        }

        if (! count($plugin_class_methods)) {
            $Debuglog->add('No functions found in file "' . $Plugin->classfile_path . '".', ['plugins', 'error']);
            return [];
        }
     ...
     */
    public function &install($classname, $plug_status = 'enabled', $classfile_path = null, $must_exists = true)
    {
        global $DB, $Debuglog;
        ...
        if (is_string($Plugin)) { // return error message from register()
            return $Plugin;
        }
        ...
        */
        ...
        if (! empty($dep_msgs['error'])) { // fatal errors (required dependencies):
            $this->unregister($Plugin, true);
        ...
        if (! empty($dep_msgs['note'])) { // Add dependency notes
        ...
        if (! defined('EVO_IS_INSTALLING') || ! EVO_IS_INSTALLING) { // do not sort, if we're installing/upgrading.. instantiating Plugins might cause a fatal error!
            $this->sort();
        }
    ...
    public function uninstall($plugin_ID)
    {
        ...
        if (strpos($plugin_events, 'RenderItemAs') !== false) { // Clear pre-rendered content cache, if RenderItemAs* events get removed:
            $DB->query('DELETE FROM T_items__prerendering WHERE 1=1');
        }
        if (strpos($plugin_events, 'FilterCommentContent') !== false) { // Clear pre-rendered comments content cache, if FilterCommentContent plugin get removed
     ...
     */
    public function save_events($Plugin, $enable_events = null, $disable_events = null)
    {
        global $DB, $Debuglog;
        ...
        if (is_null($enable_events)) { // Enable all events:
            $enable_events = $available_events;
        }
        if (is_null($disable_events)) {
            $disable_events = [];
        }
        if ($disable_events) { // Remove events to be disabled from enabled ones:
        ...
        if ($enable_events) {
            foreach ($enable_events as $l_event) {
                if (! isset($saved_events[$l_event]) || ! $saved_events[$l_event]) { // Event not saved yet or not enabled
        ...
        if ($disable_events) {
            foreach ($disable_events as $l_event) {
                if (! isset($saved_events[$l_event]) || $saved_events[$l_event]) { // Event not saved yet or enabled
        ...
        if ($r) { // Something has changed: Reload event index
            foreach (array_merge($obsolete_events, $discovered_events, $new_events_enabled, $new_events_disabled) as $event) {
                if (strpos($event, 'RenderItemAs') === 0) { // Clear pre-rendered content cache, if RenderItemAs* events have been added or removed:
                    $DB->query('DELETE FROM T_items__prerendering WHERE 1=1');
    ...
    public function reload_plugins()
    {
        ...
        $this->load_events();
        $changed = false;
        while ($loop_Plugin = &$this->get_next()) { // loop through in each plugin
            // NOTE: we don't need to handle plug_version here, because it gets handled in Plugins::register() already.

            // Discover new events:
            if ($this->save_events($loop_Plugin, [])) {
                $changed = true;
            }

            // Detect plugins with no code and try to have at least one plugin with the default code:
            if (empty($loop_Plugin->code)) { // Instantiated Plugin has no code
        ...
        if ($changed) { // invalidate all PageCaches
     ...
     */
    public function set_code($plugin_ID, $code)
    {
        global $DB;
        ...
        if (! empty($code) && isset($this->index_code_ID[$code])) {
            if ($this->index_code_ID[$code] == $plugin_ID) { // Already set to same value
                return true;
            } elseif ($this->index_code_ID[$code] > 0) { // If code exists in DB for another plugin
        ...
        if ($Plugin->code == $code) { // Don't update if code has not been changed
            return false;
        }
        ...
        if ($result) { // Update references to code:
            // Widgets
     ...
     */
    public function set_event_status($plugin_ID, $plugin_event, $enabled)
    {
        global $DB;
        ...
        if ($DB->rows_affected) {
            ...
            if (strpos($plugin_event, 'RenderItemAs') === 0) { // Clear pre-rendered content cache, if RenderItemAs* events have been added or removed:
                $DB->query('DELETE FROM T_items__prerendering WHERE 1=1');
                $ItemCache = &get_ItemCache();
                $ItemCache->clear();
            }
     ...
     */
    public function set_priority($plugin_ID, $priority)
    {
        global $DB;
        ...
        if ($Plugin->priority == $priority) { // Already set to same value
            return true;
        }
     ...
     */
    public function sort($order = 'priority')
    {
        $this->load_plugins_table();

        foreach ($this->sorted_IDs as $k => $plugin_ID) { // Instantiate every plugin, so invalid ones do not get unregistered during sorting (crashes PHP, because $sorted_IDs gets changed etc)
        ...
        switch ($order) {
            case 'name':
                usort($this->sorted_IDs, [&$this, 'sort_Plugin_name']);
                break;

            case 'group':
                usort($this->sorted_IDs, [&$this, 'sort_Plugin_group']);
                break;

            default:
                // Sort array by priority:
                usort($this->sorted_IDs, [&$this, 'sort_Plugin_priority']);
        }
     ...
     */
    public function sort_Plugin_priority(&$a_ID, &$b_ID)
    {
     ...
     * WARNING: do NOT sort by anything else than priority unless you're handling a list of NOT-YET-INSTALLED plugins
     */
    public function sort_Plugin_name(&$a_ID, &$b_ID)
    {
        $a_Plugin = &$this->get_by_ID($a_ID);
        $b_Plugin = &$this->get_by_ID($b_ID);
     ...
     * WARNING: do NOT sort by anything else than priority unless you're handling a list of NOT-YET-INSTALLED plugins
     */
    public function sort_Plugin_group(&$a_ID, &$b_ID)
    {
        $a_Plugin = &$this->get_by_ID($a_ID);
        $b_Plugin = &$this->get_by_ID($b_ID);
     ...
     */
    public function validate_dependencies(&$Plugin, $mode)
    {
        global $DB, $app_name;
        global $app_version;
        ...
        if ($mode == 'disable') { // Check the whole list of installed plugins if they depend on our Plugin or it's (set of) events.
            $required_by_plugin = []; // a list of plugin classnames that require our poor Plugin

            foreach ($this->sorted_IDs as $validate_against_ID) {
                if ($validate_against_ID == $Plugin->ID) { // the plugin itself
                    continue;
                }

                $against_Plugin = &$this->get_by_ID($validate_against_ID);

                if ($against_Plugin->status != 'enabled') { // The plugin is not enabled (this check is needed when checking deps with the Plugins_admin class)
                    continue;
                }

                $deps = $against_Plugin->GetDependencies();

                if (empty($deps['requires'])) { // has no dependencies
                    continue;
                }
                ...
                if (! empty($deps['requires']['plugins'])) {
                    foreach ($deps['requires']['plugins'] as $l_req_plugin) {
                        if (! is_array($l_req_plugin)) {
                            $l_req_plugin = [$l_req_plugin, 0];
                        }

                        if ($Plugin->classname == $l_req_plugin[0]) { // our plugin is required by this one, check if it is the only instance
                ...
                if (! empty($deps['requires']['events_by_one'])) {
                    foreach ($deps['requires']['events_by_one'] as $req_events) {
                        ...
                        if (in_array($Plugin->ID, $provided_by) && count($provided_by) < 2) { // we're the only Plugin which provides this set of events
                ...
                if (! empty($deps['requires']['events'])) {
                    foreach ($deps['requires']['events'] as $req_event) {
                        ...
                        if (in_array($Plugin->ID, $provided_by) && count($provided_by) < 2) { // we're the only Plugin which provides this event
            ...
            if (! empty($required_by_plugin)) { // Prepend the message to the beginning, because it's the most restrictive (IMHO)
        ...
        foreach ($deps as $class => $dep_list) { // class: "requires" or "recommends"
            if (! is_array($dep_list)) { // Invalid format: "throw" error (needs not translation)
                return [
                    'error' => ['GetDependencies() did not return array of arrays. Please contact the plugin developer.'],
                ];
            }
                ...
                switch ($type) {
                    case 'events_by_one':
                        foreach ($type_params as $sub_param) {
                            if (! is_array($sub_param)) { // Invalid format: "throw" error (needs not translation)
                                return [
                                    'error' => ['GetDependencies() did not return array of arrays for "events_by_one". Please contact the plugin developer.'],
                                ];
                            }
                    ...
                    case 'plugins':
                        if (! is_array($type_params)) { // Invalid format: "throw" error (needs not translation)
                            return [
                                'error' => ['GetDependencies() did not return array of arrays for "plugins". Please contact the plugin developer.'],
                            ];
                        }
                        foreach ($type_params as $plugin_req) {
                            if (! is_array($plugin_req)) {
                                $plugin_req = [$plugin_req, '0'];
                            ...
                            if ($versions = $DB->get_col('
                                ...
                                $clean_versions = preg_replace(['~^(CVS\s+)?\$' . 'Revision:\s*~i', '~\s*\$$~'], '', $versions);
                                ...
                                if (evo_version_compare($clean_oldest_enabled, $clean_req_ver, '<')) { // at least one instance of the installed plugins is not the current version
     ...
     */
    public function process_event_filtering($renderers, &$params)
    {
        if (! isset($params['event']) || ! in_array($params['event'], ['FilterItemContents', 'UnfilterItemContents'])) { // invalid event param
            return false;
        }
        ...
        if (isset($params['object_type']) && $params['object_type'] == 'Comment') { // Comment
        ...
        foreach ($filter_Plugins as $loop_filter_Plugin) { // Go through whole list of renders
            if (isset($params['object_type']) && $params['object_type'] == 'Message') { // Message
                $rendering_setting_value = $loop_filter_Plugin->get_msg_setting('msg_apply_rendering');
            } else { // Item OR Comment
                $rendering_setting_value = $loop_filter_Plugin->get_coll_setting($rendering_setting_name, $params['object_Blog']);
            }

            if ($loop_filter_Plugin->is_renderer_enabled($rendering_setting_value, $renderers)) { // Plugin is enabled to call method
     ...
     * @return mixed string rendered content on success | false on failure
     */
    public function filter_contents(&$title, &$content, $renderers, &$params)
    {
        $params = array_merge(
            [
     ...
     * @return mixed string rendered content on success | false on failure
     */
    public function unfilter_contents(&$title, &$content, $renderers, &$params)
    {
        $params = array_merge(
            [
