...
function autoform_display_field($parname, $parmeta, &$Form, $set_type, $Obj, $set_target = null, $use_value = null)
{
    ...
    if (! is_array($parmeta)) {	// Must be array:
        return;
    }

    if (! empty($parmeta['no_edit'])) { // this setting is not editable
        return;
    }

    $params = [];

    if ($use_value === null) { // outermost level
    ...
    if (isset($parmeta['group'])) {
        $group = $parmeta['group'];
        if (substr($group, -1) === ']') {	// If group name is in array format like "edit_plugin_1_set_sample_sets[0][group_name]",
        ...
        } else {	// If group name is simple like "group_name",
    ...
    foreach ($parmeta as $k => $v) {
        if (in_array($k, ['id', 'class', 'onchange', 'onclick', 'onfocus', 'onkeyup', 'onkeydown', 'onreset', 'onselect', 'cols', 'rows', 'maxlength', 'placeholder'])) {
            $params[$k] = $v;
        }
    }
    if (! empty($parmeta['multiple'])) { // "multiple" attribute for "select" inputs:
    ...
    if (strpos($parmeta['type'], 'select_') === 0) { // 'allow_none' setting for select_* types
    ...
    if (isset($parmeta['help'])) {
        if ($parmeta['help'] === true) { // link to $parname-target:
            $help_target = '#' . preg_replace(['~\]?\[\d+\]\[~', '~\]$~'], ['_', ''], $parname);
        } else {
    ...
    if (isset($parmeta['layout'])) {
        switch ($parmeta['layout']) {
            case 'begin_fieldset':
                ...
                if ($Form->layout == 'accordion' && ! empty($Form->_opentags['fieldset'])) {	// Use accordion style only for top level:
                    $Form->switch_layout('fieldset');
                }
                $fieldset_params = [];
                if (isset($parmeta['fold']) && $parmeta['fold'] === true) {	// Enable folding for the fieldset:
                    $fieldset_params['fold'] = $parmeta['fold'];
                    if (isset($parmeta['deny_fold'])) {	// TRUE to don't allow fold the block and keep it opened always on page loading:
            ...
            case 'end_fieldset':
                $Form->end_fieldset();
                if (isset($Form->saved_layouts[0]) && $Form->saved_layouts[0] == 'accordion' && ! empty($Form->_opentags['fieldset'])) {	// We should switch back to accordion style:
    ...
    if (! empty($help_icon)) { // Append help icon to note:
    ...
    if (isset($parmeta['input_prefix'])) {	// Use prefix before input element if it is defined:
        $params['input_prefix'] = $parmeta['input_prefix'];
    }

    if (isset($parmeta['input_suffix'])) {	// Use suffix after input element if it is defined:
        $params['input_suffix'] = $parmeta['input_suffix'];
    }

    if ($set_type == 'Skin' &&
        in_array($parmeta['type'], ['text', 'integer', 'color', 'select', 'select_object'])) {	// Append action icon to restore setting to default value by JavaScript:
        $params['input_suffix'] = (isset($params['input_suffix']) ? $params['input_suffix'] : '') .
            ' ' . get_icon('reload', 'imgtag', [
    ...
    if (substr($parmeta['type'], 0, 6) == 'select' && ! empty($parmeta['multiple'])) { // a "multiple" select:
        $input_name .= '[]';
    }
    ...
    if (! empty($parmeta['hide'])) {	// Hide this field on the editing form:
    ...
    switch ($parmeta['type']) {
        ...
        case 'checklist':
            ...
            foreach ($parmeta['options'] as $meta_option) {
                ...
                $options[] = [$input_name . '[' . $meta_option[0] . ']', 1, $meta_option[1], $meta_option_checked, $meta_option_disabled, $meta_option_note, $meta_option_class, $meta_option_hidden, $meta_option_label_attribs];
            }
        ...
        case 'select':
            ...
            if (! empty($parmeta['multiple'])) {	// Set specific size of multiple selector or use automatic size depending on count of options:
        ...
        case 'array:regexp':
            ...
            $Form->switch_layout('fieldset');
            if (substr_count($parname, '[') % 2) { // this refers to a specific array type set (with index pos at the end), e.g. when adding a field through AJAX:
            ...
            } else { // display all values hold in this set:
                $disp_whole_set = true;
            ...
            if (is_array($set_value) && ! empty($set_value)) { // Display value of the setting. It may be empty, if there's no set yet.
                foreach ($disp_arrays as $k => $v) {
                    ...
                    $remove_action = '';
                    if (! isset($parmeta['min_count']) || count($set_value) > $parmeta['min_count']) { // provide icon to remove this set
                        $remove_action = '<span class="pull-right">' . action_icon(
                            T_('Remove'),
                            'minus',
                            regenerate_url('action', ['action=del_settings_set&amp;set_path=' . $parname . '[' . $k . ']' . ($set_type == 'UserSettings' ? '&amp;user_ID=' . $user_ID : ''), 'plugin_ID=' . $Obj->ID]),
                            T_('Remove'),
                            5,
                            3, /* icon/text prio */
                    ...
                    if (isset($parmeta['key'])) { // KEY FOR THIS ENTRY:
                        if (! strlen($k) && isset($parmeta['key']['defaultvalue'])) { // key is not given/set and we have a default:
                    ...
                    foreach ($parmeta['entries'] as $l_set_name => $l_set_entry) {
                        if (isset($set_value[$k][$l_set_name])) {	// Use a saved value:
                            $l_value = $set_value[$k][$l_set_name];
                        } else {	// Use default value if it is defined:
            ...
            if (! is_ajax_request()) {	// Don't display "Add" button twice after AJAX request:
                global $Blog;
                ...
                echo action_icon(
                    ...
                    'add',
                    regenerate_url('action', ['action=add_settings_set', 'set_path=' . $set_path . ($set_type == 'UserSettings' ? '&amp;user_ID=' . get_param('user_ID') : ''), 'plugin_ID=' . $Obj->ID]),
                    T_('Add'),
                    5,
                    3, /* icon/text prio */
                    [
            ...
            if (! empty($disp_whole_set)) { // close the surrounding fieldset:
                $Form->end_fieldset();
            }
            $Form->switch_layout(null);
        ...
        case 'text':
            ...
            } else { // do not use size as maxlength, if not given!
                $params['maxlength'] = '';
            }

            if (isset($parmeta['hide_label'])) { // This param is used to hide a label
                $params['hide_label'] = $parmeta['hide_label'];
            }

            if ($parmeta['type'] == 'integer') {	// Set special type 'number' for integer param to initialize control arrows to allow increase/decrease a value:
                $params['type'] = 'number';
                if (isset($parmeta['valid_range']['min'])) {	// Restrict with min value:
                    $params['min'] = $parmeta['valid_range']['min'];
                }
                if (isset($parmeta['valid_range']['max'])) {	// Restrict with max value:
            ...
            if (isset($parmeta['suffix'])) {	// Set suffix text after input element:
    ...
    if (isset($original_form_fieldstart)) {	// Revert original field start html code:
        $Form->fieldstart = $original_form_fieldstart;
    }
    if (isset($original_form_fieldstart_checkbox)) {	// Revert original field start html code:
        $Form->fieldstart_checkbox = $original_form_fieldstart_checkbox;
    }
    if (isset($original_form_fieldstart_radio)) {	// Revert original field start html code:
        $Form->fieldstart_radio = $original_form_fieldstart_radio;
    }

    if ($outer_most && $has_array_type) { // Note for Non-Javascript users:
 ...
 * @return array|false
 */
function _set_setting_by_path(&$Plugin, $set_type, $path, $init_value = [])
{
    ...
    #}
    #else
    if (is_null($init_value)) { // NULL is meant to unset it
        ...
        foreach ($set_meta['entries'] as $k => $v) {
            if (isset($v['defaultvalue'])) { // set to defaultvalue
...
function get_plugin_settings_node_by_path(&$Plugin, $set_type, $path, $create = false)
{
    ...
    if (! is_array($setting)) { // this may happen, if there was a non-array setting stored previously:
        // discard those!
        $setting = [];
    }
...
function autoform_set_param_from_request($parname, $parmeta, &$Obj, $set_type, $set_target = null, $set_value = null)
{
    ...
    if (isset($parmeta['layout'])) { // a layout "setting"
        return;
    }

    if ((! empty($parmeta['disabled']) || ! empty($parmeta['no_edit']))
        && $set_value === null) { // the setting is disabled, but allow to update the value when it is forced by $set_value
        return;
    }
    ...
    if ($set_value === null) { // Get the value from request:
            ...
            case 'array:regexp':
                if (! empty($parmeta['entries'])) {
                    foreach ($l_value as $l_index => $l_index_values) {
                        if (count($parmeta['entries']) != count($l_index_values)) {	// If some entry(like checkbox) is missed:
                            foreach ($parmeta['entries'] as $parmeta_entry_key => $parmeta_entry_data) {
                                if (isset($parmeta_entry_data['type']) &&
                                    ...
                                    ! isset($l_index_values[$parmeta_entry_key])) {	// If field is checkbox but value is not passed to a submitted form becauase it was unchecked:
    ...
    if (isset($parmeta['type'])) {	// Prepare values before store them in DB:
        if (strpos($parmeta['type'], 'array') === 0) {	// Make keys (__key__) in arrays unique and remove them
            handle_array_keys_in_plugin_settings($l_value);
        } elseif ($parmeta['type'] == 'integer') {	// Convert to correct integer value:
            if ($l_value !== '') {	// Don't convert empty string '' to integer 0:
                $l_value = intval($l_value);
            }
            if (empty($l_value) && ! empty($parmeta['allow_empty']) &&
                    isset($parmeta['valid_range'], $parmeta['valid_range']['min']) && $parmeta['valid_range']['min'] > 0) {	// Convert 0 to empty value for integer field if it allows empty values:
                $l_value = null;
            }
        }
    }
    ...
    switch ($set_type) {
        ...
        case 'Widget':
            $error_value = null;
            if (isset($parmeta['type']) && $parmeta['type'] == 'checklist' && $parname == 'renderers') {	// Save "stealth" and "always" plugin render options:
                // (they are hidden or disabled checkboxes of the form and cannot be submitted automatically)
                global $Plugins;
                $widget_Blog = &$Obj->get_Blog();
                $l_value = $Plugins->validate_renderer_list(array_keys($l_value), [
                    'Blog' => &$widget_Blog,
        ...
        case 'SharedSettings':
            ...
            if (empty($error_value)) {	// Set new value:
                if ($set_type == 'MsgSettings' && $parname != 'msg_apply_rendering') {	// Use prefix 'msg_' for all message settings except of "msg_apply_rendering":
                    $Obj->Settings->set('msg_' . $parname, $l_value);
                } elseif ($set_type == 'EmailSettings' && $parname != 'email_apply_rendering') {	// Use prefix 'email_' for all email settings except of "email_apply_rendering":
                    $Obj->Settings->set('email_' . $parname, $l_value);
                } elseif ($set_type == 'SharedSettings' && $parname != 'shared_apply_rendering') {	// Use prefix 'shared_' for all shared settings except of "shared_apply_rendering":
    ...
    if ($error_value) { // A validation error has occured, record error message:
...
function autoform_validate_param_value($param_name, $value, $meta)
{
    ...
    if (! is_array($meta)) {	// Must be array:
        return;
    }

    if (is_array($value) && isset($meta['entries'])) {
        $r = true;
        if (isset($meta['key'])) { // validate keys:
        ...
        foreach ($meta['entries'] as $mk => $mv) {
            foreach ($value as $vk => $vv) {
                if (isset($mv['type']) && $mv['type'] == 'input_group' &&
                    ! empty($mv['inputs']) && is_array($mv['inputs'])) {	// Validate each input field of the setting with type "input_group":
                    foreach ($mv['inputs'] as $k => $v) {
                        if (isset($vv[$mk . $k]) && ! autoform_validate_param_value($param_name . '[' . $vk . '][' . $mk . $k . ']', $vv[$mk . $k], $v)) {	// If at least one setting of the grouped inputs has an error:
                            $r = false;
                        }
                    }
                } elseif (isset($vv[$mk]) && ! autoform_validate_param_value($param_name . '[' . $vk . '][' . $mk . ']', $vv[$mk], $mv)) {	// If single setting has an error:
                    $r = false;
                }
            }
        }
    ...
    if (isset($meta['type'])) {
        if (isset($meta['allow_empty']) && $meta['allow_empty'] && empty($value)) { // Allow an empty value
            return true;
        }

        switch ($meta['type']) {
            case 'text':
                if (isset($meta['allow_empty']) && ! $meta['allow_empty'] && $value === '') {	// Display error if the text field is required to be not empty:
            ...
            case 'select':
                $check_options = $value;
                if (! is_array($check_options)) { // no "multiple" select:
                ...
                foreach ($meta['options'] as $meta_option_key => $meta_option_value) {
                    if (is_array($meta_option_value)) {	// It is a grouped options:
                ...
                foreach ($check_options as $v) {
                    if (empty($v) && ! empty($meta['allow_none'])) { // empty is ok:
                        continue;
                    }
            ...
            case 'select_user':
                if (is_array($value) && empty($value) // empty "multiple" select
                    ...
                    if (empty($meta['allow_none'])) { // empty is not ok
                        param_error($param_name, sprintf(T_('Invalid option &laquo;%s&raquo;.'), $value));
                        return false;
                    }
                } else { // Try retrieving the value from the corresponding Cache:
                    switch ($meta['type']) {
                        case 'select_blog':
                            $Cache = &get_BlogCache();
                            break;
...
function handle_array_keys_in_plugin_settings(&$a)
{
    ...
    $new_arr = []; // use a new array to maintain order, also for "numeric" keys
    ...
    foreach (array_keys($a) as $k) {
        ...
        if (is_array($v) && isset($v['__key__'])) {
            if ($k != $v['__key__']) {
                $k = $v['__key__'];
                if (! strlen($k) || isset($a[$k])) { // key already exists (or is empty):
                    $c = 1;
...
function install_plugin_db_schema_action(&$Plugin, $force_install_db_deltas = false)
{
    ...
    $install_db_deltas = []; // This holds changes to make, if any (just all queries)
    //pre_dump( $db_layout );

    if (! empty($db_layout)) { // The plugin has a DB layout attached
        ...
        foreach (db_delta($db_layout) as $table => $queries) {
            foreach ($queries as $query_info) {
                foreach ($query_info['queries'] as $query) { // subqueries for this query (usually one, but may include required other queries)
                    $install_db_deltas[] = $query;
                }
            }
        }

        if (! empty($install_db_deltas)) { // delta queries to make
            if (empty($install_db_deltas_confirm_md5) && ! $force_install_db_deltas) { // delta queries have to be confirmed in payload
                return false;
            } elseif ($install_db_deltas_confirm_md5 == md5(implode('', $install_db_deltas)) || $force_install_db_deltas) { // Confirmed in first step:
