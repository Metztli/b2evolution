...
if (isset($_POST[$dummy_fields['login']]) && isset($_POST[$dummy_fields['pwd']])) {	// Trying to log in with a POST:
...
} elseif (isset($_GET[$dummy_fields['login']])) {	// Trying to log in with a GET; we might only provide a user here.
...
} elseif (empty($disable_http_auth) && $Settings->get('http_auth_accept') && ! $Session->has_User() && isset($_SERVER['PHP_AUTH_USER'], $_SERVER['PHP_AUTH_PW'])) {	// Trying to log in with HTTP basic authentication:
...
if (($login != null) && (! is_string($login))) { // Login must be string
    $login = null;
    if (! empty($login_action_value)) { // This was a login request with an invalid login parameter type, so it must be a doctored request
        debug_die('The type of the received login parameter is invalid!');
    }
}
if (($pass != null) && (! is_string($pass))) { // Password must be string
    $pass = null;
    if (! empty($login_action_value)) { // This was a login request with an invalid pwd parameter type, so it must be a doctored request
...
if (! empty($login_action_value) || (! empty($login) && ! empty($pass))) { // User is trying to login right now
    ...
    if ($check_login_crumb) {	// Check that this login request is not a CSRF hacked request:
    ...
    if (can_use_hashed_password()) {
        param('pwd_hashed', 'array:string', []);
    } else { // at least one plugin requests the password un-hashed:
        $pwd_hashed = [];
    }
    ...
    if ($Plugins->trigger_event('LoginAttempt', [
        'login' => &$login,
        'pass' => &$pass,
        'pass_md5' => &$pass_md5,
    ...
    if (! empty($login_error)) { // A plugin has thrown a login error..
    ...
    } else { // Check login and password
        if (is_email($login)) { // we have an email address instead of login name
        ...
        } elseif (param_check_valid_login($dummy_fields['login'])) { // Make sure that we can load the user:
            $User = &$UserCache->get_by_login($login);
        } else {
            $User = false;
        }

        if ($User) { // Check user login attempts
        ...
        if ($User && ! $pass_ok && empty($login_error)) { // check the password, if no plugin has said "it's ok":
            if (! empty($pwd_hashed)) { // password hashed by JavaScript:
                $Debuglog->add('Login: Hashed password available.', '_init_login');

                if (empty($pepper_sess)) { // no salt stored in session: either cookie problem or the user had already tried logging in (from another window for example)
                    $Debuglog->add('Login: Empty salt_sess!', '_init_login');
                    if (($pos = strpos($pass, '_hashed_')) && substr($pass, $pos + 8) == $Session->ID) { // session ID matches, no cookie problem
                        $login_error = T_('The login window has expired. Please try again.');
                        $Debuglog->add('Login: Session ID matches.', '_init_login');
                    } else { // more general error:
                ...
                } elseif ($pepper != $pepper_sess) { // submitted salt differs from the one stored in the session
                    $login_error = T_('The login window has expired. Please try again.');
                    $Debuglog->add('Login: Submitted salt and salt from Session do not match.', '_init_login');
                } else { // compare the password, using the salt stored in the Session:
                    ...
                    foreach ($pwd_hashed as $encrypted_password) {
                        ...
                        if ($pass_ok) { // Break after the first matching password
                            break;
                        }
                    }
            ...
            } else {	// Password NOT hashed by Javascript:
                $pass_ok = $User->check_password($pass);
                $Debuglog->add('Login: Compared raw passwords. Result: ' . (int) $pass_ok, '_init_login');
                if ($report_wrong_pass_hashing && $pass_ok && can_use_hashed_password()) {	// Report about this unsecure login action:
    ...
    if ($pass_ok) { // Login succeeded, set cookies
        ...
        if (check_user_status('is_closed')) { // user account was closed
            unset($current_User);
            $login_error = T_('This account is closed. You cannot log in.');
        } elseif ($Settings->get('system_lock') && ! check_user_perm('users', 'edit')) { // System is locked for maintenance and current user has no permission to log in this mode
            unset($current_User);
            $login_error = T_('You cannot log in at this time because the system is under maintenance. Please try again in a few moments.');
        } else { // save the user for later hits
            ...
            if ((! isset($locales[$current_user_locale])) || (! $locales[$current_user_locale]['enabled'])) { // Current user locale doesn't exists or it is not enabled, update to the default locale if it is valid
                $def_locale = $Settings->get('default_locale');
                if (isset($locales[$def_locale]) && $locales[$def_locale]['enabled']) { // Update user locale to the default, and add warning message about the update
            ...
            if ($Settings->get('system_lock') && check_user_perm('users', 'edit')) { // System is locked for maintenance but current user has permission to log in, Display a message about this mode
        ...
        if (! empty($login_attempts)) { // Display all attempts on success login
            ...
            foreach ($login_attempts as $attempt) {
                ...
                if (! empty($geoip_Plugin) && $Country = $geoip_Plugin->get_country_by_IP($attempt_ip)) { // Get country by IP if plugin is enabled
                    $plugin_country_by_IP = ' (' . $Country->get_name() . ')';
                }

                if ($attempt_ip != $current_ip) { // Get DNS by IP if current IP is different from attempt IP
                    $attempt_ip .= ' ' . gethostbyaddr($attempt_ip);
                }

                $Messages->add_to_group(sprintf(
                    T_('Someone tried to log in to your account with a wrong password on %s from %s%s'),
    ...
    } elseif (empty($login_error)) { // if the login_error wasn't set yet, add the default one:
        // This will cause the login screen to "popup" (again)
        if ($login_mode == 'http_basic_auth') {	// If wrong login from HTTP basic authentication
            if (! empty($is_login_page)) {	// Display this error and login form only if user really is requesting a login page:
                $login_error = T_('Wrong Login/Password provided by browser (HTTP Auth).');
            }
        } else {	// If wrong login from standard POST forms or GET request:
            $login_error = T_('The Login/Password you entered is wrong.');
        }

        $current_login_pass = $login . ':' . (empty($pwd_hashed) ? $pass : implode('', $pwd_hashed));

        if (isset($login_attempts) && $current_login_pass != $Session->get('wrong_loginpass')) {	// Save new login attempt into DB only if previous login data were different:
    ...
    (empty($login) || (($tmp_User = &$UserCache->get_by_ID($Session->user_ID)) && $login == $tmp_User->login))) { /* if the session has a user assigned to it:
    ...
    if ($Settings->get('system_lock')) { // System is locked for maintenance
        if (check_user_perm('users', 'edit')) { // Current user is a "super admin"
            if (! $Messages->count()) { // If there are no other messages yet, display a warning about the system lock
                $Messages->add(T_('The site is currently locked for maintenance.') . ' ' . sprintf(T_('Click <a %s>here</a> to access lock settings.'), 'href="' . $admin_url . '?ctrl=tools"'), 'warning');
            }
        } else { // Current user has no permission to be logged in on this mode, we must logout it
            logout();
...
} else { // The Session has no user or $login is given (and differs from current user), allow alternate authentication through Plugin:
...
if (check_user_status('can_be_validated') // user is logged in but not validated and validation is required
    && $action != 'logout'
    && $action != 'req_activate_email' && $action != 'activateacc_sec' && $validate_required) { // we're not in that action already:
...
if (! empty($login_action_value) && empty($login_error) && ($action != 'logout')) { // Trigger plugin event that allows the plugins to re-act on the login event:
    // TODO: dh> these events should provide a flag "login_attempt_failed".
    if (empty($current_User)) {
        $Plugins->trigger_event('AfterLoginAnonymousUser', []);
    } else {
        $Plugins->trigger_event('AfterLoginRegisteredUser', []);

        if (! empty($login_action_value)) { // We're coming from the Login form and need to redirect to the requested page:
            $redirect_to = param('redirect_to', 'url', $baseurl);
            if (empty($redirect_to) ||
                preg_match('#/login.php([&?].*)?$#', $redirect_to) ||
                preg_match('#/register.php([&?].*)?$#', $redirect_to) ||
                preg_match('#disp=(login|register|lostpassword)#', $redirect_to)) { // avoid redirect back to login/register screen. This shouldn't occur.
...
if (! empty($login_error) || ($login_required && ! is_logged_in())) { // ----- LOGIN FAILED ----- OR Login is required and user is not logged in yet
    ...
    if ($inskin || use_in_skin_login()) { // Use in-skin login
        $Debuglog->add('Trying to use in-skin login', '_init_login');

        if (is_logged_in()) { // user is logged in, but the email address is not validated yet
            $login = $current_User->login;
            $email = $current_User->email;
        }

        if (empty($Blog) && init_requested_coll_or_process_tinyurl(false, false)) { // $blog is set, init $Blog also
            $BlogCache = &get_BlogCache();
            $Collection = $Blog = $BlogCache->get_by_ID($blog, false, false);
        }

        $blog_skin_ID = null;
        if (! empty($Blog)) { // Blog was set
            $blog_skin_ID = $Blog->get_skin_ID();
        }

        if (! empty($blog_skin_ID)) { // Blog exists and skin ID is set
            ...
            if (file_exists($ads_current_skin_path . $disp . '.main.php')) {	// Call custom file for login disp if it exists:
                require $ads_current_skin_path . $disp . '.main.php';
            } else {	// Call index main skin file to display a login disp:
