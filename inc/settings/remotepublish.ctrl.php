...
switch ($action) {
        ...
        switch ($tab) {
            case 'eblog':
                ...
                if (param('renderers_displayed', 'integer', 0)) { // use "renderers" value only if it has been displayed (may be empty)
                    ...
                    $renderers = $Plugins->validate_renderer_list(param('eblog_renderers', 'array:string', []), $renderer_params);
    ...
    case 'test_3':
        ...
        if ($action == 'test_1') {
            if ($mbox = pbm_connect()) {	// Close opened connection
                imap_close($mbox);
            }
            ...
            if ($mbox = pbm_connect()) {
                ...
                if ($imap_obj->Nmsgs > 0) {
                    if ($action == 'test_2') {	// Pretend that we're in test mode. DO NOT save this setting!
        ...
        if (! empty($pbm_messages)) {	// We will display the output in a scrollable fieldset
