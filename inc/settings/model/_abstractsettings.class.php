...
class AbstractSettings
{
     ...
     */
    public function __construct($db_table_name, $col_key_names, $col_value_name, $cache_by_col_keys = 0)
    {
     ...
     */
    public function _load($arg1 = null, $arg2 = null, $arg3 = null)
    {
        if ($this->all_loaded) { // already all loaded
            return true;
        }
        global $DB;
        ...
        if ($this->cache_by_col_keys && isset($arg1)) { // The number of column keys to cache by is > 0
            ...
            if ($i == $this->cache_by_col_keys) { // already loaded!
                return true;
            }
        } else { // We cache everything at once!
        ...
        switch ($this->count_col_key_names) {
            case 1:
                if (! $result) { // Remember that we've tried it
     ...
     */
    public function getx($col_key1, $col_key2 = null, $col_key3 = null)
    {
        global $debug;
        ...
        switch ($this->count_col_key_names) {
            case 1:
                $this->_load($col_key1);
                if (! empty($this->cache[$col_key1]->unserialized)) {	// The value has been unserialized before:
                    $r = $this->cache[$col_key1]->value;
                } elseif (isset($this->cache[$col_key1]->value)) {	// First attempt to access the value, we need to unserialize it:
                ...
                } else {	// The value is not in the cache, we use the default:
     ...
     */
    public function get_default($last_key)
    {
     ...
     */
    public function get_cond(&$toset)
    {
        $args = func_get_args();
        array_shift($args);

        $result = call_user_func_array([&$this, 'get'], $args);

        if ($result !== null && $result !== false) { // No error and value retrieved
            $toset = $result;
            return true;
        } else {
            return false;
    ...
    public function setx()
    {
        ...
        call_user_func_array([&$this, '_load'], $args);
        ...
        if (! is_object($atCache)) {	// PHP 5.4 fix for "Warning: Creating default object from empty value"
        ...
        if (isset($atCache->value)) {
            if ((is_array($value) && serialize($value) === $atCache->value) ||
                (! is_array($value) && $atCache->value === (string) $value)) { // already set
    ...
    public function set_array($array)
    {
        foreach ($array as $lSet) {
            call_user_func_array([&$this, 'set'], $lSet);
        }
    }
    ...
    public function delete_array($array)
    {
        foreach ($array as $lDel) {
            call_user_func_array([&$this, 'delete'], [$lDel]);
        }
    }
        ...
        switch ($this->count_col_key_names) {
            case 1:
                foreach ($this->cache as $key => $value) {
                    if ($value === null) { // Remembered as not existing
                        continue;
                    }
            ...
            case 2:
                foreach ($this->cache as $key => $value) {
                    foreach ($value as $key2 => $value2) {
                        if ($value2 === null) { // Remembered as not existing
                            continue;
                        }
                ...
                foreach ($this->cache as $key => $value) {
                    foreach ($value as $key2 => $value2) {
                        foreach ($value2 as $key3 => $value3) {
                            if ($value3 === null) { // Remembered as not existing
                                continue;
                            }
     ...
     */
    public function param_Request($param_name, $set_name, $type = '', $default = '', $memorize = false, $override = false) // we do not force setting it..
    {
        $value = param($param_name, $type, null, $memorize, $override, false); // we pass NULL here, to see if it got set at all

        if ($value !== false) { // we got a value
            $this->set($set_name, $value);
            $this->dbupdate();
        } else { // get the value from user settings
            $value = $this->get($set_name);

            if (is_null($value)) { // it's not saved yet and there's not default defined ($_defaults)
                $value = $default;
            }
        }
