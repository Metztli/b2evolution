...
$Form->checkbox_input(
    'eblog_enabled',
    $Settings->get('eblog_enabled'),
    TB_('Enable Post by email'),
    [
...
$Form->checkbox_input(
    'eblog_test_mode',
    $eblog_test_mode_value,
    TB_('Test Mode'),
    [
        'note' => TB_('Check to run Post by Email in test mode. Nothing will be posted to the database nor will your inbox be altered.'),
    ]
);

$Form->text_input('eblog_server_host', $Settings->get('eblog_server_host'), 25, TB_('Mail Server'), TB_('Hostname or IP address of your incoming mail server.'), [
    'maxlength' => 255,
]);

$Form->radio('eblog_method', $Settings->get('eblog_method'), [
    ['pop3', TB_('POP3')], // TRANS: E-Mail retrieval method
    ['imap', TB_('IMAP')], // TRANS: E-Mail retrieval method
], TB_('Retrieval method'));

$Form->radio('eblog_encrypt', $Settings->get('eblog_encrypt'), [
    ['none', TB_('None')],
    ['ssl', TB_('SSL')],
    ['tls', TB_('TLS')],
], TB_('Encryption method'));

$eblog_novalidatecert_params = [
    'lines' => true,
];
if ($Settings->get('eblog_encrypt') == 'none') {
    $eblog_novalidatecert_params['disabled'] = 'disabled';
}
$Form->radio_input('eblog_novalidatecert', $Settings->get('eblog_novalidatecert'), [
    [
        'value' => 1,
        'label' => TB_('Do not validate the certificate from the TLS/SSL server. Check this if you are using a self-signed certificate.'),
    ],
    [
        'value' => 0,
        'label' => TB_('Validate that the certificate from the TLS/SSL server can be trusted. Use this if you have a correctly signed certificate.'),
    ],
], TB_('Certificate validation'), $eblog_novalidatecert_params);

$Form->text_input('eblog_server_port', $Settings->get('eblog_server_port'), 5, TB_('Port Number'), TB_('Port number of your incoming mail server (Defaults: POP3: 110, IMAP: 143, SSL/TLS: 993).'), [
    'maxlength' => 6,
]);

$Form->text_input(
    'eblog_username',
    $Settings->get('eblog_username'),
    25,
    TB_('Account Name'),
    TB_('User name for authenticating on your mail server. Usually it\'s your email address or a part before the @ sign.'),
    [
        'maxlength' => 255,
    ]
);
...
$Form->password_input(
    'eblog_password',
    $Settings->get('eblog_password'),
    25,
    TB_('Password'),
    [
        'maxlength' => 255,
        'note' => TB_('Password for authenticating on your mail server.'),
    ]
);

$Form->checkbox(
    'eblog_delete_emails',
    $Settings->get('eblog_delete_emails'),
    TB_('Delete processed emails'),
...
$Form->text_input(
    ...
    5,
    TB_('Default Category ID'),
    sprintf(TB_('This is a default category for your posts.') . '<br />' .
    TB_('You can use the following code in message body to define categories: %s. In this example 2 will be main category and 5, 47 extra categories.'), '<b>&lt;category&gt;2, 5, 47&lt;/category&gt;</b>'),
    [
        'maxlength' => 6,
    ]
);
...
$Form->text_input(
    'eblog_default_title',
    $Settings->get('eblog_default_title'),
    50,
    TB_('Default title'),
    sprintf(TB_('This is a default title for your posts.') . '<br />' .
    TB_('You can use the following code in message body to define post title: %s.'), '<b>&lt;title&gt;Post title here&lt;/title&gt;</b>'),
    [
        'maxlength' => 255,
    ]
);
...
$Form->text_input(
    'eblog_subject_prefix',
    $Settings->get('eblog_subject_prefix'),
    15,
    TB_('Subject Prefix'),
    TB_('Email subject must start with this prefix to be imported, messages that don\'t have this tag will be skipped.'),
    [
        'maxlength' => 255,
    ]
);
...
$Form->text_input(
    'eblog_body_terminator',
    $Settings->get('eblog_body_terminator'),
    15,
    TB_('Body Terminator'),
    TB_('Starting from this string, everything will be ignored, including this string.') .
    '<br />' . TB_('You can use this to remove signature from message body.'),
    [
        'maxlength' => 255,
    ]
);
...
$Form->info(TB_('Text Renderers'), $Plugins->get_renderer_checkboxes($Settings->get('eblog_renderers'), [
    'name_prefix' => 'eblog_',
    'Blog' => &$setting_Blog,
]));
...
$Form->checkbox(
    'eblog_html_enabled',
    $Settings->get('eblog_html_enabled'),
    TB_('Enable HTML messages'),
    TB_('Check this if you want HTML messages to be processed and posted in your blog.')
);

$Form->checkbox(
    'eblog_html_tag_limit',
    $Settings->get('eblog_html_tag_limit'),
    TB_('Limit allowed tags'),
