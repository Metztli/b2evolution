...
if (empty($UserSettings)) { // initialize UserSettings, because in CLI mode is not initialized yet
...
$SQL->FROM_add('INNER JOIN T_items__item ON post_main_cat_ID = cat_ID AND post_status IN (' . $DB->quote(['published', 'community', 'protected']) . ')');
...
if (empty($moderation_blogs)) { // There are no blogs where exists draft comments older then the threshold ( 24 hours by default )
...
$moderators = [];
foreach ($specific_blog_moderators as $row) { // Loop through each different blog's moderator users and their permissions
    if (! isset($moderators[$row->user_ID])) { // Initialize user perm array
        $moderators[$row->user_ID] = [];
    }
    if (isset($moderators[$row->user_ID][$row->blog_ID])) { // Update user permissions on this blog
        if ($moderators[$row->user_ID][$row->blog_ID]['perm_edit'] < $row->perm_edit_cmt) { // The user and the group advanced comment edit perm for this user are not the same, keep the higher perm value
        ...
        if ($current_perm_statuses != $row_perm_status) { // The advanced user and the group comment statuses perm for this user are not the same, the union of this perms must be accepted
            $moderators[$row->user_ID][$row->blog_ID]['perm_statuses'] = ($current_perm_statuses | $row_perm_status);
        }
    } else { // Initialize a new setting for this user / blog
        $moderators[$row->user_ID][$row->blog_ID] = [
            'perm_edit' => $row->perm_edit_cmt,
            'perm_statuses' => (int) $row->perm_cmtstatuses,
        ];
    }
}
foreach ($blog_owners as $moderator_ID => $moderator_blogs) { // Loop through each blog owner users and set the highest permission in their own blogs
    $blogs = explode(',', $moderator_blogs);
    foreach ($blogs as $blog_ID) { // Loop through each blogs of this user
        if (! isset($moderators[$moderator_ID])) { // Init this user moderator perms if it was not initialized yet
            $moderators[$moderator_ID] = [];
        }
        $moderators[$moderator_ID][$blog_ID] = [
            'perm_edit' => $max_perm_edit,
            'perm_statuses' => $max_perm_statuses,
        ];
    }
}
...
if ($def_send_moderation_reminder) { // Send comment moderation reminder is set by default
    $send_moderation_reminder_cond = '( ( uset_value IS NOT NULL AND uset_value <> \'0\' ) OR ( uset_value IS NULL ) )';
} else { // Send comment moderation reminder is NOT set by default
...
if (empty($loaded_ids)) { // UserCache result is empty which means nobody wants to receive notifications
...
$SQL->WHERE_and('post_status IN ( ' . $DB->quote(['published', 'community', 'protected']) . ' )');
...
foreach ($blog_comments as $row) {
    ...
    if (strpos($blog_moderation_statuses, $row->comment_status) === false) { // This status shouldn't be notified on this blog
        continue;
    }
    if (isset($comments_map[$row->blog_ID])) { // This blog comments were already initialized
        if (isset($comments_map[$row->blog_ID][$row->comment_status])) { // Comments with this status were already initialized
        ...
        } else { // Initialize blog comments with this status, and set the first element
            $comments_map[$row->blog_ID][$row->comment_status] = [
                $row->author_level => $row->cmt_count,
            ];
        }
    } else { // Initialize blog array, and set the first element
        $comments_map[$row->blog_ID] = [
            $row->comment_status => [
                $row->author_level => $row->cmt_count,
            ],
        ];
...
$moderator_comments = [];
foreach ($moderation_blogs as $blog_ID) { // Collect the number of comments from all blogs
    if (! isset($comments_map[$blog_ID])) { // this blog doesn't contains comments awaiting moderation which statuses corresponds to this blog moderation statutes
        continue;
    }
    $cmt_count = 0;
    foreach ($comments_map[$blog_ID] as $status => $content) { // collect the number of commetns with all statuses
        foreach ($content as $level => $count) { // collect the number of commetns with all kind of users
            $cmt_count += $count;
        }
    }
...
foreach ($loaded_ids as $moderator_ID) { // Loop trhough each moderators and send comment moderation emails if it is required
    $moderator_User = $UserCache->get_by_ID($moderator_ID);
    $blog_comments = [];
    if (in_array($moderator_ID, $global_moderators)) { // This is a global moderator user
        $blog_comments = $moderator_comments;
    } else { // This moderator user may have different permission on different blogs, collect the comments corresponding to the perms
        foreach ($moderators[$moderator_ID] as $blog_ID => $perms) {
            ...
            foreach ($comments_map[$blog_ID] as $status => $content) {
                ...
                if ($perms['perm_statuses'] & $status_perm_value) { // User has permission to edit comments with this status
                    ...
                    foreach ($ordered_statuses as $ordered_status => $order_index) {
                        ...
                        } elseif ($current_status_found && ($order_index !== 0)) { // This is a higher status then the currently checked status
                            $ordered_status_perm_value = get_status_permvalue($ordered_status);
                            if ($perms['perm_statuses'] & $ordered_status_perm_value) { // User has permission to a higher status, so the comment status can be raised
                                $raise_status_allowed = true;
                                break;
                            }
                        }
                    }
                    if (! $raise_status_allowed) { // User is not allowed to raise these comment statuses
                        continue;
                    }
                    ...
                    foreach ($content as $level => $count) {
                        ...
                        if ($allowed) { // User has permission to edit comments with this author level
                            $cmt_count += $count;
                        }
                    }
                }
            }
            if ($cmt_count > 0) { // There are comments awaiting moderation on this blog and user has permission to moderate
                $blog_comments[$blog_ID] = $cmt_count;
            }
        }
    }

    if (empty($blog_comments)) { // There are no comments awaiting moderation that this user could moderate
        continue;
    }
    ...
    if (send_mail_to_User($moderator_ID, T_('Comment moderation reminder'), 'comments_unmoderated_reminder', $params, false)) {	// Log success mail sending:
