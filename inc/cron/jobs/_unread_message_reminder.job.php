...
if (empty($Settings)) { // initialize GeneralSettings
...
if ($Settings->get('def_notify_unread_messages')) { // by default everybody should be notified, so notify those users who didn't explicitly set to not receive notifications
...
if (empty($users_to_remind)) { // There is no user to remind
...
foreach ($users_to_remind as $user_ID => $last_reminder_ts) {
    $index++;
    if (empty($last_reminder_ts)) { // Reminder was not sent yet, we don't need to check anything else, because in this case reminder must be sent
        continue;
    }
    ...
    foreach ($unread_message_reminder_delay as $lastseen => $delay) {
        if ($days_since_lastseen < $lastseen) { // We have found the correct delay value, we must check if we should send reminder
            $dont_send_reminder = false;
            break;
        }
    }
    if ($dont_send_reminder || ($days_since_reminder < $delay)) { // Don't send a new reminder, because the user was not logged in since a very long time or we have already sent a reminder in the last x days ( x = delay value )
        unset($users_to_remind_ids[$index]);
    }
}

if (empty($users_to_remind_ids)) { // There is no user to remind after we have filtered out those ussers who haven't logged in since a long time
    cron_log_append(T_('It was not necessary to send any reminder!'));
    return 1;
}
...
foreach ($users_to_remind_ids as $user_ID) {
    if (! check_cron_job_emails_limit()) {	// Stop execution for cron job because max number of emails has been already sent:
        break;
    }
    ...
    if (send_mail_to_User($user_ID, T_('You have unread messages') . '!', 'private_messages_unread_reminder', $email_template_params)) { // Update users last unread message reminder timestamp
