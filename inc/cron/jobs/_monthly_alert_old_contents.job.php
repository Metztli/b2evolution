...
if (empty($BlogCache->cache)) {	// There are no collections where we should find old posts to alert for moderators
...
$alert_post_statuses = ['published', 'community', 'protected'];
...
$specific_coll_moderators = $DB->get_results(
    '( ' . $bloguser_SQL->get() . ' ) UNION ( ' . $bloggroup_SQL->get() . ' ) ORDER BY user_ID, blog_ID',
    OBJECT,
...
$moderators = [];
foreach ($specific_coll_moderators as $row) {	// Loop through each different collection's moderator users and their permissions:
...
if ($UserSettings->get('send_pst_stale_alert')) {	// A send stale posts alerter is set by default
    $SQL->WHERE_and('( ( uset_value IS NOT NULL AND uset_value <> \'0\' ) OR ( uset_value IS NULL ) )');
} else {	// A send stale posts alerter is NOT set by default
    $SQL->WHERE_and('( uset_value IS NOT NULL AND uset_value <> \'0\' )');
}
if (count($blocked_emails)) {	// Restrict users by blocked email addresses:
...
if (empty($UserCache->cache)) {	// UserCache result is empty which means nobody wants to receive alert on old contents:
...
foreach ($BlogCache->cache as $alert_Blog) {
    ...
    if (empty($old_posts)) {	// No old posts on the collection:
        continue;
    }
    ...
    foreach ($UserCache->cache as $alert_User) {
        ...
        if (in_array($alert_User->ID, $global_moderator_IDs) ||
            (isset($coll_owners[$alert_User->ID]) && in_array($alert_Blog->ID, $coll_owners[$alert_User->ID]))) {	// Allow to send alert on all old post if the user is a global moderator or he is an owner of the collection:
            foreach ($old_posts as $old_post) {
                $moderator_old_posts[] = $old_post->post_ID;
            }
        } elseif (isset($moderators[$alert_User->ID][$alert_Blog->ID])) {	// Check what post the moderator can edit:
            $perms = $moderators[$alert_User->ID][$alert_Blog->ID];
            foreach ($old_posts as $old_post) {
                $status_perm_value = get_status_permvalue($old_post->post_status);
                if ($perms['perm_statuses'] & $status_perm_value) {	// If user has permission to edit posts with this status
                    ...
                    if ($allowed) {	// User has permission to edit the post with this author level:
                        $moderator_old_posts[] = $old_post->post_ID;
                    }
                }
            }
        ...
        if (empty($moderator_old_posts)) {	// The user has no old posts to be alerted:
            continue;
        }
        ...
        if (send_mail_to_User($alert_User->ID, sprintf(T_('Stale contents found in %s'), $alert_Blog->get('shortname')), 'posts_stale_alert', $email_params, false)) {	// Log success mail sending:
