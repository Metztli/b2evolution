...
if ($number_of_max_reminders < 3) { // The config array is wrong, it must have at least 4 elements (Reminder #1, Mark as failed, Delete warning, Delete account)
    cron_log_append('The job advanced configuration is wrong, can\'t send reminders!', 'error');
    return 3; /* error */
}

$reminder_delay_conditions = [];
...
$all_reminder_users = [];
$send_activation_users = []; // Users for cron settings "Reminder #X"
$mark_failed_users = []; // Users for cron setting "Mark as Failed / Pending delete"
$send_delete_warning_users = []; // Users for cron setting "Delete warning"
$delete_account_users = []; // Users for cron setting "Delete account"
foreach ($reminder_users as $reminder_user) {
    ...
    } elseif ($activation_reminder_count == $number_of_max_reminders - 2) {	// This user must be marked with status "Failed activation" or "Pending delete":
        $mark_failed_users[] = $reminder_user->user_ID;
    } elseif ($activation_reminder_count <= $number_of_max_reminders - 3) {	// This user must receive an activation email:
...
cron_log_append(
    sprintf(T_('%d of %d account activation reminder emails were sent!'), $success_sent_activation_users_num, $send_activation_users_num),
...
if ($mark_failed_users_num) {	// Change account status for all users who didn't receive activation reminder or account validation email in the last days of the setting "Mark as Failed / Pending delete",
    ...
    cron_log_action_end(
        "\n" . sprintf(
            '%d of %d users(with IDs: %s) were marked with status "Failed activation" or "Pending delete"!',
            $success_mark_failed_users_num,
            $mark_failed_users_num,
            implode(', ', $mark_failed_users)
        ) . "\n",
        ($mark_failed_users_num ? ($mark_failed_users_num == $success_mark_failed_users_num ? 'success' : 'warning') : null)
    );
} else {	// Don't display this as action because no users to update a status:
...
$succes_sent_delete_warning_users_num = check_cron_job_emails_limit() // Run email sending only when max emails number was not reached in the same function above
    ...
    : 0;
cron_log_append(
    sprintf('%d of %d delete warning reminder emails were sent!', $succes_sent_delete_warning_users_num, $send_delete_warning_users_num) . "\n",
...
if ($delete_users_num) {
    ...
    foreach ($delete_account_users as $delete_account_user_ID) {
        if (! ($deleted_User = &$UserCache->get_by_ID($delete_account_user_ID, false, false))) {	// Wrong user:
...
cron_log_append(
    sprintf('%d users were deleted!', $succes_deleted_users_num),
