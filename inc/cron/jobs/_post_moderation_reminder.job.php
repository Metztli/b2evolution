...
if (empty($moderation_blogs)) { // There are no blogs where exists draft posts older then the threshold ( 24 hours by default )
...
foreach ($specific_blog_moderators as $row) { // Loop through each different blog's moderator users and their permissions
    ...
    if (isset($moderators[$row->user_ID][$row->blog_ID])) {	// Update user permissions on this collection:
        // perm_edit    : 'no', 'own', 'lt', 'le', 'all' (real value from DB)
        // perm_edit_num:  1,    2,     3,    4,    5    (index of the value from DB)
        if ($moderators[$row->user_ID][$row->blog_ID]['perm_edit_num'] < $row->perm_edit_num) {	// The user and the group advanced post edit perm for this user are not the same, keep the higher perm value:
        ...
        if ($current_perm_statuses != $row_perm_status) { // The advanced user and the group post statuses perm for this user are not the same, the union of this perms must be accepted
            $moderators[$row->user_ID][$row->blog_ID]['perm_statuses'] = ($current_perm_statuses | $row_perm_status);
        }
    } else {	// Initialize a new setting for the moderator per collection:
...
if ($def_send_moderation_reminder) { // Send post moderation reminder is set by default
    $send_moderation_reminder_cond = '( ( uset_value IS NOT NULL AND uset_value <> \'0\' ) OR ( uset_value IS NULL ) )';
} else { // Send post moderation reminder is NOT set by default
...
foreach ($blog_posts as $row) {
    ...
    if (strpos($blog_moderation_statuses, $row->post_status) === false) { // This status shouldn't be notified on this blog
        continue;
    }
    if (isset($posts_map[$row->blog_ID])) { // This blog posts were already initialized
        if (isset($posts_map[$row->blog_ID][$row->post_status])) { // Comments with this status were already initialized
        ...
        } else { // Initialize blog posts with this status, and set the first element
            $posts_map[$row->blog_ID][$row->post_status] = [
                $row->author_level => $row->post_count,
            ];
        }
    } else { // Initialize blog array, and set the first element
        $posts_map[$row->blog_ID] = [
            $row->post_status => [
                $row->author_level => $row->post_count,
            ],
        ];
...
$moderator_posts = [];
foreach ($moderation_blogs as $blog_ID) { // Collect the number of posts from all blogs
    if (! isset($posts_map[$blog_ID])) { // this blog doesn't contains posts awaiting moderation which statuses corresponds to this blog moderation statutes
        continue;
    }
    $post_count = 0;
    foreach ($posts_map[$blog_ID] as $status => $content) { // collect the number of posts with all statuses
        foreach ($content as $level => $count) { // collect the number of posts with all kind of users
            $post_count += $count;
        }
    }
...
foreach ($loaded_ids as $moderator_ID) { // Loop trhough each moderators and send post moderation emails if it is required
    ...
    if (in_array($moderator_ID, $global_moderators)) { // This is a global moderator user
    ...
    } else { // This moderator user may have different permission on different blogs, collect the posts corresponding to the perms
        foreach ($moderators[$moderator_ID] as $blog_ID => $perms) {
            ...
            foreach ($posts_map[$blog_ID] as $status => $content) {
                ...
                if ($perms['perm_statuses'] & $status_perm_value) { // User has permission to edit posts with this status
                    ...
                    foreach ($ordered_statuses as $ordered_status => $order_index) {
                        if ($ordered_status == $status) {
                            ...
                            if ($perms['perm_statuses'] & $ordered_status_perm_value) { // User has permission to a higher status, so the post status can be raised
                                $raise_status_allowed = true;
                                break;
                            }
                        }
                    }
                    if (! $raise_status_allowed) { // User is not allowed to raise these post statuses
                        continue;
                    }
                    ...
                    foreach ($content as $level => $count) {
                        ...
                        if ($allowed) { // User has permission to edit posts with this author level
                            $post_count += $count;
                        }
                    }
                }
            }
            if ($post_count > 0) { // There are posts awaiting moderation on this blog and user has permission to moderate
                $blog_posts[$blog_ID] = $post_count;
            }
        }
    }

    if (empty($blog_posts)) { // There are no posts awaiting moderation that this user could moderate
        continue;
    }
    ...
    if (send_mail_to_User($moderator_ID, T_('Post moderation reminder'), 'posts_unmoderated_reminder', $params, false)) {	// Log success mail sending:
