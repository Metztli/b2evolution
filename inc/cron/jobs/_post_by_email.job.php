...
if (! $mbox = pbm_connect()) {	// We couldn't connect to the mail server
...
if (! $Settings->get('eblog_test_mode') && $del_cntr > 0) {	// We want to delete processed emails from server
    imap_expunge($mbox);
...
if ($post_cntr > 0) {
    ...
    foreach ($pbm_items as $Items) {	// Send report to post author
        $to_user_ID = 0;
        foreach ($Items as $Item) {
            if ($to_user_ID == 0 && ($item_creator_User = &$Item->get_creator_User())) {	// Get author ID:
        ...
        if (send_mail_to_User($to_user_ID, T_('Post by email report'), 'post_by_email_report', $email_template_params)) {	// Log success mail sending:
