...
if (empty($cronjob_filters)) { // No saved filter yet
    $cronjob_filters = [];
}
if ($ctst_status === null) { // Get a param values from Session
...
if ($cjob_type === null) { // Get a param values from Session
...
if (! $ctst_pending && ! $ctst_started && ! $ctst_warning && ! $ctst_timeout && ! $ctst_error && ! $ctst_imap_error && ! $ctst_finished) { // Set default status filters:
    $ctst_pending = 1;
    $ctst_started = 1;
    $ctst_warning = 1;
    $ctst_timeout = 1;
...
if (! empty($cron_job_name_query)) { // left join with the predefined cron job names, to be able to order correctly after the after the localized Name fields
    $SQL->FROM_add('LEFT JOIN ( ' . $cron_job_name_query . ' ) AS temp ON ctsk_key = temp.task_key');
}
if (! empty($cjob_type)) { // Filter by job type:
    $SQL->WHERE('ctsk_key = ' . $DB->quote($cjob_type));
}
if ($ctst_started) { // Started
    $clog_statuses[] = 'started';
}
if ($ctst_warning) { // Warning
    $clog_statuses[] = 'warning';
}
if ($ctst_timeout) { // Timed out
    $clog_statuses[] = 'timeout';
}
if ($ctst_error) { // Error
    $clog_statuses[] = 'error';
}
if ($ctst_imap_error) {	// IMAP Error
    $clog_statuses[] = 'imap_error';
}
if ($ctst_finished) { // Finished
    $clog_statuses[] = 'finished';
}
if ($ctst_pending || ! empty($clog_statuses)) { // Filter by statuses:
    $clog_statuses = empty($clog_statuses) ? [] : ['clog_status IN ( "' . implode('", "', $clog_statuses) . '" )'];
    if ($ctst_pending) { // Pending
        $clog_statuses[] = 'clog_status IS NULL';
    }
    $SQL->WHERE_and(implode(' OR ', $clog_statuses));
}
if (! empty($s)) { // Filter by name
...
if (check_user_perm('options', 'edit', false, null)) {	// Permission to edit settings:
...
$Results->cols[] = [
    ...
    'extra' => [
        'style' => 'background-color: %cron_status_color( "#status#" )%;',
...
{
    if ($seconds == -1) { // This crontab is pending or timed out, No duration time is available
        return T_('none');
    } else { // Inform about duration in seconds
...
function crontab_actions($ctsk_ID, $status)
{
    ...
    if (check_user_perm('options', 'edit', false, null)) {	// User can edit options:
        if ($status == 'pending') {	// Icon for edit action
            $col .= action_icon(T_('Edit this job'), 'edit', $admin_url . '?ctrl=crontab&amp;action=edit&amp;ctsk_ID=' . $ctsk_ID);
        } elseif ($status == 'error') {	// Icon for copy action
            $col .= action_icon(T_('Duplicate this job'), 'copy', $admin_url . '?ctrl=crontab&amp;action=copy&amp;ctsk_ID=' . $ctsk_ID);
        }

        if ($status != 'started') {	// Icon for delete action
            $col .= action_icon(
                T_('Delete this job!'),
                'delete',
