...
if ($creating && $action != 'copy') { // New cronjob
    ...
    $Form->text_input('cjob_name', $edited_Cronjob->name, 50, TB_('Job name'), '', [
        'maxlength' => 255,
    ]);
}

$Form->begin_line(TB_('Schedule date'), 'cjob_date', '', [
    'required' => true,
]);
...
$Form->time_input('cjob_time', date2mysql($edited_Cronjob->start_timestamp), '', [
    'required' => true,
]);
...
$Form->duration_input('cjob_repeat_after', $edited_Cronjob->repeat_after, '', 'days', 'minutes', [
    'minutes_step' => 1,
]);

if ($edited_Cronjob->key == 'poll-antispam-blacklist') { // Don't allow to edit this param for cron job "Poll the antispam blacklist"
    $Form->info(TB_('+/- variation of:'), TB_('Auto'));
} else {
    $Form->duration_input('cjob_repeat_variation', $edited_Cronjob->repeat_variation, TB_('+/- variation of:'), 'days', 'minutes', [
        'minutes_step' => 1,
    ]);
}
$Form->end_line();
...
if (! $creating) {	// We can edit only pending cron jobs, Show this field just for info
...
$Form->end_form([['submit', 'submit', $creating ? TB_('Create') : TB_('Save Changes!'), 'SaveButton']]);
