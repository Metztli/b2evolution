...
if (param('ctsk_ID', 'integer', '', true)) {// Load cronjob from cache:
...
switch ($action) {
    ...
    case 'copy':
        ...
        if (($action == 'edit' && $edited_Cronjob->get_status() != 'pending') ||
            ($action == 'copy' && $edited_Cronjob->get_status() != 'error')) {	// Don't edit cron jobs with not "pending" status
            header_redirect('?ctrl=crontab', 303); // Will EXIT
            // We have EXITed already at this point!!
        }

        if ($action == 'copy') {	// Reset time to now for copied cron job
            global $localtimenow;
    ...
    case 'create':
        ...
        if (! empty($edited_Cronjob)) { // It is a copy action, we should save the fields "key" & "params"
        ...
        if ($edited_Cronjob->load_from_Request()) {	// We could load data from form without errors:
            if (! empty($ctsk_key)) {	// Save controller field from copied object
                $edited_Cronjob->set('key', $ctsk_key);
            }

            if (! empty($ctsk_params)) {	// Save params field from copied object
    ...
    case 'update':
        ...
        if ($edited_Cronjob->load_from_Request()) {	// We could load data from form without errors:
            if ($edited_Cronjob->dbupdate()) {	// The job was updated successfully
                $Messages->add(TB_('The scheduled job has been updated.'), 'success');
            } else {	// Errors on updating, probably this job has not "pending" status
    ...
    case 'settings':
        ...
        foreach ($cron_jobs as $cron_job_key => $cron_job_name) {
            ...
            if ($cjob_maxemail !== null) {	// Setting only for cron jobs that use email sending:
            ...
            if ($cjob_imap_error !== null) {	// Setting only for cron jobs that use IMAP email sending:
            ...
            switch ($cron_job_key) {
                case 'send-email-campaign':
                    ...
                    if (check_user_perm('emails', 'edit')) {	// Allow to edit email cron setting "Chunk Size" only if user has a permission:
                ...
                case 'send-non-activated-account-reminders':
                    ...
                    for ($c = 0; $c <= $reminder_config_num; $c++) {
                        ...
                        if ($reminder_config_value > 0 || $c >= $reminder_config_num - 2) {	// Store only a selected reminder and 3 last options("Mark as Failed / Pending delete", "Delete warning", "Delete account"):
                            $reminder_config[$c] = $reminder_config_value;
                        }
                    }
                    if (count($reminder_config) < 4) {	// If no reminder has been selected:
                    ...
                    if (empty($reminder_config[$reminder_config_num - 2])) {	// If "Mark as Failed / Pending delete" is not selected:
                ...
                case 'send-unread-messages-reminders':
                    ...
                    for ($d = 1; $d <= 10; $d++) {
                        ...
                        if ($reminder_delay_day > 0 || $reminder_delay_spacing > 0) {	// Store only a filled reminder:
                            if (empty($reminder_delay_day)) {	// If one field is not filled:
                            ...
                            } elseif ($prev_reminder_delay_day >= $reminder_delay_day) {	// If current value is less than previous:
                            ...
                            if (empty($reminder_delay_spacing)) {	// If one field is not filled:
                            ...
                            } elseif ($prev_reminder_delay_spacing >= $reminder_delay_spacing) {	// If current value is less than previous:
                    ...
                    if (empty($reminder_delay)) {	// If no reminder has been selected:
        ...
        if (param_errors_detected()) {	// Don't store settings if errors:
            break;
        }
    ...
    case 'list':
        if ($tab == 'list') {	// Detect timed out tasks:
...
if (in_array($action, ['new', 'create', 'edit', 'update', 'copy', 'list'])) { // Initialize date picker for cronjob.form.php
    init_datepicker_js();
}
