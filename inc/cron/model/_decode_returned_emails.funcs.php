...
function dre_msg($message, $cron = false, $type = null)
{
    ...
    if (! is_array($dre_messages)) {	// Initialize global array first time:
        $dre_messages = [];
    }
    ...
    if ($cron) {	// Append a message to cron log if we are in cron mode:
        cron_log_append($message, $type);
    }

    if ($type == 'error') {	// Store an error message in system log:
...
{
    if (! extension_loaded('imap')) {	// Exit here if imap extension is not loaded:
    ...
    if ($Settings->get('repath_novalidatecert')) {	// Don't validate certificate:
        $mailserver .= '/novalidate-cert';
    }

    $mailserver .= '}';

    if (! $print_out_folders) {	// Select messages only from this IMAP folder:
    ...
    if (is_null(@get_resource_type($mbox))) {	// Not a resource
        $error = imap_errors();
        if (is_array($error)) {
            $error = implode("<br />\n", $error);
        }
    ...
    if ($print_out_folders) {	// Print out all possible folders of host:
...
function dre_process_messages(&$mbox, $limit, $cron = false)
{
    ...
    if (empty($is_cron_mode)) {	// This may take a very long time if there are many messages; No execution time limit:
        // (don't apply this for cron job because it uses a setting "Max execution time")
        set_max_execution_time(0);
    }

    if ($Settings->get('repath_ignore_read')) {	// Read status info of all messages in order to know which have already been read:
        $msg_statuses = imap_fetch_overview($mbox, '1:' . $limit);
    }

    $email_cntr = 0;
    $del_cntr = 0;
    for ($index = 1; $index <= $limit; $index++) {	// Repeat for as many messages as allowed...
        dre_msg('<hr /><h3>' . sprintf(('Processing message %s:'), '#' . $index) . '</h3>', $cron);

        if ($Settings->get('repath_ignore_read')) {	// Check if we can read this message or we should skip this:
            if (isset($msg_statuses[$index - 1]) && $msg_statuses[$index - 1]->seen == 1) {	// Skip this message because it has already been read:
                dre_msg(('Ignoring this message because it has aleady been read.'), $cron);
                if ($cron) {	// Mark the end of action for cron log:
                    cron_log_action_end('');
                }
                continue;
            } else {	// Mark this message as "Seen" in order to don't read it twice:
        ...
        if (! $mimeParser->Decode($MIMEparameters, /* BY REF */ $decodedMIME)) {	// error:
            ...
            continue;
        } else {	// the specified message data was parsed successfully:
            dre_msg(('MIME message decoding successful'), $cron);

            // STEP 2: Analyze (the first) parsed message to describe its contents:
            if (! $mimeParser->Analyze($decodedMIME[0], /* BY REF */ $parsedMIME)) {	// error:
            ...
            if (! dre_process_header($parsedMIME, /* BY REF */ $subject, /* BY REF */ $post_date, $cron)) {	// Couldn't process message headers:
                rmdir_r($tmpDirMIME);
                unlink($tmpMIME);
                if ($cron) {	// Mark the end of action for cron log:
                    cron_log_action_end('');
            ...
            if ($parsedMIME['Type'] == 'html') {	// Mail is HTML:
                if ($debug) {	// Display this info only in debug mode:
                    dre_msg(sprintf(('HTML message part saved as %s'), $parsedMIME['DataFile']), $cron);
                }
                $html_body = file_get_contents($parsedMIME['DataFile']);

                if (empty($html_body)) {	// Try to get a body text from alternative parts if main html body is empty:
                    foreach ($parsedMIME['Alternative'] as $alternative) {	// First try to get HTML alternative (when possible)
                        if ($alternative['Type'] == 'html') {	// HTML text
                            if ($debug) {	// Display this info only in debug mode:
                        ...
                        } elseif ($alternative['Type'] == 'text') {	// Plain text
                            if ($debug) {	// Display this info only in debug mode:
            ...
            } elseif ($parsedMIME['Type'] == 'text') {	// Mail is plain text:
                if ($debug) {	// Display this info only in debug mode:
                    dre_msg(sprintf(('Plain-text message part saved as %s'), $parsedMIME['DataFile']), $cron);
                }
                $strbody = file_get_contents($parsedMIME['DataFile']);
            } elseif ($parsedMIME['Type'] == 'delivery-status') {	// Mail is delivery-status:
                $strbody = $parsedMIME['Response'];
            }

            if (count($mimeParser->warnings) > 0) { // Record potential warnings:
        ...
        if (empty($html_body)) {	// Plain-text message
        ...
        if (empty($email_data['address'])) {	// Don't process a returned message without email address:
...
function dre_process_header($header, &$subject, &$post_date, $cron = false)
{
    ...
    if (function_exists('mb_decode_mimeheader')) {	// Decode email subject:
    ...
    if (! $subject_is_correct) { // Subject is not match to identify return email
...
{
    if (preg_match_all('/([a-z0-9&\-_.]+?@[\w\-]+\.([\w\-\.]+\.)?[\w]+)/is', $content, $emails)) {	// Get the returned emails
        global $Settings;

        $emails = array_unique($emails[1]);
        foreach ($emails as $e => $email) {
            if (in_array($email, [$Settings->get('notification_sender_email'), $Settings->get('notification_return_path')])) {	// Exclude Sender email & Return path
                unset($emails[$e]);
            }
        }
...
{
    $marker = 0;
    if (preg_match('~<body[^>]*>(.*?)</body>~is', $message, $result)) {	// First see if we can get contents of <body> tag
        $content = $result[1];
        $marker = 1;
    } elseif (preg_match('~<html[^>]*>(.*?)</html>~is', $message, $result)) {	// <body> was not found, use <html> contents and delete <head> section from it
        $content = preg_replace('~<head[^>]*>(.*?)</head>~is', '', $result[1]);
        $marker = 1;
    }

    if (empty($marker)) {	// None of the above methods worked, just use the original message body
        $content = $message;
    }
...
function dre_get_processing_content($content, $emails)
{
    ...
    if (count($emails) > 0) {	// If emails exist
        // Get last email
        $email = $emails[count($emails) - 1];
        if (! empty($email)) {	// Get error text after last email address
            $error_text = trim(utf8_substr($content, utf8_strpos($content, $email) + utf8_strlen($email)));
        }
        if (empty($error_text)) {	// If error text is empty we should get all content before email OR full content if no email address in content
            $error_text = empty($email) ? $content : trim(utf8_substr($content, 0, utf8_strpos($content, $email)));
        }
    } else {	// If no emails - get full content as error text
        $error_text = $content;
    }

    if (! empty($error_text)) {	// Replace all new line sumbols with space symbol
...
function dre_get_error_message($message)
{
    ...
    if (preg_match('#[\s;]{1}(5[0-9][0-9][\s\d\.\-]+([^\n]+))#i', $message, $errors)) {	// Get first found error
        $error_text = trim($errors[1]);
    } else {	// If no errors - use full content(between email and body terminator)
        $error_text = $message;
    }
...
function dre_get_error_by_pattern($content)
{
    ...
    if (empty($error_types)) {	// Error types are not defined
        return false;
    }
...
function dre_get_error_info($content)
{
    ...
    if (! empty($error_pattern)) {
        if (preg_match('#' . $error_pattern[1] . '#i', $error_full_text)) {	// If error pattern is contained in the full error text
            $error_info['text'] = $error_full_text;
            $error_info['type'] = $error_pattern[0];
        }
    }

    if (empty($error_info)) {	// Set error info from full error text If error info is not defined by some reason yet
        $error_info['text'] = $error_full_text;
...
function dre_limit_by_terminators($content)
{
    ...
    if (! empty($repath_terminators)) {
        ...
        foreach ($repath_terminators as $repath_terminator) {	// Limit by each terminator
            $repath_terminator = trim($repath_terminator);
            if (empty($repath_terminator)) {	// Skip empty string
                continue;
            }
            if (! empty($repath_terminator) && ($os_terminator = utf8_strpos($content, $repath_terminator)) !== false) {	// Remove text after terminator string
...
function dre_insert_returned_email($email_data)
{
    ...
    $DB->query(
        'INSERT INTO T_email__returns ( emret_address, emret_errormsg, emret_timestamp, emret_message, emret_headers, emret_errtype )
...
function dre_save_email_address_data($email_returned)
{
    ...
    if (empty($email_returned['address'])) {	// No emails, Exit here
        return;
    }
    ...
    if (! $EmailAddress) {	// Insert new email address
        $EmailAddress = new EmailAddress();
        $EmailAddress->set('address', $email_returned['address']);
    }

    switch ($email_returned['errtype']) {	// Error type of the returned email:
        ...
        case 'T':	// Temporary error
            if (in_array($EmailAddress->get('status'), ['suspicious1', 'suspicious2', 'suspicious3'])) { // If current status already is defined as 'suspicious1', 'suspicious2' or 'suspicious3'
                if ($EmailAddress->get('sent_last_returnerror') <= 1) {
                    if ($EmailAddress->get('status') == 'suspicious1') {	// Increase status from suspicious1 to suspicious2
                        $EmailAddress->set('status', 'suspicious2');
                    } elseif ($EmailAddress->get('status') == 'suspicious2') {	// Increase status from suspicious2 to suspicious3
                        $EmailAddress->set('status', 'suspicious3');
                    }
                }
            } elseif ($EmailAddress->get('status') == 'redemption') { // IF current status is 'redemption' we should set it as 'suspicious3'
                $EmailAddress->set_status('suspicious3');
            } else { // Update only the email addresses with level status less then Suspicious 1
