...
function pbm_process_messages(&$mbox, $limit, $cron = false)
{
    ...
    for ($index = 1; $index <= $limit; $index++) {	// Repeat for as many messages as allowed...
        ...
        $pbm_item_files = []; // reset the value for each new Item
        ...
        if (! $mimeParser->Decode($MIMEparameters, $decodedMIME)) {	// error:
            ...
            if (! $mimeParser->Analyze($decodedMIME[0], $parsedMIME)) {	// error:
            ...
            if (! pbm_process_header($parsedMIME, $subject, $post_date, $cron)) {	// Couldn't process message headers:
                rmdir_r($tmpDirMIME);
                unlink($tmpMIME);
                continue;
            }
            ...
            if ($parsedMIME['Type'] == 'html') {	// Mail is HTML:
                if ($Settings->get('eblog_html_enabled')) {	// HTML posting enabled
                ...
                foreach ($parsedMIME['Alternative'] as $alternative) {	// First try to get HTML alternative (when possible)
                    if ($alternative['Type'] == 'html' && $Settings->get('eblog_html_enabled')) {	// HTML text:
                        if ($debug) {	// Display this info only in debug mode:
        ...
        if (empty($html_body)) {	// Plain-text message
            ...
            if (($auth = pbm_get_auth_tag($content)) === false) {	// No <auth> tag, let's detect legacy "username:password" on the first line
                $a_body = explode("\n", $content, 2);
            ...
            if (($parsed_message = pbm_prepare_html_message($html_body, $cron)) === false) {	// No 'auth' tag provided, skip to the next message
                rmdir_r($tmpDirMIME);
                continue;
            }
        ...
        if (empty($user_login) || empty($user_pass)) {
            pbm_msg(sprintf(
                ('Please add username and password in message body in format %s.'),
        ...
        if (! empty($is_cron_mode)) {	// Assign current User if we are in cron mode. This is needed in order to check user permissions
            global $current_User;
        ...
        $content = preg_replace(['~^(\s*<br[\s/]*>\s*){1,}~i', '~(\s*<br[\s/]*>\s*){1,}$~i'], '', $content);

        if ($hasAttachment || $hasRelated) {	// Handle attachments
            if (isset($GLOBALS['files_Module'])) {
                if ($mediadir = $pbmBlog->get_media_dir()) {
                    if ($hasAttachment) {
                        pbm_process_attachments(
                            $content,
                            $parsedMIME['Attachments'],
                            $mediadir,
                            $pbmBlog->get_media_url(),
                            $Settings->get('eblog_add_imgtag'),
                            'attach',
                            $cron
                        );
                    }
        ...
        if ($test_mode_on) {	// Test mode
            ...
            if (! empty($pbm_item_files)) {	// Attach files
...
function pbm_process_attachments(&$content, $mailAttachments, $mediadir, $media_url, $add_img_tags = true, $type = 'attach', $cron = false)
{
    ...
    foreach ($mailAttachments as $attachment) {
        ...
        } else {	// Related attachments may not have file name, we'll generate one below
            $filename = '';
        }
        ...
        while (file_exists($mediadir . $filename)) {
            ...
            if (strlen($filename) > $filename_max_length) { // This is a special case, when the filename is longer then the maximum allowed
                ...
                if ($error_in_filename = process_filename($filename, true)) { // The file name is not valid, this is an unexpected situation, because the file name was already validated before
                    pbm_msg(('Invalid filename') . ': ' . $error_filename, $cron, 'error');
                    syslog_insert(sprintf('The posted by mail file %s has an unrecognized extension', '[[' . $filename . ']]'), 'warning', 'file');
                    break;
                }
        ...
        if ($error_in_filename) { // Don't create file with invalid file name
            continue;
        }
        ...
        if ($type == 'attach') {
            ...
            $content .= "\n";
        } elseif (! empty($attachment['ContentID'])) {	// Replace relative "cid:xxxxx" URIs with absolute URLs to media files
...
function pbm_prepare_html_message($message, $cron = false)
{
    ...
    if (($auth = pbm_get_auth_tag($content)) === false) {	// No 'auth' tag provided, exit
        pbm_msg(sprintf(
            ('&lt;auth&gt; tag not found! Please add username and password in message body in format %s.'),
            '"&lt;auth&gt;username:password&lt;/auth&gt;"'
        ), $cron);
        return false;
    }
...
function pbm_validate_user_password($user_login, $user_pass)
{
    ...
    if (! $User->check_password($user_pass, false)) {
        if (preg_match('~^[a-f0-9]{32}$~i', $user_pass)) {	// This is a hashed password, see if it's valid
            // We check it here because some crazy user may use a real 32-chars password!
            if ($User->check_password($user_pass, true)) {	// Valid password
                return $User;
            }
        }
        return false;
