...
<?php
    if (! empty($revisions_difference_title)) {	// Display title difference:
        echo $revisions_difference_title;
    } else {	// No title difference
        ?>
	<tr>
...
<?php
if (! empty($revisions_difference_content)) {	// Display content difference:
    echo $revisions_difference_content;
} else {	// No content difference
    echo '<tr><td colspan="4" class="center">' . T_('No difference.') . '</td></tr>';
}

if (is_array($revisions_difference_custom_fields)) {	// Display custom fields difference only if Item revision had at least one custom field:
    ?>
		<tr><td colspan="4">&nbsp;</td></tr>
		<tr>
...
<?php
if (empty($revisions_difference_custom_fields)) {	// No difference in custom fields:
    echo '<tr><td colspan="4" class="center">' . T_('No difference.') . '</td></tr>';
} else {	// Display custom fields difference:
    foreach ($revisions_difference_custom_fields as $revisions_diff_data) {
        if (isset($revisions_diff_data['diff_label'])) {	// Display a difference between two field labels:
            echo $revisions_diff_data['diff_label'];
        } else {	// No difference between two field labels:
            echo '<tr>'
        ...
        if (isset($revisions_diff_data['diff_value'])) {	// Display a difference between two field values:
            echo $revisions_diff_data['diff_value'];
        } else {	// No difference because one revision has no the custom field:
            echo '<tr>';
            for ($r = 1; $r <= 2; $r++) {
                echo '<td></td>';
                if (isset($revisions_diff_data['r' . $r . '_value'])) {	// Display a field value if the revision has it:
                    echo '<td class="diff-context">' . nl2br(htmlspecialchars($revisions_diff_data['r' . $r . '_value'])) . '</td>';
                } elseif ($r == 1) {	// If field exists only in new revision:
                    echo '<td class="red"><b>' . sprintf(T_('The field "%s" did not exist'), $revisions_diff_data['r' . $r . '_label']) . '</b></td>';
                } else {	// If field exists only in old revision:
...
if (is_array($revisions_difference_links)) {	// Display links/attachments difference only if Item revision had at least one attached File:
    ?>
		<tr><td colspan="4">&nbsp;</td></tr>
		<tr>
	...
	<?php
        if (empty($revisions_difference_links)) {	// No difference in attached files:
            echo '<tr><td colspan="4" class="center">' . T_('No difference.') . '</td></tr>';
        } else {	// Display links/attachments difference:
            ?>
		<tr>
			<td colspan="4">
				<table class="table table-striped table-bordered table-condensed">
					<thead>
						<tr>
						<?php
                                for ($r = 1; $r <= 2; $r++) {	// Print out table headers for two revisions:
                                    ...
                                    if ($r == 1) {	// Use ID column as separator between the compared revisions:
            ...
            ?>
                    ...
                    foreach ($revisions_difference_links as $link_ID => $links_data) {
                        echo '<tr>';
                        for ($r = 1; $r <= 2; $r++) {	// Print out differences of the compared revisions:
                            if (isset($links_data['r' . $r])) {	// If the revision has the Link/Attachment:
                                $link = $links_data['r' . $r];
                                foreach ($links_data['r' . $r] as $link_key => $link_value) {	// Print out each link propoerty:
                                    if ($link_key == 'file_ID') {	// Skip column with file ID:
                                        continue;
                                    }
                                    $r2 = $r == 1 ? 2 : 1;
                                    $class = '';
                                    if (isset($links_data['r' . $r2][$link_key]) && $link_value != $links_data['r' . $r2][$link_key]) {	// Mark the different property with red background color:
                                        $class .= 'bg-danger';
                                    }
                                    if ($link_key == 'order') {	// Order value must be aligned to the right:
                                        $class .= ' text-right';
                                    } elseif ($link_key == 'path' && $link_value === false) {	// If file was deleted:
                            ...
                            } else {	// If the revision has no the Link/Attachment:
