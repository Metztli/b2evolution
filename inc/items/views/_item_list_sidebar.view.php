...
if (get_param('tab_type') == 'post') {
    ...
    $Form->begin_fieldset(T_('Date'), [
        'id' => 'items_filter_date',
        'fold' => true,
        'default_fold' => $fold_date,
    ]);
...
if ($Blog->get_setting('use_workflow')) {	// Display only if workflow is enabled:
    ...
    $Form->begin_fieldset(T_('Assignee'), [
        'id' => 'items_filter_assignee',
        'fold' => true,
        'default_fold' => $fold_assignee,
    ]);
...
if (count($ItemStatusCache->cache)) {	// Display only if at least one status exists in DB:
    $Form->begin_fieldset(T_('Status'), [
        'id' => 'items_filter_status',
        'fold' => true,
        'default_fold' => empty($status) && empty($statuses),
    ]);
    echo '<ul>';
...
$Form->begin_fieldset(T_('Items to show'), [
    'id' => 'items_filter_show_item',
    'fold' => true,
    'default_fold' => $fold_items_to_show,
]);
?>
<div style="margin-bottom:5px">
	<input type="checkbox" name="<?php echo $pp ?>flagged" value="1" id="flagged" class="checkbox" <?php if ($flagged) {
	    echo 'checked="checked" ';
	}?> />
...
<div style="margin-bottom:5px">
	<input type="checkbox" name="<?php echo $pp ?>mustread" value="1" id="mustread" class="checkbox" <?php if ($mustread) {
	    echo 'checked="checked" ';
	}?> />
...
<input type="checkbox" name="<?php echo $pp ?>show_past" value="1" id="ts_min" class="checkbox" <?php if ($show_past) {
    echo 'checked="checked" ';
}?> />
<label for="ts_min"><?php echo T_('Past') ?></label><br />

<input type="checkbox" name="<?php echo $pp ?>show_future" value="1" id="ts_max" class="checkbox" <?php if ($show_future) {
    echo 'checked="checked" ';
}?> />
...
$exclude_statuses = array_merge(get_restricted_statuses($Blog->ID, 'blog_post!'), ['trash']);
...
$Form->begin_fieldset(T_('Item Type'), [
    'id' => 'items_filter_item_types',
    'fold' => true,
    'default_fold' => $fold_item_type,
]);
echo '<ul>';
...
$Form->begin_fieldset(T_('Involves'), [
    'id' => 'items_filter_involves',
    'fold' => true,
    'default_fold' => $fold_involves,
]);
if ($user_count) {
    if ($user_count > 20) {	// Display an input box to enter user login:
...
$fold_cat_modifier = ($ItemList->default_filters['cat_modifier'] == $ItemList->filters['cat_modifier'])
        || ! in_array($ItemList->filters['cat_modifier'], ['-', '*']);
$fold_categories = $fold_cat_array && $fold_cat_single && $fold_cat_modifier;
$Form->begin_fieldset(T_('Categories'), [
    'id' => 'items_filter_categories',
    'fold' => true,
    'default_fold' => $fold_categories,
]);
...
$Form->begin_fieldset(T_('Archive'), [
    'id' => 'items_filter_archives',
    'fold' => true,
    'default_fold' => $fold_archives && $fold_week,
]);
...
// RENDERER PLUGINS:
global $Plugins;
$renderer_plugins = $Plugins->get_renderer_options(null, [
    'setting_name' => 'coll_apply_rendering',
    'Blog' => $Blog,
]);
if (count($renderer_plugins)) {	// Display only if at least one renderer plugin is allowed to be selected/unselected per Item of the current Collection:
    $Form->begin_fieldset(T_('Uses Renderer'), [
        'id' => 'items_filter_renderer',
        'fold' => true,
        'default_fold' => empty($renderers),
    ]);
    echo '<ul>';
