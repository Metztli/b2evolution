...
if (isset($mode)) { // used by bookmarklet
...
if (! empty($original_item_ID)) {
    ...
    if ($action == 'new_version') {	// Set a flag to know this is a new version of this Item:
...
if ($edited_Item->ID > 0) {	// Set form title for editing the item:
    ...
    if (! empty($original_item_ID)) {	// Set form title for duplicating the item:
        $form_title_item_ID = sprintf(
            ($action == 'new_version' ? TB_('Add version for Item %s') : TB_('Duplicating Item %s')),
            '<a href="' . $admin_url . '?ctrl=items&amp;blog=' . $Blog->ID . '&amp;p=' . $original_item_ID . '" class="post_type_link">#' . $original_item_ID . '</a>'
        );
    } else {	// Set form title for creating new item:
        $form_title_item_ID = TB_('New Item');
    }
}
if (check_user_perm('options', 'edit')) {	// Add an icon to edit item type if current user has a permission:
...
if ($ItemType->is_enabled($edited_Item->get_blog_ID())) {	// If Item Type is enabled for the Item's Collection:
    $item_type_before = '';
    $item_type_after = '';
} else {	// Mark with orange label if Item Type is disabled for the Item's Collection:
...
$Form->begin_fieldset(
    $form_title_item_ID . get_manual_link('post-contents-panel')
...
if ($edited_Item->get_type_setting('use_short_title') == 'optional') {	// Display short title:
    $Form->begin_fieldset('', [
        'class' => 'evo_fields_table__single_row',
    ]);
    if ($edited_Item->get_type_setting('use_short_title') == 'optional') {	// Display a post short title field:
        $short_title_maxlen = intval($edited_Item->get_type_setting('short_title_maxlen'));
        $Form->text_input('post_short_title', htmlspecialchars_decode($edited_Item->get('short_title')), 50, TB_('Short title'), '', [
            'maxlength' => $short_title_maxlen,
            'data-recommended-length' => '20;30',
        ]);
    } else {	// Hide a post short title field:
...
$edit_slug_link = '';
if ($edited_Item->ID > 0 && check_user_perm('slugs', 'view')) {	// Current User has a permission to view slugs:
    // Get icon to copy canonical slug to clipboard:
    $edit_slug_link = action_icon(TB_('Copy slug to clipboard'), 'clipboard-copy', '#', TB_('Copy slug'), 3, 4, [
...
if (empty($edited_Item->tiny_slug_ID)) {	// No tiny URL:
    $tiny_slug_info = TB_('No Tiny URL yet.');
} else {	// Get a link to tiny URL:
    $tiny_slug_info = $edited_Item->get_tinyurl_link([
        'before' => TB_('Tiny URL') . ': ',
        'after' => '',
    ]) . ' ';
    // Get icon to copy tiny URL to clipboard:
    $tiny_slug_info .= action_icon(TB_('Copy Tiny URL to clipboard'), 'clipboard-copy', '#', '', null, null, [
...
$Form->text_input('post_urltitle', $edited_Item->get_slugs(), 40, TB_('URL slugs'), $edit_slug_link . $tiny_slug_info, [
    'maxlength' => 210,
]);
...
if ($edited_Item->get_type_setting('use_text') != 'never') { // Display text
    // --------------------------- TOOLBARS ------------------------------------
    ...
    $Form->fieldend = "</div>\n";
    $Form->textarea_input('content', $item_content, 16, '', [
        'cols' => 40,
...
if (empty($plugin_button) && $edited_Item->get_type_setting('use_text') != 'never') {	// If button is not displayed by any plugin and text is allowed for current item type:
...
if ($edited_Item->get_type_setting('usage') == 'widget-page' &&
    check_user_perm('blog_properties', 'edit', false, $Blog->ID)) {	// Display a button to edit widgets only if item type is used for page containers and current user has permission to edit widgets:
...
// Attachments:
if ($edited_Item->get_type_setting('allow_attachments')) {	// If attachments are allowed for the Item Type:
    $LinkOwner = new LinkItem($edited_Item, param('temp_link_owner_ID', 'integer', 0));
    if ($LinkOwner->check_perm('edit', false)) {	// If current User has a permission to edit attachments of the edited Item:
...
// Internal comments:
if ($edited_Item->can_see_meta_comments()) {	// Meta/Internal comments are allowed only when current User has a permission to view them:
...
if (count($custom_fields)) {	// Display fieldset with custom fields only if at least one exists:
    $custom_fields_title = get_manual_link('post-custom-fields-panel');
    if ($current_User->check_perm('options', 'edit')) {	// Display an icon to edit post type if current user has a permission:
    ...
    $Form->open_tab_pane([
        'id' => 'custom_fields',
        'class' => 'tab_pane_pads',
        'right_items' => $custom_fields_title,
    ]);
...
$Form->open_tab_pane([
    'id' => 'advance_properties',
    'class' => 'tab_pane_pads',
    'right_items' => get_manual_link('post-advanced-properties-panel'),
]);

$Form->switch_layout('fields_table');

if ($edited_Item->get_type_setting('use_tags') != 'never') {	// Display tags:
    $link_to_tags_manager = '';
    if (check_user_perm('options', 'view')) { // Display a link to manage tags only when current use has the rights
    ...
    $suggest_checkbox = '<label>'
        ...
        . '</label>';
    $Form->text_input('item_tags', $item_tags, 40, TB_('Tags'), $suggest_checkbox, [
        'maxlength' => 255,
...
if ($is_not_content_block) {	// Display excerpt for item with type usage except of content block:
    $edited_item_excerpt = $edited_Item->get('excerpt');
    if ($edited_Item->get_type_setting('use_excerpt') != 'never') {	// Display excerpt:
        $excerpt_checkbox = '<label>'
            ...
            . '</label>';
        $Form->textarea_input('post_excerpt', $edited_item_excerpt, 3, TB_('Excerpt'), [
...
if ($edited_Item->get_type_setting('use_url') != 'never') {	// Display url:
    ...
    $Form->text_input('post_url', $edited_Item->get('url'), 20, TB_('Link to url'), $external_canonical_url_checkbox, [
        'maxlength' => 255,
        'data-maxlength' => 255,
...
if ($is_not_content_block) {	// Display title tag, meta description and meta keywords for item with type usage except of content block:
    if ($edited_Item->get_type_setting('use_title_tag') != 'never') {	// Display <title> tag:
        $Form->text_input('titletag', $edited_Item->get('titletag'), 40, TB_('&lt;title&gt; tag'), '', [
            'maxlength' => 500,
            'data-recommended-length' => '60;65',
    ...
    if ($edited_Item->get_type_setting('use_meta_desc') != 'never') {	// Display <meta> description:
        $Form->text_input('metadesc', $edited_Item->get_setting('metadesc'), 40, TB_('&lt;meta&gt; desc'), '', [
            'maxlength' => 500,
            'data-recommended-length' => '80;120',
            'required' => ($edited_Item->get_type_setting('use_meta_desc') == 'required'),
        ]);
    } else {	// Hide <meta> description:
        $Form->hidden('metadesc', $edited_Item->get_setting('metadesc'));
    }

    if ($edited_Item->get_type_setting('use_meta_keywds') != 'never') {	// Display <meta> keywords:
        $Form->text_input('metakeywords', $edited_Item->get_setting('metakeywords'), 40, TB_('&lt;meta&gt; keywds'), '', [
            'maxlength' => 500,
            'data-recommended-length' => '200;250',
            'required' => ($edited_Item->get_type_setting('use_meta_keywds') == 'required'),
        ]);
    } else {	// Hide <meta> keywords:
        $Form->hidden('metakeywords', $edited_Item->get_setting('metakeywords'));
    }

    if ($edited_Item->get_type_setting('allow_switchable')) {	// Display "Switchable content" options:
        $Form->text_input('item_switchable_params', $edited_Item->get_setting('switchable_params'), 40, TB_('Switchable content'), '', [
            'maxlength' => 500,
...
if (isset($Blog) && $Blog->get('allowtrackbacks')) {
    $Form->open_tab_pane([
        'id' => 'allowtrackbacks',
        'class' => 'tab_pane_pads',
        'right_items' => get_manual_link('post-edit-additional-actions-panel'),
    ]);
...
if ($edited_Item->can_see_meta_comments()) {
    ...
    $Form->open_tab_pane([
        ...
        'class' => 'tab_pane_pads',
        'right_items' => get_manual_link('meta-comments-panel'),
    ]);

    if ($creating) {	// Display button to save new creating item:
        $Form->submit(['actionArray[create_edit]', /* TRANS: This is the value of an input submit button */ TB_('Save post to start adding Internal comments'), 'btn-primary']);
    } else {	// Display internal comments of the edited item:
        ...
        if ($edited_Item->can_meta_comment()) { // Display a link to add new internal comment if current user has a permission
...
if ($edited_Item->can_see_meta_comments() && // Current User must has at least a permission to view meta comments
    ...
    $Form->open_tab_pane([
        'id' => 'checklist',
        'class' => 'tab_pane_pads',
        'right_items' => get_manual_link('item-checklist-panel'),
    ]);
    if ($creating) {	// Display button to save new creating item:
        $Form->submit(['actionArray[create_edit]', /* TRANS: This is the value of an input submit button */ TB_('Save post to start adding Checklist lines'), 'btn-primary']);
    } else {
...
$Plugins->trigger_event('AdminDisplayItemFormFieldset', [
    'Form' => &$Form,
    'Item' => &$edited_Item,
    'edit_layout' => 'expert',
]);
...
modules_call_method('display_item_settings', [
    'Form' => &$Form,
    'Blog' => &$Blog,
...
if ($is_not_content_block && $edited_Item->can_edit_workflow()) {	// Display workflow properties if current user can edit at least one workflow property:
    $Form->begin_fieldset(TB_('Workflow properties') . get_manual_link('post-edit-workflow-panel'), [
        'id' => 'itemform_workflow_props',
        'fold' => true,
    ]);
...
if ($is_not_content_block) {	// Display category selector for item with type usage except of content block:
    // ################### CATEGORIES ###################
    cat_select($Form, true, true, [
        'fold' => true,
    ]);
} else {	// Use a hidden input field for category in order to don't reset this to default on each updating:
...
$Form->begin_fieldset(TB_('Properties') . get_manual_link('post-properties-panel'), [
    'id' => 'itemform_extra',
    'fold' => true,
]);
...
if ($edited_Item->get_type_setting('use_parent') != 'never') {	// Display parent ID:
    echo '<tr><td><strong>' . TB_('Parent ID') . ':</strong></td><td>';
    $Form->item_selector('post_parent_ID', $edited_Item->get('parent_ID'), '', [
...
if (check_user_perm('users', 'edit')) {	// If current User has full access to edit other users,
...
if ($edited_Item->get_type_setting('use_coordinates') != 'never') {	// Display Latitude & Longitude settings:
...
if (check_user_perm('blog_edit_ts', 'edit', false, $Blog->ID)) {	// If user has a permission to edit advanced properties of items:
    if ($edited_Item->get_type_setting('allow_featured')) { // Display featured
...
if ($Blog->get_setting('track_unread_content')) {	// Display setting to mark Item as "must read" when tracking of unread content is enabled for collection:
    $Form->checkbox_basic_input('item_mustread', $edited_Item->get_setting('mustread'), '<strong>' . TB_('Must read') . '</strong> ' . get_pro_label(), [
        'disabled' => ! is_pro(),
    ]);
}

if ($is_not_content_block && $edited_Item->get_type_setting('allow_breaks')) {	// Display "hide teaser" checkbox for item with type usage except of content block:
...
// Single/page view:
if (check_user_perm('blog_edit_ts', 'edit', false, $Blog->ID)) {	// If user has a permission to edit advanced properties of items:
    if (! in_array($edited_Item->get_type_setting('usage'), ['intro-front', 'intro-main', 'intro-cat', 'intro-tag', 'intro-sub', 'intro-all', 'content-block', 'special'])) {	// We don't need this setting for intro, content block and special items:
        echo '<div class="itemform_extra_radio">';
        $Form->radio('post_single_view', $edited_Item->get('single_view'), [
            ['normal', TB_('Normal')],
            ['404', '404'],
            ['redirected', TB_('Redirected')],
...
if (check_user_perm('blog_edit_ts', 'edit', false, $Blog->ID)) {	// If user has a permission to edit advanced properties of items:
    ...
    $Form->output = true;
    $Form->radio('item_dateset', $edited_Item->get('dateset'), [
        [0, TB_('Update to NOW')],
        [1, TB_('Set to') . ': ', '', $item_issue_date_time],
    ], TB_('Issue date'), [
        'lines' => true,
    ]);
    echo '</div>';
...
// ################### TEXT RENDERERS ###################
if ($edited_Item->get_type_setting('use_text') != 'never') {	// Display text renderers only when text content is allowed for the item type:
    $Form->begin_fieldset(
        TB_('Text Renderers') . get_manual_link('post-renderers-panel')
                . action_icon(TB_('Plugins'), 'edit', $admin_url . '?ctrl=coll_settings&amp;tab=plugins&plugin_group=rendering&amp;blog=' . $Blog->ID, TB_('Plugins'), 3, 4, [
                    'class' => 'action_icon pull-right',
                ]),
        [
            'id' => 'itemform_renderers',
            'fold' => true,
        ]
    );
...
$Form->begin_fieldset(TB_('Language / Versions') . get_manual_link('post-language-versions'), [
...
$Form->select_input_options('post_locale', $edited_Item->get_locale_options(), TB_('Language'), '', [
    'style' => 'width:auto',
]);

if ($multiple_available_locales) {	// Display this setting if we have more than 1 enabled locale:
    $Form->radio('post_locale_visibility', $edited_Item->get('locale_visibility'), [
        ['always', TB_('Show for any navigation locale')],
        ['follow-nav-locale', TB_('Show only if matching navigation locale')],
    ], '', true);
}
...
if ($item_add_version_link || $item_link_version_link || count($other_version_items) > 0) {	// Display other versions and link to add version:
    echo '<b>' . TB_('Other versions') . ':</b>';
    echo '<ul style="list-style:disc;margin-left:20px">';
    $other_version_locales = [
        $edited_Item->get('locale') => 1,
    ];
    foreach ($other_version_items as $other_version_Item) {	// Find duplicated locales:
        $other_version_locales[$other_version_Item->get('locale')] = isset($other_version_locales[$other_version_Item->get('locale')]) ? 2 : 1;
    }
    foreach ($other_version_items as $other_version_Item) {	// Display a link to another version of the Item:
        echo '<li>' . $other_version_Item->get_title([
            'link_type' => 'edit_view_url',
                ...
                . $edited_Item->get_unlink_version_link([
                    'unlink_item_ID' => $other_version_Item->ID,
                ])
            . '</li>';
    }
    if ($item_add_version_link) {	// Display link to add new version if it is allowed:
        echo '<li>' . $item_add_version_link . '</li>';
    }
    if ($item_link_version_link) {	// Display link to add new version if it is allowed:
...
if ($edited_Item->allow_comment_statuses()) {
    $Form->begin_fieldset(TB_('Comments') . get_manual_link('post-comments-panel'), [
        'id' => 'itemform_comments',
        'fold' => true,
    ]);

    ?>
			<label title="<?php echo TB_('Visitors can leave comments on this post.') ?>"><input type="radio" name="post_comment_status" value="open" class="checkbox" <?php if ($post_comment_status == 'open') {
			    echo 'checked="checked"';
			} ?> />
			<?php echo TB_('Open') ?></label><br />
		<?php
        if ($edited_Item->get_type_setting('allow_closing_comments')) { // Allow closing comments
            ?>
			<label title="<?php echo TB_('Visitors can NOT leave comments on this post.') ?>"><input type="radio" name="post_comment_status" value="closed" class="checkbox" <?php if ($post_comment_status == 'closed') {
			    echo 'checked="checked"';
			} ?> />
...
if (in_array($edited_Item->get_type_setting('usage'), ['post', 'page', 'widget-page'])) {	// Display user tagging for items which can be displayed only on disp=single, disp=page or disp=widget_page:
    // ################### USER TAGGING ###################
    $Form->begin_fieldset(
        TB_('User Tagging') . get_manual_link('post-user-tagging-panel')
                    . (check_user_perm('options', 'view') ? action_icon(TB_('User Tags'), 'edit', $admin_url . '?ctrl=usertags', TB_('User Tags'), 3, 4, [
                        'class' => 'action_icon pull-right',
                    ]) : ''),
        [
            'id' => 'itemform_usertags',
            'fold' => true,
        ]
    );
    ...
    $Form->usertag_input('user_tags', $edited_Item->get_setting('user_tags'), 40, TB_('Tags'), '', [
        'maxlength' => 255,
        'style' => 'width:100%',
...
if ($is_not_content_block &&
    check_user_perm('blog_edit_ts', 'edit', false, $Blog->ID)) {	// Display goal tracking and notifications for item with type usage except of content block
    // and if user has a permission to edit advanced properties of items:

    // ################### GOAL TRACKING ###################

    $Form->begin_fieldset(
        TB_('Goal tracking') . get_manual_link('post-goal-tracking-panel')
                    . action_icon(TB_('Goals'), 'edit', $admin_url . '?ctrl=goals&amp;blog=' . $Blog->ID, TB_('Goals'), 3, 4, [
                        'class' => 'action_icon pull-right',
                    ]),
        [
            'id' => 'itemform_goals',
    ...
    if (! empty($goal_ID) && $item_Goal = $GoalCache->get_by_ID($goal_ID, false, false)) { // Get category ID of goal
        $item_goal_cat_ID = $item_Goal->gcat_ID;
    }

    $GoalCategoryCache = &get_GoalCategoryCache(NT_('No Category'));
    $GoalCategoryCache->load_all();
    $Form->select_input_object('goal_cat_ID', $item_goal_cat_ID, $GoalCategoryCache, TB_('Category'), [
        'allow_none' => true,
    ]);

    // Get only the goals without a defined redirect url
    $goals_where_sql = 'goal_redir_url IS NULL';
    if (empty($item_goal_cat_ID)) { // Get the goals without category
        $goals_where_sql .= ' AND goal_gcat_ID IS NULL';
    } else { // Get the goals by category ID
        $goals_where_sql .= ' AND goal_gcat_ID = ' . $DB->quote($item_goal_cat_ID);
    }
    $GoalCache->load_where($goals_where_sql);
    $Form->select_input_object(
        'goal_ID',
        $edited_Item->get_setting('goal_ID'),
        $GoalCache,
        get_icon('multi_action', 'imgtag', [
            'style' => 'margin:0 5px 0 14px;position:relative;top:-1px;',
        ]) . TB_('Goal'),
        [
            'allow_none' => true,
    ...
    $Form->begin_fieldset(TB_('Notifications') . get_manual_link('post-notifications-panel'), [
        'id' => 'itemform_notifications',
        'fold' => true,
    ]);
    ...
    foreach ($notify_types as $notify_type => $notify_title) {
        if (! $edited_Item->notifications_allowed()) {	// Notifications are not allowed for the Item:
            $Form->info($notify_title, TB_('Not Possible for this post type'));
        } else {	// Notifications are allowed for the Item:
            if ($edited_Item->check_notifications_flags($notify_type)) {	// Nofications/Pings were sent:
            ...
            } elseif ($edited_Item->get_type_setting('usage') != 'post') {	// Item type is not applicable and Nofications/Pings are not sent yet:
            ...
            } else {	// Nofications/Pings are not sent yet:
            ...
            $Form->select_input_array('item_' . $notify_type, get_param('item_' . $notify_type), $notify_select_options, $notify_title, null, [
...
if (check_user_perm('blog_post!published', 'create', false, $Blog->ID)) { // Display a link to show/hide quick button to publish the post ONLY if current user has a permission:
    echo '<p>';
    if ($UserSettings->get_collection_setting('show_quick_publish', $Blog->ID)) { // The quick button is displayed
        echo action_icon('', 'deactivate', $quick_setting_url . 'hide_quick_button', TB_('Show the quick "Publish!" button when relevant.'), 3, 4);
    } else { // The quick button is hidden
...
if (empty($edited_Item->ID)) { // if we creating new post - we add slug autofiller JS
    echo_slug_filler();
} else {	// if we are editing the post
...
if ($edited_Item->can_meta_comment()) {	// Init Item Checklist JS to update red badge in tab of not checked lines:
