...
function get_actions_for_itemtype($id)
{
    ...
    // Copy Item Type:
    $action .= action_icon(
        T_('Duplicate this Item Type...'),
        'copy',
        regenerate_url('action', 'ityp_ID=' . $id . '&amp;action=new')
    );

    if (is_pro() && check_user_perm('options', 'edit')) {	// Export Item Type only for PRO version:
        $action .= action_icon(
            T_('Export this Item Type...'),
            'download',
            $admin_url . '?ctrl=exportxml&amp;action=export_itemtype&amp;ityp_ID=' . $id . '&amp;' . url_crumb('itemtype')
        );
    }

    if (! in_array($id, $default_ids)) {	// Delete only the not default item types:
        $action .= action_icon(
            T_('Delete this Item Type!'),
            'delete',
...
function ityp_row_enabled($enabled, $item_type_ID)
{
    ...
    if ($enabled) { // Enabled
        if ($perm_edit && $Blog->can_be_item_type_disabled($item_type_ID)) { // URL to disable the item type
            $status_url = $admin_url . '?ctrl=itemtypes&amp;action=disable&amp;ityp_ID=' . $item_type_ID . '&amp;blog=' . $Blog->ID . '&amp;' . url_crumb('itemtype');
        }
        $status_icon = get_icon('bullet_green', 'imgtag', [
            'title' => TB_('The item type is enabled.'),
        ]);
    } else { // Disabled
        if ($perm_edit) { // URL to enable the item type
            $status_url = $admin_url . '?ctrl=itemtypes&amp;action=enable&amp;ityp_ID=' . $item_type_ID . '&amp;blog=' . $Blog->ID . '&amp;' . url_crumb('itemtype');
        }
        if ($Blog->has_items_per_item_type($item_type_ID)) {	// Use orange icon if collection has at least one Item per this disabled Item Type:
            $status_icon = get_icon('bullet_orange', 'imgtag', [
                'title' => TB_('Disabled but used by some Items in this collection.'),
            ]);
        } else {	// Use "grey empty" icon if collection has no Items per this disabled Item Type:
            $status_icon = get_icon('bullet_empty_grey', 'imgtag', [
                'title' => TB_('The item type is disabled.'),
            ]);
        }
    }
...
function ityp_row_default($item_type_ID)
{
    ...
    if ($Blog->get_setting('default_post_type') == $item_type_ID) { // The item type is default for current collection:
        $status_icon = get_icon('bullet_black', 'imgtag', [
            'title' => sprintf(T_('The item type is the default for %s.'), $Blog->get('shortname')),
        ]);
    } else { // The item type is not default:
        if (check_user_perm('blog_properties', 'edit', false, $Blog->ID)) { // URL to use the item type as default if current user has a permission to edit collection properties:
...
if (check_user_perm('options', 'edit', false)) { // We have permission to modify:
    ...
    $Results->global_icon(
        T_('Import Item Type'),
        'import',
        $admin_url . '?ctrl=itimport&amp;it_blog_IDs[]=' . $Blog->ID,
        T_('Import Item Type') . ' &raquo;',
        3,
        4,
        [
    ...
    $Results->global_icon(
        T_('Create a new element...'),
        'new',
        regenerate_url('action', 'action=new'),
        T_('New Item Type') . ' &raquo;',
        3,
        4,
        [
