...
if ($lastedit_User = &$edited_Item->get_lastedit_User()) {	// Get login of last edit user
...
if ($revisions_count > 1) {	// Dispay the selectors to compare the revisions
    $Results->cols[] = [
        'th' => '',
        'th_class' => 'shrinkwrap',
        'td_class' => 'shrinkwrap',
...
function iver_td_actions($iver_ID, $iver_type)
{
    ...
    if ($iver_type != 'current') {
        $permanent_url = url_add_param($permanent_url, [
            'revision' => ($iver_type == 'proposed' ? 'p' : '') . $iver_ID,
        ]);
    }
    $r .= '<a href="' . $permanent_url . '" class="action_icon btn btn-info btn-xs">' . T_('View') . '</a>';

    if ($iver_type == 'archived') {	// Button to restore the version:
    ...
    if ($iver_type == 'proposed') {	// Button to accept or reject the proposed versions:
        global $admin_url;
...
$buttons = [];
if ($revisions_count > 1) {	// Button to compare the revisions
    $buttons = [['submit', '', T_('Compare selected revisions'), 'SaveButton']];
...
if ($revisions_count > 1) {	// Print JS code for selectors to compare the revisions
    ?>
<script>
