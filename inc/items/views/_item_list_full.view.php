...
if ($action == 'view') { // We are displaying a single post:
    echo '<div class="global_actions">'
            . action_icon(
                T_('Close post'),
                'close',
                regenerate_url('p,action', 'filter=restore&amp;highlight=' . $p),
                null,
                null,
                null,
                [
                    'class' => 'action_icon btn btn-default',
                ]
            )
        . '</div>';
...
} else { // We are displaying multiple posts ( Not a single post! )
...
if ($allow_items_list_form) {	// Allow to select item for action only on items list if current user can edit at least one item status:
    global $admin_url;
...
while ($Item = &$ItemList->get_item()) {
    ...
    $Item->permanent_link([
        ...
        'after' => ' ' . action_icon(
            T_('Copy Item Slug to the clipboard.'),
            'clipboard-copy',
            '#',
            null,
            null,
            null,
            [
                'class' => 'small clipboard-copy',
                'data-clipboard-text' => $Item->urltitle,
    ...
    global $admin_url;
    if (check_user_perm('slugs', 'view')) { // user has permission to view slugs:
        echo '&nbsp;' . action_icon(
            T_('Edit slugs') . '...',
            'edit',
            $admin_url . '?ctrl=slugs&amp;slug_item_ID=' . $Item->ID,
            null,
            null,
            null,
            [
    ...
    if ($parent_Item = $Item->get_parent_Item()) {	// Display parent ID if the Item has it:
        echo ' &middot; ' . T_('Parent ID') . ': ';
        if (check_user_perm('item_post!CURSTATUS', 'view', false, $parent_Item)) {	// Display parent ID as link to view the parent post if current user has a permission:
        ...
        } else {	// Display parent ID as text if current user has a permission to view the parent post:
            echo $parent_Item->ID;
        }
    }
    echo '<br>';
    ...
    if ($action != 'view' && check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) {	// Display checkbox to select item for action only on items list:
    ...
    echo ' ', T_('by'), ' ', $Item->creator_User->get_identity_link([
        'link_text' => 'name',
    ]);
    ...
    if ($Blog->get_setting('use_workflow')) { // Only display workflow properties, if activated for this blog.
        ...
        if ($Blog->get_setting('use_deadline') && ! empty($Item->datedeadline)) { // Display deadline date
    ...
    $edit_buttons = [];
    if ($item_edit_url = $Item->get_edit_url()) {	// Edit
        $edit_buttons[] = [
            'url' => $item_edit_url,
    ...
    if ($item_propose_change_url = $Item->get_propose_change_url()) {	// Propose change
        $edit_buttons[] = [
            'url' => $item_propose_change_url,
            'text' => get_icon('edit_button') . ' ' . T_('Propose change'),
        ];
    }
    if ($item_copy_url = $Item->get_copy_url()) {	// Duplicate...
        $edit_buttons[] = [
            'url' => $item_copy_url,
            'text' => get_icon('copy') . ' ' . T_('Duplicate...'),
        ];
    }
    if ($item_merge_click_js = $Item->get_merge_click_js($params)) {	// Merge with...
        $edit_buttons[] = [
    ...
    if ($edit_buttons_num > 1) {	// Display buttons in dropdown style:
        ...
        echo '</ul></div>';
    } elseif ($edit_buttons_num == 1) {	// Display single button:
    ...
    if ($next_status_in_row && $next_status_in_row[0] != 'deprecated') { // Display deprecate button if current user has the rights:
        $Item->deprecate_link('', '', get_icon('move_down_grey', 'imgtag', [
            'title' => '',
        ]), '#', button_class());
    }
	...
	<?php
        if ($action == 'view') { // We are looking at a single post, include files and comments:
            if ($comment_type == 'meta' && ! $Item->can_see_meta_comments()) { // Current user cannot views internal comments
                $comment_type = 'feedback';
            }

            if (isset($GLOBALS['files_Module'])) { // Files:
            ...
            echo action_icon(
                T_('Refresh comment list'),
                'refresh',
                url_add_param($admin_url, 'ctrl=items&amp;blog=' . $blog . '&amp;p=' . $Item->ID . '#comments'),
                ' ' . T_('Refresh'),
                3,
                4,
                [
            ...
            if ($comment_type != 'meta') { // Don't display "Recycle bin" link for internal comments, because they are deleted directly without recycle bin
                echo get_opentrash_link(true, false, [
            ...
            if ($Item->can_see_meta_comments()) { // Display tabs to switch between user and internal comments Only if current user can views internal comments
                $switch_comment_type_url = $admin_url . '?ctrl=items&amp;blog=' . $blog . '&amp;p=' . $Item->ID;
                $metas_count = generic_ctp_number($Item->ID, 'metas', 'total', true);
                $switch_comment_type_tabs = [
                    'feedback' => [
            ...
            if (($comment_type != 'meta') && // Display all comments in meta mode by default
                ($total_comments_number > 5 && $moderation_comments_number > 0)) {	// Show only requiring moderation comments:
            ...
            if ($show_comments_expiry == 'all') { // Display also the expired comments
			...
			<a id="comments"></a>
			<?php
            if ($display_params['disp_rating_summary']) { // Display a ratings summary
                echo $Item->get_rating_summary();
            }

            if ($comment_type != 'meta') { // Display this filter only for not internal comments
                ?>
				<div class="tile"><label><?php echo T_('Show') . ':' ?></label></div>
				<div class="tile">
					<input type="radio" name="show_comments" value="moderation" id="only_moderation" class="radio" <?php if ($show_comments == 'moderation') {
					    echo 'checked="checked" ';
					}?> />
					<label for="only_moderation"><?php echo T_('Requiring moderation') ?></label>
				</div>
				<div class="tile">
					<input type="radio" name="show_comments" value="valid" id="only_valid" class="radio" <?php if ($show_comments == 'valid') {
					    echo 'checked="checked" ';
					}?> />
					<label for="only_valid"><?php echo T_('Valid') ?></label>
				</div>
				<div class="tile">
					<input type="radio" name="show_comments" value="all" id="show_all" class="radio" <?php if ($show_comments == 'all') {
					    echo 'checked="checked" ';
					}?> />
                ...
                if (! empty($expiry_delay)) { // A filter to display even the expired comments
                    ?>
				<div class="tile">
					&nbsp; | &nbsp;
					<input type="radio" name="show_comments_expiry" value="expiry" id="show_expiry_delay" class="radio" <?php if ($show_comments_expiry == 'active') {
					    echo 'checked="checked" ';
					}?> />
					<label for="show_expiry_delay"><?php echo get_duration_title($expiry_delay); ?></label>
				</div>
				<div class="tile">
					<input type="radio" name="show_comments_expiry" value="all" id="show_expiry_all" class="radio" <?php if ($show_comments_expiry == 'all') {
					    echo 'checked="checked" ';
					}?> />
            ...
            echo_item_comments($blog, $Item->ID, $statuses, $currentpage, null, [], '', $expiry_statuses, $comment_type);
            echo '</div>';
            ...
            if (($comment_type == 'meta' && $Item->can_meta_comment()) // User can add internal comment on the Item
                ...
                if ($preview_Comment) {	// If preview comment is displayed currently
                    if (empty($comment_reply_ID) || empty($preview_Comment->in_reply_to_cmt_ID)) {	// Display a previewed comment under all comments only if it is not replied on any other comment:
			...
			<?php
                ...
                if ($preview_Comment) {	// Get a Comment properties from preview request:
                    ...
                    if (($Comment = get_comment_from_session('unsaved', $comment_type)) === null) {	// There is no saved Comment in Session
                        $Comment = new Comment();
                        $Comment->set('type', $comment_type);
                        $Comment->set('item_ID', $Item->ID);
                        $comment_attachments = '';
                        $checked_attachments = '';
                    } else {	// Get params from Session:
                ...
                if ($comment_type != 'meta' && $Item->can_rate()) {	// Comment rating:
                    ob_start();
                ...
                $Form->textarea_input($dummy_fields['content'], $comment_content, 12, T_('Comment text'), [
                    'cols' => 40,
                ...
                $Form->info(T_('Text Renderers'), $Plugins->get_renderer_checkboxes($comment_renderers, [
                    'Blog' => &$Blog,
                ...
                if (! empty($comment_attachments)) {	// display already attached files checkboxes
                    ...
                    foreach ($attachments as $attachment_ID) {
                        ...
                        if ($attachment_File) {
                            ...
                            $list_options[] = ['preview_attachment' . $attachment_ID, 1, $attachment_File->get('name'), $checked, false];
                        }
                    }
            ...
            if ($Item->can_edit_workflow()) {	// Display workflow properties if current user can edit at least one workflow property:
                ...
                $Form->button(['submit', 'actionArray[update_workflow]', T_('Update'), 'SaveButton']);
...
if ($action == 'view') {	// Load JS functions to work with comments
    load_funcs('comments/model/_comment_js.funcs.php');

    // Handle show_comments radioboxes
    echo_show_comments_changed($comment_type);
} elseif ($allow_items_list_form) {	// Allow to select item for action only on items list if current user can edit at least one item status:
    // Buttons to check/uncheck/invert all Items:
    $Form->checkbox_controls('selected_items', [
    ...
    $Form->button(['button', 'mass_change_main_cat', T_('Change primary category')]);
    $Form->button(['button', 'mass_add_extra_cat', T_('Add secondary category')]);
    echo '</span> ';
    if (is_pro() && check_user_perm('options', 'edit')) {	// Export Items only for PRO version:
        $Form->button(['submit', 'actionArray[mass_export]', T_('Export to XML')]);
    }
    $Form->button(['submit', 'actionArray[mass_delete]', T_('Delete'), 'btn-danger']);
