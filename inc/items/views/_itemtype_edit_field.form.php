...
// Link options:
if (! in_array(get_param('itcf_type'), ['text', 'html', 'separator'])) {
    $Form->begin_line(TB_('Link'));
    // Link:
    $Form->select_input_array('itcf_link', get_param('itcf_link'), get_item_type_field_linkto_options(get_param('itcf_type')), '', '', [
        'force_keys_as_values' => true,
    ]);
    // No follow:
    $Form->checkbox_input('itcf_link_nofollow', get_param('itcf_link_nofollow'), '', [
        'input_prefix' => '<label class="text-normal">',
        'input_suffix' => ' ' . TB_('No Follow') . '</label>',
    ]);
    $Form->end_line();
...
// Highlight options:
if (get_param('itcf_type') != 'separator') {
    $Form->select_input_array('itcf_line_highlight', get_param('itcf_line_highlight'), get_item_type_field_highlight_options('line'), TB_('Line highlight'), '', [
        'force_keys_as_values' => true,
    ]);
    $Form->select_input_array('itcf_green_highlight', get_param('itcf_green_highlight'), get_item_type_field_highlight_options('green'), TB_('Green highlight'), '', [
        'force_keys_as_values' => true,
    ]);
    $Form->select_input_array('itcf_red_highlight', get_param('itcf_red_highlight'), get_item_type_field_highlight_options('red'), TB_('Red highlight'), '', [
        'force_keys_as_values' => true,
    ]);
}
...
$Form->end_form([['submit', 'actionArray[select_custom_fields]', TB_('Update'), 'SaveButton']]);
