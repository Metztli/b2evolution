...
if (empty($plugin_button)) {	// If button is not displayed by any plugin
...
$Form->submit(['actionArray[save_propose]', TB_('Propose change'), 'btn-primary evo_propose_change_btn']);
echo '</div>';
...
if (count($custom_fields)) {	// Display fieldset with custom fields only if at least one exists:
    ...
    if (check_user_perm('options', 'edit')) {	// Display an icon to edit post type if current user has a permission:
    ...
    $Form->begin_fieldset($custom_fields_title, [
        'id' => 'itemform_custom_fields',
        'fold' => true,
    ]);
