...
class ItemListLight extends DataObjectList2
{
     ...
     */
    public $last_displayed_date = '';

    /**
     * Lazy filled
     * @access private
     */
    public $advertised_start_date;

    public $advertised_stop_date;

    /**
     * Anti infinite loops:
     */
    public $getting_adv_start_date = false;

    public $getting_adv_stop_date = false;

    public $group_by_cat = 0;
    ...
    public function __construct(
        ...
        if (! empty($filterset_name)) {	// Set the filterset_name with the filterset_name param
            $this->filterset_name = 'ItemList_filters_' . preg_replace('#[^a-z0-9\-_]#i', '', $filterset_name);
        } else {	// Set a generic filterset_name
     ...
     */
    public function set_filters($filters, $memorize = true, $use_previous_filters = false)
    {
        if (! empty($filters)) {	// Activate the filterset (fallback to default filter when a value is not set):
            if ($use_previous_filters) {	// If $this->filters were activated before(e.g. on load from request), they can be saved here
                $this->filters = array_merge($this->default_filters, $this->filters, $filters);
            } else {	// Don't use the filters from previous request
        ...
        if ($memorize) {	// set back the GLOBALS !!! needed for regenerate_url() :
            /*
            ...
            if (isset($this->filters['cat_modifier'])) {	// Update cat param with the cat modifier only if it was set explicitly, otherwise it may overwrite the global $cat variable
            ...
            if (is_null($this->default_filters['ts_min'])
                && is_null($this->default_filters['ts_max'])) {	// We have not set a strict default -> we allow overridding:
     ...
     */
    public function load_from_Request($use_filters = true)
    {
        ...
        if ((
            empty($catsel) || // 'catsel' multicats filter is not defined
        ...
        if (is_null($this->default_filters['ts_min'])
            ...
            if ($show_past != $show_future) {	// There is a point in overridding:
         ...
         */
        // Note: oftentimes, $show_statuses will have been preset to a more restrictive set of values
        $this->filters['visibility_array'] = param($this->param_prefix . 'show_statuses', 'array:string', $this->default_filters['visibility_array'], true, false, true, false);	// Array of sharings to restrict to
        ...
        if ($this->single_post) {	// We have requested a specific post
    ...
    public function query_init()
    {
        ...
        if (empty($this->filters)) {	// Filters have not been set before, we'll use the default filterset:
         ...
         * Filtering stuff:
         */
        if (! is_null($this->Blog)) {	// Get the posts only for current Blog
            $this->ItemQuery->where_chapter2(
                $this->Blog,
                $this->filters['cat_array'],
                $this->filters['cat_modifier'],
        ...
        $this->ItemQuery->where_datestart(
            $this->filters['ymdhms'],
            $this->filters['week'],
        ...
        if (! $this->single_post) {	// Restrict with locale visibility by current navigation locale ONLY for not single page:
        ...
        } elseif ($this->filters['orderby'] == 'ID_list') {	// Use blog setting here because 'orderby' might be set to 'ID_list' as default filter
            $this->filters['orderby'] = $this->Blog->get_setting('orderby');
        }

        if (isset($this->filters['orderby']) && $this->filters['orderby'] == 'numviews') {	// Order by number of views
        ...
        */
        elseif ($this->filters['unit'] == 'days'  // We are going to limit to x days: no limit
            ...
            $this->total_pages = 1;
            $this->page = 1;
        } elseif ($this->filters['unit'] == 'posts') {	// Calculate a count of the posts
            if ($this->ItemQuery->get_group_by() == '') {	// SQL query without GROUP BY clause
            ...
            } else {	// SQL query with GROUP BY clause, Summarize a count of each grouped result
        ...
        */
        elseif ($this->filters['unit'] == 'all') {	// We want ALL results!
        } elseif ($this->filters['unit'] == 'posts') {
            // TODO: dh> check if $limit is NULL!? - though it should not arrive at $page>1 then..
            // echo 'LIMIT POSTS ';
            $pgstrt = '';
            if ($this->page > 1) {	// We have requested a specific page number
                $pgstrt = (intval($this->page) - 1) * $this->limit . ', ';
            }
            $this->ItemQuery->LIMIT($pgstrt . $this->limit);
        } elseif ($this->filters['unit'] == 'days') {	// We are going to limit to x days:
            // echo 'LIMIT DAYS ';
            if (empty($this->filters['ymdhms_min'])) {	// We have no start date, we'll display the last x days:
                if (! empty($this->filters['keywords'])
                    || ! empty($this->filters['cat_array'])
                    || ! empty($this->filters['authors'])) {	// We are in DAYS mode but we can't restrict on these! (TODO: ?)
                    $limits = '';
                } else {	// We are going to limit to LAST x days:
            ...
            } else {	// We have a start date, we'll display x days starting from that point:
     ...
     */
    public function query($create_default_cols_if_needed = true, $append_limit = true, $append_order_by = true)
    {
    ...
    public function run_query(
        ...
        if (! is_null($this->rows)) {	// Query has already executed:
            return;
        }
        ...
        if (isset($this->total_rows) && (intval($this->total_rows) === 0)) {	// Count query was already executed and returned 0
            return;
        }
        ...
        if (! preg_match('/' . preg_quote('T_postcats') . '( AS ([^\s]+))?/i', $this->ItemQuery->get_from(), $match_postcats_alias)) {	// If categories table is not joined yet we should use it for column postcat_cat_ID
        ...
        if ($this->ItemQuery->get_group_by() == '') {	// Group by item ID only if another grouping is not used currently:
     ...
     */
    public function get_lastpostdate($dateformat = 'Y-m-d H:i:s')
    {
        global $localtimenow, $DB;

        if (empty($this->filters)) {	// Filters have no been set before, we'll use the default filterset:
         ...
         * filtering stuff:
         */
        $lastpost_ItemQuery->where_chapter2(
            $this->Blog,
            $this->filters['cat_array'],
            $this->filters['cat_modifier'],
            $this->filters['cat_focus'],
        ...
        $lastpost_ItemQuery->where_datestart(
            $this->filters['ymdhms'],
            $this->filters['week'],
            $this->filters['ymdhms_min'],
            $this->filters['ymdhms_max'],
     ...
     */
    public function get_filter_titles($ignore = [], $params = [])
    {
        ...
        $filter_classes = ['green'];
        $filter_class_i = 0;
        if (strpos($params['filter_mask'], '$filter_class$') !== false) {	// Initialize array with available classes for filter items
            $filter_classes = ['green', 'yellow', 'orange', 'red', 'magenta', 'blue'];
        }
        ...
        if ($params['display_category']) {
            ...
            if ($params['categories_display'] == 'toplevel' // We'd like to minimize display if possible
                ...
                $catlist = [$this->filters['cat_single']];
            } elseif (! empty($this->filters['cat_array'])) {	// We have requested specific categories...
                $catlist = $this->filters['cat_array'];
            }

            if (! empty($catlist)) {	// We want to show some category names:
                $cat_names = [];
                $ChapterCache = &get_ChapterCache();
                $catsel_param = get_param('catsel');
                foreach ($catlist as $cat_ID) {
                    if (($tmp_Chapter = &$ChapterCache->get_by_ID($cat_ID, false)) !== false) {	// It is almost never meaningful to die over an invalid cat when generating title
                        $cat_clear_url = regenerate_url((empty($catsel_param) ? 'cat=' : 'catsel=') . $cat_ID);
                        if (in_array($disp_detail, ['posts-cat', 'posts-topcat-intro', 'posts-topcat-nointro', 'posts-subcat-intro', 'posts-subcat-nointro'])) {	// Remove category url from $ReqPath when we use the cat url instead of cat ID
                        ...
                        $cat_names[] = str_replace(
                            ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                            [$params['category_text'], $tmp_Chapter->name, $cat_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask']
                        );
                    }
                }
                $filter_class_i++;
                if ($this->filters['cat_modifier'] == '*') {	// Categories with "AND" condition
                    $cat_names_string = implode($params['separator_and'], $cat_names);
                } elseif ($this->filters['cat_modifier'] == '-') {	// Categories with "NOR" condition
                    $cat_names_string = implode($params['separator_nor'], $cat_names);
                } else {	// Categories with "OR" condition
                    $cat_names_string = implode($params['separator_or'], $cat_names);
                }
                if (! empty($cat_names_string)) {
                    if ($this->filters['cat_modifier'] == '-') {	// Categories with "NOR" condition
                        $cat_names_string = $params['categories_nor_text'] . $cat_names_string;
                        $params['category_text'] = $params['categories_text'];
                    }
                    $title_array['cats'] = str_replace(
                        ['$group_title$', '$filter_items$'],
                        (count($this->filters['cat_array']) > 1 ?
                            [$params['categories_text'], $params['before_items'] . $cat_names_string . $params['after_items']] :
                            [$params['category_text'], $cat_names_string]),
                        $params['group_mask']
                    );
                }
            }
        ...
        if ($params['display_archive']) {
            if (! empty($this->filters['ymdhms'])) {	// We have asked for a specific timeframe:
                $my_year = substr($this->filters['ymdhms'], 0, 4);

                if (strlen($this->filters['ymdhms']) > 4) {	// We have requested a month too:
                ...
                if (! empty($my_day)) {	// We also want to display a day
                    $arch .= ', ' . $my_day;
                    $arch = date(locale_extdatefmt(), strtotime(implode('-', [$my_year, $my_month, $my_day])));
                }
                ...
                if ($disp_detail == 'posts-date') {	// Remove archive url from $ReqPath when we use archive url instead of tag ID:
                ...
                $arch = str_replace(
                    ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                    [$params['archives_text'], $arch, $arch_clear_icon, $filter_classes[$filter_class_i]],
                    $params['filter_mask']
                );
                $title_array['ymdhms'] = str_replace(
                    ['$group_title$', '$filter_items$'],
                    [$params['archives_text'], $arch],
                    $params['group_mask']
                );
                $filter_class_i++;
            }
        ...
        if ($params['display_keyword']) {
            if (! empty($this->filters['keywords'])) {
                if ($this->filters['phrase'] == 'OR' || $this->filters['phrase'] == 'AND') {	// Search by each keyword
                    $keywords = trim(preg_replace('/("|, *)/', ' ', $this->filters['keywords']));
                    $keywords = explode(' ', $keywords);
                } else {	// Exact match (Single keyword)
                ...
                foreach ($keywords as $keyword) {
                    ...
                    $keyword_names[] = str_replace(
                        ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                        [$params['keyword_text'], $keyword, $word_clear_icon, $filter_classes[$filter_class_i]],
                        $params['filter_mask']
                    );
                }
                $filter_class_i++;
                ...
                $title_array[] = str_replace(
                    ['$group_title$', '$filter_items$'],
                    (count($keyword_names) > 1 ?
                        [$params['keywords_text'], $params['before_items'] . $keywords . $params['after_items']] :
                        [$params['keyword_text'], $keywords]),
                    $params['group_mask']
                );
            }
        }
        ...
        if ($params['display_tag']) {
            if (! empty($this->filters['tags'])) {
                ...
                foreach ($tags as $tag) {
                    ...
                    if ($disp_detail == 'posts-tag-intro' || $disp_detail == 'posts-tag-nointro') {	// Remove tag url from $ReqPath when we use tag url instead of tag ID
                    ...
                    $tag_names[] = str_replace(
                        ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                        [$params['tag_text'], $tag, $tag_clear_icon, $filter_classes[$filter_class_i]],
                        $params['filter_mask']
                    );
                }
                $filter_class_i++;
                ...
                $title_array[] = str_replace(
                    ['$group_title$', '$filter_items$'],
                    (count($tag_names) > 1 ?
                        [$params['tags_text'], $params['before_items'] . $tags . $params['after_items']] :
                        [$params['tag_text'], $tags]),
                    $params['group_mask']
                );
            }
        }
        ...
        if ($params['display_author']) {
            if (! empty($this->filters['authors']) || ! empty($this->filters['authors_login'])) {
                ...
                if (substr($authors, 0, 1) == '-') {	// Authors are excluded
                    ...
                    foreach ($authors as $author_ID) {
                        if ($tmp_User = $UserCache->get_by_ID($author_ID, false, false)) {
                            ...
                            $author_names[] = str_replace(
                                ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                                [$params['author_text'], $tmp_User->get('login'), $user_clear_icon, $filter_classes[$filter_class_i]],
                                $params['filter_mask']
                            );
                        }
                    }
                ...
                if (count($author_names) > 0) {	// Display info of filter by authors
                    if ($exclude_authors) {	// Exclude authors
                    ...
                    $title_array[] = str_replace(
                        ...
                        (count($author_names) > 1 ?
                            [$params['authors_text'], $params['before_items'] . $author_names_string . $params['after_items']] :
                            [$params['author_text'], $author_names_string]),
                        $params['group_mask']
                    );
                }
            }
        ...
        if ($params['display_assignee']) {
            if (! empty($this->filters['assignees']) || ! empty($this->filters['assignees_login'])) {
                ...
                if ($this->filters['assignees'] == '-') {
                    ...
                    $title_array[] = str_replace(
                        ['$filter_name$', '$clear_icon$', '$filter_class$'],
                        [T_('Not assigned'), $user_clear_icon, $filter_classes[$filter_class_i]],
                    ...
                    if ($assignees) {
                        ...
                        foreach ($assignees as $user_ID) {
                            if ($tmp_User = &$UserCache->get_by_ID($user_ID, false, false)) {
                                ...
                                $assignees_names[] = str_replace(
                                    ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                                    [$params['assignes_text'], $tmp_User->get_identity_link([
                                        'link_text' => 'name',
                                    ]), $user_clear_icon, $filter_classes[$filter_class_i]],
                                    $params['filter_mask']
                                );
                            }
                        }
                    ...
                    $title_array[] = str_replace(
                        ...
                        (count($assignees_names) > 1 ?
                            [$params['assignes_text'], $params['before_items'] . implode($params['separator_comma'], $assignees_names) . $params['after_items']] :
                            [$params['assignes_text'], implode($params['separator_comma'], $assignees_names)]),
                        $params['group_mask']
                    );
                }
                $filter_class_i++;
        ...
        if ($params['display_involves']) {
            if (! empty($this->filters['involves']) || ! empty($this->filters['involves_login'])) {
                ...
                if (substr($involves, 0, 1) == '-') {	// Authors are excluded
                ...
                if ($involves) {
                    ...
                    foreach ($involves as $involves_ID) {
                        if ($tmp_User = $UserCache->get_by_ID($involves_ID, false, false)) {
                            ...
                            $involves_names[] = str_replace(
                                ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                                [$params['involves_text'], $tmp_User->get('login'), $user_clear_icon, $filter_classes[$filter_class_i]],
                                $params['filter_mask']
                            );
                        }
                    }
                ...
                if (count($involves_names) > 0) {	// Display info of filter by involves
                    if ($exclude_involves) {	// Exclude involves
                        $involves_names_string = $params['involves_nor_text'] . implode($params['separator_nor'], $involves_names);
                    } else {	// Filter by involves
                        $involves_names_string = implode($params['separator_comma'], $involves_names);
                    }

                    $title_array[] = str_replace(
                        ['$group_title$', '$filter_items$'],
                        [$params['involves_text'], $params['before_items'] . $involves_names_string . $params['after_items']],
                        $params['group_mask']
                    );
                }
            }
        ...
        if ($params['display_locale']) {
            if ($this->filters['lc'] != 'all') {
                ...
                $loc = str_replace(
                    ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                    [T_('Locale') . ': ', $this->filters['lc'], $user_clear_icon, $filter_classes[$filter_class_i]],
                    $params['filter_mask']
                );
                $title_array[] = str_replace(
                    ['$group_title$', '$filter_items$'],
        ...
        if ($params['display_status']) {
            if (! empty($this->filters['statuses']) || ! empty($this->filters['statuses_array'])) {
                ...
                if (isset($this->filters['statuses_array']) &&
                    ...
                    ! empty($this->filters['statuses_array'])) {	// Filter by array of statuses is used currently:
                    ...
                    if (strlen($filter_statuses[0]) > 1 &&
                        substr($filter_statuses[0], 0, 1) == '-') {	// Filter to exclude by statuses:
                    ...
                    } else {	// Filter to include by statuses:
                        $task_status_separator = $params['separator_or'];
                        $task_status_prefix = '';
                    }
                } else {	// No filters by status:
                    $filter_statuses = [];
                }
                $ItemStatusCache = &get_ItemStatusCache();
                $task_status_titles = [];
                foreach ($filter_statuses as $filter_status) {
                    if ($filter_status == '-') {	// Without status:
                        $status_clear_icon = $clear_icon ? action_icon(T_('Remove this filter'), 'remove', regenerate_url($filter_status_param . '=-')) : '';
                        $task_status_titles[] = str_replace(
                            ['$filter_name$', '$clear_icon$', '$filter_class$'],
                            [T_('Without status'), $status_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask_nogroup']
                        );
                    } elseif ($ItemStatus = &$ItemStatusCache->get_by_ID($filter_status, false, false)) {	// Specific status:
                        $status_clear_icon = $clear_icon ? action_icon(T_('Remove this filter'), 'remove', regenerate_url($filter_status_param . '=' . $ItemStatus->ID)) : '';
                        $task_status_titles[] = str_replace(
                            ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                            [$params['status_text'], $ItemStatus->get_name(), $status_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask']
                        );
                    }
                }
                if (count($task_status_titles) > 0) {
                    ...
                    $title_array['task_statuses'] = str_replace(
                        ['$group_title$', '$filter_items$'],
                        (count($task_status_titles) > 1 ?
                            [$params['statuses_text'], $params['before_items'] . $task_status_titles_string . $params['after_items']] :
                            [$params['status_text'], $task_status_titles_string]),
                        $params['group_mask']
                    );
                    $filter_class_i++;
                }
        ...
        if ($params['display_visibility']) {
            if (! in_array('visibility', $ignore)) {
                ...
                if (count($this->filters['visibility_array']) != count($post_statuses)) {	// Display it only when visibility filter is changed
                    $status_titles = [];
                    $filter_class_i = ($filter_class_i > count($filter_classes) - 1) ? 0 : $filter_class_i;
                    foreach ($this->filters['visibility_array'] as $status) {
                        ...
                        $status_titles[] = str_replace(
                            ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                            [$params['visibility_text'], $post_statuses[$status], $vis_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask']
                        );
                    }
                    $filter_class_i++;
                    $title_array[] = str_replace(
                        ...
                        (count($status_titles) > 1 ?
                            [$params['visibility_text'], $params['before_items'] . implode($params['separator_comma'], $status_titles) . $params['after_items']] :
                            [$params['visibility_text'], implode($params['separator_comma'], $status_titles)]),
                        $params['group_mask']
                    );
                }
            }
        ...
        if ($params['display_itemtype']) {
            ...
            if (! empty($item_type_IDs) && ! in_array('itemtype', $ignore)) {	// We want to show some Item Type names:
                $filter_class_i = ($filter_class_i > count($filter_classes) - 1) ? 0 : $filter_class_i;
                $type_names = [];
                ...
                foreach ($item_type_IDs as $item_type_ID) {
                    if (($tmp_ItemType = &$ItemTypeCache->get_by_ID($item_type_ID, false, false)) !== false) {
                        ...
                        $type_names[] = str_replace(
                            ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                            [$params['type_text'], $tmp_ItemType->name, $type_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask']
                        );
                    }
                }
                ...
                $title_array[] = str_replace(
                    ['$group_title$', '$filter_items$'],
                    (count($type_names) > 1 ?
                        [$params['types_text'], $params['before_items'] . $type_name_string . $params['after_items']] :
                        [$params['type_text'], $type_name_string]),
                    $params['group_mask']
                );
            }
        }
        ...
        if ($params['display_time']) {
            ...
            if (! empty($this->filters['ymdhms_min']) || ! empty($this->filters['ts_min'])) {
                ...
                if (! empty($this->filters['ymdhms_min'])) {
                    ...
                    $title_array['ts_min'] = str_replace(
                        ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                        [T_('Start at') . ': ', date2mysql($this->filters['ymdhms_min']), $time_clear_icon, $filter_classes[$filter_class_i]],
                        $params['filter_mask']
                    );
                } else {
                    if ($this->filters['ts_min'] == 'now') {
                        if (! in_array('hide_past', $ignore) && ($this->filters['ts_min'] != $this->default_filters['ts_min'])) {
                            ...
                            $title_array['ts_min'] = str_replace(
                                ['$filter_name$', '$clear_icon$', '$filter_class$'],
                                [T_('Hide past'), $time_clear_icon, $filter_classes[$filter_class_i]],
                                $params['filter_mask_nogroup']
                            );
                        }
                    } else {
                        ...
                        $title_array['ts_min'] = str_replace(
                            ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                            [T_('Start at') . ': ', date2mysql($this->filters['ts_min']), $time_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask']
                        );
                    }
                }
            ...
            if (! empty($this->filters['ymdhms_max']) || ! empty($this->filters['ts_max'])) {
                ...
                if (! empty($this->filters['ymdhms_max'])) {
                    ...
                    $title_array['ts_max'] = str_replace(
                        ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                        [T_('Stop at') . ': ', date2mysql($this->filters['ymdhms_max']), $time_clear_icon, $filter_classes[$filter_class_i]],
                        $params['filter_mask']
                    );
                } else {
                    if ($this->filters['ts_max'] == 'now') {
                        if (! in_array('hide_future', $ignore) && ($this->filters['ts_max'] != $this->default_filters['ts_max'])) {
                            ...
                            $title_array['ts_max'] = str_replace(
                                ['$filter_name$', '$clear_icon$', '$filter_class$'],
                                [T_('Hide future'), $time_clear_icon, $filter_classes[$filter_class_i]],
                                $params['filter_mask_nogroup']
                            );
                        }
                    } else {
                        ...
                        $title_array['ts_max'] = str_replace(
                            ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                            [T_('Stop at') . ': ', date2mysql($this->filters['ts_max']), $time_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask']
                        );
                    }
                }
        ...
        if ($params['display_limit']) {
            ...
            } elseif ($this->filters['unit'] == 'posts' || $this->filters['unit'] == 'all') {	// We're going to page, so there's no real limit here...
            } elseif ($this->filters['unit'] == 'days') {	// We are going to limit to x days:
                // echo 'LIMIT DAYS ';
                $filter_class_i = ($filter_class_i > count($filter_classes) - 1) ? 0 : $filter_class_i;
                if (empty($this->filters['ymdhms_min'])) {	// We have no start date, we'll display the last x days:
                    if (! empty($this->filters['keywords'])
                        ...
                        $title_array['posts'] = str_replace(
                            ['$filter_name$', '$clear_icon$', '$filter_class$'],
                            [sprintf(T_('Limited to last %d days'), $this->limit), $unit_clear_icon, $filter_classes[$filter_class_i]],
                            $params['filter_mask_nogroup']
                        );
                    }
                } else {	// We have a start date, we'll display x days starting from that point:
                    ...
                    $title_array['posts'] = str_replace(
                        ['$filter_name$', '$clear_icon$', '$filter_class$'],
                        [sprintf(T_('Limited to %d days'), $this->limit), $unit_clear_icon, $filter_classes[$filter_class_i]],
                        $params['filter_mask_nogroup']
                    );
                }
                $filter_class_i++;
        ...
        if ($params['display_flagged']) {
            if (! empty($this->filters['flagged'])) {	// Display when only flagged items:
                $filter_class_i = ($filter_class_i > count($filter_classes) - 1) ? 0 : $filter_class_i;
                $unit_clear_icon = $clear_icon ? action_icon(T_('Remove this filter'), 'remove', regenerate_url($this->param_prefix . 'flagged')) : '';
                $title_array['flagged'] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$'],
                    [T_('Flagged'), $unit_clear_icon, $filter_classes[$filter_class_i]],
                    $params['filter_mask_nogroup']
                );
                $filter_class_i++;
            }
        ...
        if ($params['display_mustread']) {
            if (! empty($this->filters['mustread'])) {	// Display when only "must read" items:
                $filter_class_i = ($filter_class_i > count($filter_classes) - 1) ? 0 : $filter_class_i;
                $unit_clear_icon = $clear_icon ? action_icon(T_('Remove this filter'), 'remove', regenerate_url($this->param_prefix . 'mustread')) : '';
                $title_array['mustread'] = str_replace(
                    ['$filter_name$', '$clear_icon$', '$filter_class$'],
                    [T_('Must read'), $unit_clear_icon, $filter_classes[$filter_class_i]],
                    $params['filter_mask_nogroup']
                );
                $filter_class_i++;
            }
        ...
        if ($params['display_renderer'] &&
            ...
            foreach ($this->filters['renderers'] as $renderer_plugin_code) {
                if ($renderer_Plugin = &$Plugins->get_by_code($renderer_plugin_code, false, false)) {
                    ...
                    $task_renderer_titles[] = str_replace(
                        ['$group_title$', '$filter_name$', '$clear_icon$', '$filter_class$'],
                        [$params['renderer_text'], $renderer_Plugin->name, $renderer_clear_icon, $filter_classes[$filter_class_i]],
                        $params['filter_mask']
                    );
                }
            }
            if (count($task_renderer_titles) > 0) {
                ...
                $title_array['task_renderers'] = str_replace(
                    ['$group_title$', '$filter_items$'],
                    (count($task_renderer_titles) > 1 ?
                        [$params['renderers_text'], $params['before_items'] . $task_renderer_titles_string . $params['after_items']] :
                        [$params['renderer_text'], $task_renderer_titles_string]),
                    $params['group_mask']
                );
                $filter_class_i++;
            }
     ...
     * @deprecated
     */
    public function get_total_num_posts()
    {
    ...
    public function &get_category_group()
    {
        ...
        if (empty($this->current_Obj)) {	// Very first call
            // Do a normal get_next()
            parent::get_next();
        }

        if (empty($this->current_Obj)) {	// We have reached the end of the list
    ...
    {
        if ($this->group_by_cat == 1) {	// This is the first call to get_item() after get_category_group()
            $this->group_by_cat = 2;
        ...
        if (! empty($Item) && $this->group_by_cat == 2 && $Item->main_cat_ID != $this->main_cat_ID) {	// We have just hit a new category!
     ...
     */
    public function get_advertised_start_date()
    {
        if ($this->getting_adv_start_date) {	// We would be entering an infinite loop, stop now:
        ...
        if (is_null($this->advertised_start_date)) {	// We haven't determined the start date yet:
            if (! empty($this->filters['ymdhms_min'])) {	// We have requested start date (8 digits)
                ...
                $this->advertised_start_date = mktime(0, 0, 0, substr($m, 4, 2), substr($m, 6, 2), substr($m, 0, 4));
                        ...
                        && ! empty($this->filters['ymdhms'])) {	// we want to restrict on a specific week
                $this->advertised_start_date = get_start_date_for_week(substr($this->filters['ymdhms'], 0, 4), $this->filters['week'], locale_startofweek());
            } elseif (strlen($this->filters['ymdhms']) >= 4) {	// We have requested an interval
                $m = $this->filters['ymdhms'] . '0101';
                $this->advertised_start_date = mktime(0, 0, 0, substr($m, 4, 2), substr($m, 6, 2), substr($m, 0, 4));
            } elseif ($this->filters['unit'] == 'days'
                        && ($stop_date = $this->get_advertised_stop_date()) != '') {	// We want to restrict on a specific number of days after the start date:
                $this->advertised_start_date = $stop_date - ($this->limit - 1) * 86400;
            } else {	// We cannot determine a start date, save an empty string (to differentiate from NULL)
                $this->advertised_start_date = '';
            }
        }
     ...
     *  -dstart + x days
     */
    public function get_advertised_stop_date()
    {
        if ($this->getting_adv_stop_date) {	// We would be entering an infinite loop, stop now:
        ...
        if (is_null($this->advertised_stop_date)) {	// We haven't determined the stop date yet:
            if (! empty($this->filters['ymdhms_max'])) {	// We have requested an end date (8 digits)
            ...
            } elseif (! empty($this->filters['ymdhms'])) {	// We want to restrict on an interval:
                if (strlen($this->filters['ymdhms']) >= 8) {	// We have requested a day interval
                    $m = $this->filters['ymdhms'];
                    $this->advertised_stop_date = mktime(0, 0, 0, substr($m, 4, 2), substr($m, 6, 2), substr($m, 0, 4));
                } elseif (strlen($this->filters['ymdhms']) == 6) {	// We want to go to the end of the month:
                    $m = $this->filters['ymdhms'];
                    $this->advertised_stop_date = mktime(0, 0, 0, substr($m, 4, 2) + 1, 0, substr($m, 0, 4)); // 0th day of next mont = last day of month
                } elseif (strlen($this->filters['ymdhms']) == 4) {	// We want to go to the end of the year:
                    $m = $this->filters['ymdhms'];
                    $this->advertised_stop_date = mktime(0, 0, 0, 12, 31, substr($m, 0, 4));
                }
            } elseif ($this->filters['unit'] == 'days'
                        && ($start_date = $this->get_advertised_start_date()) != '') {	// We want to restrict on a specific number of days after the start date:
                $this->advertised_stop_date = $start_date + ($this->limit - 1) * 86400;
            } else {	// We cannot determine a stop date, save an empty string (to differentiate from NULL)
                $this->advertised_stop_date = '';
            }
        }
    ...
    public function set_start_date()
    {
        ...
        if (! empty($start_date)) {	// Memorize the last displayed as the day BEFORE the one we're going to display
     ...
     */
    public function dates_to_end($before_empty_day = '<h2>', $after_empty_day = '</h2>', $format = '')
    {
        $stop_date = $this->get_advertised_stop_date();

        if (! is_null($stop_date)) {	// There is a stop date, we want to display days:
     ...
     * @param array
     */
    public function date_if_changed($params = [])
    {
        if ($this->current_Obj->get_type_setting('usage') == 'page') {	// This is not applicable to pages
            return;
        }
        ...
        if ($current_item_date != $this->last_displayed_date) {	// Date has changed...
            if ($params['date_format'] == '#') {	// No format specified, use default locale format:
                $params['date_format'] = locale_datefmt();
            }

            if ($params['empty_day_display'] && ! empty($this->last_displayed_date)) {	// We want to display ALL dates from the previous to the current:
     ...
     * Template tag
     */
    public function page_links($params = [])
    {
        $default_params = [
        ...
        if ($this->total_pages <= 1 || $this->page > $this->total_pages) {	// Single page:
        ...
        if ($this->Blog->get_setting('paged_nofollowto')) {	// We prefer robots not to follow to pages:
    ...
    public function display_list($params)
    {
        ...
        if (! empty($params['template'])) {	// DISPLAY with Quick TEMPLATE:
            ...
            if (empty($params['item_template'])) {	// Display error when no template for listing
                return sprintf('Missing %s param', '<code>item_template</code>');
            } elseif (! ($item_Template = &$TemplateCache->get_by_code($params['item_template'], false, false))) {	// Display error when no or wrong template for listing
            ...
            if (! empty($params['highlight_current'])) {	// Use template for active Item only when requested to highlight currently active Item:
                ...
                if ($active_item_template == $item_template ||
                    ! ($active_item_Template = &$TemplateCache->get_by_code($active_item_template, false, false))) {	// If active item template is not found in DB then use normal item template instead:
            ...
            } else {	// Don't highlight currently active Item because it is not requested:
            ...
            if ($crossposted_item_template == $item_template ||
                ! ($crossposted_item_Template = &$TemplateCache->get_by_code($crossposted_item_template, false, false))) {	// If crossposted item template is not found in DB then use normal item template instead:
                $crossposted_item_template = $item_template;
            }

            $this->restart();
            while ($row_Item = &$this->get_item()) {
                if (! empty($params['switch_param_code'])) {	// Start wrapper to make each item block switchable:
                ...
                if ($highlight_current_item &&
                    ! empty($Item) &&
                    $row_Item->ID == $Item->ID) {	// Use different template for currently active Item:
                    $row_item_template = $active_item_template;
                } elseif (! empty($cat) &&
                        $row_Item->main_cat_ID != $cat &&
                        in_array($cat, $row_Item->get('extra_cat_IDs'))) {	// Use different template for crossposted Item:
                    $row_item_template = $crossposted_item_template;
                } else {	// Use normal template to not active Item:
                    $row_item_template = $item_template;
                }

                // Render Item by quick template:
                echo render_template_code($row_item_template, $params, [
                    'Item' => $row_Item,
                ]);

                if (! empty($params['switch_param_code'])) {	// End of switchable item block:
                    echo '</div>';
                }
            }
        ...
        if ($params['item_group_by'] == 'chapter') {	// List grouped by chapter/category:
            $items_map_by_chapter = [];
            $chapters_of_loaded_items = [];
            $group_by_blogs = false;
            $prev_chapter_blog_ID = null;

            $this->restart();
            while ($iterator_Item = &$this->get_item()) {	// Display contents of the Item depending on widget params:
                $Chapter = &$iterator_Item->get_main_Chapter();
                if (! isset($items_map_by_chapter[$Chapter->ID])) {
                    $items_map_by_chapter[$Chapter->ID] = [];
                ...
                if (! $group_by_blogs && ($Chapter->blog_ID != $prev_chapter_blog_ID)) {	// group by blogs is not decided yet
            ...
            if ($group_by_blogs && isset($params['collist_start'])) {	// Start list of blogs
                echo $params['collist_start'];
            } else {	// Display list start, all chapters are in the same group ( not grouped by blogs )
            ...
            foreach ($chapters_of_loaded_items as $Chapter) {
                if ($group_by_blogs && $displayed_blog_ID != $Chapter->blog_ID) {
                    $Chapter->get_Blog();
                    if ($displayed_blog_ID != null) {	// Display the end of the previous blog's chapter list
            ...
            if ($content_is_displayed) {	// End of a chapter list - if some content was displayed this is always required
                echo get_widget_layout_end($item_index, $params);
            }

            if ($group_by_blogs && isset($params['collist_end'])) {	// End of blog list
                echo $params['collist_end'];
            }
        } else {	// Plain list: (not grouped by category)
            echo get_widget_layout_start($params);
        ...
        if ($content_is_displayed) {	// Some content is displayed, Print out widget
            ob_end_flush();
        } else {	// No content, Don't display widget
            ob_end_clean();
        }
     ...
     */
    public function display_list_chapter($Chapter, &$items_map_by_chapter, &$item_index, $params = [])
    {
        $content_is_displayed = false;

        if (isset($items_map_by_chapter[$Chapter->ID]) && (count($items_map_by_chapter[$Chapter->ID]) > 0)) {	// Display Chapter only if it has some items:
    ...
    public function display_list_item_contents(&$disp_Item, &$item_index, $chapter_mode = false, $params = [])
    {
        ...
        if (! empty($params['highlight_current']) && ! empty($Item) && $disp_Item->ID == $Item->ID) {	// The current page is currently displaying the Item this link is pointing to
            // Let's display it as selected
            $link_class = $params['link_selected_class'];
        } else {	// Default link class
        ...
        if ($params['disp_cat'] != 'no') {	// Display categories:
            $disp_Item->categories([
        ...
        if ($params['disp_first_image'] == 'special') {	// If we should display first picture before title then get "Cover" images and order them at top:
            $cover_image_params = [
        ...
        if ($params['attached_pics'] != 'none' && $params['disp_first_image'] == 'special') {	// We want to display first image separately before the title
            // Display before/after even if there is no image so we can use it as a placeholder.
            $this->display_list_images(
                array_merge($params, [
        ...
        if ($params['disp_title']) {	// Display title
            $disp_Item->title([
        ...
        if ($params['disp_excerpt']) {	// Display excerpt
            $excerpt = $disp_Item->get_excerpt();

            $item_permanent_url = $disp_Item->get_permanent_url();
            if (! $params['disp_teaser'] && $item_permanent_url !== false) {	// only display if there is no teaser to display
                $excerpt .= ' <a href="' . $item_permanent_url . '" class="' . $params['item_readmore_class'] . '">' . $params['item_readmore_text'] . '</a>';
            }

            if (! empty($excerpt)) {	// Note: Excerpts are plain text -- no html (at least for now)
        ...
        if ($params['disp_teaser']) {	// we want to show some or all of the post content
            $content = $disp_Item->get_content_teaser(1, false, 'htmlbody');

            if ($words = $params['disp_teaser_maxwords']) {	// limit number of words:
                $content = strmaxwords($content, $words, [
        ...
        if ($params['attached_pics'] == 'all' ||
           ($params['attached_pics'] == 'first' && $params['disp_first_image'] == 'normal') ||
             ($params['attached_pics'] == 'category' && $params['disp_first_image'] == 'normal')) {	// Display attached pictures
            if ($params['attached_pics'] == 'first' || $params['attached_pics'] == 'category') {	// Display only one first image:
                $picture_limit = 1;
            } else {
                $max_pics = intval($params['max_pics']);
                if ($max_pics > 0) {	// Limit images after title with widget param:
                    $picture_limit = $max_pics;
                    if ($params['disp_first_image'] == 'special') {	// If first image is already displayed before title, then we should skip this first to get next images:
                        $picture_limit += 1;
                    }
                } else {	// Don't limit the images:
                    $picture_limit = 1000;
                }
            }
            $this->display_list_images(
    ...
    public function display_list_images(&$content_is_displayed, $params = [])
    {
        ...
        switch ($params['item_pic_link_type']) {	// Set url for picture link
            case 'none':
                $pic_url = null;
                break;
        ...
        if ($params['attached_pics'] != 'category') {
            ...
            if ($LinkList = $LinkOwner->get_attachment_LinkList($params['limit'], $params['restrict_to_image_position'], 'image', $links_params)) {	// Get list of attached files
                $image_num = 1;
                while ($Link = &$LinkList->get_next()) {
                    if (($File = &$Link->get_File()) && $File->is_image()) {	// Get only images
                        if ($image_num < $params['start']) {	// Skip these first images
                            $image_num++;
                            continue;
                        }
        ...
        if (! empty($images)) {	// Print out images only when at least one exists:
            echo $params['before'];
            echo $images;
            echo $params['after'];
            $display_placeholder = false;
        } elseif ($params['limit'] == 1) {	// First picture is empty, fallback to category picture:
            if ($main_Chapter = &$disp_Item->get_main_Chapter()) {	// If item has a main chapter:
                $main_chapter_image_tag = $main_Chapter->get_image_tag([
                    'before' => $params['before'],
                    'after' => $params['after'],
                    'size' => $params['thumb_size'],
                    'link_to' => $pic_url,
                ]);
                if (! empty($main_chapter_image_tag)) {	// If main chapter has a correct image file:
        ...
        if ($display_placeholder) {	// Display placeholder if no images:
