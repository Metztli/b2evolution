...
class ItemType extends DataObject
{
    ...
    public $template_excerpt;

    public $template_normal;
    ...
    public $add_aggregate_rating = 1;

    public $back_instruction = 0;

    public $instruction = '';

    public $text_template;

    public $use_short_title = 'never';

    public $use_title = 'required';

    public $use_url = 'optional';

    public $podcast = 0;

    public $use_parent = 'never';

    public $use_text = 'optional';

    public $allow_html = 1;

    public $allow_breaks = 1;

    public $allow_attachments = 1;

    public $use_excerpt = 'optional';

    public $use_title_tag = 'optional';

    public $use_meta_desc = 'optional';

    public $use_meta_keywds = 'optional';

    public $use_tags = 'optional';

    public $allow_featured = 1;

    public $allow_switchable = 1;

    public $use_country = 'never';

    public $use_region = 'never';

    public $use_sub_region = 'never';

    public $use_city = 'never';

    public $use_coordinates = 'never';

    public $use_comments = 1;

    public $comment_form_msg = '';

    public $allow_comment_form_msg = 0;

    public $allow_closing_comments = 1;

    public $allow_disabling_comments = 0;

    public $use_comment_expiration = 'optional';

    public $perm_level = 'standard';

    public $evobar_link_text = null;

    public $skin_btn_text = null;

    public $short_title_maxlen = 30;

    public $title_maxlen = 100;

    public $front_order_title = 10;

    public $front_order_short_title = null;

    public $front_order_instruction = null;

    public $front_order_attachments = 30;

    public $front_order_text = 80;

    public $front_order_tags = null;

    public $front_order_excerpt = null;

    public $front_order_url = null;

    public $front_order_location = null;

    public $front_order_workflow = 50;
     ...
     * @var array
     */
    public $update_custom_fields = null;

    public $insert_custom_fields = null;

    public $delete_custom_fields = null;
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                'table' => 'T_items__item',
                'fk' => 'post_ityp_ID',
                'msg' => T_('%d related items'),
            ], // "Lignes de visit reports"
            [
                'table' => 'T_categories',
                'fk' => 'cat_ityp_ID',
                'msg' => T_('%d related categories'),
            ],
        ];
    }
    ...
    public static function get_delete_cascades()
    {
        return [
            ...
            [
                'table' => 'T_items__type_custom_field',
                'fk' => 'itcf_ityp_ID',
                'msg' => T_('%d Custom field definitions'),
            ],
            [
                'table' => 'T_items__status_type',
                'fk' => 'its_ityp_ID',
                'msg' => T_('%d Item status associations'),
            ],
        ];
    }
    ...
    public function load_from_Request()
    {
        ...
        if ($use_city == 'required') { // If city is required - all location fields also are required
            $use_country = $use_region = $use_sub_region = 'required';
        } elseif ($use_sub_region == 'required') { // If subregion is required - country & region fields also are required
            $use_country = $use_region = 'required';
        } elseif ($use_region == 'required') { // If region is required - country field also is required
     ...
     * Load custom fields from request
     */
    public function load_custom_fields_from_Request()
    {
        global $Messages;

        // Initialize the arrays
        $this->update_custom_fields = [];
        $this->insert_custom_fields = [];
        $this->delete_custom_fields = trim(param('deleted_custom_fields', 'string', ''), ', ');
        $this->delete_custom_fields = empty($this->delete_custom_fields) ? [] : explode(',', $this->delete_custom_fields);
        ...
        if (empty($custom_field_count)) {	// No custom fields to insert/update:
            return;
        }
        ...
        $inputs = [
            ...
            'link_class' => 'string',
            'line_highlight' => ['string', 'never'],
            'green_highlight' => ['string', 'never'],
            'red_highlight' => ['string', 'never'],
            'description' => 'html',
            'merge' => ['integer', 0],
        ];
        ...
        for ($i = 1; $i <= $custom_field_count; $i++) {
            ...
            foreach ($inputs as $input_name => $input_data) {
                ...
                if (isset($custom_fields_data->{$input_name . $i})) {
                    if ($input_type == 'string' &&
                        ...
                        $custom_fields_data->{$input_name . $i} === '') {	// Use default value for empty string:
                        $input_value = $input_default_value;
                    } else {	// Use submitted value:
                ...
                if ($input_value !== $input_default_value) {	// Format input value to requested type only when it is not default value:
                    if (substr($input_type, 0, 1) == '/') {	// Check value by regexp:
                        if (! empty($input_value) && ! preg_match($input_type, $input_value)) {	// Don't allow wrong value:
            ...
            if (empty($custom_field_ID) || in_array($custom_field_ID, $this->delete_custom_fields)) { // This field was deleted, don't neeed to update
                continue;
            }
            ...
            if (empty($custom_field_data['label'])) { // Field title can't be emtpy
                if (! $empty_title_error) { // This message was not displayed yet
                    $Messages->add(T_('Custom field titles can\'t be empty!'));
                    $empty_title_error = true;
                }
            } elseif (empty($custom_field_data['name'])) { // Field identical name can't be emtpy
                $Messages->add(sprintf(T_('Please enter name for custom field "%s"'), $custom_field_data['label']));
            } elseif (in_array($custom_field_data['name'], $field_names)) { // Field name must be identical
            ...
            if ($custom_field_data['type'] == 'computed') {
                if ($custom_field_data['formula'] === '') {	// Formula must be not empty:
                ...
                if (! $formula_was_changed &&
                    ...
                    $old_computed_custom_fields[$custom_field_data['name']]['formula'] != $custom_field_data['formula']) {	// Inform once user about changed formula:
            ...
            if ($custom_field_is_new) { // Insert custom field
        ...
        foreach ($custom_fields as $custom_field) {
            if ($custom_field['type'] != 'computed') {	// Skip not computed field:
                continue;
            }

            $formula_has_wrong_field = false;
            if (preg_match_all('#\$(.+?)\$#', $custom_field['formula'], $formula_fields)) {	// If formula has at least one field:
                foreach ($formula_fields[1] as $formula_field) {
                    if (! isset($custom_fields[$formula_field])) {	// Not found field:
                        $Messages->add(sprintf(
                            TB_('The field name %s is not recognized (in the formula %s of the field "%s".'),
                            '<code>' . $formula_field . '</code>',
                            '<code>' . $custom_field['formula'] . '</code>',
                            $custom_field['label']
                        ), 'warning');
                        $formula_has_wrong_field = true;
                    } elseif (! in_array($custom_fields[$formula_field]['type'], ['double', 'computed'])) {	// Field with wrong type is used in formula:
                        $Messages->add(sprintf(
                            TB_('Only numeric or computed fields can be used in formulas. Please remove the field %s from the formula %s of the field "%s".'),
                            '<code>' . $formula_field . '</code>',
                            '<code>' . $custom_field['formula'] . '</code>',
                            $custom_field['label']
                        ), 'warning');
            ...
            if ($formula_uses_function = preg_match('#[a-z0-9_]+\s*\(.*?\)#i', $custom_field['formula'])) {	// Don't allow to use functions in formula:
                $Messages->add(sprintf(
                    T_('Forbidden to use functions in formula, please fix %s of the field "%s".'),
                    '<code>' . $custom_field['formula'] . '</code>',
                    $custom_field['label']
                ), 'warning');
            }
            ...
            if (! $formula_has_wrong_field &&
                ...
                $custom_field['formula'] !== '') {	// Check for correct formula:
                ...
                try {	// Compute value:
                    ...
                    if (($formula_code_output !== '' && $formula_code_output !== false) ||
                            ! is_numeric($test_value)) {	// If output buffer contains some text it means there is some error;
                ...
                if ($test_value === null) {	// Display warning when formula cannot be executed properly:
                    $Messages->add(sprintf(
                        TB_('Please check formula %s of the field "%s" because it cannot be evaluated properly.'),
                        '<code>' . $custom_field['formula'] . '</code>',
                        $custom_field['label']
                    ), 'warning');
                }
    ...
    public function dbinsert()
    {
        ...
        if (parent::dbinsert()) {
            ...
            if (! empty($Blog)) { // Enable this item type only for selected Blog:
     ...
     * Update/Insert/Delete custom fields
     */
    public function dbsave_custom_fields()
    {
        global $DB;

        if (! empty($this->delete_custom_fields)) {	// Delete custom fields:
            $sql_data = [];
        ...
        if (! empty($this->insert_custom_fields)) {	// Insert new custom fields:
            $sql_data = [];
        ...
        if (! empty($this->update_custom_fields)) {	// Update custom fields:
            ...
            foreach ($this->update_custom_fields as $itcf_ID => $custom_field) {
                ...
                if (isset($old_custom_fields[$itcf_ID]) &&
                    $this->ID > 0 &&
                    $custom_field['name'] != $old_custom_fields[$itcf_ID]['name']) {	// Update item setting names of custom field to use new field name:
    ...
    {
        return in_array($this->usage, ['intro-front', 'intro-main', 'intro-cat', 'intro-tag', 'intro-sub', 'intro-all']);
    }
     ...
     */
    public static function get_default_ids()
    {
        global $DB;
     ...
     */
    public function get_custom_fields($type = 'all', $array_key = 'name')
    {
        if (! isset($this->custom_fields)) { // Initialize an array only first time
            if (empty($this->ID)) { // Set an empty array for new creating post type
                $this->custom_fields = [];
            } else { // Get the custom fields from DB
                global $DB;
            ...
            if ($type == 'all' || strpos($type, $custom_field['type']) !== false) {
                switch ($array_key) {
                    case 'name':
                        ...
                        if (empty($custom_field['name'])) { // Name can be empty when we are saving it with empty name and page is displayed with error messages
                            $field_index = $custom_field['ID'];
                        } else { // Get field index from name
     ...
     */
    public function get_applicable_post_status()
    {
        global $DB;
     ...
     */
    public function get_ignored_post_status()
    {
        global $DB;
     ...
     */
    public function update_item_statuses_from_Request()
    {
        global $DB;
     ...
     * @return boolean
     */
    public function is_enabled($coll_ID)
    {
        if (empty($this->ID)) {	// Item Type is not inserted in DB yet:
            return false;
        }

        if (! isset($this->enabled_colls)) {	// Load into cache where this Item Type is enabled for all collections:
            global $DB;
     ...
     */
    public function get_item_denomination($position = 'evobar_new', $default_denomination = null)
    {
        switch ($position) {
            case 'evobar_new':
