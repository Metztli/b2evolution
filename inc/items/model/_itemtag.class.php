...
class ItemTag extends DataObject
{
    ...
    public static function get_delete_cascades()
    {
        return [
            [
                ...
                'fk' => 'itag_tag_ID',
                'msg' => /* TRANS: cascade delete */ T_('%d tags from the posts'),
            ],
        ];
    }
    ...
    public function load_from_Request()
    {
        ...
        if ($existing_tag_ID = $this->dbexists('tag_name', $tag_name)) { // Other tag already exists with the same name:
            if (empty($this->ID)) { // Suggest to edit existing tag for new creating tag
                param_error('tag_name', sprintf(
                ...
                $this->merge_message = sprintf(
                    T_('The previously named "%s" tag (applied to %d posts) will be merged with the existing "%s" tag (already applied to %d posts). Are you sure?'),
                    $this->dget('name'),
                    $old_tag_posts,
                    $tag_name,
                    $new_tag_posts,
