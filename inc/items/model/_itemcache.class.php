...
class ItemCache extends DataObjectCache
{
     ...
     */
    public $urltitle_index = [];

    /**
     * Lazy filled map of items by category
     */
    public $items_by_cat_map = [];
     ...
     */
    public function __construct($objType = 'Item', $dbtablename = 'T_items__item', $dbprefix = 'post_', $dbIDname = 'post_ID')
    {
     ...
     * @return object Item
     */
    public function get_by_cat_ID($cat_ID, $order_alpha_func = 'compare_items_by_title')
    {
        $ChapterCache = &get_ChapterCache();
        $Chapter = $ChapterCache->get_by_ID($cat_ID);

        if (! isset($this->items_by_cat_map[$cat_ID])) { // Load items if not loaded yet
            $this->load_by_categories([$cat_ID], $Chapter->blog_ID);
        }
        ...
        if (! isset($this->items_by_cat_map[$cat_ID]['sorted'][$compare_method])) {	// Not sorted yet by requested method:
            if ($Chapter->get_subcat_ordering() == 'manual') {	// If manual sorting by order field:
                foreach ($this->items_by_cat_map[$cat_ID]['items'] as $i => $sorted_Item) {	// Set temp var in order to know what category order use to compare:
            ...
            usort($this->items_by_cat_map[$cat_ID]['sorted'][$compare_method], ['Item', $compare_method]);
        }
     ...
     */
    public function load_by_categories($cat_array, $coll_ID)
    {
        global $DB;

        if (empty($cat_array) && empty($coll_ID)) { // Nothing to load
            return false;
        }

        // In case of an empty cat_array param, use categoriesfrom the given collection
        if (empty($cat_array)) { // Get all categories from the given subset
            $ChapterCache = &get_ChapterCache();
        ...
        foreach ($cat_array as $cat_ID) {
            if (! isset($this->items_by_cat_map[$cat_ID])) { // This category is not loaded
                $not_loaded_cat_ids[] = $cat_ID;
                // Initialize items_by_cat_map for this cat_ID
                $this->items_by_cat_map[$cat_ID] = [
                    'items' => [],
                    'sorted' => false,
                ];
            }
        ...
        if (empty($not_loaded_cat_ids)) { // Requested categories items are all loaded
            return false;
        }
        ...
        $visibility_statuses = is_admin_page() ? get_visibility_statuses('keys', ['trash']) : get_inskin_statuses($coll_ID, 'post');
        ...
        foreach ($cat_posts as $row) { // Iterate through the post - cat pairs and fill the map
            if (empty($this->cache[$row['post_ID']])) { // The Item was not loaded because it does not correspond to the defined filters
                continue;
            }
     ...
     */
    public function load_by_IDs_or_slugs($IDs_slugs)
    {
        global $DB, $Debuglog;

        if (empty($IDs_slugs) || ! is_array($IDs_slugs)) {	// Wrong source data:
            return [];
        }
        ...
        $sql_where = [];
        if (! empty($IDs)) {	// Load Items by IDs:
            $sql_where[] = $this->dbIDname . ' IN ( ' . $DB->quote($IDs) . ' )';
        }
        if (! empty($slugs)) {	// Load Items by slugs:
        ...
        foreach ($item_rows as $Item) {
            ...
            if ($item_slug_title !== false &&
                ! isset($this->urltitle_index[$item_slug_title])) {	// Cache Item by slug:
     ...
     */
    public function &get_by_urltitle($req_urltitle, $halt_on_error = true, $halt_on_empty = true)
    {
        global $DB, $Debuglog;

        if (! isset($this->urltitle_index[$req_urltitle])) { // not yet in cache:
            // Get from SlugCache
            $SlugCache = &get_SlugCache();
            $req_Slug = $SlugCache->get_by_name($req_urltitle, $halt_on_error, $halt_on_empty);

            if ($req_Slug && $req_Slug->get('type') == 'item') {	// It is in SlugCache
     ...
     */
    public function load_urltitle_array($req_array)
    {
        global $DB, $Debuglog;
        ...
        foreach ($req_array as $urltitle) {
            if (! isset($this->urltitle_index[$urltitle])) { // not yet in cache:
                $SlugCache = &get_SlugCache();
                if ($req_Slug = $SlugCache->get_by_name($urltitle, false, false)) {
                    if ($req_Slug->get('type') == 'item') {	// Is item slug
