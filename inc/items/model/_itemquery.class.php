...
class ItemQuery extends SQL
{
    ...
    public $assignees_login;

    public $involved_user_IDs;

    public $involved_user_logins;

    public $statuses;

    public $types;

    public $itemtype_usage;
     ...
     */
    public function __construct($dbtablename, $dbprefix = '', $dbIDname)
    {
     ...
     */
    public function where_ID($p = '', $title = '')
    {
        $r = false;
        ...
        if (! empty($p)) {
            if (substr($this->p, 0, 1) == '-') {	// Starts with MINUS sign:
                $eq_p = ' <> ';
        ...
        if (! empty($title)) {
            global $DB;
            if (substr($this->title, 0, 1) == '-') {	// Starts with MINUS sign:
     ...
     */
    public function where_ID_list($pl = '')
    {
        $this->pl = clear_ids_list($pl);

        if (empty($this->pl)) {	// Nothing to filter:
            return;
        }

        if (substr($pl, 0, 1) == '-') {	// List starts with MINUS sign:
            $eq = 'NOT IN';
        } else {
            $eq = 'IN';
        }
     ...
     */
    public function where_chapter($blog, $cat = '', $catsel = [])
    {
        ...
        if (! empty($cat_array)) {	// We want to restict to some cats:
            global $DB;
            ...
            if ($cat_modifier == '*') { // We want the categories combined! (i-e posts must be in ALL requested cats)
     ...
     */
    public function where_chapter2(&$Blog, $cat_array, $cat_modifier, $cat_focus = 'wide', $coll_IDs = null)
    {
        // Save for future use (permission checks..)
        $this->blog = $Blog->ID;
        $this->Blog = $Blog;
        ...
        if (! empty($cat_array) && ($cat_focus == 'wide' || $cat_focus == 'extra')) {	// Select extra categories if we want filter by several categories:
        ...
        } elseif (get_allow_cross_posting() >= 1) {	// Select extra categories if cross posting is enabled:
            $this->FROM_add('INNER JOIN T_postcats ON ' . $this->dbIDname . ' = postcat_post_ID');
            $this->FROM_add('INNER JOIN T_categories ON postcat_cat_ID = cat_ID');
            $cat_ID_field = 'T_postcats.postcat_cat_ID';
        } else {	// Select only main categories:
            $this->FROM_add('INNER JOIN T_categories ON post_main_cat_ID = cat_ID');
            $cat_ID_field = 'post_main_cat_ID';
        }

        if (! empty($coll_IDs)) { // Force to aggregate the collection IDs from current param and not from blog setting
            $this->WHERE_and($Blog->get_sql_where_aggregate_coll_IDs('T_categories.cat_blog_ID', $coll_IDs));
        } elseif ($cat_focus == 'main') { // We are requesting a narrow search
            $this->WHERE_and('T_categories.cat_blog_ID = ' . $Blog->ID);
        } else { // Aggregate the collections IDs from blog setting
     ...
     */
    public function where_visibility($show_statuses, $aggregate_coll_IDs = null)
    {
        ...
        if (empty($aggregate_coll_IDs) && ! empty($this->blog)) { // Blog IDs are not defined, Use them depending on current collection setting
        ...
        $blog_IDs = [];
        if (empty($aggregate_coll_IDs) || $aggregate_coll_IDs == '-') { // Get status restriction only for current blog
        ...
        if ($aggregate_coll_IDs == '*') { // Get the status restrictions for all blogs
        ...
        } else { // Get the status restrictions for several blogs
            $blog_IDs = explode(',', $aggregate_coll_IDs);
        }

        $status_coll_clauses = [];
        foreach ($blog_IDs as $blog_ID) {	// Check status permission for each blog separately:
            $statuses_where_clause = statuses_where_clause($show_statuses, $this->dbprefix, $blog_ID, 'blog_post!', true, $this->author);
            if (! isset($status_coll_clauses[$statuses_where_clause])) {	// Initialize array item for each different status condition:
                $status_coll_clauses[$statuses_where_clause] = [];
            }
            // Group collections by same status condition:
            $status_coll_clauses[$statuses_where_clause][] = $blog_ID;
        }
        $status_restrictions = [];
        foreach ($status_coll_clauses as $status_coll_clause => $status_coll_IDs) {	// Initialize status permission restriction for each grouped condition that is formed above:
     ...
     */
    public function where_featured($featured = null)
    {
        $this->featured = $featured;

        if (is_null($this->featured)) { // no restriction
            return;
        } elseif (! empty($this->featured)) { // restrict to featured
            $this->WHERE_and($this->dbprefix . 'featured <> 0');
        } else { // restrict to NON featured
            $this->WHERE_and($this->dbprefix . 'featured = 0');
        }
    }
     ...
     */
    public function where_tags($tags, $tags_operator = 'OR')
    {
        global $DB;
        ...
        if ($tags_operator == 'AND') {	// Search items with ALL tags from the restriction list:
     ...
     */
    public function where_author($author_IDs)
    {
        ...
        if (substr($author_IDs, 0, 1) == '-') { // Exclude the users IF a list starts with MINUS sign:
            $eq = 'NOT IN';
        } else { // Include the users:
            $eq = 'IN';
        }
     ...
     */
    public function where_author_logins($author_logins)
    {
        ...
        if (substr($this->author_login, 0, 1) == '-') { // Exclude the users IF a list starts with MINUS sign:
            $eq = 'NOT IN';
     ...
     */
    public function where_assignees($assignees)
    {
        ...
        if ($assignees == '-') {	// List is ONLY a MINUS sign (we want only those not assigned)
        ...
        if (substr($assignees, 0, 1) == '-') {	// List starts with MINUS sign:
            $this->WHERE_and('( ' . $this->dbprefix . 'assigned_user_ID IS NULL
     ...
     */
    public function where_assignees_logins($assignees_logins)
    {
        ...
        if ($this->assignees_logins == '-') {	// List is ONLY a MINUS sign (we want only those not assigned)
            $this->WHERE_and($this->dbprefix . 'assigned_user_ID IS NULL');
        } elseif (substr($this->assignees_logins, 0, 1) == '-') {	// List starts with MINUS sign:
            $this->WHERE_and('( ' . $this->dbprefix . 'assigned_user_ID IS NULL
     ...
     */
    public function where_author_assignee($author_assignee)
    {
        global $DB;
     ...
     */
    public function where_involves($involved_user_IDs)
    {
        ...
        if (substr($involved_user_IDs, 0, 1) == '-') {	// Exclude the users IF a list starts with MINUS sign:
            $eq = 'NOT IN';
        } else {	// Include the users:
            $eq = 'IN';
        }
     ...
     */
    public function where_involves_logins($involved_user_logins)
    {
        ...
        if (substr($involved_user_logins, 0, 1) == '-') {	// Exclude the users IF a list starts with MINUS sign:
            $eq = 'NOT IN';
        ...
        if (! empty($involved_user_IDs)) {	// Filter only if correct users are found by logins:
     ...
     */
    public function where_locale($locale)
    {
        global $DB;
     ...
     */
    public function where_statuses($statuses)
    {
        ...
        if ($statuses == '-') {	// List is ONLY a MINUS sign (we want only those not assigned)
        ...
        if (substr($statuses, 0, 1) == '-') {	// List starts with MINUS sign:
            $this->WHERE_and('( ' . $this->dbprefix . 'pst_ID IS NULL
     ...
     */
    public function where_statuses_array($statuses)
    {
        if (! is_array($statuses)) {	// Wrong data:
            return;
        }

        $filter_without_status = false;
        foreach ($statuses as $s => $status_ID) {
            if ($status_ID == '-') {	// Filter by "No status":
            ...
            if (empty($status_ID)) {	// Remove a not number value from list:
                unset($statuses[$s]);
                continue;
            }
        ...
        $where_statuses = [];
        if ($filter_without_status) {	// Filter by "No status":
            $where_statuses[] = $this->dbprefix . 'pst_ID IS NULL';
        }
        if (! empty($statuses)) {	// Filter by specific statuses:
            $where_statuses[] = $this->dbprefix . 'pst_ID IN ( ' . implode(',', $statuses) . ' )';
        }

        if (! empty($where_statuses)) {	// Apply filter by statuses:
     ...
     */
    public function where_types($types)
    {
        ...
        if ($types == '-') {	// List is ONLY a MINUS sign (we want only those not assigned)
        ...
        if (substr($types, 0, 1) == '-') {	// List starts with MINUS sign:
            $this->WHERE_and('( ' . $this->dbprefix . 'ityp_ID IS NULL
     ...
     */
    public function where_itemtype_usage($itemtype_usage)
    {
        global $DB;
        ...
        if (strpos($itemtype_usage, '*featured*') !== false) {	// Get also featured posts:
        ...
        if ($itemtype_usage == '-') {	// List is ONLY a MINUS sign (we want only those not assigned)
            $this->WHERE_and($this->dbprefix . 'ityp_ID IS NULL' . $not_featured_sql_where);
        } elseif (substr($itemtype_usage, 0, 1) == '-') {	// List starts with MINUS sign:
     ...
     */
    public function where_datestart($m = '', $w = '', $dstart = '', $dstop = '', $timestamp_min = '', $timestamp_max = 'now')
    {
        ...
        if (! $start_is_set || ! $stop_is_set) {
            if (! is_null($w)  // Note: week # can be 0
                    && strlen($m) == 4) { // If a week number is specified (with a year)
                ...
                $start_is_set = true;
                $stop_is_set = true;
            } elseif (! empty($m)) {	// We want to restrict on an interval:
        ...
        if (! empty($timestamp_min)) { // Hide posts before
        ...
        if (! empty($timestamp_max)) { // Hide posts after
            // echo 'after';
     ...
     */
    public function where_datecreated($timestamp_created_max = 'now')
    {
        global $time_difference, $DB;

        if (! empty($timestamp_created_max)) { // Hide posts after
            // echo 'after';
     ...
     */
    public function where_keywords($keywords, $phrase, $exact, $keyword_scope = 'title,content')
    {
        global $DB;
        ...
        if (empty($keywords)) { // Nothing to search, Exit here:
            return;
        }
        ...
        if (empty($search_fields) && empty($search_sql)) {	// No correct fields to search, Exit here:
            return;
        }

        // Set sql operator depending on parameter:
        if (in_array(strtolower($phrase), ['or', '1', 'sentence'])) {
            $operator_sql = 'OR';
        } else {
            $operator_sql = 'AND';
        }

        if (! empty($search_fields)) {	// Do search if at least one field is requested:
            if ($exact) {	// We want exact match of each search field:
                $mask = '';
            } else {	// The words/sentence are/is to be included in the each search field:
                $mask = '%';
            }

            if ($phrase == '1' || $phrase == 'sentence') {	// Sentence search:
                ...
                $keywords = utf8_trim(str_replace([',', '"'], ' ', $keywords));
     ...
     */
    public function where_flagged($flagged = false)
    {
        global $current_User;

        $this->flagged = $flagged;

        if (! $this->flagged) {	// Don't restrict if it is not requested or if user is not logged in:
            return;
        }
     ...
     */
    public function where_locale_visibility()
    {
        if (is_admin_page()) {	// Don't restrict this in back-office:
            return;
        }
     ...
     */
    public function where_mustread($mustread = false)
    {
        global $current_User, $DB;

        $this->mustread = $mustread;

        if (! $this->mustread) {	// Don't restrict if it is not requested:
            return;
        }

        if ($mustread === 'read') {	// Get Items which are already read by current User:
            if (is_logged_in()) {	// For logged in user:
            ...
            } else {	// Decide all Items are not read by not logged in user,
                // because we store read status only for logged in users:
                $this->WHERE_and('FALSE');
                return;
            }
        } elseif ($mustread === 'unread') {	// Get Items which are NOT read by current User yet:
            if (is_logged_in()) {	// For logged in user:
     ...
     */
    public function where_renderers($renderers)
    {
        global $DB, $Plugins;

        if (empty($renderers)) {	// No filters:
            return;
        }

        foreach ($renderers as $r => $renderer) {	// Escape chars:
            $renderers[$r] = $DB->escape($renderer);
        }

        $sql_conditions = [];

        if (isset($Plugins, $this->Blog)) {	// Get default renderer plugins for current Collection:
            $default_renderers = $Plugins->validate_renderer_list(['default'], [
                'setting_name' => 'coll_apply_rendering',
                'Blog' => $this->Blog,
            ]);
            if (! empty($default_renderers)) {
                foreach ($renderers as $renderer) {
                    if (in_array($renderer, $default_renderers)) {	// If at least one default renderer plugin is used to filter then get items which still use default renderers:
     ...
     */
    public function gen_order_clause($order_by, $order_dir, $dbprefix, $dbIDname)
    {
        global $DB;

        if ($order_by == 'mustread') {	// Special ordering for "must read" items:
            $order_by = '';
            $order_dir = '';
        ...
        if (in_array('numviews', $orderby_array)) {	// Special case for numviews:
        ...
        foreach ($custom_sort_fields as $key => $field_name) {
            ...
            if (strpos($key, 'custom_double') === 0 || strpos($key, 'custom_computed') === 0) {	// Double and computed values should be compared as numbers and not like strings
                $field_value .= '+0';
            }
        ...
        if (in_array('order', $orderby_array)) {	// If list is ordered by field 'order':
            if (($order_i = array_search('order', $available_fields)) !== false) {	// Use an order per category instead of old field 'post_order':
        ...
        if (in_array('matched_tags_num', $orderby_array)) {	// Special selection to order by highest number of matched tags:
        ...
        if (strpos($this->itemtype_usage, '*featured*') !== false) {	// If we get featured posts together with other post types(like intro) then we should order featured posts below not featured posts:
     ...
     */
    public function get_orderby_select($prefix = ', ')
    {
     ...
     */
    public function get_orderby_from($prefix = '')
    {
