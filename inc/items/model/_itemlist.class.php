...
class ItemList2 extends ItemListLight
{
     ...
     */
    public function preview_from_request()
    {
        if (! is_logged_in()) { // dh> only logged in user's can preview. Alternatively we need those checks where $current_User gets used below.
            return;
        }
        ...
        if (! ($Item = &$ItemCache->get_by_ID($post_ID, false, false))) {	// Initialize new creating Item:
            $Item = new Item();
        }
        ...
        if ($urltitles === '') {	// If slugs are empty on preview form, try to get previous of from title:
        ...
        if (isset($Item->previous_status)) {	// Restrict Item status by Collection access restriction AND by CURRENT USER write perm:
        ...
        $Plugins->trigger_event('AppendItemPreviewTransact', [
            'Item' => &$Item,
        ]);

        // little funky fix for IEwin, rawk on that code
        global $Hit;
        if (($Hit->is_winIE()) && (! isset($IEWin_bookmarklet_fix))) { // QUESTION: Is this still needed? What about $IEWin_bookmarklet_fix? (blueyed)
    ...
    public function run_query(
        ...
        $select_temp_order = '';
        if (! empty($this->ItemQuery->order_by) && preg_match('/' . preg_quote('postcatsorders.postcat_order') . ' (DESC|ASC)/i', $this->ItemQuery->order_by, $order_dir_match)) {	// Move the items with NULL order to the end of the list
        ...
        if (isset($this->filters['orderby']) && $this->filters['orderby'] == 'numviews') { // special case for order by number of views
     ...
     */
    public function prevnext_item_links($params)
    {
        $params = array_merge([
        ...
        if ($current_Item) { // current Item is available, init navigation target
        ...
        if (empty($prev) || empty($next)) {	// Use separator text only when prev & next are not empty
            $params['separator'] = '';
        }
        ...
        if (! empty($output)) {	// we have some output, lets wrap it
     ...
     * Skip to previous
     */
    public function prev_item_link($before = '', $after = '', $text = '&laquo; $title$', $no_item = '', $display = true, $target_blog = '', $class = '', $itemtype_usage = '', $featured = null, $post_navigation = null)
    {
        /**
         * @var Item
         */
     ...
     * Skip to next
     */
    public function next_item_link($before = '', $after = '', $text = '$title$ &raquo;', $no_item = '', $display = true, $target_blog = '', $class = '', $itemtype_usage = '', $featured = true, $post_navigation = null)
    {
        /**
         * @var Item
         */
     ...
     */
    public function get_prev_item_url($permalink_type = '', $blogurl = '', $glue = '&amp;')
    {
        /**
         * @var Item
         */
     ...
     * @param string glue between url params
     */
    public function get_next_item_url($permalink_type = '', $blogurl = '', $glue = '&amp;')
    {
        /**
         * @var Item
         */
     ...
     */
    public function &get_prevnext_Item($direction = 'next', $itemtype_usage = '', $featured = null, $post_navigation = 'same_blog')
    {
        global $DB, $ItemCache;

        if (! $this->single_post) {	// We are not on a single post:
            $r = null;
            return $r;
        }
        ...
        if (is_null($current_Item)) {	// This happens if we are on a single post that we do not actually have permission to view
            $r = null;
            return $r;
        }

        if ($current_Item->get_type_setting('usage') != 'post') {	// We are not on a REGULAR post -- we cannot navigate:
            $r = null;
            return $r;
        }
        ...
        foreach ($orderdir as $index => $order) { // Set the corresponding order by direction depending the original value and the next/prev request
        ...
        foreach ($orderby_fields as $orderby) {
            if (strpos($orderby, 'CASE WHEN') !== false) { // This handles those cases when the order fields may be null values and we want all null values at the end of the list
                continue;
            }
            if (strpos($orderby, 'DESC')) { // Descending order
                $operator = ' < ';
            ...
            if (strpos($compare_field, 'postcatsorders.postcat_order') !== false) {	// This is an order field per category:
                $post_field_name = 'postcat_order';
            } elseif (($table_separator = strpos($compare_field, '.'))) { // This is a custom field from the item settings table
            ...
            } elseif ($compare_field == 'RAND()') { // Random order
                $post_field_name = 'RAND';
            } else { // Normal post field ( not custom and not special field RAND )
            ...
            if ($condition_separator !== '') { // Concatenate the conditions
                $equal_condition .= ' AND ';
            ...
            switch ($post_field_name) {
                ...
                case 'postcat_order':
                    ...
                    $order_cat_ID = (isset($this->filters['cat_array']) && count($this->filters['cat_array']) == 1) ? $this->filters['cat_array'][0] : null;
                    $order_field_value = $current_Item->get_order($order_cat_ID);
                    if ($order_field_value === null) {	// NULL is ordered in the end:
                ...
                default:
                    if (strpos($post_field_name, 'custom_') === 0) {
                        ...
                        if (($custom_field_value === false) || ($custom_field_value === null)) { // This custom field is not set for the current Item
            ...
            if ($condition_separator == '') { // The first where condition was set, we need to set an 'OR' separator in case if we will have further conditions
                $condition_separator = ' OR ';
            } else { // More than one WHERE condition was added, the last one always needs a ')'
                $where_condition .= ' )';
            }
        }
    ...
    public function load_list_data($params = [])
    {
        ...
        if (empty($page_post_ids)) {	// There are no items on this list:
            return;
        }
        ...
        if ($params['load_user_data']) {	// Load the user data for items:
            $this->load_user_data_for_items();
        }

        if ($params['load_postcats']) {	// Load category associations for the items of current page:
     ...
     */
    public function load_content_read_statuses()
    {
        $this->load_user_data_for_items();
    }
     ...
     * Load user data (posts/comments read statuses) for current User for each post of the current ItemList page
     */
    public function load_user_data_for_items()
    {
        if (! $this->Blog->get_setting('track_unread_content')) { // tracking unread content in this blog is turned off
            return;
        }
