...
function init_MainList($items_nb_limit)
{
    ...
    if (! $Plugins->trigger_event_first_true('InitMainList', [
        'MainList' => &$MainList,
        'limit' => $items_nb_limit,
    ])) {
        ...
        if (! $preview) {
            switch ($disp) {
                ...
                case 'posts':
                    if ($Blog->get_setting('show_post_types') != '') {	// Exclude items with types which are hidden by collection setting "Show post types":
                        $MainList->set_default_filters([
            ...
            if ($disp == 'posts' && ! empty($Skin) && $Skin->get_template('cat_array_mode') == 'parent') {	// Get items ONLY from current category WITHOUT sub-categories:
                global $cat;
                // Get ID of single selected category:
                $single_cat_ID = intval($cat);

                if ($single_cat_ID) {	// Do limit if single category is selected:
                    $MainList->set_filters([
                        'cat_array' => [$single_cat_ID],
                        'cat_modifier' => null,
                    ], true, true);
                }
            }
        ...
        } else {	// We want to preview a single post, we are going to fake a lot of things...
...
function init_inskin_editing()
{
    ...
    if ($disp == 'edit' && ! $Blog->get_setting('in_skin_editing')) {	// Redirect to the Back-office editing (setting is OFF)
        header_redirect($admin_url . '?ctrl=items&action=' . ($post_ID == 0 ? ($copy_post_ID == 0 ? 'new' : 'copy&p=' . $copy_post_ID) : 'edit&p=' . $post_ID) . '&blog=' . $Blog->ID);
    } elseif ($disp == 'proposechange' && ! $Blog->get_setting('in_skin_change_proposal')) {	// Redirect to the Back-office editing (setting is OFF)
        header_redirect($admin_url . '?ctrl=items&action=propose&blog=' . $Blog->ID . '&p=' . $post_ID);
    }

    $tab_switch_params = 'blog=' . $Blog->ID;

    if ($disp == 'proposechange') {	// Propose a change:
        ...
        if ($last_proposed_Revision = $edited_Item->get_revision('last_proposed')) {	// Suggest item fields values from last proposed change when user creates new propose change:
            $edited_Item->set('revision', 'p' . $last_proposed_Revision->iver_ID);
        }
    } elseif ($post_ID > 0) {	// Edit post
        ...
        if (! $edited_Item->check_proposed_change_restriction('warning') &&
            ($last_proposed_Revision = $edited_Item->get_revision('last_proposed'))) {	// Use item fields values from last proposed change:
            $edited_Item->set('revision', 'p' . $last_proposed_Revision->iver_ID);
        }
        ...
        if ($action == 'edit_item_type') {	// On change Item Type we should load all data from request
            $edited_Item->load_from_Request( /* editing? */ true, /* creating? */ false);
        }

        // Set action for edit mode:
        $action = 'edit';
    } elseif ($copy_post_ID > 0) {	// Copy post
        global $localtimenow;
        $action = 'new';
    ...
    } elseif (empty($action) || $action == 'new_item_type') {	// Create new post (from Front-office)
        $action = 'new';
    ...
    if ($disp != 'proposechange') {	// Restrict Item status by Collection access restriction AND by CURRENT USER write perm:Restrict item status to max allowed by item collection:
    ...
    if ($item_content === null) {	// Use text template for new creating Item:
        $item_content = $edited_Item->get_type_setting('text_template');
    }
    $item_content = prepare_item_content($item_content);

    if (! $edited_Item->get_type_setting('allow_html')) { // HTML is disallowed for this post, content is encoded in DB and we need to decode it for editing:
...
function &get_featured_Item($restrict_disp = 'posts', $coll_IDs = null, $preview = false, $load_featured = false, $load_intro = true)
{
    ...
    if ($disp != $restrict_disp || ! isset($MainList)) {	// If we're not currently displaying posts, no need to try & display a featured/intro post on top!
        $Item = null;
        return $Item;
    }

    // Convert to boolean because settings may have '0' and '1' values instead:
    $load_featured = (bool) $load_featured;

    // Check clear disp details without suffix -intro and -nointro:
    $check_clear_disp_detail = preg_replace('#(-intro|-nointro)$#', '', $disp_detail);

    if ($featured_list_type !== $load_featured || $featured_disp_detail !== $check_clear_disp_detail) {	// Reset a featured list if previous request was to load another type:
        $FeaturedList = null;
    }
    ...
    if (! isset($FeaturedList)) {	// Don't repeat if we've done this already -- Initialize the featured list only first time this function is called in a skin:
        ...
        if (! empty($cat)) {	// Get a featured post only of the selected category and don't touch the posts of the child categories:
            $featured_list_filters['cat_array'] = [$cat];
        }
        ...
        if ($load_intro) {
            if (! $MainList->is_filtered()) {	// This is not a filtered page, so we are on the home page.
                if ($restrict_disp == 'front') {	// Special Front page:
                    // Use Intro-Front posts
                    $restrict_to_types_usage = 'intro-front';
                } else {	// Default front page displaying posts:
            ...
            } else {	// We are on a filtered... it means a category page or sth like this...
                // echo $disp_detail;
    ...
    if ($preview) { // We only want want a preview of the first item
        $Item = $FeaturedList->get_by_idx(0);
    } else {
        $Item = $FeaturedList->get_item();

        if ($Item) {	// Memorize that ID so that it can later be filtered out of normal display:
...
{
    return (bool) get_featured_Item($restrict_disp, $coll_IDs, true, $check_featured);
}
...
function &get_current_Item()
{
    ...
    if (in_array($disp, ['single', 'page', 'widget_page']) &&
        isset($Item) && $Item instanceof Item) {	// Use current global Item:
        $current_Item = $Item;
    }

    if (in_array($disp_detail, ['posts-cat', 'posts-topcat-intro', 'posts-subcat-intro'])) {	// Try to get intro Item:
...
function get_urltitle($title, $locale = null)
{
    ...
    if ($slug_changed == 0) { // this should only happen when the slug is auto generated
        global $Collection, $Blog;
        if (isset($Blog)) {	// Get max length of slug from current blog setting:
            $count_of_words = $Blog->get_setting('slug_limit');
        }
        if (empty($count_of_words)) {	// Use 8 words to limit slug by default:
            $count_of_words = 8;
        }
    ...
    $urltitle = $urlbase;
    if (isset($matches[4]) && $matches[4] !== '') {	// Append only NOT empty string after last dash:
        $urltitle .= '-' . $matches[4];
    }
 ...
 */
function urltitle_validate(
    $urltitle,
    $title,
    $post_ID = 0,
    $query_only = false,
    $dbSlugFieldName = 'post_urltitle',
    $dbIDname = 'post_ID',
    ...
    if (! $query_only) {
        ...
        foreach ($DB->get_results($sql, ARRAY_A) as $row) {
            ...
            if ($existing_urltitle == $urltitle && $row[$dbIDname] != $post_ID) { // We have an exact match, we'll have to change the number.
                $exact_match = true;
            }
            if (preg_match('/-([0-9]+)$/', $existing_urltitle, $matches)) { // This one has a number, we extract it:
                $existing_number = (int) $matches[1];

                if (! isset($use_existing_number) && $row[$dbIDname] == $post_ID) { // if there is a numbered entry for the current ID, use this:
                    $use_existing_number = $existing_number;
                }

                if ($existing_number > $highest_number) { // This is the new high
        ...
        if ($exact_match && ! $query_only) { // We got an exact (existing) match, we need to change the number:
...
function get_postdata($postid)
{
    ...
    if (! empty($postdata) && $postdata['ID'] == $postid) { // We are asking for postdata of current post in memory! (we're in the b2 loop)
...
function bpost_count_words($str)
{
    ...
    foreach (preg_split(
        '#\s+#',
        convert_charset($str, 'UTF-8', $evo_charset),
        -1,
        PREG_SPLIT_NO_EMPTY
    ) as $word) {
    ...
    foreach ($statuses as $key => $status) {
        switch ($status) {
            ...
            case 'private': // It is allowed for users who has global 'editall' permission
                ...
                if (! $allowed && $dbprefix == 'comment_') {	// Allow the private comments for collection owner:
                    $allowed = (is_logged_in(false) && $current_User->check_perm_blogowner($req_blog));
                }
                if (! $allowed && check_user_perm($perm_prefix . 'private', 'create', false, $req_blog, false)) { // Own private posts/comments are allowed if user can create private posts/comments
            ...
            case 'review': // It is allowed for users who have permission to create comments with 'review' status and have at least 'lt' posts/comments edit perm
                ...
                if (! $allowed && check_user_perm($perm_prefix . 'review', 'create', false, $req_blog, false)) { // Own posts/comments with 'review' status are allowed if user can create posts/comments with 'review' status
            ...
            case 'draft': // In back-office it is always allowed for users who may create posts/commetns with 'draft' status
                ...
                if (! $allowed && check_user_perm($perm_prefix . 'draft', 'create', false, $req_blog, false)) { // In front-office only authors may see their own draft posts/comments, but only if the have permission to create draft posts/comments
        ...
        if ($allowed) { // All posts/comments with this status can be displayed in the request blog for the current User ( or for anonymous )
            $allowed_statuses[] = $status;
        }
    }

    if (count($allowed_statuses)) { // add allowed statuses condition
...
function statuses_where_clause($show_statuses = null, $dbprefix = 'post_', $req_blog = null, $perm_prefix = 'blog_post!', $filter_by_perm = true, $author_filter = null)
{
    ...
    if (is_null($req_blog)) { // try to init request blog if it was not set
        global $blog;
        $req_blog = $blog;
    }

    if (empty($show_statuses)) { // use in-skin statuses if show_statuses is empty
    ...
    if ($req_blog && $is_post_query) { // If requested blog is set, then set additional "where" clauses from modules method, before we would manipulate the $show_statuses array
        $modules_condition = modules_call_method('get_item_statuses_where_clause', [
            'blog_ID' => $req_blog,
            'statuses' => $show_statuses,
        ]);
        if (! empty($modules_condition)) {
            foreach ($modules_condition as $condition) {
                if (! empty($condition)) { // condition is not empty
                    $where[] = $condition;
                }
            }
        }
    ...
    if (is_logged_in(false) && $filter_by_perm) { // User is logged in and the account was activated
        if (check_user_perm('blogs', 'editall', false)) { // User has permission to all blogs posts and comments, we don't have to check blog specific permissions.
            $allowed_statuses_cond = get_allowed_statuses_condition($show_statuses, $dbprefix, null, $perm_prefix);
            if (! empty($allowed_statuses_cond)) { // condition is not empty
                $where[] = $allowed_statuses_cond;
            }
            $filter_by_perm = false;
            $show_statuses = null;
        } elseif (! empty($author_filter) && ($author_filter != $current_User->ID)) { // Author filter is set, but current_User is not the filtered user, then these statuses are not visible for sure
            $show_statuses = array_diff($show_statuses, ['private', 'draft']);
        }
    }

    if (($req_blog == 0) && $filter_by_perm) { // This is a very special case when we must check visibility statuses from each blog separately
        // Note: This case should not be called frequently, it may be really slow!!
        $where_condition = '';
        $condition = '';
        if (in_array('published', $show_statuses)) { // 'published' status is always allowed in case of all blogs, handle this condition separately
            $where_condition = '( ' . $dbprefix . 'status = "published" )';
            $condition = ' OR ';
        }
        if (! is_logged_in(false)) { // When user is not logged in only the 'published' status is allowed, don't check more
            return $where_condition;
        }
        if (in_array('community', $show_statuses)) { // 'community' status is always allowed in case of all blogs when a user is logged in, handle this condition separately
        ...
        $show_statuses = array_diff($show_statuses, ['published', 'community']);
        if (empty($show_statuses)) { // return if there are no other status
            return $where_condition;
        }
        // Select each blog
        $blog_ids = $DB->get_col('SELECT blog_ID FROM T_blogs', 0, 'Get IDs of all collections');
        $sub_condition = '';
        foreach ($blog_ids as $blog_id) { // create statuses where clause condition for each blog separately
            $status_perm = statuses_where_clause($show_statuses, $dbprefix, $blog_id, $perm_prefix, $filter_by_perm, $author_filter);
            if ($status_perm) { // User has permission to view some statuses on this blog
                $sub_condition .= '( ( cat_blog_ID = ' . $blog_id . ' ) AND' . $status_perm . ' ) OR ';
            }
        }
        if ($dbprefix == 'post_') { // Item query condition
            $from_table = 'FROM T_items__item';
            $reference_column = 'post_ID';
        } else { // Comment query condition
        ...
        if ($object_ids) { // If thre is at least one post or comment
            $where_condition .= $condition . '( ' . $dbprefix . 'ID IN (' . $object_ids . ') )';
        }
        return $where_condition;
    }

    if ($filter_by_perm) { // filter allowed statuses by the current User perms and by the current page ( front or back office )
        $allowed_statuses_cond = get_allowed_statuses_condition($show_statuses, $dbprefix, $req_blog, $perm_prefix);
        if (! empty($allowed_statuses_cond)) { // condition is not empty
            $where[] = $allowed_statuses_cond;
        }
    } elseif (is_array($show_statuses) && count($show_statuses)) { // we are not filtering so all status are allowed, add allowed statuses condition
...
function recreate_autogenerated_excerpts($continue_url, $remove_all = true, $detailed_progress_log = false)
{
    ...
    if ($remove_all) { // We are in the beginning of the process and first we set all autogenerated excerpts values to NULL
        if ($detailed_progress_log) { // Display detailed progess information
    ...
    } else { // Init params with a previously started process status
    ...
    $ItemCache = &get_ItemCache();
    while (count($ItemCache->load_by_sql($load_SQL)) > 0) { // New portion of items was loaded
        $processed_items = 0;
        while (($iterator_Item = &$ItemCache->get_next()) != null) { // Create new autogenerated excerpt and save it in the database
            ...
            $processed_items++;
            if ($detailed_progress_log && (($processed_items % 3) == 0)) { // Update progress info after every 3 recreated excerpt when detailed progress log was requested
        ...
        if (($max_exec_time != 0) && ($recreated_excerpts < $all_excerpt)) { // a max execution time limit is set and there are more excerpts to create
            $elapsed_time = time() - $start_time;
            if ($elapsed_time > ($max_exec_time - 10)) { // Increase the time limit in case if less than 10 seconds remained
    ...
    if ($detailed_progress_log && ($recreated_excerpts < $all_excerpt)) { // This means that we are in the end of the process but some post excerpt was updated outside of this process
 ...
 * @param array Params
 */
function cat_select($Form, $form_fields = true, $show_title_links = true, $params = [])
{
    global $blog, $Blog, $current_blog_ID, $edited_Item, $cat_select_form_fields;
    global $admin_url, $rsc_url;

    if (get_post_cat_setting($blog) < 1) { // No categories for $blog
        return;
    }
    ...
    if (empty($edited_Item->ID) && ! empty($cat)) {	// If the GET param 'cat' is defined we should preselect the category for new created post
        global $post_extracats;
        $post_extracats = [$cat];
        $edited_Item->main_cat_ID = $cat;
    }

    if ($show_title_links) { // Use in Back-office
        $fieldset_title = $params['categories_name'] . get_manual_link('post-categories-panel')
            . action_icon(T_('Categories'), 'edit', $admin_url . '?ctrl=chapters&amp;blog=' . $blog, T_('Categories'), 3, 4, [
                'class' => 'action_icon pull-right',
            ]);
    } else {
    ...
    if (get_post_cat_setting($blog) == 3) { // Main + Extra cats option is set, display header
    ...
    $cat_display_params = array_merge($params, [
        'total_count' => 0,
    ]);

    if (check_user_perm('blog_admin', '', false, $blog) &&
        (get_allow_cross_posting() >= 2 ||
      (isset($blog) && get_post_cat_setting($blog) > 1 && get_allow_cross_posting() == 1))) {	// If collection cross posting is enabled, go through collections where current Item Type is enabled or current Item already uses categories of those collections:
        $BlogCache = &get_BlogCache();
        $BlogCache->clear();
        ...
        foreach ($BlogCache->cache as $l_Blog) {	// Run recursively through the categories of the detected collections:
            if (! check_user_perm('blog_post_statuses', 'edit', false, $l_Blog->ID) ||
                ! check_user_perm('blog_admin', '', false, $l_Blog->ID)) {	// Skip collection if current user has no appropriate permissions:
                continue;
            }

            if (! blog_has_cats($l_Blog->ID)) {	// Skip collection without categories:
                continue;
            }
            ...
            $r .= get_fieldset_folding_icon($l_Blog->get('urlname'), [
    ...
    } else { // BLOG Cross posting is disabled. Current blog only:
 ...
 * @param array Params
 */
function cat_select_header($params = [])
{
    global $Blog;
    ...
    if ($params['display_main']) {	// Radio option for main category:
    ...
    if ($params['display_extra']) {	// Checkbox for extra category:
    ...
    if ($params['display_order'] &&
        (is_admin_page() || $Blog->get_setting('in_skin_editing_category_order'))) {	// Item order per category:
 ...
 */
function cat_select_display($Chapter, $callbacks, &$params = [])
{
    $params = array_merge([
        'level' => 1,
        'sorted' => true,
    ...
    if (is_array($callbacks['before_each'])) { // object callback:
    ...
    if (is_array($callbacks['after_each'])) { // object callback:
    ...
    if (is_array($callbacks['before_first'])) { // object callback:
    ...
    if (is_array($callbacks['after_last'])) { // object callback:
 ...
 */
function cat_select_before_each($cat_ID, $level, $total_count, $params = [])
{ // callback to display sublist element
    ...
    if ($thisChapter->lock && ! check_user_perm('blog_cats', '', false, $current_blog_ID)) {	// This chapter is locked and current user has no permission to edit the categories of this blog
        return;
    }
    ...
    // RADIO for main cat:
    if ($params['display_main'] && get_post_cat_setting($blog) != 2) { // if no "Multiple categories per post" option is set display radio
        if (! $thisChapter->meta
            && ! (! is_admin_page() && ($thisChapter->get_ItemType() === false) && $edited_Item->ID === 0)
            && (($current_blog_ID == $blog) || (get_allow_cross_posting($blog) >= 2))) { // This is current blog or we allow moving posts accross blogs
            if ($cat_select_form_fields) {	// We want a form field:
                ...
                if ($cat_ID == $edited_Item->main_cat_ID) { // main cat of the Item or set as default main cat above
                    $r .= ' checked="checked"';
                }
        ...
        } else { // Don't allow to select this cat as a main cat
            $r .= '<td class="selector catsel_main">&nbsp;</td>';
        }
    }

    // CHECKBOX:
    if ($params['display_extra'] && get_post_cat_setting($blog) >= 2) { // We allow multiple categories or main + extra cat,  display checkbox:
        if (! $thisChapter->meta
            && ! (! is_admin_page() && ($thisChapter->get_ItemType() === false) && $edited_Item->ID === 0)
            && (($current_blog_ID == $blog) || (get_allow_cross_posting($blog) % 2 == 1)
                || ((get_allow_cross_posting($blog) == 2) && (get_post_cat_setting($blog) == 2)))) { // This is the current blog or we allow cross posting (select extra cat from another blog)
            if ($cat_select_form_fields) {	// We want a form field:
                ...
                if (in_array($cat_ID, $post_extracats)) { // This category was selected
                    $r .= ' checked="checked"';
                }
        ...
        } else { // Don't allow to select this cat as an extra cat
            $r .= '<td class="selector catsel_main">&nbsp;</td>';
        }
    }
    ...
    if ($thisChapter->meta) {	// Change style of meta category
        $additional_style .= 'font-weight:bold;';
    }

    $chapter_lock_status = '';
    if ($thisChapter->lock) {	// Add icon for locked category
        $chapter_lock_status = '<span style="padding-left:5px">' . get_icon('file_not_allowed', 'imgtag', [
            'title' => T_('Locked'),
        ]) . '</span>';
    }
    ...
    if ($params['display_order'] &&
        (is_admin_page() || $Blog->get_setting('in_skin_editing_category_order'))) {	// Display item order per category only on back-office or when it is enabled for front-office:
...
function cat_select_new(&$cat_display_params)
{
    ...
    if (! $cat_display_params['display_new']) {	// Don't display an input to create new category:
        return '';
    }

    if (! check_user_perm('blog_cats', '', false, $blog)) {	// Current user cannot add/edit a categories for this blog
        return '';
    }
    ...
    if ($cat_display_params['display_order'] &&
        (is_admin_page() || $Blog->get_setting('in_skin_editing_category_order'))) {	// INPUT TEXT for item order in new category:
...
function attach_browse_tabs($display_tabs3 = true)
{
    ...
    if (empty($Blog)) { // No blog
        return;
    }

    $menu_entries = [];

    if ($Blog->get_setting('use_workflow') && check_user_perm('blog_can_be_assignee', 'edit', false, $Blog->ID)) { // We want to use workflow properties for this blog:
        $menu_entries['tracker'] = [
            'text' => T_('Workflow view'),
            'href' => $admin_url . '?ctrl=items&amp;tab=tracker&amp;filter=restore&amp;blog=' . $Blog->ID,
        ];
    }

    if ($Blog->get('type') == 'manual') { // Display this tab only for manual blogs
        $menu_entries['manual'] = [
            'text' => T_('Manual view'),
    ...
    $AdminUI->add_menu_entries(['collections', 'posts'], $menu_entries);
    ...
    */
    ...
    if ($display_tabs3) {
        if (check_user_perm('blog_comments', 'view', false, $Blog->ID)) {	// User has permission to edit published, draft or deprecated comments (at least one kind)
            $AdminUI->add_menu_entries(['collections', 'comments'], [
                'fullview' => [
        ...
        if (check_user_perm('meta_comment', 'view', false, $Blog->ID)) {	// Initialize menu entry for Internal comments if current user has a permission:
            $AdminUI->add_menu_entries(['collections', 'comments'], [
                'meta' => [
...
function get_item_type_tabs()
{
    ...
    if (empty($Blog)) { // Don't get the item types if Blog is not defined
        return [];
    }
    ...
    foreach ($type_usages as $type_usage) {
        if ($type_tab = get_tab_by_item_type_usage($type_usage)) {	// Only if tab exists for current item type usage:
...
function get_tab_by_item_type_usage($type_usage)
{
    switch ($type_usage) {
        case 'post':
            $type_tab = ['post', NT_('Posts')];
            break;
        case 'page':
        case 'widget-page':
            $type_tab = ['page', NT_('Pages')];
            break;
        case 'special':
            $type_tab = ['special', NT_('Special')];
            break;
        case 'intro-front':
        case 'intro-main':
        case 'intro-cat':
        ...
        case 'intro-sub':
        case 'intro-all':
            $type_tab = ['intro', NT_('Intros')];
            break;
        case 'content-block':
            $type_tab = ['content-block', NT_('Content Blocks')];
            break;
...
function get_item_type_usage_by_tab($tab_name)
{
    switch ($tab_name) {
        case 'page':
            $type_usages = ['page', 'widget-page'];
            break;
        case 'special':
            $type_usages = ['special'];
            break;
        case 'intro':
            $type_usages = ['intro-front', 'intro-main', 'intro-cat', 'intro-tag', 'intro-sub', 'intro-all'];
            break;
        case 'content-block':
            $type_usages = ['content-block'];
            break;
        case 'post':
        default:
            $type_usages = ['post'];
 ...
 */
function visibility_select(&$Form, $post_status, $mass_create = false, $labels = [], $field_label = '')
{
    $labels = array_merge(get_visibility_statuses('notes-array'), $labels);

    global $Collection, $Blog;

    $mass_create_statuses = ['redirected'];
    ...
    foreach ($labels as $status => $label) {
        if (check_user_perm('blog_post!' . $status, 'create', false, $Blog->ID) &&
            ...
            $sharing_options[] = [$status, $label[0] . ' <span class="notes">' . $label[1] . '</span>'];
        }
    }

    if (count($sharing_options) == 1) { // Only one status is available, don't show visibility select
 ...
 * sites.
 */
function item_link_by_urltitle($params = [])
{
...
function load_publish_status($creating = false)
{
    ...
    if (! in_array($publish_status, ['published', 'community', 'protected', 'private'])) { // Publish with the given status doesn't exists "published"
        if ($creating) {
...
function echo_publish_buttons($Form, $creating, $edited_Item, $inskin = false, $display_preview = false)
{
    ...
    // ---------- VISIBILITY ----------
    if (! $inskin) { // Only for back-office
        global $AdminUI;
        ...
        $exclude_statuses = array_merge(get_restricted_statuses($Blog->ID, 'blog_post!', 'create', $edited_Item->get('status'), '', $edited_Item), ['trash']);
        // Get allowed visibility statuses
        $status_options = get_visibility_statuses('', $exclude_statuses);

        if (isset($AdminUI, $AdminUI->skin_name) && $AdminUI->skin_name == 'bootstrap') { // Use dropdown for bootstrap skin
            ...
            echo '</ul>';
            echo '</div>';
        } else { // Use standard select element for other skins
    ...
    // ---------- SAVE ----------
    $save_hotkeys = ['ctrl+enter', 'command+enter'];
    $next_action = ($creating ? 'create' : 'update');
    if (! $inskin && check_user_perm('item_post!CURSTATUS', 'edit', false, $edited_Item)) { // Show Save & Edit only on admin mode
        $Form->submit([
            'actionArray[' . $next_action . '_edit]', /* TRANS: This is the value of an input submit button */ T_('Save & edit'),
            'SaveEditButton btn-status-' . $edited_Item->get('status'),
    ...
    if ($inskin) { // Front-office: display a save button with title depending on post status
    ...
    } else { // Use static button title on back-office
        $button_title = T_('Save');
    }
    $Form->submit([
        'actionArray[' . $next_action . ']',
        $button_title,
        'SaveButton btn-status-' . $edited_Item->get('status'),
        'data-shortcut' => implode(',', $save_hotkeys),
    ]);
    ...
    if ($highest_publish_status == 'published' && $UserSettings->get_collection_setting('show_quick_publish', $Blog->ID)) { // Display this button to make a post published
...
function get_item_status_buttons($edited_Item, $button_action = null, $button_class = '')
{
    ...
    if ($edited_Item !== null) {	// If the edited Item is defined, e-g on edit form:
        $item_status = $edited_Item->status;
    } else {	// If item is not defined, e-g on action for several items from list:
    ...
    // Get those statuses which are not allowed for the current User to create posts in this blog
    $exclude_statuses = array_merge(get_restricted_statuses($Blog->ID, 'blog_post!', 'create', $item_status, '', $edited_Item), ['trash']);
    // Get allowed visibility statuses:
    ...
    if (empty($status_options)) {	// If current User has no permission to edit to any status:
        return '';
    }
 ...
 */
function echo_item_type_change_buttons($edited_Item, $params = [])
{
    $params = array_merge([
    ...
    if (! ($item_Blog = &$edited_Item->get_Blog())) {	// Collection is required to dispplay buttons:
        return;
    }
    ...
    if (count($item_types) < 2) {	// No reason to display single button with current Item Type:
        return;
    }
...
function echo_item_selector_js()
{
    ...
    if (! empty($evo_item_selector_js_initialized)) {	// Don't initialize this JS code twice on same page:
        return;
    }
...
function echo_item_merge_js()
{
    ...
    if (! empty($evo_item_merge_js_initialized)) {	// Don't initialize this JS code twice on same page:
        return;
    }
...
function echo_item_add_version_js()
{
    ...
    if (! empty($evo_item_add_version_js_initialized)) {	// Don't initialize this JS code twice on same page:
        return;
    }
...
function echo_item_link_version_js()
{
    ...
    if (! empty($evo_item_link_version_js_initialized)) {	// Don't initialize this JS code twice on same page:
        return;
    }
...
function echo_item_mass_change_cat_js()
{
    ...
    if (! empty($evo_item_mass_change_cat_js_initialized)) {	// Don't initialize this JS code twice on same page:
        return;
    }
 ...
 * @param array Params
 */
function echo_autocomplete_tags($params = [])
{
    $params = array_merge([
        'input_ID' => 'item_tags',
        'item_ID' => null,
...
function check_perm_posttype($item_typ_ID, $post_extracats, $assert_post_type = true, $assert_permission = true)
{
    ...
    if (! $Blog->is_item_type_enabled($ItemType->ID)) { // Don't allow to use a not enabled post type:
        if ($assert_post_type) {
...
function &create_multiple_posts(&$Item, $linebreak = false)
{
    ...
    foreach (explode("\n", $Item->content . "\n") as $line) {
        ...
        if ($current_title === '' && $line !== '') {	// We got a new title:
            $current_title = $line;
        } elseif ($current_title !== '') {
            if ($line !== '') {	// We got a new paragraph for this post:
                if ($linebreak) {
                    $current_data .= '<p>' . $line . '</p>';
                } else {
                    $current_data .= $line . ' ';
                }
            } else {	// End of this post:
                $new_Item = clone $Item;
...
function check_cross_posting(&$post_category, &$post_extracats, $prev_main_cat = null)
{
    ...
    $post_extracats = param('post_extracats', 'array:integer', []);

    if (empty($post_category)) {	// Don't check because new category is created only in current collection:
        return true;
    }

    if (is_null($prev_main_cat)) { // new item, no need to check for previous main category
    ...
    foreach ($post_extracats as $key => $cat) {
        if (empty($cat)) {	// Skip a checking for new creating category:
            continue;
        }
        $cat_blog = get_catblog($cat);
        if (($cat_blog != $post_cat_blog) && ! ($allow_cross_posting % 2 == 1 && check_user_perm('blog_admin', '', false, $cat_blog))) { // this cat is not from the main category
            $Messages->add(T_('You are not allowed to cross post to several collections.'));
            $result = false;
        }
        if (! $result) { // no need to check other extracats
            break;
        }
    }
...
function check_categories(&$post_category, &$post_extracats, $Item = null, $from = 'backoffice')
{
    ...
    if ($from == 'backoffice' || empty($Item) || $Item->ID == 0 || $Blog->get_setting('in_skin_editing_category')) {	// If this is back-office OR categories are allowed to update from front-office:
        $post_category = param('post_category', 'integer', -1);
        $post_extracats = param('post_extracats', 'array:integer', []);
    } else {	// The updating of categories is not allowed, Use the current saved:
    ...
    if ($post_category == -1) { // no main cat select
        if (count($post_extracats) == 0) { // no extra cat select
            $post_category = $Blog->get_default_cat_ID();
        } else { // first extracat become main_cat
            if (get_allow_cross_posting() >= 2) { // allow moving posts between different blogs is enabled, set first selected cat as main cat
                $post_category = $post_extracats[0];
            } else { // allow moving posts between different blogs is disabled - we need a main cat from $blog
                foreach ($post_extracats as $cat) {
                    if (get_catblog($cat) != $blog) { // this cat is not from $blog
                        continue;
                    }
                ...
                if ($post_category == -1) { // wasn't cat selected from $blog select a default as main cat
                    $post_category = $Blog->get_default_cat_ID();
                }
            }
        }
        if ($post_category) { // If main cat is not a new category, and has been autoselected
            $Chapter = &$ChapterCache->get_by_ID($post_category);
            $post_category_Blog = $Chapter->get_Blog();
            $Messages->add(sprintf(
                T_('The main category for this post has been automatically set to "%s" (Blog "%s")'),
                $Chapter->get_name(),
                $post_category_Blog->get('name')
            ), 'warning');
        }
    }

    if (! $post_category || in_array(0, $post_extracats)) {	// if category key is 0 => means it is a new category
        if (! check_user_perm('blog_cats', '', false, $Blog->ID)) {	// Current user cannot add a categories for this blog
        ...
        if ($category_name == '') {
            ...
            check_categories_nosave($post_category, $post_extracats, $Item, $from); // set up the category parameters
            if ($show_error) { // new main category without name
    ...
    if (get_allow_cross_posting() == 2) { // Extra cats in different blogs is disabled, check selected extra cats
        $post_category_blog = get_catblog($post_category);
        $ignored_cats = '';
        foreach ($post_extracats as $key => $cat) {
            if (get_catblog($cat) != $post_category_blog) { // this cat is not from main category blog, it has to be ingnored
        ...
        if ($ingnored_length > 2) { // ingnore list is not empty
            global $admin_url;
            ...
            $Messages->add(sprintf(
                T_('The category selection "%s" was ignored since %s'),
                $ignored_cats,
                $cross_posting_text
            ), 'warning');
        }
 ...
 */
function echo_item_comments($blog_ID, $item_ID, $statuses = null, $currentpage = 1, $limit = null, $exclude_comment_IDs = [], $filterset_name = '', $expiry_status = 'active', $comment_type = 'feedback')
{
    ...
    if ($threaded_comments_mode) {	// Force comments list page size to 1000 on mode "Threaded comments":
        $limit = 1000;
    }

    if (empty($limit)) {	// Get default limit from current user's setting:
        global $UserSettings;
    ...
    if ($item_ID > 0) {	// Set filters to display only comments of the given Item:
        if ($comment_type == 'meta') {	// Check if current user can sees internal comments of this item:
            $ItemCache = &get_ItemCache();
            $Item = &$ItemCache->get_by_ID($item_ID, false, false);
            if (! $Item || ! check_user_perm('meta_comment', 'view', false, $blog_ID)) { // Current user has no permissions to view internal comments
                $comment_type = 'feedback';
            }
        }

        if (is_string($statuses)) {	// Try to get statuses from predefined constants:
            if ($statuses == '#only_moderation#') {	// Get only statuses that require moderation:
                $statuses = explode(',', $Blog->get_setting('moderation_statuses'));
            } elseif ($statuses == '#only_valid#') {	// Get only valid statuses:
                $all_statuses = get_visibility_statuses('keys', ['deprecated', 'redirected', 'trash']);
                $moderation_statuses = explode(',', $Blog->get_setting('moderation_statuses'));
                $statuses = array_diff($all_statuses, $moderation_statuses);
            } elseif ($statuses == '#all#') {	// Get all statuses:
                $statuses = get_visibility_statuses('keys', ['redirected', 'trash']);
            } elseif (strlen($statuses) > 2) {	// Get the requested statuses from string like '(published,private,draft)':
                $statuses = explode(',', substr($statuses, 1, strlen($statuses) - 2));
            } else {	// Wrong status request by string format:
                $statuses = null;
            }
        }

        if (empty($statuses)) {	// Get all status keys by default except of 'redirected', 'trash':
            $statuses = get_visibility_statuses('keys', ['redirected', 'trash']);
        }
        ...
        $CommentList->set_filters([
            'types' => $comment_type == 'meta' ? ['meta'] : ['comment', 'trackback', 'pingback', 'webmention'],
            'statuses' => $statuses,
            'expiry_statuses' => ($expiry_status == 'all' ? ['active', 'expired'] : [$expiry_status]),
    ...
    } else {	// Set filters to display all comments of the collection:
...
function echo_comment($Comment, $redirect_to = null, $save_context = false, $inlist_order = null, $display_meta_title = false, $reply_level = 0)
{
    ...
    if (empty($Comment->ID)) {	// Display warning about preview mode:
    ...
    if ($is_expired) { // comment is expired
        echo 'expired';
    } elseif ($Comment->is_meta()) { // internal comment
        echo 'meta';
    } else { // comment is not expired and not meta
        $Comment->status('raw');
    }
    echo '"' . $reply_level_style . '>';

    if (check_user_perm('comment!CURSTATUS', 'moderate', false, $Comment) ||
        ($Comment->is_meta() && check_user_perm('meta_comment', 'view', false, $Blog->ID))) {	// User can moderate this comment OR Comment is meta and current user can view internal comments of the collection:
        echo '<div class="panel-heading small">';
        echo '<div>';

        if ($Comment->is_meta()) { // Internal comment
            if ($inlist_order !== false) {	// Display order of internal comment in current list:
                echo '<span class="badge badge-info">' . $inlist_order . '</span> ';
            }

            if ($display_meta_title) {	// Display a title for internal comment:
                $comment_Item = &$Comment->get_Item();
                echo sprintf(
                    T_('<a %s>Internal comment</a> on %s'),
        ...
        if (! $Comment->is_meta()) {	// Display permalink oly for normal comments:
        ...
        if ($Comment->ID > 0 && ! $Comment->is_meta()) {	// Display checkbox to select normal existing comments for action only on view item page:
        ...
        if ($Comment->is_meta()) { // Display only author for internal comment
            $Comment->author('', '', ' &middot; ' . T_('Author') . ': ', '');
        } else { // Display the detailed info for standard comment
        ...
        if (! $Comment->is_meta()) {	// Display status banner only for normal comments:
        ...
        if (! $Comment->is_meta()) { // Don't display the titles for internal comments
            echo '<div class="bCommentTitle">';
            echo $Comment->get_title();
            if (get_param('p') == '') { // Don't display this title on a post view page
        ...
        if (check_user_perm('meta_comment', 'edit', false, $Comment)) { // Put the comment content into this container to edit by ajax
            echo '<div id="editable_comment_' . $Comment->ID . '" class="editable_comment_content">';
        }
        $Comment->content('htmlbody', 'true');
        if (check_user_perm('meta_comment', 'edit', false, $Comment)) { // End of the container that is used to edit internal comment by ajax
            echo '</div>';
        }
        echo '</div>';
        echo '</div>';

        if (! empty($Comment->ID)) {	// Display action buttons panel only for existing Comment in DB:
            ...
            if ($next_status_in_row && $next_status_in_row[0] != 'deprecated') { // Display deprecate button if current user has the rights:
                $Comment->deprecate_link('', '', get_icon('move_down_grey', 'imgtag', [
                    'title' => '',
                ]), '#', button_class(), '&amp;', true, true);
            }
            ...
            if (! $Comment->is_meta()) { // Display Spam Voting system
    ...
    } else {	// No permissions to moderate of this comment, just preview
        echo '<div class="panel-heading small">';
        echo '<div>';
        ...
        if (! empty($item_id) && $Comment->ID > 0 && ! $Comment->is_meta()) {	// Display checkbox to select normal existing comments for action only on view item page:
            echo '<input type="checkbox" name="selected_comments[]" value="' . $Comment->ID . '" /> ';
        }
        ...
        if ($Comment->can_be_displayed()) {	// Display more info if current user can view this comment on front-office:
            echo '<div class="panel-body">';
            echo '<div class="bCommentTitle">';
            echo $Comment->get_title();
            echo '</div>';
 ...
 */
function echo_comment_pagenumber($item_ID, $text, $page, $params = [])
{
    $params = array_merge([
        'page_before' => ' ',
        'page_after' => '',
 ...
 * @param array Params
 */
function echo_comment_pages($item_ID, $currentpage, $comments_number, $params = [])
{
    global $UserSettings;
    ...
    if ($comments_number == 0) { // No comments
        return;
    }

    $total_pages = ceil($comments_number / $params['page_size']);

    if ($total_pages < 2) { // No pages
        return;
    }

    if ($currentpage > $total_pages) { // current page number is greater then all page number, set current page to the last existing page
        $currentpage = $total_pages;
    }

    // Set first page
    if ($currentpage <= intval($params['list_span'] / 2)) { // the current page number is small
        $first_page = 1;
    } elseif ($currentpage > $total_pages - intval($params['list_span'] / 2)) { // the current page number is big
        $first_page = max(1, $total_pages - $params['list_span'] + 1);
    } else { // the current page number can be centered
        $first_page = $currentpage - intval($params['list_span'] / 2);
    }

    // Set last page
    if ($currentpage > $total_pages - intval($params['list_span'] / 2)) { // the current page number is big
    ...
    if ($comments_number > 0) {
        ...
        if ($last_page < $total_pages - 1) { // link to next pages
            $page_i = $last_page + floor(($total_pages - $last_page) / 2);
            echo_comment_pagenumber($item_ID, '...', $page_i, $params);
        }
        if ($last_page < $total_pages) { // link to last page
            echo_comment_pagenumber($item_ID, $total_pages, $total_pages, $params);
        }
        if ($currentpage < $total_pages) { // link to next page
...
function check_item_perm_edit($post_ID, $do_redirect = true)
{
    ...
    if ($post_ID > 0) { // Check permissions for editing of the current item:
        $ItemCache = &get_ItemCache();
        $edited_Item = $ItemCache->get_by_ID($post_ID);
        ...
        if ($user_can_edit) { // Check if the post type is enabled:
            if (! $edited_Item->is_type_enabled()) { // Don't allow to use a not enabled post type:
                $user_can_edit = false;
                if ($edited_ItemType = &$edited_Item->get_ItemType()) {
                    $permission_message = sprintf(
                        T_('The post you are trying to edit uses the post type #%d "%s" which is currently disabled. Thus you cannot edit this post.'),
                        $edited_ItemType->ID,
                        $edited_ItemType->get('name')
                    );
                } else {
    ...
    } else { // Check permissions for creating of a new item:
    ...
    if (! $user_can_edit) {
        if ($do_redirect) {	// Redirect to the blog url for users without messaging permission
            $Messages->add($permission_message);
            if (empty($Blog)) {	// Bad request without blog ID
                global $home_url;
                $redirect_to = $home_url;
            } else {	// Redirect to the current blog
                $redirect_to = $Blog->gen_blogurl();
            }
            header_redirect($redirect_to, 302);
            // will have exited
        } else {	// Current user cannot edit this post
            return false;
        }
    }
...
{
    if ($check_Blog === null) {	// Use current collection by default:
        global $Collection, $Blog;
        $check_Blog = $Blog;
    }

    if (empty($check_Blog)) {	// Strange case, but we restrict to create a new post
        return false;
    }

    if (! is_logged_in(false) || ! $check_Blog->get_setting('in_skin_editing')) {	// Don't allow anonymous users to create a new post & If setting is OFF
        return false;
    } else {	// Check permissions for current user
...
function get_item_new_link($before = '', $after = '', $link_text = '', $link_title = '#')
{
    ...
    if (! check_item_perm_create()) {	// Don't allow users to create a new post
        return false;
    }

    if ($link_text == '') {	// Default text
        $link_text = T_('Write a new post');
    }

    if ($link_title == '#') {
        $link_title = T_('Write a new post');
    }
 ...
 * @param array Params
 */
function echo_item_location_form(&$Form, &$edited_Item, $params = [])
{
    load_class('regional/model/_country.class.php', 'Country');
    load_funcs('regional/model/_regional.funcs.php');

    if (! $edited_Item->country_visible()) {	// If country is NOT visible it means all other location fields also are not visible, so exit here
        return;
    }
    ...
    $Form->begin_fieldset(T_('Location'), [
        'id' => 'itemform_location',
        'fold' => $params['fold'],
    ]);

    $table_class = '';
    if ($edited_Item->is_special()) { // A post with special type should always has the location fields as not required
    ...
    $Form->select_country('item_ctry_ID', $edited_Item->ctry_ID, $CountryCache, T_('Country'), [
        'required' => $country_is_required,
        'allow_none' => true,
    ]);

    if ($edited_Item->region_visible()) { // Region
        $region_is_required = ($edited_Item->get_type_setting('use_region') == 'required');
        $Form->select_input_options('item_rgn_ID', get_regions_option_list($edited_Item->ctry_ID, $edited_Item->rgn_ID, [
            'none_option_text' => T_('Unknown'),
        ]), T_('Region'), sprintf($button_refresh_regional, 'button_refresh_region'), [
            'required' => $region_is_required,
        ]);
    }

    if ($edited_Item->subregion_visible()) { // Subregion
        $subregion_is_required = ($edited_Item->get_type_setting('use_sub_region') == 'required');
        $Form->select_input_options('item_subrg_ID', get_subregions_option_list($edited_Item->rgn_ID, $edited_Item->subrg_ID, [
            'none_option_text' => T_('Unknown'),
        ]), T_('Sub-region'), sprintf($button_refresh_regional, 'button_refresh_subregion'), [
            'required' => $subregion_is_required,
        ]);
    }

    if ($edited_Item->city_visible()) { // City
        $city_is_required = ($edited_Item->get_type_setting('use_city') == 'required');
        $Form->select_input_options('item_city_ID', get_cities_option_list($edited_Item->ctry_ID, $edited_Item->rgn_ID, $edited_Item->subrg_ID, $edited_Item->city_ID, [
            'none_option_text' => T_('Unknown'),
        ]), T_('City'), sprintf($button_refresh_regional, 'button_refresh_city'), [
...
function display_hidden_custom_fields(&$Form, &$edited_Item)
{
    ...
    foreach ($custom_fields as $custom_field) { // For each custom field with type $type:
 ...
 */
function display_editable_custom_field($filed_name, &$Form, &$edited_Item, $params = [])
{
    $params = array_merge([
        'loop_index' => 0,
    ], $params);
    ...
    if (isset($custom_fields[$filed_name])) {	// Custom field is found by requested name:
        $custom_field = $custom_fields[$filed_name];

        $parent_Item = &$edited_Item->get_parent_Item();

        $custom_field_input_params = [];
        $custom_field_note = '';
        $parent_sync_checkbox_is_visible = false;
        if (! empty($custom_field['note'])) {	// Display a not of the custon field if it is filled:
            $custom_field_note .= $custom_field['note'];
        }
        if (is_admin_page() && $custom_field['type'] != 'separator') {	// Display a field name/code only in back-office:
            $custom_field_note .= empty($custom_field_note) ? '' : ' &middot; ';
            $custom_field_note .= T_('Field name') . ': <code>' . $custom_field['name'] . '</code>';
        }
        if ($parent_Item) {	// Display a value of parent post custom field:
            $parent_custom_field_value = $parent_Item->get_custom_field_value($custom_field['name'], $custom_field['type']);
            if ($parent_custom_field_value !== false) {	// If parent post realy has a custom field with same code and type
                $preview_parent_custom_field_value = $parent_custom_field_value;
                if (in_array($custom_field['type'], ['double', 'computed'])) {	// Use a formatted value to preview a double custom field:
                    $preview_parent_custom_field_value = $parent_Item->get_custom_field_formatted($custom_field['name'], [
                        'restrict_type' => $custom_field['type'],
                    ]);
                }
                if ($custom_field['type'] == 'html' || $custom_field['type'] == 'text') {	// Cut long values of multiline fields:
                ...
                if ($custom_field['type'] != 'computed') {	// The computed fields cannot be updated from parent here because we update them by formula on updating automatically,
                    ...
                    $custom_field_note .= ' &middot; ' . T_('Parent Item Field value') . ': '
                        . $parent_Item->get_edit_link([
                            'text' => format_to_output($preview_parent_custom_field_value, ($custom_field['type'] == 'double' ? 'raw' : 'htmlspecialchars')),
                        ])
                        . action_icon('', 'refresh', '#', null, null, null, [
        ...
        if (! $parent_sync_checkbox_is_visible) {	// Use hidden input when checkbox is not visible but we should keep all values as they were before when it was visible:
        ...
        if (! empty($custom_field['description'])) {	// Display a description in tooltip of the help icon:
            $custom_field_label .= ' ' . get_icon('help', 'imgtag', [
                'data-toggle' => 'tooltip',
        ...
        switch ($custom_field['type']) {
            case 'double':
                $Form->text_input('item_cf_' . $custom_field['name'], $custom_field['value'], 12, $custom_field_label, $custom_field_note, [
                    'maxlength' => 10000,
                    'style' => 'width:auto',
            ...
            case 'computed':
                ...
                break;
            case 'varchar':
                $Form->text_input('item_cf_' . $custom_field['name'], $custom_field['value'], 20, $custom_field_label, $custom_field_note, [
                    'maxlength' => 10000,
                    'style' => 'width:100%',
                    'required' => $custom_field['required'],
                ] + $custom_field_input_params);
                break;
            case 'text':
                $Form->textarea_input('item_cf_' . $custom_field['name'], $custom_field['value'], 5, $custom_field_label, [
            ...
            case 'image':
                ...
                break;
            case 'separator':
                if (is_admin_page() && $params['loop_index'] > 0) {	// This is a hack for back-office because there is a css table layout:
                    $Form->end_fieldset();
                }
                ...
                if (is_admin_page() && $params['loop_index'] > 0 && $params['loop_index'] < count($custom_fields)) {	// This is a hack for back-office because there is a css table layout:
                    $Form->begin_fieldset();
                }
                break;
        }
        ...
        if (empty($edited_Item->ID) && // New object is creating or copying
                ...
                ! in_array($custom_field['type'], ['computed', 'separator'])) { // Theese fields don't have an editable value
        ...
        if ($parent_Item && empty($evo_js_parent_custom_fields)) {	// JS to refresh custom field values from parent post custom fields:
            ?>
<script>
jQuery( document ).ready( function()
{
...
function display_editable_custom_fields(&$Form, &$edited_Item, $force_current_fields = false)
{
    ...
    if (empty($custom_fields)) {	// No custom fields
        return;
    }

    $c = 0;
    foreach ($custom_fields as $custom_field) {	// Loop through custom fields:
        display_editable_custom_field($custom_field['name'], $Form, $edited_Item, [
            'loop_index' => $c,
        ]);
        $c++;
    }
 ...
 * @return string
 */
function render_custom_field($value, $params = [])
{
    $params = array_merge([
    ...
    // Render stars:
    if (preg_match_all('/(#stars(:\d+.?\d+?)?(\/\d+)?)#/', $value, $star_matches)) {	// If at least one star template is found:
        foreach ($star_matches[0] as $s => $star_match) {
            ...
            if ($params['stars_value'] === null || ! is_numeric($params['stars_value'])) {	// Make active all stars by default or get active stars e.g. '2.3' from stars mask like #stars:2.3/5#:
                $stars_value = empty($star_matches[2][$s]) ? $stars_num : floatval(substr($star_matches[2][$s], 1));
            } else {	// Use a number of active stars from params if it is a numeric really
...
function render_custom_field_note_callback($m)
{
    ...
    return str_replace(
        ...
        [format_to_output($note_class, 'htmlattr'), $m[1]],
...
function set_session_Item($Item)
{
    ...
    if (! is_array($edited_items)) { // Initialize an array for Item objects
        $edited_items = [];
    }
...
function get_session_Item($item_ID = 0, $force_new = false)
{
    ...
    if (isset($edited_items[$item_ID]) && ($edited_items[$item_ID] instanceof Item)) {	// Get Item from Session:
        ...
        return $edited_Item;
    } elseif ($force_new) {	// Force to create new Item:
        load_class('items/model/_item.class.php', 'Item');
        $edited_Item = new Item();
...
function delete_session_Item($item_ID)
{
    ...
    if (! isset($edited_items[$item_ID])) { // Item doesn't exist in Session
        return;
    }
...
function prepare_item_content($content)
{
    ...
    if (strpos($content, '<code') !== false || strpos($content, '<pre') !== false) { // Call prepare_item_content_callback() on everything outside code/pre:
        $content = callback_on_non_matching_blocks(
            $content,
            '~<(code|pre)[^>]*>.*?</\1>~is',
            'replace_content',
            [$old_separators, $new_separators, 'str']
        );
 ...
 * @param array Params
 */
function items_manual_results_block($params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
    ...
    if (empty($Blog)) { // Init Blog
        $BlogCache = &get_BlogCache();
        $blog = get_param('blog');
        if (! empty($blog)) { // Get Blog by ID
            $Collection = $Blog = $BlogCache->get_by_ID($blog, false);
        }
        if (empty($Blog) && ! empty($cat_ID)) { // Get Blog from chapter ID
            $ChapterCache = &get_ChapterCache();
    ...
    if (empty($Blog) || $Blog->get('type') != 'manual') { // No Blog, Exit here
        return;
    }
    ...
    if (is_ajax_content()) {
        ...
        if ($order_action == 'update') { // Update an order to new value
                ...
                switch ($order_data[1]) {
                    case 'chapter':
                        ...
                        if ($updated_Chapter = &$ChapterCache->get_by_ID($order_obj_ID, false)) {
                            if (check_user_perm('blog_cats', '', false, $updated_Chapter->blog_ID)) { // Check permission to edit this Chapter
                    ...
                    case 'item':
                        ...
                        if ($updated_Item = &$ItemCache->get_by_ID($order_obj_ID, false)) {
                            if (check_user_perm('item_post!CURSTATUS', 'edit', false, $updated_Item)) { // Check permission to edit this Item
 ...
 * @param array Params
 */
function items_created_results_block($params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
        'edited_User' => null,
    ...
    if ($params['action'] != 'view' && ($created_items_Results->get_total_rows() > 0) && ($deleted_posts_created_count > 0)) {	// Display action icon to delete all records if at least one record exists & current user can delete at least one item created by user
    ...
    items_results($created_items_Results, [
        'field_prefix' => 'post_',
        'display_ord' => false,
 ...
 * @param array Params
 */
function items_edited_results_block($params = [])
{
    // Make sure we are not missing any param:
    $params = array_merge([
        'edited_User' => null,
    ...
    */
    ...
    if (($edited_items_Results->get_total_rows() > 0) && ($deleted_posts_edited_count > 0)) {	// Display actino icon to delete all records if at least one record exists & current user can delete at least one item created by user
    ...
    // Initialize Results object
    items_results($edited_items_Results, [
        'field_prefix' => 'post_',
        'display_ord' => false,
        'display_history' => false,
    ]);
 ...
 * @param array Params
 */
function items_list_block_by_page($params = [])
{
    $params = array_merge([
        'skin_name' => '',
    ...
    if (! skin_init_ajax($params['skin_name'], 'posts')) {	// Exit here if skin cannot be initialized
        return;
    }
...
function load_user_data_for_items($post_ids = null)
{
    ...
    if (! is_logged_in()) {	// There are no logged in user:
        return;
    }

    if (is_array($cache_items_user_data)) {	// User read statuses were already set:
        return;
    } else {	// Init with an empty array:
    ...
    if (empty($post_ids)) {	// The load was not requested for specific posts, so we have loaded all information what we have, ther rest of the posts were not read by this user:
        return;
    }
    ...
    foreach ($post_ids as $post_ID) {	// Make sure to set read statuses for each requested post ID:
        if (! isset($cache_items_user_data[$post_ID])) {	// If no item user data yet, Set default empty data:
 ...
 * @param array Params
 */
function items_results(&$items_Results, $params = [])
{
    global $Collection, $Blog;
    ...
    if ($params['display_selector'] &&
        check_user_perm('blog_post_statuses', 'edit', false, $Blog->ID)) {	// Display item selector only if current User has a permission to edit:
        $items_Results->cols[] = [
            'th' => '',
            'th_class' => 'shrinkwrap',
        ...
        $items_Results->list_mass_actions = [
            'prefix_text' => [
                'type' => 'text',
            ...
            'mass_change_renderer' => [
                'type' => 'text',
                'text' => get_mass_change_renderer_buttons('btn-xs'),
            ],
        ];
        if (is_pro() && check_user_perm('options', 'edit')) {	// Export Items only for PRO version:
            $items_Results->list_mass_actions['mass_export'] = [
                'type' => 'submit',
                'text' => T_('Export to XML'),
            ];
        }
        $items_Results->list_mass_actions['mass_delete'] = [
            'type' => 'submit',
            'text' => T_('Delete'),
            'class' => 'btn-danger',
        ];
        $items_Results->list_form_hiddens = [
            'ctrl' => 'items',
            'tab' => get_param('tab'),
    ...
    if ($params['display_date']) {	// Display Date column
        $items_Results->cols[] = [
            'th' => T_('Date'),
    ...
    if ($params['display_blog']) {	// Display Blog column
        if (! empty($Blog) && $Blog->get_setting('aggregate_coll_IDs')) { // Aggregated blog: display name of blog
            $items_Results->cols[] = [
                'th' => T_('Collection'),
                'th_class' => 'nowrap',
                'td_class' => 'nowrap',
    ...
    if ($params['display_author']) { // Display Author column:
        $items_Results->cols[] = [
            'th' => T_('Author'),
    ...
    if ($params['display_type']) { // Display Type column:
        $items_Results->cols[] = [
            'th' => T_('Type'),
    ...
    if ($params['display_title']) { // Display Title column
        $items_Results->cols[] = [
            'th' => T_('Title'),
    ...
    if ($params['display_slug']) {	// Display Slug column:
        $items_Results->cols[] = [
            'th' => T_('Slug'),
    ...
    if ($params['display_status']) { // Display status column
        $items_Results->cols[] = [
            'th' => T_('Status'),
            'th_class' => 'shrinkwrap',
            'td_class' => 'shrinkwrap left',
    ...
    if ($params['display_ord']) { // Display Ord column
        $items_Results->cols[] = [
            'th' => T_('Ord'),
            'th_class' => 'shrinkwrap hidden-xs',
    ...
    if ($params['display_history']) { // Display History (i) column
        $items_Results->cols[] = [
    ...
    if ($params['display_actions']) { // Display Actions column
        $items_Results->cols[] = [
            'th' => T_('Actions'),
            'th_class' => 'shrinkwrap hidden-xs',
            'td_class' => 'shrinkwrap hidden-xs',
...
function item_type_global_icons($object_Widget)
{
    ...
    if (! empty($Blog) && check_user_perm('blog_post_statuses', 'edit', false, $Blog->ID)) { // We have permission to add a post with at least one status:
        ...
        if (! empty($tab_type)) { // Get item types only by selected back-office tab
        ...
        if ($count_item_types > 0) {
            ...
            if ($count_item_types > 1) {	// Group only if moer than one item type for selected back-office tab:
            ...
            if (check_user_perm('admin', 'normal') &&
                check_user_perm('options', 'edit')) {	// Icon buttons for import:
                $import_buttons = [
                    'xml' => [
                ...
                if ($Blog->get('type') == 'forum') {	// Only for forums collection:
                    $import_buttons['phpbb'] = [
                        'title' => TB_('phpBB Import'),
                        'url' => $admin_url . '?ctrl=phpbbimport&amp;forum_blog_ID=' . $Blog->ID,
                    ];
                    $import_buttons['phpbb3'] = [
                ...
                foreach ($import_buttons as $import_button) {
                    $object_Widget->global_icon(
                        ...
                        4,
                        [
                            'class' => 'action_icon btn-default hidden-xs',
                        ],
                        'import',
                        [
            ...
            $object_Widget->global_icon(
                T_('Mass edit the current post list') . '...',
                'edit',
            ...
            foreach ($item_types as $item_type) {
                if (check_user_perm('blog_item_type_' . $item_type->perm_level, 'edit', false, $Blog->ID)) { // We have the permission to create posts with this post type:
                    $object_Widget->global_icon(
                        T_('Create multiple posts...'),
                        'new',
                        $admin_url . '?ctrl=items&amp;action=new_mass&amp;blog=' . $Blog->ID . '&amp;item_typ_ID=' . $item_type->ID,
                        ' ' . sprintf(T_('Mass create "%s"'), $item_type->name),
                        3,
                        4,
                        [
                            'class' => 'action_icon btn-default hidden-xs',
...
{
    $schema_titles = [];
    if ($include_false_schema) { // Include Unknown status
...
function collection_item_type_titles($coll_ID, $item_type_ID = null, $key_prefix = '', $include_no_default_type = true)
{
    ...
    if ($item_type_ID > 0) {	// Load also current item type of this collection even if the Item Type already is not enabled for collection:
    ...
    if ($include_no_default_type) {	// Include this option only when it is requested:
...
function display_mass_items_confirmation_panel()
{
    ...
    switch (param('confirm_action', 'string')) {
        case 'mass_delete':
            foreach ($selected_items as $i => $selected_item_ID) {	// Check if current User has a permission to delete the selected Item:
                if (($Item = &$ItemCache->get_by_ID($selected_item_ID, false, false))) {
                    $selected_items_list .= '<li>' . $Item->get_title([
    ...
    if (empty($selected_items_list)) {	// No selected items which can be edited by current User:
        return;
    }
...
function task_title_link($Item, $display_flag = true, $display_status = false)
{
    ...
    $col = '';
    if ($display_status && is_logged_in()) { // Display status
        $col .= $Item->get_format_status([
            'template' => '<div class="pull-right"><span class="note status_$status$" data-toggle="tooltip" data-placement="top" title="$tooltip_title$"><span>$status_title$</span></span></div>',
        ]);
    }

    if ($display_flag) { // Display country flag
        $col .= locale_flag($Item->locale, 'w16px', 'flag', '', false) . ' ';
    }

    $Item->get_Blog();

    if (is_admin_page()) { // Url to item page in backoffice
        $item_url = $admin_url . '?ctrl=items&amp;blog=' . $Item->get_blog_ID() . '&amp;p=' . $Item->ID;
    } else { // Url to item page in frontoffice
        $item_url = $Item->get_permanent_url();
    }

    if ($Item->Blog->get_setting('allow_comments') != 'never') { // The current blog can have comments:
        ...
        if ($nb_comments) {
            $comments_icon_params = [];
            $comment_moderation_statuses = $Item->Blog->get_setting('moderation_statuses');
            if (! empty($comment_moderation_statuses)) {	// Get a count of comments awaiting moderation:
    ...
    if (check_user_perm('meta_comment', 'view', false, $Item->get_blog_ID())) {	// Display icon of internal comments Only if current user can views internal comments:
        $metas_count = generic_ctp_number($Item->ID, 'metas', 'total');
        if ($metas_count > 0) {	// If at least one internal comment exists
            $col .= '<a href="' . $admin_url . '?ctrl=items&amp;blog=' . $Item->get_blog_ID() . '&amp;p=' . $Item->ID . '&amp;comment_type=meta#comments">'
                    . get_icon('comments', 'imgtag', [
                        'style' => 'color:#5bc0de',
                        'title' => T_('Internal comments'),
                    ])
                . '</a> ';
        }
    }
...
function item_row_type($Item)
{
    ...
    if (empty($type_edit_url)) { // No perm to edit post type
        return $type_title;
    } else { // Display a link to quick change type
...
function item_row_status($Item, $index, $cat_ID = null)
{
    ...
    // Get those statuses which are not allowed for the current User to create posts in this blog
    $exclude_statuses = array_merge(get_restricted_statuses($blog_ID, 'blog_post!', 'create', $Item->status, '', $Item), ['trash']);
    // Get allowed visibility statuses
    $status_options = get_visibility_statuses('', $exclude_statuses);

    if (check_user_perm('item_post!CURSTATUS', 'edit', false, $Item) &&
        isset($AdminUI, $AdminUI->skin_name) && $AdminUI->skin_name == 'bootstrap' && ! empty($status_options)) { // Use dropdown for bootstrap skin and if current user can edit this post
        $status_icon_options = get_visibility_statuses('icons', $exclude_statuses);
        ...
        $r .= '</ul>'
            . '</div>';
    } else { // Display only status badge when user has no permission to edit this post and for non-bootstrap skin
        $r = $Item->get_format_status([
...
function item_row_order($Item)
{
    ...
    if (isset($ItemList, $ItemList->filters['cat_single']) &&
        ! empty($ItemList->filters['cat_single'])) {	// Use order of single filtered category:
        ...
        $order_cat_attr = ' data-cat-id="' . $order_cat_ID . '"';
            ...
            count($ItemList->filters['cat_array']) == 1) {	// Use order of single filtered category form multiple categories selection:
    ...
    if ((! isset($ItemList, $ItemList->filters['cat_array']) || count($ItemList->filters['cat_array']) != 1) &&
        ...
        count($Item->get_orders_by_coll_ID($Blog->ID)) > 1) {	// Don't allow to edit order because in such case we display a sum of orders from all extra categories of the Item:
    ...
    } elseif (check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) {	// If current user can edit the Item then allow to edit an order by AJAX:
        return '<a href="#" rel="' . $Item->ID . '"' . $order_cat_attr . '>' . ($item_order === null ? '-' : $item_order) . '</a>';
    } else {	// If current user cannot edit the Item then display a static text
        return $item_order;
    }
}
...
{
    if (check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) {	// Allow to select Item only if current User can edit it:
...
function item_edit_actions($Item)
{
    ...
    $r .= $Item->get_copy_link([
        'before' => ' ',
        'after' => ' ',
        'text' => get_icon('copy', 'imgtag', [
    ...
    if (is_pro() && check_user_perm('options', 'edit')) {	// Export Item only for PRO version:
        $r .= action_icon(
            T_('Export this Item...'),
            'download',
 ...
 * @param array Params
 */
function manual_display_chapters($params = [])
{
    global $Collection, $Blog, $blog, $cat_ID;

    if (empty($Blog) && ! empty($blog)) { // Set Blog if it still doesn't exist
        $BlogCache = &get_BlogCache();
        $Collection = $Blog = &$BlogCache->get_by_ID($blog, false);
    }

    if (empty($Blog)) { // No Blog, Exit here
        return;
    }
    ...
    $chapter_path = [];
    if (! empty($cat_ID)) { // A category is opened
 ...
 */
function manual_display_chapter_row($Chapter, $level, $params = [])
{
    ...
    if ($perm_edit) { // Current user can edit the chapters of the blog
    ...
    if ($Chapter->get_parent_subcat_ordering() == 'manual') { // Parent chapter ordering is set to manual and not alphabetic
        if ($perm_edit) { // Add availability to edit an order if current user can edit chapters
    ...
    if ($perm_edit || $perm_create_item) { // Current user can edit the chapters of the blog or can create item in the blog
        if ($perm_edit) { // Create/Edit chapter, Move to another blog
            $r .= action_icon(T_('Edit...'), 'edit', $edit_url);
            if ($Settings->get('allow_moving_chapters')) { // If moving cats between blogs is allowed:
        ...
        if ($perm_create_item) { // Create new item
            ...
            $r .= action_icon(T_('New manual page...'), 'new', $admin_url . '?ctrl=items&action=new&blog=' . $Chapter->blog_ID . '&amp;cat=' . $Chapter->ID . $default_item_type_param . $redirect_to, null, null, null, [], [
                'style' => 'width:12px',
            ]);
        }
        if ($perm_edit) { // Delete chapter
 ...
 * @param array Params
 */
function manual_display_post_row($Item, $level, $params = [])
{
    global $line_class, $Settings;
    global $admin_url;
    global $Session;
    ...
    if ($params['chapter_ID'] != $Item->main_cat_ID) {	// Posts from extracats are displayed with italic:
    ...
    // Title
    $edit_url = $Item->ID;
    $item_icon = get_icon('file_message', 'imgtag', [
        'title' => '',
    ]);
    $item_edit_url = $Item->get_edit_url();
    $r .= '<td class="firstcol"><strong style="padding-left: ' . ($level) . 'em;">';
    if (! empty($item_edit_url)) { // If current user can edit this item
    ...
    // Category image
    $cat_thumb = '';
    if ($main_item_Chapter = &$Item->get_main_Chapter()) {	// Get image tag of main chapter of the Item:
    ...
    $r .= '<td>' . $Item->get_title(array_merge($params, [
    ...
    if (check_user_perm('slugs', 'view', false)) { // Display icon to view all slugs of this item if current user has permission
    ...
    if (isset($params['cat_order']) && $params['cat_order'] == 'manual') {
        if (check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) { // Add availability to edit an order if current user can edit this item
...
function item_td_task_cell($type, $Item, $editable = true)
{
    ...
    if ($editable && check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) { // Current user can edit this item
        return '<a href="#" rel="' . $value . '">' . $title . '</a>';
    } else { // No perms to edit item, Display only a title
        return $title;
    }
}
...
function item_td_task_class($post_ID, $post_pst_ID, $editable_class)
{
    ...
    if (check_user_perm('item_post!CURSTATUS', 'edit', false, $Item)) { // Current user can edit this item, Add a class to edit a priority by click from view list
