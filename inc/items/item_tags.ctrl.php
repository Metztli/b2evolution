...
if (param('tag_ID', 'integer', '', true)) { // Load item tag:
    $ItemTagCache = &get_ItemTagCache();
    if (($edited_ItemTag = &$ItemTagCache->get_by_ID($tag_ID, false)) === false) { // We could not find the goal to edit:
...
switch ($action) {
    ...
    case 'create':
        ...
        if ($edited_ItemTag->load_from_Request()) { // We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_ItemTag->check_delete(sprintf(TB_('Cannot delete tag "%s"'), '<b>' . $edited_ItemTag->dget('name') . '</b>'), [], true)) { // There are restrictions:
                $action = 'list';
            }
        }
        break;
    ...
    case 'unlink':
        ...
        if ($result) {
            $Messages->add(sprintf(
                TB_('Tag "%s" has been unlinked from the post "%s".'),
                '<b>' . $edited_ItemTag->dget('name') . '</b>',
                '<b>' . $edited_Item->dget('title') . '</b>'
            ), 'success');
        }
    ...
    case 'merge_cancel':
        ...
        if (empty($edited_ItemTag) || empty($old_ItemTag)) { // Wrong request, Redirect to item tags
            header_redirect($return_to ? $return_to : $admin_url . '?ctrl=itemtags', 303); // Will EXIT
            // We have EXITed already at this point!!
        }

        if ($action == 'merge_cancel') { // Merge action has been cancelled, REdirect to edit tag
        ...
        $Messages->add(sprintf(
            TB_('The previously named "%s" tag has been merged with the existing "%s" tag.'),
