...
if ($action == 'new_switchtab' && ! empty($mass_create)) {	// Replace action with mass create action
    $action = 'new_mass';
}

// for post from files
if ($action == 'group_action') { // Get the real action from the select:
...
switch ($action) {
    ...
    case 'history_restore':
        ...
        if ($restore) { // Restore a post from Session
            $edited_Item = get_session_Item($p);
        }

        if (empty($edited_Item)) { // Load post to edit from DB:
            $ItemCache = &get_ItemCache();
            $edited_Item = &$ItemCache->get_by_ID($p);
        }

        if ($action == 'propose') {	// Action to propose a change
    ...
    case 'link_version':
    case 'unlink_version':
        if ($action != 'edit_switchtab' && $action != 'edit_type') { // Stop a request from the blocked IP addresses or Domains
            antispam_block_request();
        }

        param('post_ID', 'integer', true, true);

        if ($action == 'edit_type') { // Load post from Session
        ...
        if (empty($edited_Item) || ($action == 'edit_type' && $from_tab == 'type')) { // Load post to edit from DB:
        ...
        if ($action == 'update_edit') {	// Get params to restore height and scroll position of item content field:
    ...
    case 'create_publish':
    case 'list':
        if (in_array($action, ['create_edit', 'create_link', 'create', 'create_publish'])) { // Stop a request from the blocked IP addresses or Domains
            antispam_block_request();
        }

        if ($action == 'list') {	// We only need view permission
            $selected = autoselect_blog('blog_ismember', 'view');
        } else {	// We need posting permission
            $selected = autoselect_blog('blog_post_statuses', 'edit');
        }

        if (! $selected) { // No blog could be selected
    ...
    case 'make_posts_from_files':
        ...
        $new_categories = param('new_categories', 'array:string', []);
        while ($l_File = &$selected_Filelist->get_next()) {
            ...
            } else { // Use default category ID if it was not selected on the form
            ...
            if ($edited_Item->dbinsert()) {
                ...
                if ($l_File->meta == 'notfound') { // That file has no meta data yet, create it now!
                    $l_File->dbsave();
                }
    ...
    case 'create_comments_post':
        ...
        if (empty($selected_comments)) {	// If no comments selected:
        ...
        foreach ($selected_comments as $s => $selected_comment_ID) {
            ...
            if (! $selected_Comment || $selected_Comment->item_ID != $item_ID) {	// Skip wrong comment:
                continue;
            }

            $moved_comments_IDs[] = $selected_Comment->ID;

            if ($s == 0) {	// Create post from first comment:
                if (empty($selected_Comment->author_user_ID)) {	// Don't create a post from comment with anonymous user:
                ...
                if ($new_Item->dbinsert()) {	// New post creation is success:
                ...
                } else {	// New post creation is failed:
                    $Messages->add(sprintf(TB_('Could not create new post from comment #%d'), $selected_Comment->ID), 'error');
                    break;
                }
            } else {	// Append all next comments for new created post which has been created from first comment:
        ...
        foreach ($moved_comments_IDs as $moved_comments_ID) {
            ...
            foreach ($child_comment_IDs as $child_comment_ID) {
                ...
                if ($old_in_reply_to_cmt_ID != $child_Comment->get('in_reply_to_cmt_ID')) {	// Update only if parent comment has been really corrected:
        ...
        if (count($reattached_comments_IDs)) {	// Display a message about the reattached comments:
    ...
    case 'items_visibility':
        ...
        if (empty($selected_items)) {	// If no items selected:
        ...
        foreach ($selected_items as $selected_item_ID) {
            if (($selected_Item = &$ItemCache->get_by_ID($selected_item_ID, false, false)) &&
                check_user_perm('item_post!CURSTATUS', 'edit', false, $selected_Item)) {	// If current User has a permission to edit the selected Item:
                $selected_Item->set('status', $item_status);
                if ($selected_Item->dbupdate()) {
                    if ($item_status == $selected_Item->get('status')) {	// If the item has been updated to the requested status:
                        $items_success++;
                    } else {	// If the item could not be updated to the requested status because of restriction e.g. by post status:
        ...
        if ($items_success) {	// Inform about success updates:
            $Messages->add(sprintf(TB_('Visibility of %d items have been updated to %s.'), $items_success, $item_status_title), 'success');
        }
        foreach ($items_restricted as $restricted_status => $restricted_status_num) {	// Inform about restricted updates:
        ...
        if ($items_failed) {	// Inform about failed updates:
    ...
    case 'mass_delete':
        ...
        if ($confirm) {	// Mass deleting of the selected items after confirmation:
            $ItemCache = &get_ItemCache();
            $items_success = 0;
            $items_restricted = [];
            $items_failed = 0;
            foreach ($selected_items as $selected_item_ID) {
                if (($selected_Item = &$ItemCache->get_by_ID($selected_item_ID, false, false)) &&
                    ...
                    $selected_Item->dbdelete()) {	// If current User has a permission to delete the selected Item:
                    $items_success++;
                } else {	// Wrong item or current User has no perm to delete the selected item:
                    $items_failed++;
                }
            }
            if ($items_success) {	// Inform about success deletes:
        ...
        } else {	// Redirect to page for confirmation of mass deleting items:
    ...
    case 'comments_visibility':
        ...
        if ($item_ID > 0) {	// Set an URL to redirect to item view details page after this action:
            $redirect_to = $admin_url . '?ctrl=items&blog=' . $blog . '&p=' . $item_ID . '&comment_type=feedback#comments';
        } else {	// Set an URL to redirect to comments list after this action:
        ...
        foreach ($selected_comments as $selected_comment_ID) {
            if (($selected_Comment = &$CommentCache->get_by_ID($selected_comment_ID, false, false)) &&
                ...
                check_user_perm('comment!' . $comment_status, 'moderate', false, $selected_Comment)) {	// If current User has a permission to edit the selected Comment:
                $selected_Comment->set('status', $comment_status);
                if ($selected_Comment->dbupdate()) {
                    if ($comment_status == $selected_Comment->get('status')) {	// If the comment has been updated to the requested status:
                        $comments_success++;
                    } else {	// If the comment could not be updated to the requested status because of restriction e.g. by post status:
        ...
        if ($comments_success) {	// Inform about success updates:
            $Messages->add(sprintf(TB_('Visibility of %d comments have been updated to %s.'), $comments_success, $comment_status_title), 'success');
        }
        foreach ($comments_restricted as $restricted_status => $restricted_status_num) {	// Inform about restricted updates:
        ...
        if ($comments_failed) {	// Inform about failed updates:
    ...
    case 'delete_comments':
        ...
        foreach ($selected_comments as $selected_comment_ID) {
            ...
            if (($selected_Comment = &$CommentCache->get_by_ID($selected_comment_ID, false, false)) &&
                check_user_perm('comment!CURSTATUS', 'delete', false, $selected_Comment)) {	// If current User has a permission to recycle/delete the selected Comment:
                $comment_status = $selected_Comment->get('status');
                if ($selected_Comment->dbdelete($force_permanent_delete)) {
                    if ($force_permanent_delete || $comment_status == 'trash') {	// If a selected comment has been deleted completely:
                        $comments_success_deleted++;
                    } else {	// If a selected comment has been moved to recycle bin:
            ...
            if ($force_permanent_delete || $comment_status == 'trash') {	// If a selected comment has NOT been deleted completely:
                $comments_failed_deleted++;
            } else {	// If a selected comment has NOT been moved to recycle bin:
                $comments_failed_recycled++;
            }
        }

        if ($comments_success_recycled) {	// Inform about success recycling:
            $Messages->add(sprintf(TB_('%d comments have been recycled.'), $comments_success_recycled), 'success');
        }
        if ($comments_success_deleted) {	// Inform about success deleted:
            $Messages->add(sprintf(TB_('%d comments have been deleted.'), $comments_success_deleted), 'success');
        }
        if ($comments_failed_recycled) {	// Inform about failed deletions:
            $Messages->add(sprintf(TB_('%d comments could not be recycled.'), $comments_failed_recycled), 'error');
        }
        if ($comments_failed_deleted) {	// Inform about failed deletions:
    ...
    case 'mass_change_cat':
        ...
        $ChapterCache = &get_ChapterCache();
        if ($cat_type == 'main') {	// Get a selected main category:
            $main_cat_ID = param('post_category', 'integer', true);
            $main_Chapter = &$ChapterCache->get_by_ID($main_cat_ID);
        } else {	// Get the selected extra categories:
            $extra_categories = param('post_extracats', 'array:integer');
        }

        if (empty($main_cat_ID) && empty($extra_categories)) {	// If no categories selected:
        ...
        foreach ($selected_items as $selected_item_ID) {
            if (($selected_Item = &$ItemCache->get_by_ID($selected_item_ID, false, false)) &&
                ...
                if ($cat_type == 'main') {	// Change main category:
                ...
                } elseif ($cat_type == 'extra') {	// Add extra categories to previous linked categories:
                    $selected_Item->set('extra_cat_IDs', array_unique(array_merge($current_extra_categories, $extra_categories)));
                } elseif ($cat_type == 'remove_extra') {	// Remove extra categories from previous linked categories except if an extra category is also the primary category:
                    ...
                    if (empty($remove_extra_categories)) {	// Nothing to remove, skip to next Item:
                        continue;
                    }

                    $selected_Item->set('extra_cat_IDs', array_diff($current_extra_categories, $remove_extra_categories));
                }
                if ($selected_Item->dbupdate()) {	// If the item has been updated to the requested categories:
                    $items_success++;
                    continue;
                }
            }
        ...
        if ($cat_type == 'main') {	// Report about changed main category:
            if ($items_success) {	// Inform about success updates:
        ...
        } elseif ($cat_type == 'extra') {	// Report about added extra categories:
            $extra_cats_names = [];
        ...
        } elseif ($cat_type == 'remove_extra') {	// Report about removed extra categories:
            $extra_cats_names = [];
            foreach ($extra_categories as $extra_cat_ID) {
                if ($extra_Chapter = &$ChapterCache->get_by_ID($extra_cat_ID, false, false)) {
                    $extra_cats_names[] = '"' . $extra_Chapter->get('name') . '"';
    ...
    case 'mass_change_renderer':
        ...
        if (empty($renderers)) {	// If no categories selected:
        ...
        foreach ($selected_items as $selected_item_ID) {
            if (($selected_Item = &$ItemCache->get_by_ID($selected_item_ID, false, false)) &&
                ...
                if ($selected_Item->dbupdate()) {	// If the item has been updated with the requested renderers:
                    $items_success++;
                    continue;
                }
            }
        ...
        if ($renderer_change_type == 'add_renderer') {	// Report about added renderers:
            $renderer_names = [];
        ...
        } elseif ($renderer_change_type == 'remove_renderer') {	// Report about removed extra categories:
            $renderer_names = [];
            foreach ($renderers as $code) {
                if ($renderer_Plugin = &$Plugins->get_by_code($code)) {
                    $renderer_names[] = '"' . $renderer_Plugin->name . '"';
    ...
    default:
        ...
        $module_result = modules_call_method('handle_backoffice_action', [
            'ctrl' => 'items',
            'action' => $action,
            'action_type' => 'action1',
        ]);
        if ($module_result === null) {	// Deny wrong action if it is not handled by any module:
            debug_die('unhandled action 1:' . htmlspecialchars($action));
        }
}

$AdminUI->breadcrumbpath_init(true, [
    'text' => TB_('Collections'),
    'url' => $admin_url . '?ctrl=collections',
]);
...
switch ($action) {
    ...
    case 'new_type': // this gets set as action by JS, when we switch tabs
        ...
        if (in_array($action, ['new', 'new_type'])) {
            param('restore', 'integer', 0);
            if ($restore || $action == 'new_type') { // Load post from Session
        ...
        load_class('items/model/_item.class.php', 'Item');
        if (empty($edited_Item)) { // Create new Item object
            $edited_Item = new Item();
        ...
        if ($edited_Item->main_cat_ID && (get_allow_cross_posting() < 2) && $edited_Item->get_blog_ID() != $blog) { // the main cat is not in the list of categories; this happens, if the user switches blogs during editing:
            $edited_Item->set('main_cat_ID', $Blog->get_default_cat_ID());
        }
        ...
        if (empty($item_typ_ID)) {	// No selected item type, use default:
            $title = TB_('New post');
        } else {	// Get item type to set a pge title:
        ...
        if ($action == 'new_type') {	// Save the changes of Item to Session:
    ...
    case 'new_version': // Add version
        ...
        if ($action == 'new_version') {	// Creating new version
            if (param('post_locale', 'string', null) !== null) {
                $edited_Item->set_from_Request('locale');
            }

            if (param('post_create_child', 'integer', null) === 1) {	// Set parent Item:
            ...
            if (param('post_coll_ID', 'integer', null) !== null &&
                $post_coll_ID != $edited_Item->get_blog_ID()) {	// Create Item in different collection:
                $BlogCache = &get_BlogCache();
                $linked_Blog = $BlogCache->get_by_ID($post_coll_ID, false, false);
                if (! check_user_perm('blog_post_statuses', 'edit', false, $post_coll_ID)) {	// If current User cannot create an Item in the selected locale collection,
                ...
                $post_extracats = [$edited_Item->get('main_cat_ID')];
            }

            if (param('item_typ_ID', 'integer', null) !== null) {	// Set Item Type:
                if (! $edited_Item->get_Blog()->is_item_type_enabled($item_typ_ID)) {	// Use default Item Type if it is NOT enabled for the selected collection:
        ...
        } else {	// Always duplicate images on action=copy:
        ...
        if ($duplicate_same_images) {	// Duplicate attachments from source Item:
    ...
    case 'edit_type': // this gets set as action by JS, when we switch tabs
        ...
        if ($load_from_request && ! ($action == 'edit_type' && $from_tab == 'type')) {	// We use the request variables to fill the edit form, because we need to be able to pass those values
            // from tab to tab via javascript when the editor wants to switch views...
        ...
        if ($action == 'edit_type') { // Save the changes of Item to Session
            set_session_Item($edited_Item);
        }
        break;
    ...
    case 'history_lastseen':
        ...
        if ($lastnotseen_revision_ID > 0 && $edited_Item->get_user_data('item_date') < $edited_Item->get('last_touched_ts')) {	// Redirect to compare last not seen revision with current version:
            header_redirect($admin_url . '?ctrl=items&action=history_compare&p=' . $edited_Item->ID . '&r1=a' . $lastnotseen_revision_ID . '&r2=c');
        } else {	// Redirect to view current version because User have already seen all changes before:
    ...
    case 'history_details':
        ...
        if (! $Revision) {	// Redirect to history list on wrong requested revision:
            if (substr(get_param('r'), 0, 1) == 'p') {	// When view old(not existing) proposed change:
                $Messages->add(TB_('The changes have already been accepted or rejected.'), 'error');
            } else {	// When view archived version:
    ...
    case 'history_compare':
        ...
        if (! $Revision_1 || ! $Revision_2) {	// Redirect to history list on wrong requested revision:
            if (substr(get_param('r1'), 0, 1) == 'c' && substr(get_param('r2'), 0, 1) == 'p') {	// When compare current version with old(not existing) proposed change(e.g. on opening url from old email message):
                $Messages->add(TB_('The changes have already been accepted or rejected.'), 'error');
            } else {	// When compare all other cases:
                // Don't translate because it should not happens on normal work:
                $Messages->add('The requested version does not exist.', 'error');
            }
            header_redirect($admin_url . '?ctrl=items&action=history&p=' . $edited_Item->ID);
        ...
        foreach ($r2_custom_fields as $r2_custom_field) {
            if (! isset($custom_fields[$r2_custom_field['name']])) {	// Append custom fields which don't exist in 1st revision but exist in 2nd revision:
                $custom_fields[$r2_custom_field['name']] = $r2_custom_field;
            }
        }
        $revisions_difference_custom_fields = empty($custom_fields) ? false : []; // FALSE means Item has no custom fields
        foreach ($custom_fields as $custom_field) {
            ...
            if ($r1_custom_field_value != $r2_custom_field_value) {	// If values are different:
                if ($r1_custom_field_value !== false &&
                    $r2_custom_field_value !== false) {	// Compare custom field values of 1st and 2nd revisions if they are used for both revisions:
                    $revisions_difference_custom_field = new Diff(
                        explode("\n", $r1_custom_field_value),
                        explode("\n", $r2_custom_field_value)
                    );
                    if ($custom_field['type'] == 'html' || $custom_field['type'] == 'text') {	// Display a line number for custom fields with multiple lines:
                        $revisions_difference_custom_field = $TableDiffFormatter->format($revisions_difference_custom_field);
                    } else {	// Don't display a line number for custom fields with single line:
                        ...
                        $r2_custom_field_value !== false) {	// Don't compare custom field values if at least one is not used in revision but however they are different:
                    $revisions_difference_custom_field = null;
                }
            }

            if ($revisions_difference_custom_field !== '') {	// Display custom fields only with differences:
                ...
                if ($revisions_difference_custom_field === null) {	// Store field value instead of difference if one field is not used in some revision:
                    if ($r1_custom_field_value !== false) {	// If only old revision has the field:
                        $revision_custom_fields['r1_value'] = $r1_custom_field_value;
                    } else {	// If only new revision has the field:
                        $revision_custom_fields['r2_value'] = $r2_custom_field_value;
                    }
                } else {	// Store a difference if field is used in both revisions:
                    $revision_custom_fields['diff_value'] = $revisions_difference_custom_field;
                }
                if ($revision_custom_fields['r1_label'] != $revision_custom_fields['r2_label']) {	// Compare the labels of custom fields:
        ...
        if ($r1_LinkList = $LinkOwner->get_attachment_LinkList()) {
            $revisions_difference_links = [];
            while ($r1_Link = &$r1_LinkList->get_next()) {
                $revisions_difference_links[$r1_Link->ID]['r1'] = [
        ...
        if ($r2_LinkList = $LinkOwner->get_attachment_LinkList()) {
            ...
            while ($r2_Link = &$r2_LinkList->get_next()) {
                ...
                if (isset($revisions_difference_links[$r2_Link->ID]['r1']) && $revisions_difference_links[$r2_Link->ID]['r1'] == $r_link_data) {	// The links/attachments of both revisions are equal:
                    unset($revisions_difference_links[$r2_Link->ID]);
                } else {	// The links/attachments of both revisions have at least one difference:
    ...
    case 'history_restore':
        ...
        if ($r > 0 && $edited_Item->update_from_revision($r)) {	// Update item only from revisions ($r == 0 for current version):
    ...
    case 'create_publish':
        ...
        } else {	// Use auto mode for notifications depending on the edited Item status:
        ...
        if ($action == 'create_publish') { // load publish status from param, because a post can be published to many status
        ...
        if (! empty($mass_create)) {	// ------ MASS CREATE ------
        ...
        if ($result) { // There are no validation errors
            if (isset($Items) && ! empty($Items)) {	// We can create multiple posts from single post
                foreach ($Items as $edited_Item) {	// INSERT NEW POST INTO DB:
                    $result = $edited_Item->dbinsert();
                }
            } else {	// INSERT NEW POST INTO DB:
                $result = $edited_Item->dbinsert();
            }
        ...
        if ($result && $action == 'create_link') {	// If the item has been inserted correctly and we should copy all links from the duplicated item:
            if (check_user_perm('item_post!CURSTATUS', 'edit', false, $edited_Item)
                && check_user_perm('files', 'view', false)) {	// Allow this action only if current user has a permission to view the links of new created item:
                $original_item_ID = param('p', 'integer', null);
                $ItemCache = &get_ItemCache();
                if ($original_Item = &$ItemCache->get_by_ID($original_item_ID, false, false)) {	// Copy the links only if the requested item is correct:
                    if (check_user_perm('item_post!CURSTATUS', 'view', false, $original_Item)) {	// Current user must has a permission to view an original item
                    ...
                    } else {	// Display error if user tries to duplicate the disallowed item:
        ...
        if (! $result) { // could not insert the new post ( validation errors or unsuccessful db insert )
        ...
        if (! $exit_after_save && check_user_perm('item_post!CURSTATUS', 'edit', false, $edited_Item)) {	// We want to continue editing...
            $tab_switch_params = 'p=' . $edited_Item->ID;
        ...
        $Session->set('fadeout_array', ['item-' . $edited_Item->ID]);

        if (! $valid_item_type) {	// Item Type is not enabled for this collection, we will redirect to item type selection to allow user to change it t a valid one:
            ...
            ! strpos($redirect_to, 'tab=manual')) {	// Where to go after creating the post?
            // yura> When a post is created from "workflow" or "manual" we should display a post list

            // Where do we want to go after creating?
            if ($edited_Item->Blog->get_setting('enable_goto_blog') == 'blog' && $edited_Item->can_be_displayed()) {	// Redirect to collection home page ONLY if current user can view it:
                $edited_Item->load_Blog();
                $redirect_to = $edited_Item->Blog->gen_blogurl();
            } elseif ($edited_Item->Blog->get_setting('enable_goto_blog') == 'post' && $edited_Item->can_be_displayed()) {	// Redirect to item page to view new created item ONLY if current user can view it:
        ...
        if ($redirect_to !== null) {	// The redirect url was NOT set to NULL ( $blog_redirect_setting == 'no' )
    ...
    case 'link_version':
        ...
        $ItemCache = &get_ItemCache();
        if ($dest_Item = &$ItemCache->get_by_ID($dest_post_ID, false, false)) {	// Do the linking:
            ...
            foreach ($other_version_items as $other_version_Item) {
                if ($dest_Item->get('locale') == $other_version_Item->get('locale')) {	// This is a duplicate locale
        ...
        // REDIRECT / EXIT:
        header_redirect($edited_Item->get_edit_url([
            'glue' => '&',
        ]));
        /* EXITED */
        break;
    ...
    case 'unlink_version':
        ...
        $ItemCache = &get_ItemCache();
        if ($unlink_Item = &$ItemCache->get_by_ID($unlink_item_ID, false, false)) {	// Do the unlinking:
    ...
    case 'extract_tags':
        ...
        if ($action == 'update_publish') { // load publish status from param, because a post can be published to many status
        ...
        if ($isset_category) { // we change the categories only if the check was succesful
            ...
            if ($off_limit_cats) {
                $Messages->add(sprintf(
                    TB_('Please note: this item is also cross-posted to the following other categories/collections: %s'),
                    implode(', ', $r)
                ), 'note');
            }
        ...
        $Plugins->trigger_event('AdminBeforeItemEditUpdate', [
            'Item' => &$edited_Item,
        ]);
        ...
        if ($Messages->has_errors()) {	// There have been some validation errors:
            break;
        }

        // UPDATE POST IN DB:
        if (! $edited_Item->dbupdate()) { // Could not update successful
            $Messages->add(TB_('The post couldn\'t be updated.'), 'error');
            break;
        }

        if (strpos($action, 'update') === 0) {	// Update attachments folder:
        ...
        if (in_array($action, ['update_edit', 'update', 'update_publish'])) {	// Clear all proposed changes of the updated Item:
            $edited_Item->clear_proposed_changes();
        }

        $Messages->add(TB_('Post has been updated.'), 'success');

        if ($action == 'extract_tags') {	// Extract all possible tags from item contents:
        ...
        if (! $exit_after_save) { // We want to continue editing...
            break;
        }
        ...
        if (! $valid_item_type) {	// Item Type is not enabled for this collection, we will redirect to item type selection to allow user to change it t a valid one:
            ...
            strpos($redirect_to, 'tab=tracker')) { // We should show the posts list if:
            //    a post is in "Redirected" status
            //    a post is updated from "workflow" view tab
            $blog_redirect_setting = 'no';
        } elseif (strpos($redirect_to, 'tab=manual')) {	// Use the original $redirect_to if a post is updated from "manual" view tab:
            $blog_redirect_setting = 'orig';
        } else {	// The post was changed:
            $blog_redirect_setting = $edited_Item->Blog->get_setting('editing_goto_blog');
        }

        if ($blog_redirect_setting == 'blog' && $edited_Item->can_be_displayed()) {	// Redirect to collection home page ONLY if current user can view it:
            $edited_Item->load_Blog();
            $redirect_to = $edited_Item->Blog->gen_blogurl();
        } elseif ($blog_redirect_setting == 'post' && $edited_Item->can_be_displayed()) {	// Redirect to item page to view new created item ONLY if current user can view it:
            $redirect_to = $edited_Item->get_permanent_url();
        } elseif ($blog_redirect_setting == 'orig') { // Use original $redirect_to:
            $Session->set('highlight_id', $edited_Item->ID);
            $Session->set('fadeout_array', ['item-' . $edited_Item->ID]);
            $redirect_to = url_add_param($redirect_to, 'highlight_id=' . $edited_Item->ID, '&');
        } elseif ($blog_redirect_setting == 'post_type') {	// load_from_request param set to 0 will prevent loading of Item data from request because we are not passing any item data from request!
            $redirect_to = url_add_param($admin_url, 'ctrl=items&action=edit_type&post_ID=' . $edited_Item->ID . '&load_from_request=0');
        } else { // $blog_redirect_setting == 'no', set redirect_to = NULL which will redirect to posts list
            $redirect_to = null;
        }
    ...
    case 'update_type':
        ...
        if ($Blog->is_item_type_enabled($ityp_ID)) { // Set new post type only if it is enabled for the Blog:
        ...
        if (! empty($from_tab) && $from_tab == 'type') { // It goes from items lists to update item type immediately:
            if ($Blog->is_item_type_enabled($ityp_ID)) { // Update only when the selected item type is enabled for the Blog:
        ...
        } else { // Set default redirect urls (It goes from the item edit form)
            if ($post_ID > 0) { // Edit item form
                $redirect_to = $admin_url . '?ctrl=items&blog=' . $Blog->ID . '&action=edit&restore=1&p=' . $edited_Item->ID;
            } elseif ($duplicated_item_ID > 0) { // Copy item form
    ...
    case 'update_status':
        ...
        if ($edited_Item->get('status') == 'redirected' && empty($edited_Item->url)) { // Note: post_url is not part of the simple form, so this message can be a little bit awkward there
            param_error(
                'post_url',
        ...
        if (param_errors_detected()) { // If errors then redirect to edit form
            $redirect_to = $admin_url . '?ctrl=items&blog=' . $Blog->ID . '&action=edit&restore=1&p=' . $edited_Item->ID;
        } else { // No errors, Update the item and redirect back to list
    ...
    case 'mass_save' :
        ...
        while ($Item = &$ItemList->get_item()) {	// check user permission
    ...
    case 'publish_now' :
        ...
        if ($action == 'publish_now') { // Update post dates
        ...
        // REDIRECT / EXIT
        if ($action == 'publish' && ! empty($redirect_to) && (strpos($redirect_to, $admin_url) !== 0)) { // We clicked publish button from the front office
            header_redirect($redirect_to);
        } elseif ($edited_Item->Blog->get_setting('enable_goto_blog') == 'blog' && $edited_Item->can_be_displayed()) {	// Redirect to collection home page ONLY if current user can view it:
            $edited_Item->load_Blog();
    ...
    case 'delete':
        ...
        if (! $Messages->has_errors()) {	// There have been no validation errors:
            // DELETE POST FROM DB:
    ...
    case 'list':
        ...
        if ($ItemList->single_post) {	// We have requested to view a SINGLE specific post:
            $action = 'view';
        }
        break;
    ...
    case 'append':
        ...
        if ($edited_Item->ID == $dest_post_ID) {	// If same Item is used as source for merging:
        ...
        $ItemCache = &get_ItemCache();
        if (! ($dest_Item = &$ItemCache->get_by_ID($dest_post_ID, false, false))) {	// If Item doesn't exist in DB:
            $Messages->add('Item to merge does not exist any more.', 'error');
            // REDIRECT / EXIT
            header_redirect();
        }

        if ($action == 'append') {	// If we should append item and comments at the end with new dates
            ...
            if ($latest_comment_time !== null) {	// If target Item has at lest one comment use new date/time for new appended comments:
        ...
        if (isset($append_comment_timestamp)) {	// Append action with 1 minute incrementing:
            $Comment->set('date', date2mysql($append_comment_timestamp));
            $append_comment_timestamp += 60;
        } else {	// Merge action with saving date/time:
        ...
        if (is_array($notifications_flags)) {
            foreach ($notifications_flags as $n => $notifications_flag) {
                if (! in_array($notifications_flag, ['moderators_notified', 'members_notified', 'community_notified'])) {	// Skip values which are not allowed for comment:
                    unset($notifications_flags[$n]);
                }
            }
        }
        $Comment->set('notif_flags', $notifications_flags);
        if ($Comment->dbinsert()) {	// If comment has been created try to copy all attachments from source Item:
            $DB->query('UPDATE T_links
        ...
        if (isset($append_comment_timestamp)) {	// Append comments with new dates after the latest comment of the target Item:
        ...
        } else {	// Merge comments with saving their dates:
            $DB->query('UPDATE T_comments
				  SET comment_item_ID = ' . $dest_Item->ID . '
    ...
    case 'save_propose':
        ...
        if ($edited_Item->create_proposed_change()) {	// If new proposed changes has been inserted in DB successfully:
            $Messages->add(TB_('New proposed change has been recorded.'), 'success');
            if (check_user_perm('item_post!CURSTATUS', 'edit', false, $edited_Item)) {	// Redirect to item history page with new poroposed change if current User has a permisson:
                header_redirect($admin_url . '?ctrl=items&action=history&p=' . $edited_Item->ID);
            } else {	// Redirect to item view page:
    ...
    case 'reject_propose':
        ...
        if (! $Revision || $Revision->iver_type != 'proposed') {	// Stop on wrong requested proposed change:
            debug_die('The proposed change #' . $r . ' could not be found for Item #' . $edited_Item->ID . ' in DB!');
        }

        if ($action == 'accept_propose') {	// Accept the proposed change:
        ...
        if ($result) {	// Delete also the proposed changes with custom fields and links to complete accept/reject action:
...
function init_list_mode()
{
    ...
    if (param('p', 'integer', null) || param('title', 'string', null)) {	// Single post requested, do not filter any post types. If the user
    ...
    } else {	// Store/retrieve preferred tab from UserSettings:
        ...
        if (! in_array($tab_type, ['post', 'page', 'intro', 'content-block', 'special'])) {	// Fix wrong requested type:
            $tab_type = 'post';
        }
    }

    if ($tab == 'tracker' && (! $Blog->get_setting('use_workflow') || ! check_user_perm('blog_can_be_assignee', 'edit', false, $Blog->ID))) { // Display workflow view only if it is enabled
        global $Messages;
    ...
    if (! empty($tab) && ! empty($items_list_param_prefix)) {	// Use different param prefix for each tab
    ...
    $ItemList->set_default_filters([
        'visibility_array' => get_visibility_statuses('keys'),
    ]);

    if ($Blog->get_setting('orderby') == 'RAND') {	// Do not display random posts in backoffice for easy management
        $ItemList->set_default_filters([
    ...
    // Init filter params:
    if (! $ItemList->load_from_Request()) { // If we could not init a filterset from request
...
switch ($action) {
    ...
    case 'create_publish':
        ...
        if (! blog_has_cats($blog)) {
            ...
            if (check_user_perm('blog_cats', 'edit', false, $blog)) { // If current user has a permission to create a category
                global $admin_url;
    ...
    case 'extract_tags':
        // Generate available blogs list:
        $AdminUI->set_coll_list_params('blog_ismember', 'view', [
            'ctrl' => 'items',
            'filter' => 'restore',
        ]);
        ...
        $AdminUI->htmltitle = $edited_Item->get_title([
            'title_field' => 'short_title,title',
            'link_type' => 'none',
        ]);
        if (empty($AdminUI->htmltitle)) {	// Display collection short name when item has no yet e.g. on creating or when titles are disabled for current Item Type:
        ...
        switch ($action) {
            ...
            case 'update_publish': // on error
            case 'extract_tags':
                $item_permanent_url = $edited_Item->get_permanent_url('', '', '&amp;', ['none']);
                if ($item_permanent_url !== false) {	// Display item permanent URL only if permanent type is not 'none':
                    $AdminUI->global_icon(
                        TB_('Permanent link to full entry'),
                        'permalink',
                        $item_permanent_url,
                        ' ' . TB_('Permalink'),
                        4,
                ...
                if ($Blog->get_setting('allow_comments') != 'never') {
                    ...
                    $AdminUI->global_icon(
                        $item_feedback_title,
                        ($comments_number > 0 ? 'comments' : 'nocomment'),
                        $admin_url . '?ctrl=items&amp;blog=' . $Blog->ID . '&amp;p=' . $edited_Item->ID . '#comments',
                        ' ' . $item_feedback_title,
                        4,
                        3,
                        [
                ...
                if (! empty($edited_item_url)) {	// If user has a permission to copy the edited Item:
                    $AdminUI->global_icon(
                        TB_('Duplicate this post...'),
                        'copy',
                        $edited_item_url,
                        ' ' . TB_('Duplicate...'),
                        4,
                ...
                if (check_user_perm('item_post!CURSTATUS', 'edit', false, $edited_Item)) {	// If user has a permission to merge the edited Item:
                    $AdminUI->global_icon(
                        TB_('Merge with...'),
                        'merge',
                        '#',
                ...
                if (check_user_perm('item_post!CURSTATUS', 'delete', false, $edited_Item)) {	// User has permissions to delete this post
                    $AdminUI->global_icon(
                        TB_('Delete this post'),
                        'delete',
                        $admin_url . '?ctrl=items&amp;action=delete&amp;post_ID=' . $edited_Item->ID . '&amp;' . url_crumb('item'),
                        ' ' . TB_('Delete'),
                        4,
                        3,
                        [
        ...
        if (! in_array($action, ['new_type', 'edit_type', 'history', 'history_details', 'history_compare'])) {
            if ($edited_Item->ID > 0) { // Display a link to history if Item exists in DB
                $AdminUI->global_icon(
                    TB_('Changes'),
                    '',
                    $edited_Item->get_history_url(),
                    $edited_Item->history_info_icon() . ' ' . TB_('Changes'),
                    4,
                    3,
                    [
                        'style' => 'margin-right: 3ex',
            ...
            if (count($AdminUI->global_icons) > 1 && $edited_Item->ID > 0) {
                ...
                if ($display_permalink) { // Insert the history icon right after the permalink icon
                    array_splice($AdminUI->global_icons, 1, 0, [$history_icon]);
                } else { // Move the history icon in front
                    array_unshift($AdminUI->global_icons, $history_icon);
                }
            }

            if ($action != 'propose' && $Blog->get_setting('in_skin_editing') && (check_user_perm('blog_post!published', 'edit', false, $Blog->ID) || get_param('p') > 0)) { // Show 'In skin' link if Blog setting 'In-skin editing' is ON and User has a permission to publish item in this blog
            ...
            init_hotkeys_js('blog', ['f2', 'f9']);
        }

        if (in_array($action, ['history', 'history_details', 'history_compare'])) {	// History tabs:
            if (check_user_perm('item_post!CURSTATUS', 'delete', false, $edited_Item)) {	// User has permissions to edit this Item:
                $AdminUI->global_icon(TB_('Edit current version'), 'edit', $admin_url . '?ctrl=items&amp;action=edit&amp;p=' . $edited_Item->ID, TB_('Edit current version'), 4, 3, [
                    'style' => 'margin-right:3ex',
                ]);
            }
    ...
    case 'history_details':
        ...
        init_hotkeys_js('blog', ['f2', 'ctrl+f2']);
...
if (isset($edited_Item) && ($ItemType = &$edited_Item->get_ItemType())) {	// Set a tab type for edited/viewed item:
    $tab = 'type';
    if ($tab_type = get_tab_by_item_type_usage($ItemType->usage)) {	// Only if tab exists for current item type usage:
        $tab_type = $tab_type[0];
    } else {
        $tab_type = 'all';
    }
...
if (! empty($tab) && $tab == 'type') { // Set a path from dynamic tabs
    $AdminUI->append_path_level('type_' . str_replace(' ', '_', utf8_strtolower($tab_type)));
} else { // Set a path to selected tab
    $AdminUI->append_path_level(empty($tab) ? 'full' : $tab);
}

if ((isset($tab) && in_array($tab, ['full', 'summary', 'type', 'tracker'])) || strpos($action, 'edit') === 0) { // Init JS to autcomplete the user logins
...
if ($action == 'view' || $action == 'history_compare' || strpos($action, 'edit') !== false || strpos($action, 'new') !== false || $action == 'copy') {	// Initialize js to autocomplete usernames in post/comment form
    init_autocomplete_usernames_js();
    // Require colorbox js:
    require_js_helper('colorbox');
}

if (in_array($action, ['new', 'new_version', 'copy', 'create_edit', 'create_link', 'create', 'create_publish', 'edit', 'update_edit', 'update', 'update_publish', 'extract_tags'])) { // Set manual link for edit expert mode
    ...
    default:
        ...
        if ($Blog->get('notes')) {
            ...
            if (check_user_perm('blog_properties', 'edit', false, $blog)) {
                $edit_link = action_icon(TB_('Edit') . '...', 'edit_button', $admin_url . '?ctrl=coll_settings&amp;tab=general&amp;blog=' . $Blog->ID, ' ' . TB_('Edit') . '...', 3, 4, [
