...
if (param('ityp_ID', 'integer', '', true)) { // Load itemtype from cache:
    $ItemtypeCache = &get_ItemTypeCache();
    if (($edited_Itemtype = &$ItemtypeCache->get_by_ID($ityp_ID, false)) === false) {	// We could not find the post type to edit:
...
switch ($action) {
    case 'new':
        ...
        if (! isset($edited_Itemtype)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record Itemtype and create similar
        ...
        if ($edited_Itemtype->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'update_edit':
        ...
        if ($edited_Itemtype->load_from_Request()) {	// We could load data from form without errors:
            ...
            if ($action == 'update_edit') {	// Redirect to the edit form back:
                $redirect_to = $admin_url . '?ctrl=itemtypes&action=edit&ityp_ID=' . $edited_Itemtype->ID . '&blog=' . $blog;
            } else {	// Redirect to the item types list:
    ...
    case 'delete':
        ...
        if (($item_type_blog_ID = array_search($edited_Itemtype->ID, $default_ids)) !== false) { // is default post type of the blog
            ...
            if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
                ...
                if (! $edited_Itemtype->check_delete(sprintf(TB_('Cannot delete Post Type &laquo;%s&raquo;'), $edited_Itemtype->dget('name')))) {	// There are restrictions:
                    $action = 'view';
                }
            }
        }
    ...
    case 'disable':
        ...
        if ($edited_Itemtype) { // Do only when item type exists in DB
            if ($action == 'enable') { // Enable item type for the collection
            ...
            } elseif ($Blog->can_be_item_type_disabled($edited_Itemtype->ID, true)) { // Disable item type for the collection only if it is allowed:
...
switch ($action) {
    ...
    case 'select_custom_fields':
        ...
        $custom_fields = empty($custom_fields) ? [] : explode(',', $custom_fields);
