...
if (param('pst_ID', 'integer', '', true)) {	// Load ItemStatus from cache::
    $ItemStatusCache = &get_ItemStatusCache();
    if (($edited_ItemStatus = &$ItemStatusCache->get_by_ID($pst_ID, false)) === false) {	// We could not find the post status to edit:
...
switch ($action) {
    case 'new':
        ...
        if (! isset($edited_ItemStatus)) {	// We don't have a model to use, start with blank object:
    ...
    case 'create_copy': // Record ItemStatus and create similar
        ...
        if ($edited_ItemStatus->load_from_Request()) {	// We could load data from form without errors:
            // Insert in DB:
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_ItemStatus->check_delete(sprintf(TB_('Cannot delete Post Status &laquo;%s&raquo;'), $edited_ItemStatus->dget('name')))) {	// There are restrictions:
                $action = 'view';
            }
        }
        break;
