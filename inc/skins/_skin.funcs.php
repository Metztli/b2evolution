...
function skin_init($disp)
{
    ...
    if (in_array($disp, ['threads', 'messages', 'contacts', 'msgform', 'user', 'profile', 'avatar', 'pwdchange', 'userprefs', 'subs', 'register_finish', 'visits', 'closeaccount']) &&
        $msg_Blog = &get_setting_Blog('msg_blog_ID') &&
        $Blog->ID != $msg_Blog->ID) {	// Redirect to collection which should be used for profile/messaging pages:
        header_redirect($msg_Blog->get($disp . 'url', [
            'glue' => '&',
        ]));
        // Exit here.
    }
    ...
    switch ($disp) {
        ...
        case 'mustread':
            ...
            if ($disp == 'flagged' && ! is_logged_in()) {	// Forbid access to flagged content for not logged in users:
                global $disp;
                $disp = '403';
            ...
            if ($disp == 'mustread') {	// Check access to "must read" content:
                if (! is_logged_in()) {	// Forbid access to "must read" content for not logged in users:
                    header_redirect(get_login_url('no access to must read content', null, false, null, 'access_requires_loginurl'), 302);
                    // Exit here.
                }
                if (! is_pro()) {	// Forbid access for not PRO version:
                ...
                if (! $Blog->get_setting('track_unread_content')) {	// Forbid access to "must read" content if collection doesn't track unread content:
                    global $disp;
                    $disp = '404';
            ...
            if ($disp == 'terms') {	// Initialize the redirect param to know what page redirect after accepting of terms:
            ...
            if (! empty($MainList) && $MainList->single_post &&
                $single_Item = &mainlist_get_item()) {	// If we are currently viewing a single post
    ...
    switch ($disp) {
        ...
        case 'widget_page':
        case 'terms':
            if ($disp == 'terms' && ! $Item) {	// Wrong post ID for terms page:
                global $disp;
                $disp = '404';
            ...
            if ((! $preview) && (empty($Item))) { // No Item, incorrect request and incorrect state of the application, a 404 redirect should have already happened
            ...
            if (! $preview) { // Check if item has a goal to insert a hit into DB
                $Item->check_goal();
            }
            ...
            if (! $preview &&
                ...
                $Item->get('url') != '') {	// Use post link to URL as an External Canonical URL:
                ...
                if ($item_Blog->get_setting('allow_crosspost_urls')) {	// If non-canonical URL is allowed for cross-posted items,
                    // try to get a canonical URL in the current collection even it is not main/canonical collection of the Item:
                    $canonical_url = $Item->get_permanent_url('', $Blog->get('url'), '&', [], $Blog->ID);
                } else {	// If non-canonical URL is allowed for cross-posted items, then only get canonical URL in the main collection:
                ...
                if (preg_match('|[&?](revision=(p?\d+))|', $ReqURI, $revision_param)
                        ...
                        && $item_revision = $Item->get_revision($revision_param[2])) { // A revision of the post, keep only this param and discard all others:
                ...
                if (! $canonical_is_same_url && in_array($Blog->get_setting('single_links'), ['subchap', 'chapters'])) {	// If current URL is not same as first detected category then try to check all other categories from the current collection:
                    $item_chapters = $Item->get_Chapters();
                    foreach ($item_chapters as $item_Chapter) {	// Try to find in what category the Item may has the same canonical url as current requested URL:
                        if (! $item_Blog->get_setting('allow_crosspost_urls') &&
                            $item_Blog->ID != $item_Chapter->get('blog_ID')) {	// Don't allow to use URL of categories from cross-posted collection if it is restricted:
                            continue;
                        }
                        $cat_canonical_url = $Item->get_permanent_url('', $Blog->get('url'), '&', [], $Blog->ID, $item_Chapter->ID);
                        // Keep ONLY allowed params from current URL in the canonical URL by configs AND Item's switchable params:
                        $cat_canonical_url = url_keep_canonicals_params($cat_canonical_url, '&', array_keys($Item->get_switchable_params()));
                        if ($canonical_is_same_url = is_same_url($ReqURL, $cat_canonical_url, $Blog->get_setting('http_protocol') == 'allow_both')) {	// We have found the same URL, stop find another and stay on the current page without redirect:
                            break;
                        }
                    }
                }

                if (! $canonical_is_same_url) {	// The requested URL does not look like the canonical URL for this post...
                    // url difference was resolved
                    $url_resolved = false;
                    // Check if the difference is because of an allowed post navigation param
                    if (preg_match('|[&?]cat=(\d+)|', $ReqURI, $cat_param)) { // A category post navigation param is set
                        $extended_url = '';
                        if (($post_navigation == 'same_category') && (isset($cat_param[1]))) { // navigatie through posts from the same category
                            $category_ids = postcats_get_byID($Item->ID);
                            if (in_array($cat_param[1], $category_ids)) { // cat param is one of this Item categories
                    ...
                    if (preg_match('|[&?]tag=([^&A-Z]+)|', $ReqURI, $tag_param)) { // A tag post navigation param is set
                        $extended_url = '';
                        if (($post_navigation == 'same_tag') && (isset($tag_param[1]))) { // navigatie through posts from the same tag
                            $tag_names = $Item->get_tags();
                            if (in_array($tag_param[1], $tag_names)) { // tag param is one of this Item tags
                    ...
                    if (! $url_resolved &&
                        ...
                        // EXITED.
                    } elseif ($Blog->get_setting('relcanonical_item_urls')) {	// Use rel="canoncial" with MAIN canoncial URL:
                        add_headline('<link rel="canonical" href="' . $main_canonical_url . '" />');
                    }
                } elseif ($Blog->get_setting('self_canonical_item_urls')) {	// Use self-referencing rel="canonical" tag with MAIN canoncial URL:
                    add_headline('<link rel="canonical" href="' . $main_canonical_url . '" />');
                }
            }

            if ($Blog->get_setting('single_noindex') || ! $MainList->result_num_rows) {	// We prefer robots not to index these pages,
        ...
        case 'download':
            if (empty($Item)) { // No Item, incorrect request and incorrect state of the application, a 404 redirect should have already happened
                debug_die('Invalid page URL!');
            }

            $seo_page_type = 'Download page';
            if ($Blog->get_setting($disp . '_noindex')) { // We prefer robots not to index these pages:
                $robots_index = false;
            }
            if (! $Blog->get_setting('download_enable')) {	// If download is disabled for current Collection:
                global $disp;
                $disp = '404';
            ...
            add_headline('<meta http-equiv="refresh" content="' . intval($Blog->get_setting('download_delay'))
                . '; url=' . $download_Link->get_download_url([
                    'type' => 'action',
                ]) . '" />');
            break;

        case 'posts':
            if (! $Blog->get_setting('postlist_enable')) {	// If post list is disabled for current Collection:
                global $disp;
                $disp = '404';
            ...
            $is_first_page = (empty($active_filters) || array_diff($active_filters, ['posts']) == []);
            $is_next_pages = (! $is_first_page && array_diff($active_filters, ['posts', 'page']) == []);

            if (($is_first_page && ! $is_front_disp) || $is_next_pages) {	// This is first(but not front disp) or next pages of disp=posts:
                // Do we need to handle the canoncial url?
                if (($Blog->get_setting('canonical_posts') && $redir == 'yes')
                    || $Blog->get_setting('relcanonical_posts')
                    || $Blog->get_setting('self_canonical_posts')) {	// Check if the URL was canonical:
                    $canonical_url = $Blog->get('url', [
                        'glue' => '&',
                    ]);
                    if (! $is_front_disp) {	// Append disp param only when this disp is not used as front page, because front page hides disp param in URL:
                        $canonical_url = url_add_param($canonical_url, 'disp=posts', '&');
                    }
                    if ($is_next_pages) {	// Set param for paged url:
                    ...
                    if (! is_same_url($ReqURL, $canonical_url, $Blog->get_setting('http_protocol') == 'allow_both')) {	// We are not on the canonical blog url:
                        if ($Blog->get_setting('canonical_posts') && $redir == 'yes') {	// REDIRECT TO THE CANONICAL URL:
                            header_redirect($canonical_url, (empty($display_containers) && empty($display_includes)) ? 301 : 303);
                        } elseif ($Blog->get_setting('relcanonical_posts')) {	// Use link rel="canoncial":
                            add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
                        }
                    } elseif ($Blog->get_setting('self_canonical_posts') &&
                            ! ($is_front_disp && $Blog->get_setting('self_canonical_homepage'))) {	// Use self-referencing rel="canonical" tag,
            ...
            if ($is_first_page) {	// This is first page of disp=posts:
                $disp_detail = 'posts-default';
                $seo_page_type = 'First posts page';
                if ($Blog->get_setting('posts_firstpage_noindex')) {	// We prefer robots not to index archive pages:
                    $robots_index = false;
                }
            } elseif (! empty($active_filters)) {	// The current page is being filtered...
                if ($is_next_pages) { // This is just a follow "paged" page
                    $disp_detail = 'posts-next';
                    $seo_page_type = 'Next page';

                    if (has_featured_Item()) {	// If current next page has Intro post:
                        $disp_detail .= '-intro';
                        $robots_index = ! $Blog->get_setting('paged_intro_noindex');
                    } else {	// If current next page has no Intro post:
                        $disp_detail .= '-nointro';
                        $robots_index = ! $Blog->get_setting('paged_noindex');
                    }
                } elseif (array_diff($active_filters, ['cat_single', 'cat_array', 'cat_modifier', 'cat_focus', 'posts', 'page']) == []) { // This is a category page
                    $disp_detail = 'posts-cat';
                    $seo_page_type = 'Category page';
                    if ($Blog->get_setting('chapter_noindex')) {	// We prefer robots not to index category pages:
                        $robots_index = false;
                    }
                    ...
                    if ((
                        empty($catsel) || // 'catsel' filter is not defined
                        ...
                        if (($Blog->get_setting('canonical_cat_urls') && $redir == 'yes')
                            ...
                            || $Blog->get_setting('self_canonical_cat_urls')) { // Check if the URL was canonical:
                            if (empty($Chapter) && isset($MainList->filters['cat_array'][0])) {	// Try to get Chapter from filters:
                                $Chapter = &$ChapterCache->get_by_ID($MainList->filters['cat_array'][0], false, false);
                            }

                            if (! empty($Chapter)) {
                                if ($Chapter->parent_ID) {	// This is a sub-category page (i-e: not a level 1 category)
                                ...
                                // Keep ONLY allowed params from current URL in the canonical URL by configs:
                                $canonical_url = url_keep_canonicals_params($canonical_url);
                                if (! is_same_url($ReqURL, $canonical_url, $Blog->get_setting('http_protocol') == 'allow_both')) {	// fp> TODO: we're going to lose the additional params, it would be better to keep them...
                                    // fp> what additional params actually?
                                    if ($Blog->get_setting('canonical_cat_urls') && $redir == 'yes') {	// REDIRECT TO THE CANONICAL URL:
                                        header_redirect($canonical_url, true);
                                    } elseif ($Blog->get_setting('relcanonical_cat_urls')) {	// Use rel="canonical":
                                        add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
                                    }
                                } elseif ($Blog->get_setting('self_canonical_cat_urls')) {	// Use self-referencing rel="canonical" tag:
                                    add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
                                }
                            }
                        }

                        if ($post_navigation == 'same_category') { // Category is set and post navigation should go through the same category, set navigation target param
                            $MainList->nav_target = $cat;
                        }

                        if (has_featured_Item()) {	// If current category has Intro post:
                            $disp_detail .= '-intro';
                            $robots_index = ! $Blog->get_setting('chapter_intro_noindex');
                        } else {	// If current category has no Intro post:
                            $disp_detail .= '-nointro';
                        }

                        if (empty($Chapter)) {	// If the requested chapter was not found display 404 page:
                ...
                } elseif (array_diff($active_filters, ['tags', 'posts', 'page']) == []) { // This is a tag page
                    ...
                    if (($Blog->get_setting('canonical_tag_urls') && $redir == 'yes')
                        ...
                        || $Blog->get_setting('self_canonical_tag_urls')) { // Check if the URL was canonical:
                        ...
                        if (! is_same_url($ReqURL, $canonical_url, $Blog->get_setting('http_protocol') == 'allow_both')) {
                            if ($Blog->get_setting('canonical_tag_urls') && $redir == 'yes') {	// REDIRECT TO THE CANONICAL URL:
                                header_redirect($canonical_url, true);
                            } elseif ($Blog->get_setting('relcanonical_tag_urls')) {	// Use rel="canoncial":
                                add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
                            }
                        } elseif ($Blog->get_setting('self_canonical_tag_urls')) {	// Use self-referencing rel="canonical" tag:
                            add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
                        }
                    }

                    $tag = $MainList->get_active_filter('tags');
                    if ($post_navigation == 'same_tag' && ! empty($tag)) { // Tag is set and post navigation should go through the same tag, set navigation target param
                        $MainList->nav_target = $tag;
                    }

                    if (has_featured_Item()) {	// If current tag has Intro post:
                        $disp_detail .= '-intro';
                        $robots_index = ! $Blog->get_setting('tag_intro_noindex');
                    } else {	// If current tag has no Intro post:
                        $disp_detail .= '-nointro';
                        $robots_index = ! $Blog->get_setting('tag_noindex');
                    }
                } elseif (array_diff($active_filters, ['ymdhms', 'week', 'posts', 'page']) == []) { // fp> added 'posts' 2009-05-19; can't remember why it's not in there
                    ...
                    if (($Blog->get_setting('canonical_archive_urls') && $redir == 'yes')
                        ...
                        || $Blog->get_setting('self_canonical_archive_urls')) { // Check if the URL was canonical:
                        $canonical_url = $Blog->gen_archive_url(substr($m, 0, 4), substr($m, 4, 2), substr($m, 6, 2), $w, '&', $MainList->get_active_filter('page'));
                        // Keep ONLY allowed params from current URL in the canonical URL by configs:
                        $canonical_url = url_keep_canonicals_params($canonical_url);
                        if (! is_same_url($ReqURL, $canonical_url, $Blog->get_setting('http_protocol') == 'allow_both')) {
                            if ($Blog->get_setting('canonical_archive_urls') && $redir == 'yes') {	// REDIRECT TO THE CANONICAL URL:
                                header_redirect($canonical_url, true);
                            } elseif ($Blog->get_setting('relcanonical_archive_urls')) {	// Use rel="canoncial":
                                add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
                            }
                        } elseif ($Blog->get_setting('self_canonical_archive_urls')) {	// Use self-referencing rel="canonical" tag:
                            add_headline('<link rel="canonical" href="' . $canonical_url . '" />');
                        }
                    }

                    if ($Blog->get_setting('archive_noindex')) {	// We prefer robots not to index archive pages:
                        $robots_index = false;
                    }
                } else {	// Other filtered pages:
                    // pre_dump( $active_filters );
                    $disp_detail = 'posts-filtered';
                    $seo_page_type = 'Other filtered page';

                    if (has_featured_Item()) {	// If current filtered page has Intro post:
                        $disp_detail .= '-intro';
                        $robots_index = ! $Blog->get_setting('filtered_intro_noindex');
                    } else {	// If current filtered page has no Intro post:
                        $disp_detail .= '-nointro';
        ...
        case 'search':
            ...
            if ($Blog->get_setting('filtered_noindex')) {	// We prefer robots not to index these pages:
                $robots_index = false;
            }
            if (! $Blog->get_setting('search_enable')) {	// If search is disabled for current Collection:
                global $disp;
                $disp = '404';
        ...
        case 'msgform':
            ...
            } elseif (! empty($comment_id)) { // comment id is set, try to get comment author user
                $CommentCache = &get_CommentCache();
                if ($Comment = &$CommentCache->get_by_ID($comment_id, false)) {
                    $recipient_User = &$Comment->get_author_User();
                    if (empty($recipient_User) && ($Comment->allow_msgform) && (is_email($Comment->get_author_email()))) { // set allow message form to email because comment author (not registered) accepts email
                        $allow_msgform = 'email';
                    }
                }
            } else { // Recipient was not defined, try set the blog owner as recipient
                global $Collection, $Blog;
                if (empty($Blog)) { // Blog is not set, this is an invalid request
                    debug_die('Invalid send message request!');
                }
                $recipient_User = $Blog->get_owner_User();
            }

            if ($recipient_User) { // recipient User is set
                ...
                if ($msg_type == 'email' && $recipient_User->get_msgform_possibility(null, 'email') != 'email') { // User doesn't want to receive email messages, Restrict if this was requested by wrong url:
                    $msg_type = '';
                }

                if ($allow_msgform == 'login') { // user must login first to be able to send a message to this User
                    ...
                    header_redirect(url_add_param($Blog->get('loginurl', [
                        'glue' => '&',
                    ]), 'redirect_to=' . rawurlencode($redirect_to), '&'));
                    // Exit here.
                } elseif (($allow_msgform == 'PM') && check_user_status('can_be_validated')) { // user is not activated
                    if ($recipient_User->accepts_email()) { // recipient User accepts email allow to send email
                        $allow_msgform = 'email';
                        $msg_type = 'email';
                    ...
                    } else { // Redirect to the activate info page for not activated users
                ...
                } elseif (($msg_type == 'PM') && ($allow_msgform == 'email')) { // only email is allowed but user expect private message form
                ...
                if (empty($recipient_id) && ! empty($recipient_User)) {	// Set recipient user param when it is not specified in GET/POST:
                    set_param('recipient_id', $recipient_User->ID);
                }
            }

            if ($allow_msgform == null) { // should be Prevented by UI
            ...
            if ($allow_msgform == 'PM' || $allow_msgform == 'email') { // Some message form is available
                // Get the suggested subject for the email:
                if (empty($subject)) { // no subject provided by param:
                    global $DB;
            ...
            if ($msg_Blog = &get_setting_Blog('msg_blog_ID') && $Blog->ID != $msg_Blog->ID) { // Redirect to special blog for messaging actions if it is defined in general settings
                header_redirect($msg_Blog->get('msgformurl', [
                    'glue' => '&',
                ]));
            }
        ...
        case 'threads':
            switch ($disp) {
                case 'messages':
                    ...
                    if (! is_logged_in()) { // Redirect to the login page for anonymous users
                    ...
                    if (! check_user_status('can_view_messages')) { // user status does not allow to view messages
                        if (check_user_status('can_be_validated')) { // user is logged in but his/her account is not activate yet
                    ...
                    if (! check_user_perm('perm_messaging', 'reply')) { // Redirect to the blog url for users without messaging permission
                        $Messages->add('You are not allowed to view Messages!');
                        header_redirect($Blog->gen_blogurl(), 302);
                        // will have exited
                    }

                    if (! empty($thrd_ID)) { // if this thread exists and current user is part of this thread update status because won't be any unread messages on this conversation
                    ...
                    if (($unsaved_message_params = get_message_params_from_session()) !== null) { // set Message and Thread saved params from Session
                ...
                case 'contacts':
                    ...
                    if (! check_user_status('can_view_contacts')) { // user is logged in, but his status doesn't allow to view contacts
                        if (check_user_status('can_be_validated')) { // user is logged in but his/her account was not activated yet
                    ...
                    if (has_cross_country_restriction('any') && empty($current_User->ctry_ID)) { // User may browse/contact other users only from the same country
                    ...
                    switch ($action) {
                        case 'add_user': // Add user to contacts list
                            ...
                            if ($user_ID > 0) { // Add user to contacts
                                if (create_contacts_user($user_ID)) { // Add user to the group
                                    $group_ID = param('group_ID', 'string', '');
                                    if ($result = create_contacts_group_users($group_ID, $user_ID, 'group_ID_combo')) { // User has been added to the group
                                        $Messages->add(sprintf(T_('User has been added to the &laquo;%s&raquo; group.'), $result['group_name']), 'success');
                                    } else { // User has been added ONLY to the contacts list
                                        $Messages->add('User has been added to your contacts.', 'success');
                                    }
                                }
                                header_redirect($Blog->get('userurl', [
                                    'user_ID' => $user_ID,
                                ]));
                            }
                            break;
                        ...
                        case 'remove_user': // Remove user from contacts group
                            ...
                            if ($user_ID > 0 && $group_ID > 0) { // Remove user from selected group
                                if (remove_contacts_group_user($group_ID, $user_ID)) { // User has been removed from the group
                                    if ($view == 'contacts') { // Redirect to the contacts list
                                        header_redirect($Blog->get('contactsurl', [
                                            'glue' => '&',
                                        ]));
                                    } else { // Redirect to the user profile page
                                        header_redirect($Blog->get('userurl', [
                                            'user_ID' => $user_ID,
                                        ]));
                                    }
                        ...
                        case 'add_group': // Add users to the group
                            ...
                            if ($result = create_contacts_group_users($group, $users)) {	// Users have been added to the group
                                ...
                                $redirect_to = $Blog->get('contactsurl', [
                                    'glue' => '&',
                                ]);
                        ...
                        case 'rename_group': // Rename the group
                            ...
                            if (rename_contacts_group($group_ID)) {
                                ...
                                $redirect_to = url_add_param($Blog->get('contactsurl', [
                                    'glue' => '&',
                                ]), 'g=' . $group_ID, '&');
                                if ($item_ID > 0) {
                                    $redirect_to = url_add_param($redirect_to, 'item_ID=' . $item_ID, '&');
                    ...
                    modules_call_method('switch_contacts_actions', [
                        'action' => $action,
                    ]);
                    break;
                ...
                case 'threads':
                    ...
                    if (! check_user_status('can_view_threads')) { // user status does not allow to view threads
                        if (check_user_status('can_be_validated')) { // user is logged in but his/her account is not activate yet
                            $Messages->add(T_('You must activate your account before you can read & send messages. <b>See below:</b>'));
                            header_redirect(get_activate_info_url(), 302);
                            // will have exited
                    ...
                    if ($action == 'new') { // Before new message form is displayed ...
                        if (has_cross_country_restriction('contact') && empty($current_User->ctry_ID)) { // Cross country contact restriction is enabled, but user country is not set yet
                            $Messages->add(T_('Please specify your country before attempting to contact other users.'));
                            header_redirect(get_user_profile_url());
                        } elseif (check_create_thread_limit(true)) { // don't allow to create new thread, because the new thread limit was already reached
                    ...
                    switch ($action) {
                        case 'new':
                            ...
                            modules_call_method('update_new_thread', [
            ...
            if ($msg_Blog = &get_setting_Blog('msg_blog_ID') && $Blog->ID != $msg_Blog->ID) { // Redirect to special blog for messaging actions if it is defined in general settings
                ...
                if (! empty($thrd_ID)) { // Don't forget the important param on redirect
        ...
        case 'content_requires_login':
            ...
            if (is_logged_in()) {	// Don't display this page for already logged in user:
                global $Blog;
                header_redirect($Blog->get('url'));
                // Exit here.
            }

            if ($Settings->get('http_auth_require') && ! isset($_SERVER['PHP_AUTH_USER'])) {	// Require HTTP authentication:
                header('WWW-Authenticate: Basic realm="b2evolution"');
                header('HTTP/1.0 401 Unauthorized');
            }

            if (! empty($login_mode) && $login_mode == 'http_basic_auth') {	// Display this error if user already tried to log in by HTTP basic authentication and it was failed:
                $Messages->add(T_('Wrong Login/Password provided by browser (HTTP Auth).'), 'error');
            }

            if ($disp == 'content_requires_login') {	// Set default details for this disp:
        ...
        case 'login':
            ...
            if (is_logged_in()) { // User is already logged in
                if (check_user_status('can_be_validated')) { // account is not active yet, redirect to the account activation page
            ...
            if ($login_Blog = &get_setting_Blog('login_blog_ID', $Blog) && $Blog->ID != $login_Blog->ID) { // Redirect to special blog for login/register actions if it is defined in general settings
                header_redirect($login_Blog->get('loginurl', [
                    'glue' => '&',
                ]));
            }
        ...
        case 'register':
            ...
            if (is_logged_in()) {	// If user is logged in the register form should not be displayed,
            ...
            if ($login_Blog = &get_setting_Blog('login_blog_ID', $Blog) && $Blog->ID != $login_Blog->ID) { // Redirect to special blog for login/register actions if it is defined in general settings
                header_redirect($login_Blog->get('registerurl', [
                    'glue' => '&',
                ]));
            }
            ...
            if ($comment_ID > 0) {	// Suggestion to register for anonymous user:
                $CommentCache = &get_CommentCache();
                $Comment = &$CommentCache->get_by_ID($comment_ID, false, false);
                if ($Comment && $Comment->get('author_email') !== '') {	// If comment is really from anonymous user:
        ...
        case 'lostpassword':
            ...
            if (is_logged_in()) { // If user is logged in the lost password form should not be displayed. In this case redirect to the blog home page.
                $Messages->add(T_('You are already logged in') . '.', 'note');
                header_redirect($Blog->gen_blogurl(), false);
            }

            if ($login_Blog = &get_setting_Blog('login_blog_ID', $Blog) && $Blog->ID != $login_Blog->ID) { // Redirect to special blog for login/register actions if it is defined in general settings
                header_redirect($login_Blog->get('lostpasswordurl', [
                    'glue' => '&',
                ]));
            }
        ...
        case 'activateinfo':
            ...
            if (! check_user_status('can_be_validated')) { // don't display activateinfo screen
                $after_email_validation = $Settings->get('after_email_validation');
                if ($after_email_validation == 'return_to_original') { // we want to return to original page after account activation
                    // check if Session 'activateacc.redirect_to' param is still set
                    $redirect_to = $Session->get('core.activateacc.redirect_to');
                    if (empty($redirect_to)) { // Session param is empty try to get general redirect_to param
                        $redirect_to = param('redirect_to', 'url', '');
                    } else { // cleanup validateemail.redirect_to param from session
                        $Session->delete('core.activateacc.redirect_to');
                    }
                } else { // go to after email validation url which is set in the user general settings form
                    $redirect_to = $after_email_validation;
                }
                if (empty($redirect_to) || preg_match('#disp=activateinfo#', $redirect_to)) { // redirect_to is pointing to the activate info display or is empty
            ...
            if ($login_Blog = &get_setting_Blog('login_blog_ID', $Blog) && $Blog->ID != $login_Blog->ID) { // Redirect to special blog for login/register actions if it is defined in general settings
                header_redirect($login_Blog->get('activateinfourl', [
                    'glue' => '&',
                ]));
            }
            break;
        ...
        case 'avatar':
            $action = param_action();
            if ($action == 'crop' && is_logged_in()) { // Check data for crop action:
                global $current_User, $cropped_File;
                $file_ID = param('file_ID', 'integer');
                if (! ($cropped_File = $current_User->get_File_by_ID($file_ID, $error_code))) { // Current user cannot crop this file
                    set_param('action', '');
                }
            }
            // no break
        ...
        case 'userprefs':
        case 'subs':
            if ($disp == 'pwdchange' || $disp == 'register_finish') {	// Check and redirect if current URL must be used as https instead of http:
                check_https_url('login');
            }

            $seo_page_type = 'Special feature page';
            if ($Blog->get_setting('special_noindex')) {	// We prefer robots not to index these pages:
                $robots_index = false;
            }
            ...
            if ($demo_mode && ($current_User->ID <= 7)) {	// Demo mode restrictions: users created by install process cannot be edited:
        ...
        case 'users':
            ...
            if (! $Blog->get_setting('userdir_enable')) {	// If user directory is disabled for current Collection:
                global $disp;
                $disp = '404';
        ...
        case 'visits':
            ...
            if (! is_logged_in() || ! $Settings->get('enable_visit_tracking')) {	// Check if visit tracking is enabled and the user is logged in before allowing profile visit display:
                global $disp;
                $disp = '403';
        ...
        case 'user':
            ...
            if ($Blog->get_setting('canonical_user_urls') && $redir == 'yes') {	// Check if current user profile URL can be canonical:
                if (empty($user_ID) && is_logged_in()) {	// Use ID of current User for proper redirect to canonical url like '/user:admin':
                    global $current_User;
                    $user_ID = $current_User->ID;
                }
                $canonical_url = $Blog->get('userurl', [
                    'user_ID' => $user_ID,
                    'glue' => '&',
                ]);
                // Keep ONLY allowed params from current URL in the canonical URL by configs:
                $canonical_url = url_keep_canonicals_params($canonical_url);
                if (! is_same_url($ReqURL, $canonical_url, $Blog->get_setting('http_protocol') == 'allow_both')) {	// Redirect to canonical user profile URL:
                    header_redirect($canonical_url, true);
                }
            }
        ...
        case 'anonpost':
            ...
            if (is_logged_in()) {	// The logged in user has another page to create a post:
                header_redirect(url_add_param($Blog->get('url', [
                    'glue' => '&',
                ]), 'disp=edit', '&'), 302);
                // will have exited
            } elseif (! $Blog->get_setting('post_anonymous')) {	// Redirect to the login page if current collection doesn't allow to post by anonymous user:
            ...
            if ($Chapter && $Chapter->get_ItemType() === false) {	// Don't allow to post in category without default Item Type:
        ...
        case 'proposechange':
            ...
            if ($disp == 'edit') {	// Check permission to create/edit post:
                check_item_perm_edit($post_ID);
            }

            if (! blog_has_cats($Blog->ID)) { // No categories are in this blog
                $error_message = T_('Since this blog has no categories, you cannot post into it.');
                if (check_user_perm('blog_cats', 'edit', false, $Blog->ID)) { // If current user has a permission to create a category
            ...
            if ($cat > 0 &&
                ...
                ($selected_Chapter->get_ItemType() === false)) {	// Don't allow to post in category without default Item Type:
                $Messages->add(T_('You cannot post here'), 'error');
        ...
        case 'edit_comment':
            ...
            if (empty($comment_ID)) { // Can't edit a not exisiting comment
            ...
            if (! check_user_perm('comment!CURSTATUS', 'edit', false, $edited_Comment)) { // If User has no permission to edit comments with this comment status:
        ...
        case 'usercomments':
            ...
            if ($user_ID === null && is_logged_in()) {	// Use current logged in User if it is not specified in param:
                $user_ID = $current_User->ID;
            }
            ...
            if (is_logged_in() && (! check_user_status('can_view_user', $user_ID))) { // user is logged in, but his/her status doesn't permit to view user profile
                if (check_user_status('can_be_validated')) { // user is logged in but his/her account is not active yet
                    // Redirect to the account activation page
            ...
            if ($disp == 'useritems') { // Init items list
        ...
        case 'comments':
            if (! $Blog->get_setting('comments_latest')) { // If latest comments page is disabled - Display 404 page with error message
        ...
        case 'closeaccount':
            global $disp;
            if (! $Settings->get('account_close_enabled')) {	// If an account closing page is disabled - Display 404 page with error message:
                $disp = is_logged_in() ? 'profile' : 'login';
                $Messages->add(T_('The account closing feature is disabled.'), 'error');
            } elseif (! is_logged_in() && ! $Session->get('account_closing_success')) {	// Don't display this message for not logged in users, except of one case to display a bye message after account closing:
                $disp = 'login';
                $Messages->add(T_('You must log in before you can close your account.'), 'error');
            } elseif (check_user_perm('users', 'edit', false)) {	// Don't allow admins close own accounts from front office:
                $disp = 'profile';
            ...
            } elseif ($Session->get('account_closing_success')) { // User has closed the account
                ...
                if (is_logged_in()) { // log out current User
                    logout();
                }
            }
            break;
        ...
        case 'compare':
            ...
            if (! empty($items)) {	// Check if at least one item exist in DB:
                $ItemCache = &get_ItemCache();
                $items = $ItemCache->load_list(explode(',', $items));
            }
            if (empty($items)) {	// Display 404 page when no items to compare:
                global $disp;
                $disp = '404';
    ...
    if (($disp == 'posts' || $disp == 'front') &&
        ($featured_intro_Item = &get_featured_Item($disp, null, true))) {	// We assume the current user will have read the entire intro Item and all its current comments:
    ...
    if ($version_Item = &get_current_Item()) {	// If current Item is detected
        $other_version_items = $version_Item->get_other_version_items();
        if (! empty($other_version_items)) {	// If at least one other version exists for current Item:
            // Add also current Item as first:
            array_unshift($other_version_items, $version_Item);
            $other_version_locales = [];
            $version_lang_keys = [];
            foreach ($other_version_items as $o => $other_version_Item) {	// Check to exclude what items cannot be displayed for hreflang tag:
                if (in_array($other_version_Item->get('locale'), $other_version_locales) ||
                    ! $other_version_Item->can_be_displayed()) {	// Don't add hreflang tag with same locale
            ...
            if (count($other_version_items) > 1) {	// Add hreflang tag only when at least two Items can be displayed:
    ...
    if ($Blog->get_setting('front_disp') == $disp) {	// This is the default/front collection page:
        $seo_page_type = 'Default page';
        if ($Blog->get_setting('default_noindex')) {	// We prefer robots not to index archive pages:
            $robots_index = false;
        }
    }
    ...
    if ($check_browser_version && $Hit->get_browser_version() > 0 && $Hit->is_IE(9, '<')) {	// Display info message if browser IE < 9 version and it is allowed by config var:
        global $debug;
    ...
    if ($Messages->count() && (! empty($PageCache))) { // Abort PageCache collect
...
function &get_site_Skin()
{
    ...
    if (! isset($site_Skin)) {	// Initialize site Skin only first time:
        global $Settings;
        if (! $Settings->get('site_skins_enabled')) {	// Site skins are not enabled:
            $site_Skin = null;
            return $site_Skin;
        }

        global $Session;
        if (! empty($Session)) {	// Get site skin Id depending on current session:
            if ($Session->is_mobile_session()) {	// Mobile session:
                $skin_ID = $Settings->get('mobile_skin_ID');
            } elseif ($Session->is_tablet_session()) {	// Tablet session:
                $skin_ID = $Settings->get('tablet_skin_ID');
            } elseif ($Session->is_alt_session()) {	// Session for alternative skin:
                $skin_ID = $Settings->get('alt_skin_ID');
            }
        }
        if (empty($skin_ID)) {	// Use normal skin ID by default when mobile, tablet and alt skins are not defined for site:
...
{
    if ($site_Skin = &get_site_Skin()) {	// Initialize site skin:
...
{
    if (is_ajax_content()) {	// AJAX request
        if (empty($skin_name)) {	// Don't initialize without skin name
            return false;
        }
...
function skin_init_global_vars()
{
    ...
    $credit_links = [];
    $francois_links = [
        'fr' => ['http://fplanque.net/', [[78, 'Fran&ccedil;ois'],  [100, 'Francois']]],
        '' => ['http://fplanque.com/', [[78, 'Fran&ccedil;ois'],  [100, 'Francois']]],
    ];
    $fplanque_links = [
        'fr' => ['http://fplanque.net/', [[78, 'Fran&ccedil;ois Planque'],  [100, 'Francois Planque']]],
        '' => ['http://fplanque.com/', [[78, 'Fran&ccedil;ois Planque'],  [100, 'Francois Planque']]],
    ];
    $skin_links = [
        '' => ['http://skinfaktory.com/', [[15, 'b2evo skin'], [20, 'b2evo skins'], [35, 'b2evolution skin'], [40, 'b2evolution skins'], [55, 'Blog skin'], [60, 'Blog skins'], [75, 'Blog theme'], [80, 'Blog themes'], [95, 'Blog template'], [100, 'Blog templates']]],
    ];
    $skinfaktory_links = [
        '' => [
            [73, 'http://evofactory.com/', [[61, 'Evo Factory'], [68, 'EvoFactory'], [73, 'Evofactory']]],
            [100, 'http://skinfaktory.com/', [[92, 'Skin Faktory'], [97, 'SkinFaktory'], [99, 'Skin Factory'], [100, 'SkinFactory']]],
        ],
    ];
}
 ...
 */
function skin_include($template_name, $params = [])
{
    if (is_ajax_content($template_name)) { // When we request ajax content for results table we need to hide wrapper data (header, footer & etc)
        return;
    }
    ...
    if (! empty($params['Item'])) {	// Get Item from params:
        $Item = $params['Item'];
    }

    if ($template_name == '$disp$') { // This is a special case.
        ...
        if (is_pro()) {	// Additional disp handler for PRO version:
        ...
        if ($disp_Plugins = $Plugins->get_list_by_event('GetHandledDispModes')) {
            foreach ($disp_Plugins as $disp_Plugin) { // Go through whole list of plugins providing disps
                if ($plugin_modes = $Plugins->call_method($disp_Plugin->ID, 'GetHandledDispModes', $disp_handlers)) { // plugin handles some custom disp modes
        ...
        if (empty($template_name)) {	// The caller asked not to display this handler
            $Timer->pause($timer_name);
            return;
        }
        ...
        if ($template_name[0] != '#' && // if template is not handled by plugins
            ...
            ! empty($Item) && ($ItemType = &$Item->get_ItemType())) {	// Get template name for the current Item if it is defined by Item Type:
            $item_type_template_name = $ItemType->get('template_name');
            if (! empty($item_type_template_name)) {	// The item type has a specific template for this display:
                ...
                if (($Skin->get_api_version() == 7 && file_exists($ads_current_skin_path . $Blog->get('type') . '/' . $item_type_template_name)) ||
                        ...
                        skin_fallback_path($item_type_template_name)) {	// Use template file name of the Item Type only if it exists:
    ...
    if ($template_name[0] == '#') { // This disp mode is handled by a plugin:
        $debug_info = 'Call plugin';
        $disp_handled = 'plugin';
    } elseif ($Skin->get_api_version() == 7 && file_exists($ads_current_skin_path . $Blog->get('type') . '/' . $template_name)) { // The skin has a customized handler, use that one instead:
        ...
        $disp_handled = 'custom';
    } elseif (file_exists($ads_current_skin_path . $template_name)) { // The skin has a customized handler, use that one instead:
        $file = $ads_current_skin_path . $template_name;
        $debug_info = '<b>Skin template</b>: ' . rel_path_to_base($file);
        $disp_handled = 'custom';
    } elseif ($fallback_template_path = skin_fallback_path($template_name)) { // Use the default/fallback template:
    ...
    if (strpos($template_name, '_html_') !== false) {	// We're outside of the page body: NEVER display wrap this include with a <div>
        $display_includes = false;
    } else {	// We may wrap with a <div>:
        $display_includes = ($debug == 2) || (is_logged_in() && $Session->get('display_includes_' . $Blog->ID));
    }
    if ($display_includes) { // Wrap the include with a visible div:
        echo '<div class="dev-blocks dev-blocks--include">';
        echo '<div class="dev-blocks-name">';
        if (empty($item_type_template_name)) { // Default template
    ...
    if (! $disp_handled) { // nothing handled the disp mode
    ...
    if ($display_includes) { // End of visible container:
...
function skin_fallback_path($template_name, $skin_api_version = null)
{
    ...
    if ($skin_api_version === null && ! empty($Skin)) {	// Get API version of the current skin:
        $skin_api_version = $Skin->get_api_version();
    }

    if ($skin_api_version == 7) {	// Check fallback file for v7 API skin:
    ...
    if ($skin_api_version >= 6) {	// Check fallback file for v6 API skin:
...
function skin_template_path($template_name)
{
    ...
    if (! empty($Skin) && file_exists($ads_current_skin_path . $template_name)) { // Template file exists for the current skin
        return $ads_current_skin_path . $template_name;
    } elseif ($fallback_path = skin_fallback_path($template_name)) { // Falback file exists
        return $fallback_path;
    }
 ...
 * @param array Params
 */
function siteskin_include($template_name, $params = [])
{
    ...
    if ($site_Skin && file_exists($site_Skin->get_path() . $template_name)) {	// Use site skin template:
        ...
        $disp_handled = 'fallback';
    } else {	// Site skin is wrong or the requested template file is not found in current site skin:
        $disp_handled = false;
    }

    // Do we want a visible container for DEBUG/DEV ?:
    if (strpos($template_name, '_html_') !== false || strpos($template_name, '_init.') !== false) {	// We're outside of the page body: NEVER display wrap this include with a <div>
        $display_includes = false;
    } elseif (isset($Session)) {	// We may wrap with a <div>:
        $display_includes = ($debug == 2) || (is_logged_in() && $Session->get('display_includes_' . (empty($Blog) ? 0 : $Blog->ID)));
    } else {	// Request without defined $Session, Don't display the includes:
        $display_includes = false;
    }
    if ($display_includes) {	// Wrap the include with a visible div:
    ...
    if ($disp_handled) {	// Include site skin template file:
        $Debuglog->add('siteskin_include: ' . rel_path_to_base($file), 'skins');
        require $file;
    } else {	// Nothing handled the display:
...
function skin_base_tag()
{
    ...
    if (! empty($Blog)) {	// We are displaying a blog:
        if (! empty($skin)) {	// We are using a skin:
    ...
    $target = null;
    if (! empty($disp) && strpos($disp, '-popup')) {	// We are (normally) displaying in a popup window, we need most links to open a new window!
        $target = '_blank';
    }
...
function skin_description_tag()
{
    ...
    if ($is_front) {	// Use default description:
        if (! empty($Blog)) {	// Description for the blog:
            $r = $Blog->get('shortdesc');
        }
    } elseif (in_array($disp_detail, ['posts-cat', 'posts-topcat-intro', 'posts-topcat-nointro', 'posts-subcat-intro', 'posts-subcat-nointro'])) {
        if ($Blog->get_setting('categories_meta_description') && (! empty($Chapter))) {
            $r = $Chapter->get('description');
        }
    } elseif (in_array($disp, ['single', 'page'])) {	// custom desc for the current single post:
        $Item = &$MainList->get_by_idx(0);
        if (is_null($Item)) {	// This is not an object (happens on an invalid request):
            return;
        }

        $r = $Item->get_metadesc();

        if (empty($r) && $Blog->get_setting('excerpts_meta_description')) {	// Fall back to excerpt for the current single post:
...
function skin_keywords_tag()
{
    ...
    if ($is_front) {	// Use default keywords:
        if (! empty($Blog)) {
            $r = $Blog->get('keywords');
        }
    } elseif (in_array($disp, ['single', 'page'])) {	// custom keywords for the current single post:
        $Item = &$MainList->get_by_idx(0);
        if (is_null($Item)) {	// This is not an object (happens on an invalid request):
            return;
        }
        ...
        if (empty($r) && $Blog->get_setting('tags_meta_keywords')) {	// Fall back to tags for the current single post:
...
function skin_opengraph_tags()
{
    ...
    if (empty($Blog) ||
        ...
        in_array($disp, ['content_requires_login', 'access_requires_login', 'access_denied'])) {	// Open Graph tags are not allowed for current Collection or for current disp:
        return;
    }
    ...
    if (! empty($og_image_File)) { // Open Graph image tag
...
function skin_twitter_tags()
{
    ...
    if (empty($Blog) || ! $Blog->get_setting('tags_twitter_card')) { // Twitter summary card tags are not allowed
        return;
    }
...
function skin_structured_data()
{
    ...
    if (empty($Blog) || ! $Blog->get_setting('tags_structured_data')) { // Structured data markup is not allowed
        return;
    }
    ...
    switch ($disp) {
        ...
        case 'page':
            $Item = &$MainList->get_by_idx(0);
            $creative_works_schema = ['Article', 'WebPage', 'BlogPosting', 'ImageGallery', 'DiscussionForumPosting', 'TechArticle', 'Review'];
            ...
            if ($Item && ($item_schema = $Item->get_type_setting('schema'))) {
                ...
                if (in_array($item_schema, $creative_works_schema)) {
                    ...
                    if (empty($publisher_logo_url)) { // No publisher logo found, fallback to collection logo:
                    ...
                    if (! empty($publisher_name) || ! empty($publisher_logo_url)) { // Add publisher data to markup:
                        $markup['publisher'] = [
                            '@type' => 'Organization',
                        ];
                ...
                if ($Item->get_type_setting('add_aggregate_rating')) { // Add aggregate rating:
...
function skin_widget($params)
{
    ...
    if (! file_exists($inc_path . 'widgets/widgets/_' . $widget_code . '.widget.php')) {	// For some reason, that widget doesn't seem to exist... (any more?)
 ...
 */
function widget_container($container_code, $params = [])
{
    global $Blog, $Skin;
    ...
    if (! $WidgetContainer) {	// Display error if widget container is not detected in DB by requested code:
        echo '<div class="text-danger">'
                . sprintf(
                    T_('Requested widget container %s does not exist for current collection #%d and skin type %s!'),
                    '<code>' . $container_code . '</code>',
                    $Blog->ID,
                    '<code>' . $Blog->get_skin_type() . '</code>'
                )
            . '</div>';
 ...
 */
function widget_page_containers($item_ID, $params = [])
{
    global $Blog;

    if (empty($Blog)) {	// Skip wrong reuqest without current Collection:
        return;
    }
    ...
    if (empty($widget_containers)) {	// No widget containers for current Collection and skin type:
        return;
    }
    ...
    foreach ($widget_containers as $WidgetContainer) {
        if ($WidgetContainer->get_type() == 'page' &&
            $WidgetContainer->get('item_ID') == $item_ID) {	// Display only widget page container for the requested Item:
 ...
 * @param string Container code
 */
function skin_container($sco_name, $params = [], $container_code = null)
{
    global $Skin;
...
{
    return [
        'page_top' => [NT_('Page Top'), 2],
        'header' => [NT_('Header'), 10],
        'menu' => [NT_('Menu'), 15],
        'front_page_main_area' => [NT_('Front Page Main Area'), 40],
        'front_page_secondary_area' => [NT_('Front Page Secondary Area'), 45],
        'item_list' => [NT_('Item List'), 48],
        'item_in_list' => [NT_('Item in List'), 49],
        'item_single_header' => [NT_('Item Single Header'), 50],
        'item_single' => [NT_('Item Single'), 51],
        'item_page' => [NT_('Item Page'), 55],
        'comment_list' => [NT_('Comment List'), 57],
        'comment_area' => [NT_('Comment Area'), 60],
        'sidebar' => [NT_('Sidebar'), 80],
        'sidebar_2' => [NT_('Sidebar 2'), 90],
        ...
        'login_required' => [NT_('Login Required'), 140],
        'access_denied' => [NT_('Access Denied'), 150],
        'help' => [NT_('Help'), 160],
        'register' => [NT_('Register'), 170],
        'compare_main_area' => [NT_('Compare Main Area'), 180],
        'photo_index' => [NT_('Photo Index'), 190],
        'search_area' => [NT_('Search Area'), 200],
        'sitemap' => [NT_('Site Map'), 210],
    ];
}
...
function skin_provided_by_plugin($name)
{
    ...
    if (! isset($plugin_skins) || ! isset($plugin_skins[$name])) {
        ...
        $plugin_r = $Plugins->trigger_event_first_return('GetProvidedSkins', null, [
            'in_array' => $name,
        ]);
        if ($plugin_r) {
...
function display_skin_fieldset(&$Form, $skin_ID, $display_params)
{
    ...
    if ($mode != 'customizer') {	// Except of skin customer mode:
        $Form->begin_fieldset($display_params['fieldset_title'] . ' ' . $display_params['fieldset_links']);
    }

    if (! $skin_ID) { // The skin ID is empty use the same as normal skin ID
        ...
        if ($mode != 'customizer') {	// Except of skin customer mode:
            ...
            $disp_params = [
                'skinshot_class' => 'coll_settings_skinshot',
            ];
        ...
        if (! skin_exists($edited_Skin->folder)) {
        ...
        } elseif (empty($skin_params)) { // Advertise this feature!!
            ...
            if (! $skin_fieldsets_exist) { // Enclose all skin settings in single group if no group on the skin
                array_unshift($skin_params, [
                    'layout' => 'begin_fieldset',
                    'label' => T_('Skin settings'),
                ]);
                array_push($skin_params, [
                    'layout' => 'end_fieldset',
                ]);
            }
 ...
 */
function skin_body_attrs($params = [])
{
    $params = array_merge([
        'class' => null,
    ], $params);
    ...
    if (! empty($params['class'])) { // Prepend additional classes from template skin
        $classes[] = $params['class'];
    }

    // Device class:
    if (! empty($PageCache)) { // Try to detect device only when Page Cache is defined
        if ($PageCache->is_collecting) { // Page is cached now
            $classes[] = 'unknown_device page_cached';
        } else { // Page is NOT cached now
            global $Session;
            if ($Session->is_mobile_session()) { // Mobile device
                $classes[] = 'mobile_device';
            } elseif ($Session->is_tablet_session()) { // Tablet device
    ...
    if (! empty($classes)) { // Print attr "class"
...
function skin_check_compatibility($skin_ID, $type)
{
    ...
    if (! $Skin ||
        ...
        ($type == 'site' && ! $Skin->provides_site_skin())) {	// Skin cannot be used for requested type:
        return false;
    }
...
{
    return [
        'normal' => [T_('Standard'), T_('Standard skin for general browsing')],
        'mobile' => [T_('Phone'), T_('Mobile skin for mobile phones browsers')],
        'tablet' => [T_('Tablet'), T_('Tablet skin for tablet browsers')],
        'alt' => [T_('Alt'), T_('Alt skin to display by conditions')],
        'rwd' => [T_('RWD'), T_('Skin can be used for general, mobile phones and tablet browsers and for alt skin')],
        'feed' => [T_('XML Feed'), T_('Special system skin for XML feeds like RSS and Atom')],
        'sitemap' => [T_('XML Sitemap'), T_('Special system skin for XML sitemaps')],
    ];
}
...
function get_skin_setting($setting_name, $fallback_value = null)
{
    ...
    if (isset($Skin) && $Skin instanceof Skin) {	// Try to get setting value of the current Skin:
        $setting_value = $Skin->get_setting($setting_name);
    }

    if (! isset($setting_value)) {	// Fallback to default value when no current Skin or settings is not defined:
