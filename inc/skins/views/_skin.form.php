...
$Form->text_input('skin_name', $edited_Skin->name, 128, TB_('Skin name'), TB_('As seen by blog owners'), [
    'required' => true,
]);
...
foreach ($skin_types as $skin_type_key => $skin_type_data) {
    $skin_types_options[] = [$skin_type_key, $skin_type_data[0], $skin_type_data[1]];
}
...
function display_skin_setting_link($row)
{
    ...
    if (in_array($row->skin_type, ['mobile', 'tablet', 'alt'])) {
