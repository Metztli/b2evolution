...
switch ($skin_type) {
    ...
    case 'mobile':
        $skin_ID = isset($Blog) ? $Blog->get('mobile_skin_ID', [
            'real_value' => true,
        ]) : $Settings->get('mobile_skin_ID', true);
        $fieldset_title = isset($Blog) ? TB_('Phone skin for this collection') : TB_('Phone skin for this site');
        break;

    case 'tablet':
        $skin_ID = isset($Blog) ? $Blog->get('tablet_skin_ID', [
            'real_value' => true,
        ]) : $Settings->get('tablet_skin_ID', true);
        $fieldset_title = isset($Blog) ? TB_('Tablet skin for this collection') : TB_('Tablet skin for this site');
        break;

    case 'alt':
        $skin_ID = isset($Blog) ? $Blog->get('alt_skin_ID', [
            'real_value' => true,
        ]) : $Settings->get('alt_skin_ID', true);
...
$link_select_skin = action_icon(
    TB_('Select another skin...'),
    'choose',
    regenerate_url('action,mode', 'skinpage=selection&amp;skin_type=' . $skin_type),
    ' ' . TB_('Choose a different skin') . ' &raquo;',
    3,
    4,
    [
...
if ($can_edit_skin_settings) {	// Display "Reset params" button if current User can edit skin settings:
...
if ($mode == 'customizer') {	// Display customizer tabs to switch between site/collection skins and widgets in special div on customizer mode:
    $AdminUI->display_customizer_tabs([
        'path' => isset($Blog) ? ['coll', 'skin'] : 'site',
...
if (isset($Blog)) {	// If collection skin page is opened currently:
    if (check_user_perm('options', 'view')) {	// If current user has a permission to view site skin:
...
} else {	// If site skin page is opened currently:
    if (($working_coll_ID = get_working_blog()) &&
        check_user_perm('blog_properties', 'edit', false, $working_coll_ID)) {	// If working collection is set and current user has a permission to edit the collection skin:
...
display_skin_fieldset($Form, $skin_ID, [
...
$buttons = [];
if ($can_edit_skin_settings) {	// Display a button to update skin params only when if current User can edit this:
    $buttons[] = [
        'submit',
        'save',
    ...
    if ($mode == 'customizer') {	// Display cancel button only on customizer mode:
        // Reload settings from DB, which may be different from "reset" setting since the user has clicked "Apply changes"
        $buttons[] = ['button', 'cancel', T_('Cancel'), '', 'location.reload()'];
    }
}
...
if (isset($link_reset_url)) {	// Initialize JS to confirm skin reset action if current user has a permission:
...
{
<?php
if ($mode == 'customizer') {	// If skin customizer mode:
    ?>
...
<?php
if ($mode == 'customizer') {	// Reload front-office iframe on cancel changes:
    ?>
