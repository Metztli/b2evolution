...
while (($iterator_Skin = &$SkinCache->get_next()) != null) {
    if (($iterator_Skin->type != $skin_type && $iterator_Skin->type != 'rwd') ||
        ...
        (! $is_collection_skin && ! $iterator_Skin->provides_site_skin())) {	// This skin cannot be used here...
        continue;
    }
...
$current_skin_ID = $is_collection_skin ? $edited_Blog->get($skin_type . '_skin_ID', [
    'real_value' => true,
]) : $Settings->get($skin_type . '_skin_ID', true);

if (check_user_perm('options', 'edit', false)) { // We have permission to modify:
    $block_item_Widget->global_icon(T_('Install new skin...'), 'new', $admin_url . '?ctrl=skins&amp;tab=' . ($is_collection_skin ? 'coll_skin&amp;blog=' . $edited_Blog->ID : 'site_skin') . '&amp;action=new&amp;skin_type=' . $skin_type . '&amp;redirect_to=' . rawurlencode(url_rel_to_same_host(regenerate_url('', 'skinpage=selection', '', '&'), $admin_url)), T_('Install new') . ' &raquo;', 3, 4, [
...
if ($display_same_as_normal) {	// A link to select the same skin as normal for tablet, mobile and alt skins:
    if ($is_collection_skin) {	// Collection skin:
...
if (isset($skins['yes'])) {
    foreach ($skins['yes'] as $iterator_Skin) {
        ...
        // Display skinshot:
        Skin::disp_skinshot($iterator_Skin->folder, $iterator_Skin->name, [
            'function' => 'select',
...
if ($is_collection_skin) {	// JavaScript code to confirm skin selection:
