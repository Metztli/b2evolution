...
} else { // We have NO permission to modify:
    $Results->cols[] = [
        'th' => T_('Name'),
        'order' => 'skin_name',
...
function skin_col_provide_type($Skin, $type)
{
    ...
    return $is_type_provided ? get_icon('bullet_full', 'imgtag', [
        'title' => '',
    ]) : '&nbsp;';
}
$Results->cols[] = [
...
if (check_user_perm('options', 'edit', false)) { // We have permission to modify:
    ...
    $Results->global_icon(T_('Install new skin...'), 'new', regenerate_url($ignore_params, 'action=new'), T_('Install new') . ' &raquo;', 3, 4, [
