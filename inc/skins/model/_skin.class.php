...
class Skin extends DataObject
{
     ...
     */
    public $use_min_css = false;  // true|false|'check' Set this to true for better optimization
     ...
     */
    public function __construct($db_row = null, $skin_folder = null)
    {
        // Call parent constructor:
        parent::__construct('T_skins__skin', 'skin_', 'skin_ID');

        if (is_null($db_row)) {	// We are creating an object here:
    ...
    public static function get_delete_restrictions()
    {
        return [
            [
                'table' => 'T_blogs',
                'fk' => 'blog_normal_skin_ID',
                'fk_short' => 'normal_skin_ID',
                'msg' => T_('%d blogs using this skin'),
            ],
            [
                'table' => 'T_blogs',
                'fk' => 'blog_mobile_skin_ID',
                'fk_short' => 'mobile_skin_ID',
                'msg' => T_('%d blogs using this skin'),
            ],
            [
                'table' => 'T_blogs',
                'fk' => 'blog_tablet_skin_ID',
                'fk_short' => 'tablet_skin_ID',
                'msg' => T_('%d blogs using this skin'),
            ],
            [
                'table' => 'T_blogs',
                'fk' => 'blog_alt_skin_ID',
                'fk_short' => 'alt_skin_ID',
                'msg' => T_('%d blogs using this skin'),
            ],
            [
                'table' => 'T_settings',
                'fk' => 'set_value',
                'msg' => T_('This skin is set as default skin.'),
            ...
            [
                'table' => 'T_settings',
                'fk' => 'set_value',
                'msg' => T_('The site is using this skin.'),
    ...
    {
        if ($skin_ID = $this->dbexists('skin_folder', $this->get('folder'))) {	// Use already stored skin in DB:
            $this->ID = $skin_ID;
        } else {	// Insert new skin into DB:
            $this->dbinsert();
        }
    }
    ...
    public function get_default_widgets($coll_type, $skin_type = 'normal', $context = [])
    {
        return [
            '*' => true,
        ]; // For all containers, use b2evo defaults.
    }
     ...
     * Load params
     */
    public function load_params_from_Request()
    {
        load_funcs('plugins/_plugin.funcs.php');

        // Loop through all widget params:
        foreach ($this->get_param_definitions([
            'for_editing' => true,
        ]) as $parname => $parmeta) {
     ...
     */
    public function container($sco_name, $params = [], $container_code = null)
    {
        /**
        ...
        if (! empty($WidgetContainer) && $WidgetContainer->get('coll_ID') == 0) {	// Shared container:
            $widgets_coll_ID = '';
        } else {	// Collection/skin container:
            $widgets_coll_ID = $Blog->ID;
        }

        if ($container_code === null) {	// Try to detect container in DB by name:
            global $DB;
        ...
        if ($display_debug_containers) { // Wrap container in visible container:
            echo '<div class="dev-blocks dev-blocks--container"><div class="dev-blocks-name">';
            if (check_user_perm('blog_properties', 'edit', false, $Blog->ID)) {	// Display a link to edit this widget only if current user has a permission:
        ...
        if ($params['container_item_ID'] !== null) {	// Check restriction for page containers:
            if (empty($WidgetContainer) ||
                ($WidgetContainer->get_type() == 'page' && $WidgetContainer->get('item_ID') != $params['container_item_ID'])) {	// We should not try to get widgets from this container, because it is a not proper page container:
                $Widget_array = [];
            }
        }

        if (! isset($Widget_array)) {	// Get enabled widget for the container:
            $EnabledWidgetCache = &get_EnabledWidgetCache();
            $Widget_array = &$EnabledWidgetCache->get_by_coll_container(
                $widgets_coll_ID,
                ($container_code === null ? $sco_name : $container_code),// Use container code if it is defined, otherwise use container name
                ($container_code !== null)
            ); // Get by container code if it is defined
        }

        if (! empty($Widget_array)) {
            foreach ($Widget_array as $w => $ComponentWidget) { // Let the Widget display itself (with contextual params):
                if ($w == 0) { // Use special params for first widget in the current container
                ...
                if ($w == 0) { // Restore the params for next widgets after first
        ...
        if ($display_debug_containers) {	// End of visible debug container:
            echo '</div>';
        }
        ...
        if ($container_params['container_display_if_empty'] || ! empty($Widget_array)) {	// Display container wrapper with widgets content if it is not empty or we should display it anyway:
            ...
            if (empty($Widget_array) && is_logged_in() && $Session->get('designer_mode_' . $Blog->ID)) {	// Display text for empty container on designer mode:
     ...
     */
    public function discover_containers_by_folder($folder, $exclude_files = [], $display_messages = true)
    {
        global $Messages;
        ...
        switch ($folder) {
            ...
            case '#fallback_folders#':
                ...
                for ($v = $this->get_api_version(); $v >= 5; $v--) {	// Start with fallback files of current skin version and go down to find other fallback from older versions:
                    if ($skin_fallback_path = skin_fallback_path('', $v)) {	// If fallback folder is detected for the version:
        ...
        if (! $dir = @opendir($skin_path)) { // Skin directory not found!
            if ($display_messages) {
        ...
        while (($file = readdir($dir)) !== false) {
            if (is_array($exclude_files) && in_array($file, $exclude_files)) { // Skip this file
                continue;
            }

            $af_main_path = $skin_path . $file;

            if (! is_file($af_main_path) || ! preg_match('~\.php$~', $file)) { // Not a php template file, go to next:
                continue;
            }

            if (! is_readable($af_main_path)) { // Cannot open PHP file:
                if ($display_messages) {
            ...
            if (! is_string($file_contents)) { // Cannot get contents:
                if ($display_messages) {
                    $Messages->add_to_group(sprintf(T_('Cannot read skin file &laquo;%s&raquo;!'), $skin_folder . '/' . $file), 'error', T_('File read error:'));
                }
                continue;
            ...
            if (! preg_match_all('~ (\$Skin->|skin_|widget_)container\( .*? ([\'"] (.+?) [\'"]) ~xmi', $file_contents, $matches)) { // No containers in this file, go to next:
                continue;
            }

            $c = 0;
            foreach ($matches[3] as $container) {
                if (empty($container)) { // regexp empty match -- NOT a container:
                    continue;
                }

                if ($matches[1][$c] == 'widget_') {	// Function widget_container() already uses container code as first param:
                    $container_code = $container;
                    // We should create container name from container code:
                    $container = ucwords(str_replace('_', ' ', $container_code));
                } else {	// Old functions $Skin->container() and skin_container() use container name, so we should auto convert it to code:
                    $container_code = preg_replace('/[^a-z\d]+/', '_', strtolower($container));
                }

                if (in_array($container_code, $this->container_list)) { // we already have that one
                    continue;
                }
                ...
                $this->container_list[$container_code] = [$container, $c];
            }
     ...
     * @param boolean TRUE to display messages
     */
    public function discover_containers($display_messages = true)
    {
        global $Messages;
        ...
        $skin_files = $this->discover_containers_by_folder('#skin_folder#', [], $display_messages);
    ...
    public function get_containers()
    {
        if (is_null($this->container_list)) {
            ...
            if ($this->get_api_version() > 5 || ! empty($skin_declared_containers)) {	// Get default containers and containers what declared by this skin:
                ...
                foreach ($this->container_list as $wico_code => $wico_data) {
                    if ($wico_data === null) {	// Exclude containers which are not used in the current Skin:
     ...
     * @return boolean
     */
    public function sort_containers($a_container, $b_container)
    {
     ...
     * @param array Params
     */
    public static function disp_skinshot($skin_folder, $skin_name, $disp_params = [])
    {
        global $skins_path, $skins_url, $kind;

        $disp_params = array_merge([
            'selected' => false,
        ...
        if (isset($disp_params['select_url'])) {	// Initialize params for link to SELECT new skin for collection:
            ...
            $select_a_end = '</a>';
        } elseif (isset($disp_params['function_url'])) {	// Initialize params for link to INSTALL new skin and probably select this automatically for collection:
            ...
            switch ($disp_params['function']) {
                case 'broken':
                    ...
                    if (! empty($disp_params['help_info'])) {
                        echo ' ' . get_icon('help', 'imgtag', [
                            'title' => $disp_params['help_info'],
                        ]);
                    }
                    break;
                ...
                case 'install':
                    ...
                    if (empty($kind) && get_param('tab') != 'coll_skin' && get_param('tab') != 'site_skin') {	// Don't display the checkbox on new collection creating form and when we install one skin for the selected collection:
    ...
    public function get_param_definitions($params)
    {
        ...
        if ($this->get_api_version() == 7 &&
            ...
            ! empty($Blog) &&
            method_exists($this, 'get_param_definitions_' . $Blog->get('type'))) {	// If skin has declared the method for collection kind:
            $coll_kind_param_definitions = call_user_func([$this, 'get_param_definitions_' . $Blog->get('type')], $params);
     ...
     */
    public function get_setting($parname, $group = null, $default_value = null)
    {
        global $Collection, $Blog, $Settings;

        if (! empty($group)) { // $parname is prefixed with $group, we'll remove the group prefix
        ...
        if (isset($Blog) && $this->provides_collection_skin()) {	// Get skin settings of the current collection only if skin is provided for collections:
            $value = $Blog->get_setting($setting_name);
        } elseif ($this->provides_site_skin()) {	// Get skin settings of the site only if skin is provided for site:
        ...
        if ($default_value !== null) {	// Use defined default value when it is not saved in DB yet:
     ...
     */
    public function get_checklist_setting($setting_name, $option_name)
    {
     ...
     * @return string|array|null
     */
    public function get_setting_default_value($parname, $group = null)
    {
        if (! empty($group)) {
            $parname = substr($parname, strlen($group));
        }
        ...
        if (isset($params[$parname]['defaultvalue'])) { // We have a default value:
            return $params[$parname]['defaultvalue'];
                ...
                ! empty($params[$parname]['options'])) { // Get default values for checkbox list:
            $options = [];
            foreach ($params[$parname]['options'] as $option) {
                if (isset($option[2])) { // Set default value only if it is defined by skin:
            ...
            return $options;
                        ...
                        $default_File = &get_file_by_abspath($params[$parname]['initialize_with'], true)) { // Get default value for fileselect
     ...
     * @param array Additional params
     */
    public function dynamic_style_rule($setting_name, $style_template, $params = [])
    {
        $params = array_merge([
        ...
        if ($params['value'] === null) {	// Try to get current setting value:
            $setting_value = $this->get_setting($setting_name);
        } else {	// Use custom value:
            $setting_value = $params['value'];
        }

        if ($setting_value === null) {	// No value for the requested setting:
            return;
        }
        ...
        if (is_array($params['options']) &&
            isset($params['options'][$setting_value])) {	// Get value from predefined array:
            $setting_value = $params['options'][$setting_value];
        }

        if ($params['type'] == 'image_file') {	// Special setting type as ID of image file:
        ...
        if ($params['suffix'] !== null) {	// Suffix for value:
            $setting_value .= $params['suffix'];
        }

        if ($Session->get('customizer_mode_' . $blog)) {	// If customizer mode is enabled we should append a special css comment code
            // in order to quick change the value from the customizer panel on change input value:
            $setting_options = '';

            if (is_array($params['options'])) {	// Append value presets to the comment in order to select them by JavaScript:;
            ...
            if ($params['type'] !== null) {	// Setting type:
                $setting_options .= '/type:' . $params['type'];
            }

            if ($params['check'] == 'not_empty') {	// If we should apply rule only when setting value is not empty:
                ...
                if (empty($setting_value)) {	// Don't apply rule completely when value is empty:
                    $style_template = '';
                }
        ...
        } else {	// If customizer mode is disabled
            if ($params['check'] == 'not_empty' && empty($setting_value)) {	// Don't apply rule completely when value is empty:
                return;
            }
        }
     ...
     */
    public function add_dynamic_style($style_rule)
    {
     ...
     */
    public function add_dynamic_css_headline($media_exception = null)
    {
        $dynamic_css = $this->get_dynamic_styles();
        if (! empty($dynamic_css)) {
            if ($media_exception !== null) {	// Use media exception:
...
-->
     ...
     * @param mixed parameter value
     */
    public function set_setting($parname, $parvalue)
    {
        global $Collection, $Blog, $Settings;
        ...
        if (isset($Blog)) {	// Set collection skin setting:
            $Blog->set_setting($setting_name, $parvalue);
        } else { // Set site skin setting:
     ...
     */
    public function dbupdate_settings()
    {
        global $Collection, $Blog, $Settings;

        if (isset($Blog)) {	// Update collection skin settings:
            $Blog->dbupdate();
        } else {	// Update site skin settings:
            $Settings->dbupdate();
        }
    }
     ...
     */
    public function display_init(/*optional: $features = array() */)
    {
        ...
        if (empty($features)) {	// Fall back to v5 default set of features:
            $features = ['b2evo_base_css', 'style_css', 'colorbox', 'disp_auto'];
        }
        ...
        for ($i = 0; isset($features[$i]); $i++) {
            ...
            switch ($feature) {
                ...
                case 'jquery':
                    // Include jQuery:
                    if (! in_array('superbundle', $features)) {	// Don't include when it is already bundled:
                ...
                case 'style_css':
                    ...
                    if ($this->use_min_css == false
                        || $debug
                        || ($this->use_min_css == 'check' && ! file_exists($this->get_path() . 'style.min.css'))) {	// Use readable CSS:
                        $this->require_css('style.css');
                    } else {	// Use minified CSS:
                        $this->require_css('style.min.css');
                    }

                    if ($this->get_api_version() == 7) {	// Get skin css file from folder of collection kind:
                        $skin_css_folder = $Blog->get('type') . '/';
                        if (file_exists($this->get_path() . $skin_css_folder . 'style.css') &&
                            ($this->use_min_css == false
                            || $debug
                            || ($this->use_min_css == 'check' && ! file_exists($this->get_path() . $skin_css_folder . 'style.min.css')))) {	// Use readable CSS:
                            $this->require_css($skin_css_folder . 'style.css');
                        } elseif (file_exists($this->get_path() . $skin_css_folder . 'style.min.css')) {	// Use minified CSS:
                ...
                case 'colorbox':
                    ...
                    if ($this->get_setting('colorbox')) {	// This can be enabled by a setting in skins where it may be relevant
                ...
                case 'disp_page':
                    ...
                    if (! empty($Item) && $Item->can_receive_webmentions()) {	// Send header and initialize <link> tags in order to mark current Item can receive webmentions by current User(usually anonymous user):
                    ...
                    if ($Blog->get_setting('allow_rating_comment_helpfulness')) { // Load jquery UI to animate background color on change comment status or on vote:
                        require_js_defer('#jqueryUI#', 'blog');
                    }

                    if ($Blog->get_setting('use_workflow') && check_user_perm('blog_can_be_assignee', 'edit', false, $Blog->ID)) {	// Initialize JS to autcomplete user logins and date picker to edit workflow properties:
                ...
                case 'disp_users':
                    ...
                    if (! in_array('bootstrap', $features)) { // Only for NON-bootstrap skins
                ...
                case 'disp_threads':
                    ...
                    if (in_array(get_param('action'), ['new', 'create', 'preview'])) { // Used to suggest usernames for the field "Recipients":
                ...
                case 'disp_content_requires_login':
                    ...
                    if (can_use_hashed_password()) {	// Include JS for client-side password hashing:
                ...
                case 'disp_profile':
                    ...
                    if ($UserSettings->get('control_form_abortions')) {	// Only if user wants this:
        ...
        if (! in_array('superbundle', $features)) {	// Load general JS file only when it is not bundled above:
            if ($this->get_api_version() >= 6) { // Bootstrap skin
        ...
        if (is_logged_in() && $Session->get('designer_mode_' . $Blog->ID)) {	// If desinger mode when it is turned on from evo menu under "Designer Mode/Exit Designer" or "Collection" -> "Enable/Disable designer mode":
            require_js_defer('#jquery#', 'blog');
            if (check_user_perm('blog_properties', 'edit', false, $Blog->ID)) {	// Initialize this url var only when current user has a permission to edit widgets:
                global $admin_url;
                add_js_headline('var b2evo_widget_edit_url = "' . get_admin_url('ctrl=widgets&action=edit&wi_ID=$wi_ID$&mode=customizer', '&') . '";'
                    ...
                    . 'var b2evo_widget_icon_top = \'' . format_to_js(get_icon('designer_widget_top', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_order_top',
                    ])) . '\';'
                    . 'var b2evo_widget_icon_up = \'' . format_to_js(get_icon('designer_widget_up', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_order_up',
                    ])) . '\';'
                    . 'var b2evo_widget_icon_down = \'' . format_to_js(get_icon('designer_widget_down', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_order_down',
                    ])) . '\';'
                    . 'var b2evo_widget_icon_bottom = \'' . format_to_js(get_icon('designer_widget_bottom', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_order_bottom',
                    ])) . '\';'
                    . 'var b2evo_widget_icon_duplicate = \'' . format_to_js(get_icon('duplicate', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_duplicate',
                        'title' => T_('Duplicate'),
                    ])) . '\';'
                    . 'var b2evo_widget_icon_disable = \'' . format_to_js(get_icon('minus', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_disable',
                        'title' => T_('Disable'),
                    ])) . '\';'
                    . 'var b2evo_widget_icon_add = \'' . format_to_js(get_icon('add', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_add',
                        'title' => T_('Add Widget to container'),
                    ])) . '\';'
                    . 'var b2evo_widget_icon_list = \'' . format_to_js(get_icon('designer_widget_list', 'imgtag', [
                        'class' => 'evo_designer__action evo_designer__action_list',
                        'title' => T_('Manage Widgets of container'),
                    ])) . '\';'
        ...
        if ($this->get_api_version() == 7 && method_exists($this, 'display_init_' . $Blog->get('type'))) {	// If skin has declared the method for collection kind:
            call_user_func([$this, 'display_init_' . $Blog->get('type')]);
        }
    }
    ...
    public function T_($string, $req_locale = '')
    {
        ...
        ])) == $string) {	// This skin did not provide a translation - fallback to global T_():
            ...
            case 'bootstrap':
                switch ($name) {
                    ...
                    case 'compact_results':
                        ...
                        if ($name == 'compact_results') {	// Use a little different template for compact results table:
                            $results_template = array_merge($results_template, [
                    ...
                    case 'fields_table_form':
                        return array_merge($this->get_template('Form'), [
     ...
     */
    public function require_css($css_file, $position = 'headlines')
    {
     ...
     */
    public function require_js($js_file, $async_defer = false, $output = false, $position = 'headlines')
    {
        global $app_version_long;
     ...
     */
    private $font_definitions = [
        'system_arial' => ['Arial', 'Arial, Helvetica, sans-serif'],
        'system_arialblack' => ['Arial Black', '\'Arial Black\', Gadget, sans-serif'],
        'system_arialnarrow' => ['Arial Narrow', '\'Arial Narrow\', sans-serif'],
        'system_centrygothic' => ['Century Gothic', 'Century Gothic, sans-serif'],
        'system_copperplategothiclight' => ['Copperplate Gothic Light', 'Copperplate Gothic Light, sans-serif'],
        'system_couriernew' => ['Courier New', '\'Courier New\', Courier, monospace'],
        'system_georgia' => ['Georgia', 'Georgia, Serif'],
        'system_helveticaneue' => ['Helvetica Neue', '\'Helvetica Neue\',Helvetica,Arial,sans-serif'],
        'system_impact' => ['Impact', 'Impact, Charcoal, sans-serif'],
        'system_lucidaconsole' => ['Lucida Console', '\'Lucida Console\', Monaco, monospace'],
        'system_lucidasansunicode' => ['Lucida Sans Unicode', '\'Lucida Sans Unicode\', \'Lucida Grande\', sans-serif'],
        'system_palatinolinotype' => ['Palatino Linotype', '\'Palatino Linotype\', \'Book Antiqua\', Palatino, serif'],
        'system_tahoma' => ['Tahoma', 'Tahoma, Geneva, sans-serif'],
        'system_timesnewroman' => ['Times New Roman', '\'Times New Roman\', Times, serif'],
        'system_trebuchetms' => ['Trebuchet MS', '\'Trebuchet MS\', Helvetica, sans-serif'],
        'system_verdana' => ['Verdana', 'Verdana, Geneva, sans-serif'],
    ];
     ...
     */
    public function get_font_definitions($type = 'select')
    {
        $fonts = [];
     ...
     * Uses: $this->font_definitions
     */
    public function apply_selected_font($target_element, $font_family_param, $text_size_param = null, $font_weight_param = null, $group = null)
    {
        $font_css = [];
     ...
     */
    public function show_container_when_access_denied($container_key)
    {
        global $Collection, $Blog;
     ...
     */
    public function echo_settings_form_js()
    {
    }
     ...
     */
    public function call_func_by_coll_type($func_name, $params)
    {
        global $Blog;

        if (! empty($Blog) &&
            $this->get_api_version() == 7 &&
            method_exists($this, $func_name . '_' . $Blog->get('type'))) {	// If skin has declared the method for collection kind:
            return call_user_func_array([$this, $func_name . '_' . $Blog->get('type')], $params);
        }
