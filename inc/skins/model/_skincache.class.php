...
class SkinCache extends DataObjectCache
{
     ...
     * @var array
     */
    public $cache_by_folder = [];

    public $cache_by_class = [];
     ...
     */
    public function &get_by_folder($req_folder, $halt_on_error = true)
    {
        global $DB, $Debuglog;
     ...
     * @return Skin A Skin object on success, false on failure (may also halt!)
     */
    public function &get_by_class($req_class, $halt_on_error = true)
    {
        global $DB, $Debuglog;
     ...
     * @param string
     */
    public function load_by_type($type)
    {
        /**
         * @var DB
         */
        ...
        if ($this->all_loaded || ! empty($this->loaded_types[$type])) { // Already loaded
            return false;
        }
     ...
     */
    public function &new_obj($row = null, $skin_folder = null, $halt_on_error = false)
    {
        if (is_null($skin_folder)) {	// This happens when using the default skin
        ...
        } elseif ($halt_on_error) {	// Don't allow to install skin if its class file doesn't exist:
