...
class site_Skin extends Skin
{
     ...
     * @return array
     */
    public function get_site_header_param_definitions()
    {
        global $admin_url;
        ...
        return [
            'menu_type' => [
                'label' => T_('Menu type'),
                'options' => [
                    ['auto', T_('Automatic - Collection list')],
                    ['auto_grouped', T_('Automatic - Grouped collection list')],
                    ['custom', T_('Custom menu')],
                ],
                'defaultvalue' => 'auto',
                'type' => 'radio',
                'field_lines' => true,
            ...
            'menu_ID' => [
                ...
                'allow_none' => true,
                'defaultvalue' => '',
                'hide' => in_array($this->get_setting('menu_type', null, 'auto'), ['auto', 'auto_grouped']),
            ],
            'fixed_header' => [
    ...
    {
        if ($this->get_setting('menu_type') == 'auto') {	// Skin does NOT group collections sub tabs:
            return false;
        }
     ...
     */
    public function get_header_tabs()
    {
        if ($this->get_setting('menu_type') == 'custom' &&
            ($SiteMenuCache = &get_SiteMenuCache()) &&
            ($SiteMenu = &$SiteMenuCache->get_by_ID($this->get_setting('menu_ID'), false, false))) {	// Use custom menu if it is found in DB:
            return $this->get_header_tabs_custom($SiteMenu->ID);
        } elseif ($this->get_setting('menu_type') == 'auto_grouped') {	// Use automatic grouped menu:
     ...
     * @return array
     */
    public function get_header_tabs_custom($menu_ID)
    {
        ...
        if (! ($SiteMenu = &$SiteMenuCache->get_by_ID($this->get_setting('menu_ID'), false, false))) {	// Wrong Menu:
            return $header_tabs;
        }

        // Check if the menu has a child matching the current locale:
        $localized_menus = $SiteMenu->get_localized_menus($current_locale);
        if (! empty($localized_menus)) {	// Use localized menu:
     ...
     */
    public function get_header_tab_custom($SiteMenuEntry)
    {
        global $Blog;

        $header_tab = false;

        if ($SiteMenuEntry->get('type') == 'text') {	// Only type "Text" supports sub-entries:
            ...
            if (! empty($sub_tabs)) {	// Display parent tab only if at least one sub tab is allowed for current display:
                $header_tab = [
        ...
        } elseif ($menu_entry_url = $SiteMenuEntry->get_url()) {	// Only if the menu entry is allowed for current User, page and etc.:
            $header_tab = [
                'name' => $SiteMenuEntry->get_text(),
                'url' => $menu_entry_url,
     ...
     * @return array
     */
    public function get_header_tabs_auto()
    {
        global $Blog, $disp, $current_User;

        if (isset($this->header_tabs)) {	// Get header tabs from previous request:
        ...
        foreach ($SectionCache->cache as $Section) {
            ...
            foreach ($group_blogs as $i => $group_Blog) {
                ...
                if ($current_blog_ID == $group_Blog->ID &&
                        ($this->get_info_coll_ID() != $current_blog_ID || ($current_disp != 'page' && $current_disp != 'msgform'))) {	// Mark this menu as active:
                    $coll_is_active = true;
                }
                ...
                if ($in_bloglist == 'public') {	// Everyone can view this collection, Keep this in menu:
                    $tab_items[] = $coll_data;
                    if ($coll_is_active) {
                        $this->header_tab_active = $level0_index;
                    }
                ...
                if ($in_bloglist == 'never') {	// Nobody can view this collection, Skip it:
                    continue;
                }

                if (! is_logged_in()) {	// Only logged in users have an access to this collection, Skip it:
                    continue;
                }

                if ($in_bloglist == 'member' &&
                        ! check_user_perm('blog_ismember', 'view', false, $group_Blog->ID)) {	// Only members have an access to this collection, Skip it:
                    continue;
                }
            ...
            if (! empty($tab_items)) {	// Display section only if at least one collection is allowed for current display:
                $this->header_tabs[] = [
        ...
        foreach ($BlogCache->cache as $nosec_Blog) {
            ...
            if ($current_blog_ID == $nosec_Blog->ID) {	// Mark this tab as active if this is a current collection:
                $this->header_tab_active = $level0_index;
            }
        ...
        if (isset($Blog)) {
            ...
            if ($current_disp == 'msgform') {	// Mark this menu as active:
                $this->header_tab_active = $level0_index;
            }

            if ($current_disp == 'page' && $this->get_info_coll_ID() == $Blog->ID) {	// If this menu contains the links to pages of the info/shared collection:
                $this->header_tab_active = $level0_index;
            }

            if ($contact_url = $Blog->get_contact_url()) {	// If contact page is allowed for current collection:
                $tab_item = [
                    'name' => T_('Contact'),
                    'url' => $contact_url,
                    'active' => ($current_disp == 'msgform'),
                ];
                if ($Blog->get_setting('msgform_nofollowto')) {	// Use nofollow attribute:
                    $tab_item['rel'] = 'nofollow';
                }
                $tab_items[] = $tab_item;
            }

            if (! empty($contact_url)) {	// Display additional tabs with static pages only user has an access to contact page:
                $this->header_tabs[] = [
                    'name' => T_('About'),
                    'url' => $contact_url,
                    'items' => $tab_items,
     ...
     * @return string
     */
    public function get_header_tab_attr_class($tab, $index = null, $params = [])
    {
        $params = array_merge([
            'class' => '',
            'class_active' => 'active',
        ...
        if (! empty($tab['class'])) {	// Append extra CSS classes of Menu Entry:
            $class .= ' ' . $tab['class'];
        }

        if ($this->header_tab_active === $index || ! empty($tab['active'])) {	// This tab is active currently:
     ...
     * @return integer ID
     */
    public function get_info_coll_ID()
    {
        $info_coll_ID = $this->get_setting('info_coll_ID');

        if (empty($info_coll_ID)) {	// Use same collection as "Default collection to display":
            global $Settings;
