...
if ($action != 'reset_coll') {	// Check permission to display site options:
...
if ($tab == 'system') {	// Check minimum permission:
    check_user_perm('admin', 'normal', true);
}

param('redirect_to', 'url', $admin_url . '?ctrl=skins&tab=' . $tab . (isset($blog) ? '&blog=' . $blog : ''));

if ($tab != 'system' && $tab != 'site_skin') {	// Memorize this as the last "tab" used in the Blog Settings:
    $UserSettings->set('pref_coll_settings_tab', $tab);
    $UserSettings->dbupdate();
}

if (param('skin_ID', 'integer', '', true)) {// Load file type:
    $SkinCache = &get_SkinCache();
    if (($edited_Skin = &$SkinCache->get_by_ID($skin_ID, false)) === false) {	// We could not find the skin to edit:
        unset($edited_Skin);
...
switch ($action) {
    case 'create':
        ...
        if ($tab == 'coll_skin' && ! empty($blog)) {	// We installed the skin for the selected collection:
            $BlogCache = &get_BlogCache();
            $edited_Blog = &$BlogCache->get_by_ID($blog);

            if (! skin_check_compatibility($edited_Skin->ID, 'coll')) {	// Redirect to admin skins page selector if the skin cannot be selected:
            ...
            if (param('reset_widgets', 'integer', 0)) {	// Reset previous widgets with new from skin default widget declarations:
            ...
            if ((! $Session->is_mobile_session() && ! $Session->is_tablet_session() && ! $Session->is_alt_session() && $skin_type == 'normal') ||
                    ...
                    ($Session->is_alt_session() && $skin_type == 'alt')) {	// Redirect to blog home page if we change the skin for current device type:
        ...
        } elseif ($tab == 'site_skin') {	// We installed the skin for the site:
            if (! skin_check_compatibility($edited_Skin->ID, 'site')) {	// Redirect to admin skins page selector if the skin cannot be selected:
            ...
            if (! empty($skin_type)) {
                ...
                if ($Settings->dbupdate()) {
                    ...
                    if ((! $Session->is_mobile_session() && ! $Session->is_tablet_session() && ! $Session->is_alt_session() && $skin_type == 'normal') ||
                            ($Session->is_mobile_session() && $skin_type == 'mobile') ||
                            ($Session->is_tablet_session() && $skin_type == 'tablet') ||
                            ($Session->is_alt_session() && $skin_type == 'alt')) {	// Redirect to home page if we change the skin for current device type:
                        header_redirect($baseurl);
                    } else {	// Redirect to admin skins page if we change the skin for another device type:
                        header_redirect($admin_url . '?ctrl=collections&tab=site_skin&skin_type=' . $skin_type);
                    }
                }
            } else {	// Redirect to admin skins page if we installed a site skin for an unknown device:
                header_redirect($admin_url . '?ctrl=skins&tab=site_skin');
            }
        } else {
            // We want to highlight the edited object on next list display:
            $Session->set('fadeout_array', [
                'skin_ID' => [$edited_Skin->ID],
            ]);
    ...
    case 'downgrade':
        ...
        // We want to highlight the edited object on next list display:
        $Session->set('fadeout_array', [
            'skin_ID' => $new_installed_skin_IDs,
        ]);
    ...
    case 'install':
        ...
        param('skin_folders', 'array:/([-A-Za-z0-9._]|\.\.)/', []);

        if (empty($skin_folders)) { // No selected skins
            $Messages->add(TB_('Please select at least one skin to install.'), 'error');
            header_redirect($admin_url . '?ctrl=skins&action=new');
        }

        $new_installed_skin_IDs = [];
        foreach ($skin_folders as $skin_folder) { // CREATE NEW SKIN:
            $edited_Skin = &skin_install($skin_folder);
            $new_installed_skin_IDs[] = $edited_Skin->ID;
        }
    ...
    case 'update':
        ...
        if ($edited_Skin->load_from_Request()) {	// We could load data from form without errors:
            // Update in DB:
            $edited_Skin->dbupdate();
    ...
    case 'delete':
        ...
        if (param('confirm', 'integer', 0)) { // confirmed, Delete from DB:
            ...
            if (! $edited_Skin->check_delete(sprintf(TB_('Cannot uninstall skin &laquo;%s&raquo;'), $edited_Skin->dget('name')))) {	// There are restrictions:
                $action = 'edit';
            }
        }
    ...
    case 'reset_coll':
        ...
        if ($action == 'reset_coll') {	// Collection skin:
            // Check permission:
...
} elseif ($tab == 'site_skin') {	// From Site Skin tab:
