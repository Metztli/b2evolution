...
function debug_get_config($close_handle = true)
{
    ...
    } else {	// Bad file name or file has not the rights to read/write
...
switch ($action) {
    case 'login':
        // Check password
        if (! debug_check_password()) {	// Some errors exist, show the form with password again
    ...
    case 'update_config':
        ...
        if (strpos($old_content, $file_section_start) !== false && strpos($old_content, $file_section_end) !== false) { // File contains correct section to edit
        ...
        } else { // Config editable start or end sections don't exist in the file, we should create them
            $is_php_tag_opened = strpos($old_content, '<?') !== false;
            $is_php_tag_closed = strpos($old_content, '?>') !== false;
            if ($is_php_tag_opened && $is_php_tag_closed) { // File contains the open and close tags
                $old_content_start = substr($old_content, 0, strpos($old_content, '?>'));
                $old_content_end = substr($old_content, strpos($old_content, '?>'));
            } elseif (! $is_php_tag_opened && ! $is_php_tag_closed) { // File doesn't contain the open and close php tags, Add this at the file end
                $old_content_start = $old_content . '<?php' . "\n";
                $old_content_end = '?>';
            } elseif (! $is_php_tag_closed) { // File doesn't contain only the close php tag, Insert new config text at the end
    ...
    default:
        if (file_exists($file_overrides_name)) {	// Display a form to log in if file already exists
            $action = 'password_form';
        }
        break;
}
...
if (! empty($message)) { // Display a message
...
switch ($action) {
    ...
    case 'config_form':
        ...
        $Form->textarea_input('content', $file_overrides_content, 20, T_('Config'), [
            'cols' => 50,
