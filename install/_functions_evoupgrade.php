...
function set_upgrade_checkpoint($version)
{
    ...
    if ($max_exe_time && ($elapsed_time > ($max_exe_time - 20))) { // Max exe time not disabled and we're recahing the end
...
function upg_task_start($version, $title = '')
{
    ...
    if ($old_db_version < $version) {	// Only if current DB version is older than current requested
        // Set upgrade version to use this in function upg_task_end():
        $upgrade_db_version = $version;

        if (! empty($title)) {	// Start task (Display a title):
            task_begin($title);
        }
...
function upg_task_end($print_result = true)
{
    ...
    if (! empty($upgrade_db_version)) {	// Only if the upgrade task is not ended yet:
        // End current transaction of the upgrade block:
        global $DB;
        $DB->commit();

        if ($print_result) {	// Print out the end of task:
            task_end();
        }
...
function db_create_table($table, $fields, $options = 'ENGINE = innodb')
{
    ...
    if (isset($db_config, $db_config['aliases'], $db_config['aliases'][$table])) {	// Use table name with real prefix instead of T_:
    ...
    $unique_table = $table;
    $next_table_index = 1;
    while ($DB->get_var('SHOW TABLES LIKE ' . $DB->quote($unique_table))) {	// This table already exists in DB, try to find what next table name is free:
        $unique_table = $table . '_' . $next_table_index;
        $next_table_index++;
    }
    if ($unique_table != $table) {	// Backup the existing table, in order to create the requeired table without error:
...
function db_add_col($table, $col_name, $col_desc)
{
    ...
    if (db_col_exists($table, $col_name)) { // Column exists already, make sure it's the same.
...
function db_upgrade_cols($table, $cols)
{
    ...
    foreach ($cols as $action => $cols_data) {
        foreach ($cols_data as $col_name => $col_desc) {
            switch ($action) {
                ...
                case 'MODIFY':
                case 'CHANGE':
                    if (in_array(strtolower($col_name), $existing_columns)) {	// Modify the existing column:
                        if ($action == 'CHANGE') {	// Change a column, e-g rename it:
                            $upgrade_sql_query .= ' CHANGE COLUMN ';
                        } else {	// Modify a column, update only
                ...
                case 'DROP':
                    $col_name = $col_desc;
                    if (in_array(strtolower($col_name), $existing_columns)) {	// Allow to drop only really existing column:
    ...
    if (! empty($upgrade_sql_query)) {	// Run upgrade SQL query only if at least one column should be upgraded:
...
function db_drop_index($table, $index)
{
    ...
    if (! db_index_exists($table, $index)) {	// No index is detected, Stop it:
        return;
    }
...
function db_convert_cols_to_utf8mb4($table_columns)
{
    ...
    foreach ($table_columns as $table => $columns) {
        ...
        $col_definitions = [];
        foreach ($columns as $col) {	// Update a column character set explicitly to utf8mb4:
...
function convert_lang_to_locale($table, $columnlang, $columnID)
{
    ...
    if (! preg_match('/[a-z]{2}-[A-Z]{2}(-.{1,14})?/', $default_locale)) { // we want a valid locale
    ...
    foreach ($languagestoconvert as $lkey => $lIDs) { // converting the languages we've found
        $converted = false;
        echo get_install_format_text_and_log('&nbsp; Converting lang \'' . $lkey . '\' '); // (with IDs: '. implode( ', ', $lIDs ). ').. ';

        if (preg_match('/[a-z]{2}-[A-Z]{2}(-.{1,14})?/', $lkey)) { // Already valid
            echo get_install_format_text_and_log('nothing to update, already valid!<br />', 'br');
            continue;
        }

        if ((strlen($lkey) == 2) && (substr($default_locale, 0, 2) != $lkey)) { // we have an old two letter lang code to convert
            // and it doesn't match the default locale
            foreach ($locales as $newlkey => $v) {  // loop given locales
        ...
        if (! $converted) { // we have nothing converted yet, setting default:
...
function upg_init_environment($env_vars = 'Settings')
{
    ...
    if (! is_array($upg_initialized_env_vars)) {	// Initialize array to know what objects were already initialized:
    ...
    foreach ($env_vars as $env_var) {
        if (in_array($env_var, $upg_initialized_env_vars) ||
            (isset($GLOBALS[$env_var]) && is_object($GLOBALS[$env_var]))) {	// Don't initialize same environment global variable twice:
            continue;
        }
...
function upg_clear_environment()
{
    ...
    if (empty($upg_initialized_env_vars)) {	// No temp environment global variables:
        return;
    }

    foreach ($upg_initialized_env_vars as $env_var) {	// Reset each temporary initialized environment global variable:
...
function upgrade_b2evo_tables($upgrade_action = 'evoupgrade')
{
    ...
    if ($old_db_version < 8030) {
        ...
        foreach ($DB->get_results($query, ARRAY_A) as $row) {
            ...
            if (strpos($blog_siteurl . '/', $baseurl) !== 0) { // If not found at position 0
    ...
    if ($old_db_version < 8040) { // upgrade to 0.8.7
    ...
    if ($old_db_version < 8050) { // upgrade to 0.8.9
        echo 'Upgrading blogs table... ';
        $query = "ALTER TABLE T_blogs
    ...
    if ($old_db_version < 8060) { // upgrade to 0.9
        // Important check:
        ...
        $transform = [
            'posts_per_page' => [5],      // note: moved to blogsettings in 2.0
            'what_to_show' => ['posts'],  // note: moved to blogsettings in 2.0
            'archive_mode' => ['monthly'], // note: moved to blogsettings in 2.0
            'time_difference' => [0],
            'AutoBR' => [0],
            'last_antispam_update' => ['2000-01-01 00:00:00', 'antispam_last_update'],
            'pref_newusers_grp_ID' => [$Group_Users->ID, 'newusers_grp_ID'],
            'pref_newusers_level' => [1, 'newusers_level'],
            'pref_newusers_canregister' => [0, 'newusers_canregister'],
        ];
    ...
    if ($old_db_version < 8062) { // upgrade to 0.9.0.4
    ...
    if ($old_db_version < 8064) { // upgrade to 0.9.0.6
        cleanup_comment_quotes();

        set_upgrade_checkpoint('8064');
    }

    if ($old_db_version < 8066) {	// upgrade to 0.9.1
    ...
    if ($old_db_version < 8800) { // ---------------------------------- upgrade to 1.6 "phoenix ALPHA"
    ...
    if ($old_db_version < 8900) {
        ...
        if (! isset($tableblogusers_isuptodate)) {	// We have created the blogusers table before and it's already clean!
    ...
    if ($old_db_version < 9000) {
        ...
        if (! isset($tablegroups_isuptodate)) {	// We have created the groups table before and it's already clean!
    ...
    if ($old_db_version < 9100) {	// 1.8 ALPHA
        ...
        if ($old_db_version >= 9000) { // sess_agnt_ID used in Phoenix-Alpha
        ...
        if ($old_db_version >= 9000) { // Uninstall all ALPHA (potentially incompatible) plugins
    ...
    if ($old_db_version < 9192) { // 1.8 ALPHA (block #3) - The payload that db_delta() handled before
        // This is a fix, which broke upgrade to 1.8 (from 1.6) in MySQL strict mode (inserted after 1.8 got released!):
        if ($DB->get_row('SHOW COLUMNS FROM T_hitlog LIKE "hit_referer_type"')) { // a niiiiiiiice extra check :p
    ...
    if ($old_db_version < 9290) {
        ...
        if (! is_null($old_auto_prune_stats) && $old_auto_prune_stats < 1) { // This means it has been disabled before, so set auto_prune_stats_mode to "off"!
            $DB->query('
    ...
    if ($old_db_version < 9320) { // Dropping hit_datetime because it's very slow on INSERT (dh)
        // This can be so long, it needs its own checkpoint protected block in case of failure
        if (db_index_exists('T_hitlog', 'hit_datetime')) { // only drop, if it still exists (may have been removed manually)
            echo 'Updating hitlog indexes... ';
            $DB->query('
					ALTER TABLE T_hitlog
    ...
    if ($old_db_version < 9326) {
        ...
        if (db_cols_exist('T_blogs', ['blog_pingb2evonet', 'blog_pingtechnorati', 'blog_pingweblogs', 'blog_pingblodotgs'])) {
            foreach ($DB->get_results('
                ...
                if ($row->blog_pingtechnorati || $row->blog_pingweblogs || $row->blog_pingblodotgs) { // if either one of the previous pingers was enabled, add ping-o-matic:
    ...
    if ($old_db_version < 9340) {
        ...
        if (db_index_exists('T_links', 'link_item_ID')) { // only drop, if it still exists (may have been removed manually)
            $DB->query('
					ALTER TABLE T_links
						DROP INDEX link_item_ID
					');
        }
        if (db_index_exists('T_links', 'link_dest_item_ID')) { // only drop, if it still exists (may have been removed manually)
            $DB->query('
					ALTER TABLE T_links
    ...
    */
    ...
    if ($old_db_version < 9408) {
        ...
        function add_basic_widget_9408($blog_ID, $container_name, $code, $type, $order, $params = null)
        {
            ...
            if (is_null($params)) { // NULL
                $params = 'NULL';
            } elseif (is_array($params)) { // array
        ...
        function insert_basic_widgets_9408($blog_id, $initial_install = false, $kind = '')
        {
            ...
            if ($initial_install && ! empty($blog_photoblog_ID)) { // In the case of initial install, we grab photos out of the photoblog (Blog #4)
            ...
            /* Menu */
            if ($kind != 'main') { // Don't add widgets to Menu container for Main collections
                // Home page
                add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 5, [
                    'link_type' => 'home',
                ]);
                if ($blog_id == $blog_b_ID) { // Recent Posts
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 10, [
                        'link_type' => 'recentposts',
                        'link_text' => T_('News'),
                    ]);
                }
                if ($kind == 'forum') { // Latest Topics and Replies ONLY for forum
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 13, [
                        'link_type' => 'recentposts',
                        'link_text' => T_('Latest topics'),
                    ]);
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 15, [
                        'link_type' => 'latestcomments',
                        'link_text' => T_('Latest replies'),
                    ]);
                }
                if ($kind == 'manual') { // Latest Topics and Replies ONLY for forum
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 13, [
                        'link_type' => 'recentposts',
                ...
                if ($kind == 'photo') { // Add menu with Photo index
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 18, [
                        'link_type' => 'mediaidx',
                        'link_text' => T_('Index'),
                    ]);
                }
                if ($kind == 'forum') { // Add menu with User Directory
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 20, [
                        'link_type' => 'users',
                    ]);
                }
                // Pages list:
                add_basic_widget_9408($blog_id, 'Menu', 'coll_page_list', 'core', 25);
                if ($kind == 'forum') { // My Profile
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 30, [
                        'link_type' => 'myprofile',
                    ]);
                }
                if ($kind == 'std') { // Categories
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 33, [
                        'link_type' => 'catdir',
                    ]);
                    // Archives
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 35, [
                        'link_type' => 'arcdir',
                    ]);
                    // Latest comments
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 37, [
                        'link_type' => 'latestcomments',
                    ]);
                }
                add_basic_widget_9408($blog_id, 'Menu', 'msg_menu_link', 'core', 50, [
                    'link_type' => 'messages',
                ]);
                add_basic_widget_9408($blog_id, 'Menu', 'msg_menu_link', 'core', 60, [
                    'link_type' => 'contacts',
                    'show_badge' => 0,
                ]);
                add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 70, [
                    'link_type' => 'login',
                ]);
                if ($kind == 'forum') { // Register
                    add_basic_widget_9408($blog_id, 'Menu', 'menu_link', 'core', 80, [
                        'link_type' => 'register',
                    ]);
                }
            }
            ...
            if ($blog_id != $blog_a_ID && $kind != 'forum' && (empty($events_blog_ID) || $blog_id != $events_blog_ID)) { // Item Tags
                add_basic_widget_9408($blog_id, 'Item Single', 'item_tags', 'core', 20);
            }
            if ($blog_id == $blog_b_ID) { // About Author
                add_basic_widget_9408($blog_id, 'Item Single', 'item_about_author', 'core', 25);
            }
            if (($blog_id == $blog_a_ID || (! empty($events_blog_ID) && $blog_id == $events_blog_ID)) && ! empty($install_test_features)) { // Google Maps
                add_basic_widget_9408($blog_id, 'Item Single', 'evo_Gmaps', 'plugin', 30);
            }
            if ($blog_id == $blog_a_ID || $kind == 'manual') { // Small Print
                add_basic_widget_9408($blog_id, 'Item Single', 'item_small_print', 'core', 40, [
                    'format' => ($blog_id == $blog_a_ID ? 'standard' : 'revision'),
                ]);
            }
            ...
            if ($kind == 'manual') {
                ...
                if (! empty($install_test_features)) {
                    if ($kind != 'forum' && $kind != 'manual') { // Current filters widget
                ...
                if ((! $initial_install || $blog_id != $blog_forums_ID) && $kind != 'forum') { // Don't install these Sidebar widgets for blog 'Forums'
                    ...
                    add_basic_widget_9408($blog_id, 'Sidebar', 'coll_longdesc', 'core', 40, [
                        'title' => '$title$',
                    ]);
                    ...
                    if ($blog_id == $blog_home_ID) { // Advertisements, Install only for blog #1 home blog
                        add_basic_widget_9408($blog_id, 'Sidebar', 'coll_item_list', 'core', 70, [
                    ...
                    if (! empty($blog_home_ID) && ($blog_id == $blog_a_ID || $blog_id == $blog_b_ID)) {
                        add_basic_widget_9408($blog_id, 'Sidebar', 'linkblog', 'core', 90, [
            ...
            if ($blog_id == $blog_b_ID) {
                add_basic_widget_9408($blog_id, 'Sidebar 2', 'coll_link_list', 'core', 5, [
                    'title' => 'Sidebar links',
                    'order_by' => 'RAND',
                ]);
            }
            ...
            if ($kind == 'main') { // Display blog title and tagline for main blogs
            ...
            if ($kind == 'main') { // Hide a title of the front intro post
                $featured_intro_params = [
                    'disp_title' => 0,
                ];
            }
            add_basic_widget_9408($blog_id, 'Front Page Main Area', 'coll_featured_intro', 'core', 10, $featured_intro_params);
            if ($kind == 'main') { // Add user links widget only for main kind blogs
                add_basic_widget_9408($blog_id, 'Front Page Main Area', 'user_links', 'core', 15);
            }
            $post_list_params = null;
            if ($kind == 'main') { // Display the posts from all other blogs if it is allowed by blogs setting "Collections to aggregate"
                $post_list_params = [
                    'blog_ID' => '',
                    'limit' => 50,
                    'attached_pics' => 'first',
            ...
            if ($kind != 'main') { // Don't install the "Recent Commnets" widget for Main blogs
            ...
            add_basic_widget_9408($blog_id, 'Mobile: Navigation Menu', 'menu_link', 'core', 20, [
                'link_type' => 'ownercontact',
            ]);
            add_basic_widget_9408($blog_id, 'Mobile: Navigation Menu', 'menu_link', 'core', 30, [
                'link_type' => 'home',
            ]);
            if ($kind == 'forum') { // Add menu with User Directory
                add_basic_widget_9408($blog_id, 'Mobile: Navigation Menu', 'menu_link', 'core', 40, [
                    'link_type' => 'users',
                ]);
            }

            /* Mobile Tools Menu */
            add_basic_widget_9408($blog_id, 'Mobile: Tools Menu', 'menu_link', 'core', 10, [
                'link_type' => 'login',
            ]);
            add_basic_widget_9408($blog_id, 'Mobile: Tools Menu', 'msg_menu_link', 'core', 20, [
                'link_type' => 'messages',
            ]);
            add_basic_widget_9408($blog_id, 'Mobile: Tools Menu', 'msg_menu_link', 'core', 30, [
                'link_type' => 'contacts',
                'show_badge' => 0,
            ]);
            add_basic_widget_9408($blog_id, 'Mobile: Tools Menu', 'menu_link', 'core', 50, [
                'link_type' => 'logout',
            ]);
    ...
    if ($old_db_version < 9900) {	// 3.0 part 1
    ...
    if ($old_db_version < 9910) {	// 3.0 part 2
        task_begin('Upgrading Blogs table... ');
    ...
    if ($old_db_version < 9930) {	// 3.1 continued
    ...
    if ($old_db_version < 9960) {	// 3.3
        ...
        foreach ($schema_queries as $table_name => $table_def) {
            if ($DB->query('SHOW TABLES LIKE \'' . $table_name . '\'')
                && preg_match('/\sENGINE\s*=\s*([a-z]+)/is', $table_def[1], $matches)) {	// If the table exists and has an ENGINE definition:
    ...
    if ($old_db_version < 9970) {	// 4.0 part 1
    ...
    if ($old_db_version < 9980) {	// 4.0 part 2
    ...
    if ($old_db_version < 9990) {	// 4.0 part 3
    ...
    if ($old_db_version < 10000) {	// 4.0 part 4
        ...
        if (db_col_exists('T_coll_user_perms', 'bloguser_perm_comments')) { // if user had perm_comments he now gets all 3 new perms also:
            $DB->query('UPDATE T_coll_user_perms
        ...
        if (db_col_exists('T_coll_group_perms', 'bloggroup_perm_comments')) { // if group had perm_comments he now gets all 3 new perms also:
            $DB->query('UPDATE T_coll_group_perms
    ...
    if ($old_db_version < 10100) {	// 4.1
        ...
        foreach ($rows as $row) {	// "IGNORE" is needed if we already created T_groups__groupsettings during upgrade to v0.8.9 at checkpoint 8050
    ...
    if ($old_db_version < 10400) {	// 4.2 part 2
        task_begin('Updating "Post by Email" settings...');
        $DB->query('UPDATE T_settings SET set_name = "eblog_autobr" WHERE set_name = "AutoBR"');
        task_end();

        if ($DB->get_var('SELECT set_value FROM T_settings WHERE set_name = "eblog_enabled"')) {	// eblog enabled, let's create a scheduled job for it
    ...
    if ($old_db_version < 10500) {	//  part 3
        task_begin('Upgrading hitlog table...');
        $DB->query("ALTER TABLE T_hitlog
        ...
        foreach ($userfields as $uf => $userfield) {
            if ($uf > 0) {
                if ($userfields[$uf - 1]->ufdf_ufgp_ID != $userfield->ufdf_ufgp_ID) {	// New group is starting, reset $userfield_order
                    $userfield_order = 1;
                }
            }
            $DB->query('UPDATE T_users__fielddefs
    ...
    if ($old_db_version < 10600) {	//  part 4
    ...
    if ($old_db_version < 10700) {	// part 5
    ...
    if ($old_db_version < 10800) {	// part 6 aka between "i1-i2" and "i2"
        ...
        if ($new_users_must_validate || ($new_users_must_validate == null)) { // newusers_mustvalidate setting is set to true, or it is not set at all. If it is not set, we know that the default value is true!
    ...
    if ($old_db_version < 10900) {	// part 7 aka "i3"
        ...
        if (count($users_emails) > 0) {
            ...
            foreach ($users_emails as $user_ID => $user_email) {
                if (preg_match('#@(.+)#i', strtolower($user_email), $ematch)) {	// Get email domain from user's email address
                    $email_domain = trim($ematch[1]);
                    $dom_ID = array_search($email_domain, $email_domains);

                    if (! $dom_ID) {	// Insert new email domain
        ...
        if (! empty($email_failed_logs)) {	// Update only the failed email logs to new values
    ...
    if ($old_db_version < 10970) {	// part 8/a trunk aka first part of "i4"
    ...
    if ($old_db_version < 10975) {	// part 8/b trunk aka first part of "i4"
    ...
    if ($old_db_version < 11000) {	// part 8/c trunk aka first part of "i4"
        ...
        $query = "INSERT INTO T_items__item_settings( iset_item_ID, iset_name, iset_value ) ";
        for ($i = 1; $i <= 8; $i++) { // For each custom fields:
            if ($i > 1) {
                $query .= ' UNION';
            }
        ...
        for ($i = 1; $i <= 5; $i++) { // drop custom double columns from items tabe
            db_drop_col('T_items__item', 'post_double' . $i);
        }
        for ($i = 1; $i <= 3; $i++) { // drop custom varchar columns from items tabe
        ...
        if ($ftyp) {	// Add f4v extension to mp4 file type, if not exists
        ...
        if ($result) { // blogs were found where Latitude and Longitude custom fields were set for google maps plugin
        ...
        if (! empty($result)) { // There are custom fields in blog settings
            $convert_field_values = '';
            $reorder_field_values = '';
            $old_prefix = "";
            $old_coll_ID = "";
            foreach ($result as $row) { // process each custom field
                $custom_id = uniqid('');
    ...
    if ($old_db_version < 11010) {	// part 9 trunk aka second part of "i4"
        ...
        if (empty($suspect_group_ID)) { // suspect group doesn't exists, check spammers because probably it does not exists either
            ...
            $suspect_group_ID = $DB->get_var($suspect_query);
            if ($suspect_group_ID) { // Set coll setting permissions for Misbehaving/Suspect Users in Forums
                $query = "
    ...
    if ($old_db_version < 11020) {	// part 10 trunk aka third part of "i4"
        task_begin('Upgrading users table...');
        ...
        if (count($users) > 0) {	// Get IPs only for users which have not IP in T_users table
    ...
    if ($old_db_version < 11025) {	// part 11 trunk aka fourth part of "i4"
        task_begin('Upgrading items table...');
    ...
    if ($old_db_version < 11100) {	// part 12 trunk aka "i5"
    ...
    if ($old_db_version < 11110) { // part 13 trunk aka second part of "i5"
        // Add new settings for antispam groups
        $antispam_group_settings = $DB->get_assoc('SELECT set_name, set_value
			 FROM T_settings
			WHERE set_name IN ( ' . $DB->quote(['antispam_suspicious_group', 'antispam_trust_groups']) . ')');
        if (count($antispam_group_settings) < 2) { // Insert new settings only if don't exist in DB
            task_begin('Updating general settings...');
            // Set antispam suspicious group
            if (! isset($antispam_group_settings['antispam_suspicious_group'])) { // Insert new value, Don't rewrite value if it already defined before
                ...
                if (! empty($suspect_group_ID)) { // Save setting value
            ...
            if (! isset($antispam_group_settings['antispam_trust_groups'])) { // Insert new value, Don't rewrite value if it already defined before
                $trust_groups = ['Administrators', 'Moderators', 'Trusted Users', 'Spammers/Restricted Users'];
                ...
                if (! empty($trust_groups_IDs)) { // Save setting value
                    $DB->query('INSERT INTO T_settings ( set_name, set_value ) VALUES
    ...
    if ($old_db_version < 11200) { // part 14 trunk aka "i6"
        task_begin('Upgrading comments table...');
        ...
        if (is_null($site_skins_enabled)) { // Insert new setting to disable site skins
            $DB->query('INSERT INTO T_settings ( set_name, set_value )
				VALUES ( ' . $DB->quote('site_skins_enabled') . ', ' . $DB->quote('0') . ' )');
        } elseif ($site_skins_enabled == '1') { // Disable site skins if it is enabled
            $DB->query('UPDATE T_settings
    ...
    if ($old_db_version < 11208) { // part 15.a trunk aka second part of "i6"
    ...
    if ($old_db_version < 11210) { // part 15.b trunk aka third part of "i6"
    ...
    if ($old_db_version < 11220) { // part 16.a trunk aka fourth part of "i6"
        task_begin('Upgrading countries table...');
    ...
    if ($old_db_version < 11240) { // part 16.d trunk aka seventh part of "i6"
        task_begin('Updating general settings...');
    ...
    if ($old_db_version < 11245) { // part 16.e trunk aka eighth part of "i6"
        task_begin('Updating Antispam IP Ranges table...');
        ...
        if (empty($current_default_locale)) { // The default locale is not set in the database, use the one from the config file
            global $default_locale;
            $current_default_locale = $default_locale;
        }
        if ($DB->get_var('SELECT loc_enabled FROM T_locales WHERE loc_locale = ' . $DB->quote($current_default_locale))) { // Update invalid user and collection locales to the default, but only if the default exists and it is enabled
            $DB->query(
                'UPDATE T_users
    ...
    if ($old_db_version < 11250) { // part 16.f trunk aka ninth part of "i6"
    ...
    if ($old_db_version < 11255) { // part 16.g trunk aka tenth part of "i6"
        task_begin('Updating post types...');
    ...
    if ($old_db_version < 11260) { // part 16.h trunk aka eleventh part of "i6"
    ...
    if ($old_db_version < 11270) { // part 16.i trunk aka 12th part of "i6"
    ...
    if ($old_db_version < 11280) { // part 16.j trunk aka 12th part of "i6"
    ...
    if ($old_db_version < 11285) { // part 16.k trunk aka 13th part of "i6"
    ...
    if ($old_db_version < 11286) {	// part 16.l trunk aka 14th part of "i6"
    ...
    if ($old_db_version < 11300) { // part 17 trunk aka "i7"
        task_begin('Upgrading locales table...');
    ...
    if ($old_db_version < 11310) { // part 18 trunk aka second part of "i7"
        task_begin('Upgrading cron tasks table...');
    ...
    if ($old_db_version < 11320) { // part 18.a trunk aka third part of "i7"
        ...
        if ($en_us_utf8_enabled !== null) { // The 'en-US-utf8' was enabled we must enable the 'en-US' even if it was not enabled before because we merged the two locales into one
            $en_us_enabled = $DB->get_var('SELECT loc_enabled FROM T_locales WHERE loc_locale = "en-US"');
            if ($en_us_enabled === null) { // Update "en-US-utf8" to "en-US"
                $DB->query('UPDATE T_locales SET loc_locale = "en-US" WHERE loc_locale = "en-US-utf8"');
            } elseif ($en_us_utf8_enabled && (! $en_us_enabled)) { // Enable the "en-US" locale because it was not enabled but the "en-US-utf8" was
                $DB->query('UPDATE T_locales SET loc_enabled = 1 WHERE loc_locale = "en-US"');
            }

            if ($en_us_enabled !== null) { // Remove the "en-US-utf8" locale if the "en_US" locale is already in the database
        ...
        if (! empty($existing_video_types)) { // Some video types arleady exist in DB, Exclude them from inserting
            foreach ($existing_video_types as $vtype) {
                unset($video_types[$vtype]);
            }
        }
        if (count($video_types)) { // Insert new video file types
            $DB->query("INSERT INTO T_filetypes
    ...
    if ($old_db_version < 11330) { // part 18.b trunk aka 4th part of "i7"
    ...
    if ($old_db_version < 11340) { // part 18.c trunk aka 5th part of "i7"
        task_begin('Upgrading blogs table...');
        $DB->query("ALTER TABLE T_blogs
    ...
    if ($old_db_version < 11350) { // part 18.d trunk aka 6th part of "i7"
    ...
    if ($old_db_version < 11360) { // part 18.e trunk aka 7th part of "i7"
        task_begin('Upgrade table posts... ');
        $DB->query('ALTER TABLE T_items__item
        ...
        foreach ($new_item_types as $blog_type => $item_type_data) {
            ...
            while ($item_type_ID !== null) { // Find first free post type ID starting with 100
                ...
                if ($item_type_ID === null) { // Use this free ID for new type
        ...
        $custom_fields = [];
        $blog_setting_delete_data = [];
        $blog_setting_item_types = [];
        if (count($setting_rows)) { // Initialize an array of custom fields from blog settings
            foreach ($setting_rows as $setting_row) {
                if (preg_match('/custom_(double|varchar)\d+/', $setting_row->cset_name, $matches)) { // It is a custom field
                    if (! isset($custom_fields[$setting_row->cset_coll_ID])) {
                        $custom_fields[$setting_row->cset_coll_ID] = [];
                    }
                    ...
                    foreach ($setting_rows as $s_row) {
                        if ($s_row->cset_name == 'custom_' . $cf_type . '_' . $cf_key) { // Label
                        ...
                        if ($s_row->cset_name == 'custom_fname_' . $cf_key) { // Name
            ...
            if (count($custom_fields)) { // Create post type for each blog with custom fields
                $BlogCache = &get_BlogCache();
                $itypes_insert_data = [];
                $item_type_max_ID = $DB->get_var('SELECT MAX( ityp_ID ) FROM T_items__type') + 1;
                foreach ($custom_fields as $blog_ID => $c_fields) {
                    if (! ($cf_Blog = $BlogCache->get_by_ID($blog_ID, false, false))) { // Skip incorrect blog ID
                        continue;
                    }
                    ...
                    if (count($blog_categories)) { // Set new post type for each post
                        ...
                        $page_type_max_ID = 0;
                        if (count($pages_IDs) > 0) { // We have the Pages that have the defined custom fields
        ...
        foreach ($blogs as $blog_ID) {
            if (isset($blog_setting_item_types[$blog_ID])) { // Set it from custom post type
                $current_item_type = $blog_setting_item_types[$blog_ID];
            } else { // Set it from first non reserved post type
        ...
        if (! empty($sql_update_blog_settings)) { // Execute a query to update the blog settings
            $DB->query('REPLACE INTO T_coll_settings ( cset_coll_ID, cset_name, cset_value )
        ...
        if ($DB->get_var('SELECT set_value FROM T_settings WHERE set_name = "site_color"') === null) { // Set default site color only when it is not defined yet
            $DB->query('INSERT INTO T_settings ( set_name, set_value )'
    ...
    if ($old_db_version < 11370) { // part 18.f trunk aka 8th part of "i7"
    ...
    if ($old_db_version < 11375) { // part 18.g trunk aka 9th part of "i7"
        task_begin('Upgrade table user field definitions... ');
        $DB->query('ALTER TABLE T_users__fielddefs
    ...
    if ($old_db_version < 11380) { // part 18.h trunk aka 10th part of "i7"
        task_begin('Update links table... ');
    ...
    if ($old_db_version < 11390) { // part 18.i trunk aka 11th part of "i7"
    ...
    if ($old_db_version < 11400) { // part 18.j trunk aka 12th part of "i7"
        task_begin('Upgrade table user field definitions... ');
        $DB->query('ALTER TABLE T_users__fielddefs
    ...
    if ($old_db_version < 11410) { // part 18.k trunk aka 13th part of "i7"
    ...
    if ($old_db_version < 11420) { // part 18.l trunk aka 14th part of "i7"
    ...
    if ($old_db_version < 11430) { // part 18.m trunk aka 15th part of "i7"
        task_begin('Upgrading tags table...');
        ...
        foreach ($not_ascii_tags as $not_ascii_tag) { // Replace each not ascii char with "_" in tag name in order to avoid error on below table upgrading
            $ascii_tag_name = preg_replace('/[^A-Za-z0-9_\-\s]/', '_', $not_ascii_tag->tag_name);
            $ascii_tag_name = str_replace('__', '_', $ascii_tag_name);

            if ($ascii_tag_name == $not_ascii_tag->tag_name) { // Skip this tag name because it doesn't contain not ascii chars really
                continue;
            }
    ...
    if (upg_task_start(11440, 'Upgrading base domains table...')) {	// part of 6.5.0
        $DB->query("ALTER TABLE T_basedomains
			MODIFY dom_name VARCHAR(250) COLLATE utf8_bin NOT NULL DEFAULT ''");
        upg_task_end();
    }

    if (upg_task_start(11450, 'Upgrading blog-group permissions table...')) {	// part of 6.6.0
        $DB->query("ALTER TABLE T_coll_group_perms
    ...
    if (upg_task_start(11453, 'Upgrading blog-user permissions table...')) {	// part of 6.6.0
        $DB->query("ALTER TABLE T_coll_user_perms
    ...
    if (upg_task_start(11456, 'Upgrade post types table...')) {	// part of 6.6.0
        $DB->query("ALTER TABLE T_items__type
			ADD COLUMN ityp_perm_level ENUM( 'standard', 'restricted', 'admin' ) COLLATE ascii_general_ci NOT NULL default 'standard'");
        upg_task_end();
    }

    if (upg_task_start(11460, 'Creating table for PostType-to-Collection relationships...')) {	// part of 6.6.0
    ...
    if (upg_task_start(11463, 'Updating collection permissions...')) {	// part of 6.6.0
        $DB->query('UPDATE T_coll_group_perms
    ...
    if (upg_task_start(11466, 'Updating post types table...')) {	// part of 6.6.0
        $DB->query('UPDATE T_items__type SET
    ...
    if (upg_task_start(11470, 'Updating widgets table...')) {	// part of 6.6.0
    ...
    if (upg_task_start(11480, 'Updating table for PostType-to-Collection relationships...')) {	// part of 6.6.1
        // Get all collections:
        ...
        $posttypes_collections = [];
        foreach ($collections as $collection_ID => $collection_type) {
            switch ($collection_type) { // Set what post types should not be enabled depending on the collection type:
                case 'main':
                case 'photo':
                    $skip_posttypes = [100, 200, 2000, 5000];
                    break;
                case 'forum':
                    $skip_posttypes = [1, 100, 2000, 5000];
                    break;
                case 'manual':
                    $skip_posttypes = [1, 200, 2000, 5000];
                    break;
                default: // 'std'
                    $skip_posttypes = [100, 200, 5000];
                    break;
            }
            foreach ($posttypes as $posttype_ID) {
                if (in_array($posttype_ID, $skip_posttypes)) { // Skip(Don't enable) this post type for the collection:
                    continue;
                }
                $posttypes_collections[] = '( ' . $posttype_ID . ', ' . $collection_ID . ' )';
            }
        }
        if (count($posttypes_collections)) { // Update the relationships only when at least one is required:
    ...
    if (upg_task_start(11482, 'Updating default post types for forums and manual collections...')) {	// part of 6.6.3
        $item_types = [
            'manual' => [
                'ID' => 100,
                'name' => 'Manual Page',
        ...
        foreach ($item_types as $collection_type => $item_type) {
            ...
            if (empty($item_type_ID)) { // Item type is not found in DB, Skip it:
                continue;
            }
        ...
        if (count($update_default_type_sql) > 0) { // Update default post types of collections:
            $DB->query('REPLACE INTO T_coll_settings ( cset_coll_ID, cset_name, cset_value )
    ...
    if (upg_task_start(11483, 'Updating general settings...')) {	// part of 6.6.4
        $DB->query('UPDATE T_settings
    ...
    if (upg_task_start(11484, 'Upgrade table item types...')) {	// part of 6.6.4
        $DB->query("ALTER TABLE T_items__type
			ADD ityp_use_parent ENUM( 'required', 'optional', 'never' ) COLLATE ascii_general_ci DEFAULT 'never' AFTER ityp_use_url");
        upg_task_end();
    }

    if (upg_task_start(11485, 'Upgrade table item types...')) {	// part of 6.6.6
        $DB->query('ALTER TABLE T_items__type
    ...
    if (upg_task_start(11486, 'Upgrade timestamp fields...')) {	// part of 6.6.6
        $DB->query('ALTER TABLE T_email__log
			MODIFY emlog_timestamp TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        $DB->query('ALTER TABLE T_email__returns
			MODIFY emret_timestamp TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
    ...
    if (upg_task_start(11600, 'Updating blogs settings...')) {	// part of 6.7.0
        ...
        if (count($blogs_abs_url)) { // Update only when at least one blog has an absolute base url
    ...
    if (upg_task_start(11605, 'Upgrading cron logs table...')) {	// part of 6.7.0
        $DB->query("ALTER TABLE T_cron__log
    ...
    if (upg_task_start(11630, 'Upgrading plugin settings table...')) {	// part of 6.7.0
        $DB->query('ALTER TABLE T_pluginsettings
			MODIFY pset_name VARCHAR( 60 ) COLLATE ascii_general_ci NOT NULL');
        upg_task_end();
    }

    if (upg_task_start(11635, 'Upgrading email campaigns table...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11640, 'Upgrading hitlog table...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11645)) {	// part of 6.7.0
        ...
        foreach ($collections as $coll_ID => $coll_type) {
            ...
            if ($coll_type != 'forum') {	// Item Tags:
                add_basic_widget_11670($coll_ID, 'Item Single', 'item_tags', 'core', 30);
            }
            if ($coll_type == 'std') {	// About Author:
                add_basic_widget_11670($coll_ID, 'Item Single', 'item_about_author', 'core', 40);
            }
            if ($coll_type == 'std' || $coll_type == 'manual') {	// Small Print:
                add_basic_widget_11670($coll_ID, 'Item Single', 'item_small_print', 'core', 50, [
                    'format' => ($coll_type == 'std' ? 'standard' : 'revision'),
                ]);
            }
            // Seen by:
            add_basic_widget_11670(
                $coll_ID,
                'Item Single',
                'item_seen_by',
                'core',
    ...
    if (upg_task_start(11650, 'Upgrading user organizations table...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11655, 'Creating table for Poll questions...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11660, 'Creating table for Poll options...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11665, 'Creating table for Poll answers...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11670, 'Creating default polls...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11675, 'Creating table for secondary user groups...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11695, 'Upgrade table item types...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11700, 'Update item types for new created fields...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11705, 'Upgrading users organization table...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11710, 'Deleting back-office skin "chicago"...')) {	// part of 6.7.0
        ...
        if (file_exists($skin_chicago_path) && ! rmdir_r($skin_chicago_path)) {	// Display a warning if no permissions to delete the skin folder:
    ...
    if (upg_task_start(11715, 'Replace widgets "Simple Sidebar Links list" and "Simple Linkblog Links list" with "Universal Item list"...')) {	// part of 6.7.0
        $old_widgets = $DB->get_results(
            'SELECT wi_ID, wi_code, wi_params
        ...
        foreach ($old_widgets as $old_widget) {
            ...
            $widget_params = empty($old_widget->wi_params) ? [] : unserialize($old_widget->wi_params);
    ...
    if (upg_task_start(11720, 'Replace renamed widgets...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11725, 'Upgrade table post type...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11730, 'Upgrade table invitation codes...')) {	// part of 6.7.0
    ...
    if (upg_task_start(11735, 'Upgrade table files...')) {	// part of 6.7.0
        $DB->query('ALTER TABLE T_files
			MODIFY file_type ENUM( "image", "audio", "video", "other" ) COLLATE ascii_general_ci NULL DEFAULT NULL');
        upg_task_end();
    }

    if (upg_task_start(11740, 'Update table user settings for new post/comment moderation settings...')) {	// part of 6.7.0
        $DB->query('INSERT INTO T_users__usersettings ( uset_user_ID, uset_name, uset_value )
    ...
    if (upg_task_start(11745, 'Upgrading items table...')) {	// part of 6.7.1
    ...
    if (upg_task_start(11750, 'Upgrading comments table...')) {	// part of 6.7.1
    ...
    if (upg_task_start(11755, 'Creating collection user favorites table...')) {	// part of 6.7.1-beta
        ...
        $user_colls_count = [];
        $insert_values = [];
        foreach ($colls as $coll) {	// Collection owners will automatically favorite their collections:
            if (! isset($user_colls_count[$coll->blog_owner_user_ID])) {	// Initialize a count of favorite collections for each user:
                $user_colls_count[$coll->blog_owner_user_ID] = 1;
            }
            if ($user_colls_count[$coll->blog_owner_user_ID] <= 7) {	// Make favorite only first 7 collections for each user:
                $insert_values[] = '( ' . $coll->blog_ID . ', ' . $coll->blog_owner_user_ID . ' )';
                $user_colls_count[$coll->blog_owner_user_ID]++;
            }
        }

        if (count($user_IDs) <= 10) {	// Make collections favorite for other users only if system has <= 10 users:
            foreach ($user_IDs as $user_ID) {
                if (! isset($user_colls_count[$user_ID])) {	// Initialize a count of favorite collections for each user:
                    $user_colls_count[$user_ID] = 1;
                }
                foreach ($colls as $coll) {
                    if ($user_colls_count[$user_ID] > 7) {	// Make favorite only first 7 collections for each user:
                        break;
                    }
                    if ($coll->blog_favorite && $user_ID != $coll->blog_owner_user_ID) {	// Skip collection owner, because it has this collection as favorite automatically above:
        ...
        if (count($insert_values)) {	// Insert rows of favorite collections per users in DB:
    ...
    if (upg_task_start(11760, 'Upgrading email campaigns table...')) {	// part of 6.7.1-beta
        db_add_col('T_email__campaign', 'ecmp_send_ctsk_ID', 'INT(10) UNSIGNED NULL DEFAULT NULL');
        upg_task_end();
    }

    if (upg_task_start(11765, 'Upgrading files table...')) { // part of 6.7.1-beta
    ...
    if (upg_task_start(11770, 'Upgrading files table...')) { // part of 6.7.3-beta
        db_add_index('T_files', 'file_creator_user_id', 'file_creator_user_id');
        upg_task_end();
    }

    if (upg_task_start(11775, 'Upgrading posts and comments statuses depeding on max allowed status by their collections...')) {	// part of 6.7.4-stable
        $BlogCache = &get_BlogCache();
    ...
    if (upg_task_start(11780, 'Upgrading subscription settings of collections...')) {	// part of 6.7.4-stable
        $DB->query('REPLACE INTO T_coll_settings ( cset_coll_ID, cset_name, cset_value )
    ...
    if (upg_task_start(11785, 'Upgrading email log table...')) {	// part of 6.7.7-stable
        $DB->query('ALTER TABLE T_email__log
			MODIFY emlog_message MEDIUMTEXT DEFAULT NULL');
        upg_task_end();
    }

    if (upg_task_start(11790, 'Fix number of favorite collections per each user...')) {	// part of 6.7.7-stable
        ...
        foreach ($more_fav_coll_user_IDs as $more_fav_coll_user_ID) {
            ...
            if (count($first_coll_IDs)) {	// Delete collections from favorite list:
    ...
    if (upg_task_start(11795, 'Upgrading posts table...')) {	// part of 6.7.7-stable
        $DB->query('ALTER TABLE T_items__item
			MODIFY post_excerpt_autogenerated  TINYINT(1) NOT NULL DEFAULT 1');
        upg_task_end();
    }

    if (upg_task_start(11800, 'Upgrading collection-user permissions table...')) {	// part of 6.7.7-stable
        $DB->query('ALTER TABLE T_coll_user_perms
    ...
    if (upg_task_start(11805, 'Upgrading collection-group permissions table...')) {	// part of 6.7.7-stable
        $DB->query('ALTER TABLE T_coll_group_perms
    ...
    if (upg_task_start(11810, 'Upgrading plugins table...')) {	// part of 6.7.7-stable
        $DB->query('ALTER TABLE T_plugins
			MODIFY plug_priority TINYINT UNSIGNED NOT NULL default 50');
        upg_task_end();
    }

    if (upg_task_start(12000, 'Add new types "Text" and "HTML" for custom fields of item types...')) {	// part of 6.8.0-alpha
        $DB->query('ALTER TABLE T_items__item_settings
    ...
    if (upg_task_start(12005, 'Update widget container "Item Single"...')) {	// part of 6.8.0-alpha
        ...
        foreach ($coll_item_single_widgets as $coll_item_single_widget) {
            if (! isset($coll_widgets[$coll_item_single_widget['wi_coll_ID']])) {	// If the "Item Single" has no widget "Item Content":
                $coll_widgets[$coll_item_single_widget['wi_coll_ID']] = 1;
            }
            if ($coll_item_single_widget['wi_code'] == 'item_content') {	// If the "Item Single" contains widget "Item Content" then keep an order of this widget:
                $coll_widgets[$coll_item_single_widget['wi_coll_ID']] = $coll_item_single_widget['wi_order'] + 1;
            }
        }
        $item_attachments_widget_rows = [];
        foreach ($coll_widgets as $coll_ID => $widget_order) {	// Insert new widget "Item Attachments" to each colleaction that has a container "Item Single":
            ...
            if ($not_unique_widget_ID > 0) {	// The collection has not unique widget order:
                $update_order_widgets = [];
                foreach ($coll_item_single_widgets as $coll_item_single_widget) {
                    if ($coll_item_single_widget['wi_coll_ID'] == $coll_ID && $coll_item_single_widget['wi_order'] >= $widget_order) {	// Increase a widget order to avoid mysql errors of duplicate entry:
                        $update_order_widgets[] = $coll_item_single_widget['wi_ID'];
                    }
                }
                for ($w = count($update_order_widgets) - 1; $w >= 0; $w--) {	// Update not unique widget orders:
                    $DB->query('UPDATE ' . $tableprefix . 'widget
        ...
        if (count($item_attachments_widget_rows)) {	// Insert new widgets "Item Attachments" into DB:
    ...
    if (upg_task_start(12010, 'Create new widget container "404 Page"...')) {	// part of 6.8.0-alpha
        $coll_IDs = $DB->get_col('SELECT blog_ID FROM T_blogs');
        if ($coll_IDs) {
            $page_404_widget_rows = [];
    ...
    if (upg_task_start(12015, 'Rename table "T_antispam" to "T_antispam__keyword"...')) {	// part of 6.8.0-alpha
    ...
    if (upg_task_start(12020, 'Upgrade table user post data...')) {	// part of 6.8.0-alpha
    ...
    if (upg_task_start(12025, 'Creating table for user profile visits...')) {	// part of 6.8.0-alpha
    ...
    if (upg_task_start(12030, 'Upgrade base domains table...')) { // part of 6.8.0-alpha
    ...
    if (upg_task_start(12035, 'Create table for items votes...')) {	// part of 6.8.0-alpha
    ...
    if (upg_task_start(12040, 'Upgrade table posts for voting...')) {	// part of 6.8.0-alpha
        $DB->query('ALTER TABLE T_items__item
    ...
    if (upg_task_start(12045, 'Update table post types...')) {	// part of 6.8.0-alpha
        $DB->query('UPDATE T_items__type
    ...
    if (upg_task_start(12050, 'Upgrade table post types...')) {	// part of 6.8.0-alpha
        $DB->query('ALTER TABLE T_items__type
    ...
    if (upg_task_start(12055, 'Upgrade links table...')) {	// part of 6.8.0-alpha
        db_add_col('T_links', 'link_ecmp_ID', 'int(11) unsigned  NULL COMMENT \'Used for linking files to email campaign\' AFTER link_usr_ID');
        upg_task_end();
    }

    if (upg_task_start(12060, 'Upgrade files table...')) {	// part of 6.8.0-alpha
        $DB->query('ALTER TABLE T_files
    ...
    if (upg_task_start(12065, 'Upgrade blogs table...')) { // part of 6.8.0-alpha
        db_add_col('T_blogs', 'blog_http_protocol', 'ENUM("always_redirect", "allow_both") DEFAULT "always_redirect" AFTER blog_access_type');
        upg_task_end();
    }

    if (upg_task_start(12070, 'Create table for temporary IDs...')) {	// part of 6.8.0-alpha
    ...
    if (upg_task_start(12075, 'Upgrade links table...')) {	// part of 6.8.0-alpha
        $DB->query('ALTER TABLE T_links
    ...
    if (upg_task_start(12080, 'Create table for post type status...')) { // part of 6.8.0-alpha
    ...
    if (upg_task_start(12085, 'Update widget container "Item Single"...')) {	// part of 6.8.0-alpha
        $SQL = new SQL('Get all collections that have a widget container "Item Single"');
        $SQL->SELECT('wi_ID, wi_coll_ID, wi_code, wi_order');
        $SQL->FROM($tableprefix . 'widget');
        $SQL->WHERE('wi_sco_name = "Item Single"');
        ...
        foreach ($coll_item_single_widgets as $coll_item_single_widget) {
            ...
            if ($coll_item_single_widget['wi_code'] == 'item_tags') { // If the "Item Single" contains widget "Item Tags" then make order of this widget a negative
                $coll_widgets[$coll_item_single_widget['wi_coll_ID']] = -1;
            }
            if ($coll_item_single_widget['wi_code'] == 'item_attachments' && $coll_widgets[$coll_item_single_widget['wi_coll_ID']] > 0) {	// If the "Item Single" contains widget "Item Content" and the widget order is positive then keep an order of this widget:
                $coll_widgets[$coll_item_single_widget['wi_coll_ID']] = $coll_item_single_widget['wi_order'] + 1;
            }
        }

        $item_tags_widget_rows = [];
        foreach ($coll_widgets as $coll_ID => $widget_order) {	// Insert new widget "Item Tags" to each collection that has a container "Item Single" with no existing "Item Tags" widget :
            if ($widget_order > 0) {
        ...
        if (count($item_tags_widget_rows)) {	// Insert new widgets "Item Tags" into DB:
            $DB->query('INSERT INTO ' . $tableprefix . 'widget ( wi_coll_ID, wi_sco_name, wi_order, wi_code )
    ...
    if (upg_task_start(12090, 'Creating table for Hits aggregations...')) {	// part of 6.8.0-alpha
        db_create_table(
            'T_hits__aggregate',
            "
    ...
    if (upg_task_start(12095, 'Creating table for aggregations of hit sessions...')) {	// part of 6.8.0-alpha
        db_create_table(
    ...
    if (upg_task_start(12100, 'Update widget container "Item Single" by adding "Item Location" widget...')) {	// part of 6.8.0-alpha
        $SQL = new SQL('Get all collections that have a widget container "Item Single"');
        $SQL->SELECT('wi_ID, wi_coll_ID, wi_code, wi_order');
        $SQL->FROM($tableprefix . 'widget');
        $SQL->WHERE('wi_sco_name = "Item Single"');
        ...
        $coll_item_single_widgets = $DB->get_results($SQL->get(), ARRAY_A, $SQL->title);
        $coll_widgets = [];
        $coll_attachments_widget = [];
        foreach ($coll_item_single_widgets as $coll_item_single_widget) {
            if (! isset($coll_widgets[$coll_item_single_widget['wi_coll_ID']])) {	// If the "Item Single" has no widget "Item Content":
                $coll_widgets[$coll_item_single_widget['wi_coll_ID']] = 1;
            }
            if ($coll_item_single_widget['wi_code'] == 'item_content' && ! in_array($coll_item_single_widget['wi_coll_ID'], $coll_attachments_widget)) { // If the "Item Single" contains widget "Item Content" then take order of this widget
                $coll_widgets[$coll_item_single_widget['wi_coll_ID']] = $coll_item_single_widget['wi_order'] + 1;
            }
            if ($coll_item_single_widget['wi_code'] == 'item_attachments') { // If the "Item Single" contains widget "Item Attachments" then take order of this widget
                $coll_attachments_widget[] = $coll_item_single_widget['wi_coll_ID'];
                $coll_widgets[$coll_item_single_widget['wi_coll_ID']] = $coll_item_single_widget['wi_order'] + 1;
            }
        }

        $item_locations_widget_rows = [];
        foreach ($coll_widgets as $coll_ID => $widget_order) {	// Insert new widget "Item Location" to each collection that has a container "Item Single":
        ...
        if (count($item_locations_widget_rows)) {	// Insert new widgets "Item Locations" into DB:
            $DB->query('INSERT INTO ' . $tableprefix . 'widget ( wi_coll_ID, wi_sco_name, wi_order, wi_code )
    ...
    if (upg_task_start(12105, 'Upgrade items table...')) {	// part of 6.8.0-alpha
        $DB->query("ALTER TABLE T_items__item
    ...
    if (upg_task_start(12110, 'Upgrade comments table...')) {	// part of 6.8.0-alpha
        $DB->query("ALTER TABLE T_comments
    ...
    if (upg_task_start(12115, 'Upgrading collections table...')) {	// part of 6.8.0-alpha
        $DB->query('ALTER TABLE T_blogs
			MODIFY blog_type VARCHAR( 16 ) COLLATE ascii_general_ci DEFAULT "std" NOT NULL');
        upg_task_end();
    }

    if (upg_task_start(12120, 'Upgrade locales table...')) { // part of 6.8.0-alpha
    ...
    if (upg_task_start(12125, 'Upgrade locales table...')) {	// part of 6.8.0-alpha
        ...
        foreach ($db_locale_rows as $loc_data) {
            ...
            if (preg_match('/[^dDjmnYy:\.,\-_\/\\\\ ]/', $loc_data_datefmt)) {	// If locale date format contains at least one invalid character then use default date format:
    ...
    if (upg_task_start(12130, 'Create new widget container "Item Single Header"...')) {	// part of 6.8.0-alpha
        $SQL = new SQL('Get all collections that have a widget container "Item Single"');
        ...
        $item_info_line_widget_rows = [];
        foreach ($coll_widgets as $coll_ID => $blog_type) {	// Insert new widget "Item Info Line" to each collection that has a container "Item Single":
            if (in_array($blog_type, ['forum', 'group'])) {
        ...
        if (count($item_info_line_widget_rows)) {	// Insert new widget "Item Info Line" into DB:
    ...
    if (upg_task_start(12135, 'Upgrade table of user field definitions...')) {	// part of 6.8.0-alpha
        $DB->query('ALTER TABLE T_users__fielddefs
			MODIFY ufdf_code varchar(20) COLLATE ascii_bin UNIQUE NOT NULL COMMENT "Code MUST be lowercase ASCII only"');
        upg_task_end();
    }

    if (upg_task_start(12140, 'Updating file types table...')) {	// part of 6.7.10-stable moved here so it also applies to 6.8.3 -> 6.8.4 upgrades
        $DB->query('UPDATE T_filetypes
				SET ftyp_allowed = "admin"
    ...
    if (upg_task_start(12145, 'Upgrade Temporary IDs table...')) {	// part of 6.8.4-stable
        $DB->query('ALTER TABLE T_temporary_ID
			ADD tmp_coll_ID INT(11) UNSIGNED NULL');
        upg_task_end();
    }

    if (upg_task_start(12150, 'Upgrade categories table...')) {	// part of 6.8.4-alpha
        db_add_col('T_categories', 'cat_image_file_ID', 'int(10) unsigned  NULL AFTER cat_blog_ID');
        upg_task_end();
    }

    if (upg_task_start(12155, 'Renaming notification logo setting...')) { // part of 6.8.5-stable
        $notification_logo = $DB->get_var('SELECT set_value FROM T_settings WHERE set_name = "notification_logo"');
        if (! empty($notification_logo)) {
            if (is_numeric($notification_logo)) { // We have a numeric value, it's possible that this is already a file_ID.
    ...
    if (upg_task_start(12160, 'Renaming user setting...')) {	// part of 6.8.6-stable
    ...
    if (upg_task_start(12161, 'Updating attachment positions...')) {	// part of 6.8.7-stable
        $DB->query('UPDATE T_links
    ...
    if (upg_task_start(12165, 'Upgrade table of users...')) {	// part of 6.9.0-beta
    ...
    if (upg_task_start(12170, 'Updating users pass storage...')) {	// part of 6.9.0-beta
        $DB->query('ALTER TABLE T_users MODIFY COLUMN user_pass VARBINARY(32)');
    ...
    if (upg_task_start(12175, 'Upgrade table of users...')) {	// part of 6.9.0-beta
        $DB->query('ALTER TABLE T_users
			MODIFY user_salt VARCHAR(32) NOT NULL default ""');
        upg_task_end();
    }

    if (upg_task_start(12180, 'Updating base domains table...')) {	// part of 6.9.0-beta
        ...
        if (count($domains)) {	// If at least one duplicated domain is detected,
            // Find what domains should be kept and what deleted depending on their status:
            $status_orders = ['blocked', 'suspect', 'unknown', 'trusted'];
            $duplicated_domains = [];
            foreach ($domains as $domain) {
                ...
                foreach ($status_orders as $status_order) {
                    ...
                    if ($s_index !== false) {	// If the highest status is detected:
    ...
    if (upg_task_start(12185, 'Upgrading posts table...')) {	// part of 6.9.0-beta
    ...
    if (upg_task_start(12186, 'Updating attachment positions...')) {	// part of 6.9.0-stable (This is a duplicated 12161 block from 6.8.7-stable)
        $DB->query('UPDATE T_links
			INNER JOIN T_files ON file_ID = link_file_ID
			       AND file_type NOT IN ( "image", "audio", "video" )
			  SET link_position = "attachment"
    ...
    if (upg_task_start(12190, 'Upgrade skins table...')) {	// part of 6.9.0-beta
        $DB->query('ALTER TABLE T_skins__skin
			MODIFY skin_type enum("normal","feed","sitemap","mobile","tablet","rwd") COLLATE ascii_general_ci NOT NULL default "normal"');
        upg_task_end();
    }

    if (upg_task_start(12200, 'Updating group permissions...')) {	// part of 6.9.0-beta
        $DB->query('UPDATE T_groups__groupsettings
    ...
    if (upg_task_start(12210, 'Create new widget container "Forum Front Secondary Area" for forums...')) {	// part of 6.9.1-beta
        $DB->begin();
        ...
        $item_tags_widget_rows = [];
        foreach ($forum_collections as $coll_ID) {	// Insert new widget "Collection Activity Stats" to each forum collection
            $widget_order = 10;
            ...
            $not_unique_widget_ID = $DB->get_var('SELECT wi_ID
				...
				WHERE wi_coll_ID = ' . $coll_ID . '
					...
					AND wi_order = ' . $widget_order);
            if ($not_unique_widget_ID > 0) {	// The collection has no unique widget order, move all widgets with wi_order >= 10:
                $DB->query('UPDATE ' . $tableprefix . 'widget
						SET wi_order = wi_order + 1
						WHERE wi_coll_ID =' . $coll_ID . '
						AND wi_sco_name = "Forum Front Secondary Area"
    ...
    if (upg_task_start(12220, 'Create new widget container "Item Page"...')) { // part of 6.9.1-beta
        // Get all collections
        ...
        if (count($coll_widgets)) {	// Insert new widgets into DB:
            $DB->query('INSERT INTO ' . $tableprefix . 'widget ( wi_coll_ID, wi_sco_name, wi_order, wi_code )
    ...
    if (upg_task_start(12230, 'Update menu link widget...')) {	// part of 6.9.1-beta
        $DB->query('UPDATE ' . $tableprefix . 'widget
    ...
    if (upg_task_start(12240, 'Create new widget "Item Link"...')) {	// part of 6.9.1-beta
        $SQL = new SQL('Get all collections that have a widget container "Item Single"');
        $SQL->SELECT('wi_ID, wi_coll_ID, wi_order, wi_enabled, wi_code, wi_params, blog_type');
        $SQL->FROM($tableprefix . 'widget');
        $SQL->FROM_add('LEFT JOIN T_blogs ON blog_id = wi_coll_ID');
        ...
        $item_single_rows = [];
        foreach ($collections as $collection) {	// Insert new widget "Collection Activity Stats" to each forum collection
            ...
            if (in_array($collection['wi_code'], ['item_content', 'item_attachments'])) {
                $item_single_rows[$collection['wi_coll_ID']] = $collection['wi_order'] + 1;
            }
        }

        $item_link_widget_rows = [];
        foreach ($item_single_rows as $coll_ID => $wi_order) {
            ...
            if ($not_unique_widget_ID > 0) {	// The collection has no unique widget order, move all widget order >= to current order:
                $DB->query('UPDATE ' . $tableprefix . 'widget
						SET wi_order = wi_order + 1
						WHERE wi_coll_ID = ' . $coll_ID . '
						AND wi_sco_name = "Item Single"
        ...
        if (count($item_link_widget_rows)) {	// Insert new widgets "Item Link" into DB:
            $DB->query('INSERT INTO ' . $tableprefix . 'widget ( wi_coll_ID, wi_sco_name, wi_order, wi_code )
    ...
    if (upg_task_start(12250, 'Upgrading table of post type custom fields...')) {	// part of 6.9.1-beta
        db_add_col('T_items__type_custom_field', 'itcf_note', 'VARCHAR(255) NULL DEFAULT NULL');
        upg_task_end();
    }

    if (upg_task_start(12260, 'Upgrading table for a latest versions of the PO files...')) {	// part of 6.9.1-beta
        db_add_index('T_i18n_translated_string', 'itst_iost_ID_locale', 'itst_iost_ID, itst_locale');
        upg_task_end();
    }

    if (upg_task_start(12262, 'Upgrade datetime fields to timestamp type in users table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_users
    ...
    if (upg_task_start(12263, 'Upgrade datetime fields to timestamp type in cron tasks table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_cron__task
			MODIFY ctsk_start_datetime TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        upg_task_end();
    }

    if (upg_task_start(12264, 'Upgrade datetime fields to timestamp type in cron logs table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_cron__log
    ...
    if (upg_task_start(12265, 'Upgrade datetime fields to timestamp type in posts table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_items__item
    ...
    if (upg_task_start(12266, 'Upgrade datetime fields to timestamp type in comments table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_comments
			MODIFY comment_date TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        upg_task_end();
    }

    if (upg_task_start(12267, 'Upgrade datetime fields to timestamp type in post versions table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_items__version
			MODIFY iver_edit_datetime TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        upg_task_end();
    }

    if (upg_task_start(12268, 'Upgrade datetime fields to timestamp type in links table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_links
    ...
    if (upg_task_start(12269, 'Upgrade datetime fields to timestamp type in message threads table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_messaging__thread
			MODIFY thrd_datemodified TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        upg_task_end();
    }

    if (upg_task_start(12270, 'Upgrade datetime fields to timestamp type in messages table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_messaging__message
			MODIFY msg_datetime TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        upg_task_end();
    }

    if (upg_task_start(12279, 'Upgrade datetime fields to timestamp type in user reports table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_users__reports
			MODIFY urep_datetime TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        upg_task_end();
    }

    if (upg_task_start(12280, 'Upgrade datetime fields to timestamp type in message contacts table...')) {	// part of 6.9.1-beta
        $DB->query('ALTER TABLE T_messaging__contact
			MODIFY mct_last_contact_datetime TIMESTAMP NOT NULL DEFAULT \'2000-01-01 00:00:00\'');
        upg_task_end();
    }

    if (upg_task_start(12290, 'Add new type "URL" for custom fields of item types...')) {	// part of 6.9.2-beta
        $DB->query('ALTER TABLE T_items__type_custom_field
			MODIFY COLUMN itcf_type ENUM( "double", "varchar", "text", "html", "url" ) COLLATE ascii_general_ci NOT NULL');
        upg_task_end();
    }

    if (upg_task_start(12300, 'Clear invalid values of item type custom fields...')) {	// part of 6.9.2-beta
        ...
        foreach ($custom_fields as $custom_field) {	// Delete all invalid/obsolete values of each item type with custom fields:
            $DB->query(
                'DELETE FROM T_items__item_settings
    ...
    if (upg_task_start(12310, 'Upgrading goals table...')) {	// part of 6.9.3-beta
        db_add_index('T_track__goal', 'goal_gcat_ID', 'goal_gcat_ID');
        upg_task_end();
    }

    if (upg_task_start(12320, 'Updating plugin "Short Links"...')) {	// part of 6.9.3-beta
        ...
        foreach ($coll_settings as $coll_setting) {	// Update old plugin setting to new format:
    ...
    if (upg_task_start(12330, 'Upgrade categories table...')) {	// part of 6.9.4
        db_add_col('T_categories', 'cat_social_media_image_file_ID', 'int(10) unsigned  NULL AFTER cat_image_file_ID');
        upg_task_end();
    }

    if (upg_task_start(12340, 'Upgrade collections table...')) {	// part of 6.9.4
        $DB->begin();
    ...
    if (upg_task_start(12350, 'Adding item versions index...')) { // part of 6.9.4
        db_add_index('T_items__version', 'iver_edit_user_ID', 'iver_edit_user_ID');
        upg_task_end();
    }

    if (upg_task_start(12358, 'Remove skins with duplicating class names...')) {	// part of 6.9.4
        ...
        if (count($duplicating_skins) > 0) {
            ...
            $skin_fields = ['blog_normal_skin_ID', 'blog_mobile_skin_ID', 'blog_tablet_skin_ID'];
            ...
            foreach ($old_skin_colls as $old_skin_coll) {
                $coll_update_fields = [];
                $coll_settings_update_fields = [];
                foreach ($skin_fields as $skin_field) {
                    if (isset($duplicating_skins[$old_skin_coll->$skin_field])) {	// If the skin is old folder format then use skin ID of new format skin folder:
    ...
    if (upg_task_start(12360, 'Upgrading skin table...')) { // part of 6.9.4
    ...
    if (upg_task_start(12365, 'Upgrading settings tables...')) {	// part of 6.9.4
    ...
    if (upg_task_start(12370, 'Creating email newsletters table...')) {	// part of 6.10.0-beta
        db_create_table(
            'T_email__newsletter',
            "
    ...
    if (upg_task_start(12380, 'Creating default newsletters...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12390, 'Creating email newsletter subscriptions table...')) {	// part of 6.10.0-beta
        db_create_table(
    ...
    if (upg_task_start(12400, 'Updating user newsletter subscriptions...')) {	// part of 6.10.0-beta
        ...
        if (count($newsletter_subscription_rows)) {	// Insert user newsletter subscriptions:
    ...
    if (upg_task_start(12410, 'Upgrading email campaigns table...')) {	// part of 6.10.0-beta
        $DB->query('ALTER TABLE T_email__campaign
    ...
    if (upg_task_start(12420, 'Upgrading email campaigns table...')) {	// part of 6.10.0-beta
        $DB->query('ALTER TABLE T_email__campaign
    ...
    if (upg_task_start(12430, 'Updating general default newsletter settings...')) {	// part of 6.10.0-beta
        ...
        $new_settings = [];
        if (! isset($old_settings['def_newsletter_news']) || $old_settings['def_newsletter_news']) {	// First newsletter setting(news) is enabled by default or saved in DB:
            $new_settings[] = '1';
        }
        if (! empty($old_settings['def_newsletter_ads'])) {	// Second newsletter setting(ads) is saved in DB:
            $new_settings[] = '2';
        }
    ...
    if (upg_task_start(12440, 'Upgrading email newsletters table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12450, 'Upgrading email newsletter subscriptions table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12460, 'Upgrading email newsletter subscriptions table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12470, 'Upgrading email campaigns table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12480, 'Upgrading email campaign send data table...')) { // part of 6.10.0-beta
    ...
    if (upg_task_start(12490, 'Creating user tags and user-to-tag tables...')) {	// part of 6.10.0-beta
        db_create_table(
            'T_users__tag',
            "
    ...
    if (upg_task_start(12500, 'Upgrading email campaigns table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12510, 'Upgrading email log table...')) { // part of 6.10.0-beta
    ...
    if (upg_task_start(12520, 'Creating automation tables...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12530, 'Creating automation user state table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12540, 'Upgrading email campaigns table...')) { // part of 6.10.0-beta
        db_add_col('T_email__campaign', 'ecmp_user_tag', 'VARCHAR(255) NULL AFTER ecmp_auto_send');
        upg_task_end();
    }

    if (upg_task_start(12550, 'Upgrading email campaign send data table...')) { // part of 6.10.0-beta
        db_add_col('T_email__campaign_send', 'csnd_clicked_unsubscribe', 'TINYINT(1) UNSIGNED DEFAULT 0 AFTER csnd_emlog_ID');
        upg_task_end();
    }

    if (upg_task_start(12560, 'Upgrading automation step table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12570, 'Creating automation newsletter table...')) {	// part of 6.10.0-beta
        ...
        // Remove old columns:
        db_upgrade_cols('T_automation__automation', [
            'DROP' => ['autm_enlt_ID', 'autm_autostart'],
        ]);
        upg_task_end();
    }

    if (upg_task_start(12580, 'Upgrading email campaign send data table...')) {	// part of 6.10.0-beta
        db_upgrade_cols('T_email__campaign_send', [
            'ADD' => [
    ...
    if (upg_task_start(12590, 'Upgrading email log table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12600, 'Upgrading email log table...')) {	// part of 6.10.0-beta
        db_add_col('T_email__log', 'emlog_autm_ID', 'INT UNSIGNED DEFAULT NULL');
        upg_task_end();
    }

    if (upg_task_start(12610, 'Upgrading email newsletter subscriptions table...')) {	// part of 6.10.0-beta
        db_upgrade_cols('T_email__newsletter_subscription', [
            'ADD' => [
    ...
    if (upg_task_start(12620, 'Upgrading automation newsletter table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12630, 'Upgrading email campaign send data table...')) {	// part of 6.10.0-beta
        db_upgrade_cols('T_email__campaign', [
            'ADD' => [
    ...
    if (upg_task_start(12640, 'Upgrading user organizations tables...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12650, 'Upgrading email campaign table...')) { // part of 6.10.0-beta
        db_upgrade_cols('T_email__campaign', [
            'ADD' => [
    ...
    if (upg_task_start(12660, 'Updating user settings table...')) {	// part of 6.10.0-beta
    ...
    if (upg_task_start(12670, 'Upgrading cron logs table...')) {	// part of 6.10.0-beta
        db_add_col('T_cron__log', 'clog_actions_num', 'INT UNSIGNED NULL');
        upg_task_end();
    }

    if (upg_task_start(12680, 'Upgrading email campaign table...')) {	// part of 6.10.0-beta
        db_upgrade_cols('T_email__campaign', [
            'ADD' => [
    ...
    if (upg_task_start(12690, 'Upgrading polls table...')) { // part of 6.10.0-beta
    ...
    if (upg_task_start(12700, 'Upgrading cron tasks table...')) {	// part of 6.10.1-stable
        db_add_col('T_cron__task', 'ctsk_max_exec_time', 'INT UNSIGNED DEFAULT 600 AFTER ctsk_repeat_variation');
        upg_task_end();
    }

    if (upg_task_start(12710, 'Upgrading email campaign table...')) { // part of 6.10.1-stable
    ...
    if (upg_task_start(12720, 'Upgrading cron tasks table...')) {	// part of 6.10.1-stable
    ...
    if (upg_task_start(12730, 'Upgrading email campaign table...')) { // part of 6.10.1-stable
    ...
    if (upg_task_start(12735, 'Creating default item type "Content Block"...')) {	// part of 6.10.1-stable
        $SQL = new SQL('Check at least one item type with usage "content-block" for existence');
        $SQL->SELECT('ityp_ID');
        $SQL->FROM('T_items__type');
        $SQL->WHERE('ityp_usage = "content-block"');
        $SQL->LIMIT(1);
        if (! $DB->get_var($SQL)) {	// Create one default content block item type:
            ...
            if ($r && $DB->insert_id > 0) {	// Enable new created item type for all collections:
    ...
    if (upg_task_start(12740)) {	// part of 6.10.1-stable
        ...
        if (count($collections) > 0) {	// If at least one collection exists:
            ...
            foreach ($collections as $coll_ID => $cat_ID) {
                if (! $info_Blog || ! isset($login_required_Item) || ! isset($access_denied_Item)) {	// Create a help and a register content block items once for info/shared collection if it is defined
                    // otherwise create them for each current collection:
                    if ($info_Blog) {	// Use info pages collection if it is defined:
                        $content_block_Blog = $info_Blog;
                    } else {	// Otherwise use current collection:
                        $BlogCache = &get_BlogCache();
                        $content_block_Blog = &$BlogCache->get_by_ID($coll_ID, false, false);
                    }
                    if ($content_block_Blog) {	// The collection must be always defined but we need this additional check to avoid unexpected die error during upgrade process:
                        $cat_ID = $collections[$content_block_Blog->ID];
                        if ($cat_ID === null) {	// Create new category if collection has no categories:
                        ...
                        $login_required_Item->insert(
                            1,
                            T_('Login Required'),
                            '<p class="center">' . T_('You need to log in before you can access this section.') . '</p>',
                            date('Y-m-d H:i:s'),
                            $cat_ID,
                            [],
                            '!published',
                        ...
                        $access_denied_Item->insert(
                            1,
                            T_('Access Denied'),
                            '<p class="center">' . T_('You are not a member of this collection, therefore you are not allowed to access it.') . '</p>',
                            date('Y-m-d H:i:s'),
                            $cat_ID,
                            [],
                            '!published',
                ...
                /* Login Required */
                add_basic_widget_12740($coll_ID, 'Login Required', 'content_block', 'core', 10, [
                    'item_slug' => $login_required_Item->get('urltitle'),
                ]);
                add_basic_widget_12740($coll_ID, 'Login Required', 'user_login', 'core', 20, [
                    'title' => T_('Log in to your account'),
                ]);
                /* Access Denied */
                add_basic_widget_12740($coll_ID, 'Access Denied', 'content_block', 'core', 10, [
                    'item_slug' => $access_denied_Item->get('urltitle'),
                ]);
                task_end();
            }
        }
    ...
    if (upg_task_start(12750, 'Upgrading users table...')) {	// part of 6.10.1-stable
    ...
    if (upg_task_start(12760, 'Upgrading users table...')) {	// part of 6.10.1-stable
        db_add_col('T_subscriptions', 'sub_items_mod', 'TINYINT(1) NOT NULL AFTER sub_items');
        upg_task_end();
    }

    if (upg_task_start(12770, 'Upgrading email campaigns table...')) {	// part of 6.10.1-stable
        ...
        if (count($welcome_campaigns) > 0) {	// Convert "At subscription" email campaigns to "Welcome":
            $DB->query('UPDATE T_email__campaign
				  SET ecmp_welcome = 1
    ...
    if (upg_task_start(12780, 'Upgrading cron logs table...')) {	// part of 6.10.1-stable
        db_modify_col('T_cron__log', 'clog_messages', 'MEDIUMTEXT');
        upg_task_end();
    }

    if (upg_task_start(12790, 'Upgrading for widgets "User log in"...')) {	// part of 6.10.1-stable
        ...
        foreach ($login_widgets as $login_widget) {
            ...
            if ($login_widget->wi_sco_name == 'Login Required') {	// Make form buttons larger only for login widgets of the container "Login Required":
            ...
            } else {	// Add greetings widget after each login widget from any other container:
    ...
    if (upg_task_start(12800, 'Upgrading email campaigns table...')) {	// part of 6.10.1-stable
        db_add_col('T_email__campaign', 'ecmp_sync_plaintext', 'TINYINT(1) NOT NULL DEFAULT 1 AFTER ecmp_email_plaintext');
        upg_task_end();
    }

    if (upg_task_start(12810, 'Upgrading comments table...')) {	// part of 6.10.1-stable
        db_add_col('T_comments', 'comment_anon_notify', 'TINYINT(1) NOT NULL DEFAULT 0 AFTER comment_allow_msgform');
        upg_task_end();
    }

    if (upg_task_start(12820, 'Upgrading email campaign table...')) { // part of 6.10.1-stable
        db_upgrade_cols('T_email__campaign', [
            'ADD' => [
    ...
    if (upg_task_start(12830, 'Upgrading comments table...')) {	// part of 6.10.1-stable
    ...
    if (upg_task_start(12840, 'Upgrading automation step table...')) {	// part of 6.10.0-beta
        db_add_col('T_automation__step', 'step_diagram', 'VARCHAR(64) NULL');
        upg_task_end();
    }

    if (upg_task_start(12850, 'Creating user profile visit counter table...')) { // part of 6.10.1-stable
    ...
    if (upg_task_start(12860)) {	// part of 6.10.1-stable
        ...
        if (count($collections) > 0) {	// If at least one collection exists:
            ...
            foreach ($collections as $coll_ID => $cat_ID) {
                if (! $info_Blog || ! isset($help_Item) || ! isset($register_Item)) {	// Create a help and a register content block items once for info/shared collection if it is defined
                    // otherwise create them for each current collection:
                    if ($info_Blog) {	// Use info pages collection if it is defined:
                        $content_block_Blog = $info_Blog;
                    } else {	// Otherwise use current collection:
                    ...
                    if ($content_block_Blog) {	// The collection must be always defined but we need this additional check to avoid unexpected die error during upgrade process:
                        ...
                        $register_Item->insert(
                            1,
                            T_('Register content'),
                            T_('The information you provide in this form will be recorded in your user account.')
                            . "\n\n"
                ...
                /* Help */
                add_basic_widget_12860($coll_ID, 'Help', 'content_block', 'core', 10, [
                ...
                add_basic_widget_12860($coll_ID, 'Register', 'content_block', 'core', 20, [
    ...
    if (upg_task_start(12870, 'Renaming registration widgets...')) {	// part of 6.10.1-stable
        $DB->query('UPDATE ' . $tableprefix . 'widget
    ...
    if (upg_task_start(12880, 'Adding Send reminders about inactive accounts threshold setting...')) { // part of 6.10.1-stable
        $DB->query("REPLACE INTO T_settings( set_name, set_value ) VALUES ( 'inactive_account_reminder_threshold', 0 )");
        upg_task_end();
    }

    if (upg_task_start(12890, 'Upgrading user fields table...')) {	// part of 6.10.1-stable
        db_add_index('T_users__fields', 'uf_user_ID', 'uf_user_ID');
        upg_task_end();
    }

    if (upg_task_start(12900, 'Upgrading system log table...')) {	// part of 6.10.1-stable
        db_modify_col('T_syslog', 'slg_object', 'ENUM("comment", "item", "user", "file", "email_log") COLLATE ascii_general_ci');
        upg_task_end();
    }

    if (upg_task_start(12910, 'Upgrading email campaign table...')) { // part of 6.10.2-stable
        db_modify_col('T_email__campaign', 'ecmp_email_title', 'VARCHAR(255) NULL');
        upg_task_end();
    }

    if (upg_task_start(12920, 'Upgrading post type custom fields table...')) {	// part of 6.10.3-stable
        db_upgrade_cols('T_items__type_custom_field', [
            'ADD' => [
    ...
    if (upg_task_start(12930, 'Update setting names of item custom fields...')) {	// part of 6.10.3-stable
        ...
        if (count($custom_field_settings)) {	// If at least one item has a filled custom field:
            ...
            foreach ($custom_field_settings as $custom_field_setting) {	// Update each old custom field setting name like 'custom_double_1' to new 'custom:field_code'
                $custom_field_ID = preg_replace('#^custom_[a-z]+_(\d+)$#', '$1', $custom_field_setting->iset_name);
                if (isset($custom_fields[$custom_field_ID])) {	// If custom field is detected
                    $DB->query(
                        'UPDATE T_items__item_settings
    ...
    if (upg_task_start(12940)) {	// part of 6.10.3-stable
    ...
    if (upg_task_start(12950, 'Upgrade items and item types tables to support short title...')) {	// part of 6.10.3-stable
    ...
    if (upg_task_start(12960, 'Upgrade collections table...')) {	// part of 6.10.3-stable
        ...
        if (! empty($colls)) {	// If there are collections to update
            $is_https_baseurl = preg_match('#^https://#', $baseurl);
            $coll_protocol_insert_values = [];
            foreach ($colls as $coll) {	// Check what protocol is used by the collection:
                if ($coll->blog_http_protocol == 'allow_both') {	// Use same value from collections table:
                ...
                } else {	// Don't insert collection setting with default values "always_http":
                    continue;
                }
            ...
            if (! empty($coll_protocol_insert_values)) {	// Insert new settings only if at least collection has a not default setting "SSL":
                $DB->query('INSERT INTO T_coll_settings ( cset_coll_ID, cset_name, cset_value )
    ...
    if (upg_task_start(12963, 'Upgrade items tables...')) {	// part of 6.10.3-stable
    ...
    if (upg_task_start(12966, 'Upgrade files table...')) {	// part of 6.10.3-stable
    ...
    if (upg_task_start(12969, 'Upgrade user organizations table...')) {	// part of 6.10.3-stable
        db_drop_col('T_users__organization', 'org_perm_priority');
        upg_task_end();
    }

    if (upg_task_start(12972, 'Move Order from Posts table to Categories-to-Posts relationships table...')) {	// part of 6.10.3-stable
    ...
    if (upg_task_start(12975, 'Upgrade categories table...')) {	// part of 6.10.4-stable
        db_add_col('T_categories', 'cat_ityp_ID', 'INT UNSIGNED NULL');
        upg_task_end();
    }

    if (upg_task_start(12978, 'Upgrade table item types...')) {	// part of 6.10.4-stable
        db_upgrade_cols('T_items__type', [
            'ADD' => [
    ...
    if (upg_task_start(12981, 'Upgrading post type custom fields table...')) {	// part of 6.10.4-stable
        db_add_col('T_items__type_custom_field', 'itcf_merge', 'TINYINT DEFAULT 0');
        upg_task_end();
    }

    if (upg_task_start(12984, 'Upgrading post type custom fields table...')) {	// part of 6.10.4-stable
    ...
    if (upg_task_start(12987, 'Update collection setting...')) {	// part of 6.10.4-stable
        $setting_SQL = new SQL();
        $setting_SQL->SELECT('set_value');
        $setting_SQL->FROM('T_settings');
        $setting_SQL->WHERE('set_name = "cross_post_nav_in_same_coll"');
        if ($DB->get_var($setting_SQL) === '0') {	// Move only not default value to the collection settings table:
            $DB->query('INSERT INTO T_coll_settings ( cset_coll_ID, cset_name, cset_value )
    ...
    if (upg_task_start(12990)) {	// part of 6.10.4-stable
    ...
    if (upg_task_start(12993, 'Upgrading email newsletters table...')) {	// part of 6.10.4-stable
        db_upgrade_cols('T_email__newsletter', [
            'ADD' => [
    ...
    if (upg_task_start(12996, 'Upgrading email newsletter subscriptions table...')) {	// part of 6.10.4-stable
    ...
    if (upg_task_start(12999, 'Upgrading email newsletters table...')) {	// part of 6.10.4-stable
    ...
    if (upg_task_start(13010, 'Upgrading email addresses table...')) {	// part of 6.10.4-stable
        db_upgrade_cols('T_email__address', [
            'MODIFY' => [
    ...
    if (upg_task_start(13020)) {	// part of 6.10.4-stable
    ...
    if (upg_task_start(13030, 'Upgrading email campaigns table...')) {	// part of 6.10.4-stable
        db_upgrade_cols('T_email__campaign', [
            'ADD' => [
    ...
    if (upg_task_start(13040, 'Upgrading items table...')) {	// part of 6.10.5-stable
        db_modify_col('T_items__item', 'post_title', 'VARCHAR(255) NULL');
        upg_task_end();
    }

    if (upg_task_start(13050, 'Upgrading email campaigns table...')) {	// part of 6.10.5-stable
        db_upgrade_cols('T_email__campaign', [
            'ADD' => [
    ...
    if (upg_task_start(13060, 'Updating settings for scheduled job "Send reminders about non-activated accounts"...')) {	// part of 6.10.5-stable
    ...
    if (upg_task_start(13070, 'Upgrading automation step table...')) {	// part of 6.10.6-stable
    ...
    if (upg_task_start(13075, 'Upgrading GeoIP plugin data file...')) {	// part of 6.10.6-stable
        if ($Plugins_admin = &get_Plugins_admin() &&
            $geoip_Plugin = &$Plugins_admin->get_by_code('evo_GeoIP') &&
            $geoip_Plugin->status == 'enabled') {	// Try to download only when plugin is installed and enabled:
            try {	// Download GeoIP data file:
                $geoip_Plugin->download_geoip_data();
                task_end('OK.');
            } catch (Exception $ex) {	// Unexpected error:
                if (! is_object($Plugins)) {	// Initiliaze Plugins object because it is required in the function Plugin->set_status():
                    load_class('plugins/model/_plugins.class.php', 'Plugins');
                    $Plugins = new Plugins();
                }
    ...
    if (upg_task_start(13080, 'Upgrading users table...')) {	// part of 6.10.6-stable
    ...
    if (upg_task_start(13090, 'Upgrading messages table...')) {	// part of 6.10.6-stable
        db_add_index('T_messaging__message', 'msg_author_user_ID', 'msg_author_user_ID');
        upg_task_end();
    }

    if (upg_task_start(13100, 'Upgrading comments table...')) {	// part of 6.10.6-stable
    ...
    if (upg_task_start(13110, 'Updating collection ping plugin settings...')) {	// part of 6.10.6-stable
    ...
    if (upg_task_start(13120, 'Upgrade Temporary IDs table...')) {	// part of 6.11.0-beta
    ...
    if (upg_task_start(13130, 'Upgrade links table...')) {	// part of 6.11.0-beta
        db_drop_col('T_links', 'link_ltype_ID');
        upg_task_end();
    }

    if (upg_task_start(13140, 'Creating custom fields and link versions tables for item versions...')) {	// part of 6.11.0-beta
    ...
    if (upg_task_start(13150, 'Upgrading item versions tables...')) {	// part of 6.11.0-beta
    ...
    if (upg_task_start(13160, 'Updating collection user/group permissions...')) {	// part of 6.11.0-beta
    ...
    if (upg_task_start(13170, 'Creating Markdown text file type...')) {	// part of 6.11.0-beta
        $SQL = new SQL('Check for file type .md');
        $SQL->SELECT('ftyp_ID');
        $SQL->FROM('T_filetypes');
        $SQL->WHERE('ftyp_extensions REGEXP "(^| )md( |$)"');
        if (! $DB->get_var($SQL)) {	// Insert new file type for Markdown Documentation only if it doesn't exist:
            $DB->query('INSERT INTO T_filetypes
				       ( ftyp_extensions, ftyp_name, ftyp_mimetype, ftyp_icon, ftyp_viewtype, ftyp_allowed )
    ...
    if (upg_task_start(13180, 'Upgrading cron logs table...')) {	// part of 6.11.0-beta
    ...
    if (upg_task_start(13190, 'Update skin color settings with transparency to new format...')) {	// part of 6.11.0-beta
        ...
        foreach ($color_settings as $color_setting) {
            ...
            if (! isset($color_transparency_settings[$color_set_name])) {	// Skip not color setting:
                continue;
            }
            ...
            foreach ($color_settings as $transparency_setting) {
                if ($color_setting->cset_coll_ID == $transparency_setting->cset_coll_ID &&
                    $transparency_setting->cset_name == str_replace($color_set_name, '', $color_setting->cset_name) . $transparency_set_name) {	// We found transparency value for the same color setting,
    ...
    if (upg_task_start(13200, 'Creating CSV file type...')) {	// part of 6.11.0-beta
    ...
    if (upg_task_start(13210, 'Inserting new collection settings...')) {	// part of 6.11.2-stable
    ...
    if (upg_task_start(13220, 'Upgrading cron tasks table...')) {	// part of 6.11.3-stable
        db_modify_col('T_cron__task', 'ctsk_params', 'TEXT NULL');
        upg_task_end();
    }

    if (upg_task_start(15000, 'Creating sections table...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15005, 'Upgrading collections table...')) {	// part of 7.0.0-alpha
        db_add_col('T_blogs', 'blog_sec_ID', 'INT(11) UNSIGNED NOT NULL DEFAULT 1');
        upg_task_end();
    }

    if (upg_task_start(15010, 'Create default section...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15020, 'Install default site skin...')) {	// part of 7.0.0-alpha
        load_funcs('skins/_skin.funcs.php');
        $SkinCache = &get_SkinCache();
        if (! ($default_site_Skin = &$SkinCache->get_by_folder('default_site_skin', false))) {	// If the skin doesn't exist try to install it:
    ...
    if (upg_task_start(15030, 'Creating plugin group settings table...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15040, 'Creating table for widget container...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15050, 'Inserting widget containers...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15060, 'Upgrading widgets table...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15070, 'Updating widget containers content...')) {	// part of 7.0.0-alpha
        ...
        foreach ($blog_ids as $blog_ID) {	// Create Item single container rows:
            $widget_containers_sql_rows[] = '( "item_single", "Item Single", ' . $blog_ID . ', 4 )';
        }
        if (! empty($widget_containers_sql_rows)) {	// Insert Item Single containers:
        ...
        if (! empty($widgets_insert_sql_rows)) {	// There are widgets to create, insert previously built widget records:
    ...
    if (upg_task_start(15080, 'Droping skin containers table...')) {	// part of 7.0.0-alpha
        $DB->query('DROP TABLE IF EXISTS ' . $tableprefix . 'skins__container');
        upg_task_end();
    }

    if (upg_task_start(15090, 'Upgrading widget containers table...')) {	// part of 7.0.0-alpha
        ...
        foreach ($BlogCache->cache as $Blog) {
            ...
            if (count($main_containers)) {	// If at least one main container exists:
                ...
                if (! empty($not_existing_containers)) {	// If at least we have one missed container:
                    $new_main_cnontainers_sql = [];
                    ...
                    $DB->query(
                        'INSERT INTO T_widget__container ( wico_code, wico_name, wico_coll_ID, wico_order, wico_main )
    ...
    if (upg_task_start(15100, 'Rename widget "coll_avatar" to "user_profile_pics"...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15110, 'Upgrading widget containers table...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15130, 'Converting columns to utf8mb4...')) {	// part of 7.0.0-alpha
        ...
        db_convert_cols_to_utf8mb4([
            'T_antispam__keyword' => ['askw_string'],
            'T_automation__automation' => ['autm_name'],
            'T_automation__step' => ['step_label', 'step_info'],
            'T_basedomains' => ['dom_comment'],
            'T_blogs' => ['blog_shortname', 'blog_name', 'blog_tagline', 'blog_shortdesc', 'blog_longdesc', 'blog_notes', 'blog_keywords'],
            'T_categories' => ['cat_name', 'cat_description'],
            'T_coll_settings' => ['cset_value'],
            'T_comments' => ['comment_author', 'comment_content'],
            'T_comments__prerendering' => ['cmpr_content_prerendered'],
            'T_cron__log' => ['clog_messages'],
            'T_cron__task' => ['ctsk_name', 'ctsk_params'],
            'T_email__campaign' => ['ecmp_email_title', 'ecmp_email_html', 'ecmp_email_text', 'ecmp_email_plaintext', 'ecmp_user_tag', 'ecmp_user_tag_cta1', 'ecmp_user_tag_cta2', 'ecmp_user_tag_cta3', 'ecmp_user_tag_like', 'ecmp_user_tag_dislike', 'ecmp_user_tag_activate', 'ecmp_user_tag_unsubscribe'],
            'T_email__log' => ['emlog_subject', 'emlog_headers', 'emlog_message'],
            'T_email__newsletter' => ['enlt_name', 'enlt_label'],
            'T_email__returns' => ['emret_errormsg', 'emret_headers', 'emret_message'],
            'T_files' => ['file_title', 'file_alt', 'file_desc'],
            'T_filetypes' => ['ftyp_name'],
            'T_groups' => ['grp_name'],
            'T_groups__groupsettings' => ['gset_value'],
            'T_hitlog' => ['hit_keyphrase'],
            'T_i18n_original_string' => ['iost_string'],
            'T_items__item' => ['post_content', 'post_excerpt', 'post_title', 'post_titletag'],
            'T_items__item_settings' => ['iset_value'],
            'T_items__prerendering' => ['itpr_content_prerendered'],
            'T_items__status' => ['pst_name'],
            'T_items__type' => ['ityp_name', 'ityp_description', 'ityp_instruction', 'ityp_comment_form_msg', 'ityp_evobar_link_text', 'ityp_skin_btn_text'],
            'T_items__type_custom_field' => ['itcf_label', 'itcf_note'],
            'T_items__version' => ['iver_title', 'iver_content'],
            'T_locales' => ['loc_name', 'loc_messages', 'loc_transliteration_map'],
            'T_messaging__contact_groups' => ['cgr_name'],
            'T_messaging__message' => ['msg_text'],
            'T_messaging__prerendering' => ['mspr_content_prerendered'],
            'T_messaging__thread' => ['thrd_title'],
            'T_plugingroupsettings' => ['pgset_value'],
            'T_plugins' => ['plug_name', 'plug_shortdesc'],
            'T_pluginsettings' => ['pset_value'],
            'T_pluginusersettings' => ['puset_value'],
            'T_polls__option' => ['popt_option_text'],
            'T_polls__question' => ['pqst_question_text'],
            'T_regional__city' => ['city_name'],
            'T_regional__country' => ['ctry_name'],
            'T_regional__currency' => ['curr_shortcut', 'curr_name'],
            'T_regional__region' => ['rgn_name'],
            'T_regional__subregion' => ['subrg_name'],
            'T_section' => ['sec_name'],
            'T_settings' => ['set_value'],
            'T_skins__skin' => ['skin_name'],
            'T_syslog' => ['slg_message'],
            'T_track__goal' => ['goal_name', 'goal_notes'],
            'T_track__goalcat' => ['gcat_name'],
            'T_track__goalhit' => ['ghit_params'],
            'T_users' => ['user_firstname', 'user_lastname', 'user_nickname'],
            'T_users__fielddefs' => ['ufdf_name', 'ufdf_options', 'ufdf_bubbletip'],
            'T_users__fieldgroups' => ['ufgp_name'],
            'T_users__fields' => ['uf_varchar'],
            'T_users__organization' => ['org_name'],
            'T_users__reports' => ['urep_info'],
            'T_users__tag' => ['utag_name'],
            'T_users__usersettings' => ['uset_value'],
            'T_users__user_org' => ['uorg_role'],
            'T_widget__container' => ['wico_name'],
            'T_widget__widget' => ['wi_params'],
        ]);

        upg_task_end();
    }

    if (upg_task_start(15140, 'Converting columns to utf8mb4...')) {	// part of 7.0.0-alpha
        db_convert_cols_to_utf8mb4([
            'T_email__campaign' => ['ecmp_name', 'ecmp_user_tag_sendskip', 'ecmp_user_tag_sendsuccess'],
        ]);
        upg_task_end();
    }

    if (upg_task_start(15150, 'Converting columns to utf8mb4...')) {	// part of 7.0.0-alpha
        db_convert_cols_to_utf8mb4([
            'T_items__type_custom_field' => ['itcf_format'],
        ]);
        upg_task_end();
    }

    if (upg_task_start(15160, 'Creating table for URL aliases...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15170, 'Updating Antispam IP Ranges table... ')) { // part of 7.0.0-alpha
    ...
    if (upg_task_start(15180, 'Upgrade table item types... ')) { // part of 7.0.0-alpha
    ...
    if (upg_task_start(15190, 'Installing new widgets/containers...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15210, 'Update free html widgets...')) {	// part of 7.0.0-alpha
        ...
        foreach ($free_html_widgets as $free_html_widget_ID => $free_html_widget_params) {
            $free_html_widget_params = ($free_html_widget_params === null ? [] : unserialize($free_html_widget_params));
            // Turn off all renderer plugins for old widgets:
            $free_html_widget_params['renderers'] = [];
            $DB->query('UPDATE T_widget__widget
    ...
    if (upg_task_start(15220, 'Updating table item types... ')) { // part of 7.0.0-alpha
    ...
    if (upg_task_start(15230, 'Installing new widgets/containers...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15240, 'Updating table item types...')) { // part of 7.0.0-alpha
    ...
    if (upg_task_start(15250, 'Updating table item custom fields...')) { // part of 7.0.0-alpha
        db_add_col('T_items__type_custom_field', 'itcf_schema_prop', 'VARCHAR(255) COLLATE ascii_general_ci NULL AFTER itcf_name');
        upg_task_end();
    }

    if (upg_task_start(15260, 'Installing new shared widgets/containers...')) {	// part of 7.0.0-alpha
        // Initialize environment variable $Settings in order to work with collection objects:
        upg_init_environment();
        if ($info_Blog = &get_setting_Blog('info_blog_ID')) {	// If site uses collection for info pages(shared content blocks):
    ...
    if (upg_task_start(15270, 'Upgrading widget containers table...')) {	// part of 7.0.0-alpha
        ...
        if ($widget_page_result && $DB->insert_id > 0) {	// Enable new inserted Item Type "Widget Page" for all collections:
            $DB->query('INSERT INTO T_items__type_coll ( itc_ityp_ID, itc_coll_ID )
    ...
    if (upg_task_start(15280, 'Creating item custom field values table...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15290, 'Updating item types table...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15300, 'Installing new widget container "Comment Area"...')) {	// part of 7.0.0-alpha
        install_new_default_widgets('comment_area');
        upg_task_end();
    }

    if (upg_task_start(15310, 'Installing new shared widget container "Marketing Popup"...')) {	// part of 7.0.0-alpha
        install_new_default_widgets('marketing_popup');
        upg_task_end();
    }

    if (upg_task_start(15320, 'Installing new widget container "Chapter Main Area"...')) {	// part of 7.0.0-alpha
        install_new_default_widgets('chapter_main_area');
        upg_task_end();
    }

    if (upg_task_start(15330, 'Updating widget "Email capture / Quick registration"...')) {	// part of 7.0.0-alpha
        ...
        foreach ($widgets as $widget_ID => $widget_params) {
            ...
            if (! $widget_params) {	// Skip wrong widget params:
                continue;
            }
            // Set the param values in new format:
            $widget_params['subscribe'] = [
                'post' => empty($widget_params['subscribe_post']) ? 0 : 1,
                'comment' => empty($widget_params['subscribe_comment']) ? 0 : 1,
            ];
            if (isset($widget_params['subscribe_post'])) {	// Remove old param:
                unset($widget_params['subscribe_post']);
            }
            if (isset($widget_params['subscribe_comment'])) {	// Remove old param:
    ...
    if (upg_task_start(15340, 'Updating checkbox plugins settings...')) {	// part of 7.0.0-alpha
        /**
        ...
        function upg_convert_plugin_checkbox_settings($plugin_classname, $setting_replacements)
        {
            ...
            foreach ($setting_replacements as $settings_group => $setting_replacement) {
                ...
                if (empty($plugin_settings)) {	// Nothing to update because either plugin has default settings or it doesn't exist:
                    continue;
                }
            ...
            if (! empty($new_plugin_setting_sql)) {	// Insert new setting in new format:
        ...
        upg_convert_plugin_checkbox_settings('autolinks_plugin', [
            // - general settings:
            'autolink' => ['urls', 'defs_default', 'defs_local'],
            // - message settings:
            'msg_autolink_nofollow' => ['exist', 'explicit', 'auto'],
            // - email campaign settings:
            'email_autolink_nofollow' => ['exist', 'explicit', 'auto'],
            // - shared widget container settings:
            'shared_autolink_nofollow' => ['exist', 'explicit', 'auto'],
        ]);
        // - collection settings:
        upg_convert_plugin_coll_checkbox_settings('autolinks_plugin', [
            'autolink_post_nofollow' => ['exist', 'explicit', 'auto'],
            'autolink_comment_nofollow' => ['exist', 'explicit', 'auto'],
        ]);

        // Update collection settings of the plugin "Flowplayer":
        upg_convert_plugin_coll_checkbox_settings('flowplayer_plugin', [
            'use_for' => ['posts', 'comments'],
        ]);

        // Update collection settings of the plugin "HTML 5 MediaElement.js Video and Audio Player":
        upg_convert_plugin_coll_checkbox_settings('html5_mediaelementjs_plugin', [
            'use_for' => ['posts', 'comments'],
        ]);

        // Update collection settings of the plugin "HTML 5 VideoJS Player":
        upg_convert_plugin_coll_checkbox_settings('html5_videojs_plugin', [
            'use_for' => ['posts', 'comments'],
        ]);
    ...
    if (upg_task_start(15350, 'Installing minisite collection default skin...')) {	// part of 7.0.0-alpha
        load_funcs('skins/_skin.funcs.php');
        skin_install('jared_skin', true);
        upg_task_end();
    }

    if (upg_task_start(15360, 'Fix integer unsigned ID columns...')) {	// part of 7.0.0-alpha
        $tables_num = 0;
        $columns_num = 0;
        $tables = $DB->get_col('SHOW TABLES');
        // Ignore these ID columns because they can be be signed:
        $ignore_signed_columns = [
            'step_yes_next_step_ID',
            'step_no_next_step_ID',
            'step_error_next_step_ID',
        ];
        foreach ($tables as $table_name) {
            if (! in_array($table_name, $db_config['aliases'])) {	// Skip not core table, e.g. custom plugin tables:
                continue;
            }
            $modify_columns = [];
            ...
            foreach ($columns as $column) {
                if (in_array($column->Field, $ignore_signed_columns)) {	// Skip signed columns:
                    continue;
                }
            ...
            if (! empty($modify_columns)) {	// Upgrade wrong integer unsigned ID columns:
    ...
    if (upg_task_start(15370, 'Installing new widget container "Photo Index"...')) {	// part of 7.0.0-alpha
        ...
        foreach ($skin_settings as $skin_setting) {
            $wi_params = empty($skin_setting->wi_params) ? [] : @unserialize($skin_setting->wi_params);
    ...
    if (upg_task_start(15380, 'Upgrading posts table...')) {	// part of 7.0.1-beta
    ...
    if (upg_task_start(15390, 'Creating table for collection extra locales...')) {	// part of 7.0.1-beta
    ...
    if (upg_task_start(15400, 'Inserting collection extra locales...')) {	// part of 7.0.1-beta
    ...
    if (upg_task_start(15410, 'Creating table for Post Groups...')) {	// part of 7.0.1-beta
    ...
    if (upg_task_start(15420, 'Upgrading posts table...')) {	// part of 7.0.1-beta
        db_add_col('T_items__item', 'post_igrp_ID', 'INT(10) UNSIGNED NULL AFTER post_ityp_ID');
        upg_task_end();
    }

    if (upg_task_start(15430, 'Updating collection locale setting for new posts...')) {	// part of 7.0.1-beta
        $DB->query('UPDATE T_coll_settings
    ...
    if (upg_task_start(15440, 'Upgrading table of collection extra locales and linking with other collections...')) {	// part of 7.0.1-beta
        db_add_col('T_coll_locales', 'cl_linked_coll_ID', 'INT(10) UNSIGNED NULL');
        upg_task_end();
    }

    if (upg_task_start(15450, 'Installing new default widgets...')) {	// part of 7.0.0-alpha
        install_new_default_widgets('page_top', 'coll_locale_switch');
        upg_task_end();
    }

    if (upg_task_start(15460, 'Upgrading files table...')) {	// part of 7.0.1-beta
        db_upgrade_cols('T_files', [
            'ADD' => [
    ...
    if (upg_task_start(15470, 'Creating SVG file type...')) {	// part of 7.0.1-beta
        $SQL = new SQL('Check for file type .svg');
        $SQL->SELECT('ftyp_ID');
        $SQL->FROM('T_filetypes');
        $SQL->WHERE('ftyp_extensions REGEXP "(^| )svg( |$)"');
        if (! $DB->get_var($SQL)) {	// Insert new SVG file type only if it doesn't exist:
            $DB->query('INSERT INTO T_filetypes
				       ( ftyp_extensions, ftyp_name, ftyp_mimetype, ftyp_icon, ftyp_viewtype, ftyp_allowed )
    ...
    if (upg_task_start(15480, 'Creating ICO file type...')) {	// part of 7.0.1-beta
        $SQL = new SQL('Check for file type .ico');
        $SQL->SELECT('ftyp_ID');
        $SQL->FROM('T_filetypes');
        $SQL->WHERE('ftyp_extensions REGEXP "(^| )ico( |$)"');
        if (! $DB->get_var($SQL)) {	// Insert new ICO file type only if it doesn't exist:
            $DB->query('INSERT INTO T_filetypes
				       ( ftyp_extensions, ftyp_name, ftyp_mimetype, ftyp_icon, ftyp_viewtype, ftyp_allowed )
    ...
    if (upg_task_start(15490, 'Upgrading collection permission tables...')) {	// part of 7.0.2-beta
        db_upgrade_cols('T_coll_user_perms', [
            'ADD' => [
        ...
        db_upgrade_cols('T_coll_group_perms', [
            'ADD' => [
    ...
    if (upg_task_start(15500, 'Upgrading item types and custom fields tables...')) {	// part of 7.0.2-beta
        db_upgrade_cols('T_items__type', [
            'ADD' => [
    ...
    if (upg_task_start(15510, 'Upgrading item types tables...')) {	// part of 7.0.2-beta
    ...
    if (upg_task_start(15520, 'Creating table for aggregations of goal hits...')) {	// part of 7.0.2-beta
        db_create_table(
            'T_track__goalhit_aggregate',
            '
    ...
    if (upg_task_start(15530, 'Upgrading settings of the plugin "Short Links"...')) {	// part of 7.0.2-beta
        ...
        foreach ($plugin_rows as $plugin_row) {
            ...
            $plugin_settings[$plugin_row->coll_ID_type][$set_match[1]][$set_match[2]] = $value ? $value : [];
        }

        foreach ($plugin_settings as $coll_ID_type => $plugin_data) {
            foreach ($plugin_data as $plugin_ID => $plugin_settings) {
                if (! isset($plugin_settings['link_types'])) {	// Skip if detault values are still used:
                    continue;
                }
                if (isset($plugin_settings['link_types']['abs_target_blank'])) {	// Rename old setting:
                ...
                if (isset($plugin_settings['optimize']['absolute_urls'])) {	// Add new setting:
                    $plugin_settings['link_types']['abs_url_optimize'] = $plugin_settings['optimize']['absolute_urls'];
                }
                if (isset($plugin_settings['optimize']['relative_urls'])) {	// Add new setting:
    ...
    if (upg_task_start(15540, 'Upgrading comments table...')) {	// part of 7.0.2-beta
        db_upgrade_cols('T_comments', [
            'CHANGE' => [
    ...
    if (upg_task_start(15550, 'Installing new plugin "Nofollow UGC Sponsored"...')) {	// part of 7.0.2-beta
        upg_init_environment('Settings,Plugins,Plugins_admin');
        install_plugin('nofollow_plugin', true, [], [
            'single_task' => false,
        ]);
        ...
        if ($nofollow_plugin_ID = $DB->get_var($nofollow_plugin_SQL)) {	// Only if nofollow plugin is detected in DB:
    ...
    if (upg_task_start(15560, 'Upgrading settings of the plugin "Auto Links"...')) {	// part of 7.0.2-beta
        ...
        if ($autolinks_plugin = $DB->get_row($autolinks_plugin_SQL)) {	// Only if autolinks plugin is detected in DB:
            $plugin_value = (empty($autolinks_plugin->pset_value) ? false : @unserialize($autolinks_plugin->pset_value));
            if (is_array($plugin_value) && empty($plugin_value['urls'])) {	// Try to update only not default value (default value is checked):
                $DB->query('INSERT INTO T_coll_settings ( cset_coll_ID, cset_name, cset_value )
    ...
    if (upg_task_start(15570, 'Upgrading widget containers table...')) {	// part of 7.0.2-beta
        db_upgrade_cols('T_widget__container', [
            'MODIFY' => [
    ...
    if (upg_task_start(15580, 'Creating default item type "Homepage Content Tab"...')) {	// part of 7.0.2-beta
        ...
        $SQL->LIMIT(1);
        if (! $DB->get_var($SQL)) {	// Create one default item type "Homepage Content Tab":
    ...
    if (upg_task_start(15590, 'Installing new default widgets...')) {	// part of 7.0.2-beta
        install_new_default_widgets('item_single', 'breadcrumb_path');
        upg_task_end();
    }

    if (upg_task_start(15600, 'Creating table for Menus...')) {	// part of 7.0.2-beta
    ...
    if (upg_task_start(15610, 'Creating table for Menu entries...')) {	// part of 7.0.2-beta
    ...
    if (upg_task_start(15620, 'Creating default item type "Text Ad"...')) {	// part of 7.0.2-beta
        ...
        $SQL->LIMIT(1);
        if (! $DB->get_var($SQL)) {	// Create content block item type "Text Ad":
            $r = $DB->query('INSERT INTO T_items__type ( ityp_name, ityp_usage, ityp_allow_breaks, ityp_allow_featured, ityp_use_comments )
    ...
    if (upg_task_start(15630, 'Upgrading post type custom fields table...')) {	// part of 7.0.2-beta
    ...
    if (upg_task_start(15640, 'Upgrading item types table...')) {	// part of 7.0.2-beta
        db_add_col('T_items__type', 'ityp_allow_switchable', 'TINYINT DEFAULT 1 AFTER ityp_allow_featured');
        upg_task_end();
    }

    if (upg_task_start(15650, 'Installing new widget container "Comment List"...')) {	// part of 7.0.2-beta
        install_new_default_widgets('comment_list');
        upg_task_end();
    }

    if (upg_task_start(15660, 'Upgrading base domains table...')) {	// part of 7.0.2-beta
        db_add_col('T_basedomains', 'dom_source_tag', 'VARCHAR(32) COLLATE utf8mb4_unicode_ci NULL');
        upg_task_end();
    }

    if (upg_task_start(15670, 'Creating tables for Social Networks and User Social Networks...')) {	// part of 7.0.2-beta
    ...
    if (upg_task_start(15680, 'Installing default site skins...')) {	// part of 7.1.0-beta
        load_funcs('skins/_skin.funcs.php');
    ...
    if (upg_task_start(15690, 'Upgrading menus table...')) {	// part of 7.1.0-beta
        db_add_col('T_menus__menu', 'menu_parent_ID', 'INT(10) UNSIGNED NULL DEFAULT NULL AFTER menu_ID');
        upg_task_end();
    }

    if (upg_task_start(15700, 'Creating tables for Templates...')) {	// part of 7.1.0-beta
    ...
    if (upg_task_start(15710, 'Updating plugin "Nofollow UGC Sponsored"...')) {	// part of 7.1.0-beta
        $DB->query('UPDATE T_plugins
    ...
    if (upg_task_start(15720, 'Installing new widgets/containers...')) {	// part of 7.0.0-alpha
        install_new_default_widgets('item_in_list', 'item_content');
        upg_task_end();
    }

    if (upg_task_start(15730, 'Installing new widgets/containers...')) {	// part of 7.0.0-alpha
        install_new_default_widgets('chapter_main_area', 'cat_content_list');
        upg_task_end();
    }

    if (upg_task_start(15740, 'Upgrading renderers columns...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15751, 'Creating new default templates...')) {	// part of 7.0.0-alpha
        // Delete the following existing default templates, they may be recreated at the end of upgrade process:
        $template_codes = ['iteminfo_long', 'iteminfo_short'];
        $DB->query('DELETE FROM T_templates WHERE tpl_code IN (' . $DB->quote($template_codes) . ')');
        upg_task_end();
    }

    if (upg_task_start(15760, 'Updating item footer...')) {	// part of 7.0.0-alpha
    ...
    if (upg_task_start(15770, 'Updating widget "Small Print"...')) {	// part of 7.0.0-alpha
        $SQL = new SQL();
        $SQL->SELECT('wi_ID, wi_params');
        $SQL->FROM('T_widget__widget');
        ...
        foreach ($widgets as $widget_ID => $widget_params) {
            $widget_params = (empty($widget_params) ? [] : unserialize($widget_params));
            if (isset($widget_params['format']) &&
                $widget_params['format'] == 'revision') {	// Convert "Format / Revisions" to Template "Item Details: Small Print: Revisions":
                $widget_params['template'] = 'item_details_revisions';
            } else {	// Convert "Format / Blog standard" to Template "Item Details: Small Print: Standard":
                $widget_params['template'] = 'item_details_smallprint_standard';
            }
            if (isset($widget_params['format'])) {	// Remove setting "Format" completely from the widget:
                unset($widget_params['format']);
            }
            if (isset($widget_params['avatar_size'])) {	// Remove setting "Avatar Size" completely from the widget:
    ...
    if (upg_task_start(15780, 'Updating widget "Info Line"...')) {	// part of 7.0.0-alpha
        $SQL = new SQL();
        ...
        foreach ($widgets as $row) {
            $widget_params = (empty($row->wi_params) ? [] : unserialize($row->wi_params));
            if (empty($widget_params['template'])) {	// Convert Template depending on collection type:
            ...
            if (isset($widget_params['flag_icon'])) {	// Remove setting "Display flag icon" completely from the widget:
                unset($widget_params['flag_icon']);
            }
            if (isset($widget_params['permalink_icon'])) {	// Remove setting "Display permalink icon" completely from the widget:
                unset($widget_params['permalink_icon']);
            }
            if (isset($widget_params['before_author'])) {	// Remove setting "Display author information" completely from the widget:
                unset($widget_params['before_author']);
            }
            if (isset($widget_params['display_date'])) {	// Remove setting "Post date to display" completely from the widget:
                unset($widget_params['display_date']);
            }
            if (isset($widget_params['last_touched'])) {	// Remove setting "Display date and time when item/post was last touched" completely from the widget:
                unset($widget_params['last_touched']);
            }
            if (isset($widget_params['contents_updated'])) {	// Remove setting "Display date and time when item/post contents (title, content, URL or attachments) were last updated" completely from the widget:
                unset($widget_params['contents_updated']);
            }
            if (isset($widget_params['date_format'])) {	// Remove setting "Date format" completely from the widget:
                unset($widget_params['date_format']);
            }
            if (isset($widget_params['time_format'])) {	// Remove setting "Time format" completely from the widget:
                unset($widget_params['time_format']);
            }
            if (isset($widget_params['category'])) {	// Remove setting "Display item/post category" completely from the widget:
                unset($widget_params['category']);
            }
            if (isset($widget_params['edit_link'])) {	// Remove setting "Display link to edit the item/post" completely from the widget:
                unset($widget_params['edit_link']);
            }
            if (isset($widget_params['post_time'])) {	// Remove setting previous setting "post_time" completely from the widget:
    ...
    if (upg_task_start(15790, 'Upgrading menu entries table...')) {	// part of 7.1.2-beta
        db_add_col('T_menus__entry', 'ment_item_slug', 'VARCHAR(255) COLLATE ascii_general_ci NULL AFTER ment_item_ID');
        upg_task_end();
    }

    if (upg_task_start(15800, 'Upgrading item types table...')) {	// part of 7.1.2-beta
        db_upgrade_cols('T_items__type', [
            'ADD' => [
    ...
    if (upg_task_start(15810)) {	// part of 7.1.2-beta
        global $plugins_path;
        ...
        foreach ($plugins_to_remove as $plugin_class) {
            if ($loop_Plugin = &$admin_Plugins->get_by_classname($plugin_class)) {
                echo get_install_format_text_and_log(sprintf(
                    '<span class="text-warning"><evo:warning>'
                        ...
                        . '</evo:warning></span>',
                    '<code>' . $loop_Plugin->classname . '</code>',
                    $loop_Plugin->get_help_url()
                ) . "<br />\n", 'br');
            } else {
                $plugin_deleted = true;
    ...
    if (upg_task_start(15820, 'Upgrading users table...')) {	// part of 7.1.2-beta
    ...
    if (upg_task_start(15830, 'Updating widget "Free Social Links"...')) {	// part of 7.1.2-beta
        ...
        if (! empty($widgets)) {	// If at least one widget "Free Social Links" exists in DB:
            $SQL = new SQL();
            ...
            foreach ($widgets as $widget_ID => $widget_params) {
                if (empty($widget_params)) {	// Skip widget with default settings:
                    continue;
                }
                ...
                for ($i = 1; $i <= 7; $i++) {
                    if (isset($widget_params['link' . $i])) {
                        if (isset($social_fields[$widget_params['link' . $i]])) {	// Replace old using of ufdf_ID to new using ufdf_code if we find such user field in DB by ID:
                            $widget_params['link' . $i] = $social_fields[$widget_params['link' . $i]];
                        } else {	// Reset wrong user field to default:
    ...
    if (upg_task_start(15840, 'Upgrading automation steps table...')) {	// part of 7.1.2-beta
        db_upgrade_cols('T_automation__step', [
            'MODIFY' => [
    ...
    if (upg_task_start(15850, 'Updating Item Type "Recipe"...')) {	// part of 7.1.2-beta
        $DB->query('UPDATE T_items__type
    ...
    if (upg_task_start(15860, 'Upgrading templates table...')) {	// part of 7.1.2-beta
        db_upgrade_cols('T_templates', [
            'CHANGE' => [
    ...
    if (upg_task_start(15870, 'Upgrading menus table...')) {	// part of 7.1.2-beta
        db_upgrade_cols('T_menus__menu', [
            'CHANGE' => [
    ...
    if (upg_task_start(15880, 'Upgrading templates table...')) {	// part of 7.1.2-beta
    ...
    if (upg_task_start(15890, 'Upgrading templates table...')) {	// part of 7.1.2-beta
        db_add_col('T_templates', 'tpl_owner_grp_ID', 'INT(4) NULL DEFAULT NULL AFTER tpl_context');
        upg_task_end();
    }

    if (upg_task_start(15900, 'Updating widgets "Sub-Container" and "Columns (Sub-Containers)"...')) {	// part of 7.1.2-beta
        ...
        if (! empty($widgets)) {	// If at least one widget exists in DB:
            foreach ($widgets as $widget) {
                if (empty($widget->wi_params)) {	// Skip widget with default settings:
                    continue;
                }
    ...
    if (upg_task_start(15910, 'Updating templates table...')) {	// part of 7.1.2-beta
    ...
    if (upg_task_start(15920, 'Updating templates table...')) {	// part of 7.1.2-beta
        db_upgrade_cols('T_templates', [
            'MODIFY' => [
    ...
    if (upg_task_start(15930, 'Upgrading user field definitions table...')) {	// part of 7.1.2-beta
    ...
    if (upg_task_start(15940, 'Upgrading user field definitions table...')) {	// part of 7.1.2-beta
        db_add_col('T_users__fielddefs', 'ufdf_grp_ID', 'int(10) UNSIGNED NULL');
        upg_task_end();
    }

    if (upg_task_start(15950, 'Updating templates table...')) {	// part of 7.1.2-beta
        db_upgrade_cols('T_templates', [
            'MODIFY' => [
    ...
    if (upg_task_start(15960, 'Installing widget container "Search Area" and updating "Search Form" widget...')) {	// part of 7.1.2-beta
        $SQL = new SQL();
    ...
    if (upg_task_start(15970, 'Updating "Search Form" widget...')) {	// part of 7.1.2-beta
        $SQL = new SQL();
        $SQL->SELECT('wi_ID, wi_params');
        $SQL->FROM('T_widget__widget');
        $SQL->WHERE('wi_code = "coll_search_form"');
        ...
        if (! empty($widgets)) {	// If at least one widget exists in DB:
            foreach ($widgets as $widget) {
                ...
                if (in_array($widget_params['template'], ['search_form_sidebar', 'search_form_header'])) {	// Merge previous search forms to new search_form_simple:
    ...
    if (upg_task_start(15980, 'Upgrading templates table...')) {	// part of 7.1.2-beta
        db_upgrade_cols('T_templates', [
            'MODIFY' => [
    ...
    if (upg_task_start(15990, 'Removing obsolete skins...<br>')) {	// part of 7.1.3-beta
        global $skins_path;

        $obsolete_skin_classes = ['asevo_Skin', 'basic_Skin', 'dating_mood_Skin', 'evopress_Skin', 'photoalbums_Skin', 'photoblog_Skin', 'touch_Skin'];

        load_funcs('skins/_skin.funcs.php');
        $SkinCache = &get_SkinCache();

        // Get list of available skins in skins folder:
        $skin_folders = get_filenames(
            $skins_path,
            [
                'inc_files' => false,
                'inc_dirs' => true,
        ...
        foreach ($obsolete_skin_classes as $skin_class) {
            if ($obsolete_Skin = $SkinCache->get_by_class($skin_class, false, false)) {	// Installed skin, display a warning:
                echo get_install_format_text_and_log(sprintf(
                    '<span class="text-warning"><evo:warning>'
                        ...
                        . '</evo:warning></span>',
                    '<code>' . $obsolete_Skin->get_default_name() . '</code>',
                    'https://skins.b2evolution.net/'
                ) . "<br />\n", 'br');
                if (isset($r[$skin_class][$obsolete_Skin->folder])) {	// Indicate this skin - version is installed:
            ...
            } else {	// Not installed, delete the folders:
                task_begin(sprintf('Trying to delete obsolete and unused skin: %s...', $skin_class));
                $folders_deleted = true;
                if (isset($r[$skin_class])) {	// Obsolete skin found in the skins folder:
                    foreach ($r[$skin_class] as $folder => $installed) {	// Iterate through all available versions:
    ...
    if (upg_task_start(16000, 'Upgrading skins table...')) {	// part of 7.1.3-beta
        db_modify_col('T_skins__skin', 'skin_name', 'varchar(128) COLLATE utf8mb4_unicode_ci NOT NULL');
        upg_task_end();
    }

    if (upg_task_start(16010, 'Upgrading blogs, skins, widgets tables for new skin type "Alt"...')) {	// part of 7.1.3-beta
    ...
    if (upg_task_start(16012, 'Updating site skins...')) {	// part of 7.1.5-stable
        ...
        foreach ($site_skins as $site_skin) {
            if ($site_skin->setting_grouping !== null) {	// Remove old setting "Grouping":
                $DB->query('DELETE FROM T_settings
					WHERE set_name = ' . $DB->quote('skin' . $site_skin->skin_ID . '_grouping'));
            }

            if ($site_skin->setting_menu_type !== null) {	// Don't update if "Menu type" was saved in DB:
                continue;
            }

            if ($site_skin->setting_grouping === null || $site_skin->setting_grouping == 1) {	// Set value in DB only for option "Automatic - Grouped collection list",
    ...
    if (upg_task_start(16013, 'Upgrading columns to utf8mb4_bin...')) {	// part of 7.1.5-stable
    ...
    if (upg_task_start(16083, 'Upgrading table for Menu entries and Converting menu widgets "Messaging", "Flagged Items" and "My Profile" into "Basic Menu link" widget...')) {	// part of 7.2
        db_upgrade_cols('T_menus__entry', [
            'ADD' => [
        ...
        foreach ($menu_widgets as $menu_widget) {
            $menu_widget_params = empty($menu_widget->wi_params) ? [] : unserialize($menu_widget->wi_params);
            if ($menu_widget->wi_code == 'msg_menu_link') {
                if (! isset($menu_widget_params['link_type'])) {	// Link type may be not set yet:
                ...
                if (empty($menu_widget_params['profile_picture_size'])) {	// Sset default picture profile size if it was not set yet:
    ...
    if (upg_task_start(16085, 'Creating default item type "Task"...')) {	// part of 7.2
        ...
        if (! $DB->get_var($SQL)) {	// Create one default item type "Task":
            ...
            if ($r && $DB->insert_id > 0) {	// Enable new created item type all forum and tracker:
                $DB->query('INSERT INTO T_items__type_coll ( itc_ityp_ID, itc_coll_ID )
					SELECT ' . $DB->insert_id . ', blog_ID
					  FROM T_blogs
    ...
    if (upg_task_start(16086, 'Upgrading item status table...')) {	// part of 7.2
        db_add_col('T_items__status', 'pst_order', 'INT(11) NULL DEFAULT NULL');
        upg_task_end();
    }

    if (upg_task_start(16087, 'Upgrading item status table...')) {	// part of 7.2
    ...
    if (upg_task_start(16088, 'Converting widget "Common Navigation Links" into "Embed Menu" widget...')) {	// part of 7.2
        ...
        if (! empty($nav_widgets)) {	// If at least one widget "Common Navigation Links" is found:
            ...
            $nav_menu_entries = [
                'show_home' => ['home', T_('Home')],
                'show_recently' => ['recentposts', T_('Recently')],
                'show_search' => ['search', T_('Search')],
                'show_postidx' => ['postidx', T_('Post index')],
                'show_archives' => ['arcdir', T_('Archives')],
                'show_categories' => ['catdir', T_('Categories')],
                'show_mediaidx' => ['mediaidx', T_('Photo index')],
                'show_latestcomments' => ['latestcomments', T_('Latest comments')],
                'show_owneruserinfo' => ['owneruserinfo', T_('Owner details')],
                'show_ownercontact' => ['ownercontact', T_('Contact')],
                'show_sitemap' => ['sitemap', T_('Site map')],
            ];
            foreach ($nav_widgets as $nav_widget_ID => $nav_widget_params) {
                $nav_widget_params = empty($nav_widget_params) ? [] : unserialize($nav_widget_params);

                if (empty($nav_widget_params)) {	// Set default params:
                    $nav_widget_params = [
                        'show_home' => 1,
                        'show_recently' => 1,
                        'show_archives' => 1,
                ...
                foreach ($nav_menu_entries as $nav_widget_param => $nav_menu_entry_data) {
                    if (empty($nav_widget_params[$nav_widget_param])) {	// Skip not enabled menu entry in old widget:
                        continue;
                    }
    ...
    if (upg_task_start(16089, 'Converting widget "User Tools" into "Embed Menu" widget...')) {	// part of 7.2
        $nav_widgets_SQL = new SQL('Get widgets "User Tools" before converting');
        $nav_widgets_SQL->SELECT('wi_ID, wi_params');
        $nav_widgets_SQL->FROM('T_widget__widget');
        $nav_widgets_SQL->WHERE('wi_code = "user_tools"');
        $nav_widgets = $DB->get_assoc($nav_widgets_SQL);
        if (! empty($nav_widgets)) {	// If at least one widget "User Tools" is found:
            load_class('menus/model/_sitemenu.class.php', 'SiteMenu');
            load_class('menus/model/_sitemenuentry.class.php', 'SiteMenuEntry');
            // widget_param_key => array( menu_entry_type, widget_param_for_text, {widget_param_for_show_badge} ):
            $nav_menu_entries = [
                'user_postnew_link_show' => ['postnew', 'user_postnew_link'],
                'user_messaging_link_show' => ['messages', 'user_messaging_link', 'show_badge'],
                'user_contacts_link_show' => ['contacts', 'user_contacts_link'],
                'user_view_link_show' => ['myprofile', 'user_view_link'],
                'user_profile_link_show' => ['profile', 'user_profile_link'],
                'user_picture_link_show' => ['avatar', 'user_picture_link'],
                'user_password_link_show' => ['password', 'user_password_link'],
                'user_preferences_link_show' => ['userprefs', 'user_preferences_link'],
                'user_subs_link_show' => ['usersubs', 'user_subs_link'],
                'user_admin_link_show' => ['admin', 'user_admin_link'],
                'user_logout_link_show' => ['logout', 'user_logout_link'],
            ];
            foreach ($nav_widgets as $nav_widget_ID => $nav_widget_params) {
                $nav_widget_params = empty($nav_widget_params) ? [] : unserialize($nav_widget_params);
                ...
                foreach ($nav_menu_entries as $nav_widget_param => $nav_menu_entry_data) {
                    ...
                    $nav_SiteMenuEntry->set('menu_ID', $nav_SiteMenu->ID);
                    $nav_SiteMenuEntry->set('type', $nav_menu_entry_data[0]);
                    if (isset($nav_widget_params[$nav_menu_entry_data[1]])) {	// Set text for menu entry:
                        $nav_SiteMenuEntry->set('text', $nav_widget_params[$nav_menu_entry_data[1]]);
                        // Remove old widget param:
                        unset($nav_widget_params[$nav_menu_entry_data[1]]);
                    }
                    if (isset($nav_menu_entry_data[2], $nav_widget_params[$nav_menu_entry_data[2]])) {	// Set "Show Badge" for menu entry:
    ...
    if (upg_task_start(16090, 'Installing new widget container "Site Map"...')) {	// part of 7.2
        install_new_default_widgets('sitemap');
        upg_task_end();
    }

    if (upg_task_start(16102, 'Upgrading columns to utf8mb4_bin...')) {	// part of 7.2
    ...
    if (upg_task_start(16110, 'Creating table for checklist lines...')) {	// part of 7.2
    ...
    if (upg_task_start(16130, 'Installing new widgets/containers...')) {	// part of 7.2.0-beta
        install_new_default_widgets('comment_list', 'request_title');
        upg_task_end();
    }

    if (upg_task_start(16140, 'Installing new widget title for search page and updating widget "Search Form"...')) {	// part of 7.2.0-beta
        ...
        foreach ($search_widgets as $search_widget_ID => $search_widget_params) {
            ...
            if (isset($search_widget_params['template']) &&
                $search_widget_params['template'] == 'search_form_full') {	// Add class "well" only for full form:
    ...
    if (upg_task_start(16150, 'Installing new widgets for container "Item List"...')) {	// part of 7.2.0-beta
        install_new_default_widgets('item_list', 'request_title');
        upg_task_end();
    }

    if (upg_task_start(16160, 'Dummy upgrade block, just to force execution of the upgrade procedure to update templates for cross-posted items...')) {	// part of 7.2.0-beta
        upg_task_end();
    }

    if (upg_task_start(16170, 'Upgrading menu entries table...')) {	// part of 7.2.1-beta
    ...
    if (param('exec_general_tasks', 'boolean', 1)) {	// We haven't executed these general tasks yet:
        ...
        if (! is_object($Plugins)) { // create Plugins object
            load_class('plugins/model/_plugins.class.php', 'Plugins');
            $Plugins = new Plugins();
        }
        ...
        if (is_array($check_cache_messages) && count($check_cache_messages)) { // Display errors of the cache checking
        ...
        if ($create_profile_picture_links) { // Create links for all files from the users profile_pictures folder
    ...
    $upgrade_db_deltas = []; // This holds changes to make, if any (just all queries)

    global $debug;

    foreach ($schema_queries as $table => $query_info) {	// For each table in the schema, check diffs...
        if ($debug) {
            echo get_install_format_text_and_log('<br />', 'br');
            echo get_install_format_text_and_log('Checking table: ' . $table . ': ');
        }
        $updates = db_delta($query_info[1], ['drop_column', 'drop_index'], false, true);
        if (empty($updates)) {
            if ($debug) {
                echo 'ok';
            }
            ...
            foreach ($updates as $table => $queries) {
                foreach ($queries as $qinfo) {
                    foreach ($qinfo['queries'] as $query) { // subqueries for this query (usually one, but may include required other queries)
    ...
    if (empty($upgrade_db_deltas)) {	// no upgrades needed:
        ...
        if (! $confirmed_db_upgrade) {
            if (! empty($upgrade_db_deltas_confirm_md5)) { // received confirmation from form
                if ($upgrade_db_deltas_confirm_md5 != md5(implode('', $upgrade_db_deltas))) { // unlikely to happen
        ...
        if (! $confirmed_db_upgrade) {
            ...
            if (empty($display) || $display != 'cli') {	// Don't print form on CLI mode:
            ...
            if (empty($display) || $display != 'cli') {	// Don't print form on CLI mode:
                $Form->submit(['', T_('Try to Repair/Upgrade database now!'), 'btn-warning']);
                $Form->end_form();
            }
