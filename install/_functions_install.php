...
function display_base_config_recap($display_system_check = false)
{
    ...
    if ($display_system_check) {	// Dispaly system check table:
    ...
    if ($create_demo_organization || $create_demo_users) {
        ...
        if ($create_demo_organization) {
            ...
            if ($user_org_IDs) {
                ...
                $admin_user->update_organizations($user_org_IDs, ['King of Spades'], [0], true);
                task_end();
            }
        }
...
function get_db_version()
{
    ...
    if (db_col_exists('T_settings', 'set_name')) { // we have new table format (since 0.9)
...
function get_default_locale_from_db()
{
    ...
    if (empty($DB)) { // DB doesn't exists yet
        return null;
    }
 ...
 */
function create_default_settings($override = [])
{
    ...
    if (! empty($install_site_color)) { // Set default site color
        $defaults['site_color'] = $install_site_color;
    }
    if (! empty($suspect_Group)) { // Set default antispam suspicious group
        $defaults['antispam_suspicious_group'] = $suspect_Group->ID;
    }
    $antispam_trust_groups = [];
    ...
    if (count($antispam_trust_groups) > 0) { // Set default antispam trust group
        $defaults['antispam_trust_groups'] = implode(',', $antispam_trust_groups);
    }
    if ($local_installation) { // Current installation is local
...
function install_basic_skins($install_mobile_skins = true)
{
    ...
    if ($default_site_Skin && $default_site_Skin->ID > 0) {	// Use the installed skin as default for site:
        global $Settings;
        if (empty($Settings)) {	// Initialize general settings:
            load_class('settings/model/_generalsettings.class.php', 'GeneralSettings');
            $Settings = new GeneralSettings();
        }
...
function install_basic_plugins($old_db_version = 0)
{
    ...
    if ($old_db_version < 9330) { // Upgrade to 1.9-beta
        install_plugin('ping_b2evonet_plugin');
        install_plugin('ping_pingomatic_plugin');
    }

    if ($old_db_version < 9930) { // Upgrade to 3.1.0
        install_plugin('tinymce_plugin');
    }

    if ($old_db_version < 9940) { // Upgrade to 3.2.0
        install_plugin('twitter_plugin');
    }

    if ($old_db_version < 11000) { // Upgrade to 5.0.0-alpha-4
        if ($install_test_features) {
            $captcha_qstn_plugin_settings = [
    ...
    if ($old_db_version < 11100) { // Upgrade to 5.0.0-alpha-5
        // antispam
    ...
    if ($old_db_version < 11200) { // Upgrade to 5.1.3-stable
        install_plugin('shortcodes_plugin');
    }

    if ($old_db_version < 11490) { // Upgrade to 6.7.1-stable
    ...
    modules_call_method('install_basic_plugins', [
 ...
 */
function install_plugin($plugin, $activate = true, $settings = [], $params = [])
{
    ...
    if (! ($edit_Plugin instanceof Plugin)) { // Broken plugin
    ...
    if (! empty($settings)) { // Set plugin settings
    ...
    if ($activate) { // Try to enable plugin:
        $enable_return = $edit_Plugin->BeforeEnable();
        if ($enable_return !== true) { // Warning on enable a plugin
    ...
    } else { // Set plugin status as disable
...
function install_basic_widgets()
{
    ...
    if (empty($installed_default_shared_widgets)) {	// Install default widgets only when they were not installed before,
    ...
    foreach ($BlogCache->cache as $Blog) {
        ...
        $Blog->setup_default_widgets('all', [
...
function install_htaccess($upgrade = false, $force_htaccess = false)
{
    ...
    if (! $force_htaccess) {	// Check if we run apache...
        $server = isset($_SERVER['SERVER_SOFTWARE']) ? $_SERVER['SERVER_SOFTWARE'] : '';
        if (! empty($server) && preg_match('~(Nginx|Lighttpd|Microsoft-IIS)~i', $server)) { // Skip installation if this is not an Apache server
    ...
    if ($error_message) {
        ...
        if ($htignore) { // Ignore error with .htaccess file and continue install...
            return true;
        } else { // Some errors are existing with .htaccess file, Display a link to ignore the errors and continue instalation
...
function do_install_htaccess($upgrade = false, $force_htaccess = false)
{
    ...
    if (! $force_htaccess) { // We're not forcing install, we want to check if it's supported:
        if (@file_exists($basepath . '.htaccess')) {
            if (@file_exists($basepath . 'sample.htaccess')) {
                ...
                if ($content_htaccess != $content_sample_htaccess) { // The .htaccess file has content that different from a sample file
                    if ($upgrade) {
                ...
                } else {	// Installed file is the same as recommended:
...
function track_step($current_step)
{
    ...
    if (! empty($display) && $display == 'cli') {	// Don't display HTML on CLI mode:
        return;
    }
...
function start_install_progress_bar($title, $steps = null)
{
    ...
    if (! empty($display) && $display != 'normal') { // Exit here, because we can use progress bar on normal mode (Hide on compact mode)
        return;
    }

    if ($steps !== null) { // Progress bar with steps
        $install_progress_bar_total = $steps;
        $install_progress_bar_counter = 0;
        $bar_width = '0%';
    } else { // Progress bar has no steps for update
        $bar_width = '100%';
    }
    ...
    echo '<div class="progress">'
            ...
            . '</div>'
        . '</div>';
    if ($steps !== null) { // Use this fix to keep the progress animated forever when JavaScript is disabled
        echo '<noscript>'
...
function update_install_progress_bar()
{
    ...
    if (empty($install_progress_bar_total)) { // No a number of the steps, Exit here
        return;
    }
...
function get_install_steps_count()
{
    ...
    if ($allow_evodb_reset >= 2 || ($allow_install_test_features && $allow_evodb_reset >= 1)) { // Allow to quick delete before new installation only when these two settings are enabled in config files
        $delete_contents = param('delete_contents', 'integer', 0);

        if ($delete_contents) { // A quick deletion is requested before new installation
            $steps++;
        }
    }
    ...
    if ($create_sample_contents) {
        ...
        if ($create_sample_contents == 'full') { // Array contains which collections should be installed
        ...
        } else { // Array contains which collections should be installed
        ...
        if ($install_collection_home) { // After installing of the blog "Home"
            $steps++;
        }
        if ($install_collection_bloga) { // After installing of the blog "Blog A"
            $steps++;
        }
        if ($install_collection_blogb) { // After installing of the blog "Blog B"
            $steps++;
        }
        if ($install_collection_photos) { // After installing of the blog "Photos"
            $steps++;
        }
        if ($install_collection_forums) { // After installing of the blog "Forums"
            $steps++;
        }
        if ($install_collection_manual) { // After installing of the blog "Manual"
            $steps++;
        }
        if ($install_collection_tracker) { // After installing of the blog "Tracker"
            $steps++;
        }
    }
...
function get_upgrade_steps_count()
{
    ...
    if ($new_db_version > $old_db_version) {	// Only when DB must be updated really:
        $upgrade_file_name = dirname(__FILE__) . '/_functions_evoupgrade.php';
        if (@file_exists($upgrade_file_name)) {	// If file exists we can parse to know how much the upgrade blocks will be executed:
            ...
            if (preg_match_all('#if\(\s*(\$old_db_version\s*<|upg_task_start\()\s*(\d+)#i', $upgrade_file_content, $version_matches)) {
                foreach ($version_matches[2] as $version) {
                    if ($old_db_version < $version && $new_db_version != $old_db_version) {	// Only these new blocks will be executed:
                        $steps++;
                    }
                }
            }
...
function display_install_messages($messages, $type = 'error')
{
    ...
    if ($type == 'error') { // Fix to correct bootstrap class name "alert-danger"
        $type = 'danger';
    }
...
function display_install_result_window($title, $body)
{
    ...
    if (! empty($display) && $display != 'normal') { // Exit here, because we can use the modal window ONLY on normal mode (Hide on compact mode)
        return;
    }
...
function check_quick_install_request()
{
    ...
    if (! empty($admin_login)) { // Try to use an admin login from request:
        if (param_check_valid_login('admin_login')) { // We can use this login name
            $install_login = $admin_login;
        } else { // Display an error message for impossible login name:
            $Messages->add(sprintf('ERROR: Impossible to use "%s" for the admin account. Using "admin" instead.', $admin_login));
        }
    }

    if (! empty($admin_password) && param_check_passwords('admin_password', 'admin_password', false, 5)) { // Set admin password from request:
        $install_password = $admin_password;
    }

    if ($config_is_done) { // Config is already done, Don't use url params to update basic config file
        return true;
    }
    ...
    if (! empty($conf_admin_email) ||
        ...
        ! empty($db_tableprefix)) { // Try to update basic config file ONLY when at least one of these params are defined
        if (empty($conf_admin_email) || ! param_check_email('admin_email')) { // Use default admin email if it is empty from request
        ...
        if (empty($baseurl) || ! preg_match('~https?://~', $baseurl)) { // Try to autogenerate base url if it is empty or wrong from request:
        ...
        if (update_basic_config_file($basic_config_params)) { // Basic config file has been created successfully
            // Update DB config to connect to DB correctly
            $db_config['user'] = $db_user;
            $db_config['password'] = $db_password;
            $db_config['name'] = $db_name;
            $db_config['host'] = $db_host;
        } else { // Failed on creation of basic config file
            return false;
        }
 ...
 */
function update_basic_config_file($params = [])
{
    ...
    if (! $params['print_messages']) { // Start to get all messages instead of printing on screen:
        ob_start();
    ...
    foreach ($check_install_params as $check_install_param_value => $check_install_field_title) {
        if (preg_match('#[\'\\\\]#', $params[$check_install_param_value])) {	// Param value cannot contains characters ' and \
    ...
    if (! $check_install_params_result) {	// Switch action to display a config form to fix errors:
        $action = 'start';
        if (! $params['print_messages']) {	// Return all messages instead of printing on screen:
            $basic_config_file_result_messages = ob_get_clean();
        }
        return false;
    }
    ...
    if ($params['create_db']) {	// Try to create DB if it doesn't exist yet:
    ...
    if ($DB->error) { // restart conf
        ...
        if (empty($file_loaded)) { // This should actually never happen, just in case...
        ...
        if (save_to_file($conf, $conf_filepath, 'w')) {
            ...
            $config_is_done = 1;
            if (! $params['quick_install']) { // Switch to menu only on standard installation:
                $action = 'menu';
            }
        } else {
            ?>
