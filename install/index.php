...
if ($is_cli) {	// Initialize params from CLI mode request:
    if (isset($_SERVER['argc'], $_SERVER['argv'])) {
        $argc = $_SERVER['argc'];
        $argv = $_SERVER['argv'];
    }
    if (isset($argv)) {	// may not be set for CGI
...
if (! $config_is_done) {	// Base config is not done yet, try to guess some values needed for correct display:
    $rsc_url = '../rsc/';
}
...
if ($action == 'newdb' && empty($config_is_done)) { // This is a request of quick installation
    $db_tableprefix = param('db_tableprefix', 'string', '');
    if (! empty($db_tableprefix)) { // Update global config variable $tableprefix before loading of all modules:
        global $tableprefix;
...
if ($memory_limit != -1 && $memory_limit < get_php_bytes_size('48M')) { // Deny to use on server with small memory limit size:
    $install_memory_limit_allow = false;
    if ($action != 'localeinfo') { // Restrict all actions except of action to switch a language and view page of more languages:
        $action = 'start';
    }
} else { // Allow to use installer when memory limit is enough:
...
if (! empty($default_locale) && ! empty($locales) && isset($locales[$default_locale])) { // Set correct charset, The main using is for DB connection
    $evo_charset = $locales[$default_locale]['charset'];
}


if ($action == 'newdb') { // Check request for quick installation AND Update basic config file from url params:
    $basic_config_file_result = check_quick_install_request();
}


if ($config_is_done || $try_db_connect) { // Connect to DB:
    ...
    if (! $DB->error) { // restart conf
        ...
        foreach ($required_mysql_version as $key => $value) { // check required MySQL version for the whole application and for each module
...
if (! $use_locale_from_request) { // detect language
    // try to check if db already exists and default locale is set on it
    $default_locale = get_default_locale_from_db();
    if (empty($default_locale)) { // db doesn't exists yet
        $default_locale = locale_from_httpaccept();
    }
    // echo 'detected locale: ' . $default_locale. '<br />';
    if (isset($locales[$default_locale]) && $evo_charset != $locales[$default_locale]['charset']) { // Redirect to install page with correct defined locale in order to avoid broken chars, e.g. when db locale has utf8 encoding and default locale - latin1
        header_redirect('index.php?locale=' . $default_locale);
        // Exit here.
    }
}
// Activate default locale:
if (! locale_activate($default_locale)) {	// Could not activate locale (non-existent?), fallback to en-US:
...
if ($action == 'menu-install' && ! ($old_db_version = get_db_version())) { // Force to step 3 (Select install options) if DB is not installed yet
    $action = 'menu-options';
}
...
// Form params
$booststrap_install_form_params = [
    'formstart' => '',
    'formend' => '',
...
if ($display != 'cli') {	// Don't display HTML on CLI mode:
    ?>
<!DOCTYPE html>
<html lang="<?php locale_lang() ?>">
	...
	<body>
		<div class="container" id="content_wrapper">
			<?php
                if ($display == 'normal') { // Display header only on normal mode:
                    ?>
			<div class="header">
				<nav>
...
if (! $install_memory_limit_allow) { // Display error that current memory limit size is not enough for correct using:
    display_install_messages(sprintf(
        T_('Your PHP configuration only allows us to use %s of RAM. The absolute minimum we need to install b2evolution is 48M. Please contact your web host or use a <a %s>compatible hosting provider</a>.'),
...
if (empty($date_timezone) && empty($date_default_timezone)) { // The default timezone is not set, display a warning
...
if (isset($basic_config_file_result) && ! $basic_config_file_result) {
    if (! empty($basic_config_file_result_messages)) { // Display messages that were generated on creating basic config file on quick installation:
        echo $basic_config_file_result_messages;
    }

    if ($display == 'normal') { // Set action to display a start form of installation:
        $action = 'start';
    } else { // 'compact'
...
if (($config_is_done || $try_db_connect) && ($DB->error)) { // DB connect was unsuccessful, restart conf
...
switch ($action) {
    case 'conf':
        ...
        if (! update_basic_config_file($basic_config_params) && $action == 'conf') { // Break here if some error on creating/updating basic config file
    ...
    case 'default':
         ...
         * -----------------------------------------------------------------------------------
         */
        if (! $install_memory_limit_allow) { // Don't allow any action when minimum memory limit is not enough for using:
    ...
    case 'menu':
        ...
        if (! is_null($old_db_version)) {
            ...
            if (! $DB->is_expected_connection_charset($expected_connection_charset)) {
                display_install_messages(sprintf(
                    T_('In order to install b2evolution with the %s locale, your MySQL needs to support the %s connection charset.') . ' (mysqli::set_charset(%s))',
                    $current_locale,
                    $evo_charset,
                ...
                break;
            } else { // Check if some of the tables have different charset than what we expect
                load_funcs('tools/model/_system.funcs.php');
                if (system_check_charset_update()) {
                    if (! is_null($old_db_version) && $old_db_version < $new_db_version) { // DB is not updagraded to new version yet, We should not suggest to normalize DB to avoid errors
                    ...
                    } else { // DB is already upgraded to last version, Siggest to normalize DB
    ...
    case 'menu-install':
		...
		<?php
        if ($allow_evodb_reset >= 2 || ($allow_install_test_features && $allow_evodb_reset >= 1)) { // We can allow to continue installation with deleting DB
            ?>
		...
		<?php
        } else { // Don't allow to continue installation because the DB deleting is disabled in config
            ?>
    ...
    case 'menu-options':
        ...
        ?>
			...
			<?php
        if ($allow_install_test_features) { // Checkbox to install all features
            ?>
        ...
        ?>
            ...
            if ((($allow_evodb_reset >= 2)
                ...
                && $old_db_version = get_db_version()) { // We can allow to delete DB before installation
                ?>
            ...
            } else { // Allow only install new DB without deleting previous DB
                ?>
    ...
    case 'newdb':
        ...
        if ($create_sample_contents == 'full') {	// Override create sample organization and demo user setting:
            $create_demo_organization = true;
            $create_demo_users = true;
            $create_demo_messages = true;
        }

        if ($allow_install_test_features) {	// Allow to use $allow_install_test_features from request only when it is enabled in config:
        ...
        if (set_max_execution_time(300) === false) { // max_execution_time ini setting could not be changed for this script, display a warning
        ...
        if ($htaccess != 'skip') {
            ...
            evo_flush();
            // Check for .htaccess:
            if (! install_htaccess(false, ($htaccess == 'force'))) { // Exit installation here because the .htaccess file produced some errors
                break;
            }
        }
    ...
    case 'auto_upgrade':
        ...
        display_install_messages(sprintf(
            '<p>%s<ol><li>%s</li><li>%s</li><li>%s</li></ol></p>',
            ...
            sprintf(T_('If needed, see <a %s>this man page &raquo;</a>'), 'href="http://b2evolution.net/man/auto-upgrade-procedure"')
        ), 'info');
        ...
        if ($htaccess != 'skip') {
            ...
            evo_flush();
            // Check for .htaccess:
            if (! install_htaccess(true, ($htaccess == 'force'))) { // Exit installation here because the .htaccess file produced some errors
                break;
            }
        }
        ...
        if ($is_automated_upgrade && $upgrade_result !== 'need-fix') {
            if ($upgrade_result === true) {	// After successful auto_upgrade we must remove files/folder based on the upgrade_policy.conf
            ...
            if (! switch_maintenance_mode(false, 'upgrade')) {	// If error on switching out of maintenance mode:
                $upgrade_result = false;
            }
        }

        if ($upgrade_result === 'need-fix') {	// We are waiting for the user to click on "try to repair/upgrade now"...
        ...
        } else {	// There has been an error during upgrade... (or upgrade was not performed)
...
<!-- InstanceEndEditable -->
			...
			<?php
                if ($display == 'normal') { // Display footer only on normal mode:
                    ?>
