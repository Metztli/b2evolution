...
if (! check_user_perm('admin', 'restricted')) {	// No permission to access admin...
    ...
    if ((($test_ctrl !== 'comments') && ($test_ctrl !== 'items'))
        ...
        || empty($test_action) || ! in_array($test_action, ['update', 'publish'/*, 'deprecate', 'delete'*/])) {
...
if (! check_user_status('can_access_admin')) {
    if (check_user_status('can_be_validated')) { // redirect back to the login page
...
if (isset($collections_Module)) {
    ...
    if (param('blog', 'integer', null, true) === null      // We got no explicit blog choice (not even '0' for 'no blog'):
        ...
        if (! ($Collection = $Blog = &$BlogCache->get_by_ID($blog, false, false))) { // That one doesn't exist either...
            $blog = 0;
            // Unset $Blog because otherwise isset( $Blog ) returns true and it may cause issues later
            unset($Blog, $Collection);
        }
    } elseif ($blog != $user_selected_blog) { // We have selected a new & valid blog. Update UserSettings for selected blog:
...
if (! $admin_skin || ! file_exists(sprintf($admin_skin_path, $admin_skin))) { // there's no skin for the user
    if (! $admin_skin) {
    ...
    if (! $admin_skin || ! file_exists(sprintf($admin_skin_path, $admin_skin))) { // even the default skin does not exist!
        if (! $admin_skin) {
            $Debuglog->add('There is no default admin skin set!', 'skins');
        } else {
            $Debuglog->add('The default admin skin [' . $admin_skin . '] does not exist!', ['skin', 'error']);
        }
        ...
        if ($admin_skin_dirs === false) {
            $Debuglog->add('No admin skin found! Check that the path ' . $adminskins_path . ' exists.', ['skin', 'error']);
        } elseif (empty($admin_skin_dirs)) { // No admin skin directories found
            $Debuglog->add('No admin skin found! Check that there are skins in ' . $adminskins_path . '.', ['skin', 'error']);
        } else {
...
if (empty($dont_request_controller) || ! $dont_request_controller) {	// Don't request the controller if we want initialize only the admin configs above (Used on AJAX refreshing of results table)
