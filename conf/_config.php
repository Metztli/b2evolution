...
// HARD MAINTENANCE !
if (file_exists(dirname(__FILE__) . '/maintenance.html')) { // Stop execution as soon as possible. This is useful while uploading new app files via FTP.
    header('HTTP/1.0 503 Service Unavailable');
    readfile(dirname(__FILE__) . '/maintenance.html');
    die();
} elseif (file_exists(dirname(__FILE__) . '/umaintenance.html')) { // Maintenance mode with a file - "umaintenance.html" with an "u" prevents access to the site but NOT to upgrade
...
// basic settings
if (file_exists(dirname(__FILE__) . '/_basic_config.php')) {	// Use configured base config:
...
if (file_exists(dirname(__FILE__) . '/_config_TEST.php')) { // Put testing conf in there (For testing, you can also set $install_password here):
    include_once dirname(__FILE__) . '/_config_TEST.php';   	// FOR TESTING / DEVELOPMENT OVERRIDES
}

if (empty($assets_baseurl)) {	// We need to set this var before we continue:
...
global $app_pro;
if (isset($app_pro) && $app_pro === true) {	// Load social settings only for PRO version:
    require_once dirname(__FILE__) . '/_social.php';	// social settings
}

if (file_exists(dirname(__FILE__) . '/_local.php')) { // Override for local config in there:
    include_once dirname(__FILE__) . '/_local.php';			// Will not be overridden on upgrade.
} elseif (file_exists(dirname(__FILE__) . '/_overrides_TEST.php')) { // Legacy file (not recommended):
...
// Handle debug cookie:
if ($debug == 'pwd') {	// Debug *can* be enabled/disabled by cookie:
    // Disabled until we find a reason to enable:
    $debug = 0;

    if (! empty($debug_pwd)) {	// We have configured a password that could enable debug mode:
        if (isset($_GET['debug'])) {	// We have submitted a ?debug=password
            if ($_GET['debug'] == $debug_pwd) {	// Password matches
                $debug = 1;
                evo_setcookie('debug', $debug_pwd, 0, $cookie_path, $cookie_domain, false, true);
            } else {	// Password doesn't match: turn off debug mode:
                evo_setcookie('debug', '', $cookie_expired, $cookie_path, $cookie_domain, false, true);
            }
        } elseif (! empty($_COOKIE['debug']) && $_COOKIE['debug'] == $debug_pwd) {	// We have a cookie with the correct debug password:
            $debug = 1;
        }
    }
}
...
if ($debug_jslog == 'pwd') {	// Debug *can* be enabled/disabled by cookie:
    // Disabled until we find a reason to enable:
    $debug_jslog = 0;

    if (! empty($debug_pwd)) {	// We have configured a password that could enable debug mode:
        if (isset($_GET['jslog'])) {	// We have submitted a ?jslog=password
            if ($_GET['jslog'] == $debug_pwd) {	// Password matches
                $debug_jslog = 1;
                evo_setcookie('jslog', $debug_pwd, 0, '/');
            } else {	// Password doesn't match: turn off debug mode:
                evo_setcookie('jslog', '', $cookie_expired, '/');
                if (! empty($_COOKIE['jslog_style'])) {	// Change the saved styles to hide jslog
        ...
        } elseif (! empty($_COOKIE['jslog']) && $_COOKIE['jslog'] == $debug_pwd) {	// We have a cookie with the correct debug password:
            $debug_jslog = 1;
            if (! empty($_COOKIE['jslog_style'])) {	// Change the saved styles to show jslog
...
if ($debug > 0 || $display_errors_on_production) {	// We are debugging or we want to display errors on screen production anyways:
    @ini_set('display_errors', 'On');
} else { // Do not display errors on screen:
...
if (version_compare($php_version, $required_php_version['application'], '<')) {
    $compat = sprintf(
        'You cannot use %1$s %2$s on this server because it requires PHP version %3$s or higher. You are running version %4$s.',
        $app_name,
        $app_version,
        $required_php_version['application'],
        $php_version
    );
...
if ((! empty($date_default_timezone) || empty($date_timezone))) { // Set default timezone if $date_default_timezone is set or php.ini 'date.timezone' setting was not set
