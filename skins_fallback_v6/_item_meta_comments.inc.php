...
if (is_single_page() &&
    $Item->can_see_meta_comments()) {	// Display the internal comments if current user has a permission:
    $Form = new Form();
    ...
    if ($Item->can_meta_comment() || $total_comments_number > 0) {
        $Form->begin_fieldset(
            T_('Internal comments')
                            . ($total_comments_number > 0 ? ' <span class="badge badge-important">' . $total_comments_number . '</span>' : ''),
            [
                'class' => 'evo_item_meta_comments',
            ]
        );

        if ($Item->can_meta_comment()) {	// Display a form to add new internal comment if current user has a permission:
            skin_include('_item_comment_form.inc.php', array_merge($params, [
