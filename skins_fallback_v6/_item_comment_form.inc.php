 ...
 * Comment form:
 */
if ($params['comment_type'] == 'meta') {	// Use different form anchor for internal comments:
...
if ($params['comment_type'] == 'meta') {
    ...
    if ($params['disp_comment_form'] && ($params['comment_type'] == 'meta' && $Item->can_meta_comment() ||
            $Item->can_comment($params['before_comment_error'], $params['after_comment_error'], '#', $params['comment_closed_text'], $section_title, $params))) { // We want to display the comments form and the item can be commented on:
        echo '<a name="meta-comment-form"></a>';

        echo $params['before_comment_form'];

        // INIT/PREVIEW:
        if ($Comment = get_comment_from_session('preview', $params['comment_type'])) {	// We have a comment to preview
            ...
            if (($Comment = get_comment_from_session('unsaved', $params['comment_type'])) == null) { // there is no saved Comment in Session
                $Comment = new Comment();
            ...
            if ($quoted_comment_ID || $quoted_post_ID) { // Quote for comment/post
                ...
                if (! empty($quoted_comment_ID) &&
                        ...
                        $params['comment_type'] == $quoted_Comment->get('type')) {	// Allow comment quoting only for the same comment type form:
                ...
                } elseif (! empty($quoted_post_ID) && $params['comment_type'] != 'meta') {	// Allow item quoting only for normal(not meta) comment type form:
        ...
        if ($params['comment_type'] != 'meta') {	// Display a message before comment form:
        ...
        */
        ...
        $Form->textarea_input($dummy_fields['content'], $comment_content, $params['textarea_lines'], $params['form_comment_text'], [
            'note' => $note,
            'cols' => 38,
        ...
        */
        ...
        if ($Item->can_attach()) {	// Display attach file input field when JavaScript is disabled:
            ...
            $Form->input_field([
                ...
                'note' => $params['comment_attach_info'],
                'name' => 'uploadfile[]',
                'type' => 'file',
            ]);
