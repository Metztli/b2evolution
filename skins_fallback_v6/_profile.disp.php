...
if ($params['display_profile_tabs'] && $disp != 'register_finish') { // Display profile tabs
...
switch ($disp) {
    ...
    case 'profile':
        ...
        if ($edited_User->has_avatar()) { // Load javascript function to open popup window with crop picture tool
            echo_user_crop_avatar_window();
        }
        break;
    case 'avatar':
        if ($action == 'crop') { // Form to crop picture
            require $inc_path . 'users/views/_user_crop.form.php';
        } else { // Form to view avatars
