...
if (! isset($collections_Module)) {	// The evocore framework is not used as a CMS app here / we don't know how to display a public interface...
...
if (! init_requested_coll_or_process_tinyurl(true, true)) {	// No specific collection to be displayed:
    if ($Settings->get('default_blog_ID') == -1) {	// we are going to display the admin page:
        global $dispatcher;

        if (! is_logged_in()) {	// User must be logged in and his/her account must be validated before access to admin:
    ...
    } else {	// we are going to display the default page:
