...
<html lang="<?php locale_lang() ?>">
    ...
    if ($pagenow == 'index.php' || count($BlogCache->cache) == 0) { // This page is actually included by the index.html page OR there are no blogs
        ?>
    ...
    'block_start' => '<div class="action_messages">',
        ...
        if (count($BlogCache->cache) == 0) { // There is no blog on this system!
            ...
            if (check_user_perm('blogs', 'create')) { // Display this link only for users who can create blog
                ?>
    ...
    if (count($BlogCache->cache)) { // There are blogs on this system!
        ?>
		...
		<?php // --------------------------- BLOG LIST -----------------------------
            for ($l_Blog = &$BlogCache->get_first();
                ! is_null($l_Blog);
                $l_Blog = &$BlogCache->get_next()) { # by uncommenting the following lines you can hide some blogs
