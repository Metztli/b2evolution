...
class AdminUI_general extends Menu
{
     ...
     * 		'attrs' = ''
     */
    public function breadcrumbpath_init($add_blog = true, $additional_path = [])
    {
        ...
        if (! empty($additional_path)) { // Additional path
            $path = array_merge([
                'text' => '',
                'url' => '',
                'help' => null,
        ...
        if ($add_blog && isset($Blog)) { // Add path to Blog
    ...
    public function display_breadcrumbpath_init()
    {
        $this->display_breadcrumbpath = [];
    }
     ...
     */
    public function breadcrumbpath_add($text, $url, $help = null, $attrs = '')
    {
     ...
     * @param string Additional attributes for link tag
     */
    public function display_breadcrumbpath_add($text, $url = null, $help = null, $attrs = '')
    {
        global $Collection, $Blog;
     ...
     */
    public function set_page_manual_link($topic)
    {
     ...
     */
    public function breadcrumbpath_get_html($params = [])
    {
        $params = array_merge([
     ...
     * @return string Breadcrumb path links
     */
    public function display_breadcrumbpath_get_html($params = [])
    {
        $params = array_merge([
     ...
     */
    public function unshift_menu_entries($path, $entries)
    {
     ...
     * @return string
     */
    public function get_title($reversedDefault = false)
    {
        if (isset($this->title)) {	// Explicit title has been set:
            return $this->title;
        } else {	// Fallback: implode title/text properties of the path
            ...
            if (count($titles) > 1) {	// Remove 'Dashboard' text from the title
                array_shift($titles);
            }
     ...
     * @return string
     */
    public function get_title_for_titlearea()
    {
        if (! isset($this->title_titlearea)) { // Construct path:
            $titles = [];
            foreach ($this->path as $i => $lPath) {
                if (false !== ($title_text = $this->get_prop_for_path($i, ['title', 'text']))) {
                    $titles[] = '<a href="' . $this->get_prop_for_path($i, ['href']) . '">' . $title_text . '</a>';
                }
            }
     ...
     */
    public function append_to_titlearea($string)
    {
    ...
    public function get_html_title()
    {
        ...
        if (! empty($this->htmltitle)) {	// Get html title which is specified for current page:
            $r = $this->htmltitle;
        } elseif ($htmltitle = $this->get_prop_for_node($this->path, ['htmltitle'])) {	// Explicit htmltitle set:
            $r = $htmltitle;
        } else {	// No explicit title set, construct Title from path
            $r = $this->get_title();
        }
     ...
     */
    public function get_properties_for_path($path, $prop_by_pref)
    {
        if (! is_array($path)) {
            $path = [$path];
        }
     ...
     */
    public function get_prop_for_node($path, $prop_by_pref)
    {
     ...
     */
    public function get_prop_for_path($depth, $prop_by_pref)
    {
     ...
     */
    public function disp_html_head()
    {
        ...
        if (isset($this->skin_name) && file_exists($adminskins_path . $this->skin_name . '/_html_header.inc.php')) { // Get header of the skin
            require $adminskins_path . $this->skin_name . '/_html_header.inc.php';
        } else { // Get general header
     ...
     */
    public function disp_body_top($display_messages = true)
    {
        if (is_ajax_content()) {	// Don't display this content on AJAX request
            return;
        }
        ...
        if (! empty($mode)) {
            ...
            if ($display_messages) { // Display info & error messages
                $Messages->display();
        ...
        if (show_toolbar()) { // user is logged in
            if ($this->get_show_evobar()) { // show evobar options is enabled for this admin skin
     ...
     */
    public function disp_global_footer()
    {
        if (is_ajax_content()) {	// Don't display this content on AJAX request
            return;
        }
        ...
        if (isset($this->skin_name) && file_exists($adminskins_path . $this->skin_name . '/_html_footer.inc.php')) { // Get footer of the skin
            require $adminskins_path . $this->skin_name . '/_html_footer.inc.php';
        } else { // Get general footer
     ...
     */
    public function disp_payload_begin($params = [])
    {
        if (is_ajax_content()) {	// Don't display this content on AJAX request
            return;
        }
        ...
        global $mode;
        if ($mode == 'customizer') {	// Don't display this content on skin customizer mode:
            return;
        }
        ...
        if (empty($this->displayed_sub_begin)) {	// We haven't displayed sub menus yet (tabs):
            ...
            $r .= $this->get_html_menu([$path0, $path1], 'menu3', 0, ! $params['display_menu3']);
     ...
     */
    public function disp_payload_end()
    {
        if (is_ajax_content()) {	// Don't display this content on AJAX request
            return;
        }
     ...
     */
    public function disp_view($view_name, $params = [])
    {
        global $inc_path;
     ...
     */
    public function set_coll_list_params(
        $permname = 'blog_ismember',
        $permlevel = 1,
        $url_params = [],
        $all_title = null,
        $all_url = '',
     ...
     * @return string HTML
     */
    public function get_bloglist_buttons($title = '')
    {
        global $blog, $pagenow;

        $max_buttons = 7;

        if (empty($this->coll_list_permname)) {	// We have not requested a list of blogs to be displayed
            return;
        }
        ...
        foreach ($blog_array as $l_blog_ID) { // Loop through all blogs that match the requested permission:
            ...
            if ($l_Blog->favorite() || $l_blog_ID == $blog) { // If blog is favorute OR current blog, Add blog as a button:
                ...
                if (! is_null($this->coll_list_onclick)) {	// We want to include an onclick attribute:
            ...
            if (! $l_Blog->favorite()) { // If blog is not favorute, Add it into the select list:
        ...
        if (! empty($this->coll_list_all_title)) { // We want to add an "all" button
        ...
        if ($not_favorite_blogs) { // Display select list with not favorite blogs
            $r .= '<form action="' . $pagenow . '" method="get">';
            $r .= $form_params;
            $r .= '<select name="blog" onchange="';
            if (empty($this->coll_list_onclick)) { // Just submit...
     ...
     */
    public function get_template($name, $level = 0, $die_on_unknown = false)
    {
        switch ($name) {
            case 'main':
                return [
            ...
            default:
                if ($die_on_unknown) { // Die because template name is unknown
                    debug_die('Unknown $name for AdminUI::get_template(): ' . var_export($name, true));
                } else { // Return NULL, if we want to know when template is not defined by current skin
                    return null;
                }
        }
    }
     ...
     */
    public function get_path($which, $withProps = false)
    {
        if ($which === 'last') {
        ...
        if ($withProps) {
            return [
                ...
                'props' => isset($this->pathProps[$which]) ? $this->pathProps[$which] : [],
            ];
        }
     ...
     */
    public function get_path_range($start, $end = null)
    {
        if (is_null($end)) {
            $end = $start;
        }
     ...
     */
    public function set_path_level($level, $pathKey, $pathProps = [])
    {
        // Get the parent node (the level above this one):
        if ($level == 0) { // first level in menu-path: parent node is NULL
            $parentNode = &$this->get_node_by_path(null);
        } else { // parent node is the trunk from root to previous level
            $parentNode = &$this->get_node_by_path($this->get_path_range(0, $level - 1));
        }
        if (! $parentNode) { // parent node does not exist:
            return false;
        }
     ...
     */
    public function append_path_level($path, $pathProps = [])
    {
    ...
    public function set_path()
    {
        ...
        $i = 0;
        $prevPath = [];  // Remember the path we have walked through

        // Loop though all path levels to set:
        foreach ($args as $arg) {
            if (is_array($arg)) { // Path name and properties given
                list($pathName, $pathProps) = $arg;
            } else { // Just the path name
                $pathName = $arg;
                $pathProps = [];
            }
            ...
            if ($node = &$this->get_node_by_path(array_merge($prevPath, [$pathName]))) { // the node exists in the menu entries: merge the properties
                $pathProps = array_merge($node, $pathProps);
            }
    ...
    public function init_menus()
    {
        ...
        if (! empty($this->_menus)) {	// Already initialized!
            return;
        }
     ...
     * @return string
     */
    public function get_body_bottom()
    {
        return '';
    }
     ...
     * @return string
     */
    public function get_color($what)
    {
        switch ($what) {
     ...
     */
    public function display_skin_settings($Form, $user_ID)
    {
    }
     ...
     * There is no common skin option.
     */
    public function set_skin_settings($user_ID)
    {
    }
     ...
     * @return boolean
     */
    public function get_show_evobar()
    {
        return true;
    }
     ...
     * @param array Params
     */
    public function display_customizer_tabs($params = [])
    {
    }
}
