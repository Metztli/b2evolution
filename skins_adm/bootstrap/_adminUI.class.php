...
class AdminUI extends AdminUI_general
{
     ...
     */
    public function disp_body_top($display_messages = true, $params = [])
    {
        global $Messages;
     ...
     * @return array
     */
    public function get_template($name, $depth = 0, $die_on_unknown = false)
    {
        switch ($name) {
            case 'main':
                // main level
            ...
            case 'accordion_table':
                return array_merge($this->get_template('Results'), [
    ...
    public function get_bloglist_buttons($title = '')
    {
        ...
        if ($this->coll_list_disp_sections) {	// Check if filter by section is used currently:
        ...
        if (! empty($this->coll_list_all_title)) { // We want to add an "all" button
            ...
            $button_list_all = '';
        } else {	// Default button to list all collections:
        ...
        if ($this->coll_list_disp_add && check_user_perm('blogs', 'create')) {	// Display a button to add new collection if it is requested and current user has a permission
        ...
        } else {	// No request or permission to add new collection:
            $button_add_blog = '';
        }
        ...
        if ($this->coll_list_disp_sections) {
            ...
            foreach ($SectionCache->cache as $Section) {	// Loop through all sections that match the requested permission:
        ...
        return str_replace(
            ['$button_list_all$', '$button_add_blog$', '$collection_groups$'],
            [$button_list_all, $button_add_blog, $collection_groups],
            $r
        );
    }
    ...
    public function display_customizer_tabs($params = [])
    {
        ...
        if (empty($Blog)) {	// Get last working collection:
            $BlogCache = &get_BlogCache();
            $tab_Blog = &$BlogCache->get_by_ID(get_working_blog(), false, false);
        } else {	// Use current collection:
            $tab_Blog = $Blog;
        ...
        if ($Settings->get('site_skins_enabled') &&
                check_user_perm('options', 'edit')) {	// If current User can edit site skin settings:
            $tabs['site'] = [
                'text' => T_('Site'),
        ...
        // Collection:
        if (check_user_perm('blog_properties', 'edit', false, $tab_Blog->ID)) {	// If current User can edit current collection settings:
            $tabs['coll'] = [
        ...
        if (count($BlogCache->cache) > 1) {	// If current User can edit settings of at least two collections:
            $tabs['other'] = [
                'text' => T_('Other'),
        ...
        if (count($tabs)) {	// Display tabs if they are allowed for current user by permissions:
            $path = (empty($params['path']) || is_string($params['path'])) ? [$params['path']] : $params['path'];

            $active_tab_entries = null;
            echo '<ul class="nav nav-tabs">';
            foreach ($tabs as $tab_key => $tab) {
                $is_active_tab = (isset($path[0]) && $tab_key == $path[0]);
                if ($is_active_tab && ! empty($tab['entries'])) {	// Store entries of active tab to print out it below:
            ...
            if ($active_tab_entries !== null) {	// Display sub menu entries for currently active tab:
        ...
        if (! empty($params['action_links'])) {	// Display additional action links:
