...
{
    public $code = 'b2evALnk';

    public $name = 'Auto Links';
    ...
    public $help_topic = 'autolinks-plugin';

    public $number_of_installs = null;	// Let admins install several instances with potentially different word lists
     ...
     * @var boolean
     */
    public $previous_used = false;

    public $already_linked_usernames;
     ...
     * @var array
     */
    public $tags_array = null;

    public $already_linked_tags = null;
    ...
    public function get_coll_setting_definitions(&$params)
    {
        ...
        if (isset($coll_params['autolink_defs_coll_db'])) {	// Store this param in order to put under "Autolink tags":
            $coll_param_autolink_defs_coll_db = $coll_params['autolink_defs_coll_db'];
            unset($coll_params['autolink_defs_coll_db']);
        }

        $coll_params['autolink_tag'] = [
        ...
        if (isset($coll_param_autolink_defs_coll_db)) {	// Put the setting under "Autolink tags":
     ...
     * @param object Blog
     */
    public function load_link_array($Blog)
    {
        global $plugins_path;

        if (! isset($this->link_array[0])) {	// global defs NOT already loaded
            $this->link_array[0] = [];

            if ($this->get_checklist_setting('autolink', 'defs_default')) {	// Load defaults:
                $this->read_csv_file($plugins_path . 'autolinks_plugin/definitions.default.txt', 0);
            }
            if ($this->get_checklist_setting('autolink', 'defs_local')) {	// Load local user defintions:
                $this->read_csv_file($plugins_path . 'autolinks_plugin/definitions.local.txt', 0);
            }
            $text = $this->Settings->get('autolink_defs_db', 0);
            if (! empty($text)) {	// Load local user defintions:
        ...
        if (! isset($this->link_array[$coll_ID])) {	// This blog is not loaded yet:
     ...
     */
    public function read_csv_file($filename, $coll_ID)
    {
        if (! $handle = @fopen($filename, 'r')) {	// File could not be opened:
            return;
        }
     ...
     */
    public function read_textfield($text, $coll_ID)
    {
        // split into lines:
        $lines = preg_split('#\r|\n#', $text);
     ...
     */
    public function read_line($data, $coll_ID)
    {
        if (empty($data[0])) {	// Skip empty and comment lines
            return;
        }

        $word = $data[0];
        $url = isset($data[3]) ? $data[3] : null;
        if ($url == '-' || empty($url)) { // Remove URL (useful to remove some defs on a specific site):
            unset($this->link_array[0][$word]);
            unset($this->link_array[$coll_ID][$word]);
        } else {
            if (! isset($this->link_array[$coll_ID][$word])) { // Initialize array only first time to store several previous words for each word:
     ...
     * return boolean
     */
    public function render_content(&$content, $item_Blog = null)
    {
        // Reset already linked usernames:
        $this->already_linked_usernames = [];
        ...
        $this->already_linked_tags = [];

        // reset already linked:
        $this->already_linked_array = [];
        if (preg_match_all('|[\'"](http://[^\'"]+)|i', $content, $matches)) {	// There are existing links:
            $this->already_linked_array = $matches[1];
        }

        if ($this->setting_autolink_urls) {	// Make the URLs clickable:
            $content = make_clickable($content, '&amp;', [$this, 'make_clickable_callback_urls'], '', true);
        }

        if ($this->setting_autolink_emails) {	// Make the email addresses clickable:
            $content = make_clickable($content, '&amp;', [$this, 'make_clickable_callback_emails'], '', true);
        }

        // Make the desired remaining terms/definitions, usernames or tags clickable:
        $content = make_clickable($content, '&amp;', [$this, 'make_clickable_callback_definitions'], '', true);
     ...
     */
    public function make_clickable_callback_urls($text, $moredelim = '&amp;', $additional_attrs = '')
    {
     ...
     * @return string The clickable text.
     */
    public function make_clickable_callback_emails($text, $moredelim = '&amp;', $additional_attrs = '')
    {
        if (! empty($additional_attrs)) {
            $additional_attrs = ' ' . trim($additional_attrs);
        }
     ...
     * @return string The clickable text.
     */
    public function make_clickable_callback_definitions($text, $moredelim = '&amp;')
    {
        global $evo_charset;

        if (! empty($this->replacement_link_array)) {	// Make the desired remaining terms/definitions clickable:
            $regexp_modifier = '';
            if ($evo_charset == 'utf-8') { // Add this modifier to work with UTF-8 strings correctly
                $regexp_modifier = 'u';
            }
            ...
            foreach ($text_words as $text_word) { // Trim the signs [({/ from start and the signs ])}/.,:;!? from end of each word
                $clear_word = preg_replace('#^[\[\({/]?([@\p{L}0-9_\-]{3,})[\.,:;!\?\]\)}/]?$#i', '$1', $text_word);
                if ($clear_word != $text_word) { // Append a clear word to array if word has the punctuation signs
            ...
            if ($text_contains_replacement) { // Find word with 3 characters at least:
                $text = preg_replace_callback('#(^|\s|[(),;\'\"\[{/])([@\p{L}0-9_\-\.]{3,})([\.,:;!\'\"\?\]\)}/]?)#i' . $regexp_modifier, [&$this, 'replace_callback'], $text);
            }
        ...
        $text = replace_outside_code_and_short_tags('#@([a-z0-9_.\-]+)#i', '@', $text, [$this, 'replace_usernames']);
    ...
    public function replace_callback($matches)
    {
        ...
        $link_attrs = '';
        if ($this->setting_nofollow_auto) {	// Add attribute rel="nofollow" for auto-links:
        ...
        if (substr($word, -1) == '.') { // If word has a dot in the end
        ...
        if (isset($this->replacement_link_array[$lword])) { // There is an autolink definition with the current word
            if (! empty($this->previous_lword) && isset($this->replacement_link_array[$lword][$this->previous_lword])) { // Set an previous word and url from config array:
            ...
            } else { // No previous word, it is a single word
                foreach ($this->replacement_link_array[$lword] as $previous => $url) { // Initialize an optional previous required word and url as first of the current word
                    break;
                }
            }

            if (! preg_match('#(^|[a-z]+:)//#', $url)) {	// Use default URL scheme if it is not defined by config:
                $url = 'http://' . $url;
            }

            if (in_array($url, $this->already_linked_array) || in_array($lword, $this->already_linked_usernames)) { // Do not repeat link to same destination:
            ...
            if (($disp == 'single' || $disp == 'page') &&
                isset($Item) &&
                $Item->get_permanent_url() == $url) {	// Do not make a link to same permalink URL of the current Item:
                return $r;
            }

            if (! empty($previous)) { // This definitions is a group of two word separated with space
                if ($this->previous_used || ($this->previous_lword != $previous)) { // We do not have the required previous word or it was already used to another autolink definition
        ...
        } else { // Mark that the previous word was NOT converted to a link
            $this->previous_used = false;
        }
     ...
     */
    public function replace_usernames($content, $search_list, $replace_list)
    {
        if (empty($this->setting_autolink_username)) {	// No data to correct username linking, Exit here:
            return $content;
        }
        ...
        if (preg_match_all($search_list, $content, $user_matches)) {
            ...
            if (! empty($user_matches[1])) {
                ...
                foreach ($user_matches[1] as $u => $username) {
                    if (in_array($username, $this->already_linked_usernames)) {	// Skip this username, it was already linked before
                        continue;
                    }

                    $username_source = $user_matches[0][$u];
                    if (! ($User = &$UserCache->get_by_login($username))) {	// If user is not found try to find by removing a dot at the end,
                        ...
                        if ($username_without_dot != $username &&
                            ($User = &$UserCache->get_by_login($username_without_dot))) {	// We found user by username without dots at the end:
     ...
     * @param object Blog
     */
    public function load_tags_array($Blog)
    {
        if (empty($this->setting_autolink_tag)) {	// Don't load tags because it is not required by current settings:
            return;
        }

        if (is_array($this->tags_array)) {	// The tags array is already initialized, Don't do this twice, Exit here:
            return;
        }
    ...
    {
        if (empty($this->setting_autolink_tag) || empty($this->tags_array) || empty($this->current_Blog)) {	// No data to correct tag linking, Exit here:
            return $content;
        }
        ...
        foreach ($this->tags_array as $tag_name) {
            if (in_array($tag_name, $this->already_linked_tags)) {	// Skip this tag because it has been already linked before:
                continue;
            }
            if (stristr($content, $tag_name) !== false) {	// Replace tag name with its link if it is found in text:
        ...
        if (count($tag_search_patterns)) {	// Do replacement if at least one tag is found in content:
            if (stristr($content, '<a') !== false) {	// Don't link tags in body of already existing links:
                $content = callback_on_non_matching_blocks(
                    $content,
                    '~<a[^>]*>.*?</a>~is',
                    'replace_content',
                    [$tag_search_patterns, $tag_replace_strings, 'preg', 1]
                );
            } else {	// Replace in whole content:
        ...
        foreach ($new_linked_tags as $n => $new_linked_tag) {
            if (stristr($content, $new_linked_tag . '</a>') === false) {	// This tag was not linked really in this call, Skip it:
        ...
        if (count($new_linked_tags)) {	// Append new linked tags to skip them in next times:
