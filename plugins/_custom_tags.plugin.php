...
{
    public $code = 'b2evCTag';

    public $name = 'Custom Tags';
    ...
    // Internal
    public $toolbar_label = 'Custom Tags:';
    ...
    public $configurable_shared_list = true;

    public $post_search_list;

    public $post_replace_list;

    public $comment_search_list;

    public $comment_replace_list;

    public $msg_search_list;

    public $msg_replace_list;

    public $email_search_list;

    public $email_replace_list;

    public $shared_search_list;

    public $shared_replace_list;

    public $default_search_list = '[warning] #\[warning](.+?)\[/warning]#is
[info] #\[info](.+?)\[/info]#is
[clear] #\[clear]#is
[left] #\[left](.+?)\[/left]#is
...
[note] #\[note](.+?)\[/note]#is';
    ...
    public $default_replace_list = '<div class="alert alert-warning" markdown="1">$1</div>
...
<span class="note">$1</span>';
    ...
    {
        if (! $search_list_string) { // No search list string, use default search list string
        ...
        foreach ($search_list_array as $l => $line) {
            $line = explode(' ', $line, 2);
            if (empty($line[1])) { // Bad format of search string
                unset($search_list_array[$l]);
            } else { // Replace this line with regex value (to delete a button name)
     ...
     */
    public function prepare_replace_list($replace_list_string)
    {
        if (! $replace_list_string) { // No replace list string, use default replace list string
    ...
    public function get_coll_setting_definitions(&$params)
    {
        ...
        if (! empty($params['blog_type']) && get_class($this) == 'custom_tags_plugin') {	// Set default settings depending on collection type:
        ...
        if ($this->configurable_post_list) {
            $plugin_params['coll_post_search_list'] = [
            ...
            $plugin_params['coll_post_replace_list'] = [
        ...
        if ($this->configurable_comment_list) {
            $plugin_params['coll_comment_search_list'] = [
            ...
            $plugin_params['coll_comment_replace_list'] = [
    ...
    public function get_msg_setting_definitions(&$params)
    {
        ...
        if ($this->configurable_message_list) {
            ...
            $plugin_params['msg_replace_list'] = [
    ...
    public function get_email_setting_definitions(&$params)
    {
        ...
        if ($this->configurable_email_list) {
            $plugin_params['email_search_list'] = [
            ...
            $plugin_params['email_replace_list'] = [
    ...
    public function get_shared_setting_definitions(&$params)
    {
        ...
        if ($this->configurable_shared_list) {
            $plugin_params['shared_search_list'] = [
            ...
            $plugin_params['shared_replace_list'] = [
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        if ($this->is_shared_widget($params)) {	// Use settings for shared container widgets:
        ...
        } else {	// Use settings for collection:
    ...
    public function DisplayCodeToolbar(&$params)
    {
        ...
        switch ($params['target_type']) {
            ...
            case 'Comment':
                ...
                if (! empty($params['Comment']) && ! empty($params['Comment']->item_ID)) {	// Get Blog from Comment
                    $Comment = &$params['Comment'];
                    $comment_Item = &$Comment->get_Item();
                    $item_Blog = &$comment_Item->get_Blog();
                } elseif (! empty($params['Item'])) {	// Get Blog from Item
        ...
        if (empty($apply_rendering) || $apply_rendering == 'never') {	// Don't display a toolbar if plugin is disabled:
            return false;
        }

        if (empty($search_list)) {	// No list defined
            return false;
        }
        ...
        if (empty($tagButtons)) {	// No buttons for toolbar
            return false;
        }
        ...
        echo $this->get_template('toolbar_before', [
            '$toolbar_class$' => $js_code_prefix . 'toolbar',
        ]);
