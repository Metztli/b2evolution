...
class basic_antispam_plugin extends Plugin
{
     ...
     */
    public $name = 'Basic Antispam';

    public $code = 'b2evBAspm';
    ...
    public function GetSpamKarmaForComment(&$params)
    {
        ...
        if ($max_comments != -1) { // not deactivated:
    ...
    {
        if ($this->Settings->get('trim_whitespace')) {	// Strip whitespace
            $params['comment'] = trim($params['comment']);
        }

        if ($this->Settings->get('remove_repetitions')) {	// Remove repetitions
    ...
    public function BeforeCommentFormInsert(&$params)
    {
        ...
        if (is_logged_in()) {	// If user is logged in then we can get minimum comment interval per group:
            global $current_User;
            $min_comment_interval = $this->GroupSettings->get('comment_throttling', $current_User->grp_ID);
        } else {	// Use plugin setting of minimum comment interval for all anonymous users:
            $min_comment_interval = $this->Settings->get('min_comment_interval');
        }

        if ($params['action'] != 'preview' &&
            ! empty($min_comment_interval) &&
            ! $params['Comment']->is_meta()) {	// If a comment posting should be blocked by minumum interval:
            ...
            if ($last_comment_time = $DB->get_var($SQL)) {	// If last comment is found from current IP or email address:
                $last_comment_time = mysql2date('U', $last_comment_time);
                $new_comment_time = mysql2date('U', date('Y-m-d H:i:s', $localtimenow));
                if (($new_comment_time - $last_comment_time) < $min_comment_interval) {	// Block new comment because of minumum interval:
        ...
        if ($this->is_duplicate_comment($params['Comment'])) {	// Block a duplicate comment:
        ...
        if ($this->Settings->get('block_common_spam') && preg_match_all('~\[(link|url)=~', $params['Comment']->content, $m)) {	// Block common bbcode spam comments with both [url= and [link= tags
    ...
    public function remove_repetition($str = '')
    {
        if (($newstring = @preg_replace('~(.)\\1{3,}~u', '$1$1$1', $str)) === null) {	// Some error occured, just return the original string
            $newstring = $str;
        }
        return $newstring;
    }
    ...
    public function AfterInstall()
    {
        ...
        if ($Settings->get('hit_doublecheck_referer')) { // old general settings, "transform it"
    ...
    public function AppendHitLog(&$params)
    {
        ...
        if (function_exists('register_shutdown_function')) { // register it as a shutdown function, because it will be slow!
            $this->debug_log('AppendHitLog: loading referering page.. (through register_shutdown_function())');

            register_shutdown_function([&$this, 'double_check_referer'], $Hit->referer); // this will also call Hit::record_the_hit()
        } else {
     ...
     */
    public function double_check_referer($referer)
    {
        global $Hit, $ReqURI;
     ...
     * @return boolean
     */
    public function is_referer_linking_us($referer, $uri)
    {
        ...
        if (! $fp) { // could not access referring page
        ...
        while (($s = fgets($fp, 4096)) !== false) {
            $content_ref_page .= $s;
            $bytes_read += strlen($s);
            if ($bytes_read > 512000) { // do not pull more than 500kb of data!
                break;
            }
        }
        fclose($fp);
        ...
        $possible_hosts = [$_SERVER['HTTP_HOST']];
        if ($_SERVER['SERVER_NAME'] != $_SERVER['HTTP_HOST']) {
            $possible_hosts[] = $_SERVER['SERVER_NAME'];
        }
        $search_pattern_hosts = [];
        foreach ($possible_hosts as $l_host) {
            if (preg_match('~^([^.]+\.)(.*?)([^.]+\.[^.]+)$~', $l_host, $match)) { // we have subdomains in this hostname
                if (stristr($match[1], 'www')) { // search also for hostname without 'www.'
        ...
        foreach ($search_pattern_hosts as $l_host) { // add IDN, because this could be linked:
        ...
        if (empty($uri)) { // host(s) should end with "/", "'", '"', "?" or whitespace
            $search_pattern .= '[/"\'\s?]';
        } else {
        ...
        if ($have_idn_name) { // Convert charset to UTF-8, because the decoded domain name is UTF-8, too:
        ...
        if (preg_match($search_pattern, $content_ref_page)) {
            ...
            if (strpos($referer, $ReqHost) === 0 && ! empty($uri)) { // Referer is the same host.. just search for $uri
     ...
     * @param Comment
     */
    public function is_duplicate_comment($Comment)
    {
        global $DB;
        ...
        if ($Comment->content == '') { // User may has many comments with empty content but with attachment pictures
            return false;
        }
        ...
        if (isset($Comment->author_user_ID)) { // registered user:
            $sql .= ' AND comment_author_user_ID = ' . $Comment->author_user_ID;
        } else { // visitor (also trackback):
            $sql_ors = [];
