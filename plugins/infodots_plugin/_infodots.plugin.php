...
{
    public $code = 'b2evoDot';

    public $name = 'Info dots renderer';
    ...
    public $help_topic = 'infodots-plugin';
    ...
    public $loaded_objects = null;

    public $dot_numbers = null;
     ...
     */
    public function init_html_head()
    {
    ...
    {
        if (empty($params['Item'])) {	// This plugin can works only with items:
            return false;
        }
    ...
    public function FilterCommentContent(&$params)
    {
        ...
        if (in_array($this->code, $Comment->get_renderers_validated())) {	// If apply_comment_rendering is set to render:
     ...
     */
    public function render_infodot_captions($object_ID, $content)
    {
        $this->dot_numbers = null;
        $this->object_ID = $object_ID;

        $content = replace_outside_code_tags(
            '#((<br />|<p>)\r?\n?)?\[infodot:(\d+):(-?\d+[pxecm%]*):(-?\d+[pxecm%]*)(:\d+[pxecm%]*)?\](.+?)\[enddot\](\r?\n?(<br />|</p>))?#is',
            [$this, 'load_infodot_from_source'],
            $content,
            'replace_content_callback'
        );
     ...
     */
    public function load_infodot_from_source($matches, $only_load_dot = false)
    {
        $link_ID = intval($matches[3]);

        if (empty($link_ID) || empty($matches) || empty($this->object_ID)) {	// Skip this incorrect match
            return;
        }

        $LinkCache = &get_LinkCache();
        $Link = &$LinkCache->get_by_ID($link_ID, false, false);
        if (! $Link) {	// Inform about invalid Link ID
            return '<div style="color:#F00"><b>' . T_('Invalid Link ID') . ' - ' . $matches[0] . '</b></div>';
        }

        if ($this->dot_numbers === null) {	// Init dot numbers array first time
            $this->dot_numbers = [];
        }

        if (! isset($this->dot_numbers[$link_ID])) {	// Start to calculate number of the dots for current Link object
            $this->dot_numbers[$link_ID] = 1;
        }

        if (! isset($this->loaded_objects[$this->object_ID])) {	// Load dots only once
            if ($this->dots === null) {	// Init dots array first time
                $this->dots = [];
            }

            if (! isset($this->dots[$link_ID])) {	// Init sub array for each Link
                $this->dots[$link_ID] = [];
            }
     ...
     * @param array Matches
     */
    public function load_infodot_from_rendered_content($matches)
    {
     ...
     */
    public function render_infodots(&$params, $content)
    {
        if (empty($params['File']) || empty($params['Link'])) {	// Check input data
            return;
        }
        ...
        if (! $File->is_image()) {	// This plugin works only with image files
            return;
        }

        if (($LinkOwner = &$Link->get_LinkOwner()) === false || ($Collection = $Blog = &$LinkOwner->get_Blog()) === false) {	// Couldn't get Blog object
            return;
        }
        ...
        if ($File->get_image_size('width') < $this->get_coll_setting('coll_min_width', $Blog) ||
            $thumbnail_width < $this->get_coll_setting('coll_min_width', $Blog)) {	// Don't draw a dot on image if width is less than setting value
            return;
        }

        if (! isset($this->loaded_objects[$this->object_ID])) {	// Load the info dots if they were not loaded before:
            replace_outside_code_tags(
                '#<div class="infodots_info" id="infodot_(\d+)_(\d+)" (data-)?xy="(-?\d+[pxecm%]*):(-?\d+[pxecm%]*)"[^>]*>(.+?)</div>#is',
                [$this, 'load_infodot_from_rendered_content'],
                $content,
                'replace_content_callback'
            );
            $this->loaded_objects[$this->object_ID] = 1;
        ...
        if (empty($this->dots[$Link->ID])) {	// No dots for this Link
            return;
        }

        $before_image = '<div class="infodots_image">' . "\n";
        foreach ($this->dots[$Link->ID] as $d => $dot) {	// Init html element for each dot
    ...
    {
        if (empty($params['Item']) || empty($params['Link'])) {	// Wrong input data, Exit here:
            return false;
        }
     ...
     * @return boolean true if plugin rendered this attachment
     */
    public function PrepareForRenderCommentAttachment(&$params)
    {
        if (empty($params['Comment']) || empty($params['Link'])) {	// Wrong input data, Exit here:
            return false;
        }
