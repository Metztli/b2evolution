...
class calendar_plugin extends Plugin
{
    ...
    public $subgroup = 'navigation';

    public $widget_icon = 'calendar';
    ...
    public function get_widget_param_definitions($params)
    {
        ...
        $r = [
            ...
            'item_visibility' => [
                ...
                'field_lines' => true,
                'options' => [
                    ['public', T_('show public posts')],
                    ['all', T_('show all posts the current user is allowed to see')]],
                'defaultvalue' => 'all',
            ],
            'item_type' => [
    ...
    public function SkinTag(&$params)
    {
        ...
        if (! empty($params['title'])) {	// We want to display a title for the widget block:
            echo $params['block_title_start'];
            if ($params['title_link']) {	// Set block title as link to current collection:
                echo '<a href="' . $Blog->gen_blogurl() . '" rel="nofollow">' . $params['title'] . '</a>';
            } else {	// Display a block title as simple text:
                echo $params['title'];
            }
        ...
        if (empty($params['cat_IDs'])) {	// Use default categories filter:
            $filter_cat_array = ($Calendar->link_type == 'context') ? $cat_array : [];
        } else {	// Get categories filter from widget settings:
            $filter_cat_array = sanitize_id_list($params['cat_IDs'], true);
        }

        if ($Calendar->link_type == 'context') {	// We want to preserve the current context:
        ...
        } else {	// We want to preserve only the minimal context:
            // * - - Restrict to selected blog/categories:
        ...
        if (isset($params['item_visibility']) && $params['item_visibility'] == 'public') {	// Get only the public items:
            $visibility_array = ['published'];
        } else {	// Get the current selected status items:
        ...
        if (isset($params['item_type'])) {
            if ($params['item_type'] == '#') {	// Exclude pages and intros and sidebar stuff by default:
                $item_types_usage = 'post';
            } elseif ($params['item_type'] != 'all') {	// Filter by one selected item type:
                $item_types = $params['item_type'];
            }
        }
        if (isset($item_types_usage)) {	// Filter by item types usage:
            $Calendar->ItemQuery->where_itemtype_usage($item_types_usage);
        } else {	// Filter by item types:
...
class Calendar
{
    ...
    public $headerrowstart;

    public $headerrowend;

    public $headercellstart;

    public $headercellend;

    public $cellstart;

    public $cellend;

    public $emptycellstart;
     ...
     * @access protected
     */
    public $today_is_visible;

    public $link_type;

    public $context_isolation;
     ...
     *      - 'max_timestamp' : Maximum unix timestamp the user can browse too or 'query' (Default: now + 1 year )
     */
    public function __construct($m = '', $params = [])
    {
        global $localtimenow;
        ...
        if (isset($params['itemlist_prefix'])) { // Set a prefix of the ItemList object
        ...
        if (empty($m)) { // Current month (monthly)
            $this->year = $localyearnow;
            $this->month = $localmonthnow;
            $this->mode = 'month';

            $this->today_is_visible = true;
        } else {	// We have requested a specific date
            $this->year = substr($m, 0, 4);
            if (strlen($m) < 6) { // no month provided
                $this->mode = 'year';

                if ($this->year == $localyearnow) { // we display current year, month gets current
                    $this->month = $localmonthnow;
                } else { // highlight no month, when not current year
                    $this->month = '';
                }
            } else {
        ...
        // Default values:
        if (empty($this->params['min_timestamp'])) {	// 2000-01-01:
            $this->params['min_timestamp'] = mktime(0, 0, 0, 1, 1, 2000);
        }
        if (empty($this->params['max_timestamp'])) {	// Now + 1 year:
    ...
    public function display()
    {
        ...
        if ($this->mode == 'month') {
            ...
            if ($DB->num_rows > 0) { // OK we have a month with posts! // fp>dh why did you removed that?
                foreach ($arc_result as $arc_row) {
        ...
        if ($this->displaycaption) { // caption:
            ...
            if ($this->mode == 'month') { // MONTH CAPTION:
                $text = date_i18n($this->monthformat, mktime(0, 0, 0, $this->month, 1, $this->year));

                if ($this->linktomontharchive) { // chosen month with link to archives
        ...
        if (! empty($this->headerdisplay) && ($this->mode == 'month')) { // Weekdays:
        ...
        if ($this->mode == 'year') {	// DISPLAY MONTHS:
            for ($i = 1; $i < 13; $i = $i + 1) {	// For each month:
                if (isset($monthswithposts[$i])) {
                    ...
                    if ($monthswithposts[$i] > 1 && ! empty($this->postcount_year_atitle)) { // display postcount
                        $title = sprintf($this->postcount_year_atitle, $monthswithposts[$i]);
                    } elseif (! empty($this->postcount_year_atitle_one)) { // display postcount for one post
                        $title = sprintf($this->postcount_year_atitle_one, 1);
                    } else {
                        $title = '';
                    }
                    echo $this->archive_link(T_($month_abbrev[zeroise($i, 2)]), $title, $this->year, $i);
                } elseif ($this->month == $i) { // current month
            ...
            for ($i = $calendarfirst; $i <= $calendarlast; $i = $i + 86400) { // loop day by day (86400 seconds = 24 hours; but not on days where daylight saving changes!)
                if ($dow == 7) { // We need to start a new row:
                    if ($dom_displayed >= $daysinmonth) { // Last day already displayed!
                        break;
                    }
                    echo $this->rowend;
                    echo $this->rowstart;
                ...
                if (date('m', $i) != $this->month) { // empty cell
                    ...
                    if (isset($daysinmonthwithposts[date('j', $i)])) {
                        ...
                        if ($daysinmonthwithposts[date('j', $i)] > 1 && ! empty($this->postcount_month_atitle)) { // display postcount
                            $title = sprintf($this->postcount_month_atitle, $daysinmonthwithposts[date('j', $i)]);
                        } elseif (! empty($this->postcount_month_atitle_one)) { // display postcount for one post
        ...
        if ($this->navigation == 'tfoot') { // We want to display navigation in the table footer:
            echo "<tfoot>\n";
            echo "<tr>\n";
     ...
     * @param string day
     */
    public function archive_link($text, $title, $year, $month = null, $day = null)
    {
        /**
         * @var Blog
         */
        ...
        if ($this->link_type == 'context') {	// We want to preserve context:
     ...
     * @return array
     */
    public function getNavLinks($direction)
    {
        ...
        if ($this->params['min_timestamp'] == 'query' || $this->params['max_timestamp'] == 'query') { // Do inits:
        ...
        switch ($direction) {
            case 'prev':
                ...
                if (empty($this->month)) { // if $this->month is empty, we're in mode "year" with no selected month
                    $use_range_month = 12;
                    $use_range_day = 31;
                } else {
                 ...
                 * << (PREV YEAR)
                 */
                if ($this->browseyears) {	// We want arrows to move one year at a time
                    if ($this->params['min_timestamp'] == 'query') {	// Let's query to find the correct year:
                        if ($row = $DB->get_row(
                            ...
                            OBJECT,
                            0,
                            'Calendar: find prev year with posts'
                        )
                        ) {
                            $prev_year_year = $row->year;
                            $prev_year_month = $row->month;
                        }
                    } else { // Let's see if the previous year is in the desired navigation range:
                ...
                if (! empty($prev_year_year)) {	// We have a link to display:
                    $r[] = $this->archive_link('&lt;&lt;', sprintf(
                        ...
                        $prev_year_year,
                        $prev_year_month
                    ), $prev_year_year, ($this->mode == 'month') ? $prev_year_month : null);
                }

                /*
                 * < (PREV MONTH)
                 */
                if ($this->mode == 'month') { // We are browsing months, we'll display arrows to move one month at a time:
                    if ($this->params['min_timestamp'] == 'query') {	// Let's query to find the correct month:
                        if ($row = $DB->get_row(
                    ...
                    } else { // Let's see if the previous month is in the desired navigation range:
                ...
                if (! empty($prev_month_year)) {	// We have a link to display:
            ...
            case 'next':
                 ...
                 */
                if ($this->mode == 'month') { // We are browsing months, we'll display arrows to move one month at a time:
                    if ($this->params['max_timestamp'] == 'query') {	// Let's query to find the correct month:
                        if ($row = $DB->get_row(
                            'SELECT EXTRACT(MONTH FROM ' . $this->dbprefix . 'datestart) AS month,
												EXTRACT(YEAR FROM ' . $this->dbprefix . 'datestart) AS year
								FROM (' . $this->dbtable . ' INNER JOIN T_postcats ON ' . $this->dbIDname . ' = postcat_post_ID)
                    ...
                    } else { // Let's see if the next month is in the desired navigation range:
                ...
                if (! empty($next_month_year)) {	// We have a link to display:
                 ...
                 */
                if ($this->browseyears) { // We want arrows to move one year at a time
                    if ($this->params['max_timestamp'] == 'query') {	// Let's query to find the correct year:
                        if ($row = $DB->get_row(
                            'SELECT EXTRACT(YEAR FROM ' . $this->dbprefix . 'datestart) AS year,
											EXTRACT(MONTH FROM ' . $this->dbprefix . 'datestart) AS month
								FROM (' . $this->dbtable . ' INNER JOIN T_postcats ON ' . $this->dbIDname . ' = postcat_post_ID)
                            ...
                            OBJECT,
                            0,
                            'Calendar: find next year with posts'
                        )
                        ) {
                            $next_year_year = $row->year;
                            $next_year_month = $row->month;
                        }
                    } else { // Let's see if the next year is in the desired navigation range:
                    ...
                    if (! empty($next_year_year)) {	// We have a link to display:
                        $r[] = $this->archive_link('&gt;&gt;', sprintf(
                            ($this->mode == 'month'
                            ...
                            $next_year_year,
                            $next_year_month
                        ), $next_year_year, ($this->mode == 'month') ? $next_year_month : null);
                    }
                }
                break;
        }
