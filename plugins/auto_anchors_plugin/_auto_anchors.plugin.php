...
{
    public $code = 'auto_anchors';

    public $name = 'Auto Anchors';
    ...
    public $help_topic = 'auto-anchors-plugin';
    ...
    public function get_shared_setting_definitions(&$params)
    {
        ...
        $default_params = array_merge($params, [
            'default_shared_rendering' => 'opt-in',
        ]);
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        $content = replace_outside_code_tags('#(<h([1-6])([^>]*\sid\s*=\s*["\']([^"\']+)["\'])?[^>]*)>(.+?)(</h\2>)#i', [$this, 'callback_auto_anchor'], $content, 'replace_content_callback');
     ...
     * @return string
     */
    public function callback_auto_anchor($m)
    {
        if (empty($m[4])) {	// Generate anchor from header text:
        ...
        } else {	// Use custom defined anchor:
            $anchor = $m[4];
            $anchor_attribute = '';
        }

        if (empty($anchor)) {	// Return original header tag when anchor is empty:
            return $m[0];
        }

        $header_tag_start = $m[1];
        if (strpos($header_tag_start, ' class="') !== false) {	// Append style class to current:
            $header_tag_start = str_replace(' class="', ' class="evo_auto_anchor_header ', $header_tag_start);
        } else {	// Add new class attribute:
