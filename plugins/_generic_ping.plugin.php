...
class generic_ping_plugin extends Plugin
{
    ...
    public function PluginInit(&$params)
    {
        ...
        if ($params['is_installed']) { // is not set for not-installed Plugins
     ...
     */
    public function parse_ping_url($url)
    {
        if (empty($url)) {	// Wrong URL is provided:
            return false;
        }

        if (! preg_match('#^(https?:)?//#', $url)) {	// Prepend protocol "http" automatically:
            $url = 'http://' . $url;
        }

        if (! ($url_data = parse_url($url)) || empty($url_data['host'])) {	// Wrong URL is provided:
            return false;
        }
    ...
    public function ItemSendPing(&$params)
    {
        ...
        if (! $url) {	// Wrong URL of the plugin settings:
