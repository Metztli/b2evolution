...
class archives_plugin extends Plugin
{
    ...
    public function SkinTag(&$params)
    {
        ...
        if (! isset($params['sort_order'])) {	// Set default sort order:
        ...
        while ($ArchiveList->get_item($arc_year, $arc_month, $arc_dayofmonth, $arc_w, $arc_count, $post_ID, $post_title, $permalink)) {
            ...
            switch ($params['mode']) {
                case 'monthly':
                    ...
                    if ($params['form']) { // We want a radio button:
                    ...
                    if ($params['link_type'] == 'context') {	// We want to preserve current browsing context:
                        echo '<a rel="nofollow" href="' . regenerate_url($params['context_isolation'], $itemlist_prefix . 'm=' . $arc_m) . '">' . $text . '</a>';
                    } else {	// We want to link to the absolute canonical URL for this archive:
...
class ArchiveList extends Results
{
    ...
    public function __construct(
        ...
        if (is_admin_page()) {	// Don't restrict by date in the Back-office
            $timestamp_min = null;
            $timestamp_max = null;
        } else {	// Restrict posts by date started
            $timestamp_min = $Blog->get_timestamp_min();
            $timestamp_max = $Blog->get_timestamp_max();
        }

        if ($preserve_context) {	// We want to preserve the current context:
            ...
            // * - - Restrict to selected blog/categories:
            $this->ItemQuery->where_chapter($blog, '', []);
        ...
        switch ($this->archive_mode) {
            ...
            default:
                ...
                $archives_list->set_filters([
                    'visibility_array' => ['published'],  // We only want to advertised published items
        ...
        if (in_array($this->archive_mode, ['monthly', 'daily', 'weekly'])) {
     ...
     */
    public function get_item(&$arc_year, &$arc_month, &$arc_dayofmonth, &$arc_w, &$arc_count, &$post_ID, &$post_title, &$permalink)
    {
        // echo 'getting next item<br />';

        if ($this->current_idx >= $this->result_num_rows) {	// No more entry
            return false;
        }
