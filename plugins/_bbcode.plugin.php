...
class bbcode_plugin extends custom_tags_plugin
{
    ...
    public $help_topic = 'bb-code-plugin';
    ...
    public $default_search_list = '[b] #\[b](.+?)\[/b]#is
[i] #\[i](.+?)\[/i]#is
[s] #\[s](.+?)\[/s]#is
...
Supported tags by default are: [b] [i] [s] [color=...] [size=...] [font=...] [quote] [list=1] [list=a] [list] [*] [bg=] [clear]');
     ...
     */
    public function parse_bbcode($content, $search_list, $replace_list)
    {
        if (empty($content)) { // No content, Exit here
            return $content;
        }
        ...
        foreach ($search_list as $i => $search_text) { // Split the masks to separate things
        ...
        foreach ($simple as $tag_info) { // Make simple replacements
        ...
        while ($c_pos < $content_length - 2) {
            if ($content[$c_pos] . $content[$c_pos + 1] == '[/') { // Closed tag
                if (! empty($opened_tags)) {	// Get closed tag that was opened as last
            ...
            } elseif ($content[$c_pos] == '[') { // Opened tag
                $sub_content = substr($content, $c_pos);
     ...
     * @return string Content
     */
    public function parse_anchor_links($content)
    {
        if (preg_match_all('/ href="#(c|p)([0-9]+)"/i', $content, $matches)) {
            ...
            foreach ($matches[0] as $m => $full_match) {
                ...
                $new_url = '';
                switch ($matches[1][$m]) { // Object type:
                    case 'p':
                        // Item
                        if ($Item = &$ItemCache->get_by_ID($object_ID, false, false)) { // Replace anchor url with item permanent url
                    ...
                    case 'c':
                        // Comment
                        if ($Comment = &$CommentCache->get_by_ID($object_ID, false, false)) { // Replace anchor url with comment permanent url
                ...
                if (! empty($new_url)) { // Replace relative anchor url with new absolute url
    ...
    public function FilterCommentContent(&$params)
    {
        ...
        if (in_array($this->code, $Comment->get_renderers_validated())) { // apply_comment_rendering is set to always render
            $content = &$params['data'];
            if (! isset($this->comment_search_list)) {	// Init comment search list only first time:
                $this->comment_search_list = $this->prepare_search_list($this->get_coll_setting('coll_comment_search_list', $item_Blog));
            }

            if (! isset($this->comment_replace_list)) {	// Init comment replace list only first time:
                $this->comment_replace_list = $this->prepare_replace_list($this->get_coll_setting('coll_comment_replace_list', $item_Blog));
            }

            $content = replace_outside_code_and_short_tags($this->comment_search_list, $this->comment_replace_list, $content, [$this, 'parse_bbcode']);
        }
    }
