...
{
    public $code = 'h_levels';

    public $name = 'Adjust headings';
    ...
    public $help_topic = 'adjust-headings-plugin';
     ...
     * Internal vars:
     */
    public $highest_level = null;

    public $setting_level = null;
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        if (! $setting_Blog) {	// Unknown call, Don't render this case:
            return;
        }
    ...
    public function FilterCommentContent(&$params)
    {
        ...
        if (in_array($this->code, $Comment->get_renderers_validated())) { // apply_comment_rendering is set to render
            $content = &$params['data'];
     ...
     */
    public function do_adjust_headings($content)
    {
        $this->highest_level = null;

        // Find the highest heading level in the current content:
        replace_outside_code_tags('#</h([1-6])>#i', [$this, 'callback_find_highest_heading'], $content, 'replace_content_callback');

        if (is_null($this->highest_level) || $this->highest_level == $this->setting_level) { // The html heading tags have not been detected in the content
        ...
        $content = replace_outside_code_tags('#<(/?)h([1-6])([^>]*)?>#i', [$this, 'callback_adjust_headings'], $content, 'replace_content_callback');
     ...
     * @param array Matches
     */
    public function callback_find_highest_heading($matches)
    {
        if (empty($matches)) { // Nothing to do
            return;
        }

        if ($this->highest_level == 1) { // The level is already the highest
            return;
        }

        $h_level = intval($matches[1]);

        if ($h_level > 0 && (is_null($this->highest_level) || $this->highest_level > $h_level)) { // Update the var to the highest level in the content:
     ...
     * @return string
     */
    public function callback_adjust_headings($matches)
    {
        if (empty($matches)) { // Nothing to do
            return;
        }
        ...
        if (empty($h_level)) { // Wrong heading level value, Return original html tag:
            return $matches[0];
        } else { // Adjust current heading html tag:
            $h_level += $this->setting_level - $this->highest_level;

            if ($h_level > 6) { // Limit with max 6 level because of HTML:
                $h_level = 6;
            }
