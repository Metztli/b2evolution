...
{
    public $code = 'b2evWiLi';

    public $name = 'Short Links';
    ...
    public $help_topic = 'wiki-links-plugin';
        ...
        return [
            'link_types' => [
                ...
                'options' => [
                    ...
                    ['abs_url_target_blank',  $this->T_('Open in new tab') . ' (<code>target="_blank"</code>)', 1, null, null, null, null, null, [
                        'style' => 'margin-left:20px',
                    ]],
                    ['abs_url_optimize',      $this->T_('If an absolute URL references a collection on this system, try to optimize and keep just the slug '), 1, null, null, null, null, null, [
                        'style' => 'margin-left:20px',
                    ]],
                    ['relative_urls',         sprintf($this->T_('Relative URLs (starting with %s followed by a letter or digit) in brackets'), '<code>/</code>'), 0],
                    ['rel_url_optimize',      $this->T_('If a relative URL references a collection on this system, try to optimize and keep just the slug'), 1, null, null, null, null, null, [
                        'style' => 'margin-left:20px',
                    ]],
    ...
    public function get_email_setting_definitions(&$params)
    {
        ...
        $default_params = array_merge($params, [
            'default_email_rendering' => 'opt-out',
        ]);
    ...
    public function get_shared_setting_definitions(&$params)
    {
        ...
        $default_params = array_merge($params, [
            'default_shared_rendering' => 'opt-out',
        ]);
    ...
    public function DisplayItemAsHtml(&$params)
    {
        ...
        $content = replace_outside_code_tags('#<a[^>]+href="([^"]+)"[^>]+data-function="create_post" data-coll="(\d+)"[^>]*>(.+?)</a>#i', [$this, 'callback_replace_post_links'], $content, 'replace_content_callback');
    ...
    public function render_content(&$content)
    {
        ...
        if (! empty($this->link_types['absolute_urls'])) {	// If it is allowed by plugin setting
            $search_urls = '*
				( \[\[ | \(\( )                    # Lookbehind for (( or [[
				( (https?://|mailto://|//)[^<>{}\s\]]+ ) # URL
            ...
            $content = replace_outside_code_and_short_tags($search_urls, [$this, 'callback_replace_bracketed_urls'], $content, 'replace_content', 'preg_callback');
        }

        // -------- RELATIVE BRACKETED URLS -------- :
        if (! empty($this->link_types['relative_urls'])) {	// If it is allowed by plugin setting
            $search_urls = '*
				( \[\[ | \(\( )                    # Lookbehind for (( or [[
				( (/)[^/][^<>{}\s\]]+ ) # URL
				( \s \.[a-z0-9_\-\.]+ )?           # Style classes started and separated with dot (Optional)
        ...
        if (! empty($this->link_types['cat_without_brackets']) ||
            ! empty($this->link_types['item_without_brackets'])) {	// Create the links from standalone WikiWords
            $search_wikiwords = [];
            $replace_links = [];
            ...
            if (preg_match_all($search, $content, $matches, PREG_SET_ORDER)) {
                ...
                foreach ($wikiwords as $WikiWord => $wiki_word) {
                    ...
                    if (! empty($this->link_types['item_without_brackets']) &&
                        ($Item = &$ItemCache->get_by_urltitle($wiki_word, false, false))) {	// Replace WikiWord with post permanent link if item is found:
                        $replace_links[] = '<a href="' . $Item->get_permanent_url() . '">' . $Item->get('title') . '</a>';
                    } elseif (! empty($this->link_types['cat_without_brackets']) &&
                            ($Chapter = &$ChapterCache->get_by_urlname($wiki_word, false, false))) {	// Replace WikiWord with category permanent link if Chapter is found:
                        $replace_links[] = '<a href="' . $Chapter->get_permanent_url() . '">' . $Chapter->get('name') . '</a>';
                    } else {	// Replace WikiWord with broken link if Item and Chapter are not found:
        ...
        if (! empty($this->link_types['anchor']) ||
            ...
            ! empty($this->link_types['item_id'])) {	// If it is allowed by plugin settings:
            ...
            if (preg_match_all($search, $content, $matches, PREG_SET_ORDER)) {
                ...
                foreach ($matches as $match) {
                    // Convert the WikiWord to an urltitle
                    $WikiWord = $match[0];
                    if (preg_match('/^[\p{Ll}0-9#_\-]+$/' . $regexp_modifier, $WikiWord)) {	// This WikiWord already matches a slug format
                        $Wiki_Word = $WikiWord;
                        $wiki_word = $Wiki_Word;
                    } else {	// Convert WikiWord to slug format
                        $Wiki_Word = preg_replace(['*([^\p{Lu}#_])([\p{Lu}#])*' . $regexp_modifier, '*([^0-9])([0-9])*' . $regexp_modifier], '$1-$2', $WikiWord);
                        $wiki_word = utf8_strtolower($Wiki_Word);
                    }
                ...
                foreach ($wikiwords as $WikiWord => $wiki_word) {
                    ...
                    $content = replace_outside_code_and_short_tags($search_wikiword, [$this, 'callback_replace_bracketed_words'], $content, 'replace_content', 'preg_callback');
                }
            }
        }
     ...
     */
    public function callback_replace_bracketed_urls($m)
    {
        if (! ($m[1] == '[[' && substr($m[7], 0, 2) == ']]') &&
            ! ($m[1] == '((' && substr($m[7], 0, 2) == '))')) {	// Wrong pattern, Return original text:
            return $m[0];
        }

        if (strlen($m[7]) > 2) {	// Fix Custom link text with appending chars ] or ) if it is ended with chars ] or ):
            $m[6] .= substr($m[7], 0, -2);
        }
        ...
        if ($m[3] != '/' && ! empty($this->link_types['abs_url_target_blank'])) {	// Force target to "_blank" for absolute URLs when it is defined in plugin settings:
            $custom_link_target = '_blank';
        } else {	// Use custom link target:
     ...
     */
    public function callback_replace_bracketed_words($m)
    {
        global $blog, $evo_charset, $admin_url;

        if (! ($m[1] == '[[' && substr($m[5], 0, 2) == ']]') &&
            ! ($m[1] == '((' && substr($m[5], 0, 2) == '))')) {	// Wrong pattern, Return original text:
            return $m[0];
        }

        if (strlen($m[5]) > 2) {	// Fix Custom link text with appending chars ] or ) if it is ended with chars ] or ):
            $m[4] .= substr($m[5], 0, -2);
        }
        ...
        if (isset($WikiWord_match[3])) {	// wiki word has attr "id"
        ...
        $permalink = '';
        $link_text = preg_replace(['*([^\p{Lu}_])([\p{Lu}])*' . $regexp_modifier, '*([^0-9])([0-9])*' . $regexp_modifier], '$1 $2', $WikiWord);
        $link_text = ucwords(str_replace('-', ' ', $link_text));

        if (! empty($this->link_types['item_id']) && is_numeric($this->current_wiki_word) && ($Item = &$ItemCache->get_by_ID($this->current_wiki_word, false, false))) {	// Item is found
            $permalink = $Item->get_permanent_url();
            $existing_link_text = $Item->get('title');
        } elseif (! empty($this->link_types['cat_slugs']) && $Chapter = &$ChapterCache->get_by_urlname($this->current_wiki_word, false, false)) {	// Chapter is found
            $permalink = $Chapter->get_permanent_url();
            $existing_link_text = $Chapter->get('name');
        } elseif (! empty($this->link_types['item_slugs']) && $Item = &$ItemCache->get_by_urltitle($this->current_wiki_word, false, false)) {	// Item is found
            $permalink = $Item->get_permanent_url();
            $existing_link_text = $Item->get('title');
        } elseif (! empty($this->link_types['anchor']) && isset($anchor) && ! empty($this->current_Item)) {	// Use Item's URL with anchor:
            if ($this->current_Item->get_permalink_type() != 'none') {	// Use full permanent URL like 'http://site.com/item-slug#anchor' only for normal Item that have separate single of intro page,
        ...
        if (! empty($permalink)) {	// Chapter or Item are found in DB
            ...
            if (! empty($custom_link_text)) {	// [[WikiWord custom link text]] or ((WikiWord custom link text)) or [[WikiWord .style.classes custom link text]] or ((WikiWord .style.classes custom link text))
                return '<a href="' . $permalink . $url_params . '"' . $custom_link_class . $custom_link_target . '>' . $custom_link_text . '</a>';
            } elseif ($m[1] == '[[') {	// [[Wikiword]] or [[Wikiword .style.classes]]
                return '<a href="' . $permalink . $url_params . '"' . $custom_link_class . $custom_link_target . '>' . $existing_link_text . '</a>';
            } else {	// ((Wikiword)) or ((Wikiword .style.classes))
                return '<a href="' . $permalink . $url_params . '"' . $custom_link_class . $custom_link_target . '>' . $link_text . '</a>';
            }
        } else {	// Chapter and Item are not found in DB
            if ((empty($this->link_types['item_id']) && is_numeric($this->current_wiki_word)) ||
                (empty($this->link_types['anchor']) && isset($anchor))) {	// Return original text if no found by numeric wikiword and "Item ID in brackets" is disabled:
                return $m[0];
            } else {	// Display a link to suggest to create new post from wiki word:
     ...
     * @return string
     */
    public function get_broken_link($post_slug, $text, $class = '')
    {
        global $blog, $admin_url, $evo_charset;

        if (isset($this->render_type) && $this->render_type == 'email') {	// Don't render broken link for Email Campaign because it is impossible
        ...
        if (is_numeric($post_slug) && ! is_numeric($text)) {	// Try to use custom text if it is provided instead of post ID to suggest a link to create new post:
            $post_slug = preg_replace(['*([^\p{Lu}#_])([\p{Lu}#])*' . $regexp_modifier, '*([^0-9])([0-9])*' . $regexp_modifier], '$1-$2', utf8_strtolower($text));
        }

        if (isset($blog) && ! is_numeric($post_slug)) {	// Suggest to create new post from given word:
            $before_wikiword = '<a'
                . ' href="#"'
            ...
            $after_wikiword = '</a>';
        } else {	// Don't allow to create new post from numeric wiki word:
     ...
     * @return string
     */
    public function callback_replace_post_links($matches)
    {
        if (! isset($matches[1], $matches[2], $matches[3])) {	// Return a source string when no enough data to check user permissions:
            return $matches[0];
        }
        ...
        if (! $new_post_url) {	// If user has no permission to create a post for the collection,
    ...
    public function DisplayMessageToolbar(&$params)
    {
        ...
        if (! empty($apply_rendering) && $apply_rendering != 'never') {	// Print toolbar on screen:
     ...
     * @param object Blog
     */
    public function DisplayCodeToolbar($Blog = null, $params = [])
    {
     ...
     */
    public function init_js_lang_vars($relative_to = 'rsc_url')
    {
    ...
    public function PrependItemInsertTransact(&$params)
    {
        ...
        if (! $this->is_renderer_enabled($this->get_coll_setting('coll_apply_rendering', $setting_Blog), $Item->get_renderers_validated())) {	// Don't try to optimize when this plugin is not applied for Items:
            return;
        }
    ...
    public function PrependCommentInsertTransact(&$params)
    {
        ...
        if (! $this->is_renderer_enabled($this->get_coll_setting('coll_apply_comment_rendering', $setting_Blog), $Comment->get_renderers_validated())) {	// Don't try to optimize when this plugin is not applied for Comments:
            return;
        }
    ...
    public function PrependMessageInsertTransact(&$params)
    {
        ...
        if (! $this->is_renderer_enabled($this->get_msg_setting('msg_apply_rendering'), $Message->get_renderers_validated())) {	// Don't try to optimize when this plugin is not applied for Items:
            return;
        }
    ...
    public function PrependEmailInsertTransact(&$params)
    {
        ...
        if (! $this->is_renderer_enabled($this->get_email_setting('email_apply_rendering'), $EmailCampaign->get_renderers_validated())) {	// Don't try to optimize when this plugin is not applied for Items:
            return;
        }
     ...
     */
    public function optimize_urls($content)
    {
        if (! empty($this->link_types['abs_url_optimize'])) {	// Optimize absolute URLs:
            $content = replace_outside_code_and_short_tags('*
					...
					( \]{2,} | \){2,} ) # Lookahead for )) or ]]
					*ix', // x = extended (spaces + comments allowed)
                [$this, 'optimize_urls_callback'], $content, 'replace_content', 'preg_callback');
        }

        if (! empty($this->link_types['rel_url_optimize'])) {	// Optimize relative URLs:
            $content = replace_outside_code_and_short_tags('*
					( \[\[ | \(\( ) # Lookbehind for (( or [[
					( ( /(.+/)? ) ( [^/][^<>{}\s\]\)]+ ) ) # URL
					( \s.+ )?       # Additional attributes like style classes, link target, custon link text (Optional)
     ...
     * @return string
     */
    public function optimize_urls_callback($m)
    {
        if (! ($m[1] == '[[' && substr($m[7], 0, 2) == ']]') &&
            ! ($m[1] == '((' && substr($m[7], 0, 2) == '))')) {	// Wrong pattern, Return original text:
            return $m[0];
        ...
        if (preg_match('#^(https?://|//)$#', $m[4]) &&
            ! is_internal_url($m[3])) {	// This is an absolute URL but this is an external URLs,
        ...
        if ($anchor_position !== false) {	// Remove anchor from slug:
            $slug = substr($slug, 0, $anchor_position);
        }
        $SlugCache = &get_SlugCache();
        if (! $SlugCache->get_by_name($slug, false, false)) {	// The Slug is not found in system, Keep it as is without optimization:
            return $m[0];
        }
