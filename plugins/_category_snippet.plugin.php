...
class category_snippet_plugin extends Plugin
{
    ...
    public function get_widget_param_definitions($params)
    {
        ...
        return [
            ...
            'snippets' => [
                'label' => T_('Snippets'),
                'note' => sprintf(
                    T_('Please note snippets defined here override snippets with same category defined in <a %s>current collection plugin settings</a>.'),
     ...
     * @return array
     */
    public function get_categories_selector_options()
    {
        global $Blog;
        ...
        if (get_allow_cross_posting()) {	// Get ALL Categories from ALL Collections because cross-posting is enabled in system:
        ...
        } else {	// Get Categories of the current Collection:
    ...
    public function SkinTag(&$params)
    {
        ...
        if (empty($item_cat_IDs)) {	// Don't display this widget when current Item has no categories:
        ...
        if (empty($cat_snippets)) {	// Don't display this widget when no snippets are defined for categories of the current Item::
