...
{
    public $code = 'b2evH5MP';

    public $name = 'HTML 5 MediaElement.js Video and Audio Player';
    ...
    public $allow_ext = ['flv', 'm4v', 'f4v', 'mp4', 'ogv', 'webm', 'mp3', 'm4a'];
...
.mediajs_block .mediajs_text {
     ...
     */
    public function is_file_supported($File)
    {
    ...
    public function RenderItemAttachment(&$params, $in_comments = false)
    {
        ...
        if (! $this->is_file_supported($File)) {	// This file is not supported by plugin, Exit here:
            return false;
        }
        ...
        if ($File->exists()) {
            if (! $File->is_audio() && $placeholder_File = &$Item->get_placeholder_File($File)) {	// Get placeholder/poster when image file is linked to the Item with same name as current video File:
     ...
     * @return boolean true if plugin rendered this attachment
     */
    public function RenderURL(&$params)
    {
        global $Collection, $Blog;

        if (empty($params['url']) || ! $this->is_url_supported($params['url'])) {	// This file is not supported by plugin, Exit here:
            return false;
        }

        $player_Blog = null;
        if (! empty($params['Item'])) {	// Get collection of the Item;
     ...
     */
    public function get_player($params = [])
    {
        $params = array_merge([
            'before' => '',
            'after' => '',
        ...
        if ($params['file_type'] == 'audio') {	// Audio file:
        ...
        if (! empty($params['file_caption'])) {	// Display caption:
            $r .= '<div class="mediajs_text">' . $params['file_caption'] . '</div>';
        }

        if ($params['Blog'] && $this->get_coll_setting('allow_download', $params['Blog'])) {	// Allow to download the files:
    ...
    public function require_skin()
    {
        ...
        if (! empty($skin) && $skin != 'default') {
            $skins_path = dirname($this->classfile_path) . '/skins';
            if (file_exists($skins_path . '/' . $skin . '/style.css')) {	// Require css file only if it exists
     ...
     */
    public function get_file_mimetype($file_url)
    {
        if (preg_match('#\.([a-z0-9]+)(\?.+)?$#', $file_url, $match)) {	// Get file extenssion from url string:
            $file_extenssion = strtolower($match[1]);
        }

        if (empty($file_extenssion)) {	// Use this mime-type by default on unknown extenssion:
            return 'video/mp4';
        }
