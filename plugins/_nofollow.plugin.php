...
{
    public $code = 'evo_nofollow';

    public $name = 'Tag external links';
    ...
    public $help_topic = 'tag-external-links-plugin';
    ...
    public function get_coll_setting_definitions(&$params)
    {
        ...
        if (! empty($params['blog_type'])) {	// Set the default settings depends on collection type:
        ...
        return array_merge(
            ...
            [
                'abs_links_posts' => [
                    ...
                    'type' => 'checklist',
                    'options' => [
                        ['nofollow', sprintf($this->T_('Add %s'), '<code>rel="nofollow"</code>'), $default_values['abs_links_posts_nofollow']],
                        ['ugc', sprintf($this->T_('Add %s'), '<code>rel="ugc"</code>'), $default_values['abs_links_posts_ugc']],
                        ['sponsored', sprintf($this->T_('Add %s'), '<code>rel="sponsored"</code>'), $default_values['abs_links_posts_sponsored']],
                        ['blank', sprintf($this->T_('Add %s'), '<code>target="_blank"</code>'), $default_values['abs_links_posts_blank']],
                    ],
                ],
                'abs_links_comments' => [
                    ...
                    'type' => 'checklist',
                    'options' => [
                        ['nofollow', sprintf($this->T_('Add %s'), '<code>rel="nofollow"</code>'), $default_values['abs_links_comments_nofollow']],
                        ['ugc', sprintf($this->T_('Add %s'), '<code>rel="ugc"</code>'), $default_values['abs_links_comments_ugc']],
                        ['sponsored', sprintf($this->T_('Add %s'), '<code>rel="sponsored"</code>'), $default_values['abs_links_comments_sponsored']],
                        ['blank', sprintf($this->T_('Add %s'), '<code>target="_blank"</code>'), $default_values['abs_links_comments_blank']],
                    ],
                ],
            ]
        );
    ...
    {
        $default_params = array_merge($params, [
            'default_shared_rendering' => 'never',
        ]);
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        $this->setting_target_options = [];
        if ($this->get_checklist_setting($setting_name, 'blank', 'coll', $current_Blog)) {
            $this->setting_target_options[] = 'blank';
        }

        if (! empty($this->setting_rel_options) || ! empty($this->setting_target_options)) {	// Try to find links only of at least one rel option should be added:
            $content = replace_outside_code_tags('#<a([^>]+href="https?://[^"]+"[^>]*)>(.+?)</a>#i', [$this, 'callback_render_content'], $content, 'replace_content_callback');
        }
     ...
     * @param array Matches
     */
    public function callback_render_content($link_match)
    {
        $link_attrs = $link_match[1];

        if (! empty($this->setting_rel_options)) {	// Add attribute "rel":
            if (preg_match('# rel="([^"]*)"#i', $link_attrs, $rel_match)) {	// If link already has attrbiute "rel":
                $rel_options = trim($rel_match[1]);
                $rel_options = ($rel_options === '' ? [] : explode(' ', trim($rel_match[1])));
                $rel_options_exist = true;
            } else {	// If link has no attrbiute "rel" yet:
                $rel_options = [];
                $rel_options_exist = false;
            }

            foreach ($this->setting_rel_options as $setting_rel_option) {
                if (! in_array($setting_rel_option, $rel_options)) {	// Add only new option to avoid duplicates:
                    $rel_options[] = $setting_rel_option;
                }
            }

            if ($rel_options_exist) {	// Update attribute "rel" with added options:
                $link_attrs = preg_replace('# rel="([^"]*)"#i', ' rel="' . implode(' ', $rel_options) . '"', $link_attrs);
            } else {	// Add attribute "rel":
                $link_attrs .= ' rel="' . implode(' ', $rel_options) . '"';
            }
        }

        if (! empty($this->setting_target_options) &&
            in_array('blank', $this->setting_target_options)) {	// Add attribute "target" only if it is not provided in the link:
            if (preg_match('# target="([^"]*)"#i', $link_attrs, $target_match)) {	// If link already has attrbiute "target":
                $target_option = trim($target_match[1]);
                $target_options_exist = true;
            } else {	// If link has no attrbiute "target" yet:
                $target_option = '';
                $target_options_exist = false;
            }

            if ($target_option === '') {	// Add new option to target only if this attribute is not defined or value is empty string:
                // Do NOT rewrite already defined attributes like `target="_parent"`, `target="_self"`, etc.!
                if ($target_options_exist) {	// Update attribute "target" with added options:
