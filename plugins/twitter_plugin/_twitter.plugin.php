...
class twitter_plugin extends Plugin
{
     ...
     */
    public $code = 'evo_twitter';

    public $priority = 50;

    public $version = '7.2.5';

    public $author = 'b2evolution Group';
    ...
    public $message_length_limit = 140; // The maximum allowed number of characters in a message
    ...
    {
        if (! extension_loaded('curl')) { // the plugin is not supported
    ...
    {
        $info = null;
        if (isset($params['user_ID'])) { // initialize info only once, when needs to display the link (user_ID is set)
    ...
    {
        $info = null;
        if (isset($params['blog_ID'])) { // initialize info only once, when needs to display the link
     ...
     */
    public function get_twitter_link($target_type, $target_id)
    {
        ...
        // set callback url
        $callback = $this->get_htsrv_url('twitter_callback', [], '&', true);
        ...
        if (empty($result)) { // wasn't linked to twitter
            ...
            $unlink_url = $this->get_htsrv_url('unlink_account', [
     ...
     */
    public function get_twitter_contact($oauth_token, $oauth_token_secret)
    {
        ...
        if (empty($account->errors)) {
            if (is_array($account)) { // Get only first account
                $account = $account[0];
            }
    ...
    {
        return ['unlink_account', 'twitter_callback'];
    }
     ...
     */
    public function htsrv_twitter_callback($params)
    {
        ...
        if ($target_type == 'blog') { // redirect to blog settings
            $redirect_to = url_add_param($admin_url, 'ctrl=coll_settings&tab=plugins&blog=' . $target_id);
        } elseif ($target_type == 'user') { // redirect to user advanced preferences form
        ...
        if ($target_type == 'blog') { // blog settings
            ...
            $Blog->dbupdate();
        } elseif ($target_type == 'user') { // user advanced preferences
     ...
     */
    public function htsrv_unlink_account($params)
    {
        ...
        if ($target_type == 'blog') { // Blog settings
            ...
            $Blog->dbupdate();
        } elseif ($target_type == 'user') { // User settings
            $redirect_to = url_add_param($admin_url, 'ctrl=user&user_tab=advanced&user_ID=' . $target_id);

            if (isset($current_User) && (! check_user_perm('users', 'edit')) && ($target_id != $current_User->ID)) { // user is only allowed to update him/herself
    ...
    public function get_oauth_info($params = [])
    {
        $params = array_merge([
            'type' => '',
            'ID' => '',
        ...
        if ($params['type'] == 'blog') {	// Get from CollSettings
            $blog_ID = $params['ID'];
            $try_user = false;
        } elseif ($params['type'] == 'user') {	// Get from UserSettings
            $user_ID = $params['ID'];
            $try_user = true;
        } else {	// Get from any
        ...
        if (! empty($blog_ID)) {	// CollSettings
            $BlogCache = &get_Cache('BlogCache');
            $Collection = $Blog = &$BlogCache->get_by_ID($blog_ID, false, false);
            if (! empty($Blog)) {
                $r['token'] = $this->get_coll_setting('twitter_token', $Blog);
                if (! empty($r['token'])) {	// There is already a linked twitter user in this Blog, get token secret
        ...
        if ($try_user && ! empty($user_ID)) {	// UserSettings
            $r['token'] = $this->UserSettings->get('twitter_token', $user_ID);
            if (! empty($r['token'])) {	// There is already a linked twitter user in this User, get token secret
    ...
    public function send_a_tweet($content, &$Item, &$xmlrpcresp)
    {
        global $Messages;
        ...
        if (empty($oauth['msg_format']) || empty($oauth['token']) || empty($oauth['token_secret'])) { // Not found, fallback to Trying to get twitter account for User:
        ...
        $msg = str_replace(
            ['$title$', '$excerpt$'],
            [html_entity_decode($content['title']), html_entity_decode($content['excerpt'])],
            $oauth['msg_format']
        );
        ...
        if ((utf8_strpos($msg, '$url$') === 0) && ($base_url_len + $msg_len - 5) > $this->message_length_limit) {	// The message is too long and is starting with $url$
            $max_len = $this->message_length_limit + $full_url_len - $base_url_len;
            $msg = strmaxlen(str_replace('$url$', $content['url'], $msg), $max_len, '...');
        } elseif ((utf8_strpos(strrev($msg), 'p2b# $lru$') === 0) && ($base_url_len + $msg_len - 10) > $this->message_length_limit) {	// The message is too long and is ending on '$url$ #b2p'
            ...
            $msg .= ' ' . $content['url'] . ' #b2p';
        } elseif ((utf8_strpos(strrev($msg), '$lru$') === 0) && ($base_url_len + $msg_len - 5) > $this->message_length_limit) {	// Same as above, but without '#b2p' suffix
            $max_len = $this->message_length_limit - $base_url_len - 1; // save room for space character
            $msg = strmaxlen(str_replace('$url$', '', $msg), $max_len, '...');
            $msg .= ' ' . $content['url'];
        } elseif ((utf8_strpos($msg, '$url$') !== false) && ($base_url_len + $msg_len - 5) > $this->message_length_limit) {	// Message is too long and $url$ is somewhere in the middle
        ...
        } else {	// We don't want to add URL. Crop the message if needed
