...
class auto_p_plugin extends Plugin
{
    ...
    public $help_topic = 'auto-p-plugin';
     ...
     * @var string
     */
    public $block_tags = 'blockquote|dd|div|dl|dt|fieldset|form|h[1-6]|hr|li|object|ol|p|pre|select|script|table|td|th|ul';

    public $p_allowed_in = ['address', 'applet', 'blockquote', 'body', 'button', 'center', 'dd', 'del', 'div', 'fieldset', 'form', 'iframe', 'ins', 'li', 'map', 'noframes', 'noscript', 'object', 'td', 'th'];

    public $br_allowed_in = [
        // Block level:
...
Optionally, it will also mark single line breaks with HTML &lt;BR&gt; tags.');
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        if (preg_match('/\[[a-z]+:[^\]`]+\]/i', $content)) {	// Call replace_content() on everything outside multiline short tags:
            $content = callback_on_non_matching_blocks(
                $content,
                '~\[[a-z]+:[^\]`]+\]~is',
                [$this, 'render_autop']
            );
        } else {	// No short tags, replace on the whole content:
     ...
     */
    public function render_autop($content)
    {
        ...
        $content_parts = split_outcode(['[teaserbreak]', '[pagebreak]'], $content, true);
        $content_parts[] = '';
    ...
    public function get_coll_setting_definitions(&$params)
    {
        ...
        if (! empty($params['blog_type'])) {	// Set default settings depending on collection type:
     ...
     * @return string
     */
    public function handle_blocks($text)
    {
        ...
        foreach ($blocks as $b => $block) {
            if ($block === '') {	// Skip empty string:
                continue;
            }

            if ($tag_position !== false && $b <= $tag_position + 2) {	// Skip tag name and attributes:
                if ($b == $tag_position + 2) {	// Tag position is ended here, next is text:
                    $tag_position = false;
                }
                continue;
            }
            ...
            if (strpos($block, '<') === 0 &&
                isset($blocks[$b + 1]) &&
                preg_match('~^(' . $this->block_tags . ')$~i', $blocks[$b + 1])) {	// Block tag is opening or closing here:
                $tag_position = $b;
                if (substr($block, 1, 1) == '/') {	// This is a closing of opened tag,
                    // Remove last opened tag from the array:
                    array_pop($opened_tags);
                } elseif (substr($block, -2, 1) != '/') {	// This is a start of new tag AND this tag is not single/autoclosed tag like <hr />, <img /> and etc.,
     ...
     * @return string
     */
    public function handle_pre_blocks($text, $in_tag)
    {
        ...
        if ($in_tag) {
            ...
            if (! in_array($in_tag, $this->p_allowed_in)) { // we're in a tag, where no P tags are allowed, so just do the BRs:
        ...
        if (trim($text) == '') { // there's only whitespace
            return $text;
        }

        // fix it, so no (inline) tags span across multiple blocks:
        $new_blocks = [];
        $new_blocks_nowrap = []; // blocks that should not be wrapped in P (opening without closing tag or vice versa)
        $count_new_blocks = 0;
        $looking_for_close_tag = [];
        ...
        for ($i = 0, $n = count($text_lines); $i < $n; $i = $i + 2 /* every 2nd line is a real block */) {
            ...
            if (! isset($new_blocks[$count_new_blocks])) {
                ...
                while (preg_match('~^(.*?)(<\s*/\s*(\w+)(\s+[^>]*?)?>)~is', $line_copy, $match)
                    && ! (preg_match('~^(.*?)(<\s*' . $match[3] . '(\s+[^>]*?(\s*/\s*)?)?>)~is', $new_blocks[$count_new_blocks] . $match[1]))) { // a closing tag:
                    $new_blocks[$count_new_blocks] .= $match[0];
                    $line_copy = substr($line_copy, strlen($match[0]));
                }
                if (! empty($new_blocks[$count_new_blocks])) { // we've found a closing tag with no opening tag, this must not get wrapped in P:
            ...
            if (preg_match('~^(.*?)(<\s*(\w+)(\s+[^>/]*)?(\s*/\s*)?>)~is', $line, $match)) { // a opening tag:
                $tag = $match[3];
                $pos_after_tag = strlen($match[0]);

                while (! empty($match[5]) /* "/" */) { // self-closing tag, find next:
                    $tag = false;
        ...
        if ($looking_for_close_tag) {
            ...
            if ($looking_for_close_tag['pos'] > 0) { // move part of last block without closing tag to an own block:
            ...
            } else { // the whole block should not get wrapped!
                $new_blocks_nowrap[] = $count_new_blocks;
            }
        }
        ...
        for ($i = 0, $n = count($new_blocks); $i < $n; $i = $i + 2) {
            ...
            if (empty($new_blocks[$i])) {
                if ($this_wrap_in_p) { // not the last one
                    $block = '<p></p>';
                } else {
                    $block = '';
                }
     ...
     */
    public function handle_pre_blocks_helper($block, $in_tag, $wrap_in_p, $ignore_NL = true)
    {
        ...
        if (preg_match('~^(.*?)(<\s*(\w+)(\s+[^>]*)?>)~is', $block, $match)) { // a tag:
            $tag = $match[3];
            $before_tag = $match[1];

            if (! empty($before_tag)) { // Delegate to handle_br:
            ...
            if (! empty($text_in_tag)) { // Recurse (same level) - with the optional newlines at start and end, because in an inline tag every linebreak should become a BR:
     ...
     * @return string
     */
    public function handle_br($text, $in_tag)
    {
     ...
     * @return array
     */
    public function split_text_for_tag($tag, &$text_after_tag)
    {
        ...
        while (1) {
            ...
            if (preg_match('~^(.*?)(<\s*(/)?\s*' . preg_quote($tag, '~') . '\s*(/\s*)?>)~is', $loop_text, $after_match)) {
                ...
                if ($is_closing) {
                    ...
                    if ($depth == 0) { // found the matching closing tag:
                        ...
                        break;
                    } else { // this closing tag is part of the outer:
                        $text_in_tag .= $found_tag;
                    }
                } else { // found the same, but opening tag (nested)
                    $text_in_tag .= $found_tag;
                    $depth++;
                }
            ...
            } else { // did not find the closing tag.. :/
                $closing_tag = '';
        ...
        $r = [$text_in_tag, $closing_tag, $NL_before, $NL_after];
     ...
     * @return string
     */
    public function autobr($text, $replace_last = true)
    {
        if (! $this->use_auto_br) { // don't make <br />'s
            return $text;
        }
