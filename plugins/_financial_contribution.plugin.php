...
class financial_contribution_plugin extends Plugin
{
    ...
    public $subgroup = 'other';

    public $widget_icon = 'money';
    ...
    public function SkinTag(&$params)
    {
        ...
        if (empty($Blog)) {	// Don't display this widget when no current Collection:
            $this->display_widget_debug_message('Plugin widget "' . $this->name . '" is hidden because there is no Collection.');
            return false;
        }

        if (empty($Item)) {	// Don't display this widget when no current Item:
            $this->display_widget_debug_message('Plugin widget "' . $this->name . '" is hidden because there is no Item.');
            return false;
        }

        if (! is_logged_in()) {	// Don't display this widget when current User is not logged in:
            $this->display_widget_debug_message('Plugin widget "' . $this->name . '" is hidden because you are not logged in.');
            return false;
        }

        $CurrencyCache = &get_CurrencyCache();
        if (! ($Currency = $CurrencyCache->get_by_name($this->get_coll_setting('currency', $Blog), false, false))) {	// Don't display this widget with unknown currency:
        ...
        if ($Item->get_custom_field_value($this->get_widget_setting('check_custom_field')) <= 0) {	// Don't display this widget when custom field value is less 0 for the current Item:
        ...
        $total_amount = 0;
        if (! empty($contributed_users)) {	// Display only if at least one user contributed this Item:
        ...
        $Form = new Form($this->get_htsrv_url('contribute', [], '&'), '', 'post');
        ...
        if (! empty($current_contributors_text)) {	// Current contributors list:
        ...
        $contribute_input = $Form->text_input('contribute_amount', $contributed_current_user_amount, 5, '', '', [
            'maxlength' => 16,
            'style' => 'width:auto;display:inline',
        ]);
        $contribute_button = $Form->button(['submit', 'action_name',
        ...
        $Form->output = true;
        $this->display_widget_text(
            ($is_contributed_current_user ? 'thank_you_text' : 'contribute_text'),
     ...
     */
    public function display_widget_text($setting_name, $content = null)
    {
        $settings = is_array($setting_name) ? $setting_name : [
        ...
        foreach ($settings as $setting_name => $content) {
            ...
            if ($s < $settings_num - 1) {	// Display a separator between several settings:
     ...
     * @param array Params
     */
    public function htsrv_contribute($params)
    {
        ...
        if ($contribute_amount === '' ||
            ...
            $contribute_amount < 0) {	// Allow only decimal numbers for amount::
        ...
        if (! ($Widget = &$WidgetCache->get_by_ID($widget_ID, false, false))) {	// Widget is not found by requested ID:
            // (don't translate because for normal work it must not happens):
        ...
        if (! ($Item = &$ItemCache->get_by_ID($item_ID, false, false))) {	// Item is not found by requested ID:
            // (don't translate because for normal work it must not happens):
        ...
        if ($contribute_amount > 0) {	// Insert/Update contribution:
     ...
     */
    public function get_users_by_item_ID($item_ID)
    {
        global $DB;

        if (empty($item_ID)) {	// Item ID must be defined:
            return [];
        }
