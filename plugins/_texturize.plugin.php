...
{
    public $code = 'b2WPTxrz';

    public $name = 'Texturize';
...
<ul>
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        $params['data'] = callback_on_non_matching_blocks($params['data'], '#<(pre|code)[\s\S]+?/\1>#i', [$this, 'texturize_block']);
    }
     ...
     */
    public function texturize_block($content)
    {
        $output = '';
        ...
        for ($i = 0; $i < $stop; $i++) {
            ...
            if (strlen($curl) && '<' != $curl[0] && $next) { // If it's not a tag
                ...
                $cockney = ["'tain't", "'twere", "'twas", "'tis", "'twill", "'til", "'bout", "'nuff", "'round"];
                $cockneyreplace = ["&#8217;tain&#8217;t", "&#8217;twere", "&#8217;twas", "&#8217;tis", "&#8217;twill", "&#8217;til", "&#8217;bout", "&#8217;nuff", "&#8217;round"];
