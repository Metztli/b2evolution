...
class ParsedownB2evo extends ParsedownExtra
{
    ...
    public function __construct()
    {
        ...
        if (isset($this->unmarkedBlockTypes) &&
            ...
            ($code_index = array_search('Code', $this->unmarkedBlockTypes)) !== false) {	// Don't parse code blocks by indent spaces or tab:
    ...
    {
        if ($this->b2evo_parse_font_styles) {	// Allow to parse font styles:
            return parent::inlineEmphasis($Excerpt);
        } else {	// Don't parse font styles:
            return;
        }
    }
    ...
    {
        if ($this->b2evo_parse_links) {	// Allow to parse inline links:
            $link_data = parent::inlineLink($Excerpt);
            if (is_array($link_data) && isset($link_data['element']['attributes']['href']) &&
                preg_match('/^(https?:\/\/|\/)/i', $link_data['element']['attributes']['href'])) {	// Allow link ONLY with URL which starts with "http://", "https://" or "/":
                return $link_data;
            } else {	// Deny wrong URL, because it may contains vulnerability code like "javascript:alert()":
                return;
            }
        } else {	// Don't parse inline links:
            return;
        }
    }
    ...
    {
        if ($this->b2evo_parse_images) {	// Allow to parse inline images:
            ...
            return $r;
        } else {	// Don't parse inline images:
            return;
        }
    }
    ...
    {
        if (strpos($text, '&gt;') !== false) {	// Fix the encoded chars ">" to correct parsing of blockquote:
            $text = preg_replace_callback('/(^|\n)((&gt; ?)+)/', [$this, 'b2evo_html_decode_blockquote'], $text);
        }

        if (strpos($text, '&quot;') !== false) {	// Fix the encoded chars '"' to correct parsing of links and images:
    ...
    {
        if ($Line['text'] == '[teaserbreak]' || $Line['text'] == '[pagebreak]') {	// Don't apply <p> around item content separators:
            return [
                'element' => [
                    'name' => 'notag',
                    'text' => $Line['text'],
    ...
    protected function element(array $Element)
    {
        ...
        if (isset($Element['before'])) {	// Prepend additional text:
            $r .= $Element['before'];
        }

        if (isset($Element['name']) && $Element['name'] == 'notag') {	// Don't apply any html tag, Use simple text:
            if (isset($Element['handler'])) {	// Use handler function:
                $r .= $this->{$Element['handler']}($Element['text']);
            } else {	// No handler, just text:
                $r .= $Element['text'];
            }
        } else {	// Use standard preparing for other cases:
            $r .= parent::element($Element);
        }

        if (isset($Element['after'])) {	// Append additional text:
            $r .= $Element['after'];
        }
    ...
    {
        if (! empty($Block['interrupted']) && empty($Line['indent'])) {	// If a list is interrupted now(for example, new line after list line),
            ...
            if (isset($Line['body']) &&
                isset($Block['li']['text']) &&
                is_array($Block['li']['text'])) {	// If list has at least one item:
                $after_list_items_num = count($Block['li']['text']);
                if ($after_list_items_num > $before_list_items_num &&
                    $Block['li']['text'][$after_list_items_num - 1] !== $Line['body']) {	// If new list item was added in parent::blockListContinue() above,
    ...
    {
        if (! isset($Block['element']['attributes'])) {	// Initialize attributes:
    ...
    protected function inlineCode($Excerpt)
    {
        ...
        if (isset($element_data['element'])) {	// Use special handler instead of parent::escape() to avoid htmlspecialchars():
            $element_data['element']['handler'] = 'noescapeCodeHandler';
            // Add default class for all <code> tags:
            $element_data['element']['attributes'] = [
                'class' => 'codespan',
            ];
        }
     ...
     * @return string
     */
    public function noescapeCodeHandler($text, $nonNestables)
    {
        return $text;
    }
     ...
     */
    public function disable_options($disabled_options)
    {
        if (empty($disabled_options)) {	// No blocks to disable:
            return;
        }
    ...
    protected function inlineUrl($Excerpt)
    {
        ...
        if (isset($Inline['element'])) {	// Force element type from <a> to simple text without html tag to avoid rendering of URLs as html links:
