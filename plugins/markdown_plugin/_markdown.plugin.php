...
class markdown_plugin extends Plugin
{
    ...
    public $help_topic = 'markdown-plugin';
...
> blockquote');
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        $parsedown_options = [
            ...
            'footnote' => ['Footnote', 'FootnoteMarker'],
            'abbr' => ['Abbreviation'],
        ];
        $disabled_options = [];
        ...
        if ($setting_Blog = &$this->get_Blog_from_params($params)) {	// We are rendering Item, Comment or Widget now, Get the settings depending on Collection:
            ...
            foreach ($parsedown_options as $setting_key => $option_names) {
                if (! $this->get_coll_setting($setting_key, $setting_Blog)) {	// Disable parsedown option if it is not checked in collection plugin settings:
            ...
            foreach ($parsedown_options as $setting_key => $option_names) {
                if (! $this->get_msg_setting($setting_key)) {	// Disable parsedown option if it is not checked in messaging plugin settings:
                    $disabled_options = array_merge($disabled_options, $option_names);
                }
            }
            ...
            foreach ($parsedown_options as $setting_key => $option_names) {
                if (! $this->get_email_setting($setting_key)) {	// Disable parsedown option if it is not checked in emails plugin settings:
                    $disabled_options = array_merge($disabled_options, $option_names);
                }
            }
        ...
        if (stristr($content, '<code') !== false ||
            ...
            preg_match('/\[[a-z]+:[^\]`]+\]/i', $content)) { // Call replace_content() on everything outside code/pre:
    ...
    {
        if (empty($params['Blog']) || empty($params['kind'])) { // Invalid data, Exit here
            return;
        }
    ...
    public function DisplayCommentToolbar(&$params)
    {
        ...
        if (! is_admin_page() && $disable_markdown_toolbar_for_frontoffice) {	// Disable markdown toolbar until JS can be fixed to defer load:
            return false;
        }
     ...
     * @param array Params
     */
    public function DisplayCodeToolbar($type = 'coll', $Blog = null, $params = [])
    {
        global $Hit;
        ...
        $js_config = [
            ...
            'toolbar_before' => $this->get_template('toolbar_before', [
