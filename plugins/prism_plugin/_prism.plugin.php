...
class prism_plugin extends Plugin
{
    ...
    public function get_custom_setting_definitions(&$params)
    {
        return [
            'force_load_assets' => [
                ...
                'type' => 'checklist',
                'options' => [
                    ['single', 'disp=single, disp=page', 0],
                    ['posts', 'disp=posts', 0],
                    ['comments', 'disp=comments', 0],
                    ['front', 'disp=front', 0],
                    ['other_disps', T_('other disps'), 0],
                ],
            ],
        ];
    }
     ...
     * @param mixed $params
     */
    public function FilterItemContents(&$params)
    {
     ...
     * @param mixed $params
     */
    public function UnfilterItemContents(&$params)
    {
        $content = &$params['content'];
    ...
    public function CommentFormSent(&$params)
    {
        ...
        if (! $comment_Item) {	// Incorrect item
            return false;
        }

        $item_Blog = &$comment_Item->get_Blog();
        $apply_rendering = $this->get_coll_setting('coll_apply_comment_rendering', $item_Blog);
        if ($this->is_renderer_enabled($apply_rendering, $params['renderers'])) {	// render code blocks in comment
    ...
    public function MessageThreadFormSent(&$params)
    {
        ...
        if ($this->is_renderer_enabled($apply_rendering, $params['renderers'])) {	// render code blocks in message
            $this->FilterItemContents($params);
        }
    }
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        $content = preg_replace_callback(
            '#(\<p>)?\<!--\s*codeblock([^-]*?)\s*-->(\</p>)?\<pre[^>]*><code[^>]*>([\s\S]+?)</code>\</pre>(\<p>)?\<!--\s*/codeblock\s*-->(\</p>)?#i',
            [$this, 'render_codeblock_callback'],
            $content
        );
     ...
     */
    public function render_codeblock_callback($block)
    {
     ...
     * @return string
     */
    public function encode_html_entities($block)
    {
     ...
     */
    public function filter_code($content)
    {
        ...
        // 3 - code content
        $content = preg_replace_callback(
            '#\[(codeblock|codespan)([^\]]*?)\]([\s\S]+?)?\[/\1\]#i',
            [$this, 'filter_code_callback'],
            $content
        );
     ...
     */
    public function filter_code_callback($block)
    {
        $content = isset($block[3]) ? trim($block[3]) : '';

        if (empty($content)) {	// Don't render if no code content
            return '';
        }
        ...
        if (! in_array($lang, ['php', 'css', 'javascript', 'sql', 'html', 'markup', 'apacheconf'])) {	// Use Markup for unknown language
            $lang = '';
        }

        $code_class = '';
        if ($type == 'codespan') {	// Use standard class for codespan:
            $code_class .= 'codespan';
        }
        if (! empty($lang)) {	// Use language class only for known language:
            $code_class .= ' language-' . $lang;
        }

        $content = $block[3];

        if ($type == 'codeblock') {	// Remove first empty line from codeblock content:
            $content = preg_replace('/^\r?\n/', '', $content);
        }

        $r = '<code' . (empty($code_class) ? '' : ' class="' . trim($code_class) . '"') . '>' . $content . '</code>';

        if ($type == 'codeblock') {	// Set special template and attributes only for codeblock
     ...
     * @return string Content
     */
    public function unfilter_code($content)
    {
        $content = preg_replace_callback(
            '#(<pre class="line-numbers"( data-start="(-?[0-9]+)")?>)?<code( class="([^"]+)")?>([\s\S]+?)?</code>(</pre>)?#i',
            [$this, 'unfilter_code_callback'],
            $content
        );
     ...
     * @return string formatted code || empty
     */
    public function unfilter_code_callback($block)
    {
        $content = $block[6];

        if (empty($block[1])) {	// [codespan]
            $code_tag = 'codespan';
        ...
        if (! empty($lang)) {	// Add lang attribute only if it is defined:
            preg_match('/language-([a-z]+)/', $lang, $lang_match);
            $lang = empty($lang_match[1]) ? '' : trim($lang_match[1]);
            if ($lang == 'mermaid') {	// Don't touch marmaid language because we have a separate special plugin "Mermaid Diagrams" for it:
                return $block[0];
            }
            if (empty($lang) || ! in_array($lang_match[1], ['php', 'css', 'javascript', 'sql', 'html', 'markup', 'apacheconf'])) {	// Don't allow unknown language:
                $lang = '';
            } else {	// It is allowed language
    ...
    public function load_assets()
    {
        ...
        if (is_array($evo_renderers_used_in_current_page) &&
            in_array($this->code, $evo_renderers_used_in_current_page)) {	// Load load CSS/JS files if this plugin is used on the current page by any Item, Comment, etc.:
            return true;
        }
    ...
    public function AdminDisplayToolbar(&$params)
    {
        ...
        if (empty($apply_rendering) || $apply_rendering == 'never') {	// Don't display toolbar:
            return false;
        }
     ...
     * @param array Params
     */
    public function display_toolbar($params)
    {
        $params = array_merge([
