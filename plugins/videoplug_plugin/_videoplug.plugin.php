...
{
    public $code = 'evo_videoplug';

    public $name = 'Video Plug';
    ...
    public function GetDefaultSettings(&$params)
    {
        return [
            'youtube_load' => [
                ...
                'type' => 'radio',
                'options' => [
                    ['standard', T_('Standard') . ' <code>&lt;iframe&gt;</code>'],
                    ['lazyload', T_('Lazy-Load')],
                ],
                'field_lines' => true,
    ...
    public function SkinBeginHtmlHead(&$params)
    {
        ...
        expose_var_to_js('evo_plugin_videoplug_config', [
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        if ($setting_Blog = &$this->get_Blog_from_params($params)) {	// We are rendering Item, Comment or Widget now, Get the settings depending on Collection:
            $this->video_width = $this->get_coll_setting('width', $setting_Blog);
            $this->video_height = $this->get_coll_setting('height', $setting_Blog);
        } elseif (! empty($params['Message'])) {	// We are rendering Message now:
            $this->video_width = $this->get_msg_setting('width');
            $this->video_height = $this->get_msg_setting('height');
        } elseif (! empty($params['EmailCampaign'])) {	// We are rendering EmailCampaign now:
            $this->video_width = $this->get_email_setting('width');
            $this->video_height = $this->get_email_setting('height');
        } else {	// Unknown call, Don't render this case:
            return;
        }
        ...
        if (! empty($setting_Blog) && (
            ...
            $content = replace_outside_code_and_short_tags(
                '#(<a[^>]+href=")?(https?://(.+\.)?(rutube.ru|youtube.com|youtu.be|dailymotion.com|vimeo.com|facebook.com|wistia.com)([^"\s\n\r<]+))("[^>]*>(.+?)</a>)?#i',
                [$this, 'parse_video_url_callback'],
                $content,
                'replace_content',
                'preg_callback'
            );
        ...
        $content = move_short_tags($content, '/\[video:(rutube|youtube|dailymotion|vimeo|facebook|wistia):?[^\[\]]*\]/i');

        // Replace video tags with html code:
        $content = replace_outside_code_tags(
            '#\[video:(rutube|youtube|dailymotion|vimeo|facebook|wistia|google|livevideo|ifilm):([^:\[\]\\\/]*|https?:\/\/.*\.facebook\.com\/[^:]*):?(\d+%?)?:?(\d+%?)?:?([^:\[\]\\\/]*)\]#',
            [$this, 'parse_video_tag_callback'],
            $content,
            'replace_content',
            'preg_callback'
        );
     ...
     */
    public function parse_video_url_callback($m)
    {
        if (isset($m[7]) && $m[7] != $m[2]) {	// Skip if link text is different than URL(e.g. <a href="https://youtu.be/abc123">Click to see Video!</a>):
            return $m[0];
        }
        ...
        switch ($m[4]) {
            case 'rutube.ru':
                if (preg_match('#(^/(embed/)?|[\?&]v=)([^&=\?]+)(&|$)#', $m[5], $code)) {
                    if ($this->get_setting('rutube_load') == 'standard') {	// Use standard loading with iframe:
                        $video_block = '<iframe id="rtplayer" type="text/html" src="//www.rutube.ru/embed/' . $code[3] . '" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
                    } else {	// Use Lazy-loading:
                        $video_block = '<div class="evo_rutube" data-embed="' . $code[3] . '"><div class="evo_rutube__play_button"></div></div>';
                    }
                }
                break;
            ...
            case 'youtu.be':
                if (preg_match('#(^/(embed/)?|[\?&]v=)([^&=\?]+)(&|$)#', $m[5], $code)) {
                    if ($this->get_setting('youtube_load') == 'standard') {	// Use standard loading with iframe:
        ...
        if (! isset($video_block)) {	// No found correct video code in the URL:
            return $m[0];
        }

        $style = '';

        if (! empty($this->video_width)) {	// Set width depending on what units are used:
            $style .= 'width:' . (strpos($this->video_width, '%') === false ? $this->video_width . 'px' : $this->video_width) . ';';
        }

        if (! empty($this->video_height)) {	// Set height depending on what units are used:
     ...
     * @param array Matches:
     *              0 - Full video tag
     *              1 - Video type: rutube, youtube, dailymotion, vimeo, facebook, wistia, google, livevideo, ifilm
     *              2 - Video code/key
     *              3 - Width
     *              4 - Height
     *              5 - Extra params
     * @return string HTML video code
     */
    public function parse_video_tag_callback($m)
    {
        switch ($m[1]) {
            case 'rutube':
                $video_block = '<iframe id="rtplayer" type="text/html" src="//rutube.ru/embed/' . $m[2] . (! empty($m[5]) ? '?' . $m[5] : '') . '" allowfullscreen="allowfullscreen" frameborder="0"></iframe>';
                break;
        ...
        if (! empty($width)) {	// Set width depending on what units are used:
        ...
        if (! empty($height)) {	// Set height depending on what units are used:
				...
				while( 1 )
				{
					...
					switch( tag )
					{
						case 'rutube':
							...
							regexp_ID = /^[a-z0-9_-]+$/i;
							regexp_URL = /^(.+rutube(?:-nocookie)?\.ru\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|.+rutu\.be\/)([a-z0-9_-]{11})((?:\?|&).*)?/i;
							break;
