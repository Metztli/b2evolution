...
class table_contents_plugin extends Plugin
{
    ...
    public $subgroup = 'infoitem';

    public $short_desc;

    public $long_desc;

    public $help_topic = 'table-of-contents-plugin';

    public $widget_icon = 'list';
    ...
    public function SkinBeginHtmlHead(&$params)
    {
        ...
        if ($disp == 'single' || $disp == 'page') {	// Initialize JS for better scrolling only on Item's page:
    ...
    public function RenderItemAsHtml(&$params)
    {
        ...
        $content = replace_outside_code_tags('#\[toc\]#i', [$this, 'callback_render_toc'], $content, 'replace_content_callback');
     ...
     */
    public function genereate_toc($Item, $content)
    {
        $toc = '';

        if (empty($Item)) {	// Item must be defined for initialize URL:
            return $toc;
        }

        if (preg_match_outcode('#<h([1-6])[^>]+id=([^>\s]+)[^>]*>(.+?)</h\1>#i', $content, $header_matches)) {	// If at least one `<Hx id="xxx">` is found in content:
     ...
     */
    public function callback_render_toc($toc_matches)
    {
        if ($this->cached_toc === null) {	// Initialize table of contents once for the requested content and store into cache:
    ...
    public function SkinTag(&$params)
    {
        ...
        if ($disp != 'single' && $disp != 'page') {	// Don't display this widget for not post pages:
        ...
        if (empty($toc)) {	// Don't display widget when current Item has no anchor header tags in content:
