...
{
    public $code = 'b2evEmailEl';

    public $name = 'Email Elements';
    ...
    public $help_topic = 'email-elements-plugin';

    public $number_of_installs = 1;

    public $cta_numbers = [1, 2, 3];

    public $button_styles = ['primary', 'success', 'warning', 'danger', 'info', 'default', 'link', 'image'];

    public $default_button_styles = [
        'button' => 'primary',
        'cta' => 'primary',
        'like' => 'success',
        'dislike' => 'danger',
    ...
    public function DisplayCodeToolbar($params = [])
    {
        ...
        $image_link_ID_input = '<div id="image_link_ID_wrapper" style="display:none">' . $Form->text_input('image_link_ID', '', null, T_('Image Link ID'), '', [
            'required' => true,
        ]) . '</div>';
        $button_text_input = $Form->text_input('button_text', '', null, T_('Text'), '', [
            'style' => 'width:100%;',
        ]);
        $button_url_input = $Form->text_input('button_url', '', null, T_('URL'), '', [
            'style' => 'width:100%;',
        ]);
    ...
    public function RenderInlineTags(&$params)
    {
        if (! isset($params['inline_tags']) ||
            ! is_array($params['inline_tags'])) {	// No inline tags to render:
            return [];
        }

        if (! isset($params['Object']) ||
            ! is_object($params['Object']) ||
            get_class($params['Object']) != 'EmailCampaign') {	// Not supported object by this plugin:
            return [];
        }
        ...
        foreach ($params['inline_tags'] as $inline_tag) {
            ...
            foreach ($matches[0] as $i => $current_element) {
                ...
                $style = $style[0];
                if (in_array($style, $this->button_styles)) {	// If 2nd/3rd option is a style then 3rd is an URL:
                    $url = isset($options[1 + $index_shift]) ? trim($options[1 + $index_shift]) : null;
                } else {	// Otherwise 2nd/3rd option is an URL, i.e. this short tag should use default style:
                    $url = trim($matches[2][$i]);
                    $style = $this->default_button_styles[$type];
                }
                if (empty($style)) {	// Use default button style if it is not defined in short tag:
                    $style = $this->default_button_styles[$type];
                }
                if ($style == 'image') {	// Check if correct image link ID is used:
                    $image_style = '';
                    if ($style_link_data !== null && preg_match('/^(\d+)(\..+)?$/', $style_link_data, $style_link_match)) {	// Corrent link ID and/or style class:
                        $style_link_ID = $style_link_match[1];
                        if (isset($style_link_match[2])) {	// Convert a separator between several classes like '.rounded.border' to '.rounded+.border' for function emailskin_style():
                    ...
                    if (! ($image_Link = &$LinkCache->get_by_ID($style_link_ID, false, false))) {	// Link is not found in DB:
                        $current_element_error_message = sprintf(T_('Link %s doesn\'t exist!'), '#' . $style_link_ID);
                    } elseif (! ($image_File = &$image_Link->get_File()) || ! $image_File->is_image()) {	// File is not image:
                        $current_element_error_message = sprintf(T_('File of the Link %s is not an image!'), '#' . $style_link_ID);
                    }
                    if ($current_element_error_message !== null) {	// Replace original short tag with error message:
                    ...
                    $text = $image_Link->get_tag(array_merge($params, [
                ...
                if (empty($url) && in_array($type, ['button', 'like', 'dislike', 'cta'])) {	// Use default destination of the Email Campaign if optional URL if not defined:
                ...
                switch ($type) {
                    ...
                    case 'dislike':
                        $url_params = [
                            'evo_mail_function' => $type,
                        ];
                        break;

                    case 'cta':
                        $cta_num = trim($options[0]);
                        if (! in_array($cta_num, $this->cta_numbers)) {	// Don't allow CTA with wrong number:
                            unset($url);
                            break;
                        }

                        $url_params = [
                            'evo_mail_function' => $type . $cta_num,
                        ];
                        break;
                    ...
                    case 'activate':
                        ...
                        if (! empty($redirect_to)) {	// Append a redirect URL after activation if it is defined in options of the short tag:
                            $url = url_add_param($url, [
                ...
                if (! empty($text) && ! empty($url) && ! validate_url($url)) {	// If button/link is correct and can be rendered:
                    if ($url_params !== null) {	// Additional URL param:
                        $url = url_add_param($url, $url_params);
                    }
                    $link_tag = get_link_tag($url, $text, in_array($style, ['link', 'image']) ? '' : 'div.btn a+a.btn-' . $style);
                    // Render short tag:
    ...
    {
        if (get_link_owner_type($params['link_ID']) != 'emailcampaign') {	// Allow additional inline tag 'clickable' only for Email Campaigns:
            return [];
        }

        return [
            'clickable' => T_('Clickable'),
        ];
    }
    ...
    {
        if (empty($params['source_tag'])) {	// No source tag:
            return;
        }

        if (! preg_match('#\[(button|like|dislike|cta|activate|unsubscribe)(:(\d+))?:image\#(\d+)([^\[\]]*?)](.*?)\[\/\1]#', $params['source_tag'], $tag_match)) {	// The requested tag is not supoerted by this plugin:
            return;
        }
    ...
    public function DisplayImageInlineTagForm(&$params)
    {
        ...
        if (empty($params['Form'])) {	// Form object must be passed for this event:
            return false;
        }
        ...
        switch ($params['display_tag']) {
            case 'clickable':
                ...
                $Form->output = true;
                $Form->radio('clickable_type', param('clickable_type', 'string', 'button'), [
                    ['button', T_('Button')],
                    ['cta', T_('Call to Action'), $cta_select],
                    ['like', T_('Like')],
                    ['dislike', T_('Dislike')],
                    ['activate', T_('Activate')],
                    ['unsubscribe', T_('Unsubscribe')],
                ], T_('Type'), true);
                return true;
        }
