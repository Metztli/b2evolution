...
class test_plugin extends Plugin
{
    ...
    public $help_url = '';  // empty URL defaults to manual wiki
    ...
    public function GetDefaultSettings(&$params)
    {
        $r = [
            ...
            'select_multiple' => [
                ...
                'allow_none' => true,
                'options' => [
                    'sci' => $this->T_('Scissors'),
                    'pap' => $this->T_('Paper'),
                    'sto' => $this->T_('Stone'),
                ],
                'defaultvalue' => ['sci', 'sto'],
        ...
        if ($params['for_editing']) { // we're asked for the settings for editing:
    ...
    public function get_email_setting_definitions(&$params)
    {
        ...
        $default_params = array_merge($params, [
            'default_email_rendering' => 'stealth',
        ]);
    ...
    public function get_shared_setting_definitions(&$params)
    {
        ...
        $default_params = array_merge($params, [
            'default_shared_rendering' => 'stealth',
        ]);
    ...
    public function GetDependencies()
    {
        return [
            'recommends' => [
                'events_by_one' => [['Foo', 'Bar'], ['FooBar', 'BarFoo']], // a plugin that provides "Foo" and "Bar", and one (may be the same) that provides "FooBar" and "BarFoo"
                'events' => ['some_event', 'some_other_event'],
                'plugins' => [['some_plugin', '1']], // at least version 1 of some_plugin
            ],
     ...
     * @return integer
     */
    public function test_plugin_get_random(&$params)
    {
     ...
     */
    public function GetHandledDispModes()
    {
        return [
     ...
     */
    public function HandleDispMode($params)
    {
     ...
     * @param array Params
     */
    public function htsrv_test_action($params)
    {
        global $DB;
        ...
        if (empty($params['param_1'])) {	// Nothing to do:
            return;
        }
    ...
    public function AdminAfterEvobarInit()
    {
        ...
        global $topleft_Menu;
        $topleft_Menu->add_menu_entries('tools', [
            'urls_sep' => [
                'separator' => true,
            ],
            'urls' => [
    ...
    public function AdminDisplayToolbar(&$params)
    {
        echo $this->get_template('toolbar_before', [
            '$toolbar_class$' => $this->code . '_toolbar',
        ]);
    ...
    public function AdminTabAction()
    {
        ...
        $this->text_from_AdminTabAction = '<p>This is text from AdminTabAction for the TEST plugin.</p>'
            ...
            . $Plugins->get_trigger_event_first_return('test_plugin_get_random', [
                'min' => -1000,
                'max' => 1000,
            ]) . '</p>';
    ...
    public function InitMainList(&$params)
    {
        ...
        if (! $preview) {
            if ($disp == 'page') {
                $params['MainList']->set_default_filters([
                    'itemtype_usage' => 'page', // pages
                ]);
            }

            if ($disp == 'search' && param('s', 'string')) {	// Here we allow b2evolution to search in posts and in pages
            ...
            if ($disp == 'posts' && ! empty($params['MainList']->filters['tags'])) {	// If the MainList if filtered by tag we search for posts in all public blogs
                if (! empty($params['MainList']->filters['tags'])) {	// All public blogs
    ...
    public function SkinEndHtmlHead(&$params)
    {
        echo '<script>
			jQuery( document ).ready( function()
			{
				...
				{
					jQuery.ajax( {
						url: "' . $this->get_htsrv_url('test_action', [
            'param_1' => 'value_1',
        ], '&', true) . '",
						data: "param2=value2",
    ...
    {
        $params['kinds'] = array_merge($params['kinds'], [
            'test_kind' => [
    ...
    public function GetSpamKarmaForComment(&$params)
    {
        ...
        if ($count > 5) {	// If comment has more 5 urls decide this comment is spam:
            return 100;
        }
    ...
    public function AlternateAuthentication(&$params)
    {
        if (0) { // you should only enable it for test purposes, because it automagically logs every user in as "demouser"!
            ...
            if ($demo_User = &$UserCache->get_by_login('demouser')) { // demouser exists:
    ...
    {
        if (! isset($params['form_type']) || $params['form_type'] != 'comment') {	// Apply captcha only for comment form:
            return false;
        }
        ...
        if (empty($question)) {	// No question detected:
            return false;
        }
    ...
    {
        if (! empty($params['is_preview']) || ! isset($params['form_type']) || $params['form_type'] != 'comment') {	// We should not apply captcha to the requested form:
            return true;
        }
        ...
        if ($posted_answer != utf8_strtolower($current_question['answer'])) {	// Wrong answer:
    ...
    public function GetAdditionalColumnsTable(&$params)
    {
        ...
        if (is_null($params['Results']) || ! is_object($params['Results'])) {	// Results must be object
            return;
        }

        if (in_array($params['table'], ['sessions', 'activity', 'ipranges', 'top_ips'])) {	// Display column only for required tables by Test plugin:
            $column = [
                'th' => 'TEST Column',
                'td' => 'TEST Value',
            ];
     ...
     */
    public function get_captcha_questions()
    {
        return [
            '1' => [
                'question' => 'Question 1?',
     ...
     */
    public function get_captcha_question()
    {
        global $Session;
        ...
        if (empty($this->question_ID)) {	// Assign new random question for current Session:
            $question = $this->get_new_captcha_question();
        }

        if (empty($question) && ! empty($this->question_ID)) {	// Get question data:
            $questions = $this->get_captcha_questions();
            if (isset($questions[$this->question_ID])) {
                $question = $questions[$this->question_ID];
            }

            if (empty($question)) {	// Assign random question if previous question doesn't exist in config:
     ...
     */
    public function get_new_captcha_question()
    {
        global $Session;
