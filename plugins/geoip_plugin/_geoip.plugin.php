...
class geoip_plugin extends Plugin
{
    ...
    public $plugin_actions = ['geoip_download', 'geoip_find_country', 'geoip_fix_country'];
     ...
     */
    public $geoip_file_path = '';

    public $geoip_file_name = 'GeoLite2-Country.mmdb';

    /**
     * URL to the GeoIP's legacy database download page
     * @var string
     */
    public $geoip_manual_download_url = '';
    ...
    public function GetDefaultSettings(&$params)
    {
        ...
        return [
            ...
            'detect_registration' => [
                ...
                'options' => [
                    ['no', TB_('No')],
                    ['auto', TB_('Auto select current country in list')],
                    ['hide', TB_('Hide country selector if a country has been detected')],
                ],
                'field_lines' => true,
                'note' => '',
                'defaultvalue' => 'no',
    ...
    {
        if (! file_exists($this->geoip_file_path)) {	// GeoIP DB doesn't exist in the right folder
            global $admin_url;

            if (is_install_page()) {	// Display simple warning on install pages
            ...
            } else {	// Display full detailed warning on backoffice pages
                return sprintf(
                    TB_('GeoIP Country database not found. Download the <b>GeoLite Country DB in binary format</b> from here: <a %s>%s</a> and then upload the %s file to the folder: %s. Click <a href="%s">here</a> for automatic download.'),
    ...
    public function AfterUserInsert(&$params)
    {
        ...
        // Update current user
        $user_update_sql = '';
        if ($this->Settings->get('force_account_creation')) {	// Force country to the country detected by GeoIP
    ...
    {
        if (! is_valid_ip_format($IP)) {	// Don't try to search country by invalid IP address:
            return false;
        }
        ...
        if (! $country_code) {	// No found country with current IP address
            return false;
        }
        ...
        if (! $country_ID) {	// No found country in the b2evo DB
            return false;
        }
    ...
    public function GetUserFromCountrySuffix(&$params)
    {
        ...
        $reload_icon = ' ' . action_icon(TB_('Ask GeoIP'), 'reload', '', TB_('Ask GeoIP'), 3, 4, [
            'id' => 'geoip_load_country',
            'class' => 'roundbutton roundbutton_text middle',
        ])
...
<?php
    ...
    {
        return ['load_country', 'download_geoip_data'];
    }
     ...
     *   - 'user_ID': User ID
     */
    public function htsrv_load_country($params)
    {
        ...
        if (empty($user_ID)) {	// Bad request
            return;
        }

        $UserCache = &get_UserCache();
        if (! ($User = &$UserCache->get_by_ID($user_ID))) {	// No user exists
            return;
        }
        ...
        if (empty($Country)) {	// No found country
            echo sprintf(TB_('No country found for IP address %s'), int2ip($UserSettings->get('created_fromIPv4', $User->ID)));
        } else {	// Display country name with flag and Update user's field 'From Country'
     ...
     */
    public function htsrv_download_geoip_data()
    {
    ...
    public function DisplayRegisterFormBefore(&$params)
    {
        ...
        if (isset($params['Widget'])) {	// Get a setting for quick registration widget:
            $registration_require_country = ($params['Widget']->disp_params['ask_country'] != 'no');
        } else {	// Get a setting for normal registration form:
            $registration_require_country = in_array('country', get_registration_template_required_fields());
        }
        if (! $registration_require_country) {	// Country is not required on registration form. Exit here.
            return;
        }

        $detect_registration = $this->Settings->get('detect_registration');
        if ($detect_registration == 'no') {	// No detect country on registration
            return;
        }

        $Country = $this->get_country_by_IP(get_ip_list(true));

        if (! $Country) {	// No found country by IP address
            return;
        }
        ...
        switch ($detect_registration) {
            case 'auto':
                ...
                if (empty($country)) {	// Set country ID if user didn't select country yet
            ...
            case 'hide':
                ...
                if (! isset($params['Form'])) {	// there's no Form where we add to, but we create our own form:
                    $Form = new Form(regenerate_url());
                } else {
                    $Form = &$params['Form'];
                }

                if (isset($params['Widget'])) {	// Hide country selector on widget:
    ...
    public function GetAdditionalColumnsTable(&$params)
    {
        ...
        if (in_array($params['table'], ['sessions', 'activity', 'ipranges', 'top_ips'])) {	// Display column only for required tables by GeoIP plugin
            $column = [
                'th' => TB_('Country'),
     ...
     */
    public function display_tool_form($params = [])
    {
        if ($this->status != 'enabled') {	// Don't allow use this tool when GeoIP plugin is not enabled
            echo '<p class="error">' . TB_('You must enable the GeoIP plugin before to use this tool.') . '</p>';
            return;
        }

        if (! check_user_perm('options', 'edit')) {	// Current User must has a permission to run tools:
            return;
        }
        ...
        if ($params['display_info']) {	// Display info about this form:
        ...
        if ($params['display_button_find']) {	// Display a button to find country for users:
        ...
        if ($params['display_button_fix']) {	// Display a button to fix country for users:
        ...
        if ($params['display_version']) {	// Display plugin version and links to update:
            global $admin_url;
            if (file_exists($this->geoip_file_path)) {
                $datfile_info = sprintf(TB_('Last updated on %s'), date(locale_datetimefmt(), filemtime($this->geoip_file_path)));
            } else {
    ...
    public function BeforeBlockableAction()
    {
        ...
        foreach ($request_ip_list as $IP_address) {
            ...
            if (antispam_block_by_country($Country->ID, false)) {	// Block the action if the country is blocked
     ...
     */
    public function print_tool_log($message, $type = 'info')
    {
        switch ($type) {
            case 'success':
     ...
     */
    public function download_geoip_data()
    {
        // Display a process of downloading of GeoLite2-Country.mmdb
        global $admin_url;

        if ($this->Settings->get('download_url') == '') {	// If "DB Download URL" is not defined:
            $this->print_tool_log(sprintf(TB_('Enter URL of %s for download'), '<code>GeoLite2-Country....tar.gz</code>'), 'error');
            return;
        }

        $this->print_tool_log(sprintf(
            TB_('Downloading %s file from the url: %s ...'),
        ...
        if ($gzip_contents === false || $info['status'] != 200) {	// Download failed
            if (empty($info['error'])) {	// Some unknown error
            ...
            } else {	// Display an error of request
        ...
        } elseif (! is_writable($plugin_dir)) {	// Check the write rights
        ...
        if (! preg_match('#\.tar\.gz$#', $gzip_tar_file_name)) {	// Set proper file name because file cannot be created from long url with specific chars like ?, & and etc.:
            $gzip_tar_file_name = 'GeoIP-' . date('Y-m-d-His') . '.tar.gz';
        }
        $gzip_tar_file_path = rtrim(sys_get_temp_dir(), '/') . '/' . $gzip_tar_file_name;

        if (! save_to_file($gzip_contents, $gzip_tar_file_path, 'w')) {	// Impossible to save file...
            $this->print_tool_log(sprintf(TB_('Unable to create file: %s'), '<code>' . $gzip_tar_file_path . '</code>'), 'error');

            if (file_exists($gzip_tar_file_path)) {	// Remove file from disk
                if (! @unlink($gzip_tar_file_path)) {	// File exists without the write rights
        ...
        if (! defined('Phar::TAR')) {	// No extension
        ...
        if (! @unlink($tar_file_path)) {	// Failed to remove tar file:
        ...
        } else {	// Failed to remove gzip file:
        ...
        if (file_exists($old_geoip_file_path)) {	// Try to remove old data file if it exists on the disk:
        ...
        if ($enable_return === true) {	// Successfully enabled the plugin:
            if ($this->status != 'enabled') {	// Enable this plugin automatically:
                $Plugins->set_Plugin_status($this, 'enabled');
                $this->print_tool_log(TB_('The plugin has been enabled.'), 'success');
            }
        } else {	// Some restriction for enabling:
            $this->print_tool_log(TB_('The plugin could not be automatically enabled.'), 'warning');

            if ($this->status != 'needs_config') {	// Make this plugin incomplete because it cannot be enabled:
     ...
     */
    public function geoip_find_country()
    {
        global $DB, $Messages;
        ...
        foreach ($users as $user_ID => $created_fromIPv4) {
            ...
            if (empty($created_fromIPv4)) {	// No defined IP, Skip this user
        ...
        if ($count_nofound_country > 0) {	// If some users have IP address with unknown country
     ...
     */
    public function geoip_fix_country()
    {
        global $DB;
        ...
        foreach ($users as $user) {
            ...
            if (empty($user->session_IP)) {	// No defined session IP:
                $users_report .= '<b class="orange">' . TB_('Session IP address is not set!') . '</b>';
            } else {	// Get Country by session IP address:
            ...
            if (! $Country) {
                $users_report .= ' - ';
                if (empty($user->reg_ctry_ID)) {	// No defined registration country:
                    $users_report .= '<b class="orange">' . TB_('Registration country is not set!') . '</b>';
                } else {	// Get Country by registration IP address:
                    $CountryCache = &get_CountryCache();
            ...
            if (! $Country) {
                $users_report .= ' - ';
                if (empty($user->reg_IP)) {	// No defined registration IP:
            ...
            if (! $Country) {	// No found country in 3 steps above:
                $count_nofound_country++;
                $users_report .= ' - <b class="text-danger">' . TB_('Country is not detected!') . '</b><br />';
                continue;
            }
...
function geoip_get_country_by_IP($IP)
{
    ...
    if ($Plugins && $geoip_Plugin = &$Plugins->get_by_code('evo_GeoIP')) {
        if (strlen(intval($IP)) == strlen($IP)) {	// IP is in integer format, We should convert it to normal IP
            $IP = int2ip($IP);
        }

        if ($Country = $geoip_Plugin->get_country_by_IP($IP)) {	// Get country flag + name
            load_funcs('regional/model/_regional.funcs.php');
