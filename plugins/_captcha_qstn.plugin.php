...
class captcha_qstn_plugin extends Plugin
{
    ...
    public function PluginSettingsUpdateAction()
    {
        ...
        if (empty($questions)) { // No questions
        ...
        foreach ($questions as $question) {
            if (preg_match('/(.+\?)(.+)/i', $question, $match)) { // Check a question for correct format
                $question = trim($match[1]);
                $answers = trim($match[2]);
                if (! empty($question) && ! empty($answers)) { // Save this question in DB
                    $SQL = new SQL('Get captcha question');
                    $SQL->SELECT('cptq_ID');
                    $SQL->FROM($this->get_sql_table('questions'));
                    $SQL->WHERE('cptq_question = ' . $DB->quote($question));

                    if ($question_ID = $DB->get_var($SQL)) { // This question already exists, we should only update the answers
            ...
            } elseif (! empty($question)) { // Incorrect format of question
                $Messages->add(sprintf(T_('Invalid question format in the line %s.'), '"<b>' . $question . '</b>"'), 'error');
            }
        }

        if (param_errors_detected()) { // Don't save the changes on errors
            return false;
        } else { // Save the changes on success
            if (count($insert_data)) { // Insert new questions in DB:
            ...
            if (count($update_data)) { // Update the questions in DB:
    ...
    {
        if (! empty($params['is_preview']) || ! isset($params['form_type']) || ! $this->does_apply($params['form_type'])) {	// We should not apply captcha to the requested form:
            return true;
        }
        ...
        foreach ($answers as $answer) {
            if ($posted_answer == $answer) {	// Correct answer is found in DB
    ...
    public function CaptchaQuestion()
    {
        ...
        if (empty($this->question_ID)) {	// Get question from DB by current IP address
        ...
        if (empty($this->question_ID)) {	// Assign new random question for current IP address
        ...
        if (empty($question) && ! empty($this->question_ID)) {	// Get question data
            ...
            if (empty($question)) {	// Assign random question if previous question doesn't exist in DB
     ...
     * @return object Question row from DB table "questions"
     */
    public function CaptchaQuestionNew()
    {
        global $DB, $Session;
     ...
     */
    public function CaptchaQuestionCleanup()
    {
        global $DB, $Session;
    ...
    {
        if (! isset($params['form_type']) || ! $this->does_apply($params['form_type'])) {	// Exit here if the form type is not defined or this captcha plugin should not be applied for the requested form:
            return false;
        }

        switch ($params['form_type']) {	// Set a position where we should display the captcha depending on form type:
            case 'register':
            case 'item':
            case 'comment':
            case 'message':
        ...
        if (empty($question)) {	// No the defined questions
            return false;
        }
     ...
     * @return boolean
     */
    public function does_apply($form_type)
    {
        switch ($form_type) {
            case 'item':
                if (! is_logged_in()) {
     ...
     * @access private
     */
    public function validate_questions()
    {
        global $DB;
