...
{
    public $code = 'evo_datetag';

    public $name = 'Date tag';
    ...
    public $help_topic = 'date-tag-plugin';
    ...
    {
        if (isset($params['Comment'])) {	// Render for Comment:
            $this->rendering_Object = $params['Comment'];
        } elseif (isset($params['Item'])) {	// Render for Item:
            $this->rendering_Object = $params['Item'];
        } elseif (isset($params['Message'])) {	// Render for Message:
            $this->rendering_Object = $params['Message'];
        } elseif (isset($params['EmailCampaign'])) {	// Render for EmailCampaign:
            $this->rendering_Object = $params['EmailCampaign'];
        } elseif (isset($params['Widget'])) {	// Render for Widget:
            $this->rendering_Object = $params['Widget'];
        } else {	// Don't render if no proper object is provided:
            return false;
        }
     ...
     */
    public function render_dates($content)
    {
        return replace_outside_code_tags('#\[date(:([^\]]+))?\]#i', [$this, 'get_date_text'], $content, 'replace_content_callback');
    }
     ...
     */
    public function get_date_text($matches)
    {
        if (empty($matches)) {	// No dates found
            return;
        }
        ...
        // Date format:
        if (isset($options[1])) {	// Use custom date format:
            $date_format = $options[1];
        } else {	// Use a date format from the rendering object:
            switch ($object_type) {
                case 'Item':
        ...
        // Date class:
        if (isset($options[3])) {	// Check class:
        ...
        if ($date_source === false) {	// Display error if source cannot be used for the rendering object:
            return get_rendering_error(sprintf(T_('%s is not valid for %s'), $matches[0], $object_type), 'span');
        }

        if (preg_match('/-?\d{1,2}([\.,:]\d{1,2})?/', $date_offset) && ! empty($date_offset)) {	// Shift date:
            $date_offset = preg_split('/[\.,:]/', $date_offset);
            $o_date_source = $date_source;
            // Shift with hours:
            $date_source += rtrim($date_offset[0], '\\') * 3600;
            if (isset($date_offset[1])) {	// Shift with minutes:
                if ($date_offset[0] < 0) {	// Use correct sign for minutes as hours have:
        ...
        if (! empty($date_class)) {	// Apply class to date text if it is provided:
