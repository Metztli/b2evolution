...
class polls_plugin extends Plugin
{
     ...
     */
    public function render_polls_data($content, $params = [], $format = 'html')
    {
        if (isset($params['check_code_block']) && $params['check_code_block'] && ((stristr($content, '<code') !== false) || (stristr($content, '<pre') !== false))) {	// Call $this->render_polls_data() on everything outside code/pre:
            $params['check_code_block'] = false;
            $content = callback_on_non_matching_blocks(
                $content,
                '~<(code|pre)[^>]*>.*?</\1>~is',
                [$this, 'render_polls_data'],
                [$params]
            );
            return $content;
        }
        ...
        if (count($tags[0]) > 0) {	// If at least one poll inline tag is found in content:
            ...
            foreach ($tags[0] as $t => $source_tag) {	// Render poll inline tag as html with widget "Poll":
                $poll_ID = $tags[1][$t];
    ...
    public function DisplayCodeToolbar($params = [])
    {
        ...
        if ($Hit->is_lynx()) { // let's deactive toolbar on Lynx, because they don't work there
            return false;
        }
     ...
     * @param array Associative array of parameters
     */
    public function render_email($params = [])
    {
